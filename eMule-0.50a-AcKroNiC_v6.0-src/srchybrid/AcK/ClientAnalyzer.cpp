...
void	CAntiLeechData::DecFastXSCounter()
{
	...
	if(m_pData->iFastXSAsks > 0)
	{
		--m_pData->iFastXSAsks;
		//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
		//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: FastXS for (%s) decreased: %i", m_pParent->DbgGetClientInfo(), m_pData->iFastXSAsks);
		ReCheckScore();
	}
	EvalXS();
}
...
void CAntiLeechData::EvalXS()
{
	...
	if(m_uiLastXS)
		theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: Received XS request after %s from %s - fastXS count: %u", CastSecondsToHM((dwNow - m_uiLastXS)/1000), m_pParent->DbgGetClientInfo(), m_pData->iFastXSAsks);
	else
		theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: Received first XS in this session from %s", m_pParent->DbgGetClientInfo());
	m_uiLastXS = dwNow;
}
...
void	CAntiLeechData::IncXSAnsw()	
{
	...
	++m_pData->uXSAnsw;
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
	//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: XS Answs for (%s) increased: %i", m_pParent->DbgGetClientInfo(), m_pData->uXSAnsw);
	if(b != IsXSExploiter())
	{
		if(m_pParent && thePrefs.GetLogAnalyzerEvents())
...
void CAntiLeechData::Check4NickThief()
{
	...
	CString strNick = m_pParent->/*GetUserName()*/GetUserName(false); //>>> taz::FunnyNick
	if(strNick.IsEmpty())
	{
...
{	
	...
	const CString OurMod = /*MOD_VERSION*/theApp.m_strModLongVersion; //cache it //>>> taz::-modname-
