...
void CUpDownClient::Init()
{
	...
	m_uiRealVersion = 0;
	pAntiLeechData = NULL; //>>> WiZaRd::ClientAnalyzer
#else
...
//<<< WiZaRd::More GPLEvilDoers
	...
	m_bSendIP = false; //>>> taz::Inform Clients after IP Change [Stulle]
...
//>>> taz::more mods icons
					else if(StrStrI(m_strModVersion,_T("AcKroNiC"))!=0)
						m_uModClient = MOD_AcK;
					else if(StrStrI(m_strModVersion,_T("Beba"))!=0)
						m_uModClient = MOD_Beba;
					else if(StrStrI(m_strModVersion,_T("DreaMule"))!=0)
						m_uModClient = MOD_DreaMule;
					else if(StrStrI(m_strModVersion,_T("Katana"))!=0)
						m_uModClient = MOD_Katana;
					else if(StrStrI(m_strModVersion,_T("SharkX"))!=0)
						m_uModClient = MOD_SharkX;
					else if(StrStrI(m_strModVersion,_T("Spike2"))!=0)
						m_uModClient = MOD_Spike2;
					else if(StrStrI(m_strModVersion,_T("TK4"))!=0)
						m_uModClient = MOD_TK4;
					else if(StrStrI(m_strModVersion,_T("Tombstone"))!=0)
						m_uModClient = MOD_Tombstone;
					else if(StrStrI(m_strModVersion,_T("VeryCD"))!=0)
						m_uModClient = MOD_VeryCD;
					else if(StrStrI(m_strModVersion,_T("ZZUL"))!=0)
						m_uModClient = MOD_ZZUL;
//<<< taz::more mods icons
					else
...
#ifdef CLIENTANALYZER
		CTag tag8(ET_MOD_VERSION, theApp.m_strModLongVersion);
#else
...
{
//>>> Spike2::Emulate other [???/WiZaRd/Torni/Spike2]
	//data->WriteHash16(thePrefs.GetUserHash());//removed see "emulate other" [Spike2]
	uchar hash[16]; 
...
#ifdef CLIENTANALYZER
		CTag tagMODVersion(ET_MOD_VERSION, theApp.m_strModLongVersion);
#else
...
//			case 40:			// removed "case 40"... this is now integrated here
			case SO_SHAREAZA2:
			case SO_SHAREAZA3:
			case SO_SHAREAZA4:
//<<< Spike2::Enhanced Client Recognition
...
//<<< Spike2::Enhanced Client Recognition
			default:
				if (m_bIsML || m_byCompatibleClient == SO_MLDONKEY || m_byCompatibleClient == SO_MLDONKEY2 || m_byCompatibleClient == SO_MLDONKEY3){ //>>> Spike2::Enhanced Client Recognition
					m_clientSoft = SO_MLDONKEY;
					pszSoftware = _T("MLdonkey");
				}
				else if (m_bIsHybrid || m_byCompatibleClient == SO_EDONKEYHYBRID){ //>>> Spike2::Enhanced Client Recognition
...
//					else
					if (StrStr(m_strModVersion, L"Plus 1"))
					{
						m_clientSoft = SO_EMULEPLUS;
						pszSoftware = L"eMule Plus";
					}
			...
			else if (m_clientSoft == SO_EMULEPLUS) 
			{
				if(nClientMinVersion == 0)
				{
					if(nClientUpVersion == 0)
						iLen = _sntprintf(szSoftware, ARRSIZE(szSoftware), L"%s v%u", pszSoftware, nClientMajVersion);
					else
						iLen = _sntprintf(szSoftware, ARRSIZE(szSoftware), L"%s v%u%c", pszSoftware, nClientMajVersion, L'a' + nClientUpVersion - 1);
				}
				else
				{
					if(nClientUpVersion == 0)
...
//<<< Spike2::Enhanced Client Recognition
	...
	if (m_bIsHybrid || m_byCompatibleClient == SO_EDONKEYHYBRID){ //>>> Spike2::Enhanced Client Recognition
		m_clientSoft = SO_EDONKEYHYBRID;
		// seen:
		// 105010	0.50.10
		// 10501	0.50.1
...
void CUpDownClient::ProcessPublicIPAnswer(const BYTE* pbyData, UINT uSize){
	...
	if (m_fNeedOurPublicIP == 1){ // did we?
		...
		theApp.CheckIdChange(); //>>> taz::Inform Clients after IP Change [Stulle]
	}	
}
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32 nLength)
{
	...
	*/
	...
	while (!resToken.IsEmpty())
	{
		resToken.Trim();
#ifdef CLIENTANALYZER
		if (resToken.GetLength() > 3 //>>> WiZaRd::Don't accept too short filters - unfair!
...
Packet* GetEmptyXSPacket(const CUpDownClient* forClient, CKnownFile* kreqfile, uint8 byRequestedVersion, uint16 nRequestedOptions)
{
	...
	if (thePrefs.GetDebugSourceExchange())
		AddDebugLogLine(false, _T("SXSend: Client source response SX2=%s, Version=%u; Count=%u, %s, File=\"%s\""), bIsSX2Packet ? _T("Yes") : _T("No"), byUsedVersion, 0, forClient->DbgGetClientInfo(), kreqfile->GetFileName());
	return result;
}
...
void	CUpDownClient::ProcessSourceRequest(const BYTE* packet, const UINT size, const UINT opcode, const UINT uRawSize, CSafeMemFile* data, CKnownFile* kreqfile)
{
	...
	if(!bMultipacket)
	{
		...
		CheckHandshakeFinished(OP_EMULEPROT, opcode); //>>> taz::morph
	}
...
bool CUpDownClient::IsModFaker()
{
	...
	if(fModVersion >= /*10.0*/6.0f && CString(m_pszUsername).Right(m_strModVersion.GetLength()+1)!=m_strModVersion + _T("\xBB"))  //>>> taz::-modname-
		return true;
	return false;
}
