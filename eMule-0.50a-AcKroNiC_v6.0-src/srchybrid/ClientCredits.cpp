...
float CClientCredits::GetScoreRatio(uint32 dwForIP) /*const*/
{
	...
	*/
	...
	double result = min(!GetUploadedTotal() ? 10.0F : (float)(((double)GetDownloadedTotal()*2.0)/(double)GetUploadedTotal()), (float)sqrt((float)(GetDownloadedTotal()/1048576.0) + 2.0F));	
	if (result < 1.0F)
		result = 1.0F;
	else if (result > 10.0F)
		result = 10.0F;
	m_fLastScoreRatio = result; //>>> WiZaRd::CPU calm down
	return result;
#else
