...
#include "UploadQueue.h" //>>> WiZaRd::ClientAnalyzer
#endif
...
//Reported by Seppl: rare upload isn't correct if we only look at our current sources!
					...
					pAntiLeechData->AddDownloaded(uTransferredFileDataSize, GetAvailablePartCount() != reqfile->GetPartCount(), /*reqfile->GetSourceCount()*/srccount);
				}
#endif
			}
			//MORPH - Avoid Credits Accumulate faker
