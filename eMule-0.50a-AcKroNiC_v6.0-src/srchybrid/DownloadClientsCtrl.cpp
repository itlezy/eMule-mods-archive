		...
		case 8:
			if(client->Credits())
				...
				if (client->GetAntiLeechData())
					_sntprintf(pszText, cchTextMax, _T("%.1f/%.1f"),client->credits->GetScoreRatio(client->GetIP()), client->GetAntiLeechData()->GetScore());
				else
					_sntprintf(pszText, cchTextMax, _T("%.1f/?"),client->credits->GetScoreRatio(client->GetIP()));
#else
