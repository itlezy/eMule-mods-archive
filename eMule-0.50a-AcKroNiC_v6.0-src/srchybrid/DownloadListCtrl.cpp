...
#include "Ntservice.h" 
//>>> taz::drop sources
#include "AcK/SivkaFileSettings.h" // drop sources - Stulle
...
//	InsertColumn(4, GetResString(IDS_DL_SPEED),			LVCFMT_RIGHT, DFLT_DATARATE_COL_WIDTH);
	CString buffer = GetResString(IDS_DL_SPEED);
	if(thePrefs.ShowMinRQR())
		buffer.AppendFormat(L"/%s", GetResString(IDS_MINRQR_PREFIX));
	InsertColumn(4, buffer, LVCFMT_RIGHT, DFLT_DATARATE_COL_WIDTH);
//<<< WiZaRd::minRQR [WiZaRd]	
...
//>>> taz::more mods icons
	m_ImageList.Add(CTempIconLoader(L"AcK"));
	m_ImageList.Add(CTempIconLoader(L"Beba"));
	m_ImageList.Add(CTempIconLoader(L"DreaMule"));
	m_ImageList.Add(CTempIconLoader(L"Katana"));
	m_ImageList.Add(CTempIconLoader(L"SharkX"));
	m_ImageList.Add(CTempIconLoader(L"Spike2"));
	m_ImageList.Add(CTempIconLoader(L"TK4"));
	m_ImageList.Add(CTempIconLoader(L"Tombstone"));
	m_ImageList.Add(CTempIconLoader(L"VeryCD"));
	m_ImageList.Add(CTempIconLoader(L"ZZUL"));
//<<< taz::more mods icons
...
//>>> taz::minRQR
	...
	if(thePrefs.ShowMinRQR())
		strRes.AppendFormat(L"/%s", GetResString(IDS_MINRQR_PREFIX));
	hdi.pszText = const_cast<LPTSTR>((LPCTSTR)strRes);
			...
			else if (thePrefs.ShowMinRQR())
			{
				...
				if (uiMinRQR != _UI32_MAX)
				{
					...
					else
					{
						...
						strBuffer.Format(L"%s: %u", GetResString(IDS_MINRQR_PREFIX), uiMinRQR);
...
void CDownloadListCtrl::DrawSourceItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		case 0: {	// icon, name, status
			...
			if (pClient->Credits() && pClient->Credits()->GetScoreRatio(pClient->GetIP()) > 1) {
#else
...
*/
		m_DropMenu.AddMenuTitle(GetResString(IDS_DROP_MENU), true);
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPLOWTOLOWIPSRCS,GetResString(IDS_DROP_LOW2LOW));
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPUNKNOWNERRORBANNEDSRCS,GetResString(IDS_DROP_MISC));
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPNONEEDEDSRCS,GetResString(IDS_DROP_NNS));
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPFULLQSRCS,GetResString(IDS_DROP_FULL));
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPHIGHQRSRCS,GetResString(IDS_DROP_HQ));
		m_DropMenu.AppendMenu(MF_STRING, MP_CLEANUP_NNS_FQS_HQRS_NONE_ERROR_BANNED_LOWTOLOWIP,GetResString(IDS_DROP_ALL));
		m_DropMenu.AppendMenu(MF_STRING, MP_DROPLEECHER,GetResString(IDS_DROP_LEECHER));
//<<< taz::menu titles
		m_FileMenu.AppendMenu(MF_SEPARATOR);
...
*/
		m_FileMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_DropMenu.m_hMenu, GetResString(IDS_DROP_MENU),L"FUNNYNICK");
//<<< taz::menu titles
		// <== drop sources - Stulle
//<<< taz::drop sources
	}
