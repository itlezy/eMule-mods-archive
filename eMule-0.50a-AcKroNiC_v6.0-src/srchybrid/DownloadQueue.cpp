...
DWORD quicktime; // Quick start [TPT] - Stulle //>>> taz::Quick start [TPT]
...
void CDownloadQueue::Init(){
	...
	m_SettingsSaver.LoadSettings(); //>>> taz::drop sources // drop sources - Stulle
	...
	if (!cur_udpserver)
	{
		while ((cur_udpserver = theApp.serverlist->GetSuccServer(cur_udpserver)) != NULL) {
			if (cur_udpserver == pConnectedServer)
				continue;
			if (thePrefs.GetDeadServerRetries() > 0) //>>> taz::Don't remove dead servers on 0 retries
...
\********************/
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		if (cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY)
		{
			if (thePrefs.GetGlobalHlAll() || cur_file->GetGlobalHL()) //>>> taz::drop sources // Global Source Limit (customize for files) - Stulle
			aCount++;
...
void CDownloadQueue::RemoveSourceAndDontAsk(CUpDownClient* toremove, CPartFile *file, bool bDoStatsUpdate/*, POSITION pos*/) //>>> taz::don't drop complete sources
{
//>>> taz::don't drop complete sources
	if (thePrefs.DontDropCompleteSources() && toremove->IsCompleteSource()
#ifdef CLIENTANALYZER
	&& !(toremove->GetAntiLeechData() && toremove->GetAntiLeechData()->GetBadForThisSession(AT_UDPFNFFAKER | AT_FILEFAKER))
#endif
	)
		return;
//>>> taz::don't drop complete sources
	if( RemoveSource(toremove, bDoStatsUpdate/*, pos*/) ){
		...
		file->m_ShowDroppedSrc++; //>>> taz::show # of dropped sources //>>> taz::fix count only dropped
	}
//	file->m_ShowDroppedSrc++; //>>> taz::show # of dropped sources //>>> taz::fix count only dropped
}
