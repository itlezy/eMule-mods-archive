...
bool CFileIdentifier::CalculateMD4HashByHashSet(bool bVerifyOnly, bool bDeleteOnVerifyFail, bool slowdown) //>>> taz::Nice Hash [Xman]
{
	...
	uchar aucResult[16];
	CKnownFile::CreateHash(buffer, m_aMD4HashSet.GetCount()*16, aucResult, NULL, slowdown); //>>> taz::Nice Hash [Xman]
	delete[] buffer;
	if (bVerifyOnly)
	{
