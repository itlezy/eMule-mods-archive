...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	for (hashcount = 0; togo >= PARTSIZE; )
	{
		...
		if (!CreateHash(file, PARTSIZE, newhash, pBlockAICHHashTree, true)) { //>>> taz::Nice Hash [Xman]
			LogError(_T("Failed to hash file \"%s\" - %s"), strFilePath, _tcserror(errno));
			fclose(file);
			delete[] newhash;
			return false;
	...
	md4clr(lasthash);
	if (!CreateHash(file, togo, lasthash, pBlockAICHHashTree, true)) { //>>> taz::Nice Hash [Xman]
		LogError(_T("Failed to hash file \"%s\" - %s"), strFilePath, _tcserror(errno));
		fclose(file);
		delete[] lasthash;
		return false;
...
// WebCache ////////////////////////////////////////////////////////////////////////////////////
	...
	if (!hashcount){
		...
		m_FileIdentifier.CalculateMD4HashByHashSet(false, true, true); //>>> taz::Nice Hash [Xman]
	}
...
void CKnownFile::CreateHash(CFile* pFile, uint64 Length, uchar* pMd4HashOut, CAICHHashTree* pShaHashOut, bool slowdown) //>>> taz::Nice Hash [Xman]
{
	ASSERT( pFile != NULL );
	...
	uint32 timeStart = ::GetTickCount(); //>>> taz::Nice Hash [Xman]
...
bool CKnownFile::CreateHash(FILE* fp, uint64 uSize, uchar* pucHash, CAICHHashTree* pShaHashOut, bool slowdown) //>>> taz::Nice Hash [Xman]
{
	bool bResult = false;
	CStdioFile file(fp);
	try
	{
		CreateHash(&file, uSize, pucHash, pShaHashOut, slowdown); //>>> taz::Nice Hash [Xman]
		bResult = true;
	}
...
bool CKnownFile::CreateHash(const uchar* pucData, uint32 uSize, uchar* pucHash, CAICHHashTree* pShaHashOut, bool slowdown) //>>> taz::Nice Hash [Xman]
{
	bool bResult = false;
