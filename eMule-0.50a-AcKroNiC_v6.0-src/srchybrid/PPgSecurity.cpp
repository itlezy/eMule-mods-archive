...
//	ON_BN_CLICKED(IDC_USESECIDENT, OnSettingsChange) //>>> taz::mandatory
...
void CPPgSecurity::LoadSettings(void)
{
	...
	GetDlgItem(IDC_USESECIDENT)->EnableWindow(FALSE); //>>> taz::mandatory
...
void CPPgSecurity::Localize(void)
{
	if(m_hWnd)
	{
		...
		SetTool(IDC_SEARCHSPAMFILTER,IDS_SEARCHSPAMFILTER_TIP); 
