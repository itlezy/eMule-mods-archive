...
BOOL CPPgServer::OnApply()
{
	...
	if (thePrefs.m_uDeadServerRetries < /*1*/0) //>>> taz::Don't remove dead servers on 0 retries
		thePrefs.m_uDeadServerRetries = /*1*/0; //>>> taz::Don't remove dead servers on 0 retries
