...
#include "UploadQueue.h" //>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]
...
BEGIN_MESSAGE_MAP(CPPgTweaks, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_INFQ_ENABLE, OnInfQClick) //>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]
END_MESSAGE_MAP()
...
//>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]
	CheckDlgButton(IDC_INFQ_ENABLE, thePrefs.infiniteQueue ? BST_CHECKED : BST_UNCHECKED);
	//using "OnInfQClick" here would (wrongly) enable the "apply" button
...
BOOL CPPgTweaks::OnApply()
{
	...
	*/
	...
	thePrefs.infiniteQueue = (IsDlgButtonChecked(IDC_INFQ_ENABLE) == BST_CHECKED); //>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]

	//WiZaRd: This is to update the queue size... 
	//if we do not call this then the old size will be kept until an update of the queue occurs
...
//<<< WiZaRd::ClientAnalyzer
		...
		*/
		...
		GetDlgItem(IDC_FILEBUFFERSIZE_STATIC)->SetWindowText(temp);
//>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]
		//Note: Do *not* rely on the pref setting here!
		if(IsDlgButtonChecked(IDC_INFQ_ENABLE) == BST_CHECKED)
			temp.Format(L"%s: \x221E", GetResString(IDS_QUEUESIZE));
		else
//<<< taz::InfiniteQueue [WiZaRd/SLUGFILLER]
		temp.Format(_T("%s: %s"), GetResString(IDS_QUEUESIZE), GetFormatedUInt(m_iQueueSize));
		GetDlgItem(IDC_QUEUESIZE_STATIC)->SetWindowText(temp);

		GetDlgItem(IDC_INFQ_ENABLE)->SetWindowText(GetResString(IDS_INFQ_ENABLE)); //>>> taz::InfiniteQueue [WiZaRd/SLUGFILLER]
        //MORPH START leuk_he tooltipped
...
#ifdef CLIENTANALYZER
				if (m_htiLogAnalyzerEvents)     m_ctrlTreeOptions.SetCheckBoxEnable(m_htiLogAnalyzerEvents, bCheck);
#endif
...
{	
	...
	{
		GetDlgItem(IDC_QUEUESIZE)->EnableWindow(TRUE);		
		temp.Format(L"%s: %s", GetResString(IDS_QUEUESIZE), GetFormatedUInt(m_iQueueSize));
	}
	GetDlgItem(IDC_QUEUESIZE_STATIC)->SetWindowText(temp);
	SetModified(TRUE);
}
