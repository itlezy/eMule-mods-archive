...
public:
		...
		: CDlgPageWizard(nIDTemplate, pszCaption, pszHeaderTitle, pszHeaderSubTitle)
	{
		...
		m_iKademlia = 1;
		m_iED2K = 1;
		m_iReqObfus = 0; // MORPH lh require obfuscated server connection, default 
	}
	...
	: CDlgPageWizard(CPPgWiz1Server::IDD)
{
	...
	m_iED2K = 1;
	m_pbUDPDisabled = NULL;
	m_iReqObfus = 0; // MORPH lh require obfuscated server connection
}
...
BOOL CPPgWiz1Server::OnSetActive(){
	if (m_pbUDPDisabled != NULL){
		...
		if (*m_pbUDPDisabled){
			...
			// MORPH START
			m_iReqObfus = 0;
			CheckDlgButton(IDC_WIZARDREQUIREOBFUSCATED,BST_UNCHECKED); // if udp is disabled obfuscated server is not a good default because crypting will fail.
			// MORPH END
...
//	page6b.m_psh.hbmHeader = bmHeader_mor;
	...
	page2.m_iAutoConnectAtStart = (thePrefs.DoAutoConnect() ? 1 : 0); //>>> taz::preferences
	page2.m_iAutoStart = (thePrefs.GetAutoStart() ? 1 : 0); //>>> taz::preferences
...
// MORPH less is more
	...
	page6.m_iSafeServerConnect = (thePrefs.IsSafeServerConnectEnabled() ? 1 : 0); //>>> taz::preferences
	page6.m_iKademlia = (thePrefs.networkkademlia ? 1 : 0); //>>> taz::preferences
	page6.m_iED2K = (thePrefs.GetNetworkED2K() ? 1 : 0); //>>> taz::preferences
	page6.m_iReqObfus = (thePrefs.m_bCryptLayerRequiredStrictServer ? 1 : 0); //>>> taz::preferences
  ...
  page6b.m_iShowMoreControls = thePrefs.IsExtControlsEnabled(); // MORPH startup wizard
	...
	if (iResult == IDCANCEL) {
		...
		/* MORPH only when changed...(required icw with random ports)  */
...
//MORPH END   - Removed, Less is more 
	...
	thePrefs.m_bCryptLayerRequiredStrictServer = (page5.m_iObfuscation != 0 && page6.m_iReqObfus != 0); //>>> taz::store obfuscated server connection
	thePrefs.m_bCryptLayerRequested = page5.m_iObfuscation != 0;
	if (page5.m_iObfuscation != 0)
		thePrefs.m_bCryptLayerSupported = true;
