...
void CPartFile::Init(){
	m_uiMinRQR = _UI32_MAX; //>>> WiZaRd::minRQR [WiZaRd]
...
//>>> taz::More info about corrupted .met/.part file [Aenarion]
			if (thePrefs.m_bShowCorrupted)
				AfxMessageBox(GetResString(IDS_ACK_WARNING_MET), MB_ICONERROR);
//<<< taz::More info about corrupted .met/.part file [Aenarion]
		}else{
			TCHAR buffer[MAX_CFEXP_ERRORMSG];
			error->GetErrorMessage(buffer,ARRSIZE(buffer));
...
//MORPH START - Changed by Stulle, No zz ratio for http traffic
{
	...
	else
	{
		...
		*/
		...
		UINT uiMinRQR = _UI32_MAX; //>>> WiZaRd::minRQR [WiZaRd]
//>>> taz::drop sources
		m_RemoteQueueRank_New = m_MaxRemoveQRS; // drop sources - Stulle
//		uint16 m_ShowDroppedSrc_Temp = 0; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
//<<< taz::drop sources
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;)
		{
...
//>>> taz::drop sources
							...
							*/
							if( ((dwCurTick - lastpurgetime) > SEC2MS(30)) && theApp.GetReAskTick() < dwCurTick && (this->GetSourceCount() >= (GetMaxSources()*.8 )) )
							// <== auto drop immunity - Stulle
//<<< taz::drop sources
							{
//>>> taz::drop sources
						...
						theApp.downloadqueue->RemoveSource( cur_src );
								*/
								theApp.downloadqueue->RemoveSourceAndDontAsk(cur_src, this, true); //>>> taz::don't drop complete sources
								// <== drop sources - Stulle
//								m_ShowDroppedSrc_Temp++; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
//<<< taz::drop sources
...
//>>> taz::drop sources
					...
					*/
					if(m_EnableAutoDropNNS && theApp.GetReAskTick() < dwCurTick) // auto drop immunity - Stulle
					{
							if(dwCurTick > (m_TimerForAutoNNS + m_AutoNNS_Timer)
							&& GetSourceCount() > ((GetMaxSources() * m_MaxRemoveNNSLimit)/100))
							{
								if(!cur_src->SwapToAnotherFile( _T("Removed: NoNeededSources"), true, true, true , NULL )) // fix for sivka's version by Cyrex2001
								theApp.downloadqueue->RemoveSourceAndDontAsk(cur_src, this, true/*, pos2*/); //>>> taz::don't drop complete sources
...
//								m_ShowDroppedSrc_Temp++; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
					...
					// <== drop sources - Stulle
//<<< taz::drop sources
					uiMinRQR = min(uiMinRQR, _UI32_MAX-2); //>>> WiZaRd::minRQR [WiZaRd]
...
//>>> taz::drop sources
					// ==> drop sources - Stulle
					if(cur_src->GetRemoteQueueRank() > m_RemoteQueueRank_New)
...
//>>> taz::drop sources
						...
						*/
						if(m_EnableAutoDropFQS && theApp.GetReAskTick() < dwCurTick) // auto drop immunity - Stulle
						{
							if(dwCurTick > (m_TimerForAutoFQS + m_AutoFQS_Timer)
							&& GetSourceCount() > ((GetMaxSources() * m_MaxRemoveFQSLimit)/100))
							{
								theApp.downloadqueue->RemoveSourceAndDontAsk(cur_src, this, true/*, pos2*/); //>>> taz::don't drop complete sources //modified by sivka
...
//								m_ShowDroppedSrc_Temp++; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
						...
						// <== drop sources - Stulle
//<<< taz::drop sources
						uiMinRQR = min(uiMinRQR, _UI32_MAX-1); //>>> WiZaRd::minRQR [WiZaRd]
					}
//>>> taz::drop sources
					// ==> drop sources - Stulle
					else if(cur_src->GetRemoteQueueRank() > m_MaxRemoveQRS)
					{
						if(m_EnableAutoDropQRS && theApp.GetReAskTick() < dwCurTick) // auto drop immunity - Stulle
						{
							if( dwCurTick > (m_TimerForAutoHQRS + m_AutoHQRS_Timer)
...
//>>> WiZaRd::minRQR [WiZaRd]
					else if(cur_src->GetRemoteQueueRank())
						uiMinRQR = min(uiMinRQR, cur_src->GetRemoteQueueRank()); 
//<<< WiZaRd::minRQR [WiZaRd]
...
//>>> taz::drop sources
		m_RemoteQueueRank_Old = m_RemoteQueueRank_New; // drop sources - Stulle
//		m_ShowDroppedSrc = (uint16)(m_ShowDroppedSrc+ m_ShowDroppedSrc_Temp); //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
//<<< taz::drop sources
		...
		*/
		...
		m_uiMinRQR = uiMinRQR; //>>> WiZaRd::minRQR [WiZaRd]
	}
...
//>>> taz::drop sources
	...
	if(theApp.clientlist->DontAskThisIP(userid)){
		...
		//	theApp.emuledlg->AddDebugLogLine(false, "DONT ASK THIS IP=%s! CPartFile::CanAddSource() ignored!", ipstr(userid));
		return false;
	}
	// <== drop sources - Stulle
//<<< taz::drop sources
	...
	*/
	...
	for (part_idx = 0; part_idx < GetPartCount(); ++part_idx)
	{
		if (sender->IsPartAvailable(part_idx) && GetNextEmptyBlockInPart(part_idx, 0))
		{
			complete_src = 0;
			incomplete_src = 0;
			first_last_mod = /*0*/ 2; //>>> taz::fix ICS preview priority
			...
			{
				if (part_idx == 0 || part_idx == (GetPartCount() - 1))		first_last_mod = /*2*/ 0; //>>> taz::fix ICS preview priority
				else if (part_idx == 1 || part_idx == (GetPartCount() - 2))	first_last_mod = 1;
				else														first_last_mod = /*0*/ 2; //>>> taz::fix ICS preview priority
				//MORPH START - Changed by SiRoB, Preview like in 0.44b
                /*
				if (!thePrefs.GetPreviewPrio())								first_last_mod = 2; //>>> taz::fix ICS preview priority
				*/
				const bool isPreviewEnable = (thePrefs.GetPreviewPrio() || thePrefs.IsExtControlsEnabled() && GetPreviewPrio()) && IsPreviewableFileType();
				if (!isPreviewEnable)								first_last_mod = /*0*/ 2; //>>> taz::fix ICS preview priority
									  ...
									  uint16* count) /*const*/
{
	...
	/*const bool isPreviewEnable = (thePrefs.GetPreviewPrio() || thePrefs.IsExtControlsEnabled() && GetPreviewPrio()) && IsPreviewableFileType();*/ //>>> taz::ICS enabled for preview as well
	uint16 countbackup = *count;
	if(/*!isPreviewEnable &&*/ IsComplete(0,(uint64)PARTSIZE-1, false) && IsComplete((uint64)PARTSIZE*(GetPartCount()-1),GetFileSize()-(uint64)1, false) && thePrefs.UseICS() && GetNextRequestedBlockICS(sender,newblocks,count)) //>>> taz::ICS enabled for preview as well
		return true;
	*count = countbackup;
...
//>>> taz::drop sources
	// ==> Global Source Limit (customize for files) - Stulle
	// this file is definatly NOT active
	if(thePrefs.GetGlobalHlAll() == false && m_bGlobalHL == false)
		return true;
	// <== Global Source Limit (customize for files) - Stulle
//<<< taz::drop sources
...
//>>> taz::drop sources
// ==> drop sources - remove LowID to LowID sources from DL Queue - Stulle
...
// ==> drop sources - remove unknown, error and banned sources from DL Queue - Stulle
...
// ==> drop sources - remove not needed sources from DL Queue - Stulle
...
// ==> drop sources - remove full queue sources from DL Queue - Stulle
...
// ==> drop sources - remove high queue rating sources from DL Queue - Stulle
...
// ==> drop sources - CleanUp => NNS, FQS, HQRS, UNKOWN, ERROR and BANNED sources from DL Queue - Stulle
...
// ==> drop sources - remove leecher sources from DL Queue - Stulle
void CPartFile::RemoveLeechers()
{
//	uint16 m_ShowDroppedSrc_Temp = 0; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
	...
	for(POSITION pos2, pos1 = srclist.GetHeadPosition(); (pos2=pos1)!=NULL; ){
		...
		if(cur_src->IsBadGuy()){ //>>> taz::ClientAnalyzer
#else
		if(cur_src->/*GetScoreReduce()*/IsLeecher()){ //>>> taz::adjust for MorphXT (no Reduce Score for leecher)
#endif
			theApp.downloadqueue->RemoveSourceAndDontAsk(cur_src, this, true/*, pos2*/); //>>> taz::don't drop complete sources
//			m_ShowDroppedSrc_Temp++; //>>> taz::don't drop complete sources // show # of dropped sources - Stulle
		}
