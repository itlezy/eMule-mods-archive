...
#include "ModVersion.h" //>>> taz::NewDefaultNick [WiZaRd]
...
#ifdef CLIENTANALYZER
uint8	CPreferences::m_uiCreditSystem; //>>> WiZaRd::ClientAnalyzer
#else
...
//MORPH START - Added by SiRoB / Commander, Wapserver [emulEspaa]
	...
	//MORPH END - Added by SiRoB / Commander, Wapserver [emulEspaa]
...
// <== Global Source Limit (customize for files) - Stulle
bool	CPreferences::m_bDontDropCompleteSources; //>>> taz::don't drop complete sources
//<<< taz::drop sources
//>>> taz::ASF
TCHAR    CPreferences::ServerFilterVal[768]; //Mulo da Soma - Add more blanks in text-area
TCHAR    CPreferences::ServerFilterDesc[768]; //Mulo da Soma - Add more blanks in text-area
UINT	 CPreferences::ServerFilterByUsers;
UINT	 CPreferences::ServerFilterByFiles;
bool	CPreferences::m_bASF_ON;
bool	CPreferences::m_bASF_AutoFilter;
//<<< taz::ASF
bool	CPreferences::m_bShowLowID; //>>> taz::lowid notifier [chamblard]
bool	CPreferences::m_bShowCorrupted; //>>> taz::More info about corrupted .met/.part file [Aenarion]
//>>> taz::AcK filters [Aenarion/Xanatos]
bool	CPreferences::m_bFamilyFilter;
TCHAR	CPreferences::m_cFamilyFilter[512];
bool	CPreferences::m_isPWProtShow;
CString	CPreferences::m_sPassFF;
bool	CPreferences::m_bFakeFilter;
TCHAR	CPreferences::m_cFakeFilter[512];
//<<< taz::AcK filters [Aenarion/Xanatos]
bool	CPreferences::m_bTrayComplete; //>>> taz::Completed in Tray [Stulle]
//>>> WiZaRd::minRQR [WiZaRd]
bool	CPreferences::m_bShowMinRQR;
//<<< WiZaRd::minRQR [WiZaRd]
uint8	CPreferences::BANTHRES; //>>> taz::Variable corrupted blocks ban threshold [Spike2]
...
//>>> taz::PowerShare stats [taz/pP]
	ini.WriteUInt64(L"PowershareGlobalComplete", theStats.GetUploadedPowershareComplete_Global());
	ini.WriteUInt64(L"PowershareGlobalPartfile", theStats.GetUploadedPowersharePartfile_Global());
//<<< taz::PowerShare stats [taz/pP]
...
//>>> Spike2::Enhanced Client Recognition
				case SO_HYDRANODE:
				case SO_EMULEPLUS:
				case SO_TRUSTYFILES:
//<<< Spike2::Enhanced Client Recognition
...
//>>> taz::Powershare stats [taz/pP]
	theStats.AddupUploadedPowershareComplete_Global(ini.GetUInt64(L"PowershareGlobalComplete", 0));
	theStats.AddupUploadedPowersharePartfile_Global(ini.GetUInt64(L"PowershareGlobalPartfile", 0));
//<<< taz::Powershare stats [taz/pP]
...
void CPreferences::SavePreferences()
{
	...
	*/
	...
	ini.WriteBool(L"LogAnalyzerEvents", m_bLogAnalyzerEvents);
#else
...
//>>> taz::Quick start [TPT]
	...
	ini.WriteInt(L"QuickStartMaxConnBack", m_iQuickStartMaxConnBack); //>>> taz::just for compat with StulleMule
	ini.WriteInt(L"QuickStartMaxConnPerFiveBack", m_iQuickStartMaxConnPerFiveBack); //>>> taz::just for compat with StulleMule
...
//>>> taz::drop sources
	...
	// <== Global Source Limit (customize for files) - Stulle
	ini.WriteBool(L"DontDropCompleteSources", m_bDontDropCompleteSources); //>>> taz::don't drop complete sources
//<<< taz::drop sources
	//MORPH START - Added by Stulle, Adjustable NT Service Strings
	...
	//MORPH END   - Added by Stulle, Adjustable NT Service Strings
//>>> taz::ASF
	ini.WriteString(L"SrvFilterVal", ServerFilterVal);
	ini.WriteString(L"SrvFilterDesc", ServerFilterDesc);
	ini.WriteInt(L"SrvFilterUsers",ServerFilterByUsers);
	ini.WriteInt(L"SrvFilterFiles",ServerFilterByFiles);
	ini.WriteBool(L"ASF_ON", m_bASF_ON);
	ini.WriteBool(L"ASF_AutoFilter", m_bASF_AutoFilter);
//<<< taz::ASF
	ini.WriteBool(L"Open_lowid_URL", m_bShowLowID);  //>>> taz::lowid notifier [chamblard]
	ini.WriteBool(L"ShowCorruptedInfo", m_bShowCorrupted); //>>> taz::More info about corrupted .met/.part file [Aenarion]
//>>> taz::AcK filters [Aenarion/Xanatos]
	ini.WriteBool(L"FamilyFilterIsActive",m_bFamilyFilter);
	ini.WriteString(L"FamilyFilter",m_cFamilyFilter);
	ini.WriteString(L"PassFF",/*GetPassFF()*/m_sPassFF);//Family filter
	ini.WriteBool(L"1",m_isPWProtShow);//Family Filter
	ini.WriteBool(L"FakeFilterIsActive",m_bFakeFilter);
	ini.WriteString(L"FakeFilter",m_cFakeFilter);
//<<< taz::AcK filters [Aenarion/Xanatos]
	ini.WriteBool(L"TrayComplete", m_bTrayComplete); //>>> taz::Completed in Tray [Stulle]
//>>> WiZaRd::minRQR [WiZaRd]
	ini.WriteBool(L"MinRQR", m_bShowMinRQR);
//<<< WiZaRd::minRQR [WiZaRd]
	ini.WriteInt(L"BANTHRES",BANTHRES); //>>> taz::Variable corrupted blocks ban threshold [Spike2]
//>>> Spike2::Emulate other
	ini.WriteBool(L"Emulate eDonkey", m_bEmueDonkey);
	ini.WriteBool(L"Emulate eDonkeyHybrid", m_bEmueDonkeyHybrid);
	ini.WriteBool(L"Emulate MLDonkey", m_bEmuMLDonkey);
	ini.WriteBool(L"Emulate Shareaza", m_bEmuShareaza);
	ini.WriteBool(L"Emulate Lphant", m_bEmuLphant);
//<<< Spike2::Emulate other
}
...
void CPreferences::LoadPreferences()
{
	...
	*/
	...
	if (strNick.IsEmpty() /*|| IsDefaultNick(strNick)*/) //>>> taz::NewDefaultNick [WiZaRd]
  ...
  // MORPH END leuk_he upnp bindaddr
	...
	m_uDeadServerRetries=ini.GetInt(L"DeadServerRetry",0);	// morph 1 --> 3 for no fallback from obfuscated -> normal. 
	...
	if (IsRunningAeroGlassTheme())
		...
		mintotray=ini.GetBool(L"MinToTray", /*false*/true); //>>> taz::defaults
	...
	m_iLastLogPaneID=ini.GetInt(L"LastLogPaneID",1); //>>> taz::defaults
	m_bSafeServerConnect =ini.GetBool(L"SafeServerConnect", true); //>>> taz::defaults
	...
	m_bpreviewprio=ini.GetBool(L"PreviewPrio",true); //>>> taz::defaults
	...
	m_btransferfullchunks=/*ini.GetBool(L"FullChunkTransfers",true)*/true; //>>> taz::mandatory
	m_istartnextfile=ini.GetInt(L"StartNextFile",0);
	m_bshowoverhead=ini.GetBool(L"ShowOverhead",true); //>>> taz::defaults
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	m_bExtControls =ini.GetBool(L"ShowExtControls", true); //>>> taz::defaults
	// MORPH START show less controls
...
#ifdef CLIENTANALYZER
	m_uiCreditSystem=(uint8)ini.GetInt(L"UseCreditSystem", 2); //>>> WiZaRd::ClientAnalyzer
	m_bLogAnalyzerEvents = ini.GetBool(L"LogAnalyzerEvents", true);
#else
	/*
	...
	*/
	...
	m_bShowDwlPercentage=ini.GetBool(L"ShowDwlPercentage",true); //>>> taz::defaults
	...
	m_bShowActiveDownloadsBold = ini.GetBool(L"ShowActiveDownloadsBold", true); //>>> taz::defaults
	...
	*/
	...
	m_bUseSecureIdent=/*ini.GetBool(L"SecureIdent",true)*/true; //>>> taz::mandatory
    ...
    m_bShowClientPercentage=ini.GetBool(L"ShowClientPercentage",true);  //Commander - Added: Client Percentage //>>> taz::defaults
	...
	//EastShare - added by AndCycle, IP to Country
	m_iIP2CountryNameMode = ini.GetInt(L"IP2Country", IP2CountryName_SHORT); //>>> taz::defaults
	m_bIP2CountryShowFlag = ini.GetBool(L"IP2CountryShowFlag", true); //>>> taz::defaults
	//EastShare - added by AndCycle, IP to Country
	...
	//EastShare - Added by Pretender, TBH-AutoBackup
	infiniteQueue=ini.GetBool(L"InfiniteQueue",true);	//Morph - added by AndCycle, SLUGFILLER: infiniteQueue //>>> taz::defaults
	...
	m_bCryptLayerRequested = ini.GetBool(L"CryptLayerRequested", true); //>>> taz::defaults
  ...
  m_bCryptLayerRequiredStrictServer = ini.GetBool(L"CryptLayerRequiredStrictServer",false); // MORPH lh require obfuscated server connection 
	...
	// Mighty Knife: Static server handling
	SetDontRemoveStaticServers (/*ini.GetBool (L"DontRemoveStaticServers",true)*/ true); //>>> taz::mandatory
	// [end] Mighty Knife
	...
	m_bFakeAlyzerIndications = ini.GetBool(L"FakeAlyzerIndications",true); //MORPH - Added by Stulle, Fakealyzer [netfinity] //>>> taz::defaults
...
//>>> taz::Quick start [TPT]
	...
	m_iQuickStartMaxConnBack=/*ini.GetInt(L"QuickStartMaxConnBack", 400)*/ maxconnections; //>>> taz::use connection setting
	m_iQuickStartMaxConnPerFiveBack=/*(uint16)ini.GetInt(L"QuickStartMaxConnPerFiveBack", 30)*/ MaxConperFive; //>>> taz::use connection setting
...
//>>> taz::drop sources
	...
	// <== Global Source Limit (customize for files) - Stulle
	m_bDontDropCompleteSources = ini.GetBool(L"DontDropCompleteSources", true); //>>> taz::don't drop complete sources
//<<< taz::drop sources
	//MORPH START - Added by Stulle, Adjustable NT Service Strings
	...
	//MORPH END   - Added by Stulle, Adjustable NT Service Strings
//>>> taz::ASF
//	_stprintf(ServerFilterVal,L"%s",ini.GetString(L"SrvFilterVal",L"best|china|chipprofis|ddload|eserver|fast|fuck|game|geizadler|hotzone|icupid|itx|kostenlos|lair|p2p4ever|pirate|pronmaster|rackbox|razorback|rohan|saugs|sex|sinowish|sharing|sheisner|sofort|sonny|speed|teen|telcoo|top|warez|wild|wmule"));//Updated by Mulo da Soma
	_stprintf(ServerFilterVal,L"%s",ini.GetString(L"SrvFilterVal",L"Ass|best|boobies|China|ddload|eserver|fast|fuck|hentaimaster|hOTzOne|icupid|Octopussy|Orgasmule|pirate|porn|prOnMaSter|Razorb|Saugs|sex|ServerList|shar|Speedup|teen|ultranova|warez|Wild|wmule|XeMule|xMule|Xvids|XXX"));//Updated by Mulo da Soma
//	_stprintf(ServerFilterDesc,L"%s",ini.GetString(L"SrvFilterDesc",L"ass|bang|bite|breizh|built|cr4ck|ddload|dial|donkey|eserver|fast|fr33|fuck|geizadler|high|hotzone|kick|kostenlos|land|ninja|opteron|p2p4ever|piracy|pirates|porn|riaa|search|serverlist|sex|sinowish|sonny|sofort|speed|teen|via|wild|wmule|xxx"));//Updated by Mulo da Soma
	_stprintf(ServerFilterDesc,L"%s",ini.GetString(L"SrvFilterDesc",L"ass|Bang|china|cum|ddload|desc|dial|donkey|eserver|fast|fuck|high speed|hotzone|kick|Land|list|P2P|Pirate|porn|pussy|saugs|shar|Search|serverlist|sex|teen|Ultranova|Wmule|XeMule|xMule|Xvids|xxx"));//Updated by Mulo da Soma
	ServerFilterByUsers=ini.GetInt(L"SrvFilterUsers",3000);//change value by Mulo da Soma
	if(ServerFilterByUsers > 1000000000) ServerFilterByUsers = 1000000000; // is it necessary ??
	ServerFilterByFiles=ini.GetInt(L"SrvFilterFiles",200000);//change value by Mulo da Soma
	if(ServerFilterByFiles > 1000000000) ServerFilterByFiles = 1000000000; // is it necessary ??
	m_bASF_ON = ini.GetBool(L"ASF_ON", true);
	m_bASF_AutoFilter = (m_bASF_ON) ? ini.GetBool(L"ASF_AutoFilter", false) : false;
//<<< taz::ASF
	m_bShowLowID = ini.GetBool(L"Open_lowid_URL", true); //>>> taz::lowid notifier [chamblard]
	m_bShowCorrupted = ini.GetBool(L"ShowCorruptedInfo", true); //>>> taz::More info about corrupted .met/.part file [Aenarion]
//>>> taz::AcK filters [Aenarion/Xanatos]
	m_bFamilyFilter=ini.GetBool(L"FamilyFilterIsActive",false);
//    _stprintf(m_cFamilyFilter,L"%s",ini.GetString(L"FamilyFilter",L"sex,fuck,xxx,lolita,erotic,anal,suck,pussy,nude,teen,blowjob,cum,facial,porn,tit,deepthroat,blow job,deep throat,mature,incesti,porno,sesso,figa,culo,vergine,scopano"));
    _stprintf(m_cFamilyFilter,L"%s",ini.GetString(L"FamilyFilter",L"sex,fuck,xxx,lolit,erotic,anal,suck,pussy,nude,teen,blowjob,cum,facial,porn,tit,deepthroat,mature,cazz,incest,sesso,fig,cul,vergin,scopa")); //Updated by Mulo da Soma
	m_sPassFF=ini.GetString(L"PassFF",L"");
	m_isPWProtShow=ini.GetBool(L"1",false);
	m_bFakeFilter=ini.GetBool(L"FakeFilterIsActive",true);
//    _stprintf(m_cFakeFilter,L"%s",ini.GetString(L"FakeFilter",L"velocizzare emule|abc|velocizzare mulo|trucchi emule|velocizzare download|trucco release|acelerador emule|acelerar|acceleratore emule|aumentare velocit|emule velocidad maxima|guida release emule|truco emule|rapido emule|emule velocissimo|scaricare veloce|emule rapido|speed up emule|emule very fast|leecher speed|emule leecher|leecher mods|leecher mod|leecher version|p2p acelerator|emule acelerator"));
    _stprintf(m_cFakeFilter,L"%s",ini.GetString(L"FakeFilter",L"velocizzare emule|abc|velocizzare mulo|trucchi emule|velocizzare download|trucco release|acelerador emule|acelerar|acceleratore emule|aumentare velocit|emule velocidad maxima|guida release emule|truco emule|rapido emule|emule velocissimo|scaricare veloce|emule rapido|speed up emule|emule very fast|leecher speed|emule leecher|leecher mods|leecher mod|leecher version|p2p acelerator|emule acelerator")); //Updated by Mulo da Soma
//>>> taz::AcK filters [Aenarion/Xanatos]
	m_bTrayComplete = ini.GetBool(L"TrayComplete",true); //>>> taz::Completed in Tray [Stulle]
//>>> WiZaRd::minRQR [WiZaRd]
	m_bShowMinRQR = ini.GetBool(L"MinRQR", true);
//<<< WiZaRd::minRQR [WiZaRd]
	BANTHRES = (uint8) ini.GetInt(L"BANTHRES", 32); //>>> taz::Variable corrupted blocks ban threshold [Spike2]
//>>> Spike2::Emulate other
	m_bEmueDonkey = ini.GetBool(L"Emulate eDonkey", true);
	m_bEmueDonkeyHybrid = ini.GetBool(L"Emulate eDonkeyHybrid", true);
	m_bEmuMLDonkey = ini.GetBool(L"Emulate MLDonkey", true);
	m_bEmuShareaza = ini.GetBool(L"Emulate Shareaza", true);
	m_bEmuLphant = ini.GetBool(L"Emulate Lphant", true);
//<<< Spike2::Emulate other
...
// End -Random Ports-
	...
	// emulEspaa: Added by MoNKi [MoNKi: -Random Ports-]
	if(original)
		return port;
