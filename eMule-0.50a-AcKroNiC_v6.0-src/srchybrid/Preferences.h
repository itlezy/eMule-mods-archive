...
enum EDefaultDirectory{
	...
	EMULE_WAPSERVERDIR = 13, // Wapserver [emulEspaסa]
...
public:
    ...
    //MORPH START - Added by SiRoB / Commander, Wapserver [emulEspaסa]
	...
	//MORPH END - Added by SiRoB / Commander, Wapserver [emulEspaסa]
...
//>>> taz::drop sources
	...
	// <== Global Source Limit (customize for files) - Stulle
	static	bool m_bDontDropCompleteSources; //>>> taz::don't drop complete sources
//<<< taz::drop sources
	...
	friend class CSivkaFileSettings; //>>> taz::drop sources // drop sources - Stulle
	...
	*/
	...
	// emulEspaסa: Modified by MoNKi [MoNKi: -UPnPNAT Support-]
	/*
	static	uint16	GetPort()							{return port;}
	static	uint16	GetUDPPort()						{return udpport;}
	*/
		// emulEspaסa: Modified by MoNKi [MoNKi: -Random Ports-]
		/*
...
//	static	bool	IsDefaultNick(const CString strCheck); //>>> taz::NewDefaultNick [WiZaRd]
...
//>>> taz::drop sources
	...
	static bool		DontDropCompleteSources() {return m_bDontDropCompleteSources;} //>>> taz::don't drop complete sources
//<<< taz::drop sources
...
//>>> taz::ASF
public:
	static	UINT	GetServerFilterByUsers()		{return ServerFilterByUsers;}
	static	UINT	GetServerFilterByFiles()		{return ServerFilterByFiles;}
	static  TCHAR   ServerFilterVal[768]; //Mulo da Soma - Add more blanks in text-area
	static  TCHAR   ServerFilterDesc[768]; //Mulo da Soma - Add more blanks in text-area
	static	UINT	ServerFilterByUsers;
	static	UINT	ServerFilterByFiles;
	static	void	SetASF(bool in)				{m_bASF_ON = in;}
	static	bool	ASF_ON()				{return m_bASF_ON;}
	static	void	SetASF_AutoFilter(bool in)		{m_bASF_AutoFilter = in;}
	static	bool	ASF_AutoFilter()			{return m_bASF_AutoFilter;}
private:
	static	bool	m_bASF_ON;
	static	bool	m_bASF_AutoFilter;
//<<< taz::ASF
//>>> taz::lowid notifier [chamblard]
public:
	static	bool	m_bShowLowID;
//<<< taz::lowid notifier [chamblard]
//>>> taz::More info about corrupted .met/.part file [Aenarion]
public:
	static	bool	m_bShowCorrupted;
//<<< taz::More info about corrupted .met/.part file [Aenarion]
//>>> taz::AcK filters [Aenarion/Xanatos]
public:
	static	bool    m_bFamilyFilter;
 	static	TCHAR	m_cFamilyFilter[512];
	static	bool	m_isPWProtShow;
	static	CString	m_sPassFF;
	static  CString GetPassFF()				{ return m_sPassFF; }
	static  void    SetPWProtShow(bool bEnable)             { m_isPWProtShow=bEnable; }
	static	bool	m_bFakeFilter;
	static	TCHAR	m_cFakeFilter[512];
//<<< taz::AcK filters [Aenarion/Xanatos]
//>>> taz::Completed in Tray [Stulle]
private:
	static	bool	m_bTrayComplete;
public:
	static	bool	GetTrayComplete()		{ return m_bTrayComplete; }
	static	void	SetTrayComplete(bool b)		{ m_bTrayComplete = b; }
//<<< taz::Completed in Tray  [Stulle]
...
private:
	static  uint8   BANTHRES;
public:
	static	uint8	GetBANTHRES()				{return BANTHRES; }
	static	void	SetBANTHRES(uint8 BANTHRESVal)		{ BANTHRES = BANTHRESVal; }
//<<< taz::Variable corrupted blocks ban threshold [Spike2]
