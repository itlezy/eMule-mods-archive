...
//>>> taz::AcK filters [Aenarion/Xanatos]
	if (thePrefs.m_bFamilyFilter) 
	{ 
		if (thePrefs.m_cFamilyFilter != NULL) 
		{
			CString FileName=toshow->GetFileName();
			CString Token;
			CString strlink=thePrefs.m_cFamilyFilter;
			strlink.MakeLower();
			int curPos=0;
			Token= strlink.Tokenize(L",",curPos);
			while (Token != L"") 
			{
				if (FileName.MakeLower().Find(Token)>-1)
				{	
					//++theApp.emuledlg->searchwnd->nFFResultsFiltered;
