...
//>>> taz::AcK filters [Aenarion/Xanatos]
			if (thePrefs.m_bFakeFilter) 
			{
				if (thePrefs.m_cFakeFilter != NULL) 
				{
					CString resTok;
					CString strlink=thePrefs.m_cFakeFilter;
					strlink.MakeLower();
					int curPos=0;
					resTok= strlink.Tokenize(L"|",curPos);
					while (resTok != L"") 
					{
						if (pParams->strExpression.MakeLower().Find(resTok)>-1)
						{   
							AfxMessageBox(GetResString(IDS_AVVISO));
							//pParams->strExpression = L"";
							break;
							//return;
						}
						resTok= strlink.Tokenize(L"|",curPos);  
