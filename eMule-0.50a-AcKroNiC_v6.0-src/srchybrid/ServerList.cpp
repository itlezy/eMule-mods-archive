...
*/
	if ((thePrefs.GetFilterServerByIP() && theApp.ipfilter->IsFiltered(pServer->GetIP())) ||
		(thePrefs.ASF_AutoFilter() && !ASFValidServer(pServer))){
		{
//<<< taz::ASF
			if (thePrefs.GetLogFilteredIPs())
...
//>>> taz::ASF
bool CServerList::ASFValidServer(const CServer* srv)
{
	if (!thePrefs.ASF_ON())
		return true;
	if (thePrefs.ServerFilterVal != NULL)
	{
		CString FileName=srv->GetListName();
		CString Token;
		CString strlink=thePrefs.ServerFilterVal;
		strlink.MakeLower();
		int curPos=0;
		Token= strlink.Tokenize(_T("|"),curPos);
		while (Token != _T(""))
		{
			if (FileName.MakeLower().Find(Token)>-1)
			{
				 return false;
			 }
	                 Token= strlink.Tokenize(_T("|"),curPos);
	...
	if (thePrefs.ServerFilterDesc != NULL)
	{
		CString FileDesc=srv->GetDescription();
		CString TokenDesc;
		CString strlinkDesc=thePrefs.ServerFilterDesc;
		strlinkDesc.MakeLower();
		int curPosDesc=0;
		TokenDesc= strlinkDesc.Tokenize(_T("|"),curPosDesc);
		while (TokenDesc != _T(""))
		{
			if (FileDesc.MakeLower().Find(TokenDesc)>-1)
			{
				 return false;
			 }
	                 TokenDesc= strlinkDesc.Tokenize(_T("|"),curPosDesc);
	...
	uint32 si;
	uint32 no;
	si=(uint32)(srv->GetUsers());
	no=(uint32)(srv->GetFiles());
	if(si ==0 && no ==0) return true; //In questo caso non so quanti utenti e file abbia il server e non lo filtro.
	if(thePrefs.GetServerFilterByUsers() != 0 || thePrefs.GetServerFilterByFiles() != 0){
		if ((si > thePrefs.GetServerFilterByUsers())){
			if ((no > thePrefs.GetServerFilterByFiles())){
				return true;
			}
			else
				return false;
