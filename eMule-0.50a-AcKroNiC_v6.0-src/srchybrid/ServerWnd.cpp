...
#include "ModVersion.h" //>>> taz::New Version Check
...
BEGIN_MESSAGE_MAP(CServerWnd, CResizableDialog)
	...
	ON_BN_CLICKED(IDC_SRVFILTER_BTN, OnBnClickedServerFilterDlg) //>>> taz::ASF
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	//MORPH END   - Added by SiRoB, XML News [O]
	AddAnchor(IDC_SRVFILTER_BTN, MIDDLE_RIGHT); //>>> taz::ASF
...
//>>> taz::AcK (default log tab)
	logbox->ShowWindow(/*SW_HIDE*/SW_SHOW);
	servermsgbox->ShowWindow(/*SW_SHOW*/SW_HIDE);
//<<< taz::AcK (default log tab)
...
//<<< taz::AcK (default log tab)
	...
	//MORPH END   - Added by SiRoB, XML News [O]
	GetDlgItem(IDC_SRVFILTER_BTN)->EnableWindow(thePrefs.ASF_ON() && !thePrefs.ASF_AutoFilter()); //>>> taz::ASF
...
void CServerWnd::Localize()
{
	...
	GetDlgItem(IDC_SRVFILTER_BTN)->SetWindowText(GetResString(IDS_SERVERFILTER)); //>>> taz::ASF
...
//>>> taz::ASF
	if (thePrefs.GetFilterServerByIP() || (thePrefs.ASF_ON() && thePrefs.ASF_AutoFilter())){
		...
		// if (toadd->HasDynIP())
		// 	return false;
		if (theApp.ipfilter->IsFiltered(toadd->GetIP()) || !theApp.serverlist->ASFValidServer(toadd))
		{
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, L"Ignored server (IP=%s) - IP filter (%s)", ipstr(toadd->GetIP()), theApp.ipfilter->GetLastHit());
			delete toadd;
			return false;
		}
	}
	...
	*pResult = 0;
	...
	if (pEnLink && pEnLink->msg == WM_LBUTTONDOWN)
	{
		...
		//MORPH START - Added by SiRoB, New Version Check
			...
			strUrl = /*_T("http://emulemorph.sourceforge.net/")*/ MOD_FORUM_URL; //>>> taz::adjust
...
//>>> taz::ASF
	GetDlgItem(IDC_SRVFILTER_BTN)->GetWindowRect(rcDlgItem);
	ScreenToClient(rcDlgItem);
	//MORPH START - Changed by SiRoB, XML News
	/*
	rcDlgItem.top = splitpos + 9;
	...
	*/
	...
	GetDlgItem(IDC_SRVFILTER_BTN)->MoveWindow(rcDlgItem);
//<<< taz::ASF
...
void CServerWnd::ReattachAnchors()
{
	...
	RemoveAnchor(IDC_SRVFILTER_BTN); //>>> taz::ASF
	...
	GetDlgItem(IDC_SRVFILTER_BTN)->Invalidate(); //>>> taz::ASF
	...
	if (morphlog->IsWindowVisible())
		morphlog->Invalidate();
	//MORPH END   - Added by SiRoB, Morph Log
	m_wndSplitter.Invalidate(); //>>> taz::fix Repaint Splitter - ReattachAnchors [moloko+]
}
...
//>>> taz::ASF
void CServerWnd::OnBnClickedServerFilterDlg()
{
	int count=0;
	int scount=0;
	...
	for (POSITION pos=theApp.serverlist->list.GetHeadPosition(); pos!=NULL; theApp.serverlist->list.GetNext(pos)) {
		CServer* cur_srv=theApp.serverlist->list.GetAt(pos);
		if (!cur_srv->IsStaticMember())
			if (!theApp.serverlist->ASFValidServer(cur_srv)) {
				if (cur_srv->IsStaticMember()) {
				...
				count++;
				theApp.serverlist->RemoveServer(cur_srv);
				if ((pos=theApp.serverlist->list.GetHeadPosition())==NULL)
					break;
			}
	}
	...
	if (count) {
		...
		AddLogLine(false, L"Removed %i (%i Static) Servers using 'Advanced Server Filter'",count,scount);
	}
}
//<<< taz::ASF