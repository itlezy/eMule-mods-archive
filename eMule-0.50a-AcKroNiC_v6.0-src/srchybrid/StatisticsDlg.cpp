...
//>>> taz::PowerShare stats
				cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_TOTAL), CastItoXBytes(theStats.GetUploadedPowershareTotal_Session(), false, false)); // total up
				stattree.SetItemText(up_S[1], cbuffer);
				if (forceUpdate || stattree.IsExpanded(up_S[1]))
				{
					cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_COMPLETE), CastItoXBytes(theStats.GetUploadedPowershareComplete_Session(), false, false)); // from complete files
					stattree.SetItemText(h_Powershare_Session[0], cbuffer);
					cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_INCOMPLETE), CastItoXBytes(theStats.GetUploadedPowersharePartfile_Session(), false, false)); // from incomplete files
					stattree.SetItemText(h_Powershare_Session[1], cbuffer);
				}
//<<< taz::PowerShare stats
				...
				stattree.SetItemText(up_S[/*1*/2], cbuffer); //>>> taz::PowerShare stats
				...
				*/
				...
				stattree.SetItemText(up_S[/*5*/6], cbuffer); //>>> taz::PowerShare stats
				if (forceUpdate || stattree.IsExpanded(up_S[/*5*/6])) //>>> taz::PowerShare stats
				{
...
//>>> taz::PowerShare stats
				cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_TOTAL), CastItoXBytes(theStats.GetUploadedPowershareTotal_Global(), false, false)); // total up
				stattree.SetItemText(up_T[1], cbuffer);
				if (forceUpdate || stattree.IsExpanded(up_T[1]))
				{
					cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_COMPLETE), CastItoXBytes(theStats.GetUploadedPowershareComplete_Global(), false, false)); // from complete files
					stattree.SetItemText(h_Powershare_Global[0], cbuffer);
					cbuffer.Format(L"%s %s", GetResString(IDS_PS_STATS_INCOMPLETE), CastItoXBytes(theStats.GetUploadedPowersharePartfile_Global(), false, false)); // from incomplete files
					stattree.SetItemText(h_Powershare_Global[1], cbuffer);
				}
//<<< taz::PowerShare stats
				...
				cbuffer.Format(_T("%s: %u"), GetResString(IDS_STATS_ULSES), statGoodSessions + statBadSessions);
				stattree.SetItemText(up_T[/*1*/2], cbuffer); //>>> taz::PowerShare stats
				if (forceUpdate || stattree.IsExpanded(up_T[/*1*/2]))  //>>> taz::PowerShare stats
				{
					// Set Successful Upload Sessions & Average Uploaded Per Session
					if (statGoodSessions>0) 
					{ // Blackholes are when God divided by 0
...
//>>> taz::Enhanced Client Recognition [Stulle]
			...
			if (stattree.ItemHasChildren(clisoft[6])){
				...
				while( hChild != NULL && mpos != NULL )
				{
					...
					cbuffer.Format(L"%s: %i (%1.1f%%)", name, count, (double)count/myStats[ 5]*100);
					stattree.SetItemText(hChild, cbuffer);
					hChild = stattree.GetNextSiblingItem( hChild );
				}
				while (hChild != NULL){
...
//<<< taz::Enhanced Client Recognition [Stulle]
		...
		cbuffer.Format(GetResString(IDS_STATS_LEECHERCLIENTS), theStats.leecherclients);
...
void CStatisticsDlg::CreateMyTree() 
{
	...
	for(int i = 0; i</*6*/_countof(up_S); i++) //>>> taz::PowerShare stats //>>> WiZaRd::fix
...
//>>> taz::PowerShare stats
	up_S[1] = stattree.InsertItem(GetResString(IDS_PS_STATS_TOTAL), up_S[0]);
	for (int i = 0; i < ARRSIZE(h_Powershare_Session); i++)
		h_Powershare_Session[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), up_S[1]);
//<<< taz::PowerShare stats
	for(int i = 0; i<4; i++) 
		up_ssessions[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), up_S[/*5*/6]); //>>> taz::PowerShare stats //MORPH - Added by Yun.SF3, ZZ Upload System
...
//>>> taz::PowerShare stats
	up_T[1] = stattree.InsertItem(GetResString(IDS_PS_STATS_TOTAL), up_T[0]);
	for (int i = 0; i < ARRSIZE(h_Powershare_Global); i++)
		h_Powershare_Global[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), up_T[1]);
//<<< taz::PowerShare stats
	up_T[/*1*/2]= stattree.InsertItem(GetResString(IDS_FSTAT_WAITING),h_up_total); //>>> taz::PowerShare stats // Upload Sessions (Total)
	for(int i = 0; i<4; i++) 
		up_tsessions[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), up_T[/*1*/2]); //>>> taz::PowerShare stats
