...
//>>> WiZaRd::ClientAnalyzer
    ...
    return (uint32)(10.0f * credits->GetScoreRatio(GetIP()) * (float)GetFilePrioAsNumber());
		case 2:
			if(pAntiLeechData)
				return (uint32)(10.0f * pAntiLeechData->GetScore() * (float)GetFilePrioAsNumber());
	...
	while (togo){
		...
		if (IsUploadingToPeerCache())
		{
			...
			if (m_iHttpSendState == 0)
			{
				...
				*/
#ifdef CLIENTANALYZER
				str.AppendFormat("Server: eMule/%s %s\r\n", CStringA(theApp.m_strCurVersionLong), CStringA(theApp.m_strModLongVersion));
#else
    ...
    if (GetFileUploadSocket() && (m_ePeerCacheUpState != PCUS_WAIT_CACHE_REPLY))
	{
		...
		if(pAntiLeechData)
		{
			...
			if(GetUploadFileID())
			{
				...
				*/
				CKnownFile* file = CheckAndGetReqUpFile();;
				if(file)
				{
...
//>>> taz::PowerShare stats [taz/pP]
	...
	*/
	...
	{
		// session
		theStats.AddupUploadedPowersharePartfile_Session(sentBytesPartFile);
		theStats.AddupUploadedPowershareComplete_Session(sentBytesCompleteFile);
		// global
		theStats.AddupUploadedPowersharePartfile_Global(sentBytesPartFile);
		theStats.AddupUploadedPowershareComplete_Global(sentBytesCompleteFile);
	}
//<<< taz::PowerShare stats [taz/pP]
