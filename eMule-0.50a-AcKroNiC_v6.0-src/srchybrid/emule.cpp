...
#include "WebServer.h"
#include "WapServer/WapServer.h" //MORPH - Added by SiRoB / Commander, Wapserver [emulEspaa]
...
// MOD Note: end
   ...
   if (CemuleApp::m_szMMVersion[0]!=0)
	...
	m_dwReAskTick = 0; //>>> taz::drop sources // auto drop immunity - Stulle
...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	//MORPH START - Added by Commander, Custom incoming / temp folder icon [emulEspaa]
	if(thePrefs.ShowFolderIcons()){
		theApp.AddIncomingFolderIcon();
		theApp.AddTempFolderIcon();
	}
	//MORPH END   - Added by Commander, Custom incoming / temp folder icon [emulEspaa]
	...
	*/
	...
	wapserver = new CWapServer(); //MORPH START - Added by SiRoB / Commander, Wapserver [emulEspaa]
...
// Commander - Added: Custom incoming / temp folder icon [emulEspaa] - Start
...
// Commander - Added: Custom incoming / temp folder icon [emulEspaa] - End
...
// End MoNKi
// Commander - Added: FriendLinks [emulEspaa] - Start
...
{
	if(thePrefs.IsRASAIC()==false && thePrefs.IsIQCAOC()==false)
		return;

	uint32 uTempID[3] = {0,0,0};
//>>> taz::adjust
/*
	if(thePrefs.GetCheckCon())
		uTempID[0] = conchecker.GetID();
*/
//<<< taz::adjust
	if(serverconnect->IsConnected())
		uTempID[1] = serverconnect->GetClientID();
	uTempID[2] = GetPublicIP();

//	AddLogLine(false,_T("$$$ Accessed CheckIdChange()"));
	for(int i=0; i<3; i++)
	{
...
//>>> taz::adjust
/*
			AddLogLine(false,_T("Detected with %s"),
				(i == 0)?_T("Con Checker"):((i==1)?_T("Server connection"):_T("Kad connection")));
*/
//<<< taz::adjust
//>>> taz::Quick start [TPT]
			...
			// <== Quick start [TPT] - Stulle
//<<< taz::Quick start [TPT]
			SetReAskTick(::GetTickCount()+1200000); // auto drop immunity - Stulle
		}
//		AddLogLine(false,_T("$$$ i: %i | m_uLastValidID: %u | uTempID: %u | with: %s"),
//			i,
//			m_uLastValidID[i],
//			uTempID[i],
//			(i == 0)?_T("Con Checker"):((i==1)?_T("Server connection"):_T("Kad connection")));
