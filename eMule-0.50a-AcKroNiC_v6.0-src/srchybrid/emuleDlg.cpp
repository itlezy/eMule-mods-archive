...
BOOL CemuleDlg::OnInitDialog()
{
	...
	*/
	...
	theApp.SetReAskTick(GetTickCount()+1200000); //>>> taz::drop sources // auto drop immunity - Stulle
...
*/
	...
	m_sVerCheck.Format(L"%s%s", CemuleApp::m_szMVersion, L".dyndns.info.");
	wcstombs_s(&CharactersConverted, mod_version, m_sVerCheck.GetLength()+1, m_sVerCheck, _TRUNCATE);
 	if (WSAAsyncGetHostByName(m_hWnd, UM_MVERSIONCHECK_RESPONSE, mod_version, m_acMVCDNSBuffer, sizeof(m_acMVCDNSBuffer)) == 0){
//<<< taz::-modname-
		AddLogLine(true,GetResString(IDS_NEWVERSIONFAILED));
	}
}
...
//>>> taz::Completed in Tray [Stulle]
	if (thePrefs.GetTrayComplete())
	{
		if(::GetTickCount() - m_dwTrayTime > SEC2MS(10))
		{
			int iTotal = 0;
			m_bTrayBool = (transferwnd->GetDownloadList()->GetCompleteDownloads(-1, iTotal) > 0);
...
*/
			ShellExecute(NULL, NULL, MOD_FORUM_URL, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
//<<< taz::adjust
			break;
		}
		//MORPH END   - Added by SiRoB, New Version Check
...
*/
				// 99.major.minor.build+1
				uint32 dwResult = ntohl(((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr);
				uint8 abyCurVer[4] = { (CemuleApp::m_nMVersionBld > 0) ? (uint8)(CemuleApp::m_nMVersionBld + 1) : 201, (uint8)(CemuleApp::m_nMVersionMin), (uint8)(CemuleApp::m_nMVersionMjr), 0 };
//<<< taz::adjust
				dwResult &= 0x00FFFFFF;
				if (dwResult > *(uint32*)abyCurVer){
