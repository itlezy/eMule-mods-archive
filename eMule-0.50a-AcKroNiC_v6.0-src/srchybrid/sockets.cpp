...
#include "Log.h"
#include "langids.h" //>>> taz::lowid notifier [chamblard]
...
//>>> taz::lowid notifier [chamblard]
	if (thePrefs.m_bShowLowID && IsConnected() && IsLowID()){
		CString theUrl;
		switch (thePrefs.GetLanguageID()){
		case LANGID_IT_IT :
			theUrl = L"http://www.ackronic.net/ackws/mod/lowid/LowidIt.htm";
			break;
		case LANGID_FR_FR :
			theUrl = L"http://www.ackronic.net/ackws/mod/lowid/LowidFr.htm";
			break;
		default :
			theUrl = L"http://www.ackronic.net/ackws/mod/lowid/LowidEn.htm";
			break;
		}
		ShellExecute(NULL, NULL, theUrl, NULL, NULL, SW_SHOWDEFAULT);
	}
//<<< taz::lowid notifier [chamblard]
