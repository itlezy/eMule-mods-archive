...
#define URLINDICATOR	_T("http:|www.|.de |.net |.com |.org |.to |.tk |.cc |.fr |ftp:|ed2k:|https:|ftp.|.info|.biz|.uk|.eu|.es|.tv|.cn|.tw|.ws|.nu|.jp|.it") //Anis -> o.O e i .it fanno così ribrezzo all'estero?? xD
...
{	
	m_abyUpPartStatusHidden = NULL; //>>> Anis::Intelligent SOTN
	//>>> Anis::ICS [enkeyDEV]
	m_abyIncPartStatus = NULL;
	m_incompletepartVer = 0;
	//### Anis::ICS [enkeyDEV]
	m_abySeenPartStatus = NULL; //>>> Anis::AntiHideOS [netfinity]
	m_abyfiledata = NULL;
	...
	// Mod Adu
 	...
 	aduver = 0;
 	// Fine adu
	hiscompletedparts_percent_up=-1; //Anis percentuale upload.
...
void CUpDownClient::ClearHelloProperties()
{
	...
	m_fDirectUDPCallback = 0;
	m_fSupportsFileIdent = 0;
	m_incompletepartVer = 0; //>>> Anis::ICS [enkeyDEV]
}
				...
				if (temptag.IsInt()) {
					...
					// Mod Adu
 					// lupz
						...
						if (bDbgInfo)
							m_strHelloInfo.AppendFormat(_T("\n  ClientVer=%u.%u.%u.%u  Comptbl=%u"), nClientMajVersion, nClientMinVersion, nClientUpVersion, 0, m_byCompatibleClient);
					}
					// Fine Mod Adu
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	const bool bSendICS = !m_pszUsername || GetIncompletePartVersion(); //>>> Anis::ICS [enkeyDEV]
	bool wastePackets = ( m_dwUserIP && HasFastwebIP() ? true : false );
	CSafeMemFile data(128);
	data.WriteUInt8((uint8)theApp.m_uCurVersionShort);
	data.WriteUInt8(EMULE_PROTOCOL);
 	// Stefano Picerno: Aggiungo il tag adu
 	data.WriteUInt32(7 /*eMule 0.45b*/ + 1 /*ET_MOD_VERSION*/ + ( wastePackets ? 1 /*AdunanzA*/ : 0 )); // nr. of tags
	...
	// lupz
	  //
	  // Trasmetto che versione del client sto usando
	  CString mod(ADU_MOD);
	  CTag adumod(ET_MOD_VERSION, mod);
	  adumod.WriteTagToFile(&data);
	  // Fine Mod AdunanzA


  // Modifica AdunanzA
  // lupz
  // Non mandiamo in giro pacchetti inutili
  if ( wastePackets ) {
	  // Stefano Picerno: Aggiungo il tag adu
	  // AdunanzA mod
	  // Kaiser/Emanem - 6/4/2004 21:29
	  //
		...
		switch (temptag.GetNameID())
		{
			...
			case ET_MOD_VERSION:
				if (temptag.IsStr()) {
					...
					// Mod Adu
					// lupz
					// aggiungo il riconoscimento di versione
					if (m_strModVersion.Find(_T("AdunanzA")) > -1) {
						uint32 adumaj=0, adumin=0, adubeta=0;
						if  (_stscanf(m_strModVersion.GetBuffer(0),_T("AdunanzA %u.%ub%u"), &adumaj, &adumin, &adubeta) < 2)
							_stscanf(m_strModVersion.GetBuffer(0),_T("AdunanzA %u.%u"), &adumaj, &adumin);
			...
			case ET_SUPERADU3:
				...
				{
					tagval = 0;
					AddDebugLogLine(false, _T("SP: Il client %s:%d (%s) ha inviato un tag ET_SUPERADU3 corrotto"), 
							ipstr( m_dwUserIP ), m_nUserPort, GetUserName()
							);
					if (bDbgInfo)
						m_strMuleInfo.AppendFormat(_T("\n  ***UnkTag=%s"), temptag.GetFullInfo());
				}
				...
				// Mod Adu
				// lupz
				// il client non si è identificato. supponiamo si tratti di una 3.7
				if (!aduver)
...
//#ifdef _DEBUG || _BETA // TheKing0 - per avere + info ...
	...
	if (m_byDataCompVer == 0) {
		...
		m_byAcceptCommentVer = 0;
		m_nUDPPort = 0;
		m_incompletepartVer = 0;	//>>> Anis::ICS [enkeyDEV] //hmmm why is this reset?
	}
  ...
  //Mod Adu
  //Ded
  CString reason = IsBadModstring(m_strModVersion);
  if (!reason.IsEmpty()) {
	  CString msg;
	  msg.Format(_T("Bad modstring: User %s, version: %s, ban string: %s"), m_pszUsername, m_strModVersion, reason);
	  Ban(msg);
  }
}
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile* data)
{
	...
	// Mod Adu
 	// Emanem
 	// mando l'id corretto
 	// Se il client e' FastWeb
 	SOCKADDR_IN sockAddr = {0};
 	int nSockAddrLen = sizeof(sockAddr);
 	socket->GetPeerName((SOCKADDR*)&sockAddr, &nSockAddrLen);
 	DWORD tmpIp = sockAddr.sin_addr.S_un.S_addr;
 	DWORD tmpPort = ntohs( sockAddr.sin_port );
 	SetIP( tmpIp );
  
 	// Stefano Picerno: Gestisco i client fuori dalla rete fastweb
 	if ( HasFastwebIP() == false )
 	{
	...
	//tigerjact incremento di 1 per la modstring
	uint32 tagcount = 7;
	...
	CTag tagUdpPorts(CT_EMULE_UDPPORTS,((uint32)kadUDPPort << 16) | ((uint32)thePrefs.GetUDPPort() <<  0)); 
	...
	CTag tagMisOptions1(CT_EMULE_MISCOPTIONS1, (nAICHVer << 29) |(uUnicodeSupport << 28) |(uUdpVer << 24) |(uDataCompVer << 20) |(uSupportSecIdent << 16) |(uSourceExchange1Ver << 12) |(uExtendedRequestsVer <<  8) |(uAcceptCommentVer <<  4) |(uPeerCache <<  3) | (uNoViewSharedFiles <<  2) | (uMultiPacket <<  1) | (uSupportPreview <<  0));
	...
	// Mod Adu
	// lupz
	// opzioni separate per l'interno di fastweb

	UINT uSupportsCryptLayer;
	UINT uRequestsCryptLayer;
	UINT uRequiresCryptLayer;
	if (HasFastwebIP()) {
		uSupportsCryptLayer = (thePrefs.IsClientCryptLayerSupported() ? 1 : 0)&& (!thePrefs.m_AduNoObf); //0; tigerjatc
		uRequestsCryptLayer = thePrefs.IsAduClientCryptLayerRequested() ? 1 : 0;
		uRequiresCryptLayer	= thePrefs.IsAduClientCryptLayerRequired() ? 1 : 0;
	} else {
		uSupportsCryptLayer	= thePrefs.IsClientCryptLayerSupported() ? 1 : 0;
		uRequestsCryptLayer = thePrefs.IsClientCryptLayerRequested() ? 1 : 0;
		uRequiresCryptLayer	= thePrefs.IsClientCryptLayerRequired() ? 1 : 0;
	}
	...
	const UINT uDirectUDPCallback	= (Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsFirewalled() && !Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) && Kademlia::CUDPFirewallTester::IsVerified()) ? 1 : 0;
	const UINT uFileIdentifiers		= 1;

	CTag tagMisOptions2(CT_EMULE_MISCOPTIONS2, (uFileIdentifiers << 13) | (uDirectUDPCallback << 12) | (uSupportsCaptcha << 11) | (uSupportsSourceEx2 << 10) | (uRequiresCryptLayer	<< 9) | (uRequestsCryptLayer << 8) | (uSupportsCryptLayer << 7) | (uReserved << 6) | (uExtMultiPacket <<  5) | (uSupportLargeFiles <<  4) | (uKadVersion <<  0));
	tagMisOptions2.WriteTagToFile(data);

	// eMule Version
	CTag tagMuleVersion(CT_EMULE_VERSION, (CemuleApp::m_nVersionMjr	<< 17) | (CemuleApp::m_nVersionMin	<< 10) | (CemuleApp::m_nVersionUpd	<<  7));
	tagMuleVersion.WriteTagToFile(data);

	//tigerjact invio modstring
	CTag tagModVersion(CT_MOD_VERSION, ADU_MOD);
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	//
	if ( (m_nConnectingState != CCS_NONE && !(socket && socket->GetProxyConnectFailed())) || m_nDownloadState == DS_ERROR)
	{
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, bool bNoCallbacks, CRuntimeClass* pClassSocket)
{
	...
	// Mod Adu
	// lupz
	// opzioni separate per interno fastweb
	//if ( (RequiresCryptLayer() && !thePrefs.IsClientCryptLayerSupported()) || (thePrefs.IsClientCryptLayerRequired() && !SupportsCryptLayer()) ){
	bool incompatible = RequiresCryptLayer() && !thePrefs.IsClientCryptLayerSupported();
	if (HasFastwebIP())
		incompatible = incompatible || (thePrefs.IsAduClientCryptLayerRequired() && !SupportsCryptLayer());
	else
		incompatible = incompatible || (thePrefs.IsClientCryptLayerRequired() && !SupportsCryptLayer());
	...
	if ( HasLowID() )
	{
		...
		if (!( (SupportsDirectUDPCallback() && thePrefs.GetUDPPort() != 0 && GetConnectIP() != 0) || (HasValidBuddyID() && Kademlia::CKademlia::IsConnected()) || theApp.serverconnect->IsLocalServer(GetServerIP(), GetServerPort()) ))
		{
			// Nope
	...
	else if (HasValidBuddyID() && Kademlia::CKademlia::IsConnected())
	{
		...
		if( GetBuddyIP() && GetBuddyPort())
		{
			...
			Packet* packet = new Packet(&bio, theApp.rm->kadOpcode);
...
{
	if(m_bFriendSlot == bNV)
		return;

	m_bFriendSlot = bNV;
	if(GetUploadState() == US_CONNECTING || GetUploadState() == US_UPLOADING)
		UpdateRemainingUp();
...
void CUpDownClient::InitClientSoftwareVersion()
{
	...
	if (m_bEmuleProtocol || iHashType == SO_EMULE){
		...
		if (m_byEmuleVersion == 0){
			...
			// Mod Adu
			// lupz
			UINT nClientMajVersion = m_nClientVersion / (100U * 10U * 100U);
			UINT nClientMinVersion = m_nClientVersion / (100U * 10U) - (nClientMajVersion * 100U);
			UINT nClientUpVersion = m_nClientVersion / 100U - (nClientMajVersion * 100U * 10U) - (nClientMinVersion * 10U) ;
			// Fine Mod Adu
		...
		DWORD aduResult = GetClientAduType();

		if (aduResult == ADUNANZA_ICON_FW )
		{
			m_strClientSoftware += _T(" - Fastweb");
		}
		else if (aduResult == ADUNANZA_ICON_ADU )
		{
		// Mod Adu
		// lupz
		// scriviamo che versione usano
			if (aduver & 0x7f)
				m_strModVersion.Format(_T("v%u.%ub%u"), aduver >> 12, (aduver >> 7) & 0x1f, aduver & 0x7f);
			else
				m_strModVersion.Format(_T("v%u.%u"), aduver >> 12, (aduver >> 7) & 0x1f);

			m_strClientSoftware += _T(" - AdunanzA ") + m_strModVersion;
		// Fine Mod Adu
		}
		// Fine
		return;
...
CString CUpDownClient::DbgGetClientInfo(bool bFormatIP) const
{
	...
	if (this != NULL)
	{
		try{
			...
			{
				if (GetConnectIP())
					str.Format(_T("%u@%s (%s) '%s' (%s,%s/%s/%s)"), GetUserIDHybrid(), ipstr(GetServerIP()), ipstr(GetConnectIP()), GetUserName(), DbgGetFullClientSoftVer(), DbgGetDownloadState(), DbgGetUploadState(), DbgGetKadState());
				else
					str.Format(_T("%u@%s '%s' (%s,%s/%s/%s)"), GetUserIDHybrid(), ipstr(GetServerIP()), GetUserName(), DbgGetFullClientSoftVer(), DbgGetDownloadState(), DbgGetUploadState(), DbgGetKadState());
			}
			else
				str.Format(bFormatIP ? _T("%-15s '%s' (%s,%s/%s/%s)") : _T("%s '%s' (%s,%s/%s/%s)"), ipstr(GetConnectIP()), GetUserName(), DbgGetFullClientSoftVer(), DbgGetDownloadState(), DbgGetUploadState(), DbgGetKadState());
		}
		catch(...){
...
bool CUpDownClient::ShouldReceiveCryptUDPPackets() const {
	...
	// lupz
	// opzioni separate per l'interno di fastweb
	AddDebugLogLine(DLP_LOW, false, _T("IP %s , Fastweb? %d, Crypt? %d, IsAduClientCryptLayerRequested? %d, RequestsCryptLayer()? %d"), ipstr(m_dwUserIP), HasFastwebIP(), thePrefs.IsClientCryptLayerSupported() && SupportsCryptLayer() && theApp.GetPublicIP() != 0 && HasValidHash() && (thePrefs.IsAduClientCryptLayerRequested() || RequestsCryptLayer()), thePrefs.IsAduClientCryptLayerRequested(), RequestsCryptLayer());
	
	if (HasFastwebIP())
		return (thePrefs.IsClientCryptLayerSupported() && SupportsCryptLayer() && theApp.GetPublicIP() != 0 && HasValidHash() && (thePrefs.IsAduClientCryptLayerRequested() || RequestsCryptLayer()) );
	else
		return (thePrefs.IsClientCryptLayerSupported() && SupportsCryptLayer() && theApp.GetPublicIP() != 0 && HasValidHash() && (thePrefs.IsClientCryptLayerRequested() || RequestsCryptLayer()) );
	// Fine Mod Adu
}
...
//Mod Adu
//Ded
//Ban modstring remote
CString CUpDownClient::IsBadModstring(CString modver) {
	CString check;
	for (POSITION pos = theApp.rm->bMods.GetHeadPosition(); pos != NULL; ) {
		check = theApp.rm->bMods.GetNext(pos);
		if (modver.Find(check) != -1)
			return check;
	}
	return _T("");
}
