...
void CClientCreditsList::SaveList() //Anis modified for adu
{
	...
	while (pos)
	{
		...
		{
			if (!AduIsFastWebIP(cur_credit->m_dwIdentIP)) { //Anis -> memorizzo i crediti solo se i client non sono adu
...
// Anis metto nel pattune la vecchia funzione di emule rendendola compatibile con i muli ufficiali.

const float CClientCredits::GetScoreRatio(const CUpDownClient* client) const // Anis -> e ne metto una più golosa xD
{
	uint32 dwForIP=client->GetIP();
	if (AduIsFastWebIP(dwForIP)) {
	...
	if((GetCurrentIdentState(dwForIP) == IS_IDFAILED || GetCurrentIdentState(dwForIP) == IS_IDBADGUY || GetCurrentIdentState(dwForIP) == IS_IDNEEDED) && 
		theApp.clientcredits->CryptoAvailable() == true){
			...
			//return 1.0f;
			return 0.8f; // 80% for non SUI-clients.. (and also bad guys)
		}
...
const float CClientCredits::GetBonusAdu(const CUpDownClient* client) const
{
	uint32 dwForIP=client->GetIP();
	if (AduIsFastWebIP(dwForIP)) {
	#define PENALTY_UPSIZE 8388608 //8 MB
	...
	if((GetCurrentIdentState(dwForIP) == IS_IDFAILED || GetCurrentIdentState(dwForIP) == IS_IDBADGUY || GetCurrentIdentState(dwForIP) == IS_IDNEEDED) && 
		theApp.clientcredits->CryptoAvailable() == true){
			// bad guy - no credits for you
			//return 1.0f;
			return m_bonusfaktor; // 80% for non SUI-clients.. (and also bad guys)
		}
		...
		// Factor 1
		float result = (uploadTotal == 0) ? 10.0f : (float)(2*downloadTotal)/(float)uploadTotal;
...
const float CClientCredits::GetMioScoreAdu(uint32 dwForIP) const
{
	if (AduIsFastWebIP(dwForIP)) {
	// check the client ident status
	if ( ( GetCurrentIdentState(dwForIP) == IS_IDFAILED || GetCurrentIdentState(dwForIP) == IS_IDBADGUY || GetCurrentIdentState(dwForIP) == IS_IDNEEDED) && theApp.clientcredits->CryptoAvailable() ){
		// Anis -> scartiamo la gente marcia.
		return 1.0f;
	}
...
// fine gestione crediti AdunanzA
