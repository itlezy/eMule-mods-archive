...
public:
	...
	~CClientCredits();
	// Anis -> SISTEMA CREDITI ADUNANZA
	const float	GetScoreRatio(const CUpDownClient* client) const;
	const float	GetMioScoreAdu(uint32 dwForIP) const;
	const float	GetBonusAdu(const CUpDownClient* client)	const;
	// Anis 
	// Fine Mod Adu
