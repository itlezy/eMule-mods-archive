...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		if (client->HasFastwebIP())
			ireq = thePrefs.IsAduClientCryptLayerRequested();
		else
			ireq = thePrefs.IsClientCryptLayerRequested();

		if (client->SupportsCryptLayer() && thePrefs.IsClientCryptLayerSupported() && (client->RequestsCryptLayer() || ireq) 
		// Fine Mod Adu
		...
		if(client->HasFastwebIP())
			...
			buffer.Format(_T("%s"),(client->HasLowID() ? _T("ID Basso"):GetResString(IDS_IDHIGH)));
#endif
		// Fine mod Adu
		...
		if (client->Credits()){
			...
			buffer.Format(_T("%.1f %+.1f [%.1f]"),(float)client->Credits()->GetScoreRatio(client)- (float)client->Credits()->GetBonusAdu(client),(float)client->Credits()->GetBonusAdu(client),(float)client->Credits()->GetMioScoreAdu(client->GetIP()));	// Anis  crediti
