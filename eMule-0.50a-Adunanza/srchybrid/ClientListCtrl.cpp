...
void CClientListCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("ClientAdunanzA"))); //9
	m_ImageList.Add(CTempIconLoader(_T("ClientFastWeb"))); //10
	// fine mod Adu
...
void CClientListCtrl::AddClient(const CUpDownClient *client)
{
	...
	theApp.emuledlg->transferwnd->UpdateListCount(wnd2Clients);
}
