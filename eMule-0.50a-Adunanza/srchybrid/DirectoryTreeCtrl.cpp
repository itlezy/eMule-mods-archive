...
#define TM_FOUNDNETWORKDRIVE		(WM_USER + 0x101)	// Anis: shareSubdir - Multi-threading
...
BEGIN_MESSAGE_MAP(CDirectoryTreeCtrl, CTreeCtrl)
	...
	ON_WM_CHAR()
	ON_WM_DESTROY()
	ON_MESSAGE(TM_FOUNDNETWORKDRIVE, OnFoundNetworkDrive)	// Anis: shareSubdir - Multi-threading
END_MESSAGE_MAP()
...
void CDirectoryTreeCtrl::ShareSubDirTree(HTREEITEM hItem, BOOL bRecurse)
{
	...
	*/
	CheckChanged(hItem, !GetCheck(hItem), bRecurse==TRUE);	// Anis: shareSubdir - Handled in a much cleaner fashion
...
void CDirectoryTreeCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_SPACE)
	{
		...
		if (hItem)
		{
			...
			if (!IsDisabled(hItem))	// Anis: shareSubdir
...
void CDirectoryTreeCtrl::Init(void)
{
	...
	// Anis START: shareSubdir - local drives before network drives
	TVSORTCB tvs;
...
HTREEITEM CDirectoryTreeCtrl::AddChildItem(HTREEITEM hRoot, CString strText)
{
	...
	// Anis START: shareSubdir - always include lParam for sort
	if (hRoot == NULL) {
	...
	if (wWinVer != _WINVER_95_ && wWinVer != _WINVER_NT4_)
	{
		...
		else
		{
			...
			if (hRoot == NULL && shFinfo.szDisplayName[0] != _T('\0'))
			{
				...
				itInsert.item.lParam = (LPARAM)pti;
				*/
				// Anis START: shareSubdir remove - lParam already present
				strText = shFinfo.szDisplayName;
				strText.Trim();	// Anis: shareSubdir - clean up whitespaces
				itInsert.item.pszText = strText.GetBuffer();
				itInsert.item.cchTextMax = strText.GetLength();
				// Anis END: shareSubdir remove - lParam already present
			}
		}
...
// Anis END : shareSubdir - allow checking indirect share
{
	if (strDir.Right(1) != _T('\\'))
		strDir += _T('\\');
	for (POSITION pos = m_lstShared.GetHeadPosition(); pos != NULL; )
	...
	// Anis: shareSubdir - check second list
	strDir.MakeLower();
	for (POSITION pos = m_lstSharedSubdir.GetHeadPosition(); pos != NULL; )
	{
		CString str = m_lstSharedSubdir.GetNext(pos);
...
void CDirectoryTreeCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	// Anis START: shareSubdir - add second list
...
// Anis START: shareSubdir - Multi-threading
