...
void CUpDownClient::DrawStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	s_StatusBar.Fill(crNeither);
		...
		&& (m_abyPartStatus 
			|| m_abyIncPartStatus	//>>> Anis::ICS [enkeyDEV]
			|| m_abySeenPartStatus	//>>> Anis::AntiHideOS [netfinity]
			)
		) 
	{
		COLORREF crBoth;
		...
		COLORREF crClientPartial = RGB(170, 50, 224); //>>> Anis::ICS [enkeyDEV]
		COLORREF crClientSeen = RGB(150, 240, 240); //>>> Anis::AntiHideOS [netfinity]
		if (g_bLowColorDesktop) {
			crBoth = RGB(0, 0, 0);
...
//>>> Anis::Optimization
	if ((GetIP()!=0 && GetIP() == tocomp->GetIP())
		...
		|| (GetConnectIP()!=0 && GetConnectIP() == tocomp->GetConnectIP())) //Anis: fallback for "fresh" clients
	{
		//The IP of both match
		if (GetUserPort()!=0 && GetUserPort() == tocomp->GetUserPort())
...
void CUpDownClient::ProcessFileStatus(bool bUdpPacket, CSafeMemFile* data, CPartFile* file)
{
	...
	else
	{
		...
		uint16 owned = 0; //>>> Anis: Anti HideOS
		const bool checkSeenParts = IsPartialSource(); //>>> Anis::AntiHideOS [netfinity]
		...
		m_bCompleteSource = (owned == m_nPartCount); //>>> Anis: Anti HideOS
	}
...
void CUpDownClient::ProcessHashSet(const uchar* packet, uint32 size, bool bFileIdentifiers)
{
	...
	if (bFileIdentifiers)
	{
		...
		bool bPerformFirstHash = true;		// Anis: SafeHash - Rehash
		if (m_fHashsetRequestingAICH && !bAICH)
		{
			...
			reqfile->SetAICHHashSetNeeded(true);
			bPerformFirstHash = false;		// Anis: SafeHash - Rehash
		}
		...
		m_fHashsetRequestingMD4 = 0;
		m_fHashsetRequestingAICH = 0;
		if(bPerformFirstHash) reqfile->PerformFirstHash();		// Anis: SafeHash - Rehash
	}
	else
	{
		if (!m_fHashsetRequestingMD4)
		...
		reqfile->PerformFirstHash();		// Anis: SafeHash - Rehash
	}
	SendStartupLoadReq();
}

//dazzle : [CreateBlockRequests] : CUpDownClient :
void CUpDownClient::CreateBlockRequests(int iMaxBlocks)
{
	...
	if (m_DownloadBlocks_list.IsEmpty())
	{
        ...
        if(iMaxBlocks > m_PendingBlocks_list.GetCount()) {
            countinit = (uint16)(iMaxBlocks - m_PendingBlocks_list.GetCount());
        } else {
            countinit = 0;
        }
		count=countinit;
...
//dazzle : [CreateBlockRequests] : CUpDownClient : if no blocks available, try to drop the slowest source, then try again

		else {
		extern bool AduStream;
		if(AduStream) { // Anis -> Fonti lente non vanno bene nello streaming
...
void CUpDownClient::ProcessBlockPacket(const uchar *packet, uint32 size, bool packed, bool bI64Offsets)
{
	...
	if (IsAduClient())
		theStats.stat_Adu_sessionReceivedBytes += uTransferredFileDataSize;
	// Fine mod Adu
...
void CUpDownClient::UDPReaskForDownload()
{
	...
	if (GetUDPPort() != 0 && GetUDPVersion() != 0 && thePrefs.GetUDPPort() != 0 &&
		!theApp.IsFirewalledClient(this) && !(socket && socket->IsConnected()) && !thePrefs.GetProxySettings().UseProxy)
	{ 
		if( !HasLowID() )
		{
...
bool CUpDownClient::DoSwap(CPartFile* SwapTo, bool bRemoveCompletely, LPCTSTR reason, bool bHandleClientReq) // NEO: MCM - [ManualClientManagement] <-- Xanatos --
{
	...
	POSITION adupos = reqfile->adusrclist.Find(this);
	if(adupos)
    	reqfile->adusrclist.RemoveAt(adupos);
	// Fine mod Adu
	...
	if (IsAduClient())
		SwapTo->adusrclist.AddTail(this);
	// Fine mod Adu
...
uint32 CUpDownClient::GetTimeUntilReask(const CPartFile* file, const bool allowShortReaskTime, const bool useGivenNNP, const bool givenNNP) const {
    ...
    if(lastAskedTimeTick != 0) {
        ...
        // Stefano Picerno: Il valore ritornato da questo metodo dipende da FILEREASKTIME per gli esterni,
        //                  ADU_FILEREASKTIME per i client Adu.
        DWORD dwReAskTime;
        if ( IsAduClient() )
          dwReAskTime = ADU_FILEREASKTIME;
        else
          dwReAskTime = FILEREASKTIME;
        ...
        if(allowShortReaskTime || file == reqfile && GetDownloadState() == DS_NONE) {
            ...
            reaskTime = dwReAskTime *2;
        } else {
            reaskTime = dwReAskTime;
        }
