...
void CDownloadClientsCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("ClientAdunanzA"))); // 18
	m_ImageList.Add(CTempIconLoader(_T("ClientFastWeb")));  // 19
	//Anis -> Stefano Picerno, potevi almeno fare la lista ordinata?!
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				switch (iColumn)
				{
					case 0:{
						...
						if(client->IsObfuscatedConnectionEstablished() || (!(client->socket != NULL && client->socket->IsConnected()) && (client->SupportsCryptLayer() && thePrefs.IsClientCryptLayerSupported() && (client->RequestsCryptLayer() || thePrefs.IsClientCryptLayerRequested()))))
							nOverlayImage |= 2;
...
void CDownloadClientsCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient()) ? MF_ENABLED : MF_GRAYED),MP_STOP_CLIENT, GetResString(IDS_X_STOP_CLIENT), _T("CANCEL")); // NEO: MCM - [ManualClientManagement]
	ClientMenu.AppendMenu(MF_SEPARATOR);
...
void CDownloadClientsCtrl::AddClient(const CUpDownClient *client)
{
	...
	theApp.emuledlg->transferwnd->UpdateListCount(wnd2Downloading); 
}
