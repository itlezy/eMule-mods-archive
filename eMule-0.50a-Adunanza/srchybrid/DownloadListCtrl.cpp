...
#include "AdunanzA.h"
#include "DAMessageBox.h" //Anis -> Streaming

//Anis -> Revisione in data 19/10/2011
//Anis -> File principale per implementazione dello streaming.
//Anis -> Per lo streaming vedere anche PartFile.cpp
...
void DoEvents()	//Anis - Alta qualità!
{
    MSG msg;
    BOOL result;
...
void CDownloadListCtrl::Init()
{
	...
	InsertColumn(14, GetResString(IDS_SHOW_DROPPED_SRC) ,LVCFMT_LEFT, 60);
...
void CDownloadListCtrl::SetAllIcons()
{
	...
	// Aggiungo l'icona Adunanza
 	m_ImageList.Add(CTempIconLoader(_T("ClientAdunanzA"))); //21
 	m_ImageList.Add(CTempIconLoader(_T("ClientFastWeb"))); //22
 	// fine mod Adu
...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		...
		case 5: {	// progress
			...
			if (thePrefs.GetUseDwlPercentage()) //Anis
			{
...
void CDownloadListCtrl::DrawSourceItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem) {
	if(lpRect->left < lpRect->right) { 
		...
		CString buffer;
		CUpDownClient *lpUpDownClient = (CUpDownClient*)pCtrlItem->value;
		switch(nColumn) {
		...
		case 0:		// icon, name, status
			{
				...
				// Modifiche Adunanza
				DWORD aduType = lpUpDownClient->GetClientAduType();
		...
		case 3:		// completed
			...
			if (nColumn == 2 || IsColumnHidden(2)) {
				if (pCtrlItem->type == AVAILABLE_SOURCE && lpUpDownClient->GetTransferredDown()) {
		...
		case 4:		// speed
			if (pCtrlItem->type == AVAILABLE_SOURCE && lpUpDownClient->GetDownloadDatarate()){
		...
		case 5:		// file info
			{
				...
				if(pCtrlItem->dwUpdated + DLC_BARUPDATE < dwTicks || cx !=  iWidth  || !pCtrlItem->dwUpdated) { 
					...
					rec_status.bottom = iHeight; 
					rec_status.right = iWidth; 
					lpUpDownClient->DrawStatusBar(&cdcStatus,  &rec_status,(pCtrlItem->type == UNAVAILABLE_SOURCE), thePrefs.UseFlatBar()); 
...
// ZZ:DownloadManager -->
                if(thePrefs.IsExtControlsEnabled()) {
                    if(lpUpDownClient->IsInNoNeededList(pCtrlItem->owner)) {
...
void CDownloadListCtrl::ControllaStreaming() { // Anis -> Implementazione Streaming
	...
	while (!selectedList.IsEmpty()){
		...
		if (pPartFile->IsReadyForPreview() && !AduStream)
			StreamOK = true; //Streaming OK!
	}
}
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			m_FileMenu.EnableMenuItem(MP_STREAMING, (iSelectedItems == 1 && AduStream == 1 && file->IsMovie() && !file->GetCompletionError() && file->GetPercentCompleted() < 100) ? MF_ENABLED : MF_GRAYED); //Anis -> Implementazione Streaming
			m_FileMenu.EnableMenuItem(MP_STARTSTREAM, (iSelectedItems == 1 && StreamOK && file->IsMovie() && !file->IsStopped() && file->IsReadyForPreview() && file->GetAduSourceCount() > 0 && file->GetPercentCompleted() > thePrefs.m_BufferStreaming) ? MF_ENABLED : MF_GRAYED); //Anis -> Implementazione Streaming
			m_FileMenu.EnableMenuItem(MP_STOPSTREAM, (iSelectedItems == 1 && StreamSTOP && file->IsMovie() && !file->IsStopped()) ? MF_ENABLED : MF_GRAYED); //Anis -> Implementazione Streaming
			m_FileMenu.EnableMenuItem(MP_INFO_STREAM, (iSelectedItems == 1) ? MF_ENABLED : MF_GRAYED); //Anis -> info
			if(!AduStream) {
				if(file->GetPercentCompleted() == 100)
				m_FileMenu.EnableMenuItem(MP_STOPSTREAM, MF_ENABLED); //Anis -> Implementazione Streaming
			}
			...
			if(thePrefs.IsExtControlsEnabled())
			{
				...
				m_PreviewMenu.CheckMenuItem(MP_PAUSEONPREVIEW, MF_UNCHECKED);
			...
			if (thePrefs.IsExtControlsEnabled()) {
				A4AFMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && content->type == UNAVAILABLE_SOURCE) ? MF_ENABLED : MF_GRAYED),MP_SWAP_TO_CLIENT,GetResString(IDS_X_SWAP_TO_CLIENT), _T("SWAPTO"));
				A4AFMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && content->type == AVAILABLE_SOURCE) ? MF_ENABLED : MF_GRAYED),MP_SWAP_FROM_CLIENT,GetResString(IDS_X_SWAP_FROM_CLIENT), _T("SWAPFROM"));
				A4AFMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && (content->type == AVAILABLE_SOURCE || client->IsSwapingDisabled()) ) ? MF_ENABLED : MF_GRAYED),MP_LOCK_CLIENT,GetResString((client && client->IsSwapingDisabled()) ? (content->owner == client->GetRequestFile() ? IDS_X_MP_UNLOCK_CLIENT : IDS_X_MP_UNLOCKFROM_CLIENT) : IDS_X_MP_LOCK_CLIENT), (client && client->IsSwapingDisabled()) ? (content->owner == client->GetRequestFile() ? _T("UNLOCK") : _T("UNLOCKFROM")) : _T("LOCKTO"));
				A4AFMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient()) ? MF_ENABLED : MF_GRAYED),MP_DROP_CLIENT, GetResString(IDS_X_DROP_CLIENT), _T("DROPCLIENT"));
				if(client && client->GetDownloadState() == DS_DOWNLOADING)
					A4AFMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient()) ? MF_ENABLED : MF_GRAYED),MP_STOP_CLIENT, GetResString(IDS_X_STOP_CLIENT), _T("CANCEL"));
		...
		m_FileMenu.EnableMenuItem(MP_STARTSTREAM, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_STOPSTREAM, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_STREAMING, MF_GRAYED); //Anis -> Implementazione streaming
		m_FileMenu.EnableMenuItem(MP_INFO_STREAM, MF_GRAYED); //Anis -> Info
...
void CDownloadListCtrl::StoppaRiprendi(bool s_r) { //Anis MOD ADU (passo 0 per stoppare, 1 per riprendere i download)

		if (s_r)
			theApp.downloadqueue->SetCatStatus(-1,MP_STOP);
		else
			theApp.downloadqueue->SetCatStatus(-1,MP_RESUME);

}

void CDownloadListCtrl::StopStreaming(CPartFile* file) //Anis Hireche's function 
{
		// Invoking the Dialog
		float totFile = theApp.downloadqueue->filelist.GetCount();
		float conteggio = 0;
		CString result;
		AduWait m_pDialog (theApp.emuledlg);
		m_pDialog.SetText(L"AdunanzA P2P Streaming v.1.0");
		Sleep(10);
		m_pDialog.Create(IDD_ADU_WAITING);
		BeginWaitCursor();
		m_pDialog.ShowWindow(SW_SHOW);
		//Anis
		SetRedraw(false);

		theApp.downloadqueue->SetCatStatus(-1,MP_RESUME); //Anis -> Riprendo tutti i download

		file->SetAutoUpPriority(true);
		file->SetAutoDownPriority(true);
		file->SetPreviewPrio(false);

		for (POSITION pos = theApp.downloadqueue->filelist.GetHeadPosition();pos != NULL;) //Anis -> list cicle
		{
			CPartFile* cur_file = theApp.downloadqueue->filelist.GetAt(pos);
			//Anis -> list cicle (non riprendo file che erano precedentemente in stop)
			if(cur_file->isStreamingStopped == true)
			{
				if (cur_file->CanStopFile())
				{
					HideSources(cur_file);
					cur_file->StopFile();
				}
			}
			else if(cur_file->isStreamingPaused == true)
			{
				//Anis -> list cicle (non riprendo file che erano precedentemente in pausa)
				if (cur_file->CanPauseFile())
				{	
					cur_file->PauseFile();
				}
			}
			theApp.downloadqueue->filelist.GetNext(pos);
			result.Format(_T("Disattivazione AduStreaming: %.2f%%"), ((conteggio / totFile) *100));
			m_pDialog.SetText(result);
			conteggio++;
			DoEvents();
			Sleep(10);
		}
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			switch (wParam)
			{
				...
				case MP_INFO_STREAM: 
				{
						...
						adu_controllo = true;
						CString msg;
						msg.Format(_T("Si avvisa che al fine di velocizzare i tempi di inizio dello streaming, tutti gli altri eventuali download verranno stoppati. Non appena lo streaming sarà disponibile, verrà abilitata la casella del menu contestuale: 'Start Streaming', accessibile cliccando il tasto destro del mouse nel file interessato o selezionandolo e usufruire della nuova barra degli strumenti.\n\nSi ricorda anche che non è concesso lo streaming parallelo, ma esclusivamente di un file alla volta. Selezionare quindi l'opzione dal menu contestuale: 'Stop Streaming' prima di poter abilitare lo streaming su un'altro file usando lo stesso metodo precedentemente descritto. Raccomandiamo inoltre di non occupare banda con altri software P2P.\n\nNon è possibile usare tale funzione in dati diversi dal formato video. Prestazioni maggiori si otterranno attivando lo streaming su file con il numero di fonti più alto possibile. Tutti i download precedentemente stoppati verranno riabilitati automaticamente non appena lo streaming sarà disabilitato o al prossimo riavvio di eMule AdunanzA. La fluidità della visualizzazione non è affatto garantita in quanto dipende dalla disponibilità delle parti del file in questione nonchè dal tipo di connessione utilizzata attualmente dal computer."));
						CDAMessageBox info(NULL,msg,false,true);
						info.DoModal();
						adu_controllo = false;
						break;
				}
				...
				{
					//Anis
					//Lancio vlc aprendo file .part
					//Codice per la concatenazione delle parti su partfile.cpp
					if(file->GetCompletionError() || file->GetPercentCompleted() == 100) {
						AfxMessageBox(_T("Errore durante l'attivazione dello streaming."));
						break;
					}
				...
				case MP_STOPSTREAM: 
				{ 
					StopStreaming(file);  //Anis
					break;
				}

				case MP_STREAMING: 
				{ //Anis -> Implementazione  AduStreaming
					if(file->GetPercentCompleted() > thePrefs.m_BufferStreaming)
					{
						CString text; 
						text.Format(_T("Impossibile attivare lo streaming in un file scaricato per più del %u%% in modalità standard.\nLa percentuale di tolleranza dipende dall'impostazione relativa modificabile dalle opzioni."), thePrefs.m_BufferStreaming);
						CDAMessageBox info(NULL, text, false, false);
						info.DoModal();
						break;
					}

					extern bool NoPlayer;
					if(NoPlayer) 
					{
						CDAMessageBox info(NULL,
						_T("Non è stato rilevato nessun lettore multimediale compatibile con questa funzione.\nAl fine di garantire la migliore esperienza, AduTeam consiglia VLC per un ottimale riproduzione dello streaming.\n\n")
						_T("Se compare questo messaggio di errore pur avendo VLC installato, si ricorda di settarlo in: 'Opzioni->File' cliccando sul tasto 'Rileva VLC Automaticamente' o impostandolo in modo manuale."),
						 false,false);
						info.DoModal();
						break;
					}
					...
					float totFile = (theApp.downloadqueue->filelist.GetCount()) * 2;
					float conteggio = 0;
					CString result;
					// Invoking the Dialog
					AduWait m_pDialog (theApp.emuledlg);
					...
					CPartFile* partfile = selectedList.GetHead();
					CPartFile* pPartFile = selectedList.RemoveHead();
					file->SetPreviewPrio(true);
					partfile->SetAutoDownPriority(false);
					partfile->SetAutoUpPriority(false); // Anis
					if(pPartFile->IsPausingOnPreview())
						pPartFile->SetPauseOnPreview(false);
					partfile->SetDownPriority(PR_VERYHIGH);
					partfile->SetUpPriority(PR_VERYHIGH);
					//Anis -> sfiziosità durata 3 ore di scrittura. 7 righe..... =.=
					//Peccato che non posso usarla perchè incompatibile con l'algoritmo di countdown di inizio streaming...
					
					//haha, ora è stata usata per memorizzare i file stoppati dall'utente. Il codice non si butta mai... :P
					for (POSITION pos = theApp.downloadqueue->filelist.GetHeadPosition();pos != NULL;) //Anis -> list cicle
					{
						CPartFile* cur_file = theApp.downloadqueue->filelist.GetAt(pos);
						cur_file->isStreamingStopped = false;
						cur_file->isStreamingPaused = false;
						theApp.downloadqueue->filelist.GetNext(pos);
						result.Format(_T("Attivazione AduStreaming: %.2f%%"), ((conteggio / totFile) *100));
						m_pDialog.SetText(result);
						conteggio++;
						DoEvents();
						Sleep(10);
					...
					for (POSITION pos = theApp.downloadqueue->filelist.GetHeadPosition();pos != NULL;) //Anis -> list cicle
					{
						...
						if (cur_file->IsStopped())
							cur_file->isStreamingStopped = true;
						
						else if (cur_file->IsPaused())
							cur_file->isStreamingPaused = true;
...
*/
				...
				case MP_SWAP_TO_CLIENT:
					if (file->GetStatus(false) == PS_READY || file->GetStatus(false) == PS_EMPTY)
					{
						if (client->GetDownloadState() != DS_DOWNLOADING)
						{
							client->DoSwap(file, false, _T("Manual: Swap to this File"),true); // ZZ:DownloadManager
							UpdateItem(file);
						}
					}
					break;
				case MP_SWAP_FROM_CLIENT:
					if (file->GetStatus(false) == PS_READY || file->GetStatus(false) == PS_EMPTY)
					{
						if (client->GetDownloadState() != DS_DOWNLOADING)
						{
							client->SwapToAnotherFile(_T("Manual: Swap to an Other File"), false, true, false, NULL, true, true, true); // ZZ:DownloadManager
							UpdateItem(file);
						}
					}
					break;
				case MP_LOCK_CLIENT:
					if (file->GetStatus(false) == PS_READY || file->GetStatus(false) == PS_EMPTY)
					{
						client->DisableSwaping(!client->IsSwapingDisabled());
						UpdateItem(file);
					}
					break;
				case MP_DROP_CLIENT:
...
int CDownloadListCtrl::Compare(const CPartFile *file1, const CPartFile *file2, LPARAM lParamSort)
{
	...
	switch (lParamSort)
	{
		...
		case 6: //sources asc
			comp=CompareUnsigned(file1->GetAduSourceCount(), file2->GetAduSourceCount());
			break;
...
int CDownloadListCtrl::Compare(const CUpDownClient *client1, const CUpDownClient *client2, LPARAM lParamSort, int sortMod)
{
	// Mod Adu
	// Emanem
	// Comunque un client Adu viene prima di un client non Adu
	// Ded: Modificata per farsì che in entrambi i sensi di ordinamento gli adu vengano sempre prima!
	if (client1->IsAduClient() && !client2->IsAduClient())
		return (-1*sortMod);
	if (!client1->IsAduClient() && client2->IsAduClient())
		return sortMod;
	// fine mod Adu
...
void CDownloadListCtrl::CreateMenues()
{
	...
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	m_FileMenu.AppendMenu(MF_STRING, MP_STREAMING, _T("AdunanzA Streaming Request"), _T("VLCICON")); //Anis -> implementazione streaming.
	m_FileMenu.AppendMenu(MF_STRING, MP_STARTSTREAM, _T("Start Streaming"), _T("DOWNLOAD")); //Anis -> implementazione streaming.
	m_FileMenu.AppendMenu(MF_STRING, MP_STOPSTREAM, _T("Stop Streaming"), _T("UPLOAD")); //Anis -> implementazione streaming.
	m_FileMenu.AppendMenu(MF_STRING, MP_INFO_STREAM, _T("Guida Streaming"), _T("INFO")); //Anis -> implementazione streaming.
	m_FileMenu.AppendMenu(MF_SEPARATOR);
...
bool CDownloadListCtrl::ReportAvailableCommands(CList<int>& liAvailableCommands)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			CPartFile* file = (CPartFile*)content->value; //Anis
			// enable commands if there is at least one item which can be used for the action
			if (iFilesToCancel > 0)
			...
			if (iSelectedItems == 1 && AduStream == 1 && file->IsMovie() && !file->GetCompletionError() && file->GetPercentCompleted() < 100)
				liAvailableCommands.AddTail(MP_STREAMING);
			if (iSelectedItems == 1 && StreamOK == 1 && file->IsMovie() && !file->IsStopped() && file->IsReadyForPreview() && file->GetAduSourceCount() > 0 && file->GetPercentCompleted() > thePrefs.m_BufferStreaming)
				liAvailableCommands.AddTail(MP_STARTSTREAM);
			if (iSelectedItems == 1 && StreamSTOP == 1 && file->IsMovie() && !file->IsStopped())
				liAvailableCommands.AddTail(MP_STOPSTREAM);
			...
			if (iSelectedItems > 0)
			{
				liAvailableCommands.AddTail(MP_INFO_STREAM);
