...
#include "Log.h"
#include "PartFile.h"
#include "KnownFileList.h" //>>> Anis::Indicate already downloaded files
...
for (int i=0;i<thePrefs.tempdir.GetCount();i++) {
    	CStringList metsfound;	// Anis: SafeHash - ensure each met is loaded once per tempdir
		...
		// Anis: SafeHash
			...
			if (eResult == PLR_LOADSUCCESS)
			{
				...
				// Anis: SafeHash remove - part files are shared later
				/*
...
//>>> Anis::Indicate already downloaded files
		//AddDownload(newfile, (paused==1));
		if(!AddDownload(newfile, paused == 1))
		{
			delete newfile;
			newfile = NULL;
		}
...
//>>> Anis::Indicate already downloaded files
	...
	if ((newfile = GetFileByID(toadd->GetFileHash())) == NULL)
//### Anis::Indicate already downloaded files
		return;
/*
...
//		AddDownload(newfile,thePrefs.AddNewFilesPaused());
		if(!AddDownload(newfile, thePrefs.AddNewFilesPaused()))
		{
			delete newfile;
			newfile = NULL;
		}
//### Anis::Indicate already downloaded files
	...
	//Anis: only add host sources to existing downloads... useless otherwise ;)
//	} // Moved down
...
bool CDownloadQueue::AddDownload(CPartFile* newfile, bool paused) {
	...
	if(GetFileByID(newfile->GetFileHash()) != NULL)
		return false; //no double adds... can this actually happen?

	//maybe also add a switch to the prefs to allow the user to skip this check?
	CKnownFile* file = theApp.knownfiles->FindKnownFileByID(newfile->GetFileHash());
	if(file)
	{
		...
		CString msg;
		msg.Format(GetResString(IDS_ALREADYDOWNLOADEDWARNING), newfile->GetFileName(), file->GetFileName(), file->GetUtcCFileDate().Format(L"%x %X"));
		if(AfxMessageBox(msg, MB_YESNO | MB_DEFBUTTON2) == IDNO)
			return false;
	}
	...
	SetAutoCat(newfile);// HoaX_69 / Anis: AutoCat
...
void CDownloadQueue::Process(){
	...
	uint32 downspeedext = 0;
	uint64 maxDownloadext = thePrefs.m_AduExtMaxDown;
	
	if ((maxDownloadext != UNLIMITED*1024) && (datarateext > 1500))
	{
		downspeedext = (UINT)((maxDownloadext*100)/(datarateext+1));
		if (downspeedext < 50)
		{
			downspeedext = 50;}
		else
	...
	uint32 downspeedkadu = 0;
	uint64 maxDownloadkadu = thePrefs.m_AduKaduMaxDown;
	
	if ((maxDownloadkadu != UNLIMITED*1024) && (dataratekadu > 1500)){
		downspeedkadu = (UINT)((maxDownloadkadu*100)/(dataratekadu+1));
		if (downspeedkadu < 50)
		{downspeedkadu = 50;}
		else{ if (downspeedkadu > 200)
			{downspeedkadu = 200;
			}
			}
	}
	...
	while(avarage_dr_listKADU.GetCount()>0 && (GetTickCount() - avarage_dr_listKADU.GetHead().timestamp > 10*1000) )
	{	m_datarateMSkadu-=avarage_dr_listKADU.RemoveHead().datalen;
	}
	
	while(avarage_dr_listEXT.GetCount()>0 && (GetTickCount() - avarage_dr_listEXT.GetHead().timestamp > 10*1000) )
	{	m_datarateMSext-=avarage_dr_listEXT.RemoveHead().datalen;
	}
	...
	if (avarage_dr_listKADU.GetCount()>1){
		dataratekadu = (UINT)(m_datarateMSkadu / avarage_dr_listKADU.GetCount());
	} else {
	dataratekadu = 0;
	}

	if (avarage_dr_listEXT.GetCount()>1){
		datarateext = (UINT)(m_datarateMSext / avarage_dr_listEXT.GetCount());
	} else {
	datarateext = 0;
	}
	
	uint32 datarateX=0;
	uint32 datarateXext=0;
	uint32 datarateXkadu=0;
	udcounter++;
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		if (cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY){
			...
			//tigerjact recupero i datarate per le reti interne ed esterne		
			takelimit parzial;
			parzial = cur_file->Process(downspeed, downspeedext, downspeedkadu, udcounter);
			datarateX += parzial.limit;
			datarateXkadu += parzial.limitkad;
			datarateXext += parzial.limitext;
	...
	TransferredData newitemext = {datarateXext , ::GetTickCount()};
	TransferredData newitemkadu = {datarateXkadu , ::GetTickCount()};
	avarage_dr_list.AddTail(newitem);
	avarage_dr_listEXT.AddTail(newitemext);
	avarage_dr_listKADU.AddTail(newitemkadu);
	m_datarateMS+=datarateX;
	m_datarateMSext+=datarateXext;
	m_datarateMSkadu+=datarateXkadu;
...
bool CDownloadQueue::CheckAndAddSource(CPartFile* sender,CUpDownClient* source){
	...
	if (source->IsAduClient())
		sender->adusrclist.AddTail(source);
	// Fine mod Adu
	return true;
}
...
bool CDownloadQueue::RemoveSource(CUpDownClient* toremove, bool bDoStatsUpdate)
{
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		// Mod Adu
		// lupz
		// rimuovo la fonte dalla lista fonti adunanza
		POSITION adupos = cur_file->adusrclist.Find(toremove);
		if (adupos)
	    	cur_file->adusrclist.RemoveAt(adupos);
		// Fine mod Adu
...
void CDownloadQueue::DeleteAll(){
	...
	for (pos = filelist.GetHeadPosition();pos != 0;){
		...
		// Mod Adu
		// lupz
		// pulisco la lista fonti adunanza
		cur_file->adusrclist.RemoveAll();
		// Fine mod Adu
...
void CDownloadQueue::SetCatStatus(UINT cat, int newstatus)
{
	...
	while (pos != 0)
	{
		...
		if (cat==-1 || (cat==-2 && cur_file->GetCategory()==0) || (cat==0 && cur_file->CheckShowItemInGivenCat(cat)) || (cat>0 && cat==cur_file->GetCategory()))
		{
			switch (newstatus){
				case MP_CANCEL:
					cur_file->DeleteFile();
	...
	for (int ix=1;ix<thePrefs.GetCatCount();ix++){	
		...
		if (!thePrefs.GetCategory(ix)->ac_regexpeval) {
			...
			while (!cmpExt.IsEmpty()) {
				...
				//  thanks to: bluecow, khaos and Anis
...
void CDownloadQueue::KademliaSearchFile(uint32 searchID, const Kademlia::CUInt128* pcontactID, const Kademlia::CUInt128* pbuddyID, uint8 type, uint32 ip, uint16 tcp, uint16 udp, uint32 dwBuddyIP, uint16 dwBuddyPort, uint8 byCryptOptions)
{
	...
	//Do we need more sources?
 	...
 	// Mod Adu
 	// Emanem/lupz
 	// Cerchiamo sorgenti su Kademlia se non abbiamo trovato sufficenti fonti adunanza
 	//if(!(!temp->IsStopped() && thePrefs.GetMaxSourcePerFile() > temp->GetSourceCount()))
 	if ( temp->IsStopped() || temp->GetAduSourceCount() > temp->GetMaxSourcePerFileSoft() )
  		return;
 	// fine mod Adu
	...
	if (type == 3)
	{
		CUpDownClient* client = theApp.clientlist->FindClientByUserHash( cID );
		CString sName, sIp	;
		int nPort=0;
		if (client)
		{
			sName = client->GetUserName();
			sIp = ipstr( client->GetUserIDHybrid() );
			nPort = client->GetKadPort();
		}
		else
		{
			sName = _T("<NotFound>");
		...
		AddDebugLogLine(false, _T("SP: Ricevuta una fonte KAdu firewalled: ip=%s:%d (%s)"), 
			sIp, nPort, 
			sName
			);
	}
...
void CDownloadQueue::RemoveSourceAndDontAsk(CUpDownClient* client, bool bUpdateStats, uint8 uDropType)
{
	if(RemoveSource(client, bUpdateStats) && !client->IsFriend())
		theApp.clientlist->AddToDontAskThisIP(client->GetIP(), uDropType);
}

//Anis -> funzione in più utilissima
