...
{
	InitLanguages(GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_INSTLANGDIR*/), GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_ADDLANGDIR*/, false));

	bool bFoundLang = false;
	if (m_wLanguageID)
//tigerjact, prendo la dll dalla config
		bFoundLang = LoadLangLib(GetMuleDirectory(EMULE_CONFIGDIR )/*EMULE_INSTLANGDIR)*/, GetMuleDirectory(EMULE_CONFIGDIR /*EMULE_ADDLANGDIR*/, false), m_wLanguageID);	
...
//tigerjact, prendo la dll dalla config
		bFoundLang = LoadLangLib(GetMuleDirectory(EMULE_CONFIGDIR )/*EMULE_INSTLANGDIR)*/, GetMuleDirectory(EMULE_CONFIGDIR /*EMULE_ADDLANGDIR*/, false), lidLocale);
		if (!bFoundLang){
			LoadLangLib(GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_INSTLANGDIR*/), GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_ADDLANGDIR*/, false), LANGID_EN_US);
...
bool CPreferences::IsLanguageSupported(LANGID lidSelected, bool bUpdateBefore){
	InitLanguages(GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_INSTLANGDIR*/), GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_ADDLANGDIR*/, false), bUpdateBefore);
	if (lidSelected == LANGID_EN_US)
		return true;
	const SLanguage* pLang = s_aLanguages;
	for (;pLang->lid;pLang++){
		if (pLang->lid == lidSelected && pLang->bSupported){
			bool bResult = CheckLangDLLVersion(GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_INSTLANGDIR*/) + CString(pLang->pszISOLocale) + _T(".dll"));
			return bResult; // || CheckLangDLLVersion(GetMuleDirectory(EMULE_CONFIGDIR/*EMULE_ADDLANGDIR*/, false) + CString(pLang->pszISOLocale) + _T(".dll"));
		}
	}
	return false; 
}
