...
void CIrcWnd::ParseChangeMode(const CString& sChannel, const CString& sChanger, CString sCommands, const CString& sParams)
{
	...
	try
	{
		if( sCommands.GetLength() >= 2 )
		{
			...
			//Mod Adu
			//Ded
			//Evitiamo che la gente tenti di "impostare le modalità +runtpl"
			{
				AddInfoMessageF( sChannel, GetResString(IDS_IRC_SETSMODE), sChanger.IsEmpty()?sChannel:sChanger, sCommandsOrig, sParamsOrig);
			}
			//Mod Adu
			//Fine
		}
