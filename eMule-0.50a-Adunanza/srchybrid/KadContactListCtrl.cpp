...
void CKadContactListCtrl::OnNmDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (iSel != -1){
		Kademlia::CContact* contact = (Kademlia::CContact*)GetItemData(iSel);

		CString id, tcpport, udpport, ipaddress, distance;

		contact->GetClientID( &id );
		contact->GetDistance( &distance );
		contact->GetIPAddress( &ipaddress );
		contact->GetTCPPort( &tcpport );
		contact->GetUDPPort( &udpport );
		byte type = contact->GetType();

		AddDebugLogLine( false, _T( "Kademlia contact details - id:%s ipaddress:%s tcpport:%s udpport:%s type:%d distance:%s" ),
			id, ipaddress, tcpport, udpport, type, distance );
	}
}
