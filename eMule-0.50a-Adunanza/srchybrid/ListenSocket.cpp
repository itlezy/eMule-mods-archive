			...
			switch(opcode)
			{
				...
				case OP_HELLO:
				{
					...
					theApp.emuledlg->transferwnd->GetClientList()->RefreshClient(client);
 					...
 					// Codice AdunanzA
 					// Emanem	17:01	6/4/2004
 					//
 					// Mod Adunanza necessaria per spedire il pacchetto di info
 					// su richiesta ad ogni connessione.
 					// Questo codice dovrebbe essere utilizzato dal mulo rispondente.
					...
					// lupz: Meglio mandare i pacchetti in ordine
 					if (client->GetHashType() == SO_EMULE && bIsMuleHello && client->HasFastwebIP())
					 		client->SendMuleInfoPacket(false);
...
//### Anis::ICS [enkeyDEV]
				...
				case OP_REQUESTPARTS:
				{
					...
					bool bFirst = true;  //>>> Anis::Optimization
...
//### Anis::ICS [enkeyDEV]
				...
				case OP_REASKCALLBACKTCP:
				{
					...
					{
						// Anis -> rimosso per coda infinita
...
bool CListenSocket::Rebind() //Anis -> uPnP support
{
