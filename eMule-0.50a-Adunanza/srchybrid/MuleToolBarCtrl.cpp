...
void CMuleToolbarCtrl::Init(void)
{
	...
	// Emanem
	// Anis -> AduBrowser
	// Aggiungo il pulsante al sito Adu
	lLen2 = _tcslen(GetResString(IDS_EM_ADUNANZA_BROWSER)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_EM_ADUNANZA_BROWSER), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
	// fine mod Adu
...
void CMuleToolbarCtrl::ChangeToolbarBitmap(const CString& path, bool bRefresh)
{
	...
	if (!path.IsEmpty() && Bitmap.LoadImage(path))
	{
		...
		if ((bm.bmWidth == NUM_BUTTON_BITMAPS*m_sizBtnBmp.cx || bm.bmWidth == (NUM_BUTTON_BITMAPS-1)*m_sizBtnBmp.cx) && bm.bmHeight == m_sizBtnBmp.cy) //Ded : supporto toolbar bitmap senza icona A3
		{
			...
			if (ImageList.Create(m_sizBtnBmp.cx, bm.bmHeight, bAlpha ? ILC_COLOR32 : (theApp.m_iDfltImageListColorFlags | ILC_MASK), 0, 1))
			{
				...
				// Ded - START - se non c'è l'icona A3 nella bitmap gli faccio aggiungere quella di default
				if (bm.bmWidth == (NUM_BUTTON_BITMAPS-1)*m_sizBtnBmp.cx)
					ImageList.Add(CTempIconLoader(_T("PULSANTEA3"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
				// Ded - END
	...
	if (!bResult)
	{
		...
		ImageList.Add(CTempIconLoader(_T("BN_KADU"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
