...
BOOL CPPgDirectories::OnInitDialog()
{
	...
	m_ctlUncPaths.InsertColumn(0, GetResString(IDS_UNCLIST_INACTIVE  ), LVCFMT_LEFT, 130);  // sharesubdir ==> this can be better
	m_ctlUncPaths.InsertColumn(1,GetResString(IDS_SUBDIRS), LVCFMT_LEFT,80); // sharesubdir + column for inactive shares
...
void CPPgDirectories::LoadSettings(void)
{
	...
	m_ShareSelector.SetSharedDirectories(&thePrefs.shareddir_list, &thePrefs.sharedsubdir_list);	// Anis: shareSubdir
	FillUncList();
}
