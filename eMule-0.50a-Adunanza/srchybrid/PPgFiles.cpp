...
BEGIN_MESSAGE_MAP(CPPgFiles, CPropertyPage)
	...
	ON_WM_HELPINFO()
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_BUTTON2, &CPPgFiles::OnBnClickedButton2)
END_MESSAGE_MAP()
...
BOOL CPPgFiles::OnApply()
{
	...
	if (thePrefs.m_strVideoPlayer != L"") NoPlayer = false;
	else NoPlayer = true;
	LoadSettings();
	SetModified(FALSE);
...
void CPPgFiles::OnBnClickedButton2()
{
	...
	if(ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, REG_VLC_DIR, 0, KEY_READ, &hKey)) {
		...
		if (thePrefs.m_strVideoPlayer == _T("")) {
			AfxMessageBox(_T("VLC media player non rilevato!")); 
			return;
		}
	...
	///////////////ANIS ADU MOD -> RILEVAZIONE AUTOMATICA VLC///////////////	
	GetDlgItem(IDC_VIDEOPLAYER)->SetWindowTextW(thePrefs.m_strVideoPlayer);
	SetModified(false);
	switch(rilevato) {
		case false: AfxMessageBox(_T("VLC media player non rilevato!")); break;
		case true: AfxMessageBox(_T("VLC media player rilevato con successo!"),MB_ICONINFORMATION); break;
	}
}
