	...
	, m_ctrlTreeOptions(theApp.m_iDfltImageListColorFlags)
{
	...
	m_bLogUPnP = false; //Anis -> uPnP NAT
	m_htiLogUPnP = NULL;
}
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		// eMule Advertisement support.
		//        
        m_htiAdsDisable2 = m_ctrlTreeOptions.InsertCheckBox(GetResString(IDS_SPONSORDISABLE), TVI_ROOT, m_bAdsDisable2);
	...
	DDX_TreeCheck(pDX, IDC_EXT_OPTS, m_htiAdsDisable2, m_bAdsDisable2);
	if (m_htiLogLevel){
...
BOOL CPPgTweaks::OnInitDialog()
{
	...
	if (thePrefs.GetEnableVerboseOptions())
	{
		...
		m_bLogUPnP = thePrefs.GetUPnPVerboseLog(); //Anis
	}
	m_bLog2Disk = thePrefs.log2disk;
	m_bAdsDisable2 = thePrefs.adsDisable2;
...
BOOL CPPgTweaks::OnApply()
{
	...
	if (thePrefs.GetEnableVerboseOptions())
	{
		if (!thePrefs.GetDebug2Disk() && m_bVerbose)
			theVerboseLog.Open();
		else if (thePrefs.GetDebug2Disk() && (!m_bVerbose))
			theVerboseLog.Close();
	...
	thePrefs.adsDisable2 = m_bAdsDisable2; 
...
LRESULT CPPgTweaks::OnTreeOptsCtrlNotify(WPARAM wParam, LPARAM lParam)
{
	if (wParam == IDC_EXT_OPTS)
	{
		...
		if (m_htiVerbose && pton->hItem == m_htiVerbose)
		{
		  ...
		  else if (m_htiAdsDisable2 && pton->hItem == m_htiAdsDisable2)
        {
			BOOL bCheck;
			if (m_ctrlTreeOptions.GetCheckBox(m_htiAdsDisable2, bCheck) && bCheck)
            {
                CDlgAdsOption dlgAds;
                if (IDOK == dlgAds.DoModal())
                    m_ctrlTreeOptions.SetCheckBox(m_htiAdsDisable2, false);
            }
        }
		SetModified();
	}
