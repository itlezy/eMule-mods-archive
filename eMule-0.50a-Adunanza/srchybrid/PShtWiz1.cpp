...
BOOL CPPgWiz1Server::OnInitDialog()
{
	...
	// Modifica AdunanzA
	// lupz
	// Non facciamo fare casini agli utenti.. :)
	// Anis -> non funziona, fixo io.
	// Andava disabilitato dal dialog
	// Fine mod Adu
	return TRUE;
}
...
BOOL FirstTimeWizard()
{
	...
	thePrefs.m_bCryptLayerSupported = true; //Anis -> Supportato per default
	...
	thePrefs.SetNewAutoDown(1);
	thePrefs.SetNewAutoUp(1);
	thePrefs.SetTransferFullChunks(1);
	thePrefs.SetSafeServerConnectEnabled(0);
	thePrefs.SetAutoConnect(true);
	thePrefs.SetNetworkKademlia(1);
	// fine mod Adu	
	return TRUE;
}
