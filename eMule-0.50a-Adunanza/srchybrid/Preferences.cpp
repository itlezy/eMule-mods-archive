...
#include "Version.h"
#include "AdunanzA.h"
#include "XMessageBox.h" // Anis -> Miglioria per le unità rimovibili
#include "RemoteSettings.h" // Anis -> check nodes
...
bool NoPlayer = false; //Anis -> devo sapere se ho VLC installato o meno.

int		CPreferences::m_bShareOnlyTheNeed; //>>> Anis::Intelligent SOTN
uint16	CPreferences::m_uiSmallFileSlotSize; // Anis -> + spinta in upload alle parti piccolissime
bool	CPreferences::m_bUseSaveLoadSources; // Salviamo fonti su file
 //Anis -> uPnP
...
uint8	CPreferences::m_uiStartUpFlag = 0;
...
uint8	CPreferences::m_uiPaybackFirst; //>>> Anis::Payback First
bool	CPreferences::m_bAutoDropSystem; //Anis -> Drop System
...
bool	CPreferences::adsDisable2;

// Modifiche AdunanzA
uint32	CPreferences::m_AduMaxUpSlots;
bool	CPreferences::m_AduSendStats;
bool	CPreferences::m_AduNoTips;
uint32	CPreferences::m_AduTips;
uint32	CPreferences::m_AduNoObf;
//tigerjact ripartitore di banda
bool	CPreferences::m_AduRipBanda;
uint32	CPreferences::m_AduKaduMaxDown;
uint32	CPreferences::m_AduExtMaxDown;
bool	CPreferences::upextfull;
uint64	CPreferences::ExtAdapterSentBytes;
uint64	CPreferences::AduAdapterSentBytes;
int CPreferences::maxexttimewait; 
int CPreferences::extfullwaittime; 
bool CPreferences::checkextfull;
uint32  CPreferences::m_AduValRipBanda;
//Anis -> gestione automatica refresh cartella condivisa
//Anis -> UPNP - NAT
uint8	CPreferences::m_uiDirectoryWatcherMode;
UINT	CPreferences::m_uiASFUfreq;
...
int     CPreferences::m_iDetectuPnP; //leuk_he autodetect in startup Anis
bool    CPreferences::m_bUPnPForceUpdate;
//Anis -> UPNP - NAT fine
CStringList CPreferences::sharedsubdir_list;	// Anis: shareSubdir
...
// fine mod Adu
uint16	CPreferences::adsl_fiber; //Anis
uint16	CPreferences::m_BufferStreaming;
...
void CPreferences::Init()
{
	...
	if (sdirfile->Open(strFullPath, CFile::modeRead | CFile::shareDenyWrite | (bIsUnicodeFile ? CFile::typeBinary : 0)))
	{
		try {
			...
			while (sdirfile->ReadString(toadd))
			{
				...
				// Anis: SafeHash remove - removed installation dir unsharing
				/*
	...
	if (!PathFileExists(GetMuleDirectory(EMULE_INCOMINGDIR)) && !::CreateDirectory(GetMuleDirectory(EMULE_INCOMINGDIR),0)) 
	{
		m_uiStartUpFlag |= 0x01;

	}
	if (!PathFileExists(GetTempDir()) && !::CreateDirectory(GetTempDir(),0)) 
	{
		m_uiStartUpFlag |= 0x02;
	}

		if(m_uiStartUpFlag == 0)
			break;

		CString strError = L"";
		if(m_uiStartUpFlag & 0x01)
			strError.Format(GetResString(IDS_ERR_CREATE_DIR), GetResString(IDS_PW_INCOMING), GetMuleDirectory(EMULE_INCOMINGDIR), GetErrorMessage(GetLastError()));
		if(m_uiStartUpFlag & 0x02)
		{
			if(!strError.IsEmpty())
				strError.Append(L"\n");
			strError.Format(GetResString(IDS_ERR_CREATE_DIR), GetResString(IDS_PW_TEMP), GetTempDir(), GetErrorMessage(GetLastError()));
		...
		strMessage.Format(GetResString(IDS_DIR_PROBLEM), strError);
		int nResult = XMessageBox(NULL, strMessage, GetResString(IDS_ERROR), MB_CANCELTRYCONTINUE | MB_DEFBUTTON2 | MB_ICONERROR);
		if(nResult == IDCANCEL)
		{
			m_uiStartUpFlag = 1;
			break;
		...
		{
			//what shall we do if the default directories cannot be created!? 
			//for now, we simply tell the user about it...
			if(m_uiStartUpFlag & 0x01)
			{
				m_strIncomingDir = GetDefaultDirectory(EMULE_INCOMINGDIR, true); // will also try to create it if needed
				if (!PathFileExists(GetMuleDirectory(EMULE_INCOMINGDIR)))
...
uint64 CPreferences::GetMaxDownloadInBytesPerSec(bool dynamic){
	...
	//Anis: if you CAN not upload, why should you be punished!?
	if(theApp.uploadqueue && theApp.uploadqueue->GetWaitingUserCount() && theApp.uploadqueue->GetUploadQueueLength())
	{
		...
		const double uploaded = max(SESSIONMAXTRANS, (double)theStats.sessionSentBytes /*- (double)theStats.sessionSentBytesToFriend - (double)theStats.sessionSentBytesViaPartPS*/); //>>> Anis::MultiFU //>>> Anis::PowerShare
		const double sessionUlDlRatio = (uploaded + 1.0) / (theStats.sessionReceivedBytes - sesDownData_URL + 1.0); //>>> Anis::Exclude HTTP Traffic [Quezl]
...
// <-----khaos-
LONG GetStringRegKey(HKEY hKey, const std::wstring &strValueName, std::wstring &strValue, const std::wstring &strDefaultValue) //Funzione di Anis
{ 
...
void CPreferences::SavePreferences()
{
	...
	ini.WriteInt(L"SOTN", m_bShareOnlyTheNeed); //>>> Anis::Intelligent SOTN
	ini.WriteBool(L"UseSaveLoadSources", m_bUseSaveLoadSources); //>>> JvA::SLS [enkeyDEV]
	ini.WriteInt(L"TecnologiaConnessione", adsl_fiber);
	ini.WriteInt(L"BufferStreaming", m_BufferStreaming);
	//Anis -> UPNP NAT
	...
	//Anis -> UPNP NAT fine
	ini.WriteInt(L"DirWatcherFreq", m_uiASFUfreq);
	...
	ini.WriteInt(L"UploadCapacity",maxGraphUploadRate);
	...
	ini.WriteBool(L"AdsDisable",adsDisable2);
	...
	ini.WriteInt(L"PCPort", m_nPeerCachePort);
 	...
 	ini.WriteInt(_T("AduMaxUpSlots"), m_AduMaxUpSlots, _T("AdunanzA"));
	ini.WriteInt(_T("AduValRipBanda"), m_AduValRipBanda, _T("AdunanzA"));
	ini.WriteBool(_T("AduRipBanda"), m_AduRipBanda, _T("AdunanzA"));
	ini.WriteInt(_T("AduKaduMaxDown"), m_AduKaduMaxDown, _T("AdunanzA"));
	ini.WriteInt(_T("AduExtMaxDown"), m_AduExtMaxDown, _T("AdunanzA"));
 	ini.WriteInt(_T("AduSendStats"), m_AduSendStats, _T("AdunanzA"));
 	ini.WriteInt(_T("AduTips"), m_AduTips, _T("AdunanzA"));
 	ini.WriteInt(_T("AduNoObf"), m_AduNoObf, _T("AdunanzA"));
}
...
void CPreferences::LoadPreferences()
{
	...
	maxGraphDownloadRate=ini.GetInt(L"DownloadCapacity",16384);
	if (maxGraphDownloadRate == 0)
		maxGraphDownloadRate = 16384;

	maxGraphUploadRate = ini.GetInt(L"UploadCapacity", 4096);
	if (maxGraphUploadRate == 0)
		maxGraphUploadRate = 4096;

	minupload=(uint16)ini.GetInt(L"MinUpload", 10);
	...
	udpport = ini.GetInt(L"UDPPort", 0);
	if (udpport == 0)
		udpport = 4672;
	...
	//Anis -> misure di sicurezza
	extern bool FileExist(LPCTSTR filename);
	extern CString m_sFilenameUPDATEREXE;
	extern CString m_sFilenameUPDATER;

	if(FileExist(m_sFilenameUPDATEREXE))
		DeleteFile(m_sFilenameUPDATEREXE);

	if(FileExist(m_sFilenameUPDATER))
		DeleteFile(m_sFilenameUPDATER);

	theApp.rm->UpdateURL = ADURM_URL; // configuro la lingua italiana
	theApp.rm->CheckUpdate(); // Anis -> configuro i remote settings (per non aspettare 60 secondi dal primo avvio)
	theApp.rm->SaveSettings(); // salvo la configurazione
 	// Mod Adu
 	// Lingua Italiana prestabilita
 	m_wLanguageID = ini.GetWORD(_T("Language"),LANGID_IT_IT);
 	if (m_bFirstStart) { //primo avvio di adunanza
 		m_wLanguageID = LANGID_IT_IT;
		// Setto l'url remota compilata
		//WSADATA wsadata; //Anis -> mio esperimento funzionante ma molto pericoloso. In pratica se il dominio viene perso tutti i muli adu non possono più aprirsi...  LoL.
		//WSAStartup(0x101, &wsadata);
		//if(gethostbyname("www.adunanza.net") != NULL) {
			thePrefs.maxconnections = 1200;
			thePrefs.maxsourceperfile = 600;
			adsl_fiber = SCONOSCIUTO;
		//}
		/*else {
			MessageBox(NULL,_T("Errore critico nel core di AdunanzA. Rivolgersi al supporto tecnico nel forum ufficiale di AdunanzA. codice: 3"),
			_T("AdunanzA Core Error"),MB_ICONERROR);
			WSACleanup();
			exit(1);
		}*/
   }
		...
		WizardNotOpen = true;
		AduUpdate(); //Anis -> faccio quì il controllo dell'update.
 	// fine mod Adu
	}

	::AfxBeginThread(CemuleDlg::CheckNodes, NULL, 0, 0, 0, NULL);
	m_uiPaybackFirst = 3;
	m_bShareOnlyTheNeed = ini.GetInt(L"SOTN", -1); //>>> Anis::Intelligent SOTN
	m_uiSmallFileSlotSize = (uint16)ini.GetInt(L"SmallFileSlotSize", 1024); //Anis -> Small file slot
	//>>> JvA::SLS [enkeyDEV]
	m_bUseSaveLoadSources = ini.GetBool(L"UseSaveLoadSources", true);
	m_iActivationLimitSLS = (uint8)ini.GetInt(L"ActivationLimitSLS", 100);
	m_iSourcesToSaveSLS = (uint8)ini.GetInt(L"SourcesToSaveSLS", 25);
	//### JvA::SLS [enkeyDEV]
	//Anis gestione automatica refresh cartella condivisa
	m_uiDirectoryWatcherMode = ASFU_SIMPLE;
	m_uiASFUfreq = ini.GetInt(L"DirWatcherFreq", 60); //1h for "simple" mode - 0 for ASFU_DEFAULT
	//fine
	m_bAutoDropSystem = true;
	m_BufferStreaming = ini.GetInt(L"BufferStreaming", 3);
	if(m_BufferStreaming > 10 || m_BufferStreaming < 1)
		m_BufferStreaming = 3;
    ...
    SetUpnpDetect(ini.GetInt(L"uPnPDetect", UPNP_DO_AUTODETECT, L"eMule")); //leuk_he autodetect upnp in Anis
	...
	adsl_fiber = ini.GetInt(L"TecnologiaConnessione", SCONOSCIUTO);
	...
	splitterbarPosition=ini.GetInt(L"SplitterbarPosition",66); //Anis
	...
	m_bAutoUpdateServerList = ini.GetBool(L"Serverlist", true);
	if (m_bFirstStart)
		m_bAutoUpdateServerList = true;
	...
	m_bPreventStandby = ini.GetBool(L"PreventStandby", true);
	...
	filterLANIPs=ini.GetBool(L"FilterBadIPs",false);
	...
	m_uMinFreeDiskSpace = 500*1024*1024;
	...
	m_strIRCServer = ini.GetString(L"DefaultIRCServerNew", L"chat.adunanza.net");
	m_strIRCNick = ini.GetString(L"IRCNick", L"Adu3_User");
	m_bIRCAddTimeStamp = ini.GetBool(L"IRCAddTimestamp", true);
	m_bIRCUseChannelFilter = ini.GetBool(L"IRCUseFilter", false);
	...
	m_bshowoverhead=ini.GetBool(L"ShowOverhead",true);
	...
	// Mod Adu
	m_iFileBufferSize = 15*1024*1024;
	// Fine Mod Adu
	...
	m_iSearchMethod=SearchTypeKademlia; //ini.GetInt(L"SearchMethod",SearchTypeKademlia);
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	networkkademlia=true;//ini.GetBool(L"NetworkKademlia",true);
	...
	m_strVideoPlayer = ini.GetString(L"VideoPlayer", L""); //Anis
///////////////ANIS ADU MOD -> RILEVAZIONE AUTOMATICA VLC///////////////
	if(m_strVideoPlayer == L"") {
	HKEY hKey; 
	std::wstring VLCdir;
	if(ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, REG_VLC_DIR, 0, KEY_READ, &hKey)) {
		GetStringRegKey(hKey, REG_VLC_VAR, VLCdir, L""); 
		m_strVideoPlayer = VLCdir.c_str();
		if (m_strVideoPlayer == _T(""))
			goto skip;
		m_strVideoPlayer += VLC_EXE_NAME;
	}	
	}
	skip:

///////////////ANIS ADU MOD -> RILEVAZIONE AUTOMATICA VLC///////////////	
	if (m_strVideoPlayer == L"") NoPlayer = true;
	...
	messageFilter=ini.GetStringLong(L"MessageFilter",L"Your client has an infinite queue|Your client is connecting too fast|zambor|FX the BEST|DI-Emule|slot|Ultra2|HyperMule|fastest");
	...
	m_iExtractMetaData = ini.GetInt(L"ExtractMetaData", 0); // 0=disable, 1=mp3, 2=MediaDet
	...
	// modifica AdunanzA
	m_AduMaxUpSlots = ini.GetInt(_T("AduMaxUpSlots"), ADUNANZA_DEF_UPSL, _T("AdunanzA"));
	//tigerjact ripartirore di banda
	m_AduValRipBanda = ini.GetInt(_T("AduValRipBanda"), 6, _T("AdunanzA"));
	m_AduRipBanda = ini.GetBool(_T("AduRipBanda"), true, _T("AdunanzA"));
	m_AduKaduMaxDown = ini.GetInt(_T("AduKaduMaxDown"), UNLIMITED*1024, _T("AdunanzA"));
	m_AduExtMaxDown = ini.GetInt(_T("AduExtMaxDown"), UNLIMITED*1024, _T("AdunanzA"));
	
	//tigerjact
	upextfull = false;
	maxexttimewait = ADU_EXT_FULL_WAIT_TIME;
	extfullwaittime = 0;
	checkextfull = true;
	ExtAdapterSentBytes = 0;
	AduAdapterSentBytes = 0;

	m_AduSendStats = ini.GetBool(_T("AduSendStats"), true, _T("AdunanzA"));
	m_AduTips = ini.GetInt(_T("AduTips"), 0, _T("AdunanzA")) & (~(ADUTIP_LOWUPLIMITS|ADUTIP_BROKENKAD));
	m_AduNoTips = ini.GetBool(_T("AduNoTips"), false, _T("AdunanzA"));
	...
	m_AduNoObf = ini.GetInt(_T("AduNoObf"), 1, _T("AdunanzA"));

	m_btransferfullchunks = true;
	m_bPeerCacheEnabled = false;

	if	(!IsLanguageSupported(m_wLanguageID, false))
	{
		// download file
		...
		// Modifica AdunanzA
		// lupz
		if (m_wLanguageID == LANGID_IT_IT)
			strUrl = ADU_LANG_URL;
		else			
			strUrl.Format(MIRRORS_URL,nRand, VERSION_MJR, VERSION_MIN, VERSION_UPDATE, VERSION_BUILD);
		// Fine Mod AdunanzA

		strUrl += GetLangDLLNameByID(m_wLanguageID);
		// safeto
		//tigerjact, uso la cofig per mettere la lingua
		CString strFilename = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR);
		if (!PathFileExists(strFilename))
			CreateDirectory(strFilename,0);
		strFilename.Append(GetLangDLLNameByID(m_wLanguageID));
		// start
		CHttpDownloadDlg dlgDownload;
		dlgDownload.m_strTitle = _T("Downloading language file");
		dlgDownload.m_sURLToDownload = strUrl;
		dlgDownload.m_sFileToDownloadInto = strFilename;
		if (dlgDownload.DoModal() != IDOK || !thePrefs.IsLanguageSupported(m_wLanguageID, true))
		{
			CString strErr;
...
void CPreferences::SaveCats()
{
	...
	for (int i = 0; i < catMap.GetCount(); i++)
	{
		...
		ini.WriteStringUTF8(L"URL", catMap.GetAt(i)->catURL); //Mod Adu - Ded - URL per categorie
	}
}
...
void CPreferences::LoadCats()
{
	...
	for (int i = 0; i <= iNumCategories; i++)
	{
		...
		newcat->catURL = ini.GetStringUTF8(L"URL"); //Mod Adu - Ded - URL per categorie
		AddCat(newcat);
	}
}
...
//Anis -> Inizio descrizione completa ///
/*
La prima cosa che controllo è la versione del sistema operativo. Nel caso questa sia maggiore o uguale a 0x600 (Vista)
controllo che nel registro ci sia la chiave che identifica la scelta effettuata dall'utente durante la fase di installazione.
Questo per stabilire se impostare le cartelle di condivisione pubbliche o private.
Se il sistema operativo è minore di 0x600, sistematicamente le cartelle sono private ma situate nella root di eMule, poichè la cartella Download non esiste nei vecchi OS.
Visto che io ed Hammon abbiamo stabilito di semplificare l'installer, togliendo tutto ciò che è decisamente superfluo, indipendentemente dal sistema operativo esisterà solo la gestione delle cartelle privata.
Per settare le cartelle dei documenti pubblici sarà invece necessario farlo manualmente dalle opzioni di emule nel caso qualcuno lo desiderasse.
*/
//Anis -> Fine descrizione completa ///

//Anis -> Funzione adattata completamente per nuove necessità vedere descrizione in alto.
		...
		*pszFileName = L'\0';
		...
		if (GetWindowsVersion() >= _WINVER_VISTA_) {
			...
			if (hShell32){
					...
					if (pfnSHGetKnownFolderPath != NULL && (*pfnSHGetKnownFolderPath)(FOLDERID_Downloads, 0, NULL, &pszPersonalDownloads) == S_OK && (*pfnSHGetKnownFolderPath)(FOLDERID_PublicDownloads, 0, NULL, &pszPublicDownloads) == S_OK && (*pfnSHGetKnownFolderPath)(FOLDERID_LocalAppData, 0, NULL, &pszLocalAppData) == S_OK)
					{
						if (_tcsclen(pszPersonalDownloads) < MAX_PATH - 40 && _tcsclen(pszPublicDownloads) < MAX_PATH - 40 && _tcsclen(pszLocalAppData) < MAX_PATH - 30)
						{
							...
							strSelectedDataBaseDirectory = strPersonalDownloads + _T("eMule AdunanzA\\");
							strSelectedConfigBaseDirectory = strLocalAppData + _T("eMule AdunanzA\\");
		...
		else if(GetWindowsVersion() == _WINVER_XP_) { //Anis
					...
					if (!strAppData.IsEmpty() && !strPersonal.IsEmpty()) {
						if (strAppData.GetLength() < MAX_PATH - 30 && strPersonal.GetLength() < MAX_PATH - 40){
							...
							if (strAppData.Right(1) != _T("\\"))
								strAppData += _T("\\");
							m_nCurrentUserDirMode = 0;
							strSelectedDataBaseDirectory = strPersonal + _T("eMule AdunanzA\\");
							strSelectedConfigBaseDirectory = strAppData + _T("eMule AdunanzA\\");
						}
						else
							ASSERT( false );
					}
