...
// Mod Adu
// mostro la A3 di default
const CString strDefaultToolbar = _T("009901020304050607089909101211");
// Fine Mod Adu
...
struct Category_Struct{
	...
	CString autocat;
	CString	regexp;
	CString	catURL; //Mod Adu - Ded - URL per categorie 
	int		filter;
	bool	filterNeg;
	bool	care4all;
	bool	ac_regexpeval;
...
public:
	...
	static int      m_iDetectuPnP; //leuk_he autodetect in startup Anis
	...
	static	int  	GetUpnpDetect()					{ return m_iDetectuPnP; } //leuk_he autodetect upnp in Anis
	static	void    SetUpnpDetect(int on)				{ m_iDetectuPnP=on; } //leuk_he autodetect upnp in Anis
	...
	static	CStringList sharedsubdir_list;	// Anis: shareSubdir
	...
	static	void	SetMaxGraphDownloadRate(int in)		{maxGraphDownloadRate=(in)?in:10;}
	...
	static	bool	GetDebug2Disk()						{return m_bVerbose && log2disk;}
	...
	// PeerCache
	static	bool	IsPeerCacheDownloadEnabled()		{m_bPeerCacheEnabled = false; return m_bPeerCacheEnabled;}
	...
	static bool		IsAduClientCryptLayerRequested()	{return IsClientCryptLayerRequested() && !m_AduNoObf;} // TheKing0 fix
	static bool		IsAduClientCryptLayerRequired()		{return IsAduClientCryptLayerRequested() && m_bCryptLayerRequired;}
	...
	static bool		m_AduSendStats;
	static bool		m_AduNoTips;
	static bool		m_AduRipBanda;
	static uint32	m_AduValRipBanda;
	static uint32	m_AduKaduMaxDown;
	static uint32	m_AduExtMaxDown;
	static bool		upextfull;
	static int		maxexttimewait;
	static int		extfullwaittime;
	static bool		checkextfull;
	static uint64	ExtAdapterSentBytes;
	static uint64	AduAdapterSentBytes;
	static uint32	m_AduTips;
	static uint32	m_AduNoObf;
...
private:
	static  int		m_bShareOnlyTheNeed;
	static	uint8	m_uiPaybackFirst;
public:
	static	int		IsShareOnlyTheNeed()				{return m_bShareOnlyTheNeed;}
	static	void	SetShareOnlyTheNeed(const int& i)	{m_bShareOnlyTheNeed = i;}


	static	uint8	GetPaybackFirst()					{return m_uiPaybackFirst;}
	static	void	SetPaybackFirst(const uint8 i)		{m_uiPaybackFirst = i;}

//Anis -> boost file uploader parti piccole
private:
	static	uint16	m_uiSmallFileSlotSize;
public:
	static	uint16	GetSmallFileSize()					{return m_uiSmallFileSlotSize;}
...
//<<< JvA::SLS [enkeyDEV]
static	uint8	m_uiStartUpFlag; // Anis -> miglioria per dispositivi mobili
//Anis -> gestione automatica del refresh dei file condivisi
private:
	static	uint8	m_uiDirectoryWatcherMode;
	static	UINT	m_uiASFUfreq;
public:
	static	uint8	GetDirectoryWatcherMode()				{return m_uiDirectoryWatcherMode;}
	static	void	SetDirectoryWatcherMode(const uint8 i)	{m_uiDirectoryWatcherMode = i;}
	static	UINT	GetASFUfreq()							{return m_uiASFUfreq;}
	static	void	SetASFUfreq(const UINT& i)				{m_uiASFUfreq = i;}
//Anis Fine
	private:
...
public:
	...
	static	uint16	m_BufferStreaming;
};
