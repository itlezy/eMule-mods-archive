...
//Anis -> NOTA:
/* Il secondo stimatore di fonti scritto da lupz è stato cancellato poichè da risultati troppo fasulli.
Ottimo lavoro invece per il primo stimatore, corretto ed efficente!!! (:
*/
...
uint32 CSearchFile::AddOrigSources(uint32 count)
{
	for (int i = 0; i < taglist.GetSize(); i++)
	{
		CTag* pTag = taglist[i];
		if (pTag->GetNameID() == FT_KADSOURCES)
		{
			if (m_bKademlia)
			{
				if (count > pTag->GetInt())
	...
	CTag* pTag = new CTag(FT_KADSOURCES, count);
	taglist.Add(pTag);
	return count;
}
// Fine Mod Adu
