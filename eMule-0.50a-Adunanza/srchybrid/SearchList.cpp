...
{	
	...
	if (file 
		...
		&& toadd->GetClientID() 
		&& toadd->GetClientPort() 
		&& CPartFile::CanAddSource(toadd->GetClientID(), toadd->GetClientPort(), toadd->GetClientServerIP(), toadd->GetClientServerPort())) // pre-filter sources which would be dropped in CPartFile::AddSources
	{
//>>> adapted from CPartFile::AddSources()
		...
		sources.SeekToBegin();
		file->AddSources(&sources,toadd->GetClientServerIP(),toadd->GetClientServerPort(), SF_SEARCH);	// Anis: sourceFrom*/
		UINT debug_lowiddropped = 0;
		UINT debug_possiblesources = 0;
		uint32 userid = toadd->GetClientID();
		uint16 port = toadd->GetClientPort();
		uint32 serverip = toadd->GetClientServerIP();
		uint16 serverport = toadd->GetClientServerPort();
		...
		if (!IsLowID(userid))
		{
			...
			if (bAdd && theApp.ipfilter->IsFiltered(userid))
			{
				if (thePrefs.GetLogFilteredIPs())
		...
		if (bAdd && !CanAddSource(userid, port, serverip, serverport, &debug_lowiddropped))
		{
			//if (thePrefs.GetLogFilteredIPs())
		...
		*/
		...
		if( bAdd && file->GetMaxSources() > file->GetSourceCount() )
		{
			debug_possiblesources++;
			CUpDownClient* newsource = new CUpDownClient(file,port,userid,serverip,serverport,true);
			if(theApp.downloadqueue->CheckAndAddSource(file,newsource))
				newsource->SetSourceFrom(SF_SEARCH);
		}
		else
		{
			// since we may received multiple search source UDP results we have to "consume" all data of that packet
			//sources->Seek(((count-1)-i)*(4+2), SEEK_CUR);
			if(file->GetKadFileSearchID())
				Kademlia::CSearchManager::StopSearch(file->GetKadFileSearchID(), false);
		}

		if (thePrefs.GetDebugSourceExchange())
			AddDebugLogLine(false, _T("SearchCatch: Server source response; Count=%u, Dropped=%u, PossibleSources=%u, File=\"%s\""), 1, debug_lowiddropped, debug_possiblesources, file->GetFileName());
//### adapted from CPartFile::AddSources()
	}
//### Anis::searchCatch [Anis]
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
			...
			&& md4cmp(parent->GetFileHash(), toadd->GetFileHash()) == 0)
		{
			...
			UINT uAvail, uOrigAvail = 0; // Mod Adu: lupz
			if (bClientResponse) {
			...
			if (toadd->IsKademlia())
				uOrigAvail = toadd->GetIntTagValue(FT_KADSOURCES);
			...
			else
			{
				...
				for (POSITION pos2 = list->GetHeadPosition(); pos2 != NULL && !bFound; )
				{
						...
						&& toadd->GetFileName().CompareNoCase(child->GetFileName()) == 0)	// same name
					{
						...
						if (bHasOrigSources)
							child->AddOrigSources(uOrigSources);
			...
			// Mod Adu
			// lupz
			UINT uAllChildsOrigSourceCount = 0;     // kad only: the max. sources found
			// Fine Mod Adu
			...
			for (POSITION pos2 = list->GetHeadPosition(); pos2 != NULL; )
			{
				...
				if (child->GetListParent() == parent)
				{
					...
					if (parent->IsKademlia())
					{
						...
						if (child->GetCompleteSourceCount() > uAllChildsCompleteSourceCount) //Mod Adu: lupz Supported :) // not yet supported
			...
			if (bestEntry)
			{
				...
				if (bHasMultipleAICHHashs || !bAICHHashValid)
					...
					// lupz
				if ( parent->IsKademlia() )
					parent->SetIntTagValue(FT_KADSOURCES, uAllChildsOrigSourceCount);
				// Fine Mod Adu
			}
