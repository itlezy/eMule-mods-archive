...
CString CSearchListCtrl::GetCompleteSourcesDisplayString(const CSearchFile* pFile, UINT uSources, bool* pbComplete) const
{
	...
	// lupz
	if (pFile->IsKademlia()) {
		str.Format(_T("%u"), uCompleteSources);
	}
	else if (iComplete < 0)			// '< 0' ... unknown
	// Fine Mod Adu
	{
		str = _T("?");
		if (pbComplete)
...
int CSearchListCtrl::Compare(const CSearchFile *item1, const CSearchFile *item2, LPARAM lParamSort, bool bSortAscending)
{
	...
	switch (lParamSort)
	{
		...
		case 3: // complete sources asc
			...
			// lupz
			// sort kadu
			if (item1->IsKademlia() || item2->IsKademlia())
				return CompareUnsigned(item1->GetIntTagValue(FT_COMPLETE_SOURCES), item2->GetIntTagValue(FT_COMPLETE_SOURCES));
			// Fine Mod Adu
			else if (item1->GetIntTagValue(FT_SOURCES) == 0 || item2->GetIntTagValue(FT_SOURCES) == 0)
...
COLORREF CSearchListCtrl::GetSearchItemColor(/*const*/ CSearchFile* src)
{
	...
	if (srccnt > 0)
		--srccnt;
	AddDebugLogLine(DLP_LOW, false, _T("srccnt: %d"),srccnt);
