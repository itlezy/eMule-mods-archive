...
LRESULT CSearchParamsWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	InitMethodsCtrl();
	if (m_ctlMethod.SetCurSel(thePrefs.GetSearchMethod()) == CB_ERR) //Anis - Lupz -> KAdu per default
		m_ctlMethod.SetCurSel(SearchTypeKademlia);
...
void CSearchParamsWnd::SetAllIcons()
{
	...
	// Mod adunanza : invece di SearchMethod_KADEMLIA uso l'icona di adunanza
	//Anis - Evolution -> Inizio integrazione 07/02/2012 - 02:40
	iml.Add(CTempIconLoader(_T("ClientAdunanzA")));
	iml.Add(CTempIconLoader(_T("Evolution")));
	// Fine mod adunanza
...
void CSearchParamsWnd::InitMethodsCtrl()
{
	...
	VERIFY( m_ctlMethod.AddItem(_T("Evolution"), 4) == SearchTypeEvolution );
	...
	// Mod Adu
	// lupz
	// usiamo ricerca kadu per default
	m_ctlMethod.SetCurSel(iMethod != CB_ERR ? iMethod : SearchTypeKademlia);
	// Fine Mod Adu
}
...
{
	if(m_ctlMethod.GetCurSel() == SearchTypeEvolution)
	{
		extern bool adu_controllo;
		adu_controllo = true;
		CString msg(_T("Il motore di ricerca Evolution è ancora in fase di sviluppo.\nSarà pertanto disponibile nelle future versioni di eMule AdunanzA.\nUtilizzare il metodo di ricerca eD2k o KAdu.\nIn attesa del compimento dei lavori, l'AduTeam vi augura un buon download!"));
