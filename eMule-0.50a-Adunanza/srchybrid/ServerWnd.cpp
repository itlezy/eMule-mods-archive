...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		...
		servermsgbox->AppendText(_T("eMule ") + theApp.m_strCurVersionLong + _T("\n"));
		/*
...
void CServerWnd::OnSvrTextChange()
{
	...
	// Stefano Picerno: Abilito il bottone "Update server.met from URL" anche quando thePrefs.adresses_list.IsEmpty() == false
	//GetDlgItem(IDC_UPDATESERVERMETFROMURL)->EnableWindow( GetDlgItem(IDC_SERVERMETURL)->GetWindowTextLength()>0 || thePrefs.adresses_list.IsEmpty() == false );
	// Fine modifica
}
