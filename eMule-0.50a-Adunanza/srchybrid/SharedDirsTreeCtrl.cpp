...
BEGIN_MESSAGE_MAP(CSharedDirsTreeCtrl, CTreeCtrl)
	...
	ON_MESSAGE(TM_FOUNDNETWORKDRIVE, OnFoundNetworkDrive)
END_MESSAGE_MAP()
...
void CSharedDirsTreeCtrl::InitalizeStandardItems(){
	...
	// Mod Adu
	// lupz
	// eliminiamo il tab "Tutte le cartelle" visto che crea confusione
	...
	while (pos != NULL){
		...
		switch( pCurrent->m_eItemType ){
			...
			case SDI_TEMP:
				...
				break;
			case SDI_DIRECTORY: {
				// Anis START: shareSubdir - do both lists as well
				/*
				// add subdirectories
				bool bShowWarning = false;
...
{	
	...
	if (pSelectedDir != NULL && pSelectedDir->m_eItemType != SDI_UNSHAREDDIRECTORY && pSelectedDir->m_eItemType != SDI_FILESYSTEMPARENT){
		...
		m_ShareDirsMenu.EnableMenuItem(MP_SHAREDIR, !FileSystemTreeIsShared(pSelectedDir->m_strFullPath, true, true) ? MF_ENABLED : MF_GRAYED);	// Anis: shareSubdir - don't suggest double sharing
...
//								m_pSharedFilesCtrl->UpdateFile(file); //>>> Anis::Optimization
								break;
							case MP_PRIOLOW:
//								file->SetAutoUpPriority(false); //>>> Anis::Optimization
...
//								m_pSharedFilesCtrl->UpdateFile(file);  //>>> Anis::Optimization
						...
						m_pSharedFilesCtrl->UpdateFile(file, false, &bSelected); //>>> Anis::Optimization
					}
//>>> Anis::Optimization
					if(bSelected)
						theApp.emuledlg->sharedfileswnd->ShowSelectedFilesDetails();
...
void CSharedDirsTreeCtrl::FileSystemTreeCreateTree()
{
	...
	if (dwRet > 0 && dwRet < _countof(drivebuffer))
	{
		...
		while(*pos != _T('\0')){
			...
			// Mod Adu
			// lupz
			// Rimuovo i riferimenti al tab tutte le cartelle
	...
	AfxBeginThread(EnumNetworkDrivesThreadProc, (LPVOID)this, NULL, NULL, NULL, NULL);
}
...
void CSharedDirsTreeCtrl::RemoveSharedDirectory(CString strDir, bool bSubDirectories){
	...
	// Anis START: shareSubdir - check second list
...
void CSharedDirsTreeCtrl::FileSystemTreeUpdateBoldState(const CDirectoryItem* pDir){
	...
	// eliminiamo il tab "Tutte le cartelle" visto che crea confusione
	if (pDir == NULL) {
		if (m_pRootUnsharedDirectries != NULL)
...
void CSharedDirsTreeCtrl::FileSystemTreeSetShareState(const CDirectoryItem* pDir, bool bSubDirectories){
	if (m_bUseIcons && pDir->m_htItem != NULL)
		//MORPH START - Changed, ShareSubDir [Anis]
		/*
		...
		*/
		...
		//MORPH END   - Changed, ShareSubDir [Anis]
	if (bSubDirectories){
		POSITION pos = pDir->liSubDirectories.GetHeadPosition();
		while (pos != NULL){
...
void CSharedDirsTreeCtrl::EditSharedDirectories(const CDirectoryItem* pDir, bool bAdd, bool bSubDirectories){
	...
	// Anis START: shareSubdir - sync second list
