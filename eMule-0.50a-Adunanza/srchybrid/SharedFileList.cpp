...
{	
	m_keylistfile = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + _T("key_np.dat");
	readFile();
	ResetNextKeyword();
	SetNextPublishTime(0);
}
...
{
	try {
		CSafeBufferedFile klist_file;
		CFileException fexp;
		if (klist_file.Open(m_keylistfile,CFile::modeRead|CFile::osSequentialScan|CFile::typeBinary|CFile::shareDenyWrite, &fexp)) {
	...
	{
		AddDebugLogLine( false, _T("Exception in CPublishKeywordList::readFile (IO error(%i))"), error->m_cause);
		error->Delete();
	}
	catch (...) 
	{
		AddDebugLogLine(false, _T("Exception in CPublishKeywordList::readFile"));
	}
}
...
void CPublishKeywordList::saveFile()
{
	try {
		CSafeBufferedFile klist_file;
		CFileException fexp;
		DeleteFile(m_keylistfile);
		if (klist_file.Open(m_keylistfile, CFile::modeWrite|CFile::modeCreate|CFile::typeBinary|CFile::shareDenyWrite, &fexp)) {
			...
			while (pos) {
				...
				CStringW ThisFileName = pPubKw->GetKeyword();
				UINT32 ThisPublicTIme = pPubKw->GetNextPublishTime();
				klist_file. WriteStringMIO( ThisFileName,utf8strRaw);//pPubKw->GetKeyword());
				klist_file.WriteUInt32(ThisPublicTIme);//pPubKw->GetNextPublishTime());
	...
	{
		AddDebugLogLine( false, _T("Exception in CPublishKeywordList::saveFile (IO error(%i))"), error->m_cause);
		error->Delete();
	}
	catch (...) 
	{
		AddDebugLogLine(false, _T("Exception in CPublishKeywordList::saveFile"));
	}
}
...
int CAddFileThread::Run()
{
	...
	if (newrecord->CreateFromFile(m_strDirectory, m_strFilename, m_partfile) && theApp.emuledlg && theApp.emuledlg->IsRunning()) // Anis: SafeHash - in case of shutdown while still hashing
	{
	...
	else
	{
		...
		// Anis: SafeHash - inform main program of hash failure
...
/* old code:
	...
	for (int ix=1;ix<thePrefs.GetCatCount();ix++)
	{
		...
		if( l_sAdded.Find( ltempDir ) == NULL && //ssd
			!FindStartingWith(&l_sAddedWithSubdir, ltempDir) ) {	// Anis: shareSubdir
	...
	if (!waitingforhash_list.IsEmpty()) {
			...
			while (p != NULL) {
				...
				Log(_T("Nuovo file: '%s'"), hashfilename);
...
void CSharedFileList::AddFilesFromDirectory(const CString& rstrDirectory, bool bWithSubdir)	// Anis: shareSubdir
{
	CFileFind ff;
	...
	while (!end)
	{
		...
		*/
		CheckAndAddSingleFile(ff,bWithSubdir);
		// Anis: shareSubdir
	}
	ff.Close();
}
	...
	if (bShared && !bExclude){
		...
		if (!bNoUpdate)
		{
			if (iDoAsfuReset == -1)
				...
				iDoAsfuReset = 1; // we would have reset but don't do so just now
		}
	}	
...
{
	bool bAdded = false;
	RemoveFromHashing(toadd);	// Anis: SafeHash - hashed ok, remove from list, in case it was on the list
...
{
	m_dwLastReloaded = ::GetTickCount(); //>>> Anis::Simple Auto-Reload
	// Anis: SafeHash - don't allow to be called until after the control is loaded
	if (!output)
		return;
	// Anis: SafeHash
	...
	/* Anis: SafeHash remove - check moved up
	if (output)
	*/
...
void CSharedFileList::SetOutputCtrl(CSharedFilesCtrl* in_ctrl)
{
	...
	output->ReloadFileList();
	/* official 
	HashNextFile();		// Anis: SafeHash - if hashing not yet started, start it now
	*/
	LoadSingleSharedFilesList(); // MORPH Anis: SafeHash - load shared files after everything
	Reload();		// MORPH Anis: SafeHash - load shared files after everything
}
...
void CSharedFileList::HashNextFile(){
	// Anis: SafeHash
	if (!theApp.emuledlg || !theApp.emuledlg->IsRunning() || !::IsWindow(theApp.emuledlg->m_hWnd))
		return;
	if (theApp.emuledlg && theApp.emuledlg->IsRunning())
	...
	if (waitingforhash_list.IsEmpty())
		AddLogLine(false, _T("Hashing di nuovi files completati."));
	...
	UnknownFile_Struct* nextfile = waitingforhash_list.RemoveHead();
	currentlyhashing_list.AddTail(nextfile);	// Anis: SafeHash - keep track
	...
	// Anis: SafeHash - nextfile deleting handled elsewhere
	//delete nextfile;
}
...
{
//>>> Anis::PowerShare - CPU!
	if(::GetTickCount() > toupdate->guifileupdatetime) //once per second
		return;
	toupdate->guifileupdatetime = ::GetTickCount() + 1000;
	if(theApp.emuledlg->sharedfileswnd->m_hWnd && output)
//### Anis::PowerShare - CPU!
...
{
	ReloadIfNeeded(); //>>> Anis::Simple Auto-Reload
	Publish();
...
bool CSharedFileList::ShouldBeShared(CString strPath, CString strFilePath, bool bMustBeShared) const
{
	...
	for (POSITION pos = thePrefs.sharedsubdir_list.GetHeadPosition(); pos != 0; ) {
		...
		if (strPath.GetLength() > len && strPath[len-1] != _T('\\') && strPath[len] != _T('\\'))
			continue;
		if (CompareDirectories(subdir, strPath.Left(subdir.GetLength())))
			continue;
		return true;
	}
	// Anis: shareSubdir
...
bool CSharedFileList::ExcludeFile(CString strFilePath)
{
	...
	bool bSingleShared = bShared; //>>> taz::Automatic shared files updater [MoNKi/Stulle/Anis]
	...
	// add to exclude list
	if (!bSingleShared) //>>> taz::Automatic shared files updater [MoNKi/Stulle/Anis]
	m_liSingleExcludedFiles.AddTail(strFilePath);
//>>> taz::Automatic shared files updater [MoNKi/Stulle/Anis]
	else 
		theApp.ResetDirectoryWatcher();
//### taz::Automatic shared files updater [MoNKi/Stulle/Anis]
...
void CSharedFileList::CheckAndAddSingleFile(const CFileFind& ff, bool bWithSubdir){
// Anis: shareSubdir
		// Anis START: shareSubdir - recursive call
	...
	else
	{
		...
		// Anis: SafeHash - don't double hash, MY way
...
//>>> Anis::Simple Auto-Reload
void CSharedFileList::ReloadIfNeeded()
{
	if(thePrefs.GetDirectoryWatcherMode() != ASFU_SIMPLE)
		return;

	const DWORD dwCurTick = ::GetTickCount();
	if(dwCurTick - m_dwLastReloaded > MIN2MS(thePrefs.GetASFUfreq()))
