...
public:
	...
	bool	AddSingleSharedFile(const CString& rstrFilePath, bool bNoUpdate, int &iDoAsfuReset);
	...
	void	HashFailed(UnknownFile_Struct* hashed);		// Anis: SafeHash
	...
	//Anis -> non consideriamo directory vuote
	void GetUsefulDirectories(CStringList& dirList) const;
	CMap<CCKey,const CCKey&,CKnownFile*,CKnownFile*> m_Files_map;


protected:
	bool	AddFile(CKnownFile* pFile);
	void	AddFilesFromDirectory(const CString& rstrDirectory, bool bWithSubdir = false);
...
private:
	...
	CTypedPtrList<CPtrList, UnknownFile_Struct*> currentlyhashing_list;	// Anis: SafeHash
