...
void CStatisticsDlg::SetAllIcons()
{
	...
	// Mod Adunanza
	iml.Add(CTempIconLoader(_T("ClientAdunanzA")));			// Adunanza
	// Fine Mod adunanza
...
CString genRatioString( uint64 num, uint64 den )
{
	CString buf(_T("Waiting..."));

	if ( ( num > den ) && (den != 0) )
		buf.Format(_T("%.2f : 1"), (float)num / den );
	else
	{
		if (num != 0)
			buf.Format(_T("1 : %.2f"), (float)den / num );
	}
	return buf;
}
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate) 
{
		...
		if (forceUpdate || stattree.IsExpanded(h_download)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				{ 
					...
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
					cbuffer.Format( _T("KAdu: %u (%.1f%%)") , myStats.a[20], myStats.a[0] ? (myStats.a[20] * 100.0 / myStats.a[0]) : 0.0 );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
					cbuffer.Format( _T("eD2K/KAdu: %u (%.1f%%)") , myStats.a[21], myStats.a[0] ? (myStats.a[21] * 100.0 / myStats.a[0]) : 0.0 );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclinet)) 
		{
			...
			cbuffer.Format( _T("KAdu: %u (%.1f%%)") , myStats[16], totalclient ? (myStats[16] * 100.0 / totalclient) : 0.0 );
			stattree.SetItemText( clinet[1] , cbuffer );
			cbuffer.Format( _T("eD2K/KAdu: %u (%.1f%%)") , myStats[17], totalclient ? (myStats[17] * 100.0 / totalclient) : 0.0 );
	...
	{		
		...
		// Anis: SafeHash - extra statistics
	...
	// AdunanzA Statistics
	// Emanem
	if (forceUpdate || stattree.IsExpanded(h_Adu_main))
	{
		uint64 stat_External_sessionReceivedBytes = theStats.sessionReceivedBytes - theStats.stat_Adu_sessionReceivedBytes;
		uint64 stat_External_sessionSentBytes     = theStats.sessionSentBytes - theStats.stat_Adu_sessionSentBytes;
		cbuffer.Format(  _T("Downloaded Data: %s (External: %s)"), CastItoXBytes( theStats.stat_Adu_sessionReceivedBytes ), CastItoXBytes( stat_External_sessionReceivedBytes ));
		stattree.SetItemText( h_Adu_cur_downloaded , cbuffer );
		cbuffer.Format(  _T("Uploaded Data: %s (External: %s)"), CastItoXBytes( theStats.stat_Adu_sessionSentBytes ),CastItoXBytes( stat_External_sessionSentBytes ));
		stattree.SetItemText( h_Adu_cur_uploaded , cbuffer );
		cbuffer.Format(  _T("Downloaded External/AdunanzA: %s"), genRatioString( stat_External_sessionReceivedBytes,theStats.stat_Adu_sessionReceivedBytes));
		stattree.SetItemText( h_Adu_dlExt_dlAdu , cbuffer );
		cbuffer.Format(  _T("Uploaded External/AdunanzA: %s"), genRatioString(stat_External_sessionSentBytes, theStats.stat_Adu_sessionSentBytes));
		stattree.SetItemText( h_Adu_upExt_upAdu , cbuffer );

		//tigerjact per calcolo media di upload
		//Anis -> 30/01/2012
		theStats.stat_Adu_AvgUP_Ext = (float) (theStats.sessionSentBytes - theStats.stat_Adu_sessionSentBytes)/(((GetTickCount() - theStats.transferStarttime) / 1000)*1024);
		cbuffer.Format(  _T("Media Upload Esterno a Kadu: %.2f KB/s"), theStats.stat_Adu_AvgUP_Ext);
		stattree.SetItemText( h_Adu_upExt_media , cbuffer );
		cbuffer.Format(  _T("AdunanzA Upload/Download: %s"), genRatioString(theStats.stat_Adu_sessionSentBytes, theStats.stat_Adu_sessionReceivedBytes));
		stattree.SetItemText( h_Adu_Adu_updl , cbuffer );
		cbuffer.Format(  _T("External Upload/Download: %s"), 
		genRatioString(stat_External_sessionSentBytes, stat_External_sessionReceivedBytes));
...
void CStatisticsDlg::CreateMyTree() 
{
	...
	// mod Adu
	// Emanem
	h_Adu_main = stattree.InsertItem(_T("AdunanzA"), 18, 18, h_transfer);
	h_Adu_cur_downloaded=stattree.InsertItem(_T("Downloaded Data (Current): %s"),h_Adu_main);
	h_Adu_cur_uploaded=stattree.InsertItem(_T("Uploaded Data (Current): %s"),h_Adu_main);
	h_Adu_dlExt_dlAdu=stattree.InsertItem(_T("Downloaded External/AdunanzA: %s"),h_Adu_main);
	h_Adu_upExt_upAdu=stattree.InsertItem(_T("Uploaded External/AdunanzA: %s"),h_Adu_main);
	h_Adu_upExt_media=stattree.InsertItem(_T("MEDIA Uploaded External: %s"),h_Adu_main);
	h_Adu_Adu_updl=stattree.InsertItem(_T("AdunanzA Upload/Download: %s"),h_Adu_main);
	h_Adu_Ext_updl=stattree.InsertItem(_T("External Upload/Download: %s"),h_Adu_main);
	stattree.SetItemState(h_Adu_main, TVIS_BOLD, TVIS_BOLD);
	// fine mod Adu
