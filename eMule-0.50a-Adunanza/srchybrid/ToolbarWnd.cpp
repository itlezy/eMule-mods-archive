...
void CToolbarWnd::FillToolbar()
{
	...
	//Anis -> AduStreaming
	atb1[18].iBitmap = 14;
	atb1[18].idCommand = MP_STREAMING;
	atb1[18].fsState = TBSTATE_WRAP;
	atb1[18].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[18].iString = m_btnBar->AddString(_T("AdunanzA Streaming Request"));

	atb1[19].iBitmap = 15;
	atb1[19].idCommand = MP_STARTSTREAM;
	atb1[19].fsState = TBSTATE_WRAP;
	atb1[19].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[19].iString = m_btnBar->AddString(_T("Start Streaming"));

	atb1[20].iBitmap = 16;
	atb1[20].idCommand = MP_STOPSTREAM;
	atb1[20].fsState = TBSTATE_WRAP;
	atb1[20].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[20].iString = m_btnBar->AddString(_T("Stop Streaming"));

	atb1[21].iBitmap = 17;
	atb1[21].idCommand = MP_INFO_STREAM;
	atb1[21].fsState = TBSTATE_WRAP;
	atb1[21].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[21].iString = m_btnBar->AddString(_T("Guida Streaming"));
...
LRESULT CToolbarWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	if (bNeedMonoIcons)
	{
		...
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("VLCICON"))));
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("DOWNLOAD"))));
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("UPLOAD"))));
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("INFO"))));
