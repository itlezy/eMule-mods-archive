...
void CTransferWnd::OnInitialUpdate()
{
	...
	m_uWnd2 = (uint8)thePrefs.GetTransferWnd2();
	ShowWnd2(m_uWnd2);
	...
	//Mod Adu - Ded - Icona categorie per download completati - START
	catImg.DeleteImageList();
	catImg.Create(16,16,theApp.m_iDfltImageListColorFlags|ILC_MASK,0,1);
	catImg.SetBkColor(CLR_NONE);
	catImg.Add(CTempIconLoader(_T("DOWNLOAD")));
	m_dlTab.SetImageList(&catImg);
	//Mod Adu - END
...
void CTransferWnd::ShowQueueCount() //Anis -> 1 argomento in meno
{
	TCHAR buffer[100];
	const UINT number = theApp.uploadqueue->GetWaitingUserCount();
	// Stefano Picerno: Aggiunta la visualizzazione del numero di client adu in coda
	_sntprintf(buffer, _countof(buffer), _T("%u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(", %u AdunanzA)"), number, theApp.clientlist->GetBannedCount(), theApp.uploadqueue->GetAdunanzAUserCount() );
...
void CTransferWnd::UpdateListCount(const uint8 listindex)
{
	CString strBuffer = L"";
	switch (m_dwShowListIDC)
	{
...
//					uint32 itemCount = iCount == -1 ? uploadlistctrl.GetItemCount() : iCount;
					const UINT itemCount = uploadlistctrl.GetItemCount();
...
void CTransferWnd::ShowWnd2(const uint8 uWnd2)
{
	switch(uWnd2)
	{
		case wnd2Uploading:
	...
	SetWnd2(uWnd2);
	queuelistctrl.ShowWindow(uWnd2 == wnd2OnQueue);
	clientlistctrl.ShowWindow(uWnd2 == wnd2Clients);
	uploadlistctrl.ShowWindow(uWnd2 == wnd2Uploading);
	downloadclientsctrl.ShowWindow(uWnd2 == wnd2Downloading);
	GetDlgItem(IDC_QUEUE_REFRESH_BUTTON)->ShowWindow(uWnd2 == wnd2OnQueue);
	SetWnd2Icon(uWnd2);			//this works because I changed the order in TransferWsnd.h
	UpdateListCount(m_uWnd2);
}

void CTransferWnd::SetWnd1Icon(const uint8 iIcon)
{
	TBBUTTONINFO tbbi = {0};
...
void CTransferWnd::SetWnd2Icon(const uint8 iIcon)
{
	TBBUTTONINFO tbbi = {0};
	tbbi.cbSize = sizeof tbbi;
	tbbi.dwMask = TBIF_IMAGE;
...
void CTransferWnd::SetWnd2(const uint8 uWnd2)
{
	m_uWnd2 = uWnd2;
...
void CTransferWnd::OnNmRClickDltab(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	menu.AppendMenu((thePrefs.GetCategory(rightclickindex)->catURL.GetLength()==0)? MF_GRAYED:MF_STRING, MP_CAT_URL, GetResString(IDS_CATURL), _T("WEB"));  //Mod Adu - Ded - URL per categorie
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{ 
		...
		//Mod Adu - Ded - URL per categorie - START
		case MP_CAT_URL:
			if (thePrefs.GetCategory(rightclickindex)->catURL.GetLength() > 0)
				ShellExecute(NULL, NULL, thePrefs.GetCategory(rightclickindex)->catURL, NULL, NULL, SW_SHOWDEFAULT);
			break;
		//Mod Adu - END
...
void CTransferWnd::EditCatTabLabel(int index,CString newlabel)
{
	...
	tabitem.mask = TCIF_IMAGE | TCIF_TEXT; //Mod Adu - Ded - Icona categorie per download completati
...
int CTransferWnd::AddCategory(CString newtitle,CString newincoming,CString newcomment, CString newautocat, bool addTab)
{
	...
	newcat->catURL = ""; //Mod Adu - Ded - URL per categorie
