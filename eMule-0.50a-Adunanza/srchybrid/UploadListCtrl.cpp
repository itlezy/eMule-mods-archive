...
void CUploadListCtrl::Init()
{
	...
	InsertColumn(8, GetResString(IDS_DL_REMAINS),	LVCFMT_LEFT,  DFLT_DATARATE_COL_WIDTH); // VQB: fullChunk
...
void CUploadListCtrl::Localize()
{
	...
	const UINT resStrings[] = 
	{
		...
		IDS_STATUS,
		IDS_UPSTATUS,
		IDS_DL_REMAINS, // VQB: fullChunk
	};

	for (int icol=0; icol < pHeaderCtrl->GetItemCount(); icol++) 
	{
		strRes = GetResString(resStrings[icol]);
		strRes.Remove(L':');
...
void CUploadListCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("ClientAdunanzA"))); // 15
	m_ImageList.Add(CTempIconLoader(_T("ClientFastWeb")));  // 16
...
void CUploadListCtrl::GetItemDisplayText(const CUpDownClient *client, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 4:
			if (client->HasLowID()) 
			{
				if(client->HasFastwebIP())
					...
					_sntprintf(pszText, cchTextMax, _T("%s (%s)"), CastSecondsToHM(client->GetWaitTime() / 1000), _T("ID Basso"));
			}	
			else
		...
		case 8:
		{
			...
			{
				sBuffer.Format(_T("%s (%s)"), CastItoXBytes(remain), GetResString(IDS_UNKNOWN));
			}
			else
			{
				sint32 timeleft = (sint32)((float)remain / (float)client->GetDatarate());
				sBuffer.Format(_T("%s (%s)"), CastItoXBytes(remain), CastSecondsToHM(timeleft));
			}
...
int CUploadListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	switch (iColumn)
	{
		...
		// VQB: fullChunk
		case 8:
			iResult = CompareUnsigned(item1->GetRemainingUp(), item2->GetRemainingUp());			
			break;
		// VQB: fullChunk
	}
