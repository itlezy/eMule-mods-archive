...
#endif
#include "resource.h"
#include "ReadWriteLock.h" //Anis -> hash sicuro
#include "uPnP_NAT.h" //Anis -> UPNP NAT

#define	DEFAULT_NICK		_T("http://www.adunanza.net")
...
public:
	...
	CReadWriteLock		m_threadlock; //Anis -> hash sicuro
	void RebindUPnP(); //Anis -> UPNP NAT 
	CUPnP_IGDControlPoint *m_UPnP_IGDControlPoint; //Anis -> UPNP NAT
	// ZZ:UploadSpeedSense -->
	...
	CIPFilter*			ipfilter;
	CRemoteIPFilter*	remoteipfilter;
...
// Implementierung
	...
	bool		IsFirewalled();
	// Stefano Picerno: Come IsFirewalled, ma fa controlli aggiuntivi sul client specificato
	bool		IsFirewalledClient(CUpDownClient* client);
	// Fine Modifica
...
private:
	...
	static UINT __cdecl			CheckDirectoryForChangesThread(LPVOID pParam);
public:
	void ResetDirectoryWatcher();
	void EndDirectoryWatcher();
	void DirectoryWatcherExternalReload();
//Anis AutoDropImmunity
