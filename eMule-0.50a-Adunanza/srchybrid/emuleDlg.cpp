...
#include "SafeFile.h"
#include "HttpComm.h"
#include "Dbt.h" //Anis -> refresh automatico libreria file condivisi
#include "HttpDownloadDlg.h" //Anis
#include "AduWizard.h"//Anis
#include "AduWebBrowser.h" //Anis AduBrowser
...
UINT CemuleDlg::CheckNodes(LPVOID lpParameter) { //Anis -> Funzione che gestisce lo scaricamento dei nodi automatico
  try {
	  while (true) {
	  	::Sleep(60000);
		if(WizardNotOpen) {
			if (Kademlia::CKademlia::IsRunning()) 
			{
				...
				{
					Kademlia::CKademlia::Stop(); // Anis -> Stoppo Kadu
					DeleteFile(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + _T("nodes.dat"));
					//Kademlia::CKademlia::m_pInstance->m_pRoutingZone->ReadFile(); //Anis -> alternativa che si poteva usare.
					...
					dlgDownload.DoModal();
					Kademlia::CKademlia::Start(); // Anis -> riavvio KAdu per far leggere nodes.dat
				}
			}
		}
	}
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	if(show_transfer)
	{
		SetActiveDialog(transferwnd); //Anis -> trasferimenti come interfaccia iniziale
		theApp.AddEd2kLinksToDownload(theApp.temp_link, 0);
		show_transfer = false;
	}
	else
		SetActiveDialog(serverwnd); //Anis -> Server come interfaccia iniziale
	...
	// anchors
	AddAnchor(*iebrowser,		TOP_LEFT, BOTTOM_RIGHT); //Anis
	...
	try
	{
		switch(theApp.emuledlg->status)
		{
			...
			case 5:
				...
				theApp.emuledlg->status++;
				break;
			// Anis: SafeHash - delay load shared files
			case 6:
				theApp.emuledlg->status++;
				theApp.sharedfiles->SetOutputCtrl(&theApp.emuledlg->sharedfileswnd->sharedfilesctrl);
			...
			default:
				...
				// wait until emule is ready before opening the Anis
				if (thePrefs.IsFirstStart())
				{
...
void CemuleDlg::ShowTransferStateIcon()
{
	...
	if (m_uUpDatarate > 1000 && m_uDownDatarate > 1000)  // Anis -> da risultati un pò sballati il mulo nativo. Sistemiamoli.
		statusbar->SetIcon(SBarUpDown, transicons[3]);
	else if (m_uUpDatarate > 1000) // Anis
		statusbar->SetIcon(SBarUpDown, transicons[2]);
	else if (m_uDownDatarate > 1000) // Anis
...
LRESULT CemuleDlg::OnFileHashed(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam)
	{
		...
		// Anis: SafeHash - could have been canceled
...
void InviaStatisticheAnonime()
{
	...
	if (thePrefs.m_AduSendStats && theApp.rm->enableAduStats)
	{
		...
		sprintf(aduAnonymStats, "up_val=%i&band_val=%i&sent_val=%I64u&recv_val=%I64u&sent_adu_val=%I64u&recv_adu_val=%I64u&time_val=%i&adu_var=%i.%i&conn_var=%i&city_var=%i&ads=%i&bw_ext_aut=%i&bw_up_ext=%u&bw_up_ext_userset=%u",
			...
			thePrefs.m_AduRipBanda ? 0 : 1, //0 se la modalità automatica è attiva
...
// Anis -> Riscritta da 0
/* Anis -> La gestione della chiusura di eMule AdunanzA è stata oggetto di numerose revisioni in parte dovute al forte cambiamento del codice dalle precedenti versioni.
è stato quindi necessario un riadattamento del codice e una divisione di esso su più procedure e/o funzioni.
Il fatto che la deallocazione della memoria è stata rimossa per alcuni puntatori ad oggetti è normale poichè di ciò se ne occupa il sistema operativo stesso nella fase di distruzione del processo.
...
void CemuleDlg::OnClose()
{
	...
	DestroyMiniMule();
	m_wndTaskbarNotifier->DestroyWindow(); //Anis - CrashFIX di chiusura. sarà almeno il decimo che sistemo.... =.=
...
void CemuleDlg::Localize()
{
	...
	::AfxBeginThread(RefreshStatsBar, NULL, 0, 0, 0, NULL);  //Anis -> Controllo IP nuovi.
	ShowUserStateIcon();
	toolbar->Localize();
	ShowConnectionState();
	ShowTransferRate(true);
...
void CemuleDlg::StartBrowser()
{
	...
	COleVariant vEmpty;
	LPCTSTR vURL;
	vURL = theApp.rm->HomeAduBrowser; //Anis -> HomePage AdunanzA ;)
	...
	switch (wParam) {
		case PBT_APMRESUMEAUTOMATIC:
		{
			if (m_bEd2kSuspendDisconnect || m_bKadSuspendDisconnect)
			{
				...
				theApp.RebindUPnP(); //Anis integriamo l'UPNP compatibile con NAT
