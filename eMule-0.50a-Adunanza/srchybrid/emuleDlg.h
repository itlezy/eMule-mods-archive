...
public:
    ...
    void ShowPing();
	void Localize();
	static UINT CheckNodes(LPVOID lpParameter);
	...
	static UINT RefreshStatsBar(LPVOID lpParameter);
protected:
	HICON			m_hIcon;
	bool			ready;
	...
	void ShowSplash();
	void DestroySplash();
	CCriticalSection crt;  //Anis -> ThreadSync
	// Mini Mule
