...
CIndexed::CIndexed()
{
	...
	m_sSourceFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR);
	m_sSourceFileName.Append(_T("src_index.adu.dat"));
	//tigerjact
	//m_sKeyFileName = CMiscUtils::GetAppDir();
	//m_sKeyFileName.Append(CONFIGFOLDER);
	m_sKeyFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR);
	m_sKeyFileName.Append(_T("key_index.adu.dat"));
	//tigerjact
	//m_sLoadFileName = CMiscUtils::GetAppDir();
	//m_sLoadFileName.Append(CONFIGFOLDER);
	m_sLoadFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR);
	m_sLoadFileName.Append(_T("load_index.adu.dat"));
	...
	try
	{
		...
		if (fileKey.Open(m_sKeyFileName, CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite))
		{
			...
			if( uVersion < 3 )
			{
				...
				if( tSaveTime + theApp.rm->kadIndexLifeK > now ) // lupz Mod scadenza
				{
								...
								while( uNumName )
								{
									...
									while( uTotalTags )
									{
										...
										if(pTag)
										{
											...
											else if ( !pTag->m_name.Compare(TAG_KADFIRSTPUBLISH) || 
											            !pTag->m_name.Compare(TAG_KADLASTPUBLISH) ||
														!pTag->m_name.Compare(TAG_KADCOMPLETE_FIRSTPUBLISH) ||
														!pTag->m_name.Compare(TAG_KADCOMPLETE_LASTPUBLISH))
											{
												uint32 tmpValue;
						                        CKadTagNameString tmpTagName = CKadTagNameString(pTag->m_name);
												tmpValue = pTag->GetInt() + (now - tSaveTime);
												delete pTag;
												pTag = new CKadTagUInt32(tmpTagName, tmpValue);
												pToAdd->m_listTag.push_back(pTag);											
											}
											// Fine mod adu
											else
									...
									uint8 uLoad;
									if(AddKeyword(uKeyID, uSourceID, pToAdd, uLoad, false, true))
										uTotalKeyword++;
									else
										delete pToAdd;
									uNumName--;
		...
		if (fileSource.Open(m_sSourceFileName, CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite))
		{
			...
			if( uVersion < 3 )
			{
				...
				if( tSaveTime + theApp.rm->kadIndexLifeS > time(NULL) ) // lupz Mod scadenza
				{
	...
	if (!m_bDataLoaded){
		...
		try
		{
			...
			if (fileSource.Open(m_sSourceFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				// Mod Adu
				// lupz
				// memorizzo la data di salvataggio e non la data di scadenza
				//fileSource.WriteUInt32(time(NULL)+theApp.rm->kadRepublishTimeS);
				fileSource.WriteUInt32(time(NULL));
				// Fine Mod Adu	
			...
			if (fileKey.Open(m_sKeyFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				// lupz
				// memorizzo la data di salvataggio e non la data di scadenza
				//fileKey.WriteUInt32(time(NULL)+theApp.rm->kadRepublishTimeK);
				fileKey.WriteUInt32(time(NULL));
				// Fine Mod adu
...
bool CIndexed::AddKeyword(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CKeyEntry* pEntry, uint8& uLoad, bool bIgnoreThreadLock, bool loading)
{	
	...
	// Mod Adu
	// lupz
	// al primo publish c'è solo una fonte
	...
	else
	{
		...
		if(pCurrKeyHash->mapSource.Lookup(CCKey(uSourceID.GetData()), pCurrSource))
		{
			...
			if (pCurrSource->ptrlEntryList.GetCount() > 0)
			{
				...
				// PRIMO STIMATORE
				//
				// detto in parole povere se ricevo 20 publish per quel file negli ultimi REPUBLISHTIMEK secondi
				// la stima sarà 20
				//
				// questo algoritmo infatti calcola infatti il cosidetto moving average del publish rate
				// su una finestra di REPUBLISHTIMEK secondi
				//
				// in precedenza avevo usato qualcosa di simile all'EWMA (Exponentially Weighted Moving Average)
				// ma visto che richiedeva molto tempo per arrivare a regime ne ho sostanzialmente inventato
				// uno molto più efficiente
				//
				// il pricipio base è il seguente sommo una fonte ad ogni publish e calcolo la frequenza di publish
				// quando saranno passati REPUBLISHTIMEK inizio a eliminare i publish vecchi secondo il rate stimato
				// i risultati sono estremamente validi (simulando l'algoritmo si ottengono risultati ottimi)
				//
				// l'errore tende a circa il 5-10% dopo 1/3 del REPUBLISHTIMEK
				//
				// inoltre riallineando le date FIRSTPUBLISH e LASTPUBLISH all'avvio i client con poco uptime
				// non sballeranno la loro stima ma impiegheranno solo più tempo ad effettuarne una accurata
				//
				// con un trucco riusciamo a supportare anche eventuali cambiamenti del REPUBLISHTIMEK
				// (con qualche oscillazione alla riconfigurazione)
				
				//Anis -> Ma cristo cosa si dichiarano cose che non devono essere usate?!
				currEntry = pCurrSource->ptrlEntryList.GetHead();
				avail = currEntry->GetFloatTagValue(TAG_KADSOURCES);
				firstPublish = currEntry->GetIntTagValue(TAG_KADFIRSTPUBLISH);
				uint32 publishInterval = (uint32)(now - currEntry->GetIntTagValue(TAG_KADLASTPUBLISH));
				uint32 pubkRTK = currEntry->GetIntTagValue(TAG_KADREPUBLISHTIMEK);
		        avail = CalcolaStima(avail, firstPublish, publishInterval, pubkRTK, pEntry->m_uIP == currEntry->m_uIP, now);
				cavail = currEntry->GetFloatTagValue(TAG_KADCOMPLETE_SOURCES);
				cfirstPublish = currEntry->GetIntTagValue(TAG_KADCOMPLETE_FIRSTPUBLISH);
				uint32 complete = currEntry->GetIntTagValue(TAG_KADCOMPLETE);
  
				if (complete) {
					uint32 cpublishInterval = (uint32)(now - currEntry->GetIntTagValue(TAG_KADCOMPLETE_LASTPUBLISH));
				...
				AddDebugLogLine(DLP_LOW, false, _T("Stimate %.0f (Normalizzate: %.0f) con il publish di %s: ed2k://|file|%s|%u|%s|/"),
					avail,
					NormalizzaStima(avail, firstPublish, now),
					ipstr(ntohl(pEntry->m_uIP)),
					pEntry->m_fileName,
					pEntry->m_uSize,
					str );
#endif
			...
			pEntry->m_listTag.push_back(new CKadTagFloat(TAG_KADSOURCES, avail));
			pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADFIRSTPUBLISH, now));
			pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADLASTPUBLISH, now));
			pEntry->m_listTag.push_back(new CKadTagFloat(TAG_KADCOMPLETE_SOURCES, cavail));
			pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADCOMPLETE_FIRSTPUBLISH, now));
			pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADCOMPLETE_LASTPUBLISH, now));
			pCurrSource->ptrlEntryList.AddHead(pEntry);
			return true;
		}
		else
		{
			...
			// Mod Adu
			// lupz
			// se non sto caricando da file (dove ha già messo i tag)
			// setto i tag necessari alla stima
			if (!loading) {
				pEntry->m_listTag.push_back(new CKadTagFloat(TAG_KADSOURCES, avail));
				pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADFIRSTPUBLISH, now));
				pEntry->m_listTag.push_back(new CKadTagUInt32(TAG_KADLASTPUBLISH, now));
...
bool CIndexed::AddSources(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CEntry* pEntry, uint8& uLoad, bool bIgnoreThreadLock)
{
	...
	else
	{
		...
		for(POSITION pos1 = pCurrSrcHash->ptrlistSource.GetHeadPosition(); pos1 != NULL; )
		{
			...
			if( pCurrSource->ptrlEntryList.GetSize() )
			{
				...
				if( pCurrEntry->m_uIP == pEntry->m_uIP && ( pCurrEntry->m_uTCPPort == pEntry->m_uTCPPort || pCurrEntry->m_uUDPPort == pEntry->m_uUDPPort ))
				{
					...
					pCurrSource->ptrlEntryList.AddHead(pEntry);
					uLoad = (uint8)((uSize*100)/theApp.rm->kadMaxSrcFile);
					return true;
				}
			}
			else
		...
		if( uSize > theApp.rm->kadMaxSrcFile )
		{
...
bool CIndexed::AddNotes(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CEntry* pEntry, uint8& uLoad, bool bIgnoreThreadLock)
{
	...
	else
	{
		...
		for(POSITION pos1 = pCurrNoteHash->ptrlistSource.GetHeadPosition(); pos1 != NULL; )
		{
			...
			if( pCurrNote->ptrlEntryList.GetSize() )
			{
				...
				if(pCurrEntry->m_uIP == pEntry->m_uIP || pCurrEntry->m_uSourceID == pEntry->m_uSourceID)
				{
					...
					pCurrNote->ptrlEntryList.AddHead(pEntry);
					uLoad = (uint8)((uSize*100)/theApp.rm->kadMaxNotFile);
					return true;
				}
			}
			else
		...
		if( uSize > theApp.rm->kadMaxNotFile )
		{
...
void CIndexed::SendValidKeywordResult(const CUInt128& uKeyID, const SSearchTerm* pSearchTerms, uint32 uIP, uint16 uPort, bool bOldClient, uint16 uStartPosition, CKadUDPKey senderUDPKey)
{
	...
	if(m_mapKeyword.Lookup(CCKey(uKeyID.GetData()), pCurrKeyHash))
	{
		...
		byIO.WriteByte(theApp.rm->kadOpcode);
							...
							while( uNumKeys && !m_pOwner->m_bAbortLoading )
							{
								...
								while( uNumSource && !m_pOwner->m_bAbortLoading )
								{
									...
									while( uNumName && !m_pOwner->m_bAbortLoading)
									{
										...
										uint8 uLoad;
										if(m_pOwner->AddKeyword(uKeyID, uSourceID, pToAdd, uLoad, true, true))
											uTotalKeyword++;
										else
											delete pToAdd;
										uNumName--;
