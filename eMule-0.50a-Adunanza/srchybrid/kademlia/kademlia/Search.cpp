...
void CSearch::ProcessResponse(uint32 uFromIP, uint16 uFromPort, ContactList *plistResults)
{
	...
	if (plistResults->size() > GetRequestContactCount() && !(pRequestedMoreNodesContact == pFromContact && plistResults->size() <= theApp.rm->kadFindValue) )
	{
...
void CSearch::ProcessResultKeyword(const CUInt128 &uAnswer, TagList *plistInfo, uint32 uFromIP, uint16 uFromPort)
{
	...
	uint32 kadavailability = 0;
	uint32 ckadavailability = 0;
 	uint32 first = 0;
	uint32 last = 0;
	...
	for (TagList::const_iterator itTagList = plistInfo->begin(); itTagList != plistInfo->end(); ++itTagList)
	{
		...
		else if (!pTag->m_name.Compare(TAG_FILESIZE))
		{
			...
			sCodec = pTag->GetStr();
 			...
 			first = pTag->GetInt();
 		else if (!pTag->m_name.Compare(TAG_KADLASTPUBLISH))
 			last = pTag->GetInt();
 		else if (!pTag->m_name.Compare(TAG_KADSOURCES))
 			kadavailability = pTag->GetFloat();
 		else if (!pTag->m_name.Compare(TAG_KADCOMPLETE_SOURCES))
 			ckadavailability = pTag->GetFloat();
...
/*#ifdef _DEBUG
	...
	kadavailability = sqrt((sqrt(NormalizzaStima(kadavailability, first, last))*kadavailability)); //Anis -> My new formula
	...
	if (ckadavailability > 65500)
		ckadavailability = 65500;
	
	//Anis -> incrementiamo di uno al fine di cammuffare quel falso valore in cui ci vengono mostrate 0 fonti complete e 0 fonti. Se un file viene trovato, deve avere almeno una fonte.
	if (ckadavailability < 1)
		ckadavailability++;
	
	if (kadavailability < 1)
		kadavailability++;
 	// Fine Mod Adu
	...
	theApp.searchlist->KademliaSearchKeyword(m_uSearchID, &uAnswer, sName, uSize, sType, uPublishInfo
		, aAICHHashs, aAICHHashPopularity, m_pSearchTerm, 10,
		    ...
		    3, TAG_SOURCES, (uint32)kadavailability,
			3, TAG_COMPLETE_SOURCES, (uint32)ckadavailability,
			3, TAG_KADSOURCES, uAvailability); // arrotondiamo per difetto
	...
	try
	{
		...
		if (bReAskMore)
		{
			if (pRequestedMoreNodesContact == NULL)
			{
				...
				ASSERT( byContactCount == theApp.rm->kadFindValue);
				byContactCount = theApp.rm->kadFindValue;
			}
			else
				ASSERT( false );
		}
...
void CSearch::PreparePacketForTags(CByteIO *byIO, CKnownFile *pFile, uint8 byTargetKadVersion)
{
	...
	try
	{
		if (pFile && byIO)
		{
			...
			listTag.push_back(new CKadTagUInt(TAG_KADREPUBLISHTIMEK, theApp.rm->kadRepublishTimeK));
 			listTag.push_back(new CKadTagUInt(TAG_KADCOMPLETE, ( pFile->IsPartFile() ? 0 : 1 )));
...
uint8 CSearch::GetRequestContactCount() const
{
	// Returns the amount of contacts we request on routing queries based on the search type
		switch(m_uType)
		{
			...
			case NODESPECIAL:
			case NODEFWCHECKUDP:
				return theApp.rm->kadFindNode;
				break;
			case FILE:
			case KEYWORD:
			case FINDSOURCE:
			case NOTES:
				return theApp.rm->kadFindValue;
				break;
			case FINDBUDDY:
			case STOREFILE:
			case STOREKEYWORD:
			case STORENOTES:
				return theApp.rm->kadStore;
				break;
			default:
