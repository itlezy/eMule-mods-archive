...
public:
	...
	bool			HasValidHash() const							{ return ((const uint32*)m_achUserHash)[0] != 0 || ((const uint32*)m_achUserHash)[1] != 0 || ((const uint32*)m_achUserHash)[2] != 0 || ((const uint32*)m_achUserHash)[3] != 0; }
	...
	bool			AddReqBlock(Requested_Block_Struct* reqblock, const bool bChecksNecessary = true); //>>> Anis::Optimization
...
//	bool			HasCollectionUploadSlot() const					{ return m_bCollectionUploadSlot; }
	bool			HasSmallFileUploadSlot() const;
//### Anis::Small File Slot
...
private:
	...
	uint32	m_nRemainingUp;	// VQB: fullChunk
public:
	// VQB: fullChunk
	uint32			GetRemainingUp() const		{return m_nRemainingUp;}
	void			ResetRemainingUp();
...
protected:
    ...
    UINT		m_slotNumber;
	//bool		m_bCollectionUploadSlot; Anis -> non serve più
...
public :
	//Anis Percentuale Client (xtreme emule mod)
	uint32 droptime; // Ottimizzazione gestione upload Anis
	sint8	hiscompletedparts_percent_up;
	sint8	GetHisCompletedPartsPercent_UP() const		{return hiscompletedparts_percent_up;}

	DWORD GetIsAduSoftware(void) const {return isAduSoftware;}
	void SetIsAduSoftware(DWORD in = 0){isAduSoftware = in;}

	// Mod Adu
	// lupz
	// definisco i client adunanza vecchi
	bool  IsOldAdu(uint32 oldver = ADU_VER_CODE(ADU_VER_MAJ,ADU_VER_MIN,0)) const {return isAduSoftware && aduver < oldver;}
	DWORD GetClientAduType(void) const;
	bool  HasFastwebIP() const;
	bool  IsAduClient() const;
private:
	// Mod Adu
	// lupz
	// versione del modifica adunanza
	uint32 aduver;
	DWORD isAduSoftware;
	// fine mod Adu
};
