...
//>>> WiZaRd::XS-Workaround
	...
	const UINT uSourceExchangeVer	= (m_bySourceExchangeVer != 0 && m_bySourceExchangeVer < 4) ? m_bySourceExchangeVer : 4;
//<<< WiZaRd::XS-Workaround
...
void CUpDownClient::InitClientSoftwareVersion()
{
	...
	if (m_bEmuleProtocol || iHashType == SO_EMULE){
		...
		switch(m_byCompatibleClient){
			...
			default:
				if (m_bIsML || m_byCompatibleClient == SO_MLDONKEY || m_byCompatibleClient == SO_MLDONKEY2 || m_byCompatibleClient == SO_MLDONKEY3){ // Spike2 - Enhanced Client Recognization
					m_clientSoft = SO_MLDONKEY;
					pszSoftware = _T("MLdonkey");
				}
				else if (m_bIsHybrid || m_byCompatibleClient == SO_EDONKEYHYBRID){ // Spike2 - Enhanced Client Recognization
					...
					} // Spike2 - Enhanced Client Recognization
				}
				else{
	...
	if (m_bIsHybrid || m_byCompatibleClient == SO_EDONKEYHYBRID){ // Spike2 - Enhanced Client Recognization
		m_clientSoft = SO_EDONKEYHYBRID;
		// seen:
		// 105010	0.50.10
		// 10501	0.50.1
...
// Spike2 - Enhanced Client Recognization - START (Thanx to WiZaRd for this one !)
...
*/
// WiZaRd::FiX
//you should never "append" via + operator! really bad style AND dangerous!
	CString ret = _T("");
	if (m_strModVersion.IsEmpty())
