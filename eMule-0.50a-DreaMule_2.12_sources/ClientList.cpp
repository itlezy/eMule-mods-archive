		...
		if (cur_client->Credits() != NULL)
		{
			switch (cur_client->Credits()->GetCurrentIdentState(cur_client->GetIP()))
			{
				case IS_IDENTIFIED:
					...
					break;
				//>>> WiZaRd 
				//I never understood why to count IS_NEEDED as failed... 
				case IS_IDNEEDED:
					break; 
				//<<< WiZaRd 
				case IS_IDFAILED:
				//case IS_IDNEEDED: // WiZaRd: see above
				case IS_IDBADGUY:
					stats[13]++;
					break;
			}
...
void CClientList::RemoveFromKadList(CUpDownClient* torem){
	...
	if(pos)
	{
		...
		{
			m_pBuddy = NULL;
			m_nBuddyStatus = false; //>>> WiZaRds Missing Buddy Check // updated by Spike2
			theApp.emuledlg->serverwnd->UpdateMyInfo();
		}
