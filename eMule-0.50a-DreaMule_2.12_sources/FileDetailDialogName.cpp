...
BEGIN_MESSAGE_MAP(CFileDetailDialogName, CResizablePage)
	...
	ON_BN_CLICKED(IDC_RESETFDC, OnBnClickedResetfdc)
END_MESSAGE_MAP()
	...
	: CResizablePage(CFileDetailDialogName::IDD, 0)
{
	...
	m_sortindex = 1;
	m_bSelf = false;
	m_bResetFDC = false; //[FDC] for apply TK4 Mod version 1.3d
}
...
BOOL CFileDetailDialogName::OnInitDialog()
{
	...
	AddAnchor(IDC_RESETFDC, BOTTOM_RIGHT); //TK4 Mod [FDC] Anchor reset button
	...
	//TK4 Mod 1.3d [FDC] activate reset button if disparity check set 	
	CPartFile* file = STATIC_DOWNCAST(CPartFile, (*m_paFiles)[0]);
    GetDlgItem(IDC_RESETFDC)->EnableWindow(file->DissimilarName());
	//TK4 Mod [FDC] end
...
void CFileDetailDialogName::Localize()
{
	if (thePrefs.GetLanguageID() != MAKELANGID(LANG_ENGLISH,SUBLANG_DEFAULT)){
	    ...
	    GetDlgItem(IDC_RESETFDC)->SetWindowText(GetResString(IDS_RESETFDC)); // [FDC] button text
    }
}
...
BOOL CFileDetailDialogName::OnApply()
{
	if (!m_bDataChanged)
		RenameFile();
	if(m_bResetFDC){//[FDC] Apply the reset TK4 Mod version 1.3d
		            CPartFile* file = STATIC_DOWNCAST(CPartFile, (*m_paFiles)[0]);
                    file->ResetFDC();
					}
	return CResizablePage::OnApply();
}
/*Reset File Name Disparity Check flag [FDC] TK4 Mod 1.3d*/
void CFileDetailDialogName::OnBnClickedResetfdc()
{
     m_bResetFDC = true; //[FDC] Set 'true' for applying the reset TK4 Mod version 1.3d
	 SetModified();
}