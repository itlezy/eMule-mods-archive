...
CPreferencesDlg::CPreferencesDlg()
{
	...
	m_wndeXtra.m_psp.dwFlags &= ~PSH_HASHELP;  //TK4 Mod
	...
	CTreePropSheet::SetPageIcon(&m_wndeXtra, _T("EXTRA"));  //TK4 Mod
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
...
void CPreferencesDlg::Localize()
{
	...
	if (pTree)
	{
		...
		pTree->SetItemText(GetPageTreeItem(c++), RemoveAmbersand(GetResString(IDS_EXTRAS)));				//new item extras starting with hotkey hide TK4 Mod
	#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
