...
{ 
	...
	int iCanConnect = iSelectedItems; //>>> WiZaRd::Obfuscation FiX
	while (pos)
	{
		const CServer* pServer = (CServer*)GetItemData(GetNextSelectedItem(pos));
//>>> WiZaRd::Obfuscation FiX
...
//>>> WiZaRd::Obfuscation FiX
	...
	ServerMenu.AppendMenu(MF_STRING | (iSelectedItems > 0 && iCanConnect > 0 ? MF_ENABLED : MF_GRAYED), MP_CONNECTTO, GetResString(IDS_CONNECTTHIS), _T("CONNECT"));
//<<< WiZaRd::Obfuscation FiX
