...
void CAddSourceDlg::OnBnClickedRadio1()
{
	...
	GetDlgItem(IDC_NAT_PORT)->EnableWindow(true);
...
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
	GetDlgItem(IDC_NAT_PORT)->EnableWindow(false);
#endif //NATTUNNELING // NEO: UTCP END <-- Xanatos --
	...
	switch (m_nSourceType)
	{
		case 0:
		{
			...
			if (IsGoodIPPort(ip, port)
			 ...
			 || theApp.lancast->IsLanIP(ip) // LAN clients would always be filtered, so don't filter identified LanCast users
#endif //LANCAST // NEO: NLC END <-- Xanatos --
			 )
			{
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
				CUpDownClient* toadd;
				if(IsDlgButtonChecked(IDC_NAT_PORT)){
					toadd = new CUpDownClient(m_pFile,0,1,0,0,false);
					toadd->SetUDPPort(port);
					toadd->SetConnectIP(ip);
					toadd->SetNatTraversalSupport(true);
				}else
					toadd = new CUpDownClient(m_pFile, port, ntohl(ip), 0, 0);
#else
