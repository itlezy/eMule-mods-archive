...
//>>> WiZaRd::ShareFilter
			...
			if (!rastrFilter.IsEmpty())
			{
				...
				GetItemDisplayText(toadd, theApp.emuledlg->sharedfileswnd->GetFilterColumn(), szFilterTarget, _countof(szFilterTarget));
...
//>>> WiZaRd::ShareFilter
				...
				if (!rastrFilter.IsEmpty())
				{
					// filtering is done by text only for all columns to keep it consistent and simple for the user
					TCHAR szFilterTarget[256];
					GetItemDisplayText(cur_file, theApp.emuledlg->sharedfileswnd->GetFilterColumn(), szFilterTarget, _countof(szFilterTarget));
					for (int i = 0; i < rastrFilter.GetSize(); ++i)
					{
						const CString& rstrExpr = rastrFilter.GetAt(i);
						bool bAnd = true;
...
//>>> WiZaRd::ShareFilter
void CHistoryListCtrl::GetItemDisplayText(const CKnownFile* file, const int iSubItem, LPTSTR pszText, const int cchTextMax) const
{
	...
	switch (iSubItem)
	{
		...
		case 3:
			...
			break;
		case 4:
			buffer = file->GetUtcCFileDate().Format(L"%x %X");
			break;
		case 5:
			if (theApp.sharedfiles->IsFilePtrInList(file))
