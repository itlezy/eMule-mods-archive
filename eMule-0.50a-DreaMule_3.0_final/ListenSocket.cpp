...
bool CClientReqSocket::Create()
{
	...
	if(thePrefs.ReuseTCPPort())
		return (CAsyncSocketEx::Create(thePrefs.GetPort(true), SOCK_STREAM, FD_WRITE | FD_READ | FD_CLOSE | FD_CONNECT, thePrefs.GetBindAddrA(), TRUE) != FALSE);
	else
...
bool CListenSocket::StartListening(){
	...
	{
		AddLogLine(false,_T("Você especificou um Ip para Ligar. Tente ligar em: %s"), thePrefs.GetBindAddrW());//DreamTradução
	}
	//Xman Info about binding

#ifdef NATTUNNELING // NEO: RTP - [ReuseTCPPort] -- Xanatos -->
	bool ret=Create(thePrefs.GetPort(true), SOCK_STREAM, FD_ACCEPT, thePrefs.GetBindAddrA(), FALSE/*bReuseAddr*/) && Listen();
#else
	...
	{
		AddLogLine(false,_T("binding feito com sucesso"));//DreamTradução
	}
	//Xman Info about binding
...
void CListenSocket::Process()
{
	...
	for(POSITION pos = socket_list.GetHeadPosition(); pos != NULL; ){
		...
		if(cur_sock->deletethis == true){
			//Xman improved socket closing
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
			if(cur_sock->m_SocketData.hSocket != NULL && cur_sock->m_SocketData.hSocket != INVALID_SOCKET || cur_sock->HaveNatLayer(true)){
#else
