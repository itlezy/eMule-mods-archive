...
void CMuleStatusBarCtrl::OnLButtonDblClk(UINT /*nFlags*/, CPoint point)
{
	...
	{
		case SBarLog:
			AfxMessageBox(_T("DreaMule ") + GetResString(IDS_SV_LOG) + _T("\n\n") + GetText(SBarLog));
			break;
...
//>>> WiZaRd::ShutDownIfFinished
		case SBarSysInfo:
			thePrefs.SetShutDownIfFinished(!thePrefs.IsShutDownIfFinished());
...
CString CMuleStatusBarCtrl::GetPaneToolTipText(EStatusBarPane iPane) const
{
	...
	switch (iPane)
	{
	case SBarServer: // X-Ray :: Statusbar
...
//>>> WiZaRd::ShutDownIfFinished
	case SBarSysInfo:
		strText.Format(L"%s %s", GetResString(IDS_SHUTDOWNIFFINISHED), GetResString(thePrefs.IsShutDownIfFinished() ? IDS_YES : IDS_NO));
		break;
//<<< WiZaRd::ShutDownIfFinished
	}
	return strText;
