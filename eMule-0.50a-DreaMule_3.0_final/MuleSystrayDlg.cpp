...
BOOL CMuleSystrayDlg::OnInitDialog()
{
	...
	if(p)
	{
		...
		m_ctrlSpeed.m_hIcon = theApp.LoadIcon(_T("BTVERYCD"), m_ctrlSpeed.m_sIcon.cx, m_ctrlSpeed.m_sIcon.cy);
...
*/
	...
	//DreaMule open shared folder on tray
	p = GetDlgItem(IDC_TRAY_OPENFOLDER);
	if(p)
	{
		p->GetWindowRect(r);
		ScreenToClient(r);
		m_ctrlPreferences.Create(NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_TRAY_OPENFOLDER);
		m_ctrlPreferences.m_nBtnID = IDC_TRAY_OPENFOLDER;
		//p->GetWindowText(m_ctrlPreferences.m_strText);
		m_ctrlPreferences.m_strText = GetResString(IDS_TRAY_OPENFOLDER);
		...
		m_ctrlPreferences.m_hIcon = theApp.LoadIcon(_T("FOLDERS"), m_ctrlPreferences.m_sIcon.cx, m_ctrlPreferences.m_sIcon.cy);
	...
	m_ctrlSidebar.SetWindowText(_T("DreaMule "));
