...
BEGIN_MESSAGE_MAP(CPPgFiles, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_SHUTDOWNIFFINISHED, OnSettingsChange) //>>> WiZaRd::ShutDownIfFinished
END_MESSAGE_MAP()
...
void CPPgFiles::LoadSettings(void)
{
	...
	CheckDlgButton(IDC_SHUTDOWNIFFINISHED, thePrefs.IsShutDownIfFinished()!=0); //>>> WiZaRd::ShutDownIfFinished
}
...
//>>> WiZaRd::ShutDownIfFinished
	const bool bChecked = IsDlgButtonChecked(IDC_SHUTDOWNIFFINISHED)!=0;
	if(thePrefs.IsShutDownIfFinished() != bChecked)
	{
		thePrefs.SetShutDownIfFinished(bChecked); 
...
void CPPgFiles::Localize(void)
{
	if(m_hWnd)
	{
		...
		GetDlgItem(IDC_SHUTDOWNIFFINISHED)->SetWindowText(GetResString(IDS_SHUTDOWNIFFINISHED)); //>>> WiZaRd::ShutDownIfFinished	
	}
}
