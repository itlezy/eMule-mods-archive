...
void CPartFile::Init(){
	...
	bNamecontinuitybad = 0;	// Tux: Feature: Filename disparity check
	m_uTransferred = 0;
...
//>>> WiZaRd::minRQR [WiZaRd]
						if(cur_src->IsRemoteQueueFull())
							uiMinRQR = min(uiMinRQR, _UI32_MAX-1);
...
//<<< WiZaRd::minRQR [WiZaRd]
					...
					if(cur_src->SupportsL2HAC() && cur_src->HasLowID() && (::GetTickCount()-cur_src->m_dwLastTriedToConnect < 59*60*1000))
...
{
//>>> WiZaRd::MediaPlayer
	if(theApp.emuledlg && theApp.emuledlg->mediaplayerwnd)
		theApp.emuledlg->mediaplayerwnd->StopFile(this);
		Sleep(100);
//<<< WiZaRd::MediaPlayer
...
void CPartFile::ResumeFileInsufficient()
{
	...
	if (!insufficient)
		return;
	AddLogLine(false, _T("Continuando download de \"%s\""), GetFileName());//DreamTradução
	insufficient = false;
	SetActive(theApp.IsConnected());
	m_LastSearchTime = 0;
	UpdateDisplayedInfo(true);
...
bool CPartFile::IsReadyForPreview() const
{
	...
	if (thePrefs.IsMoviePreviewBackup())
	{
		...
		return (GetStatus() != PS_COMPLETE &&  GetStatus() != PS_COMPLETING && (uint64)GetFileSize()>1024 && (uint64)GetCompletedSize()>1024 && ((GetFreeDiskSpaceX(GetTempPath()) + 100000000) > (2*(uint64)GetFileSize())));
	}
	else
	{
