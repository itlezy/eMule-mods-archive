...
//<<< WiZaRd::IntelliFlush
bool	CPreferences::m_bMinimizeOnClose; //>>> WiZaRd::Minimize on Close
...
//<<< WiZaRd::minRQR [WiZaRd]
//>>> WiZaRd::MediaPlayer
bool	CPreferences::m_bVLCMuted;
int		CPreferences::m_iVLCVolume;
//<<< WiZaRd::MediaPlayer
//>>> WiZaRd::ShutDownIfFinished
//NOTE: do not save this! 
//1) bad feature
//2) otherwise if someone starts an "empty" eMule he might be shutdown immediately ;)
bool	CPreferences::m_bShutDownIfFinished; 
//<<< WiZaRd::ShutDownIfFinished
bool	CPreferences::m_bDisplayStatusSmilies; //>>> WiZaRd::StatusSmilies
...
//>>> WiZaRd::IntelliFlush
	ini.WriteBool(L"IntelliFlush", m_bUseIntelliFlush);
	ini.WriteInt(L"FileBufferFlushTime", m_uiFileBufferFlushTime);
//<<< WiZaRd::IntelliFlush
...
//boizaum
	...
	ini.SetSection(L"MiscTweaks");
	ini.WriteBool(L"MinimizeOnClose", m_bMinimizeOnClose); //>>> WiZaRd::Minimize on Close
	ini.WriteBool(L"DisplayStatusSmilies", m_bDisplayStatusSmilies); //>>> WiZaRd::StatusSmilies
 //>>> WiZaRd::MediaPlayer
	 ini.SetSection(L"MediaPlayer");
	 ini.WriteBool(L"Muted", m_bVLCMuted);
	 ini.WriteInt(L"Volume", m_iVLCVolume);
//<<< WiZaRd::MediaPlayer
}
...
void CPreferences::LoadPreferences()
{
	...
	if (strNick.IsEmpty() || IsDefaultNick(strNick))
		SetUserNick(_T("[DreaMule]Usuario")); //(DEFAULT_NICK); //Xman UserNick
...
//>>> WiZaRd::Better Defaults
	m_uDeadServerRetries=ini.GetInt(L"DeadServerRetry",3);
...
//<<< WiZaRd::Better Defaults
	...
	m_strIRCServer = ini.GetString(L"DefaultIRCServerNew", L"irc.hub4ever.org");
...
//>>> WiZaRd::IntelliFlush
	m_bUseIntelliFlush = ini.GetBool(L"IntelliFlush", true);
	m_uiFileBufferFlushTime = (UINT)ini.GetInt(L"FileBufferFlushTime", 60000);
//<<< WiZaRd::IntelliFlush
	...
	m_iSearchMethod=ini.GetInt(L"SearchMethod",1);//global as deafault
...
//<<< WiZaRd::minRQR [WiZaRd]
	...
	ini.SetSection(L"MiscTweaks");
	m_bMinimizeOnClose = ini.GetBool(L"MinimizeOnClose", true); //>>> WiZaRd::Minimize on Close
	m_bDisplayStatusSmilies = ini.GetBool(L"DisplayStatusSmilies", true); //>>> WiZaRd::StatusSmilies
//>>> WiZaRd::MediaPlayer
	ini.SetSection(L"MediaPlayer");
	m_bVLCMuted = ini.GetBool(L"Muted", false);
	m_iVLCVolume = ini.GetInt(L"Volume", 50);
//<<< WiZaRd::MediaPlayer 
}
...
{
	if(thePrefs.ReuseTCPPort() && !bLokal && theApp.GetPublicPort())
