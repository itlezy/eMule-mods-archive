...
void CSearchListCtrl::Init(CSearchList* in_searchlist)
{
	...
	InsertColumn(13,GetResString(IDS_FILEID),LVCFMT_LEFT,220);
...
void CSearchListCtrl::Localize()
{
	...
	for (int icol=0;icol<pHeaderCtrl->GetItemCount();icol++) {
		switch (icol) {
			...
			case 13: strRes = GetResString(IDS_FILEID); break;
		}
			...
			case MP_PREVIEW:
				if (file){
					if (file->GetPreviews().GetSize() > 0){
						...
						// add to res - later
						AddLogLine(true, _T("Preview Requisitado - favor Esperar"));
					}
				}
				return TRUE;
...
void CSearchListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CSearchFile* content = (CSearchFile*)lpDrawItemStruct->itemData;
		// BEGIN netfinity: Fakealyzer - View files with different colours depending if tags match the filetype
		
	{
			COLORREF cr = odc->GetBkColor();
			uint32 nBitrate = content->GetIntTagValue(FT_MEDIA_BITRATE);
			uint32 nAvgBitrate = nBitrate;
			if ((content->GetFileName().Right(4).MakeLower() == _T(".wmv")) && (((uint64)content->GetFileSize() / 128)< 100000))
				odc->SetBkColor(RGB(GetRValue(cr),GetGValue(cr)*0.85,GetBValue(cr)*0.85));
			if (content->GetIntTagValue(FT_MEDIA_LENGTH) > 0)
				nAvgBitrate = (uint32) ((uint64)content->GetFileSize() / 128) / content->GetIntTagValue(FT_MEDIA_LENGTH);
			if (nBitrate || nAvgBitrate)
			{
				...
				if (content->GetFileType() == ED2KFTSTR_AUDIO)
				{
					...
					if (!isNameMismatch && (nBitrate >= 128 && (nBitrate % 8 == 0) && isMP3) && (nBitrate < 1500) && (nBitrate <= 2 * nAvgBitrate)) // No serious files have lower rate than this
						odc->SetBkColor(RGB(GetRValue(cr)*0.85,GetGValue(cr),GetBValue(cr)*0.85));
				...
				else // Only audio and video files have bitrate, so it must be fake!
				{
					odc->SetBkColor(RGB(GetRValue(cr),GetGValue(cr)*0.85,GetBValue(cr)*0.85));
				}
			}
		}
		// END netfinity: Fakealyzer - View files with different colours depending if tags match the filetype
