...
//>>> WiZaRd::Search Redesign
#define UNI_OFF		0
#define UNI_ON		1
#define UNI_ACTIVE	2
bool	bVisible1 = true;
bool	bVisible2 = false;//boizaum auto hide params
//<<< WiZaRd::Search Redesign
...
//	ON_BN_CLICKED(IDC_MORE, OnBnClickedMore) //>>> WiZaRd::Search Redesign
	...
	ON_WM_LBUTTONDBLCLK() //>>> WiZaRd::Search Redesign
	ON_WM_SYSCOMMAND()
	ON_WM_SETCURSOR()
	// Tux: LiteMule: Remove Help
END_MESSAGE_MAP()
...
//	m_rcMore.SetRectEmpty(); //>>> WiZaRd::Search Redesign
...
//>>> WiZaRd::Search Redesign
	DDX_Control(pDX, IDC_SEARCHOPT1, m_ctrlSearchControlsFrm);
	DDX_Control(pDX, IDC_SEARCHOPT2, m_ctrlSearchParamsFrm);
...
//>>> WiZaRd::Search Redesign
//	m_ctl.GetWindowRect(&m_rcUnicode);
...
//<<< WiZaRd::Search Redesign	
	...
	if (m_ctlMethod.SetCurSel(thePrefs.GetSearchMethod()) == CB_ERR)
		...
		m_ctlMethod.SetCurSel(SearchTypeEd2kGlobal);
...
//>>> WiZaRd::Search Redesign
#define GROUP_OFFSET_TOP	18	//for the icon
#define GROUP_OFFSET_BOTTOM	2
#define GROUP_OFFSET_LR		2
#define GROUP_PADDING_LR	(2*GROUP_OFFSET_LR)
#define	GROUP_PADDING_TB	(GROUP_OFFSET_TOP+GROUP_OFFSET_BOTTOM)
#define GROUP_HWADD			(2*GROUP_OFFSET_LR)
#define GROUP_HADD			(GROUP_OFFSET_TOP+GROUP_OFFSET_BOTTOM)
//<<< WiZaRd::Search Redesign
...
//		m_ctlUnicode.MoveWindow(rcClient.left + m_rcUnicode.left, rcClient.top + m_rcUnicode.top, m_rcUnicode.Width(), m_rcUnicode.Height()); //>>> WiZaRd::Search Redesign
...
//		m_ctlMore.MoveWindow(rcClient.left + m_rcMore.left, rcClient.top + m_rcMore.top, m_rcMore.Width(), m_rcMore.Height()); //>>> WiZaRd::Search Redesign
...
//>>> WiZaRd::Search Redesign
		//Horizontal layout
		m_ctlOpts.GetWindowRect(&rcOptsClnt);
		CRect name, start;
		m_ctlStart.GetWindowRect(&start);
		GetDlgItem(IDC_MSTATIC3)->GetWindowRect(&name);
		ScreenToClient(&rcOptsClnt);
		ScreenToClient(&name);
		ScreenToClient(&start);
		m_ctrlSearchControlsFrm.MoveWindow(name.left-GROUP_OFFSET_LR, rcClient.top, start.right-name.left+GROUP_PADDING_LR, rcClient.Height()-GROUP_PADDING_LR);
		m_ctrlSearchParamsFrm.MoveWindow(rcOptsClnt.left-GROUP_OFFSET_LR, rcClient.top, rcOptsClnt.Width()+GROUP_PADDING_LR, rcClient.Height()-GROUP_PADDING_LR); 
//<<< WiZaRd::Search Redesign
	}
	else if (cx < MIN_HORZ_WIDTH)
	{
		...
		y += GROUP_OFFSET_TOP; //>>> WiZaRd::Search Redesign
...
//		m_ctlMore.MoveWindow(rcClient.left + m_rcStart.Width() + 4, y, m_rcMore.Width(), m_rcMore.Height()); //>>> WiZaRd::Search Redesign
...
//>>> WiZaRd::Search Redesign
		//Vertical layout
		CRect name, cancel, frm;
		GetDlgItem(IDC_MSTATIC3)->GetWindowRect(&name);
		m_ctlCancel.GetWindowRect(&cancel);
		ScreenToClient(&name);
		ScreenToClient(&cancel);
		m_ctrlSearchControlsFrm.MoveWindow(name.left-GROUP_OFFSET_LR, rcClient.top, rcDropDownArrow.right-name.left+GROUP_PADDING_LR, cancel.bottom-name.top+GROUP_PADDING_TB);
		m_ctrlSearchControlsFrm.GetWindowRect(&frm);
		ScreenToClient(&frm);
		m_ctrlSearchParamsFrm.MoveWindow(name.left-GROUP_OFFSET_LR, frm.bottom, rcDropDownArrow.right-name.left+GROUP_PADDING_LR, rcClient.Height()-frm.Height()+GROUP_PADDING_LR); 
//<<< WiZaRd::Search Redesign

//>>> WiZaRd::Search Redesign
		int iOptsHeight = rcClient.Height()-frm.Height()-GROUP_OFFSET_BOTTOM;
//		int iOptsHeight = rcClient.bottom - y - 2;
//		m_ctlOpts.MoveWindow(rcClient.left, y, rcClient.Width(), iOptsHeight);
		m_ctlOpts.MoveWindow(rcClient.left, frm.bottom+GROUP_OFFSET_TOP, rcClient.Width(), iOptsHeight);
//<<< WiZaRd::Search Redesign		
		CRect rcOptsClnt;
		m_ctlOpts.GetClientRect(&rcOptsClnt);
...
//			else if (iMethod == SearchTypeEd2kServer && m_searchdlg->IsLocalEd2kSearchRunning())
			else if (!m_bMoreMode && iMethod == SearchTypeEd2kServer && m_searchdlg->IsLocalEd2kSearchRunning())
//<<< WiZaRd::Search Redesign
...
{
//>>> WiZaRd::Search Redesign
	m_ctrlSearchControlsFrm.SetIcon(L"SEARCH");
	m_ctrlSearchParamsFrm.SetIcon(L"SEARCHPARAMS");
//<<< WiZaRd::Search Redesign
...
void CSearchParamsWnd::Localize()
{
	...
	//DreaMule
	m_ctrlSearchControlsFrm.SetWindowText(GetResString(IDS_SEARCH_PANEL));
	m_ctrlSearchParamsFrm.SetWindowText(GetResString(IDS_PARAMS_PANEL));
	GetDlgItem(IDC_STATIC6)->SetWindowText(GetResString(IDS_SEARCH_LINE1));
	GetDlgItem(IDC_STATIC7)->SetWindowText(GetResString(IDS_SEARCH_LINE2));
	GetDlgItem(IDC_STATIC8)->SetWindowText(GetResString(IDS_SEARCH_LINE3));
	GetDlgItem(IDC_STATIC9)->SetWindowText(GetResString(IDS_SEARCH_LINE4));
	GetDlgItem(IDC_STATICa)->SetWindowText(GetResString(IDS_SEARCH_LINE5));
	//DreaMule
//	GetDlgItem(IDC_SEARCH_UNICODE)->SetWindowText(GetResString(IDS_SEARCH_UNICODE)); //>>> WiZaRd::Search Redesign
...
//	m_ctlMore.SetWindowText(GetResString(IDS_MORE)); //>>> WiZaRd::Search Redesign
...
//		if (pMsg->wParam == VK_RETURN && m_ctlStart.IsWindowEnabled())
		if (!m_bMoreMode && pMsg->wParam == VK_RETURN && m_ctlStart.IsWindowEnabled())
//<<< WiZaRd::Search Redesign
		{
...
//<<< WiZaRd::Search Redesign
//	m_ctlMore.EnableWindow(FALSE); //>>> WiZaRd::Search Redesign
...
{
//>>> WiZaRd::Search Redesign
	//if(!m_bMoreMode) //important... do NOT toggle if we are in more mode or we can only click it once
	ToggleStartMore(1);
//<<< WiZaRd::Search Redesign
...
//	bool bUnicodeIsDisabled = m_ctlUnicode.IsWindowEnabled() && m_ctlUnicode.GetCheck() == 0;
	bool bUnicodeIsDisable = m_uiUnicode == UNI_ON;
//<<< WiZaRd::Search Redesign
...
//>>> WiZaRd::Search Redesign
	if(bOfferUnicode)
		...
		m_uiUnicode = UNI_OFF;
	if(!bUnicodeIsDisable && bOfferUnicode)
		m_uiUnicode = UNI_ACTIVE;
...
//>>> WiZaRd::Search Redesign
		if(m_uiUnicode >= UNI_ON && pParams->bUnicode)
			m_uiUnicode = UNI_ACTIVE;
...
SSearchParams* CSearchParamsWnd::GetParameters()
{
	...
	if (!IsValidEd2kString(strExpression)){
		AfxMessageBox(GetResString(IDS_SEARCH_EXPRERROR) + _T("\n\n") + GetResString(IDS_SEARCH_INVALIDCHAR), MB_ICONWARNING);
		return NULL;
	}
...
//	pParams->bUnicode = bUnicode;
	pParams->bUnicode = m_uiUnicode == UNI_ACTIVE;
//<<< WiZaRd::Search Redesign
	return pParams;
}
...
//>>> WiZaRd::Search Redesign
void CSearchParamsWnd::ToggleStartMore(const sint8 iStart)
{
	if(iStart == 1)
		m_bMoreMode = false;
	else if(iStart == 0)
		m_bMoreMode = true;
	if(m_bMoreMode)
		m_ctlStart.SetWindowText(GetResString(IDS_MORE));
	else
		m_ctlStart.SetWindowText(GetResString(IDS_SW_START));
}
//<<< WiZaRd::Search Redesign
...
//>>> WiZaRd::Search Redesign
/*void	CSearchParamsWnd::OnBnClickedOpt1()
{
	GetDlgItem(IDC_MSTATIC3)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_SEARCHNAME)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_DD)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_MSTATIC7)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_TYPESEARCH)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_METH)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_COMBO1)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_SEARCH_RESET)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_STARTS)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_CANCELS)->ShowWindow(bVisible1 ? SW_HIDE : SW_SHOW);
	bVisible1 = !bVisible1;
}*/

//Show/Hide the text/params box
void	CSearchParamsWnd::OnBnClickedOpt2()
{
	GetDlgItem(IDC_SEARCH_OPTS)->ShowWindow(bVisible2 ? SW_HIDE : SW_SHOW);
	GetDlgItem(IDC_STATIC6)->ShowWindow(bVisible2 ? SW_SHOW : SW_HIDE);
	GetDlgItem(IDC_STATIC7)->ShowWindow(bVisible2 ? SW_SHOW : SW_HIDE);
	GetDlgItem(IDC_STATIC8)->ShowWindow(bVisible2 ? SW_SHOW : SW_HIDE);
	GetDlgItem(IDC_STATIC9)->ShowWindow(bVisible2 ? SW_SHOW : SW_HIDE);
	GetDlgItem(IDC_STATICa)->ShowWindow(bVisible2 ? SW_SHOW : SW_HIDE);
	

	bVisible2 = !bVisible2;
}

void CSearchParamsWnd::OnLButtonDblClk(UINT /*nFlags*/, CPoint point)
{
	CRect /*opts1,*/ opts2;
//	GetDlgItem(IDC_SEARCHOPT1)->GetWindowRect(&opts1);
	GetDlgItem(IDC_SEARCHOPT2)->GetWindowRect(&opts2);
//	ScreenToClient(&opts1);
	ScreenToClient(&opts2);

/*	if(opts1.PtInRect(point))
		OnBnClickedOpt1();
	else*/ if(opts2.PtInRect(point))
		OnBnClickedOpt2();
}
//<<< WiZaRd::Search Redesign