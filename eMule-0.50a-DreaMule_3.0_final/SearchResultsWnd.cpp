...
//>>> WiZaRd::Search Redesign
	if(bMoreResultsAvailable && m_iSentMoreReq < MAX_MORE_SEARCH_REQ)
	{
		m_pwndParams->ToggleStartMore(0);
		m_pwndParams->m_ctlStart.EnableWindow(TRUE);
	}
...
//>>> WiZaRd::Search Redesign
//		m_pwndParams->m_ctlMore.EnableWindow(FALSE);
	m_pwndParams->ToggleStartMore(1);
//<<< WiZaRd::Search Redesign
...
void CSearchResultsWnd::DeleteAllSearches()
{
	...
	CWnd* pWndFocus = GetFocus();
//	m_pwndParams->m_ctlMore.EnableWindow(FALSE); //>>> WiZaRd::Search Redesign
	m_pwndParams->m_ctlCancel.EnableWindow(FALSE);
	m_pwndParams->m_ctlStart.EnableWindow(TRUE);
//>>> WiZaRd::Search Redesign
	m_pwndParams->ToggleStartMore(1);
