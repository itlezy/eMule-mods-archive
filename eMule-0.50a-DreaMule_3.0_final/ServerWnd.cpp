...
BEGIN_MESSAGE_MAP(CServerWnd, CResizableDialog)
	...
	ON_BN_CLICKED(IDC_VERKAD, OnBnVerKad)
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		// servermsgbox->SetProfileSkinKey(_T("ServerInfoLog")); // X-Ray :: Toolbar :: Cleanup
		...
		// servermsgbox->ApplySkin(); // X-Ray :: Toolbar :: Cleanup
		servermsgbox->SetTitle(GetResString(IDS_SV_SERVERINFO));

		servermsgbox->AppendText(MOD_VERSION); //Xman ModID
	...
	if (logbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_LOGBOX)){
		// logbox->SetProfileSkinKey(_T("Log")); // X-Ray :: Toolbar :: Cleanup
	...
	if (debuglog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_DEBUG_LOG)){
		// debuglog->SetProfileSkinKey(_T("VerboseLog")); // X-Ray :: Toolbar :: Cleanup
		...
		// debuglog->ApplySkin(); // X-Ray :: Toolbar :: Cleanup
	...
	AddAnchor(IDC_VERKAD, TOP_RIGHT);
...
void CServerWnd::Localize()
{
	serverlistctrl.Localize();
		...
		//DreaMule
			GetDlgItem(IDC_VERKAD)->SetWindowText(GetResString(IDS_SEE_KAD));
		//DreaMule
