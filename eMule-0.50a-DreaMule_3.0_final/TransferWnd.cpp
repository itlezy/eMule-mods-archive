...
BOOL CTransferWnd::OnInitDialog()
{
	...
	GetDlgItem(IDC_VLC_PREVIEW_DL)->EnableWindow(FALSE); //>>> WiZaRd::MediaPlayer
...
void CTransferWnd::Localize()
{
	...
	GetDlgItem(IDC_VLC_PREVIEW_DL)->SetWindowText(GetResString(IDS_VLC_PREVIEW));
...
//	m_btnWnd1->GetWindowRect(rcBtnWnd1);
	GetDlgItem(IDC_VLC_PREVIEW_DL)->GetWindowRect(&rcBtnWnd1);
//<<< WiZaRd::MediaPlayer
...
void CTransferWnd::ResetTransToolbar(bool bShowToolbar, bool bResetLists)
{
	...
	RemoveAnchor(IDC_VLC_PREVIEW_DL); //>>> WiZaRd::MediaPlayer
...
//>>> WiZaRd::MediaPlayer
	...
	CRect rcBtn;
	GetDlgItem(IDC_VLC_PREVIEW_DL)->GetClientRect(&rcBtn);
	GetDlgItem(IDC_VLC_PREVIEW_DL)->MoveWindow(rcBtn1.right+2, rcBtn1.top, rcBtn.Width(), rcBtn.Height());
	AddAnchor(IDC_VLC_PREVIEW_DL, TOP_LEFT);
//<<< WiZaRd::MediaPlayer	
...
//>>> WiZaRd::MediaPlayer
void CTransferWnd::OnBnClickedPreview()
{
	theApp.emuledlg->transferwnd->downloadlistctrl.PostMessage(WM_COMMAND, MP_VLCVIEW, 0);
}
//<<< WiZaRd::MediaPlayer