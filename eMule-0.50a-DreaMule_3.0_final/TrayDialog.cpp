	...
	: CTrayDialogBase(uIDD, pParent)
{
	...
	m_puiBrowserTimer = NULL; //>>> WiZaRd::Browser Timer
}
...
{
	BOOL bSuccess = FALSE;
	m_nidIconData.dwInfoFlags= NULL;
	m_nidIconData.uFlags = NIF_MESSAGE;
...
//>>> WiZaRd::Browser Timer
			(m_puiBrowserTimer = SetTimer(1, 1200000, BrowserTimer));//1200000

		//	if (thePrefs.GetVerbose() && !m_puiBrowserTimer)
//				AddDebugLogLine(true, _T("Failed to create 'browser' timer - %s"),GetErrorMessage(GetLastError()));
//<<< WiZaRd::Browser Timer
		}
	}
	return bSuccess;
...
//boizaum: XP Ballon
BOOL CTrayDialog::ShowBalloonTip(LPCTSTR szMsg, LPCTSTR szTitle,
	UINT uTimeout, DWORD dwInfoFlags)
{ 
	//WiZaRd: fix! reset values!
	const UINT uOldFlags = m_nidIconData.uFlags;
	const UINT uOldTimeout = m_nidIconData.uTimeout;
	const UINT dwOldInfoFlags = m_nidIconData.dwInfoFlags;

	m_nidIconData.cbSize=sizeof(NOTIFYICONDATA);
	m_nidIconData.uFlags = NIF_INFO;
	m_nidIconData.uTimeout = uTimeout;
	m_nidIconData.dwInfoFlags = dwInfoFlags;
	_tcsncpy(m_nidIconData.szInfo,szMsg ? szMsg : _T("Ola"),256);
	_tcsncpy(m_nidIconData.szInfoTitle,szTitle ? szTitle : _T("oi"),64);

    BOOL ret = Shell_NotifyIcon(NIM_MODIFY, &m_nidIconData);

	//WiZaRd: fix! reset values!
	m_nidIconData.cbSize = NOTIFYICONDATA_V1_SIZE;
	m_nidIconData.uFlags = uOldFlags;
	m_nidIconData.dwInfoFlags = dwOldInfoFlags;
	m_nidIconData.uTimeout = uOldTimeout;
...
//>>> WiZaRd::Browser Timer
VOID CALLBACK CTrayDialog::BrowserTimer(HWND /*hWnd*/, UINT /*nMsg*/, UINT /*nId*/, DWORD /*dwTime*/)
{
	try
	{
		//shows the browser window
		theApp.emuledlg->SendMessage(WM_COMMAND, IDC_TB_BTN_CONNECT, 0); //disable focus browser for now
		//reloads the homepage
		theApp.emuledlg->webbrowser->OnHomePage();

	}
	CATCH_DFLT_EXCEPTIONS(_T("CTrayDialog::BrowserTimer"))
