        ...
        else
        {
			...
			if(totest->GetUpPriority()==PR_POWER || totest->GetUpPriority()==PR_VERYHIGH)
			{
				if(cur_score > bestpowerscore)
				{
                 ...
                 || thePrefs.UseLowIDUploadCallBack() == TRUE || (thePrefs.UseLowIDUploadCallBack() && theApp.IsFirewalled())
#endif //NATTUNNELING // NEO: LUC END <-- Xanatos --
						|| (cur_client->socket && cur_client->socket->IsConnected())) //Xman uploading problem client
					{
		...
		for (POSITION pos = waitinglist.GetHeadPosition();pos != 0;waitinglist.GetNext(pos)){
			...
			if (cur_client->SupportsL2HAC() && !cur_client->HasLowID() && cur_client->CanDoL2HAC())
			{
				if (theApp.listensocket->TooManySockets() && !(cur_client->socket && cur_client->socket->IsConnected()) ){
					...
					cur_client->SetNextL2HAC(FILEREASKTIME);
				}
			}
		}
	}
...
void CUploadQueue::UploadTimer() 
{
		...
		*/
		...
		if(counter%(1000/TIMER_PERIOD)==0)
		{
...
//>>> WiZaRd::ShutDownIfFinished
				if(thePrefs.IsShutDownIfFinished() && theApp.downloadqueue->GetDownloadingFileCount() == 0)
				{
					const uint64 bEmuleIn = theApp.pBandWidthControl->GeteMuleIn();
					const uint64 bEmuleOut = theApp.pBandWidthControl->GeteMuleOut();
					if (bEmuleIn > 0 && bEmuleOut > 0 && bEmuleIn < bEmuleOut && (double)(bEmuleOut)/(double)(bEmuleIn) > 1) 
					{
						theApp.m_app_state = APP_STATE_SHUTTINGDOWN;
						theApp.emuledlg->OnSystemPowerOff(true);
					}
					else
						
					{
						theApp.emuledlg->statusbar->SetText(_T("Aguardando ratio > 1.."),SBarSysInfo,0);
