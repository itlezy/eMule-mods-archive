...
CemuleApp theApp(_T("DreaMule"));// Mostrar Mod ?
...
#error "You are using an MFC version which may require a special version of the above function!"
...
#ifndef _DEBUG //WiZaRd: don't show in debug builds... "asserts" and errors can occur 
	...
	UpdateSplash(GetResString(IDS_SPLASH_LOADING)); 
	//Xman end
	...
	UpdateSplash(GetResString(IDS_SPLASH_LOADING1)); //Xman new slpash-screen arrangement
	...
	UpdateSplash(GetResString(IDS_SPLASH_LOADING2)); //Xman new slpash-screen arrangement
	dlp = new CDLP(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR)); //Xman DLP
	UpdateSplash(GetResString(IDS_SPLASH_LOADING3)); //Xman new slpash-screen arrangement
	ip2country = new CIP2Country(); //EastShare - added by AndCycle, IP to Country
	UpdateSplash(GetResString(IDS_SPLASH_LOADING4)); //Xman new slpash-screen arrangement
...
#endif //NATTUNNELING // NEO: UTCP END <-- Xanatos --
	UpdateSplash(GetResString(IDS_SPLASH_LOADING5)); //Xman new slpash-screen arrangement
	clientcredits = new CClientCreditsList();
	theModIconMap = new CModIconMapper(); //>>> WiZaRd::ModIconMappings
	UpdateSplash(GetResString(IDS_SPLASH_LOADING6)); //Xman new slpash-screen arrangement
	downloadqueue = new CDownloadQueue();	// bugfix - do this before creating the uploadqueue
	uploadqueue = new CUploadQueue();
	UpdateSplash(GetResString(IDS_SPLASH_LOADING7)); //Xman new slpash-screen arrangement
	...
	UpdateSplash(GetResString(IDS_SPLASH_LOADING8)); //Xman new slpash-screen arrangement
	dlg.DoModal();
...
bool CemuleApp::ProcessCommandline()
{
	...
	// Tux: Fix: Running Instances Fix by WiZaRd [start]
	...
	// Tux: Fix: Running Instances Fix by WiZaRd [end]
	//if (!bIgnoreRunningInstances){ //boizaum always focus new windows
	...
	//}//boizaum always focus new windows
...
void CemuleApp::SetPublicIP(const uint32 dwIP){
	if (dwIP != 0){
		...
		AddDebugLogLine(DLP_VERYLOW, false, _T("Deletado public IP"));
	
	if (dwIP != 0 && dwIP != m_dwPublicIP && serverlist != NULL){
		m_dwPublicIP = dwIP;
		serverlist->CheckForExpiredUDPKeys();
		theApp.emuledlg->webbrowser->OnHomePage(); //>>> WiZaRd::Browser Update
	}
	else
	m_dwPublicIP = dwIP;
}
...
bool CemuleApp::DoCallback( CUpDownClient *client )
{
	...
	if(thePrefs.IsNatTraversalEnabled() && client->SupportsNatTraversal() && client->GetUserPort() == 0)
	{
	...
	if(client->GetXsBuddyIP())
	{
		if(theApp.IsFirewalled())
		{
			...
			if(thePrefs.IsNatTraversalEnabled() && client->SupportsNatTraversal() 
	...
	if(Kademlia::CKademlia::IsConnected())
	{
		if(theApp.serverconnect->IsConnected())
		{
			if(theApp.serverconnect->IsLowID())
			{
				...
				{
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
					if(thePrefs.IsNatTraversalEnabled() && client->SupportsNatTraversal(true)
				...
				else
				{
					...
					{
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
						if(thePrefs.IsNatTraversalEnabled() && client->SupportsNatTraversal() && theApp.serverconnect->GetCurrentServer()->SupportsNatTraversal())
						{
...
BOOL CemuleApp::AddUPnPNatPort(MyUPnP::UPNPNAT_MAPPING *mapping, bool tryRandom){
	...
	if(m_UPnPNat.AddNATPortMapping(mapping, tryRandom) == MyUPnP::UNAT_OK ){
		if(theApp.emuledlg->IsRunning()){
			AddLogLine(false, _T("Adicionado UPnP NAT : (%s) NAT ROUTER/FIREWALL:%i -> %s:%i"),//DreamTradução
		...
		if(theApp.emuledlg->IsRunning()){
			AddLogLine(false, _T("Erro adicionando UPnP NAT: (%s) NAT ROUTER/FIREWALL:%i -> %s:%i (%s)"),
...
void CemuleApp::ShowSplash(bool start)
{
	...
	if (m_pSplashWnd == NULL)
	{
		...
		if (m_pSplashWnd != NULL)
		{
			...
			if (m_pSplashWnd->Create(NULL,MOD_VERSION ,0,CSS_FADE | CSS_CENTERSCREEN | CSS_SHADOW | CSS_HIDEONCLICK))
			{
