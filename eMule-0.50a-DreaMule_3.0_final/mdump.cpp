...
LONG CMiniDumper::TopLevelFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	if (hDll)
	{
		if (pfnMiniDumpWriteDump)
		{
			...
			// Do *NOT* localize that string (in fact, do not use MFC to load it)!
			if (MessageBox(NULL, _T("DreaMule travou :-(\r\n\r\nUm arquivo diagnostico pode ser criado para o autor resolver esse problema.Esse arquivo será salvado no disco e você deverá mandar para ele depois.\r\n\r\nVocê quer criar esse arquivo?"), m_szAppName, MB_ICONSTOP | MB_YESNO) == IDYES)
			{
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					{
						// Do *NOT* localize that string (in fact, do not use MFC to load it)!
						_sntprintf(szResult, ARRSIZE(szResult), _T("Arquivo salvdo em \"%s\".\r\n\r\npor favor mande esse arquivo com detalhes do ocorrido para dreamule@gmail.com !\r\n\r\nObrigado por ajudar a melhorar o dreamule."), szDumpPath); //Xman Xtreme Mod
