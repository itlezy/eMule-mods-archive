...
#include "kademlia/kademlia/kademlia.h" // reconnect to Kademlia
#include "Log.h"
...
{
	if (connectionattemps.GetCount() < 1)	// Tux: Newbie security: remove SafeServerConnect: changed line
	{
...
void CServerConnect::ConnectToServer(CServer* server, bool multiconnect, bool bNoCrypt)
{
	...
	{
		AddLogLine(true,_T("você não pode contectar no servidor filtrado: %s, %s"),ipstr(server->GetIP()), server->GetDescription() );
		return;
	}
	//Xman end
	...
	if(thePrefs.ReuseTCPPort())
		newsocket->Create(thePrefs.GetPort(true), SOCK_STREAM, FD_READ | FD_WRITE | FD_CLOSE | FD_CONNECT, thePrefs.GetBindAddrA(), TRUE);
	else
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:
			...
			Kademlia::CKademlia::Stop(); // reconnect to Kademlia
			break;
...
{ 
	...
	try
	{
 // reconnect to Kademlia - START
		if( thePrefs.GetNetworkKademlia() )
		{
			Kademlia::CKademlia::Start();
		}
 // reconnect to Kademlia - END
	...
	CATCH_DFLT_EXCEPTIONS(_T("CServerConnect::RetryConnectTimer"))
		...
		//         The exceptions thrown in callback function are not intercepted by the dbghelp.dll (e.g. eMule Dump, crashRpt, etc...)
			...
			if(theApp.emuledlg != NULL)
				AddLogLine(true, _T("Falha desconhecida em %s"), __FUNCTION__);//DreamTradução
		}
		// Maella end
...
{
	connectedsocket = NULL;
	max_simcons = 1;	// Tux: Newbie security: remove SafeServerConnect: changed line
	connecting = false;
	connected = false;
	clientid = 0;
	singleconnecting = false;
