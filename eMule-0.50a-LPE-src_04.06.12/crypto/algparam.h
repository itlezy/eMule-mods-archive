...
#else
			((*reinterpret_cast<std::string *>(m_pValue) += "ThisPointer:") += typeid(T).raw_name()) += ';';
#endif
		}
...
#else
		if (!m_found && strncmp(m_name, "ThisPointer:", 12) == 0 && strcmp(m_name+12, typeid(T).raw_name()) == 0)
#endif
		{
...
#else
			((*reinterpret_cast<std::string *>(m_pValue) += "ThisObject:") += typeid(T).raw_name()) += ';';
		if (!m_found && strncmp(m_name, "ThisObject:", 11) == 0 && strcmp(m_name+11, typeid(T).raw_name()) == 0)
#endif
		{
			NameValuePairs::ThrowIfTypeMismatch(m_name, typeid(T), *m_valueType);
...
#else
				throw InvalidArgument(std::string(typeid(T).raw_name()) + ": Missing required parameter '" + name + "'");
#endif
			(m_pObject->*pm)(value);
		}
		return *this;
...
#else
				throw InvalidArgument(std::string(typeid(T).raw_name()) + ": Missing required parameter '" + name1 + "'");
#endif
...
#else
				throw InvalidArgument(std::string(typeid(T).raw_name()) + ": Missing required parameter '" + name2 + "'");
#endif
