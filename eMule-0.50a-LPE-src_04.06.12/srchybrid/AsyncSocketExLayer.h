...
protected:
	...
	int ReceiveFromNext(void *lpBuf, int nBufLen, SOCKADDR* lpSockAddr, int* nSockAddrLen, int nFlags = 0);// X: [SUDPS] - [CAsyncSocketEx UDP Support]
	int SendNext(const void *lpBuf, int nBufLen, int nFlags = 0);
	int SendToNext(const void *lpBuf, int nBufLen, const SOCKADDR* lpSockAddr, int nSockAddrLen, int nFlags = 0);// X: [SUDPS] - [CAsyncSocketEx UDP Support]

	CAsyncSocketEx *m_pOwnerSocket;

	//Calls OnLayerCallback on owner socket
	int DoLayerCallback(int nType, INT_PTR nParam1, INT_PTR nParam2, char* str = 0);
