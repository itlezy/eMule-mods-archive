...
BOOL CBase64Coding::Encode( const char * source, size_t number_of_bytes_to_encode, char * destination )
{
	...
	size_t number_of_bytes_encoded = 0;
	...
	while( loop_index < number_of_bytes_to_encode )
	{
		...
		destination[ number_of_bytes_encoded++ ] = static_cast< char >( alphabet[ ( byte_1 >> 2 ) ] );

		if ( ++loop_index >= number_of_bytes_to_encode )
		{
			// We're at the end of the data to encode

			destination[ number_of_bytes_encoded++ ] = alphabet[ ( ( ( byte_1 & 0x03 ) << 4 )) ];
		...
		destination[ number_of_bytes_encoded++ ] = alphabet[ ( ( ( byte_1 & 0x03 ) << 4 ) | ( byte_2 >> 4 ) ) ];

		if ( ++loop_index >= number_of_bytes_to_encode )
		{
			// We ran out of bytes, we need to add the last half of byte_2 and pad
			destination[ number_of_bytes_encoded++ ] = alphabet[ ( ( ( byte_2 & 0x0F ) << 2 )) ];
			destination[ number_of_bytes_encoded ] = END_OF_BASE64_ENCODED_DATA;
			// 1999-09-01
			// Thanks go to Yurong Lin (ylin@dial.pipex.com) for finding a bug here.
			// We must NULL terminate the string before letting CString have the buffer back.
		...
		destination[ number_of_bytes_encoded++ ] = alphabet[ ( ( ( byte_2 & 0x0F ) << 2 ) | ( ( byte_3 & 0xC0 ) >> 6 ) ) ];
		destination[ number_of_bytes_encoded++ ] = alphabet[ ( byte_3 & 0x3F ) ];
