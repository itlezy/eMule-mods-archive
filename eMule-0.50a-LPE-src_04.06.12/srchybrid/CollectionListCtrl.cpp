...
public:
	CCollectionFileDetailsSheet(CAtlList<CAbstractFile*>& aFiles, UINT uPshInvokePage = 0, CListCtrlItemWalk* pListCtrl = NULL);
...
CCollectionFileDetailsSheet::CCollectionFileDetailsSheet(CAtlList<CAbstractFile*>& aFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
...
BOOL CCollectionFileDetailsSheet::OnInitDialog()
{
	...
	EnableSaveRestore(_T("CollectionFileDetailsSheet"), !thePrefs.prefReadonly); // call this after(!) OnInitDialog // X: [ROP] - [ReadOnlyPreference]
	UpdateTitle();
	return bResult;
}
