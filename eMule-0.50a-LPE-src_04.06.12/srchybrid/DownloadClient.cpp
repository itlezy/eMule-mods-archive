...
void CUpDownClient::ProcessBlockPacket(const uchar *packet, uint32 size, bool packed, bool bI64Offsets)
{
	...
	thePrefs.Add2SessionTransferData(GetClientSoft(), GetUserPort(), false, false, uTransferredFileDataSize, false); // ==> Pay Back First
	// <-----khaos-
...
{
	// BEGIN netfinity: UDP Reask security check - Don't accept responses if we haven't asked (can be from bogus clients)
	if (!m_bUDPPending)
	{
		if (thePrefs.GetVerbose())
			DebugLogWarning(_T("UDP ACK-Answer: %s - ack without request"),GetUserName());
		return;
	}
	// END netfinity: UDP Reask security check
	m_bUDPPending = false;
...
void CUpDownClient::UDPReaskFNF()
{
	...
	{
		if (thePrefs.GetVerbose())
			DebugLogWarning(_T("UDP FNF-Answer: %s - ack without request"),GetUserName());
		return;
	}
	// END netfinity: UDP Reask security check
	m_bUDPPending = false;
	if (GetDownloadState() != DS_DOWNLOADING){ // avoid premature deletion of 'this' client
		...
		if(reqfile && GetUploadState()!=US_NONE)
		{
			...
			if(upfile && upfile == reqfile) //we speak about the same file
			{
				...
				if(!theApp.uploadqueue->RemoveFromUploadQueue(this, _T("Src says he does not have the file he's dl'ing")))
					theApp.uploadqueue->RemoveFromWaitingQueue(this);
				else
					ASSERT(!theApp.uploadqueue->RemoveFromWaitingQueue(this));
			}
		}
		//Xman end
...
bool CUpDownClient::DoSwap(CPartFile* SwapTo, bool bRemoveCompletely) {
	...
	if(pos)
	{
		...
		else
		{
			...
			//Maella end
			theApp.emuledlg->transferwnd->downloadclientsctrl.RemoveClient(this);
		}
		...
		if(pos2)
			theApp.emuledlg->transferwnd->downloadclientsctrl.RefreshClient(this,SwapTo);
		else
			theApp.emuledlg->transferwnd->downloadclientsctrl.AddClient(this,SwapTo);
