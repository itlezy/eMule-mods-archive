...
// morph4u :: PercentBar :: Start
        case 5: {	// progress
			 ...
			 if (szItem[0] != 0) {
					COLORREF oldclr = dc->SetTextColor(::GetSysColor(thePrefs.IsProgressBar() ? COLOR_GRAYTEXT : COLOR_WINDOWTEXT));
...
//Upload Permission +
				...
				else if (pFile->GetPermissions() == PERM_UPMANA)
					...
					while(!selectedList.IsEmpty()) { 
						...
						switch (wParam)
						{
							...
							case MP_PERMNONE:
								...
								break;
                             case MP_PERMUPMANA:
								file->SetPermissions(PERM_UPMANA);
								break;
...
//Upload Permission +
    ...
    if(thePrefs.IsExtControlsEnabled())
	  m_FileMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_PermMenu.m_hMenu, GetResString(IDS_PERMISSION) + _T(" (") + GetResString(IDS_PW_CON_UPLBL) + _T(')'));	
//Upload Permission -
