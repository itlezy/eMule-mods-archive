...
void CDownloadQueue::Init(){
	...
	theApp.emuledlg->transferwnd->UpdateFilesCount();// show in downloadwindow
	if(count == 0) {
...
void CDownloadQueue::AddFileLinkToDownload(CED2KFileLink* pLink, size_t cat, LPCTSTR filename)// X: [IP] - [Import Parts]
{
	...
	if (pLink->HasHostnameSources())
	{
		...
		while (pos != NULL)
		{
			...
			m_srcwnd.AddToResolve(pLink->GetHashKey(), pUnresHost->strHostname, pUnresHost->nPort);
		}
	}
...
void CDownloadQueue::AddDownload(CPartFile* newfile,bool paused) {
	...
	if(thePrefs.GetNotifier()){
		...
		theApp.emuledlg->TraySetBalloonToolTip(GetResString(IDS_CL_DOWNLSTATUS), msgTemp);
	}
}
...
void CDownloadQueue::Process(){
	...
	if(deltaTime > 0){
		...
		//used this instead of GetMaxDownloadEx()
		const float maxDownload = thePrefs.GetNAFCFullControl() == false ? 
		thePrefs.GetMaxDownload() : theApp.pBandWidthControl->GetMaxDownload(); //in [kb/s]
...
CString CDownloadQueue::getTextList()
{
	CString out;

	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		CPartFile* file = filelist.GetNext(pos);
		out.AppendFormat(_T("\n%s\t [%.2f%%] %i/%i - %s"),
					file->GetFileName(),
...
bool CDownloadQueue::CheckAndAddSource(CPartFile* sender,CUpDownClient* source){
		...
		for (POSITION pos2 = cur_file->srclist.GetHeadPosition();pos2 != 0; ){
			...
			if (cur_client->Compare(source, true) || cur_client->Compare(source, false)){
				...
				if (cur_client->AddRequestForAnotherFile(sender)){
					theApp.emuledlg->transferwnd->downloadclientsctrl.AddClient(cur_client,sender);
					delete source;
	...
	theApp.emuledlg->transferwnd->downloadclientsctrl.AddClient(source,sender);
...
bool CDownloadQueue::RemoveSource(CUpDownClient* toremove, bool bDoStatsUpdate)
{
	...
	//Xman end
	theApp.emuledlg->transferwnd->downloadclientsctrl.RemoveClient(toremove,true);
...
void CDownloadQueue::RemoveFile(CPartFile* toremove)
{
	...
	if (pos != NULL){
		...
		theApp.emuledlg->transferwnd->downloadlistctrl.RemoveFile(toremove);
	}
	// Maella end
	SortByPriority();
	CheckDiskspace();	// SLUGFILLER: checkDiskspace
...
void CDownloadQueue::KademliaSearchFile(uint32 searchID, const Kademlia::CUInt128* pcontactID, const Kademlia::CUInt128* pbuddyID, uint8 type, uint32 ip, uint16 tcp, uint16 udp, uint32 dwBuddyIP, uint16 dwBuddyPort, uint8 byCryptOptions)
{
	...
	{
		if (thePrefs.GetLogFilteredIPs())
			AddDebugLogLine(false, _T("IPfiltered source IP=%s received from Kademlia for File: %s"), ipstr(ED2Kip), temp->GetFileName()); //Xman show filename
		return;
	}
...
void CDownloadQueue::DoQuickStart()
{
	...
	if (QuickStartTime > 0){
		...
		theApp.emuledlg->toolMenu.ModifyMenu(2 ,MF_BYPOSITION | MF_STRING, MP_HM_QUICKSTART,GetResString(IDS_X_QUICK_START_STOP)); // NEO: QS - [QuickStart] <-- Xanatos --
...
void CDownloadQueue::StopQuickStart()
{
	...
	theApp.emuledlg->toolMenu.ModifyMenu(2 ,MF_BYPOSITION | MF_STRING, MP_HM_QUICKSTART,GetResString(IDS_X_QUICK_START_START)); // NEO: QS - [QuickStart] <-- Xanatos --
	m_bQuickStartDone = true;
...
CString CDownloadQueue::GetFilesCount(size_t inCategory)
{
	...
	CString strBuffer;
	strBuffer.Format(_T(" (%u/%u)/%s (%u)"), countreadyfiles, iCount, GetResString(IDS_DL_SOURCES), countsources);
	return strBuffer;
}
