...
BOOL CDropDownButton::Init(bool bSingleDropDownBtn, bool bWholeDropDown)
{
	...
	if (m_bSingleDropDownBtn)
	{
		TBBUTTON atb[1] = {0};
		atb[0].iBitmap = -1;
		atb[0].idCommand = GetWindowLongPtr(m_hWnd, GWL_ID);
...
void CDropDownButton::SetWindowText(LPCTSTR pszString)
{
	int cx = 0;
	if (!m_bSingleDropDownBtn)
		cx = GetBtnWidth(GetWindowLongPtr(m_hWnd, GWL_ID));

	TBBUTTONINFO tbbi = {0};
	tbbi.cbSize = sizeof tbbi;
	tbbi.dwMask = TBIF_TEXT;
	tbbi.pszText = const_cast<LPTSTR>(pszString);
	SetButtonInfo(GetWindowLongPtr(m_hWnd, GWL_ID), &tbbi);

	if (cx)
		SetBtnWidth(GetWindowLongPtr(m_hWnd, GWL_ID), cx);
}
...
void CDropDownButton::RecalcLayout(bool bForce)
{
	...
	bool bDropDownBtn = (GetBtnStyle(GetWindowLongPtr(m_hWnd, GWL_ID)) & BTNS_DROPDOWN) != 0;
	if (bDropDownBtn)
		RemoveBtnStyle(GetWindowLongPtr(m_hWnd, GWL_ID), BTNS_DROPDOWN);
	if (bDropDownBtn || bForce)
	{
		CToolBarCtrlX::RecalcLayout();
		if (bDropDownBtn)
			AddBtnStyle(GetWindowLongPtr(m_hWnd, GWL_ID), BTNS_DROPDOWN);
	}
}
