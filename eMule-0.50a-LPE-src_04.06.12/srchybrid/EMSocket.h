...
///	DWORD GetLastCalledSend() { return lastCalledSend; }
	...
	*/
	...
	typedef CAtlList<float> BlockHistory;
	...
	float GetandStepBlockRatio() {
			...
			if(m_blockhistory.GetCount()>HISTORY_SIZE) // ~ 20 seconds
			{
			...
			blockedsendcount=0;
			sendcount=0;
			avg_block_ratio = sum_blockhistory / m_blockhistory.GetCount();
...
private:
	...
	CAtlList<Packet*> controlpacket_queue;
	CAtlList<StandardPacketQueueEntry> standartpacket_queue;
