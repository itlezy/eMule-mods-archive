...
{
	for (size_t i = 0; i < rFileIdentifier.m_aMD4HashSet.GetCount(); i++)
	{
		uchar* pucHashSetPart = new uchar[16];
		md4cpy(pucHashSetPart, rFileIdentifier.m_aMD4HashSet[i]);
		m_aMD4HashSet.Add(pucHashSetPart);
	}
	for (size_t i = 0; i < rFileIdentifier.m_aAICHPartHashSet.GetCount(); i++)
...
bool CFileIdentifier::CalculateMD4HashByHashSet(bool bVerifyOnly, bool bDeleteOnVerifyFail, bool slowdown)
{
	...
	for (size_t i = 0; i < m_aMD4HashSet.GetCount(); i++)
...
bool CFileIdentifier::SetMD4HashSet(const CAtlArray<uchar*>& aHashset)
{
	// delete hashset
	for (size_t i = 0; i < m_aMD4HashSet.GetCount(); i++)
	...
	// set new hash
	for (size_t i = 0; i < aHashset.GetCount(); i++)
	{
...
uchar* CFileIdentifier::GetMD4PartHash(size_t part) const
{
	if (part >= m_aMD4HashSet.GetCount())
		return NULL;
...
void CFileIdentifier::WriteMD4HashsetToFile(CFileDataIO* pFile) const
{
	ASSERT( !isnulmd4(m_abyMD4Hash) );
	pFile->WriteHash16(m_abyMD4Hash);
	size_t uParts = m_aMD4HashSet.GetCount();
	pFile->WriteUInt16((uint16)uParts);
	for (size_t i = 0; i < uParts; i++)
...
bool CFileIdentifier::SetAICHHashSet(const CFileIdentifier& rSourceHashSet)
{
	...
	for (size_t i = 0; i < rSourceHashSet.m_aAICHPartHashSet.GetCount(); i++)
...
void CFileIdentifier::WriteAICHHashsetToFile(CFileDataIO* pFile) const
{
	...
	ASSERT( HasExpectedAICHHashCount() );
	m_AICHFileHash.Write(pFile);
	size_t uParts = m_aAICHPartHashSet.GetCount();
	pFile->WriteUInt16((uint16)uParts);
	for (size_t i = 0; i < uParts; i++)
...
bool CFileIdentifier::VerifyAICHHashSet()
{
	...
	size_t uPartCount = (uint16)(((uint64)m_rFileSize + (PARTSIZE - 1)) / PARTSIZE);
	if (uPartCount <= 1)
		return true; // No AICH Part Hashs
	for (size_t nPart = 0; nPart < uPartCount; nPart++)
	{
		uint64 nPartStartPos = (uint64)nPart*PARTSIZE;
		size_t nPartSize = (size_t)min(PARTSIZE, (uint64)GetFileSize()-nPartStartPos);
