...
void CPreferences::GetLanguages(CAtlArray<WORD>& aLanguageIDs)
{
	const SLanguage* pLang = s_aLanguages;
	while (pLang->lid){
		if (pLang->bSupported)// X: [UIC] - [UIChange]
...
void CPreferences::SetLanguage()
{
	...
	if (GetResString(IDS_SOCKETS_INIT_FAILED).IsEmpty()) {
		LoadLangLib(GetMuleDirectory(EMULE_INSTLANGDIR), GetMuleDirectory(EMULE_ADDLANGDIR, false), LANGID_EN_US);
		m_wLanguageID = LANGID_EN_US;
	}
...
/*
CString CPreferences::GetLangNameByID(LANGID lidSelected){ // X: [AEWI] - [AJAX Enabled Web Interface]
	const SLanguage* pLang = s_aLanguages;
	for (;pLang->lid;pLang++){
		if (pLang->lid == lidSelected)
			return CString(pLang->pszISOLocale); 
	}
	ASSERT ( false );
	return _T("");
}
...
CString CPreferences::GetHtmlCharset()
{
	...
	if (pszHtmlCharset == NULL || pszHtmlCharset[0] == _T('\0'))
	{
		...
		if (pszLcLocale)
		{
			...
			UINT uCodepage = 0;
			if (_stscanf_s(pszLcLocale, _T("%[a-zA-Z_].%u"), szLocaleID, &uCodepage) == 2 && uCodepage != 0)
			{
