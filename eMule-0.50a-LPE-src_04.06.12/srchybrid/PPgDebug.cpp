...
void CPPgDebug::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		if (piml){
			HICON hIcon = theApp.LoadIcon(_T("KADSERVER"));
			if (hIcon){
				iImgServer = piml->Add(hIcon);
				VERIFY( ::DestroyIcon(hIcon) );
			}

			hIcon = theApp.LoadIcon(_T("CLIENTSKNOWN"));
			if (hIcon){
...
#undef ADD_INTEGER_ITEM
	...
	for (size_t i = 0; i < ARRSIZE(m_cb); i++)
	...
	for (size_t i = 0; i < ARRSIZE(m_lv); i++)
		DDX_TreeEdit(pDX, IDC_DEBUG_OPTS, m_lv[i], m_levels[i]);

	for (size_t i = 0; i < ARRSIZE(m_htiInteger); i++)
...
LRESULT CPPgDebug::OnTreeOptsCtrlNotify(WPARAM wParam, LPARAM /*lParam*/)
{
	if (wParam == IDC_DEBUG_OPTS){
		...
		SetModified();
		m_bModified = true; // X: [CI] - [Code Improvement] Apply if modified
	}
	return 0;
}