...
#include "sharedfilelist.h"//Upload Permission
	...
	, m_ctrlTreeOptions(theApp.m_iDfltImageListColorFlags)
{
	...
	*/
	...
	// <== Invisible Mode [TPT/MoNKi] - Stulle
	m_iPermission = 0;//Upload Permission
...
//Upload Permission +
	m_htiPermission = NULL;
	m_htiPermAll = NULL;
	m_htiPermNoone = NULL;
	m_htiPermUpMana = NULL;
	m_htiQueueRankMax = NULL;
	m_htiUpDownMax = NULL;
	m_htiOnlyEmule = NULL;
//Upload Permission -
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		int iImgPermission = 8;//Upload Permission
		...
		if (piml){
			...
			iImgPermission =piml->Add(CTempIconLoader(_T("UPLOAD")));//Upload Permission
	}
		...
		*/
		...
		m_htiQuickStartEnable = m_ctrlTreeOptions.InsertCheckBox(_T("")/*GetResString(IDS_X_QUICK_START_ENABLE)*/, TVI_ROOT, m_uQuickStart, 0);
		m_htiQuickStartTime = m_ctrlTreeOptions.InsertItem(_T("")/*GetResString(IDS_X_QUICK_START_TIME)*/, TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, /*m_htiQuickStart*/m_htiQuickStartEnable);
		m_ctrlTreeOptions.AddEditBox(m_htiQuickStartTime,RUNTIME_CLASS(CNumTreeOptionsEdit));
		m_htiQuickStartTimePerFile = m_ctrlTreeOptions.InsertItem(_T("")/*GetResString(IDS_X_QUICK_START_TIME_PER_FILE)*/, TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, /*m_htiQuickStart*/m_htiQuickStartEnable);
		m_ctrlTreeOptions.AddEditBox(m_htiQuickStartTimePerFile,RUNTIME_CLASS(CNumTreeOptionsEdit));
		m_htiQuickMaxConperFive = m_ctrlTreeOptions.InsertItem(_T("")/*GetResString(IDS_X_QUICK_START_MAXPER5)*/, TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, /*m_htiQuickStart*/m_htiQuickStartEnable);
		m_ctrlTreeOptions.AddEditBox(m_htiQuickMaxConperFive,RUNTIME_CLASS(CNumTreeOptionsEdit));
		m_htiQuickMaxHalfOpen = m_ctrlTreeOptions.InsertItem(_T("")/*GetResString(IDS_X_QUICK_START_MAXHALF)*/, TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, /*m_htiQuickStart*/m_htiQuickStartEnable);
		m_ctrlTreeOptions.AddEditBox(m_htiQuickMaxHalfOpen,RUNTIME_CLASS(CNumTreeOptionsEdit));
		m_htiQuickMaxConnections = m_ctrlTreeOptions.InsertItem(_T("")/*GetResString(IDS_X_QUICK_START_MAXCON)*/, TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, /*m_htiQuickStart*/m_htiQuickStartEnable);
		...
		//Upload Permission +
		m_htiPermission = m_ctrlTreeOptions.InsertGroup(_T("")/*GetResString(IDS_DEFAULT) + _T(" ") + GetResString(IDS_PW_CON_UPLBL) + _T(" ") + GetResString(IDS_PERMISSION))*/, iImgPermission, TVI_ROOT);
		m_htiPermAll = m_ctrlTreeOptions.InsertRadioButton(_T("")/*GetResString(IDS_PW_EVER)*/, m_htiPermission, m_iPermission == 0);
		m_htiPermNoone = m_ctrlTreeOptions.InsertRadioButton(_T("")/*GetResString(IDS_PW_NOONE)*/, m_htiPermission, m_iPermission == 1);
		m_htiPermUpMana = m_ctrlTreeOptions.InsertRadioButton(_T("")/*GetResString(IDS_KADSTATUS_ACTIVE)*/, m_htiPermission, m_iPermission == 2);
		m_htiOnlyEmule = m_ctrlTreeOptions.InsertCheckBox(_T("Only eMule"), m_htiPermUpMana, m_bOnlyEmule);
		m_htiQueueRankMax = m_ctrlTreeOptions.InsertItem(_T("QueueRank (max.QR)"), TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, m_htiPermUpMana);
		m_ctrlTreeOptions.AddEditBox(m_htiQueueRankMax, RUNTIME_CLASS(CNumTreeOptionsEdit));
        m_htiUpDownMax = m_ctrlTreeOptions.InsertItem(_T("UL > DL (max.MB)"), TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, m_htiPermUpMana);
		m_ctrlTreeOptions.AddEditBox(m_htiUpDownMax, RUNTIME_CLASS(CNumTreeOptionsEdit));
		//Upload Permission -
		...
		m_ctrlTreeOptions.Expand(m_htiQuickStartEnable, TVE_EXPAND); // NEO: QS - [QuickStart]
		m_ctrlTreeOptions.Expand(m_htiInvisibleMode, TVE_EXPAND);  // Invisible Mode [TPT/MoNKi]
//Upload Permission +
		m_ctrlTreeOptions.Expand(m_htiPermission, TVE_EXPAND);
		m_ctrlTreeOptions.Expand(m_htiPermUpMana, TVE_EXPAND);
  //Upload Permission -
...
//Upload Permission +
	DDX_TreeRadio(pDX, IDC_EXT_OPTS, m_htiPermission, m_iPermission);
    DDX_TreeEdit(pDX, IDC_EXT_OPTS, m_htiQueueRankMax, m_iMaxQR);
	DDX_TreeEdit(pDX, IDC_EXT_OPTS, m_htiUpDownMax, m_iSLI);
	DDX_TreeCheck(pDX, IDC_EXT_OPTS, m_htiOnlyEmule, m_bOnlyEmule);
//Upload Permission -
...
//Upload Permission +
	m_iPermission = thePrefs.m_iPermission;
    m_iSLI = thePrefs.m_iSLI;
	m_iMaxQR = thePrefs.m_iMaxQR;
	m_bOnlyEmule = thePrefs.m_bOnlyEmule;
//Upload Permission -
...
//Upload Permission +
		thePrefs.SetPermission(m_iPermission);
		theApp.sharedfiles->Reload(false);// X: [QOH] - [QueryOnHashing]
        thePrefs.m_iMaxQR = m_iMaxQR;
		thePrefs.m_iSLI = m_iSLI;
		thePrefs.m_bOnlyEmule = m_bOnlyEmule;
//Upload Permission -
...
//Upload Permission +
		if (m_htiPermission) m_ctrlTreeOptions.SetItemText(m_htiPermission, GetResString(IDS_DEFAULT) + _T(" ") + GetResString(IDS_PW_CON_UPLBL) + _T(" ") + GetResString(IDS_PERMISSION));
		if (m_htiPermAll) m_ctrlTreeOptions.SetItemText(m_htiPermAll, GetResString(IDS_PW_EVER));
		if (m_htiPermNoone) m_ctrlTreeOptions.SetItemText(m_htiPermNoone, GetResString(IDS_PW_NOONE));
		if (m_htiPermUpMana) m_ctrlTreeOptions.SetItemText(m_htiPermUpMana, GetResString(IDS_KADSTATUS_ACTIVE));
//Upload Permission -
