...
struct FlushDone_Struct
{
	...
	FlushDone_Struct() : evTerminated(false) {}
};
...
public:
	...
	EPartFileLoadResult	LoadPartFile(LPCTSTR in_directory, LPCTSTR filename); //filename = *.part.met
	...
	void	DrawProgressBar(CDC* dc, LPCRECT rect, bool bFlat); // morph4u :: PercentBar :: Start
	void	DrawStatusBar(CDC* dc, LPCRECT rect, bool bFlat);
	void	DrawPartStatusBar(CDC* dc, LPCRECT rect, UINT nPart, bool bFlat);
...
//	uint64	GetRemainingAvailableData(const CUpDownClient* sender) const;
	...
	EPartFileStatus	GetPartFileStatus(); // Status Icons
...
private:
	...
	static CBarShader s_ProgressBar; // morph4u :: PercentBar :: Start
...
public:
	...
	virtual bool run();
	virtual void setRemoved(void*file);
private:
