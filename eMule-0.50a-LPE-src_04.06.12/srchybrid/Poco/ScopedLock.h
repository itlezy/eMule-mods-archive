...
{
public:
	SingleLock(M& mutex, bool autolock = false): _mutex(mutex), locked(false)
	{
		if(autolock)
			Lock();
	}
	...
	{
		if (!locked)
			locked = _mutex.Lock(milliseconds);
		return locked;
	}
	bool tryLock()
	{
...
private:
	...
	SingleLock();
	SingleLock(const SingleLock&);
	SingleLock& operator = (const SingleLock&);
};
