...
CPreviewThread::CPreviewThread(CPartFile* pPartFile, LPCTSTR pszCommand, LPCTSTR pszCommandArgs)
: Thread(true), m_pPartfile(pPartFile), m_strCommand(pszCommand), m_strCommandArgs(pszCommandArgs)
{
}
...
void CPreviewApps::UpdateApps()
{
	...
	{
		struct _stat64 st;
		if (_tstat64(GetDefaultAppsFile(), &st) == 0 && st.st_mtime > m_tDefAppsFileLastModified)
			ReadAllApps();
	}
}

size_t CPreviewApps::GetAllMenuEntries(CMenu& rMenu, const CPartFile* file)
{
	UpdateApps();

	for (size_t i = 0; i < m_aApps.GetCount(); i++)
	{
...
void CPreviewApps::RunApp(CPartFile* file, size_t uMenuID)
{
...
size_t CPreviewApps::GetPreviewApp(const CPartFile* file)
{
	...
	for (size_t i = 0; i < m_aApps.GetCount(); i++)
	{
		...
		for (size_t j = 0; j < rApp.astrExtensions.GetCount(); j++)
		{
...
{
	size_t iApp = GetPreviewApp(file);
	if (iApp == (size_t)-1)
		return NotHandled;
	...
	{
		if (file->headerSize<=pApp.ullMinStartOfFile)
			return No;
	}
