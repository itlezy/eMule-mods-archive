...
CAtlList<CAICHRequestedData> CAICHRecoveryHashSet::m_liRequestedData;
CAtlList<CAICHHash>		  CAICHRecoveryHashSet::m_liAICHHashsStored;
Poco::FastMutex			  CAICHRecoveryHashSet::m_mutKnown2File;
...
CAtlList<CAICHHashTree>	CAICHRecoveryHashSet::m_liBufferedHashTree;
uint32	CAICHRecoveryHashSet::m_nLastSaved = 0;
uint64	CAICHRecoveryHashSet::m_uBufferedSize = 0;
Poco::FastMutex	CAICHRecoveryHashSet::m_mutSaveHashSet;
//zz_fly :: end
...
bool CAICHUntrustedHash::AddSigningIP(uint32 dwIP, bool bTestOnly){
	...
	for (size_t i=0; i < m_adwIpsSigning.GetCount(); i++){
...
bool CAICHRecoveryHashSet::GetPartHashs(CAtlArray<CAICHHash>& rResult) const
{
	ASSERT( m_pOwner );
	...
	size_t uPartCount = (uint16)(((uint64)m_pOwner->GetFileSize() + (PARTSIZE - 1)) / PARTSIZE);
	if (uPartCount <= 1)
		return true; // No AICH Part Hashs
	for (size_t nPart = 0; nPart < uPartCount; nPart++)
	{
		uint64 nPartStartPos = (uint64)nPart*PARTSIZE;
		uint64 nPartSize = (uint64)min(PARTSIZE, (uint64)m_pOwner->GetFileSize()-nPartStartPos);
...
bool CAICHRecoveryHashSet::ReadRecoveryData(uint64 nPartStartPos, CSafeMemFile* fileDataIn){
	...
	*/
	...
	uint8 nLevel = 0;
	size_t nPartSize = (uint32)min(PARTSIZE, (uint64)m_pOwner->GetFileSize()-nPartStartPos);
	m_pHashTree.FindHash(nPartStartPos, nPartSize,&nLevel);
	...
	if (VerifyHashTree(true)){
		...
		for (size_t nPartPos = 0; nPartPos < nPartSize; nPartPos += EMBLOCKSIZE){
...
bool CAICHRecoveryHashSet::SaveHashSet(){
	...
	Poco::FastMutex::SingleLock lockSaveHashSet(m_mutSaveHashSet);
...
bool CAICHRecoveryHashSet::SaveHashSetToFile(bool forced){
	...
	Poco::FastMutex::SingleLock lockKnown2Met(m_mutKnown2File);
...
void CAICHRecoveryHashSet::UntrustedHashReceived(const CAICHHash& Hash, uint32 dwFromIP){
	...
	bool bFound = false;
	bool bAdded = false;
	for (size_t i = 0; i < m_aUntrustedHashs.GetCount(); i++){
	...
	if (!bFound)
	{
		for (size_t i = 0; i < m_aUntrustedHashs.GetCount(); i++)
		{
			...
			{
				AddDebugLogLine(DLP_LOW, false, _T("Received different AICH hashs for file %s from IP/20 %s, ignored"), (m_pOwner != NULL) ? m_pOwner->GetFileName() : _T(""), ipstr(dwFromIP)); // dontsueme
	...
	size_t nSigningIPsTotal = 0;	// unique clients who send us a hash
	size_t nMostTrustedPos = (size_t)-1;  // the hash which most clients send us
	size_t nMostTrustedIPs = 0;
	for (size_t i = 0; i < m_aUntrustedHashs.GetCount(); i++){
		nSigningIPsTotal += m_aUntrustedHashs[i].m_adwIpsSigning.GetCount();
		if (m_aUntrustedHashs[i].m_adwIpsSigning.GetCount() > nMostTrustedIPs){
			nMostTrustedIPs = m_aUntrustedHashs[i].m_adwIpsSigning.GetCount();
			nMostTrustedPos = i;
		}
	}
	if (nMostTrustedPos == (size_t)-1 || nSigningIPsTotal == 0){
		ASSERT( false );
		return;
	}
...
void CAICHRecoveryHashSet::ClientAICHRequestFailed(CUpDownClient* pClient){
	...
	if(theApp.downloadqueue->IsPartFile(data.m_pPartFile)){
		...
		data.m_pPartFile->RequestAICHRecovery(data.m_nPart, true);
	}
}
...
bool CAICHRecoveryHashSet::IsPartDataAvailable(uint64 nPartStartPos){
	...
	size_t nPartSize = (size_t)(min(PARTSIZE, (uint64)m_pOwner->GetFileSize()-nPartStartPos));
	for (size_t nPartPos = 0; nPartPos < nPartSize; nPartPos += EMBLOCKSIZE){
...
#define TESTSIZE m_pHashTree.m_nDataSize
	...
	for (i = 0; i+9728000 < TESTSIZE; i += 9728000){
		...
		/*uint32 nRandomCorruption = rng.getUInt32() % (file.GetLength()-4);
...
bool CAICHRecoveryHashSet::TrustMajority(){// X: [IP] - [Import Parts]
	size_t nMostTrustedPos = (size_t)-1;  // the hash which most clients send us
	size_t nMostTrustedIPs = 0;
	for (size_t i = 0; i < m_aUntrustedHashs.GetCount(); i++){
		if (m_aUntrustedHashs[i].m_adwIpsSigning.GetCount() > nMostTrustedIPs){
	...
	if (nMostTrustedPos == (size_t)-1)
		return false;
	SetStatus(AICH_TRUSTED);
	if (GetMasterHash() != m_aUntrustedHashs[nMostTrustedPos].m_Hash){
		SetMasterHash(m_aUntrustedHashs[nMostTrustedPos].m_Hash, AICH_TRUSTED);
		FreeHashSet();
	}
	return true;
