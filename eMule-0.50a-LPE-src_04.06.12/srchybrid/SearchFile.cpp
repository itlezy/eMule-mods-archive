						 ...
						 uint32 nSearchID, uint32 nServerIP, uint16 nServerPort, LPCTSTR pszDirectory, bool bKademlia, bool bServerUDPAnswer)
{
	...
	for (UINT i = 0; i < tagcount; i++)
	{
		...
		if (tag)
		{
			...
			if (tag->GetNameID() == FT_FILERATING && tag->IsInt())
			{
				...
				tag->SetInt(uAvgRating / (255/5/*RatingExcellent*/));
			}
			else if (tag->GetNameID() == FT_AICH_HASH && tag->IsStr())
			{
				CAICHHash hash;
				if (DecodeBase32(tag->GetStr(),hash) == HASHSIZE)
	...
	SetFileName(GetStrTagValue(FT_FILENAME), false, rstrFileType[0] == 0, true);
...
void CSearchFile::StoreToFile(CFileDataIO& rFile) const
{
	...
	for (size_t pos = 0; pos < taglist.GetCount(); pos++){
...
{
	return GetInt64TagValue(FT_LASTSEENCOMPLETE);// X: [64T] - [64BitTime]
}
