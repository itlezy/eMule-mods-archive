...
CServer::CServer(const ServerMet_Struct* in_data)
{
	...
	if((isfiltered = theApp.ipfilter->IsFiltered(ip))// X: [CI] - [Code Improvement]
		 ...
		 AddDebugLogLine(false, _T("IPFilter(Server.Met): Filtered server \"%s\" (IP=%s) - IP filter"), GetAddress(), ipstr(GetIP()));
	files = 0;
	users = 0;
	m_uPreference = 0;
	ping = 0;
...
CServer::CServer(uint16 in_port, LPCTSTR i_addr)
{
	...
	if((isfiltered = theApp.ipfilter->IsFiltered(ip))// X: [CI] - [Code Improvement]
		 && thePrefs.GetLogFilteredIPs())
		AddDebugLogLine(false, _T("IPFilter(AddServer): Filtered server \"%s\" (IP=%s) - IP filter"), GetAddress(), ipstr(GetIP()));
	_tcscpy_s(ipfull, ipstr(ip));
	files = 0;
	users = 0;
	m_uPreference = 0;
