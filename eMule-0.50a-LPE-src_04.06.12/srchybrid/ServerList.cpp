...
bool CServerList::AddServerMetToList(const CString& strFile, bool bMerge) 
{
	...
	if (!servermet.Open(strFile ,CFile::modeRead|CFile::osSequentialScan|CFile::typeBinary|CFile::shareDenyWrite, &fexp)){
		if (!bMerge && thePrefs.GetNetworkED2K()){ //morph4u todo ???
	...
	try{
		uint8 version = servermet.ReadUInt8();
		if (version != 0xE0 && version != MET_HEADER){
			servermet.Close();
			LogError(LOG_STATUSBAR,GetResString(IDS_ERR_BADSERVERMETVERSION), version);
			return false;
		}
		theApp.emuledlg->serverwnd->serverlistctrl.ShowWindow(SW_HIDE);
		theApp.emuledlg->serverwnd->serverlistctrl.SetRedraw(FALSE);
		uint_ptr fservercount = servermet.ReadUInt32();
		
		ServerMet_Struct sbuffer;
		uint_ptr iAddCount = 0;
		for (uint_ptr j = 0; j < fservercount; j++)
		{
			// get server
	...
	theApp.emuledlg->serverwnd->serverlistctrl.ShowWindow(SW_SHOW);
	return true;
}
...
bool CServerList::AddServer(const CServer* pServer, bool bAddTail)
{
	...
	if (thePrefs.GetFilterServerByIP()){
		...
		//if (pServer->HasDynIP())
		//	return false;
		if (pServer->IsFiltered()){// X: [CI] - [Code Improvement]
			//if (thePrefs.GetLogFilteredIPs())
...
void CServerList::ServerStats()
{
	...
	if (theApp.IsConnected() && theApp.serverconnect->IsUDPSocketAvailable() && list.GetCount() > 0)
	{
		...
		uint64 tNow = (uint64)time(NULL);// X: [64T] - [64BitTime]
		...
		if (!ping_server->GetCryptPingReplyPending() && (tNow - ping_server->GetLastPingedTime()) >= UDPSERVSTATREASKTIME && theApp.GetPublicIP() != 0 && thePrefs.IsServerCryptLayerUDPEnabled()){
			...
			SFMT&rng = *t_rng;
			uint32 uChallenge = 0x55AA0000 + rng.getUInt16();
			ping_server->SetChallenge(uChallenge);
			PokeUInt32(packet->pBuffer, uChallenge);
			ping_server->SetLastPinged(GetTickCount());
			ping_server->SetLastPingedTime(tNow - (rng.getUInt32() % HR2S(1)));
...
{
	size_t i = 0;
	for (POSITION pos1 = list.GetHeadPosition(), pos2; (pos2 = pos1) != NULL; ) {// X: [CI] - [Code Improvement]
...
void CServerList::GetUserSortedServers()
{
	...
	list.RemoveAll();
	size_t iServers = serverListCtrl.GetItemCount();
	for (size_t i = 0; i < iServers; i++)
...
CServer* CServerList::GetNextServer(bool bOnlyObfuscated)
{
	...
	while (!nextserver && i < list.GetCount()){
		...
		if((!bOnlyObfuscated || ((CServer*)list.GetAt(posIndex))->SupportsObfuscationTCP())
			&& (!thePrefs.GetFilterServerByIP() || !((CServer*)list.GetAt(posIndex))->IsFiltered()))// X: [CI] - [Code Improvement]
...
bool CServerList::SaveServermetToFile()
{
	...
	try{
		...
		size_t fservercount = list.GetCount();
		servermet.WriteUInt32((uint32)fservercount);
		
		for (size_t j = 0; j < fservercount; j++)
		{
...
//>>> Spike2::Aux Ports support [lugdunummaster]
			//servermet.WriteUInt16(nextserver->GetPort());
			servermet.WriteUInt16(nextserver->GetConnPort());
//<<< Spike2::Aux Ports support [lugdunummaster]
			
			uint32 uTagCount = 0;
			uint_ptr uTagCountFilePos = (uint_ptr)servermet.GetPosition();
...
//>>> Spike2::Aux Ports support [lugdunummaster]
			...
			{
				CTag auxportslist("auxportslist", nextserver->GetPort());
...
//<<< Spike2::Aux Ports support [lugdunummaster]
		...
		oldservermet += SERVER_MET_FILENAME _T(".old");
...
void CServerList::AddServersFromTextFile(const CString& strFilename)
{
	...
	if (!f.Open(strFilename, (bIsUnicodeFile ? (CFile::modeRead | CFile::shareDenyWrite | CFile::typeBinary)
...
size_t CServerList::GetPositionOfServer(const CServer* pServer) const
{
	size_t iPos = (size_t)-1;
	for (POSITION pos = list.GetHeadPosition(); pos != NULL; ){
		iPos++;
...
void CServerList::CheckForExpiredUDPKeys() {
	...
	uint_ptr cKeysTotal = 0;
	uint_ptr cKeysExpired = 0;
	uint_ptr cPingDelayed = 0;
	const uint32 dwIP = theApp.GetPublicIP();
	const uint64 tNow = (uint64)time(NULL);// X: [64T] - [64BitTime]
	ASSERT( dwIP != 0 );
	...
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
		...
		if (pServer->SupportsObfuscationUDP() && pServer->GetServerKeyUDP(true) != 0 && pServer->GetServerKeyUDPIP() != dwIP){
			...
			if (tNow - pServer->GetRealLastPingedTime() < UDPSERVSTATMINREASKTIME){
				...
				pServer->SetLastPingedTime((tNow - (uint64)UDPSERVSTATREASKTIME) + (UDPSERVSTATMINREASKTIME - (tNow - pServer->GetRealLastPingedTime())));// X: [64T] - [64BitTime]
			}
			else
