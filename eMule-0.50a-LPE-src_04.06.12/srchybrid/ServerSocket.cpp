...
BOOL CServerSocket::OnHostNameResolved(const SOCKADDR_IN *pSockAddr)
{
	...
	if (cur_server->HasDynIP())
	{
		...
		cur_server->SetFiltered(theApp.ipfilter->IsFiltered(pSockAddr->sin_addr.S_un.S_addr));// X: [CI] - [Code Improvement]
		...
		//
		if (thePrefs.GetFilterServerByIP() && cur_server->IsFiltered()) {// X: [CI] - [Code Improvement]
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(TCP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter"), pServer ? pServer->GetAddress() : cur_server->GetAddress(), ipstr(cur_server->GetIP()));
			if (pServer)
		...
		if (pServer)
			pServer->SetFiltered(cur_server->IsFiltered());// X: [CI] - [Code Improvement]
	}
...
void CServerSocket::OnReceive(int nErrorCode){
	...
	if(connectionstate == CS_ERROR){// X: [BF] - [Bug Fix] Memory Leak on Low ID Smart Check
		serverconnect->DestroySocket(this);
		return;
	}
		...
		switch (opcode)
		{
				...
				while (!message.IsEmpty())
				{
					if (_tcsnicmp(message, _T("server version"), 14) == 0){
						...
						if (pServer){
							...
							if (_stscanf_s(strVer, _T("%u.%u"), &nVerMaj, &nVerMin) == 2)
			...
			case OP_IDCHANGE:{
				...
				if (cur_server){
					...
					if (size >= sizeof(LoginAnswer_Struct)+4){
						...
						if (thePrefs.GetDebugServerTCPLevel() > 0){
							...
							if (dwFlags & SRV_TCPFLG_COMPRESSION)
								strInfo.Append(_T("  Compression=1"));// X: [CI] - [Code Improvement]
					...
					pServer = theApp.serverlist->GetServerByAddress(cur_server->GetAddress(),(uint16) rport);
					if (pServer) {
						pServer->SetTCPFlags(cur_server->GetTCPFlags());
						if (ConnPort) 
							pServer->SetPort((uint16)ConnPort);
						theApp.emuledlg->serverwnd->serverlistctrl.RefreshServer(pServer);
						theApp.emuledlg->serverwnd->UpdateMyInfo();
					}
					//Morph End - added by AndCycle, aux Ports, by lugdunummaster
				...
				if (connectionstate != CS_CONNECTED) {
					ASSERT(serverconnect->IsConnecting());
				   ...
				   && serverconnect->GetClientID() != theApp.last_valid_serverid)
				{
					...
					// about the new IP.
					bool immediate = (GetTickCount() - theApp.last_ip_change > FILEREASKTIME + 60000);
					theApp.clientlist->TrigReaskForDownload(immediate);
					...
					AddLogLine(false, _T("Change from %u (%s ID) to %u (%s ID) detected%s"), 
						...
						immediate? _T(", all sources will be reasked immediately")
				...
				theApp.internetmaybedown = 0; //we have to reopen here if we are using server only
				// Xman end
			...
			case OP_CALLBACKREQUESTED:{
				...
				if (size >= 6)
				{
					...
					if (theApp.ipfilter->IsFiltered(dwIP)){
						...
						if (thePrefs.GetLogFilteredIPs())
							AddDebugLogLine(false, _T("Ignored callback request (IP=%s) - IP filter"), ipstr(dwIP));
						break;
					}
