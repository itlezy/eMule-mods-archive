...
static const UINT colStrID[]={
	...
	,IDS_CODEC
	,IDS_ONQUEUE
    ,IDS_PERMISSION//Upload Permission
};
...
CSharedFilesCtrl::~CSharedFilesCtrl()
{
	...
	if (m_PermMenu) VERIFY( m_PermMenu.DestroyMenu() );//Upload Permission
...
void CSharedFilesCtrl::Init()
{
 ...
 // <== Spread bars [Slugfiller/MorphXT] - Stulle
	...
	InsertColumn(19,GetResString(IDS_PERMISSION),       LVCFMT_LEFT,  80);//Upload Permission
	SetAllIcons();
	CreateMenues();
	LoadSettings();
	...
	for(int iCurrent = 0; iCurrent < iCount; iCurrent++){
		...
		if(iColumnWidth > 2*sm_iSubItemInset && cur_rec.right>0){// X: [DDHC] - [Don't Draw Hidden Column]
			...
			//Upload Permission +
				if(pKnownFile)
				{
					int Perm = pKnownFile->GetPermissions()>=0?pKnownFile->GetPermissions():thePrefs.GetPermission();
					if (iColumn==0)
						dc.SetTextColor(::GetSysColor(COLOR_WINDOWTEXT));
					else if (Perm == PERM_NOONE)
						dc.SetTextColor(::GetSysColor(COLOR_GRAYTEXT));
					else if (Perm == PERM_UPMANA)
						dc.SetTextColor(RGB(128,0,64)); 
				}
			//Upload Permission -
			...
			case 19:{
				...
				switch (Perm)
				{
					case PERM_ALL:
						...
						break;
					case PERM_NOONE: 
						buffer = GetResString(IDS_PW_NOONE); 
						break;
                    case PERM_UPMANA: 
						buffer = GetResString(IDS_KADSTATUS_ACTIVE); 
						break;
					default: 
						buffer = _T("?");
						break;
...
// ShellContextMenu :: End
	...
	UINT uPrioMenuItem = 0;
        UINT uPermMenuItem = 0;//Upload Permission
...
//Upload Permission +
			...
			else if (((CKnownFile*)pFile)->GetPermissions() == PERM_UPMANA)
				uCurPermMenuItem = MP_PERMUPMANA;
			else
				ASSERT(0);
...
//Upload Permission +
		...
		CString buffer;
		switch (thePrefs.GetPermission()){
			case PERM_ALL:
				buffer.Format(_T(" (%s)"),GetResString(IDS_PW_EVER));
				break;
			case PERM_NOONE:
				buffer.Format(_T(" (%s)"),GetResString(IDS_PW_NOONE));
				break;
            case PERM_UPMANA:
				buffer.Format(_T(" (%s)"),GetResString(IDS_KADSTATUS_ACTIVE));
				break;
			default:
				buffer = _T(" (?)");
				break;
	...
	m_PermMenu.CheckMenuRadioItem(MP_PERMALL,MP_PERMUPMANA, uPermMenuItem, 0); 
//Upload Permission -
...
// ShellContextMenu :: End
			...
			case MP_OPENFOLDER:
				if (file && !file->IsPartFile())
					...
					theDesktop.ShellExecute(_T("open"), file->GetPath()); // netfinity: Better support for WINE Gnome/KDE desktops
				break; 
// ShellContextMenu :: Start
			//case MP_RENAME:
// ShellContextMenu :: End
			...
			case MP_PERMALL: {
					...
					if (file == NULL && IsKindOfCKnownFile(file)/*!file->IsKindOf(RUNTIME_CLASS(CKnownFile))*/) 
						...
						case MP_PERMNONE:
							...
							break;
                        case MP_PERMUPMANA:
							((CKnownFile*)file)->SetPermissions(PERM_UPMANA);
							break;
...
//Upload Permission +
		...
		m_PermMenu.AppendMenu(MF_STRING,MP_PERMNONE, GetResString(IDS_PW_NOONE));
		m_PermMenu.AppendMenu(MF_STRING,MP_PERMUPMANA, GetResString(IDS_KADSTATUS_ACTIVE));
...
//Upload Permission +
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_PermMenu.m_hMenu, GetResString(IDS_PERMISSION) + _T(" (") + GetResString(IDS_PW_CON_UPLBL) + _T(')'));	
//Upload Permission -
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
