...
void CSpeedGraph::Init(uint_ptr trafficEntries) 
{
...
void CSpeedGraphWnd::Update_TrafficGraph()
{
	if(!CemuleDlg::IsRunning())
		return;
	uint32 eMuleIn = theApp.emuledlg->GetDownloadDatarate();	//uint32 eMuleInOverall;
	uint32 eMuleOut = theApp.emuledlg->GetUploadDatarate(); //uint32 eMuleOutOverall;
...
void CSpeedGraphWnd::OnContextMenu(CWnd* /*pWnd*/, CPoint point){
	theApp.emuledlg->ShowTrayPopup(point);
}
