...
void CTreeOptionsCtrlEx::UpdateCheckBoxGroup(HTREEITEM hItem)
{
	...
	if (hParent && IsCheckBox(hParent))
	{
		...
		else
		{
			...
			SetEnabledSemiCheckBox(hParent, IsItemEnabled(hParent));
		}
	}
...
BOOL CTreeOptionsCtrlEx::NotifyParent(UINT_PTR uCode, HTREEITEM hItem)
{
	CWnd* pWnd = GetParent();
	if (!pWnd)
		return FALSE;
	...
	ton.nmhdr.idFrom = GetWindowLongPtr(m_hWnd, GWL_ID);
	ton.nmhdr.code = uCode;
	ton.hItem = hItem;
	return (BOOL)pWnd->SendMessage(UM_TREEOPTSCTRL_NOTIFY, GetWindowLongPtr(m_hWnd, GWL_ID), (LPARAM)&ton);
}
...
void EditTextFloatFormat(CDataExchange* pDX, int nIDC, HTREEITEM hItem, void* pData, double value, int nSizeGcvt)
{
	...
	{
		TCHAR szBuffer[400];
		_sntprintf(szBuffer, _countof(szBuffer) - 1, _T("%.*g"), nSizeGcvt, value);
	...
	// only supports windows output formats - no floating point
{
	...
	{
		TCHAR szT[64];
		_vsntprintf(szT, _countof(szT) - 1, lpszFormat, pData);
