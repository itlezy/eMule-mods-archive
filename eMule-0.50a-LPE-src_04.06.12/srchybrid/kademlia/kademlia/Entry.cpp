...
unordered_map<uint32, uint32> CKeyEntry::s_mapGlobalPublishIPs;
#else
CAtlMap<uint32, uint32> CKeyEntry::s_mapGlobalPublishIPs;
#endif
...
size_t CEntry::GetTagCount() const // Adds filename and size to the count if not empty, even if they are not stored as tags
{
	return m_listTag.size() + ((m_uSize != 0) ? 1 : 0) + (GetCommonFileName().IsEmpty() ? 0 : 1);
}

void CEntry::WriteTagListInc(CDataIO* pData, size_t nIncreaseTagNumber){
	...
	size_t uCount = GetTagCount() + nIncreaseTagNumber; // will include name and size tag in the count if needed
	ASSERT( uCount <= 0xFF );
...
bool CKeyEntry::SearchTermsMatch(const SSearchTerm* pSearchTerm) const
{
	...
	{
		size_t iStrSearchTerms = pSearchTerm->m_pastr->GetCount();
...
#ifdef REPLACE_ATLMAP
	if (bIncrease)
		++s_mapGlobalPublishIPs[uIP & 0xFFFFFF00];
	else{
		unordered_map<uint32, uint32>::iterator it = s_mapGlobalPublishIPs.find(uIP & 0xFFFFFF00);
		if(it != s_mapGlobalPublishIPs.end()){
			if(--it->second == 0)
				s_mapGlobalPublishIPs.erase(it);
		}
		else
			ASSERT( false );
	}
...
void CKeyEntry::MergeIPsAndFilenames(CKeyEntry* pFromEntry){
	...
	if (pFromEntry == NULL || pFromEntry->m_pliPublishingIPs == NULL){
		...
		if (m_pliPublishingIPs == NULL)
			m_pliPublishingIPs = new CAtlList<structPublishingIP>();
...
#ifdef REPLACE_ATLMAP
		uint32 nCount = 0;
		unordered_map<uint32, uint32>::const_iterator it = s_mapGlobalPublishIPs.find(curEntry.m_uIP & 0xFFFFFF00);
		if(it != s_mapGlobalPublishIPs.end())
			nCount = it->second;
#else
		uint32 nCount = 0;
...
void CKeyEntry::WritePublishTrackingDataToFile(CDataIO* pData){
	...
	uint16 nNewIdxPos = 0;
	CAtlArray<uint16> aNewIndexes;
	for (size_t i = 0; i < m_aAICHHashs.GetCount(); i++)
	{
	...
	{
		if (m_anAICHHashPopularity[i] > 0)
			pData->WriteArray(m_aAICHHashs[i].GetRawHashC(), HASHSIZE);
	}
...
void CKeyEntry::ReadPublishTrackingDataFromFile(CDataIO* pData, bool bIncludesAICH){
	...
	if (bIncludesAICH)
	{
		...
		{
			CAICHHash hash;
			pData->ReadArray(hash.GetRawHash(), HASHSIZE);
	...
	time_t nDbgLastTime = 0; // X: [64T] - [64BitTime]
#endif
...
void CKeyEntry::WriteTagListWithPublishInfo(CDataIO* pData){
	...
	uint32 uPublishers = m_pliPublishingIPs->GetCount() & 0xff;
	uint32 uNames = m_listFileNames.GetCount() & 0xff;
	...
	if (!m_aAICHHashs.IsEmpty())
	{
		...
		for (size_t i = 0; i < m_aAICHHashs.GetCount(); i++)
		{
			...
			if (1 + (HASHSIZE * (byCount + 1)) + (1 * (byCount + 1)) > 250)
			{
		...
		CSafeMemFile fileAICHTag(1+byCount*(1+16));
		...
		const BYTE* byBuffer = fileAICHTag.GetBuffer();
