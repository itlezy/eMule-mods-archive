...
CAtlList<uint32> CKademlia::m_liStatsEstUsersProbes;
...
void CKademlia::Process()
{
	...
	uint_ptr uMaxUsers = 0;
	uint_ptr uTempUsers = 0;
	time_t tNow = time(NULL);
	ASSERT(m_pInstance->m_pPrefs != NULL);
	uint64 uLastContact = m_pInstance->m_pPrefs->GetLastContact();// X: [64T] - [64BitTime]
...
uint_ptr CKademlia::GetKademliaUsers(bool bNewMethod)
{
	if( m_pInstance && m_pInstance->m_pPrefs ){
		if (bNewMethod)
...
uint_ptr CKademlia::GetKademliaFiles()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint_ptr CKademlia::CalculateKadUsersNew(){
	...
	size_t nCut = liMedian.GetCount() / 6;
	...
	nMedian = (uint_ptr)(nAverage / liMedian.GetCount());
	...
	return (uint_ptr)((float)nMedian*fFirewalledModifyTotal);
}
...
bool CKademlia::IsRunningInLANMode()
{
	...
	if (m_tLANModeCheck + 10 <= time(NULL))
	{
		...
		size_t nCount = GetRoutingZone()->GetNumContacts();
