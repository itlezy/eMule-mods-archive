...
void CPrefs::SetKademliaFiles()
{
	...
	uint_ptr nKadAverage = Kademlia::CKademlia::GetIndexed()->GetFileKeyCount();
...
uint64 CPrefs::GetLastContact() const
{
...
uint_ptr CPrefs::GetKademliaUsers() const
{
	return m_uKademliaUsers;
}

void CPrefs::SetKademliaUsers(uint_ptr uVal)
{
	m_uKademliaUsers = uVal;
}

uint_ptr CPrefs::GetKademliaFiles() const
{
...
void CPrefs::SetExternKadPort(uint16 uVal, uint32 uFromIP)
{
	...
	{
		for (size_t i = 0; i < m_anExternPortIPs.GetCount(); i++)
		{
		...
		for (size_t i = 0; i < m_anExternPorts.GetCount(); i++)
		{
