...
CSearch::~CSearch()
{
	...
	for (ContactUnorderedMap::const_iterator itContactMap = m_mapInUse.begin(); itContactMap != m_mapInUse.end(); ++itContactMap)
	...
	if(CKademlia::IsRunning() && GetNodeLoad() > 20)
	{
		switch(GetSearchTypes())
		{
			case CSearch::STOREKEYWORD:
				Kademlia::CKademlia::GetIndexed()->AddLoad(GetTarget(), ((uint64)(DAY2S(7)*((double)GetNodeLoad()/100.0))+(uint64)time(NULL)));// X: [64T] - [64BitTime]
				break;
		}
	}
...
void CSearch::JumpStart()
{
	...
	if (m_uLastResponse + SEC(3) > (uint64)time(NULL))
		return;
	...
	if (pRequestedMoreNodesContact == NULL && GetRequestContactCount() == KADEMLIA_FIND_VALUE && m_mapTried.size() >= 3*KADEMLIA_FIND_VALUE)
	{
		...
		for (size_t i = 0; i < KADEMLIA_FIND_VALUE; i++)
		{
	...
	try
	{
		if (pFromContact != NULL)
			...
			//bool bProvidedCloserContacts = false;
				MAP<uint32, uint32> mapReceivedIPs;
				MAP<uint32, uint32> mapReceivedSubnets;
			...
			m_mapResponded.insert(uFromDistance);
...
void CSearch::StorePacket()
{
	...
	switch(m_uType)
	{
		case FILE:
			{
				CSafeMemFile m_pfileSearchTerms(16+2+8);
				m_pfileSearchTerms.WriteUInt128(&m_uTarget);
				if (pFromContact->GetVersion() >= KADEMLIA_VERSION3_47b)// X: [CI] - [Code Improvement]
				{
...
#endif
						if (pFromContact->GetVersion() >= KADEMLIA_VERSION6_48b){// X: [CI] - [Code Improvement]
		...
		case KEYWORD:
			{
				...
				CSafeMemFile m_pfileSearchTerms(16+2+m_uSearchTermsDataSize);
				m_pfileSearchTerms.WriteUInt128(&m_uTarget);
				if (pFromContact->GetVersion() >= KADEMLIA_VERSION3_47b)// X: [CI] - [Code Improvement]
				{
				...
				else if (pFromContact->GetVersion() >= KADEMLIA_VERSION3_47b)// X: [CI] - [Code Improvement]
				{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
					if (thePrefs.GetDebugClientKadUDPLevel() > 0)
						DebugSend("KADEMLIA2_SEARCH_KEY_REQ", pFromContact->GetIPAddress(), pFromContact->GetUDPPort());
		...
		/*case NOTES:
			{
				...
				CSafeMemFile m_pfileSearchTerms(16+16);
				m_pfileSearchTerms.WriteUInt128(&m_uTarget);
				...
				if (pFile)
				{
					...
					if( theApp.IsFirewalled() )
					{
						...
						if (bDirectCallback){
							...
							if (pFromContact->GetVersion() >= KADEMLIA_VERSION2_47a)// X: [CI] - [Code Improvement]
							{
		...
		case STOREKEYWORD:
			{
				...
				while(iCount && (itListFileID != m_listFileIDs.end()))
				{
					...
					else if (pFromContact->GetVersion() >= KADEMLIA_VERSION2_47a)// X: [CI] - [Code Improvement]
					{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
						if (thePrefs.GetDebugClientKadUDPLevel() > 0)
							DebugSend("KADEMLIA2_PUBLISH_KEY_REQ", pFromContact->GetIPAddress(), pFromContact->GetUDPPort());
		...
		case FINDBUDDY:
			{
				...
				CSafeMemFile m_pfileSearchTerms(16+16+2);
...
void CSearch::ProcessResultFile(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	for (TagList::const_iterator itTagList = plistInfo->begin(); itTagList != plistInfo->end(); ++itTagList)
	{
		...
		else if (!pTag->m_name.Compare(TAG_BUDDYHASH))
		{
			...
			if (pTag->IsStr() && pTag->GetStr().GetLength() == 16*2 && wcsmd4(pTag->GetStr(), ucharBuddyHash))
...
void CSearch::ProcessResultKeyword(const CUInt128 &uAnswer, TagList *plistInfo, uint32 uFromIP, uint16 uFromPort)
{
	...
	CAtlArray<CAICHHash> aAICHHashs;
	CAtlArray<uint8> aAICHHashPopularity;
...
void CSearch::SendFindValue(CContact* pContact, bool bReAskMore)
{
	...
	try
	{
		...
		if (pContact->GetVersion() >= KADEMLIA_VERSION2_47a)// X: [CI] - [Code Improvement]
		{
			if (pContact->GetVersion() >= KADEMLIA_VERSION6_48b){// X: [CI] - [Code Improvement]
...
static size_t GetMetaDataWords(CAtlArray<CString>& rastrWords, const CString& rstrData)
{
...
static bool IsRedundantMetaData(const CAtlArray<CString>& rastrFileNameWords, const CString& rstrMetaData)
{
	...
	{
		iMetaDataWords++;
		for (size_t i = 0; i < rastrFileNameWords.GetCount(); i++)
		{
	...
	try
	{
		if (pFile && byIO)
		{
			...
			if (pFile->GetMetaDataVer() > 0)
			{
				...
				CAtlArray<CString> astrFileNameWords;
				for (int iIndex = 0; iIndex < ARRSIZE(_aMetaTags); iIndex++)
				{
					...
					if (pTag)
					{
						...
						if (pTag->IsStr())
						{
							...
							{
								if (astrFileNameWords.GetCount() == 0)
...
uint_ptr CSearch::GetAnswers() const
{
