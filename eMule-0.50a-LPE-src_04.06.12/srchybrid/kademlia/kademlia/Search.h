...
namespace Kademlia
{
	...
	class CSearch
	{
		...
		private:
			...
			uint64 m_uLastResponse;// X: [64T] - [64BitTime]
			uint32 m_uSearchID;
			...
			ContactUnorderedMap m_mapTried;
#ifdef HAVE_UNORDERED
			unordered_set<CUInt128, uint128_unordered, uint128_unordered> m_mapResponded;
#else
			//std::map<CUInt128, bool> m_mapResponded;
			std::set<CUInt128> m_mapResponded;
#endif
			ContactMap m_mapBest;
			ContactList m_listDelete;
			ContactUnorderedMap m_mapInUse;
