...
CAtlList<CContact> CUDPFirewallTester::m_liPossibleTestClients;
CAtlList<UsedClient_Struct> CUDPFirewallTester::m_liUsedTestClients;
...
void CUDPFirewallTester::SetUDPFWCheckResult(bool bSucceeded, bool bTestCancelled, uint32 uFromIP, uint16 nIncomingPort){	
	...
	if (!bTestCancelled){
		...
		if (bSucceeded){	//one positive result is enough
			...
			else if (nIncomingPort != 0 && nIncomingPort == CKademlia::GetPrefs()->GetExternalKadPort()){
...
void CUDPFirewallTester::QueryNextClient(){ // try the next available client for the firewallcheck
	...
	while (!m_liPossibleTestClients.IsEmpty()){
		...
		if (ntohl(curContact.GetIPAddress()) == theApp.GetPublicIP() || curContact.GetClientID() == CKademlia::GetPrefs()->GetKadID())
			continue;
