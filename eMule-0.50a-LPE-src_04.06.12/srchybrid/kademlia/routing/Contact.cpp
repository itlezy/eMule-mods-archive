...
{
	if (m_bGuiRefs)
		theApp.emuledlg->ContactRem(this);
}
...
{
	m_uDistance.ToHexString(psDistance);// X: [UIC] - [UIChange]
}
...
{
	uint64 uHours = (time(NULL)-m_tCreated)/HR2S(1);// X: [64T] - [64BitTime]
	switch(uHours)
	{
		case 0:
			m_byType = 2;
