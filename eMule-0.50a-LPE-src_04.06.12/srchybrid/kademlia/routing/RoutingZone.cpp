...
DWORD CRoutingZone::m_nLastSaved;
...
CRoutingZone::CRoutingZone()
{
	...
	m_nLastSaved = ::GetTickCount();// X: [AC] - [ActionChange] Auto Save Nodes
}
...
CRoutingZone::~CRoutingZone()
{
	...
	if ((m_pSuperZone == NULL) && (m_sFilename.GetLength() > 0))
	{
		...
		//theApp.emuledlg->serverwnd->HideContacts();
		WriteFile();
	}
	...
	//if (m_pSuperZone == NULL)
		//theApp.emuledlg->serverwnd->ShowContacts();
}
			...
			if (uNumContacts != 0 && uNumContacts * 25 <= (file.GetLength() - file.GetPosition()))
			{
				...
				while ( uNumContacts )
				{
					...
					if( byType < 4)
					{
						...
						{
							if (::theApp.ipfilter->IsFiltered(uhostIP)||(uUDPPort == 53 && uContactVersion <= KADEMLIA_VERSION5_48a)  /*No DNS Port without encryption*/)
							{
...
void CRoutingZone::ReadBootstrapNodesDat(CFileDataIO& file){
	...
	if (uNumContacts != 0 && uNumContacts * 25 == (file.GetLength() - file.GetPosition()))
	{
		...
		while ( uNumContacts )
		{
			...
			if (::IsGoodIPPort(uhostIP, uUDPPort))
			{
				if (::theApp.ipfilter->IsFiltered(uhostIP) || (uUDPPort == 53 && uContactVersion <= KADEMLIA_VERSION5_48a) )
				{
					if (::thePrefs.GetLogFilteredIPs())
						AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u)--read known.dat") , ipstr(uhostIP), uUDPPort);
				}
		...
		if (file.Open(m_sFilename, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary|CFile::shareDenyWrite, &fexp))
		{
			...
			for (ContactMap::iterator itContactMap = mapContacts.begin(); itContactMap != mapContacts.end(); )
			{
				...
				if (pContact->GetVersion() <= 1)
					...
					mapContacts.erase(itContactMap++);
#else
					itContactMap = mapContacts.erase(itContactMap);
#endif
				else
					++itContactMap;
			}
...
bool CRoutingZone::Add(const CUInt128 &uID, uint32 uIP, uint16 uUDPPort, uint16 uTCPPort, uint8 uVersion, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, bool bFromNodesDat, bool bFromHello)
{
	...
	if (::IsGoodIPPort(uhostIP, uUDPPort))
	{
		...
		if (::thePrefs.GetLogFilteredIPs()) /*!(uUDPPort == 53 && uVersion <= KADEMLIA_VERSION5_48a)*/
	...
	else
	{
		...
		else if (m_pBin->GetRemaining())
		{
			...
			if(m_pBin->AddContact(pContact))
			{
				...
				theApp.emuledlg->ContactAdd(pContact);
...
void CRoutingZone::GetClosestTo(uint32 uMaxType, const CUInt128 &uTarget, const CUInt128 &uDistance, uint32 uMaxRequired, ContactMap *pmapResult, bool bEmptyFirst, bool bInUse) const
{
	...
	sint_ptr iCloser = uDistance.GetBitNumber(m_uLevel);
...
void CRoutingZone::RandomBin(ContactList *pmapResult, bool bEmptyFirst)
{
	if (IsLeaf())
		...
		m_pSubZones[t_rng->getUInt8()&1]->RandomBin(pmapResult, bEmptyFirst);
}
...
void CRoutingZone::Split()
{
	...
	{
		uint_ptr iSuperZone = (*itContactList)->m_uDistance.GetBitNumber(m_uLevel);
...
uint_ptr CRoutingZone::EstimateCount()
{
	if( !IsLeaf() )
		return 0;
	if( m_uLevel < KBASE )
		return (uint_ptr)(pow(2.0F, (int)m_uLevel)*K);
	...
	return (uint_ptr)((pow(2.0F, (int)m_uLevel-2))*(float)K*fModify*fFirewalledModifyTotal);
}
...
//Spike2: FiX by netfinity - START
/*
	for (ContactList::const_iterator itContactList = listEntries.begin(); itContactList != listEntries.end(); ++itContactList)
	{
		pContact = *itContactList;
*/
	for (ContactList::const_iterator itContactList = listEntries.begin(); itContactList != listEntries.end();)
	{
...
//Spike2: FiX by netfinity - END
	...
	if(pContact != NULL)
	{
		...
		else if (pContact->GetVersion() >= KADEMLIA_VERSION2_47a){// X: [CI] - [Code Improvement]
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
			if (thePrefs.GetDebugClientKadUDPLevel() > 0)
				DebugSend("KADEMLIA2_HELLO_REQ", pContact->GetIPAddress(), pContact->GetUDPPort());
#endif
	...
	if (::GetTickCount() - m_nLastSaved > MIN2MS(40) && m_pSuperZone == NULL)
		WriteFile(); // 40 minutes // X: [AC] - [ActionChange] Auto Save Nodes
}
...
size_t CRoutingZone::GetNumContacts() const
{
	if (IsLeaf())
