...
// gomez82 >>> Maella::Code Improvement (CPU load)- 
/*
...
#else
	POSITION endpos = m_Spans.FindFirstKeyAfter(end);
#endif
...
void CBarShader::FillRect(CDC *dc, LPRECT rectSpan, float fRed, float fGreen,
	...
	// MagicAngel ==> dc's background mode restore - YumeYao
	int nOldMode = dc->SetBkMode(TRANSPARENT); // Do not use GetBkMode to avoid extra entry in Import Table
	COLORREF cOld = dc->GetBkColor();
	// MagicAngel <== dc's background mode restore - YumeYao
	if(bFlat) {
