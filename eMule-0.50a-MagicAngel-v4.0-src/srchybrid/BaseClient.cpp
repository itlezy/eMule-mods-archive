...
const CString CUpDownClient::str_ANTAddOn=CUpDownClient::GetANTAddOn(); // => DLP [Xman] - sFrQlXeRt
...
void CUpDownClient::Init()
{
	...
	*/
	...
	//m_bGPLEvildoer = false; // => Removed GPL Breaker detection - sFrQlXeRt
	...
	dwThisClientIsKnownSince = ::GetTickCount();
	// <== Sivka-Ban [cyrex2001] - sFrQlXeRt
	m_bIsPunishDonkeys = false; // => Punish Clients without SUI - sFrQlXeRt
	// ==> Anti Uploader Ban [Stulle] - sFrQlXeRt
	m_bAntiUploaderCaseThree = false; 
	m_bAntiUploaderBanActive = false; // => Anti Uploader Ban for Punish Donkeys without SUI - sFrQlXeRt
	// <== Anti Uploader Ban [Stulle] - sFrQlXeRt

	// ==> Spread Requests [Stulle] - sFrQlXeRt
	...
	// Stulle: result between 27 and 31 this is useful to use TCP-Connection from older clients
	// <== Spread Requests [Stulle] - sFrQlXeRt

	//==sFrQlXeRt=> Secure Source Exchange
	m_bSuspectXSClient = false;
	m_bWaitingForXSAnswer = false;
	m_uNotRequestedXSAnswers = 0;
	m_uReceivedXSSources = 0;
	m_uFilteredXSPackets = 0;
	m_uReceivedBadIPs = 0;
	m_uReceivedFilteredIPs = 0;
	m_uReceivedUselessIPs = 0;
	//<=sFrQlXeRt== Secure Source Exchange
...
CUpDownClient::~CUpDownClient(){
	...
	if(Credits())
	{
		...
		if(Credits()->GetDownloadedTotal()==0 && Credits()->GetUploadedTotal()==0 &&
			(!theApp.clientcredits->IsSaveUploadQueueWaitTime() || Credits()->GetSecureWaitStartTime(GetIP(), true)==0))
		//<=sFrQlXeRt== Fix for SUQWT and Extended credit-table-arrangement
...
// Removed Morph Anti-Leecher - sFrQlXeRt/gomez82
// ==> DLP [Xman] - sFrQlXeRt
void CUpDownClient::TestLeecher()
{
	...
	if (thePrefs.IsDetectModstrings() && theApp.dlp->IsDLPavailable() && theApp.MagicDLP->IsMagicDLPavailable())
	{
		if(old_m_strClientSoftwareFULL.IsEmpty() || old_m_strClientSoftwareFULL != DbgGetFullClientSoftVer() )
		{
			...
			// ==> MagicDLP - sFrQlXeRt
			LPCTSTR reason = theApp.MagicDLP->DLPCheckModstring_GPL(m_strModVersion, m_strClientSoftware);
			if (reason){
				BanLeecher(reason, 16); 
				return;
			}
			// <== MagicDLP - sFrQlXeRt

			// use MagicDLP if reason != NULL - sFrQlXeRt
			reason = (theApp.MagicDLP->DLPCheckModstring_Hard(m_strModVersion,m_strClientSoftware)!=NULL ? theApp.MagicDLP->DLPCheckModstring_Hard(m_strModVersion,m_strClientSoftware) : theApp.dlp->DLPCheckModstring_Hard(m_strModVersion,m_strClientSoftware));
			if(reason)
			{
				BanLeecher(reason,5);
				return;
			...
			// use MagicDLP if reason != NULL - sFrQlXeRt
			reason = (theApp.MagicDLP->DLPCheckModstring_Soft(m_strModVersion,m_strClientSoftware)!=NULL ? theApp.MagicDLP->DLPCheckModstring_Soft(m_strModVersion,m_strClientSoftware) : theApp.dlp->DLPCheckModstring_Soft(m_strModVersion,m_strClientSoftware));
			if(reason)
			{
				BanLeecher(reason,4);
				return;
			...
			// ==> MagicDLP - sFrQlXeRt
			reason = theApp.MagicDLP->DLPCheckModstring_Bad(m_strModVersion, m_strClientSoftware);
			if (reason){
				BanLeecher(reason, 17);
				return;
			}
			// <== MagicDLP - sFrQlXeRt
			else if(IsLeecher()==4)
	...
	// ==> AJ-community-detection by userhash, code from xtreme 6.1 [evcz]
	if(thePrefs.IsDetectUserhashes() && HasValidHash() && theApp.dlp->IsDLPavailable())
	{
		...
		{
			//BanLeecher(_T("*AJ*"),23);
			BanLeecher(_T("Applejuice-Community"),16); //GPL breaker!!! upload ban and STFU!!!
			return;
		}
	}
	// <== AJ-community-detection by userhash, code from xtreme 6.1 [evcz]

	if (thePrefs.IsDetectUsernames() && theApp.dlp->IsDLPavailable() && theApp.MagicDLP->IsMagicDLPavailable())
	{
		...
		if(m_pszUsername!=NULL && (old_m_pszUsername.IsEmpty() || old_m_pszUsername!=m_pszUsername)) //remark: because old_m_pszUsername is CString and there operator != is defined, it isn't a pointer comparison 
		{
			...
			// ==> MagicDLP - sFrQlXeRt
			LPCTSTR reason = theApp.MagicDLP->DLPCheckUsername_GPL(m_pszUsername);
			if (reason){
				BanLeecher(reason, 16);
				return;
			}
			...
			// use MagicDLP if reason != NULL - sFrQlXeRt
			reason = (theApp.MagicDLP->DLPCheckUsername_Hard(m_pszUsername)!=NULL ? theApp.MagicDLP->DLPCheckUsername_Hard(m_pszUsername) : theApp.dlp->DLPCheckUsername_Hard(m_pszUsername));
			if(reason)
			{
				BanLeecher(reason,5); 
				return;
...
//find gamer snake
			...
			if (HasValidHash())
			{
				...
				//zz_fly :: Anti-Leecher
				//note: change this to hard ban
				if(reason && StrStrI(reason, _T("Fake VeryCD")))
				{
					if(GetClientSoft() == SO_EMULE)
					{
			...
			// use MagicDLP if reason != NULL - sFrQlXeRt
			reason = (theApp.MagicDLP->DLPCheckUsername_Soft(m_pszUsername)!=NULL ? theApp.MagicDLP->DLPCheckUsername_Soft(m_pszUsername) : theApp.dlp->DLPCheckUsername_Soft(m_pszUsername));
			if(reason)
			{
				BanLeecher(reason,10); 
				return;
			}
	...
	if (thePrefs.IsDetectFakeEmule() && m_nClientVersion > MAKE_CLIENT_VERSION(0, 30, 0) && m_byEmuleVersion > 0 && m_byEmuleVersion != 0x99 && m_clientSoft == SO_EMULE)
	{
	...
	{
		// thx to ScarAngel for some codes - sFrQlXeRt
		static 	const float MOD_FLOAT_VERSION = (float)_tstof(theApp.m_strModVersion.Mid(theApp.m_uModLength));
		const float MagicAngelVersion = GetMagicAngelVersion(m_strModVersion);
		if(thePrefs.IsDetectModThief())
		{
			if(MagicAngelVersion == MOD_FLOAT_VERSION && m_nClientVersion != MAKE_CLIENT_VERSION(CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin, CemuleApp::m_nVersionUpd))
			{
				m_uModClient = MOD_NONE; //==sFrQlXeRt=> Fixed showing Magic Angel Mod Icon for Mod Thief Leechers
				BanLeecher(_T("Mod-ID Faker"),6);
				return;
			}
			if(MagicAngelVersion >= 1.4f && CString(m_pszUsername).Right(m_strModVersion.GetLength()+1)!=m_strModVersion + _T("\xBB")) //Fixed - avoiding compiling error in different codepage
			{
				m_uModClient = MOD_NONE; //==sFrQlXeRt=> Fixed showing Magic Angel Mod Icon for Mod Thief Leechers
...
void CUpDownClient::ProcessBanMessage()
{
	...
	{
		AddMorphLogLine(m_strBanMessage); // changed for Morph log - sFrQlXeRt
...
void CUpDownClient::ClearHelloProperties()
{
	...
	m_fRequiresCryptLayer = 0;
	m_fSupportsSourceEx2 = 0;
	m_strModVersion.Empty();// gomez82 >>> WiZaRd::Missing code?
	m_fSupportsCaptcha = 0;
	m_fDirectUDPCallback = 0;
	m_fSupportsFileIdent = 0;
}
...
// gomez82 <<< WiZaRd::FiX?
	...
	*/
	...
	// ==> DLP [Xman] - sFrQlXeRt
	CString strBanReason = NULL; 
	bool nonofficialopcodes = false; 
	CString unknownopcode;
	bool wronghello = false; 
	uint32 hellotagorder = 1; 
	bool foundmd4string = false;
	// <== DLP [Xman] - sFrQlXeRt
	//MORPH START - Added, Bad Shareaza detection [zz_fly]
	bool bWasUDPPortSent = false;
	bool bIsBadShareaza = false;
	//MORPH END   - Added, Bad Shareaza detection [zz_fly]
...
// gomez82 <<< fixes
			...
			case CT_MOD_VERSION:
				//m_strNotOfficial.AppendFormat(_T(",MID=%s"),temptag.GetFullInfo()); //MOPRH - Added by SiRoB, Control Mod Tag // => Removed Morph Anti-Leecher - sFrQlXeRt
				if (temptag.IsStr())
				{
				...
				//CheckForGPLEvilDoer();  // => Removed GPL Breaker detection - sFrQlXeRt
				break;

			// MORPH START - Append WC info to m_strNotOfficial
			case WC_TAG_VOODOO:
				nonofficialopcodes = true; // => DLP [Xman] - sFrQlXeRt
				if (temptag.IsInt()) {
					//m_strNotOfficial.AppendFormat(_T(",WCV=%s"),temptag.GetFullInfo()); // => Removed Morph Anti-Leecher - sFrQlXeRt
				}
				// ==> Removed Morph Anti-Leecher - sFrQlXeRt
				/*
				//MOPRH START - Added by SiRoB,  Control Mod Tag
			...
			case WC_TAG_FLAGS:
				nonofficialopcodes = true; // => DLP [Xman] - sFrQlXeRt
				//m_strNotOfficial.AppendFormat(_T(",WCF=%s"),temptag.GetFullInfo()); //MOPRH - Added by SiRoB, Control Mod Tag // => Removed Morph Anti-Leecher - sFrQlXeRt
				break;
			...
			case CT_EMULE_UDPPORTS:
				...
				*/
				// <== Removed Morph Anti-Leecher - sFrQlXeRt
				bWasUDPPortSent = true; //MORPH - Added, Bad Shareaza detection [zz_fly]
				break;
			...
			case CT_EMULE_MISCOPTIONS1:
				...
				*/
				// <== Removed Morph Anti-Leecher - sFrQlXeRt
				//MORPH START - Added, Bad Shareaza detection [zz_fly]
				if(!bWasUDPPortSent && !bIsBadShareaza)
					bIsBadShareaza = true;
				//MORPH END   - Added, Bad Shareaza detection [zz_fly]
				break;
	...
	if (credits == NULL){
		...
		// <== Extended credit-table-arragement [Xman] - sFrQlXeRt
			if (thePrefs.GetLogBannedClients())
				...
				AddMorphLogLine(_T("Clients: %s (%s), Banreason: Userhash changed (Found in TrackedClientsList)"), GetUserName(), ipstr(GetConnectIP()));
				// <== Angel Argos - sFrQlXeRt
			Ban();
		}	
	}
		...
		if (thePrefs.GetLogBannedClients())
			...
			AddMorphLogLine(_T("Clients: %s (%s), Banreason: Userhash changed"), GetUserName(), ipstr(GetConnectIP()));
			// <== Angel Argos - sFrQlXeRt
		Ban();
	}
	...
	*/
	...
	// note: we test if the client supports SUI in UploadClient.cpp
	if ((thePrefs.IsPunishMlDonkey() && GetClientSoft()==SO_MLDONKEY) ||
		(thePrefs.IsPunishEdonkey() && GetClientSoft()==SO_EDONKEY)  ||
		(thePrefs.IsPunishEdonkeyHybrid() && GetClientSoft()==SO_EDONKEYHYBRID) ||
		(thePrefs.IsPunishShareaza() && GetClientSoft()==SO_SHAREAZA) ||
		(thePrefs.IsPunishLphant() && GetClientSoft()==SO_LPHANT)
		)
		m_bIsPunishDonkeys = true;
	// <== Punish Clients without SUI - sFrQlXeRt
	...
	if(thePrefs.IsDetectFakeEmule() && // Anti-Leecher enabled
		strBanReason.IsEmpty() && // and we do not have a ban, yet
		bIsBadShareaza && // was UDP sent after Misc Options tag
		m_clientSoft == SO_EMULE) // but pretends to be a mule?
	...
	*/
	...
	//MORPH START - Added by SiRoB, Dynamic FunnyNick
	//UpdateFunnyNick(); // => DLP: we do this after the Anti-Leecher Code - sFrQlXeRt
	//MORPH END   - Added by SiRoB, Dynamic FunnyNick
	...
	//==MagicAngel=> Secure Source Exchange - sFrQlXeRt
	if (WaitingForXSAnswer()) {
		// We asked this client to send us Sources on our last connect but he
		// hasn't send any Sources to us.
		// Note: Anti XS-Exploit does nearly the same, but we have to send at least 3 requests
		// and the other Client has to send at least 1 request that he can become a XS-Exploiter.
		//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: No answer to XS Request at last connect from Client %s"), DbgGetClientInfo());
		DebugLog(LOG_ERROR, _T("Secure XS: No answer to XS Request at last connect from Client %s"), DbgGetClientInfo());
		SetWaitingForXSAnswer(false);
	}
	//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
	...
	if (thePrefs.IsDetectCreditHack() && theApp.GetID()!=m_nUserIDHybrid && memcmp(m_achUserHash, thePrefs.GetUserHash(), 16)==0)
	{
		strBanReason = _T("Anti Credit Hack");
		BanLeecher(strBanReason,15); // extra case for credit hack - sFrQlXeRt
		return bIsMule;
	}
	if(strBanReason.IsEmpty()==false && thePrefs.IsDetectBadHello())
	{
		BanLeecher(strBanReason,2); 
		return bIsMule;
	}
	if(foundmd4string && thePrefs.IsDetectWrongHello()) // ban it as 'WrongHello' - sFrQlXeRt
		{
			//strBanReason = _T("md4-string in opcode");  //==sFrQlXeRt=> Show more info about the tag
			BanLeecher(strBanReason, 19);
			return bIsMule;
		}
	if(thePrefs.IsDetectWrongHello() && (m_clientSoft==SO_EMULE || (m_clientSoft==SO_XMULE && m_byCompatibleClient!=SO_XMULE)))
	{
		if(wronghello)
		{
		...
		{
			UINT uAddHelloDataSize = (UINT)(data->GetLength() - data->GetPosition());
			strBanReason.Format(_T("Extra Bytes (Hello) %u"), uAddHelloDataSize);
		...
		if(m_fSupportsAICH > 1  && m_clientSoft == SO_EMULE && m_nClientVersion <= MAKE_CLIENT_VERSION(CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin, CemuleApp::m_nVersionUpd))
			{
				...
				BanLeecher(strBanReason,21); //Applejuice 
				return bIsMule;
		}
		// <== evcz updated to xtreme 5.4.2
	}
	...
	if(IsLeecher()==1 || IsLeecher()==2) // we also need to check category 2 - sFrQlXeRt
	{
	...
	UpdateFunnyNick(); // Dynamic FunnyNick [SiRoB] - sFrQlXeRt
	// <== DLP [Xman] - sFrQlXeRt
...
// 	socket->SendPacket(packet,true);// gomez82 in MA is like this
...
//>>> WiZaRd::Safe Tag Sending
	CList<CTag*>	tagList; 
	const EUtf8Str bUnicode = (!m_pszUsername || m_bUnicodeSupport) ? utf8strRaw : utf8strNone;
	const bool bSend = (m_strModVersion.GetLength() || m_pszUsername==NULL) && !IsLeecher(); //>>> WiZaRd::Easy ModVersion

	tagList.AddTail(new CTag(ET_COMPRESSION,1));
	tagList.AddTail(new CTag(ET_UDPVER, 4));
	tagList.AddTail(new CTag(ET_UDPPORT, thePrefs.GetUDPPort()));
	tagList.AddTail(new CTag(ET_SOURCEEXCHANGE, 3));
	tagList.AddTail(new CTag(ET_COMMENTS, 1));
	tagList.AddTail(new CTag(ET_EXTENDEDREQUEST, 2));
	...
	if (thePrefs.CanSeeShares() != vsfaNobody) // set 'Preview supported' only if 'View Shared Files' allowed
		dwTagValue |= 128;
	tagList.AddTail(new CTag(ET_FEATURES, dwTagValue));
//>>> WiZaRd::Easy ModVersion
	if(bSend){
		tagList.AddTail(new CTag(ET_MOD_VERSION, theApp.m_strModVersion));
		tagList.AddTail(new CTag(ET_INCOMPLETEPARTS, 1));
}
//<<< WiZaRd::Easy ModVersion

	data.WriteUInt32(tagList.GetCount());
	while(!tagList.IsEmpty())
	{
		CTag* tag = tagList.RemoveHead();
		tag->WriteTagToFile(&data, bUnicode);
		delete tag;
	}
...
void CUpDownClient::ProcessMuleInfoPacket(const uchar* pachPacket, uint32 nSize)
{
	...
	//CString strBanReason=NULL; //MORPH - Added by SiRoB, Control mod Tag // => Removed Morph Anti-Leecher - sFrQlXeRt
	for (uint32 i = 0; i < tagcount; i++)
	{
		...
		switch (temptag.GetNameID())
		{
			...
			default:
				...
				*/
				...
				if (bDbgInfo)
					m_strMuleInfo.AppendFormat(_T("\n  ***UnkTag=%s"), temptag.GetFullInfo());
				break; // X-Ray :: FiXeS :: MissingCode :: WiZaRd
		}
	}
	...
	*/
	...
	if(thePrefs.IsDetectWrongHello() && (m_clientSoft==SO_EMULE || (m_clientSoft==SO_XMULE && m_byCompatibleClient!=SO_XMULE)))
	{
		...
		{
			UINT uAddHelloDataSize = (UINT)(data.GetLength() - data.GetPosition());
			strBanReason.Format(_T("Extra Bytes (Info) %u"), uAddHelloDataSize);
			BanLeecher(strBanReason,13);
		}
	}
	// <== DLP [Xman] - sFrQlXeRt
}
		...
		|| (thePrefs.IsEmueDonkeyHybrid() && GetClientSoft() == SO_EDONKEYHYBRID))
	{
		...
		if (thePrefs.IsEmuLog())
		{
			...
			if(GetClientSoft() == SO_EDONKEY)
				buffer.Format(_T("[EMULATE] Emulate eDonkey (%s)"),DbgGetClientInfo());
			else if(GetClientSoft() == SO_EDONKEYHYBRID)
				buffer.Format(_T("[EMULATE] Emulate eDonkeyHybrid (%s)"),DbgGetClientInfo());
			DebugLog(LOG_USC | DLP_VERYLOW,buffer);
		}
	}
	// ==evcz=> Send Random Userhash to Shareaza clients without SUI
	// not fair??? :/
	// shareaza FOR NOW do not support secure ident... no need to send our real userhash...
	// just send a different one any time we will send it...
	/*
	else if(thePrefs.IsEmuShareaza() && GetClientSoft() == SO_SHAREAZA && credits->GetCurrentIdentState(GetIP()) != IS_IDENTIFIED)
	{
		for (int i = 0; i < 8; i++)
		{
			uint16 random = GetRandomUInt16();
			memcpy(&hash[i*2], &random, 2);
		}
		hash[5] = 14;
		hash[14] = 111;
		//no need to log as we already logged when we faked the version
	}
	*/
	// <=evcz== Send Random Userhash to Shareaza clients without SUI
...
//>>> WiZaRd::Safe Tag Sending
	...
	const EUtf8Str bUnicode = (!m_pszUsername || m_bUnicodeSupport) ? utf8strRaw : utf8strNone;
//>>> WiZaRd::Easy ModVersion
	const bool bSend = (m_strModVersion.GetLength() || m_pszUsername==NULL) && !IsLeecher();
//<<< WiZaRd::Easy ModVersion
	// eD2K Name
	// TODO implement multi language website which informs users of the effects of bad mods
CString m_strNick = thePrefs.GetUserNick();
if(IsLeecher()==0 || (!thePrefs.IsInformLeechers() && IsLeecher()==11 || IsLeecher()==6)) // send Nickaddon only to non Leechers or NickThiefs/ModThiefs
{
// gomez82 >>> evcz::Emulate Community Nick Addons
		if (thePrefs.IsEmuCommunityNickAddons()&& m_pszUsername != NULL){
		
		//we need to choose if we want to move this ip or not...
		//might help in case verycd get listed in DLP, but might expose us in case some of those leecher have anti community addon feature :S
		//we should also check the list to keep only the community addon worth to use... and stripe out the addons not usefull

			//if we know the other client nickname, let check his nickname and eventually emulate his community nick addons

				//these will be added BEFORE the nick		
				CStringArray frontCommunityAddon;
				frontCommunityAddon.Add(_T("[CHN]"));
				frontCommunityAddon.Add(_T("[VeryCD]"));
				frontCommunityAddon.Add(_T("[TuoTu]"));
				frontCommunityAddon.Add(_T("[SiLU]"));
				frontCommunityAddon.Add(_T("[WWR]"));
				frontCommunityAddon.Add(_T("[ROCKFORCE]"));
				//frontCommunityAddon.Add(_T("[KOR]"));
				//frontCommunityAddon.Add(_T("[KOREA]"));
				frontCommunityAddon.Add(_T("[GSB]"));
				frontCommunityAddon.Add(_T("[OKR]"));
				frontCommunityAddon.Add(_T("[power-portal.to]"));
				frontCommunityAddon.Add(_T("[Ultimativ-Board]"));
				frontCommunityAddon.Add(_T("[Sbw-Crew]"));
				frontCommunityAddon.Add(_T("[UBR-Crew]"));
				frontCommunityAddon.Add(_T("[Saugstube]"));
				frontCommunityAddon.Add(_T("[Soneros]"));
				frontCommunityAddon.Add(_T("[ballroomdancing.tk]"));
				frontCommunityAddon.Add(_T("[Tr-dizi]"));
				frontCommunityAddon.Add(_T("[Tr-p2p]"));
				frontCommunityAddon.Add(_T("[TNPG]"));
				frontCommunityAddon.Add(_T("[BSwdvd]"));
				frontCommunityAddon.Add(_T("[nc]"));

				//these will be added AFTER the nick
				CStringArray backCommunityAddon;
				backCommunityAddon.Add(_T("[TSP]"));
				backCommunityAddon.Add(_T("[SG-X]"));
				backCommunityAddon.Add(_T("[SFF]"));
				backCommunityAddon.Add(_T("[XCS]"));
				backCommunityAddon.Add(_T("[TBRG]"));
				backCommunityAddon.Add(_T("[ARC]"));
				backCommunityAddon.Add(_T("[AFF]"));
				backCommunityAddon.Add(_T("[SPD]"));
				backCommunityAddon.Add(_T("[eselkult.tk]"));
				backCommunityAddon.Add(_T("[RSVCD-Crew]"));
				backCommunityAddon.Add(_T("[emuleech]"));
				backCommunityAddon.Add(_T("[B5-Board]"));
				backCommunityAddon.Add(_T("[Checkpoint-Muli]"));
				//backCommunityAddon.Add(_T("[RUS]"));
				//backCommunityAddon.Add(_T("[ITA]"));
				//backCommunityAddon.Add(_T("[IL]"));
				backCommunityAddon.Add(_T("[soneros.net]"));
				backCommunityAddon.Add(_T("[ePlus]"));
				backCommunityAddon.Add(_T("[emule.de v16 webcache]"));
				backCommunityAddon.Add(_T("[e-sipa]"));
				backCommunityAddon.Add(_T("[sipacik]"));
				backCommunityAddon.Add(_T("[UtFuk]"));
				backCommunityAddon.Add(_T("[TurkSipa]"));

				CString m_frontAddonCommunity;
				CString m_backAddonCommunity;
				
				CString m_usernameToAnalize = m_pszUsername;

				//check front
				for(int i=0; i<=frontCommunityAddon.GetUpperBound(); i++)
				{
					//does the other client have the front addon?
					if(m_usernameToAnalize.Find(frontCommunityAddon[i]) != -1)
					{
						//are we missing that addon?
						if(m_strNick.Find(frontCommunityAddon[i]) == -1)
						{
							//tweak our nick (we are here because we do not already have the necessary fancy things)
							m_frontAddonCommunity.Append(frontCommunityAddon[i]);	
						}
					}
				}
				//check back... maybe he have both?
				for(int i=0; i<=backCommunityAddon.GetUpperBound(); i++)
				{
					//does the other client have the back addon?
					if(m_usernameToAnalize.Find(backCommunityAddon[i]) != -1)
					{
						//are we missing that addon?
						if(m_strNick.Find(backCommunityAddon[i]) == -1)
						{
							//tweak our nick (we are here because we do not already have the necessary fancy things)
							m_backAddonCommunity.Append(backCommunityAddon[i]);
						}
					}
				}
				
				//build the nickname mixing addons, nickname and anti nick-thief
				m_strNick = m_frontAddonCommunity + m_strNick + _T(" ") + str_ANTAddOn + m_backAddonCommunity;
				//anti mod-thief
				//m_strNick.AppendFormat(_T(" «%s»"), theApp.m_strModVersion);
				m_strNick.AppendFormat(_T("\x20\xAB%s\xBB"), theApp.m_strModVersion);

				//write to log
				if (thePrefs.IsEmuLog() && (m_frontAddonCommunity.GetLength()>0 || m_backAddonCommunity.GetLength()>0))
				{
					CString buffer;
					buffer.Format( _T("[EMULATE] Emulated Community Nick Addon for user %s, nick sent: %s"),m_pszUsername,m_strNick);
					//DebugLog(LOG_MORPH | LOG_SUCCESS | DLP_VERYLOW,buffer); //will remove LOG_MORPH... here only for debugging
					DebugLog(LOG_SUCCESS | DLP_VERYLOW,buffer);
				}
			}
			// gomez82 <<< evcz::Emulate Community Nick Addons
	tagList.AddTail(new CTag(CT_NAME, m_strNick)); //>>> WiZaRd::ClientAnalyzer
	}
	else if(!thePrefs.IsInformLeechers()) // send the standard-nick to all other Leechers
	{
	CString m_strNick = thePrefs.GetUserNick();
	tagList.AddTail(new CTag(CT_NAME, m_strNick)); //>>> WiZaRd::ClientAnalyzer
	}
	// ==> Inform Leechers - sFrQlXeRt
	else if(thePrefs.IsInformLeechers() && IsLeecher()>0 && IsLeecher()!=18){
		CString m_strNick;
		// ==> Inform Leechers Text - evcz
		if(thePrefs.GetInformLeechersText().GetLength() > 0) { // no need to check for this twice - sFrQlXeRt
			// add ':' at the end if it isn't already there - sFrQlXeRt
			CString strInformText = thePrefs.GetInformLeechersText().Right(1) == _T(':') ? thePrefs.GetInformLeechersText() : thePrefs.GetInformLeechersText() + _T(':');
			//m_strNick.Format(_T("%s %s (%s «%s»)"), strInformText, LeecherReason, str_ANTAddOn, theApp.m_strModVersion);
			m_strNick.Format(_T("%s %s (%s \xAB%s\xBB)"), strInformText, LeecherReason, str_ANTAddOn, theApp.m_strModVersion);
		}
		else
		// <== Inform Leechers Text - evcz
		//m_strNick.Format(_T("You are banned because you are using a Leecher Mod: %s (%s «%s»)"), LeecherReason, str_ANTAddOn, theApp.m_strModVersion);
			m_strNick.Format(_T("You are banned because you are using a Leecher Mod: %s (%s \xAB%s\xBB)"), LeecherReason, str_ANTAddOn, theApp.m_strModVersion);
	tagList.AddTail(new CTag(CT_NAME, m_strNick)); //>>> WiZaRd::ClientAnalyzer

	}
	else if(thePrefs.IsInformLeechers() && IsLeecher()==18){
		CString m_strNick;
		//m_strNick.Format(_T("You are banned because your client asked too fast (%s «%s»)"), str_ANTAddOn, theApp.m_strModVersion);
		m_strNick.Format(_T("You are banned because your client asked too fast (%s \xAB%s\xBB)"), str_ANTAddOn, theApp.m_strModVersion);
	tagList.AddTail(new CTag(CT_NAME, m_strNick)); //>>> WiZaRd::ClientAnalyzer
	...
	// eD2K Version
//>>> Spike2::Emulate other [Spike2/WiZaRd]
	if (thePrefs.IsEmuShareaza() && GetClientSoft() == SO_SHAREAZA)
		tagList.AddTail(new CTag(CT_VERSION, SHAREAZAEMUVERSION));
	else 
//<<< Spike2::Emulate other [Spike2/WiZaRd]
		tagList.AddTail(new CTag(CT_VERSION,EDONKEYVERSION));
	...
	{
		tagList.AddTail(new CTag(CT_EMULE_BUDDYIP, theApp.clientlist->GetBuddy()->GetIP() )); 
	...
	tagList.AddTail(new CTag(CT_EMULE_MISCOPTIONS2, 
		//				(RESERVED				     )
				(uFileIdentifiers		<< 13) |
...
//>>> Spike2::Emulate other [Spike2]
	if (thePrefs.IsEmuShareaza() && GetClientSoft() == SO_SHAREAZA)
	{
		tagList.AddTail(new CTag(CT_EMULE_VERSION,
			(SO_SHAREAZA				<< 24) |	//>>> shadow2004::Emulate others [Spike2]
			(2							<< 17) |
			(2							<< 10) |
			(5 							<<  7) |	// 1 -> 5
			(0								 )
	...
	{
		tagList.AddTail(new CTag(CT_EMULE_VERSION,
			(SO_LPHANT					<< 24) |	//>>> shadow2004::Emulate others [Spike2]
			(2							<< 17) |	// 2 -> 3   Crumbs-Support??
			(9							<< 10) |	// 9 -> 0
			(0							<<  7)		
			));
	}
	...
	{
		tagList.AddTail(new CTag(CT_EMULE_VERSION,
			(SO_MLDONKEY				<< 24) |	//>>> shadow2004::Emulate others [Spike2]
			(2							<< 17) |
			(7							<< 10) |
			(6							<<  7)		// 3 -> 6
			));
...
//>>> WiZaRd::Easy ModVersion
	if(bSend){
		tagList.AddTail(new CTag(CT_MOD_VERSION, theApp.m_strModVersion));
//<<< WiZaRd::Easy ModVersion
		tagList.AddTail(new CTag(ET_INCOMPLETEPARTS, 1));
}
...
/*
	...
	//MORPH START - Added by SiRoB, Don't send MOD_VERSION to client that don't support it to reduce overhead
	bool bSendModVersion = (m_strModVersion.GetLength() || m_pszUsername==NULL) && !IsLeecher();
	if (bSendModVersion) tagcount+=(1+1);
	//MORPH END   - Added by SiRoB, Don't send MOD_VERSION to client that don't support it to reduce overhead
	//m_bSendOldMorph = GetOldMorph();//MORPH - prevent being banned by old MorphXT //==MagicAngel=> We aren't banned by old Morph - sFrQlXeRt
	...
	// we don't send the official GPL Breaker Nick
	if(IsLeecher()==0 || (!thePrefs.IsInformLeechers() && IsLeecher()==11 || IsLeecher()==6)) // send Nickaddon only to non Leechers or NickThiefs/ModThiefs
	{
		// thx to ScarAngel for some codes
		CString m_strNick = thePrefs.GetUserNick();

		// ==evcz=> Emulate Community Nick Addons
		if (thePrefs.IsEmuCommunityNickAddons() && m_pszUsername != NULL){
		...
		//we should also check the list to keep only the community addon worth to use... and stripe out the addons not usefull
			...
			//if we know the other client nickname, let check his nickname and eventually emulate his community nick addons
				...
				if (thePrefs.IsEmuLog() && (m_frontAddonCommunity.GetLength()>0 || m_backAddonCommunity.GetLength()>0))
				{
					CString buffer;
					buffer.Format( _T("[EMULATE] Emulated Community Nick Addon for user %s, nick sent: %s"),m_pszUsername,m_strNick);
					//DebugLog(LOG_MORPH | LOG_USC | DLP_VERYLOW,buffer); //will remove LOG_MORPH... here only for debugging
					DebugLog(LOG_USC | DLP_VERYLOW,buffer);
				}
			}
			else {
				//we do not know his nick, we cannot analyze it, will send our real one
				//add anti nick-thief + anti mod-thief
				//m_strNick.AppendFormat(_T(" %s «%s»"), str_ANTAddOn, theApp.m_strModVersion);
				m_strNick.AppendFormat(_T(" %s\x20\xAB%s\xBB"), str_ANTAddOn, theApp.m_strModVersion);
			}
			//<=evcz== Emulate Community Nick Addons
			CTag tagName(CT_NAME, m_strNick);
			tagName.WriteTagToFile(data, utf8strRaw);
	}
	else if(!thePrefs.IsInformLeechers()) // send the standard-nick to all other Leechers
	{
	...
	if (thePrefs.IsEmuShareaza() && GetClientSoft() == SO_SHAREAZA)
	{
		...
		{
			CString buffer;
			buffer.Format( _T("[EMULATE] Emulate Shareaza (%s)"),DbgGetClientInfo());
			DebugLog(LOG_USC | DLP_VERYLOW,buffer);
		}
	}
	else {
...
//				(RESERVED				     ) 
	...
	else if (thePrefs.IsEmuLphant() && GetClientSoft() == SO_LPHANT)
	{
		...
		{
			CString buffer;
			buffer.Format( _T("[EMULATE] Emulate Lphant (%s)"),DbgGetClientInfo());
			DebugLog(LOG_USC | DLP_VERYLOW,buffer);
		}
	}
	else if (thePrefs.IsEmuMLDonkey() && GetClientSoft() == SO_MLDONKEY)
...
//				(RESERVED			     ) 
	...
	if (bSendModVersion) { //MORPH - Added by SiRoB, Don't send MOD_VERSION to client that don't support it to reduce overhead
		...
		//==MagicAngel=> We aren't banned by old Morph - sFrQlXeRt
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	*/
	...
	socket = NULL;
	//ml >>> Xman Code Improvement: don't refresh list-item on deletion
	if(bDelete==false)
...
// gomez82 <<< WiZaRd::Optimization
	...
	if (uClientIP)
	{
		...
		{
//			theStats.filteredclients++; // gomez82 >>> WiZaRd::Optimization
	...
	if (m_bEmuleProtocol || iHashType == SO_EMULE){
		...
		switch(m_byCompatibleClient){
			...
			case SO_AMULE:
				...
				break;
			case SO_SHAREAZA:
			// ==> Enhanced Client Recognization [Spike2] - sFrQlXeRt
			//case 40:
			case SO_SHAREAZA2:
			case SO_SHAREAZA3:
			case SO_SHAREAZA4:
...
bool CUpDownClient::IsBanned() const
{
	...
	if ((m_nUploadState != US_BANNED)||( m_nDownloadState == DS_DOWNLOADING) )
		return false;
...
void CUpDownClient::AssertValid() const
{
	...
	*/
	...
	(void)dwThisClientIsKnownSince;
	// <== Sivka-Ban [cyrex2001] - sFrQlXeRt
	CHECK_BOOL(m_bAntiUploaderCaseThree); // => Anti Uploader Ban [Stulle] - sFrQlXeRt
#undef CHECK_PTR
#undef CHECK_BOOL
}
#endif
		...
		if(theApp.m_bneedpublicIP && !::IsLowID(dwIP)) //this is the case we have kad-only but no upload->inet down ?
		{
			if(theApp.last_valid_ip != 0 && theApp.last_valid_ip != dwIP)
			{
				...
				else if (thePrefs.IsRASAIC())
				{
					...
					if (thePrefs.UseQuickStartOnIPChange() && thePrefs.UseQuickStart())
						theApp.downloadqueue->DoQuickStart();
					// <== Quickstart [DavidXanatos] - sFrQlXeRt
					SetLastAskedTime(); //not for this source
				}

				//==sFrQlXeRt=> Reconnect Kad on IP change [Xman]
				if (thePrefs.ReconnectKadOnIpChange() && Kademlia::CKademlia::IsConnected() && Kademlia::CKademlia::GetPrefs()->GetIPAddress())
...
bool CUpDownClient::ProcessUnknownHelloTag(CTag *tag, CString &pszReason)
{
	...
	if (strSnafuTag != NULL)
	{
		pszReason.Format(_T("Suspect Hello-Tag: %s, %s"),strSnafuTag, tag->GetFullInfo()); //==sFrQlXeRt=> Show more info about the tag
	}

	if (strSnafuTag==NULL && tag->IsStr() && tag->GetStr().GetLength() >= 32) {
		pszReason.Format(_T("md4-string in opcode: %s"), tag->GetFullInfo()); //==sFrQlXeRt=> Show more info about the tag
		foundmd4 = true;
	}
...
void CUpDownClient::ProcessUnknownInfoTag(CTag *tag, CString &pszReason)
{
	...
	if (strSnafuTag != NULL)
	{
		pszReason.Format(_T("Suspect eMuleInfo-Tag: %s, %s"), strSnafuTag, tag->GetFullInfo()); //==sFrQlXeRt=> Show more info about the tag
	}
}
...
float CUpDownClient::GetMagicAngelVersion(CString modversion) const
{
	// thx to ScarAngel for some codes - sFrQlXeRt
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32 nLength)
{
	...
	*/
	if ( (thePrefs.MsgOnlyFriends() && !IsFriend()) || (thePrefs.MsgOnlySecure() && GetUserName()==NULL) || (thePrefs.IsDetectSpam() && (IsLeecher()/* || TestLeecher()*/)))
	...
	{
		resToken.Trim();
// gomez82 >>> WiZaRd::Don't accept too short filters - unfair!
//		if (strMessageCheck.Find(resToken.MakeLower()) > -1){
		if (resToken.GetLength() > 3 
			&& strMessage.Find(resToken.MakeLower()) > -1)
		{
// gomez82 <<< WiZaRd::Don't accept too short filters - unfair!
...
void CUpDownClient::SendSharedDirectories()
{
	...
	*/
	...
	// move single shared files to new list to save some loops when using this a lot
	...
	if (theApp.sharedfiles->ProbablyHaveSingleSharedFiles())
	{
		for (POSITION pos = theApp.sharedfiles->m_liSingleSharedFiles.GetHeadPosition(); pos != NULL; theApp.sharedfiles->m_liSingleSharedFiles.GetNext(pos))
			liSingleSharedFiles.AddTail(theApp.sharedfiles->m_liSingleSharedFiles.GetAt(pos));
	}
	...
	for (POSITION pos = theApp.sharedfiles->m_Files_map.GetStartPosition();pos != 0;){
		...
		if (cur_file->IsLargeFile() && !SupportsLargeFiles())
			continue;
		if (cur_file->IsPartFile()) // partfiles will be taken care of differently
			continue;
		...
		int Perm = cur_file->GetPermissions()>=0?cur_file->GetPermissions():thePrefs.GetPermissions();
		// Mighty Knife: Community visible filelist
		if ( Perm == PERM_NOONE 
			|| (Perm == PERM_COMMUNITY && !(IsCommunity() || IsFriend()) ) 
			|| (Perm == PERM_FRIENDS && !IsFriend()) )
			continue;
		// [end] Mighty Knife
		// xMule_MOD: showSharePermissions
		CString path = theApp.sharedfiles->GetPseudoDirName(cur_file->GetPath());
		path.MakeLower();
	...
	// add temporary folder if there are any temp files
	if (theApp.downloadqueue->GetFileCount() > 0)
		toSend.AddTail(CString(OP_INCOMPLETE_SHARED_FILES));
	// add "Other" folder (for single shared files) if there are any single shared files
	if (theApp.sharedfiles->ProbablyHaveSingleSharedFiles())
		toSend.AddTail(CString(OP_OTHER_SHARED_FILES));

	// should actually be empty here but we delete it all just in case
	ASSERT( liSingleSharedFiles.IsEmpty() );
	if(liSingleSharedFiles.IsEmpty() == false)
		liSingleSharedFiles.RemoveAll();

	//build packet
	CSafeMemFile tempfile(80);
	tempfile.WriteUInt32(toSend.GetCount());
	for (POSITION pos = toSend.GetHeadPosition();pos != 0;toSend.GetNext(pos))
		tempfile.WriteString((LPCTSTR)toSend.GetAt(pos), GetUnicodeSupport());
	// SLUGFILLER: shareSubdir - enumerate according to shared files
...
#define MAX_SUFFIXSIZE 11
	...
	// ==> FunnyNick Tag [Stulle/Aireoreion] - sFrQlXeRt
	/*
...
// credits->GetUploadedTotal() <== data amount the other client got from us
		...
		credits->GetDownloadedTotal() != NULL)
	{
		...
		{
			case CS_1:{
				return (credits->GetDownloadedTotal() >= (int)thePrefs.GetAntiUploaderBanLimit()*1024<<20);
					  } break;

			case CS_2:{
				if(credits->GetUploadedTotal() != NULL &&
					credits->GetUploadedTotal() < credits->GetDownloadedTotal()){
					return (credits->GetDownloadedTotal() - credits->GetUploadedTotal() >= (int)thePrefs.GetAntiUploaderBanLimit()*1024<<20);
				}
				else
					return false;
					  } break;
			...
			case CS_3:{
					...
					credits->GetUploadedTotal() < credits->GetDownloadedTotal())
				{
					...
					else if (credits->GetDownloadedTotal() - credits->GetUploadedTotal() >= (int)thePrefs.GetAntiUploaderBanLimit()*1024<<20)
					{
...
void CUpDownClient::CheckAntiUploaderBan()
{
	if (AntiUploaderBanActive())
		m_bAntiUploaderBanActive = true;
	else
		m_bAntiUploaderBanActive = false;
}
// <== Anti Uploader Ban for Punish Donkeys without SUI - sFrQlXeRt
// <== Anti Uploader Ban [Stulle] - sFrQlXeRt

// Idea of Aireoreion (iONiX)
COLORREF CUpDownClient::GetClientColor(COLORREF oldColor) const
{
	if(thePrefs.ColoredUpload()==1) 
	{
		UINT uRQR = GetRemoteQueueRank();

		if ( uRQR > 0 )
		{ 
			if ( uRQR < 101 ) //Green 				
				return RGB(2,185,12); 
			else if ( uRQR < 501 ) //Bright green 				
				return RGB(121,209,33); 
			else if ( uRQR < 1001 ) //Yellow 				
				return RGB(169,192,16);
			else if ( uRQR < 2001 ) //Orange 				
				return RGB(255,178,0); 
			else if ( uRQR < 3001 ) //Dark orange 				
				return RGB(216,145,48); 
			else //Red 				
				return RGB(208,13,2); 
		}
		else
		{ 
			if ( IsRemoteQueueFull() ) //Dark blue 				
				return RGB(71,94,95); 
			else if ( GetDownloadState() == DS_DOWNLOADING ) //Blue //==sFrQlXeRt=> 'GetDownloadState()' is better here than 'GetDownloadDatarate()'
				return RGB(0,0,211); 
			else if ( GetDownloadState() == DS_NONEEDEDPARTS ) // Brown //==sFrQlXeRt=> Color for Clients with no needed parts
				return RGB(138,46,0);
			else //Black //==sFrQlXeRt=> we are in the queue of this client (or our have a download state like connecting/too many connections/...)
				return RGB(0,0,0);          
		} 
	}
	else if (thePrefs.ColoredUpload()==2) // idea of Stulle 
	{
		if (IsFriend()) //Blue 
			return RGB(100,175,255); 
		else if (IsPBForPS()) //Red
			return RGB(255,60,8); 
	}
	return oldColor;
}
