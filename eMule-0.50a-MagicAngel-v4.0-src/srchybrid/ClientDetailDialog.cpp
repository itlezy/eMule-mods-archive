...
#include "UploadQueue.h" // => Queuerank at Clientdetail [Xman] - sFrQlXeRt
...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		// ==> Leecher at Clientdetail - sFrQlXeRt
		if (client->IsLeecher()>0)
			buffer.Format(_T("%s"), client->LeecherReason);
		else
			buffer.Format(GetResString(IDS_NO));
		GetDlgItem(IDC_STATIC_LEECHER_INFO)->SetWindowText(buffer);
		// <== Leecher at Clientdetail - sFrQlXeRt

		// ==> Punishment at Clientdetail - sFrQlXeRt
		if (client->IsLeecher()>0 && client->AntiUploaderBanActive()==false){
			if (client->m_iPunishment == 0)
				buffer.Format(_T("upload ban"));
			else if (client->m_iPunishment == -1)
				buffer.Format(_T("IP ban"));
			else if (client->m_iPunishment <= 9)
				buffer.Format(_T("score *0.%i"), client->m_iPunishment);
			else
				buffer.Format(_T("no ban"));
		}
		else if (client->IsLeecher()>0 && client->AntiUploaderBanActive())
			buffer.Format(_T("Anti Uploader Ban"));
		else
			buffer.Format(_T("-"));
		GetDlgItem(IDC_STATIC_PUNISHMENT_INFO)->SetWindowText(buffer);
		// <== Punishment at Clientdetail - sFrQlXeRt

		// ==> Queuerank at Clientdetail [Xman] - sFrQlXeRt
...
void CClientDetailPage::Localize()
{
	...
	GetDlgItem(IDC_STATIC_PUNISHMENT)->SetWindowText(GetResString(IDS_PUNISHMENT)); // => Punishment at Clientdetail - sFrQlXeRt
	GetDlgItem(IDC_STATIC_QUEUERANK)->SetWindowText(GetResString(IDS_QUEUERANK)); // => Queuerank at Clientdetail [Xman] - sFrQlXeRt
}
