...
CDeletedClient::CDeletedClient(const CUpDownClient* pClient)
{
	...
	// ==> Extended credit-table-arragement [Xman] - sFrQlXeRt
	//make the Tracked-client-list independent, track new port & hash 
...
void CClientList::TrigReaskForDownload(bool bImmediate)
{
	...
	{				
		...
		{
			if(bImmediate)
				// We are allowed to reask immediately (last IP Change was for over 30 minutes).
				// Note: We use 2*FILEREASKTIME here because we also want to reask NNP Sources immediately.
				cur_client->SetLastAskedTime(cur_client->reqfile, dwCurTick - 2*FILEREASKTIME);
			else {
				// Code by Stulle: Rask the source either immediately or as soon as the MIN_REQUESTTIME expired.
				DWORD ReAskTime = thePrefs.SpreadReask() ? cur_client->GetSpreadReAskTime()+thePrefs.GetReAskTimeDif() : thePrefs.GetReAskTimeDif(); // ==> manual enable spread reask - sFrQlXeRt
