...
void CClientUDPSocket::OnReceive(int nErrorCode)
{
	...
	theApp.last_traffic_reception = ::GetTickCount(); //Threading Info: synchronized with the main thread //==MagicAngel=> Reask Sources after IP Change v4 [Xman] - sFrQlXeRt
					...
					case OP_KADEMLIAHEADER:
					{
						...
						if(thePrefs.IsDetectFakeEmule())
						{
							...
							if(byOpcode == KADEMLIA_FIREWALLED2_REQ)
							{
								...
								{
									client->BanLeecher(_T("Vagaa detected"), 24); //Bad Leecher, Hard Ban
									break;
								}
							}
						}
	...
	switch(opcode)
	{
		...
		case OP_REASKFILEPING:
		{
			...
			if (sender)
			{
				...
				if( sender->IsBanned() )
					break;
				if(thePrefs.IsDetectAgressive() && sender->uiULAskingCounter > uint16 (thePrefs.GetAgressiveCounter()) && ((::GetTickCount()-sender->dwThisClientIsKnownSince)/sender->uiULAskingCounter) < uint32 (MIN2MS(thePrefs.GetAgressiveTime()))){
					if (thePrefs.IsAgressiveLog())
					{
						AddMorphLogLine(_T("OP_REASKFILEPING: (%s/%u)=%s ==> %s(%s) ASK TO FAST!"), 
					...
					sender->BanLeecher(_T("Agressive"), 18);
					/*
...
// gomez82 >>> JvA::no need to set this twice (already done @ SetConnectOptions)
...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32 dwIP, uint16 nPort){
	// NOTE: *** This function is invoked from a *different* thread!
	// gomez82 >>> TK4 MOD 2.0c - Save converting dwIP from uint32 to a CStringT to ASCII then back to uint32
