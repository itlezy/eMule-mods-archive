...
void CDownloadListCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("LEECHER"))); // => Leecher Icon - sFrQlXeRt
...
void CDownloadListCtrl::DrawSourceItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		case 0: {	// icon, name, status
			...
			// ==> Leecher Icon - sFrQlXeRt
			if ( pClient->IsLeecher()>0 && pClient->GetClientSoft()==SO_EMULE )
				m_ImageList.Draw(dc, 35, point2, ILD_NORMAL | INDEXTOOVERLAYMASK(uOvlImg)); //FIXED: Leecher Icon overlay [evcz]
			// <== Leecher Icon - sFrQlXeRt
			//MORPH - Removed by SiRoB, Friend Addon
			/*
			if (pClient->IsFriend())
				m_ImageList.Draw(dc, 6, point2, ILD_NORMAL | INDEXTOOVERLAYMASK(uOvlImg));
			else*/else if (pClient->GetClientSoft() == SO_EDONKEYHYBRID)
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			*/
			...
			//MagicAngel ==> Show Preview in root menu if Preview is available and only one player configured - YumeYao
			//               And optimized code.
			//               PLEASE TAKE CARE OF THIS IF BLOCK'S BOTTOM FOR RECYCLING PreviewWithMenu.
			/*
			...
			*/
			...
			bool b_subMenuAdded = false;
			FileMenuItemType u_itemType = e_preview;
			CMenu* ParentOfGetPreviewParts = thePrefs.IsExtControlsEnabled() ? &m_PreviewMenu : &m_FileMenu;
			if ((iFilesToPreview - 1) || (iSelectedItems - 1)) //Not possible for previewing
			{
				if (ParentOfGetPreviewParts == &m_PreviewMenu) //thePrefs.IsExtControlsEnabled()
				{
					m_PreviewMenu.EnableMenuItem(MP_PAUSEONPREVIEW, iFilesCanPauseOnPreview > 0 ? MF_ENABLED : MF_GRAYED);
					m_PreviewMenu.CheckMenuItem(MP_PAUSEONPREVIEW, (iSelectedItems > 0 && iFilesDoPauseOnPreview == iSelectedItems) ? MF_CHECKED : MF_UNCHECKED);
				}
				...
				{
					bool b_dlprvprt;
					if (iFilesPreviewType == 1 && iFilesNotDone == 1)
					{
						b_dlprvprt = true;
						if (ParentOfGetPreviewParts == &m_FileMenu) //!thePrefs.IsExtControlsEnabled()
						{
							u_itemType = e_dlprvprt;
							m_FileMenu.ModifyMenu(MP_PREVIEW, MF_STRING | MF_BYCOMMAND, MP_TRY_TO_GET_PREVIEW_PARTS, GetResString(IDS_DL_TRY_TO_GET_PREVIEW_PARTS), _T("FILEDOWNLOADPREVIEWFIRST")); //MorphXT - Icon Added
						}
					}
					if (b_dlprvprt || (ParentOfGetPreviewParts == &m_PreviewMenu))
					{
						ParentOfGetPreviewParts->EnableMenuItem(MP_TRY_TO_GET_PREVIEW_PARTS, (b_dlprvprt && !thePrefs.GetPreviewPrio()) ? MF_ENABLED : MF_GRAYED);
						ParentOfGetPreviewParts->CheckMenuItem(MP_TRY_TO_GET_PREVIEW_PARTS, (iFilesGetPreviewParts == 1 || thePrefs.GetPreviewPrio()) ? MF_CHECKED : MF_UNCHECKED);
					}
				}
				if (ParentOfGetPreviewParts == &m_PreviewMenu) 
				{
					m_FileMenu.ModifyMenu(MP_PREVIEW, MF_STRING|MF_POPUP|MF_BYCOMMAND|MF_ENABLED, (UINT_PTR)m_PreviewMenu.m_hMenu, GetResString(IDS_DL_PREVIEW), _T("PREVIEW"));
					goto ReplacePreviewSubMenu;
				}
				else goto SkipPreviewMenuItem;
			}

			//Possible for previewing
			PreviewWithMenu.CreateMenu();
			b_subMenuAdded = true;
			if (thePrefs.GetExtraPreviewWithMenu()) //"Display Preview With menu in root context menu"
				{;} else
			{
				//We want to replace the menu, so add preview and separator first
				PreviewWithMenu.AppendMenu(MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW), _T("PREVIEW"));
				PreviewWithMenu.AppendMenu(MF_SEPARATOR, 0, NULL);
			}
			int iPreviewMenuEntries = thePreviewApps.GetAllMenuEntries(PreviewWithMenu, (iSelectedItems == 1) ? file1 : NULL);
			if (iPreviewMenuEntries > 0) //Multiple players
			{
				if (thePrefs.GetExtraPreviewWithMenu())
				{
					m_FileMenu.InsertMenu(MP_METINFO, MF_STRING|MF_POPUP|MF_BYCOMMAND|MF_ENABLED, (UINT_PTR)PreviewWithMenu.m_hMenu, GetResString(IDS_DL_PREVIEW), _T("PREVIEW"));
				}
				else {
					m_FileMenu.ModifyMenu(MP_PREVIEW, MF_STRING|MF_POPUP|MF_BYCOMMAND|MF_ENABLED, (UINT_PTR)PreviewWithMenu.m_hMenu, GetResString(IDS_DL_PREVIEW), _T("PREVIEW"));
ReplacePreviewSubMenu:
					u_itemType = e_submenu;
					goto SkipPreviewMenuItem;
				}
			}
			m_FileMenu.EnableMenuItem(MP_PREVIEW, MF_ENABLED);
SkipPreviewMenuItem:
			//MagicAngel <== Show Preview in root menu if Preview is available - YumeYao
			...
			if(!thePrefs.IsLessControls())
			{
				...
				m_FileMenu.ModifyMenu(MP_SR13_ImportParts, MF_STRING, MP_SR13_ImportParts,(iFilesToImport > 0) ? GetResString(IDS_IMPORTPARTS_STOP) :GetResString(IDS_IMPORTPARTS), _T("FILEIMPORTPARTS")); // Renamed Function - YumeYao
			...
			//MagicAngel ==> Change preview display logic -- Restore Menu items - YumeYao
			...
			if (e_preview != u_itemType)
			{
				// ==> YumeYao's expected code start
				//UINT u_submenu = (e_submenu == u_itemType) ? (b_subMenuAdded ? (UINT)PreviewWithMenu.m_hMenu : (UINT)m_PreviewMenu.m_hMenu) : u_itemType;
				//VERIFY( m_FileMenu.ModifyMenu(u_submenu, MF_BYCOMMAND | MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW), _T("PREVIEW")));
				// <== YumeYao's expected code end

				//simply using ModifyMenu just doesn't work here when replacing a sub menu.
				//I'm a C programmer who know about C++ but know little about MFC,
				//so use remove and insert for sub menu instead - ModifyMenu is far faster than Remove and Insert.
				register UINT u_submenu;
				if ((u_submenu = u_itemType) != e_submenu) {
					VERIFY( m_FileMenu.ModifyMenu(u_submenu, MF_BYCOMMAND | MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW), _T("PREVIEW")));
				}
				else {
					u_submenu = b_subMenuAdded ? (UINT)PreviewWithMenu.m_hMenu : (UINT)m_PreviewMenu.m_hMenu;
					VERIFY( m_FileMenu.RemoveMenu(u_submenu, MF_BYCOMMAND));
					VERIFY( m_FileMenu.InsertMenu(MP_METINFO, MF_BYCOMMAND | MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW), _T("PREVIEW")));
				}
			}
		...
		else { //if (content != NULL && content->type == FILE_TYPE)  --> Client Menu
...
// <-- ZZ:DownloadManager
		...
		//MagicAngel ==> Change preview display logic - YumeYao
		//if (thePrefs.IsExtControlsEnabled()) {
		//	if (!thePrefs.GetPreviewPrio())
		//	{
...
void CDownloadListCtrl::CreateMenues()
{
	...
	//MagicAngel ==> Preview Menu change - YumeYao
	// Extended: Submenu with Preview options, Normal: Download preview-needed parts item
	...
	*/
	if (thePrefs.IsExtControlsEnabled())
	{
		m_PreviewMenu.CreateMenu();
		m_PreviewMenu.AddMenuTitle(NULL, true);
	    m_PreviewMenu.AppendMenu(MF_STRING, MP_TRY_TO_GET_PREVIEW_PARTS, GetResString(IDS_DL_TRY_TO_GET_PREVIEW_PARTS), _T("FILEDOWNLOADPREVIEWFIRST")); //MorphXT - Icon Added
		m_PreviewMenu.AppendMenu(MF_STRING, MP_PAUSEONPREVIEW, GetResString(IDS_PAUSEONPREVIEW), _T("FILEDOWNLOADPREVIEWPAUSE"));
		//Append This menu to FileMenu later at run-time.
	}
	//Preserve this item here, it will be served as is or replaced with other things.
	m_FileMenu.AppendMenu(MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW), _T("PREVIEW"));
	//MagicAngel <== Preview Menu change - YumeYao
