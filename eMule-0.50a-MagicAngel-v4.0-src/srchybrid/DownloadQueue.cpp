...
// gomez82 <<< WiZaRd::Faster UploadTimer
	...
	if (!m_bQuickStartDone)
		if(thePrefs.UseQuickStart() || thePrefs.OnQuickStart()){
			if(!thePrefs.OnQuickStart())
				DoQuickStart();
	...
	*/
	...
	uint32 datarateX=0;
	if(b100ms) // gomez82 >>> WiZaRd::Faster UploadTimer - Important! only inc after 100ms so CPartFile::Process with udcounter == 10 isn't called too often
		++udcounter;
...
void CDownloadQueue::DoQuickStart()
{
	...
	// changed some codes arround QuickStartTime, because we don't have the NeoMule configuration in preferences.h
	int	GetQuickStartTimePerFileMs = thePrefs.GetQuickStartTimePerFile()*1000;
	int	GetQuickStartMaxTimeMs = MIN2MS(thePrefs.GetQuickStartMaxTime());
	DWORD QuickStartTime = min((CountFiles * GetQuickStartTimePerFileMs), GetQuickStartMaxTimeMs);

	if (QuickStartTime > 0){
		m_bQuickStartDone = false;
		thePrefs.SetOnQuickStart(true);
		m_dwQuickStartEndTime = ::GetTickCount() + QuickStartTime;
		AddLogLine(false, GetResString(IDS_QUICKSTART_START), MS2MIN((float)QuickStartTime));
	}else
		m_bQuickStartDone = true;
}
...
{
	thePrefs.SetOnQuickStart(false);
	m_bQuickStartDone = true;
	AddLogLine(false, GetResString(IDS_QUICKSTART_END));
}
// <== Quickstart [DavidXanatos] - sFrQlXeRt
