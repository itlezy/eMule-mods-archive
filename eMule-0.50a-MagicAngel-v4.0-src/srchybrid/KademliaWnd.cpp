...
// ==> MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
#define NODES_DAT_STRINGS_PROFILE _T("AC_NodeDatURLs.dat")
// <== MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
...
// ==> MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
	if (m_pacNodesDatURL)
		m_pacNodesDatURL->SaveList(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + NODES_DAT_STRINGS_PROFILE);
// <== MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
	return TRUE;
}
...
// ==> MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
		...
		if (m_pacNodesDatURL->Bind(::GetDlgItem(m_hWnd, IDC_BOOTSTRAPURL), ACO_UPDOWNKEYDROPSLIST | ACO_AUTOSUGGEST | ACO_FILTERPREFIXES ))
			m_pacNodesDatURL->LoadList(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + NODES_DAT_STRINGS_PROFILE);
// <== MagicAngel - Add Kad nodes.dat URL list (just like Server.met URL list) - YumeYao
	}
