 ...
 // MORPH END
			...
			switch(opcode)
			{
				case OP_HELLOANSWER:
				{
					...
					client->ProcessBanMessage(); // => DLP [Xman] - sFrQlXeRt
				...
				case OP_STARTUPLOADREQ:
				{
					...
					{
						// ==> Sivka-Ban [cyrex2001] - sFrQlXeRt
						if( thePrefs.IsDetectAgressive() && client->uiULAskingCounter > uint16 (thePrefs.GetAgressiveCounter()) && ((::GetTickCount()-client->dwThisClientIsKnownSince)/client->uiULAskingCounter) < uint32 (MIN2MS(thePrefs.GetAgressiveTime()))){
							if (thePrefs.IsAgressiveLog())
							{
								AddMorphLogLine(_T("OP_STARTUPLOADREQ: (%s/%u)=%s ==> %s(%s) ASK TO FAST!"), 
							...
							client->BanLeecher(_T("Agressive"), 18);
							/*
					...
					// ==> DLP [Xman] - sFrQlXeRt
						...
						if(thePrefs.IsDetectWrongHello() && client->AntiUploaderBanActive()==false) // ban it as 'WrongHello' - sFrQlXeRt // ==> Anti Uploader Ban [Stulle] - sFrQlXeRt
						{
							client->BanLeecher(_T("wrong OPSTARTUPLOADREQ"), 20); 
				...
				case OP_REQUESTPARTS:
				{
					...
					{
						theApp.uploadqueue->RemoveFromUploadQueue(client, GetResString(IDS_BANNED_CLIENT_UPLOAD)); 
						client->SetUploadFileID(NULL); 
						AddMorphLogLine(GetResString(IDS_BANNED_CLIENT_UPLOAD),client->DbgGetClientInfo());
					}
					// <== DLP [Xman] - sFrQlXeRt
		...
		try
		{
			...
			switch(opcode)
			{
					...
					while (data_in.GetLength()-data_in.GetPosition() && !bAnswerFNF)
					{
						...
						switch (opcode_in)
						{
							...
							case OP_REQUESTSOURCES:
							{
								...
								// ==> DLP [Xman] - sFrQlXeRt
								// Anti-XS-Exploit
								if(thePrefs.IsDetectXSExploiter() && client->IsXSExploiter()) {
									CString strBanReason;
									strBanReason.Format(_T("XS-Exploiter: requests=%u, answers=%u"), client->GetXSReqs(), client->GetXSAnswers());
									client->BanLeecher(strBanReason, 8);
									break; //no answer
								}
								// <== DLP [Xman] - sFrQlXeRt
 				...
 				case OP_ANSWERSOURCES:
				{
					...
					theStats.AddDownDataOverheadSourceExchange(uRawSize);
					client->IncXSAnswer(); //==MagicAngel=> DLP [Xman] - sFrQlXeRt
					client->CheckHandshakeFinished(OP_EMULEPROT, opcode);//MOrph
				...
				case OP_REQUESTPARTS_I64:
				{
					...
					*/
					...
					if(client->GetUploadState()==US_BANNED) //just to be sure
					{
						...
						client->SetUploadFileID(NULL); 
						AddMorphLogLine(_T("banned client was in upload: %s"),client->DbgGetClientInfo());
					}
					// <== DLP [Xman] - sFrQlXeRt
			    ...
			    // <--- enkeyDEV: ICS
			    //Morph End - added by AndCycle, ICS
				//==MagicAngel=> Recognize MlDonkey XS Answer [Spike2/ideas by Wiz] - sFrQlXeRt
				case OP_XSMLDONKEY:
				{
					if (thePrefs.GetDebugClientTCPLevel() > 0)
