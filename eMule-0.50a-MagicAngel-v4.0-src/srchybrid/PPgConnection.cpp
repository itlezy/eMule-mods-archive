...
BOOL CPPgConnection::OnApply()
{
...

    {
	    ...
	    if ((UINT)thePrefs.GetMaxGraphUploadRate(true) < ulSpeed && ulSpeed != UNLIMITED)// gomez82 >>> vs2005 fixed Warning
	...
	if( (UINT)thePrefs.GetMaxGraphDownloadRate() < thePrefs.GetMaxDownload() && thePrefs.GetMaxDownload()!=UNLIMITED ) //==MagicAngel=> Fixed Morph compiler warning - sFrQlXeRt
