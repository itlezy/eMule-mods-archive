...
#include "Log.h"
#include "MagicAngel/DLP.h" // => DLP (check own usernick if valid) [Xman] - sFrQlXeRt
...
BOOL CPPgGeneral::OnApply()
{
	...
	if((theApp.dlp->IsDLPavailable() && (theApp.dlp->DLPCheckUsername_Hard(strNick) || theApp.dlp->DLPCheckUsername_Soft(strNick))) ||
		(theApp.MagicDLP->IsMagicDLPavailable() && (theApp.MagicDLP->DLPCheckUsername_GPL(strNick) || theApp.MagicDLP->DLPCheckUsername_Hard(strNick) || theApp.MagicDLP->DLPCheckUsername_Soft(strNick)))) // => MagicDLP - sFrQlXeRt
	{
		AfxMessageBox(_T("This nick is not allowed"));
		strNick = DEFAULT_NICK; // changed to default nick - sFrQlXeRt
		GetDlgItem(IDC_NICK)->SetWindowText(strNick);
	}
	// <== DLP [Xman] - sFrQlXeRt
