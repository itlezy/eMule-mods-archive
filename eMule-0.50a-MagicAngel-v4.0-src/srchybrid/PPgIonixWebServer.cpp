	...
	:  CPPgtooltipped(CPPgIonixWebServer::IDD)
{
	...
	m_imageList.Add(CTempIconLoader(_T("WEB"))); //==MagicAngel=> Fixed Morph tab bug - sFrQlXeRt
...
void CPPgIonixWebServer::InitTab(bool firstinit, int Page)
{
	if (m_tabCtr.GetSafeHwnd() != NULL  && firstinit ) {
		...
		//==MagicAngel=> Fixed Morph tab bug - sFrQlXeRt
		m_tabCtr.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, WEBSERVER, _T("Web server"), 0, (LPARAM)WEBSERVER); 
		m_tabCtr.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, MULTIWEBSERVER, _T("Multi user"), 1, (LPARAM)MULTIWEBSERVER); // note that the string Multi user is REAL HARD coded 
		m_tabCtr.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, NTSERVICE, /*_T("NT service") minor display fix;)*/_T("NT Service"), 1, (LPARAM)MULTIWEBSERVER); // note that the string Multi user is REAL HARD coded
		//<=MagicAngel== Fixed Morph tab bug - sFrQlXeRt
	}
