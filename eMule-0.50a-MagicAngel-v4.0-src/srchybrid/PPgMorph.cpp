	...
	, m_ctrlTreeOptions(theApp.m_iDfltImageListColorFlags)
{
	...
	// <== [MoNKi: -USS initial TTL-] - Stulle
	m_htiMinUpload = NULL;
	//m_htiUpSecu = NULL; // => Removed Morph Anti-Leecher - sFrQlXeRt
	m_htiDlSecu = NULL;
	m_htiDisp = NULL;
...
void CPPgMorph::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		*/
		// <== Removed Morph Anti-Leecher - sFrQlXeRt
		//MORPH START - Added by schnulli900, filter clients with failed downloads [Xman]
		m_htiFilterClientFailedDown = m_ctrlTreeOptions.InsertCheckBox(GetResString(IDS_FILTER_CLIENTFAILEDDOWN), m_htiUM, m_bFilterClientFailedDown); 
		//MORPH END   - Added by schnulli900, filter clients with failed downloads [Xman]
...
{	
	if(m_hWnd)
	{
		...
		//SetTool(m_htiUpSecu,IDS_SECURITY_TIP); // => Removed Morph Anti-Leecher - sFrQlXeRt
