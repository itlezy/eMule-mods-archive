...
void CPPgMorph2::Localize(void)
{
	if(m_hWnd)
	{
		...
		//MORPH START - Added by milobac and Yun.SF3, FakeCheck, FakeReport, Auto-updating
		//MagicAngel ==> StringID Fix - YumeYao
		//GetDlgItem(IDC_MORPH2_FILE)->SetWindowText(GetResString(IDS_FILES));
		GetDlgItem(IDC_MORPH2_FILE)->SetWindowText(GetResString(IDS_FILE));
		//MagicAngel <== StringID Fix - YumeYao
...
{
	CString strBuffer = _T("http://emulepawcio.sourceforge.net/nieuwe_site/Ipfilter_fakes/ipfilter.rar");
