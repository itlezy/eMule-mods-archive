...
// MORPH END leuk_he tooltipped
{
	...
	m_imageList.Add(CTempIconLoader(_T("CLIENTSONQUEUE"))); //==MagicAngel=> Fixed Morph tab bug - sFrQlXeRt
	// MORPH end tabbed option [leuk_he]
}
...
void CPPgWebServer::InitTab(bool firstinit, int Page)
{
	if (firstinit) {
		...
		m_tabCtr.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, MULTIWEBSERVER, _T("Multi user"), 1, (LPARAM)MULTIWEBSERVER); 
		m_tabCtr.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, NTSERVICE, _T("NT Service"), 1, (LPARAM)NTSERVICE); 
		//<=MagicAngel== Fixed Morph tab bug - sFrQlXeRt
	}
