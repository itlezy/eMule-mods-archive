...
//MORPH START - Changed by Stulle, No zz ratio for http traffic
{
	...
	// ==> Buffer Time Limit sFrQlXeRt - evcz
	//if ((m_nTotalBufferData > thePrefs.GetFileBufferSize()) || (dwCurTick > (m_nLastBufferFlushTime + BUFFER_TIME_LIMIT)))
	if ((m_nTotalBufferData > thePrefs.GetFileBufferSize()) || (dwCurTick > (m_nLastBufferFlushTime + (thePrefs.GetBufferTimeLimit()*60000))))
	// <== Buffer Time Limit sFrQlXeRt - evcz
	{
...
bool CPartFile::CanAddSource(uint32 userid, uint16 port, uint32 serverip, uint16 serverport, UINT* pdebug_lowiddropped, bool Ed2kID, bool bFilterLowID) //==sFrQlXeRt=> Secure Source Exchange
{
	...
	// normal eMule won't send us any LowID Sources via Source Exchange
	if (bFilterLowID && IsLowID(hybridID))
		return false;
	//<=sFrQlXeRt== Secure Source Exchange
...
//				m_nCompleteSourcesCount;
			...
			//  Adjust network accounts for 80%, we account for 20% with what we see and make sure we are still above the normal.
			{
				...
				if( m_nCompleteSourcesCount < m_nCompleteSourcesCountLo )
					m_nCompleteSourcesCount = m_nCompleteSourcesCountLo;
				// gomez82 >>>  Code Fix: must use m_nCompleteSourcesCountLo instead of m_nCompleteSourcesCount
				/*
...
UINT AFX_CDECL CPartFile::CompleteThreadProc(LPVOID pvParams) 
{ 
...
void CPartFile::AddClientSources(CSafeMemFile* sources, uint8 uClientSXVersion, bool bSourceExchange2, /*const */CUpDownClient* pClient) //==sFrQlXeRt=> Secure Source Exchange
{
	//==MagicAngel=> Secure Source Exchange - sFrQlXeRt
	//==>fix crash when adding an ed2k link with IP - evcz
	if(pClient)
{
	//<==fix crash when adding an ed2k link with IP - evcz
		if (!pClient->WaitingForXSAnswer()) {
			pClient->AddNotRequestedXSAnswer();
			//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received not requested XS Packet from Client %s"), pClient->DbgGetClientInfo());
			if (thePrefs.GetLogFilteredIPs())
				DebugLog(LOG_ERROR, _T("Secure XS: Received not requested XS Packet from Client %s"), pClient->DbgGetClientInfo());
		}
		pClient->SetWaitingForXSAnswer(false);
	} // fix crash when adding an ed2k link with IP - evcz
	//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
	...
	if (!bSourceExchange2){
		...
		if (nCount*(4+2+4+2) == uDataSize)
		{
			...
			if (uClientSXVersion < 1) {
				...
				pClient->AddFilteredXSPacket(); //==MagicAngel=> Secure Source Exchange - sFrQlXeRt
				return;
			}
			uPacketSXVersion = 1;
		}
	...
	//==MagicAngel=> Secure Source Exchange - sFrQlXeRt
	if(pClient) // fix crash when adding an ed2k link with IP - evcz
		pClient->AddReceivedXSSources(nCount);
	//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
		...
		if (uPacketSXVersion >= 3)
		{
			...
			if (!IsLowID(dwID))
			{
				if (!IsGoodIP(dwIDED2K))
				{
					...
					pClient->AddReceivedBadIP();
					//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received bad IP from Client %s"), pClient->DbgGetClientInfo()); // remove this later
					if (thePrefs.GetLogFilteredIPs())
						DebugLog(LOG_ERROR, _T("Secure XS: Received bad IP from Client %s"), pClient->DbgGetClientInfo()); // remove this later
					//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
					continue;
				}
				if (theApp.ipfilter->IsFiltered(dwIDED2K))
				{
					...
					*/
					pClient->AddReceivedFiteredIP();
					if (thePrefs.GetLogFilteredIPs()){
						//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received filtered IP from Client %s (%s - %s)"), pClient->DbgGetClientInfo(), ipstr(dwID), theApp.ipfilter->GetLastHit()); // remove this later (evcz: i think it's worth to keep this line in verbose log (no needed in morphlog)... really... ;) )
						DebugLog(LOG_ERROR, _T("Secure XS: Received filtered IP from Client %s (%s - %s)"), pClient->DbgGetClientInfo(), ipstr(dwID), theApp.ipfilter->GetLastHit()); //replaced original log, this one has more usefull infos
					}
					//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
					continue;
				}
			...
			// additionally check for LowID and own IP
			if (!CanAddSource(dwID, nPort, dwServerIP, nServerPort, NULL, false, true)) //==MagicAngel=> Secure Source Exchange - sFrQlXeRt
			{
				//==MagicAngel=> Secure Source Exchange - sFrQlXeRt
				//if (thePrefs.GetLogFilteredIPs())
				//	AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange"), ipstr(dwIDED2K));
				// normal eMule won't send us sources with same IP and Port as we have or LowID Sources
				if(pClient)//fix crash when adding an ed2k link with IP - evcz
				{
					pClient->AddReceivedUselessIP();
					if (thePrefs.GetLogFilteredIPs())
						DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received useless IP from Client %s"), pClient->DbgGetClientInfo()); // remove this later
				}
		...
		else
		{
			...
			if (!IsLowID(dwID))
			{
				...
				{ 
					...
					//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received bad IP from Client %s"), pClient->DbgGetClientInfo()); // remove this later
					if (thePrefs.GetLogFilteredIPs())
						DebugLog(LOG_ERROR, _T("Secure XS: Received bad IP from Client %s"), pClient->DbgGetClientInfo());
					//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
					continue;
				}
				if (theApp.ipfilter->IsFiltered(dwID))
				{
					...
					*/
					...
					{
						//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received filtered IP from Client %s (%s - %s)"), pClient->DbgGetClientInfo(), ipstr(dwID), theApp.ipfilter->GetLastHit()); // remove this later (evcz: i think it's worth to keep this line in verbose log (no needed in morphlog)... really... ;) )
						DebugLog(LOG_ERROR, _T("Secure XS: Received filtered IP from Client %s (%s - %s)"), pClient->DbgGetClientInfo(), ipstr(dwID), theApp.ipfilter->GetLastHit());
					}
					//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
					continue;
				}
			...
			// additionally check for LowID and own IP
			if (!CanAddSource(dwID, nPort, dwServerIP, nServerPort, NULL, true, true)) //==MagicAngel=> Secure Source Exchange - sFrQlXeRt
			{
				//==MagicAngel=> Secure Source Exchange - sFrQlXeRt
				//if (thePrefs.GetLogFilteredIPs())
				...
				if(pClient)//fix crash when adding an ed2k link with IP - evcz
				{
					...
					//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received useless IP from Client %s"), pClient->DbgGetClientInfo()); // remove this later
					if (thePrefs.GetLogFilteredIPs())
						DebugLog(LOG_ERROR, _T("Secure XS: Received useless IP from Client %s"), pClient->DbgGetClientInfo());
				}
				else if (thePrefs.GetLogFilteredIPs())
					AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange"), ipstr(dwID));
				//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
	...
	if(pClient)//fix crash when adding an ed2k link with IP - evcz
	{
		if (pClient->IsSuspectXSClient()) {
			//DebugLog(LOG_MORPH | LOG_ERROR, _T("Secure XS: Received XS Packet from suspect Client. Name %s, not requested XS Answers %u, filtered XS Packets %u, bad IPs %u, filtered IPs %u, useless IPs %u, received Sources %u"),
			DebugLog(LOG_ERROR, _T("Secure XS: Received XS Packet from suspect Client. Name %s, not requested XS Answers %u, filtered XS Packets %u, bad IPs %u, filtered IPs %u, useless IPs %u, received Sources %u"),
				pClient->GetUserName(), pClient->GetNotRequestedXSAnswers(), pClient->GetFilteredXSPackets(), pClient->GetReceivedBadIPs(), pClient->GetReceivedFilteredIPs(), pClient->GetReceivedUselessIPs(), pClient->GetReceivedXSSources());
			UINT uSumBadSources = pClient->GetFilteredXSPackets() + pClient->GetReceivedBadIPs() + pClient->GetReceivedFilteredIPs() + pClient->GetReceivedUselessIPs();
			if (uSumBadSources >= pClient->GetReceivedXSSources()*0.25f) { // 25% of his sent Sources were bad
				// TODO: disconnect and add to IPFilter
				DebugLog(LOG_MORPH | LOG_WARNING, _T("Added Client %s to IPFilter because %u%% of his sent Sources were bad (not requested XS Answers %u, filtered XS Packets %u, bad IPs %u, filtered IPs %u, useless IPs %u, received Sources %u)"), pClient->DbgGetClientInfo(), 100*uSumBadSources/pClient->GetReceivedXSSources(), pClient->GetNotRequestedXSAnswers(), pClient->GetFilteredXSPackets(), pClient->GetReceivedBadIPs(), pClient->GetReceivedFilteredIPs(), pClient->GetReceivedUselessIPs(), pClient->GetReceivedXSSources());
			}
		}
	}
	//<=MagicAngel== Secure Source Exchange - sFrQlXeRt
...
//	m_nTotalBufferData += lenData; //MORPH - Flush Thread, Moved above
	...
	//MORPH START - Added by SiRoB, Import Parts
		...
		FlushBuffer();
	//MORPH END   - Added by SiRoB, Import Parts
	//==MagicAngel=> Intelligent Filebuffer Flushing [WiZaRd] - sFrQlXeRt
	else if(thePrefs.UseIntelligentFlush())
	{
