...
//#define BUFFER_TIME_LIMIT	60000   // Max milliseconds before forcing a flush // ==> Buffer Time Limit sFrQlXeRt - evcz
...
public:
	...
	*/
	...
	static bool CanAddSource(uint32 userid, uint16 port, uint32 serverip, uint16 serverport, UINT* pdebug_lowiddropped = NULL, bool Ed2kID = true, bool bFilterLowID = false); //==sFrQlXeRt=> Secure Source Exchange
	...
	void	AddClientSources(CSafeMemFile* sources, uint8 sourceexchangeversion, bool bSourceExchange2, /*const */CUpDownClient* pClient = NULL); //==sFrQlXeRt=> Secure Source Exchange
...
private:
	...
	static UINT AFX_CDECL CompleteThreadProc(LPVOID pvParams); // Lord KiRon - Used as separate thread to complete file
