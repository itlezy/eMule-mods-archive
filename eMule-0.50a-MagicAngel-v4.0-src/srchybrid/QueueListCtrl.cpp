...
void CQueueListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	*/
	...
	if(thePrefs.ColoredUpload() != 0 && thePrefs.ColorQueue()) 
		crOldTextColor = dc.SetTextColor(client->GetClientColor(crOldTextColor));
	// <== Colored Upload - sFrQlXeRt
	CHeaderCtrl *pHeaderCtrl = GetHeaderCtrl();
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				switch (iColumn)
				{
					case 0:{
						...
						if(client->IsLeecher()>0 && client->GetClientSoft() == SO_EMULE)
							iImage = 19;
						// <== Leecher Icon - sFrQlXeRt
						...
						if (client->IsFriend())
							iImage = 2;
						else*/else if (client->GetClientSoft() == SO_EDONKEYHYBRID)// ==> Leecher Icon - sFrQlXeRt
							iImage = 4;
...
void CQueueListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && (client->IsBanned() || client->IsLeecher()>0)) ? MF_ENABLED : MF_GRAYED), MP_UNBAN, GetResString(IDS_UNBAN));
	// <== Angel Argos - sFrQlXeRt
	...
	if (iSel != -1){
		...
		switch (wParam){
			...
			case MP_UNBAN:
				// ==> Angel Argos - sFrQlXeRt
				//if (client->IsBanned()){
				if (client->IsBanned() || client->IsLeecher()>0){
				// <== Angel Argos - sFrQlXeRt
