	...
	try
	{
		switch (opcode)
		{
			...
			case OP_IDCHANGE:{
				...
				//==MagicAngel=> Reask Sources after IP Change v4 [Xman] - sFrQlXeRt - YumeYao
				if(thePrefs.IsRASAIC()
					&& serverconnect->GetClientID() != 0 && theApp.last_valid_ip != 0
					...
					&& serverconnect->GetClientID() != theApp.last_valid_serverid)
				{
...

					{
						...
						if(GetTickCount() - theApp.last_ip_change > FILEREASKTIME + 60000){
							...
							AddLogLine(false, GetResString(IDS_ID_CHANGE_DETECTED), 
								theApp.last_valid_serverid,
								(theApp.last_valid_serverid < 16777216) ? GetResString(IDS_LOW_ID) : GetResString(IDS_HIGH_ID),
								serverconnect->GetClientID(),
								(serverconnect->GetClientID() < 16777216)  ? GetResString(IDS_LOW_ID) : GetResString(IDS_HIGH_ID),
								GetResString(IDS_REASK_ALL_SOURCES));
						}
						else {
							...
							AddLogLine(false, GetResString(IDS_ID_CHANGE_DETECTED), 
								...
								serverconnect->GetClientID(),
								(serverconnect->GetClientID() < 16777216)  ? GetResString(IDS_LOW_ID) : GetResString(IDS_HIGH_ID),
								GetResString(IDS_REASK_ALL_SOURCES_DELAYED));
						}
				...
				theApp.bInternetMaybeDown = false; //we have to reopen here if we are using server only
				//<=MagicAngel== Reask Sources after IP Change v4 [Xman] - sFrQlXeRt  - YumeYao
