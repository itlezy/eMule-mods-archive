	...
	:CWinApp(lpszAppName)
{
	...
	last_valid_ip = 0;
	recheck_ip = 0;// gomez82 >>> 
	last_traffic_reception = 0;
	bInternetMaybeDown = true;
	//<=MagicAngel== Reask Sources after IP Change v4 [Xman] - sFrQlXeRt
	m_bAutoStart = false;
...
// MOD Note: end
   ...
   if (CemuleApp::m_szMMVersion[0]!=0)
	...
	// ==> DLP [ScarAngel] - sFrQlXeRt
	m_strModVersionPure = CemuleApp::m_szMVersion;
	m_strModVersionPure.AppendFormat(_T(" "));
	m_uModLength = (uint8)(m_strModVersionPure.GetLength());
	// <== DLP [ScarAngel] - sFrQlXeRt
}
...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	*/
	...
	dlp = new CDLP(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR)); // => DLP [Xman] - sFrQlXeRt
	MagicDLP = new CMagicDLP(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR)); // => MagicDLP - sFrQlXeRt
...
*/
	...
	//if we have a valid ip and receive a new one or if the IP is about to be resetted
	//const bool bIPChangeOrReset = (dwIP == 0 || (dwLastRealIP != 0 && dwIP != dwLastRealIP));
	const bool bFirstConnect = (dwIP != 0 && dwLastRealIP == 0);
	const bool bRealIPChange = (dwIP != 0 && dwLastRealIP != 0 && dwLastRealIP != dwIP);
	if(bRealIPChange)
		theApp.QueueLogLineEx(LOG_WARNING, _T("*** Detected IP change from %s (%s) to %s (%s)"),
		ipstr(dwLastRealIP), ::IsLowID(dwLastRealIP) ? GetResString(IDS_IDLOW) : GetResString(IDS_IDHIGH),
		ipstr(dwIP), ::IsLowID(dwIP) ? GetResString(IDS_IDLOW) : GetResString(IDS_IDHIGH));
	if(dwIP != 0)
		dwLastRealIP = dwIP;
	m_dwPublicIP = dwIP;
	if(bRealIPChange)
	...
	//if(bIPChangeOrReset)
	//{
	//	//there is a problem with webserver - not dynamically assignable
	//}
...
{
	//WiZaRd: no need to keep that static
