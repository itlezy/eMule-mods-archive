...
BEGIN_MESSAGE_MAP(CemuleDlg, CTrayDialog)
	...
	ON_MESSAGE(UM_MAVERSIONCHECK_RESPONSE, OnMAVersionCheckResponse)
	// => DLP [Xman] - sFrQlXeRt
	ON_MESSAGE(UM_DLPVERSIONCHECK_RESPONSE, OnDLPVersionCheckResponse) 
	// => MagicDLP - sFrQlXeRt
	ON_MESSAGE(UM_MDLPVERSIONCHECK_RESPONSE, OnMDLPVersionCheckResponse)
	...
	: CTrayDialog(CemuleDlg::IDD, pParent)
{
	...
	m_bCheckwasDone = false; // => DLP [Xman] - sFrQlXeRt
}
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	if (pSysMenu != NULL){
		...
		// ==> Magic Angel version check - sFrQlXeRt
		ASSERT( (MP_MAVERSIONCHECK & 0xFFF0) == MP_MAVERSIONCHECK && MP_MAVERSIONCHECK < 0xF000);
		pSysMenu->AppendMenu(MF_STRING, MP_MAVERSIONCHECK, GetResString(IDS_MAVERSIONCHECK));
		// <== Magic Angel version check - sFrQlXeRt
...
void CemuleDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	switch (nID /*& 0xFFF0*/)
	{
		...
		//MORPH END   - Added by SiRoB, New Version check
		// ==> Magic Angel version check - sFrQlXeRt
	    case MP_MAVERSIONCHECK:
			DoMAVersioncheck(true);
			break;
		// <== Magic Angel version check - sFrQlXeRt
...
void CemuleDlg::AddLogText(UINT uFlags, LPCTSTR pszText)
{
	...
	if (iLen >= 0)
	{
		// ==> Changed Morph Log - sFrQlXeR
		//if (!(uFlags & LOG_DEBUG)
		if (!(uFlags & LOG_DEBUG) && !(uFlags & LOG_MORPH))
		// <== Changed Morph Log - sFrQlXeRt
		{
...
#endif
	delete theApp.dlp;				theApp.dlp=NULL; // => DLP [Xman] - sFrQlXeRt
	delete theApp.MagicDLP;			theApp.MagicDLP = NULL; // => MagicDLP - sFrQlXeRt
	}
	catch(...)
	{
...
LRESULT CemuleDlg::OnTaskbarNotifierClicked(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	switch (m_wndTaskbarNotifier->GetMessageType())
	{
		...
		case TBN_NEWMAVERSION:
		{
			ShellExecute(NULL, NULL, _T("http://magicangel.sourceforge.net/updates/MagicAngel"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		// <== Magic Angel version check - sFrQlXeRt
		// ==> DLP [Xman] - sFrQlXeRt
		case TBN_NEWDLPVERSION:
		{
			ShellExecute(NULL, NULL, _T("https://specialdlp.svn.sourceforge.net/svnroot/specialdlp/tags/x86/all-verycd/antiLeech.dll.new"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		// <== DLP [Xman] - sFrQlXeRt
	}
...
void CemuleDlg::Localize()
{
	...
	if (pSysMenu)
	{
		...
		//MORPH END   - Added by SiRoB, New Version check
		// ==> Magic Angel version check - sFrQlXeRt
		VERIFY( pSysMenu->ModifyMenu(MP_MAVERSIONCHECK, MF_BYCOMMAND | MF_STRING, MP_MAVERSIONCHECK, GetResString(IDS_MAVERSIONCHECK)) );
		// <== Magic Angel version check - sFrQlXeRt
...
LRESULT CemuleDlg::OnMVersionCheckResponse(WPARAM /*wParam*/, LPARAM lParam)
{
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			{
				uint32 dwResult = ((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr;
				// ==> Morph-Version-Check Changed for Magic Angel - sFrQlXeRt
...
// ==> Magic Angel version check - sFrQlXeRt
void CemuleDlg::DoMAVersioncheck(bool manual) {
	if (!manual && thePrefs.GetLastMAVC()!=0) {
		CTime last(thePrefs.GetLastMAVC());
		struct tm tmTemp; // gomez82 >>> vs2005
		time_t tLast=safe_mktime(last.GetLocalTm(&tmTemp));// gomez82 >>> vs2005
		time_t tNow=safe_mktime(CTime::GetCurrentTime().GetLocalTm(&tmTemp));// gomez82 >>> vs2005
		if ( (difftime(tNow,tLast) / 86400)<thePrefs.GetUpdateDays() )
			return;
	}
	if (WSAAsyncGetHostByName(m_hWnd, UM_MAVERSIONCHECK_RESPONSE, "magicangelvercheck.dyndns.info", m_acMAVCDNSBuffer, sizeof(m_acMAVCDNSBuffer)) == 0){
		AddLogLine(true,GetResString(IDS_NEWVERSIONFAILED));
	}
	// ==> DLP [Xman] - sFrQlXeRt
	else {
		WSAAsyncGetHostByName(m_hWnd, UM_DLPVERSIONCHECK_RESPONSE, "dlp.dyndns.info", m_acDLPBuffer, sizeof(m_acDLPBuffer));
		WSAAsyncGetHostByName(m_hWnd, UM_MDLPVERSIONCHECK_RESPONSE, "mdlp.dyndns.info", m_acMDLPBuffer, sizeof(m_acMDLPBuffer)); // => MagicDLP - sFrQlXeRt
	}
	// <== DLP [Xman] - sFrQlXeRt
}

LRESULT CemuleDlg::OnMAVersionCheckResponse(WPARAM /*wParam*/, LPARAM lParam)
{
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		int iBufLen = WSAGETASYNCBUFLEN(lParam);
		if (iBufLen >= sizeof(HOSTENT))
		{
			LPHOSTENT pHost = (LPHOSTENT)m_acMAVCDNSBuffer;
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				uint32 dwResult = ((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr;
				uint8 abyCurVer[4] = { (uint8)(CemuleApp::m_nMVersionBld + 1), (uint8)(CemuleApp::m_nMVersionMin), (uint8)(CemuleApp::m_nMVersionMjr), 0};
				dwResult &= 0x00FFFFFF;
				if (dwResult > *(uint32*)abyCurVer){
					thePrefs.UpdateLastMAVC();
					SetActiveWindow();
					Log(LOG_SUCCESS|LOG_STATUSBAR,GetResString(IDS_NEWMAVERSIONAVL));
					ShowNotifier(GetResString(IDS_NEWMAVERSIONAVLPOPUP), TBN_NEWMAVERSION);
					if (AfxMessageBox(GetResString(IDS_NEWMAVERSIONAVL)+GetResString(IDS_VISITMAVERSIONCHECK),MB_YESNO)==IDYES) {
						ShellExecute(NULL, NULL, _T("http://magicangel.sourceforge.net/updates/MagicAngel"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT); // ==> evcz changed download url
					}
				}
				else{
					thePrefs.UpdateLastMAVC();
					AddLogLine(true,GetResString(IDS_NONEWMAVERVERSION));
				}
				return 0;
			}
		}
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				uint32 dwResult = ((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr;
				dwResult &= 0x00FFFFFF;
				thePrefs.UpdateLastMAVC(); // update together with the Magic Angel version check - sFrQlXeRt
				if (theApp.dlp->IsDLPavailable() && dwResult > theApp.dlp->GetDLPVersion()){
					//if(theApp.IsSplash()) theApp.DestroySplash(); //Xman new slpash-screen arrangement // we don't need this - sFrQlXeRt
					SetActiveWindow();
					Log(LOG_SUCCESS|LOG_STATUSBAR,GetResString(IDS_NEWDLP));
					ShowNotifier(GetResString(IDS_NEWDLP), TBN_NEWDLPVERSION);
					if (m_bCheckwasDone==false && AfxMessageBox(GetResString(IDS_NEWDLPDOWNLOAD),MB_YESNO)==IDYES) {
						ShellExecute(NULL, NULL, _T("https://specialdlp.svn.sourceforge.net/svnroot/specialdlp/tags/x86/all-verycd/antiLeech.dll.new"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT); // ==> evcz changed download url
					}
					m_bCheckwasDone=false;
				}
				else{
					AddLogLine(true, GetResString(IDS_NO_NEWDLP));
				}
				return 0;
			}
		}
...
// ==> MagicDLP - sFrQlXeRt
LRESULT CemuleDlg::OnMDLPVersionCheckResponse(WPARAM /*wParam*/, LPARAM lParam)
{
	//Xman Info:
	//IP-samples:
	//5.0.0.99 --> DLP version 5
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			LPHOSTENT pHost = (LPHOSTENT)m_acMDLPBuffer;
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				uint32 dwResult = ((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr;
				dwResult &= 0x00FFFFFF;
				thePrefs.UpdateLastMAVC(); // update together with the Magic Angel version check - sFrQlXeRt
				if (theApp.MagicDLP->IsMagicDLPavailable() && dwResult > theApp.MagicDLP->GetMagicDLPVersion()){
					SetActiveWindow();
					Log(LOG_SUCCESS|LOG_STATUSBAR,GetResString(IDS_NEWMDLP));
					ShowNotifier(GetResString(IDS_NEWMDLP), TBN_NEWMAVERSION);
					if (m_bCheckwasDone==false && AfxMessageBox(GetResString(IDS_NEWMDLPDOWNLOAD),MB_YESNO)==IDYES) {
						ShellExecute(NULL, NULL, _T("http://magicangel.sourceforge.net/updates/MagicAntiLeech.dll.new"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT); // ==> evcz changed download url
					}
					m_bCheckwasDone=false;
				}
				else{
					AddLogLine(true, GetResString(IDS_NO_NEWMDLP));
				}
				return 0;
			}
		}
