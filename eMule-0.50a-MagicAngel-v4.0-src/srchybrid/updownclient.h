...
//class CTag; //Added by SiRoB, SNAFU // => Removed Morph Anti-Leecher - sFrQlXeRt
class CTag; // => DLP [Xman] - sFrQlXeRt
...
public:
	...
	//const CString&	GetClientModTag() const							{ return m_strNotOfficial; } // => Removed Morph Anti-Leecher - sFrQlXeRt
	...
	*/
	...
	//void			CheckForGPLEvilDoer(); // => Removed GPL Breaker detection - sFrQlXeRt
	...
	*/
	...
	//void			BanLeecher(LPCTSTR pszReason = NULL); //MORPH - Added by IceCream, Anti-leecher feature // => Removed Morph Anti-Leecher - sFrQlXeRt
...
// <-- ZZ:DownloadManager
	...
	*/
	...
	//LPCTSTR		TestLeecher(); //MORPH - Added by IceCream, anti-leecher feature // => Removed Morph Anti-Leecher - sFrQlXeRt
...
protected:
	...
	void	Init();
	// ==> DLP [Xman] - sFrQlXeRt
	// decide if Hello or Hello-Answer -> now independent from any other variables
	...
	//TCHAR*	old_m_pszUsername; //MORPH - Added by IceCream, Antileecher feature // => Removed Morph Anti-Leecher - sFrQlXeRt
	...
	bool	m_bIsML;
	//--group to aligned int32
	bool	m_bGPLEvildoer; // => Removed GPL Breaker detection - sFrQlXeRt // - Kept to align to int32 - YumeYao
...
/*MORPH - FIX for zz code*/UINT		m_nCurSessionPayloadUp;
	...
	// Download
	//
	//CPartFile*	reqfile; //==MagicAngel=> Reask Sources after IP Change v4 [Xman] - sFrQlXeRt (moved to 'public')
...
public:
	//bool	IsMorphLeecher(); // Morph Start - added by Stulle, Morph Leecher Detection // => Removed Morph Anti-Leecher - sFrQlXeRt
...
//==============================MagicAngel=>
	...
	CString	LeecherReason; // => Leecher at Clientdetail - sFrQlXeRt
	int		m_iPunishment; // => Angel Argos - sFrQlXeRt

	// ==> Anti Uploader Ban [Stulle] - sFrQlXeRt
	bool	AntiUploaderBanActive();
	bool	GetAntiUploaderCaseThree()	{return m_bAntiUploaderCaseThree;}
	void	CheckAntiUploaderBan(); // => Anti Uploader Ban for Punish Donkeys without SUI - sFrQlXeRt
	// <== Anti Uploader Ban [Stulle] - sFrQlXeRt

	 //==sFrQlXeRt=> Reask Sources after IP Change v4 [Xman]
	void		SetLastAskedTime(CPartFile* partFile, const uint32& in)	{ m_fileReaskTimes.SetAt(partFile, in); } // thx 4 this code fly out to WiZaRd 
	CPartFile*	reqfile; // moved to 'public'
	 //<=sFrQlXeRt== Reask Sources after IP Change v4 [Xman]
	//==sFrQlXeRt=> Secure Source Exchange
	//void	SetSuspectXSClient()				{m_bSuspectXSClient = true;}
	void	SetWaitingForXSAnswer(bool bIn)		{m_bWaitingForXSAnswer = bIn;}
	void	AddNotRequestedXSAnswer()			{m_uNotRequestedXSAnswers++; m_bSuspectXSClient = true;}
	void	AddReceivedXSSources(UINT uSources) {m_uReceivedXSSources += uSources;}
	void	AddFilteredXSPacket()				{m_uFilteredXSPackets++; m_bSuspectXSClient = true;}
	void	AddReceivedBadIP()					{m_uReceivedBadIPs++; m_bSuspectXSClient = true;}
	void	AddReceivedFiteredIP()				{m_uReceivedFilteredIPs++; m_bSuspectXSClient = true;}
	void	AddReceivedUselessIP()				{m_uReceivedUselessIPs++; m_bSuspectXSClient = true;}

	bool	IsSuspectXSClient()	const			{return m_bSuspectXSClient;}
	bool	WaitingForXSAnswer() const			{return m_bWaitingForXSAnswer;}
	uint16	GetNotRequestedXSAnswers() const	{return m_uNotRequestedXSAnswers;}
	UINT	GetReceivedXSSources() const		{return m_uReceivedXSSources;}
	uint16	GetFilteredXSPackets() const		{return m_uFilteredXSPackets;}
	uint16	GetReceivedBadIPs() const			{return m_uReceivedBadIPs;}
	uint16	GetReceivedFilteredIPs() const		{return m_uReceivedFilteredIPs;}
	uint16	GetReceivedUselessIPs() const		{return m_uReceivedUselessIPs;}
	//<=sFrQlXeRt== Secure Source Exchange

	DWORD	GetSpreadReAskTime() const {return m_dwSpreadReAskTime;} // => Spread Requests [Stulle] - sFrQlXeRt
	COLORREF	GetClientColor(COLORREF oldColor) const; // ==> Colored Upload - sFrQlXeRt

private:
	// ==> DLP [Xman] - sFrQlXeRt
//	uint8	m_bLeecher; // moved down for alignment - YumeYao
	CString	old_m_strClientSoftwareFULL;
	CString	old_m_pszUsername;
	CString m_strBanMessage;
	uint8	m_bLeecher; // moved here for alignment - YumeYao
	uint8   uhashsize;

	bool	m_bIsPunishDonkeys; // moved here for alignment - YumeYao

	// Anti-Nick-Changer
	uint8 m_uNickchanges; 
	...
	// simple Anti-Thief
	float	GetMagicAngelVersion(CString modversion) const;
...
//	bool	m_bIsPunishDonkeys;// => Punish Donkeys without SUI - sFrQlXeRt // moved up for alignment - YumeYao
	// ==> Anti Uploader Ban [Stulle] - sFrQlXeRt
	bool	m_bAntiUploaderCaseThree; 
	bool	m_bAntiUploaderBanActive; // => Anti Uploader Ban for Punish Donkeys without SUI - sFrQlXeRt
	// <== Anti Uploader Ban [Stulle] - sFrQlXeRt

	//==sFrQlXeRt=> Secure Source Exchange
	bool	m_bSuspectXSClient;
	bool	m_bWaitingForXSAnswer;
	uint16	m_uNotRequestedXSAnswers;
	UINT	m_uReceivedXSSources;
	uint16	m_uFilteredXSPackets;
	uint16	m_uReceivedBadIPs;
	uint16	m_uReceivedFilteredIPs;
	uint16	m_uReceivedUselessIPs;
	//<=sFrQlXeRt== Secure Source Exchange

	DWORD	m_dwSpreadReAskTime; // => Spread Requests [Stulle] - sFrQlXeRt

protected:
	CPartFile*	m_fileFNF; //==sFrQlXeRt=> File Faker Detection [DavidXanatos]
...
public:
	...
	uint8   non_used1; // alignment - YumeYao
