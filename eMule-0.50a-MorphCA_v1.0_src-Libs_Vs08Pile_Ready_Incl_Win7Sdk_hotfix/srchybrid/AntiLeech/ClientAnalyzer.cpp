...
//For Xman and other who can not read properly: THIS is the data WE SENT! 
//Rare data is counted TWICE as is partial data as we can expect something for that in return!
...
//For Xman and others who can not read properly: THIS is the data WE GOT! 
//Rare data is counted TWICE as is FULL data as we should payback such nice behavior!
...
void	CAntiLeechData::AddSpam()		
{
	...
	if(m_pParent && thePrefs.GetLogAnalyzerEvents())
		theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"Analyzer: Spamcounter for (%s) increased: %i", m_pParent->DbgGetClientInfo(), m_pData->iBlockedMessages);
...
void	CAntiLeechData::DecSpam()		
{
	...
	--m_pData->iBlockedMessages;
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
	//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: Spamcounter for (%s) decreased: %i", m_pParent->DbgGetClientInfo(), m_pData->iBlockedMessages);
	if(bCheckNeeded)
		ReCheckScore();
}
...
void	CAntiLeechData::AddReask(const UINT time)	
{
	...
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents() && bBadTimeDiff1 != bBadTimeDiff2) //log only if needed //morph4u not needed
	//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"Analyzer: Reask for (%s) updated: %i reasks @ %s avg", m_pParent->DbgGetClientInfo(), m_pData->uReasks, CastSecondsToHM(m_pData->uAvgReaskTime/1000));	
...
void	CAntiLeechData::IncFastXSCounter()
{
	++m_pData->iFastXSAsks;
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
	//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"Analyzer: FastXS for (%s) increased: %i", m_pParent->DbgGetClientInfo(), m_pData->iFastXSAsks);
...
void	CAntiLeechData::IncXSAsks()	
{
	...
	++m_pData->uXSAsks;	
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
	//	theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_INFO, L"XS Asks for (%s) increased: %i", m_pParent->DbgGetClientInfo(), m_pData->uXSAsks);
	if(b != IsXSExploiter())
	{
		if(m_pParent && thePrefs.GetLogAnalyzerEvents())
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_ERROR, L"Analyzer: (%s) reached XS exploiter level: %u : %u", m_pParent->DbgGetClientInfo(), m_pData->uXSAsks, m_pData->uXSAnsw);
		ReCheckScore();
	}
}
...
void	CAntiLeechData::IncXSAnsw()	
{
	...
	++m_pData->uXSAnsw;
	//if(m_pParent && thePrefs.GetLogAnalyzerEvents())
	//	theApp.QueueDebogLogLineEx(LOG_MORPH | LOG_INFO, L"Analyzer: XS Answs for (%s) increased: %i", m_pParent->DbgGetClientInfo(), m_pData->uXSAnsw);
	if(b != IsXSExploiter())
	{
		if(m_pParent && thePrefs.GetLogAnalyzerEvents())
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_SUCCESS, L"Analyzer: (%s) left XS exploiter level: %u : %u", m_pParent->DbgGetClientInfo(), m_pData->uXSAsks, m_pData->uXSAnsw);
		ReCheckScore();
	}
}
...
//		const float fTimeDiff = (GetAvgReaskTime() + AT_REASKBUFFER - m_uiReaskTime)/60000.0f; //BuGFiX: how could I miss that!? :confused:
	...
	if(m_uiBadForThisSession != 0)
	{
		...
		if((m_uiBadForThisSession & AT_NICKTHIEF) || (m_uiBadForThisSession & AT_MODTHIEF) || (m_uiBadForThisSession & AT_MODFAKER))
		...
		if((m_uiBadForThisSession & AT_FILEFAKER) || (m_uiBadForThisSession & AT_UDPFNFFAKER))
	...
	m_fLastScore = cur_score / AT_BASESCORE;
	return m_fLastScore;
}
...
{	
	...
	if(m_pParent)
	{
...
#endif
		{
			...
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_ERROR, L"Analyzer: Marked as %s: (%s)", reason, m_pParent->DbgGetClientInfo());
		}
...
void CAntiLeechData::ClearBadForThisSession(const UINT i)
{
	...
	{
		if(thePrefs.GetLogAnalyzerEvents())
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_ERROR, L"Analyzer: client %s was NOT cleared from %s state (blocked by timer)", m_pParent ? m_pParent->DbgGetClientInfo() : L"NULL", reason);
		return;
	}
	...
	if(m_pParent)
	{
		...
		if(m_pParent && thePrefs.GetLogAnalyzerEvents())
#endif
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_SUCCESS, L"Analyzer: Cleared from %s state: (%s)", reason, m_pParent->DbgGetClientInfo());
	}
	m_uiBadForThisSession &= ~i;
	ReCheckScore();
}
...
{	
	...
	const CString OurMod = theApp.m_strModLongVersion; //cache it
...
//<<<
			sDaS(theApp.m_strModLongVersion, 0)
		};
...
CString	CAntiLeechData::GetAntiLeechDataString(/*const bool bBasics*/) const
{
	CString reason = L"";
	if(m_uiBadForThisSession != 0)
	{
		reason = L"Very bad: " + GetReasonString(m_uiBadForThisSession);
// 		if(!bBasics)
// 			reason.Append(L" || "); //just a separator
	}

// 	if(!bBasics)
// 	{
// 		reason.Append(L"\nAnalyzer:\n");
// 		reason.AppendFormat(L"UL: Full: %s | Part: %s | Rare: %s\n", CastItoXBytes(GetDownloadedFull(), false, false), CastItoXBytes(GetDownloadedPartial(), false, false), CastItoXBytes(GetDownloadedRare(), false, false));
// 		reason.AppendFormat(L"DL: Full: %s | Part: %s | Rare: %s\n", CastItoXBytes(GetUploadedFull(), false, false), CastItoXBytes(GetUploadedPartial(), false, false), CastItoXBytes(GetUploadedRare(), false, false));
// 	}
// 	else
//	{
		//[SEPPL] - Don't let the score fall too low because of UL/DL 
...
//	if(bBasics)
		reason.Replace(L"\n", L" "); //try to get a single line with spacing
...
//>>> .met-Merger			
			if(bCustomLoad)
			{
				...
				{
					theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"DBG: WARNING - Found duplicate credits for %s", md4str(newcredits->GetKey()));
...
void CTempCAList::Verify(CUpDownClient* client)
{
	...
	{		
		...
		{
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"%s: verification of CA data for %s canceled (%s is \"better\")", md4str(client->GetUserHash()), client->DbgGetClientInfo(), pLeft->DbgGetClientInfo());
...
//			ASSERT(pos != pList->GetHeadPosition());
			theApp.QueueDebugLogLineEx(LOG_MORPH | LOG_WARNING, L"%s: swapping CA data for %s and %s during verification", md4str(client->GetUserHash()), client->DbgGetClientInfo(), pLeft->DbgGetClientInfo());
