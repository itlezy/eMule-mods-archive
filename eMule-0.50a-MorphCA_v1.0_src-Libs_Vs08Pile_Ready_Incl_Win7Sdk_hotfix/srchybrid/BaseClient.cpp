...
#include "Version.h"
//<<< WiZaRd::ClientAnalyzer
#include "ModVersion.h" //>>> WiZaRd::Easy ModVersion/Same ModVersion Detection
#endif
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32 nLength)
{
	...
	{
		if (thePrefs.IsAdvSpamfilterEnabled() && !(IsFriend() || IsCommunity()) && GetMessagesSent() == 0)
		{
			SetSpammer(true);
	...
	if (thePrefs.IsChatCaptchaEnabled() && !(IsFriend() || IsCommunity())) { 
	...
	if (thePrefs.IsAdvSpamfilterEnabled() && !(IsFriend() || IsCommunity())) // friends are never spammer... (but what if two spammers are friends :P )
	{	
		bool bIsSpam = false;
		if (IsSpammer())
			bIsSpam = true;
