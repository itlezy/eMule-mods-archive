		...
		if (!IsColumnHidden(iColumn))
		{
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				if(pKnownFile)
				{
					...
					//powershare + behind filename
					else if(pKnownFile->GetPowerShared() && (pKnownFile->IsPartFile()))
						dc->SetTextColor((COLORREF)RGB(192,0,0)); //red
					//powershare -
					else if (Perm == PERM_NOONE)
						dc->SetTextColor((COLORREF)RGB(192,192,192)); //grey
					else if (Perm == PERM_FRIENDS)
						dc->SetTextColor((COLORREF)RGB(0,170,0)); //green
					// Mighty Knife: Community visible filelist
					else if (Perm == PERM_COMMUNITY)
						dc->SetTextColor((COLORREF)RGB(255,128,0)); //orange
					// [end] Mighty Knife
					else if (Perm == PERM_ALL)
						dc->SetTextColor((COLORREF)RGB(0,0,0));//black
					//powershare + full line
					if(pKnownFile->GetPowerShared() && (!pKnownFile->IsPartFile()))
						dc->SetTextColor((COLORREF)RGB(192,0,0)); //red
					//powershare -
				}
