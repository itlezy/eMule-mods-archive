...
void CStatisticsDlg::OnContextMenu(CWnd* /* pWnd */, CPoint doWhere)
{
	...
	mnuContext.AddMenuTitle(_T("Statistics Display Options"), true);
    if(m_bTreepaneHidden) mnuContext.AppendMenu(MF_STRING, MP_HIDE_STREE, _T("Restore Statistics Tree"), _T("RESTSTATSTREE"));
        else    mnuContext.AppendMenu(MF_STRING, MP_HIDE_STREE, _T("Hide Statistics Tree"), _T("HIDESTATSTREE"));
    mnuContext.AppendMenu(MF_STRING, MP_HIDE_SCOPES, _T("Hide Scopes"), _T("HIDESCOPES"));
    mnuContext.AppendMenu(MF_SEPARATOR);
    mnuContext.AppendMenu(myFlags, MP_MAX_DL, _T("DL Speed Scope Full Height"), _T("DLSTATSMAX"));
    mnuContext.AppendMenu(myFlags, MP_MAX_UP, _T("UL Speed Scope Full Height"), _T("ULSTATSMAX"));
    mnuContext.AppendMenu(myFlags, MP_MAX_ST, _T("Connection Scope Full Height"), _T("STSTATSMAX"));
    mnuContext.AppendMenu(myFlags, MP_RESET_OSCOPES, _T("Reset Scope Spacing"), _T("RESETOSCOPE")); 
	mnuContext.AppendMenu(MF_SEPARATOR);
	mnuContext.AppendMenu(MF_STRING, MP_STATISICS_PREF, _T("Preferences"), _T("STATISTICS"));
...
{	
	if(thePrefs.GetSplitterbarPositionStat_HL()>97 && thePrefs.GetSplitterbarPositionStat_HR()>97) return;
...
{	
	if(thePrefs.GetSplitterbarPositionStat_HL()>97 && thePrefs.GetSplitterbarPositionStat_HR()<1) return;
...
{	
	if(thePrefs.GetSplitterbarPositionStat_HL()<1 && thePrefs.GetSplitterbarPositionStat_HR()<1) return;
