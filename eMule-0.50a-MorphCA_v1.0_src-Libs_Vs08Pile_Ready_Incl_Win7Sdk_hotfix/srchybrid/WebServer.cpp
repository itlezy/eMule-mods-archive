...
CString strVersionCheck;
	...
	*/
	...
	Out.Replace(_T("[Connect]"), _GetPlainResString(IDS_WEB_CONNECT));
	...
	else if (theApp.serverconnect->IsConnected() && !disconnectissued)
	{
		...
		if (cur_server) {
			...
			if (IsSessionAdmin(Data,sSession)) HTTPConText+=_T(" (<a href=\"?ses=") + sSession + _T("&w=server&c=disconnect\">")+_GetPlainResString(IDS_WEB_DISCONNECT)+_T("</a>)");
	...
	if (Kademlia::CKademlia::IsConnected()) {
		if (Kademlia::CKademlia::IsFirewalled()) {
			...
			HTTPConText.AppendFormat(_T(", <a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>)"), sSession , GetResString(IDS_WEB_DISCONNECT) );
		} else {
		...
		if (Kademlia::CKademlia::IsRunning()) {
			...
			HTTPConText.AppendFormat(_T(" (<a href=\"?ses=%s&w=kad&c=connect\">%s</a>)"),  sSession , GetResString(IDS_WEB_CONNECT) );
		}
	}
...
CString CWebServer::_GetAddServerBox(ThreadData Data)
{
	...
	Out.Replace(_T("[ConnectNow]"), _GetPlainResString(IDS_WEB_CONNECT));
	...
	Out.Replace(_T("[Disconnect]"), _GetPlainResString(IDS_WEB_DISCONNECT));
	...
	Out.Replace(_T("[Execute]"), _GetPlainResString(IDS_WEB_PERFORM));
...
CString CWebServer::_GetKadDlg(ThreadData Data)
{
	...
	if (Kademlia::CKademlia::IsConnected()) {
		if (Kademlia::CKademlia::IsFirewalled()) {
			...
			buffer.AppendFormat(_T("<br /><a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>"), sSession , GetResString(IDS_WEB_DISCONNECT) );
		} else {

			Out.Replace(_T("[KADSTATUS]"), GetResString(IDS_CONNECTED));
			buffer.Format(_T("<a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>"),  sSession , GetResString(IDS_WEB_DISCONNECT) );
		}
	}
	else {
		if (Kademlia::CKademlia::IsRunning()) {
			...
			buffer.Format(_T("<a href=\"?ses=%s&w=kad&c=connect\">%s</a>"),  sSession , GetResString(IDS_WEB_CONNECT) );
		}
	}
