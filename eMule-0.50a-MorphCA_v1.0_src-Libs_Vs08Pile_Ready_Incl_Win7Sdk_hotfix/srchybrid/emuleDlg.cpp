...
// ==> Show runtime on title +
	...
	if (IsWindowVisible() && thePrefs.ShowRatesOnTitle() && thePrefs.ShowRuntimeOnTitle())
	{
		TCHAR szBuff[128];
           ...
           _sntprintf(szBuff,_countof(szBuff),_T("(U:%.1f D:%.1f) eMule v%s [%s]  %s  (%s)"),(float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong,theApp.m_strModLongVersion, thePrefs.GetUserNick(), CastSecondsToLngHM(sessionRunTime));
		szBuff[_countof(szBuff) - 1] = _T('\0');
	...
	else if (IsWindowVisible() && thePrefs.ShowRuntimeOnTitle())
	{
		TCHAR szBuff[128];
           ...
           _sntprintf(szBuff,_countof(szBuff),_T("eMule v%s [%s]  %s  (%s)"), theApp.m_strCurVersionLong, theApp.m_strModLongVersion, thePrefs.GetUserNick(), CastSecondsToLngHM(sessionRunTime)); 
		   szBuff[_countof(szBuff) - 1] = _T('\0');
