...
void CUpDownClient::Init()
{
	...
	m_bMorphCA = false; //>>> WiZaRd::Same ModVersion Detection
	m_uiRealVersion = 0;
...
//>>> WiZaRd::Same ModVersion Detection
				m_bMorphCA = (_tcsncmp(m_strModVersion, m_strModTag, m_strModTag.GetLength())==0);
//<<< WiZaRd::Same ModVersion Detection
#else
				CheckForGPLEvilDoer();
#endif
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	bool bSendModVersion = (m_strModVersion.GetLength() || m_pszUsername==NULL) && !IsBadGuy();
	if (bSendModVersion)
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile* data)
{
	...
	*/
	...
	else if(GetClientSoft() == SO_SHAREAZA && credits->GetCurrentIdentState(GetIP()) != IS_IDENTIFIED)
	{
		for (int i = 0; i < 8; i++)
    { 
...
*/
    if(GetUploadState() != US_NONE && thePrefs.ShowBlockRatio())
	{
		CEMSocket* s = socket;
		if (s != NULL)
		{
			if (m_pPCUpSocket)
				s = m_pPCUpSocket;
			if (s->GetBlockRatio() >= 1.0f)
				strState.AppendFormat(L", BR: %0.0f%%", s->GetBlockRatio());
			if (s->GetBlockRatioOverall() >= 1.0f)
				strState.AppendFormat(L", OBR: %0.0f%%", s->GetBlockRatioOverall());
		}
	}
...
{
	CString fileList;
    fileList += GetResString(IDS_CLIENT)+_T(":  ")+(GetUserName(false)); //no funnynick
    fileList += _T("\n\n");
	...
	if ( reqfile  && reqfile->IsPartFile())
	{
		...
		{
			fileList += _T("\n") ; 
			fileList += m_OtherRequests_list.GetNext(pos)->GetFileName(); 
		}
		for(POSITION pos = m_OtherNoNeeded_list.GetHeadPosition();pos!=0;)// X: [CI] - [Code Improvement]
		{
			fileList += _T("\n") ;
			fileList += m_OtherNoNeeded_list.GetNext(pos)->GetFileName();
		}
	}
	else
	...
	*/
	...
	if(uploadfile)
		...
		fileList += GetResString(IDS_LISTREQNOUL);
          AfxMessageBox(fileList, MB_ICONINFORMATION | MB_OK); //morph4u added info icon
...
//>>> WiZaRd::ClientAnalyzer
//>>> WiZaRd::ClientAnalyzer
bool CUpDownClient::IsMorphCA() const
{
	//for jerryBG: don't show MorphCA icon for modthieves :)
