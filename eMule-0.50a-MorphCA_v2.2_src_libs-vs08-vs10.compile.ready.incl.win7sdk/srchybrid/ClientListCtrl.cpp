...
void CClientListCtrl::Init()
{
	...
	if (thePrefs.IsCommunityEnabled ())//ShowColumn (8); //Removed by SiRoB, some people may prefere disable it
		InsertColumn(10,GetResString(IDS_COMMUNITY),LVCFMT_LEFT,100);
	else
		InsertColumn(10,GetResString(IDS_COMMUNITY),LVCFMT_LEFT,100,-1,true);
	// [end] Mighty Knife
#endif
	// EastShare - Added by Pretender, Friend Tab
	InsertColumn(8,GetResString(IDS_FRIENDLIST),LVCFMT_LEFT,75);
	// EastShare - Added by Pretender, Friend Tab
	// Commander - Added: IP2Country column - Start
	if (thePrefs.GetIP2CountryNameMode() == IP2CountryName_DISABLE)
		InsertColumn(9,GetResString(IDS_COUNTRY),LVCFMT_LEFT,100,-1,true);
	else
		InsertColumn(9,GetResString(IDS_COUNTRY),LVCFMT_LEFT,100);
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				switch (iColumn)
				{
					case 0:{
						...
						if(theApp.ip2country->ShowCountryFlag() && IsColumnHidden(9)){
							cur_rec.left+=20;
...
void CClientListCtrl::GetItemDisplayText(const CUpDownClient *client, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 5:
			...
			*/
			_sntprintf(pszText, cchTextMax, _T("%s%s"), client->DbgGetFullClientSoftVer(), client->GetClientModTag());
			//MORPH END
			if (pszText[0] == _T('\0'))
