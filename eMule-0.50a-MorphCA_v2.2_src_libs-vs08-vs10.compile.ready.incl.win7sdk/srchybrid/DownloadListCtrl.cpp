...
void CDownloadListCtrl::Init()
{
	...
	*/
	...
	InsertColumn(18, GetResString(IDS_AVGQR) ,LVCFMT_LEFT, 80); // Average Queue Ranking [Xman] - Stulle
...
#ifdef CLIENTANALYZER
                        else if (morphca != -1 && pClient->IsMorphCA())
			m_ImageList.Draw(dc, morphca, point2, ILD_NORMAL | INDEXTOOVERLAYMASK(uOvlImg));
			else if (badguy != -1 && pClient->IsBadGuy())
			m_ImageList.Draw(dc, badguy, point2, ILD_NORMAL | INDEXTOOVERLAYMASK(uOvlImg));
#endif
			...
			if (pCtrlItem->type == AVAILABLE_SOURCE && pClient->GetDownloadState() == DS_DOWNLOADING) {
				...
				if(pCtrlItem->dwUpdatedchunk + DLC_BARUPDATE < dwTicks || cx !=  iWidth  || !pCtrlItem->dwUpdatedchunk) { 
					...
					*/
					...
					buffer.AppendFormat(_T(" / %.1f%%"), pClient->GetDownChunkProgressPercent());//changed
...
void CDownloadListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	*/
	...
	if (content->type == FILE_TYPE)
	{
		...
		if (((CPartFile*)content->value)->GetTransferringSrcCount() > 0)
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			*/
		    ...
		    m_FileMenu.EnableMenuItem((UINT_PTR)m_PreviewMenu.m_hMenu, (iSelectedItems == 1) ? MF_ENABLED : MF_GRAYED); 
			...
			m_FileMenu.EnableMenuItem(MP_VIEWFILECOMMENTS, (iSelectedItems >= 1 /*&& iFilesNotDone == 1*/) ? MF_ENABLED : MF_GRAYED);
				...
				m_FileMenu.ModifyMenuAndIcon(MP_SR13_ImportParts, MF_STRING, MP_SR13_ImportParts,(iFilesToImport > 0) ? GetResString(IDS_IMPORTPARTS_STOP) :GetResString(IDS_IMPORTPARTS), _T("")); 
			...
			*/
			if (client1->GetClientSoft() == client2->GetClientSoft())
			{
				if(client2->GetVersion() == client1->GetVersion() && (client1->GetClientSoft() == SO_EMULE || client1->GetClientSoft() == SO_AMULE))
					return CompareOptLocaleStringNoCase(client2->DbgGetFullClientSoftVer(), client1->DbgGetFullClientSoftVer());
				else
					return client1->GetVersion() - client2->GetVersion();
			}
			else
...
void CDownloadListCtrl::CreateMenues()
{
	...
	if (thePrefs.IsExtControlsEnabled())
	{
		...
		m_PreviewMenu.AppendMenu(MF_STRING, MP_PREVIEW, GetResString(IDS_DL_PREVIEW));
		...
		m_FileMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_PreviewMenu.m_hMenu, GetResString(IDS_DL_PREVIEW));
	}
	else
	...
	m_FileMenu.AppendMenu(MF_STRING, MP_VIEWFILECOMMENTS, GetResString(IDS_CMT_SHOWALL));
		//MORPH START - Added by SiRoB, Import Parts [SR13]
		m_FileMenu.AppendMenu(MF_STRING,MP_SR13_ImportParts, GetResString(IDS_IMPORTPARTS));
 		//m_FileMenu.AppendMenu(MF_STRING,MP_SR13_InitiateRehash, GetResString(IDS_INITIATEREHASH));
		//MORPH END   - Added by SiRoB, Import Parts [SR13]
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	...
	if (thePrefs.IsExtControlsEnabled()) {
		...
		m_SourcesMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_A4AFMenuFlag.m_hMenu, GetResString(IDS_A4AF_FLAGS));
		//MORPH END   - Added by SiRoB, Advanced A4AF Flag derivated from Khaos
