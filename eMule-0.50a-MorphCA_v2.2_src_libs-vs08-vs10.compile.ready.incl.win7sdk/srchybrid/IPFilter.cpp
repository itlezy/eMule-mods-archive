	...
	*/
	...
	if(thePrefs.IsIPFilterViaDynDNS())
	{
		...
		if(theApp.emuledlg->preferenceswnd &&
			theApp.emuledlg->preferenceswnd->m_wndSecurity &&
			theApp.emuledlg->preferenceswnd->m_wndSecurity.GetDlgItem(IDC_IPFILTER_VERSION) != NULL)
		{
			CString strBuffer = NULL;
			strBuffer.Format(_T("v%u"), thePrefs.GetIPFilterVersionNum());
			theApp.emuledlg->preferenceswnd->m_wndSecurity.GetDlgItem(IDC_IPFILTER_VERSION)->SetWindowText(strBuffer);
		}
	}
	else
		...
		theApp.emuledlg->preferenceswnd->m_wndSecurity.GetDlgItem(IDC_IPFILTER_VERSION) != NULL)
	{
		...
		theApp.emuledlg->preferenceswnd->m_wndSecurity.GetDlgItem(IDC_IPFILTER_VERSION)->SetWindowText(sTime);
	}
	//MORPH END   - Added by Stulle, New IP Filter by Ozzy [Stulle/Ozzy]
	thePrefs.Save();
