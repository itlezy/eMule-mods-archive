...
CPartFile::~CPartFile()
{
	...
	ASSERT(m_FlushSetting == NULL); // flush was reported done but thread not properly ended?
		...
		if (m_hpartfile.m_hFile != INVALID_HANDLE_VALUE)
			FlushBuffer(true); //FlushBuffer(true, false, true); //???
		}
		catch(CFileException* e){
			e->Delete();
		} 
...
EPartFileLoadResult CPartFile::LoadPartFile(LPCTSTR in_directory,LPCTSTR in_filename, EPartFileFormat* pOutCheckFileFormat)
{
	...
	m_uTransferred = 0;
	m_uTransferredSession = 0; // X-Ray :: SessionDownload
			    ...
			    switch (newtag->GetNameID()){
				   ...
				   // statistics
					...
					case FT_ATLASTREQ:{
						ASSERT( newtag->IsInt() );
						if (newtag->IsInt())
							statistic.SetLastRequest(newtag->GetInt());
						delete newtag;
						break;
					}
...
bool CPartFile::SavePartFile(bool bDontOverrideBak)
{
	...
	try{
		...
		// X-Ray :: SaveLastRequest :: Start
		if (statistic.GetLastRequest().GetTime()){
			CTag tagLastReq(FT_ATLASTREQ, (uint32)statistic.GetLastRequest().GetTime());
			tagLastReq.WriteTagToFile(&file);
			uTagCount++;
		}
		// X-Ray :: SaveLastRequest :: End
...
//				m_nCompleteSourcesCount;
	...
	*/
	...
	UpdatePowerShareLimit(m_nVirtualCompleteSourcesCount<=999, iCompleteSourcesCountInfoReceived>1 && m_anStates[DS_TOOMANYCONNS]==0 && m_anStates[DS_CONNECTING]==0 && m_nVirtualCompleteSourcesCount==1,bPowerShareLimit);//morph4u max PS for partfile limit 5 --> 999
	// <== Limit PS by amount of data uploaded - Stulle
	//MORPH END   - Added by SiRoB, Avoid misusing of powersharing
...
#endif
				continue;
			//morph4u todo - why is this comment out ? 
                        /*
