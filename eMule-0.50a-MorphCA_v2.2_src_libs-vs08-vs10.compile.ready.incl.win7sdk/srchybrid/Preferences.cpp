...
bool CPreferences::m_bAutoCompress;
...
bool	CPreferences::m_bShowRuntimeOnTitle; //show string in title
...
int     CPreferences::DisconnectTime; 
//morph4u server-auto-disconnect -
// NEO: QS - [QuickStart]
...
// NEO: QS END
bool	CPreferences::m_bShowMinQR;//morph4u show minqr
bool    CPreferences::m_bEmuLog;
bool    CPreferences::m_ShowCpu; // ==> CPU/MEM usage [$ick$/Stulle] - Max 
...
bool	CPreferences::m_bDropBlockers;
...
void CPreferences::SavePreferences()
{
	...
	ini.WriteBool(L"ShowRuntimeOnTitle", m_bShowRuntimeOnTitle,_T("eMule"));
...
//morph4u server-auto-disconnect +
	...
	ini.WriteInt(L"AutoDisconnectTime", DisconnectTime); 
//morph4u server-auto-disconnect -
    // NEO: QS - [QuickStart]
	...
	// NEO: QS END
	ini.WriteBool(L"ShowMinQR", m_bShowMinQR);//morph4u show minqr
	ini.WriteBool(L"EmuLog", m_bEmuLog);
    ini.WriteBool(L"ShowCPU", m_ShowCpu); // ==> CPU/MEM usage [$ick$/Stulle] - Max 
    ...
    // X-Ray :: SlotControl :: Start
	ini.WriteBool(L"DropBlockers", m_bDropBlockers);
	// X-Ray :: SlotControl :: End
	//MORPH START - Added, Downloaded History [Monki/Xman]
	...
	ini.WriteBool(L"DirWatcher", m_bDirectoryWatcher);	// Tux: Feature: Automatic shared files updater
	ini.WriteBool(L"VariableCompression", m_bAutoCompress);
}
...
void CPreferences::ResetStatsColor(int index)
{
	switch(index)
	{
		...
		case 13: m_adwStatsColors[13]=RGB(  0,255,  0);break;
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	if(m_bFirstStart)
	{
		m_uiCreditSystem = 2;  //>>> WiZaRd::ClientAnalyzer
	}
	else
	{
 ...
 //Commander - Added: Invisible Mode [TPT] - Start
	...
	//EastShare - added by AndCycle, IP to Country
	m_bShowRuntimeOnTitle = ini.GetBool(L"ShowRuntimeOnTitle",false); //show string in title
	...
	//MORPH START - Added by SiRoB, Upload Splitting Class
	globaldataratefriend=ini.GetInt(L"GlobalDataRateFriend",0);
	maxglobaldataratefriend=ini.GetInt(L"MaxGlobalDataRateFriend",95);
	globaldataratepowershare=ini.GetInt(L"GlobalDataRatePowerShare",0);
	maxglobaldataratepowershare=ini.GetInt(L"MaxGlobalDataRatePowerShare",95); 
	...
	maxclientdatarate=ini.GetInt(L"MaxClientDataRate",0); //0= slotfocus
	//MORPH END   - Added by SiRoB, Upload Splitting Class
...
//morph4u server-auto-disconnect +
	DisconnectTime = (uint8)ini.GetInt(L" AutoDisconnectTime", 20); 
	...
	// NEO: QS END
	m_bShowMinQR = ini.GetBool(L"ShowMinQR", true);//morph4u show minqr
	m_bEmuLog = ini.GetBool(L"EmuLog",false);
    m_ShowCpu = ini.GetBool(L"ShowCPU",false); // ==> CPU/MEM usage [$ick$/Stulle] - Max 
	...
	m_bShowFileStatusIcons = ini.GetBool(L"ShowFileStatusIcon", true); // X-Ray :: FileStatusIcons
	m_showblockratio=ini.GetBool(L"ShowSocketBlockRatio", false);
    // X-Ray :: SlotControl :: Start
	m_bDropBlockers = ini.GetBool(L"DropBlockers", true);
    // X-Ray :: SlotControl :: End
	...
	m_bMMOpen = ini.GetBool(L"MiniMuleOpen",false);
	m_bDirectoryWatcher = ini.GetBool(L"DirWatcher", false);	// Tux: Feature: Automatic shared files updater
	...
	//MORPH END   - Added, Downloaded History [Monki/Xman]
   	...
   	m_bAutoCompress=ini.GetBool(L"VariableCompression",true);
	LoadCats();
	SetLanguage();
}
