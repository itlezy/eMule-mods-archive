...
void CSharedFilesCtrl::Init()
{
	...
	InsertColumn(18,GetResString(IDS_LASTREQUEST),		LVCFMT_LEFT,  220); // X-Ray :: SaveLastRequest
	//MORPH START - Added by SiRoB, ZZ Upload System
	InsertColumn(19,GetResString(IDS_POWERSHARE_COLUMN_LABEL),LVCFMT_LEFT,70);
	//MORPH END - Added by SiRoB, ZZ Upload System
	//MORPH START - Added by IceCream, SLUGFILLER: showComments
	//MORPH START - Added by IceCream, SLUGFILLER: Spreadbars
	InsertColumn(20,GetResString(IDS_SF_UPLOADED_PARTS),LVCFMT_LEFT,170); // SF
	InsertColumn(21,GetResString(IDS_SF_TURN_PART),LVCFMT_LEFT,100); // SF
	InsertColumn(22,GetResString(IDS_SF_TURN_SIMPLE),LVCFMT_LEFT,100); // VQB
	InsertColumn(23,GetResString(IDS_SF_FULLUPLOAD),LVCFMT_LEFT,100); // SF
	//MORPH END   - Added & Modified by IceCream, SLUGFILLER: Spreadbars
	//MORPH START - Added by SiRoB, HIDEOS
	InsertColumn(24,GetResString(IDS_HIDEOS),LVCFMT_LEFT,100);
	//MORPH END   - Added by SiRoB, HIDEOS
	//MORPH START - Added by SiRoB, SHARE_ONLY_THE_NEED
	InsertColumn(25,GetResString(IDS_SHAREONLYTHENEED),LVCFMT_LEFT,100);
	//MORPH END   - Added by SiRoB, SHARE_ONLY_THE_NEED
	// Mighty Knife: CRC32-Tag
	InsertColumn(26,GetResString(IDS_CRC_CALCULATED),LVCFMT_LEFT,120);
	InsertColumn(27,GetResString(IDS_CRC_CHECKOK),LVCFMT_LEFT,100);
	// [end] Mighty Knife
...
// X-Ray :: SaveLastRequest :: Start
	strRes = GetResString(IDS_LASTREQUEST);
	hdi.pszText = const_cast<LPTSTR>((LPCTSTR)strRes);
	pHeaderCtrl->SetItem(18, &hdi);
	// X-Ray :: SaveLastRequest :: End
...
#ifdef EVENLINE	
	InitItemMemDC(dc, lpDrawItemStruct->rcItem, pKnownFile && pKnownFile->IsPartFile()? RGB(255,250,200)/*Partfile*/: pKnownFile && pKnownFile->GetPowerShared() ? RGB(255, 210, 210)/*Power*/:((lpDrawItemStruct->itemID % 2)?m_crEvenLine:m_crWindow), lpDrawItemStruct->itemState);
#else
	//colors
		...
		if (!IsColumnHidden(iColumn))
		{
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				GetItemDisplayText(file, iColumn, szItem, _countof(szItem));
					...
					//	dc->SetTextColor((COLORREF)RGB(0,0,192));//blue
...
// X-Ray :: SaveLastRequest :: Start
			case 18:
			if(pKnownFile->statistic.GetLastRequest() == NULL)
					_tcsncpy(pszText, GetResString(IDS_NEVER), cchTextMax);
			else
					_tcsncpy(pszText, pKnownFile->statistic.GetLastRequest().Format(thePrefs.GetDateTimeFormat()), cchTextMax);
			break;
		// X-Ray :: SaveLastRequest :: End
...
void CSharedFilesCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	int iPsAmountLimit = -1; // Limit PS by amount of data uploaded - Stulle
	...
	UINT uPsAmountLimitMenuItem = 0; // Limit PS by amount of data uploaded - Stulle
...
// X-Ray :: SaveLastRequest :: Start
		case 18:
			if(kitem1->statistic.GetLastRequest() > kitem2->statistic.GetLastRequest())
				iResult = 1;
			else if(kitem1->statistic.GetLastRequest() < kitem2->statistic.GetLastRequest())
				iResult = -1;
			else
				iResult = 0;
			break;
...
void CSharedFilesCtrl::CreateMenues()
{
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
		...
		//MORPH	Start	- Added by AndCycle, SLUGFILLER: Spreadbars - per file
		m_SharedFilesMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_SpreadbarMenu.m_hMenu, GetResString(IDS_SPREADBAR));
		//MORPH	End	- Added by AndCycle, SLUGFILLER: Spreadbars - per file
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_COPYFEEDBACK, GetResString(IDS_COPYFEEDBACK));
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_COPYFEEDBACK_US, GetResString(IDS_COPYFEEDBACK_US));
