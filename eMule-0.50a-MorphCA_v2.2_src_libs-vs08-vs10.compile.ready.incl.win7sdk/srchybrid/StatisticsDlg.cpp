...
#ifdef CLIP_STAT
#include "Sockets.h" // X-Ray :: ClipStat
#endif
...
#endif
	iml.Add(CTempIconLoader(_T("FILEPOWERSHARE"))); // 19 //>> pP: PowerShare
...
void CStatisticsDlg::RepaintMeters() 
{
	...
	*/
	...
	/*
	m_DownloadOMeter.SetLegendLabel(GetResString(IDS_ST_CURRENT),0);			// Download current
	*/
...
void CStatisticsDlg::SetCurrentRate(float uploadrate, float downloadrate)
{
	...
	// current rate to powershare
	m_dPlotDataUp[2] = uploadrate - (float)theApp.uploadqueue->GetDatarateExcludingPowershare() / 1024 + m_dPlotDataUp[3];
...
void CStatisticsDlg::CreateMyTree() 
{
	...
	//>> pP: PowerShare
	h_PowershareStats = stattree.InsertItem(GetResString(IDS_POWERSHARESTAT), 19, 19);
	...
	stattree.SetItemState(h_PowershareStats, TVIS_BOLD, TVIS_BOLD); //>> pP: PowerShare
...
CString CStatisticsDlg::CreateModFeedback() const
{
	...
	// System Infos
	fstats.AppendFormat(L"[b]Basic System Infos:[/b]\r\n");
	fstats.Append(L"[b]------------------------[/b]\r\n");
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_PW_NICK), thePrefs.GetUserNick());
  	fstats.AppendFormat(L"|--> Modversion: %s\r\n", _T("eMule v") + theApp.m_strCurVersionLong);
        fstats.AppendFormat(L"|--> CPU Infos: %s\r\n",theApp.sysinfo->GetCPUNameString());
	fstats.AppendFormat(L"|--> CPU (Usage): %s (%d%%)\r\n", GetHertz(theApp.sysinfo->GetCPUSpeed()), theApp.sysinfo->GetCpuUsage());
	fstats.AppendFormat(L"|--> RAM (Usage): %s (%s)\r\n", CastItoXBytes(theApp.sysinfo->GetTotalPhys()), CastItoXBytes((UINT)theApp.sysinfo->GetProcessMemoryUsage(), true));
	...
	fstats.AppendFormat(L"[b]Emule Settings:[/b]\r\n"); //Title
	fstats.AppendFormat(L"[b]------------------------[/b]\r\n");
	...
	if (thePrefs.IsUseGlobalHL()){
		fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_CLIPSTAT_GHL), GetResString(IDS_ENABLED));
		fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_CLIPSTAT_GHLSOURCES), thePrefs.GetGlobalHL());
	} else {
		fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_CLIPSTAT_GHL), GetResString(IDS_DISABLED));
	...
	fstats.AppendFormat(L"|--> %s: TCP: %u UDP: %u\r\n", GetResString(IDS_CLIPSTAT_PORTS), thePrefs.GetPort(), thePrefs.GetUDPPort());
...
#ifdef CLIENTANALYZER    
        fstats.Append(L"\r\n");
        fstats.AppendFormat(L"[b]ClientAnalyzer[/b]\r\n"); //Title
        fstats.AppendFormat(L"[b]------------------------[/b]\r\n");
