...
void CTransferWnd::ShowQueueCount() // X: [SFH] - [Show IP Filter Hits]
{
	...
	*/
	if(thePrefs.IsInfiniteQueueEnabled()){
			...
			TCHAR symbol[4] = "IFQ";
		#endif
		_sntprintf(buffer, _countof(buffer), _T("%u / %s (%s: %i, ") + GetResString(IDS_STATS_FILTEREDCLIENTS) + _T(")"), theApp.uploadqueue->GetWaitingUserCount(), symbol, GetResString(IDS_BANNED), theApp.clientlist->GetBannedCount(), theStats.filteredclients); // \x221E -> InfiniteSign // X: [SFH] - [Show IP Filter Hits]
	}
	else {
...
void CTransferWnd::CreateCategoryMenus()
{
	...
	*/
	...
	m_mnuCategory.AppendMenu(MF_SEPARATOR);
	m_mnuCategory.AppendMenu(MF_STRING,MP_HM_OPENINC, GetResString(IDS_OPENINC));
	m_mnuCategory.AppendMenu(MF_SEPARATOR);
...
void CTransferWnd::ShowRessources()
{
	...
	{
    buffer.Format(_T(""),theApp.sysinfo->GetCpuUsage());
	SetDlgItemText(IDC_CPU, _T(""));
	this->GetDlgItem(IDC_CPUCOUNT)->SetWindowText(buffer);

    buffer.Format(_T(""),CastItoXBytes(theApp.sysinfo->GetMemoryUsage()));
