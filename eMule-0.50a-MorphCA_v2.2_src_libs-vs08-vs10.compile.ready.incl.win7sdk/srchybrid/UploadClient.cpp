...
void CUpDownClient::DrawUpStatusBarChunkText(CDC* dc, RECT* cur_rec) const //Fafner: part number - 080317
{
	...
	Sbuffer.AppendFormat(_T(" / %.1f%%"),GetUpChunkProgressPercent()); //changed
...
void CUpDownClient::CreatePackedPackets(byte* data, uint32 togo, Requested_Block_Struct* currentblock, bool bFromPF){
	...
	*/
	int	compressLevel = thePrefs.m_iCompressLevel;// MORPH setable compresslevel [leuk_he];
	if (thePrefs.m_bAutoCompress && thePrefs.GetMaxUpload() > 500.0f)
		compressLevel = 1;
	else if (thePrefs.m_bAutoCompress && thePrefs.GetMaxUpload() > 200.0f)
		compressLevel = 3;
	else if (thePrefs.m_bAutoCompress && thePrefs.GetMaxUpload() > 80.0f)
		compressLevel = 5;
	else if (thePrefs.m_bAutoCompress && thePrefs.GetMaxUpload() > 50.0f)
		compressLevel = 7;
...
{
//Don't Ban Friends / Community
#ifdef COMMUNITY	
        if(IsFriend() || IsCommunity())
#else
        if(IsFriend())
#endif
		return;
