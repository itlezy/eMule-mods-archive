...
//XMessageBox -
    ...
    if ((uFlags & LOG_MORPH) && !thePrefs.GetVerbose())
		return;
	...
	if (iLen >= 0)
	{
		if (!(uFlags & LOG_DEBUG) && !(uFlags & LOG_MORPH)) //MORPH  - Added by SiRoB, Morph Log
		{
		...
		//MORPH START - Added by SiRoB, Morph Log
		else if (thePrefs.GetVerbose() && (uFlags & LOG_MORPH))
		{
			serverwnd->morphlog->AddTyped(temp, iLen, uFlags);
			if (IsWindow(serverwnd->StatusSelector) && serverwnd->StatusSelector.GetCurSel() != CServerWnd::PaneMorphLog)
				serverwnd->StatusSelector.HighlightItem(CServerWnd::PaneMorphLog, TRUE);
			/*
			if (thePrefs.GetMorph2Disk())
				themorphLog.Log(temp, iLen);
			*/
		}
                else
		//MORPH END   - Added by SiRoB, Morph Log
...
void CemuleDlg::OnClose()
{
	...
	theApp.EndDirectoryWatcher();	// Tux: Automatic shared files updater
	theApp.m_app_state = APP_STATE_SHUTTINGDOWN;
...
void CemuleDlg::ShowToolPopup(bool toolsonly, bool speedgraph) // X: [SGW] - [SpeedGraphWnd]
{
    ...
    if(!speedgraph){
    //open log +
	menu.AppendMenu(MF_STRING,MP_HM_OPENLOG, GetResString(IDS_OPENLOG) + _T("..."));
	//open log -
...
void CemuleDlg::SaveSettings (bool _shutdown) {
	/* morph4u - removed cause new ipfilter update in ppgsecurity
	if (_shutdown) {
		theApp.emuledlg->preferenceswnd->m_wndSecurity.DeleteDDB();
	}
    */
