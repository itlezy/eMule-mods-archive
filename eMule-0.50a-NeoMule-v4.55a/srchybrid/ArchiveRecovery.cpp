									   ...
									   bool preview, bool bCreatePartFileCopy)
{
	...
	try
	{
		...
		if (bCreatePartFileCopy)
		{
			...
			tempFileName = partFile->GetTempPath() + partFile->GetFileName(true).Mid(0, 5) + _T("-rec.tmp"); // NEO: PP - [PasswordProtection] <-- Xanatos --
		...
		CString outputFileName = partFile->GetTempPath()  + partFile->GetFileName(true).Mid(0, 5) + _T("-rec") + ext; // NEO: PP - [PasswordProtection] <-- Xanatos --
		CFile output;
