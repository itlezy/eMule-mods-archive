...
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
				if (pSocket->m_SocketData.hSocket == INVALID_SOCKET && !pSocket->HaveNatLayer()) {
#else
...
#ifndef NOLAYERS
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
	// Note: we can not yet remove the Nat Layer, becouse we are waiting on the FIN_ACK and would like to process it correctly.
	//			RemoveAllLayers will be called by CEMSocket::OnClose when the FIN_ACK have been recived, 
	//			if this shuld fail the simed delete from CListenSocket will delete the socket and ~CEMSocket will call RemoveAllLayers.
	if(HaveNatLayer() == false)
#endif //NATTUNNELING // NEO: UTCP END <-- Xanatos --
		RemoveAllLayers();
#endif //NOLAYERS
...
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
BOOL CAsyncSocketEx::HaveNatLayer(bool bActive)
{
	if(m_pFirstLayer && m_pFirstLayer->IsNatLayer())
	{
...
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
	if(HaveNatLayer())
		return ((CNatSocket*)m_pFirstLayer)->GetSockOpt(nOptionName,lpOptionValue,lpOptionLen);
#endif //NATTUNNELING // NEO: UTCP END <-- Xanatos --
...
#ifdef NATTUNNELING // NEO: UTCP - [UserModeTCP] -- Xanatos -->
	if(HaveNatLayer())
		return ((CNatSocket*)m_pFirstLayer)->SetSockOpt(nOptionName,lpOptionValue,nOptionLen);
#endif //NATTUNNELING // NEO: UTCP END <-- Xanatos --
