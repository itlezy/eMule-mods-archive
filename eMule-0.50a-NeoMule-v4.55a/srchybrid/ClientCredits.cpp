...
#define CLIENTS2_MET_FILENAME	_T("clients2.met")
...
CClientCredits::CClientCredits(CFileDataIO* file)
{
	...
	m_pCreditsEx = new CreditStructEx;
	memset(m_pCreditsEx, 0, sizeof(CreditStructEx));
	// NEO: SQ END
	...
	for (UINT j = 0; j < tagcount; j++){
		...
		switch(newtag->GetNameID()){
			case CT_UPLOADEDLO:{
				if (newtag->IsInt())
					m_pCredits->nUploadedLo = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_UPLOADEDHI:{
				if (newtag->IsInt())
					m_pCredits->nUploadedHi = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_DOWNLOADEDLO:{
				if (newtag->IsInt())
					m_pCredits->nDownloadedLo = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_DOWNLOADEDHI:{
				if (newtag->IsInt())
					m_pCredits->nDownloadedHi = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_LASTSEEN:{
			...
			case CT_SECUREWAITTIME:{
				if (newtag->IsInt())
					m_pCreditsEx->nSecuredWaitTime = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_UNSECUREWAITTIME:{
				if (newtag->IsInt())
					m_pCreditsEx->nUnSecuredWaitTime = newtag->GetInt();
				delete newtag;
				break;
			}
			// NEO: SQ END
			...
			case CT_DELTALO:{
				if (newtag->IsInt())
					((uint32*)&m_pCreditsEx->nDelta)[0] = newtag->GetInt();
				delete newtag;
				break;
			}
			case CT_DELTAHI:{
				if (newtag->IsInt())
					((uint32*)&m_pCreditsEx->nDelta)[1] = newtag->GetInt();
				delete newtag;
				break;
			}
			// NEO: NCS END
	...
	m_dwWaitTimeIP = 0;
	m_uUsed = 0; // NEO: XCTA - [XmanExtenedCreditTableArragement]
}
// NEO: MC END <-- Xanatos --
...
CClientCredits::CClientCredits(CreditStruct* in_credits)
{
	...
	m_dwSecureWaitTime = 0;
	m_dwWaitTimeIP = 0;
	m_uUsed = 0; // NEO: XCTA - [XmanExtenedCreditTableArragement] <-- Xanatos --
}
...
CClientCredits::~CClientCredits()
{
	...
	delete m_pCredits;
	delete m_pCreditsEx; // NEO: SQ - [SaveUploadQueue] <-- Xanatos --
}

// NEO: MC - [MetCredits] -- Xanatos -->
bool CClientCredits::IsEmpty() {
	return !m_pCredits->nUploadedLo && !m_pCredits->nUploadedHi && !m_pCredits->nDownloadedLo && !m_pCredits->nDownloadedHi && !m_pCredits->nKeySize 
		&& !m_pCreditsEx->nSecuredWaitTime && !m_pCreditsEx->nUnSecuredWaitTime // NEO: SQ - [SaveUploadQueue]
		&& !m_pCreditsEx->nDelta;	// NEO: NCS - [NeoCreditSystem]
}
...
void CClientCredits::WriteToFile(CFileDataIO* file) 
{
	...
	// standard tags
	if (m_pCredits->nUploadedLo || m_pCredits->nUploadedHi) {
		CTag(CT_UPLOADEDLO, m_pCredits->nUploadedLo).WriteTagToFile(file);
		CTag(CT_UPLOADEDHI, m_pCredits->nUploadedHi).WriteTagToFile(file);
		uTagCount += 2;
	}
	if (m_pCredits->nDownloadedLo || m_pCredits->nDownloadedHi) {
		CTag(CT_DOWNLOADEDLO, m_pCredits->nDownloadedLo).WriteTagToFile(file);
		CTag(CT_DOWNLOADEDHI, m_pCredits->nDownloadedHi).WriteTagToFile(file);
		uTagCount += 2;
	}
	...
	// NEO: SQ - [SaveUploadQueue]
	if (m_pCreditsEx->nSecuredWaitTime) {
		CTag(CT_SECUREWAITTIME, m_pCreditsEx->nSecuredWaitTime).WriteTagToFile(file);
		uTagCount++;
	}
	if (m_pCreditsEx->nUnSecuredWaitTime) {
		CTag(CT_UNSECUREWAITTIME, m_pCreditsEx->nUnSecuredWaitTime).WriteTagToFile(file);
		uTagCount++;
	}
	// NEO: SQ END

	// NEO: NCS - [NeoCreditSystem]
	if (m_pCreditsEx->nDelta) {
		CTag(CT_DELTALO, ((uint32*)&m_pCreditsEx->nDelta)[0]).WriteTagToFile(file);
		CTag(CT_DELTAHI, ((uint32*)&m_pCreditsEx->nDelta)[1]).WriteTagToFile(file);
		uTagCount += 2;
	}
	// NEO: NCS END
...
sint64	CClientCredits::GetTransferDelta(bool bSession, bool bNeg) const{
	...
	return bNeg ? (-1*m_pCreditsEx->nDelta) : (m_pCreditsEx->nDelta); // NEO: MC - [MetCredits]
}
...
{
	SaveList();
	SaveList2(); // NEO: MC - [MetCredits] <-- Xanatos --
...
{
	// NEO: MC - [MetCredits] -- Xanatos -->
	bool bLoadNew = (PathFileExists(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS2_MET_FILENAME) == TRUE);
	if(bLoadNew && PathFileExists(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_MET_FILENAME)){ // new exist and ald exist to
		struct _stat fileinfo;
		uint32 tLastModified1 = 0; // normal format file last modifyed
		if (_tstat(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_MET_FILENAME, &fileinfo) == 0)
			tLastModified1 = fileinfo.st_mtime;
		else
			AddDebugLogLine(false, _T("Failed to get file date for \"%s\" - %s"), thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_MET_FILENAME, _tcserror(errno));
		uint32 tLastModified2 = 0; // new format file last modifyed
		if (_tstat(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS2_MET_FILENAME, &fileinfo) == 0)
			tLastModified2 = fileinfo.st_mtime;
		else
			AddDebugLogLine(false, _T("Failed to get file date for \"%s\" - %s"), thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS2_MET_FILENAME, _tcserror(errno));
	...
	CString strFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + (bLoadNew ? CLIENTS2_MET_FILENAME : CLIENTS_MET_FILENAME);
	// NEO: MC END <-- Xanatos --
...
// NEO: MC - [MetCredits] -- Xanatos -->
void CClientCreditsList::SaveList2()
{
	if (thePrefs.GetLogFileSaving())
		AddDebugLogLine(false, _T("Saving clients credit list file \"%s\""), CLIENTS2_MET_FILENAME);
	m_nLastSaved = ::GetTickCount();

	CString name = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS2_MET_FILENAME;
	CSafeBufferedFile file;	
	CFileException fexp;
	if (!file.Open(name, CFile::modeWrite|CFile::modeCreate|CFile::typeBinary|CFile::shareDenyWrite, &fexp)){
		CString strError(GetResString(IDS_ERR_FAILED_CREDITSAVE));
...
CClientCredits* CClientCreditsList::GetCredit(const uchar* key)
{
	...
	result->SetLastSeen();
	result->Use(); // NEO: XCTA - [XmanExtenedCreditTableArragement] <-- Xanatos --
	return result;
}
...
void CClientCreditsList::Process()
{
	...
	if (::GetTickCount() - m_nLastSaved > MIN2MS(39))
	{
		...
		while (pos){
			m_mapClients.GetNextAssoc(pos, tmpkey, cur_credit);
			if(cur_credit->IsUsed())
				continue;

			if(cur_credit->IsEmpty()
			 && (time(NULL) - cur_credit->GetLastSeen() > CREDIT_CLEANUP_TIME)) //not seen for > 3 hours
			{
	...
	if (::GetTickCount() - m_nLastSaved > MIN2MS(13)){
		//SaveList(); // we save regulary only the new one
		SaveList2(); // NEO: MC - [MetCredits] <-- Xanatos --
	}
}
	...
	if (m_pCredits->nKeySize != 0){	// this client is a SecureHash Client
		if (GetCurrentIdentState(dwForIP) == IS_IDENTIFIED){ // good boy
			...
			if (dwForIP == m_dwWaitTimeIP){
				...
				//m_dwUnSecureWaitTime = ::GetTickCount();
				m_dwUnSecureWaitTime = time(NULL); // NEO: CUT - [CleanUploadTiming] <-- Xanatos --
...
void CClientCredits::SetSecWaitStartTime(uint32 dwForIP)
{
	...
	{
		m_dwUnSecureWaitTime = time(NULL) - m_pCreditsEx->nUnSecuredWaitTime - 1;
		m_dwSecureWaitTime = time(NULL) - m_pCreditsEx->nSecuredWaitTime - 1;
	}
	else
	// NEO: SQ END <-- Xanatos --
	// NEO: CUT - [CleanUploadTiming] -- Xanatos -->
	{
		m_dwUnSecureWaitTime = time(NULL) - 1;
		m_dwSecureWaitTime = time(NULL) - 1;
	}
	// NEO: CUT END <-- Xanatos --
...
{
	if (m_dwUnSecureWaitTime) m_pCreditsEx->nUnSecuredWaitTime = time(NULL) - m_dwUnSecureWaitTime + 1;
	if (m_dwSecureWaitTime) m_pCreditsEx->nSecuredWaitTime = time(NULL) - m_dwSecureWaitTime + 1;
}

void CClientCredits::ClearQueueWaitTime() 
{
	m_pCreditsEx->nUnSecuredWaitTime = 0;
	m_pCreditsEx->nSecuredWaitTime = 0;
}
// NEO: SQ END <-- Xanatos --
