...
struct CreditStructEx{
	uint32		nSecuredWaitTime;
	uint32		nUnSecuredWaitTime;
	sint64		nDelta; // NEO: NCS - [NeoCreditSystem]
};
// NEO: SQ END <-- Xanatos --
#pragma pack()
...
public:
	CClientCredits(CFileDataIO* file); // NEO: MC - [MetCredits] <-- Xanatos --
	...
	void	AddDownloaded(uint32 bytes, uint32 dwForIP, bool bHaveFile = true); // NEO: NCS - [NeoCreditSystem] <-- Xanatos --
	//void	AddUploaded(uint32 bytes, uint32 dwForIP);
	void	AddUploaded(uint32 bytes, uint32 dwForIP, bool bHaveFile = true); // NEO: NCS - [NeoCreditSystem] <-- Xanatos --
	uint64	GetUploadedTotal() const;
	uint64	GetDownloadedTotal() const;
	float	GetScoreRatio(uint32 dwForIP) const;
	float	GetRemoteScoreRatio() const; // NEO: RSR - [RemoteScoreRatio] <-- Xanatos --
	// NEO: NCS - [NeoCreditSystem] -- Xanatos -->
	sint64	GetTransferDelta(bool bSession, bool bNeg = false) const; 
	uint32	GetUploadedSession() const;
	uint32	GetDownloadedSession() const; 
	float	GetNeoScoreRatio(bool bHaveFile, uint32 dwForIP) const;
	// NEO: NCS END <-- Xanatos --
	void	SetLastSeen()					{m_pCredits->nLastSeen = time(NULL);}
	uint32	GetLastSeen()					{return m_pCredits->nLastSeen;} // NEO: MC - [MetCredits] <-- Xanatos --
...
private:
	...
	CreditStructEx*	m_pCreditsEx; // NEO: SQ - [SaveUploadQueue] <-- Xanatos --
	uint32			m_nUploaded; // uploaded TO him during session // NEO: NCS - [NeoCreditSystem] <-- Xanatos --
	uint32			m_nDownloaded; // downloaded from him during session // NEO: NCS - [NeoCreditSystem] <-- Xanatos --
	...
	CArray<CTag*,CTag*> taglist; // NEO: MC - [MetCredits] <-- Xanatos --
	uint16			m_uUsed; // NEO: XCTA - [XmanExtenedCreditTableArragement] <-- Xanatos --
};
...
protected:
	void	LoadList();
	void	SaveList();
	void	SaveList2(); // NEO: MC - [MetCredits] <-- Xanatos --
