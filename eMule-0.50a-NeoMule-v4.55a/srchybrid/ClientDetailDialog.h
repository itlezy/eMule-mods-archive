...
class CClientDetailDialog : public CModListViewWalkerPropertySheet // NEO: MLD - [ModelesDialogs] <-- Xanatos --
{
	DECLARE_DYNAMIC(CClientDetailDialog)

public:
	//CClientDetailDialog(CUpDownClient* pClient, CListCtrlItemWalk* pListCtrl = NULL);
	CClientDetailDialog(CUpDownClient* pClient, CListCtrlItemWalk* pListCtrl = NULL, UINT uInvokePage = 0); // NEO: XSF - [ExtendedSharedFiles] <-- Xanatos --
...
protected:
	...
	//void Construct();
	void Construct(UINT uInvokePage = 0); // NEO: XSF - [ExtendedSharedFiles] <-- Xanatos --
