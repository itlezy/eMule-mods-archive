...
CClientList::CClientList(){
	...
	m_dwLastClientCleanUp = ::GetTickCount(); // NEO: NSS - [NeoSourceStorage] <-- Xanatos --
	...
	m_globDeadSourceList.Init(); // NEO: SDT - [SourcesDropTweaks] <-- Xanatos --
	m_pBuddy = NULL;
...
#ifdef NATTUNNELING // NEO: XSB - [XSBuddy] -- Xanatos -->
		if(toremove == m_pXsBuddy){
			m_pXsBuddy = NULL;
			toremove->SetXsBuddyStatus(XB_NONE);
		}
#endif //NATTUNNELING // NEO: XSB END <-- Xanatos --
		list.RemoveAt(pos);
	}
...
bool CClientList::AttachToAlreadyKnown(CUpDownClient** client, CClientReqSocket* sender, EAttachMode AttachMode){ // NEO: FIX - [BetterClientAttaching] <-- Xanatos --
	...
	if(AttachMode == ATTACH_ANSWER){
		// David: We check only for same user hast but different IP, the case of the same IP should have been checked befoure we have send the hello
	...
	if (found_client != NULL){
					...
					&& (found_client->GetIP() != tocheck->GetIP() || found_client->GetUserPort() != tocheck->GetUserPort() ) )
				{
					...
					if (found_client->Credits() && found_client->Credits()->GetCurrentIdentState(found_client->GetIP()) == IS_IDENTIFIED){
...
#endif // ARGOS // NEO: NA END <-- Xanatos --
						{
							...
							tocheck->Ban(_T("Userhash invalid")); // NEO: NA - [NeoArgos] <-- Xanatos --
							//tocheck->Ban();
						}
						return false;
					}
		...
		if(AttachMode != ATTACH_LOAD && tocheck->GetDownloadState() == DS_LOADED)
			found_client->SetDownloadState(DS_NONE);
...
#ifdef ARGOS // NEO: NA - [NeoArgos] -- Xanatos -->
		if (dwBantime + NeoPrefs.GetBanTimeMs() > ::GetTickCount())
#else
...
void CClientList::Process()
{
...
#ifdef ARGOS // NEO: NA - [NeoArgos] -- Xanatos -->
			if (dwBantime + NeoPrefs.GetBanTimeMs() < cur_tick )
#else
		...
		switch(cur_client->GetKadState())
		{
			...
			case KS_CONNECTED_BUDDY:
				...
				if( m_nBuddyStatus != Connected )
				{
					...
					if(theApp.IsFirewalled() && NeoPrefs.IsInformOnBuddyChange()){
						InformSourcesAboutIDChage();
...
#ifdef NATTUNNELING // NEO: XSB - [XSBuddy] -- Xanatos -->
	if(theApp.IsFirewalled() && GetXsBuddyStatus() == XB_NONE)
	{
		m_pXsBuddy = NULL;
		uint16 skipped = 0;
		for (POSITION pos = list.GetHeadPosition(); pos != NULL; )
		{
			...
			if(cur_client->SupportsNatTraversal())
				continue;

			if(cur_client->GetXsBuddyStatus() == XB_NONE)
			{
				m_pXsBuddy = cur_client;
				cur_client->SetXsBuddyStatus(XB_CONNECTING);
		...
		// When no valid cleints was found but some have status XB_DENIDED 
		// we reset them and try again to become a buddy of one of those cleints
		if(m_pXsBuddy == NULL && skipped)
			for (POSITION pos = list.GetHeadPosition(); pos != NULL; list.GetNext(pos)->SetXsBuddyStatus(XB_NONE));
	}
	else if(GetXsBuddyStatus() == XB_HIGH_BUDDY)
	{
		// we reuse the PingPong mecha from kad for timing,
		// but use own mod packets to keep things clean
		// Note: we use only one packet for Ping, the LowID send the ping and the HighID answers it
		if( m_pXsBuddy->SendBuddyPingPong() )
		{
			if (thePrefs.GetDebugClientTCPLevel() > 0)
				DebugSend("OP_XS_BuddyPing", m_pXsBuddy);
			Packet* buddyPing = new Packet(OP_XS_BUDDYPING, 0, OP_MODPROT);
			theStats.AddUpDataOverheadOther(buddyPing->size);
			m_pXsBuddy->SetLastBuddyPingPongTime();
			m_pXsBuddy->SafeSendPacket(buddyPing);	
		}
	}
#endif //NATTUNNELING // NEO: XSB END <-- Xanatos --
	...
	// NEO: RIC - [ReaskOnIDChange] -- Xanatos -->
	///////////////////////////////////////////////////////////////////////////
	// inform cleints about our new IP
	//
	if(m_bNotifyIdChage)
	{
		bool bNotDone = false;
	...
	if (m_dwLastClientCleanUp + CLIENTLIST_CLEANUP_TIME < cur_tick ){
		...
		for (pos1 = list.GetHeadPosition();( pos2 = pos1 ) != NULL;){
			...
			if ((pCurClient->GetUploadState() == US_NONE || pCurClient->GetUploadState() == US_BANNED && !pCurClient->IsBanned())
				...
				&& pCurClient->GetRequestFile() == NULL // NEO: FIX - [CleanUpClientList] <-- Xanatos --
...
#ifdef NATTUNNELING // NEO: XSB - [XSBuddy] -- Xanatos -->
EXsBuddy CClientList::GetXsBuddyStatus(){ 
	if(m_pXsBuddy == NULL) 
		return XB_NONE;
	if(m_pXsBuddy->GetXsBuddyStatus() == XB_DENIDED)
		return XB_NONE;
	return m_pXsBuddy->GetXsBuddyStatus(); 
}

bool CClientList::CanAcceptXsBuddy(){
	uint16 buddys = 0;
	for (POSITION pos = list.GetHeadPosition(); pos != NULL; )
	{
		if(list.GetNext(pos)->GetXsBuddyStatus() == XB_LOW_BUDDY)
			buddys ++;
	}

	// for the moment we allow up to 3 buddys
	return buddys < 3;
}
#endif //NATTUNNELING // NEO: XSB END <-- Xanatos --
...
void CClientList::InformSourcesAboutIDChage()
{
	...
	for(POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		...
		if(NeoPrefs.IsReAskOnIPChange() && cur_client->GetRequestFile() != NULL) // The source will be informed on reask, no need to create extra overhead
			continue;
