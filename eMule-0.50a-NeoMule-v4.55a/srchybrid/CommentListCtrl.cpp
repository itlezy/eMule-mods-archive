...
#include "AbstractFile.h" // NEO: XC - [ExtendedComments] <-- Xanatos --
...
/*void CCommentListCtrl::AddItem(const CUpDownClient* client)
{
	...
	CClientFileStatus* status = client->GetFileStatus((CKnownFile*)client->GetRequestFile());
	if(!status)
		return;
	int iRating = status->GetFileRating();
	SComment* pComment = new SComment(pClientCookie, iRating, status->GetFileComment(),
										status->GetFileName(), client->GetUserName(), 0/eD2K/);
...
void CCommentListCtrl::AddItem(const KnownComment* comment)
{
	const void* pClientCookie = comment;
	if (FindClientComment(pClientCookie) != -1)
		return;
	int iRating = comment->m_uRating;
	SComment* pComment = new SComment(pClientCookie, iRating, comment->m_strComment,
									  comment->m_strFileName, comment->m_strUserName, 0/*eD2K*/);
	AddComment(pComment);
}
// NEO: XC END <-- Xanatos --
