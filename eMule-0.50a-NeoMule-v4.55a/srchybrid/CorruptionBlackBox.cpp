...
void CCorruptionBlackBox::TransferredData(uint64 nStartPos, uint64 nEndPos, uint32 dwSenderIP) // NEO: VOODOO - [UniversalPartfileInterface] <-- Xanatos --
{
	...
	if(dwSenderIP == 0)
		return;
	// NEO: PIX END <-- Xanatos --
	//uint32 dwSenderIP = pSender->GetIP(); // NEO: VOODOO - [UniversalPartfileInterface] <-- Xanatos --
	...
	if (nRelEndPos >= PARTSIZE){
		...
		TransferredData(nTmpStartPos, nEndPos, dwSenderIP); // NEO: VOODOO - [UniversalPartfileInterface] <-- Xanatos --
	}
		...
		for(int k = 0; k < aGuiltyClients.GetCount(); k++){
			...
			if ( nCorruptPercentage > CBB_BANTHRESHOLD){
				...
				if(NeoPrefs.UseVoodooTransfer())
					theApp.voodoo->ManifestCorruptedSenderWarning(aGuiltyClients[k]);
#endif // VOODOO // NEO: VOODOO END <-- Xanatos --
			}
			else{
