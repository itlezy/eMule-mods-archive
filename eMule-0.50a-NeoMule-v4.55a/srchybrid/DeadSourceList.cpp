...
#define BLOCKTIME		(::GetTickCount() + (m_Owner == NULL ? NeoPrefs.PartPrefs.GetGlobalDeadTimeMs() : m_Owner->PartPrefs->GetDeadTimeMs()))
#define BLOCKTIMEFW		(::GetTickCount() + (m_Owner == NULL ? NeoPrefs.PartPrefs.GetGlobalDeadTimeMsFW() : m_Owner->PartPrefs->GetDeadTimeMsFW()))
// NEO: SDT END <-- Xanatos --
...
void CDeadSourceList::Init(CPartFile* Owner){ // NEO: SDT - [SourcesDropTweaks] <-- Xanatos --
	m_dwLastCleanUp = ::GetTickCount();
	m_Owner = Owner; // NEO: SDT - [SourcesDropTweaks] <-- Xanatos --
	//if(bGlobalList){
	if(m_Owner == NULL){ // NEO: SDT - [SourcesDropTweaks] <-- Xanatos --
...
void CDeadSourceList::AddDeadSource(const CUpDownClient* pToAdd, bool bDrop){ // NEO: SDT - [SourcesDropTweaks] <-- Xanatos --
	...
	if(pToAdd->IsLanClient()) // Don't add lan clinets to dead source list
		return;
#endif //LANCAST // NEO: NLC END <-- Xanatos --

	// NEO: SDT - [SourcesDropTweaks] -- Xanatos -->
	if(bDrop)
		m_mapDeadSources.SetAt(CDeadSource(pToAdd->GetUserIDHybrid(), pToAdd->GetUserPort(), pToAdd->GetServerIP(), 0), m_Owner->PartPrefs->GetDropTimeMs() );
	// NEO: SDT END <-- Xanatos --
	if(!pToAdd->HasLowID())
