	...
	if ((hMem = data.GetGlobalData(CF_HDROP)) != NULL)
	{
		...
		if ((lpDrop = (LPDROPFILES)GlobalLock(hMem)) != NULL)
		{
			if (lpDrop->fWide)
			{
				...
				CString		strFiles = GetResString(IDS_X_DROPED_FILES);
				CStringList listFolders;
				CString		strFolders = GetResString(IDS_X_DROPED_FOLDERS);
				...
				while (*pszFileNameW != L'\0')
				{
					...
					int iLen = wcslen(pszFileNameW);
					LPCWSTR pszExtW = GetFileExtW(pszFileNameW, iLen);
					if (pszExtW != NULL && wcsicmp(pszExtW, FILEEXTDOT_INETSHRTCUTW) == 0)
					{
						USES_CONVERSION;
						AddUrlFileContents(W2CT(pszFileNameW));
						//if (FAILED(AddUrlFileContents(W2CT(pszFileNameW))))
						//	break;
					}
					else
					{
						DWORD attrib = GetFileAttributes ( pszFileNameW );
						if ( attrib != INVALID_FILE_ATTRIBUTES )
						{
							CString sFile(pszFileNameW);
							int pos = sFile.ReverseFind(_T('\\'));
							if(pos != -1)
								sFile = sFile.Mid(pos+1);
							if(attrib & FILE_ATTRIBUTE_DIRECTORY)
							{
								if(listFolders.GetCount() < 10)
									strFolders.AppendFormat(_T("%s\n"),sFile);
								else if(listFolders.GetCount() == 10)
									strFolders.Append(_T("\n..."));
								else
									strFolders.Append(_T("."));
								listFolders.AddHead(pszFileNameW);
							}
							else
							{
								if(listFiles.GetCount() < 10)
									strFiles.AppendFormat(_T("%s\n"),sFile);
								else if(listFiles.GetCount() == 10)
									strFiles.Append(_T("\n..."));
								else
									strFiles.Append(_T("."));
								listFiles.AddHead(pszFileNameW);
							}
						}
					}
					hrPasteResult = S_OK;
				...
				bool AddedToShare = false;

				// NEO: SSD - [ShareSubDirectories]
				if(listFolders.GetCount() > 0)
				{
					switch(AfxMessageBox(strFolders,MB_YESNOCANCEL,NULL))
					{
					case IDYES:
						for (POSITION pos = listFolders.GetHeadPosition();pos != 0;){
							CString sFolder = listFolders.GetNext(pos);

							bool dontadd = false;
							for (POSITION pos = thePrefs.sharedsubdir_list.GetHeadPosition(); pos != 0; ) {
								if (CompareDirectories(thePrefs.sharedsubdir_list.GetNext(pos), sFolder))
								{
									dontadd = true;
									break;
								}
							...
							thePrefs.shareddir_list.AddTail(sFolder);
						}
					...
					case IDNO:
						for (POSITION pos = listFolders.GetHeadPosition();pos != 0;){
							...
							for (POSITION pos = thePrefs.shareddir_list.GetHeadPosition(); pos != 0; ) {
								if (CompareDirectories(thePrefs.shareddir_list.GetNext(pos), sFolder))
								{
									dontadd = true;
									break;
								}
				...
				if(listFiles.GetCount() > 0 && AfxMessageBox(strFiles,MB_YESNO,NULL) == IDYES)
				{
					for (POSITION pos = listFiles.GetHeadPosition();pos != 0;){
						CString sFile = listFiles.GetNext(pos);
	...
	else if (pDataObject->IsDataAvailable(CF_HDROP))
	{
		...
		if ((hMem = pDataObject->GetGlobalData(CF_HDROP)) != NULL)
		{
			...
			if ((lpDrop = (LPDROPFILES)GlobalLock(hMem)) != NULL)
			{
				...
				if (lpDrop->fWide)
				{
					bResult = TRUE; // NEO: SDD - [ShareDargAndDrop] <-- Xanatos --
