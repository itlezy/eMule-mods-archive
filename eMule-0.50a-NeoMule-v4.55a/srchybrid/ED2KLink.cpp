...
CED2KLink* CED2KLink::CreateLinkFromUrl(const TCHAR* uri)
{
	...
	if (strTok.CompareNoCase(_T("ed2k://")) == 0)
	{
		...
		{
			CString strFnick = strURI.Tokenize(_T("|"), iPos);
			if (!strFnick.IsEmpty())
			{
				CString strFhash = strURI.Tokenize(_T("|"), iPos);
				if (!strFhash.IsEmpty() && (strURI.Tokenize(_T("|"), iPos) == _T("/")))
				{
					if (strURI.Tokenize(_T(","), iPos) == _T("addr"))
					{
						CString strFip = strURI.Tokenize(_T(":"), iPos);
						if (!strFip.IsEmpty())
						{
							CString strFport = strURI.Tokenize(_T("|"), iPos);
							if (!strFport.IsEmpty() && strURI.Tokenize(_T("|"), iPos) == _T("/"))
							{
								/*CString s;
								s.Format(_T("ed2k://|friend|%s|%s|/|addr,%s:%s|/%s"), strFnick, strFhash, strFip, strFport, "--nada");
								AfxMessageBox(s);*/
								return new CED2KFriendLink(strFnick, strFhash, strFip, strFport);
							}
						}
					} else
						return new CED2KFriendLink(strFnick, strFhash, _T(""), _T(""));
				}
			}
		}
		// NEO: TFL END  <-- Xanatos --
...
// NEO: TFL - [TetraFriendLinks] -- Xanatos -->
////////////////////////////////////////////////////////
//Format: ed2k://|friend|nick|hash|/|addr,1.2.3.4:4662|/
////////////////////////////////////////////////////////
CED2KFriendLink* CED2KServerListLink::GetFriendLink()
{
	return 0;
}

CED2KFriendLink* CED2KServerLink::GetFriendLink()
{ 
	return 0;
}

CED2KFriendLink* CED2KFileLink::GetFriendLink()
{ 
	return 0;
}

CED2KFriendLink::CED2KFriendLink (const TCHAR* name, const TCHAR* hash, const TCHAR* ip, const TCHAR* port)
: m_name(name)
{
	if ( (_tcslen(hash) == 1) )
		m_hashash = 0;
	else
	{
		...
		for ( int idx = 0 ; idx < 16 ; ++idx) {
			m_hash[idx] = (BYTE)(FromHexDigit(*hash++)*16);
			m_hash[idx] = (BYTE)(m_hash[idx] + FromHexDigit(*hash++));
		}
		m_hashash = 1;
	}
...
CED2KFriendLink::~CED2KFriendLink()
{
}

void 
CED2KFriendLink::GetLink(CString& lnk) const	//T-ToDo, improve
{
	TCHAR buffer[32];
	lnk = _T("ed2k://|friend|");
	lnk += m_name;
...
CED2KFriendLink* CED2KFriendLink::GetFriendLink()
{
	return this;
}
