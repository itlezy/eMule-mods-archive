...
public:
	...
	typedef enum { kServerList, kServer , kFile , kNodesList , kFriend , kInvalid } LinkType; // NEO: TFL - [TetraFriendLinks] <-- Xanatos --
	...
	virtual class CED2KFriendLink* GetFriendLink() = 0;	// NEO: TFL - [TetraFriendLinks] <-- Xanatos --
};
...
public:
	...
	virtual CED2KFriendLink* GetFriendLink(); // NEO: TFL - [TetraFriendLinks] <-- Xanatos --
...
public:
	...
	virtual CED2KFriendLink* GetFriendLink() 		{ return NULL; } // NEO: TFL - [TetraFriendLinks] <-- Xanatos --
...
{
public:
	CED2KFriendLink(const TCHAR* name, const TCHAR* hash, const TCHAR* ip, const TCHAR* port);
	...
	virtual CED2KFileLink* GetFileLink();
	virtual CED2KNodesListLink* GetNodesListLink()		{ return NULL; }
	virtual CED2KFriendLink* GetFriendLink();

	virtual void GetLink(CString& lnk) const;

	const TCHAR* GetName()	const	{ return m_name;}
	const uchar* GetHash()	const	{ return m_hash;}	//T-ToDo
	const uint32 GetIP()	const	{ return m_ip;}
	const uint16 GetPort()	const	{ return m_port;}
	const uint32 GetHasHash()	const	{ return m_hashash;}

private:
	CED2KFriendLink(); // Not defined
	CED2KFriendLink(const CED2KFriendLink&); // Not defined
	CED2KFriendLink& operator=(const CED2KFriendLink&); // Not defined
