...
void CFileDetailDialogInfo::RefreshData()
{
	...
	if (m_paFiles->GetSize() == 1)
	{
		...
		if (file->GetAICHHashset()->HasValidMasterHash() && file->GetAICHHashset()->GetStatus() != AICH_EMPTY) {
			SetDlgItemText(IDC_FD_AICHHASH, file->GetAICHHashset()->GetMasterHash().GetString());

			switch (file->GetAICHHashset()->GetStatus()) {
			case AICH_ERROR:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_X_AICH_HASH_COMPLETE));
				break;
			case AICH_UNTRUSTED:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_X_AICH_HASH_UNTRUSTED));
				break;
			case AICH_TRUSTED:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_X_AICH_HASH_TRUSTED));
				break;
			case AICH_VERIFIED:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_X_AICH_HASH_VERIFYED));
				break;
			case AICH_HASHSETCOMPLETE:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_X_AICH_HASH_COMPLETE));
				break;
			default:
				SetDlgItemText(IDC_FD_AICHHASH2, GetResString(IDS_UNKNOWN));
			}
		}else{
			SetDlgItemText(IDC_FD_AICHHASH, GetResString(IDS_UNKNOWN));
			SetDlgItemText(IDC_FD_AICHHASH2, _T(""));
		}
		// NEO: SCV END <-- Xanatos --
	...
	else
	{
		...
		SetDlgItemText(IDC_FD_AICHHASH2, sm_pszNotAvail); // NEO: SCV - [SubChunkVerification] <-- Xanatos --
	}
