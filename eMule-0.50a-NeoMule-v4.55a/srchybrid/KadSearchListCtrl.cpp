...
#include "PartFile.h"
#include "kademlia/kademlia/Defines.h" // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
...
void CKadSearchListCtrl::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("KadBuddySearch")));
	iml.Add(CTempIconLoader(_T("KadStoreNote")));
	iml.Add(CTempIconLoader(_T("KadNoteSearch")));
	// NEO: KII < -- Xanatos --
...
void CKadSearchListCtrl::UpdateSearch(int iItem, const Kademlia::CSearch* search)
{
	...
	uint32 uBaseTime = 0; // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
	switch(search->GetSearchTypes()){
		case Kademlia::CSearch::FILE:
			...
			uBaseTime = SEARCHFILE_LIFETIME - SEC(20); // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
			break;
		case Kademlia::CSearch::KEYWORD:
			id = GetResString(IDS_KAD_SEARCHKW);
			SetItem(iItem,0,LVIF_IMAGE,0,1,0,0,0,0);
			uBaseTime = SEARCHKEYWORD_LIFETIME - SEC(20); // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
			break;
		...
		case Kademlia::CSearch::NODEFWCHECKUDP:
			...
			//uBaseTime = SEARCHNODE_LIFETIME; // NEO: KII - [KadInterfaceImprovement] <-- Xanatos -- // near to always we will quit erlyer
			break;
		case Kademlia::CSearch::STOREFILE:
			id = GetResString(IDS_KAD_STOREFILE);
			SetItem(iItem,0,LVIF_IMAGE,0,3,0,0,0,0);
			uBaseTime = SEARCHSTOREFILE_LIFETIME - SEC(20); // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
			break;
		case Kademlia::CSearch::STOREKEYWORD:
			id = GetResString(IDS_KAD_STOREKW);
			SetItem(iItem,0,LVIF_IMAGE,0,4,0,0,0,0);
			uBaseTime = SEARCHSTOREKEYWORD_LIFETIME - SEC(20); // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
			break;
		//JOHNTODO: -
		...
		case Kademlia::CSearch::FINDBUDDY:
			...
			// NEO: KII - [KadInterfaceImprovement] -- Xanatos -->
			SetItem(iItem,0,LVIF_IMAGE,0,5,0,0,0,0);
			uBaseTime = SEARCHFINDBUDDY_LIFETIME - SEC(20); 
			// NEO: KII END <-- Xanatos --
			break;
		case Kademlia::CSearch::STORENOTES:
			id=GetResString(IDS_STORENOTES);
			// NEO: KII - [KadInterfaceImprovement] -- Xanatos -->
			SetItem(iItem,0,LVIF_IMAGE,0,6,0,0,0,0);
			uBaseTime = SEARCHSTORENOTES_LIFETIME - SEC(20);
			// NEO: KII END <-- Xanatos --
			break;
		case Kademlia::CSearch::NOTES:
			id=GetResString(IDS_NOTES);
			// NEO: KII - [KadInterfaceImprovement] -- Xanatos -->
			SetItem(iItem,0,LVIF_IMAGE,0,7,0,0,0,0);
			uBaseTime = SEARCHNOTES_LIFETIME - SEC(20);
			// NEO: KII END <-- Xanatos --
			break;
		// NEO: KII - [KadInterfaceImprovement] -- Xanatos -->
		case Kademlia::CSearch::FINDSOURCE:
			uBaseTime = SEARCHFINDSOURCE_LIFETIME - SEC(20);
	...
	// NEO: KII - [KadInterfaceImprovement] -- Xanatos -->
	{
		id = GetResString(IDS_KADSTATUS_ACTIVE);
		if(uBaseTime)
			id.AppendFormat(_T(" %i%%"),(int)(100*search->GetDuration()/uBaseTime));
		SetItemText(iItem,colStop,id);
	}
	// NEO: KII END <-- Xanatos --
