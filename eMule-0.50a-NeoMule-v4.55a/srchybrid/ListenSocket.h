...
public:
	...
	void		SetOnLan();
	bool		IsLanSocket() { return isLanSocket; }
...
protected:
	...
	bool	ProcessModPacket(const BYTE* packet, uint32 size, UINT opcode, UINT uRawSize); // NEO: NMP - [NeoModProt] <-- Xanatos --
...
public:
	...
	bool	TooManySocketsOCC(); // NEO: OCC - [ObelixConnectionControl] <-- Xanatos --
	uint32	GetMaxConnectionReached()	{return maxconnectionreached;}
	bool    IsValidSocket(CClientReqSocket* totest);
	//void	AddConnection();
	void	AddConnection(bool created); // NEO: SCM - [SmartConnectionManagement] <-- Xanatos --
	...
	void	SetLimitForConnections(); // NEO: SCM - [SmartConnectionManagement] <-- Xanatos --
...
private:
	...
	int		m_CreateSocketsLimit;	
	int		m_AcceptSocketsLimit;	
	int		m_AllSocketsLimit;	
	// NEO: SCM END <-- Xanatos --
};
