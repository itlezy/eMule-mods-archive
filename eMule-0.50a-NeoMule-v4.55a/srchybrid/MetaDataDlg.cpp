...
void CMetaDataDlg::RefreshData()
{
	...
	{
		// NEO: MLD - [ModelesDialogs] -- Xanatos -->
		if(!theApp.downloadqueue->IsPartFile((CKnownFile*)(*m_paFiles)[0]) 
		&& !theApp.knownfiles->IsFilePtrInList((CKnownFile*)(*m_paFiles)[0])
		&& !theApp.searchlist->IsFileList((CAbstractFile*)(*m_paFiles)[0]))
			return;
		// NEO: MLD END <-- Xanatos --
