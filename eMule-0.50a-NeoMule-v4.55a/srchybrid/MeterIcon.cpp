...
CMeterIcon::CMeterIcon()
{
	...
	m_pLimits = NULL;
	m_pColors = NULL;
	m_pColors2 = NULL; // NEO: NSTI - [NewSystemTrayIcon] <-- Xanatos --
	m_nEntries = 0;
}
...
CMeterIcon::~CMeterIcon()
{
	...
	delete[] m_pLimits;
	delete[] m_pColors;
	delete[] m_pColors2; // NEO: NSTI - [NewSystemTrayIcon] <-- Xanatos --
}

// NEO: NSTI - [NewSystemTrayIcon] -- Xanatos -->
COLORREF CMeterIcon::GetMeterColor(int nBar, int nLevel) const
{
	if(nBar == 1)
		return m_pColors2[nLevel-1];
	else
		return m_pColors[nLevel-1];
}
...
{// begin DrawIconMeter
	...
	if(nLevel == 0)
		return true;
	if(nLevel == 1 && m_nSpacingWidth == 1)
	{
		SetPixel(hDestDC, m_sDimensions.cx-nPos-1, m_sDimensions.cy-1, GetMeterColor(nPos, 1));
		SetPixel(hDestDCMask, m_sDimensions.cx-nPos-1, m_sDimensions.cy-1, RGB(0,0,0));
		return true;
	}
	HPEN hPen = CreatePen(PS_SOLID,1,GetMeterColor(nPos, nLevel));
	if(hPen == NULL)
		return false;
	HGDIOBJ hOldPen = SelectObject(hDestDC,hPen);
	if(hOldPen == NULL)
		return false;
	if(!Rectangle(hDestDC,
		m_sDimensions.cx-nPos*m_nSpacingWidth-m_nSpacingWidth,
		m_sDimensions.cy-((nLevel*(m_sDimensions.cy-1)/m_nMaxVal)+1),
		m_sDimensions.cx-nPos*m_nSpacingWidth,
		m_sDimensions.cy))
		return false;
	if(!DeleteObject(SelectObject(hDestDC,hOldPen)))
		return false;
...
// NEO: NSTI - [NewSystemTrayIcon] -- Xanatos -->
bool CMeterIcon::SetColorLevels(const COLORREF* pColors, const COLORREF* pColors2, int nEntries)
{
	delete[] m_pColors;
	delete[] m_pColors2;
	m_pColors = NULL;
	...
	m_pColors2 = new COLORREF[nEntries];

	for(int i = 0;i < nEntries;i++)
	{
		m_pColors[i] = pColors[i];
		m_pColors2[i] = pColors2[i];
	}
	m_nEntries = nEntries;
	return true;
}
