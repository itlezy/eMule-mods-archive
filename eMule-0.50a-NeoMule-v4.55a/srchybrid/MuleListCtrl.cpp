...
CMuleListCtrl::CMuleListCtrl(PFNLVCOMPARE pfnCompare, DWORD dwParamSort)
{
	...
	UpdateSortHistoryMain(m_dwParamSort, 0); // NEO: SE - [SortExtension] <-- Xanatos --
	UpdateSortHistoryAlt(m_dwParamSort, 0); // NEO: SEa - [SortAltExtension] <-- Xanatos --
...
void CMuleListCtrl::SaveSettings()
{
	...
	ini.WriteInt( m_Name + _T("TableSortItem"), m_iCurrentSortItem ); // NEO: SEa - [SortAltExtension] <-- Xanatos --
	ini.WriteInt( m_Name + _T("TableSortAscending"), GetSortType( m_atSortArrow ));

	// NEO: SEa - [SortAltExtension] -- Xanatos -->
	if(m_bDual){
		CString strSortHistAlt;
		POSITION posAlt = m_liSortHistory.GetTailPosition();
		if (posAlt != NULL) {
			strSortHistAlt.Format(_T("%d"), m_liSortHistory.GetPrev(posAlt));
			while (posAlt != NULL) {
				strSortHistAlt.AppendChar(_T(','));
				strSortHistAlt.AppendFormat(_T("%d"), m_liSortHistory.GetPrev(posAlt));
			}
		}
		ini.WriteString(m_Name + _T("SortHistoryAlt"), strSortHistAlt);

		ini.WriteInt( m_Name + _T("TableSortItemAlt"), m_iCurrentSortItemAlt );
		ini.WriteInt( m_Name + _T("TableSortAscendingAlt"), GetSortType( m_atSortArrowAlt ));
	}
	// NEO: SEa END <-- Xanatos --
	...
	// delete[] piSortHist; // NEO: SE - [SortExtension] <-- Xanatos --
	delete[] piColOrders;
	delete[] piColWidths;
	delete[] piColHidden;
}
...
void CMuleListCtrl::LoadSettings()
{
	...
	while (!strTemp.IsEmpty()) {
		UpdateSortHistoryMain((int)_tstoi(strTemp), 0);	// NEO: SEa - [SortAltExtension] 
		nOffset = ini.Parse(strSortHist, nOffset, strTemp);
	}
	// NEO: SE END <-- Xanatos --
	...
	// NEO: SEa - [SortAltExtension] -- Xanatos -->
	if(m_bDual){
		CString strSortHistAlt = ini.GetString(m_Name + _T("SortHistoryAlt"));
		int nOffsetAlt = 0;
		CString strTempAlt;
		nOffsetAlt = ini.Parse(strSortHistAlt, nOffsetAlt, strTempAlt);
		while (!strTempAlt.IsEmpty()) {
			UpdateSortHistoryAlt((int)_tstoi(strTempAlt), 0);
			nOffsetAlt = ini.Parse(strSortHistAlt, nOffsetAlt, strTempAlt);
		}

		m_iCurrentSortItemAlt= ini.GetInt( m_Name + _T("TableSortItemAlt"), 0);
		m_atSortArrowAlt= GetArrowType(ini.GetInt(m_Name +_T("TableSortAscendingAlt"), arrowUp));
		if (m_liSortHistoryAlt.IsEmpty())
			m_liSortHistoryAlt.AddTail(m_iCurrentSortItemAlt);
	}
	// NEO: SEa END <-- Xanatos --
...
void CMuleListCtrl::SetSortArrow(int iColumn, ArrowType atType) {
	...
	if(iColumn != GetSortItem()) { // NEO: SEa - [SortAltExtension] <-- Xanatos --
		pHeaderCtrl->GetItem(GetSortItem(), &headerItem); // NEO: SEa - [SortAltExtension] <-- Xanatos --
		headerItem.fmt &= ~(HDF_IMAGE | HDF_BITMAP_ON_RIGHT);
		pHeaderCtrl->SetItem(GetSortItem(), &headerItem); // NEO: SEa - [SortAltExtension] <-- Xanatos --
		SetSortItem(iColumn); // NEO: SEa - [SortAltExtension] <-- Xanatos --
	...
	if(iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) {
		// NEO: SEa - [SortAltExtension] -- Xanatos -->
		if(m_bAlt)
			m_atSortArrowAlt = atType; 
		else
			m_atSortArrow = atType; 
		// NEO: SEa END <-- Xanatos --

		HINSTANCE hInstRes = AfxFindResourceHandle(MAKEINTRESOURCE(atType), RT_BITMAP); // NEO: SEa - [SortAltExtension] <-- Xanatos --
		if (hInstRes != NULL){
			HBITMAP hbmSortStates = (HBITMAP)::LoadImage(hInstRes, MAKEINTRESOURCE(atType), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_LOADMAP3DCOLORS); // NEO: SEa - [SortAltExtension] <-- Xanatos --
...
// NEO: SEa - [SortAltExtension] <-- Xanatos -->
void CMuleListCtrl::AlterSortArrow()
{
	HDITEM headerItem;
	headerItem.mask = HDI_FORMAT;
	CHeaderCtrl* pHeaderCtrl = GetHeaderCtrl();

	pHeaderCtrl->GetItem(m_bAlt ? m_iCurrentSortItem : m_iCurrentSortItemAlt, &headerItem);
	headerItem.fmt &= ~(HDF_IMAGE | HDF_BITMAP_ON_RIGHT);
	headerItem.iImage = 0;
	pHeaderCtrl->SetItem(m_bAlt ? m_iCurrentSortItem : m_iCurrentSortItemAlt, &headerItem);
}
// NEO: SEa END <-- Xanatos --
...
int CMuleListCtrl::UpdateLocation(int iItem) {
	...
	if(notFirst) {
		...
		int iResult = MultiSortProc(dwpItemData, GetParamAt(pos, iNewIndex)); // NEO: SE - [SortExtension] <-- Xanatos --
		if(iResult < 0) {
			...
			while(iDist > 1) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(posPrev, iNewIndex - iDist)) < 0) { // NEO: SE - [SortExtension] <-- Xanatos --
			...
			while(--iNewIndex >= 0) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(pos, iNewIndex)) >= 0) // NEO: SE - [SortExtension] <-- Xanatos --
					break;
			}
	...
	if(notLast) {
		...
		if(iResult > 0) {
			...
			while(iDist > 1) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(posNext, iNewIndex + iDist)) > 0) { // NEO: SE - [SortExtension] <-- Xanatos --
					iNewIndex = iNewIndex + iDist;
					pos = posNext;
				} else {
					posNext = pos;
...
BOOL CMuleListCtrl::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult) {
	...
	case LVM_SORTITEMS:
		...
		UpdateSortHistory(m_dwParamSort, 0); // NEO: SE - [SortExtension] <-- Xanatos --
		m_SortProc = (PFNLVCOMPARE)lParam;
		// NEO: SE - [SortExtension] -- Xanatos -->
		lParam = (LPARAM)MultiSortCallback; // SLUGFILLER: multiSort - hook our own callback for automatic layered sorting
		wParam = (WPARAM)this;
		// NEO: SE END <-- Xanatos --
	...
	case LVM_INSERTITEMW:
		//try to fix position of inserted items
		{
			...
			if(notFirst) {
				int iNewIndex = iItem - 1;
				POSITION pos = m_Params.FindIndex(iNewIndex);
				int iResult = MultiSortProc(pItem->lParam, GetParamAt(pos, iNewIndex)); // NEO: SE - [SortExtension] <-- Xanatos --
				if(iResult < 0) {
					POSITION posPrev = pos;
					int iDist = iNewIndex / 2;
					while(iDist > 1) {
						...
						if(MultiSortProc(pItem->lParam, GetParamAt(posPrev, iNewIndex - iDist)) < 0) { // NEO: SE - [SortExtension] <-- Xanatos --
							iNewIndex = iNewIndex - iDist;
							pos = posPrev;
						} else {
							posPrev = pos;
					...
					while(--iNewIndex >= 0) {
						m_Params.GetPrev(pos);
						if(MultiSortProc(pItem->lParam, GetParamAt(pos, iNewIndex)) >= 0) // NEO: SE - [SortExtension] <-- Xanatos --
							break;
					}
			...
			if(notLast) {
				...
				if(iResult > 0) {
					...
					while(iDist > 1) {
						...
						if(MultiSortProc(pItem->lParam, GetParamAt(posNext, iNewIndex + iDist)) > 0) { // NEO: SE - [SortExtension] <-- Xanatos --
							iNewIndex = iNewIndex + iDist;
							pos = posNext;
						} else {
							posNext = pos;
...
void CMuleListCtrl::OnSysColorChange()
{
	...
	if(GetSortItem() >= 0) // NEO: SEa - [SortAltExtension] <-- Xanatos --
		SetSortArrow(GetSortItem(), (ArrowType)(m_bAlt ? m_atSortArrowAlt : m_atSortArrow)); // NEO: SEa - [SortAltExtension] <-- Xanatos --
...
void CMuleListCtrl::UpdateSortHistoryMain(int dwNewOrder, int dwInverseValue){ // NEO: SEa - [SortAltExtension] <-- Xanatos --
	int dwInverse = (dwNewOrder >= dwInverseValue) ? (dwNewOrder-dwInverseValue) : (dwNewOrder+dwInverseValue); // NEO: SE - [SortExtension] <-- Xanatos --
...
// NEO: SEa - [SortAltExtension] -- Xanatos -->
void CMuleListCtrl::UpdateSortHistoryAlt(int dwNewOrder, int dwInverseValue){
	if(dwNewOrder > 2000 && dwNewOrder < 4000)
		dwNewOrder -= 2000;
	int dwInverse = (dwNewOrder >= dwInverseValue) ? (dwNewOrder-dwInverseValue) : (dwNewOrder+dwInverseValue); // NEO: SE - [SortExtension]
	// delete the value (or its inverse sorting value) if it appears already in the list
	POSITION pos1, pos2;
	for (pos1 = m_liSortHistoryAlt.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
		m_liSortHistoryAlt.GetNext(pos1);
		if (m_liSortHistoryAlt.GetAt(pos2) == dwNewOrder || m_liSortHistoryAlt.GetAt(pos2) == dwInverse)
			m_liSortHistoryAlt.RemoveAt(pos2);
	}
	m_liSortHistoryAlt.AddHead(dwNewOrder);

	// limit it to MAX_SORTORDERHISTORY entries for now, just for performance
	//if (m_liSortHistoryAlt.GetSize() > MAX_SORTORDERHISTORY)
	//	m_liSortHistoryAlt.RemoveTail();
}
// NEO: SEa END <-- Xanatos --

// NEO: SE - [SortExtension] -- Xanatos -->
int CMuleListCtrl::MultiSortProc(LPARAM lParam1, LPARAM lParam2) 
{
	if (m_bStopSort)
		return 0;

	CList<int, int>	&liSortHistory = m_bAlt ? m_liSortHistoryAlt : m_liSortHistory; // NEO: SEa - [SortAltExtension]
	for (POSITION pos = liSortHistory.GetHeadPosition(); pos != NULL; ) {
		// Use sort history for layered sorting
		int dwParamSort = liSortHistory.GetNext(pos);
		if(IsAlternate()) dwParamSort += 2000; // NEO: SEa - [SortAltExtension]
...
UINT CMuleListCtrl::DisableAutoSort()
{
	return NeoPrefs.DisableAutoSort();
}
// NEO: SE END <-- Xanatos --
...
int CMuleListCtrl::GetItemUnderMouse(CPoint* pt)
{
	CPoint _pt;
	if(!pt)
	{
