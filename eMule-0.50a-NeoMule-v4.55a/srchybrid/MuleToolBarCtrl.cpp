...
void CMuleToolbarCtrl::Init(void)
{
	...
	lLen2 = _tcslen(GetResString(IDS_X_EM_VOODOO)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_X_EM_VOODOO), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
	...
	m_ctrlSpeedMeter.CreateEx(WS_EX_STATICEDGE, NULL, NULL, m_bUseSpeedMeter ? WS_CHILD | WS_VISIBLE : WS_CHILD, CRect(0,0,0,0)/*rClient*/, this, 123);
...
void CMuleToolbarCtrl::OnNMRclick(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	// NEO: PSM - [PlusSpeedMeter] -- Xanatos -->
	menuToolbar.AppendMenu(MF_SEPARATOR);
	menuToolbar.AppendMenu(MF_STRING,MP_ENABLE_SPEEDMETER, GetResString(IDS_X_SPEEDMETER));
	menuToolbar.CheckMenuItem(MP_ENABLE_SPEEDMETER, m_bUseSpeedMeter ? MF_CHECKED : MF_UNCHECKED);
	// NEO: PSM END <-- Xanatos --
	CPoint point;
	GetCursorPos(&point);
...
#ifdef VOODOO // NEO: VOODOO - [UniversalPartfileInterface] -- Xanatos -->
		if ((bm.bmWidth == NUM_BUTTON_BITMAPS*m_sizBtnBmp.cx || bm.bmWidth == (NUM_BUTTON_BITMAPS+1)*m_sizBtnBmp.cx) && bm.bmHeight == m_sizBtnBmp.cy)
#else
		if (bm.bmWidth == NUM_BUTTON_BITMAPS*m_sizBtnBmp.cx && bm.bmHeight == m_sizBtnBmp.cy)
#endif // VOODOO // NEO: VOODOO END <-- Xanatos --
		{
...
#ifdef VOODOO // NEO: VOODOO - [UniversalPartfileInterface] -- Xanatos -->
				if(bm.bmWidth == NUM_BUTTON_BITMAPS*m_sizBtnBmp.cx)
					ImageList.Add(CTempIconLoader(_T("VOODOOTB"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
#endif // VOODOO // NEO: VOODOO END <-- Xanatos --
...
#ifdef VOODOO // NEO: VOODOO - [UniversalPartfileInterface] -- Xanatos -->
		ImageList.Add(CTempIconLoader(_T("VOODOOTB"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
		ASSERT( ImageList.GetImageCount() == NUM_BUTTON_BITMAPS+1 );
#else
...
BOOL CMuleToolbarCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	switch (wParam)
	{
		...
		// NEO: PSM - [PlusSpeedMeter] -- Xanatos -->
		case MP_ENABLE_SPEEDMETER:{
			ShowSpeedMeter(!m_bUseSpeedMeter);
			break;
		}
		// NEO: PSM END <-- Xanatos --
...
void CMuleToolbarCtrl::ShowSpeedMeter(bool bShow)
{
	...
	if (bShow)
	{
		if (!m_ctrlSpeedMeter.IsWindowVisible())
		{
			...
			SetSpeedMeterRange(iMaxRange, 0);
