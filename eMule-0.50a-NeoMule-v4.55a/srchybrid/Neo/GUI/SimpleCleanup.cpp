...
BEGIN_MESSAGE_MAP(CSimpleCleanupDialog, CModResizableDialog) // NEO: MLD - [ModelesDialogs]
	...
	ON_BN_CLICKED(IDC_BUTTONCHANGE, OnBnClickedButtonedit)
	ON_WM_CLOSE()
END_MESSAGE_MAP()

CSimpleCleanupDialog::CSimpleCleanupDialog(CWnd* pParent /*=NULL*/)
	: CModResizableDialog(CSimpleCleanupDialog::IDD, pParent) // NEO: MLD - [ModelesDialogs]
{
	m_ParentWnd = pParent->m_hWnd;
}
...
void CSimpleCleanupDialog::Localize () {
	SetWindowText (GetResString (IDS_X_SC_HEADLINE));
	GetDlgItem (IDC_SC_STATIC1)->SetWindowText (GetResString (IDS_X_SC_SEARCHFOR));
	GetDlgItem (IDC_SC_STATIC2)->SetWindowText (GetResString (IDS_X_SC_REPLACEBY));
	GetDlgItem (IDC_SC_STATIC3)->SetWindowText (GetResString (IDS_X_REPLACECHAR));
	GetDlgItem (IDC_SC_STATIC4)->SetWindowText (GetResString (IDS_X_REPLACECHARBY));
	GetDlgItem (IDC_CHECKDOTTOSPACE)->SetWindowText (GetResString (IDS_X_CHECKDOTTOSPACE));
	GetDlgItem (IDC_CHECKUNDERLINETOSPACE)->SetWindowText (GetResString (IDS_X_CHECKUNDERLINETOSPACE));
	GetDlgItem (IDC_CHECKCONVERTAPOSTROPHES)->SetWindowText (GetResString (IDS_X_CHECKCONVERTAPOSTROPHES));
	GetDlgItem (IDC_CHECKCONVERTHEX)->SetWindowText (GetResString (IDS_X_CHECKCONVERTHEX));
	GetDlgItem (IDC_CHECKSEARCHANDREPLACE)->SetWindowText (GetResString (IDS_X_CHECKSEARCHANDREPLACE));
	GetDlgItem (IDC_CHECKCASESENSITIVE)->SetWindowText (GetResString (IDS_X_CHECKCASESENSITIVE));
	GetDlgItem (IDC_CHECKCHARACTERREPLACE)->SetWindowText (GetResString (IDS_X_CHECKCHARACTERREPLACE));
	GetDlgItem (IDC_NEWCHARACTER)->SetWindowText (GetResString (IDS_X_SC_ADD));
	GetDlgItem (IDC_BUTTONCHANGE)->SetWindowText (GetResString (IDS_X_SC_EDIT));
	GetDlgItem (IDC_DELETECHARACTER)->SetWindowText (GetResString (IDS_X_SC_DELETE));
...
BOOL CSimpleCleanupDialog::OnInitDialog() {
	...
	m_ReplaceListBox->InsertColumn (0,GetResString (IDS_X_SEARCHFORCHAR),0,R.Width ()/2);
	m_ReplaceListBox->InsertColumn (1,GetResString (IDS_X_REPLACEBYCHAR),0,R.Width ()/2);

	WriteConfig ();

	EnableSaveRestore(_T("SimpleCleanup"));
...
void CSimpleCleanupDialog::OnOK()
{
	...
	if(IsWindow(m_ParentWnd))
		::SendMessage(m_ParentWnd,UM_SIMPLE_CLEANUP_NOTIFY,0,0);

	CModResizableDialog::OnOK(); // NEO: MLD - [ModelesDialogs]
}
