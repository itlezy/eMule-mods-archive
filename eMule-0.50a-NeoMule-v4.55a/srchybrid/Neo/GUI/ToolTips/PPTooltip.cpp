...
//	TRACE (_T("CPPToolTip::DisplayToolTip()\n"));
	...
	case PPTOOLTIP_EFFECT_HARDBUMP:
	case PPTOOLTIP_EFFECT_METAL:
		SetShade(rect, m_pToolInfo.nEffect, (BYTE)m_nGranularity, 5, m_pToolInfo.crBegin);
	}
#endif
...
{
	CSize sz (0, 0);
	HICON hIcon = theApp.LoadIcon(nID);
	if (hIcon != NULL)
	{
	...
	case PPTOOLTIP_EFFECT_METAL:
		...
		for(i = 0; i < sYSize; i++) {
			for(j = 0; j < sXSize; j++) {
				...
				posDst[j]=(BYTE)d;
				posDst[j]= posDst[j] + (BYTE)(rand()/grainx2);
			}
			posDst+=bytes;
		}
	...
	case PPTOOLTIP_EFFECT_HARDBUMP:	// 
		...
		for(i = 0; i < sYSize; i++) {
			...
			for(j = 0; j < sXSize; j++) {
				...
				if ((x+xs)<0) posDst[j]=(BYTE)(idxmax-((j*128)/d));
				posDst[j]=posDst[j]+(BYTE)(rand()/grainx2-granularity);
			}
			posDst+=bytes;
		}
...
{
	ModifyMaskTool(nIndexTool, nMask, (UINT)-1);
}
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::SetDirection(UINT nDirection /* = PPTOOLTIP_RIGHT_TOP */, int nIndexTool /* = PPTOOLTIP_TOOL_NOEXIST */)
{
...
void CPPToolTip::SetBehaviour(UINT nBehaviour /* = PPTOOLTIP_MULTIPLE_SHOW */, int nIndexTool /* = PPTOOLTIP_TOOL_NOEXIST */)
{
...
void CPPToolTip::AddTool(CWnd * pWnd, CString sTooltipText, UINT nIdIcon, CSize /*szIcon*/ /* = CSize(0, 0) */,
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
	HICON hIcon = theApp.LoadIcon(nIdIcon);
