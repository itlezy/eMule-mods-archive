...
void CreateNetworkInfo(CRichEditCtrlX& rCtrl, CHARFORMAT& rcfDef, CHARFORMAT& rcfBold, bool bFullInfo)
{
	...
	if(NeoPrefs.IsNAFCEnabled()){
		rCtrl.SetSelectionCharFormat(rcfBold);
		rCtrl << GetResString(IDS_X_CD_ADAPTER) << _T("\r\n");
		rCtrl.SetSelectionCharFormat(rcfDef);

		rCtrl << GetResString(IDS_X_IP) << _T("\t") << ipstr(theApp.bandwidthControl->GetAdapterIP()) << _T("\r\n\r\n");
	}
	// NEO: NAFC END
	...
	if(NeoPrefs.IsCheckConnection() || NeoPrefs.IsUSSEnabled() || NeoPrefs.IsDSSEnabled()){
		rCtrl.SetSelectionCharFormat(rcfBold);
		rCtrl << GetResString(IDS_X_CD_PINGER) << _T("\r\n");
		rCtrl.SetSelectionCharFormat(rcfDef);

		buffer = theApp.bandwidthControl->GetPingedServer();
		rCtrl << GetResString(IDS_X_PING_SERVER) << _T(":\t") << buffer << _T("\r\n");
		...
		if(theApp.bandwidthControl->IsPingPreparing())
			...
			buffer = GetResString(IDS_X_SS_WORKING);
        else
			buffer = GetResString(IDS_X_SS_FAILED);

		rCtrl << GetResString(IDS_STATUS) << _T(":\t") << buffer << _T("\r\n\r\n");
	}
#endif // NEO_BC // NEO: NBC END <-- Xanatos --
	...
	if (theApp.serverconnect->IsConnected()){
		...
		if (srv){
			...
			{
				buffer.Format(_T("%u"),cur_server->GetServerCredits());
				rCtrl << GetResString(IDS_X_INFOCREDITS) << _T("\t") << buffer << _T("\r\n");
			}
			// NEO: KLC END <-- Xanatos --
