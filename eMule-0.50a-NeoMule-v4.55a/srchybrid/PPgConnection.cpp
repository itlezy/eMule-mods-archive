...
#include "MuleToolbarCtrl.h" // NEO: PSM - [PlusSpeedMeter] <-- Xanatos --
...
BEGIN_MESSAGE_MAP(CPPgConnection, CPropertyPage)
	...
	//ON_EN_CHANGE(IDC_MAXSOURCEPERFILE, OnSettingsChange) // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
...
BOOL CPPgConnection::OnApply()
{
	...
	theApp.emuledlg->toolbar->SetSpeedMeterRange(thePrefs.GetMaxGraphUploadRate(true),thePrefs.GetMaxGraphDownloadRate()); // NEO: PSM - [PlusSpeedMeter] <-- Xanatos --
...
{	
	if (m_hWnd)
	{
		...
		GetDlgItem(IDC_MAXSRCHARD_LBL)->SetWindowText(GetResString(IDS_X_HARDLIMIT)); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
