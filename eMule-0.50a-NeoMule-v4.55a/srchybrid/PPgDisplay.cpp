...
void CPPgDisplay::LoadSettings(void)
{
	...
	CheckDlgButton(IDC_SHOWCATINFO,thePrefs.ShowCatTabInfos()); // NEO: MOD - [MoreCategoryInfo] <-- Xanatos --
	...
	CheckDlgButton(IDC_SHOWDWLPERCENT,thePrefs.GetUseDwlPercentage() ); // NEO: MOD - [Percentage] <-- Xanatos --
...
BOOL CPPgDisplay::OnApply()
{
	...
	thePrefs.m_uShowDwlPercentage = IsDlgButtonChecked(IDC_SHOWDWLPERCENT); // NEO: MOD - [Percentage] <-- Xanatos --
	...
	thePrefs.ShowCatTabInfos(IsDlgButtonChecked(IDC_SHOWCATINFO));// NEO: MOD - [MoreCategoryInfo] <-- Xanatos --
	...
	if (!thePrefs.ShowRatesOnTitle())
		...
		theApp.emuledlg->SetWindowText(theApp.GetAppTitle(true)); // NEO: NV - [NeoVersion] <-- Xanatos --
