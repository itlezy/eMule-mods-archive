...
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
		case ACTION_CONHALF		: return GetResString(IDS_MAXHALFOPENCONS);

		case ACTION_ULMODE		: return GetResString(IDS_X_UPLOAD_MODE);
		case ACTION_ULMAX		: return GetResString(IDS_X_MAX_BC_UPLOAD);
		case ACTION_ULMIN		: return GetResString(IDS_X_MIN_BC_UPLOAD);

		case ACTION_UPBASEPING	: return GetResString(IDS_X_BASE_PING_UP) + GetResString(IDS_X_UP);
		case ACTION_UPTOLERANCE	: return GetResString(IDS_X_PING_UP_TOLERANCE) + GetResString(IDS_X_UP);
		case ACTION_UPPROZENT	: return GetResString(IDS_X_PING_UP_PROZENT) + GetResString(IDS_X_UP);

		case ACTION_UPSTREAM	: return GetResString(IDS_X_UP_STREAM);

		case ACTION_DLMODE		: return GetResString(IDS_X_DOWNLOAD_MODE);
		case ACTION_DLMAX		: return GetResString(IDS_X_MAX_BC_DOWNLOAD);
		case ACTION_DLMIN		: return GetResString(IDS_X_MIN_BC_DOWNLOAD);

		case ACTION_DOWNBASEPING: return GetResString(IDS_X_BASE_PING_DOWN) + GetResString(IDS_X_DOWN);
		case ACTION_DOWNTOLERANCE: return GetResString(IDS_X_PING_DOWN_TOLERANCE) + GetResString(IDS_X_DOWN);
		case ACTION_DOWNPROZENT	: return GetResString(IDS_X_PING_DOWN_PROZENT) + GetResString(IDS_X_DOWN);

		case ACTION_DOWNSTREAM	: return GetResString(IDS_X_DOWN_STREAM);

	
		case ACTION_MANCON		: return GetResString(IDS_X_MANAGE_CONNECTIONS);
		case ACTION_MANSET		: return GetResString(IDS_X_MANAGE_CONNECTIONS_FACTOR);

		case ACTION_INCLUDEOH	: return GetResString(IDS_X_INCLUDE_OVERHEAD);
#endif // NEO_BC // NEO: NBCs END <-- Xanatos --
	}
	return NULL;
}
...
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_CONHALF,GetResString(IDS_MAXHALFOPENCONS));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_ULMODE,GetResString(IDS_X_UPLOAD_MODE));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_ULMAX,GetResString(IDS_X_MAX_BC_UPLOAD));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_ULMIN,GetResString(IDS_X_MIN_BC_UPLOAD));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_UPBASEPING,GetResString(IDS_X_BASE_PING_UP) + GetResString(IDS_X_UP));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_UPTOLERANCE,GetResString(IDS_X_PING_UP_TOLERANCE) + GetResString(IDS_X_UP));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_UPPROZENT,GetResString(IDS_X_PING_UP_PROZENT) + GetResString(IDS_X_UP));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_UPSTREAM,GetResString(IDS_X_UP_STREAM));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DLMODE,GetResString(IDS_X_DOWNLOAD_MODE));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DLMAX,GetResString(IDS_X_MAX_BC_DOWNLOAD));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DLMIN,GetResString(IDS_X_MIN_BC_DOWNLOAD));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DOWNBASEPING,GetResString(IDS_X_BASE_PING_DOWN) + GetResString(IDS_X_DOWN));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DOWNTOLERANCE,GetResString(IDS_X_PING_DOWN_TOLERANCE) + GetResString(IDS_X_DOWN));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DOWNPROZENT,GetResString(IDS_X_PING_DOWN_PROZENT) + GetResString(IDS_X_DOWN));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_DOWNSTREAM,GetResString(IDS_X_DOWN_STREAM));

	
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_MANCON,GetResString(IDS_X_MANAGE_CONNECTIONS));
	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_MANSET,GetResString(IDS_X_MANAGE_CONNECTIONS_FACTOR));

	m_ActionSel.AppendMenu(MF_STRING,MP_SCHACTIONS+ACTION_INCLUDEOH,GetResString(IDS_X_INCLUDE_OVERHEAD));
#endif // NEO_BC // NEO: NBCs END <-- Xanatos --
...
{ 
	...
	// add
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
	if (wParam>=MP_SCHACTIONS && wParam<MP_SCHACTIONS+30 && m_actions.GetItemCount()<25)
#else
...
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
	else if (wParam>=MP_SCHACTIONS+30 && wParam<=MP_SCHACTIONS+80)
#else
...
#endif // NEO_BC // NEO: NBCs END <-- Xanatos --
	{
		CString newval;
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
		newval.Format(_T("%i"),wParam-MP_SCHACTIONS-32);
#else
        ...
        { 
			if (item!=-1) {
				...
				switch (m_actions.GetItemData(item)) {
					...
					case 9:
					case 16:
						prompt=GetResString(IDS_X_SCHED_MODE);
						break;
					case 23:
					case 25:
						prompt=GetResString(IDS_X_SCHED_BOOL);
						break;
#endif // NEO_BC // NEO: NBCs END <-- Xanatos --
