...
BEGIN_MESSAGE_MAP(CPPgTweaks, CPropertyPage)
	ON_WM_HSCROLL()
	ON_WM_DESTROY()
	ON_MESSAGE(WM_TREEOPTSCTRL_NOTIFY, OnTreeOptsCtrlNotify) // NEO: FIX - [TreeControl] <-- Xanatos --
	ON_WM_HELPINFO()
END_MESSAGE_MAP()
...
{
	//m_iFileBufferSize = 0; // NEO: MOD - [BufferCustomisation] <-- Xanatos --
	m_iQueueSize = 0;
	m_iMaxConnPerFive = 0;
	m_iMaxHalfOpen = 0;
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_FILEBUFFERSIZE, m_ctlFileBuffSize); // NEO: MOD - [BufferCustomisation] <-- Xanatos --
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		m_ctrlTreeOptions.AddEditBox(m_htiYourHostname, RUNTIME_CLASS(CTreeOptionsEdit)); // NEO: FIX - [TreeControl] <-- Xanatos --
...
{
	m_iMaxConnPerFive = thePrefs.GetMaxConperFive(true); // NEO: QS - [QuickStart] <-- Xanatos --
	m_iMaxHalfOpen = thePrefs.GetMaxHalfConnections(true); // NEO: QS - [QuickStart] <-- Xanatos --
	...
	// NEO: MOD - [BufferCustomisation] -- Xanatos --
	...
	m_ctlQueueSize.SetRange(10, 100, TRUE);
...
BOOL CPPgTweaks::OnApply()
{
	...
	theApp.scheduler->original_cons5s = thePrefs.GetMaxConperFive(true); // NEO: QS - [QuickStart] <-- Xanatos --
	thePrefs.SetMaxHalfConnections(m_iMaxHalfOpen ? m_iMaxHalfOpen : DFLT_MAXHALFOPEN);
	theApp.scheduler->original_conshalf = thePrefs.GetMaxHalfConnections(true); // NEO: QS - [QuickStart] <-- Xanatos --
	...
	if (!thePrefs.log2disk && m_bLog2Disk){
		theLog.Open();
		theModLog.Open(); // NEO: ML - [ModLog] <-- Xanatos --
	}else if (thePrefs.log2disk && !m_bLog2Disk){
		theLog.Close();
		theModLog.Close(); // NEO: ML - [ModLog] <-- Xanatos --
	}
	...
	//thePrefs.m_iFileBufferSize = m_iFileBufferSize; // NEO: MOD - [BufferCustomisation] <-- Xanatos --
...
LRESULT CPPgTweaks::OnTreeOptsCtrlNotify(WPARAM wParam, LPARAM lParam)
{
	if (wParam == IDC_EXT_OPTS)
	{
		...
		{
			UINT bCheck; // NEO: FIX - [TreeControl] <-- Xanatos --
