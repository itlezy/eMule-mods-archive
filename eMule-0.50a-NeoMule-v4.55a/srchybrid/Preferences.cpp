...
bool	CPreferences::m_bMiniMuleAutoClose;
int		CPreferences::m_iMiniMuleTransparency;
// NEO: MOD END <-- Xanatos --
...
//UINT	CPreferences::maxsourceperfile; // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
...
DWORD	CPreferences::m_adwStatsColors[41]; // NEO: NS - [NeoStats] <-- Xanatos --
...
UINT	CPreferences::statsTransferGraphRatio;
UINT	CPreferences::statsSourceGraphRatio;
// NEO: NS END <-- Xanatos --
...
UINT	CPreferences::m_uLastCategory; // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
...
bool	CPreferences::dontcompressblocks; // NEO: MOD - [IsCompressible] <-- Xanatos --
...
int		CPreferences::catRes; // NEO: NSC - [NeoSharedCategories] <-- Xanatos --
...
UINT	CPreferences::m_uRememberDownloadedFiles; // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
...
void CPreferences::Init()
{
	...
	strFullPath = GetMuleDirectory(EMULE_CONFIGDIR) + L"sharedsubdir.dat";
	...
	// share single files
	strFullPath = GetMuleDirectory(EMULE_CONFIGDIR) + L"sharedfiles.dat";
	sdirfile = new CStdioFile();
	bIsUnicodeFile = IsUnicodeFile(strFullPath); // check for BOM
	// open the text file either in ANSI (text) or Unicode (binary), this way we can read old and new files
	// with nearly the same code..
	...
	// NEO: VSF - [VirtualSharedFiles] -- Xanatos -->
	CString fullpath = GetMuleDirectory(EMULE_CONFIGDIR) + L"virtualdirs.ini";
	CStdioFile file;
...
void CPreferences::Uninit()
{
	while (!catMap.IsEmpty())
	{
		...
		if(delcat->KnownPrefs)
			delete delcat->KnownPrefs;
		if(delcat->PartPrefs)
			delete delcat->PartPrefs;
...
uint64 CPreferences::GetMaxDownloadInBytesPerSec(bool dynamic){
	...
	// Note: this is only provisorc the final have Neo bandwidth control...
	if(NeoPrefs.IsFriendSlotLimit() && theApp.uploadqueue->GetFriendSlots() > 0) // if it isn't we work liek the official with one unrestricted slot
	{
		uint64 sessionSentBytes = theStats.sessionSentBytes - theStats.sessionSentBytesToFriend;
		uint64 sessionReceivedBytes = theStats.sessionSentBytes;
		if (sessionSentBytes != 0 && sessionReceivedBytes != 0 && sessionReceivedBytes > 3 * sessionSentBytes
		 && theApp.uploadqueue->GetWaitingUserCount() > 10 && theApp.uploadqueue->GetUploadQueueLength())
		{
			float r1 = (float)sessionSentBytes/sessionReceivedBytes;	// 1/6					| 1/2
			float r2 = (float)maxup/maxdownload;						// 1/3					| 1/3
			float r3 = r1/r2;											// 1/6//1/3 = 3/6 = 1/2 | 1/2//1/3 = 3/2 = 1+1/2
			if(r3 < 1.0f) // ratio violated
				return (uint64)min(maxup * r3, 3);
		}
	}
...
void CPreferences::Add2SessionTransferData(UINT uClientID, UINT uClientPort, BOOL bFromPF, 
										   BOOL bUpDown, uint32 bytes, bool sentToFriend, bool sentToRelease)
#else
void CPreferences::Add2SessionTransferData(UINT uClientID, UINT uClientPort, BOOL bFromPF, 
...
#ifdef BW_MOD// NEO: BM - [BandwidthModeration] -- Xanatos -->
			theApp.UpdateSentBytes(bytes, sentToFriend, sentToRelease);
#else
...
bool CPreferences::Save(){
	...
	SavePreferences();
	SaveStats();
	SaveCats(); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	...
	if (sdirfile.Open(strFullPath, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite | CFile::typeBinary))
	{
		try{
			...
			for (POSITION pos = sharedsubdir_list.GetHeadPosition();pos != 0;){
				sdirfile.WriteString(sharedsubdir_list.GetNext(pos));
				sdirfile.Write(L"\r\n", sizeof(TCHAR)*2);
			}
			if (thePrefs.GetCommitFiles() >= 2 || (thePrefs.GetCommitFiles() >= 1 && !theApp.emuledlg->IsRunning())){
				sdirfile.Flush(); // flush file stream buffers to disk buffers
	...
	if (sdirfile.Open(strFullPath, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite | CFile::typeBinary))
	{
		try{
			...
			for (POSITION pos = sharedfile_list.GetHeadPosition();pos != 0;){
				sdirfile.WriteString(sharedfile_list.GetNext(pos));
				sdirfile.Write(L"\r\n", sizeof(TCHAR)*2);
			}
			if (thePrefs.GetCommitFiles() >= 2 || (thePrefs.GetCommitFiles() >= 1 && !theApp.emuledlg->IsRunning())){
				sdirfile.Flush(); // flush file stream buffers to disk buffers
...
void CPreferences::SavePreferences()
{
	...
	//ini.WriteInt(L"MaxSourcesPerFile",maxsourceperfile ); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	ini.WriteBool(L"DontCompressBlocks",dontcompressblocks); // NEO: MOD - [IsCompressible]
	// NEO: MOD END <-- Xanatos --
	...
	ini.WriteInt(L"LastCategory",m_uLastCategory); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	...
	ini.WriteBool(L"RestoreLastMainWndDlg",m_bRestoreLastMainWndDlg); 	// NEO: MOD - [MissingPrefs] -- Xanatos -->
	ini.WriteInt(L"LastMainWndDlgID",m_iLastMainWndDlgID);
	ini.WriteBool(L"RestoreLastLogPane",m_bRestoreLastLogPane); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	ini.WriteBool(L"IconflashOnNewMessage",m_bIconflashOnNewMessage); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	// NEO: MOD - [MissingPrefs] -- Xanatos -->
	ini.WriteBool(L"MiniMuleAutoClose", m_bMiniMuleAutoClose);
	...
	ini.WriteBool(L"AdjustNTFSDaylightFileTime", m_bAdjustNTFSDaylightFileTime); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	ini.WriteInt(L"DebugSearchResultDetailLevel", m_iDebugSearchResultDetailLevel); // NEO: MOD - [MissingPrefs] <-- Xanatos --
#endif
	...
	ini.WriteBool(L"PreviewOnIconDblClk",m_bPreviewOnIconDblClk); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	ini.WriteInt(L"LogFileFormat", (int)m_iLogFileFormat);
	// NEO: MOD END <-- Xanatos --
	...
	ini.WriteBool(L"PreferRestrictedOverUser", m_bPreferRestrictedOverUser); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	ini.WriteBool(L"OpenPortsOnStartUp", m_bOpenPortsOnStartUp);
	ini.WriteInt(L"DebugLogLevel", m_byLogLevel);
	ini.WriteBool(L"AICHTrustEveryHash", m_bTrustEveryHash); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	ini.WriteInt(L"RememberDownloadedFiles", m_uRememberDownloadedFiles); // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
	...
	ini.WriteInt(L"CryptTCPPaddingLength", m_byCryptTCPPaddingLength); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	ini.WriteInt(L"statsTransferGraphRatio", statsTransferGraphRatio,L"Statistics");
	ini.WriteInt(L"statsSourceGraphRatio", statsSourceGraphRatio,L"Statistics");
	// NEO: NS END <-- Xanatos --
	ini.WriteString(L"statsExpandedTreeItems", m_strStatsExpandedTreeItems);
	CString buffer2;
	for (int i=0;i<GetNumStatsColors();i++) { // NEO: NS - [NeoStats] <-- Xanatos --
		buffer.Format(L"0x%06x",GetStatsColor(i));
		buffer2.Format(L"StatColor%i",i);
		ini.WriteString(buffer2,buffer,L"Statistics" );
	}
	ini.WriteBool(L"ShowVerticalHourMarkers", m_bShowVerticalHourMarkers, L"Statistics"); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	...
	ini.WriteInt(L"MaxFileUploadSizeMB", m_iWebFileUploadSizeLimitMB ); // NEO: MOD - [MissingPrefs] <-- Xanatos --
	// NEO: MOD - [MissingPrefs] -- Xanatos -->
	CString sAllowedRemoteAccessIPs;
	for (int i = 0; thePrefs.m_aAllowedRemoteAccessIPs.GetCount(); i++)
		sAllowedRemoteAccessIPs += ipstr(thePrefs.m_aAllowedRemoteAccessIPs[i]) + _T(";");
	sAllowedRemoteAccessIPs.TrimRight(_T(";"));
	ini.WriteString(L"AllowedIPs",sAllowedRemoteAccessIPs);
	// NEO: MOD END <-- Xanatos --
	...
	ini.WriteBool(L"Show", m_bPeerCacheShow); // NEO: MOD - [MissingPrefs] <-- Xanatos --
...
void CPreferences::ResetStatsColor(int index)
{
	switch(index)
	{
		...
		// NEO: VOODOO - [UniversalPartfileInterface]
...
void CPreferences::LoadPreferences()
{
	...
	if (strNick.IsEmpty() || IsDefaultNick(strNick))
		SetUserNick(_T("NeoMule.sf.net")); // NEO: NV - [NeoVersion] <-- Xanatos --
	...
	//maxsourceperfile=ini.GetInt(L"MaxSourcesPerFile",400 ); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	dontcompressblocks=ini.GetBool(L"DontCompressBlocks",false); // NEO: MOD - [IsCompressible] <-- Xanatos --

	m_uDeadServerRetries=ini.GetInt(L"DeadServerRetry",3); // NEO: MOD - [DeadServerRetry3] <-- Xanatos --
	...
	m_uLastCategory = ini.GetInt(L"LastCategory",0); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	...
	// NEO: MOD - [MissingPrefs] -- Xanatos -->
	m_bMiniMuleAutoClose = ini.GetBool(L"MiniMuleAutoClose", false);
	m_iMiniMuleTransparency = ini.GetInt(L"MiniMuleTransparency", 0);
	// NEO: MOD END <-- Xanatos --
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	m_uRememberDownloadedFiles = ini.GetInt(L"RememberDownloadedFiles", TRUE); // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	statsTransferGraphRatio = ini.GetInt(L"statsTransferGraphRatio", 1, L"Statistics");
	statsSourceGraphRatio = ini.GetInt(L"statsSourceGraphRatio", 1, L"Statistics"); 
	// NEO: NS END <-- Xanatos --
...
void CPreferences::SaveCats()
{
	...
	_tremove(strCatIniFilePath);
	CIni ini(strCatIniFilePath);
	ini.WriteInt(L"Count", catMap.GetCount() - 1, L"General");
	ini.WriteInt(L"Reserved", catRes); // NEO: NSC - [NeoSharedCategories] <-- Xanatos --
	for (int i = 0; i < catMap.GetCount(); i++)
	{
		...
		ini.WriteStringUTF8(L"Temp", catMap.GetAt(i)->strTempPath); // NEO: MTD - [MultiTempDirectories] <-- Xanatos --
		...
		ini.WriteInt(L"uplPriority",catMap.GetAt(i)->boost);
		ini.WriteBool(L"ReleaseMode", catMap.GetAt(i)->release); // NEO: SRS - [SmartReleaseSharing]
		ini.WriteBool(L"ResumeFileOnlyInSameCat", catMap.GetAt(i)->bResumeFileOnlyInSameCat!=FALSE); 
		...
		ini.WriteBool(L"vfStandby", catMap.GetAt(i)->viewfilters.bStandby!=FALSE); // NEO: SD - [StandByDL]
		ini.WriteBool(L"vfSuspend", catMap.GetAt(i)->viewfilters.bSuspend!=FALSE); // NEO: SC - [SuspendCollecting]
		...
		// NEO: FCFG - [FileConfiguration] -- Xanatos -->
		ini.WriteInt(L"KnownPrefs", catMap.GetAt(i)->KnownPrefs != NULL);
		ini.WriteInt(L"FilePrefs", catMap.GetAt(i)->PartPrefs != NULL);
		
		if(catMap.GetAt(i)->KnownPrefs && !catMap.GetAt(i)->KnownPrefs->IsEmpty()){
			strSection.Format(L"CatKnownPrefs#%i", i);
			ini.SetSection(strSection);
			catMap.GetAt(i)->KnownPrefs->Save(ini);

		}

		if(catMap.GetAt(i)->PartPrefs && !catMap.GetAt(i)->PartPrefs->IsEmpty()){
			strSection.Format(L"CatPartPrefs#%i", i);
			ini.SetSection(strSection);
			catMap.GetAt(i)->PartPrefs->Save(ini);
...
void CPreferences::LoadCats()
{
	...
	catRes = ini.GetInt(L"Reserved", 0); // NEO: NSC - [NeoSharedCategories] <-- Xanatos --
	for (int i = 0; i <= iNumCategories; i++)
	{
		...
		// NEO: NXC - [NewExtendedCategories] -- Xanatos -->
		if(newcat->strTitle.IsEmpty()){
			if(i > 0)
				newcat->strTitle.Format(_T("Cat %i"),i);
			else
				newcat->strTitle = _T("Default");
		}
		// NEO: NXC END <-- Xanatos --
		newcat->strIncomingPath = ini.GetStringUTF8(L"Incoming");
	  if(!newcat->strIncomingPath.IsEmpty()) // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	  {
		...
		if (!IsShareableDirectory(newcat->strIncomingPath)) {
			...
			//MakeFoldername(newcat->strIncomingPath);
			newcat->strIncomingPath = _T(""); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
		}
	  }
	    // NEO: MTD - [MultiTempDirectories] -- Xanatos -->
		newcat->strTempPath = ini.GetStringUTF8(L"Temp");
		if(!newcat->strTempPath.IsEmpty())
			MakeFoldername(newcat->strTempPath); 
		// NEO: MTD END <-- Xanatos --
		...
		newcat->boost = ini.GetInt(L"uplPriority",PR_NORMAL);
		newcat->release = ini.GetBool(L"ReleaseMode", false); // NEO: SRS - [SmartReleaseSharing]
		newcat->bResumeFileOnlyInSameCat = ini.GetBool(L"ResumeFileOnlyInSameCat", false); 
		...
		newcat->viewfilters.bStandby = ini.GetBool(L"vfStandby", true); // NEO: SD - [StandByDL]
		newcat->viewfilters.bSuspend = ini.GetBool(L"vfSuspend", true); // NEO: SC - [SuspendCollecting]
		...
		newcat->viewfilters.bSeenComplet = ini.GetBool(L"vfSeenComplet", true); 
		newcat->viewfilters.nFSizeMin = ini.GetUInt64(L"vfFSizeMin", 0);
		newcat->viewfilters.nFSizeMax = ini.GetUInt64(L"vfFSizeMax", 0);
		newcat->viewfilters.nRSizeMin = ini.GetUInt64(L"vfRSizeMin", 0);
		newcat->viewfilters.nRSizeMax = ini.GetUInt64(L"vfRSizeMax", 0);
		...
		// NEO: FCFG - [FileConfiguration] -- Xanatos -->
		int haveKnonPrefs = ini.GetInt(L"KnownPrefs", 0);
		int havePartPrefs = ini.GetInt(L"FilePrefs", 0);
		
		if(haveKnonPrefs){
			newcat->KnownPrefs = new CKnownPreferencesEx(CFP_CATEGORY);
			((CKnownPreferencesEx*)newcat->KnownPrefs)->KnownPrefs = &NeoPrefs.KnownPrefs;
			strSection.Format(L"CatKnownPrefs#%i", i);
			ini.SetSection(strSection);
			newcat->KnownPrefs->Load(ini);

		}else
			newcat->KnownPrefs = NULL;

		if(havePartPrefs){
			newcat->PartPrefs = new CPartPreferencesEx(CFP_CATEGORY);
			((CPartPreferencesEx*)newcat->PartPrefs)->PartPrefs = &NeoPrefs.PartPrefs;
			strSection.Format(L"CatPartPrefs#%i", i);
			ini.SetSection(strSection);
			newcat->PartPrefs->Load(ini);
...
// NEO: NSC - [NeoSharedCategories] -- Xanatos -->
int	CPreferences::InsertCat(Category_Struct* cat, bool share) 
{
	catMap.InsertAt(share ? GetFullCatCount() : GetCatCount(),cat); 																
	if(share) 
		catRes++;
	return (share ? GetFullCatCount() : GetCatCount())-1;
} 
// NEO: NSC END <-- Xanatos --

// NEO: FCFG - [FileConfiguration] -- Xanatos -->
int CPreferences::FindCategory(Category_Struct* cat)
{
	for (int i = 0; i < catMap.GetCount(); i++)
	{
		if(catMap.GetAt(i) == cat)
			return i;
	}
	return -1;
}
...
/*			How is this spposed to work?
*
*	Some nat devices allows incomming packets form any ip on a specyfyed port 
*	when the port is used for outgoing connections to other adresses.
*	In this case we only need to know the port the nat device give us, this
*	we get reported my eserver version 17.15 or higher, or we get it from 
*	nat-t enabled clients.
*	We publish the reported port in kad etc so we can get open status and be 
*	reached by other clients.
*	For opening listen port and the port to reuse we use always the preset local port.
*	Actualy this tuns us just really HighID
*
*/
uint16 CPreferences::GetPort(bool bLokal)
{
	if(NeoPrefs.ReuseTCPPort() && !bLokal && theApp.GetPublicPort())
		return theApp.GetPublicPort();
	return port;
}
...
UINT CPreferences::GetMaxConnections(bool bOriginal)
{
	return (!bOriginal && NeoPrefs.m_bOnQuickStart) ? NeoPrefs.m_iQuickMaxConnections : maxconnections;
}

UINT CPreferences::GetMaxHalfConnections(bool bOriginal)
{
	return (!bOriginal && NeoPrefs.m_bOnQuickStart) ? NeoPrefs.m_iQuickMaxHalfOpen : maxhalfconnections;
}

UINT CPreferences::GetMaxConperFive(bool bOriginal)
{
	return (!bOriginal && NeoPrefs.m_bOnQuickStart) ? NeoPrefs.m_iQuickMaxConperFive : MaxConperFive;
}
// NEO: QS END <-- Xanatos --
