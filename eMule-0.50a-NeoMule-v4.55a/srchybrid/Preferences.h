...
struct Category_Struct{
	...
	CString	strTempPath; // NEO: MTD - [MultiTempDirectories] <-- Xanatos --
	CString	strTitle;
	CString	strComment;
	DWORD	color;
	UINT	prio;
	...
	UINT	iAdvA4AFMode;
	UINT	boost;
	bool	release; // NEO: SRS - [SmartReleaseSharing]
...
public:
	...
	static	int		m_iMiniMuleTransparency;
	// NEO: MOD END <-- Xanatos --
	...
	//static	UINT	maxsourceperfile; // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	static	DWORD	m_adwStatsColors[41]; // NEO: NS - [NeoStats] <-- Xanatos --
	...
	static	UINT	statsTransferGraphRatio; 
	static	UINT	statsSourceGraphRatio;
	// NEO: NS END <-- Xanatos --
	...
	static	UINT	m_uLastCategory; // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	...
	static	bool	dontcompressblocks; // NEO: MOD - [IsCompressible] <-- Xanatos --
	...
	static UINT		m_uRememberDownloadedFiles; // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
	...
	static	uint16	GetPort(bool bLokal = false);
#else
	...
	static	void	Add2SessionTransferData(UINT uClientID, UINT uClientPort, BOOL bFromPF, BOOL bUpDown, uint32 bytes, bool sentToFriend = false, bool sentToRelease = false);
#else
	...
	static	void	SetStatsMemoryGraphRatio(UINT in)	{ statsMemoryGraphRatio = in; }
	static	UINT	GetStatsMemoryGraphRatio()			{ return statsMemoryGraphRatio; }
	// NEO: SI END <-- Xanatos --
	// NEO: NS - [NeoStats] -- Xanatos -->
	static	void	SetStatsTransferGraphRatio(UINT in)	{ statsTransferGraphRatio = in; }
	static	UINT	GetStatsTransferGraphRatio()		{ return statsTransferGraphRatio; }
	static	void	SetStatsSourceGraphRatio(UINT in)	{ statsSourceGraphRatio = in; }
	static	UINT	GetStatsSourceGraphRatio()			{ return statsSourceGraphRatio; } 
	// NEO: NS END <-- Xanatos --
	...
	static	UINT	GetMaxConnections(bool bOriginal = false);
	static	UINT	GetMaxHalfConnections(bool bOriginal = false);
	// NEO: QS END <-- Xanatos --
	//static	UINT	GetMaxConnections()					{return maxconnections;}
	//static	UINT	GetMaxHalfConnections()				{return maxhalfconnections;}
	//static	UINT	GetMaxSourcePerFileDefault()		{return maxsourceperfile;} // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	// NEO: MOD - [MissingPrefs] -- Xanatos -->
	static	bool	GetMiniMuleAutoClose()				{return m_bMiniMuleAutoClose;}
	static	int		GetMiniMuleTransparency()			{return m_iMiniMuleTransparency;}
	// NEO: MOD END <-- Xanatos --
	...
	static	UINT	GetMaxConperFive(bool bOriginal = false); // NEO: QS - [QuickStart] <-- Xanatos --
	...
	static	int		InsertCat(Category_Struct* cat, bool share = false);
	static	int		GetCatCount()						{return catMap.GetCount() - catRes;}
	static	int		GetFullCatCount()					{return catMap.GetCount();}
	// NEO: NSC END <-- Xanatos --
	static	int		FindCategory(Category_Struct* cat); // NEO: FCFG - [FileConfiguration] <-- Xanatos --
	// NEO: NXC - [NewExtendedCategories] -- Xanatos --
	...
	static	const CString &GetCatPath(int index) { 
		if (index>=0 && index<catMap.GetCount()){
			if(catMap.GetAt(index)->strIncomingPath.IsEmpty())
				return m_strIncomingDir;
			return catMap.GetAt(index)->strIncomingPath;
		}else
			return m_strIncomingDir;
	}
	static	bool	GetResumeOnlyInCat(int index)		{if (index>=0 && index<catMap.GetCount()) return catMap.GetAt(index)->bResumeFileOnlyInSameCat;	else return false;}
	// NEO: NXC END <-- Xanatos --
	...
	//static	void	SetMaxSourcesPerFile(UINT in)		{maxsourceperfile=in;} // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	static	bool	GetDontCompressBlocks()				{return dontcompressblocks;} // NEO: MOD - [IsCompressible] <-- Xanatos --
	...
	static	UINT	IsRememberingDownloadedFiles()		{return m_uRememberDownloadedFiles;} // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
	...
	static	void	SetRememberDownloadedFiles(UINT nv)	{m_uRememberDownloadedFiles = nv;} // NEO: MOD - [RemoveUnusedAICHhashes] <-- Xanatos --
...
protected:
	...
	static	int		catRes; // NEO: NSC - [NeoSharedCategories] <-- Xanatos --
