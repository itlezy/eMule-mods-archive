...
BOOL CPreviewThread::Run()
{
	...
	try{
		...
		CString strExtension = CString(_tcsrchr(m_pPartfile->GetFileName(true), _T('.'))); // NEO: PP - [PasswordProtection] <-- Xanatos --
		//CString strPreviewName = m_pPartfile->GetTempPath() + m_pPartfile->GetFileName().Mid(0, 5) + _T("_preview") + strExtension;
		CString strPreviewName = m_pPartfile->GetTempPath() + m_pPartfile->GetFileName(true).Mid(0, 5) + _T("_preview") + strExtension; // NEO: PP - [PasswordProtection] <-- Xanatos --
...
int CPreviewApps::GetPreviewApp(const CPartFile* file)
{
	...
	LPCTSTR pszExt = PathFindExtension(file->GetFileName(true)); // NEO: PP - [PasswordProtection] <-- Xanatos --
	if (pszExt == NULL)
		return -1;
