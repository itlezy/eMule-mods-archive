...
void CScheduler::SaveOriginals() {
	...
	original_conshalf=thePrefs.GetMaxHalfConnections(true); // NEO: QS - [QuickStart] <-- Xanatos --
	//original_sources=thePrefs.GetMaxSourcePerFileDefault(); 
	original_sources=NeoPrefs.PartPrefs.GetSourceLimit(); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
	original_UlMode=NeoPrefs.GetUploadControl();
	original_UlMax=NeoPrefs.GetMaxBCUpload();
	original_UlMin=NeoPrefs.GetMinBCUpload();

	original_UpBasePing=NeoPrefs.GetBasePingUp();
	original_UpTolerance=NeoPrefs.GetPingUpTolerance();
	original_UpProzent=NeoPrefs.GetPingUpProzent();

	original_UpStream=NeoPrefs.GetMaxUpStream();

	original_DlMode=NeoPrefs.GetDownloadControl();
	original_DlMax=NeoPrefs.GetMaxBCDownload();
	original_DlMin=NeoPrefs.GetMinBCDownload();

	original_DownBasePing=NeoPrefs.GetBasePingDown();
	original_DownTolerance=NeoPrefs.GetPingDownTolerance();
	original_DownProzent=NeoPrefs.GetPingDownProzent();

	original_DownStream=NeoPrefs.GetMaxDownStream();
	
	original_ManageCons=NeoPrefs.IsManageConnections();
	original_ManageFactor=NeoPrefs.GetManageConnectionsFactor();

	original_IncludeOh=NeoPrefs.IsIncludeOverhead();
...
void CScheduler::RestoreOriginals() {
	...
	NeoPrefs.PartPrefs.SetSourceLimit(original_sources); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	thePrefs.SetMaxHalfConnections(original_conshalf); // NEO: QS - [QuickStart] <-- Xanatos --

#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
	NeoPrefs.SetUploadControl(original_UlMode);
	NeoPrefs.SetMaxBCUpload(original_UlMax);
	NeoPrefs.SetMinBCUpload(original_UlMin);

	NeoPrefs.SetBasePingUp(original_UpBasePing);
	NeoPrefs.SetPingUpTolerance(original_UpTolerance);
	NeoPrefs.SetPingUpProzent(original_UpProzent);

	NeoPrefs.SetMaxUpStream(original_UpStream);

	NeoPrefs.SetDownloadControl(original_DlMode);
	NeoPrefs.SetMaxBCDownload(original_DlMax);
	NeoPrefs.SetMinBCDownload(original_DlMin);

	NeoPrefs.SetBasePingDown(original_DownBasePing);
	NeoPrefs.SetPingDownTolerance(original_DownTolerance);
	NeoPrefs.SetPingDownProzent(original_DownProzent);

	NeoPrefs.SetMaxDownStream(original_DownStream);
	
	NeoPrefs.SetManageConnections(original_ManageCons);
	NeoPrefs.SetManageConnectionsFactor(original_ManageFactor);

	NeoPrefs.SetIncludeOverhead(original_IncludeOh);
#endif // NEO_BC // NEO: NBCs END <-- Xanatos --
}
	...
	for (int ai=0;ai<16;ai++) {
		...
		switch (schedule->actions[ai]) {
			...
			case 3 :
				...
				NeoPrefs.PartPrefs.SetSourceLimit(_tstoi(schedule->values[ai])); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
				if (makedefault)
			...
			// NEO: QS END <-- Xanatos --
#ifdef NEO_BC // NEO: NBCs - [NeoBandwidthControlSheduler] -- Xanatos -->
			case 9 : NeoPrefs.SetUploadControl(_tstoi(schedule->values[ai]));
				if (makedefault) original_UlMode=_tstoi(schedule->values[ai]);
				break;
			case 10 : NeoPrefs.SetMaxBCUpload((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_UlMax=(float)_wtof(schedule->values[ai]);
				break;
			case 11 : NeoPrefs.SetMinBCUpload((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_UlMin=(float)_wtof(schedule->values[ai]);
				break;

			case 12 : NeoPrefs.SetBasePingUp(_tstoi(schedule->values[ai]));
				if (makedefault) original_UpBasePing=_tstoi(schedule->values[ai]);
				break;
			case 13 : NeoPrefs.SetPingUpTolerance(_tstoi(schedule->values[ai]));
				if (makedefault) original_UpTolerance=_tstoi(schedule->values[ai]);
				break;
			case 14 : NeoPrefs.SetPingUpProzent(_tstoi(schedule->values[ai]));
				if (makedefault) original_UpProzent=_tstoi(schedule->values[ai]);
				break;

			case 15 : NeoPrefs.SetMaxUpStream((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_UpStream=(float)_wtof(schedule->values[ai]);
				break;


			case 16 : NeoPrefs.SetDownloadControl(_tstoi(schedule->values[ai]));
				if (makedefault) original_DlMode=_tstoi(schedule->values[ai]);
				break;
			case 17 : NeoPrefs.SetMaxBCDownload((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_DlMax=(float)_wtof(schedule->values[ai]);
				break;
			case 18 : NeoPrefs.SetMinBCDownload((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_DlMin=(float)_wtof(schedule->values[ai]);
				break;

			case 19 : NeoPrefs.SetBasePingDown(_tstoi(schedule->values[ai]));
				if (makedefault) original_DownBasePing=_tstoi(schedule->values[ai]);
				break;
			case 20 : NeoPrefs.SetPingDownTolerance(_tstoi(schedule->values[ai]));
				if (makedefault) original_DownTolerance=_tstoi(schedule->values[ai]);
				break;
			case 21 : NeoPrefs.SetPingDownProzent(_tstoi(schedule->values[ai]));
				if (makedefault) original_DownProzent=_tstoi(schedule->values[ai]);
				break;

			case 22 : NeoPrefs.SetMaxDownStream((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_DownStream=(float)_wtof(schedule->values[ai]);
				break;


			case 23 : NeoPrefs.SetManageConnections(I2B(_tstoi(schedule->values[ai])));
				if (makedefault) original_ManageCons=I2B(_tstoi(schedule->values[ai]));
				break;
			case 24 : NeoPrefs.SetManageConnectionsFactor((float)_tstoi(schedule->values[ai]));
				if (makedefault) original_ManageFactor=(float)_wtof(schedule->values[ai]);
				break;

			case 25 : NeoPrefs.SetIncludeOverhead(I2B(_tstoi(schedule->values[ai])));
