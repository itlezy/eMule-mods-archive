...
#include "emuledlg.h"
#include "Searchdlg.h"
#include "searchlist.h" // NEO: MLD - [ModelesDialogs] <-- Xanatos --
	...
	: CAbstractFile(copyfrom)
{
	...
	theApp.searchlist->m_Files_map.SetAt(this,1); // NEO: MLD - [ModelesDialogs] <-- Xanatos --
}
...
CSearchFile::~CSearchFile()
{
	...
	theApp.searchlist->m_Files_map.RemoveKey(this); // NEO: MLD - [ModelesDialogs] <-- Xanatos --
}
...
void CSearchFile::GetTooltipFileInfo(CString &info)
{
	CAbstractFile::GetTooltipFileInfo(info);

	info.AppendFormat(GetResString(IDS_X_RATING), GetRateString(m_uUserRating));
}
