...
UINT CSearchList::ProcessSearchAnswer(const uchar* in_packet, uint32 size, bool bOptUTF8,
									  ...
									  uint32 nClientIP, uint16 nClinetPort)
#else
UINT CSearchList::ProcessSearchAnswer(const uchar* in_packet, uint32 size, bool bOptUTF8,
									  uint32 nServerIP, uint16 nServerPort, bool* pbMoreResultsAvailable)
#endif //LANCAST // NEO: NLC END <-- Xanatos --
{
	...
	if(NeoPrefs.UseVoodooSearch(true))
	{
		...
		if(m_nCurED2KSearchID == NULL && theApp.voodoo->m_SearchMasterMap.Lookup(NULL,Master))
		{
			if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
			{
				Master.MasterSocket->SendSearchResult(Master.MasterID,SearchTypeEd2kServer,packet,nServerIP,nServerPort);
			}
			return 0;
		}
	}
	...
	for (UINT i = 0; i < results; i++){
		...
		if(nClientIP){
			ASSERT(nClinetPort);
			toadd->SetClientID(nClientIP);
			toadd->SetClientPort(nClinetPort);
			toadd->SetPreviewPossible( /*Sender->GetPreviewSupport() &&*/ ED2KFT_VIDEO == GetED2KFileTypeID(toadd->GetFileName()) );
		}
#endif //LANCAST // NEO: NLC END <-- Xanatos --
...
UINT CSearchList::ProcessUDPSearchAnswer(CFileDataIO& packet, bool bOptUTF8, uint32 nServerIP, uint16 nServerPort)
{
	...
	if(NeoPrefs.UseVoodooSearch(true))
	{
		...
		if(m_nCurED2KSearchID == NULL && theApp.voodoo->m_SearchMasterMap.Lookup(NULL,Master))
		{
			if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
			{
				Master.MasterSocket->SendSearchResult(Master.MasterID,SearchTypeEd2kGlobal,*((CSafeMemFile*)&packet),nServerIP,nServerPort);
			}
			return 0;
		}
	}
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP)
{
	...
	if(theApp.downloadqueue->IsFileExisting(toadd->GetFileHash(),false))
		theApp.downloadqueue->CahceUDPServer(toadd); 
	// NEO: XUC END <-- Xanatos --
										...
										uint64 size, LPCTSTR type, UINT uKadPublishInfo,  UINT numProperties, ...)
{
	...
	{
		SSearchMaster Master;
		if(theApp.voodoo->m_SearchMasterMap.Lookup(searchID,Master))
		{
			if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
			{
				Master.MasterSocket->SendSearchResult(Master.MasterID,SearchTypeKademlia,*temp);
			}
			delete temp;
			return;
		}
...
bool CSearchList::IsFileList(const CAbstractFile* file) const
{
	UINT val = 0;
	if (file)
		m_Files_map.Lookup(file,val);
	return val != 0;
}
// NEO: MLD END <-- Xanatos --