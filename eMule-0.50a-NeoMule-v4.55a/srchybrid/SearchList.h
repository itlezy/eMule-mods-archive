...
public:
	...
	//UINT	ProcessSearchAnswer(const uchar* packet, uint32 size, CUpDownClient* Sender, bool* pbMoreResultsAvailable, LPCTSTR pszDirectory = NULL); // NEO: XSF - [ExtendedSharedFiles] <-- Xanatos --
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
	UINT	ProcessSearchAnswer(const uchar* packet, uint32 size, bool bOptUTF8, uint32 nServerIP, uint16 nServerPort, bool* pbMoreResultsAvailable, uint32 nClientIP = 0, uint16 nClinetPort = 0);
#else
	...
	void	SetCurED2KSearchID(DWORD ID) {m_nCurED2KSearchID = ID;}
	DWORD	GetCurED2KSearchID() {return m_nCurED2KSearchID;}
	...
	bool			IsFileList(const CAbstractFile* file) const; // NEO: MLD - [ModelesDialogs] <-- Xanatos --
protected:
	// NEO: MLD - [ModelesDialogs] -- Xanatos -->
	friend class CSearchFile;
	CMap<const CAbstractFile*,const CAbstractFile*,UINT,UINT> m_Files_map; 
	// NEO: MLD END <-- Xanatos --
