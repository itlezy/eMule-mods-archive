...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	m_SearchFileMenu.EnableMenuItem(MP_SEARCHHASH, theApp.emuledlg->searchwnd->CanSearchRelatedFiles() ? MF_ENABLED : MF_GRAYED); // NEO: MOD - [SearchHash] <-- Xanatos --
	...
	if (selectedList.GetCount() > 0)
	{
		...
		switch (wParam)
		{
			...
			case MPG_ALTENTER:
			{
				...
				POSITION pos = selectedList.GetHeadPosition();
				while (pos)
					paFiles.Add(selectedList.GetNext(pos));
				//CFileDetailDialog sheet(&paFiles, 0, this);
				// NEO: NFDD END <-- Xanatos --
				//CSearchResultFileDetailSheet sheet(selectedList, 0, this);
				//sheet.DoModal();
				// NEO: MLD - [ModelesDialogs] -- Xanatos -->
				CFileDetailDialog* dlg = new CFileDetailDialog(&paFiles, 0, this);
				dlg->OpenDialog(); 
				// NEO: MLD END <-- Xanatos --
				return TRUE;
			}
			case MP_CMT:
			{
				...
				while (pos)
					paFiles.Add(selectedList.GetNext(pos));
				//CFileDetailDialog sheet(&paFiles, IDD_COMMENTLST, this);
				// NEO: NFDD END <-- Xanatos --
				//CSearchResultFileDetailSheet sheet(selectedList, IDD_COMMENTLST, this);
				//sheet.DoModal();
				// NEO: MLD - [ModelesDialogs] -- Xanatos -->
				CFileDetailDialog* dlg = new CFileDetailDialog(&paFiles, IDD_COMMENTLST, this);
				dlg->OpenDialog(); 
				// NEO: MLD END <-- Xanatos --
				return TRUE;
			}
			...
			case MP_SEARCHHASH:
				// just a shortcut for the user typing into the searchfield "ed2k::[filehash]"
...
void CSearchListCtrl::CreateMenues()
{
	...
	m_SearchFileMenu.AppendMenu(MF_STRING, MP_SEARCHHASH, GetResString(IDS_X_SEARCHHASH), _T("KadCurrentSearches")); // NEO: MOD - [SearchHash] <-- Xanatos --
}
...
void CSearchListCtrl::SetHighlightColors()
{
	...
	// NEO: MOD - [MoreSearchColors] -- Xanatos -->
	m_crSearchResultDownloading		= RGB(240,0,0);
	m_crSearchResultDownloadStopped = RGB(200,60,0);
	m_crSearchResultShareing		= RGB(40,180,120);
	m_crSearchResultKnown			= RGB(0,100,0);
	m_crSearchResultCancelled		= RGB(220,160,0);
	// NEO END <-- Xanatos --
