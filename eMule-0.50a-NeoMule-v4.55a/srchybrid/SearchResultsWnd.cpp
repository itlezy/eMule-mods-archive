...
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
#include "Neo/VooDoo/voodoo.h"
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --
...
BEGIN_MESSAGE_MAP(CSearchResultsWnd, CResizableFormView)
	...
	ON_NOTIFY(NM_CLICK, IDC_CATTAB2, OnNMClickCattab) // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
END_MESSAGE_MAP()
...
void CSearchResultsWnd::StartSearch(SSearchParams* pParams)
{
	switch (pParams->eType)
	{
		...
		case SearchTypeLancast:
#endif //LANCAST // NEO: NLC END
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
		{
			bool bRet = false;
			if(m_pwndParams->m_ctlVoodoo.GetCheck() != TRUE)
				bRet = StartNewSearch(pParams);
			if(m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE && NeoPrefs.UseVoodooSearch())
				bRet |= StartNewVoodooSearch(pParams, !bRet);
			if(!bRet)
				delete pParams;
		}
#else
		if(!StartNewSearch(pParams))
			delete pParams; // NEO: CI#1 - [CodeImprovement] 
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --
	    ...
	    if (theApp.serverconnect->IsConnected())
		{
			...
			else
			{
				...
				if(NeoPrefs.UseVoodooSearch(true))
				{
					...
					if(theApp.searchlist->GetCurED2KSearchID() == NULL && theApp.voodoo->m_SearchMasterMap.Lookup(NULL,Master))
					{
						if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
						{
							Master.MasterSocket->SendSearchState(NULL,SS_FINISH,SF_END);
							StopGlobalSearch();
							return;
						}
					}
...
#ifdef VOODOO // NEO: VOODOOss - [VoodooSearchForwarding] -- Xanatos -->
		if(NeoPrefs.UseVoodooSearch() && m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE)
			theApp.voodoo->ManifestSearchCommand(pParams->dwSearchID,SC_CANCEL);
#endif // VOODOO // NEO: VOODOOss END <-- Xanatos --
...
void CSearchResultsWnd::CancelEd2kSearch()
{
	...
	StopGlobalSearch(); // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --

	SearchCanceled(m_nEd2kSearchID);
}

void CSearchResultsWnd::StopGlobalSearch() // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
{
	...
	searchprogress.SetPos(0); // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
 ...
 // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
...
void CSearchResultsWnd::CancelKadSearch(UINT uSearchID)
{
	...
	{
		SSearchMaster Master;
		if(theApp.voodoo->m_SearchMasterMap.Lookup(uSearchID,Master))
		{
			//theApp.voodoo->m_SearchMasterMap.RemoveKey(uSearchID);
			Master.KillTimer = ::GetTickCount(); // kad results may aproach delayd, so wait some time
			if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
			{
				Master.MasterSocket->SendSearchState(Master.MasterID,SS_FINISH,SF_KAD);
			}
			return;
		}
	}
...
void CSearchResultsWnd::LocalEd2kSearchEnd(UINT count, bool bMoreResultsAvailable)
{
	...
	if(NeoPrefs.UseVoodooSearch(true))
	{
		...
		if(theApp.searchlist->GetCurED2KSearchID() == NULL && theApp.voodoo->m_SearchMasterMap.Lookup(NULL,Master))
		{
			if(theApp.voodoo->IsValidSocket(Master.MasterSocket))
			{
				Master.MasterSocket->SendSearchState(NULL,SS_FINISH,globsearch ? SF_HALTED : SF_DONE);
			}
			else
			{
				CancelSearch();
			}
			return; // stop the search here, the master will decide about the start of a global search
		}
	}
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --

	//if (!canceld && count > MAX_RESULTS)
	if (!canceld && count > (MAX_RESULTS * MAX_MORE_SEARCH_REQ)) // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
		CancelEd2kSearch();
	if (!canceld) {
		if (!globsearch)
			SearchCanceled(m_nEd2kSearchID);
...
{
	// NEO: EGS - [ExtendetGlobalSearch] -- Xanatos -->
	if (global_search_timer && count > (UINT)(MAX_RESULTS*(m_iSentMoreReq+1)))
	{ 
		VERIFY( KillTimer(global_search_timer) );
		global_search_timer = 0;
		m_pwndParams->m_ctlMore.EnableWindow(m_iSentMoreReq < MAX_MORE_SEARCH_REQ);
		m_pwndParams->m_ctlStart.EnableWindow(true); // X?
		if(m_iSentMoreReq >= MAX_MORE_SEARCH_REQ)
			CancelSearch();
	} 
	// NEO: EGS END <-- Xanatos --
...
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
void CSearchResultsWnd::HoldGlobalSearch()
{
	if (global_search_timer)
	{ 
		VERIFY( KillTimer(global_search_timer) );
...
void CSearchResultsWnd::FinishVoodooSearch(CVoodooSocket* slave, DWORD dwSearchID, uint32 uValue)
{
	switch(uValue){
		case SF_HALTED:
			if(theApp.searchlist->GetED2KResultCount() < (MAX_RESULTS * MAX_MORE_SEARCH_REQ))
				slave->SendSearchCommand(NULL,SC_MORE); // initial global search
			break;
		case SF_END: // no more servers no more results
			slave->m_bHaveMoreResults = false;
		case SF_DONE: // let the processor function set the more flag
			slave->SendSearchCommand(NULL,SC_CANCEL);
			if(slave->m_bHaveMoreResults)
				m_pwndParams->m_ctlMore.EnableWindow(m_iSentMoreReq < MAX_MORE_SEARCH_REQ);
		case SF_KAD:
		case SF_ERROR:
		default:
			SetInactiveSearchResultsIcon(dwSearchID ? dwSearchID : m_nEd2kSearchID);
	}
}

void CSearchResultsWnd::CheckGlobalSearch(CVoodooSocket* slave, UINT count)
{
	if (count > (UINT)(MAX_RESULTS*(m_iSentMoreReq+1))){
		slave->SendSearchCommand(NULL,SC_HOLD);
		slave->m_bHaveMoreResults = true;
		m_pwndParams->m_ctlMore.EnableWindow(m_iSentMoreReq < MAX_MORE_SEARCH_REQ);
		m_pwndParams->m_ctlStart.EnableWindow(true); // X?
		if(m_iSentMoreReq >= MAX_MORE_SEARCH_REQ)
		{
...
void CSearchResultsWnd::DownloadSelected(bool bPaused)
{
	...
	if (useCat == -1 && NeoPrefs.SelectCatForNewDL()) // user may have preselected
	{
		...
		if (useCat == -1) // only positiv values are valid if -1 is returned than the user canceled
			return;
	}
	// NEO: NXC END <-- Xanatos --
	...
	while (pos != NULL)
	{
		...
		if (iIndex >= 0)
		{
			...
			theApp.downloadqueue->AddSearchToDownload(&tempFile, bPaused, useCat); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
	...
	// NEO: NXC - [NewExtendedCategories] -- Xanatos -->
	if(bCreatedNewCat)
		theApp.downloadqueue->StartDLInEmptyCats(useCat);
	// NEO: NXC END <-- Xanatos --
}
...
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
	iml.Add(CTempIconLoader(_T("LANCAST"), 16, 16));
#endif //LANCAST // NEO: NLC END <-- Xanatos --
...
bool CSearchResultsWnd::StartNewSearch(SSearchParams* pParams)
{
	...
	if (pParams->eType == SearchTypeAutomatic){
		...
		if (!theApp.serverconnect->IsConnected() && Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsConnected())
			...
			//delete pParams; // NEO: CI#1 - [CodeImprovement] <-- Xanatos --
			return false;
		}
		else {
			// connected to both
...
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
	if (eSearchType == SearchTypeLancast)
	{
		if (!theApp.lancast->IsConnected())
		{
			AfxMessageBox(GetResString(IDS_X_ERR_LANCAST_NOTCONNECTED), MB_ICONWARNING);
			//delete pParams; // NEO: CI#1 - [CodeImprovement]
			return false;
		}
...
bool CSearchResultsWnd::DoNewEd2kSearch(SSearchParams* pParams)
{
	...
	{
		StartGlobalSearch(packet, bPacketUsing64Bit); // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
	}
	else{
		globsearch = false;
		delete packet;
...
void CSearchResultsWnd::StartGlobalSearch(Packet* packet, bool bPacketUsing64Bit) // NEO: EGS - [ExtendetGlobalSearch] <-- Xanatos --
{
...
bool CSearchResultsWnd::SearchMore()
{
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
	if(m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE && NeoPrefs.UseVoodooSearch())
		theApp.voodoo->ManifestSearchCommand(NULL,SC_MORE);
	if(m_pwndParams->m_ctlVoodoo.GetCheck() != TRUE)
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --
  ...
  if(globsearch){
	canceld = false;
	if (global_search_timer == 0)
		VERIFY( (global_search_timer = SetTimer(TimerGlobalSearch, UDPSEARCHSPEED, 0)) != NULL ); // NEO: ECFG - [EngineConfiguration] <-- Xanatos --
  }else{
  // NEO: EGS END <-- Xanatos --
	if (!theApp.serverconnect->IsConnected())
		return false;
...
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
bool CSearchResultsWnd::DoNewLanSearch(SSearchParams* pParams)
{
	CSafeMemFile data(100);
	if (!GetSearchPacket(&data, pParams, true, NULL) || data.GetLength() == 0)
		return false;
	...
	Packet* packet = new Packet(&data,OP_LANCASTPROT);
	packet->opcode = OP_FILESEARCH;

	theApp.lancast->BroadcastPacket(packet);
...
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
bool CSearchResultsWnd::StartNewVoodooSearch(SSearchParams* pParams, bool bNew)
{
	if(bNew){
		CStringA strResultType = pParams->strFileType;
		m_nEd2kSearchID++;
		pParams->dwSearchID = m_nEd2kSearchID;
		theApp.searchlist->NewSearch(&searchlistctrl, strResultType, m_nEd2kSearchID, pParams->eType, pParams->strExpression);
		theApp.searchlist->SetCurED2KSearchID(m_nEd2kSearchID);
		CreateNewTab(pParams);
	}

	theApp.voodoo->ManifestNewSearch(pParams);
...
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
	else if (pParams->eType == SearchTypeLancast)
		newitem.iImage = sriLan;
#endif //LANCAST // NEO: NLC END <-- Xanatos --
	else{
...
#ifdef VOODOO // NEO: VOODOOss - [VoodooSearchForwarding] -- Xanatos -->
	if(NeoPrefs.UseVoodooSearch() && m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE)
		theApp.voodoo->ManifestSearchCommand((nSearchID == m_nEd2kSearchID) ? NULL : nSearchID, SC_CANCEL);
#endif // VOODOO // NEO: VOODOOss END <-- Xanatos --
...
#ifdef VOODOO // NEO: VOODOOss - [VoodooSearchForwarding] -- Xanatos -->
	if(NeoPrefs.UseVoodooSearch() && m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE)
		theApp.voodoo->ManifestSearchCommand(NULL,SC_CANCEL);
#endif // VOODOO // NEO: VOODOOss END <-- Xanatos --
...
#ifdef VOODOO // NEO: VOODOOss - [VoodooSearchForwarding] -- Xanatos -->
			if(NeoPrefs.UseVoodooSearch() && m_pwndParams->m_ctlVoodoo.GetCheck() != FALSE)
				theApp.voodoo->ManifestSearchCommand(((const SSearchParams*)item.lParam)->dwSearchID,SC_CANCEL);
#endif // VOODOO // NEO: VOODOOss END <-- Xanatos --
			delete (SSearchParams*)item.lParam;
		}
	}
...
void CSearchResultsWnd::UpdateCatTabs()
{
	...
	for (int ix=0;ix<thePrefs.GetCatCount();ix++) {
		...
		CString label=thePrefs.GetCategory(ix)->strTitle; // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
		label.Replace(_T("&"),_T("&&"));
...
// NEO: NXC - [NewExtendedCategories] -- Xanatos -->
void CSearchResultsWnd::OnNMClickCattab(NMHDR* /*pNMHDR*/, LRESULT *pResult)
{
	POINT point;
	::GetCursorPos(&point);
...
void CSearchResultsWnd::SearchRelatedFiles(CPtrList& listFiles, bool bHash) // NEO: MOD - [SearchHash] <-- Xanatos --
{
	SSearchParams* pParams = new SSearchParams;
	//pParams->strExpression = _T("related");
	pParams->strExpression = bHash ? _T("ed2k") : _T("related"); // NEO: MOD - [SearchHash] <-- Xanatos --
