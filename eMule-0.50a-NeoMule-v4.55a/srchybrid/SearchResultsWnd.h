...
public:
	...
	void	StartGlobalSearch(Packet* packet, bool bPacketUsing64Bit); 
	// NEO: EGS END <-- Xanatos --
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
	void	HoldGlobalSearch();
	void	FinishVoodooSearch(CVoodooSocket* slave, DWORD dwSearchID, uint32 uValue = 0);
	void	CheckGlobalSearch(CVoodooSocket* slave, UINT count);
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --

#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
	bool	DoNewLanSearch(SSearchParams* pParams);
...
protected:
...
#ifdef VOODOO // NEO: VOODOOs - [VoodooSearchForwarding] -- Xanatos -->
	bool StartNewVoodooSearch(SSearchParams* pParams, bool bNew = true);
#endif // VOODOO // NEO: VOODOOs END <-- Xanatos --
	void SearchStarted();
	...
	afx_msg void CSearchResultsWnd::OnNMClickCattab(NMHDR *pNMHDR, LRESULT *pResult); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
