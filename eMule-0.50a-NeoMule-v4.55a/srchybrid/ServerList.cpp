...
#include "ServerWnd.h"
#include "Log.h"
#include "SharedFileList.h" // NEO: PSA - [PreferShareAll] <-- Xanatos --
...
void CServerList::ServerStats()
{
	...
	if (theApp.GetConState(true) && theApp.serverconnect->IsUDPSocketAvailable() && list.GetCount() > 0) // NEO: NCC - [NeoConnectionChecker] <-- Xanatos --
	{
		...
		if (ping_server->GetFailedCount() >= thePrefs.GetDeadServerRetries()) {
			// NEO: MOD - [DontRemoveStaticServers] -- Xanatos -->
			if (ping_server->IsStaticMember() && NeoPrefs.IsDontRemoveStaticServers())
				return;
			// NEO: MOD END <-- Xanatos --
			theApp.emuledlg->serverwnd->serverlistctrl.RemoveServer(ping_server);
			return;
		}
		ServerStats(ping_server); // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
	}
}

void CServerList::ServerStats(CServer* ping_server, bool bNoCrypt, bool bForce) // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
{
		uint32 tNow = (uint32)time(NULL); // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
		srand(tNow);
		...
		if (!bNoCrypt && !ping_server->GetCryptPingReplyPending() && ((theApp.GetPublicIP() != 0 || bForce) && thePrefs.IsServerCryptLayerUDPEnabled())){ // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
		...
		else if (bNoCrypt || ping_server->GetCryptPingReplyPending() || theApp.GetPublicIP() == 0 || !thePrefs.IsServerCryptLayerUDPEnabled()){ // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
