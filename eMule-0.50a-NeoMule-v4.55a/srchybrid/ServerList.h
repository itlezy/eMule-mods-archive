...
public:
	...
	bool		IsValidServer(CServer* server) const { return list.Find(server) != NULL; } // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
	...
	void		ServerStats();
	void		ServerStats(CServer* ping_server, bool bNoCrypt = false, bool bForce = false); // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
	...
	void		PushBackNoShare(); // NEO: PSA - [PreferShareAll] <-- Xanatos --
