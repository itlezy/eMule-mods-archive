	...
	try
	{
		switch (opcode)
		{
				...
				while (!message.IsEmpty())
				{
					...
					if (_tcsnicmp(message, _T("server version"), 14) == 0){
						...
						if (pServer){
							...
							pServer->ResetServerBlacklists();
							cur_server->SetServerBlacklists(pServer->ServerBlacklists());
							// NEO: KLC END <-- Xanatos --
			...
			case OP_IDCHANGE:{
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
							const DWORD dwKnownBits = SRV_TCPFLG_COMPRESSION | SRV_TCPFLG_NEWTAGS | SRV_TCPFLG_UNICODE | SRV_TCPFLG_RELATEDSEARCH | SRV_TCPFLG_TYPETAGINTEGER | SRV_TCPFLG_LARGEFILES | SRV_TCPFLG_TCPOBFUSCATION | SRV_TCPFLG_NATTRAVERSAL;
#else
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
							if (dwFlags & SRV_TCPFLG_NATTRAVERSAL)
								strInfo.AppendFormat(_T("  NatTraversal=1"));
#endif //NATTUNNELING // NEO: NATT END <-- Xanatos --
							Debug(_T("%s\n"), strInfo);
						}
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
				uint32 dwServerReportedPort = 0;
				if (size >= 24)
					dwServerReportedPort = *((uint32*)(packet + 20));
#endif //NATTUNNELING // NEO: NATT END <-- Xanatos --
				...
				// NEO: RLD - [ReconnectOnLowID] -- Xanatos -->
				if (NeoPrefs.ReConnectOnLowID())
					if (IsLowID(la->clientid)){
						if (!(theApp.serverconnect->GetLowIdRetris() > NeoPrefs.GetLowIDRetrys())){
							SetConnectionState(CS_ERROR);
							ModLog(LOG_STATUSBAR, GetResString(IDS_X_RECONECT_ON_LOW_ID));
							theApp.serverconnect->AddLowIdRetried();
							break;
						}
					}else
						theApp.serverconnect->ResetLowIdRetried();
				// NEO: RLD END <-- Xanatos --
				...
				pServer->SetServerKeyUDP(pServer->GetServerKeyUDP(true)); // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --

#ifdef NATTUNNELING // NEO: RTP - [ReuseTCPPort] -- Xanatos -->
				if(dwServerReportedPort != 0)
					theApp.SetPublicPort((uint16)dwServerReportedPort);
			...
			{
				if (thePrefs.GetDebugServerTCPLevel() > 0)
					Debug(_T("ServerMsg - OP_Nat_CallbackRequested: %s\n"), (size >= 27) ? _T("With Cryptflag and Userhash") : _T("Without Cryptflag and Userhash"));

				uint32 uIP = PeekUInt32(packet);
				uint16 uPort = PeekUInt16(packet+4);

				uint32 uReqID = PeekUInt32(packet+6);

				//Obfuscation support
				uint8 byCryptOptions = 0;
				uchar achUserHash[16];
				if (size >= 27){
					byCryptOptions = packet[10];
					md4cpy(achUserHash, packet + 11);
				}

				if(CNatTunnel* tunnel = theApp.natmanager->HandleCallbackRequest((uchar*)&uReqID,ed2k_id,uIP,uPort,byCryptOptions,achUserHash))
				{
					uint32 ourID = theApp.serverconnect->GetClientID();
					// Note: due to the delayed callback we have to request a ping answer in order to unset the sheduled callback
					theApp.natmanager->SendNatPing(uIP,uPort,true,(uchar*)&ourID,ed2k_id);

					if(theApp.serverconnect->IsConnected() && uReqID != 0){ // uReqID == 0 server knows our port and send the requester request on its own
						tunnel->PrepareCallback((uchar*)&uReqID,ed2k_id);
					}
				}
				break;
			}
...
{
	// NEO: KLC - [KhaosLugdunumCredits] -- Xanatos -->
	if (cur_server && connectionstate == CS_CONNECTED && newstate != CS_CONNECTED)
		cur_server->SetDisconnectedTick();
	else if (cur_server && newstate == CS_CONNECTED && connectionstate != CS_CONNECTED)
	{
		if (cur_server->GetDisconnectedTick())
		{
			uint32 nNewCredits = cur_server->GetServerCredits() + ((GetTickCount() - cur_server->GetDisconnectedTick()) / 1000);
			if (nNewCredits < DEFAULT_CREDITS)
				cur_server->SetServerCredits(nNewCredits);
			else
				cur_server->SetServerCredits(DEFAULT_CREDITS);
		}

		cur_server->ResetServerBlacklists();
		if(NeoPrefs.UseLugdunumCredits() && (cur_server->ServerBlacklists() || NeoPrefs.UseLugdunumCredits() == 2))
			DebugLog(GetResString(IDS_X_KLF_ENABLE), cur_server->GetServerCredits());
		else
			DebugLog(GetResString(IDS_X_KLF_DISABLE));
	}
	// NEO: KLC END <-- Xanatos --
