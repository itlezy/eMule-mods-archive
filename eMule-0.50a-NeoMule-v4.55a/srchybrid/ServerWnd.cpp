	...
	: CResizableDialog(CServerWnd::IDD, pParent)
{
	...
	modlogbox = new CHTRichEditCtrl; // NEO: ML - [ModLog] <-- Xanatos --
...
CServerWnd::~CServerWnd()
{
	...
	delete debuglog;
	delete modlogbox; // NEO: ML - [ModLog] <-- Xanatos --
	delete logbox;
	delete servermsgbox;
}
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		...
		servermsgbox->AppendText(theApp.GetAppTitle()+_T("\n")); // NEO: NV - [NeoVersion] <-- Xanatos --
	...
	GetDlgItem(IDC_MOD_LOG)->GetWindowRect(rect);
	GetDlgItem(IDC_MOD_LOG)->DestroyWindow();
	::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rect, 2);
	if (modlogbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_MOD_LOG)){
		modlogbox->SetProfileSkinKey(_T("ModLog"));
		modlogbox->ModifyStyleEx(0, WS_EX_STATICEDGE, SWP_FRAMECHANGED);
		modlogbox->SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELONG(3, 3));
		if (theApp.m_fontLog.m_hObject)
			modlogbox->SetFont(&theApp.m_fontLog);
		modlogbox->ApplySkin();
		modlogbox->SetTitle(GetResString(IDS_SV_LOG));
		modlogbox->SetAutoURLDetect(FALSE);
	}
	// NEO: ML END <-- Xanatos --
	...
	// NEO: ML - [ModLog] -- Xanatos -->
	name = GetResString(IDS_X_SV_MOD_LOG);
	name.Replace(_T("&"), _T("&&"));
	newitem.mask = TCIF_TEXT | TCIF_IMAGE;
	newitem.pszText = const_cast<LPTSTR>((LPCTSTR)name);
	newitem.iImage = 0;
	VERIFY( StatusSelector.InsertItem(StatusSelector.GetItemCount(), &newitem) == PaneModLog );
	// NEO: ML END <-- Xanatos --
	...
	AddAnchor(*modlogbox, MIDDLE_LEFT, BOTTOM_RIGHT); // NEO: ML - [ModLog] <-- Xanatos --
	...
	modlogbox->ShowWindow(SW_HIDE); // NEO: ML - [ModLog] <-- Xanatos --
...
void CServerWnd::Localize()
{
	...
	item.mask = TCIF_TEXT;
	item.pszText = const_cast<LPTSTR>((LPCTSTR)name);
	StatusSelector.SetItem(PaneModLog, &item);
	// NEO: ML END <-- Xanatos --
...
void CServerWnd::OnBnClickedResetLog()
{
	...
	// NEO: ML - [ModLog] -- Xanatos -->
	if (cur_sel == PaneModLog)
	{
		theApp.emuledlg->ResetModLog();
		theApp.emuledlg->statusbar->SetText(_T(""), SBarLog, 0);
	}
	// NEO: ML END <-- Xanatos --
	if (cur_sel == PaneLog)
...
void CServerWnd::UpdateLogTabSelection()
{
	...
	if (cur_sel == PaneModLog)
	{
		...
		servermsgbox->ShowWindow(SW_HIDE);
		logbox->ShowWindow(SW_HIDE);
		modlogbox->ShowWindow(SW_SHOW);
		modlogbox->Invalidate();
		if (modlogbox->IsAutoScroll() && (StatusSelector.GetItemState(cur_sel, TCIS_HIGHLIGHTED) & TCIS_HIGHLIGHTED))
			modlogbox->ScrollToLastLine(true);
		modlogbox->Invalidate();
		StatusSelector.HighlightItem(cur_sel, FALSE);
	}
	// NEO: ML END <-- Xanatos --
...
#ifdef MOD_VERSION // NEO: NV - [NeoVersion] -- Xanatos -->
			strUrl = _T("http://sourceforge.net/projects/neomule/");
#else // NEO: NV END <-- Xanatos --
			// MOD Note: Do not remove this part - Merkur
...
void CServerWnd::DoResize(int delta)
{
	...
	CSplitterControl::ChangeHeight(modlogbox, -delta, CW_BOTTOMALIGN); // NEO: ML - [ModLog] <-- Xanatos --
...
void CServerWnd::InitSplitter()
{
	...
	// NEO: ML - [ModLog] -- Xanatos -->
	modlogbox->GetWindowRect(rcDlgItem);
	ScreenToClient(rcDlgItem);
	rcDlgItem.top=splitpos+35;
	rcDlgItem.bottom = rcWnd.bottom-12;
	modlogbox->MoveWindow(rcDlgItem);
	// NEO: ML END <-- Xanatos --
...
void CServerWnd::ReattachAnchors()
{
	...
	RemoveAnchor(*logbox);
	RemoveAnchor(*modlogbox); // NEO: ML - [ModLog] <-- Xanatos --
	...
	AddAnchor(*modlogbox,  CSize(0, thePrefs.GetSplitterbarPositionServer()), BOTTOM_RIGHT); // NEO: ML - [ModLog] <-- Xanatos --
