...
class CServer;
class CCollection;
class CLocalFilesDialog; // NEO: XSF - [ExtendedSharedFiles] <-- Xanatos --
...
public:
	...
	void	AddFilesFromDirectory(const CString& rstrDirectory, bool bWithSubdir = false); // NEO: SSD - [ShareSubDirectories] <-- Xanatos --
	void	AddFileToShare(CFileFind &ff, CString realDirName, const CString rstrDirectory); // NEO: SSF - [ShareSingleFiles] <-- Xanatos --
	...
	CMap<CCKey,const CCKey&,CKnownFile*,CKnownFile*>* GetFileMap() {return &m_Files_map;} // NEO: MOD - [GetFileMap] <-- Xanatos --
	// NEO: XCk - [KnownComments] -- Xanatos -->
	uint32  GetLastPublishKadNotes()				{return m_lastPublishKadNotes;} 
	void	SetLastPublishKadNotes(uint32 tNext)	{m_lastPublishKadNotes = tNext;} 
	// NEO: XCk END <-- Xanatos --
	// NEO: XSF - [ExtendedSharedFiles] -- Xanatos -->
	void	ShowLocalFilesDialog(bool reloadOnly = false);
	void	LocalFilesDialogClosed();
	// NEO: XSF END <-- Xanatos --
...
private:
	...
	CLocalFilesDialog*	dialog; // NEO: XSF - [ExtendedSharedFiles] <-- Xanatos --
	...
	uint32 m_lastReload; // NEO: MOD - [RefreshShared] <-- Xanatos --
};
