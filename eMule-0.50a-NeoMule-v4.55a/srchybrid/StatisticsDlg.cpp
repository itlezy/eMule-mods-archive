...
CStatisticsDlg::CStatisticsDlg(CWnd* pParent /*=NULL*/)
	...
	, m_DownloadOMeter(7)
	, m_UploadOMeter(9) // NEO: BM - [BandwidthModeration]
	, m_Statistics(9) // NEO: SI - [SysInfo]
	// NEO: NS END <-- Xanatos --
	//, m_DownloadOMeter(3)
	//, m_Statistics(4)
	//, m_UploadOMeter(5)
...
BOOL CStatisticsDlg::OnInitDialog()
{
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_Statistics.SetTrendRatio(1, thePrefs.GetStatsTransferGraphRatio()); 
	m_Statistics.SetTrendRatio(2, thePrefs.GetStatsTransferGraphRatio());
	m_Statistics.SetTrendRatio(3, thePrefs.GetStatsTransferGraphRatio());
	// NEO: SI - [SysInfo]
	//m_Statistics.SetTrendRatio(4, );
	m_Statistics.SetTrendRatio(5, thePrefs.GetStatsMemoryGraphRatio());
	// NEO: SI END
	m_Statistics.SetTrendRatio(6, thePrefs.GetStatsSourceGraphRatio()*10);
	m_Statistics.SetTrendRatio(7, thePrefs.GetStatsSourceGraphRatio()*10);
	//m_Statistics.SetTrendRatio(8, );
	// NEO: NS END <-- Xanatos --
...
void CStatisticsDlg::RepaintMeters() 
{
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_DownloadOMeter.SetPlotColor(thePrefs.GetStatsColor(15), 3);	// Download current (excl. overhead)
	m_DownloadOMeter.SetPlotColor(thePrefs.GetStatsColor(20), 4);	// adapter
	m_DownloadOMeter.SetPlotColor(thePrefs.GetStatsColor(16), 5);	// Limit
	m_DownloadOMeter.SetPlotColor(thePrefs.GetStatsColor(27), 6);	// Download voodoo // NEO: VOODOO - [UniversalPartfileInterface]
	// NEO: NS END <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(13), 4);		// adapter
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(17), 5);		// Limit 
 #ifdef BW_MOD// NEO: BM - [BandwidthModeration]
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(23), 7);		// Upload release
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(24), 8);		// Upload friends
 #endif // BW_MOD // NEO: BM END
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(28), 6);		// Download voodoo // NEO: VOODOO - [UniversalPartfileInterface]
	// NEO: NS END <-- Xanatos --
	...
	m_Statistics.SetPlotColor(thePrefs.GetStatsColor(18), 4);
	m_Statistics.SetPlotColor(thePrefs.GetStatsColor(19), 5);
	// NEO: SI END <-- Xanatos --
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_Statistics.SetPlotColor(thePrefs.GetStatsColor(25), 6);
	m_Statistics.SetPlotColor(thePrefs.GetStatsColor(26), 7);
	m_Statistics.SetPlotColor(thePrefs.GetStatsColor(29), 8);
	// NEO: NS END <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_DownloadOMeter.SetLegendLabel(GetResString(IDS_X_ST_DLSLOTSOVERHEAD), 3); // Download current (incl. overhead)
	m_DownloadOMeter.SetLegendLabel(GetResString(IDS_X_ST_DLADAPTER), 4);		// Download Adapter
	m_DownloadOMeter.SetLegendLabel(GetResString(IDS_X_ST_DLIMIT), 5);			// Limit
	m_DownloadOMeter.SetLegendLabel(GetResString(IDS_X_ST_DVOODOO), 6);			// Download voodoo // NEO: VOODOO - [UniversalPartfileInterface]
	// NEO: NS END <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_ULSLOTSOVERHEAD), 3);	// Upload current (excl. overhead)
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_ULADAPTER), 4);			// Upload Adapter
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_ULIMIT), 5);			// Limit 
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_UVOODOO), 6);			// Upload voodoo // NEO: VOODOO - [UniversalPartfileInterface]
#ifdef BW_MOD// NEO: BM - [BandwidthModeration]
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_URELEASE), 7);			// Upload release
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_X_ST_UFRIENDS), 8);			// Upload friends
#endif // BW_MOD // NEO: BM END
	// NEO: NS END <-- Xanatos --
	...
	// NEO: NS - [NeoStats] -- Xanatos -->
	if(thePrefs.GetStatsTransferGraphRatio() > 1){
		Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_ST_ACTIVEU_ZZ), thePrefs.GetStatsTransferGraphRatio());
		m_Statistics.SetLegendLabel(Buffer, 1);
		Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_SP_TOTALUL), thePrefs.GetStatsTransferGraphRatio());
		m_Statistics.SetLegendLabel(Buffer, 2);
		Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_ST_ACTIVED), thePrefs.GetStatsTransferGraphRatio());
		m_Statistics.SetLegendLabel(Buffer, 3);
	}else{
	...
	// NEO: SI - [SysInfo]
	//Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_ST_ACTIVED), );
	m_Statistics.SetLegendLabel(GetResString(IDS_X_ST_CPU), 4);
	if(thePrefs.GetStatsMemoryGraphRatio() > 1){
		Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_X_ST_MEM), thePrefs.GetStatsMemoryGraphRatio());
		m_Statistics.SetLegendLabel(Buffer, 5);
	}else{
		m_Statistics.SetLegendLabel(GetResString(IDS_X_ST_MEM), 5);
	}
	// NEO: SI END
	Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_X_ST_SRC), thePrefs.GetStatsSourceGraphRatio()*10);
	m_Statistics.SetLegendLabel(Buffer, 6);
	Buffer.Format(_T("%s (1:%u)"), GetResString(IDS_X_ST_QUEUE), thePrefs.GetStatsSourceGraphRatio()*10);
	m_Statistics.SetLegendLabel(Buffer, 7);
	m_Statistics.SetLegendLabel(GetResString(IDS_X_ST_PING), 8);
	// NEO: NS END <-- Xanatos --
}
...
void CStatisticsDlg::SetCurrentRate(float uploadrate, float downloadrate)
{
	...
	m_dPlotDataDown[2] = theApp.bandwidthControl->GetCurrStatsDataDownload();
	// current rate (incl. overhead)
	m_dPlotDataDown[3] = theApp.bandwidthControl->GetCurrStatsDownload();
	// current rate to friends
	m_dPlotDataDown[4] = theApp.bandwidthControl->GetCurrStatsGlobalDownload();
	// current limit
	m_dPlotDataDown[5] = theApp.bandwidthControl->GetCurrStatsDownloadLimit();
#ifdef VOODOO // NEO: VOODOO - [UniversalPartfileInterface]
	m_dPlotDataDown[6] = ((float)theApp.voodoo->GetDownDatarate() / 1024);
#else
	m_dPlotDataDown[6] = 0; 
	...
	// Upload
	double m_dPlotDataUp[9];
	m_dPlotDataUp[0] = theStats.GetAvgUploadRate(AVG_SESSION);
	m_dPlotDataUp[1] = theStats.GetAvgUploadRate(AVG_TIME);
	// current rate to network (standardPackets + controlPackets)
	m_dPlotDataUp[2] = theApp.bandwidthControl->GetCurrStatsDataUpload();
	// current rate (incl. overhead)
	m_dPlotDataUp[3] = theApp.bandwidthControl->GetCurrStatsUpload();
	// current rate to friends
	m_dPlotDataUp[4] = theApp.bandwidthControl->GetCurrStatsGlobalUpload();
	// current limit
	m_dPlotDataUp[5] = theApp.bandwidthControl->GetCurrStatsUploadLimit();
#ifdef VOODOO // NEO: VOODOO - [UniversalPartfileInterface]
	m_dPlotDataUp[6] = ((float)theApp.voodoo->GetUpDatarate() / 1024);
#else
	m_dPlotDataUp[6] = 0;
#endif // VOODOO // NEO: VOODOO END
 #ifdef BW_MOD// NEO: BM - [BandwidthModeration]
	m_dPlotDataUp[7] = theApp.bandwidthControl->GetCurrStatsReleaseUpload();
	m_dPlotDataUp[8] = theApp.bandwidthControl->GetCurrStatsFriendUpload();
 #else
	m_dPlotDataUp[7] = 0;
	m_dPlotDataUp[8] = 0;
 #endif // BW_MOD // NEO: BM END
	...
	double m_dPlotDataDown[6]; // NEO: NS - [NeoStats] <-- Xanatos --
	m_dPlotDataDown[0] = theStats.GetAvgDownloadRate(AVG_SESSION);
	m_dPlotDataDown[1] = theStats.GetAvgDownloadRate(AVG_TIME);
	m_dPlotDataDown[2] = downloadrate;
	for(int i=3;i<6) m_dPlotDataDown[i] = 0; // NEO: NS - [NeoStats] <-- Xanatos --
	m_DownloadOMeter.AppendPoints(m_dPlotDataDown);

	// Upload
	double m_dPlotDataUp[6]; // NEO: NS - [NeoStats] <-- Xanatos --
	m_dPlotDataUp[0] = theStats.GetAvgUploadRate(AVG_SESSION);
	m_dPlotDataUp[1] = theStats.GetAvgUploadRate(AVG_TIME);
	// current rate to network (standardPackets + controlPackets)
	...
	// current rate to friends
	for(int i=3;i<6) m_dPlotDataUp[i] = 0; // NEO: NS - [NeoStats] <-- Xanatos --
	...
	double m_dPlotDataMore[9]; // NEO: NS - [NeoStats] <-- Xanatos --
	...
	// NEO: SI - [SysInfo] -- Xanatos -->
	m_dPlotDataMore[4] = NeoPrefs.DrawSysInfoGraph() ? (double)(theApp.sysinfo->GetCpuUsage()*thePrefs.GetStatsMax()/100) : 0;
	m_dPlotDataMore[5] = NeoPrefs.DrawSysInfoGraph() ? (double)(theApp.sysinfo->GetMemoryUsage()/1024) : 0;
	// NEO: SI END <-- Xanatos --
	// NEO: NS - [NeoStats] -- Xanatos -->
	m_dPlotDataMore[6] = theApp.downloadqueue->GetGlobalSourceCount();
	m_dPlotDataMore[7] = theApp.uploadqueue->GetWaitingUserCount();
#ifdef NEO_BC // NEO: NBC - [NeoBandwidthControl]
	if (NeoPrefs.IsCheckConnection() || NeoPrefs.IsUSSEnabled() || NeoPrefs.IsDSSEnabled())
		m_dPlotDataMore[8] = theApp.bandwidthControl->GetCurrStatsPing();
	else
#endif // NEO_BC // NEO: NBC END
		m_dPlotDataMore[8] = 0.0;
	// NEO: NS END <-- Xanatos --
		...
		if (forceUpdate || stattree.IsExpanded(h_download)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				{ 
					...
					cbuffer.Format( _T("%s: %u") , GetResString(IDS_X_XS_WAITCBK) , myStats.a[30] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
#endif //NATTUNNELING // NEO: XSB END <-- Xanatos --

					// NEO: TCR - [TCPConnectionRetry] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString(IDS_X_CONNECTION_RETRY) , myStats.a[23] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
					// NEO: TCR END <-- Xanatos --

#ifdef NEO_SS // NEO: NSS - [NeoSourceStorage] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString(IDS_X_LOADED) , myStats.a[25] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
#endif // NEO_SS // NEO: NSS END <-- Xanatos --

					// NEO: XSC - [ExtremeSourceCache] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString(IDS_X_CACHED) , myStats.a[24] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
					// NEO: XSC END <-- Xanatos --
					...
					i++;
#ifdef NEO_SS // NEO: NSS - [NeoSourceStorage] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString( IDS_X_VIASTORAGE ) , myStats.a[25] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
#endif // NEO_SS // NEO: NSS END <-- Xanatos --
#ifdef LANCAST // NEO: NLC - [NeoLanCast] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString( IDS_X_VIALANCAST ) , myStats.a[28] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
#endif //LANCAST // NEO: NLC END <-- Xanatos --
#ifdef VOODOO // NEO: VOODOOx - [VoodooSourceExchange] -- Xanatos -->
					cbuffer.Format( _T("%s: %u") , GetResString( IDS_X_VIAVOODOO ) , myStats.a[29] );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				theApp.clientlist->ReleaseModStatistics(&clientMods); // NEO: MIDS - [ModIDStat] <-- Xanatos --
...
void CStatisticsDlg::SetARange(bool SetDownload, int maxValue)
{
	if (SetDownload) 
	{
		...
		// NEO: NS - [NeoStats] -- Xanatos -->
		m_DownloadOMeter.SetRange(0, maxValue, 3);
		m_DownloadOMeter.SetRange(0, maxValue, 4);
		m_DownloadOMeter.SetRange(0, maxValue, 5);
		m_DownloadOMeter.SetRange(0, maxValue, 6); // NEO: VOODOO - [UniversalPartfileInterface]
		// NEO: NS END <-- Xanatos --
	}
	else
	{
		...
		// NEO: NS - [NeoStats] -- Xanatos -->
		m_UploadOMeter.SetRange(0, maxValue, 5);
		m_UploadOMeter.SetRange(0, maxValue, 6); // NEO: VOODOO - [UniversalPartfileInterface]
 #ifdef BW_MOD// NEO: BM - [BandwidthModeration]
		m_UploadOMeter.SetRange(0, maxValue, 7);
		m_UploadOMeter.SetRange(0, maxValue, 8);
 #endif // BW_MOD // NEO: BM END
		// NEO: NS END <-- Xanatos --
	}
}
