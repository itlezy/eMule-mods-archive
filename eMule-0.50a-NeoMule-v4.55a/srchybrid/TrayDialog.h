...
public:
	...
	BOOL TrayUpdate();
	//BOOL TrayShow();
	BOOL TrayShow(bool bStatic = false); // NEO: STI - [StaticTray] <-- Xanatos --
	//BOOL TrayHide();
	BOOL TrayHide(bool bStatic = false); // NEO: STI - [StaticTray] <-- Xanatos --
	virtual BOOL ShowWindow(int nCmdShow) = 0; // NEO: TPP - [TrayPasswordProtection] <-- Xanatos --
