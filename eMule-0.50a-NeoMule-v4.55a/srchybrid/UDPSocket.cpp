...
CUDPSocket::CUDPSocket()
{
	...
	m_port = 0; // NEO: MOD - [BindToAdapter] <-- Xanatos --
}
...
bool CUDPSocket::Create()
{
	if (thePrefs.GetServerUDPPort())
	{
		...
		if (!CAsyncSocket::Create(thePrefs.GetServerUDPPort()==0xFFFF ? 0 : thePrefs.GetServerUDPPort(), SOCK_DGRAM, FD_READ | FD_WRITE, theApp.GetBindAddress())){ // NEO: MOD - [BindToAdapter] <-- Xanatos --
...
bool CUDPSocket::Rebind(){
	...
	//if (thePrefs.GetServerUDPPort()==m_port)
	//	return false;
...
#ifdef NEO_DBT // NEO: NDBT - [NeoDownloadBandwidthThrottler] -- Xanatos -->
int	CUDPSocket::Receive(uint32 size, UINT hyperreceiving)
{
...
bool CUDPSocket::ProcessData(bool /*ignore*/){
	receiveLocker.Lock();
...
void CUDPSocket::OnReceive(int nErrorCode)
{
...
#endif // NEO_DBT // NEO: NDBT END <-- Xanatos --
	{
		...
		ProcessReceivedData(buffer,length,sockAddr,iSockAddrLen,dwError);
		// NEO: NDBT END <-- Xanatos --
	}
}

void CUDPSocket::ProcessReceivedData(BYTE* buffer, int length, SOCKADDR_IN &sockAddr, int iSockAddrLen, DWORD dwError) // NEO: NDBT - [NeoDownloadBandwidthThrottler] <-- Xanatos --
{
	BYTE* pBuffer = buffer; // NEO: NDBT - [NeoDownloadBandwidthThrottler] <-- Xanatos --
	if (length != SOCKET_ERROR)
	{
 			...
 			case OP_GLOBSERVSTATRES:{
				...
				if (size >= 28){
					...
					if (thePrefs.GetDebugServerUDPLevel() > 0){
						CString strInfo;
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
						const DWORD dwKnownBits = SRV_UDPFLG_EXT_GETSOURCES | SRV_UDPFLG_EXT_GETFILES | SRV_UDPFLG_NEWTAGS | SRV_UDPFLG_UNICODE | SRV_UDPFLG_EXT_GETSOURCES2 | SRV_UDPFLG_LARGEFILES | SRV_UDPFLG_UDPOBFUSCATION | SRV_UDPFLG_TCPOBFUSCATION | SRV_UDPFLG_NATTRAVERSAL;
#else
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
						if (uUDPFlags & SRV_UDPFLG_NATTRAVERSAL)
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
			// Note: we send the udp callbacks only to a server we are connected to so we will never use the dns queue for this packets
			SendBuffer(nIP, pServerPacket->m_nPort, pServerPacket->m_pPacket, pServerPacket->m_uSize, false);
#else
...
#ifdef NEO_UBT // NEO: NUBT - [NeoUploadBandwidthThrottler] -- Xanatos -->
SocketSentBytes CUDPSocket::Send(uint32 maxNumberOfBytesToSend, uint32 /*minFragSize*/, bool /*onlyAllowedToSendControlPacket*/, uint16 maxNumberOfPacketsToSend) 
#else
...
// <-- ZZ:UploadBandWithThrottler (UDP)
	...
	for (;controlpacket_queue.GetHeadPosition() != 0 && sentPackets < maxNumberOfPacketsToSend 
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
        int sendSuccess = 0;
		if(packet->bFromClient)
			sendSuccess = theApp.clientudp->SendTo((char*)packet->packet, packet->size, packet->dwIP, packet->nPort);
		else
			sendSuccess = SendTo(packet->packet, packet->size, packet->dwIP, packet->nPort);
#else
...
int CUDPSocket::SendTo(BYTE* lpBuf, int nBufLen, uint32 dwIP, uint16 nPort)
{
	...
	if (iResult == SOCKET_ERROR) {
		...
		theApp.bandwidthControl->AddUp(iResult,true);
#endif // NEO_BC // NEO: NBC END <-- Xanatos --
	return 1; // success
}

#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
void CUDPSocket::SendBuffer(uint32 nIP, uint16 nPort, BYTE* pPacket, UINT uSize, bool bClient)
#else
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
	newpending->bFromClient = bClient;
#endif //NATTUNNELING // NEO: NATT END <-- Xanatos --
    sendLocker.Lock();
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
		// Note: the udp callbacks have to be sent from our cleint socket
		SendBuffer(nIP, nPort, pRawPacket, uRawPacketSize, packet && packet->opcode == OP_NAT_CALLBACKREQUEST);
#else
