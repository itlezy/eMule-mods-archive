	...
	if (_ParseURL(Data.sURL, _T("w")) == _T("password"))
	{
		...
		if(MD5Sum(_ParseURL(Data.sURL, _T("p"))).GetHash() == thePrefs.GetWSPass())
		{
	        if (!justAddLink) 
	        {
				...
				ses.lastcat= 0; //- thePrefs.GetCatFilter(0); // NEO: NXC - [NewExtendedCategories] <-- Xanatos --
...
//	Auto-refresh code
	...
	Out.Replace(_T("[eMuleAppName]"), MOD_NAME); // NEO: NV - [NeoVersion] <-- Xanatos --
	//Out.Replace(_T("[version]"), theApp.m_strCurVersionLong);
	Out.Replace(_T("[version]"), theApp.m_strNeoVersionLong); // NEO: NV - [NeoVersion] <-- Xanatos --
...
CString CWebServer::_GetTransferList(ThreadData Data)
{
	...
	for (int i=0;i<partlist.GetCount();i++) {
		...
		if (!pPartFile || pPartFile->IsPWProt())
			continue;
		// NEO: PP END <-- Xanatos --
		...
		if (pPartFile)
		{
			...
			switch(pPartFile->GetStatus())
			{
				...
				case PS_IMPORTING:
					dFile.sFileState = _T("importing");
					break;
				// NEO: PIX END <-- Xanatos --
				// NEO: MTD - [MultiTempDirectories] -- Xanatos -->
				case PS_MOVING:
					dFile.sFileState = _T("moving");
					break;
				// NEO: MTD END <-- Xanatos --
				case PS_PAUSED:
...
CString CWebServer::_GetPreferences(ThreadData Data)
{
	...
	if ((_ParseURL(Data.sURL, _T("saveprefs")) == _T("true")) && IsSessionAdmin(Data,sSession) )
	{
		...
		if(!_ParseURL(Data.sURL, _T("maxsources")).IsEmpty())
			...
			NeoPrefs.PartPrefs.SetSourceLimit(_tstoi(_ParseURL(Data.sURL, _T("maxsources")))); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
	...
	sRefresh.Format(_T("%d"), NeoPrefs.PartPrefs.GetSourceLimit()); // NEO: SRT - [SourceRequestTweaks] <-- Xanatos --
...
CString CWebServer::GetClientSummary(CUpDownClient* client) {
	...
	buffer+= GetResString(IDS_UPLOADTIME) + _T(": ") + CastSecondsToHM(client->GetUpStartTimeDelay()) + _T("\n"); // NEO: CUT - [CleanUploadTiming] <-- Xanatos --
...
CString CWebServer::_GetCommentlist(ThreadData Data)
{
	...
	{ 
		...
		if(const CClientFileStatus* status = cur_src->GetFileStatus(cur_src->GetRequestFile()))
		{
			if (status->HasFileRating() || !status->GetFileComment().IsEmpty())
			{
				commentlines.AppendFormat( pThis->m_Templates.sCommentListLine,
				_SpecialChars(cur_src->GetUserName()),
				_SpecialChars(status->GetFileName()),
				_SpecialChars(status->GetFileComment()),
				_SpecialChars(GetRateString(status->GetFileRating()))
				);
			} 
		}
