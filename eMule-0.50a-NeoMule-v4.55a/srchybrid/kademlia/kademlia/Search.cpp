...
#include "../../knownfilelist.h" // NEO: XCk - [KnownComments] <-- Xanatos --
#include "../../Packets.h"
#include "../../partfile.h"
#include "../../emuledlg.h"
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
							// Note: using -1 isn't a protocol violation in any kind as a user could also set this value by hand
							// on the other side the propability of this being done manualy is extremly low 1 to 65 000 is the statistic propability but the real one is far bellow
							// So this is a valid and roule conform way of identifying NAT-T enabled clients
							if(NeoPrefs.IsNatTraversalEnabled())
								listTag.push_back(new CKadTagUInt(TAG_SOURCEPORT, (uint16)-1)); // -1 meens NAT-T enabled cleint
							else
#endif //NATTUNNELING // NEO: NATT END <-- Xanatos --
							listTag.push_back(new CKadTagUInt(TAG_SOURCEPORT, thePrefs.GetPort()));
							if (!CKademlia::GetPrefs()->GetUseExternKadPort())
		...
		case STORENOTES:
			{
				...
				if(!pFile)
					pFile = theApp.knownfiles->FindKnownFileByID(ucharFileid);
				// NEO: XCk END <-- Xanatos --

				if (pFile)
				{
					// We set this mostly for GUI resonse.
					m_sFileName = pFile->GetFileName(); // NEO: KII - [KadInterfaceImprovement] <-- Xanatos --
					...
					listTag.push_back(new CKadTagStr(TAG_FILENAME, pFile->GetFileName(true))); // NEO: PP - [PasswordProtection] <-- Xanatos --
		...
		case FINDSOURCE:
			{
				...
				if(NeoPrefs.IsNatTraversalEnabled() && theApp.IsFirewalled())
				{
					if(CNatTunnel* tunnel = theApp.natmanager->FindPendingTunnel(m_uTarget.GetData(), kad_id)) // check if we have a pending tunel for this kad id
					{
						tunnel->PrepareCallback(m_uTarget.GetData(), kad_id, pFromContact->GetIPAddress(), pFromContact->GetUDPPort(), true);
						tunnel->SendCallback();
					}
					break;
				}
...
void CSearch::PreparePacketForTags(CByteIO *byIO, CKnownFile *pFile)
{
	...
	try
	{
		if (pFile && byIO)
		{
			...
			CString strED2KFileType(GetED2KFileTypeSearchTerm(GetED2KFileTypeID(pFile->GetFileName(true)))); // NEO: PP - [PasswordProtection] <-- Xanatos --
...
uint32 CSearch::GetDuration() const
{
	return time(NULL) - m_tCreated;
}
// NEO: KII END <-- Xanatos --
