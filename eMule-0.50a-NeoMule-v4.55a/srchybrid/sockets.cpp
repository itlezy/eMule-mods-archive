...
#include "IPFilter.h" // NEO: MOD - [ServerIPFilter] <-- Xanatos --
...
void CServerConnect::TryAnotherConnectionRequest()
{
	if (connectionattemps.GetCount() < (thePrefs.IsSafeServerConnectEnabled() ? 1 : 2))
	{
		...
		if (next_server == NULL)
		{
			if (connectionattemps.GetCount() == 0)
			{
				...
				else if (m_idRetryTimer == 0)
				{
					...
					m_idRetryTimer = ::GetTickCount() + 1000*CS_RETRYCONNECTTIME; // NEO: MOD - [ServerConnect] <-- Xanatos --
		...
		// NEO: FIX - [FailingServerWorkaround] -- Xanatos -->
		if(next_server->GetDisconnectedTick() && ::GetTickCount() - next_server->GetDisconnectedTick() < MIN2MS(3))
			return;
		// NEO: FIX END <-- Xanatos --
...
void CServerConnect::ConnectToAnyServer(UINT startAt, bool prioSort, bool isAuto, bool bNoCrypt)
{
	...
	if( NeoPrefs.IsPreferShareAll() && prioSort )
...
{
	// NEO: MOD - [ServerIPFilter] -- Xanatos -->
	if(theApp.ipfilter->IsFiltered(server->GetIP()))
	{
		ModLog(GetResString(IDS_X_SVR_IP_BANNED),ipstr(server->GetIP()), server->GetListName() );
	...
	if(NeoPrefs.ReuseTCPPort())
		newsocket->Create(thePrefs.GetPort(true), SOCK_STREAM, FD_READ | FD_WRITE | FD_CLOSE | FD_CONNECT, CT2CA(theApp.GetBindAddress()), TRUE); // NEO: MOD - [BindToAdapter] <-- Xanatos --
	else
	...
	theApp.serverlist->ServerStats(server, bNoCrypt, true);
	connectionattemps.SetAt(sPendingCon(server,bNoCrypt), newsocket);
	// NEO: GSI END <-- Xanatos --
}
...
void CServerConnect::StopConnectionTry()
{
	...
	if (m_idRetryTimer) 
	{ 
		//KillTimer(NULL, m_idRetryTimer); // NEO: MOD - [ServerConnect] <-- Xanatos --
		m_idRetryTimer= 0; 
	} 
...
{
	theApp.SetConnectionEstablished(); // NEO: NCC - [NeoConnectionChecker] <-- Xanatos --
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		if(NeoPrefs.IsNatTraversalEnabled())
			dwNatFlag |= SRVCAP_NATTRAVERSAL;

		CTag tagFlags(CT_SERVER_FLAGS, SRVCAP_ZLIB | SRVCAP_NEWTAGS | SRVCAP_LARGEFILES | SRVCAP_UNICODE | dwCryptFlags | dwNatFlag);
#else
	...
	else if (sender->GetConnectionState() == CS_CONNECTED)
	{
		...
		if(!NeoPrefs.UseLugdunumCredits() || (!sender->cur_server->ServerBlacklists() && NeoPrefs.UseLugdunumCredits() == TRUE))
		{
...
bool CServerConnect::SendUDPPacket(Packet* packet, CServer* host, bool delpacket, uint16 nSpecialPort, BYTE* pRawPacket, uint32 nLen){ 
	//if (theApp.IsConnected()){ // NEO: GSI - [GetServerInfoFirst] -- Xanatos --
	if (udpsocket != NULL) 
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		...
		case CS_SERVERFULL:{
			...
			//DWORD tmpkey;
			sPendingCon tmpkey; // NEO: GSI - [GetServerInfoFirst] <-- Xanatos --
...
// NEO: MOD - [ServerConnect] -- Xanatos -->
void CServerConnect::Process()
{
	if(m_idRetryTimer > ::GetTickCount()){
		StopConnectionTry();
		if (!IsConnected()){
			if (m_uStartAutoConnectPos >= theApp.serverlist->GetServerCount())
				m_uStartAutoConnectPos = 0;
			ConnectToAnyServer(m_uStartAutoConnectPos, true, true);
		}
	}

	// NEO: KLC - [KhaosLugdunumCredits]
	if (connected && connectedsocket)
		connectedsocket->cur_server->IncServerCredits();
	// NEO: KLC END
...
{ 
	...
	while (pos){
		...
		// NEO: GSI - [GetServerInfoFirst] -- Xanatos -->
		// Note: In order to can work obtimaly with obfiscation and aspectal NAT-T we need 
		//			to get the server info packet, so we at first request this one
		//			and when we get it we connect, or we wait for a timeout.
		//			This behavioure was suggested by eserver stuff.
		if(theApp.serverlist->IsValidServer(tmpkey.Server)){
			if ((tmpkey.Server->GetChallenge() == 0 || dwCurTick - tmpkey.dwStartTime > SEC2MS(4)) && tmpsock->GetConnectionState() == CS_NOTCONNECTED)
				tmpsock->ConnectTo(tmpkey.Server,tmpkey.bNoCrypt);
			else if (tmpkey.Server->GetCryptPingReplyPending() && dwCurTick - tmpkey.dwStartTime > SEC2MS(2))
				theApp.serverlist->ServerStats(tmpkey.Server,true); // try unencrypted
		}else
			tmpkey.dwStartTime = 0; // force destroy
		// NEO: GSI END <-- Xanatos --

		if (dwCurTick - tmpkey.dwStartTime > dwServerConnectTimeout){
...
#ifdef NATTUNNELING // NEO: NATT - [NatTraversal] -- Xanatos -->
bool CServerConnect::CanCallback(uint32 dwIP, uint16 nPort){
	if (IsConnected()){
		if (connectedsocket->cur_server->GetIP() == dwIP && connectedsocket->cur_server->GetPort() == nPort){
			if(!IsLowID() || connectedsocket->cur_server->SupportsNatTraversal())
				return true;
		}
	}
	return false;
...
bool CServerConnect::CanLow2Low()
{
	if (IsConnected() && IsLowID())
		return connectedsocket->cur_server->SupportsNatTraversal();
	return false;
}
#endif //NATTUNNELING // NEO: NATT END <-- Xanatos --
