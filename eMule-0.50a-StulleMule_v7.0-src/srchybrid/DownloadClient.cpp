...
#include "UploadQueue.h" // UDPReaskFNF-Fix against Leechers [Xman/idea by WiZaRd] - Stulle
...
bool CUpDownClient::IsSourceRequestAllowed() const
{
	...
	if(thePrefs.GetAntiXSExploiter() && IsXSExploiter())
...
void CUpDownClient::UDPReaskFNF()
{
	...
	if (GetDownloadState() != DS_DOWNLOADING){ // avoid premature deletion of 'this' client
		...
		// ==> UDPReaskFNF-Fix against Leechers [Xman/idea by WiZaRd] - Stulle
...
uint32 CUpDownClient::GetTimeUntilReask(const CPartFile* file, const bool allowShortReaskTime, const bool useGivenNNP, const bool givenNNP) const {
    ...
    if(lastAskedTimeTick != 0) {
		...
		// ==> Prevent from missing an Upload by Xtreme - Stulle
		} else if(IsXtremeClient()) {
			if(GetSpreadReAskTime() > FILEREASKTIME) // try to spread if possible
				reaskTime = FILEREASKTIME;
			else
				reaskTime = GetSpreadReAskTime();
		// <== Prevent from missing an Upload by Xtreme - Stulle
        } else {
            ...
            reaskTime = FILEREASKTIME;
			*/
			reaskTime = GetSpreadReAskTime() + thePrefs.GetReAskTimeDif();
			// <== Spread Requests - Stulle
			// <== Timer for ReAsk File Sources - Stulle
        }
