...
DWORD quicktime; // Quick start [TPT] - Stulle
...
void CDownloadQueue::Init(){
	...
	m_dwResTimer = ::GetTickCount(); // CPU/MEM usage [$ick$/Stulle] - Stulle
	...
	m_SettingsSaver.LoadSettings(); // drop sources - Stulle
...
// <== Quick start [TPT] - Stulle
	...
	*/
	...
	if(theApp.uploadqueue->GetUploadQueueLength() > 0 && bRatio) {
        // has this client downloaded more than it has uploaded this session? (friends excluded)
        // then limit its download speed from all clients but friends
        // limit will be removed as soon as upload has catched up to download
        if(theStats.sessionReceivedBytes/uRatio > (theStats.sessionSentBytes-theStats.sessionSentBytesToFriend) &&
           datarate > 1500) {
            ...
            uint32 secondsNeededToEvenOut = (uint32)((theStats.sessionReceivedBytes/uRatio-(theStats.sessionSentBytes-theStats.sessionSentBytesToFriend))/(theApp.uploadqueue->GetToNetworkDatarate()+1));
            uint32 tempDownspeed = max(min(3*100/max(secondsNeededToEvenOut, 1), 200), 30);
        ...
        // then limit its download speed from all friends
        // limit will be removed as soon as upload has catched up to download
        if(theStats.sessionReceivedBytes/uRatio > (theStats.sessionSentBytes) &&
           datarate > 1500) {

            float secondsNeededToEvenOut = (float)((theStats.sessionReceivedBytes/uRatio-theStats.sessionSentBytes)/(theApp.uploadqueue->GetDatarate()+1));
            uint32 tempDownspeed = max(min((uint32)(3*100/max(secondsNeededToEvenOut, 1)), 200), 30);
...
\********************/
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		if (cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY)
		{
			if (thePrefs.GetGlobalHlAll() || cur_file->GetGlobalHL()) // Global Source Limit (customize for files) - Stulle
			aCount++;
...
void CDownloadQueue::InitTempVariables(CPartFile* file)
{
	...
	thePrefs.SetMaxSourcesPerFileTemp(file->GetPrivateMaxSources()); // hardlimit - Stulle
...
void CDownloadQueue::RemoveSourceAndDontAsk(CUpDownClient* toremove, bool bDoStatsUpdate/*, POSITION pos*/)
{
	if( RemoveSource(toremove, bDoStatsUpdate/*, pos*/) ){
		if( !toremove->IsFriend() ){ // [-List of Dont Ask This IPs-]
			theApp.clientlist->AddToDontAskThisIP(toremove->GetIP());
			//if( thePrefs.GetLogBannedClients() )
			//	theApp.emuledlg->AddDebugLogLine(false, "DONT ASK THIS IP=%s => CDownloadQueue::RemoveSourceAndDontAsk()!", ipstr(toremove->GetIP()));
		}
	}
}
