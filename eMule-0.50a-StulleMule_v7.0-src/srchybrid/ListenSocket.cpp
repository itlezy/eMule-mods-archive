			...
			switch(opcode)
			{
				...
				case OP_SETREQFILEID:
				{
					...
					if (size == 16)
					{
						...
						*/
						...
						// ==> File Faker Check [Xman] - Stulle
						if(thePrefs.IsFileFaker() && reqfile->IsPartFile() && ((CPartFile*)reqfile)->m_DeadSourceList.IsDeadSource(client))
						{
							client->BanLeecher(NULL,BAN_FILEFAKER);
							break;
						}
						// <== File Faker Check [Xman] - Stulle
				...
				case OP_FILEREQANSNOFIL:
				{
					...
					if (size == 16)
					{
						...
						// ==> File Faker Check [Xman] - Stulle
						if(thePrefs.IsFileFaker() && client->GetUploadState()!=US_NONE && reqfile->GetFileSize()>PARTSIZE)
						{
				...
				case OP_STARTUPLOADREQ:
				{
					...
					{
						// ==> Sivka-Ban [cyrex2001] - Stulle
						if( thePrefs.GetEnableSivkaBan() && client->uiULAskingCounter > uint16 (thePrefs.GetSivkaAskCounter()) && ((::GetTickCount()-client->dwThisClientIsKnownSince)/client->uiULAskingCounter) < uint32 (MIN2MS(thePrefs.GetSivkaAskTime()))){
							if (thePrefs.SivkaAskLog)
								{
								DebugLog(LOG_MORPH, _T("OP_STARTUPLOADREQ: (%s/%u)=%s ==> %s(%s) ASK TO FAST, BANNED!!!"), 
							...
							client->BanLeecher(NULL,BAN_SIVKA);
				...
				case OP_REQUESTPARTS:
				{
					...
					if(client->IsLeecher()) {
					*/
					if(client->IsLeecher() && client->GetScoreReduce()==false) {
					// <== Reduce Score for leecher - Stulle
		...
		try
		{
			...
			switch(opcode)
			{
					...
					while (data_in.GetLength()-data_in.GetPosition() && !bAnswerFNF)
					{
						...
						switch (opcode_in)
						{
							...
							case OP_REQUESTSOURCES:
							{
								...
								if(thePrefs.GetAntiXSExploiter() && client->IsXSExploiter())
									break; //no answer
								// <== Anti-XS-Exploit [Xman] - Stulle
 				...
 				case OP_ANSWERSOURCES:
				{
					...
					client->IncXSAnswer(); // Anti-XS-Exploit [Xman] - Stulle
 				...
 				case OP_ANSWERSOURCES2:
				{
					...
					client->IncXSAnswer(); // Anti-XS-Exploit [Xman] - Stulle //Fafner: missing - 080531
