...
uint32 CUpDownClient::GetScore(bool sysvalue, bool isdownloading, bool onlybasevalue) const
{
	...
	*/
	...
	// ==> Release Score Assurance - Stulle
	...
	if (GetScoreReduce())
		fBaseValue *= thePrefs.GetReduceFactor();
	// <== Reduce Score for leecher - Stulle
...
bool CUpDownClient::IsMoreUpThanDown() const{
	// ==> Disable PS/PBF for leechers [Stulle/idea by sfrqlxert] - Stulle
	if (thePrefs.GetNoBadPushing() && GetScoreReduce())
		return false;
	// <== Disable PS/PBF for leechers [Stulle/idea by sfrqlxert] - Stulle
	...
	*/
	return (currentReqFile && currentReqFile->IsPartFile()==false &&
...
bool CUpDownClient::IsPBForPS() const
{
	...
	*/
	...
	if (
		...
		// ==> Pay Back First for insecure clients - Stulle
		!currentReqFile->IsPartFile() && credits->GetPayBackFirstStatus2() && thePrefs.IsPayBackFirst2() ||
		// <== Pay Back First for insecure clients - Stulle
...
*/
void CUpDownClient::BanLeecher(LPCTSTR pszReason, uint8 uReason){
// <== Reduce Score for leecher - Stulle
	// ==> Anti Friend Ban - Stulle
	if(IsFriend())
	{
	...
	// ==> Reduce Score for leecher - Stulle
	SetBanReason(uReason);
	CString strTemp = NULL;
	if((pszReason == NULL && uReason > GOOD_BOY) || uReason < BAN_NORELAY_COUNT)
		strTemp = GetBanReasonString();
	else if (pszReason != NULL)
		strTemp = pszReason;
	else
		strTemp = _T("No Reason");

	bool bScoreReduce = GetScoreReduce();
	if (!m_bLeecher){
		theStats.leecherclients++;
		m_bLeecher = true;
		DebugLog(LOG_MORPH,_T("[%s](%s)-(%s) Client %s"),strTemp ,bScoreReduce?_T("reduced score"):_T("ban"),m_strNotOfficial ,DbgGetClientInfo());
	}
	if (bScoreReduce)
		return;
	// <== Reduce Score for leecher - Stulle
