...
// MOD Note: end
	...
	m_dwReAskTick = 0; // auto drop immunity - Stulle

	conchecker.SetPreferences(); // Connection Checker [eWombat/WiZaRd] - Stulle
...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	theAntiNickClass.Init(); // AntiNickThief Class - Stulle
	thePrefs.Init();
	theStats.Init();
	...
	sysinfo = new CSystemInfo();// CPU/MEM usage [$ick$/Stulle] - Stulle 
...
void CemuleApp::SetPublicIP(const uint32 dwIP){
	if (dwIP != 0){
		...
		if ( GetPublicIP() == 0)
			...
			{	// Reconnect Kad on IP-Change [Xman] - Stulle
...
//EastShare - Modified by Pretender, [MoNKi: -Check already downloaded files-]
/*
...
if ( askIfAlreadyDownloaded )
{
	...
	theApp.downloadqueue->AddFileLinkToDownload(pFileLink, cat, true,fromclipboard);
//EastShare END
...
{ 
	...
	{ 
		...
		DWORD state=conchecker.Check(/*true*/); 
...
CFont* CemuleApp::GetFontByStyle(DWORD nStyle)
{
