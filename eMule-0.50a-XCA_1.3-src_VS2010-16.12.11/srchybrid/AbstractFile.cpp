	...
	: m_FileIdentifier(pAbstractFile->m_FileIdentifier, m_nFileSize)
{
	...
	//Xman Code Improvement for choosing to use compression
	compressible = pAbstractFile->compressible;
	//Xman end
	//Xman Code Improvement for HasCollectionExtention
	m_bhasCollectionExtention = pAbstractFile->m_bhasCollectionExtention;
	//Xman end


	const CAtlList<Kademlia::CEntry*>& list = pAbstractFile->getNotes();
...
void CAbstractFile::CopyTags(const CAtlArray<CTag*>& tags)
{
...
{ 
	...
	UpdateCompressible();// X: [DCE] - [DontCompressExt]
	//Xman end

	//Xman Code Improvement for HasCollectionExtention
	#define COLLECTION_FILEEXTENSION	_T(".emulecollection")
	m_bhasCollectionExtention = !(m_strFileName.Find(COLLECTION_FILEEXTENSION) == -1);
	//Xman end
} 

void CAbstractFile::UpdateCompressible()// X: [DCE] - [DontCompressExt]
{ 
	compressible = !thePrefs.dontcompressext||!checkExt(m_strFileName, thePrefs.compressExt);// X: [CI] - [Code Improvement]
}
...
void CAbstractFile::SetInt64TagValue(uint8 tagname, uint64 uValue)
{
	...
	CTag* pTag = new CTag(tagname, uValue, true);// X: [BF] - [Bug Fix]
	taglist.Add(pTag);
}

LPCTSTR CAbstractFile::GetStrTagValue(uint8 tagname) const
{
	for (size_t i = 0; i < taglist.GetCount(); i++){
		const CTag* pTag = taglist[i];
...
LPCTSTR CAbstractFile::GetStrTagValue(LPCSTR tagname) const
{
	for (size_t i = 0; i < taglist.GetCount(); i++){
		const CTag* pTag = taglist[i];
...
void CAbstractFile::RefilterKadNotes(bool bUpdate){
	...
	for (POSITION pos1 = m_kadNotes.GetHeadPosition(), pos2;( pos2 = pos1 )!= NULL;){// X: [CI] - [Code Improvement]
		Kademlia::CEntry* entry = m_kadNotes.GetNext(pos1);
...
CString CAbstractFile::GetED2kLink(bool bHashset, bool bHTML, bool bHostname, bool bSource, uint32 dwSourceIP) const
{
	...
	if (bHashset && GetFileIdentifierC().GetAvailableMD4PartHashCount() > 0 && GetFileIdentifierC().HasExpectedMD4HashCount()){
		strLink += _T("p=");
		for (size_t j = 0; j < GetFileIdentifierC().GetAvailableMD4PartHashCount(); j++)
		{
			if (j > 0)
				strLink += _T(':');
