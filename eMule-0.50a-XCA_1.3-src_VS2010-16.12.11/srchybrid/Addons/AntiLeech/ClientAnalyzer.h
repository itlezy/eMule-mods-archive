...
// KNOW YOUR ENEMY
	...
	void	SetLastSeen()		{m_pData->dwLastSeen = (uint_ptr)time(NULL);}

	DWORD	GetFirstMet() const	{return m_pData->dwFirstMet;}	
	void	SetFirstMet()		{m_pData->dwFirstMet = (uint_ptr)time(NULL);}

//keep bad status for some time
	bool	IsBlockedByBadTimer() const	{return time(NULL) - m_pData->dwBadTimer < 7200;} // = 2h
	void	SetBadTimer()		{m_pData->dwBadTimer = (uint_ptr)time(NULL);}
