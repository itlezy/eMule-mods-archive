...
class EncodeFileName
{
  ...
  public:
	...
	void Decode(char *Name,byte *EncName, uint_ptr EncSize,wchar_t *NameW, uint_ptr MaxDecSize);
};
...
void EncodeFileName::Decode(char *Name,byte *EncName,uint_ptr EncSize,wchar_t *NameW,
							uint_ptr MaxDecSize)
{
  uint_ptr EncPos = 0, DecPos = 0;
...
static LCX_COLUMN_INIT s_aColumns[] =
{
	...
	{ ARCHPREV_COL_SIZE, _T("Size"),  IDS_DL_SIZE,      LVCFMT_RIGHT, -1, 1, ASCENDING, NONE, _T("9999999999 B") },
...
BOOL CArchivePreviewDlg::OnInitDialog()
{
	...
	else
	{
		...
		nDelta2 += rc.Height();
		CSplitterControl::ChangePos(&m_ContentList, 0, -nDelta1);
		CSplitterControl::ChangeHeight(&m_ContentList, nDelta1 + nDelta2);
		CSplitterControl::ChangePos(&m_progressbar, 0, nDelta2);
	...
	AddAnchor(m_progressbar, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddAnchor(m_ContentList, TOP_LEFT, BOTTOM_RIGHT);
...
void CArchivePreviewDlg::OnBnClickedCreateRestored() {
	...
	if ( ((CShareableFile*)(*m_paFiles)[0])->IsPartFile()==false)
		return;
...
/*****************************************
				A C E
 *****************************************/
sint_ptr CArchivePreviewDlg::ShowAceResults(sint_ptr succ, archiveScannerThreadParams_s* tp) {
	...
	if (!tp->ai->ACEdir->IsEmpty())
	{
		...
		while (pos != NULL)
		{
			...
			bool bCompleteEntry = !tp->file->IsPartFile() || ((CPartFile*)tp->file)->IsComplete(block->data_offset, block->data_offset + block->PACK_SIZE, true);
			...
			if (bIsDirectory)
				...
				// size (uncompressed)
				///temp = CastItoXBytes(size);
				temp.Format(_T("%u B"), block->ORIG_SIZE);// X: [UIC] - [UIChange]
...
/*****************************************
				I S O
 *****************************************/
sint_ptr CArchivePreviewDlg::ShowISOResults(sint_ptr succ, archiveScannerThreadParams_s* tp) {
	...
	while (pos != NULL)
	{
		...
		bool bCompleteEntry = !tp->file->IsPartFile() || ((CPartFile*)tp->file)->IsComplete(
		...
		if (file->fileFlags & ISO_DIRECTORY) {
			temp.AppendChar(_T('D'));
		} else {
			filecount++;

			// size
			//temp.Format(_T("%s"), CastItoXBytes(size));
			temp.Format(_T("%u B"), LODWORD(file->dataSize));// X: [UIC] - [UIChange]
		...
		{
			if (!temp.IsEmpty()) temp.AppendChar(_T(','));
			temp.AppendChar(_T('H'));
		}
		if (file->fileFlags & ISO_READONLY)
		{
			if (!temp.IsEmpty()) temp.AppendChar(_T(','));
			temp.AppendChar(_T('R'));
		}
...
/*****************************************
				R A R
 *****************************************/
sint_ptr CArchivePreviewDlg::ShowRarResults(sint_ptr succ, archiveScannerThreadParams_s* tp) {
	...
	if (!tp->ai->RARdir->IsEmpty())
	{
		...
		while (pos != NULL)
		{
			...
			bool bCompleteEntry = !tp->file->IsPartFile() || ((CPartFile*)tp->file)->IsComplete(block->offsetData, block->offsetData + block->dataLength, true);
			...
			if ( (block->HEAD_FLAGS & 0xE0) != 0xE0) {
				...
				///temp = CastItoXBytes(size);
				temp.Format(_T("%I64u B"), size);// X: [UIC] - [UIChange]
				m_ContentList.SetItemText(iItem,uSubId++,temp);
...
/*****************************************
				Z I P
 *****************************************/
sint_ptr CArchivePreviewDlg::ShowZipResults(sint_ptr succ, archiveScannerThreadParams_s* tp) {
	...
	if (!tp->ai->centralDirectoryEntries->IsEmpty())
	{
		...
		while (pos != NULL)
		{
			...
			if (tp->file->IsPartFile()) {
				...
				const CPartFile* pf = (CPartFile*)tp->file;
			...
			// size (uncompressed)
			if (!bIsDirectory) {
				///temp = CastItoXBytes(cdEntry->lenUncompressed);
				temp.Format(_T("%u B"), cdEntry->lenUncompressed);// X: [UIC] - [UIChange]
				m_ContentList.SetItemText(iItem,uSubId++,temp);
...
// ##################################################################
archiveScannerThreadParams_s::~archiveScannerThreadParams_s() {
	POSITION pos = filled->GetHeadPosition();
	while (pos != NULL)
		delete filled->GetNext(pos);
	filled->RemoveAll();
	delete filled;
	filled=NULL;
	...
	if (ai->RARdir) {
		RAR_BlockFile *block;
		POSITION pos = ai->RARdir->GetHeadPosition();
		while (pos != NULL)
		{
			block = ai->RARdir->GetNext(pos);
			delete [] block->FILE_NAME;
			delete block;
		}
		delete ai->RARdir;
	...
	if (ai->centralDirectoryEntries) {
		pos = ai->centralDirectoryEntries->GetHeadPosition();
		while (pos != NULL)
		{
			ZIP_CentralDirectory *cdEntry = ai->centralDirectoryEntries->GetNext(pos);
			// cleanup
			delete [] cdEntry->filename;
			if (cdEntry->lenExtraField > 0)
				delete [] cdEntry->extraField;
		...
		delete ai->centralDirectoryEntries;
		ai->centralDirectoryEntries=NULL;
	}
	...
	if (ai->ACEdir){
		pos = ai->ACEdir->GetHeadPosition();
		while (pos != NULL)
		{
			ACE_BlockFile *block= ai->ACEdir->GetNext(pos);
			delete block;
		}
		delete ai->ACEdir;
		ai->ACEdir=NULL;
...
void CArchivePreviewDlg::UpdateArchiveDisplay(bool doscan) {
	...
	CShareableFile* file = (CShareableFile*)(*m_paFiles)[0];
	...
	CAtlList<Gap_Struct*> *filled = new CAtlList<Gap_Struct*>;
	if (file->IsPartFile() && ((CPartFile*)(file))->xState != POFC_WAITING) {// X: [POFC] - [PauseOnFileComplete]
	...
	switch(type) {
		case ARCHIVE_ZIP:
			ai->centralDirectoryEntries= new CAtlList<ZIP_CentralDirectory*>;
			break;
		case ARCHIVE_RAR:
			ai->RARdir = new CAtlList<RAR_BlockFile*>;
			break;
		case ARCHIVE_ACE:
			ai->ACEdir = new CAtlList<ACE_BlockFile*>;
			break;
		case IMAGE_ISO:
			ai->ISOdir = new CAtlList<ISO_FileFolderEntry*>;
			break;
	}

	// prepare threadparams
	m_activeTParams = new archiveScannerThreadParams_s(file, ai, filled, type, m_hWnd, m_progressbar.m_hWnd);
	if(!m_activeTParams->start())
		SetDlgItemText(IDC_INFO_STATUS, GetResString(IDS_ERROR) );
	}

void archiveScannerThreadParams_s::run() {
	...
	if (!inFile.Open(file->GetFilePath(), CFile::modeRead | CFile::shareDenyNone)){
		ret=-1;
	} else {
		switch (type) {
			case ARCHIVE_ZIP:
				ret=CArchiveRecovery::recoverZip(&inFile, NULL, this, filled, (inFile.GetLength() == file->GetFileSize()));
				break;
			case ARCHIVE_RAR:
				ret=CArchiveRecovery::recoverRar(&inFile, NULL, this, filled);
				break;
			case ARCHIVE_ACE:
				ret=CArchiveRecovery::recoverAce(&inFile, NULL, this, filled);
				break;
			case IMAGE_ISO:
				ret=CArchiveRecovery::recoverISO(&inFile, NULL, this, filled);
				break;
		}
	...
	if(m_bIsValid && IsWindow(ownerHwnd))
		::SendMessage(ownerHwnd, UM_ARCHIVESCANDONE, ret, (LPARAM)this);
}
