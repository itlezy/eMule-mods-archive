...
void CArchiveRecovery::recover(CPartFile *partFile, bool preview, bool bCreatePartFileCopy)
{
	...
	CArchiveRecovery *tp = new CArchiveRecovery(partFile, filled, preview, bCreatePartFileCopy);
	if(!tp->start())
	{
...
void CArchiveRecovery::run()
{
	...
	InitThreadLocale();
	if (!performRecovery())
		theApp.QueueLogLine(true, _T("%s \"%s\""), GetResString(IDS_RECOVERY_FAILED), partFile->GetFileName());

	partFile->m_bRecoveringArchive = false;
}

bool CArchiveRecovery::performRecovery()
{
...
bool CArchiveRecovery::recoverZip(CFile *zipInput, CFile *zipOutput, archiveScannerThreadParams_s* aitp ,
								  CAtlList<Gap_Struct*> *filled, bool fullSize)
{
	bool retVal = false;
	INT_PTR fileCount = 0;
	try
	{
		...
		if (aitp==NULL) {
			centralDirectoryEntries= new CAtlList<ZIP_CentralDirectory*>;
		} else {
		...
		if (fullSize && readZipCentralDirectory(zipInput, centralDirectoryEntries, filled))
		{
			...
			bool deleteCD;
			for (INT_PTR i=centralDirectoryEntries->GetCount(); i>0; i--)
			{
				deleteCD = false;
...
bool CArchiveRecovery::readZipCentralDirectory(CFile *zipInput, CAtlList<ZIP_CentralDirectory*> *centralDirectoryEntries, CAtlList<Gap_Struct*> *filled)
{
	bool retVal = false;
	try
	{
...
bool CArchiveRecovery::processZipEntry(CFile *zipInput, CFile *zipOutput, uint32 available, CAtlList<ZIP_CentralDirectory*> *centralDirectoryEntries)
{
	if (available < 26)
		return false;
...
CArchiveRecovery::~CArchiveRecovery()
{
	POSITION pos = filled->GetHeadPosition();
	while (pos != NULL)
...
bool CArchiveRecovery::CopyFile(CPartFile *partFile, CAtlList<Gap_Struct*> *filled, CString tempFileName)
{
	bool retVal = false;
	try
	{
...
bool CArchiveRecovery::IsFilled(uint32 start, uint32 end, CAtlList<Gap_Struct*> *filled)
{
	POSITION pos = filled->GetHeadPosition();
	Gap_Struct *fill;
	while (pos != NULL)
...
#define TESTCHUNKSIZE 51200ULL	 // 50k buffer 
	...
	try
	{
		...
		while (pos != NULL)
		{
			...
			while ((block = scanForAceFileHeader(aceInput, aitp, (fill->end - filesearchstart ))) != NULL)
			{
				if (aitp){
					if (!aitp->m_bIsValid){
						delete block; // memleak fix gureedo
						return false;
					}
		...
		while (available > 0)
		{
			...
			while (foundPos != NULL)
			{
				...
				if (newblock->FNAME_SIZE>0) {
					newblock->FNAME = new char[newblock->FNAME_SIZE+1];
...
//				int blockleft=newblock->HEAD_SIZE - newblock->FNAME_SIZE -					(sizeof(ACE_BlockFile)- (3*sizeof(uint16)) - sizeof(newblock->data_offset) - 2* sizeof(char*) ) ;
				...
				if (mempos < &blockmem[0] + newblock->HEAD_SIZE) {
					...
					if (newblock->COMM_SIZE) {
						newblock->COMMENT = new char[newblock->COMM_SIZE];
...
void CArchiveRecovery::ISOReadDirectory(archiveScannerThreadParams_s* aitp, UINT32 startSec, CFile* isoInput, CString currentDirName)
{
	...
	while (aitp->m_bIsValid)
	{
		...
		if (aitp->ai->isoInfos.iJolietUnicode){
			for (size_t i = 0; i < file->nameLen/sizeof(uint16); i++)
...
uint16 CArchiveRecovery::readUInt16(CFile *input)
{
	...
	if (input->Read(b, 2) == 0)
		retVal = (b[1] << 8) + b[0];*/
	input->Read((BYTE*)&retVal, 2);// X: [CI] - [Code Improvement]
	return retVal;
}
...
uint32 CArchiveRecovery::readUInt32(CFile *input)
{
	...
	/*BYTE b[4];
	if (input->Read(b, 4) == 0)
		retVal = (b[3] << 24) + (b[2] << 16) + (b[1] << 8) + b[0];*/// X: [CI] - [Code Improvement]
	input->Read((BYTE*)&retVal, 4);
	return retVal;
}

uint16 CArchiveRecovery::calcUInt16(BYTE *input)
{
	return *(uint16*)input;//(((uint16)input[1]) << 8) + ((uint16)input[0]);// X: [CI] - [Code Improvement]
}

uint32 CArchiveRecovery::calcUInt32(BYTE *input)
{
	return *(uint32*)input;//(((uint32)input[3]) << 24) + (((uint32)input[2]) << 16) + (((uint32)input[1]) << 8) + ((uint32)input[0]);// X: [CI] - [Code Improvement]
}
...
void CArchiveRecovery::writeUInt16(CFile *output, uint16 val)
{
	...
	output->Write((BYTE*)val, 2);// X: [CI] - [Code Improvement]
}
