...
struct ACE_ARCHIVEHEADER 
{
	...
	~ACE_ARCHIVEHEADER() {
		if (AV)		{ delete [] AV;		AV=NULL;}
		if (COMMENT){ delete [] COMMENT;COMMENT=NULL;}
		if (DUMP)	{ delete [] DUMP;	DUMP=NULL;}
	}
};
#pragma pack()
#pragma pack(1)
...
struct archiveinfo_s {
	CAtlList<ZIP_CentralDirectory*> *centralDirectoryEntries;
	CAtlList<RAR_BlockFile*> *RARdir;
	CAtlList<ACE_BlockFile*> *ACEdir;
	CAtlList<ISO_FileFolderEntry*> *ISOdir;
...
class archiveScannerThreadParams_s : public Poco::Thread
{
public:
	CShareableFile*	file;
	archiveinfo_s*	ai;
	CAtlList<Gap_Struct*> *filled;
	int				type;
	HWND			ownerHwnd;
	HWND			progressHwnd;
	int				curProgress;
	bool			m_bIsValid;
	archiveScannerThreadParams_s(CShareableFile* file, archiveinfo_s* ai, CAtlList<Gap_Struct*> *filled, int type, HWND ownerHwnd, HWND progressHwnd)
		...
		, type(type)
		, ownerHwnd(ownerHwnd)
		, progressHwnd(progressHwnd)
		, m_bIsValid(true){}
	virtual void run();
	virtual ~archiveScannerThreadParams_s();
};

class CArchiveRecovery : public Poco::Thread
{
public:
	CArchiveRecovery(CPartFile *_partFile, CAtlList<Gap_Struct*> *_filled, bool _preview, bool _bCreatePartFileCopy) : Thread(true), partFile(_partFile), filled(_filled), preview(_preview), bCreatePartFileCopy(_bCreatePartFileCopy){}
	virtual void run();
	virtual ~CArchiveRecovery();
	static void recover(CPartFile *partFile, bool preview = false, bool bCreatePartFileCopy = true);
	static bool recoverZip(CFile *zipInput, CFile *zipOutput, archiveScannerThreadParams_s* ai, CAtlList<Gap_Struct*> *filled, bool fullSize);
	static bool recoverRar(CFile *rarInput, CFile *rarOutput, archiveScannerThreadParams_s* ai, CAtlList<Gap_Struct*> *filled);
	static bool recoverAce(CFile *aceInput, CFile *aceOutput, archiveScannerThreadParams_s* ai, CAtlList<Gap_Struct*> *filled);
	static bool recoverISO(CFile *aceInput, CFile *aceOutput, archiveScannerThreadParams_s* ai, CAtlList<Gap_Struct*> *filled);
...
private:
	...
	static bool processZipEntry(CFile *zipInput, CFile *zipOutput, uint32 available, CAtlList<ZIP_CentralDirectory*> *centralDirectoryEntries);
	static bool readZipCentralDirectory(CFile *zipInput, CAtlList<ZIP_CentralDirectory*> *centralDirectoryEntries, CAtlList<Gap_Struct*> *filled);
	...
	static bool CopyFile(CPartFile *partFile, CAtlList<Gap_Struct*> *filled, CString tempFileName);
	static bool IsFilled(uint32 start, uint32 end, CAtlList<Gap_Struct*> *filled);
