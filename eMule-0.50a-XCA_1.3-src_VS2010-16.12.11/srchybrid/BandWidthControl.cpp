...
void CBandWidthControl::checkAdapterIndex(uint32 highid)
{
	...
	//if(m_fGetNumberOfInterfaces != NULL && m_fGetIfTable != NULL && m_fGetIpAddrTable != NULL){
		DWORD dwNumIf = 0;
		if(/*m_f*/GetNumberOfInterfaces(&dwNumIf) == NO_ERROR && dwNumIf > 0 ){
			PMIB_IFTABLE mibIfTable=NULL;
			DWORD dwSize = 0;
			DWORD dwRetVal = 0;
			mibIfTable = (MIB_IFTABLE*) malloc(sizeof(MIB_IFTABLE));
			if (/*m_f*/GetIfTable(mibIfTable, &dwSize, true) == ERROR_INSUFFICIENT_BUFFER) {
				free(mibIfTable);
				mibIfTable = (MIB_IFTABLE *) malloc (dwSize);
			}

			if ((dwRetVal = /*m_f*/GetIfTable(mibIfTable, &dwSize, true)) == NO_ERROR) {
				...
				if(gethostname(hostName, sizeof(hostName)) == 0){
					...
					if(lphost != NULL){
						...
						if (/*m_f*/GetIpAddrTable(mibIPAddrTable, &dwSize, FALSE) == ERROR_INSUFFICIENT_BUFFER) {
							free( mibIPAddrTable );
						...
						if(/*m_f*/GetIpAddrTable(mibIPAddrTable, &dwSize, FALSE) == NO_ERROR){
...
void CBandWidthControl::Process()
{
	...
	if(::GetTickCount()-processtime >= 1000){
		...
		/**/ 	if(/*m_f*/GetIfEntry(&ifRow) == NO_ERROR){
		/**/ 		s_Log = 0;
		/**/ 
		...
		{
			if(m_statisticHistory.GetCount() > 10){
									 ...
									 uint32& networkIn, uint32& networkOut) const 
{
	...
	/*->*/ m_statisticLocker.Lock();
	/**/ if(m_statisticHistory.GetCount() >= 2 && samples >= 1){
	/**/ 
												...
												uint32& eMuleInSession, uint32& eMuleOutSession) const 
{
	...
	// Check if the list is already long enough
	/*->*/ m_statisticLocker.Lock();
	/**/ if(m_statisticHistory.GetCount() >= 2){
	/**/
	/**/	const Statistic& newestSample = m_statisticHistory.GetHead();
...
{
	m_statisticLocker.Lock();
	AddLogLine(false, _T("Bandwidthcontrol: number of statistic-elements: %u"), m_statisticHistory.GetCount());
	m_statisticLocker.Unlock();
}
#endif