		...
		switch (temptag.GetNameID())
		{
			...
			case CT_EMULE_MISCOPTIONS1:
				...
				if (temptag.IsInt()) {
					...
					m_fSupportsPreview		= (uOption >>  0) & 0x01;
					dwEmuleTags |= 2;
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
					if (bDbgInfo) {
...
//<<< zz_fly::Bad Shareaza detection
			...
			case CT_EMULE_MISCOPTIONS2:
				...
				if (temptag.IsInt()) {
					...
					m_fSupportsCaptcha	    = (uOption >>  11) & 0x01;
...
//>>> WiZaRd::Easy ModVersion
	...
	data.WriteUInt32(7+bSend); // nr. of tags
//<<< WiZaRd::Easy ModVersion
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, bool bNoCallbacks/*, CRuntimeClass* pClassSocket*/)
{
	...
	if (!HasLowID())
	{
		...
		*/
		if(!IsCUrlClient(this))
			socket = new CClientReqSocket(this);
		else
			socket = new CHttpClientDownSocket(this);
...
void CUpDownClient::SendPreviewAnswer(const CKnownFile* pForFile, CxImage** imgFrames, uint8 nCount)
{
	...
	data.WriteHash16(pForFile?pForFile->GetFileHash():_aucZeroHash);
	data.WriteUInt8(nCount);
	bool bSucc = true; // WiZaRd
	...
	packet = new Packet(&data, OP_EMULEPROT, OP_PREVIEWANSWER);
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
	if (thePrefs.GetDebugClientTCPLevel() > 0)
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32 nLength)
{
	...
	{
		resToken.Trim();
		if (strMessageCheck.Find(resToken.MakeLower()) >= 0){
			if ( thePrefs.IsAdvSpamfilterEnabled() && !IsFriend() && GetMessagesSent() == 0 ){
				SetSpammer(true);
				theApp.emuledlg->chatwnd->chatselector.EndSession(this);
			}
...
void CUpDownClient::ProcessCaptchaRequest(CSafeMemFile* data){
		...
		&& theApp.emuledlg->chatwnd->chatselector.GetItemByClient(this) != NULL)
	{
		...
		size_t nTagCount = data->ReadUInt8();
		for (size_t i = 0; i < nTagCount; i++)
			CTag tag(data, true);
...
void CUpDownClient::ShowRequestedFiles()
{
	...
	AfxMessageBox(fileList, MB_ICONINFORMATION | MB_OK);// morph4u
