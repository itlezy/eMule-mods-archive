...
// fairly simply captcha generator, might be improved if spammers think its really worth it solving captchas on eMule
...
void CCaptchaGenerator::ReGenerateCaptcha(uint32 nLetterCount)
{
	...
	for (uint32 i = 0; i < nLetterCount; i++) {
		...
		CString strLetter(schCaptchaContent[rng.getUInt16() % ARRSIZE(schCaptchaContent)]);
		m_strCaptchaText += strLetter;
		
		uint16 nRandomSize = rng.getUInt16() % 10;
		uint16 nRandomOffset = 3 + rng.getUInt16() % 11;
		imgLetter.DrawString(NULL, nRandomOffset, 32, strLetter, imgLetter.RGBtoRGBQUAD(RGB(0, 0, 0)), _T("Arial"), 40 - nRandomSize, 1000);
		float fRotate = (float)(35 - (rng.getUInt16() % 70));
