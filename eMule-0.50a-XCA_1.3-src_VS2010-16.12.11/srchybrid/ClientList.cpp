...
*/
#ifdef REPLACE_ATLMAP
void CClientList::GetStatistics(uint_ptr &ruTotalClients, uint_ptr stats[NUM_CLIENTLIST_STATS], unordered_map<uint32, uint_ptr>**clientMaps, StatsMaps& statsmaps)
#else
void CClientList::GetStatistics(uint_ptr &ruTotalClients, uint_ptr stats[NUM_CLIENTLIST_STATS], CAtlMap<uint32, uint_ptr>**clientMaps, StatsMaps& statsmaps)
#endif
{
	...
	//pLocations.RemoveAll();// X: [IP2L] - [IP2Location]
	static uint32 lastmodlistclean;
	if(::GetTickCount()-lastmodlistclean> HR2MS(6))
	{
		//don¡ät clean it up every time -> jumping statistics
	...
	for (POSITION pos = list.GetHeadPosition(); pos != NULL; )
	{
		...
		switch (cur_client->GetClientSoft())
		{
			...
			case SO_OLDEMULE:
				...
				if(clientMaps[0]){
					...
					CStringToUIntPTRMap* &versionMods = (*statsmaps.clientMods)[cur_client->GetVersion()];
					if(versionMods == NULL)
						versionMods = new CStringToUIntPTRMap();
					++(*versionMods)[cur_client->GetClientModVer()];
#else
					...
					ASSERT(statsmaps.clientMods);
					if (!statsmaps.clientMods->Lookup(cur_client->GetVersion(), versionMods)){
						versionMods = new CRBMap<CString, uint_ptr>;
						//versionMods->RemoveAll();
						statsmaps.clientMods->SetAt(cur_client->GetVersion(), versionMods);
					}

					CRBMap<CString, uint_ptr>::CPair *pPairMod = versionMods->Lookup(cur_client->GetClientModVer());
					if(pPairMod != NULL)
						++pPairMod->m_value;
					else
						versionMods->SetAt(cur_client->GetClientModVer(), 1);
#endif
				}
				...
				if (!strMODName.IsEmpty())
				{
					...
					CAtlMap<POSITION, uint_ptr>::CPair *pPairMOD = statsmaps.MODs.Lookup(pos_MOD);
					if(pPairMOD != NULL)
						++pPairMOD->m_value;
					else
						statsmaps.MODs.SetAt(pos_MOD, 1);
#endif
				}
				//Xman end
				break;
		...
		if(statsmaps.Locations && !cur_client->m_structUserLocation->locationName.IsEmpty())
			++(*statsmaps.Locations)[cur_client->m_structUserLocation];
		++statsmaps.Countries[cur_client->m_structUserLocation->country];
#else
		Location_Struct* lstruct = cur_client->m_structUserLocation;
		if(statsmaps.Locations && !lstruct->locationName.IsEmpty()){
			CRBMap<Location_Struct*, uint_ptr>::CPair *pPairLocation = statsmaps.Locations->Lookup(lstruct);
			if(pPairLocation != NULL)
				++pPairLocation->m_value;
			else
				statsmaps.Locations->SetAt(lstruct, 1);
		}
		Country_Struct*	cstruct = lstruct->country;
		CAtlMap<Country_Struct*, uint_ptr>::CPair *pPair = statsmaps.Countries.Lookup(cstruct);
		if (pPair != NULL)
			pPair->m_value++;
		else
			statsmaps.Countries.SetAt(cstruct, 1);
#endif
...
void CClientList::ResetIP2Country(){
	...
	for(POSITION pos = list.GetHeadPosition(); pos != NULL; ) { 
		cur_client = theApp.clientlist->list.GetNext(pos); 
