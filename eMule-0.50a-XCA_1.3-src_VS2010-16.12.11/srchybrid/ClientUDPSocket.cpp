...
CClientUDPSocket::CClientUDPSocket() : m_bWouldBlock(false), m_port(0)
{
}
		...
		case OP_REASKCALLBACKUDP:
		{
			...
			if( buddy )
			{
				...
				if (!md4cmp(packet, buddy->GetBuddyID()))
				{
					...
					response->pBuffer = new char[size-10];
...
//>>> WiZaRd::ClientAnalyzer
				if (sender->GetRequestFile() == reqfile && sender->IsCompleteSource())
				{
		...
		case OP_DIRECTCALLBACKREQ:
		{
			...
			if (Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsFirewalled())
			{
				...
				if (pRequester == NULL) {
					...
					*/	//Enig123::optimization - SkipDupCheck
					theApp.clientlist->AddClient(pRequester, true);
				}
...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32 dwIP, uint16 nPort){
	...
	*/	//Enig123
	...
	if (iResult == (uint32)SOCKET_ERROR){
...
bool CClientUDPSocket::Create()
{
	...
	{
		ret = CAsyncSocketEx::Create(thePrefs.GetUDPPort(), SOCK_DGRAM, FD_READ | FD_WRITE, thePrefs.GetBindAddrW()) != FALSE;// X: [SUDPS] - [CAsyncSocketEx UDP Support]
		if (ret)
		{
