...
BEGIN_MESSAGE_MAP(CCollectionCreateDialog, CModResizableDialog) // NEO: MLD - [ModelesDialogs] <-- Xanatos --
...
CCollectionCreateDialog::CCollectionCreateDialog(CWnd* pParent /*=NULL*/)
	: CModResizableDialog(CCollectionCreateDialog::IDD, pParent) // NEO: MLD - [ModelesDialogs] <-- Xanatos --
	, m_pCollection(NULL)
...
BOOL CCollectionCreateDialog::OnInitDialog(void)
{
	...
	AddAnchor(m_CollectionAvailListCtrl, TOP_LEFT, BOTTOM_CENTER);// X: [CI] - [Code Improvement]
	AddAnchor(m_CollectionListCtrl, TOP_CENTER, BOTTOM_RIGHT);
	AddAnchor(m_CollectionListLabel, TOP_CENTER);
	AddAnchor(m_CollectionListIcon, TOP_CENTER);
	AddAnchor(m_AddCollectionButton, MIDDLE_CENTER);
	AddAnchor(m_RemoveCollectionButton, MIDDLE_CENTER);
	AddAnchor(m_SaveButton, BOTTOM_RIGHT);
	AddAnchor(m_CancelButton, BOTTOM_RIGHT);
	...
	AddAnchor(m_CollectionNameEdit, BOTTOM_LEFT, BOTTOM_RIGHT);// X: [CI] - [Code Improvement]
	AddAnchor(m_CollectionCreateFormatCheck, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddAnchor(m_CollectionCreateSignNameKeyCheck, BOTTOM_LEFT, BOTTOM_RIGHT);
	...
	for (CCollectionFilesMap::const_iterator it = m_pCollection->m_CollectionFilesMap.begin(); it != m_pCollection->m_CollectionFilesMap.end(); ++it)
		m_CollectionListCtrl.AddFileToList(it->second);
#else
	...
	m_SaveButton.EnableWindow(m_CollectionListCtrl.GetItemCount() > 0);// X: [CI] - [Code Improvement]
...
{
	CAtlList<CKnownFile*> knownFileList;
...
void CCollectionCreateDialog::UpdateAvailFiles(void)
{
	...
	const CKnownFilesMap& Files_Map = m_bSharedFiles?theApp.sharedfiles->GetSharedFiles():theApp.knownfiles->GetKnownFiles();
#ifdef REPLACE_ATLMAP
	for (CKnownFilesMap::const_iterator it = Files_Map.begin(); it != Files_Map.end(); ++it)
		m_CollectionAvailListCtrl.AddFileToList(it->second);
#else
	/*CAtlMap<CCKey,CKnownFile*,CCKeyTraits> Files_Map;
	if (m_bSharedFiles)
