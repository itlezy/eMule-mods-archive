...
BOOL CCommentDialog::OnInitDialog()
{
	...
	AddAnchor(m_lstComments, TOP_LEFT, BOTTOM_RIGHT);// X: [CI] - [Code Improvement]
...
BOOL CCommentDialog::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		for (int i = 0; i < m_paFiles->GetSize(); i++)
		{
			...
			CKnownFile* file = (CKnownFile*)(*m_paFiles)[i];
...
{ 
    ...
    for (int i = 0; i < m_paFiles->GetSize(); i++)
    {
		...
		CShareableFile* file = (CShareableFile*)(*m_paFiles)[i];
		if (file->IsPartFile())
		{
		...
		else if (!IsKindOfCKnownFile(file)/*!file->IsKindOf(RUNTIME_CLASS(CKnownFile))*/
			|| theApp.sharedfiles->GetFileByID(file->GetFileHash()) == NULL)
			continue;
	
		const CAtlList<Kademlia::CEntry*>& list = file->getNotes();
...
void CCommentDialog::OnBnClickedSearchKad()
{
	if (m_bEnabled && Kademlia::CKademlia::IsConnected())
	{
	    ...
	    for (int i = 0; i < iMaxSearches; i++)
	    {
			//CAbstractFile* file = STATIC_DOWNCAST(CAbstractFile, (*m_paFiles)[i]);
			CShareableFile* file = (CShareableFile*)(*m_paFiles)[i];
 			if (file && IsKindOfCKnownFile(file)/*file->IsKindOf(RUNTIME_CLASS(CKnownFile))*/ && theApp.sharedfiles->GetFileByID(file->GetFileHash()) != NULL)
			{
