...
void CCorruptionBlackBox::Init(EMFileSize nFileSize) {
	m_aaRecords.SetCount((size_t)((uint64)(nFileSize + (uint64)(PARTSIZE - 1)) / (PARTSIZE)));
}
...
void CCorruptionBlackBox::TransferredData(uint64 nStartPos, uint64 nEndPos, const CUpDownClient* pSender){
	...
	size_t nPart = (size_t)(nStartPos / PARTSIZE);
	...
	if (nPart >= m_aaRecords.GetCount()){
		//ASSERT( false );
		m_aaRecords.SetCount(nPart+1);
	}
	size_t posMerge = (size_t)-1;
#ifdef _DEBUG// X: [RDL] - [Remove Debug Log]
	uint64 ndbgRewritten = 0;
#endif
	for (size_t i= 0; i < m_aaRecords[nPart].GetCount(); i++){
		...
		// check if there is already an pending entry and overwrite it
			...
			if (m_aaRecords[nPart][i].m_nStartPos >= nRelStartPos && m_aaRecords[nPart][i].m_nEndPos <= nRelEndPos){
				...
				DEBUG_ONLY( AddDebugLogLine(DLP_DEFAULT, false, _T("CorruptionBlackBox: Debug: %I64u bytes were rewritten and records replaced with new stats (1)"), (nRelEndPos - nRelStartPos)+1) );
				return;
			}
...
#ifdef _DEBUG// X: [RDL] - [Remove Debug Log]
	if (ndbgRewritten > 0){
		AddDebugLogLine(DLP_DEFAULT, false, _T("CorruptionBlackBox: Debug: %I64u bytes were rewritten and records replaced with new stats (2)"), ndbgRewritten);
	}
#endif
}
...
void CCorruptionBlackBox::VerifiedData(uint64 nStartPos, uint64 nEndPos){
	...
	//INT_PTR nDbgOldEntries = m_aaRecords[nPart].GetCount();
	CAtlMap<uint32, uint32> mapDebug;
#endif
	for (size_t i= 0; i < m_aaRecords[nPart].GetCount(); i++){
...
/*#ifdef _DEBUG
	INT_PTR nClients = mapDebug.GetCount();
#else
	INT_PTR nClients = 0;
#endif
	AddDebugLogLine(DLP_DEFAULT, false, _T("Found and marked %I64u recorded bytes of %I64u as verified in the CorruptionBlackBox records, %i(%i) records found, %i different clients"), nDbgVerifiedBytes, (nEndPos-nStartPos)+1, m_aaRecords[nPart].GetCount(), nDbgOldEntries, nClients);*/
}
...
void CCorruptionBlackBox::EvaluateData(size_t nPart)
{
	CAtlArray<uint32> aGuiltyClients;
	for (size_t i= 0; i < m_aaRecords[nPart].GetCount(); i++)
	...
	for(size_t k = 0; k < aGuiltyClients.GetCount();){
		// remove doubles
		for(size_t y = k+1; y < aGuiltyClients.GetCount();){
	...
	if (aGuiltyClients.GetCount() > 0){
		...
		CAtlArray<uint64> aDataCorrupt;
		CAtlArray<uint64> aDataVerified;
		aDataCorrupt.SetCount(aGuiltyClients.GetCount());
		aDataVerified.SetCount(aGuiltyClients.GetCount());
		for (size_t j = 0; j < aGuiltyClients.GetCount(); j++)
			aDataCorrupt[j] = aDataVerified[j] = 0;

		// now the parsing
		for (size_t nPart = 0; nPart < m_aaRecords.GetCount(); nPart++){
			for (size_t i = 0; i < m_aaRecords[nPart].GetCount(); i++){
				for(size_t k = 0; k < aGuiltyClients.GetCount(); k++){
		...
		for(size_t k = 0; k < aGuiltyClients.GetCount(); k++){
			...
			size_t nCorruptPercentage;
			if ((aDataVerified[k] + aDataCorrupt[k]) > 0)
				nCorruptPercentage = (size_t)(((uint64)aDataCorrupt[k]*100)/(aDataVerified[k] + aDataCorrupt[k]));
			else {
				AddDebugLogLine(DLP_HIGH, false, _T("CorruptionBlackBox: Program Error: No records for guilty client found!")); //Spike2 - fixed typo ;)
				ASSERT( false );
...
#ifdef CLIENTANALYZER
					theApp.antileechlist->AddCorruptPartSender(pEvilClient->GetUserHash());
#endif
