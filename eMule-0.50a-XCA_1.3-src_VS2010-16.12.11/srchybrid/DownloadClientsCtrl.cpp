...
void CDownloadClientsCtrl::SetAllIcons() 
{
	...
	theApp.SetClientIcon(m_ImageList);//0-5+3
			...
			switch(iColumn){
				case 0:{
...
#endif					
					{
						...
						if (client->credits && client->credits->GetMyScoreRatio(client->GetIP()) > 1.0f)
							image++;
					}
...
#define	DrawClientPercentText	dc.DrawText(szItem, -1, &cur_rec, ((MLC_DT_TEXT | DT_RIGHT) & ~DT_LEFT) | DT_CENTER)
				...
				default:
					if(szItem[0] != 0)
						dc.DrawText(szItem, -1, &cur_rec, (iColumn== 2 || iColumn == 7) ? MLC_DT_TEXT : MLC_DT_TEXT | DT_RIGHT);
					break;
			}
		}
...
void CDownloadClientsCtrl::GetItemDisplayText(const CUpDownClient *client, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 4:
			if(thePrefs.GetUseDwlPercentage() && client->GetHisCompletedPartsPercent_Down() >=0)
		...
		case 7:
			...
			{
				case SF_SERVER:
					_tcsncpy(pszText, GetResString(IDS_SERVER), cchTextMax);
					break;
				case SF_KADEMLIA:
