...
#undef SCHEME_ENTRY
	...
	if (wcsncmp(pszUrl, PREFIXW, _countof(PREFIXW)-1) == 0)
		return TRUE;
	return FALSE;
}
...
LPCSTR GetFileExtA(LPCSTR pszPathA, size_t iLen /*= -1*/)
{
	// Just search the last '.'-character which comes after an optionally 
	// available last '\'-char.
	size_t iPos = iLen != (size_t)-1 ? iLen : strlen(pszPathA);
	while (iPos-- > 0)
	{
		if (pszPathA[iPos] == '.')
...
// This function is thought to be used only for filenames which have been
// validated by 'GetFullPathName' or similar functions.
LPCWSTR GetFileExtW(LPCWSTR pszPathW, size_t iLen /*= -1*/)
{
	// Just search the last '.'-character which comes after an optionally 
	// available last '\'-char.
	size_t iPos = iLen != (size_t)-1 ? iLen : wcslen(pszPathW);
	while (iPos-- > 0)
	{
		if (pszPathW[iPos] == L'.')
...
HRESULT CMainFrameDropTarget::AddUrlFileContents(LPCTSTR pszFileName)
{
	...
	_tsplitpath_s(pszFileName, NULL, 0, NULL, 0, NULL, 0, szExt, _countof(szExt));
