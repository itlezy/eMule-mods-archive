...
CED2KFileLink::CED2KFileLink(const TCHAR* pszName, const TCHAR* pszSize, const TCHAR* pszHash, 
							 const CAtlArray<CString>& astrParams, const TCHAR* pszSources)
	: m_size(pszSize)
{
	...
	if (_tstoi64(pszSize)<=0)
		throw GetResString(IDS_ERR_NOTAFILELINK);
	//if (_tstoi64(pszSize) > OLD_MAX_EMULE_FILE_SIZE && !thePrefs.CanFSHandleLargeFiles()) // X: [TD] - [TempDir] moved
		//throw GetResString(IDS_ERR_FSCANTHANDLEFILE);
	
	if(!wcsmd4(pszHash, m_hash))
		throw GetResString(IDS_ERR_ILLFORMEDHASH);

	bool bError = false;
	for (size_t i = 0; !bError && i < astrParams.GetCount(); i++)
	{
		...
		else if (strTok == _T('p'))
		{
			...
			while (!strHash.IsEmpty())
			{
				...
				if (strHash.GetLength() != 16*2 || !wcsmd4(strHash, aucPartHash)){
					bError = true;
					break;
				}
		...
		else if (strTok == _T('h'))
		{
			...
			if (!strHash.IsEmpty())
			{
				if (DecodeBase32(strHash, m_AICHHash.GetRawHash(), HASHSIZE) == HASHSIZE){
		...
		if (strTok == _T("file"))
		{
			...
			if (!strName.IsEmpty())
			{
				...
				if (!strSize.IsEmpty())
				{
					...
					if (!strHash.IsEmpty())
					{
						CAtlArray<CString> astrEd2kParams;
