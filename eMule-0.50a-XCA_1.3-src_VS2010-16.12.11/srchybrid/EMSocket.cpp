...
#if defined(HAVE_VISTA_SDK) || defined(HAVE_WIN7_SDK_H)
#include "NetF.h"
#endif
// netfinity: end
...
namespace {
#ifdef EMSOCKET_DEBUG // X: [RDL] - [Remove Debug Log]
	inline void EMTrace(char* fmt, ...) {
		va_list argptr;
		char bufferline[512];
		va_start(argptr, fmt);
		_vsnprintf(bufferline, _countof(bufferline) - 1, fmt, argptr);
		va_end(argptr);
		//(Ornis+)
...
//			case PROXYTYPE_HTTP10: // netfinity: Adapted for new AsyncSocketEx code
		...
		Create(0, SOCK_STREAM, FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE, thePrefs.GetBindAddrW());
...
void CEMSocket::OnReceive(int nErrorCode){
	...
	//static char GlobalReadBuffer[2000000]; //Xman include ACK
...
//Xman include ACK end
{
	...
	*/
	...
	uint32 readMax = sizeof(GlobalReadBuffer) - (uint32)pendingHeaderSize;
	//zz_fly
	//netfinity: Special case when socket is closing but data still in buffer, need to empty buffer or deadlock forever
	/*
	...
	*/
	...
	while ((rend - rptr >= PACKET_HEADER_SIZE) || ((pendingPacket != NULL) && (rend - rptr > 0)))
	{
		...
		if(pendingPacket == NULL){
			...
			*/	//Enig123::optimizations from eMule Plus
			Header_Struct *pNewHeader = reinterpret_cast<Header_Struct*>(rptr);
			...
			*/
			if(pNewHeader->packetlength > sizeof(GlobalReadBuffer)) {
				/*
				delete pendingPacket;
				pendingPacket = NULL;
				*/
...
void CEMSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32 actualPayloadSize, bool bForceImmediateSend){
    ...
    if (byConnected == ES_DISCONNECTED) {
        ...
        if (!delpacket){
			...
			*/	//Enig123::bugfix by Xanatos [cyrex2001]
			packet = new Packet(packet);	//Enig123::bugfix by Xanatos [cyrex2001]
	    }
...
SocketSentBytes CEMSocket::Send(uint32 maxNumberOfBytesToSend, uint32 /*minFragSize*/, bool onlyAllowedToSendControlPacket) {
	...
	if(IsEncryptionLayerReady())
	{
		...
		while(sentStandardPacketBytesThisCall + sentControlPacketBytesThisCall < maxNumberOfBytesToSend && anErrorHasOccured == false && // don't send more than allowed. Also, there should have been no error in earlier loop
			(!controlpacket_queue.IsEmpty()
				|| (!onlyAllowedToSendControlPacket && !standartpacket_queue.IsEmpty()) //zz_fly :: TODO, why we call SendControlData() when we have not control packet to send?
			...
			) 
		{
			...
			if(sendbuffer == NULL) {
				...
				if(!controlpacket_queue.IsEmpty()) {
					...
					m_currentPacket_is_controlpacket = true;
					curPacket = controlpacket_queue.RemoveHead();
				} else if(!onlyAllowedToSendControlPacket 
					&& !standartpacket_queue.IsEmpty()) { //Xman look for 4.2: I use this code, although it is redundant, but there were a few very suspicious crashes at this point
					// There's a standard packet to send
					m_currentPacket_is_controlpacket = false;
					StandardPacketQueueEntry queueEntry = standartpacket_queue.RemoveHead();
					curPacket = queueEntry.packet;
