...
void CEditDelayed::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == DELAYED_EVALUATE_TIMER_ID)
	{
		...
		if (cur_tick - m_dwLastModified >= 400)
		{
			DoDelayedEvalute();
			m_dwLastModified = cur_tick;
		}
	}
...
void CEditDelayed::OnInit(CFilterItem&fi)
{
	UpdateFrom(fi);
	SetEditRect(false);
	CRect rectWindow;
...
void CEditDelayed::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (m_pctrlColumnHeader != NULL)
	{
		if (point.x <= ICON_LEFTSPACE)
		{
			...
			int nIdx;
			size_t ignoredColums = m_aIgnoredColums;
			for (int i = 0; i < nCount ;i++) {
				nIdx = m_pctrlColumnHeader->OrderToIndex(i);
				m_pctrlColumnHeader->GetItem(nIdx, &hdi);
				szBuffer[_countof(szBuffer) - 1] = _T('\0');
				if(ignoredColums!=0){
					bool ignore = (ignoredColums & 0x01) == 1;
					ignoredColums>>=1;
					if(ignore) // ignore hidden columns
						continue;
				}
				if (hdi.cxy > 0)
					menu.AppendMenu(MF_STRING | ((m_nCurrentColumnIdx == (size_t)nIdx) ? MF_CHECKED : MF_UNCHECKED), MP_FILTERCOLUMNS + nIdx, hdi.pszText);
			}
...
void CEditDelayed::ShowColumnText(bool bShow,bool bForce)
{
	if (bShow)
	{
		if (!bForce && GetWindowTextLength() != 0 && !m_bShowsColumnText) {
			ASSERT(0);
			return;
		}
...
BOOL CEditDelayed::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{
		if (m_nCurrentColumnIdx != wParam - MP_FILTERCOLUMNS)
		{
...
void CEditDelayed::SaveTo(CFilterItem&fi){
	if(m_pctrlColumnHeader != &fi.m_ctlListHeader)
		return;
	fi.m_strEvaluatedContent=m_strLastEvaluatedContent;
	fi.m_nFilterColumn=m_nCurrentColumnIdx;
}

void CEditDelayed::UpdateFrom(CFilterItem&fi){
	if(m_pctrlColumnHeader == &fi.m_ctlListHeader)
		return;
	m_pctrlColumnHeader=&fi.m_ctlListHeader;
	m_nCurrentColumnIdx=fi.m_nFilterColumn;
	m_strLastEvaluatedContent=fi.m_strEvaluatedContent;
	m_aIgnoredColums=fi.IgnoredColums;
	if(m_strLastEvaluatedContent.GetLength()>0||GetFocus()==(CWnd*)this){
		m_bShowsColumnText=false;
		SetWindowText(m_strLastEvaluatedContent);
		SetSel((DWORD)-1);
	}
	else
		ShowColumnText(true,true);
}
