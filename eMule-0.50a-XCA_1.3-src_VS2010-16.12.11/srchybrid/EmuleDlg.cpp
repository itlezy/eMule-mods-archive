...
#include "SearchParams.h" // X: [BF] - [Bug Fix] cause ASSERT when AddAnchor
#include "SplashScreenEx.h"//Xman new slpash-screen arrangement
...
BEGIN_MESSAGE_MAP(CemuleDlg, CTrayDialog)
	...
	ON_COMMAND(SC_MINIMIZE, MinimizeWindow)//speedgraph
	// quick-speed changer -- 
	/*ON_COMMAND_RANGE(MP_QS_U20, MP_QS_UP10, QuickSpeedUpload)// X: [UIC] - [UIChange] remove SysOptions
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	// ==> Invisible Mode [TPT/MoNKi] - Stulle
	m_bStartMinimized = !thePrefs.IsFirstStart() && (thePrefs.GetStartMinimized() || theApp.DidWeAutoStart() ||(thePrefs.m_bInvisibleMode && thePrefs.m_bInvisibleModeStart));
	// <== Invisible Mode [TPT/MoNKi] - Stulle
	//Xman new slpash-screen arrangement
	/*
	...
	*/
	...
	SetAllIcons();
	ShowConnectionState();//Localize();// X: [RUL] - [Remove Useless Localize]
	...
	if (m_bStartMinimized)
	{
		...
		*/
		if (*thePrefs.GetMinTrayPTR() || thePrefs.m_bInvisibleMode && (theApp.DidWeAutoStart() || thePrefs.m_bInvisibleModeStart))
		// <== Invisible Mode [TPT/MoNKi] - Stulle
		{
...
/*
	...
	// initalize PeerCache
	//theApp.m_pPeerCache->Init(thePrefs.GetPeerCacheLastSearch(), thePrefs.WasPeerCacheFound(), thePrefs.IsPeerCacheDownloadEnabled(), thePrefs.GetPeerCachePort()); // X: [RPC] - [Remove PeerCache]
	...
	try
	{
		switch(theApp.emuledlg->status){
			...
			case 2:
				break;
			case 3:{
				if(theApp.m_pSplashWnd // shenjunqiu
					&& (thePrefs.GetWindowsVersion() >= _WINVER_VISTA_))//Xman new slpash-screen arrangement
					theApp.m_pSplashWnd->RedrawWindow();
				bool bError = false;
				theApp.emuledlg->status++;
...
void CemuleDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	/*if (nID >= MP_QS_U20 && nID <= MP_QS_UP10) {// X: [UIC] - [UIChange] remove SysOptions
...
void CemuleDlg::ShowMessageState(uint_ptr iconnr)
{
	m_iMsgBlinkState = false;
	m_iMsgIcon = iconnr;
...
CString CemuleDlg::GetUpDatarateString(/*UINT uUpDatarate*/)
{
	...
	if (thePrefs.ShowOverhead())
		...
		*/
		...
		_sntprintf(szBuff, _countof(szBuff) - 1, _T("%.1f"), (float)m_uUpDatarate/1024);
...
CString CemuleDlg::GetDownDatarateString(/*UINT uDownDatarate*/)
{
	...
	if (thePrefs.ShowOverhead())
		...
		*/
		...
		_sntprintf(szBuff, _countof(szBuff) - 1, _T("%.1f"), (float)m_uDownDatarate/1024);
	szBuff[_countof(szBuff) - 1] = _T('\0');
	return szBuff;
}
...
CString CemuleDlg::GetTransferRateString()
{
	TCHAR szBuff[128];
	if (thePrefs.ShowOverhead())
		_sntprintf(szBuff, _countof(szBuff) - 1, GetResString(IDS_UPDOWN),
				  (float)m_uUpDatarate/1024, (float)m_uploadOverheadRate/1024,
				  (float)m_uDownDatarate/1024, (float)m_downloadOverheadRate/1024);
	//Xman end
	else
		_sntprintf(szBuff, _countof(szBuff) - 1, GetResString(IDS_UPDOWNSMALL), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024);
	szBuff[_countof(szBuff) - 1] = _T('\0');
	return szBuff;
}
...
//<<< WiZaRd::ToolTip-FiX
	...
	if (IsWindowVisible() || bForceAll)
	{
		...
		{
			TCHAR szBuff[128];
		_sntprintf(szBuff, _countof(szBuff) - 1, _T("(U:%.1f D:%.1f) eMule v%s  (%s)"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong, CastSecondsToLngHM(sessionRunTime));
			szBuff[_countof(szBuff) - 1] = _T('\0');
			SetWindowText(szBuff);
		}
	}
	if (m_pMiniMule && m_pMiniMule->m_hWnd && m_pMiniMule->IsWindowVisible() && !thePrefs.m_bMiniMuleAutoClose)
	{
		...
		switch (pLink->GetKind()) {
			case CED2KLink::kFile:
			{
				...
				if(theApp.knownfiles->CheckAlreadyDownloadedFileQuestion(pFileLink->GetHashKey(),pFileLink->GetName()))
					theApp.downloadqueue->AddFileLinkToDownload(pFileLink,transferwnd->GetDownloadList()->curTab);// X: [AC] - [ActionChange] use transferwnd's select cat as default cat
				//Xman end
			}
			break;
			...
			case CED2KLink::kFriend:
				{
					...
					if (theApp.friendlist->SearchFriend(userHash,0,0) == NULL )
						theApp.friendlist->AddFriend(userHash, 0, 0, 0, 0, pFriendLink->GetUserName(), 1);
				}
				break;
	...
	if (data->dwData == OP_ED2KLINK)
	{
		...
		if (clcommand==_T("status")) {
			...
			if (file){
				...
				// Xman
				// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
				strBuff.Format(GetResString(IDS_UPDOWNSMALL), (float)m_uDownDatarate/1024, (float)m_uUpDatarate/1024);
				// Maella end
...
//XMessageBox +
		...
		int nResult = XMessageBox(GetSafeHwnd(), GetResString(IDS_MAIN_EXIT), GetResString(IDS_CLOSEEMULE), MB_YESNO | MB_DEFBUTTON1 | MB_DONOTASKAGAIN | MB_ICONQUESTION, &params);
//XMessageBox -
...
//			}
	...
	//transferwnd->GetDownloadList()->SaveSettings();
	//transferwnd->GetDownloadClientsList()->SaveSettings();
	//transferwnd->GetUploadList()->SaveSettings();
	//transferwnd->GetQueueList()->SaveSettings();
	//transferwnd->GetClientList()->SaveSettings();
	...
	//theApp.m_pPeerCache->Save(); // X: [RPC] - [Remove PeerCache]
	...
	//delete theApp.m_pPeerCache;		theApp.m_pPeerCache = NULL; // X: [RPC] - [Remove PeerCache]
	...
	theApp.UpdateSplash(_T("Unloading IP to Country ..."));  //Xman new slpash-screen arrangement
...
//>>> WiZaRd::ClientAnalyzer
	...
	delete theApp.antileechlist;	theApp.antileechlist = NULL;
//<<< WiZaRd::ClientAnalyzer
#endif	
...
void CemuleDlg::UpdateTrayIcon()
{
	...
	// prepare it up
	COLORREF aColors[2] = { thePrefs.GetStatsColor(14), thePrefs.GetStatsColor(11) }; // set the corresponding color for each level
	m_TrayIcon.Init(
		theApp.IsConnected()?
	...
	int aiVals[2] = { iUpRateProcent, iDownRateProcent };
...
void CemuleDlg::Localize()// X: [RUL] - [Remove Useless Localize]
{
	...
	transferwnd->Localize();
	searchwnd->Localize();
	sharedfileswnd->LocalizeAll();
	chatwnd->Localize();
	chatwnd->m_FriendListCtrl.Localize();
	kademliawnd->LocalizeAll();

	CMenu* pSysMenu = GetSystemMenu(FALSE);
	if (pSysMenu)
	{
		if (!thePrefs.GetMinToTray())// X: [AL] - [Additional Localize]
			VERIFY( pSysMenu->ModifyMenu(MP_MINIMIZETOTRAY, MF_BYCOMMAND | MF_STRING, MP_MINIMIZETOTRAY, GetResString(IDS_PW_TRAY)) );
...
/*		UINT uOptMenuPos = pSysMenu->GetMenuItemCount() - 1;// X: [UIC] - [UIChange] remove SysOptions
...
/*// X: [UIC] - [UIChange] remove SysOptions
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		case MP_HM_REMOVEALLBANNEDCLIENTS:{ // X: [DUL] - [Don't Upload to Leecher]
...
void CemuleDlg::ShowToolPopup(bool toolsonly, bool speedgraph) // X: [SGW] - [SpeedGraphWnd]
{
	POINT point;
	::GetCursorPos(&point);
	...
	menu.AddMenuTitle(speedgraph?NULL:(LPCTSTR)(GetResString(toolsonly?IDS_TOOLS:IDS_HOTMENU)), true);
	...
	CString schedonoff = GetResString((!thePrefs.IsSchedulerEnabled())?IDS_HM_SCHED_ON:IDS_HM_SCHED_OFF);
	...
	if (theApp.scheduler->GetCount()>0) {
		...
		for (size_t i=0; i<theApp.scheduler->GetCount();i++)
	...
	menu.AppendMenu(MF_STRING,MP_VIEWCOLLECTION, GetResString(IDS_VIEWCOLLECTION));
	menu.AppendMenu(MF_STRING,MP_HM_QUICKSTART,GetResString(thePrefs.m_bOnQuickStart ? IDS_X_QUICK_START_STOP : IDS_X_QUICK_START_START)); // NEO: QS - [QuickStart] <-- Xanatos --
	if(!speedgraph){
		menu.AppendMenu(MF_STRING,MP_HM_RESUME_DOWNLOAD, GetResString(IDS_SW_RESUMEDOWNLOAD) + _T("..."));
		menu.AppendMenu(MF_STRING,MP_HM_REMOVEALLBANNEDCLIENTS, GetResString(IDS_SW_REMOVEALLBANNEDCLIENTS)); // X: [DUL] - [Don't Upload to Leecher]
	}
	if(theApp.m_pSpeedGraphWnd)
		menu.AppendMenu(thePrefs.showSpeedGraph?MF_CHECKED:MF_UNCHECKED,MP_HM_TOGGLESPEEDGRAPH, GetResString(IDS_TOGGLESPEEDGRAPH)); // X: [SGW] - [SpeedGraphWnd]
	menu.AppendMenu(MF_SEPARATOR);
	...
	if(speedgraph && !IsWindowVisible()||IsIconic()){
	menu.AppendMenu(MF_SEPARATOR);
		menu.AppendMenu(MF_STRING,MP_RESTORE, GetResString(IDS_MAIN_POPUP_RESTORE));
	}
    else if (!toolsonly||speedgraph) {
		menu.AppendMenu(MF_SEPARATOR);
		menu.AppendMenu(MF_STRING,SC_MINIMIZE, GetResString(IDS_PW_TRAY));
	}
	if (!toolsonly||speedgraph) {
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				if (dwResult > *(uint32*)abyCurVer){
					SetActiveWindow();
#ifndef _BETA
					Log(LOG_SUCCESS|LOG_STATUSBAR|LOG_DONTNOTIFY,GetResString(IDS_NEWVERSIONAVL));
					...
					if (!thePrefs.GetNotifierOnNewVersion()){
						...
						XMSGBOXPARAMS params;
						params.nTimeoutSeconds = 10;
						if (XMessageBox(GetSafeHwnd(), GetResString(IDS_NEWVERSIONAVL)+GetResString(IDS_VISITVERSIONCHECK), _T("eMule"), MB_YESNO | MB_DEFBUTTON2 | MB_ICONINFORMATION, &params) == IDYES) {
							CString theUrl;
							theUrl.Format(_T("/en/version_check.php?version=%i&language=%i"),theApp.m_uCurVersionCheck,thePrefs.GetLanguageID());
							theUrl = thePrefs.GetVersionCheckBaseURL()+theUrl;
							ShellExecute(NULL, NULL, theUrl, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
...
LRESULT CemuleDlg::OnWebServerClearCompleted(WPARAM wParam, LPARAM lParam) // X: [AEWI] - [AJAX Enabled Web Interface] remove clear specific file
{
	//if(!wParam)
	//{
	size_t cat=(size_t)lParam;
...
LRESULT CemuleDlg::OnWebGUIInteraction(WPARAM wParam, LPARAM lParam) {
	...
	switch (wParam) {
		...
		case WEBGUIIA_WINFUNC:{
			if (thePrefs.GetWebAdminAllowedHiLevFunc())
			{
				try {
					...
					{
						AddLogLine(true, GetResString(lParam==1?IDS_WEB_SHUTDOWNSYSTEM:IDS_WEB_REBOOT) + _T(' ') + GetResString(IDS_FAILED)); // X: [BF] - [Bug Fix]
				}
			}
			else 
				AddLogLine(true, GetResString(lParam==1?IDS_WEB_SHUTDOWNSYSTEM:IDS_WEB_REBOOT) + _T(' ') + GetResString(IDS_ACCESSDENIED)); // X: [BF] - [Bug Fix]
			break;
		}
		...
		case WEBGUIIA_UPDATESERVERMETFROMURL:
			serverwnd->UpdateServerMetFromURL((TCHAR*)lParam);
			break;
		case WEBGUIIA_SHOWSTATISTICS:
			statisticswnd->ShowStatistics(lParam!=0);
			break;
		case WEBGUIIA_DELETEALLSEARCHES:
			searchwnd->DeleteAllSearches();
			break;
		...
		case WEBGUIIA_KAD_START:
			Kademlia::CKademlia::Start();
			theApp.emuledlg->ShowConnectionState(); // X: [BF] - [Bug Fix]
			break;
...
BOOL CemuleDlg::OnDeviceChange(UINT nEventType, DWORD_PTR dwData)
{
	...
	if ((nEventType == DBT_DEVICEARRIVAL || nEventType == DBT_DEVICEREMOVECOMPLETE) && !IsBadReadPtr((void *)dwData, sizeof(DEV_BROADCAST_HDR)))
	{
		...
		if (pHdr->dbch_devicetype == DBT_DEVTYP_VOLUME && !IsBadReadPtr((void *)dwData, sizeof(DEV_BROADCAST_VOLUME)))
		{
			...
			for (size_t uDrive = 0; uDrive <= 25; uDrive++)
			{
				UINT uMask = 1 << uDrive;
				if (pVol->dbcv_unitmask & uMask)
				{
					...
					if (pVol->dbcv_flags & (DBTF_MEDIA | DBTF_NET))
						g_VolumeInfo.ClearCache(uDrive); // X: [FSFS] - [FileSystemFeaturesSupport]
...
//>>> WiZaRd::Additional Thumbbuttons
			case TBB_OPENINC:
				m_thbButtons[i].hIcon   =  theApp.LoadIcon(L"OPENFOLDER", 16, 16);
				tooltip = GetResString(IDS_OPENINC)+L"...";
				break;
//<<< WiZaRd::Additional Thumbbuttons
		}
