...
//Copyright (C)2002-2009 Merkur ( strEmail.Format("%s@%s", "devteam", "emule-project.net") / http://www.emule-project.net )
//
...
int CEncryptedDatagramSocket::EncryptSendClient(uchar** ppbyBuf, int nBufLen, const uchar* pachClientHashOrKadID, bool bKad, uint32 nReceiverVerifyKey, uint32 nSenderVerifyKey) const{
	...
	SFMT&rng = *t_rng;
	uint16 nRandomKeyPart = rng.getUInt16();//(uint16)cryptRandomGen.GenerateWord32(0x0000, 0xFFFF);
	MD5Sum md5;
	if (bKad){
	...
	for (int j = 0; j < byPadLen; j++){
		uint8 byRand = rng.getUInt8();	// they actually dont really need to be random, but it doesn't hurts either
