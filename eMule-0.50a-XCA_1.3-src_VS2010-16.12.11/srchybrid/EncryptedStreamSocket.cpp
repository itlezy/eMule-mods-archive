...
: m_fiReceiveBuffer(512), m_fiSendBuffer(128)
{
...
int CEncryptedStreamSocket::Send(const void* lpBuf, int nBufLen, int nFlags){
	...
	else if (m_bServerCrypt && m_StreamCryptState == ECS_ENCRYPTING && m_fiSendBuffer.GetLength() != 0){
...
bool CEncryptedStreamSocket::IsEncryptionLayerReady(){
	return ( (m_StreamCryptState == ECS_NONE || m_StreamCryptState == ECS_ENCRYPTING || m_StreamCryptState == ECS_UNKNOWN )
		&& (m_fiSendBuffer.GetLength() == 0 || (m_bServerCrypt && m_NegotiatingState == ONS_BASIC_SERVER_DELAYEDSENDING)) );
}
...
int CEncryptedStreamSocket::Receive(void* lpBuf, int nBufLen, int nFlags){
	...
	switch (m_StreamCryptState) {
		...
		case ECS_NEGOTIATING:{
			...
			if (nRead == (-1))
				return 0;
			if (nRead != (uint32)m_nObfuscationBytesReceived && m_StreamCryptState != ECS_ENCRYPTING){
				// this means we have more data then the current negotiation step required (or there is a bug) and this should never happen
...
void CEncryptedStreamSocket::SetConnectionEncryption(bool bEnabled, const uchar* pTargetClientHash, bool bServerConnection){
	...
	if (bEnabled && pTargetClientHash != NULL && !bServerConnection){
		...
		m_nRandomKeyPart = t_rng->getUInt32();//cryptRandomGen.GenerateWord32();
...
void CEncryptedStreamSocket::StartNegotiation(bool bOutgoing){
	...
	if (!bOutgoing){
		...
		SFMT&rng = *t_rng;
		size_t byPadding = rng.getUInt8()/*cryptRandomGen.GenerateByte()*/ % (thePrefs.GetCryptTCPPaddingLength() + 1);
		CSafeMemFile fileRequest(1+4+4+1+1+1+byPadding);
		...
		for (size_t i = 0; i < byPadding; i++)
			fileRequest.WriteUInt8(rng.getUInt8()/*cryptRandomGen.GenerateByte()*/);
		...
		SFMT&rng = *t_rng;
		size_t byPadding = rng.getUInt8()/*cryptRandomGen.GenerateByte()*/ % 16; // add random padding
		CSafeMemFile fileRequest(1+PRIMESIZE_BYTES+1+byPadding);
		const uint8 bySemiRandomNotProtocolMarker = GetSemiRandomNotProtocolMarker();	
		fileRequest.WriteUInt8(bySemiRandomNotProtocolMarker);
...
int CEncryptedStreamSocket::Negotiate(const uchar* pBuffer, uint32 nLen){
	...
	//Enig123::don't have to be members
	uint8 m_dbgbyEncryptionSupported = 0xFF;
	uint8 m_dbgbyEncryptionRequested = 0xFF;
	uint8 m_dbgbyEncryptionMethodSet = 0xFF;
		...
		while (m_NegotiatingState != ONS_COMPLETE && m_nReceiveBytesWanted > 0){		
			...
			switch (m_NegotiatingState){
				...
				case ONS_BASIC_CLIENTA_RANDOMPART:{
					...
					achKeyData[16] = MAGICVALUE_REQUESTER;
					m_fiReceiveBuffer.Read(achKeyData + 17, 4); // random key part sent from remote client
				...
				case ONS_BASIC_CLIENTA_MAGICVALUE:{
					uint32 dwValue = m_fiReceiveBuffer.ReadUInt32();
				...
				case ONS_BASIC_CLIENTA_METHODTAGSPADLEN:
					m_dbgbyEncryptionSupported = m_fiReceiveBuffer.ReadUInt8();
					m_dbgbyEncryptionRequested = m_fiReceiveBuffer.ReadUInt8();
					...
					m_nReceiveBytesWanted = m_fiReceiveBuffer.ReadUInt8();
				...
				case ONS_BASIC_CLIENTA_PADDING:{
					...
					int nSockAddrLen = sizeof(sockAddr);
					GetPeerName((SOCKADDR*)&sockAddr, &nSockAddrLen);
					const size_t byPaddingLen = theApp.serverconnect->AwaitingTestFromIP(sockAddr.sin_addr.S_un.S_addr) ? 16 : (thePrefs.GetCryptTCPPaddingLength() + 1);
					SFMT&rng = *t_rng;
					size_t byPadding = rng.getUInt8()/*cryptRandomGen.GenerateByte()*/ % byPaddingLen;
					// ignore the random bytes, send the response, set status complete
					CSafeMemFile fileResponse(4+1+1+byPadding);
					...
					for (size_t i = 0; i < byPadding; i++)
						fileResponse.WriteUInt8(rng.getUInt8());
				...
				case ONS_BASIC_CLIENTB_MAGICVALUE:{
					if (m_fiReceiveBuffer.ReadUInt32() != MAGICVALUE_SYNC){
				...
				case ONS_BASIC_CLIENTB_METHODTAGSPADLEN:{
					m_dbgbyEncryptionMethodSet = m_fiReceiveBuffer.ReadUInt8();
				...
				case ONS_BASIC_SERVER_DHANSWER:{
					...
					m_fiReceiveBuffer.Read(aBuffer, PRIMESIZE_BYTES);
				...
				case ONS_BASIC_SERVER_PADDING:{
					SFMT&rng = *t_rng;
					size_t byPadding = (uint8)(rng.getUInt8()/*cryptRandomGen.GenerateByte()*/ % 16);
...
int CEncryptedStreamSocket::SendNegotiatingData(const void* lpBuf, uint32 nBufLen, uint32 nStartCryptFromByte, bool bDelaySend){	
	...
	if (lpBuf != NULL){
		...
		if (m_fiSendBuffer.GetLength() != 0){
			...
			if (m_NegotiatingState == ONS_BASIC_SERVER_DELAYEDSENDING)
				...
				ASSERT( false );
			m_fiSendBuffer.SeekToEnd();
			m_fiSendBuffer.Write(pBuffer, nBufLen);
			delete [] pBuffer;
			pBuffer = NULL;
	...
	if (lpBuf == NULL || bProcess){
		...
		if (m_fiSendBuffer.GetLength() == 0 || nStartCryptFromByte != 0){
			ASSERT( false );
			return 0;							// or not
		}
		nBufLen = (uint32)m_fiSendBuffer.GetLength();
		pBuffer = new BYTE[nBufLen];
		memcpy(pBuffer, m_fiSendBuffer.GetBuffer(), nBufLen);
		m_fiSendBuffer.SetLength(0);
	}
    ASSERT( m_fiSendBuffer.GetLength() == 0 );
	uint32 result = 0;	
	if (!bDelaySend)
	...
	if (result == (uint32)SOCKET_ERROR || bDelaySend){
		...
		if (result < nBufLen){
			m_fiSendBuffer.Write(pBuffer + result, nBufLen - result);			
		}
		delete [] pBuffer;
		return result;
	}
...
uint8 CEncryptedStreamSocket::GetSemiRandomNotProtocolMarker() const{
	...
	int i;
	for (i = 0; i < 128; i++){
		bySemiRandomNotProtocolMarker = rng.getUInt8();//cryptRandomGen.GenerateByte();
		bool bOk = false;
