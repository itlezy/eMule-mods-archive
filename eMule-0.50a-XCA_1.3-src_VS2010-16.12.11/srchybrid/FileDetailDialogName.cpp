...
BOOL CFileDetailDialogName::OnInitDialog()
{
	...
	AddAnchor(m_listFileNames, TOP_LEFT, BOTTOM_RIGHT);// X: [CI] - [Code Improvement]
...
BOOL CFileDetailDialogName::OnSetActive()
{
	...
	{
		m_bSelf = true;
		//SetDlgItemText(IDC_FILENAME,STATIC_DOWNCAST(CPartFile, (*m_paFiles)[0])->GetFileName());
		SetDlgItemText(IDC_FILENAME,((CPartFile*)(*m_paFiles)[0])->GetFileName());
		m_bSelf = false;
		RefreshData();
		m_bDataChanged = false;
	}
...
{
	//m_listFileNames.SaveSettings();// X: [CI] - [Code Improvement]

	for (size_t i=0;i<m_listFileNames.GetItemCount();++i) {
...
void CFileDetailDialogName::FillSourcenameList()
{
	...
	const CPartFile* file = (CPartFile*)(*m_paFiles)[0];
