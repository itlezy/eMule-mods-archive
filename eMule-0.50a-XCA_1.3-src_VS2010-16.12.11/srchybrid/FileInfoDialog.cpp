...
struct SMediaInfoThreadResult
{
	...
	CAtlArray<SMediaInfo>* paMediaInfo;
	CStringA strInfo;
};
...
class CGetMediaInfoThread : public Poco::Thread
{
public:
	CGetMediaInfoThread(HWND hWnd, const CSimpleArray<void*>* paFiles, HFONT hFont) : Thread(true)
	{
		m_hWndOwner = hWnd;
		for (int i = 0; i < paFiles->GetSize(); i++)
			//m_aFiles.Add(STATIC_DOWNCAST(CShareableFile, (*paFiles)[i]));
			m_aFiles.Add((CShareableFile*)(*paFiles)[i]);
		m_hFont = hFont;
	}
	virtual void run();
...
public:
	CMediaInfoDLL()
	{
		...
		m_hLib = NULL;
		m_ullVersion = 0;
		// X: [RMIS] - [Remove MediaInfo 0.4 Support]
	...
	BOOL Initialize()
	{
		if (!m_bInitialized)
		{
			...
			CString strPath = thePrefs.m_strMediaInfoDllPath;
			...
			if (m_hLib == NULL)
			{
				...
				if (key.Open(HKEY_CURRENT_USER, _T("Software\\MediaInfo"), KEY_READ) == ERROR_SUCCESS)
				{
					...
					{
						LPTSTR pszResult = PathCombine(strPath.GetBuffer(MAX_PATH), szPath, 
#ifndef _WIN64
				_T("MEDIAINFO.DLL")
#else
				_T("MEDIAINFOX64.DLL")
...
//	ReplaceRichEditCtrl(&m_fi, this, GetDlgItem(IDC_FD_XI1)->GetFont());// X: [CI] - [Code Improvement] Don't need replace under VS2005+
	...
	if (!m_bReducedDlg)
	{
		...
		AddAnchor(m_fi, TOP_LEFT, BOTTOM_RIGHT);
...
BOOL CFileInfoDialog::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		m_fi.SetWindowText(strWait);

		(new CGetMediaInfoThread(m_hWnd, m_paFiles, (HFONT)GetDlgItem(IDC_FD_XI1)->GetFont()->m_hObject))->start();
		m_bDataChanged = false;
	}
	return TRUE;
}
...
void CGetMediaInfoThread::run()
{
	...
	CAtlArray<SMediaInfo>* paMediaInfo = new CAtlArray<SMediaInfo>;
	try
	{
		CRichEditStream re;
		re.Attach(hwndRE);
...
LRESULT CFileInfoDialog::OnMediaInfoResult(WPARAM, LPARAM lParam)
{
	...
	m_fi.SetWindowText(_T(""));

	SMediaInfoThreadResult* pThreadRes = (SMediaInfoThreadResult*)lParam;
	if (pThreadRes == NULL)
		return 1;
	CAtlArray<SMediaInfo>* paMediaInfo = pThreadRes->paMediaInfo;
	if (paMediaInfo == NULL) {
		delete pThreadRes;
		return 1;
	}

	if (paMediaInfo->GetCount() != m_paFiles->GetSize())
	{
		SetDlgItemText(IDC_FORMAT, _T(""));
		SetDlgItemText(IDC_FILESIZE, _T(""));
		SetDlgItemText(IDC_LENGTH, _T(""));
	...
	for (size_t i = 0; i < paMediaInfo->GetCount(); i++)
	{
	...
	if (ami.fFileLengthSec) {
		CString strLength(CastSecondsToHM((uint_ptr)ami.fFileLengthSec));
...
bool CGetMediaInfoThread::GetMediaInfo(HWND hWndOwner, const CShareableFile* pFile, SMediaInfo* mi, bool bSingleFile)
{
	...
	bool bHasDRM = false;
	if (!pFile->IsPartFile() || ((CPartFile*)pFile)->GetHeaderSizeOnDisk()>1024) {
	...
	if (pFile->IsPartFile())
	{
		...
		if (((CPartFile*)pFile)->GetHeaderSizeOnDisk()<=16*1024)
			return bFoundHeader || !mi->strMimeType.IsEmpty();
	}

	TCHAR szExt[_MAX_EXT];
	_tsplitpath_s(pFile->GetFileName(), NULL, 0, NULL, 0, NULL, 0, szExt, _countof(szExt));
	_tcslwr_s(szExt);
	...
	//
	if (thePrefs.m_bMediaInfo_ID3LIB &&
		(_tcscmp(szExt, _T(".mp3"))==0 || _tcscmp(szExt, _T(".mp2"))==0 || _tcscmp(szExt, _T(".mp1"))==0 || _tcscmp(szExt, _T(".mpa"))==0))
	{
		try
		{
			...
			while ((frame = iter->GetNext()) != NULL)
			{
				...
				{
					wchar_t *sText = ID3_GetStringW(frame, ID3FN_TEXT);
					UINT lLength = _tcstoul(sText, NULL/*, 10*/);
				...
				{
					wchar_t *sText = ID3_GetStringW(frame, ID3FN_TEXT);
					uint_ptr lLength = _tcstoul(sText, NULL/*, 10*/) / 1000;
					if (lLength)
					{
						CString strLength;
				...
				case ID3FID_PICTURE:
				{
					...
					*sFormat   = ID3_GetStringW(frame, ID3FN_IMAGEFORMAT);
					...
					strFidInfo << _T("(") << sDesc << _T(")[") << sFormat << _T(", ")
							   << (UINT) nPicType << _T("]: ") << sMimeType << _T(", ") <<(UINT) nDataSize << _T(" bytes");
					delete[] sMimeType;
					delete[] sDesc;
					delete[] sFormat;
					break;
				...
				case ID3FID_GENERALOBJECT:
				{
					...
					*sFileName = ID3_GetStringW(frame, ID3FN_FILENAME);
					...
					strFidInfo << _T("(") << sDesc << _T(")[")
							   << sFileName << _T("]: ") << sMimeType << _T(", ") <<(UINT) nDataSize << _T(" bytes");
					delete[] sMimeType;
					delete[] sDesc;
					delete[] sFileName;
					break;
				...
				case ID3FID_UNIQUEFILEID:
				{
					...
					strFidInfo << sOwner << _T(", ") <<(UINT) nDataSize << _T(" bytes");
					delete[] sOwner;
					break;
				}
				...
				case ID3FID_POPULARIMETER:
				{
					...
					strFidInfo << sEmail << _T(", counter=") <<(UINT) nCounter << _T(" rating=") <<(UINT) nRating;
					delete[] sEmail;
					break;
				}
				...
				case ID3FID_GROUPINGREG:
				{
					...
					nDataSize = frame->GetField(ID3FN_DATA)->Size();
					strFidInfo << _T("(") <<(UINT) nSymbol << _T("): ") << sOwner << _T(", ") <<(UINT) nDataSize << _T(" bytes");
					break;
				}
	...
	// X: [BF] - [Bug Fix] move down, use id3lib for mp3 file
	////////////////////////////////////////////////////////////////////////////
	// Check for WM file
	//
#ifdef HAVE_WMSDK_H
	bool bIsWM = false;
	if (thePrefs.m_bMediaInfo_WM)
	{
		try
		{
			...
			try
			{
				if (theMediaInfoDLL.Initialize())
				{
					...
					if (Handle)
					{
						...
						struct tm tmUtc = {0};
						if (_stscanf_s(strDate, _T("UTC %u-%u-%u %u:%u:%u"), &tmUtc.tm_year, &tmUtc.tm_mon, &tmUtc.tm_mday, &tmUtc.tm_hour, &tmUtc.tm_min, &tmUtc.tm_sec) == 6)
						{
							tmUtc.tm_mon -= 1;
							tmUtc.tm_year -= 1900;
							tmUtc.tm_isdst = -1;
...
#ifdef HAVE_QEDIT_H
		if (thePrefs.m_bMediaInfo_MediaDet
									...
									if (SUCCEEDED(hr = pMediaDet->get_StreamType(&major_type)))
									{
										if (major_type == MEDIATYPE_Video)
										{
											...
											if (SUCCEEDED(pMediaDet->get_StreamLength(&fLength)) && fLength)
											{
												...
												{
													CString strLength;
													SecToTimeLength((uint_ptr)fLength, strLength);
													mi->strInfo << _T("   ") << GetResString(IDS_LENGTH) << _T(":\t") << strLength;
													if (pFile->IsPartFile()){
														mi->strInfo.SetSelectionCharFormat(mi->strInfo.m_cfRed);
														mi->strInfo << _T(" (") + GetResString(IDS_ESTIMATED)+ _T(')');
...
void CFileInfoDialog::Localize()
{
	...
	SetDlgItemText(IDC_FD_XI9,GetResString(IDS_WIDTH)+_T(" x ")+GetResString(IDS_HEIGHT)+_T(':'));
