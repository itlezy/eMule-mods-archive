...
CFrameGrabThread::CFrameGrabThread(const CKnownFile* in_pOwner, CString in_strFileName,uint8 in_nFramesToGrab, double in_dStartTime, bool in_bReduceColor, uint16 in_nMaxWidth, void* in_pSender)
: Thread(true), strFileName(in_strFileName), nFramesToGrab(in_nFramesToGrab), dStartTime(in_dStartTime), bReduceColor(in_bReduceColor), nMaxWidth(in_nMaxWidth), pOwner(in_pOwner), pSender(in_pSender)
{
}
