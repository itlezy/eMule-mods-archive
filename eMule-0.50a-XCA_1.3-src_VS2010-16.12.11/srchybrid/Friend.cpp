...
CFriend::CFriend(const uchar* abyUserhash, uint64 dwLastSeen, uint32 dwLastUsedIP, uint16 nLastUsedPort, 
				 uint64 dwLastChatted, LPCTSTR pszName, uint32 dwHasHash){// X: [64T] - [64BitTime]
...
void CFriend::LoadFromFile(CFileDataIO* file, bool I64Time)// X: [E64T] - [Enable64BitTime]
{
	...
	if(I64Time){// X: [E64T] - [Enable64BitTime]
		m_dwLastSeen = file->ReadUInt64();
		m_dwLastChatted = file->ReadUInt64();
	}
	else{
		m_dwLastSeen = file->ReadUInt32();
		m_dwLastChatted = file->ReadUInt32();
	}
	size_t tagcount = file->ReadUInt32();
...
void CFriend::WriteToFile(CFileDataIO* file, bool I64Time)// X: [E64T] - [Enable64BitTime]
{
	file->WriteHash16(m_abyUserhash);
	file->WriteUInt32(m_dwLastUsedIP);
	file->WriteUInt16(m_nLastUsedPort);
	if(I64Time){// X: [E64T] - [Enable64BitTime]
		file->WriteUInt64(m_dwLastSeen);
		file->WriteUInt64(m_dwLastChatted);
	}
	else{
		...
		case FCR_USERHASHVERIFIED:
			...
			else
			{
				...
				for (POSITION pos = m_liConnectionReport.GetHeadPosition(); pos != 0; m_liConnectionReport.GetNext(pos)){
					m_liConnectionReport.GetAt(pos)->ReportConnectionProgress(GetLinkedClient(),  _T(" ...OK\n"), true);
					m_liConnectionReport.GetAt(pos)->ReportConnectionProgress(GetLinkedClient(), _T("*** ") _T("Authenticating friend") /*to stringlist*/, false);
				}
