...
#define FF_FRIENDSLOT	"FU" // ZZUL-TRA :: SaveFU
...
public:
	...
	CFriend(const uchar* abyUserhash, uint64 dwLastSeen, uint32 dwLastUsedIP, uint16 nLastUsedPort, 
            uint64 dwLastChatted, LPCTSTR pszName, uint32 dwHasHash);// X: [64T] - [64BitTime]
	~CFriend();

	uchar	m_abyUserhash[16];
	uint64	m_dwLastSeen;// X: [64T] - [64BitTime]
	uint32	m_dwLastUsedIP;
	uint16	m_nLastUsedPort;
	uint64	m_dwLastChatted;// X: [64T] - [64BitTime]
	CString m_strName;
	...
	void	LoadFromFile(CFileDataIO* file, bool I64Time);// X: [E64T] - [Enable64BitTime]
	void	WriteToFile(CFileDataIO* file, bool I64Time);// X: [E64T] - [Enable64BitTime]
...
private:
	...
	CAtlList<CFriendConnectionListener*> m_liConnectionReport;
