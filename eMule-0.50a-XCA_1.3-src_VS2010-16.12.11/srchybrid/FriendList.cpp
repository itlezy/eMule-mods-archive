...
bool CFriendList::LoadList(){
	...
	try {
		...
		for (UINT i = 0; i < nRecordsNumber; i++) {
			...
			Record->LoadFromFile(&file,I64Time);// X: [E64T] - [Enable64BitTime]
...
void CFriendList::SaveList(){
	...
	try{
		...
		file.WriteUInt8(I64Time?(MET_HEADER|I64TIMEMASK):MET_HEADER);// X: [E64T] - [Enable64BitTime]
		file.WriteUInt32((uint32)m_listFriends.GetCount());
		for (POSITION pos = m_listFriends.GetHeadPosition();pos != 0;)
			m_listFriends.GetNext(pos)->WriteToFile(&file,I64Time);// X: [E64T] - [Enable64BitTime]
...
bool CFriendList::AddFriend(const uchar* abyUserhash, uint64 dwLastSeen, uint32 dwLastUsedIP, uint16 nLastUsedPort, 
							uint64 dwLastChatted, LPCTSTR pszName, uint32 dwHasHash){// X: [E64T] - [Enable64BitTime]
