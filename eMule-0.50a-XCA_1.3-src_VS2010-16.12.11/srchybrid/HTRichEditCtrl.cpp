...
#ifdef USE_METAFILE
CStringToHMETAFILEMap CHTRichEditCtrl::sm_aSmileyBitmaps;
#else
CStringToHBITMAPMap CHTRichEditCtrl::sm_aSmileyBitmaps;
#endif
...
#ifdef USE_METAFILE
	for (CStringToHMETAFILEMap::const_iterator it = sm_aSmileyBitmaps.begin(); it != sm_aSmileyBitmaps.end(); ++it)
		VERIFY( DeleteEnhMetaFile(it->second);
#else
	for (CStringToHBITMAPMap::const_iterator it = sm_aSmileyBitmaps.begin(); it != sm_aSmileyBitmaps.end(); ++it)
		VERIFY( DeleteObject(it->second) );
#endif
	sm_aSmileyBitmaps.clear();
#else
...
{
	if (m_astrBuff.GetCount() > 0) // flush buffer
	{
		for (size_t i = 0; i < m_astrBuff.GetCount(); i++)
		{
...
{
#ifdef REPLACE_ATLMAP
	CStringToHBITMAPMap::const_iterator it = sm_aSmileyBitmaps.find(pszSmileyID);
	if(it != sm_aSmileyBitmaps.end())
		return it->second;
#else
	HBITMAP pvData;
...
#ifdef REPLACE_ATLMAP
	sm_aSmileyBitmaps[pszSmileyID] = hBitmap;
#else
