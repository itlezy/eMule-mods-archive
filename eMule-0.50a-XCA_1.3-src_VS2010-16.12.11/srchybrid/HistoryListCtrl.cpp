...
void CHistoryListCtrl::Init(void)
{
	...
	InsertColumn(8,GetResString(IDS_SF_REQUESTS),     LVCFMT_RIGHT,80);
	InsertColumn(9,GetResString(IDS_SF_ACCEPTS),      LVCFMT_RIGHT,120);
	//EastShare END
	InsertColumn(10,GetResString(IDS_SHAREFACTOR),     LVCFMT_LEFT,100); //Xman advanced upload-priority
	LoadSettings();
...
void CHistoryListCtrl::ReloadFileList(void)
{
	...
	for (CKnownFilesMap::const_iterator it = theApp.knownfiles->GetKnownFiles().begin(); it != theApp.knownfiles->GetKnownFiles().end(); ++it)
	{
		...
		if(thePrefs.GetShowSharedInHistory()||!theApp.sharedfiles->IsFilePtrInList(cur_file))
			AddFile(cur_file);
	}
#else
	CKnownFile * cur_file;
	if(theApp.knownfiles->GetKnownFiles().IsEmpty()==false)
	{
		...
		/*if(thePrefs.GetShowSharedInHistory()){
			...
			CKnownFilesArray *files = theApp.knownfiles->GetDownloadedFiles();
			for(CKnownFilesArray::const_iterator it = files->begin(); it != files->end(); ++it){
				AddFile(*it);
			}
			delete files;
		}*/
...
void CHistoryListCtrl::GetItemDisplayText(const CAbstractFile* file, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
	...
	switch(iSubItem){
		...
		case 5:
			...
			break;
        case 6:
			_tcsncpy(pszText, const_cast<CKnownFile*>(pKnownFile)->GetFileComment(), cchTextMax);
			break;
//EastShare START - Added by Pretender
		case 7:
...
void CHistoryListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	m_HistoryMenu.CheckMenuItem(MP_VIEWSHAREDFILES, thePrefs.GetShowSharedInHistory()?MF_CHECKED:MF_UNCHECKED);
	...
	m_HistoryMenu.AppendMenu(MF_POPUP | flag, (UINT_PTR)WebMenu.m_hMenu, GetResString(IDS_WEBSERVICES));

	m_HistoryMenu.AppendMenu(MF_SEPARATOR); 
	m_HistoryMenu.AppendMenu(MF_STRING | (GetItemCount() > 0 ? MF_ENABLED : MF_GRAYED), MP_FIND, GetResString(IDS_FIND));
...
void CHistoryListCtrl::ShowSelectedFileComments() {
	POINT point;
	::GetCursorPos(&point);
	CPoint p = point; 
	ScreenToClient(&p); 
...
void CHistoryListCtrl::RemoveFile(CKnownFile *toRemove) {
	...
	if (IsCPartFile(toRemove)/*toRemove->IsKindOf(RUNTIME_CLASS(CPartFile))*/)
