...
BOOL CHttpDownloadDlg::OnInitDialog() 
{
	...
	if (!m_FileToWrite.Open(m_sFileToDownloadInto, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite))
	{
		...
		XMessageBox(NULL, sMsg, _T("eMule"), MB_OK | MB_ICONERROR, &params);
		EndDialog(IDCANCEL);
		return TRUE;
	}
	...
	// X-Ray :: NewHTTPProgressbar :: Start
	m_ctrlProgress.SetGradientColors(RGB(60, 180, 40), RGB(130, 220, 40));
	m_ctrlProgress.SetTextColor(RGB(0, 0, 0));
	m_ctrlProgress.SetShowPercent();
	// X-Ray :: NewHTTPProgressbar :: End
...
{
	CString sRate = CastItoXBytes(KbPerSecond, true, true);
...
void CHttpDownloadDlg::run()
{
    ...
    bool skipAgent = wcsstr(m_sURLToDownload, _T("sourceforge")) != NULL; //MORPH - Added by WiZaRd, Fix broken HTTP downloads 
	m_hInternetSession = ::InternetOpen(
		//Xman Send user agent as firefox for http downloads when obfucscation enabled (leuk_he)
		(!skipAgent && thePrefs.IsClientCryptLayerRequested())?_T("Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0;"):AfxGetAppName(),
		//Xman end
	...
	if (m_sUserName.GetLength())
		...
		m_hHttpConnection = ::InternetConnect(m_hInternetSession, m_sServer, m_nPort, m_sUserName, 
                                          m_sPassword, m_dwServiceType, 0, (DWORD_PTR) this);
	else
		// Elandal: Assumes sizeof(void*) == sizeof(unsigned long)
		m_hHttpConnection = ::InternetConnect(m_hInternetSession, m_sServer, m_nPort, NULL, 
                                          NULL, m_dwServiceType, 0, (DWORD_PTR) this);
	...
	if (!skipAgent)//MORPH - Added by WiZaRd, Fix broken HTTP downloads  
	...
	if (m_pLastModifiedTime && (*(uint_ptr*)m_pLastModifiedTime) != 0) {
		CString	sPrevLocale(_tsetlocale(LC_TIME, NULL));

		_tsetlocale(LC_TIME, _T("English"));
		CTime t = CTime::GetCurrentTime();
		CString time_header = CTime(*m_pLastModifiedTime).Format(_T("If-Modified-Since: %a, %d %b %Y %H:%M:%S GMT\r\n"));
		_tsetlocale(LC_TIME, sPrevLocale);

		HttpAddRequestHeaders(m_hHttpFile, time_header, (DWORD)-1L, HTTP_ADDREQ_FLAG_ADD);
    }  
...
resend:
	...
	else
	{
		...
		else if(nStatusCode == HTTP_STATUS_NOT_MODIFIED)
		{
			m_pLastModifiedTime = NULL;
			//Delete the file being downloaded to if it is present
			m_FileToWrite.Close();
			::DeleteFile(m_sFileToDownloadInto);
			SetProgressRange(1024);
			SetProgress(1024);
			PostMessage(WM_HTTPDOWNLOAD_THREAD_FINISHED);
			return;
		}
                                                    ...
                                                    DWORD& dwLastPercentage, BOOL bGotFileSize, DWORD dwFileSize)
{
	...
	const DWORD dwNowTicks = GetTickCount();
