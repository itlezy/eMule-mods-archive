...
void CPreparedHyperText::PrepareText(const CString& sText)
{
	...
	for(int i = 0; i <= last; i++)
	{
		...
		case ftp:
			...
			{
				int len = i == last ? i - WordPos + 1 : i - WordPos;
				CString s = _T("ftp://") + m_sText.Mid(WordPos, len);
				RemoveLastSign(s);
				m_Links.push_back(CHyperLink(WordPos, WordPos + len - 1, s, s, (LPCTSTR)NULL));
				state = space;
			}
		...
		case www3:
			...
			{
				int len = i == last ? i - WordPos + 1 : i - WordPos;
				CString s = _T("http://") + m_sText.Mid(WordPos, len);
				RemoveLastSign(s);
				m_Links.push_back(CHyperLink(WordPos, WordPos + len - 1, s, s, (LPCTSTR)NULL));
				state = space;
			}
		...
		case mail:
			...
			{
				int len = i == last ? i - WordPos + 1 : i - WordPos;
				CString s = _T("mailto:") + m_sText.Mid(WordPos, len);
				RemoveLastSign(s);
				m_Links.push_back(CHyperLink(WordPos, WordPos + len - 1, s, s, (LPCTSTR)NULL));
				state = space;
			}
...
void CPreparedHyperText::AppendText(const CString& sText){
	...
	m_sText+=sText;
	for(std::list<CHyperLink>::const_iterator it = ht.m_Links.begin(); it != ht.m_Links.end(); it++)
	{
		CHyperLink hl = *it;
		hl.m_iBegin += len;
		hl.m_iEnd += len;
...
LRESULT CHyperTextCtrl::OnPaint(WPARAM wParam, LPARAM lParam){
	...
	for(std::vector<CVisLine>::const_iterator it = m_VisLines.begin(); it != m_VisLines.end(); it++){
		int iLastX = dc.m_ps.rcPaint.left;

		for(CVisLine::const_iterator jt = it->begin(); jt != it->end(); jt++){
			if (jt->m_pKeyWord)
...
void CHyperTextCtrl::OnLButtonDown(UINT nFlags,CPoint pt){
	...
	if (PtInRect(&rcClient,pt) && m_iLineHeight)
	{
		...
		UINT i = pt.y / m_iLineHeight;
		if(i < m_VisLines.size()){
			std::vector<CVisLine>::const_iterator it = m_VisLines.begin() + i;
			for(CVisLine::const_iterator jt = it->begin(); jt != it->end(); jt++)
				if(pt.x >= jt->m_rcBounds.left && pt.x <= jt->m_rcBounds.right)
				{
					if(jt->m_pHyperLink != NULL)
...
void CHyperTextCtrl::UpdateVisLines(){
	...
	{
		for(std::vector<CVisLine>::const_iterator itv = m_VisLines.begin(); itv != m_VisLines.end(); itv++)
			for(CVisLine::const_iterator jt = itv->begin(); jt != itv->end(); jt++)
			{
				if(jt->m_pHyperLink != NULL)
	...
	std::vector<CLineInfo>::const_iterator it = m_Lines.begin();
	int iVertPos = 0;
	int iHorzPos = 0;
	...
	for(; it != m_Lines.end(); it++)
	{
		...
		std::vector<CLinePartInfo>::const_iterator jt;
