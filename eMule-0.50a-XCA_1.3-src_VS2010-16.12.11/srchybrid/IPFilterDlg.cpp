...
BEGIN_MESSAGE_MAP(CIPFilterDlg, CModResizableDialog) // NEO: MLD - [ModelesDialogs] <-- Xanatos --
	...
	ON_COMMAND(MP_REMOVE, OnBnClickedDelete)
...
CIPFilterDlg::CIPFilterDlg(CWnd* pParent /*=NULL*/)
	: CModResizableDialog(CIPFilterDlg::IDD, pParent) // NEO: MLD - [ModelesDialogs] <-- Xanatos --
{
...
#define COMPARE_NUM( a, b ) ((a) < (b))			\
	...
	else if (s_lParamSort == IPFILTER_COL_DESC && !thePrefs.noIPFilterDesc)// X: [NIPFD] - [No IPFilter Description]
	{
		iResult = _stricmp/*CompareLocaleStringNoCase*/((*((const SIPFilter**)lParam1))->GetDescA(), (*((const SIPFilter**)lParam2))->GetDescA());
	}
	else
	{
		//ASSERT(0);
...
BOOL CIPFilterDlg::OnInitDialog()
{
	...
	AddAnchor(m_ipfilter, TOP_LEFT, BOTTOM_RIGHT);// X: [CI] - [Code Improvement]
	...
	GetDlgItem(IDC_SAVE)->EnableWindow(!thePrefs.noIPFilterDesc);// X: [NIPFD] - [No IPFilter Description]
	// localize
...
void CIPFilterDlg::InitIPFilters()
{
	...
	m_uIPFilterItems = (UINT)ipfilter.size();
	if(m_uIPFilterItems > 0){// X: [CI] - [Code Improvement]
		m_ppIPFilterItems = new const SIPFilter*[m_uIPFilterItems];
		if (m_ppIPFilterItems == NULL)
			m_uIPFilterItems = 0;
		else{
			// copy all the data at once
			memcpy(m_ppIPFilterItems, &ipfilter[0], sizeof(*m_ppIPFilterItems) * m_uIPFilterItems);
			for (UINT i = 0; i < m_uIPFilterItems; i++)
			{
				const SIPFilter* pFilter = m_ppIPFilterItems[i];
				m_ulFilteredIPs += pFilter->end - pFilter->start;
			}
			m_ulFilteredIPs += m_uIPFilterItems; //one addition instead of m_uIPFilterItems*additions
			SortIPFilterItems();
		}
	}
	...
	if (pDispInfo->item.mask & LVIF_TEXT && pDispInfo->item.cchTextMax > 0) // *have* to check that flag!!// X: [CI] - [Code Improvement]
	{
		switch (pDispInfo->item.iSubItem)
		{
			...
			case IPFILTER_COL_LEVEL:
				_ultot_s(m_ppIPFilterItems[pDispInfo->item.iItem]->level, pDispInfo->item.pszText, pDispInfo->item.cchTextMax, 10);// X: [CI] - [Code Improvement]
				break;
			case IPFILTER_COL_HITS:
				_ultot_s(m_ppIPFilterItems[pDispInfo->item.iItem]->hits, pDispInfo->item.pszText, pDispInfo->item.cchTextMax, 10);// X: [CI] - [Code Improvement]
				break;
			case IPFILTER_COL_DESC:
				if (!thePrefs.noIPFilterDesc)// X: [NIPFD] - [No IPFilter Description]
					_tcsncpy(pDispInfo->item.pszText, m_ppIPFilterItems[pDispInfo->item.iItem]->GetDesc(), pDispInfo->item.cchTextMax);
				break;
		}
...
void CIPFilterDlg::OnBnClickedAppend()
{
	...
	if (DialogBrowseFile(strFilePath, _T("All IP Filter Files (*ipfilter.dat;*ip.prefix;*.p2b;*.p2p;*.p2p.txt;*.zip;*.gz;*.rar)|*ipfilter.dat;*ip.prefix;*.p2b;*.p2p;*.p2p.txt;*.zip;*.gz;*.rar|eMule IP Filter Files (*ipfilter.dat;*ip.prefix)|*ipfilter.dat;*ip.prefix|Peer Guardian Files (*.p2b;*.p2p;*.p2p.txt)|*.p2b;*.p2p;*.p2p.txt|Text Files (*.txt)|*.txt|ZIP Files (*.zip;*.gz)|*.zip;*.gz|RAR Files (*.rar)|*.rar|All Files (*.*)|*.*||")))
	{
		...
		_tsplitpath_s(strFilePath, NULL, 0, NULL, 0, NULL, 0, szExt, _countof(szExt));
		_tcslwr_s(szExt);
	...
	else
	{
	    ...
	    while (pos)
	    {
		    ...
		    if (pFilter)
		    {
				...
				bool isTemp = pFilter->IsTemp();
			    if (theApp.ipfilter->RemoveIPFilter(pFilter, isTemp))
			    {
					if(!isTemp)
						theApp.ipfilter->SetModified();
				    aItems.Add(iItem);
	    ...
	    for (size_t i = aItems.GetCount(); i > 0; )
