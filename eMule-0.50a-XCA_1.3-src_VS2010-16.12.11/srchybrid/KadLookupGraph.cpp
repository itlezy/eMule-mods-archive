...
void CKadLookupGraph::OnPaint()
{
	...
	if (m_pLookupHistory != NULL && m_pLookupHistory->GetHistoryEntries().GetCount() >= 1)
	{
		...
		size_t uMaxNodes = uHistWidth / NODE_ENTRY_WIDTH;
		size_t uNodeEntryWidth = 0;
		...
		size_t iVisibleNodes = min(uMaxNodes, m_pLookupHistory->GetHistoryEntries().GetCount());

		// Set the scaling. 3 times the highest distance of the 1/3 closest nodes is the max distance
		CAtlArray<CUInt128> aClosest;
		for (size_t i = 1; i <= iVisibleNodes; i++)
		{
			...
			{
				size_t iReplace = (size_t)-1;
				for (size_t j = 0; j < aClosest.GetCount(); j++)
				{
					if ((iReplace == (size_t)-1 && aClosest[j] > m_pLookupHistory->GetHistoryEntries()[m_pLookupHistory->GetHistoryEntries().GetCount() - i]->m_uDistance)
						|| (iReplace != (size_t)-1 && aClosest[j] > aClosest[iReplace]))
					{
						iReplace = j;
					}
				}
				if (iReplace != (size_t)-1)
		...
		uMaxScalingDistance.ShiftLeft(1);
		uMaxScalingDistance.Add(uTmpScalingDistance);
		...
		m_pToolTip->Activate(m_iHotItemIdx != (size_t)-1 ? TRUE : FALSE);
		UpdateToolTip();

		CAtlArray<bool> abHotItemConnected;
		if (m_iHotItemIdx != (size_t)-1)
		{
			abHotItemConnected.SetCount(iVisibleNodes);
			for (size_t i = 0; i < iVisibleNodes; i++)
		...
		if (g_gdiplus.IsInited())
		{
			...
			for (size_t i = 0; i < iVisibleNodes; i++)
			{
				...
				for (size_t j = 0; j < sEntry->m_liReceivedFromIdx.GetCount(); j++)
				{
					size_t iIdx = sEntry->m_liReceivedFromIdx[j];
		...
		for (size_t i = 0; i < iVisibleNodes; i++)
		{
			...
			if (m_iHotItemIdx != (size_t)-1 && !abHotItemConnected[i])
...
size_t CKadLookupGraph::CheckHotItem(const CPoint& point) const
{
	for (size_t i = 0; i < m_aNodesDrawRects.GetCount(); i++)
	{
...
void CKadLookupGraph::UpdateToolTip()
{
	...
	if (m_iHotItemIdx != (size_t)-1)
	{
		size_t iHotItemRealIdx = m_pLookupHistory->GetHistoryEntries().GetCount() - (m_iHotItemIdx + 1);
		...
		if (sEntry->m_dwAskedContactsTime > 0)
		{
			...
			{
				uint32 iUseful = 0;
				for (size_t i = 0; i < m_pLookupHistory->GetHistoryEntries().GetCount(); i++)
				{
					for (size_t j = 0; j < m_pLookupHistory->GetHistoryEntries()[i]->m_liReceivedFromIdx.GetCount(); j++)
					{
