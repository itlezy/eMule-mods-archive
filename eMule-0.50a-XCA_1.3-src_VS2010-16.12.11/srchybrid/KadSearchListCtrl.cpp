...
*/
	static const int colWidth[]={60,DFLT_HASH_COL_WIDTH,100,DFLT_FILENAME_COL_WIDTH,100,100,100,100};
...
void CKadSearchListCtrl::Localize()
{
	...
	/*INT_PTR iItems = GetItemCount();
	for (INT_PTR i = 0; i < iItems; i++)
...
int CKadSearchListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	switch (LOWORD(lParamSort))
	{
		...
		case colResponses:
			iResult = CompareUnsigned_PTR(item1->GetAnswers(), item2->GetAnswers());
			break;
...
{
	size_t iInterestingItem = (size_t)-1;
	size_t iItem = (size_t)-1;
	...
	for (size_t i = 1; i <= GetItemCount(); i++)
	{
		...
		if (pSearch != NULL && !pSearch->GetLookupHistory()->IsSearchStopped() && !pSearch->GetLookupHistory()->IsSearchDeleted())
		{
			...
			switch (pSearch->GetSearchTypes())
			{
				...
				case Kademlia::CSearch::STOREFILE:
				case Kademlia::CSearch::STOREKEYWORD:
					iInterestingItem = GetItemCount() - i;
					break;
				/*case Kademlia::CSearch::NODE:
				case Kademlia::CSearch::NODECOMPLETE:
				case Kademlia::CSearch::NODESPECIAL:
		...
		if (iInterestingItem != (size_t)-1)
			break;
	}
	if (iInterestingItem != (size_t)-1)
	{
		if (bMark)
			SetItemState(iInterestingItem, LVIS_SELECTED, LVIS_SELECTED);
		return ((Kademlia::CSearch*)GetItemData(iInterestingItem))->GetLookupHistory();
	}
	if (iItem != (size_t)-1)
	{
		if (bMark)
