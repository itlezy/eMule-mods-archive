...
void CKnownFile::DrawShareStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	/*
	if(m_ClientUploadList.GetCount() > 0 || m_nCompleteSourcesCountHi > 1) {
	...
	if(m_ClientUploadList.GetCount() > 0 && GetPartCount()>1) //looks bad if always red for files with only one chunk
	{
		const COLORREF crMissing = RGB(255, 0, 0);
		statusBar.Fill(crMissing);
...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	m_SOTNAvailPartFrequency.SetCount(GetPartCount()); // ZZUL-TRA :: SOTN :: Start
		...
		switch (newtag->GetNameID()){
			...
			case FT_FILESIZE:{
				...
				if (newtag->IsInt64(true))
				{
					...
					m_AvailPartFrequency.SetCount(GetPartCount());
					m_SOTNAvailPartFrequency.SetCount(GetPartCount()); // ZZUL-TRA :: SOTN 
					for (uint16 i = 0; i < GetPartCount();i++)
                                         {
						m_AvailPartFrequency[i] = 0;
						m_SOTNAvailPartFrequency[i] = 0; // ZZUL-TRA :: SOTN 
			...
			case FT_KADLASTPUBLISHNOTES:{
				ASSERT( newtag->IsInt64() );
				if (newtag->IsInt64())
					SetLastPublishTimeKadNotes( newtag->GetInt64() );
				delete newtag;
				break;
			}
			case FT_FLAGS:
...
bool CKnownFile::WriteToFile(CFileDataIO* file,bool I64Time)// X: [E64T] - [Enable64BitTime]
{
	...
	if (!ShouldPartiallyPurgeFile())
	{
		...
		if (m_lastPublishTimeKadNotes){
			CTag kadLastPubNotes(FT_KADLASTPUBLISHNOTES, m_lastPublishTimeKadNotes,I64Time);// X: [E64T] - [Enable64BitTime]
...
void CKnownFile::UpdateMetaDataTags()
{
	...
	_tsplitpath_s(GetFileName(), NULL, 0, NULL, 0, NULL, 0, szExt, _countof(szExt));
	_tcslwr_s(szExt);
...
{
	if(m_lastPublishTimeKadNotes < (uint64)time(NULL)
		&& (!GetFileComment().IsEmpty() || GetFileRating() != 0))
	{
		m_lastPublishTimeKadNotes = (uint64)time(NULL)+KADEMLIAREPUBLISHTIMEN;
		return true;
	}
	return false;
}
...
{
	return GrabImage(GetPath() + _T('\\') + GetFileName(), nFramesToGrab,  dStartTime, bReduceColor, nMaxWidth, pSender);
}
...
bool CKnownFile::GrabImage(CString strFileName,uint8 nFramesToGrab, double dStartTime, bool bReduceColor, uint16 nMaxWidth, void* pSender)
{
	if (!IsMovie())
		return false;
	(new CFrameGrabThread(this, strFileName, nFramesToGrab, dStartTime, bReduceColor, nMaxWidth, pSender))->start();
	return true;
}
...
uint32 *CKnownFile::CalcPartSpread(){
	...
	for (POSITION pos = statistic.GetSpreadList()->GetHeadPosition(); pos != 0; statistic.GetSpreadList()->GetNext(pos)){
		Spread_Struct* cur_spread = statistic.GetSpreadList()->GetAt(pos);
