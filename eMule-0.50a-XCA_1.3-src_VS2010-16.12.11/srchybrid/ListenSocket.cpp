...
bool CClientReqSocket::CheckTimeOut()
{
	...
	if(client && client->GetUploadState()==US_UPLOADING && isready==false/* && client->HasPeerCacheState()==false*/ && client->GetUpStartTimeDelay()>=MIN2MS(1)) //Xman Xtreme Upload: Peercache-part // X: [RPC] - [Remove PeerCache]
	{
		//timeout_timer = ::GetTickCount();
...
//<<< WiZaRd::Missing code?
						...
						if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64)PARTSIZE)
						{
							...
							*/
							if (partfile->GetMaxSources() > partfile->GetSourceCount() && partfile->IsGlobalSourceAddAllowed()==true) //Xman GlobalMaxHarlimit for fairness
...
//								break; //no info, yet, don't parse/react
							...
							if(theApp.uploadqueue->IsDownloading(client))
								theApp.uploadqueue->RemoveFromUploadQueue(client, L"Src says he does not have the file he's downloading (TCP)", /*true, true,*/ CUpDownClient::USR_NONE); // Maella -Upload Stop Reason- //morph4u todo
...
//<<< WiZaRd - in any case, we add this client to our downloadqueue
				...
				case OP_CANCELTRANSFER:
				{
					...
					theApp.uploadqueue->RemoveFromUploadQueue(client, _T("Remote client canceled transfer."), /*true, true,*/ CUpDownClient::USR_CANCELLED); // Maella -Upload Stop Reason-
#else
...
#ifdef CLIENTANALYZER
	{
						...
						if(client->GetAntiLeechData())
							client->GetAntiLeechData()->AddULSession(true);
						theApp.uploadqueue->RemoveFromUploadQueue(client, _T("Remote client ended transfer."), /*true, true,*/ CUpDownClient::USR_CANCELLED); // Maella -Upload Stop Reason-
					}
#else
						//Xman Code Improvement
