...
BOOL CMetaDataDlg::OnInitDialog()
{
	...
	AddAnchor(m_tags, TOP_LEFT, BOTTOM_RIGHT);
...
CString GetTagNameByID(uint_ptr id)
{
	switch (id)
	{
...
CString GetValue(const CTag* pTag)
{
	...
	{
		if (pTag->GetNameID() == FT_MEDIA_LENGTH)
		...
		if(pTag->GetNameID() == FT_LASTSEENCOMPLETE){
			//SecToTimeLength(pTag->GetInt64(), strValue);
			CTime lastseen(pTag->GetInt64());
			strValue = lastseen.Format(thePrefs.GetDateTimeFormat());
		}
		else
...
CString GetValue(const Kademlia::CKadTag* pTag) // FIXME LARGE FILES
{
	...
	else if (pTag->IsInt())
	{
		if (pTag->m_name.Compare(TAG_MEDIA_LENGTH) == 0)
			SecToTimeLength((uint_ptr)pTag->GetInt(), strValue);
...
CString GetType(uint_ptr uType)
{
	CString strValue;
	if (uType == 1)
		strValue = _T("Hash");
...
void CMetaDataDlg::RefreshData()
{
	...
	if (m_paFiles->GetSize() >= 1)
	{
		...
		if (!((CAbstractFile*)(*m_paFiles)[0])->HasNullHash())
		{
			LVITEM lvi;
			lvi.mask = LVIF_TEXT;
			lvi.iItem = INT_MAX;
			...
			if (iItem >= 0)
			{
				...
				strBuff = md4str(((CAbstractFile*)(*m_paFiles)[0])->GetFileHash());
				lvi.pszText = const_cast<LPTSTR>((LPCTSTR)strBuff);
		...
		//const CAtlArray<CTag*>& aTags = STATIC_DOWNCAST(CAbstractFile, (*m_paFiles)[0])->GetTags();
		const CAtlArray<CTag*>& aTags = ((CAbstractFile*)(*m_paFiles)[0])->GetTags();
		size_t iTags = aTags.GetCount();
		for (size_t i = 0; i < iTags; i++)
		{
