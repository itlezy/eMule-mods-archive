...
public:
	...
	DWORD_PTR GetItemData(size_t iItem);

	// Retrieves the number of items in the control.
	size_t GetItemCount() const { return m_Params.GetCount(); };
...
protected:
	...
	size_t UpdateLocation(size_t iItem);
	size_t MoveItem(size_t iOldIndex, size_t iNewIndex);
	void SetColors();
	void InitItemMemDC(CMemoryDC *dc, RECT &rcItem, COLORREF defaultcolor, UINT state);
...
/*	static __inline bool HaveIntersection(const CRect &rc1, const CRect &rc2) {
	...
	static const int sm_i2IconOffset;
...
private:
	...
	int m_iRedrawCount;
	CAtlList<DWORD_PTR> m_Params;

	DWORD_PTR GetParamAt(POSITION pos, size_t iPos) {
		LPARAM lParam = m_Params.GetAt(pos);
		if(lParam == 0xFEEBDEEF) //same as MLC_MAGIC!
			m_Params.SetAt(pos, lParam = CListCtrl::GetItemData((int)iPos));
		return lParam;
	}

	CAtlList<int> m_liDefaultHiddenColumns;
