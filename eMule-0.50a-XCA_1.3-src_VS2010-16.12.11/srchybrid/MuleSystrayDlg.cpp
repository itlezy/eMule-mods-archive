...
*/
	...
	*/
	...
	if (/*thePrefs.GetWindowsVersion() == _WINVER_95_ ||	thePrefs.GetWindowsVersion() == _WINVER_NT4_ || */g_bLowColorDesktop)
	{
...
void CMuleSystrayDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if(pScrollBar == (CScrollBar*)&m_ctrlDownSpeedSld)
	{
		...
		if(m_nDownSpeedTxt == 0)
			m_nDownSpeedTxt = UNLIMITED;
		thePrefs.maxdownload = (float)m_nDownSpeedTxt;
		m_nDownSpeedTxt=min((int)thePrefs.GetMaxDownload(),m_nDownSpeedTxt);
		if(m_nDownSpeedTxt > thePrefs.maxGraphDownloadRate||m_nDownSpeedTxt < 1)//{
			m_nDownSpeedTxt = 0;
//			m_DownSpeedInput.EnableWindow(false);
//		}
//		else
//			m_DownSpeedInput.EnableWindow(true);
	...
	else if(pScrollBar == (CScrollBar*)&m_ctrlUpSpeedSld)
	{
		...
		if(m_nUpSpeedTxt < 1){
			...
			m_nDownSpeedTxt=(int)thePrefs.maxGraphDownloadRate;
//			m_UpSpeedInput.EnableWindow(false);
		}
		else{
			thePrefs.maxupload = (float)m_nUpSpeedTxt;
			m_nDownSpeedTxt=min((int)thePrefs.GetMaxDownload(),m_nDownSpeedTxt);
//			m_UpSpeedInput.EnableWindow(true);
		...
		thePrefs.maxdownload = (float)((m_nDownSpeedTxt == 0) ? UNLIMITED : m_nDownSpeedTxt);
		theApp.scheduler->SaveOriginals(); // Don't reset Connection Settings for Webserver/CML/MM [Stulle] - Stulle
	}
