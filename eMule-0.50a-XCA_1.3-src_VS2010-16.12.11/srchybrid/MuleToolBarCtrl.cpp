...
void CMuleToolbarCtrl::Init(void)
{
	...
	_tcscpy_s(cButtonStrings, GetResString(IDS_MAIN_BTN_CONNECT));
	...
	UpdateIdealSize();// X: [RUL] - [Remove Useless Localize]
	SaveCurHeight();
}
...
void CMuleToolbarCtrl::SetAllButtonsStrings()
{
	...
	_sntprintf(TBStrings[0], _countof(TBStrings[0]) - 1, _T("%s"), buffer);
	...
	{
		_sntprintf(TBStrings[i], _countof(TBStrings[0]) - 1, _T("%s"), GetResString(TBStringIDs[i-1]));
...
void CMuleToolbarCtrl::SetAllButtonsWidth()
{
	...
	else
	{
		...
		const int iSmallIconsButtonHeight = (theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0)) ? 22 :
			(GetSystemMetrics(SM_CYSCREEN) <= 600 ? 16 : 28);
...
void CMuleToolbarCtrl::OnNmRClick(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	UINT i = 1;
	CString toolbardir = thePrefs.GetMuleDirectory(EMULE_TOOLBARDIR, false);
	if (!toolbardir.IsEmpty() && PathFileExists(toolbardir))
	{
		CAtlArray<CString> astrToolbarFiles;
		for (size_t f = 0; f < _countof(s_apszTBFiles); f++)
		{
			WIN32_FIND_DATA FileData;
			HANDLE hSearch = FindFirstFile(toolbardir + _T('\\') + s_apszTBFiles[f], &FileData);
		...
		if (astrToolbarFiles.GetCount() > 0)
		{
			...
			for (size_t f = 0; f < astrToolbarFiles.GetCount(); f++)
			{
				const CString& bitmapFileName = astrToolbarFiles.GetAt(f);
				CString bitmapBaseName;
				LPCTSTR pszTbBaseExt = wcsistr(bitmapFileName, EMULTB_BASEEXT);
				if (pszTbBaseExt)
				...
				m_astrToolbarPaths.Add(toolbardir + _T('\\') + bitmapFileName);
	...
	i = 1;
	CString skindir = thePrefs.GetMuleDirectory(EMULE_SKINDIR, false);
	if (!skindir.IsEmpty() && PathFileExists(skindir))
	{
		CAtlArray<CString> astrSkinFiles;
		for (int f = 0; f < _countof(s_apszSkinFiles); f++)
		{
			WIN32_FIND_DATA FileData;
			HANDLE hSearch = FindFirstFile(skindir + _T('\\') + s_apszSkinFiles[f], &FileData);
			if (hSearch != INVALID_HANDLE_VALUE)
			{
				do {
		...
		{
			Sort(astrSkinFiles);
			for (size_t f = 0; f < astrSkinFiles.GetCount(); f++)
			{
				const CString& skinFileName = astrSkinFiles.GetAt(f);
				CString skinBaseName;
				LPCTSTR pszSkinBaseExt = wcsistr(skinFileName, _T(".") EMULSKIN_BASEEXT _T(".ini"));
				if (pszSkinBaseExt)
				...
				m_astrSkinPaths.Add(skindir + _T('\\') + skinFileName);
...
BOOL CMuleToolbarCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	switch (wParam)
	{
		case MP_SELECTTOOLBARBITMAPDIR:{
			...
			_sntprintf(buffer, _countof(buffer) - 1, _T("%s"), thePrefs.GetMuleDirectory(EMULE_TOOLBARDIR));
		...
		case MP_SELECT_SKIN_DIR:{
			TCHAR buffer[MAX_PATH];
			_sntprintf(buffer, _countof(buffer) - 1, _T("%s"), thePrefs.GetMuleDirectory(EMULE_SKINDIR));
			buffer[_countof(buffer) - 1] = _T('\0');
