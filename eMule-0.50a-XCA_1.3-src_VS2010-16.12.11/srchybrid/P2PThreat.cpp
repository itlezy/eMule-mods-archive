	...
	if (regkey.Open(hParent, pszKey) == ERROR_SUCCESS)
	{
		...
		if (_tcscmp(pszName,_T("*"))==0) //(pszName == _T("*"))//Xman
		{
			while (!bFound)
			{
				...
				if (_tcscmp(pszTempValue, strValue) == 0)//if (_tcscmp(pszTempValue, pszValue) == 0) // X: [BF] - [Bug Fix]
					bFound = true;
			}
		}
		else if(_tcscmp(pszName,_T("########"))==0) //(pszName == _T("########"))//Xman
		{
			while (!bFound)
			{
				...
				if (_tcslen(pszTempName) == 8)
				{
					...
					if (bIsHex && _tcscmp(pszTempValue, strValue) == 0)//if (bIsHex && _tcscmp(pszTempValue, pszValue) == 0) // X: [BF] - [Bug Fix]
						bFound = true;
				}
			}
		}
