...
//CBarShader CPartFile::s_ChunkBar(16); //Xman
...
// Xman end
{
	...
	const CAtlList<Kademlia::CEntry*>& list = searchresult->getNotes();
...
CPartFile::~CPartFile()
{
	...
	if (m_hpartfile.m_hFile != INVALID_HANDLE_VALUE){
		...
		if(m_FlushThread)
		{
			if(theApp.m_DiscAccessQueue.inQueue(m_FlushThread))
...
EPartFileLoadResult CPartFile::ImportShareazaTempfile(LPCTSTR in_directory,LPCTSTR in_filename, EPartFileFormat* pOutCheckFileFormat) 
{
	...
	try{
		...
		if(gotostring(sdFile,nVersion >= 29 ? (uchar*)&lSize : (uchar*)&nSize,nVersion >= 29 ? 8 : 4)) // search the gap list
		{
			...
			if(badGapList){
				while (gaplist.GetCount()>0 )// X: [CI] - [Code Improvement]
			...
			if (pOutCheckFileFormat == NULL || (pOutCheckFileFormat != NULL && (newtag->GetNameID()==FT_FILESIZE || newtag->GetNameID()==FT_FILENAME))){
				switch (newtag->GetNameID()){
					...
					case FT_KADLASTPUBLISHNOTES:{
						...
						if (newtag->IsInt64())// X: [64T] - [64BitTime]
						{
							SetLastPublishTimeKadNotes(newtag->GetInt64());// X: [64T] - [64BitTime]
						}
						delete newtag;
						break;
												}
...
bool CPartFile::SavePartFile(bool bDontOverrideBak)
{
	...
	if (!m_PartsHashing){
		...
		m_tLastModified = lwtime.GetTime();// X: [64T] - [64BitTime]
		m_tUtcLastModified = m_tLastModified;
		if (m_tUtcLastModified == 0){
			if (thePrefs.GetVerbose())
	...
	try{
		...
		if (GetLastPublishTimeKadNotes()){
			CTag kadLastPubNotes(FT_KADLASTPUBLISHNOTES, GetLastPublishTimeKadNotes(),I64Time);// X: [E64T] - [Enable64BitTime]
...
void CPartFile::DrawShareStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool bFlat) const
{
	...
	// Set Size
	CBarShader chunkBar(rect->bottom - rect->top);// X: [CI] - [Code Improvement] BarShader
	...
	if (!onlygreyrect){
		...
		for (size_t i = 0; i < GetPartCount(); i++){
			...
			*/
			if(IsPartShareable(i)) {
				if(GetStatus() != PS_PAUSED || m_ClientUploadList.GetCount() > 0 || m_nCompleteSourcesCountHi > 0) {
					uint_ptr frequency;
					...
					chunkBar.FillRange(PARTSIZE*(uint64)(i),PARTSIZE*(uint64)(i+1),
						(frequency > 0 )?
						((22*(frequency-1) >= 210)?
							RGB(0, 0, 255)
							:RGB(0, 210-22*(frequency-1), 255))
						:crMissing);
				} else {
   	...
   	chunkBar.Draw(dc, rect->left, rect->top, rect->right - rect->left, bFlat); // X: [CI] - [Code Improvement] BarShader
} 
	...
	else
	{
	    ...
	    for (POSITION pos = m_BufferedData_list.GetHeadPosition();pos !=  0;){
			...
			if(bufEnd != item->start){
				if(bufEnd)
					chunkBar.FillRange(bufStart, bufEnd - 1, crInBuffer);
				bufStart = item->start;
			}
			bufEnd = item->end + 1;
	    }
		...
		else
		{
			...
			if(isLimited == false){
				// Always call this method to avoid a flag (enabled/disable)
					...
					*
					if (cur_src->IsDownloadingFromPeerCache() && (m_sourceListChange == false || m_downloadingDeleteList.Find(cur_src) == NULL) && cur_src->m_pPCDownSocket && cur_src->m_pPCDownSocket->IsConnected())
					//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
			...
			*/
			...
			switch (cur_src->GetDownloadState())
			{
				...
				case DS_ONQUEUE:{			
					...
					// Maella -Spread Request- (idea SlugFiller)					
					if(theApp.IsConnected() == true || IsCUrlClient(cur_src)/*cur_src->IsKindOf(RUNTIME_CLASS(CUrlClient))*/){ // X: [CI] - [Code Improvement] allow download from urlclient when server and kad is not connected
...
void CPartFile::UpdatePartsInfo()
{
	...
	if(!stopped){ // X-Ray :: Optimizations - No Need to Refresh a stopped File
		for (POSITION pos = srclist.GetHeadPosition(); pos != 0; ){
			...
			if(cur_src->GetDownloadState()==DS_ONQUEUE && (cur_src->IsBadGuy() || cur_src->IsRemoteQueueFull() || cur_src->GetRemoteQueueRank()>4000)) 
#else
...
void CPartFile::PerformFileCompleteEnd(DWORD dwResult)
{
	if (dwResult & FILE_COMPLETION_THREAD_SUCCESS)
	{
		...
		if(thePrefs.m_bHistoryShowShared && theApp.emuledlg && CemuleDlg::IsRunning()){
	...
	if(!A4AFsrclist.IsEmpty())
	{
		for(POSITION pos1 = A4AFsrclist.GetHeadPosition();pos1 != NULL;){// X: [CI] - [Code Improvement]
			...
			{ 
				...
				//theApp.emuledlg->transferwnd->GetDownloadList()->RemoveSource(client,this);
			}
			else{
...
bool CPartFile::IsReadyForPreview() const
{
	...
	if (IsArchive(true))
	{
		...
		//	&& CVolumeInfo::_GetFreeDiskSpace(thePrefs.GetTempDir())+100000000 > 2*GetFileSize())
		//	return true;
		...
		uint64 uMinFreeDiskSpace = (thePrefs.IsCheckDiskspaceEnabled() && thePrefs.GetMinFreeDiskSpace() > 0)
			...
			: 20*1024*1024;
		uMinFreeDiskSpace += thePrefs.GetPreviewCopiedArchives()?(uint64)(GetFileSize() * (uint64)2): (uint64)(GetCompletedSize() + (uint64)16*1024);
		if (CVolumeInfo::_GetFreeDiskSpace(GetTempPath()) < uMinFreeDiskSpace)
			return false;
		return true; 
	}
...
#ifdef CLIENTANALYZER
		if (cur_src->HasLowID() || cur_src->IsBadGuy() || !cur_src->IsValidSource())
#else
								...
								const CUpDownClient* client)
{
	...
	m_uTransferredSession += transize; // X-Ray :: SessionDownload
...
void CPartFile::FlushBuffer(bool forcewait, bool bForceICH, bool bOnlyCompletedPart)// X: [GB] - [Global Buffer]
{
	...
	if (forcewait) { //We need to wait for flush thread to terminate
		if (m_FlushThread) { //We are flushing something to disk
			m_FlushThread->evTerminated.wait();
			FlushDone();
			}
	} else if (m_FlushThread != NULL) { //Some thing is going to be flushed or allready flushed wait the window call back to call FlushDone()
		return;
	}
...
// X: [GB] - [Global Buffer]
		...
		m_FlushThread = new CPartFileFlushThread(this);
		m_FlushThread->bForceICH = bForceICH;
		m_FlushThread->changedPart = changedPart;
...
{
	if (m_FlushThread == NULL) //Already do in normal process
		return;
	...
	if (m_FileIdentifier.HasExpectedMD4HashCount()){
		...
		{
			--uPartNumber;
			if (!m_FlushThread->changedPart[uPartNumber])
				continue;
			...
			*/
			...
			else if (IsCorruptedPart(uPartNumber) && (thePrefs.IsICHEnabled() || m_FlushThread->bForceICH))
			{
				if (CemuleDlg::IsRunning()) //Xman Flush Thread/Safehash (Morph)
				{
...
CPartFileFlushThread::CPartFileFlushThread(CPartFile* partfile) : Runnable(Poco::Thread::PRIO_LOW), evTerminated(false), m_partfile(partfile)
{
}	

CPartFileFlushThread::~CPartFileFlushThread()
{
	delete[] changedPart;
}
...
bool CPartFileFlushThread::run()
{
	...
	catch(...)
	{
		...
		delete m_partfile->m_FlushThread;
...
void CPartFile::FlushBuffersExceptionHandler(CFileException* error)
{
	if (thePrefs.IsCheckDiskspaceEnabled() && error->m_cause == CFileException::diskFull)
	{
		...
		if (CemuleDlg::IsRunning() && thePrefs.GetNotifierOnImportantError()){
			CString msg;
...
bool CPartFile::GrabImage(uint8 nFramesToGrab, double dStartTime, bool bReduceColor, uint16 nMaxWidth, void* pSender)
{
	if (!IsPartFile()){
		...
		if ( ((GetStatus() != PS_READY && GetStatus() != PS_PAUSED) || m_bPreviewing || GetPartCount() < 2 || GetHeaderSizeOnDisk()<PARTSIZE)  )
			return false;
...
void CPartFile::AICHRecoveryDataAvailable(uint_ptr nPart)
{
	...
	if (IsComplete((uint64)nPart*PARTSIZE, (((uint64)nPart*PARTSIZE)+length)-1, true)){
		...
		if (!HashSinglePart(nPart))
		{
			...
			if (posCorrupted)
				corrupted_list.RemoveAt(posCorrupted);
			if (status == PS_EMPTY && CemuleDlg::IsRunning()){
...
void CPartFile::RemoveLeecherSources()
{
	...
	{
		CUpDownClient* cur_src = srclist.GetNext(pos);
		if (cur_src != NULL && cur_src->GetDownloadState() != DS_DOWNLOADING && cur_src->IsBadGuy()) 
		{
			cur_src->droptime=::GetTickCount(); 
			theApp.downloadqueue->RemoveSource(cur_src);
			//removed ++; // Count removed source
...
{ 
	...
	if (srclist.GetCount() == 0 || GetStatus() == PS_COMPLETE || GetStatus() == PS_INSUFFICIENT || IsStopped()) 
		minQR = 0; 
	else 
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL; ) 
		{ 
...
bool CPartFile::IsGlobalSourceAddAllowed()
{
	if(thePrefs.m_bAcceptsourcelimit==false)
		return true;
	return (theApp.downloadqueue->GetGlobalSources()<thePrefs.m_uMaxGlobalSources);
}
...
bool CPartFile::IsSourceSearchAllowed()
{
	...
	return (thePrefs.m_uMaxGlobalSources > theApp.downloadqueue->GetGlobalSources() + 50);
}
//Xman end
		...
		if(chunksList.IsEmpty() == TRUE && chunklist_initialized==false){ //Xman Dynamic block request
			...
			if(cc==1/*Maella*/ ){
				...
				*/
				...
				const bool isPreviewEnable = (thePrefs.GetPreviewPrio() || thePrefs.IsExtControlsEnabled() && GetPreviewPrio())/* && IsPreviewableFileType()*/;// X: [AC] - [ActionChange] Allow all file type to download the preview part
