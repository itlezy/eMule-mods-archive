...
CPartFileConvert::CPartFileConvert() : Thread(true)
{
}
...
int CPartFileConvert::ScanFolderToAdd(CString folder,bool deletesource) {
	...
	while (bWorking) {
		...
		if (finder.IsDirectory() && finder.GetFileName().GetAt(0)!=_T('.'))
...
void CPartFileConvert::StartThread() {
	...
	{
		convertPfThread = new CPartFileConvert();
		if(!convertPfThread->start())
			convertPfThread = NULL;
	}
}

void CPartFileConvert::run()
{
...
int CPartFileConvert::performConvertToeMule(CString folder)
{
	...
	if (pfconverting->partmettype==PMT_SPLITTED ) {
		try {
			...
			if (CVolumeInfo::_GetFreeDiskSpace(thePrefs.GetTempDir()) < ((uint64)maxindex*PARTSIZE) ) {
				delete file;
			...
			file->CreatePartFile();
			if (file->GetStatus() == PS_ERROR){// X: [BF] - [Bug Fix]
				delete file;
				return CONV_FAILED;
			}
	...
	else //if (pfconverting->partmettype==PMT_DEFAULTOLD || pfconverting->partmettype==PMT_NEWOLD || Shareaza  ) 
	{
		...
		if (!pfconverting->removeSource && (CVolumeInfo::_GetFreeDiskSpace(thePrefs.GetTempDir()) < pfconverting->spaceneeded) ) {
			delete file;
			return CONV_OUTOFDISKSPACE;
		}
	...
	while (file->gaplist.GetCount()>0 )// X: [CI] - [Code Improvement]
	...
	if (pfconverting->partmettype==PMT_NEWOLD || pfconverting->partmettype==PMT_SPLITTED ) {
		...
		file->leftsize=file->GetFileSize()-file->completedsize;
