...
uint64	CPreferences::versioncheckLastAutomatic;// X: [64T] - [64BitTime]
...
CAtlArray<UINT> CPreferences::m_aAllowedRemoteAccessIPs;
...
CAtlList<CString> CPreferences::addresses_list;
...
/*uint64	CPreferences::m_uPeerCacheLastSearch;
...
bool	CPreferences::m_bMediaInfo_RIFF;
bool	CPreferences::m_bMediaInfo_RM;
bool	CPreferences::m_bMediaInfo_ID3LIB;
bool	CPreferences::m_bMediaInfo_WM;
bool	CPreferences::m_bMediaInfo_MediaDet;
bool	CPreferences::m_bMiniMuleAutoClose;
UINT	CPreferences::m_uMiniMuleTransparency;
CString	CPreferences::m_strMediaInfoDllPath;
CString	CPreferences::m_strNotifierMailEncryptCertName;
CString	CPreferences::m_strInternetSecurityZone;
...
bool	CPreferences::noIPFilterDesc;// X: [NIPFD] - [No IPFilter Description]
CString	CPreferences::speedgraphwindowConfiguration; // X: [SGW] - [SpeedGraphWnd]
int		CPreferences::speedgraphwindowLeft;
...
bool	CPreferences::m_bShowFileStatusIcons; // X-Ray :: FileStatusIcons
...
bool	CPreferences::m_bShowSessionDownload; // X-Ray :: SessionDownload
bool	CPreferences::m_bShowMinQR; 
...
void CPreferences::SaveStats(int bBackUp){
	...
	ini.WriteUInt64(L"DownData_URL", GetCumDownData(SOFT_URL));
	...
	ini.WriteUInt64(L"DownDataPort_PeerCache", GetCumDownDataPort(PORT_PEERCACHE));
	...
	ini.WriteUInt64(L"UpDataPort_PeerCache", GetCumUpDataPort(PORT_PEERCACHE));
										   ...
										   BOOL bUpDown, uint32 bytes, bool sentToFriend)
{
	...
	if(bUpDown){
		...
		switch (uClientPort){
			...
			case (UINT)-1:			sesUpDataPort[PORT_PEERCACHE]+=bytes;	break;
		...
		switch (uClientID){
			...
			case SO_URL:			sesDownData[SOFT_URL]+=bytes;			break;
		}
		...
		switch (uClientPort){
			...
			case (UINT)-1:			sesDownDataPort[PORT_PEERCACHE]+=bytes;break;
...
bool CPreferences::LoadStats(int loadBackUp)
{
	...
	cumUpDataPort[PORT_PEERCACHE]			= ini.GetUInt64(L"UpDataPort_PeerCache");
	...
	cumDownData[SOFT_URL]					= ini.GetUInt64(L"DownData_URL");
	...
	cumDownDataPort[PORT_PEERCACHE]		= ini.GetUInt64(L"DownDataPort_PeerCache");
...
void CPreferences::SavePreferences()
{
	...
	//---
	ini.WriteUInt64(L"VersionCheckLastAutomatic", versioncheckLastAutomatic);
...
//    ini.WriteInt(L"MinUpload", minupload);
	...
	if(m_bRestoreLastMainWndDlg)
		ini.WriteInt(L"LastMainWndDlgID",m_iLastMainWndDlgID);
	if(m_bRestoreLastLogPane)
...
//    ini.WriteInt(L"USSNumberOfPings", m_iDynUpNumberOfPings);
	...
	// Section: "PeerCache"
	//
	/*ini.WriteUInt64(L"LastSearch", m_uPeerCacheLastSearch, L"PeerCache");// X: [64T] - [64BitTime]
...
/*
	...
	*/
	...
	ini.WriteBool(L"NoIPFilterDesc",noIPFilterDesc);// X: [NIPFD] - [No IPFilter Description]
	...
	ini.WriteBool(L"ShowFileStatusIcon", m_bShowFileStatusIcons); // X-Ray :: FileStatusIcons
	...
	// <== Invisible Mode [TPT/MoNKi] - Stulle
	ini.WriteBool(L"ShowSessionDownload", m_bShowSessionDownload); // X-Ray :: SessionDownload
	ini.WriteBool(L"ShowMinQR", m_bShowMinQR);
...
/*#else
	...
	if (strNick.IsEmpty() || IsDefaultNick(strNick))
		SetUserNick(MOD_USERNICK); //DEFAULT_NICK
...
//TK4 MOD -
	...
	MaxConperFive=ini.GetInt(L"MaxConnectionsPerFiveSeconds",DFLT_MAXCONPERFIVE);
	...
	m_bIconflashOnNewMessage=ini.GetBool(L"IconflashOnNewMessage",true); // Spike2 - false-->true
	...
	if(m_bRestoreLastMainWndDlg=ini.GetBool(L"RestoreLastMainWndDlg",false))
		m_iLastMainWndDlgID=ini.GetInt(L"LastMainWndDlgID",0);
	if(m_bRestoreLastLogPane=ini.GetBool(L"RestoreLastLogPane",false))
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
...
#ifdef CLIENTANALYZER
	//"first start" means that either no prefs were found (in this case the default mechanism would be sufficient) 
	//or that another version was run up to now - this is why I enforce a check-and-set mechanism here
	if(m_bFirstStart)
	{
		m_uiCreditSystem = 1;  //>>> WiZaRd::ClientAnalyzer
	}
	else
	{
		m_uiCreditSystem=(uint8)ini.GetInt(L"UseCreditSystem", 1); //>>> WiZaRd::ClientAnalyzer
	}
#else
	...
	m_bShowCopyEd2kLinkCmd=ini.GetBool(L"ShowCopyEd2kLinkCmd",true); //morph4u
	...
	if (m_sToolbarBitmapFolder.IsEmpty()) // We want GetDefaultDirectory to also create the folder, so we have to know if we use the default or not
		m_sToolbarBitmapFolder = GetDefaultDirectory(EMULE_TOOLBARDIR, false);
	...
	if (m_strSkinProfileDir.IsEmpty()) // We want GetDefaultDirectory to also create the folder, so we have to know if we use the default or not
		m_strSkinProfileDir = GetDefaultDirectory(EMULE_SKINDIR, false);
...
//    m_iDynUpNumberOfPings = ini.GetInt(L"USSNumberOfPings", 1);
	...
	m_bMediaInfo_RIFF = ini.GetBool(L"MediaInfo_RIFF", true);
	m_bMediaInfo_RM = ini.GetBool(L"MediaInfo_RM", true);
	m_bMediaInfo_ID3LIB = ini.GetBool(L"MediaInfo_ID3LIB", true);
	m_bMediaInfo_WM = ini.GetBool(L"MediaInfo_WM", true);
	m_bMediaInfo_MediaDet = ini.GetBool(L"MediaInfo_MediaDet", true);
	m_bMiniMuleAutoClose = ini.GetBool(L"MiniMuleAutoClose", false);
	m_uMiniMuleTransparency = ini.GetInt(L"MiniMuleTransparency", 0);
	m_strMediaInfoDllPath = ini.GetString(L"MediaInfo_MediaInfoDllPath",
#ifndef _WIN64
		_T("MEDIAINFO.DLL")
#else
		_T("MEDIAINFOX64.DLL")
	...
	m_strNotifierMailEncryptCertName = ini.GetString(L"NotifierMailEncryptCertName");
	m_strInternetSecurityZone = ini.GetString(L"InternetSecurityZone", _T("Untrusted"));
	...
	statsConnectionsGraphRatio = ini.GetInt(L"statsConnectionsGraphRatio", 3);
	...
	// Section: "WebServer"
	//
	m_strWebPassword = ini.GetString(L"Password", NULL, L"WebServer").MakeUpper(); // X: [CI] - [Code Improvement] md5 string capitalized
	m_strWebLowPassword = ini.GetString(L"PasswordLow").MakeUpper();
	...
	// Section: "PeerCache"
	//
/*	m_uPeerCacheLastSearch = ini.GetUInt64(L"LastSearch", 0, L"PeerCache");// X: [64T] - [64BitTime]
...
*/
	...
	//Xman GlobalMaxHarlimit for fairness
	m_bAcceptsourcelimit=ini.GetBool(L"Acceptsourcelimit", false);
...
/*
	...
	*/
	...
	m_bPauseOnFileComplete=ini.GetBool(L"PauseOnFileComplete",false); // NEO: POFC - [PauseOnFileComplete]
	...
	shareExt.MakeLower();
	noIPFilterDesc=ini.GetBool(L"NoIPFilterDesc",false);// X: [NIPFD] - [No IPFilter Description]
	showSpeedGraph=ini.GetBool(L"ShowSpeedGraph",true); // X: [SGW] - [SpeedGraphWnd]
	speedgraphwindowConfiguration = ini.GetString(L"SpeedGraphConfiguration", GetMuleDirectory(EMULE_CONFIGDIR) + L"SpeedGraph.ini");
	...
	m_bShowFileStatusIcons = ini.GetBool(L"ShowFileStatusIcon", false); // X-Ray :: FileStatusIcons
	...
	// <== Invisible Mode [TPT/MoNKi] - Stulle
	// NEO: QS END
	m_bShowSessionDownload = ini.GetBool(L"ShowSessionDownload", false); // X-Ray :: SessionDownload
	m_bShowMinQR = ini.GetBool(L"ShowMinQR", false); 
...
// ==> Pay Back First [AndCycle/SiRoB/Stulle] - Stulle
	...
	m_iPayBackFirstLimit = (uint8)((temp >= 1 && temp <=255) ? temp : 10);
	//m_bPayBackFirst2=ini.GetBool(_T("IsPayBackFirst2"),false);
	//temp = ini.GetInt(_T("PayBackFirstLimit2"), 50);
	//m_iPayBackFirstLimit2 = (uint16)((temp >= 1 && temp <=1024) ? temp : 50);
	// <== Pay Back First [AndCycle/SiRoB/Stulle] - Stulle
}
/*
//Xman Xtreme Upload
