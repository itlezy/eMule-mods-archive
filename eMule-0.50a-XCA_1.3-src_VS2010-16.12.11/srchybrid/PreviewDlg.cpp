...
{
	for (size_t i = 0; i < ARRSIZE(m_icons); i++)
	{
		if (m_icons[i])
...
BOOL PreviewDlg::OnInitDialog()
{
	...
	title.Remove(_T('&'));
	title.AppendFormat(_T(": %s"), m_pFile->GetFileName());
	SetWindowText(title);
...
void PreviewDlg::ShowImage(int nNumber)
{
	...
	CDC* pDC = m_ImageStatic.GetDC();
	HBITMAP hbitmap = m_ImageStatic.SetBitmap(m_pFile->GetPreviews()[nNumber]->MakeBitmap(pDC->m_hDC));
	m_ImageStatic.ReleaseDC(pDC);
	if (hbitmap)
