...
CSearchList::~CSearchList()
{
	...
	for(unordered_map<uint32, UDPServerRecord*>::const_iterator it = m_aUDPServerRecords.begin(); it != m_aUDPServerRecords.end(); ++it)
		delete it->second;
	m_aUDPServerRecords.clear();
#else
...
void CSearchList::NewSearch(CSearchListCtrl* pWnd, CStringA strResultFileType, uint32 nSearchID, ESearchType eSearchType, CString strSearchExpression)
{
	if (pWnd)
		outputwnd = pWnd;
									  ...
									  CUpDownClient* Sender, bool* pbMoreResultsAvailable, LPCTSTR pszDirectory)
{
	...
	for (UINT i = 0; i < results; i++){
		...
		if (toadd->IsLargeFile() && (Sender == NULL || !Sender->SupportsLargeFiles())){
			...
			delete toadd;// X: [BF] - [Bug Fix] soullingyun
			continue;
		}
		if (Sender){
...
UINT CSearchList::ProcessUDPSearchAnswer(CFileDataIO& packet, bool bOptUTF8, uint32 nServerIP, uint16 nServerPort)
{
	...
	bool bFound = false;
	for (size_t i = 0; i != m_aCurED2KSentRequestsIPs.GetCount(); i++){
	...
	for (size_t i = 0; i != m_aCurED2KSentReceivedIPs.GetCount(); i++){
	...
	if (bNewResponse){
#ifdef REPLACE_ATLMAP
		ASSERT(m_ReceivedUDPAnswersCount.find(m_nCurED2KSearchID) != m_ReceivedUDPAnswersCount.end());
		++m_ReceivedUDPAnswersCount[m_nCurED2KSearchID];
#else
		uint32 nResponses = 0;
...
#ifdef REPLACE_ATLMAP
	UDPServerRecord* &pRecord = m_aUDPServerRecords[nServerIP];
	if(pRecord == NULL){
...
{
#ifdef REPLACE_ATLMAP
	unordered_map<uint32, UINT>::const_iterator it = m_foundSourcesCount.find(nSearchID);
	ASSERT(it != m_foundSourcesCount.end());
	return it->second;
#else
	UINT nSources = 0;
...
void CSearchList::GetWebList(std::vector<SearchFileStruct>&SearchFileArray) const
{
	for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL;)
	{
		SearchListsStruct* listCur = m_listFileLists.GetNext(pos);
		for(POSITION pos2 = listCur->m_listSearchFiles.GetHeadPosition(); pos2 != NULL; )
		{
			...
			structFile.sortIndex[0] = pFile->GetFileName();
			structFile.sortIndex[1] = pFile->GetFileTypeDisplayStr();
			structFile.sortIndex[2] = md4str(pFile->GetFileHash());
			...
			SearchFileArray.push_back(structFile);
		}
	}
}
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP)
{
	...
	if (file
		...
		&& CPartFile::CanAddSource(toadd->GetClientID(), toadd->GetClientPort(), toadd->GetClientServerIP(), toadd->GetClientServerPort())){
	...
	while (!strToken.IsEmpty()){
		...
		if (!bClientResponse && toadd->GetSearchID() == m_nCurED2KSearchID){
			for (size_t i = 0; i < m_astrSpamCheckCurSearchExp.GetCount(); i++){
	...
	{
#ifdef REPLACE_ATLMAP
		ASSERT(m_foundFilesCount.find(toadd->GetSearchID()) != m_foundFilesCount.end());
		++m_foundFilesCount[toadd->GetSearchID()];
#else
		UINT tempValue = 0;
...
#ifdef REPLACE_ATLMAP
	ASSERT(m_foundSourcesCount.find(nSearchID) != m_foundSourcesCount.end());
	m_foundSourcesCount[nSearchID] += ( (bSpam && thePrefs.IsSearchSpamFilterEnabled()) ? min(nCount, 5) : nCount);
#else
	UINT tempValue = 0;
...
void CSearchList::KademliaSearchKeyword(uint32 searchID, const Kademlia::CUInt128* fileID, LPCTSTR name,
										...
										, CAtlArray<CAICHHash>& raAICHHashs, CAtlArray<uint8>& raAICHHashPopularity
...
void CSearchList::DoSpamRating(CSearchFile* pSearchFile, bool bIsClientFile, bool bMarkAsNoSpam, bool bRecalculateAll, bool bUpdate, uint32 dwFromUDPServerIP){
	...
	*/
	...
	// 1- filehash
	CSKey key(pSearchFile->GetFileHash());
#ifdef REPLACE_ATLMAP
	CKnownSpamHashsMap::const_iterator it = m_mapKnownSpamHashs.find(key);
	if(it != m_mapKnownSpamHashs.end()){
		bool bSpam = it->second;
#else
	bool bSpam = false;
	if (m_mapKnownSpamHashs.Lookup(key, bSpam)){
#endif
...
#ifdef REPLACE_ATLMAP
			m_mapKnownSpamHashs.erase(it);
#else
			m_mapKnownSpamHashs.RemoveKey(key);
#endif
		else
			bSureNegative = true;
	}
...
#ifdef REPLACE_ATLMAP
		m_mapKnownSpamHashs[key] = false;
#else
		m_mapKnownSpamHashs.SetAt(key, false);
#endif
#ifndef _DEBUG
	else if (bSureNegative && !bMarkAsNoSpam)
	{
		...
		//4 - Sizes
		for (size_t i = 0; i < m_aui64KnownSpamSizes.GetCount(); i++){
		...
		if (!bIsClientFile){ // only to skip some useless calculations
...
#ifdef REPLACE_ATLMAP
				unordered_set<uint32>::const_iterator it;
				if (pTempFile->GetServers()[i].m_nIP != 0 && pTempFile->GetServers()[i].m_bUDPAnswer
					&& (it = m_mapKnownSpamServerIPs.find(pTempFile->GetServers()[i].m_nIP)) != m_mapKnownSpamServerIPs.end()){
#else
				bool bFound = false;
				if (pTempFile->GetServers()[i].m_nIP != 0 && pTempFile->GetServers()[i].m_bUDPAnswer 
...
#ifdef REPLACE_ATLMAP
						if (pSearchFile->GetServers().GetSize() == 1 && m_mapKnownSpamServerIPs.size() <= 10){
#else
...
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamServerIPs.erase(it);
#else
			...
			for (int i = 0; i != pTempFile->GetServers().GetSize(); i++){
#ifdef REPLACE_ATLMAP
				unordered_map<uint32, UDPServerRecord*>::const_iterator it;
				if (!bMarkAsNoSpam && pTempFile->GetServers()[i].m_bUDPAnswer && (it = m_aUDPServerRecords.find(pTempFile->GetServers()[i].m_nIP)) != m_aUDPServerRecords.end() && it->second != NULL){
					UDPServerRecord* pRecord = it->second;
#else
				UDPServerRecord* pRecord = NULL;
...
#ifdef REPLACE_ATLMAP
			unordered_map<uint32, UINT>::const_iterator it = m_ReceivedUDPAnswersCount.find(pTempFile->GetSearchID());
			ASSERT(it != m_ReceivedUDPAnswersCount.end());
			uint32 nResponses = it->second;
			it = m_RequestedUDPAnswersCount.find(pTempFile->GetSearchID());
			ASSERT(it != m_RequestedUDPAnswersCount.end());
			uint32 nRequests = it->second;
#else
			uint32 nResponses = 0;
		...
		// 6 Sources
#ifdef REPLACE_ATLMAP
		unordered_set<uint32>::const_iterator it2;
		if (IsValidSearchResultClientIPPort(pTempFile->GetClientID(), pTempFile->GetClientPort())
			&& !::IsLowID(pTempFile->GetClientID())
			&& (it2 = m_mapKnownSpamSourcesIPs.find(pTempFile->GetClientID())) != m_mapKnownSpamSourcesIPs.end())
#else
		bool bFound = false;
		if (IsValidSearchResultClientIPPort(pTempFile->GetClientID(), pTempFile->GetClientPort())
			&& !::IsLowID(pTempFile->GetClientID())
...
#endif
		{
...
#ifdef REPLACE_ATLMAP
				m_mapKnownSpamSourcesIPs.erase(it2);
#else
			...
			for (int i = 0; i != pTempFile->GetClients().GetSize(); i++){
				...
				if (pTempFile->GetClients()[i].m_nIP != 0
					&& (it = m_mapKnownSpamSourcesIPs.find(pTempFile->GetClients()[i].m_nIP)) != m_mapKnownSpamSourcesIPs.end())
#else
				if (pTempFile->GetClients()[i].m_nIP != 0 
...
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamSourcesIPs.erase(it);
#else
	...
	if (pParent != NULL){
		...
		}*/ // X: No Spam Icon for Child SearchFile
	}
	else
...
#ifdef REPLACE_ATLMAP
			unordered_map<uint32, UDPServerRecord*>::const_iterator it;
			if (pTempFile->GetServers()[i].m_bUDPAnswer && (it = m_aUDPServerRecords.find(pTempFile->GetServers()[i].m_nIP)) != m_aUDPServerRecords.end() && it->second != NULL)
			{
				UDPServerRecord* pRecord = it->second;
#else
			UDPServerRecord* pRecord = NULL;
...
#ifdef REPLACE_ATLMAP
		unordered_map<uint32, UDPServerRecord*>::const_iterator it = m_aUDPServerRecords.find(dwFromUDPServerIP);
		if(it != m_aUDPServerRecords.end() && it->second != NULL)
			++it->second->m_nSpamResults;
#else
		UDPServerRecord* pRecord = NULL;
...
uint32 CSearchList::GetSpamFilenameRatings(const CSearchFile* pSearchFile, bool bMarkAsNoSpam){
	for (size_t i = 0; i < m_astrKnownSpamNames.GetCount(); i++){
	...
	if (!m_astrKnownSimilarSpamNames.IsEmpty() && !pSearchFile->GetNameWithoutKeyword().IsEmpty()){
		...
		for (size_t i = 0; i < m_astrKnownSimilarSpamNames.GetCount(); i++){
			bool bRemove = false;
...
#ifdef REPLACE_ATLMAP
		m_RequestedUDPAnswersCount[nSearchID] = (UINT)m_aCurED2KSentRequestsIPs.Add(dwServerIP) + 1;
#else
		m_RequestedUDPAnswersCount.SetAt(nSearchID, (UINT)m_aCurED2KSentRequestsIPs.Add(dwServerIP) + 1);
#endif
	}
	else
		ASSERT( false );
...
void CSearchList::MarkFileAsSpam(CSearchFile* pSpamFile, bool bRecalculateAll, bool bUpdate){
...
#ifdef REPLACE_ATLMAP
	m_mapKnownSpamHashs[CSKey(pSpamFile->GetFileHash())] = true;
#else
	...
	{
#ifdef REPLACE_ATLMAP
		m_mapKnownSpamSourcesIPs.insert(pSpamFile->GetClientID());
#else
...
#ifdef REPLACE_ATLMAP
			m_mapKnownSpamSourcesIPs.insert(pSpamFile->GetClients()[i].m_nIP);
#else
...
#ifdef REPLACE_ATLMAP
			m_mapKnownSpamServerIPs.insert(pSpamFile->GetServers()[i].m_nIP);
#else
...
void CSearchList::LoadSpamFilter(){
...
#ifdef REPLACE_ATLMAP
	unordered_set<uint32>().swap(m_mapKnownSpamServerIPs);
	unordered_set<uint32>().swap(m_mapKnownSpamSourcesIPs);
	CKnownSpamHashsMap().swap(m_mapKnownSpamHashs);
#else
		...
		for (UINT i = 0; i < RecordsNumber; i++) {
			...
			switch (tag.GetNameID()){
				case SP_FILEHASHSPAM:
					...
					if (tag.IsHash())
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamHashs[CSKey(tag.GetHash())] = true;
#else
				...
				case SP_FILEHASHNOSPAM:
					...
					if (tag.IsHash()){
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamHashs[CSKey(tag.GetHash())] = false;
#else
				...
				case SP_FILESOURCEIP:
					...
					if (tag.IsInt())
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamSourcesIPs.insert(tag.GetInt());
#else
				...
				case SP_FILESERVERIP:
					...
					if (tag.IsInt())
#ifdef REPLACE_ATLMAP
						m_mapKnownSpamServerIPs.insert(tag.GetInt());
#else
...
#ifdef REPLACE_ATLMAP
						m_aUDPServerRecords[PeekUInt32(&pBuffer[0])] = pRecord;
#else
	...
	DebugLog(_T("Loaded search Spam Filter. Entries - ServerIPs: %u, SourceIPs, %u, Hashs: %u, PositiveHashs: %u, FileSizes: %u, FullNames: %u, SimilarNames: %u")
		, m_mapKnownSpamSourcesIPs.size(), m_mapKnownSpamServerIPs.size(), m_mapKnownSpamHashs.size() - nDbgFileHashPos, nDbgFileHashPos
...
void CSearchList::SaveSpamFilter(){
	...
	try{
...
#ifdef REPLACE_ATLMAP
		for (CKnownSpamHashsMap::const_iterator it = m_mapKnownSpamHashs.begin(); it != m_mapKnownSpamHashs.end(); ++it)
		{
			bool bSpam = it->second;
			const CSKey&key = it->first;
#else
...
#endif
			CTag tag(bSpam?SP_FILEHASHSPAM:SP_FILEHASHNOSPAM, (BYTE*)key.m_key);
			tag.WriteNewEd2kTag(&file);
			nCount++;
		}

#ifdef REPLACE_ATLMAP
		for (unordered_set<uint32>::const_iterator it = m_mapKnownSpamServerIPs.begin(); it != m_mapKnownSpamServerIPs.end(); ++it)
		{
			uint32 dwIP = *it;
#else
...
#ifdef REPLACE_ATLMAP
		for (unordered_set<uint32>::const_iterator it = m_mapKnownSpamSourcesIPs.begin(); it != m_mapKnownSpamSourcesIPs.end(); ++it)
		{
			uint32 dwIP = *it;
#else
		...
		while( pos != NULL )
		{
			...
			CTag tag(SP_UDPSERVERSPAMRATIO, (uint32)sizeof(abyBuffer), abyBuffer);
			tag.WriteNewEd2kTag(&file);
			nCount++;
		}
		...
		if (nCount > 0){
			...
			for (pos = m_listFileLists.GetTailPosition(); pos != NULL; ){
				...
				if (pParams != NULL){
					...
					file.WriteUInt32((uint32)pSl->m_listSearchFiles.GetCount());
					POSITION pos2;
	...
	try {
		...
		for (int i = 0; i < nCount; i++){
			...
			if (pParams->eType == SearchTypeKademlia){
				if(nHighestKadSearchID == 0xFFFFFFFF || nHighestKadSearchID < pParams->dwSearchID){
				ASSERT( pParams->dwSearchID < 0x80000000 );
				nHighestKadSearchID = pParams->dwSearchID;
			}
			}
			else if (nHighestEd2kSearchID == 0xFFFFFFFF || nHighestEd2kSearchID < pParams->dwSearchID){
				ASSERT( pParams->dwSearchID >= 0x80000000 && !pParams->bClientSharedFiles);
