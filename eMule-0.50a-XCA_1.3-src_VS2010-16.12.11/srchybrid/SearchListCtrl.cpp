...
/*	CHeaderCtrl* pHeaderCtrl = GetHeaderCtrl();
	...
	for (int icol=0;icol<pHeaderCtrl->GetItemCount();icol++) {
		switch (icol) {
			...
			case 2: strRes = GetResString(IDS_SEARCHAVAIL) + (thePrefs.m_bExtControls ? _T(" (") + GetResString(IDS_DL_SOURCES) + _T(')') : _T("")); break;
...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iToPreview == 1) {
		if (m_SearchFileMenu.InsertMenu(MP_FIND, MF_STRING | MF_ENABLED, MP_PREVIEW, GetResString(IDS_DL_PREVIEW)))
	...
	if (thePrefs.IsSearchSpamFilterEnabled() && m_SearchFileMenu.InsertMenu(MP_REMOVESELECTED, MF_STRING | MF_ENABLED, MP_MARKASSPAM, (bContainsNotSpamFile || iSelected == 0) ? GetResString(IDS_MARKSPAM) : GetResString(IDS_MARKNOTSPAM))){
...
void CSearchListCtrl::CreateMenues()
{
	...
	m_SearchFileMenu.AppendMenu(MF_STRING, MP_CMT, GetResString(IDS_CMT_ADD));
...
void CSearchListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	ImageList_Draw(theApp.GetSystemImageList(), iImage, dc, cur_rec.left + iIndent + TREE_WIDTH, cur_rec.top + iIconPosY, ILD_TRANSPARENT);*/ // X: No Spam Icon for Child SearchFile
	...
	switch (nColumn)
	{
		case 0:			// file name
		{
			...
			if (thePrefs.IsSearchSpamFilterEnabled() && src->IsConsideredSpam())
				...
				&& (src->HasComment() || src->HasRating() || src->IsKadCommentSearchRunning()))
				iconIndex = src->UserRating(true);
			UINT uOffset = 16;
			if(iconIndex != 0){
				uOffset += sm_i2IconOffset;
				POINT point = {lpRect->left + uOffset, lpRect->top + iIconPosY};
				m_ImageList.Draw(dc, iconIndex, point, ILD_NORMAL);
				uOffset += 16;
			}
