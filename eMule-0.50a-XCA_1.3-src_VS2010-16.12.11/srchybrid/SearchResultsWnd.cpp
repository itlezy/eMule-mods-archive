...
#define	SEARCH_LIST_MENU_BUTTON_XOFF	2 //8->2
#define	SEARCH_LIST_MENU_BUTTON_WIDTH	200
...
void CSearchResultsWnd::OnInitialUpdate()
{
	...
	AddAnchor(m_ctlFilter, TOP_RIGHT);// X: [CI] - [Code Improvement]
	//AddAnchor(IDC_SDOWNLOAD, BOTTOM_LEFT);
	AddAnchor(searchlistctrl, TOP_LEFT, BOTTOM_RIGHT);// X: [CI] - [Code Improvement]
	AddAnchor(searchprogress, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddAnchor(IDC_CLEARALL, TOP_RIGHT);// X: [UIC] - [UIChange]
	AddAnchor(m_ctlOpenParamsWnd, TOP_RIGHT);// X: [CI] - [Code Improvement]
	AddAnchor(searchselect.m_hWnd, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_STATIC_DLTOof, TOP_LEFT);
	AddAnchor(*m_cattabs, TOP_LEFT, TOP_RIGHT);
...
void CSearchResultsWnd::CancelSearch(UINT uSearchID)
{
	...
	switch(pParams->eType){
		...
		case SearchTypeKademlia:
			...
			SearchCanceled(pParams->dwSearchID);
			break;
	}
}
...
void CSearchResultsWnd::DownloadSelected(bool bPaused)
{
	...
	while (pos != NULL)
	{
		...
		if (iIndex >= 0)
		{
			...
			//if ( theApp.knownfiles->CheckAlreadyDownloadedFileQuestion(tempFile.GetFileHash(), tempFile.GetFileName()) )// X-Ray :: SearchCatch
...
void CSearchResultsWnd::Localize()// X: [RUL] - [Remove Useless Localize]
{
	UpdateCatTabs();
	UpdateFilterLabel();
...
void CSearchResultsWnd::LocalizeAll()// X: [RUL] - [Remove Useless Localize]
{
	Localize();
...
CString DbgGetFileMetaTagName(UINT uMetaTagID)
{
	switch (uMetaTagID)
	{
		case FT_FILENAME:			return _CString(_T("@Name"));
		case FT_FILESIZE:			return _CString(_T("@Size"));
		case FT_FILESIZE_HI:		return _CString(_T("@SizeHI"));
		case FT_FILETYPE:			return _CString(_T("@Type"));
		case FT_FILEFORMAT:			return _CString(_T("@Format"));
		case FT_LASTSEENCOMPLETE:	return _CString(_T("@LastSeenComplete"));
		case FT_SOURCES:			return _CString(_T("@Sources"));
		case FT_COMPLETE_SOURCES:	return _CString(_T("@Complete"));
		case FT_MEDIA_ARTIST:		return _CString(_T("@Artist"));
		case FT_MEDIA_ALBUM:		return _CString(_T("@Album"));
		case FT_MEDIA_TITLE:		return _CString(_T("@Title"));
		case FT_MEDIA_LENGTH:		return _CString(_T("@Length"));
		case FT_MEDIA_BITRATE:		return _CString(_T("@Bitrate"));
		case FT_MEDIA_CODEC:		return _CString(_T("@Codec"));
		case FT_FILECOMMENT:		return _CString(_T("@Comment"));
		case FT_FILERATING:			return _CString(_T("@Rating"));
		case FT_FILEHASH:			return _CString(_T("@Filehash"));
	}
...
CAtlArray<CString> g_astrParserErrors;

static TCHAR s_chLastChar = 0;
static CString s_strSearchTree;

bool DumpSearchTree(size_t& iExpr, const CSearchExpr& rSearchExpr, int iLevel, bool bFlat)
{
...
void ParsedSearchExpression(const CSearchExpr* pexpr)
{
	...
	int iNonDefTags = 0;
	//CStringA strDbg;
	for (size_t i = 0; i < pexpr->m_aExpr.GetCount(); i++)
	{
...
#ifdef _DEBUG// X: [RDL] - [Remove Debug Log]
		m_strDbg.Append(_T("AND "));// X: [CI] - [Code Improvement]
#endif
	}
...
bool GetSearchPacket(CSafeMemFile* pData, SSearchParams* pParams, bool bTargetSupports64Bit, bool* pbPacketUsing64Bit)
{
	...
	if (!pParams->strBooleanExpr.IsEmpty())
	{
		...
		{
		    s_SearchExpr.m_aExpr.RemoveAll();
			CString strError(GetResString(IDS_SEARCH_EXPRERROR) + _T("\n\n") + g_astrParserErrors[g_astrParserErrors.GetCount() - 1]);
		    throw new CMsgBoxException(strError, MB_ICONWARNING );
	    }
	...
	//for (size_t i = 0; i < s_SearchExpr.m_aExpr.GetCount(); i++){
	...
	for (size_t j = 0; j < s_SearchExpr.m_aExpr.GetCount(); j++)
	{
...
bool CSearchResultsWnd::StartNewSearch(SSearchParams* pParams)
{
	...
	if (eSearchType == SearchTypeEd2kServer || eSearchType == SearchTypeEd2kGlobal)
	{
		...
		{
			AfxMessageBox(ex->m_strMsg, ex->m_uType);
			ex->Delete();
			delete pParams;
			return false;
		}
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams)
{
	...
	{
		delete[] pSearchTermsData;
		throw new CMsgBoxException(strException, MB_ICONWARNING);
	}
...
void CSearchResultsWnd::DeleteSearch(uint32 nSearchID)
{
	...
	if (iTabItems > 0){
		...
		if (iCurSel == CB_ERR)
			...
			iCurSel = iTabItems - 1;
		searchselect.SetCurSel(iCurSel);	// returns CB_ERR if error or no prev. selection(!)
		if (searchselect.GetCurSel() == CB_ERR)					// if still error
...
void CSearchResultsWnd::SetSelectedCat(int iSel)
{
	m_cattabs->SetCurSel(iSel);
}
...
void CSearchResultsWnd::SearchRelatedFiles(CAtlList<CAbstractFile*>& listFiles)
{
	...
	CString strNames;
	while (pos != NULL){
		CAbstractFile* pFile = listFiles.GetNext(pos);
...
LRESULT CSearchResultsWnd::OnChangeFilter(WPARAM wParam, LPARAM lParam)
{
	...
	bool bColumnDiff = (searchlistctrl.m_nFilterColumn != wParam);
	searchlistctrl.m_nFilterColumn = wParam;
	...
	CAtlArray<CString>& m_astrFilter=searchlistctrl.m_astrFilter;
...
void CSearchResultsWnd::OnSearchListMenuBtnDropDown(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	menuFileSizeFormat.CheckMenuRadioItem(MP_SHOW_FILESIZE_DFLT, MP_SHOW_FILESIZE_MBYTE, MP_SHOW_FILESIZE_DFLT + thePrefs.m_eFileSizeFormat, 0);
...
BOOL CSearchResultsWnd::OnCommand(WPARAM wParam, LPARAM lParam)
{
	if (wParam>=MP_CAT_SET0 && wParam<=MP_CAT_SET0+99){
		searchlistctrl.SetFilter(wParam - MP_CAT_SET0);
		return TRUE;
	}
	switch (wParam)
	{
...
void CSearchResultsWnd::UpdateFilterLabel(){
	CString str;
	str.Format(_T("%s(%s)"), GetResString(IDS_SW_RESULT), GetFilterLabel());
	m_btnSearchListMenu->SetWindowText(str);
}
