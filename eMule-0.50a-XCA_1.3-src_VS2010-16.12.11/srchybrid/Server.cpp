...
#include "IPFilter.h"// X: [CI] - [Code Improvement]
...
{
	port = in_data->port;
	realport = 0;//>>> Spike2::Aux Ports support [lugdunummaster]
	ip = in_data->ip;
	_tcscpy_s(ipfull, ipstr(ip));
	if((isfiltered = theApp.ipfilter->IsFiltered(ip))// X: [CI] - [Code Improvement]
		 && thePrefs.GetLogFilteredIPs())
		 AddDebugLogLine(false, _T("IPFilter(Server.Met): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), GetAddress(), ipstr(GetIP()), theApp.ipfilter->GetLastHit());
	files = 0;
	users = 0;
	m_uPreference = 0;
	ping = 0;
	...
	m_structServerCountry = theApp.ip2country->GetLocationFromIP(ip)->country; //EastShare - added by AndCycle, IP to Country// X: [IP2L] - [IP2Location]
}
...
CServer::CServer(uint16 in_port, LPCTSTR i_addr)
{
	...
	if((isfiltered = theApp.ipfilter->IsFiltered(ip))// X: [CI] - [Code Improvement]
		 && thePrefs.GetLogFilteredIPs())
		AddDebugLogLine(false, _T("IPFilter(AddServer): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), GetAddress(), ipstr(GetIP()), theApp.ipfilter->GetLastHit());
	_tcscpy_s(ipfull, ipstr(ip));
	files = 0;
	users = 0;
	m_uPreference = 0;
...
CServer::CServer(const CServer* pOld)
{
	port = pOld->port;
	ip = pOld->ip; 
	isfiltered = pOld->isfiltered;// X: [CI] - [Code Improvement]
	staticservermember = pOld->IsStaticMember();
	_tcscpy_s(ipfull, pOld->ipfull);
	files = pOld->files;
	users = pOld->users;
	realport = pOld->realport; //>>> Spike2::Aux Ports support [lugdunummaster]
	...
	m_structServerCountry = pOld->m_structServerCountry; //EastShare - added by AndCycle, IP to Country // X: [CI] - [Code Improvement]
}
...
bool CServer::AddTagFromFile(CFileDataIO* servermet)
{
	...
	case ST_DYNIP:
		ASSERT( tag->IsStr() );
		if (tag->IsStr() && tag->GetStr()[0] != 0){
			if (m_strDynIP.IsEmpty()) {
				// set dynIP and reset available (out-dated) IP
				SetDynIP(tag->GetStr());
				SetIP(0);
				isfiltered = false;// X: [CI] - [Code Improvement]
			}
		}
		break;
	case ST_MAXUSERS:
...
//>>> Spike2::Aux Ports support [lugdunummaster]
		...
		{
			ASSERT( tag->IsInt() || tag->IsStr());
			if (tag->IsInt())
				realport = (uint16)tag->GetInt();
			//for morph and others:
			else if(tag->IsStr())
				realport = (uint16)_tstoi(tag->GetStr());
		}
//<<< Spike2::Aux Ports support [lugdunummaster]
		else{
...
void CServer::ResetIP2Country(){
	m_structServerCountry = theApp.ip2country->GetLocationFromIP(ip)->country;// X: [IP2L] - [IP2Location]
}
