...
void CServerListCtrl::RefreshServer(const CServer* server)
{
	if (!server || !CemuleDlg::IsRunning())
		return;
	// X-Ray :: Optimizations :: Start -  Don't Refresh item if not needed
...
void CServerListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	*/
	//TK4 Mod grey out Filtered servers or Dead servers
		if(server->GetFailedCount() >= thePrefs.GetDeadServerRetries() || server->IsFiltered())// X: [CI] - [Code Improvement]
	{
			...
			switch(iColumn){
				case 0:
				{
					...
					if(theApp.ip2country->ShowCountryFlag() )
						theApp.ip2country->GetFlagImageList()->Draw(dc, server->GetCountryFlagIndex(), point, ILD_NORMAL);
					else
						m_ImageList.Draw(dc, 0, point, ILD_NORMAL);
						//m_ImageList.DrawIndirect(dc, 0, point2, CSize(16,16), CPoint(0,0), ILD_NORMAL);

					cur_rec.left += 19 + sm_iLabelOffset;
...
void CServerListCtrl::GetItemDisplayText(const CServer* server, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
	...
	switch(iSubItem){
		...
		case 1:{
			...
			_tcsncpy(pszText, temp, cchTextMax);
			break;
		}
		case 2:
		...
		case 3:
			if(server->GetPing())
				_ultot_s(server->GetPing(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 4:
			if(server->GetUsers())
				_ultot_s(server->GetUsers(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 5:
			if(server->GetMaxUsers())
				_ultot_s(server->GetMaxUsers(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 6:
			if(server->GetFiles())
				_ultot_s(server->GetFiles(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 7:
		{
			...
			_tcsncpy(pszText, GetResString(StrID), cchTextMax);
			break;
		}
		case 8:
			_ultot_s(server->GetFailedCount(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 9:
		...
		case 10:
			if(server->GetSoftFiles())
				_ultot_s(server->GetSoftFiles(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 11:
			if(server->GetHardFiles())
				_ultot_s(server->GetHardFiles(), pszText, cchTextMax, 10);// X: [CI] - [Code Improvement]
			break;
		case 12:
		{
