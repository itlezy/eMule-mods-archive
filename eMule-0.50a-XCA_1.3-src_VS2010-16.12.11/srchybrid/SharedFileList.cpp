...
/*
void CSharedFileList::CopySharedFileMap(CAtlMap<CCKey,CKnownFile*,CCKeyTraits> &Files_Map)
{
...
bool CSharedFileList::SafeAddKFile(CKnownFile* toadd, bool bOnlyAdd)
{
	...
	if (bAdded && output){
		...
		theApp.emuledlg->sharedfileswnd->ShowFilesCount(); //Xman Code Improvement for ShowFilesCount

		//Xman [MoNKi: -Downloaded History-]
		if(thePrefs.m_bHistoryShowShared && !toadd->IsPartFile()){
...
void CSharedFileList::Reload(bool bNotify)// X: [QOH] - [QueryOnHashing]
{
	...
	if (!output)
		return;
	// END SLUGFILLER: SafeHash
	g_VolumeInfo.ClearCache((size_t)-1); // X: [FSFS] - [FileSystemFeaturesSupport]
#ifdef REPLACE_ATLMAP
	...
	theApp.emuledlg->sharedfileswnd->historylistctrl.ReloadFileList();
...
#endif
		if (!IsCPartFile(file)/*!file->IsKindOf(RUNTIME_CLASS(CPartFile))*/)
