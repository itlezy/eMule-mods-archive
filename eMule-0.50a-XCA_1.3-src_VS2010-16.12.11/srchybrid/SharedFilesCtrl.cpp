...
void CSharedFilesCtrl::Init()
{
	...
	InsertColumn(19,GetResString(IDS_SF_UPLOADED_PARTS),LVCFMT_LEFT,150);
...
void CSharedFilesCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	//InitItemMemDC(dc, lpDrawItemStruct->rcItem, (pKnownFile && pKnownFile->GetUpPriority()==PR_POWER) ? RGB(255, 210, 210) : ((lpDrawItemStruct->itemID % 2)?m_crEvenLine:m_crWindow), lpDrawItemStruct->itemState);
	InitItemMemDC(dc, lpDrawItemStruct->rcItem, pKnownFile && pKnownFile->IsPartFile()? RGB(255,250,200)/*Partfile*/: pKnownFile && pKnownFile->GetUpPriority()==PR_POWER ? RGB(255, 210, 210)/*Power*/:((lpDrawItemStruct->itemID % 2)?m_crEvenLine:m_crWindow), lpDrawItemStruct->itemState);
	...
	for(int iCurrent = 0; iCurrent < iCount; iCurrent++){
		...
		if(iColumnWidth > 2*sm_iSubItemInset && cur_rec.right>0){// X: [DDHC] - [Don't Draw Hidden Column]
			...
			switch(iColumn){
				...
				case 8:
					if (pKnownFile && pKnownFile->GetPartCount()){
						cur_rec.bottom--;
						cur_rec.top++;
...
void CSharedFilesCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	//Xman PowerRelease
	m_SharedFilesMenu.EnableMenuItem(MP_PRIOPOWER, iSelectedItems >0 ? MF_ENABLED : MF_GRAYED);
	//Xman end
					...
					if (delsucc){
						if (IsKindOfCKnownFile(myfile)/*myfile->IsKindOf(RUNTIME_CLASS(CKnownFile))*/){
							...
							if(!thePrefs.m_bHistoryShowShared){
								theApp.emuledlg->sharedfileswnd->historylistctrl.AddFile((CKnownFile*)myfile);
			...
			case MP_ULFEEDBACK: 
				{ 
					...
					if(!selectedList.IsEmpty())
					{
						...
						feed.AppendFormat(_T("Mod: %s \r\n"),_T("eMule v") + theApp.m_strCurVersionLong);  
					}

					while (!selectedList.IsEmpty())
					{
						CShareableFile* pshfile = selectedList.RemoveHead();
						if (IsKindOfCKnownFile(pshfile)/*pshfile->IsKindOf(RUNTIME_CLASS(CKnownFile))*/){
							CKnownFile* file = (CKnownFile*)pshfile;
	...
	if (iSel != -1)
	{
		...
		if (file)
		{
			...
			//if (GetKeyState(VK_MENU) & 0x8000)
                          if (GetKeyState(VK_MENU) & 0x8000 || (IsKindOfCKnownFile(file)/*file->IsKindOf(RUNTIME_CLASS(CKnownFile))*/ && ((CKnownFile*)file)->IsPartFile()))
			{
...
void CSharedFilesCtrl::CreateMenues()
{
    ...
    // Xman: IcEcRacKer Copy UL-feedback
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_ULFEEDBACK, _T("Copy UL-Feedback")); 
