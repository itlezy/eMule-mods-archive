...
void CStatisticFile::AddBlockTransferred(uint64 start, uint64 end, uint32 count){
	...
	if (spreadlist.IsEmpty()) {
		if (start > 0)
			spreadlist.AddTail(new Spread_Struct(0, start, 0));
		spreadlist.AddTail(new Spread_Struct(start, end, count));
		return;
	}
	...
	if (cur_spread->end <= end) {
		if (cur_spread->end <= start) {
			if (cur_spread->end == start) {
				...
				spreadlist.AddTail(new Spread_Struct(cur_spread->end, start, 0));
			}
			spreadlist.AddTail(new Spread_Struct(start, end, count));
			return;
		}
		if (cur_spread->end == end) {
			...
			if (cur_spread->start < start) {
				...
				spreadlist.AddTail(new Spread_Struct(start, end, count+cur_spread->count));
				return;
			}
		} else {
			spreadlist.AddTail(new Spread_Struct(cur_spread->end, end, count));
		}
		...
		spreadlist.InsertAfter(pos, new Spread_Struct(start, cur_spread->end, count+cur_spread->count));
		cur_spread->end = start;
		return;
	}
	...
	if (cur_spread->end == start) {
		...
		Spread_Struct* new_spread = new Spread_Struct(start, cur_spread->end, cur_spread->count);
	...
	spreadlist.InsertAfter(pos, new Spread_Struct(end, cur_spread->end, cur_spread->count));
	cur_spread->count += count;
	cur_spread->end = end;
}
//Xman end
...
void CStatisticFile::UpdateCountedTransferred() 
{
	...
	if(time(NULL) - m_tlastdataupdate > 3600 * 12) // every 12 hours
	{
		...
		//and it's crash-safe
		uint64 difference = (time(NULL)-m_tlastdataupdate)/(3600*12); //in half days
...
void CStatisticFile::DrawSpreadBar(CDC* dc, RECT* rect, bool bFlat) /*const*/
{
	...
	if(!InChangedSpreadBar || lastSize!=iWidth || lastbFlat!= bFlat){
		...
		s_SpreadBar.Draw(&cdcStatus, 0, 0, iWidth, bFlat);
	}
	else
		hOldBitmap = cdcStatus.SelectObject(m_bitmapSpreadBar);
