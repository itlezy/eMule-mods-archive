...
struct Spread_Struct{
	Spread_Struct(uint64 _start, uint64 _end, uint32 _count) : start(_start), end(_end), count(_count){}
	uint64 start;
	uint64 end;
	uint32 count;
};
...
*/
	...
	// ==> Removed Spreadbars (old version) [SlugFiller] - Stulle
	//CAtlList<Spread_Struct*>*	GetSpreadList()	{return &spreadlist;}
	// <== Removed Spreadbars (old version) [SlugFiller] - Stulle
	//Xman advanced upload-priority
	...
	uint64 m_tlastdataupdate;// X: [64T] - [64BitTime]
	//Xman end
...
private:
	// ==> Removed Spreadbars (old version) [SlugFiller] - Stulle
	//CAtlList<Spread_Struct*> spreadlist; //Xman PowerRelease
	// <== Removed Spreadbars (old version) [SlugFiller] - Stulle
	uint32 requested;
	uint32 accepted;
	uint64 transferred;
