...
#ifdef REPLACE_ATLMAP
	m_mapIconNameToIconIdx.clear();
	m_mapMenuIdToIconIdx.clear();

	for (CStringToHBITMAPMap::const_iterator it = m_mapIconNameToBitmap.begin(); it != m_mapIconNameToBitmap.end(); ++it)
		VERIFY( DeleteObject(it->second) );
	m_mapIconNameToBitmap.clear();
#else
...
{
#ifdef REPLACE_ATLMAP
	ASSERT( m_mapIconNameToIconIdx.size() == 0 );
	ASSERT( m_mapMenuIdToIconIdx.size() == 0 );
	ASSERT( m_mapIconNameToBitmap.size() == 0 );
#else
...
void CTitleMenu::MeasureItem(LPMEASUREITEMSTRUCT mi)
{
	if (mi->itemID == MP_TITLE)
	{
		...
		::ReleaseDC(HWND_DESKTOP, dc.Detach());
...
#ifdef REPLACE_ATLMAP
		unordered_map<UINT_PTR, int>::const_iterator it = m_mapMenuIdToIconIdx.find(di->itemID);
		if(it == m_mapMenuIdToIconIdx.end())
			return;
		int nIconPos = it->second;
#else
		int nIconPos;
...
static HBITMAP Create32BitHBITMAP(HDC hdc, int cx, int cy, void **ppvBits = NULL)
{
    ...
    HDC hdcUsed = hdc ? hdc : GetDC(HWND_DESKTOP);
    if (hdcUsed)
    {
        hBmp = CreateDIBSection(hdcUsed, &bmi, DIB_RGB_COLORS, ppvBits, NULL, 0);
        if (hdc == NULL)
            ReleaseDC(HWND_DESKTOP, hdcUsed);
    }
    return hBmp;
}
...
void CTitleMenu::SetMenuBitmap(UINT nFlags, UINT_PTR nIDNewItem, LPCTSTR /*lpszNewItem*/, LPCTSTR lpszIconName)
{
	if (!m_bIconMenu || (nFlags & MF_SEPARATOR) != 0/* || !(thePrefs.GetWindowsVersion() >= _WINVER_2K_ )*/) {
		ASSERT(m_bIconMenu == NULL || lpszIconName == NULL);
		return;
	}
	...
	if (thePrefs.GetWindowsVersion() >= _WINVER_VISTA_)
	{
		...
		if (!strIconLower.IsEmpty())
		{
			HBITMAP hBmp = NULL;
#ifdef REPLACE_ATLMAP
			CStringToHBITMAPMap::const_iterator it = m_mapIconNameToBitmap.find(strIconLower);
			if(it != m_mapIconNameToBitmap.end())
				hBmp = it->second;
			else
#else
	...
	else
	{
		...
		{
#ifdef REPLACE_ATLMAP
			CStringToIntMap::const_iterator it = m_mapIconNameToIconIdx.find(strIconLower);
			if(it != m_mapIconNameToIconIdx.end())
			{
				nPos = it->second;
			...
			else
			{
				...
				if (hIcon)
				{
					...
					if (nPos != -1)
					{
						...
						m_mapMenuIdToIconIdx[nIDNewItem] = nPos;
#else
						m_mapIconNameToIconIdx.SetAt(strIconLower, nPos);
						m_mapMenuIdToIconIdx.SetAt(nIDNewItem, nPos);
#endif
