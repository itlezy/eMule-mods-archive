...
#endif
#include "multimon.h" // X-Ray :: FiXeS :: Bugfix :: bscabral
...
// X-Ray :: FiXeS :: Bugfix :: Start :: bscabral
BOOL CALLBACK EnumMon(HMONITOR /*hMonitor*/, HDC /*hdcMonitor*/, LPRECT lprcMonitor, LPARAM dwData)
{
	LPRECT rect = (LPRECT)dwData;
	UnionRect(rect, rect, lprcMonitor);
	return TRUE;
}

void GetRealScreenRect(LPRECT destRct)
{
	RECT rect = {0, 0, 0, 0};
	EnumDisplayMonitors(NULL, NULL, EnumMon, (LPARAM)&rect);
	CopyRect(destRct, &rect);
}
// X-Ray :: FiXeS :: Bugfix :: End :: bscabral
...
void CToolTipCtrlX::CustomPaint(LPNMTTCUSTOMDRAW pNMCD)
{
	...
	int iCaptionEnd = 0;// X: [CI] - [Code Improvement]
	if(bShowFileIcon){
		iCaptionEnd = strText.Find(_T("\n<br_head>\n"));
		if(iCaptionEnd<0)
			iCaptionEnd = 0;
	} // special tooltip with file icon
	...
	while (iPos != -1)
	{
		...
		if (iColon != -1) {
			...
			if (hTheme && bUseEmbeddedThemeFonts) {
				CRect rcExtent;
				RECT rcBounding = {0, 0, 32767, 32767};
			...
			int tempMaxCol1Width = siz.cx;// X: [CI] - [Code Improvement]
			if(bShowFileIcon && iPos <= iCaptionEnd + strLine.GetLength())
				tempMaxCol1Width += iIconDrawingWidth;
			if(iMaxCol1Width<tempMaxCol1Width)
				iMaxCol1Width = tempMaxCol1Width;
			...
			if (iPos <= iCaptionEnd)
				iCaptionHeight += iTextHeight/*siz.cy + iLineHeightOff*/;
			else
				sizText.cy += iTextHeight/*siz.cy + iLineHeightOff*/;
			...
			int tempMaxSingleLineWidth = siz.cx;// X: [CI] - [Code Improvement]
			if(bShowFileIcon && iPos <= iCaptionEnd)
				tempMaxSingleLineWidth += iIconDrawingWidth;
			if(iMaxSingleLineWidth<tempMaxSingleLineWidth)
				iMaxSingleLineWidth = tempMaxSingleLineWidth;
			if (bShowFileIcon && iPos <= iCaptionEnd + strLine.GetLength())
				iCaptionHeight += siz.cy + iLineHeightOff;
			else
