...
#define DTOOLBAR_NUM_BUTTONS 17+1 //>>> WiZaRd::OpenIncoming+1
...
//>>> WiZaRd::OpenIncoming
	atb1[17].iBitmap = 14;
	atb1[17].idCommand = MP_HM_OPENINC;
	atb1[17].fsState = TBSTATE_ENABLED | TBSTATE_WRAP;
	atb1[17].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[17].iString = m_btnBar->AddString(GetResString(IDS_OPENINC)+L"...");
//<<< WiZaRd::OpenIncoming
...
LRESULT CToolbarWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	iml.Add(CTempIconLoader(_T("OPENFOLDER")));	 //>>> WiZaRd::OpenIncoming
	...
	if (bNeedMonoIcons)
	{
		...
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("OPENFOLDER"))));	 //>>> WiZaRd::OpenIncoming
...
void CToolbarWnd::OnAvailableCommandsChanged(CAtlList<int>* liCommands)
{
	TBBUTTONINFO tbbi = {0};
