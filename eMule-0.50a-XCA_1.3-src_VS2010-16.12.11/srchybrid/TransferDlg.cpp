...
int CTransferDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	DockControlBar(m_pwndToolbar, AFX_IDW_DOCKBAR_TOP, (LPRECT)NULL); //morph4 top
...
void CTransferDlg::Localize()// X: [RUL] - [Remove Useless Localize]
{
	m_pwndTransfer->LocalizeAll();
...
void CTransferDlg::ShowQueueCount()
{
	m_pwndTransfer->ShowQueueCount();
}

void CTransferDlg::UpdateFilesCount()
{
	m_pwndTransfer->UpdateFilesCount();
}
...
size_t	 CTransferDlg::AddCategoryInteractive()
{
...
void CTransferDlg::ResetTransToolbar(bool bShowToolbar)
{
	m_pwndTransfer->ResetTransToolbar(bShowToolbar);
}
...
void CTransferDlg::UpdateListCount(CTransferWnd::EWnd2 listindex, size_t iCount)
{
	m_pwndTransfer->UpdateListCount((CTransferWnd::EWnd2)listindex, iCount);
}

size_t	CTransferDlg::AddCategory(CString newtitle,CString newincoming,CString newtemp,CString newcomment,CString newautocat,bool addTab)// X: [TD] - [TempDir]
{
	return m_pwndTransfer->AddCategory(newtitle, newincoming, newtemp, newcomment, newautocat, addTab);
}
...
void CTransferDlg::ShowBufferUsage(uint64 totalsize)// X: [GB] - [Global Buffer]
{
	m_pwndTransfer->ShowBufferUsage(totalsize);
}
//////////////////////////////////////////////////////////////////
