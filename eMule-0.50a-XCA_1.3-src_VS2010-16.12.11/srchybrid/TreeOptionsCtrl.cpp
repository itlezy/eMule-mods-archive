...
void CTreeOptionsCtrl::HandleCheckBox(HTREEITEM hItem, BOOL bCheck)
{
	...
	if (hParent && IsCheckBox(hParent))
	{
		...
		else
		{
			...
			SetEnabledSemiCheckBox(hParent, IsItemEnabled(hParent));// X: [UIC] - [UIChange]
		}
	}
...
void CTreeOptionsCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (bHit)
	{
		if (IsCheckBox(hItem))
		{
			...
			if (IsItemEnabled(hItem))// X: [UIC] - [UIChange]
			{
				//Turn of redraw to Q all the changes we're going to make here
				SetRedraw(FALSE);
...
BOOL CTreeOptionsCtrl::SetRadioButton(HTREEITEM hParent, int nIndex)
{
	...
	while (hChild)
	{
		...
		else
		{
			...
			//Turn this item off
			if (IsItemEnabled(hChild))// X: [UIC] - [UIChange]
...
BOOL CTreeOptionsCtrl::SetGroupEnable(HTREEITEM hItem, BOOL bEnable)
{
	...
	SetItemEnable(hItem, bEnable);// X: [UIC] - [UIChange]
...
BOOL CTreeOptionsCtrl::SetRadioButtonEnable(HTREEITEM hItem, BOOL bEnable)
{
	...
	BOOL bCheck;
	VERIFY(GetRadioButton(hItem, bCheck));
	int nImage = bEnable?(bCheck?3:2):(bCheck?7:6);
...
BOOL CTreeOptionsCtrl::OnSelchanged(NMHDR *pNMHDR, LRESULT *pResult) 
{
	...
	if (!m_bBeingCleared)
	{
		...
		if (pNMTreeView->itemNew.hItem != NULL)
		{
			...
			if (pItemData && pItemData->m_pRuntimeClass1 && IsItemEnabled(pNMTreeView->itemNew.hItem))// X: [UIC] - [UIChange]
...
BOOL CTreeOptionsCtrl::OnNmClick(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
	...
	{
		CTreeOptionsItemData* pItemData = (CTreeOptionsItemData*) GetItemData(hItem);
		if (IsItemEnabled(hItem) && (uFlags & TVHT_ONITEM) && pItemData && pItemData->m_pRuntimeClass1 && m_hControlItem == NULL)// X: [UIC] - [UIChange]
			CreateNewChildControl(hItem);
...
BOOL CTreeOptionsCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	case CDDS_ITEMPREPAINT:
		{
			if(!IsItemEnabled((HTREEITEM) pCustomDraw->nmcd.dwItemSpec)) // Gray disabled items// X: [UIC] - [UIChange]
	...
	case CDDS_ITEMPOSTPAINT:
		{
			...
			if (pItemData && pItemData->m_Type == CTreeOptionsItemData::ColorBrowser && pItemData->m_bDrawColorForIcon)
			{
				...
				r.DeflateRect(1, 1);
				dc.FillSolidRect(&r, IsItemEnabled(hItem) ? CLR_DEFAULT : GetSysColor(COLOR_GRAYTEXT));// X: [UIC] - [UIChange]
				r.DeflateRect(1, 1);
				//r.right = r.left + 16;
...
{
	return _CString(_T("Please specify a folder"));
}

CString CTreeOptionsEdit::GetBrowseForFileCaption()
{
	return _CString(_T("Please specify a file"));
}

CString CTreeOptionsEdit::GetFileExtensionFilter()
{
	return _CString(_T("All Files (*.*)|*.*||"));
}
...
// X: [UIC] - [UIChange]
void CTreeOptionsCtrl::SetItemEnable(HTREEITEM hItem, BOOL bEnable){
	CTreeOptionsItemData* pItemData = (CTreeOptionsItemData*) GetItemData(hItem);
	pItemData->m_bEnabled = bEnable;
}

BOOL CTreeOptionsCtrl::IsItemEnabled(HTREEITEM hItem) const	
{ 
	CTreeOptionsItemData* pItemData = (CTreeOptionsItemData*) GetItemData(hItem);
