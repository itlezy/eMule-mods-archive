...
/*	if ( m_pfnOpenSCManager && m_pfnOpenService && 
	...
	{*/
		...
		schSCManager = /*m_pfn*/OpenSCManager( 
			NULL,				// local machine 
		...
		schService = /*m_pfn*/OpenService( schSCManager, L"upnphost", GENERIC_READ ); 
		if ( schService == NULL )
		{
			/*m_pfn*/CloseServiceHandle( schSCManager );
			return false;
		}
		...
		if ( /*m_pfn*/QueryServiceStatusEx( schService, SC_STATUS_PROCESS_INFO,
			(LPBYTE)&ssStatus, sizeof(SERVICE_STATUS_PROCESS), &nBytesNeeded ) )
		{
			if ( ssStatus.dwCurrentState == SERVICE_RUNNING )
				bResult = true;
		}
		/*m_pfn*/CloseServiceHandle( schService );

		if ( !bResult )
		{
			schService = /*m_pfn*/OpenService( schSCManager, L"upnphost", SERVICE_START );
			if ( schService )
			{
				// Power users have only right to start service, thus try to start it here
				if ( /*m_pfn*/StartService( schService, 0, NULL ) ){
					bResult = true;
...
void CUPnPImplWinServ::StopUPnPService(){
	...
	if ( m_bInited/* && m_pfnOpenService && m_pfnCloseServiceHandle && m_pfnControlService &&  m_pfnOpenSCManager*/)
	{
		...
		schService = /*m_pfn*/OpenService(schSCManager, L"upnphost", SERVICE_STOP);
		if (schService){
			SERVICE_STATUS structServiceStatus; 
			if (/*m_pfn*/ControlService( schService, SERVICE_CONTROL_STOP, &structServiceStatus) != 0){
...
void CUPnPImplWinServ::StartDiscovery(uint16 nTCPPort, uint16 nUDPPort, uint16 nUDPServerPort, uint16 nTCPWebPort, bool bSecondTry)
{
	...
	if (nTCPPort != 0){
		m_nTCPPort = nTCPPort;
		m_nUDPPort = nUDPPort;
		m_nUDPServerPort = nUDPServerPort;
...
void CUPnPImplWinServ::AddDevice(DevicePointer device, bool bAddChilds, size_t nLevel)
{
	if (nLevel > 10){
		ASSERT( false );
		return;
...
{
   std::vector< ServicePointer >::const_iterator Iter;
...
CString CUPnPImplWinServ::GetLocalRoutableIP(ServicePointer pService)
{
	...
	//if (m_pfGetBestInterface != NULL){
		try {	// just to be sure; another call from iphlpapi used earlier in eMule seemed to crash on some systems according to dumps
			hrRes = /*m_pf*/GetBestInterface( ip, &nInterfaceIndex );
		}
		catch(...) {
			ASSERT( false );
			}
	...
	//if (m_pfGetIpAddrTable != NULL){
		try {	// just to be sure; another call from iphlpapi used earlier in eMule seemed to crash on some systems according to dumps
			hrRes = /*m_pf*/GetIpAddrTable( ipAddr, &nSize, FALSE );
		}
		catch(...) {
			ASSERT( false );
		}
		...
		if ( SUCCEEDED( hr ) && ! strActionResult.IsEmpty() )
		{
				...
				stristr( strActionResult, L"|VT_BSTR=eMule UDP|" ) != NULL )
			{
				...
				else // different IP found in the port mapping entry
				{
					...
					CString strUDPPort, strUDPServerPort, strTCPPort, strTCPWebPort;
					strUDPPort.Format(_T("|VT_UI2=%u"), m_nUDPPort);
					strUDPServerPort.Format(_T("|VT_UI2=%u"), m_nUDPServerPort);
					...
					if ((strTCPPort.CompareNoCase(strPort) == 0 && strProtocol.CompareNoCase(_T("|VT_BSTR=TCP|")) == 0)
						...
						|| (strUDPServerPort.CompareNoCase(strPort) == 0 && strProtocol.CompareNoCase(_T("|VT_BSTR=UDP|")) == 0)
...
void CUPnPImplWinServ::CreatePortMappings(ServicePointer pService)
{
	...
	LPCTSTR strFormatString = L"|VT_BSTR=|VT_UI2=%s|VT_BSTR=%s|VT_UI2=%s|VT_BSTR=%s|"
	...
	if (m_nUDPServerPort != 0){
		CString strPortUDPServer;
		strPortUDPServer.Format( L"%hu", m_nUDPServerPort );
		strInArgs.Format( strFormatString, strPortUDPServer, L"UDP", strPortUDPServer, m_sLocalIP, L"UDP" );
		hr = InvokeAction( pService, L"AddPortMapping", strInArgs, strResult );
		if ( FAILED( hr ) )
			return (void)UPnPMessage( hr );
	}
...
{
	static MAP<HRESULT, std::string> messages;

	if ( hr >= UPNP_E_ACTION_SPECIFIC_BASE && hr <= UPNP_E_ACTION_SPECIFIC_MAX )
		return _CString(_T("Action Specific Error"));
