...
bool CUrlClient::SetUrl(LPCTSTR pszUrl, uint32 nIP)
{
	...
	m_structUserLocation = theApp.ip2country->GetLocationFromIP(m_nConnectIP); //MORPH Added by SiRoB, IP to Country URLClient// X: [IP2L] - [IP2Location]
...
bool CUrlClient::SendHttpBlockRequests()
{
	...
	((CHttpClientDownSocket*)socket)->SetHttpState(HttpStateRecvExpected);
	return true;
}
...
bool CUrlClient::ProcessHttpDownResponse(const CStringAArray& astrHeaders)
{
	...
	if (sscanf_s(rstrHdr, "HTTP/%u.%u %u", &uHttpMajVer, &uHttpMinVer, &uHttpStatusCode) != 3){
		CString strError;
	...
	for (size_t i = 1; i < astrHeaders.GetCount(); i++)
	{
		...
		else if (bExpectData && _strnicmp(rstrHdr, "Content-Range:", 14) == 0)
		{
			...
			if (sscanf_s((LPCSTR)rstrHdr + 14," bytes %I64u - %I64u / %I64u", &ui64Start, &ui64End, &ui64Len) != 3){
				CString strError;
				strError.Format(_T("Unexpected HTTP header field \"%hs\""), rstrHdr);
				throw strError;
			}
	...
	for (POSITION pos = m_PendingBlocks_list.GetHeadPosition(); pos != NULL; )
	{
		...
		if (cur_block->block->StartOffset <= nStartPos && nStartPos <= cur_block->block->EndOffset)
		{
			...
			if (thePrefs.GetDebugClientTCPLevel() > 0){
				...
				//void* p = m_pPCDownSocket ? (void*)m_pPCDownSocket : (void*)socket;// X: [RPC] - [Remove PeerCache]
				Debug(_T("%08x  Start=%I64u  End=%I64u  Size=%u  Left=%I64u  %s\n"), socket, nStartPos, nEndPos, uSize, cur_block->block->EndOffset - (nStartPos + uSize) + 1, DbgGetFileInfo(reqfile->GetFileHash()));
			}
#endif
...
void CUrlClient::SendCancelTransfer()
{
	if (socket)
	{
		...
		((CHttpClientDownSocket*)socket)->SetHttpState(HttpStateUnknown);
		socket->Safe_Delete();
	}
}
