...
bool UploadBandwidthThrottler::ReplaceSocket(ThrottledFileSocket* normalsocket, ThrottledFileSocket* pcsocket, ThrottledFileSocket* newsocket)
{
	...
	if(pcsocket!=NULL)
	{
		for(slotnumber=0;slotnumber<m_StandardOrder_list.GetCount();slotnumber++)
	...
	if(slotnumber>=0 && slotnumber<m_StandardOrder_list.GetCount())
	{
...
uint32 UploadBandwidthThrottler::GetHighestNumberOfFullyActivatedSlotsSinceLastCallAndReset() {
    ...
    //if(m_highestNumberOfFullyActivatedSlots > (uint32)m_StandardOrder_list.GetCount()) {
    //    theApp.QueueDebugLogLine(true, _T("UploadBandwidthThrottler: Throttler wants new slot when get-method called. m_highestNumberOfFullyActivatedSlots: %i m_StandardOrder_list.GetCount(): %i tick: %i"), m_highestNumberOfFullyActivatedSlots, m_StandardOrder_list.GetCount(), ::GetTickCount());
    //}
...
void UploadBandwidthThrottler::AddToStandardList(uint32 index, ThrottledFileSocket* socket) {
	if(socket != NULL) {
		...
		if(index > (uint32)m_StandardOrder_list.GetCount()) {
			index = m_StandardOrder_list.GetCount();
        }
