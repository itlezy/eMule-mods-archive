...
public:
    ...
    virtual ~UploadBandwidthThrottler(void);
    ...
    size_t GetStandardListSize() { return m_StandardOrder_list.GetCount(); };

	//void ReplaceSocket(ThrottledFileSocket* oldsocket, ThrottledFileSocket* newsocket); //Xman Xtreme Upload: Peercache-part
	//bool ReplaceSocket(ThrottledFileSocket* normalsocket, ThrottledFileSocket* pcsocket, ThrottledFileSocket* newsocket); //Xman Xtreme Upload: Peercache-part // X: [RPC] - [Remove PeerCache]
...
private:
    ...
    CAtlList<ThrottledControlSocket*> m_ControlQueue_list; // a queue for all the sockets that want to have Send() called on them. // ZZ:UploadBandWithThrottler (UDP)
    CAtlList<ThrottledControlSocket*> m_ControlQueueFirst_list; // a queue for all the sockets that want to have Send() called on them. // ZZ:UploadBandWithThrottler (UDP)
    CAtlList<ThrottledControlSocket*> m_TempControlQueue_list; // sockets that wants to enter m_ControlQueue_list // ZZ:UploadBandWithThrottler (UDP)
    CAtlList<ThrottledControlSocket*> m_TempControlQueueFirst_list; // sockets that wants to enter m_ControlQueue_list and has been able to send before // ZZ:UploadBandWithThrottler (UDP)

    CAtlArray<ThrottledFileSocket*> m_StandardOrder_list; // sockets that have upload slots. Ordered so the most prioritized socket is first

	CAtlList<ThrottledFileSocket*> m_StandardOrder_list_full; //Xman Xtreme Upload

    Poco::FastMutex sendLocker;
    Poco::FastMutex tempQueueLocker;

	//Poco::Event pauseEvent;
	...
	typedef CAtlList<ratio_struct> HealthHistory;
