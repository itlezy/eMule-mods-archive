...
#define	DrawClientPercentText	dc.DrawText(szItem, -1, &cur_rec, ((MLC_DT_TEXT | DT_RIGHT) & ~DT_LEFT) | DT_CENTER)
						...
						DrawClientPercentText;++cur_rec.left;++cur_rec.right;--cur_rec.top;--cur_rec.bottom;
						DrawClientPercentText;++cur_rec.left;++cur_rec.right;++cur_rec.top;++cur_rec.bottom;
						DrawClientPercentText;--cur_rec.left;--cur_rec.right;++cur_rec.top;++cur_rec.bottom;
						DrawClientPercentText;--cur_rec.top;--cur_rec.bottom;
						dc.SetTextColor(RGB(255,255,255));
				...
				default:
					...
					if(szItem[0] != 0)
						dc.DrawText(szItem, -1, &cur_rec, (iColumn== 2 || iColumn == 3) ? MLC_DT_TEXT | DT_RIGHT : MLC_DT_TEXT);
					break;
			}
		}
					...
					{	
                       CString buffer;
						...
						if (client->GetDownloadDatarate() > 0){
							buffer.Format(_T("%.1f KB/s"),(float)client->GetDownloadDatarate()/1024);
						}
						else if (qr)
...
void CUploadListCtrl::RefreshClient(const CUpDownClient *client)
{
	...
	if (theApp.emuledlg->activewnd != theApp.emuledlg->transferwnd || !/*theApp.emuledlg->transferwnd->GetUploadList()->*/IsWindowVisible())
		return;
