...
protected:
	...
	static UINT g_ButtonResId[LAST_BUTTON];
...
UINT CXDialogTemplate::g_ButtonResId[] =		
{
	IDS_ABORT,
	IDS_CANCEL,
	IDS_CONTINUE,
...
#ifdef XMESSAGEBOX_AUTO_VISTA_STYLE
	//if (IsVistaOrLater())
	if (thePrefs.GetWindowsVersion() >= 6)
	...
	m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)				//+++1.8
{
	...
	if (pXMB->nIdCustomButtons)
	{
		...
		if (::LoadString(hInstanceStrings,
						 ...
						 m_szCustomButtons,
						 _countof(m_szCustomButtons)-1) == 0)
	...
	{
		// load from string
		_tcsncpy(m_szCustomButtons, pXMB->szCustomButtons, _countof(m_szCustomButtons)-1);
	}
	m_szCustomButtons[_countof(m_szCustomButtons)-1] = _T('\0');
	...
	else
	{
		...
		_tcsncpy(szCustomButtons, m_szCustomButtons, _countof(szCustomButtons)-1);
	...
	for (int index = 0; index < LAST_BUTTON; index++)
	{
		...
		//{
			...
			//if (m_szButtonText[index][0] == _T('\0'))
				_tcscpy(m_szButtonText[index], GetResString(g_ButtonResId[index]));
		//}
		/*else
		{
			_tcscpy(m_szButtonText[index], GetResString(g_ButtonResId[index]));
...
void CXDialogTemplate::LoadButtonStringsFromResources(HINSTANCE hInstance)
{
	...
	for (int index = 0; index < LAST_BUTTON; index++)
	{
		...
		if (index == eReport)
		{
			...
			if (m_szButtonText[index][0] == _T('\0'))
			{
				if (g_ButtonText[index].resId != 0)
				{
					CString str = GetResString(g_ButtonText[index].resId);
					if (!str.IsEmpty())
					{
						_tcsncpy(m_szButtonText[index], str, MaxButtonStringSize);
						rc = 1;
					}
				}
			}
			...
			for (nID = 1; nID <= ID_XMESSAGEBOX_LAST_ID; nID++)
			{
				...
				if (hwndChild && ::IsWindow(hwndChild))
				{
					...
					if (Me->m_nDisabledSeconds)
					{
						...
						::GetClassName(hwndChild, szClassName, _countof(szClassName)-2);
		...
		case WM_COMMAND:
		{
			...
			switch (wParam)
			{
				...
				case IdExHelp:
				{
					...
					::GetModuleFileName(NULL, szBuf, _countof(szBuf) - 1);
					if (_tcslen(szBuf) > 0)
					{
				...
				default:
				{
						...
						(Me->m_lpszModule[0] != _T('\0')))
					{
						...
						szPathName[0] = _T('\0');
						::GetModuleFileName(NULL, szPathName, _countof(szPathName)-1);
						...
						::GetPrivateProfileString(_T("DoNotAsk"),	// section name
												  ...
												  _countof(szBuf)-1,	// size of destination buffer
		...
		if (::IsWindow(hWnd))
		{
			...
			while (hwndChild)
			{
				if (::IsWindow(hwndChild))
				{
					...
					if (_tcsicmp(szClassName, _T("Button")) == 0)
					{
						::GetWindowText(hwndChild, szButton, _countof(szButton)-1);	// button text
			...
			_tcscat(pszText, _T("\r\n"));
			_tcscat(pszText, pszDivider);
			theApp.CopyTextToClipboard(pszText);
...
int CXDialogTemplate::Display()
{
	...
	TCHAR szTitle[1024];
	_tcsncpy(szTitle, m_lpszCaption, _countof(szTitle)-1);
	szTitle[_countof(szTitle)-1] = _T('\0');
							 ...
							 int nLine)
{
	...
	if (lpszCompanyName && lpszModule)
	{
		// key is module name and line
		TCHAR szKey[MAX_PATH*2];
		_tcsncpy(szKey, lpszModule, _countof(szKey)-30);
		szKey[_countof(szKey)-30] = 0;
		TRACE(_T("szKey=<%s>\n"), szKey);
