...
CPacketTracking::CPacketTracking(){
	dwLastTrackInCleanup = GetTickCount() + MIN2MS(12); // relax on startup
}
...
void CPacketTracking::AddTrackedOutPacket(uint32 dwIP, uint8 byOpcode){
	...
	while (!listTrackedRequests.IsEmpty()){
		if (sTrack.dwInserted - listTrackedRequests.GetTail().dwInserted > SEC2MS(180))
...
bool CPacketTracking::IsOnOutTrackList(uint32 dwIP, uint8 byOpcode, bool bDontRemove){
	...
	for (POSITION pos = listTrackedRequests.GetHeadPosition(); pos != NULL; listTrackedRequests.GetNext(pos)){
		if (listTrackedRequests.GetAt(pos).dwIP == dwIP && listTrackedRequests.GetAt(pos).byOpcode == byOpcode && cur_tick - listTrackedRequests.GetAt(pos).dwInserted < SEC2MS(180)){
			if (!bDontRemove)
...
bool CPacketTracking::InTrackListIsAllowedPacket(uint32 uIP, uint8 byOpcode, bool /*bValidSenderkey*/){
...
#ifdef REPLACE_ATLMAP
	TrackPacketsIn_Struct* &pTrackEntry = m_mapTrackPacketsIn[uIP];
	if(pTrackEntry == NULL){
#else
	...
	for (size_t i = 0; i < pTrackEntry->m_aTrackedRequests.GetCount(); i++){
		if (pTrackEntry->m_aTrackedRequests[i].m_byOpcode == byOpcode){
			...
			uint32 tempLastExpire = rCurTrackedRequest.m_dwFirstAdded + SEC2MS(iSecondsPerPacket) * rCurTrackedRequest.m_nCount;// X: [CI] - [Code Improvement]
			if(pTrackEntry->m_dwLastExpire<tempLastExpire)
				pTrackEntry->m_dwLastExpire = tempLastExpire;
...
void CPacketTracking::InTrackListCleanup(){
	const uint32 dwCurrentTick = ::GetTickCount();
	const int dbgOldSize = (int)m_liTrackPacketsIn.GetCount();
	dwLastTrackInCleanup = dwCurrentTick;
	for (POSITION pos1 = m_liTrackPacketsIn.GetHeadPosition(), pos2;( pos2 = pos1 ) != NULL;) {// X: [CI] - [Code Improvement]
		TrackPacketsIn_Struct* curEntry = m_liTrackPacketsIn.GetNext(pos1);
		if (curEntry->m_dwLastExpire < dwCurrentTick){
#ifdef REPLACE_ATLMAP
			ASSERT(m_mapTrackPacketsIn.find(curEntry->m_uIP) != m_mapTrackPacketsIn.end());
			m_mapTrackPacketsIn.erase(curEntry->m_uIP);
#else
	...
	DebugLog(_T("Cleaned up Kad Incoming Requests Tracklist, entries before: %i, after %u"), dbgOldSize, m_liTrackPacketsIn.GetCount());
}
...
void CPacketTracking::AddLegacyChallenge(CUInt128 uContactID, CUInt128 uChallengeID, uint32 uIP, uint8 byOpcode){
	...
	while (!listChallengeRequests.IsEmpty()){
		if (sTrack.dwInserted - listChallengeRequests.GetTail().dwInserted > SEC2MS(180)){
...
bool CPacketTracking::IsLegacyChallenge(CUInt128 uChallengeID, uint32 uIP, uint8 byOpcode, CUInt128& ruContactID){
	...
	for (POSITION pos = listChallengeRequests.GetHeadPosition(); pos != NULL; listChallengeRequests.GetNext(pos)){
		if (listChallengeRequests.GetAt(pos).uIP == uIP && listChallengeRequests.GetAt(pos).byOpcode == byOpcode 
			&& cur_tick - listChallengeRequests.GetAt(pos).dwInserted < SEC2MS(180))
		{
...
bool CPacketTracking::HasActiveLegacyChallenge(uint32 uIP) const
{
	const DWORD cur_tick = GetTickCount();
	for (POSITION pos = listChallengeRequests.GetHeadPosition(); pos != NULL; listChallengeRequests.GetNext(pos)){
		if (listChallengeRequests.GetAt(pos).uIP == uIP && cur_tick - listChallengeRequests.GetAt(pos).dwInserted < SEC2MS(180))
			return true;
	}
	return false;
}