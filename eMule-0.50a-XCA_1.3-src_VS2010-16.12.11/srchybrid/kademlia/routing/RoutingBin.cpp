...
unordered_map<uint32, uint32> CRoutingBin::s_mapGlobalContactIPs;
unordered_map<uint32, uint32> CRoutingBin::s_mapGlobalContactSubnets;
#else
CAtlMap<uint32, uint32> CRoutingBin::s_mapGlobalContactIPs;
CAtlMap<uint32, uint32> CRoutingBin::s_mapGlobalContactSubnets;
#endif
...
size_t CRoutingBin::GetSize() const
{
...
size_t CRoutingBin::GetRemaining() const
{
	return (size_t)K - m_listEntries.size();
}
...
#ifdef REPLACE_ATLMAP
	if (bIncrease){
		if (++s_mapGlobalContactIPs[uIP] > MAX_CONTACTS_IP){
			ASSERT( false );
			DebugLogError(_T("RoutingBin Global IP Tracking inconsitency on increase (%s)"), ipstr(ntohl(uIP)));
		}
	}
	else{
		unordered_map<uint32, uint32>::iterator it = s_mapGlobalContactIPs.find(uIP);
		if(it != s_mapGlobalContactIPs.end()){
			if(--it->second == 0)
				s_mapGlobalContactIPs.erase(it);
		}
		else{
			ASSERT( false );
...
#ifdef REPLACE_ATLMAP
	if (bIncrease){
		if (++s_mapGlobalContactSubnets[uIP & 0xFFFFFF00] > MAX_CONTACTS_SUBNET && !::IsLANIP(ntohl(uIP))){
			ASSERT( false );
			DebugLogError(_T("RoutingBin Global Subnet Tracking inconsitency on increase (%s)"), ipstr(ntohl(uIP)));
		}
	}
	else{
		unordered_map<uint32, uint32>::iterator it = s_mapGlobalContactSubnets.find(uIP & 0xFFFFFF00);
		if(it != s_mapGlobalContactSubnets.end()){
			if(--it->second == 0)
				s_mapGlobalContactSubnets.erase(it);
		}
		else{
			ASSERT( false );
...
#ifdef REPLACE_ATLMAP
	unordered_map<uint32, uint32>::const_iterator it = s_mapGlobalContactIPs.find(uNewIP);
	if(it != s_mapGlobalContactIPs.end() && it->second >= MAX_CONTACTS_IP){
#else
	uint32 nSameIPCount = 0;
...
#ifdef REPLACE_ATLMAP
		it = s_mapGlobalContactSubnets.find(uNewIP & 0xFFFFFF00);
		if(it != s_mapGlobalContactSubnets.end() && it->second >= MAX_CONTACTS_SUBNET && !::IsLANIP(ntohl(uNewIP))){
#else
...
CContact* CRoutingBin::GetRandomContact(uint32 nMaxType, uint32 nMinKadVersion)
{
	...
	uint32 nRandomStartPos = t_rng->getUInt16() % m_listEntries.size();
	uint32 nIndex = 0;
	for (ContactList::const_iterator itContactList = m_listEntries.begin(); itContactList != m_listEntries.end(); ++itContactList)
	{
		CContact* pContact = *itContactList;
...
bool CRoutingBin::CheckGlobalIPLimits(uint32 uIP, uint16 uPort, bool bLog)
{
	// no more than 1 KadID per IP
#ifdef REPLACE_ATLMAP
	unordered_map<uint32, uint32>::const_iterator it = s_mapGlobalContactIPs.find(uIP);
	if(it != s_mapGlobalContactIPs.end() && it->second >= MAX_CONTACTS_IP){
#else
	uint32 nSameIPCount = 0;
	s_mapGlobalContactIPs.Lookup(uIP, nSameIPCount);
	...
	//  no more than 10 IPs from the same /24 netmask global, except if its a LANIP (if we don't accept LANIPs they already have been filtered before)
#ifdef REPLACE_ATLMAP
	it = s_mapGlobalContactSubnets.find(uIP & 0xFFFFFF00);
	if(it != s_mapGlobalContactSubnets.end() && it->second >= MAX_CONTACTS_SUBNET && !::IsLANIP(ntohl(uIP))){
#else
	uint32 nSameSubnetGlobalCount = 0;
