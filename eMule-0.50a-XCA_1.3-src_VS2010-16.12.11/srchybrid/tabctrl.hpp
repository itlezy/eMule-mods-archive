...
public:
	...
	size_t GetLastMovementSource() const			{ return m_nSrcTab; }
	size_t GetLastMovementDestionation() const	{ return m_nDstTab; }

	BOOL ReorderTab(size_t nSrcTab, size_t nDstTab);
	void SetTabTextColor(int i, DWORD color);

protected:
	size_t  m_nSrcTab;       // Specifies the source tab that is going to be moved.
	size_t  m_nDstTab;       // Specifies the destination tab (drop position).
