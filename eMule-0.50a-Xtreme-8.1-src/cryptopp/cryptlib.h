...
// X :VC's Run-Time Type Info May Cause Memory Leak,So replace name() with rawname()
#define USE_RAWNAME 1
...
#else
			: InvalidArgument("NameValuePairs: type mismatch for '" + name + "', stored '" + stored.raw_name() + "', trying to retrieve '" + retrieving.raw_name() + "'")
#endif
...
#else
		return GetValue((std::string("ThisObject:")+typeid(T).raw_name()).c_str(), object);
#endif
	}
...
#else
		return GetValue((std::string("ThisPointer:")+typeid(T).raw_name()).c_str(), p);
#endif
	}
