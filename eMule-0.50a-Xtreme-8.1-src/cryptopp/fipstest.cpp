...
//	MeterFilter verifier(new FileSink("c:\\dt.tmp"));
	...
	{
#ifdef CRYPTOPP_WIN32_AVAILABLE
		OutputDebugString(_T("Crypto++ DLL integrity check failed. Cannot open file for reading."));
#endif
		return false;
	}
	...
	{
		OutputDebugString(_T("In memory integrity check failed. This may be caused by debug breakpoints or DLL relocation.\n"));
		moduleStream.clear();
...
//	OutputDebugString(((_T("Crypto++ DLL integrity check failed. Actual MAC is: ") + hexMac) + _T("\n")).c_str());
	OutputDebugString(_T("Crypto++ DLL integrity check failed. Actual MAC is: \n"));
#endif
	return false;
}
