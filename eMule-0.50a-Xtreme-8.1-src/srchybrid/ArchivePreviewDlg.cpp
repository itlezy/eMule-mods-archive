...
BOOL CArchivePreviewDlg::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		//zz_fly :: do not auto preview big archive
		/*
		UpdateArchiveDisplay(thePrefs.m_bAutomaticArcPreviewStart);
		*/
		UpdateArchiveDisplay(thePrefs.m_bAutomaticArcPreviewStart, true);
		//zz_fly :: do not auto preview big archive
		m_bDataChanged = false;
	}
	return TRUE;
...
*/
void CArchivePreviewDlg::UpdateArchiveDisplay(bool doscan, bool skipBigArchive) {
//zz_fly :: do not auto preview big archive
	...
	//zz_fly :: do not auto preview big archive
	if(skipBigArchive && (thePrefs.m_uAutoPreviewLimit != 0) && (file->GetFileSize() >= thePrefs.m_uAutoPreviewLimit))
		return;
	//zz_fly :: do not auto preview big archive
