...
void CDownloadQueue::Init(){
	...
	if(count == 0) {
		...
		//SortByPriority(); //zz_fly :: remove useless code :: Enig123
		CheckDiskspace();
	}
...
void CDownloadQueue::Process(){
	...
	//zz_fly :: avoid multi-processing
	//note: in some case, a new loop is started before the prior one ends.
	//		this case may cause crash, and we try to avoid it. thanks Enig123.
#ifdef _DEBUG
	try{
	if(isDownloadQueueProcessing){
		AddDebugLogLine(true, _T("CDownloadQueue::Process(): skip a loop because the prior one not ends."));
	}
	else{
		isDownloadQueueProcessing = true;
#endif
	//zz_fly :: end
	...
	*/
	...
	//zz_fly :: avoid multi-processing
		...
		AddDebugLogLine(true, _T("CDownloadQueue::Process(): an unknown error happened."));
	}
	isDownloadQueueProcessing = false;
#endif
	//zz_fly :: end
