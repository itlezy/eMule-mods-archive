			...
			case MPG_F2:
				if (pKnownFile && !pKnownFile->IsPartFile()){
					...
					if (!inputbox.WasCancelled() && newname.GetLength()>0)
					{
						...
						{
							//theApp.sharedfiles->RemoveKeywords(pKnownFile); //zz_fly :: http://forum.emule-project.net/index.php?showtopic=138576
							pKnownFile->SetFileName(newname);
							//theApp.sharedfiles->AddKeywords(pKnownFile); //zz_fly
						}
