...
BEGIN_MESSAGE_MAP(CSharedFilesWnd, CResizableDialog)
	...
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_DOWNHISTORYLIST, OnLvnItemchangedHlist)
...
*/
void CSharedFilesWnd::ShowSelectedFilesDetails(bool bForce, bool bHistory)
//Xman end
{
		...
		*/
		...
		while (pos != NULL){
			...
			*/
			...
			if(bHistory){
				index = historylistctrl.GetNextSelectedItem(pos);
				if(index >=0 ){
					pFile = (CShareableFile*)historylistctrl.GetItemData(index);
					
				}
			}
			else{
				index = sharedfilesctrl.GetNextSelectedItem(pos);
				if(index >=0 ){
					pFile = (CShareableFile*)sharedfilesctrl.GetItemData(index);
				}
			}
			if (pFile != NULL){
				selectedList.AddTail(pFile);
				m_bChanged |= m_dlgDetails.GetItems().GetSize() <= i || m_dlgDetails.GetItems()[i] != pFile;
				i++;
			}
			//Xman
...
{
	ShowSelectedFilesDetails(false,true);
}
