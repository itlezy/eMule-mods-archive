...
#include "Addons/Modname/Modname.h" //>>> WiZaRd::Easy ModVersion/Same ModVersion Detection
...
{
	m_abyUpPartStatusHidden = NULL; // ZZUL-TRA :: SOTN :: Start
#ifdef CLIENTANALYZER
...
// ZZUL-TRA :: ICS :: End
	m_abySeenPartStatus = NULL; // ZZUL-TRA :: AntiHideOS 
	m_bZZULTRA = false; // ZZUL-TRA :: SameModVersionDetection 
...
//	m_iHttpSendState = 0; // X: [RPC] - [Remove PeerCache]
	m_fPeerCache = 0;
	// X: [RPC] - [Remove PeerCache]
...
//ZZUL -
	m_pszFunnyNick = 0; // ZZUL-TRA :: FunnyNick
	m_nDifferenceQueueRank = 0; // ZZUL-TRA :: QRdiff
...
/*	if (m_pPCDownSocket){
	...
	DeleteFunnyNick(); //>> evl: FunnyNicks [xrmb]
...
//	m_uPeerCacheUploadPushId = 0;
	...
	m_fDirectUDPCallback = 0;
	m_fSupportsFileIdent = 0;
	m_incompletepartVer = 0; // ZZUL-TRA :: ICS :: Start
}
...
// ZZUL-TRA :: SameModVersionDetection :: Start
				m_bZZULTRA = (_tcsncmp(m_strModVersion, m_strModTag, m_strModTag.GetLength())==0);
// ZZUL-TRA :: SameModVersionDetection :: End
			...
			case CT_EMULE_UDPPORTS:
				...
				if (temptag.IsInt()) {
					...
					dwEmuleTags |= 1;
#ifdef CLIENTANALYZER
					bWasUDPPortSent = true; //>>> zz_fly::Bad Shareaza detection
#endif
				}
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
				else if (bDbgInfo)
...
#ifdef CLIENTANALYZER
					m_uiRealVersion = temptag.GetInt();
#endif
...
//<<< WiZaRd::ClientAnalyzer
	...
	UpdateFunnyNick(); // ZZUL-TRA :: FunnyNick
...
//>>> WiZaRd::Easy ModVersion
	...
	const bool bSendICS = !m_pszUsername || GetIncompletePartVersion(); // ZZUL-TRA :: ICS 
	data.WriteUInt32(7+bSend+bSendICS); // nr. of tags
//<<< WiZaRd::Easy ModVersion
...
// ZZUL-TRA :: ICS :: End
	...
	if (m_byDataCompVer == 0) {
		...
		m_byAcceptCommentVer = 0;
		m_nUDPPort = 0;
		m_incompletepartVer = 0;	// ZZUL-TRA :: ICS //hmmm why is this reset?
	}
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
...
void CUpDownClient::InitClientSoftwareVersion()
{
	...
	*/
	...
	// ZZUL-TRA :: EnhancedClientRecognization :: End
		...
		*/
		else if (m_nClientVersion >= 10100 && m_nClientVersion <= 10409){
		// ZZUL-TRA :: EnhancedClientRecognization :: End
...
// ZZUL-TRA :: AntiHideOS :: End
	...
	delete m_pReqFileAICHHash;
	m_pReqFileAICHHash = NULL;
	SetRemoteQueueRank(0); // ZZUL-TRA :: QRdiff
}
...
void CUpDownClient::AssertValid() const
{
	...
	CHECK_BOOL(m_bZZULTRA);
#ifdef CLIENTANALYZER
	CHECK_PTR(pAntiLeechData);
	(void)m_uiRealVersion;
#endif
	(void)m_abyIncPartStatus;  // ZZUL-TRA :: ICS 
	(void)m_abySeenPartStatus; // ZZUL-TRA :: AntiHideOS 
	CHECK_OBJ(socket);
	CHECK_PTR(credits);
	CHECK_PTR(m_Friend);
	CHECK_OBJ(reqfile);
...
void CUpDownClient::ProcessPublicIPAnswer(const BYTE* pbyData, UINT uSize){
	...
	if (m_fNeedOurPublicIP == 1){ // did we?
		...
		theApp.CheckIdChange(); // ZZUL-TRA :: ReAskSrcAfterIPChange
	}	
}
...
// ZZUL-TRA :: SameModVersionDetection :: Start
bool CUpDownClient::IsZZULTRA() const
{
#ifdef CLIENTANALYZER
//>>> WiZaRd::ClientAnalyzer
	//don't show ZZUL-TRA icon for modthieves/nickthieves :)
	if(pAntiLeechData && pAntiLeechData->GetBadForThisSession(AT_MODTHIEF))
		return false;
    //morph4u
    if(pAntiLeechData && pAntiLeechData->GetBadForThisSession(AT_NICKTHIEF))
		return false;
#endif
	return m_bZZULTRA;
}
...
CString	CUpDownClient::GetCountryName(const bool bLongName) const
{
	return theApp.ip2country->GetCountryNameFromRef(m_structUserCountry, bLongName);
}
...
void CUpDownClient::SetIP(const uint32 val) //Only use this when you know the real IP or when your clearing it.
{
	m_dwUserIP = val;
	m_nConnectIP = val;	
	ResetIP2Country();
...
LPCTSTR	CUpDownClient::GetUserName(bool bFN) const
{
	if (!bFN || !m_pszFunnyNick)
		return m_pszUsername;

	//>> pP: Leecher Secure
#ifdef  CLIENTANALYZER
	if (pAntiLeechData && pAntiLeechData->GetBadForThisSession(0xff))
		return m_pszUsername;
#endif
	//<< pP: Leecher Secure

	return m_pszFunnyNick;
}

void CUpDownClient::DeleteFunnyNick()
{
	//if (m_pszFunnyNick)
	{
...
void CUpDownClient::UpdateFunnyNick()
{
	...
	//<< pP: LeecherSecure
	if(m_pszUsername == NULL || 
		!IsEd2kClient() || // no FunnyNick for http DL
	...
	{
		_T("ATX-"),			//0
		_T("Gameboy "),	_T("PS/2-"),	_T("USB-"),    _T("Angry "),       _T("Atrocious "), _T("Attractive "), _T("Bad "),       _T("Barbarious "),_T("Beautiful "),	   _T("Black "),	  //10
		_T("Blond "),   _T("Blue "),	_T("Bright "), _T("Brown "),       _T("Cool "),      _T("Cruel "),      _T("Cubic "),     _T("Cute "),		_T("Dance "),		   _T("Dark "),		  //20
		_T("Dinky "),   _T("Drunk "),	_T("Dumb "),   _T("E"),            _T("Electro "),   _T("Elite "),      _T("Fast "),      _T("Flying "),	_T("Fourios "),		   _T("Frustraded "), //30
		_T("Funny "),   _T("Furious "),	_T("Giant "),  _T("Giga "),        _T("Green "),     _T("Handsome "),   _T("Hard "),      _T("Harsh "),		_T("Hiphop "),		   _T("Holy "),		  //40
		_T("Horny "),   _T("Hot "),		_T("House "),  _T("I"),			   _T("Lame "),      _T("Leaking "),    _T("Lone "),      _T("Lovely "),	_T("Lucky "),		   _T("Micro "),	  //50
		_T("Mighty "),  _T("Mini "),	_T("Nice "),   _T("Orange "),	   _T("Pretty "),    _T("Red "),        _T("Sexy "),      _T("Slow "),		_T("Smooth "),		   _T("Stinky "),	  //60
		_T("Strong "),  _T("Super "),   _T("Unholy "), _T("White "),	   _T("Wild "),      _T("X"),           _T("XBox "),      _T("Yellow "),	_T("Kentucky Fried "), _T("Mc"),		  //70
		_T("Alien "),   _T("Bavarian "),_T("Crazy "),  _T("Death "),	   _T("Drunken "),   _T("Fat "),        _T("Hazardous "), _T("Holy "),		_T("Infested "),	   _T("Insane "),	  //80
		_T("Mutated "), _T("Nasty "),   _T("Purple "), _T("Radioactive "), _T("Ugly "),      _T("Green "),																					  //86
	}; 
#define NB_PREFIX 87 
#define MAX_PREFIXSIZE 15
	...
	{
		_T("16"),																																				// 0
		_T("3"),	_T("6"),		_T("7"),		_T("Abe"),		  _T("Bee"),     _T("Bird"),	_T("Boy"),    _T("Cat"),	  _T("Cow"),  _T("Crow"),		//10
		_T("DJ"),	_T("Dad"),		_T("Deer"),		_T("Dog"),		  _T("Donkey"),  _T("Duck"),	_T("Eagle"),  _T("Elephant"), _T("Fly"),  _T("Fox"),		//20
		_T("Frog"),	_T("Girl"),		_T("Girlie"),	_T("Guinea Pig"), _T("Hasi"),    _T("Hawk"),	_T("Jackal"), _T("Lizard"),	  _T("MC"),	  _T("Men"),		//30
		_T("Mom"),	_T("Mouse"),	_T("Mule"),		_T("Pig"),		  _T("Rabbit"),  _T("Rat"),		_T("Rhino"),  _T("Smurf"),	  _T("Snail"),_T("Snake"),		//40
		_T("Star"),	_T("Tiger"),	_T("Wolf"),		_T("Butterfly"),  _T("Elk"),	 _T("Godzilla"),_T("Horse"),  _T("Penguin"),  _T("Pony"), _T("Reindeer"),	//50
		_T("Sheep"),_T("Sock Puppet"),_T("Worm"),	_T("Bermuda"),    _T("Bitch")																				//55 ;)
	};
#define NB_SUFFIX 56 
