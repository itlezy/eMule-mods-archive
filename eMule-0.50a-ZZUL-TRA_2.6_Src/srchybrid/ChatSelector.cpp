...
#define	TIME_STAMP_FORMAT		_T("[%H:%M:%S] ") //morph4u changed
...
CChatItem* CChatSelector::StartSession(CUpDownClient* client, bool show)
{
	...
	// <CB Mod : ChatLog : Date 11.07.07>
	chatitem->m_FileLog = NULL;
	CString path, cleanname;
	cleanname = client->GetUserName();
	// make sure the nickname contains valid chars for the file name
	cleanname = cleanname.SpanIncluding(_T("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz .-!$&()[]{}ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝàáâãäåæçèéêëìíîïðñòóõöøùúûüýÿ"));

	path.Format(_T("logs\\%s_%s.log"), md4str(client->GetUserHash()), cleanname);
	
	FILE* folder = _tfopen(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + _T("logs"), _T("r"));
	if (folder == NULL)
		if (_wmkdir(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + _T("logs")) != 0)
			//; // We should prompt that a problem occured when trying to create the logs folder
		
	chatitem->m_FileLog = _tfopen(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + path, _T("a+t"));
	// </CB Mod : ChatLog>
	...
	//chatitem->log->AppendKeyWord(GetResString(IDS_CHAT_START) + client->GetUserName() + _T("\n"), STATUS_MSG_COLOR); //morph4u changed
	chatitem->log->AppendKeyWord(GetResString(IDS_CHAT_START) + client->GetUserName() + _T(" ")  +   client->DbgGetFullClientSoftVer()   + _T(" ")+  client->GetCountryName() + _T("\n"), STATUS_MSG_COLOR);
...
void CChatSelector::ProcessMessage(CUpDownClient* sender, const CString& message)
{
	...
	if (ci->m_FileLog){
		...
		msg.Format(_T("[%s] %s --> %s\n"),CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), sender->GetUserName(), message);

		TUnicodeToUTF8<2048> utf8(msg, msg.GetLength());
		fwrite((LPCSTR)utf8, 1, utf8.GetLength(), ci->m_FileLog);
		fflush(ci->m_FileLog);
	}
	// </CB Mod : ChatLog>
...
bool CChatSelector::SendMessage(const CString& rstrMessage)
{
	...
	//update the remote clients nickname if necessary
	CString name;
	if (ci->client->GetUserName() != NULL)
		name = ci->client->GetUserName();
	else
		name.Format(_T("(%s)"), GetResString(IDS_UNKNOWN));
	ci->log->SetTitle(name);
#endif
	if (ci->client->socket && ci->client->socket->IsConnected())
	{
		...
		if (ci->m_FileLog){
			CString msg;
				...
				msg.Format(_T("[%s] %s --> %s\n"),CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), thePrefs.GetUserNick(), rstrMessage);
...
void CChatSelector::ConnectingResult(CUpDownClient* sender, bool success)
{
	...
	if (!success){
		...
		if (ci->m_FileLog){
			...
			msg.Format(_T("[%s] %s --> %s\n"),CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), thePrefs.GetUserNick(), ci->strMessagePending);
