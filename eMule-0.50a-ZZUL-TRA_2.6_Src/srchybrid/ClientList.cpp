...
#include "Preferences.h" // ZZUL-TRA :: DropSystem
#include "server.h" // ZZUL-TRA :: ReAskSrcAfterIPChange
...
void CClientList::Process()
{
	...
	// ZZUL-TRA :: DropSystem :: Start
	//
	if((m_dwLastCleanUpDontAskThisIP + MIN2MS(10)) < cur_tick){ // one cleanup every 10min should be enough
		m_dwLastCleanUpDontAskThisIP = cur_tick;

		POSITION pos = m_DontAskThisIPList.GetStartPosition();
		while (pos != NULL){
			CDontAskThisIPValues* pValue = NULL;
			uint32 uKey;
			m_DontAskThisIPList.GetNextAssoc(pos, uKey, pValue);
			const uint8 strType = pValue->uDropType;
			uint32 uDontAskTime = pValue->uTimestamp;

			switch(strType){
				case 1:		uDontAskTime += TIM_DROPSYS_BLOCK_NNS_DEF;	break;
				case 2:		uDontAskTime += TIM_DROPSYS_BLOCK_FQS_DEF;	break;
				case 3:		uDontAskTime += TIM_DROPSYS_BLOCK_HQS_DEF;	break;
				default:	break;
			}

			if(uDontAskTime < cur_tick){
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8 byConnectOptions)
{
	...
	const bool bNewClient = pNewClient == NULL; //>>> WiZaRd::Save CPU
	if (!pNewClient)
...
//	AddClient(pNewClient);
	if(bNewClient)
	{
		m_KadList.AddTail(pNewClient); //>>> WiZaRd - it IS new here!
	//This method checks if this is a dup already.
		AddClient(pNewClient, true); 
	}
	else
		AddToKadList(pNewClient); //WiZaRd - dupcheck fix? ^^
//<<< WiZaRd::Save CPU
	return true;
}
...
//>> evl: FunnyNicks [xrmb]
void CClientList::UpdateFunnyNicks()
{
	for (POSITION pos = list.GetHeadPosition(); pos;)
	{
		...
		if (client->IsBadGuy() || client->IsBanned())
		{
			if (client->HasFunnyNick())
				client->DeleteFunnyNick();

			continue;
		}
#endif
		//<< pP: LeecherSecure
		client->UpdateFunnyNick();
	}
}
//<< evl: FunnyNicks [xrmb]
...
bool CClientList::DontAskThisIP(uint32 dwIP)
{
	...
	if(m_DontAskThisIPList.Lookup(dwIP, pValue)){
		const uint8 uDropType = pValue->uDropType;
		uint32 uDontAskTime = pValue->uTimestamp;
		...
		if(uDontAskTime > ::GetTickCount())
			return true;
	}
	return false; 
}
void CClientList::AddToDontAskThisIP(uint32 dwIP, uint8 uDropType)
{
	CDontAskThisIPValues* pValue = new CDontAskThisIPValues;
	pValue->uTimestamp = ::GetTickCount();
	pValue->uDropType = uDropType;
	m_DontAskThisIPList.SetAt(dwIP, pValue);
}
// ZZUL-TRA :: DropSystem :: End
...
void CClientList::TrigReaskForDownload(){
	...
	const bool bLowID = theApp.IsFirewalled(); // we will not inform but do a complete reask, if we are on LowID

	for(POSITION pos = list.GetHeadPosition(); pos;){
		CUpDownClient* cur_client = list.GetNext(pos);

		const DWORD ReAskTime = FILEREASKTIME;
		...
		if(cur_client->reqfile){
			...
			*/
			...
			if ((cur_client->GetClientSoft() == SO_EMULE || cur_client->GetClientSoft() == SO_EMULEPLUS || cur_client->GetClientSoft() == SO_EDONKEY || cur_client->GetClientSoft() == SO_EDONKEYHYBRID) && !bLowID){ // inform
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
				if (thePrefs.GetDebugClientTCPLevel() > 0)
...
void CClientList::ResetIP2Country(){
	for(POSITION pos = list.GetHeadPosition(); pos != NULL;) {
		list.GetNext(pos)->ResetIP2Country();
	}
}
// ZZUL-TRA :: IP2Country :: End

// ZZUL-TRA :: CompatClientStats :: Start
// code optimized by WiZaRd
void CClientList::GetCompatClientsStats(CRBMap<CString, uint32> *compatClients)
{
	...
	for(POSITION pos = list.GetHeadPosition(); pos != NULL;){	
		...
		{
			case SO_HYDRANODE:
				strLookup = L"Hydranode";
				break;

			case SO_EMULEPLUS:
				strLookup = L"eMule Plus";
				break;

			case SO_TRUSTYFILES:
				strLookup = L"TrustyFiles";
				break;
		...
		if (!compatClients->Lookup(strLookup, uCount))
			uCount = 1;
		else
			uCount++;
		compatClients->SetAt(strLookup, uCount);
	}
}
