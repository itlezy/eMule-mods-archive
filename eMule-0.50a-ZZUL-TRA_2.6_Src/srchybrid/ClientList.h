...
// ZZUL-TRA :: DropSystem :: Start
public:
	void AddToDontAskThisIP(uint32 dwIP, uint8 uDropType);
	void RemoveDontAskThisIP(uint32 dwIP) {m_DontAskThisIPList.RemoveKey(dwIP);}
	bool DontAskThisIP(uint32 dwIP);

private:
	CMap<uint32, uint32, CDontAskThisIPValues*, CDontAskThisIPValues*> m_DontAskThisIPList;
	DWORD m_dwLastCleanUpDontAskThisIP;
// ZZUL-TRA :: DropSystem :: End

public:
	void UpdateFunnyNicks();			//>> evl: xrmb_FunnyNick
	void TrigReaskForDownload(); // ZZUL-TRA :: ReAskSrcAfterIPChange
	void ResetIP2Country(); // ZZUL-TRA :: IP2Country
	void GetCompatClientsStats(CRBMap<CString, uint32> *compatClients); // ZZUL-TRA :: CompatClientStats
};
