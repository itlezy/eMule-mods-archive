...
void CUpDownClient::DrawStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	if (!onlygreyrect && reqfile 
		&& (m_abyPartStatus 
			|| m_abyIncPartStatus	// ZZUL-TRA :: ICS 
			|| m_abySeenPartStatus	// ZZUL-TRA :: AntiHideOS 
			)
		) 
	{
		COLORREF crBoth;
...
COLORREF crClientPartial = RGB(170, 50, 224); // ZZUL-TRA :: ICS 
		COLORREF crClientSeen = RGB(150, 240, 240); // ZZUL-TRA :: AntiHideOS
		if (g_bLowColorDesktop) {
			crBoth = RGB(0, 0, 0);
...
// ZZUL-TRA :: ICS :: Start
			if (m_abyPartStatus && m_abyPartStatus[i])
			//if (m_abyPartStatus[i])
// ZZUL-TRA :: ICS :: End
			{				
				if (reqfile->IsComplete(uStart,PARTSIZE*(uint64)(i+1)-1, false))
...
#endif
		const bool checkSeenParts = IsPartialSource(); // ZZUL-TRA :: AntiHideOS 
...
// ==> Average Queue Ranking [Xman] 
	if(reqfile)
		reqfile->CalcAvgQr(nr, m_nRemoteQueueRank);
	// <== Average Queue Ranking [Xman] 

// ZZUL-TRA :: QRdiff :: Start
	if( nr == 0 || GetDownloadState() == DS_NONEEDEDPARTS ) //buggy, e.g. Shareaza
		m_nDifferenceQueueRank = 0;   
	//the first QR does NOT show a difference!
	//if we don't check here then all clients will be shown with "+..." after first reask
...
// ZZUL-TRA :: ICS :: End
// ZZUL-TRA :: AntiHideOS :: Start
bool CUpDownClient::IsPartialSource() const
{
