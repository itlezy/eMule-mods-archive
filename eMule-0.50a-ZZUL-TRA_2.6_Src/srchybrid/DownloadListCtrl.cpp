...
#include "UploadQueue.h" // ZZUL-TRA :: PSExtension
...
static const UINT colStrID[]={
	...
	,IDS_CAT
	,IDS_ADDEDON
    ,IDS_SHOW_DROPPED_SRC // show # of dropped sources 
	,IDS_AVGQR // Average Queue Ranking [Xman] 
};
...
// ZZUL-TRA :: ULPrioExtension :: Start
	if (m_PrioUpMenu)
		VERIFY( m_PrioUpMenu.DestroyMenu() );
	// ZZUL-TRA :: ULPrioExtension :: End
// ZZUL-TRA :: PSExtension :: Start
    if (m_PowershareUpMenu) 
		VERIFY( m_PowershareUpMenu.DestroyMenu() );
	if (m_PowerShareLimitUpMenu) 
		VERIFY( m_PowerShareLimitUpMenu.DestroyMenu() );
	if (m_PsAmountLimitUpMenu) 
		VERIFY( m_PsAmountLimitUpMenu.DestroyMenu() );
	// ZZUL-TRA :: PSExtension :: End
    if (m_SourcesMenu)
...
void CDownloadListCtrl::Init()
{
	...
	InsertColumn(14, GetResString(IDS_SHOW_DROPPED_SRC) ,LVCFMT_LEFT, 50); // show # of dropped sources 
	InsertColumn(15, GetResString(IDS_AVGQR) ,LVCFMT_LEFT, 50); // Average Queue Ranking [Xman] 
...
void CDownloadListCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("KAD_RATING_SEARCH"))); // rating for comments are searched on kad
// ZZUL-TRA :: QRdiff :: Start
	m_ImageList.Add(CTempIconLoader(_T("QR_EQUAL")));//15
	m_ImageList.Add(CTempIconLoader(_T("QR_UP")));//16
	m_ImageList.Add(CTempIconLoader(_T("QR_DOWN")));//17
// ZZUL-TRA :: QRdiff :: End
// ZZUL-TRA :: FileStatusIcons :: Start
	...
	// ZZUL-TRA :: FileStatusIcons :: End
	m_ImageList.Add(CTempIconLoader(_T("ZZULTRA"))); //23 ZZUL-TRA :: SameModVersionDetection 
#ifdef CLIENTANALYZER
	m_ImageList.Add(CTempIconLoader(_T("BADGUY"))); //24
#endif
    ...
    m_ImageList.SetOverlayImage(m_ImageList.Add(CTempIconLoader(_T("STATUS_PREVIEW"))), 4); //preview icon
	m_ImageList.SetOverlayImage(m_ImageList.Add(CTempIconLoader(_T("SHAREVISIBLE"))), 5); //morph4u share visible
...
void CDownloadListCtrl::GetFileItemDisplayText(CPartFile *lpPartFile, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 2:		// transferred
			...
			// ZZUL-TRA :: SessionDownload :: Start
...
//morph4u min. queuerank in downloadlistctrl +
		case 4:{	// speed
			...
			else if (thePrefs.IsExtControlsEnabled())
			{
				CString buffer;
	...
	switch (nColumn)
	{
		case 0: {	// file name
			...
			if (thePrefs.IsFileStatusIcons()){
				...
				switch (pPartFile->GetPartFileStatus()){
					case PS_DOWNLOADING:
						iIcon = 18;
						break;
                ...
                if (pPartFile->IsReadyForPreview() && pPartFile->IsMovie()) //preview icon
			...
			if (thePrefs.ShowRatingIndicator() && (pPartFile->HasComment() || pPartFile->HasRating() || pPartFile->IsKadCommentSearchRunning())){
				m_ImageList.Draw(dc, pPartFile->UserRating(true) + 8, CPoint(rcDraw.left + 2, rcDraw.top + iIconPosY), ILD_NORMAL);//14-8 removed icons
...
// ZZUL-TRA :: PercentBar :: Start
				if (thePrefs.UsePercentBar())
				    pPartFile->DrawProgressBar(&cdcStatus,  &rec_status, thePrefs.UseFlatBar());
				else
// ZZUL-TRA :: PercentBar :: End
			...
			{
				COLORREF oldclr = thePrefs.UsePercentBar() ? dc->SetTextColor(RGB(0, 0, 0)): dc->SetTextColor(RGB(255, 255, 255)); // ZZUL-TRA :: PercentBar
				...
				dc->DrawText(szItem, -1, &rcDraw, (MLC_DT_TEXT & ~DT_LEFT) | DT_CENTER);					
	                        */
					if(!thePrefs.GetUseSessionDownload())
					_sntprintf(szItem, _countof(szItem), L"%.1f%%", pPartFile->GetPercentCompleted());
					else
					_sntprintf(szItem, _countof(szItem), L"%.1f%% / %.1f%%", pPartFile->GetPercentCompleted(), (pPartFile->GetPercentCompleted() - pPartFile->GetPercentCompletedInitial()));
				szItem[_countof(szItem) - 1] = _T('\0');
                                dc->DrawText(szItem, -1, &rcDraw, (MLC_DT_TEXT & ~DT_LEFT) | DT_CENTER);
					// ZZUL-TRA :: SessionDownload :: End
				dc->SetBkMode(iOMode);
...
void CDownloadListCtrl::GetSourceItemDisplayText(const CtrlItem_Struct *pCtrlItem, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		// ZZUL-TRA :: TotalUpDown :: Start
		{
			if (pClient->Credits())
			{
				const uint64 uUploadedTotal = pClient->Credits()->GetUploadedTotal();
				const uint64 uDownloadedTotal = pClient->Credits()->GetDownloadedTotal();
		...
		case 6:		// sources
			_tcsncpy(pszText, pClient->DbgGetFullClientSoftVer(), cchTextMax); //>>> WiZaRd::Show ModVer
			break;
		...
		case 7:		// prio
			...
			{
						const int iQRdiff = pClient->GetDifference();
						if(iQRdiff)
						{
							if (iQRdiff > 0)
								_sntprintf(pszText, cchTextMax, L"QR: %u (+%d)", pClient->GetRemoteQueueRank(), iQRdiff);
							else
								_sntprintf(pszText, cchTextMax, L"QR: %u (%d)", pClient->GetRemoteQueueRank(), iQRdiff);
						}
				else
...
// ZZUL-TRA :: SameModVersionDetection :: End
			...
			else if (pClient->GetClientSoft() == SO_EMULE)
			...
			//MORPH END   - Added by IceCream, [sivka: -A4AF counter, ahead of user nickname-]
                     ...
                     // ZZUL-TRA :: Colors :: Start
                        ...
                        if (pClient->IsFriend() && pClient->GetFriendSlot())
			             	dc->SetBkColor(RGB(210,240,255)); //friendslot blue
						else if(pClient->IsFriend())
							dc->SetBkColor(RGB(200,250,200)); //friend green
						_tcsncpy(szItem, pClient->GetUserName(), _countof(szItem) - 1);
						szItem[_countof(szItem) - 1] = _T('\0');
		...
		case 7: {		// prio
			if (pClient->GetDownloadState() == DS_ONQUEUE)
			{
				...
				else if (pClient->GetRemoteQueueRank())
				{
					...
					POINT point = {rcRect.left, rcRect.top+1};
					const int iQRdiff = pClient->GetDifference();
					if(iQRdiff)
					{
						if (iQRdiff > 0)
						{
							...
							m_ImageList.Draw(dc, 17, point, ILD_NORMAL);
						}
						else
						{
							dc->SetTextColor(RGB(0, 200, 0));
							m_ImageList.Draw(dc, 16, point, ILD_NORMAL);
						}
					}
					else
						m_ImageList.Draw(dc, 15, point, ILD_NORMAL);

					rcRect.left += 10;
					dc->DrawText(szItem, -1, &rcRect, MLC_DT_TEXT | uDrawTextAlignment);
					if(iQRdiff)
...
// ZZUL-TRA :: ColoredClientstate :: Start
		case 8:	{	// status
		...
		// ZZUL-TRA :: ColoredClientstate :: End
...
void CDownloadListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	*/
	...
	hdi.mask = HDI_TEXT;
	CString strRes;
		strRes.Format(_T("%s"), GetResString(IDS_DL_SOURCES));
	hdi.pszText = const_cast<LPTSTR>((LPCTSTR)strRes);
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
	        ...
	        UINT uPrioUpMenuItem = 0; // ZZUL-TRA :: ULPrioExtension
            // ZZUL-TRA :: PSExtension :: Start
			int iPowerShareLimitUp = -1; 
			int iPsAmountLimitUp = -1; 
			UINT uPowershareUpMenuItem = 0; 
			UINT uPowerShareLimitUpMenuItem = 0; 
			UINT uPsAmountLimitUpMenuItem = 0; 
		    // ZZUL-TRA :: PSExtension :: End
			...
			while (pos)
			{
				...
				// ZZUL-TRA :: PSExtension :: Start
				UINT uCurPowershareUpMenuItem = 0;
				if (pFile->GetPowerSharedMode()==-1)
					uCurPowershareUpMenuItem = MP_POWERSHAREUP_DEFAULT;
				else
					uCurPowershareUpMenuItem = MP_POWERSHAREUP_DEFAULT+1 + pFile->GetPowerSharedMode();
				
				if (bFirstItem)
					uPowershareUpMenuItem = uCurPowershareUpMenuItem;
				else if (uPowershareUpMenuItem != uCurPowershareUpMenuItem)
					uPowershareUpMenuItem = 0;

				UINT uCurPowerShareLimitUpMenuItem = 0;
				int iCurPowerShareLimitUp = pFile->GetPowerShareLimit();
				if (iCurPowerShareLimitUp==-1)
					uCurPowerShareLimitUpMenuItem = MP_POWERSHARE_LIMIT_UP;
				else
					uCurPowerShareLimitUpMenuItem = MP_POWERSHARE_LIMIT_SET_UP;

				if (bFirstItem)
				{
					uPowerShareLimitUpMenuItem = uCurPowerShareLimitUpMenuItem;
					iPowerShareLimitUp = iCurPowerShareLimitUp;
				}
				else if (uPowerShareLimitUpMenuItem != uCurPowerShareLimitUpMenuItem || iPowerShareLimitUp != iCurPowerShareLimitUp)
				{
					uPowerShareLimitUpMenuItem = 0;
					iPowerShareLimitUp = -1;
				}

				UINT uCurPsAmountLimitUpMenuItem = 0;
				int iCurPsAmountLimitUp = pFile->GetPsAmountLimit();
				if (iCurPsAmountLimitUp==-1)
					uCurPsAmountLimitUpMenuItem = MP_PS_AMOUNT_LIMIT_UP;
				else
					uCurPsAmountLimitUpMenuItem = MP_PS_AMOUNT_LIMIT_SET_UP;

				if (bFirstItem)
				{
					uPsAmountLimitUpMenuItem = uCurPsAmountLimitUpMenuItem;
					iPsAmountLimitUp = iCurPsAmountLimitUp;
				}
				else if (uPsAmountLimitUpMenuItem != uCurPsAmountLimitUpMenuItem || iPsAmountLimitUp != iCurPsAmountLimitUp)
				{
					uPsAmountLimitUpMenuItem = 0;
					iPsAmountLimitUp = -1;
				}
				// ZZUL-TRA :: PSExtension :: End
            ...
            // ZZUL-TRA :: PSExtension :: Start
			m_FileMenu.EnableMenuItem((UINT_PTR)m_PowershareUpMenu.m_hMenu, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
	        CString buffer;
			...
			case 1:
				...
				break;
			case 2:
				buffer.Format(_T(" (%s)"),GetResString(IDS_PRIOAUTO));
				break;
			case 3:
			...
			m_PowershareUpMenu.ModifyMenu(MP_POWERSHAREUP_DEFAULT, MF_STRING,MP_POWERSHAREUP_DEFAULT, GetResString(IDS_DEFAULT) + buffer);
			m_PowershareUpMenu.CheckMenuRadioItem(MP_POWERSHAREUP_DEFAULT, MP_POWERSHAREUP_LIMITED, uPowershareUpMenuItem, 0);
 
			m_PowershareUpMenu.EnableMenuItem((UINT_PTR)m_PowerShareLimitUpMenu.m_hMenu, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
			if (thePrefs.GetPowerShareLimit()==0)
				buffer.Format(_T(" (%s)"),GetResString(IDS_DISABLED));
			else
				buffer.Format(_T(" (%u)"),thePrefs.GetPowerShareLimit());

			m_PowerShareLimitUpMenu.ModifyMenu(MP_POWERSHARE_LIMIT_UP, MF_STRING,MP_POWERSHARE_LIMIT_UP, GetResString(IDS_DEFAULT) + buffer);

			if (iPowerShareLimitUp==-1)
				buffer = GetResString(IDS_EDIT);
			else if (iPowerShareLimitUp==0)
				buffer = GetResString(IDS_DISABLED);
			else
				buffer.Format(_T("%i"),iPowerShareLimitUp);

			m_PowerShareLimitUpMenu.ModifyMenu(MP_POWERSHARE_LIMIT_SET_UP, MF_STRING,MP_POWERSHARE_LIMIT_SET_UP, buffer);
			m_PowerShareLimitUpMenu.CheckMenuRadioItem(MP_POWERSHARE_LIMIT_UP, MP_POWERSHARE_LIMIT_SET_UP, uPowerShareLimitUpMenuItem, 0);
			m_PowershareUpMenu.EnableMenuItem((UINT_PTR)m_PsAmountLimitUpMenu.m_hMenu, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
			if (iPsAmountLimitUp==0)
				buffer.Format(_T(" (%s)"),GetResString(IDS_DISABLED));
			else
				buffer.Format(_T(" (%i%%)"),thePrefs.GetPsAmountLimit());		

			m_PsAmountLimitUpMenu.ModifyMenu(MP_PS_AMOUNT_LIMIT_UP, MF_STRING,MP_PS_AMOUNT_LIMIT_UP, GetResString(IDS_DEFAULT) + buffer);

			if (iPsAmountLimitUp==-1)
				buffer = GetResString(IDS_EDIT);
			else if (iPsAmountLimitUp==0)
				buffer = GetResString(IDS_DISABLED);
			else
				buffer.Format(_T("%i%%"),iPsAmountLimitUp);

			m_PsAmountLimitUpMenu.ModifyMenu(MP_PS_AMOUNT_LIMIT_SET_UP, MF_STRING,MP_PS_AMOUNT_LIMIT_SET_UP, buffer);

			m_PsAmountLimitUpMenu.CheckMenuRadioItem(MP_PS_AMOUNT_LIMIT_UP, MP_PS_AMOUNT_LIMIT_SET_UP, uPsAmountLimitUpMenuItem, 0);
			// ZZUL-TRA :: PSExtension :: End
			...
			m_FileMenu.EnableMenuItem(MP_IMPORT, (iSelectedItems == 1 && iFilesNotDone == 1) ? MF_ENABLED : MF_GRAYED); // ZZUL-TRA :: PartImportExport
...
// <-- ZZ:DownloadManager
		...
		m_FileMenu.EnableMenuItem((UINT_PTR)m_PrioUpMenu.m_hMenu, MF_GRAYED); // ZZUL-TRA :: ULPrioExtension
	    m_FileMenu.EnableMenuItem((UINT_PTR)m_PowershareUpMenu.m_hMenu, MF_GRAYED); // ZZUL-TRA :: PSExtension
		...
		m_FileMenu.EnableMenuItem(MP_IMPORT, MF_GRAYED); // ZZUL-TRA :: PartImportExport
		m_FileMenu.EnableMenuItem(MP_CLEARCOMPLETED, GetCompleteDownloads(curTab,total) > 0 ? MF_ENABLED : MF_GRAYED);
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			switch (wParam)
			{
						...
						if ((removecompl && !validdelete) || (validdelete && AfxMessageBox(quest + fileList, MB_DEFBUTTON2 | MB_ICONQUESTION | MB_YESNO) == IDYES))
						{
							...
							while (!selectedList.IsEmpty())
							{
								...
								switch (selectedList.GetHead()->GetStatus())
								{
									...
									case PS_HASHING:
									case PS_COMPLETING:
									case PS_IMPORTING: // ZZUL-TRA :: PartImportExport
              ...
              // ZZUL-TRA :: ULPrioExtension :: Start
                ...
                // ZZUL-TRA :: PSExtension :: Start
				case MP_POWERSHAREUP_ON:
				case MP_POWERSHAREUP_OFF:
				case MP_POWERSHAREUP_DEFAULT:
				case MP_POWERSHAREUP_AUTO:
				case MP_POWERSHAREUP_LIMITED: 
					{
						SetRedraw(FALSE);
				...
				case MP_POWERSHAREUP_LIMITED:
					...
					break;
							...
							theApp.sharedfiles->UpdateFile((CPartFile*)file);
						}
						SetRedraw(TRUE);
				...
				case MP_POWERSHARE_LIMIT_UP:
				case MP_POWERSHARE_LIMIT_SET_UP:
					{
                        CKnownFile* pKnownFile = (CKnownFile*)file; 
						POSITION pos = selectedList.GetHeadPosition();
						int newPowerShareLimitUp = -1;
						if (wParam==MP_POWERSHARE_LIMIT_SET_UP)
						{
							InputBox inputbox;
							CString title=GetResString(IDS_POWERSHARE);
							CString currPowerShareLimitUp;
							if (pKnownFile)
								currPowerShareLimitUp.Format(_T("%i"), (pKnownFile->GetPowerShareLimit()>=0)?pKnownFile->GetPowerShareLimit():thePrefs.GetPowerShareLimit());
							else
								currPowerShareLimitUp = _T("0");
							inputbox.SetLabels(GetResString(IDS_POWERSHARE), GetResString(IDS_POWERSHARE_LIMIT), currPowerShareLimitUp);
							inputbox.SetNumber(true);
							int result = inputbox.DoModal();
							if (result == IDCANCEL || (newPowerShareLimitUp = inputbox.GetInputInt()) < 0)
								break;
						}
						SetRedraw(FALSE);
						while (pos != NULL)
						{
							CShareableFile* file = selectedList.GetNext(pos);
					if  (newPowerShareLimitUp == ((CKnownFile*)file)->GetPowerShareLimit())
								break;
					((CKnownFile*)file)->SetPowerShareLimit(newPowerShareLimitUp);
				...
				case MP_PS_AMOUNT_LIMIT_UP:
				case MP_PS_AMOUNT_LIMIT_SET_UP:
					{
                        CKnownFile* pKnownFile = (CKnownFile*)file; 
						POSITION pos = selectedList.GetHeadPosition();
						int newPsAmountLimitUp = -1;
						if (wParam==MP_PS_AMOUNT_LIMIT_SET_UP)
						{
							InputBox inputbox;
							CString title=GetResString(IDS_POWERSHARE);
							CString currPsAmountLimitUp;
							if (pKnownFile)
								currPsAmountLimitUp.Format(_T("%i"), ((pKnownFile->GetPsAmountLimit()>=0)?pKnownFile->GetPsAmountLimit():thePrefs.GetPsAmountLimit()));
							else
								currPsAmountLimitUp = _T("0");
							inputbox.SetLabels(GetResString(IDS_POWERSHARE), GetResString(IDS_PS_AMOUNT_LIMIT_LABEL), currPsAmountLimitUp);
							inputbox.SetNumber(true);
							int result = inputbox.DoModal();
							if (result == IDCANCEL || (newPsAmountLimitUp = inputbox.GetInputInt()) < 0)
								break;
							if (newPsAmountLimitUp > MAX_PS_AMOUNT_LIMIT)
							{
						...
						{
							CShareableFile* file = selectedList.GetNext(pos);
							if  (newPsAmountLimitUp == ((CKnownFile*)file)->GetPsAmountLimit())
								break;
							((CKnownFile*)file)->SetPsAmountLimit(newPsAmountLimitUp);
							if (((CKnownFile*)file)->IsPartFile())
								((CPartFile*)file)->UpdatePartsInfo();
							else
								((CKnownFile*)file)->UpdatePartsInfo();
...
void CDownloadListCtrl::CreateMenues()
{
	...
	m_PrioUpMenu.AppendMenu(MF_STRING, MP_PRIOUPAUTO, GetResString(IDS_PRIOAUTO));
      if(thePrefs.IsExtControlsEnabled()){
	   m_FileMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_PrioUpMenu.m_hMenu, GetResString(IDS_PRIORITY) + _T(" (") + GetResString(IDS_PW_CON_UPLBL) + _T(")")/*, _T("FILEPRIORITYUP")*/); // ZZUL-TRA :: ULPrioExtension
	}
	// ZZUL-TRA :: ULPrioExtension :: End

    // ZZUL-TRA :: PSExtension :: Start
    m_PowershareUpMenu.CreateMenu();
	m_PowershareUpMenu.AppendMenu(MF_STRING,MP_POWERSHAREUP_DEFAULT,GetResString(IDS_DEFAULT));
	m_PowershareUpMenu.AppendMenu(MF_STRING,MP_POWERSHAREUP_OFF,GetResString(IDS_DISABLED));
	m_PowershareUpMenu.AppendMenu(MF_STRING,MP_POWERSHAREUP_ON,GetResString(IDS_ENABLED));
	m_PowershareUpMenu.AppendMenu(MF_STRING,MP_POWERSHAREUP_AUTO,GetResString(IDS_PRIOAUTO));
	m_PowershareUpMenu.AppendMenu(MF_STRING,MP_POWERSHAREUP_LIMITED,GetResString(IDS_POWERSHARE_LIMITED)); 
	m_PowerShareLimitUpMenu.CreateMenu();
	m_PowerShareLimitUpMenu.AppendMenu(MF_STRING,MP_POWERSHARE_LIMIT_UP,	GetResString(IDS_DEFAULT));
	m_PowerShareLimitUpMenu.AppendMenu(MF_STRING,MP_POWERSHARE_LIMIT_SET_UP,	GetResString(IDS_DISABLED));
	m_PsAmountLimitUpMenu.CreateMenu();
	m_PsAmountLimitUpMenu.AppendMenu(MF_STRING,MP_PS_AMOUNT_LIMIT_UP,	GetResString(IDS_DEFAULT));
	m_PsAmountLimitUpMenu.AppendMenu(MF_STRING,MP_PS_AMOUNT_LIMIT_SET_UP,	GetResString(IDS_DISABLED));
	m_PowershareUpMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
	m_PowershareUpMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_PowerShareLimitUpMenu.m_hMenu, GetResString(IDS_POWERSHARE_LIMIT));
	m_PowershareUpMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_PsAmountLimitUpMenu.m_hMenu, GetResString(IDS_PS_AMOUNT_LIMIT));
	if(thePrefs.IsExtControlsEnabled()){
		m_FileMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_PowershareUpMenu.m_hMenu, GetResString(IDS_POWERSHARE)/*, _T("FILEPOWERSHARE")*/); // ZZUL-TRA :: PSExtension
	}
	// ZZUL-TRA :: PSExtension :: End
...
// ZZUL-TRA :: PartImportExport :: Start
	m_SourcesMenu.AppendMenu(MF_STRING, MP_IMPORT, GetResString(IDS_IMPORT_PARTS)/*, _T("PARTIMPORT")*/); 
	// ZZUL-TRA :: PartImportExport :: End
