...
CEMSocket::CEMSocket(void){
    ...
    //lastSent = ::GetTickCount()-1000;//ZZUL 

	m_bAccelerateUpload = false;

    m_actualPayloadSize = 0;
    m_actualPayloadSizeSentForThisPacket = 0;//ZZUL 
...
// ZZUL-TRA :: BlockRatio :: Start
	m_fAvgBlockRatio = 0.0f;
	m_fBlockHistorySum = 0.0f;
	m_uSendCount = 0;
	m_uBlockSendCount = 0;
	m_uSendCountOverall = 0;
	m_uBlockSendCountOverall = 0;
...
void CEMSocket::OnReceive(int nErrorCode){
	...
	static char GlobalReadBuffer[PARTSIZE+10*1024];//ZZUL +
...
void CEMSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32 actualPayloadSize, bool bForceImmediateSend){
    ...
    BOOL sendSignalNoLongerBusy = false;//ZZUL -
    ...
    if(m_bConnectionIsReadyForSend && IsEncryptionLayerReady() && !(m_bBusy && onlyAllowedToSendControlPacket)) {
             ...
             )
		{
                   ...
                   anErrorHasOccured == false)
			{
		        ...
		        uint32 result = CEncryptedStreamSocket::Send(sendbuffer+sent,min(tosend, bufferSize-1)); // deadlake PROXYSUPPORT - changed to AsyncSocketEx
...
void CEMSocket::AssertValid() const
{
    ...
    (void)m_actualPayloadSizeSentForThisPacket;//ZZUL 
...
// ZZUL-TRA :: BlockRatio :: Start
void CEMSocket::GetAndStepBlockRatio()
{
	float fNewSample = 0.0f;
	if(m_uSendCount > 0)
		fNewSample = 100.0f*m_uBlockSendCount/m_uSendCount;
	m_blockhistory.AddHead(fNewSample);
	m_fBlockHistorySum += fNewSample;

	if(m_blockhistory.GetSize() > 20){ // ~ 20 seconds
		const float& fSubstract = m_blockhistory.RemoveTail(); //susbtract the old element
		m_fBlockHistorySum -= fSubstract;
		if(m_fBlockHistorySum < 0)
			m_fBlockHistorySum = 0; //fix possible rounding error
	}
	m_uBlockSendCount = 0;
	m_uSendCount = 0;
	m_fAvgBlockRatio = m_fBlockHistorySum/m_blockhistory.GetSize();
}
// ZZUL-TRA :: BlockRatio :: End