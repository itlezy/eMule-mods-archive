...
void CKadContactHistogramCtrl::Localize()
{
	...
	//m_strYaxis = GetResString(IDS_KADCONTACTLAB);//morph4u changed
	if (m_hWnd)
		Invalidate();
}
...
void CKadContactHistogramCtrl::OnPaint()
{
	...
	CString m_strYaxis;//morph4u added
	m_strYaxis.Format(_T("%s (%i)"), GetResString(IDS_KADCONTACTLAB), contactCount);//morph4u added
