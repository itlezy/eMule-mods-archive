...
#include "MuleStatusBarCtrl.h" // ZZUL-TRA :: HashProgress
#include "Addons/Modname/Modname.h" // ZZUL-TRA :: Feedback
...
//ZZUL-TRA :: PowerShare :: End
	...
	m_uOnUploadQueue = 0; // ZZUL-TRA :: SeeOnQueue
}
...
void CKnownFile::AssertValid() const
{
	...
	m_SOTNAvailPartFrequency.AssertValid(); // ZZUL-TRA :: SOTN
	(void)m_strDirectory;
	(void)m_strFilePath;
	(void)m_iPartCount;
...
// ZZUL-TRA :: SOTN :: End
	for (UINT i = 0; i < partcount; i++)
	{
		...
		m_SOTNAvailPartFrequency[i] = 0; // ZZUL-TRA :: SOTN 
	}
...
//ZZUL-TRA :: PowerShare :: End
		...
		cur_src->GetUploadingAndUploadedPart(m_AvailPartFrequency, m_SOTNAvailPartFrequency); // ZZUL-TRA :: SOTN 
	}
...
//ZZUL-TRA :: PowerShare :: Start
	...
	UpdatePowerShareLimit(m_nCompleteSourcesCountHi==1 && m_nVirtualCompleteSourcesCount==0 && iCompleteSourcesCountInfoReceived>1,bPowerShareLimit);
//ZZUL-TRA :: PowerShare :: End
...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	m_SOTNAvailPartFrequency.SetSize(GetPartCount()); // ZZUL-TRA :: SOTN :: Start
	for (UINT i = 0; i < GetPartCount();i++)
	{
		m_AvailPartFrequency[i] = 0;
		m_SOTNAvailPartFrequency[i] = 0; // ZZUL-TRA :: SOTN :: Start
	}
	...
	AddLogLine(true,_T("Hashing done: %s"), GetFilePath()); // ZZUL-TRA :: HashProgress
...
bool CKnownFile::CreateAICHHashSetOnly()
{
	...
	for (hashcount = 0; togo >= PARTSIZE; )
	{
		...
		{
			CString strPercent;
			strPercent.Format(_T("Hashing: %d%% - %s"), (hashcount+1)*100/GetPartCount(), GetFileName());
			theApp.emuledlg->statusbar->SetText(strPercent, 0, 0);
		}
		// ZZUL-TRA :: HashProgress :: End
		...
		switch (newtag->GetNameID()){
			...
			case FT_FILESIZE:{
				...
				if (newtag->IsInt64(true))
				{
					...
					m_AvailPartFrequency.SetSize(GetPartCount());
					m_SOTNAvailPartFrequency.SetSize(GetPartCount()); // ZZUL-TRA :: SOTN 
					for (UINT i = 0; i < GetPartCount();i++)
					{
						m_AvailPartFrequency[i] = 0;
						m_SOTNAvailPartFrequency[i] = 0; // ZZUL-TRA :: SOTN 
...
void CKnownFile::UpdatePowerShareLimit(bool autopowershare, bool limitedpowershare)
{
	...
	m_bpowershared = ((temppowershared&1) || ((temppowershared == 2) && m_bPowerShareAuto)) && !((temppowershared == 3) && m_bPowerShareLimited);
...
CString CKnownFile::GetFeedBackString(const bool& bBBCodeStyle) const
{
	CString feed; 

	if(bBBCodeStyle)
	{
		feed.AppendFormat(L"[b][COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %s [%s][/COLOR]\r\n", GetResString(IDS_SF_STATISTICS),thePrefs.GetUserNick(), MOD_VERSION); 
		feed.AppendFormat(L"[COLOR=#0000CC]%s/%s:[/COLOR][COLOR=#FF0000] %s[/COLOR]\r\n", GetResString(IDS_DATE), GetResString(IDS_TIME),CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()));
    	feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %s[/COLOR]\r\n", GetResString(IDS_DL_FILENAME), GetFileName()); 
		...
		if(pfile){
			feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %.2f%% (%s)[/COLOR]\r\n", GetResString(IDS_DL_TRANSFCOMPL), pfile->GetPercentCompleted(), CastItoXBytes(pfile->GetCompletedSize()));
			feed.AppendFormat(L"[COLOR=#0000CC]%s/%s:[/COLOR][COLOR=#FF0000] %s (%i)[/COLOR]\r\n", GetResString(IDS_ST_DOWNLOAD), GetResString(IDS_CLIENTS), CastItoXBytes(pfile->GetDatarate(), false, true), pfile->GetTransferringSrcCount());
		} else
			feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] 100%%[/COLOR]\r\n", GetResString(IDS_DL_TRANSFCOMPL));
		//ZZUL-TRA :: PowerShare :: Start
		if(GetPowerShared())
		    feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %s %s[/COLOR]\r\n", GetResString(IDS_PRIORITY), GetResString(IDS_POWERSHARE_PREFIX), GetUpPriorityDisplayString());
		else
		//ZZUL-TRA :: PowerShare :: End
		...
		feed.AppendFormat(L"[COLOR=#0000CC]%s[/COLOR][COLOR=#FF0000] %u (%u)[/COLOR]\r\n", GetResString(IDS_SF_REQUESTED), statistic.GetRequests(), statistic.GetAllTimeRequests()); 
		feed.AppendFormat(L"[COLOR=#0000CC]%s[/COLOR][COLOR=#FF0000] %u (%u)[/COLOR]\r\n", GetResString(IDS_SF_ACCEPTS), statistic.GetAccepts(), statistic.GetAllTimeAccepts()); 
		feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %u (%u)[/COLOR]\r\n", GetResString(IDS_COMPLSOURCES), m_nCompleteSourcesCount, m_nVirtualCompleteSourcesCount);
		feed.AppendFormat(L"[COLOR=#0000CC]%s:[/COLOR][COLOR=#FF0000] %u[/COLOR][/b]", GetResString(IDS_ONQUEUE), GetOnUploadqueue()); // ZZUL-TRA :: SeeOnQueue
     }
     else
     {
		...
		feed.AppendFormat(L"%s %u (%u)\r\n", GetResString(IDS_SF_ACCEPTS), statistic.GetAccepts(), statistic.GetAllTimeAccepts()); 
		feed.AppendFormat(L"%s: %u (%u)\r\n", GetResString(IDS_COMPLSOURCES), m_nCompleteSourcesCount, m_nVirtualCompleteSourcesCount);
		feed.AppendFormat(L"%s: %u", GetResString(IDS_ONQUEUE), GetOnUploadqueue()); // ZZUL-TRA :: SeeOnQueue
	 }
...
uint8* CKnownFile::GetPartStatus(CUpDownClient* client) const
{
	...
	bool bSOTN = GetShareOnlyTheNeed() && (!client || client->GetIncompletePartVersion() == 0); // ZZUL-TRA :: ICS 
//>>> Create tmp array
