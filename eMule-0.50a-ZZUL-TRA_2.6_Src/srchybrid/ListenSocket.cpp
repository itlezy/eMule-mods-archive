...
//ZZUL +
        if(client->GetNumberOfRequestedBlocksInQueue() == 0 && client->GetPayloadInBuffer() == 0 && client->GetSessionPayloadUp() > 0) {
            uTimeout += MIN2MS(3);
        }
//ZZUL -
	}
				...
				case OP_REQUESTFILENAME:
				{
					...
					if (size >= 16)
					{
						...
						if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64)PARTSIZE)
						{
							...
							*/
							if (((CPartFile*)reqfile)->GetMaxSources() > ((CPartFile*)reqfile)->GetSourceCount() && ((CPartFile*)reqfile)->IsSrcReqOrAddAllowed())
...
// ZZUL-TRA :: ICS :: End
				...
				case OP_FILEREQANSNOFIL:
				{
					...
					{
#ifdef CLIENTANALYZER
						//WiZaRd: first, check whether the file is in downloadqueue, if it's not, we don't have to care
						...
						//2nd, check whether he meant his own reqfile
						//why should he send that for a different file!? 
						//might happen due to swapping, I guess...
						if(client->GetUploadState() != US_NONE && client->GetUploadFileID() && md4cmp(packet, client->GetUploadFileID()) == 0)
						{
//							if(client->m_abyUpPartStatus() == NULL)
//								break; //no info, yet, don't parse/react

							if(thePrefs.GetLogAnalyzerEvents())
								DebugLogWarning(L"Dropped src: (%s) does not seem to have own reqfile (TCP)!", DbgGetClientInfo());
							if(theApp.uploadqueue->IsDownloading(client))
								theApp.uploadqueue->RemoveFromUploadQueue(client, L"Src says he does not have the file he's downloading (TCP)", true, true);
						...
						{
							//well, if we end up here then he actually MEANT that he doesn't have the file we currently requested off him
							reqfile->m_DeadSourceList.AddDeadSource(client);
						}
						else
						{					
//#ifdef _DEBUG
							CString sfind = L"not found";
//#endif
							bool bFound = false;
...
//#ifdef _DEBUG
							theApp.QueueDebugLogLineEx(LOG_INFO, L"DBG: %s: OP_FILEREQANSNOFIL break because of different file (%s)", client->DbgGetClientInfo(), sfind);
//#endif
				...
				case OP_STARTUPLOADREQ:
				{
					...
					if (size == 16)
					{
						...
						if (reqfile)
						{
							...
							client->SetUploadFileID(reqfile);
							client->SendCommentInfo(reqfile);
					        // ZZUL-TRA :: Improvements :: Start :: WiZaRd
							// in any case, we add this client to our downloadqueue
							...
							// ZZUL-TRA :: Improvements :: End :: WiZaRd
 ...
 //morph4u to be sure
                   if ( (client->GetUploadState()==US_BANNED) 
			             && !client->IsFriend())
					{
						theApp.uploadqueue->RemoveFromUploadQueue(client,_T("Banned client detected during upload"), true, true); 
...
//ZZUL +
                   uchar tempfileid[16];
                    ...
                    if(md4cmp(reqfilehash, client->GetUploadFileID()) != 0 && md4cmp(reqfilehash, tempfileid) != 0){
                        ...
                        if(!currequpfile) {
                            ...
                            if(client->HasLowID() && (client->IsFriend() && client->GetFriendSlot()) == true) {
                                ...
                                // This checks which of client and bestClient would be on top on the queue, if client is put back.
                            ...
                            if(allowSwitch == false) {
                                ...
                                AddDebugLogLine(false, _T("%s: Request blocks discarded. Client tried to switch reqfile during upload session. FileID: (expected=%s, asked=%s)"), 
                                    client->GetUserName(), md4str(uploadFileId), md4str(reqfilehash));

                                if(client->GetSessionUp() == 0) {
                                    // Remove client from the upload queue
                                    theApp.uploadqueue->RemoveFromUploadQueue(client, _T("Client tried to switch reqfile during upload session."), true, true);

                                    // Put back with waiting time intact
                                    theApp.uploadqueue->AddClientToQueue(client, true, true);
                                    AddDebugLogLine(false, _T("Put client back on queue, with waiting time intact."));
					...
					while (data_in.GetLength()-data_in.GetPosition() && !bAnswerFNF)
					{
						...
						switch (opcode_in)
						{
							case OP_REQUESTFILENAME:
							{
								...
								// ZZUL-TRA :: Improvements :: Start :: Xman
								// if we are downloading this file, this could be a new source
								// no passive adding of files with only one part
								if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64)PARTSIZE){
									// ZZUL-TRA :: Global Hardlimit :: Start
								...
								// ZZUL-TRA :: Improvements :: End :: Xman
...
// ZZUL-TRA :: ICS :: End
							...
							{
#ifdef CLIENTANALYZER
								client->ProcessSourceRequest(packet, size, opcode_in, uRawSize, &data_in, reqfile);
#else
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
								if (thePrefs.GetDebugClientTCPLevel() > 0)
				...
				case OP_MULTIPACKETANSWER_EXT2:
				{
					...
					if (opcode == OP_MULTIPACKETANSWER_EXT2)
					{
						...
						if (client->GetRequestFile()==NULL)
							throw GetResString(IDS_ERR_WRONGFILEID) + _T(" (OP_MULTIPACKETANSWER_EXT2; client->GetRequestFile()==NULL)");
						if (reqfile != client->GetRequestFile())
							throw GetResString(IDS_ERR_WRONGFILEID) + _T(" (OP_MULTIPACKETANSWER_EXT2; reqfile!=client->GetRequestFile())");
					}
					else
					{
						uchar reqfilehash[16];
...
// ZZUL-TRA :: ICS :: End
				...
				{
#ifdef CLIENTANALYZER
					client->ProcessSourceRequest(packet, size, opcode, uRawSize, NULL, NULL);
#else
					CSafeMemFile data(packet, size);
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
					if (thePrefs.GetDebugClientTCPLevel() > 0)
				...
				{
#ifdef CLIENTANALYZER
					client->ProcessSourceAnswer(packet, size, opcode, uRawSize);
					break;
				}
#else
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)// X: [RDL] - [Remove Debug Log]
...
void CClientReqSocket::OnConnect(int nErrorCode)
{
	...
	{ 
//ZZUL +
	    CString strTCPError = GetErrorMessage(nErrorCode);
		if (thePrefs.GetVerbose())
		{
