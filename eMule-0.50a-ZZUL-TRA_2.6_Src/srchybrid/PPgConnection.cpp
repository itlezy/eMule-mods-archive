...
{
	guardian = false;
	Slots = thePrefs.GetSlots(); // ZZUL-TRA :: SlotControl
}
...
void CPPgConnection::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_UP_SLOTS_SLIDER, m_ctlSlots); // ZZUL-TRA :: SlotControl
}
...
// ZZUL-TRA :: SlotControl :: Start
	...
	if(Slots == 1)
		temp.Format(L"%s", GetResString(IDS_SLOTFOCUS));
	else {
		temp.Format(L"%.1f %s",(float)(MaxUpSpeed)/Slots, GetResString(IDS_KBYTESPERSEC));
	}
...
// ZZUL-TRA :: SlotControl :: End
	...
	if (lastmaxgd!=thePrefs.maxGraphDownloadRate)
		...
		theApp.m_pSpeedGraphWnd->SetUSpeedMeterRange((uint32)thePrefs.GetMaxGraphUploadRate(true)); // X: [SGW] - [SpeedGraphWnd]
...
// ZZUL-TRA :: SlotControl :: Start
    if (pScrollBar->GetSafeHwnd() == m_ctlSlots.m_hWnd)
	{
		...
		GetDlgItem(IDC_UP_SLOTS)->SetWindowText(temp);
		if(Slots == 1){
			temp.Format(GetResString(IDS_SLOTFOCUS));
		}else{
			uint32 MaxUpSpeed = (thePrefs.maxupload==UNLIMITED)?thePrefs.GetMaxGraphUploadRate(true):thePrefs.maxupload;
			temp.Format(L"%.1f %s",(float)(MaxUpSpeed)/Slots, GetResString(IDS_KBYTESPERSEC));
		}
