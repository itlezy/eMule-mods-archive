...
BEGIN_MESSAGE_MAP(CPPgSecurity, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_AUTOUPDATEIPFILTER, OnSettingsChange) // ZZUL-TRA :: IPFilterUpdate
END_MESSAGE_MAP()
...
void CPPgSecurity::LoadSettings(void)
{
	...
	CheckDlgButton(IDC_AUTOUPDATEIPFILTER, thePrefs.AutoUpdateIPFilter());// ZZUL-TRA :: IPFilterUpdate
...
// ZZUL-TRA :: IPFilterUpdate :: End
	...
	thePrefs.SetAutoUpdateIPFilter(IsDlgButtonChecked(IDC_AUTOUPDATEIPFILTER)!=0);// ZZUL-TRA :: IPFilterUpdate
...
void CPPgSecurity::Localize(void)
{
	if (m_hWnd)
	{
		...
		GetDlgItem(IDC_AUTOUPDATEIPFILTER)->SetWindowText(GetResString(IDS_AUTOUPDATEIPFILTER));// ZZUL-TRA :: IPFilterUpdate
	}
}
