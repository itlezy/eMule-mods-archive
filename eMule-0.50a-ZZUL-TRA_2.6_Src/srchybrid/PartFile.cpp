...
CBarShader CPartFile::s_ProgressBar(16); // ZZUL-TRA :: PercentBar
...
void CPartFile::Init(){
	...
	m_uLastSoureCacheProcessTime = ::GetTickCount(); // ZZUL-TRA :: Sourcecache

// ZZUL-TRA :: SessionDownload :: Start
	percentcompletedinitial = 0;
	m_uTransferredSession = 0;
 ...
 // ZZUL-TRA :: DropSystem :: Start
	...
	m_ShowDroppedSrc = 0; // show # of dropped sources 
...
CPartFile::~CPartFile()
{
	...
	ClearSourceCache(); // ZZUL-TRA :: Sourcecache
}
...
void CPartFile::AssertValid() const
{
	...
	ASSERT( status == PS_READY || status == PS_EMPTY || status == PS_WAITINGFORHASH || status == PS_ERROR || status == PS_COMPLETING || status == PS_COMPLETE || status == PS_IMPORTING); // ZZUL-TRA :: PartImportExport
	CHECK_BOOL(newdate);
	(void)lastpurgetime;
	...
	(void)s_LoadBar;
	(void)s_ChunkBar;
	(void)s_ProgressBar; // ZZUL-TRA :: PercentBar
...
// ZZUL-TRA :: SessionDownload :: Start
	ASSERT( percentcompletedinitial >= 0.0F && percentcompletedinitial <= 100.0F );
	(void)m_uTransferredSession;
	// ZZUL-TRA :: SessionDownload :: End
	...
	// ZZUL-TRA :: DropSystem :: End
	(void)m_ShowDroppedSrc; // show # of dropped sources 
}
...
void CPartFile::CreatePartFile(UINT cat)
{
	...
	m_SrcIncPartFrequency.SetSize(GetPartCount()); // ZZUL-TRA :: ICS 
	for (UINT i = 0; i < GetPartCount();i++)
	{
		m_SrcpartFrequency[i] = 0;
		m_SrcIncPartFrequency[i] = 0; // ZZUL-TRA :: ICS 
	}
	paused = false;
...
EPartFileLoadResult CPartFile::LoadPartFile(LPCTSTR in_directory,LPCTSTR in_filename, EPartFileFormat* pOutCheckFileFormat)
{
	...
	m_uTransferred = 0;
	m_uTransferredSession = 0; // ZZUL-TRA :: SessionDownload
...
//ZZUL-TRA :: PowerShare :: Start
 ...
 //ZZUL-TRA :: PowerShare :: End
	...
	try{
		...
		m_SrcpartFrequency.SetSize(GetPartCount());
		m_SrcIncPartFrequency.SetSize(GetPartCount()); // ZZUL-TRA :: ICS :: Start
		for (UINT i = 0; i < GetPartCount();i++)
		{
			m_SrcpartFrequency[i] = 0;
			m_SrcIncPartFrequency[i] = 0; // ZZUL-TRA :: ICS :: Start
		}
		SetStatus(PS_EMPTY);
		...
		if (!isnewstyle) // not for importing
		{
			...
			if (m_tUtcLastModified != fdate){
				theApp.SplashHide(); //ZZUL-TRA :: NewSplashscreen
				CString strFileInfo;
...
void CPartFile::DrawProgressBar(CDC* dc, LPCRECT rect, bool bFlat)
{
	...
	else
	{
	    ...
	    gaprect.bottom = gaprect.top + 16;
	    gaprect.left = rect->left;
    
		//if(!bFlat) {  //morph4u, use ever flat style
		//    s_ProgressBar.SetWidth((int)( (uint64)(m_nFileSize - allgaps)*blockpixel + 0.5F));
		//    s_ProgressBar.Fill(crProgress);
		//    s_ProgressBar.Draw(dc, gaprect.left, gaprect.top, false);
		//} else {
...
void CPartFile::DrawStatusBar(CDC* dc, LPCRECT rect, bool bFlat) /*const*/
{
	...
	else
	{
		...
		if (notgray) {
			crMissing = RGB(255, 0, 0);
			if (bFlat) {
				crHave = RGB(140, 140, 140);
				crPending = RGB(255, 208, 0);
			} else {
				crHave = RGB(104, 104, 104);
				crPending = RGB(255, 208, 0);
			}
            crStartedButIncomplete = RGB(0, 0, 0);
		} else {
	...
	else
	{
		    ...
		    for (UINT i = 0; i < GetPartCount(); i++){
			    if (gapstart >= (uint64)i*PARTSIZE && gapstart <= (uint64)(i+1)*PARTSIZE - 1){ // is in this part?
				    ...
				    if (m_SrcpartFrequency.GetCount() >= (INT_PTR)i && m_SrcpartFrequency[(uint16)i])
				    {
						...
						{
                            if (notgray) {
                                double factor = min(22, 210/(double)max(GetSourceCount(),1));
								color = RGB(0,
											(210 - factor*(m_SrcpartFrequency[(uint16)i] - 1) <  0) ?  0 : 210 - factor*(m_SrcpartFrequency[(uint16)i] - 1),
											255);
                            } else
								color = RGB(64,
...
#ifdef CHUNK_DOTS
        {
            ...
            if(!bFlat) {
			    ...
			    s_LoadBar.SetFileSize(1ui64);
			    s_LoadBar.Fill(crDot);
			    for(uint64 i=PARTSIZE; i<m_nFileSize; i+=PARTSIZE)
				    s_LoadBar.Draw(dc, gaprect.left+(uint32)(i*(double)w/(uint64)m_nFileSize), gaprect.top, false);
		    } else {
			    for(uint64 i=PARTSIZE; i<m_nFileSize; i+=PARTSIZE){
				    gaprect.left = gaprect.right = rect->left+(uint32)(i*(double)w/(uint64)m_nFileSize);
				    gaprect.right++;
...
//Spike2: Off course the original code mustn't be used for IntelliFlush
/*
...
//>>> WiZaRd::IntelliFlush
	if((m_nTotalBufferData && m_nTotalBufferData > thePrefs.GetFileBufferSize())
...
//<<< WiZaRd::IntelliFlush
	...
	else
	{
		...
		uint16 m_ShowDroppedSrc_Temp = 0; // show # of dropped sources 
				...
				case DS_LOWTOLOWIP:
				{
					...
					if( cur_src->HasLowID() )
					{
						...
						if( !theApp.CanDoCallback( cur_src ) )
						{
							...
							if( ((dwCurTick - lastpurgetime) > SEC2MS(30)) && (this->GetSourceCount() >= (GetMaxSources()*.8 )) && theApp.GetReAskTick() < dwCurTick ) // ZZUL-TRA :: AutoDropImmunity
							{
								theApp.downloadqueue->RemoveSource( cur_src );
									m_ShowDroppedSrc_Temp++; // show # of dropped sources 
		...
		m_ShowDroppedSrc = (uint16)(m_ShowDroppedSrc+ m_ShowDroppedSrc_Temp); // show # of dropped sources 
		...
		*/		
		...
		if ( !m_bLocalSrcReqQueued && ((!m_LastSearchTime) || (dwCurTick - m_LastSearchTime) > SERVERREASKTIME) && theApp.serverconnect->IsConnected()
			...
			&& IsSrcReqOrAddAllowed() // ZZUL-TRA :: Global Hardlimit 
	...
	for (UINT i = 0; i < count; i++)
	{
		...
		*/
		else
		{
			// since we may received multiple search source UDP results we have to "consume" all data of that packet
			if ((byCryptOptions & 0x80) != 0)
				AddToSourceCache(port, userid, serverip, serverport, SF_CACHE, true, achUserHash, byCryptOptions);
			else
				AddToSourceCache(port, userid, serverip, serverport, SF_CACHE, true, NULL, byCryptOptions);

			if(!bStopKadSearch && GetKadFileSearchID()){
				Kademlia::CSearchManager::StopSearch(GetKadFileSearchID(), false);
				bStopKadSearch = true;
			}
		}
...
// ZZUL-TRA :: AntiHideOS :: End
			for (uint16 i = 0; i < partcount; i++)
			{
				if (cur_src->IsPartAvailable(i))
				{
					...
					cur_src->m_abySeenPartStatus[i] = 1; // ZZUL-TRA :: AntiHideOS 
				}
// ZZUL-TRA :: AntiHideOS :: Start
...
//<<< WiZaRd::Optimization
	...
	NewSrcIncPartsInfo(); // ZZUL-TRA :: ICS 
...
//ZZUL-TRA :: PowerShare :: Start
	...
	UpdatePowerShareLimit(iCompleteSourcesCountInfoReceived>1 && m_anStates[DS_TOOMANYCONNS]==0 && m_anStates[DS_CONNECTING]==0 && m_nVirtualCompleteSourcesCount==1,bPowerShareLimit);
//ZZUL-TRA :: PowerShare :: Start
	UpdateDisplayedInfo();
}	
...
BOOL CPartFile::PerformFileComplete() 
{
	...
	m_sourcesaver.DeleteFile(this); // ZZUL-TRA :: SLS
...
CString CPartFile::getPartfileStatus() const
{
	switch(GetStatus()){
		...
		case PS_IMPORTING:
			return GetResString(IDS_IMPORTING);
...
void CPartFile::AddClientSources(CSafeMemFile* sources, uint8 uClientSXVersion, bool bSourceExchange2, const CUpDownClient* pClient)
{
	...
	for (UINT i = 0; i < nCount; i++)
	{
		...
		*/
			...
			if(uPacketSXVersion >= 4)
				AddToSourceCache(nPort, dwID, dwServerIP, nServerPort, SF_CACHE, false, achUserHash, byCryptOptions);
			else if(uPacketSXVersion >= 3)
				AddToSourceCache(nPort, dwID, dwServerIP, nServerPort, SF_CACHE, false, achUserHash);
			else if(uPacketSXVersion >= 2)
				AddToSourceCache(nPort, dwID, dwServerIP, nServerPort, SF_CACHE, true, achUserHash);
			else
				AddToSourceCache(nPort, dwID, dwServerIP, nServerPort, SF_CACHE, true);
		}
		// ZZUL-TRA :: Sourcecache :: End
	}
}
								...
								const CUpDownClient* client)
{
	...
	m_uTransferredSession += transize; // ZZUL-TRA :: SessionDownload
...
//>>> WiZaRd::IntelliFlush
	...
	else
	{
		...
		{
			if (thePrefs.GetLogFileSaving()) //>> pP: proper logging
...
{
// ZZUL-TRA :: ICS :: Start
	//ICS only... could add a switch here
		return GetNextRequestedBlockICS(sender, newblocks, count);
}

bool CPartFile::GetNextRequestedBlockStd(CUpDownClient* sender, 
									  Requested_Block_Struct** newblocks, 
									  uint16* count) /*const*/
{
// ZZUL-TRA :: ICS :: End
...
void CPartFile::AddToSourceCache(uint16 uPort, uint32 uID, uint32 uServerIP, uint16 uServerPort, ESourceFrom nSourceFrom, bool bED2kIDFlag, const uchar* achUserHash, uint8 byCryptOptions)
{
	PartfileSourceCache new_src;
	new_src.uPort = uPort;
	new_src.uID = uID;
	new_src.uServerIP = uServerIP;
	new_src.uServerPort = uServerPort;
	new_src.bED2kIDFlag = bED2kIDFlag;
	new_src.nSourceFrom = nSourceFrom;
	new_src.uExpires = ::GetTickCount() + SOURCECACHELIFETIME;
	new_src.byCryptOptions = byCryptOptions;
	if(achUserHash != NULL)
	{
		md4cpy(new_src.achUserHash, achUserHash);
		new_src.bWithUserHash = true;
	}
	else
		new_src.bWithUserHash = false;

	m_SourceCache.AddTail(new_src);
}

void CPartFile::ProcessSourceCache()
{
	if(m_uLastSoureCacheProcessTime + SOURCECACHEPROCESSLOOP < ::GetTickCount())
	{
		uint32 uCurrentTime = ::GetTickCount(); // cache value
		m_uLastSoureCacheProcessTime = uCurrentTime;
		...
		while(!m_SourceCache.IsEmpty() && m_SourceCache.GetHead().uExpires < uCurrentTime)
			m_SourceCache.RemoveHead();

		uint32 uSourcesAdded = 0;
		while(!m_SourceCache.IsEmpty() && GetMaxSources() > this->GetSourceCount() + 1) //let room for 1 passiv source
		{
			PartfileSourceCache cur_src = m_SourceCache.RemoveHead();
			CUpDownClient* new_src = new CUpDownClient(this, cur_src.uPort, cur_src.uID, cur_src.uServerIP, cur_src.uServerPort, cur_src.bED2kIDFlag);
			new_src->SetSourceFrom(cur_src.nSourceFrom);
			new_src->SetConnectOptions(cur_src.byCryptOptions, true, false);

			if(cur_src.bWithUserHash)
				new_src->SetUserHash(cur_src.achUserHash);
			if(theApp.downloadqueue->CheckAndAddSource(this, new_src))
				uSourcesAdded++;
		}

		if(uSourcesAdded && thePrefs.GetDebugSourceExchange())
			DebugLog(LOG_SUCCESS, L"Sourcecache: %u sources added via sourcache. File: %s", uSourcesAdded, GetFileName()); 
	}
}
// ZZUL-TRA :: Sourcecache :: End
...
UINT CPartFile::GetMinQR() 
{ 
	UINT minQR = (UINT)~0, qr; 
	if (srclist.GetSize() == 0 || GetStatus() == PS_COMPLETE || GetStatus() == PS_INSUFFICIENT || IsStopped()) 
		minQR = 0; 
	else 
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL; ) 
		{ 
			...
			if (cur_src->IsRemoteQueueFull()) { 
				if (minQR == 0) 
					minQR = (UINT)~1; // Flag for queue full 
			} else if (cur_src->IsInNoNeededList(this)) { 
				if (minQR == 0) 
					minQR = (UINT)~2; // Flag for no needed parts 
			} else { 
				qr = cur_src->GetRemoteQueueRank(); 
				if (qr) 
					minQR = min(minQR, qr); 
			} 
		} 
...
void CPartFile::CalcAvgQr(UINT inqr,UINT oldqr)
{
	...
	if(m_countqr!=0)
		m_avgqr=(uint16)(m_sumqr/m_countqr);
	else
		m_avgqr=0;
}
// <== Average Queue Ranking [Xman] 
...
/*
					...
					first_last_mod = 2; //>>> taz::Fix was 0
				const bool isPreviewEnable = (thePrefs.GetPreviewPrio() || thePrefs.IsExtControlsEnabled() && GetPreviewPrio()) && IsPreviewableFileType();
				if (!isPreviewEnable)			
					first_last_mod = 2; //>>> taz::Fix was 0
*/
