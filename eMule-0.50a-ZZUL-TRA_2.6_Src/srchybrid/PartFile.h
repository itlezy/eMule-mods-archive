...
#include "Addons/SLS/SourceSaver.h" // ZZUL-TRA :: SLS
...
// ZZUL-TRA :: FileStatusIcons :: End
	PS_IMPORTING		= 13 // ZZUL-TRA :: PartImportExport
};
...
enum EPartFileOp{
	...
	PFOP_HASHING,
	PFOP_COPYING,
	PFOP_IMPORTING, // ZZUL-TRA :: PartImportExport
	PFOP_UNCOMPRESSING
};
...
class CxImage;
class CSafeMemFile;
struct ImportInfo; // ZZUL-TRA :: PartImportExport
...
public:
	...
	void	DrawProgressBar(CDC* dc, LPCRECT rect, bool bFlat); // ZZUL-TRA :: PercentBar :: Start
	...
	EPartFileStatus	GetPartFileStatus(); // ZZUL-TRA :: FileStatusIcons
...
//	int		GetCommonFilePenalty() const;
	...
	void	AddToSourceCache(uint16 uPort, uint32 uID, uint32 uServerIP, uint16 uServerPort, ESourceFrom nSourceFrom, bool bED2kIDFlag = false, const uchar* achUserHash = NULL, uint8 byCryptOptions = 0);
...
private:
	...
	static CBarShader s_ProgressBar; // ZZUL-TRA :: PercentBar :: Start
	...
	uint32						m_uLastSoureCacheProcessTime;
	// ZZUL-TRA :: Sourcecache :: End
...
public:
	bool	GetNextRequestedBlockICS(CUpDownClient* sender, Requested_Block_Struct** newblocks, uint16* count) /*const*/;
	bool	GetNextRequestedBlockStd(CUpDownClient* sender, Requested_Block_Struct** newblocks, uint16* count) /*const*/;
