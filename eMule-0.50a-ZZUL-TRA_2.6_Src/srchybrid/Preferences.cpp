...
#include "XMessageBox.h" //XMessageBox +

#pragma warning(disable:4516) // access-declarations are deprecated; member using-declarations provide a better alternative
#pragma warning(disable:4244) // conversion from 'type1' to 'type2', possible loss of data
#pragma warning(disable:4100) // unreferenced formal parameter
#pragma warning(disable:4702) // unreachable code
// ZZUL-TRA :: NewCrypto++ :: Start
/*
#include <crypto51/osrng.h>
*/
#include <cryptopp/osrng.h>
// ZZUL-TRA :: NewCrypto++ :: End
...
DWORD	CPreferences::m_adwStatsColors[16];//powershare graph
...
bool	CPreferences::m_bLogAnalyzerEvents;
#endif
...
#ifdef CLIENTANALYZER
int  	CPreferences::m_uiCreditSystem; //>>> WiZaRd::ClientAnalyzer
#else
...
bool	CPreferences::m_bAutoDisconnect;
int     CPreferences::m_iDisconnectTime; 
// ZZUL-TRA :: ServerAutoDisconnect :: End

// ZZUL-TRA :: SlotControl :: Start 
uint16	CPreferences::m_uiClientDataRate, CPreferences::m_Prev_uiClientDataRate;
// ZZUL-TRA :: SlotControl :: End

// ZZUL-TRA :: DropSystem :: Start
bool	CPreferences::m_bAutoDropSystem;
// ZZUL-TRA :: DropSystem :: End
...
uint64	CPreferences::UploadedPowershareComplete;
uint64	CPreferences::UploadedPowersharePartfile;
// ZZUL-TRA :: PowersahreStat :: End

// ZZUL-TRA :: IP2Country :: Start
uint8	CPreferences::m_uiIP2CountryNameMode;
bool	CPreferences::m_bIP2CountryShowFlag;

//UINT	CPreferences::m_uiIP2CountryVersion;
//bool	CPreferences::m_bIP2CountryAutoUpdate;
//TCHAR	CPreferences::m_strIP2CountryUpdateURL[256];
//TCHAR	CPreferences::m_strIP2CountryUpdateVerURL[256];
//SYSTEMTIME CPreferences::m_stIP2CountryVersion;
...
uint16	CPreferences::m_iPayBackFirstLimit;
...
bool	CPreferences::m_bShowFileStatusIcons; // ZZUL-TRA :: FileStatusIcons
bool	CPreferences::m_bShowRuntimeOnTitle; // ==> Show runtime on title
bool	CPreferences::m_bDirectoryWatcher;	// ZZUL-TRA :: AutoSharedFilesUpdater
bool	CPreferences::m_bUsePercentBar; // ZZUL-TRA :: PercentBar :: Start
bool	CPreferences::m_bFunnyNick; // ZZUL-TRA :: FunnyNick
...
bool	CPreferences::m_bShowSessionDownload; // ZZUL-TRA :: SessionDownload
int CPreferences::m_iClientbantime; // Spike2 - variable ClientBanTime
CString CPreferences::m_strBrokenURLs; //MORPH - Added by WiZaRd, Fix broken HTTP downloads
bool	CPreferences::m_bShowDownloadColor; 
//log friends activities
bool	CPreferences::m_bLogFriendlistActivities;
//log friends activities
// NEO: QS - [QuickStart]
bool	CPreferences::m_uQuickStart;
int		CPreferences::m_iQuickStartTime;
int		CPreferences::m_iQuickStartTimePerFile;
UINT	CPreferences::m_iQuickMaxConperFive;
UINT	CPreferences::m_iQuickMaxHalfOpen;
UINT	CPreferences::m_iQuickMaxConnections;
bool	CPreferences::m_bOnQuickStart;
// NEO: QS END
bool     CPreferences::m_bShowCpu; // ZZUL-TRA :: SysInfo
...
// ZZUL-TRA :: SessionRatio :: Start
	  uint64 m_uimaxDownload = maxdownload*1024;
      ...
      if(theApp.uploadqueue && theApp.uploadqueue->GetWaitingUserCount() && theApp.uploadqueue->GetUploadQueueLength())
      {
	     ...
	     const double uploaded = max(SESSIONMAXTRANS, (double)theStats.sessionSentBytes /*- (double)theStats.sessionSentBytesToFriend - (double)thePrefs.GetUploadedPowersharePartfile()*/); //MultiFU //PowerShare
	     const double sessionUlDlRatio = (uploaded + 1.0) / (theStats.sessionReceivedBytes - sesDownData_URL + 1.0); //Exclude HTTP Traffic
	     ...
	     {		
		  ...
		  m_uimaxDownload = (uint64)min(max(minDownload, (9*currUpload*sessionUlDlRatio)), m_uimaxDownload);
	     }	
      }
	...
	uint64 m_uioffimaxDownload = 0;
	if( maxup < 4*1024 )
		m_uioffimaxDownload = (( (maxup < 10*1024) && ((uint64)maxup*3 < maxdownload*1024) )? (uint64)maxup*3 : maxdownload*1024);
	else
		m_uioffimaxDownload = (( (maxup < 10*1024) && ((uint64)maxup*4 < maxdownload*1024) )? (uint64)maxup*4 : maxdownload*1024);
	...
	return min(m_uimaxDownload, m_uioffimaxDownload);
/*
	//dont be a Lam3r :)
	UINT maxup;
	if (dynamic && thePrefs.IsDynUpEnabled() && theApp.uploadqueue->GetWaitingUserCount() != 0 && theApp.uploadqueue->GetDatarate() != 0) {
	...
	switch (bUpDown){
		case true:
			...
			switch (uClientID){
				...
				// ZZUL-TRA :: EnhancedClientRecognization :: Start
				case SO_HYDRANODE:
				case SO_EMULEPLUS:
				case SO_TRUSTYFILES:
				// ZZUL-TRA :: EnhancedClientRecognization :: End
				case SO_CDONKEY:
				case SO_LPHANT:
...
void CPreferences::SavePreferences()
{
...
#ifdef CLIENTANALYZER
	ini.WriteBool(L"LogAnalyzerEvents", m_bLogAnalyzerEvents); //why should one use get if we want to SAVE something?!
#endif
	...
	// ini.WriteBool(L"FullChunkTransfers", m_btransferfullchunks); // ZZUL-TRA :: FullChunk
...
#ifdef CLIENTANALYZER
	ini.WriteInt(L"UseCreditSystem",m_uiCreditSystem); //>>> WiZaRd::ClientAnalyzer
#else
...
// ZZUL-TRA :: Advanced official preferences :: Start
	...
	ini.WriteBool(L"ICH",IsICHEnabled());
	...
	//ini.WriteBool(L"EnabledDeprecated", m_bPeerCacheEnabled, L"PeerCache");// X: [RPC] - [Remove PeerCache]
// ZZUL-TRA :: Advanced official preferences :: End
	...
	CString buffer2;
	for (int i=0;i<GetNumStatsColors();i++) { //powershare graph
...
// ZZUL-TRA :: Advanced official preferences :: Start
    ...
    ini.WriteString(L"AllowedIPs",WriteAllowedIPs);  //sectionn Webserver end
// ZZUL-TRA :: Advanced official preferences :: End
...
//ZUL-TRA  ////////////////////////////////////////////////////////////////////////
	...
	ini.WriteBool(L"EmuLog", m_bEmuLog, L"ZZUL-TRA");// ZZUL-TRA :: EmulateOthers 

//ZZUL-TRA :: PowerShare :: Start
	ini.WriteInt(L"PowerShareLimit",PowerShareLimit);
    ini.WriteInt(L"PowershareMode",m_iPowershareMode); 
	ini.WriteInt(L"PsAmountLimit",PsAmountLimit);
//ZZUL-TRA :: PowerShare :: End

// ZZUL-TRA :: SlotControl :: Start 
	ini.WriteInt(L"DataRate", m_uiClientDataRate);
// ZZUL-TRA :: SlotControl :: End
...
// ZZUL-TRA :: ServerAutoDisconnect :: Start
	ini.WriteBool(L"AutoDisconnect", m_bAutoDisconnect);
	ini.WriteInt(L"AutoDisconnectTime", m_iDisconnectTime); 
// ZZUL-TRA :: ServerAutoDisconnect :: End

// ZZUL-TRA :: DropSystem :: Start
	ini.WriteBool(L"AutoDropSystem", m_bAutoDropSystem);
	// ZZUL-TRA :: DropSystem :: End
...
// ZZUL-TRA :: Global Hardlimit :: Start
	...
	ini.WriteBool(L"ShowFileStatusIcon", m_bShowFileStatusIcons); // ZZUL-TRA :: FileStatusIcons
...
// ZZUL-TRA :: IP2Country :: Start
	ini.WriteInt(L"IP2Country", m_uiIP2CountryNameMode); 
	ini.WriteBool(L"IP2CountryShowFlag", m_bIP2CountryShowFlag);
	/*
	ini.WriteInt(L"IP2CountryVersion", (UINT)m_uiIP2CountryVersion);
	ini.WriteBool(L"AutoUpdateIP2Country", m_bIP2CountryAutoUpdate);
	ini.WriteString(L"UpdateURLIP2Country", m_strIP2CountryUpdateURL);
	ini.WriteString(L"UpdateVerURLIP2Country", m_strIP2CountryUpdateVerURL);
	ini.WriteBinary(L"IP2CountryVersion2", (LPBYTE)&m_stIP2CountryVersion, sizeof(m_stIP2CountryVersion)); 
*/
	// ZZUL-TRA :: IP2Country :: End

// ZZUL-TRA :: SLS :: Start
	ini.WriteBool(L"UseSaveLoadSources", m_bUseSaveLoadSources);
	ini.WriteInt(L"ActivationLimitSLS", m_iActivationLimitSLS);
	ini.WriteInt(L"SourcesToSaveSLS", m_iSourcesToSaveSLS);
	// ZZUL-TRA :: SLS :: End

// ZZUL-TRA :: PaybackFirst :: Start
	ini.WriteBool(L"IsPayBackFirst", m_bPayBackFirst);
	ini.WriteInt(L"PayBackFirstLimit", m_iPayBackFirstLimit);
	// ZZUL-TRA :: PaybackFirst :: End

	ini.WriteBool(L"ShowRuntimeOnTitle", m_bShowRuntimeOnTitle);// ==> Show runtime on title
	ini.WriteBool(L"DirWatcher", m_bDirectoryWatcher);	// ZZUL-TRA :: AutoSharedFilesUpdater
    ini.WriteBool(L"PercentBar", m_bUsePercentBar); // ZZUL-TRA :: PercentBar :: Start
	ini.WriteBool(L"FunnyNick", m_bFunnyNick); // ZZUL-TRA :: FunnyNick
// ZZUL-TRA :: IPFilterUpdate :: Start
...
// ZZUL-TRA :: IPFilterUpdate :: End
	ini.WriteBool(L"ShowSessionDownload", m_bShowSessionDownload); // ZZUL-TRA :: SessionDownload
    	ini.WriteInt(L"Clientbantime",m_iClientbantime); //Spike2 - Variable ClientBanTime
	ini.WriteString(L"BrokenURLs", m_strBrokenURLs); //MORPH - Added by WiZaRd, Fix broken HTTP downloads
	ini.WriteBool(L"ShowDownloadColor", m_bShowDownloadColor); 
    ini.WriteBool(L"ShowCPU", m_bShowCpu); // ZZUL-TRA :: SysInfo
// NEO: QS - [QuickStart]
	ini.WriteBool(L"UseQuickStart", m_uQuickStart);
	...
	// NEO: QS END
	ini.WriteBool(L"LogFriendActivities", m_bLogFriendlistActivities);//log friends activities
// ==> Advanced Transfer Window Layout - Stulle
#ifdef ATWL
	ini.WriteInt(L"TransferWnd1",m_uTransferWnd1);
	ini.WriteInt(L"TransferWnd2",m_uTransferWnd2);
...
void CPreferences::ResetStatsColor(int index)
{
	switch(index)
	{
		...
		case 13: m_adwStatsColors[13]=RGB(  0,255,  0);break;//friends green
		case 14: m_adwStatsColors[14]=RGB(255,190,190);break;
		case 15: m_adwStatsColors[15]=RGB(255, 0, 255);break; //powershare graph
	}
}
...
/*#else
	...
	updatenotify=ini.GetBool(L"UpdateNotifyTestClient",false); //default disabled
...
//TK4 MOD +
	...
	if (splitterbarPositionStat_HR > splitterbarPositionStat_HL || 
		splitterbarPositionStat_HL > 102 ||	splitterbarPositionStat_HR > 102){
//TK4 MOD -
	...
	m_bIconflashOnNewMessage=ini.GetBool(L"IconflashOnNewMessage",true);//changed
	...
	m_bRestoreLastMainWndDlg=ini.GetBool(L"RestoreLastMainWndDlg",true);//changed
	...
	filterserverbyip=ini.GetBool(L"FilterServersByIP",true);//changed
...
/*#else// X: [RDL] - [Remove Debug Log]
...
#ifdef CLIENTANALYZER
		m_bLogAnalyzerEvents = ini.GetBool(L"LogAnalyzerEvents", false);
#endif
	...
	*/
	...
	if (m_iFileBufferSize == 0)
		m_iFileBufferSize = /*256*1024*/ 4096*1024; //>> pP: ModPrefs
	else
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	m_bExtControls =ini.GetBool(L"ShowExtControls",true); //changed
...
#ifdef CLIENTANALYZER
	m_uiCreditSystem=ini.GetInt(L"UseCreditSystem", 1); //>>> WiZaRd::ClientAnalyzer 
#else
	...
	m_bShowDwlPercentage=ini.GetBool(L"ShowDwlPercentage",true); //changed
	...
	m_bShowCopyEd2kLinkCmd=ini.GetBool(L"ShowCopyEd2kLinkCmd",true); //changed

	m_iMaxChatHistory=ini.GetInt(L"MaxChatHistoryLines",100);
	if (m_iMaxChatHistory < 1)
		m_iMaxChatHistory = 100;
	if (m_iMaxChatHistory > 2048)  m_iMaxChatHistory = 2048; // ZZUL-TRA :: Advanced official preferences
	maxmsgsessions=ini.GetInt(L"MaxMessageSessions",50);
	if (maxmsgsessions > 6000)  maxmsgsessions = 6000; // ZZUL-TRA :: Advanced official preferences
	if (maxmsgsessions < 0 )  maxmsgsessions = 0; // ZZUL-TRA :: Advanced official preferences
...
//ZZUL-TRA  /////////////////////////////////////////////////////////////////////
	...
	m_bEmuLog = ini.GetBool(L"EmuLog",false, L"ZZUL-TRA");// ZZUL-TRA :: EmulateOthers 

//ZZUL-TRA :: PowerShare :: Start
	PowerShareLimit=ini.GetInt(L"PowerShareLimit",0);
	m_iPowershareMode=ini.GetInt(L"PowershareMode",2); //default: 2=auto, 0=disabled
	uint32 m_uTemp = ini.GetInt(L"PsAmountLimit",0);
...
// ZZUL-TRA :: SlotControl :: Start
    if(m_bFirstStart)
	{
		...
		m_Prev_uiClientDataRate = 3;
	}
	else
	{
		m_uiClientDataRate = (uint16)ini.GetInt(L"DataRate", 0);
...
// ZZUL-TRA :: ServerAutoDisconnect :: Start
	m_iDisconnectTime = ini.GetInt(L"AutoDisconnectTime", 20); 
	m_bAutoDisconnect = ini.GetBool(L"AutoDisconnect", false); 
    // ZZUL-TRA :: ServerAutoDisconnect :: End

// ZZUL-TRA :: DropSystem :: Start
	m_bAutoDropSystem = ini.GetBool(L"AutoDropSystem", true);
	// ZZUL-TRA :: DropSystem :: End

// ZZUL-TRA :: Global Hardlimit :: Start
	m_bGlobalHL = ini.GetBool(L"GlobalHL", false);
	uint32 m_uGlobalHlStandard = VAL_GSL_DEF;
	if(maxGraphUploadRate != UNLIMITED){
		...
		m_uGlobalHlStandard = max(VAL_GSL_MIN, min(m_uGlobalHlStandard, VAL_GSL_MAX));
	}
	m_uGlobalHL = ini.GetInt(L"GlobalHLvalue", m_uGlobalHlStandard);
	// ZZUL-TRA :: Global Hardlimit :: End
	...
	m_bShowFileStatusIcons = ini.GetBool(L"ShowFileStatusIcon", false); // ZZUL-TRA :: FileStatusIcons
...
// ZZUL-TRA :: SLS :: Start
	...
	m_iActivationLimitSLS = (uint8)ini.GetInt(L"ActivationLimitSLS", VAL_SLS_ACTIVATION_DEF);
	m_iSourcesToSaveSLS = (uint8)ini.GetInt(L"SourcesToSaveSLS", VAL_SLS_SAVE_DEF);
	// ZZUL-TRA :: SLS :: End

	// ZZUL-TRA :: PaybackFirst :: Start
	m_bPayBackFirst = ini.GetBool(L"IsPayBackFirst", false);
	m_iPayBackFirstLimit = (uint16)ini.GetInt(L"PayBackFirstLimit", VAL_PBF_SUI_DEF);
	// ZZUL-TRA :: PaybackFirst :: End

	m_bShowRuntimeOnTitle = ini.GetBool(L"ShowRuntimeOnTitle",false); // ==> Show runtime on title
	m_bDirectoryWatcher = ini.GetBool(L"DirWatcher", false);	// ZZUL-TRA :: AutoSharedFilesUpdater
    m_bUsePercentBar = ini.GetBool(L"PercentBar",false); // ZZUL-TRA :: PercentBar :: Start
	m_bFunnyNick = ini.GetBool(L"FunnyNick", false); // ZZUL-TRA :: FunnyNick
...
// ZZUL-TRA :: IP2Country :: Start
	/*
	pst = NULL;
	usize = sizeof m_stIP2CountryVersion;
	if (ini.GetBinary(L"IP2CountryVersion2", &pst, &usize) && usize == sizeof m_stIP2CountryVersion)
		memcpy(&m_stIP2CountryVersion, pst, sizeof m_stIP2CountryVersion);
	else
		memset(&m_stIP2CountryVersion, 0, sizeof m_stIP2CountryVersion);
	delete[] pst;
    */
	m_uiIP2CountryNameMode = (uint8)ini.GetInt(L"IP2Country", 0); 
	m_bIP2CountryShowFlag = ini.GetBool(L"IP2CountryShowFlag", true);
    /*
	m_bIP2CountryAutoUpdate = ini.GetBool(L"AutoUpdateIP2Country", false);
	m_uiIP2CountryVersion = (UINT)ini.GetInt(L"IP2CountryVersion", 0);
	_stprintf(m_strIP2CountryUpdateURL, L"%s", ini.GetString(L"UpdateURLIP2Country", L"http://ip-to-country.webhosting.info/downloads/ip-to-country.csv.zip"));
	_stprintf(m_strIP2CountryUpdateVerURL, L"%s", ini.GetString(L"UpdateVerURLIP2Country", L"http://ip-to-country.webhosting.info/downloads/latest"));
    */
	// ZZUL-TRA :: IP2Country :: End

	m_bShowSessionDownload = ini.GetBool(L"ShowSessionDownload", false); // ZZUL-TRA :: SessionDownload
	m_iClientbantime = (uint8)ini.GetInt(L"Clientbantime", 4); // Spike2 - variable ClientBanTime
	m_strBrokenURLs = ini.GetStringLong(L"BrokenURLs", L"sourceforge"); //MORPH - Added by WiZaRd, Fix broken HTTP downloads  
	m_bShowDownloadColor = ini.GetBool(L"ShowDownloadColor", false); 
    m_bShowCpu = ini.GetBool(L"ShowCPU",false); // ZZUL-TRA :: SysInfo
// NEO: QS - [QuickStart]
	m_uQuickStart = ini.GetBool(L"UseQuickStart", false);
	...
	m_iQuickMaxConperFive = ini.GetInt(L"QuickMaxConperFive", (thePrefs.MaxConperFive<INT_MAX/2)?(int)(thePrefs.MaxConperFive*1.5):(int)thePrefs.MaxConperFive);
	m_iQuickMaxHalfOpen = ini.GetInt(L"QuickMaxHalfOpen", (int)(thePrefs.maxhalfconnections));
	m_iQuickMaxConnections = ini.GetInt(L"QuickMaxConnections", (thePrefs.maxconnections<INT_MAX/2)?(int)(thePrefs.maxconnections*1.5):(int)thePrefs.maxconnections);

	if(m_iQuickStartTime<TIM_QUICK_START_MIN)
		m_iQuickStartTime=TIM_QUICK_START_MIN;
	else if(m_iQuickStartTime>TIM_QUICK_START_MAX)
		m_iQuickStartTime=TIM_QUICK_START_MAX;

	if(m_iQuickStartTimePerFile<VAL_QUICK_START_PF_MIN)
		m_iQuickStartTimePerFile=VAL_QUICK_START_PF_MIN;
	else if(m_iQuickStartTimePerFile>VAL_QUICK_START_PF_MAX)
		m_iQuickStartTimePerFile=VAL_QUICK_START_PF_MAX;
	// NEO: QS END

	m_bLogFriendlistActivities = ini.GetBool(L"LogFriendActivities", false); //log friends activities
	...
	CheckZZULTRAPreferences(); // ZZUL-TRA :: SafePrefsettings
...
UINT CPreferences::GetMaxConnections()
{
	return m_bOnQuickStart ? m_iQuickMaxConnections : maxconnections;
}

UINT CPreferences::GetMaxHalfConnections()
{
	return m_bOnQuickStart ? m_iQuickMaxHalfOpen : maxhalfconnections;
}

UINT CPreferences::GetMaxConperFive()
{
	return m_bOnQuickStart ? m_iQuickMaxConperFive : MaxConperFive;
}
// NEO: QS END <-- Xanatos --
...
// ZZUL-TRA :: SafePrefsettings :: Start
void CPreferences::CheckZZULTRAPreferences()
{
	// ZZUL-TRA :: PaybackFirst :: Start
	MinMax(&m_iPayBackFirstLimit, (uint16)VAL_PBF_SUI_MIN, (uint16)VAL_PBF_SUI_MAX);
	// ZZUL-TRA :: PaybackFirst :: End
	// ZZUL-TRA :: SLS :: Start
	MinMax(&m_iActivationLimitSLS, (uint8)VAL_SLS_ACTIVATION_MIN, (uint8)VAL_SLS_ACTIVATION_MAX);
	MinMax(&m_iSourcesToSaveSLS, (uint8)VAL_SLS_SAVE_MIN, (uint8)VAL_SLS_SAVE_MAX);
	// ZZUL-TRA :: SLS :: End
	MinMax(&m_uGlobalHL, (UINT)VAL_GSL_MIN, (UINT)VAL_GSL_MAX); // ZZUL-TRA :: Global Hardlimit
}
// ZZUL-TRA :: SafePrefsettings :: End