...
#include "Opcodes.h" // ZZUL-TRA :: SlotControl
const CString strDefaultToolbar = _T("009901020304050607990809"); //Remove IRC //Remove Help
...
public:
	...
	static	DWORD	m_adwStatsColors[16];//powershare graph 
...
#ifdef CLIENTANALYZER
	static	int  	UseCreditSystem()					{return m_uiCreditSystem;}
#else
...
//ZZUL-TRA :: PowerShare :: Start
	...
	static  int	PsAmountLimit; 
	static	int	GetPsAmountLimit() {return PsAmountLimit;} 
//ZZUL-TRA :: PowerShare :: End

// ZZUL-TRA :: SlotControl :: Start 
private:
	static	uint16	m_Prev_uiClientDataRate;
public:
	static	bool	IsUseSlotFocus()			{return (m_uiClientDataRate == 0);}
	static	void	SetUseSlotFocus(const bool b)		{if (b) {m_Prev_uiClientDataRate = m_uiClientDataRate; m_uiClientDataRate = 0;} else m_uiClientDataRate = m_Prev_uiClientDataRate;}

private:
	static	uint16	m_uiClientDataRate;
public:
	static	uint16	GetClientDataRate()				{return m_uiClientDataRate;}
	static	void	SetClientDataRate(const uint16 i)	{if (i == 0) m_Prev_uiClientDataRate = m_uiClientDataRate; m_uiClientDataRate = i;}

public:
	static	uint16	GetSlots()				{return (m_uiClientDataRate == 0 ? 1 : (uint16) ((float) ((maxupload == UNLIMITED) ? GetMaxGraphUploadRate(true) : maxupload)  * (float) 1024 / ((float) m_uiClientDataRate)));}
	static	void	SetSlots(const uint16 in)		{SetClientDataRate(in == 1 ? 0 : ((uint16) ((float) ((maxupload == UNLIMITED) ? GetMaxGraphUploadRate(true) : maxupload) * ((float) 1024) / ((float) in))));}
...
private:
	static	bool	m_bAutoDropSystem;
public:
	static	void	SetAutoDropSystem(const bool& in)		{m_bAutoDropSystem = in;}
	static	bool	GetAutoDropSystem()					{return m_bAutoDropSystem;}
...
private:
	...
	//static	SYSTEMTIME m_stIP2CountryVersion;
...
public:
	...
	static	void	SetIP2CountryNameMode(const uint8& in)			{m_uiIP2CountryNameMode = in;}
	static	void	SetIP2CountryShowFlag(const bool& in)			{m_bIP2CountryShowFlag = in;}
	/*
	static	bool	IsAutoUpdateIP2CountryEnabled()					{return m_bIP2CountryAutoUpdate;}
	static	void	SetAutoUpdateIP2Country(const bool& in)			{m_bIP2CountryAutoUpdate = in;}
	...
	static	void	SetIP2CountryUpdateURL(const CString& in)		{_stprintf(m_strIP2CountryUpdateURL, _T("%s"), in);}
	static	void	SetIP2CountryUpdateVerURL(const CString& in)	{_stprintf(m_strIP2CountryUpdateVerURL, _T("%s"), in);}
	static	UINT	GetIP2CountryVersion()							{return m_uiIP2CountryVersion;}
	static	void	SetIP2CountryVersion(const UINT& in)			{m_uiIP2CountryVersion = in;}
	static	LPSYSTEMTIME GetIP2CountryVersion2()					{return &m_stIP2CountryVersion;}
        */
...
private:
	...
	static	uint16	m_iPayBackFirstLimit;
public:
	static	bool	IsPayBackFirst()						{return m_bPayBackFirst;}
	static	uint16	GetPayBackFirstLimit()					{return m_iPayBackFirstLimit;}
	static	void	SetPayBackFirst(const bool& in)			{m_bPayBackFirst = in;}
	static	void	SetPayBackFirstLimit(const uint16& in)	{m_iPayBackFirstLimit = in;}
// ZZUL-TRA :: PaybackFirst :: End

// ZZUL-TRA :: ServerAutoDisconnect :: Start
    static  int     m_iDisconnectTime; 
    static  int     GetDisconnectTime()					{return m_iDisconnectTime; } 
	static  void    SetDisconnectTime(int DisconnectTimeVal) { m_iDisconnectTime = DisconnectTimeVal; } 
	static	bool    m_bAutoDisconnect;
	static	bool	IsAutoDisconnect()							{ return m_bAutoDisconnect;}
	static	void	SetAutoDisconnect(bool in)			{m_bAutoDisconnect=in;}
...
private:
    static bool     m_bEmuLog;
public:
	static  bool    IsEmuLog()                       {return m_bEmuLog;} 
...
private:
	static uint64	UploadedPowershareComplete;
	static uint64	UploadedPowersharePartfile;
public:
	static void		AddupUploadedPowershareComplete(uint64 i)	{UploadedPowershareComplete += i;}
	static void		AddupUploadedPowersharePartfile(uint64 i)	{UploadedPowersharePartfile += i;}
	static uint64	GetUploadedPowershareComplete()				{return UploadedPowershareComplete;}
	static uint64	GetUploadedPowersharePartfile()				{return UploadedPowersharePartfile;}
	static uint64	GetUploadedPowershareTotal()				{return (UploadedPowersharePartfile+UploadedPowershareComplete);}
...
public:
       static  bool m_bShowRuntimeOnTitle;
	   static  bool	ShowRuntimeOnTitle()		{ return m_bShowRuntimeOnTitle; }
// <== Show runtime on title - 

// ZZUL-TRA :: PercentBar :: Start
	   static  bool m_bUsePercentBar;
	   static  bool	UsePercentBar()		{ return m_bUsePercentBar; }
...
public:
	static	bool	IsFunnyNick()						{return m_bFunnyNick;}
	static	void	SetFunnyNick(const bool& in)		{m_bFunnyNick = in;}
...
public:
	...
	static	void	SetUseSessionDownload(const bool& in)	{m_bShowSessionDownload = in;}
// ZZUL-TRA :: SessionDownload :: End

public:
	static  int   m_iClientbantime; //Spike2 - variable ClientBanTime
       static  int   GetClientbantime()					{return m_iClientbantime; } // SPIKE2 - Variable ClientBanTime

	static	bool  m_bShowDownloadColor;
	static	bool  GetDownloadColor()					{return m_bShowDownloadColor;}
// ZZUL-TRA :: SysInfo :: Start
private:
	  static bool   m_bShowCpu;
public:
	  static bool	GetShowCpu()						{ return m_bShowCpu; }
	  static void	SetShowCpu(bool bo)			{m_bShowCpu = bo;}
...
public:
	static	void	CheckZZULTRAPreferences();
