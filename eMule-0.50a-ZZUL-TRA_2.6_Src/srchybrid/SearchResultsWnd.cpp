...
#include "XMessageBox.h" //>>> WiZaRd::Kad Term Cleanup
...
// 	}
	if (pParams->strKeyword.FindOneOf(g_aszInvKadKeywordChars) != -1)
	{
		CString strError;
		strError.Format(GetResString(IDS_KAD_SEARCH_KEYWORD_INVALID), g_aszInvKadKeywordChars);
		...
		strMessage.AppendFormat(GetResString(IDS_KAD_SEARCH_KEYWORD_INVALID_EMF), strStripped);

 		XMSGBOXPARAMS params;
 		int nResult = XMessageBox(NULL, strMessage, GetResString(IDS_ERROR), MB_YESNO | MB_DEFBUTTON1 | MB_ICONWARNING, &params);
		if(nResult == IDYES)
