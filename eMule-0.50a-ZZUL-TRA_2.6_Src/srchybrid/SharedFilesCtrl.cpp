...
#include "uploadqueue.h" //ZZUL-TRA :: PowerShare
...
// ZZUL-TRA :: Spreadbars :: Start
	,IDS_SF_UPLOADED_PARTS
	// ZZUL-TRA :: Spreadbars :: End
	,IDS_ONQUEUE // ZZUL-TRA :: SeeOnQueue
	,IDS_POWERSHARE //ZZUL-TRA :: PowerShare
};
...
void CSharedFilesCtrl::Init()
{
    ...
    // ZZUL-TRA :: Spreadbars :: Start
	InsertColumn(18,GetResString(IDS_SF_UPLOADED_PARTS),LVCFMT_LEFT,  150);
	// ZZUL-TRA :: Spreadbars :: End
	InsertColumn(19,GetResString(IDS_ONQUEUE),			LVCFMT_LEFT,  80); // ZZUL-TRA :: SeeOnQueue
	InsertColumn(20,GetResString(IDS_POWERSHARE),       LVCFMT_LEFT,  100); //ZZUL-TRA :: PowerShare

	SetAllIcons();
	CreateMenues();
	LoadSettings();

    // ZZUL-TRA :: Spreadbars :: Start morph4u partcount
	CFont* pFont = GetFont();
	LOGFONT lfFont = {0};
...
#ifdef EVENLINE	
	InitItemMemDC(dc, lpDrawItemStruct->rcItem, pKnownFile && pKnownFile->IsPartFile()? RGB(255,250,200)/*Partfile*/: pKnownFile && pKnownFile->GetPowerSharedMode()==3 ? RGB(250,230,200)/*PSLimited*/:pKnownFile && pKnownFile->GetPowerShared() ? RGB(255, 210, 210)/*Power*/:((lpDrawItemStruct->itemID % 2)?m_crEvenLine:m_crWindow), lpDrawItemStruct->itemState);
#else
		//colors
        ...
        :
        //partfile color +
        ((pKnownFile && pKnownFile->IsPartFile())?
			RGB(255,250,200) //yellow
		:
		//partfile color -
		//powershare limited +
		((pKnownFile && pKnownFile->GetPowerSharedMode()==3)?
			RGB(250,230,200) //orange
			:
		//powershare limited -
		((pKnownFile && pKnownFile->GetPowerShared())?
			RGB(255,210,210)
		:
		m_crWindow))
		//partfile color +
				...
				switch (iColumn)
				{
					case 0: {
						...
						if (thePrefs.ShowRatingIndicator() && (file->HasComment() || file->HasRating() || file->IsKadCommentSearchRunning()))
						{
							...
							cur_rec.left += 2 + 12; //16->12
							iIconDrawWidth += 2 + 12; //16->12
						}
                    ...
                    //morph4u partcount +
						{
						CString buffer;
							if(pKnownFile->GetPartCount() >=0)
							{
							COLORREF oldclr = dc.SetTextColor(RGB(0,0,0));
							int iOMode = dc.SetBkMode(TRANSPARENT);
                            // ZZUL-TRA :: Start 
							if(thePrefs.IsExtControlsEnabled())
					           buffer.Format(_T("%u"), pKnownFile->GetPartCount());
                            // ZZUL-TRA :: End
								CFont *pOldFont = dc.SelectObject(&m_fontBoldSmaller);
#define DLC_DT_TEXT (DT_LEFT|DT_SINGLELINE|DT_VCENTER|DT_NOPREFIX|DT_END_ELLIPSIS)
#define	DrawPartCount	dc.DrawText(buffer, buffer.GetLength(),&cur_rec, ((DLC_DT_TEXT | DT_RIGHT) & ~DT_LEFT) | DT_CENTER)
								cur_rec.top-=1;cur_rec.bottom-=1;
								DrawPartCount;cur_rec.left+=1;cur_rec.right+=1;
								DrawPartCount;cur_rec.left+=1;cur_rec.right+=1;
								DrawPartCount;cur_rec.top+=1;cur_rec.bottom+=1;
								DrawPartCount;cur_rec.top+=1;cur_rec.bottom+=1;
								DrawPartCount;cur_rec.left-=1;cur_rec.right-=1;
								DrawPartCount;cur_rec.left-=1;cur_rec.right-=1;
								DrawPartCount;cur_rec.top-=1;cur_rec.bottom-=1;
								DrawPartCount;cur_rec.left++;cur_rec.right++;
					...
					case 18:
						...
						pKnownFile->statistic.DrawSpreadBar(dc,&cur_rec,thePrefs.UseFlatBar());
						{
							...
							COLORREF oldclr = dc.SetTextColor(RGB(0,0,0));
							int iOMode = dc.SetBkMode(TRANSPARENT);
                                buffer.Format( _T("%.2f"), pKnownFile->statistic.GetSpreadSortValue()); 
							    //buffer.Format( _T("%.0f%%"), pKnownFile->statistic.GetSpreadSortValue()*100); //changed to %
								CFont *pOldFont = dc.SelectObject(&m_fontBoldSmaller);
#define DLC_DT_TEXT (DT_LEFT|DT_SINGLELINE|DT_VCENTER|DT_NOPREFIX|DT_END_ELLIPSIS)
#define	DrawSpreadbarText	dc.DrawText(buffer, buffer.GetLength(),&cur_rec, ((DLC_DT_TEXT | DT_RIGHT) & ~DT_LEFT) | DT_CENTER)
								cur_rec.top-=1;cur_rec.bottom-=1;
								DrawSpreadbarText;cur_rec.left+=1;cur_rec.right+=1;
								DrawSpreadbarText;cur_rec.left+=1;cur_rec.right+=1;
								DrawSpreadbarText;cur_rec.top+=1;cur_rec.bottom+=1;
								DrawSpreadbarText;cur_rec.top+=1;cur_rec.bottom+=1;
								DrawSpreadbarText;cur_rec.left-=1;cur_rec.right-=1;
								DrawSpreadbarText;cur_rec.left-=1;cur_rec.right-=1;
								DrawSpreadbarText;cur_rec.top-=1;cur_rec.bottom-=1;
								DrawSpreadbarText;cur_rec.left++;cur_rec.right++;                           
								dc.SetTextColor(RGB(255,255,255));
								DrawSpreadbarText;
								dc.SelectObject(pOldFont);
							    dc.SetBkMode(iOMode);
...
//ZZUL-TRA :: PowerShare :: End
            ...
            case 19:
				_sntprintf(pszText, cchTextMax, L"%u", pKnownFile->GetOnUploadqueue());
				break;
			...
			case 20:{
				...
				if(powersharemode == 2){
					...
					buffer.AppendFormat(_T(" [") + GetResString((powershared)?IDS_POWERSHARE_ON:IDS_POWERSHARE_OFF) + _T("]"));
				}else if (powersharemode == 1)
					buffer.Append(GetResString(IDS_ENABLED));
...
//ZZUL-TRA :: PowerShare :: End
	...
	m_SharedFilesMenu.EnableMenuItem(MP_SPREADBAR_RESET, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED); // ZZUL-TRA :: Spreadbars 
...
#endif
// ZZUL-TRA :: Feedback :: Start
	m_SharedFilesMenu.EnableMenuItem(MP_FILEFEEDBACK, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
	m_SharedFilesMenu.EnableMenuItem(MP_FILEFEEDBACK_BB, iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
// ZZUL-TRA :: Feedback :: End
...
BOOL CSharedFilesCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	CTypedPtrList<CPtrList, CKnownFile*> selectedKnown; // ZZUL-TRA :: Feedback
	POSITION pos = GetFirstSelectedItemPosition();
	while (pos != NULL){
...
// ZZUL-TRA :: Feedback :: Start
			...
			case MP_FILEFEEDBACK_BB:
			{
				...
				while(!selectedKnown.IsEmpty())
				{
					if (!str.IsEmpty())
						str.Append(_T("\r\n\r\n"));
					str.Append(selectedKnown.RemoveHead()->GetFeedBackString(wParam == MP_FILEFEEDBACK_BB));
				}
				theApp.CopyTextToClipboard(str);
				break; 
			} 
...
//ZZUL-TRA :: PowerShare :: Start
			...
			case MP_PS_AMOUNT_LIMIT_SET:
			{
				...
				if (wParam==MP_PS_AMOUNT_LIMIT_SET)
				{
					...
					if (pKnownFile)
						currPsAmountLimit.Format(_T("%i"), ((pKnownFile->GetPsAmountLimit()>=0)?pKnownFile->GetPsAmountLimit():thePrefs.GetPsAmountLimit()));
					else
...
int CSharedFilesCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	*/
	...
	// ZZUL-TRA :: PowerShare :: End
				...
				//morph4u partcount sortable +
				case 8:
					iResult = CompareUnsigned64(kitem1->GetPartCount(), kitem2->GetPartCount());
...
//ZZUL-TRA :: PowerShare :: Start
		...
		m_PowershareMenu.AppendMenu(MF_STRING,MP_POWERSHARE_OFF,GetResString(IDS_DISABLED));
		m_PowershareMenu.AppendMenu(MF_STRING,MP_POWERSHARE_ON,GetResString(IDS_ENABLED));
		m_PowershareMenu.AppendMenu(MF_STRING,MP_POWERSHARE_AUTO,GetResString(IDS_PRIOAUTO));
...
//ZZUL-TRA :: PowerShare :: End
    ...
    m_CollectionsMenu.AppendMenu(MF_STRING,MP_CREATECOLLECTION, GetResString(IDS_CREATECOLLECTION));
	m_CollectionsMenu.AppendMenu(MF_STRING,MP_MODIFYCOLLECTION, GetResString(IDS_MODIFYCOLLECTION));
...
//ZZUL-TRA :: PowerShare :: End
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_SPREADBAR_RESET, GetResString(IDS_SPREAD_RESET)); // ZZUL-TRA :: Spreadbars 
 	
    // ZZUL-TRA :: Feedback :: Start
    if(thePrefs.IsExtControlsEnabled())
	m_SharedFilesMenu.AppendMenu(MF_STRING, MP_FILEFEEDBACK, GetResString(IDS_CREATE_FEEDBACK));
	m_SharedFilesMenu.AppendMenu(MF_STRING, MP_FILEFEEDBACK_BB, GetResString(IDS_CREATE_FEEDBACK_BB));
	// ZZUL-TRA :: Feedback :: End
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_FIND, GetResString(IDS_FIND));
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
