...
#include "Sockets.h" // ZZUL-TRA :: ClipStat
#include "Addons/SysInfo/CPU.h" // ZZUL-TRA :: SysInfo
...
CStatisticsDlg::CStatisticsDlg(CWnd* pParent /*=NULL*/)
	...
	, m_DownloadOMeter(3)
	, m_Statistics(4)
	, m_UploadOMeter(6) //5->6 powershare graph
...
void CStatisticsDlg::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("POWERSHARE"))); // 18 // ZZUL-TRA :: PowersahreStat 
#ifdef CLIENTANALYZER
	iml.Add(CTempIconLoader(L"BADGUY"));						// 19
#endif
...
void CStatisticsDlg::RepaintMeters() 
{
	...
	m_UploadOMeter.SetPlotColor(thePrefs.GetStatsColor(15) ,5);//powershare graph
	...
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_POWERSHARESTAT),5);//powershare graph			// Upload powershare slots
...
void CStatisticsDlg::SetCurrentRate(float uploadrate, float downloadrate)
{
	...
	// Upload
	double m_dPlotDataUp[6];//5->6 powershare graph
	...
	m_dPlotDataUp[5] = uploadrate - (float)theApp.uploadqueue->GetDatarateExcludingPowershare() / 1024;//powershare graph
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(down_S[0])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(hdown_spb)) 
					{
						...
						//uint64	PortDataPeerCache =	thePrefs.GetDownDataPort_PeerCache();// X: [RPC] - [Remove PeerCache]
			...
			if (forceUpdate || stattree.IsExpanded(h_down_total)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(down_T[0])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(hdown_tpb)) 
					{
						...
						//uint64	PortDataPeerCache =	thePrefs.GetCumDownDataPort_PeerCache();// X: [RPC] - [Remove PeerCache]
			...
			if (forceUpdate || stattree.IsExpanded(h_up_session)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(up_S[0])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(hup_spb)) 
					{
						...
						//uint64	PortDataPeerCache =	thePrefs.GetUpDataPort_PeerCache();// X: [RPC] - [Remove PeerCache]
			...
			if (forceUpdate || stattree.IsExpanded(h_up_total)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(up_T[0])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(hup_tpb)) 
					{
						...
						//uint64	PortDataPeerCache =	thePrefs.GetCumUpDataPort_PeerCache();// X: [RPC] - [Remove PeerCache]
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hup[mx])) 
					{
						...
						if (forceUpdate || stattree.IsExpanded(time_aap_up[mx][0])) 
						{
							...
							if (forceUpdate || stattree.IsExpanded(time_aap_up_hd[mx][1])) 
							{
								...
								//uint64	PortDataPeerCache =	(uint64)(thePrefs.GetCumUpDataPort_PeerCache() * avgModifier[mx]);// X: [RPC] - [Remove PeerCache]
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hdown[mx])) 
					{
						...
						if (forceUpdate || stattree.IsExpanded(time_aap_down[mx][0])) 
						{
							...
							if (forceUpdate || stattree.IsExpanded(time_aap_down_hd[mx][1])) 
							{
								...
								//uint64	PortDataPeerCache =	(uint64)(thePrefs.GetCumDownDataPort_PeerCache() * avgModifier[mx]);// X: [RPC] - [Remove PeerCache]
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				for (uint32 i = 0; i < MAX_SUB_CLIENT_VERSIONS; i++)
				{
					...
					if (clientMods.Lookup(topver, versionMods)) {
						...
						if (stattree.ItemHasChildren(cli_versions[i])){
							HTREEITEM hChild = stattree.GetChildItem(cli_versions[i]); 
							while (hChild != NULL && mpos != NULL) {
								CString name;
								uint32 count;
								versionMods->GetNextAssoc(mpos, name, count);
								if (name.IsEmpty())
									name.Append(L"Official eMule");
								cbuffer.Format(L"%s: %i (%1.1f%%)", name, count, (double)count/topcnt*100);
				...
				theApp.clientlist->ReleaseModStatistics(&clientMods); // ZZUL-TRA :: ModID
			...
			// ZZUL-TRA :: CompatClientStats :: Start
			...
			if (stattree.ItemHasChildren(clisoft[6])){
				...
				while(hChild != NULL && mpos != NULL){
					compatClients.GetNextAssoc(mpos, strName, uCount);
					cbuffer.Format(L"%s: %i (%1.1f%%)", strName, uCount, (double)uCount/myStats[5]*100);
					stattree.SetItemText(hChild, cbuffer);
					hChild = stattree.GetNextSiblingItem(hChild);
				}
				while (hChild != NULL){
			...
			while(mpos != NULL){
				compatClients.GetNextAssoc(mpos, strName, uCount);
				cbuffer.Format(_T("%s: %i (%1.1f%%)"), strName, uCount, (double)uCount/myStats[5]*100);
				stattree.InsertItem(cbuffer, clisoft[6]);
			}
			// ZZUL-TRA :: CompatClientStats :: End
...
// ZZUL-TRA :: PowersahreStat :: Start
	if (forceUpdate || stattree.IsExpanded(h_PowershareStats))
	{
		cbuffer.Format(L"%s %s", GetResString(IDS_UPPOWERSHARE), CastItoXBytes(thePrefs.GetUploadedPowershareTotal(), false, false)); // total up
		stattree.SetItemText(h_Powershare[0], cbuffer);
		cbuffer.Format(L"%s %s", GetResString(IDS_COMPLETEPOWERSHARE), CastItoXBytes(thePrefs.GetUploadedPowershareComplete(), false, false)); // from complete files
		stattree.SetItemText(h_Powershare[1], cbuffer);
		cbuffer.Format(L"%s %s", GetResString(IDS_INCOMPLETEPOWERSHARE), CastItoXBytes(thePrefs.GetUploadedPowersharePartfile(), false, false)); // from incomplete files
		stattree.SetItemText(h_Powershare[2], cbuffer);
	}
...
void CStatisticsDlg::SetARange(bool SetDownload, int maxValue)
{
	...
	else
	{
		...
		m_UploadOMeter.SetRange(0, maxValue, 5); //powershare graph
	}
}
...
void CStatisticsDlg::CreateMyTree() 
{
...
#ifdef CLIENTANALYZER
	h_Analyzer = stattree.InsertItem(L"ClientAnalyzer", 19, 19);
	...
	// ZZUL-TRA :: PowersahreStat :: Start
	h_PowershareStats = stattree.InsertItem(GetResString(IDS_POWERSHARESTAT), 18, 18);
	for (int i = 0; i <ARRSIZE(h_Powershare); i++)
		h_Powershare[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), h_PowershareStats);
	// ZZUL-TRA :: PowersahreStat :: Start
	...
	stattree.SetItemState(h_PowershareStats, TVIS_BOLD, TVIS_BOLD); // ZZUL-TRA :: PowersahreStat 
...
CString GetHertz(const DWORD dwSpeed)
{
	CString ret = L"";
	if(dwSpeed < 1000)
		ret.Format(L"%uMHz", dwSpeed);
	else if(dwSpeed < 1000*1000)
		ret.Format(L"%.2fGHz", dwSpeed/1000.0f);
	else //*cough*
		ret.Format(L"%.2fTHz", dwSpeed/1000000.0f);
	return ret;
}

CString CStatisticsDlg::CreateModFeedback() const
{
	CString fstats = L"";
	CString temp = L"";

	// System Infos
	fstats.AppendFormat(L"Basic System Infos:\r\n");
	fstats.Append(L"------------------------\r\n");
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_PW_NICK), thePrefs.GetUserNick());
  	fstats.AppendFormat(L"|--> Modversion: %s\r\n", _T("eMule v") + theApp.m_strCurVersionLong);
	fstats.AppendFormat(L"|--> CPU Infos: %s\r\n",theApp.pSysInfo->GetCPUNameString());
	fstats.AppendFormat(L"|--> CPU (Usage): %s (%d%%)\r\n", GetHertz(theApp.pSysInfo->GetCPUSpeed()), theApp.pSysInfo->GetCpuUsage());
	fstats.AppendFormat(L"|--> RAM (Usage): %s (%s)\r\n", CastItoXBytes(theApp.pSysInfo->GetTotalPhys()), CastItoXBytes((UINT)theApp.pSysInfo->GetProcessMemoryUsage(), true));
	fstats.AppendFormat(L"|--> OS Info: %s \r\n",theApp.pSysInfo->GetOSType());
	
	// HDD-Info
	const uint64 ui64FreeBytes = GetFreeTempSpace(-1);
	temp.Format(GetResString(IDS_DWTOT_FS), CastItoXBytes(ui64FreeBytes, false, false));
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	fstats.Append(L"\r\n");

	fstats.AppendFormat(L"Emule Settings:\r\n"); 
	fstats.AppendFormat(L"------------------------\r\n");
	fstats.AppendFormat(L"|--> %s ED2K/KAD : %s/%s\r\n", _T("Network"), theApp.serverconnect->IsConnected() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED), Kademlia::CKademlia::IsConnected() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED));
	fstats.AppendFormat(L"|--> Max. %s: %s (Limit: %s)\r\n", GetResString(IDS_ST_UPLOAD), CastItoXBytes((UINT)thePrefs.GetMaxGraphUploadRate(true), true, true), thePrefs.GetMaxUpload() == UNLIMITED ? GetResString(IDS_PW_UNLIMITED) : CastItoXBytes(thePrefs.GetMaxUpload(), true, true));
	fstats.AppendFormat(L"|--> Max. %s: %s (Limit: %s)\r\n", GetResString(IDS_ST_DOWNLOAD), CastItoXBytes((UINT)thePrefs.GetMaxGraphDownloadRate(), true, true), thePrefs.GetMaxDownload() == UNLIMITED ? GetResString(IDS_PW_UNLIMITED) : CastItoXBytes(thePrefs.GetMaxDownload(), true, true));
	// ZZUL-TRA :: Global Hardlimit :: Start
	if (thePrefs.IsUseGlobalHL())
	{		
	fstats.AppendFormat(L"|--> %s %u\r\n", GetResString(IDS_PRF_DOWN_GHL_CHK), thePrefs.GetGlobalHL());
	}
	else
	// ZZUL-TRA :: Global Hardlimit :: End
	fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_HARDLIMIT), thePrefs.maxsourceperfile);
	fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_PW_MAXC), thePrefs.maxconnections);
	fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_MAXCON5SECLABEL), thePrefs.MaxConperFive);
	fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_MAXHALFOPENCONS), thePrefs.GetMaxHalfConnections());
	fstats.AppendFormat(L"|--> %s (USS): %s\r\n", GetResString(IDS_DYNUP), thePrefs.IsDynUpEnabled() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED));
	temp.Format(L"%s %s", GetResString(IDS_PW_SERVER), GetResString(IDS_PW_FASTSRVCON));
	fstats.AppendFormat(L"|--> %s: %s\r\n", temp, thePrefs.IsSafeServerConnectEnabled() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED));
	fstats.AppendFormat(L"|--> UPnP: %s\r\n", thePrefs.IsUPnPEnabled() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED));
	fstats.Append(L"\r\n");

	//Mod-Settings
	fstats.AppendFormat(L"Mod Settings:\r\n"); //Title
	fstats.AppendFormat(L"------------------------\r\n");

	if(thePrefs.UseCreditSystem()==0)
	  fstats.AppendFormat(L"|--> Credit System: %s\r\n", GetResString(IDS_DISABLED));
  else if (thePrefs.UseCreditSystem()==1)
	  fstats.AppendFormat(L"|--> Credit System: %s\r\n", _T("Lovelace"));
	else 
	  fstats.AppendFormat(L"|--> Credit System: %s\r\n", _T("ClientAnalyzer"));
	
	fstats.AppendFormat(L"|--> Slotfocus: %s\r\n", thePrefs.IsUseSlotFocus() ? GetResString(IDS_ENABLED) : GetResString(IDS_DISABLED));
    //if(thePrefs.UseSaveLoadSources())
		fstats.AppendFormat(L"|--> SLS: %u|%u\r\n", thePrefs.GetActivationLimitSLS(), thePrefs.GetSourcesToSaveSLS());
	//else
	//	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_CLIPSTAT_SLS), GetResString(IDS_DISABLED));

	fstats.Append(L"\r\n");	

	//upload
	fstats.AppendFormat(L"%s Stats:\r\n", GetResString(IDS_PW_CON_UPLBL)); //Title
	fstats.AppendFormat(L"------------------------\r\n");
	const __int64 sessionRunTime = (__int64)((GetTickCount()-theStats.starttime)/1000);
	fstats.AppendFormat(L"|--> %s: %s (%1.1f%%)\r\n", GetResString(IDS_STATS_UPTIME), CastSecondsToLngHM(theStats.GetUploadTime()), (double) (100 * theStats.GetUploadTime()) / sessionRunTime);
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_ST_UPLOAD), CastItoXBytes(theStats.rateUp, true, true));
	fstats.AppendFormat(L"|--> %s %s\r\n", GetResString(IDS_CD_AUP), CastItoXBytes(theStats.GetAvgUploadRate(AVG_SESSION), true, true));
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_STATS_MAXUL), CastItoXBytes(theStats.maxUp, true, true));
    //
	float myAverageUpRate = theStats.GetAvgUploadRate(AVG_SESSION);
	if (myAverageUpRate>theStats.maxUpavg)
	theStats.maxUpavg = myAverageUpRate;
    //
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_STATS_MAXAVGUL), CastItoXBytes(theStats.maxUpavg, true, true));
	fstats.AppendFormat(L"|--> %s %s\r\n", GetResString(IDS_UPPOWERSHARE), CastItoXBytes(thePrefs.GetUploadedPowershareTotal(), false, false)); // total up

	temp.Format(GetResString(IDS_STATS_UDATA), CastItoXBytes(theStats.sessionSentBytes, false, false));
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_ST_ACTIVEU), theApp.uploadqueue->GetUploadQueueLength());
	temp.Format(GetResString(IDS_STATS_WAITINGUSERS), theApp.uploadqueue->GetWaitingUserCount());
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	fstats.Append(L"\r\n");

	//download
	CDownloadQueue::SDownloadStats mStats;  
	theApp.downloadqueue->GetDownloadSourcesStats(mStats);
	fstats.AppendFormat(L"%s Stats:\r\n", GetResString(IDS_PW_CON_DOWNLBL)); //title
	fstats.AppendFormat(L"------------------------\r\n");
	fstats.AppendFormat(L"|--> %s: %s (%1.1f%%)\r\n", GetResString(IDS_STATS_DOWNTIME), CastSecondsToLngHM(theStats.GetDownloadTime()), (double) (100 * theStats.GetDownloadTime()) / sessionRunTime);//down time 
	temp.Format(GetResString(IDS_STATS_ACTDL), mStats.a[1]);
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	fstats.AppendFormat(L"|--> %s: %s\r\n", GetResString(IDS_ST_DOWNLOAD), CastItoXBytes(theStats.rateDown, true, true));
	fstats.AppendFormat(L"|--> %s %s\r\n", GetResString(IDS_CD_ADOWN), CastItoXBytes(theStats.GetAvgDownloadRate(AVG_SESSION), true, true));
	temp.Format(GetResString(IDS_STATS_MAXDL), CastItoXBytes(theStats.maxDown, true, true));
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	temp.Format(GetResString(IDS_STATS_MAXAVGDL), CastItoXBytes(theStats.maxDownavg, true, true));
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	temp.Format(GetResString(IDS_STATS_DDATA), CastItoXBytes(theStats.sessionReceivedBytes, false, false));
	fstats.AppendFormat(L"|--> %s\r\n", temp);

	temp.Format(GetResString(IDS_STATS_FOUNDSRC), mStats.a[0]); 
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_TOOMANYCONNS), mStats.a[9]);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_ONQUEUE), mStats.a[2]);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_ASKING), mStats.a[5] );
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_CONNECTING), mStats.a[7] );
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_NONEEDEDPARTS), mStats.a[4]);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_ASKED4ANOTHERFILE), mStats.a[15]);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_QUEUEFULL), mStats.a[3]);
	fstats.AppendFormat( L"|--> %s: %u\r\n", GetResString(IDS_NOCONNECTLOW2LOW), mStats.a[10] );
#ifndef CLIENTANALYZER    
	fstats.AppendFormat (L"|--> %s: %u\r\n", GetResString(IDS_BANNED), theApp.clientlist->GetBannedCount());
#endif
	fstats.Append(L"\r\n");

	//mix
	fstats.AppendFormat(L"%s:\r\n", GetResString(IDS_SF_STATISTICS));
	fstats.AppendFormat(L"------------------------\r\n");
	if(theStats.sessionReceivedBytes > 0 && theStats.sessionSentBytes > 0){
		// Session
		if (theStats.sessionReceivedBytes<theStats.sessionSentBytes) 
			fstats.AppendFormat(L"|--> %s %.2f : 1\r\n", GetResString(IDS_STATS_SRATIO),(float)theStats.sessionSentBytes/theStats.sessionReceivedBytes);
		else 
			fstats.AppendFormat(L"|--> %s 1 : %.2f\r\n", GetResString(IDS_STATS_SRATIO),(float)theStats.sessionReceivedBytes/theStats.sessionSentBytes);
	} else 
		fstats.AppendFormat(L"|--> %s %s\r\n", GetResString(IDS_STATS_SRATIO), GetResString(IDS_FSTAT_WAITING));

	uint64 bytesLargestFile = 0;
	const uint64 allsize = theApp.sharedfiles->GetDatasize(bytesLargestFile); // Func returns total share size and sets pointeredd uint64 to largest single filesize

	temp.Format(GetResString(IDS_SF_SIZE), CastItoXBytes(allsize, false, false));
	fstats.AppendFormat(L"|--> %s\r\n", temp);

	temp.Format(GetResString(IDS_SHAREDFILESCOUNT), theApp.sharedfiles->GetCount());
	fstats.AppendFormat(L"|--> %s\r\n", temp);

	uint64 ui64TotFileSize = 0;
	uint64 ui64TotBytesLeftToTransfer = 0;
	uint64 ui64TotNeededSpace = 0;
	int iActiveFiles = theApp.downloadqueue->GetDownloadFilesStats(ui64TotFileSize,ui64TotBytesLeftToTransfer,ui64TotNeededSpace);

	temp.Format(GetResString(IDS_DWTOT_NR), iActiveFiles);
	fstats.AppendFormat(L"|--> %s\r\n", temp);

	fstats.AppendFormat(L"|--> %s: %u\r\n" , GetResString(IDS_STATS_COMPDL), thePrefs.GetDownSessionCompletedFiles() );
	...
	theApp.clientlist->GetStatistics(totalclient, myStats,
		...
		clientVersionEMule,
		clientVersionAMule);
	const UINT otherclient = totalclient-myStats[12]-myStats[13]; //this includes IS_IDNEEDED and clients without credits
	fstats.AppendFormat(L"|--> %s: %u (%.1f%%) : %u (%.1f%%) : %u (%.1f%%)\r\n", GetResString(IDS_STATS_SECUREIDENT), myStats[12] , (totalclient)?((double)100*myStats[12] / totalclient):0, myStats[13] , (totalclient)?((double)100*myStats[13] / totalclient ):0, otherclient, (totalclient)?((double)100*otherclient/totalclient):0); 

	UINT statGoodSessions =	(thePrefs.GetDownS_SuccessfulSessions() + mStats.a[1]);// Add Active Downloads
	UINT statBadSessions =	thePrefs.GetDownS_FailedSessions();
	fstats.AppendFormat( L"|--> %s: %u\r\n" , GetResString(IDS_STATS_DLSES) , statGoodSessions + statBadSessions ); 			
	...
	if (statGoodSessions > 0)  
		...
		percentSessions = 0; 
	fstats.AppendFormat( L"|--> %s: %u (%1.2f%%)\r\n" , GetResString(IDS_STATS_SDLSES) , statGoodSessions , percentSessions ); 
	percentSessions = statBadSessions?(100 - percentSessions):0; 
	fstats.AppendFormat( L"|--> %s: %u (%1.2f%%)\r\n" , GetResString(IDS_STATS_FDLSES) , statBadSessions , percentSessions );

	//UL part START 
	statGoodSessions = theApp.uploadqueue->GetSuccessfullUpCount() + theApp.uploadqueue->GetUploadQueueLength(); 
	statBadSessions = theApp.uploadqueue->GetFailedUpCount(); 
	fstats.AppendFormat( L"|--> %s: %u\r\n" , GetResString(IDS_STATS_ULSES) , statGoodSessions + statBadSessions );                 
	if (statGoodSessions > 0)   
		percentSessions = (double) 100 * statGoodSessions / (statGoodSessions + statBadSessions);  
	else 
		percentSessions = 0;  

	temp.Format(GetResString(IDS_STATS_SUCCUPCOUNT), statGoodSessions , percentSessions);
	fstats.AppendFormat(L"|--> %s\r\n", temp);

	percentSessions = statBadSessions?(100 - percentSessions):0;  
	temp.Format(GetResString(IDS_STATS_FAILUPCOUNT), statBadSessions , percentSessions);
	fstats.AppendFormat(L"|--> %s\r\n", temp);
	//UL part END

#ifdef CLIENTANALYZER    
        fstats.Append(L"\r\n");
		fstats.AppendFormat(L"ClientAnalyzer:\r\n"); //Title
        fstats.AppendFormat(L"------------------------\r\n");
        fstats.AppendFormat(L"|--> Nickthieveries: %u\r\n", theApp.antileechlist->GetBadActionCounter(0));
        fstats.AppendFormat(L"|--> Modthieveries: %u\r\n", theApp.antileechlist->GetBadActionCounter(1));
        fstats.AppendFormat(L"|--> File fakes: %u\r\n", theApp.antileechlist->GetBadActionCounter(2));
        fstats.AppendFormat(L"|--> UDP-FNF fakes: %u\r\n", theApp.antileechlist->GetBadActionCounter(3));
        fstats.AppendFormat(L"|--> Fast asks: %u\r\n", theApp.antileechlist->GetBadActionCounter(4));
        fstats.AppendFormat(L"|--> Spams: %u\r\n", theApp.antileechlist->GetBadActionCounter(5));
        fstats.AppendFormat(L"|--> FastXS: %u\r\n", theApp.antileechlist->GetBadActionCounter(6));
        fstats.AppendFormat(L"|--> Mod fakes: %u\r\n", theApp.antileechlist->GetBadActionCounter(7));
        fstats.AppendFormat(L"|--> %s: %u\r\n", GetResString(IDS_BANNED), theApp.clientlist->GetBannedCount());
        fstats.Append(L"\r\n");
#endif
