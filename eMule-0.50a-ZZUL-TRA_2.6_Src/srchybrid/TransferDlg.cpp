...
int CTransferDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	DockControlBar(m_pwndToolbar, AFX_IDW_DOCKBAR_TOP, (LPRECT)NULL); //morph4u changed
...
void CTransferDlg::ShowRessources(const DWORD dwCurTick)
{
	m_pwndTransfer->ShowRessources(dwCurTick);
}
// ZZUL-TRA :: SysInfo :: End

//void CTransferDlg::ShowQueueCount(uint32 number)
void CTransferDlg::ShowQueueCount() // X: [SFH] - [Show IP Filter Hits]
{
	//m_pwndTransfer->ShowQueueCount(number);
	m_pwndTransfer->ShowQueueCount(); // X: [SFH] - [Show IP Filter Hits]
}
...
//Xman end
{
	m_pwndTransfer->UpdateFilesCount(iCount, countsources, countreadyfiles); //Xman see all sources
}
