...
#define	WND1_BUTTON_XOFF	2 //morph4u 8->2
//Xman see all sources
/*
#define	WND1_BUTTON_WIDTH	170
*/
#define	WND1_BUTTON_WIDTH	220
//Xman end
...
#define	WND2_BUTTON_XOFF	2 //morph4u 8->2
//Xman see all sources
/*
#define	WND2_BUTTON_WIDTH	170
*/
#define	WND2_BUTTON_WIDTH	220
//Xman end
...
void CTransferWnd::OnInitialUpdate()
{
    ...
    AddAnchor(IDC_MEMCOUNT,BOTTOM_LEFT); 
	AddAnchor(IDC_MEM,BOTTOM_LEFT); 
...
void CTransferWnd::ShowQueueCount() // X: [SFH] - [Show IP Filter Hits]
{
	TCHAR buffer[100];
	// MORPH START
	/*
	...
	*/
	_sntprintf(buffer, _countof(buffer), _T("%u / %u (%s: %i, ") + GetResString(IDS_STATS_FILTEREDCLIENTS) + _T(")"), theApp.uploadqueue->GetWaitingUserCount(), (thePrefs.GetQueueSize() + max(thePrefs.GetQueueSize()/4, 200)), GetResString(IDS_BANNED), theApp.clientlist->GetBannedCount() , theStats.filteredclients); // X: [SFH] - [Show IP Filter Hits]
	// MORPH END
...
void CTransferWnd::UpdateListCount(EWnd2 listindex, int iCount /*=-1*/)
{
	...
	// ==> Advanced Transfer Window Layout - Stulle [Fafner: fix not updating lists]
#ifdef ATWL
			switch (m_dwTopListIDC) {
				case IDC_DOWNLOADLIST:
					break;
...
// ZZUL-TRA :: SysInfo :: Start
void CTransferWnd::ShowRessources(const DWORD /*dwCurTick*/)
{
	CString buffer = NULL;

    if (thePrefs.GetShowCpu())
	{
	buffer.Format(_T("%d%%"),theApp.pSysInfo->GetCpuUsage());
	SetDlgItemText(IDC_CPU, _T("CPU:"));
	this->GetDlgItem(IDC_CPUCOUNT)->SetWindowText(buffer);

    buffer.Format(_T("%s"),CastItoXBytes((UINT)theApp.pSysInfo->GetProcessMemoryUsage(),true));
	SetDlgItemText(IDC_MEM, _T("RAM:"));
	this->GetDlgItem(IDC_MEMCOUNT)->SetWindowText(buffer);
    }
	else
	{
    buffer.Format(_T(""),theApp.pSysInfo->GetCpuUsage());
	SetDlgItemText(IDC_CPU, _T(""));
	this->GetDlgItem(IDC_CPUCOUNT)->SetWindowText(buffer);

    buffer.Format(_T(""),CastItoXBytes((UINT)theApp.pSysInfo->GetProcessMemoryUsage(),true));
	SetDlgItemText(IDC_MEM, _T(""));
	this->GetDlgItem(IDC_MEMCOUNT)->SetWindowText(buffer);
    }
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{ 
		...
		case MP_CAT_REMOVE: {
			...
			VerifyCatTabSize();
			if (toreload)
			{ // ZZUL-TRA :: AutoSharedFilesUpdater
				theApp.emuledlg->sharedfileswnd->Reload();
...
//ZZUL -
	if (thePrefs.ShowCatTabInfos()) {
		...
		newlabel.Format(_T("%s%s %i/%i"),title, prioStr,dwl,count);
//ZZUL +
    } else {
        newlabel += prioStr;
//ZZUL -
...
// <== Advanced Transfer Window Layout - Stulle
	...
	uploadlistctrl.GetWindowRect(rcDown);
	ScreenToClient(rcDown);
		rcDown.right = rcWnd.right; // jerry :: BySideFrameBarsGone
	rcDown.bottom = rcWnd.bottom - 20;
