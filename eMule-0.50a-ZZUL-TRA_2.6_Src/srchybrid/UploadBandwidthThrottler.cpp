...
UINT UploadBandwidthThrottler::RunInternal() {
    ...
    uint32 rememberedSlotCounter = 0; //no slotfocus
    DWORD lastTickReachedBandwidth = timeGetTime();
	uint32 uLastBlockProcess = timeGetTime() >> 10; // ZZUL-TRA :: BlockRatio
...
// ZZUL-TRA :: SlotControl :: Start
		const UINT uiClientDataRate = thePrefs.GetClientDataRate(); 
...
// ZZUL-TRA :: SlotControl :: Start 
	if(!thePrefs.IsUseSlotFocus()){//switch is for deactivating slotfocus
		    // Equal bandwidth for all slots
            uint32 maxSlot = (uint32)m_StandardOrder_list.GetSize();
			if(maxSlot * uiClientDataRate > allowedDataRate) {
					maxSlot = allowedDataRate/uiClientDataRate;
		//	if(maxSlot > 0 && allowedDataRate/maxSlot < UPLOAD_CLIENT_DATARATE) { 
		//		maxSlot = allowedDataRate/ (UINT)UPLOAD_CLIENT_DATARATE;
			}
...
// ZZUL-TRA :: SlotControl :: Start 
	if(!thePrefs.IsUseSlotFocus()){//switch is for deactivating slotfocus
                    if(thisLoopTick != lastTickReachedBandwidth) {
