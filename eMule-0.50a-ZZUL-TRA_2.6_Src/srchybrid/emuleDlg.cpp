...
BEGIN_MESSAGE_MAP(CemuleDlg, CTrayDialog)
	...
	ON_MESSAGE(WM_HOTKEY, OnHotKey)	// ZZUL-TRA :: InvisibleMode 
...
// ZZUL-TRA :: InvisibleMode :: Start
	...
	// PeerCache DNS
	//ON_MESSAGE(UM_PEERCHACHE_RESPONSE, OnPeerCacheResponse)// X: [RPC] - [Remove PeerCache]
	...
	ON_MESSAGE(TM_FILEIMPORTED, OnFileImported)
	...
	: CTrayDialog(CemuleDlg::IDD, pParent)
{
	...
	m_bInitedCOM = false;
#endif
	b_HideApp = false; // ZZUL-TRA :: InvisibleMode
}
...
// ZZUL-TRA :: InvisibleMode :: Start
	...
	theApp.SetReAskTick(GetTickCount() + MIN2MS(5)); // ZZUL-TRA :: AutoDropImmunity - 5 minutes drop immunity //morph4u 20->5
		...
		switch(theApp.emuledlg->status){
			...
			case 4:{
				...
				if (!bError){ // show the success msg, only if we had no serious error
					...
					Log(LOG_SUCCESS, L"Winsock: Version %d.%d [%.40s] %.40s", HIBYTE( theApp.m_wsaData.wVersion ),LOBYTE(theApp.m_wsaData.wVersion ),
						CString(theApp.m_wsaData.szDescription), CString(theApp.m_wsaData.szSystemStatus));
					if (theApp.m_wsaData.iMaxSockets!=0) 
						Log(LOG_SUCCESS, L"Winsock: Max. Sockets %d", theApp.m_wsaData.iMaxSockets);
					else 
						Log(LOG_SUCCESS, L"Winsock: Unlimited Sockets");	
			...
			case 5:
				...
				break;
			default:
				theApp.SplashHide(SW_SHOW); //ZZUL-TRA :: NewSplashscreen
// ZZUL-TRA :: InvisibleMode :: Start
				if (!thePrefs.m_bInvisibleMode)
...
void CemuleDlg::StopTimer()
{
	...
	theApp.splashscreenfinished = true; //ZZUL-TRA :: NewSplashscreen
...
//XMessageBox +
		{
			theApp.SplashHide(SW_HIDE); //ZZUL-TRA :: NewSplashscreen
...
//<<< WiZaRd::ToolTip-FiX
	...
	if (IsWindowVisible() || bForceAll)
	{
		...
		if (thePrefs.GetMaxDownload() * 1024 != thePrefs.GetMaxDownloadInBytesPerSec(true))
			strTransferRate.Append(_T("  L"));
		// ZZUL-TRA :: SessionRatio :: End
		statusbar->SetText(strTransferRate, SBarUpDown, 0);
		ShowTransferStateIcon();
	}
// ==> Show runtime on title +
    time_t sessionRunTime = (GetTickCount() - theStats.starttime) / 1000; 

	if (IsWindowVisible() && thePrefs.ShowRatesOnTitle() && thePrefs.ShowRuntimeOnTitle())
	{
		TCHAR szBuff[128];
		_sntprintf(szBuff,_countof(szBuff),_T("(U:%.1f D:%.1f) eMule v%s  |  %s  (%s)"),(float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong, thePrefs.GetUserNick(), CastSecondsToLngHM(sessionRunTime));
		szBuff[_countof(szBuff) - 1] = _T('\0');
	...
	else if (IsWindowVisible() && thePrefs.ShowRuntimeOnTitle())
	{
		TCHAR szBuff[128];
		_sntprintf(szBuff,_countof(szBuff),_T("eMule v%s  |  %s  (%s)"), theApp.m_strCurVersionLong, thePrefs.GetUserNick(), CastSecondsToLngHM(sessionRunTime)); 
	    szBuff[_countof(szBuff) - 1] = _T('\0');
...
void CemuleDlg::SetStatusBarPartsSize()
{
	...
	int aiWidths[4] = //5->4
	{ 
		rect.right - 375 - ussShift, //345 -> 375 
		rect.right - 125 - ussShift,
...
//XMessageBox +
		...
		int nResult = XMessageBox(GetSafeHwnd(), GetResString(IDS_MAIN_EXIT), GetResString(IDS_WEB_SHUTDOWN), MB_YESNO | MB_DEFBUTTON1/*MB_DEFBUTTON2*/ | MB_DONOTASKAGAIN | MB_ICONQUESTION, &params);
		//changed: IDS_CLOSEEMULE to IDS_WEB_SHUTDOWN cause IDS_CLOSEEMULE in all morphxt language files comment out cause other exit dialog in morphxt
//XMessageBox -
...
{
	if(theApp.IsSplash()) theApp.DestroySplash(); //ZZUL-TRA :: NewSplashscreen
...
//ZZUL-TRA :: NewSplashscreen :: End
	...
	theApp.EndDirectoryWatcher();	// ZZUL-TRA :: AutoSharedFilesUpdater
	theApp.m_app_state = APP_STATE_SHUTTINGDOWN;
	...
	theApp.UpdateSplash(_T("Waiting for Hash End")); //ZZUL-TRA :: NewSplashscreen

	// try to wait untill the hashing thread notices that we are shutting down
	CSingleLock sLock1(&theApp.hashing_mut); // only one filehash at a time
	sLock1.Lock(2000);

	theApp.UpdateSplash(_T("Saving Settings ...")); //ZZUL-TRA :: NewSplashscreen
...
// ZZUL-TRA :: InvisibleMode :: Start
	...
	theApp.UpdateSplash(_T("Clearing Displayed Items ...")); //ZZUL-TRA :: NewSplashscreen
	...
	theApp.UpdateSplash(_T("Deleting sockets ..."));  //ZZUL-TRA :: NewSplashscreen
	...
	theApp.UpdateSplash(_T("Clearing Lists ...")); //ZZUL-TRA :: NewSplashscreen
	...
	theApp.UpdateSplash(_T("Saving Credits ...")); //ZZUL-TRA :: NewSplashscreen
	delete theApp.clientcredits;	theApp.clientcredits = NULL;	// CClientCreditsList::SaveList
	theApp.UpdateSplash(_T("Clearing Queues ...")); //ZZUL-TRA :: NewSplashscreen
	...
	theApp.UpdateSplash(_T("Unloading IP-Filter ...")); //ZZUL-TRA :: NewSplashscreen
	...
	theApp.UpdateSplash(_T("Unloading IP to Country ..."));  //ZZUL-TRA :: NewSplashscreen
	delete theApp.ip2country;		theApp.ip2country = NULL; // ZZUL-TRA :: IP2Country
	delete theApp.pSysInfo;			theApp.pSysInfo = NULL;			// ZZUL-TRA :: SysInfo
	delete theApp.dlp;              theApp.dlp=NULL; // ZZUL-TRA :: DLP_OwnNickCheck
#ifdef CLIENTANALYZER
//>>> WiZaRd::ClientAnalyzer
...
//<<< WiZaRd::ClientAnalyzer
#endif	
        theApp.UpdateSplash(_T("Shutdown complete")); //ZZUL-TRA :: NewSplashscreen
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
	    ...
	    //open log +
		case MP_HM_OPENLOG:
			ShellExecute(NULL, _T("open"), thePrefs.GetMuleDirectory(EMULE_LOGDIR),NULL, NULL, SW_SHOW); 
			break;
		//open log -
		case MP_HM_CON:
			OnBnClickedConnect();
...
void CemuleDlg::ShowToolPopup(bool toolsonly, bool speedgraph) // X: [SGW] - [SpeedGraphWnd]
{
    ...
    if(!speedgraph){
 //open log +
	menu.AppendMenu(MF_STRING,MP_HM_OPENLOG, GetResString(IDS_OPENLOG) + _T("...")/*, _T("FILEINFO")*/);
	//open log -
    ...
    menu.AppendMenu(MF_SEPARATOR);
	if(!speedgraph){
	menu.AppendMenu(MF_STRING,MP_HM_REMOVEALLBANNEDCLIENTS, GetResString(IDS_SW_REMOVEALLBANNEDCLIENTS)); 
	}
    ...
    if(speedgraph && !IsWindowVisible()||IsIconic()){
        menu.AppendMenu(MF_SEPARATOR);
		menu.AppendMenu(MF_STRING,MP_RESTORE, GetResString(IDS_MAIN_POPUP_RESTORE)/*, _T("RESTOREWINDOW")*/);
	}
    else if (!toolsonly||speedgraph) {
		menu.AppendMenu(MF_SEPARATOR);
		menu.AppendMenu(MF_STRING,SC_MINIMIZE, GetResString(IDS_PW_TRAY)/*, _T("RESTOREWINDOW")*/);
	}
...
LRESULT CemuleDlg::OnKickIdle(UINT /*nWhy*/, long lIdleCount)
{
	LRESULT lResult = 0;
	//ZZUL-TRA :: NewSplashscreen :: Start
	if (theApp.IsSplash() && theApp.splashscreenfinished)
	{
...
void CemuleDlg::UpdateThumbBarButtons(bool initialAddToDlg) {
	...
	for (int i=TBB_FIRST; i<=TBB_LAST;i++) {
		...
		switch(i) {
			case TBB_CONNECT:
				{
//morph4u changed thumbbar button +
