...
void CChatSelector::ProcessMessage(CUpDownClient* sender, const CString& message)
{
	...
	//AddLogLine(true, GetResString(IDS_NEWMSG), sender->GetUserName(), ipstr(sender->GetConnectIP()));	// Tux: Fix: Fix for new msg log entry [Avi3k]
	...
	switch (wParam) {
		case MP_DETAIL:{
			...
			if (ci) {
				...
				dialog.DoModal();
				*/
				ci->client->GetDetailDialogInterface()->OpenDetailDialog();
...
void CChatSelector::OnContextMenu(CWnd*, CPoint point)
{
	...
	GetCurrentChatItem();
	if (pFriend == NULL)
		menu.AppendMenu(MF_STRING, MP_ADDFRIEND, GetResString(IDS_ADDTOFRIENDLIST), _T("ADDFRIEND"));
	else
		menu.AppendMenu(MF_STRING, MP_REMOVEFRIEND, GetResString(IDS_REMOVEFRIEND), _T("DELETEFRIEND"));
	
	menu.AppendMenu(MF_STRING, MP_REMOVE, GetResString(IDS_FD_CLOSE), _T("DELETE"));	// Tux: Improvement: added icon
