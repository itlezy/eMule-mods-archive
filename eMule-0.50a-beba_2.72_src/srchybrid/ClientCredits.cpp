...
#include <crypto56/base64.h>
#include <crypto56/osrng.h>
#include <crypto56/files.h>
#include <crypto56/sha.h>
...
#define CLIENTS_SUQWT2_FILENAME	_T("clients.met.SUQWTv2.met")
#define CLIENTS_BACKUP_FILENAME	CLIENTS_SUQWT2_FILENAME _T(".bak")
...
// Tux: Feature: SUQWT [start]
// Conditions to determine an active record.
// Returns true if the client has been seen recently
bool CClientCredits::IsActive(uint32 dwExpired) {
    return (GetUploadedTotal() || GetDownloadedTotal() || m_pCredits->nSecuredWaitTime || m_pCredits->nSecuredWaitTime) &&
...
{
	CString strFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_SUQWT2_FILENAME;	// Tux: Feature: SUQWT: changed file name
	...
	// Tux: Feature: SUQWT [start]
	// -> added some extra file handling
		// If the SUQWT credits file is not found try to open the backup
		strFileName = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_BACKUP_FILENAME;
		if (!file.Open(strFileName, iOpenFlags, &fexp)) {
			if (fexp.m_cause != CFileException::fileNotFound){
				CString strError(GetResString(IDS_ERR_LOADCREDITFILE));
				TCHAR szError[MAX_CFEXP_ERRORMSG];
			...
			// If the SUQWT credits file is not found try to open the old v30 one
			...
			if (!file.Open(strFileName, iOpenFlags, &fexp)){
				...
				// No credit file available
				AddLogLine(false, GetResString(IDS_ERR_SUQWT_NO_CRED));
				return;
			}
			else
				AddLogLine(false, GetResString(IDS_ERR_SUQWT_OLD_CRED));
		}
		else {
			AddLogLine(false, GetResString(IDS_ERR_SUQWT_BAK_CRED));
			// Copy the backup as the normal file
			if (!::CopyFile(strFileName, thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + CLIENTS_SUQWT2_FILENAME, FALSE))
	...
	try{
		...
		// Tux: Feature: SUQWT [start]
		// If the copy of the backup has not been done previously
		if (strBakFileName == strFileName)
			bCreateBackup = FALSE;
		// Tux: Feature: SUQWT [end]
		...
		const uint32 dwExpired = (uint32)time(NULL) - 12960000; // today - 150 day // Tux: others: fixed warning C4244 [modified]
		uint32 cDeleted = 0;
		uint32 oldImportedCount = 0;	// Tux: Feature: SUQWT
		for (UINT i = 0; i < count; i++){
			...
			if (version == CREDITFILE_VERSION)
				...
				file.Read(&newcstruct->abyKey, sizeof(CreditStruct_30c));
				oldImportedCount++;
			}
			// Tux: Feature: SUQWT [end]
		...
		// Tux: Feature: SUQWT [start]
		if (oldImportedCount > 0)
			AddLogLine(false, GetResString(IDS_CREDITFILEOLDIMPORTED), oldImportedCount);
		// Tux: Feature: SUQWT [end]
	}
...
void CClientCreditsList::SaveList()
{
	...
	CFileException fexp;
	if (!file.Open(name+_T(".SUQWTv2.met"), CFile::modeWrite|CFile::modeCreate|CFile::typeBinary|CFile::shareDenyWrite, &fexp)){	// Tux: Feature: SUQWT: changed line
	...
	// Tux: Feature: SUQWT [start]
	CFile fileBack; // Also open a file to save original 30c format.
	if (!fileBack.Open(name, CFile::modeWrite|CFile::modeCreate|CFile::typeBinary, &fexp)){
		CString strError(GetResString(IDS_ERR_FAILED_CREDITSAVE));
		TCHAR szError[MAX_CFEXP_ERRORMSG];
		if (fexp.GetErrorMessage(szError, ARRSIZE(szError))){
			strError += _T(" - ");
	...
	const uint32 dwExpired = (uint32)time(NULL) - 12960000; // today - 150 day
	// Tux: Feature: SUQWT [end]
	count = 0;
	while (pos)
	{
		...
		// Tux: Feature: SUQWT [start]
		if (cur_credit->IsActive(dwExpired)) {
			cur_credit->SaveUploadQueueWaitTime();
			memcpy(pBuffer+(count*sizeof(CreditStruct)), cur_credit->GetDataStruct(), sizeof(CreditStruct));
			memcpy(pBuffer30c+(count*sizeof(CreditStruct_30c)), (uint8 *)cur_credit->GetDataStruct() + 8, sizeof(CreditStruct_30c));	// SUQWT - Save 0.30c CreditStruct
			count++; 
		}
		// Tux: Feature: SUQWT [end]
	}
	...
	try{
		// Tux: Feature: SUQWT [start]
		uint8 version30c = CREDITFILE_VERSION_30; // Save as old version format
	...
	delete[] pBuffer;
	delete[] pBuffer30c;	// Tux: Feature: SUQWT
}
...
{
	return (m_nMyPublicKeyLen > 0 && m_pSignkey != 0 );	// Tux: Newbie security: Secure identification always on: changed line
}
...
void CClientCredits::SetSecWaitStartTime(uint32 dwForIP)
{
	// Tux: Feature: SUQWT [start]
	// -> changed values
	m_dwUnSecureWaitTime = (uint32)(::GetTickCount() - ((sint64)m_pCredits->nUnSecuredWaitTime) - 1);
	m_dwSecureWaitTime = (uint32)(::GetTickCount() - ((sint64)m_pCredits->nSecuredWaitTime) - 1);
	// Tux: Feature: SUQWT [end]
	m_dwWaitTimeIP = dwForIP;
}
...
void CClientCredits::SaveUploadQueueWaitTime(int iKeepPct) {
    if (m_dwUnSecureWaitTime)
		m_pCredits->nUnSecuredWaitTime = ((GetTickCount() - m_dwUnSecureWaitTime) / 100) * iKeepPct;
    if (m_dwSecureWaitTime)
		m_pCredits->nSecuredWaitTime = ((GetTickCount() - m_dwSecureWaitTime) / 100) * iKeepPct;
    ClearWaitStartTime();
}

// Clear the wait times.
...
uint32 CClientCredits::GetSUQWT(uint32 dwForIP) const {
	if (m_pCredits->nKeySize != 0) {	// this client is a SecureHash Client
		if (GetCurrentIdentState(dwForIP) == IS_IDENTIFIED) // good boy
			return m_pCredits->nSecuredWaitTime;
	}
	return m_pCredits->nUnSecuredWaitTime;
}
// Tux: Feature: SUQWT [end]