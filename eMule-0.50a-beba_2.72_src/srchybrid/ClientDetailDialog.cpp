...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		GetDlgItem(IDC_DSOFT)->SetWindowText(client->DbgGetFullClientSoftVer());	// Tux: Improvement: show full version information: changed line
		...
		buffer = L"?";	// Tux: Feature: Client Analyzer
		if (client->Credits()){
...
IMPLEMENT_DYNAMIC(CClientDetailDialog, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs: changed line

BEGIN_MESSAGE_MAP(CClientDetailDialog, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs: changed line
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
void CClientDetailDialogInterface::OpenDetailDialog(CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CUpDownClient*> aClients;
	aClients.Add(STATIC_DOWNCAST(CUpDownClient, GetOwner()));
	OpenDetailDialog(&aClients, pListCtrl);
}

void CClientDetailDialogInterface::OpenDetailDialog(const CSimpleArray<CUpDownClient*>* paClients, CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CModelessPropertySheetInterface*> aInterfaces;
	for (int i = 0; i < paClients->GetSize(); i++)
		aInterfaces.Add(STATIC_DOWNCAST(CModelessPropertySheetInterface, (*paClients)[i]->GetDetailDialogInterface()));
	OpenPropertySheet(&aInterfaces, paClients, pListCtrl);
}

CModelessPropertySheet* CClientDetailDialogInterface::CreatePropertySheet(va_list args)
{
	...
	return STATIC_DOWNCAST(CModelessPropertySheet, new CClientDetailDialog(paClients, pListCtrl));
}
