...
#include "server.h"	// Tux: Improvement: Inform sources of ID-change
...
CClientList::CClientList(){
	// Tux: Improvement: Relax on startup [start]
...
*/
	...
	m_dwLastTrackedCleanUp = cur_tick + TRACKED_CLEANUP_TIME;
	m_dwLastClientCleanUp =  cur_tick + CLIENTLIST_CLEANUP_TIME;
	// Tux: Improvement: Relax on startup [end]
		...
		if (cur_client->Credits() != NULL)
		{
			switch (cur_client->Credits()->GetCurrentIdentState(cur_client->GetIP()))
			{
				case IS_IDENTIFIED:  
					stats[12]++;  
					break;  
				case IS_IDNEEDED:	// Tux: Fix: Fix by WiZaRd: moved up
					break;  
				case IS_IDFAILED:  
				//case IS_IDNEEDED:	// Tux: Fix: Fix by WiZaRd: moved up
				case IS_IDBADGUY:  
		...
		if (sender){
			if (found_client->socket){
				if (found_client->socket->IsConnected() 
					&& (found_client->GetConnectIP() != tocheck->GetConnectIP() || found_client->GetUserPort() != tocheck->GetUserPort() ) )	// Tux: Fix: use GetConnectIP instead of GetIP
				{
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8 byConnectOptions)
{
	...
	const bool bNew = pNewClient == NULL;	// Tux: Fix: AddClient fix [WiZaRd]
	...
	// Tux: Fix: AddClient fix [WiZaRd] [start]
	/*
	m_KadList.AddTail(pNewClient);
	//This method checks if this is a dup already.
	AddClient(pNewClient);
	*/
	AddToKadList(pNewClient); //just to be sure
	AddClient(pNewClient, bNew);
	// Tux: Fix: AddClient fix [WiZaRd] [end]
	return true;
}
...
void CClientList::RemoveFromKadList(CUpDownClient* torem){
	...
	if(pos)
	{
		...
		{
			m_pBuddy = NULL;
			m_nBuddyStatus = false;	// Tux: Fix: Missing Buddy Check [WiZaRd]
			theApp.emuledlg->serverwnd->UpdateMyInfo();
		}
	...
	if (m_dwLastClientCleanUp + CLIENTLIST_CLEANUP_TIME < cur_tick ){
		...
		for (pos1 = list.GetHeadPosition();( pos2 = pos1 ) != NULL;){
			...
			if ((pCurClient->GetUploadState() == US_NONE || pCurClient->GetUploadState() == US_BANNED && !pCurClient->IsBanned())
				...
				&& !pCurClient->GetDetailDialogInterface()->IsDialogOpen()	// Tux: Feature: Modeless dialogs
...
void CClientList::TrigReaskForDownload(){
	...
	for (POSITION pos = list.GetHeadPosition(); pos;){
		CUpDownClient* cur_client = list.GetNext(pos);
		DWORD ReAskTime = SPREADREASKTIME+FILEREASKTIME;
		...
		if (cur_client->reqfile)
		{
			...
			*/
			// Tux: Improvement: Inform sources of ID change [start]
			// -> eMules/eDonkeys should understand the OP_CHANGE_CLIENT_ID. So we inform them with this opcode. Other clients
			// -> will be reasked, as we can't always know, if they support this opcode.
			bool lowID = (theApp.serverconnect->IsLowID() || Kademlia::CKademlia::IsFirewalled()); // we will not inform but do a complete reask, if we are on LowID

			if ((cur_client->GetClientSoft() == SO_EMULE || cur_client->GetClientSoft() == SO_EMULEPLUS || cur_client->GetClientSoft() == SO_EDONKEY || cur_client->GetClientSoft() == SO_EDONKEYHYBRID) && !lowID)
			{
				if (thePrefs.GetDebugClientTCPLevel() > 0)
					DebugSend("OP__Change_Client_Id", cur_client);
				...
				cur_client->SafeConnectAndSendPacket(packet);
			}
			else
			// Tux: Improvement: Inform sources of ID change [end]
				// reask
		...
		// Tux: Improvement: Inform sources of ID change [start]
		else if (cur_client->GetUploadState() == US_ONUPLOADQUEUE)
		{
			bool lowID = (theApp.serverconnect->IsLowID() || Kademlia::CKademlia::IsFirewalled());
...
// Tux: Feature: Mod Statistics [start]
void CClientList::GetModStatistics(CRBMap<UINT, CRBMap<CString, UINT>* > *clientMods)
{
	...
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		...
		// -> listing mod thieves and fakers here would be pointless
		if (cur_client->GetAntiLeechData() && // avoid a crash :)
		   (cur_client->GetAntiLeechData()->GetBadForThisSession(AT_MODTHIEF) ||
		    cur_client->GetAntiLeechData()->GetBadForThisSession(AT_MODFAKER)))
			continue;
...
void CClientList::ReleaseModStatistics(CRBMap<UINT, CRBMap<CString, UINT>* > *clientMods){
	if (!clientMods)
		return;
...
// Tux: Feature: Mod Statistics [end]
