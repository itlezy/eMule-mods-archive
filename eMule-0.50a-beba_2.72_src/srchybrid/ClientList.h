...
public:
	...
	void	DeleteAll();
	void	ClearBanList();	// Tux: Feature: Clear Ban List
	...
	void TrigReaskForDownload();	// Tux: Feature: Reask sources after IP change

	// Tux: Feature: Mod Statistics [start]
	void	GetModStatistics(CRBMap<UINT, CRBMap<CString, UINT>* > *clientMods);
	void	ReleaseModStatistics(CRBMap<uint32, CRBMap<CString, uint32>* > *clientMods);
	// Tux: Feature: Mod Statistics [end]

	void GetCompatClientsStats(CRBMap<CString, uint32> *compatClients); // Tux: Feature: Enhanced Client Recognition v2
