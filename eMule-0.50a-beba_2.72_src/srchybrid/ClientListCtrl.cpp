...
void CClientListCtrl::GetItemDisplayText(const CUpDownClient *client, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 5:
			_tcsncpy(pszText, client->DbgGetFullClientSoftVer(), cchTextMax);	// Tux: Feature: Mod Version: Using DbgFullClientSoftVer
			if (pszText[0] == _T('\0'))
...
void CClientListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && client->IsBanned()) ? MF_ENABLED : MF_GRAYED), MP_UNBAN, GetResString(IDS_UNBAN), _T("UPLOAD"));	// Tux: Fix: missing unban menu item fix [fox88]
		...
		switch (wParam){
			...
			case MP_LIST_REQUESTED_FILES: {
				if (theApp.clientlist->IsValidClient(client))
				{
					...
					if (theApp.downloadqueue->IsPartFile((CKnownFile*)client->GetRequestFile()))
					{
						...
						{
							if (theApp.downloadqueue->IsPartFile((CKnownFile*)client->m_OtherRequests_list.GetNext(pos1)))
							{
