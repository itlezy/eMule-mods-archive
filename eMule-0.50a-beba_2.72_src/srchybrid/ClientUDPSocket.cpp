		...
		case OP_REASKFILEPING:
		{
			...
			if (sender)
			{
				...
				if (md4cmp(reqfilehash, sender->GetUploadFileID()) == 0)
				{
					...
					{
						sender->ProcessExtendedInfo(&data_in, reqfile, true);	// Tux: Improvement: Better Passive Source Finding: added bool argument
					}
					...
					sender->SetLastL2HACExecution();	// Tux: Feature: L2HAC
					if (thePrefs.GetDebugClientUDPLevel() > 0)
...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32 dwIP, uint16 nPort){
	// NOTE: *** This function is invoked from a *different* thread!
	// Tux: Improvement: SendTo optimization [start]
	...
	// Tux: Improvement: SendTo optimization [end]
