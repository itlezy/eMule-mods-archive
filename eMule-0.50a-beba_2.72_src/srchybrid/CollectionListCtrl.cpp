...
class CCollectionFileDetailsSheet : public CModelessPropertySheet	// Tux: Feature: Modeless dialogs: changed line
{
...
IMPLEMENT_DYNAMIC(CCollectionFileDetailsSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs: changed line

BEGIN_MESSAGE_MAP(CCollectionFileDetailsSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs: changed line
	ON_WM_DESTROY()
...
///////////////////////////////////////////////////////////////////////////////
// CCollectionFileDetailsSheetInterface

CCollectionFileDetailsSheetInterface::CCollectionFileDetailsSheetInterface(CAbstractFile* owner)
	: CModelessPropertySheetInterface(STATIC_DOWNCAST(CObject, owner))
{
}

void CCollectionFileDetailsSheetInterface::OpenDetailSheet(CTypedPtrList<CPtrList, CAbstractFile*>& aFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CModelessPropertySheetInterface*> aInterfaces;
	POSITION pos = aFiles.GetHeadPosition();
	while (pos)
		aInterfaces.Add(STATIC_DOWNCAST(CModelessPropertySheetInterface, aFiles.GetNext(pos)->GetDetailSheetInterface()));
	OpenPropertySheet(&aInterfaces, &aFiles, uPshInvokePage, pListCtrl);
}

CModelessPropertySheet* CCollectionFileDetailsSheetInterface::CreatePropertySheet(va_list args)
{
	CTypedPtrList<CPtrList, CAbstractFile*>* paFiles = (CTypedPtrList<CPtrList, CAbstractFile*>*)va_arg(args, LPVOID);
	UINT uPshInvokePage = va_arg(args, UINT);
	CListCtrlItemWalk* pListCtrl = va_arg(args, CListCtrlItemWalk*);
	return STATIC_DOWNCAST(CModelessPropertySheet, new CCollectionFileDetailsSheet(*paFiles, uPshInvokePage, pListCtrl));
}
...
void CCollectionListCtrl::OnNmRClick(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	if(abstractFileList.GetCount() > 0)
	{
		...
		dialog.DoModal();
		*/
		abstractFileList.GetHead()->GetDetailSheetInterface()->OpenDetailSheet(abstractFileList, 0, this);
		// Tux: Feature: Modeless dialogs [end]
	}
	*pResult = 0;
}
