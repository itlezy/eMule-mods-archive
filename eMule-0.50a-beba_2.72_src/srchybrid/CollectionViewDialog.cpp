...
BEGIN_MESSAGE_MAP(CCollectionViewDialog, CModelessResizableDialog)
...
CCollectionViewDialog::CCollectionViewDialog(CWnd* pParent /*=NULL*/)
	...
	: CModelessResizableDialog(CCollectionViewDialog::IDD, pParent)
	// Tux: Feature: Modeless dialogs [end]
	, m_pCollection(NULL)
	, m_bDeleteThis(false)	// Tux: Feature: Modeless dialogs
{
	m_icoWnd = NULL;
	m_icoColl = NULL;
}
...
void CCollectionViewDialog::SetCollection(CCollection* pCollection, bool bDeleteThis)
// Tux: Feature: Modeless dialogs [end]
{
	if (!pCollection) {
		ASSERT(0);
	...
	m_bDeleteThis = bDeleteThis;	// Tux: Feature: Modeless dialogs
}
