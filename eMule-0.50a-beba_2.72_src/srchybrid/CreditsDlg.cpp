...
#include "Preferences.h"	// Tux: Feature: TITS :D
...
BOOL CCreditsDlg::OnInitDialog() 
{
	...
	srand(time(NULL));
	UINT randTits = (rand() % 12) + 1;
	if (thePrefs.GetEasterEgg() && randTits > 9) { // should be enough
		if (randTits > 11) // a very, very small chance :)
			VERIFY( m_imgSplash.Attach(theApp.LoadImage(_T("TITSSCREEN"), _T("JPG"))) );
		else
			VERIFY( m_imgSplash.Attach(theApp.LoadImage(_T("BIRDSSCREEN"), _T("JPG"))) );
	}
	else
