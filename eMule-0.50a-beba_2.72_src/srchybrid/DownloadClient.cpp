...
void CUpDownClient::SendFileRequest()
{
	...
	if (SupportMultiPacket() || SupportsFileIdentifiers())
	{
		...
		if (IsSourceRequestAllowed())
		{
			...
			m_fSourceExchangeRequested = TRUE;	// Tux: Feature: Client Analyzer
		...
		{
			if (!reqfile->GetAICHRecoveryHashSet()->HasValidMasterHash() || (reqfile->GetAICHRecoveryHashSet()->GetStatus() != AICH_VERIFIED && reqfile->GetAICHRecoveryHashSet()->GetStatus() != AICH_HASHSETCOMPLETE)) {	// Tux: Improvement: request AICH hashset only if necessary
				m_fAICHHashRequested = TRUE;	// Tux: Feature: Client Analyzer
				if (thePrefs.GetDebugClientTCPLevel() > 0)
...
void CUpDownClient::ProcessFileInfo(CSafeMemFile* data, CPartFile* file)
{
	...
	if (!reqfile->DissimilarName() && thePrefs.GetNamecheckenabled())
...
void CUpDownClient::SetDownloadState(EDownloadState nNewState, LPCTSTR pszReason){
	if (m_nDownloadState != nNewState){
		// Tux: Improvement: NoNeededRequeue [WiZaRd] [start]
		...
		// Tux: Improvement: NoNeededRequeue [WiZaRd] [end]
		switch( nNewState )
		{
			case DS_CONNECTING:
	            m_dwLastTriedToConnect = ::GetTickCount();
		...
		bool bBan = false;	// Tux: Feature: Client Analyzer
...
void CUpDownClient::SendBlockRequests()
{
	...
	if(IsEmuleClient() && m_byCompatibleClient==0 && reqfile->GetFileSize()-reqfile->GetCompletedSize() <= (uint64)PARTSIZE*4) {
		...
		// -> doubled min. values
		//Raised threshold as anything below 3kB/s suggest there is a problem
		if (GetDownloadDatarate() < 1200 || GetSessionPayloadDown() < 40*1024)  
			blockCount = 1;
		else if (GetDownloadDatarate() < 2400) 
			blockCount = 2;	
...
void CUpDownClient::UDPReaskFNF()
{
	...
	if (GetDownloadState() != DS_DOWNLOADING){ // avoid premature deletion of 'this' client
		...
		// Tux: Feature: Client Analyzer [start]: UDPFNF Fix
...
bool CUpDownClient::SwapToAnotherFile(LPCTSTR reason, bool bIgnoreNoNeeded, bool ignoreSuspensions, bool bRemoveCompletely, CPartFile* toFile, bool allowSame, bool /*isAboutToAsk*/, bool debug)
{
	...
	bool doAgressiveSwapping = true;	// Tux: Improvement: Arabella Tweak: always set on true
    if(printDebug)
...
uint32 CUpDownClient::GetTimeUntilReask(const CPartFile* file, const bool allowShortReaskTime, const bool useGivenNNP, const bool givenNNP) const {
	...
	if(lastAskedTimeTick != 0) {
		...
		if(allowShortReaskTime || file == reqfile && GetDownloadState() == DS_NONE)
			reaskTime = MIN_REQUESTTIME+m_dwAskDelay;	// Tux: Feature: spreadReask: added m_dwAskDelay
		// Tux: Feature: Client Analyzer [start]
		...
		// Tux: Feature: Client Analyzer / BadClientFlag [start]
		...
		//in that case we have to make an exception for those bad clients
			...
			reaskTime = MIN2MS(29);
		// Tux: Feature: Client Analyzer / BadClientFlag [end]
		...
		if (/*thePrefs.UseLowID2HighIDAutoCallback() && */HasLowID() && GetL2HACSupport())
			reaskTime = FILEREASKTIME;
		else
			reaskTime = FILEREASKTIME+m_dwAskDelay;	// Tux: Feature: spreadReask
		}
