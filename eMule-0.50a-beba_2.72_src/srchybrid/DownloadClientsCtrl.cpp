...
//#include "UploadQueue.h"	// Tux: Improvement: Not actually needed
...
void CDownloadClientsCtrl::GetItemDisplayText(const CUpDownClient *client, int iSubItem, LPTSTR pszText, int cchTextMax)
{
	...
	switch (iSubItem)
	{
		...
		case 1:
			_tcsncpy(pszText, client->DbgGetFullClientSoftVer(), cchTextMax); // Tux: Feature: Mod Version: Using DbgGetFullClientSoftVer
			break;
...
int CDownloadClientsCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	switch (iColumn)
	{
		...
		case 2:	{
			// Tux: Fix: Bugfix by Xman [start]
			// -> using CPartFile instead of CKnownFile
			const CPartFile* file1 = item1->GetRequestFile();
			const CPartFile* file2 = item2->GetRequestFile();
			// Tux: Fix: Bugfix by Xman [end]
