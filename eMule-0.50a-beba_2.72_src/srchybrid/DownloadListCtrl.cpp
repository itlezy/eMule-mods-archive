...
#include "ClientList.h"	// Tux: Feature: List all requested files
...
CDownloadListCtrl::~CDownloadListCtrl()
{
	...
	// Tux: Feature: Drop Menu [start]
	if (m_DropMenu)
		VERIFY( m_DropMenu.DestroyMenu() );
	// Tux: Feature: Drop Menu [end]
	if (m_SourcesMenu)
...
void CDownloadListCtrl::Init()
{
	...
	if (tooltip){
		...
		tooltip->SetDelayTime(TTDT_INITIAL, /*thePrefs.GetToolTipDelay()**/1000);	// Tux: LiteMule: Remove Tooltip Delay: changed line :)
	}
	...
	InsertColumn(6, GetResString(IDS_DL_SOURCES),		LVCFMT_LEFT,  60);	// Tux: others: left looks better :)
...
void CDownloadListCtrl::SetAllIcons()
{
	...
	m_ImageList.Add(CTempIconLoader(_T("Dissimilar_Name")));	// Tux: Feature: Filename disparity check
	badguy = m_ImageList.Add(CTempIconLoader(_T("BADGUY")));	// Tux: Feature: Client Analyzer: Bad guy icon
...
void CDownloadListCtrl::RemoveSource(CUpDownClient* source, CPartFile* owner)
{
	...
	for(ListItems::iterator it = rangeIt.first; it != rangeIt.second; ){
		...
		if(owner == NULL || owner == delItem->owner){
			...
			if (owner == NULL || owner->srcarevisible) { // Tux: Improvement: Add and remove sources only from downloading list if they are visible [dolphinX]
...
bool CDownloadListCtrl::RemoveFile(const CPartFile* toremove)
{
	...
	for(ListItems::iterator it = m_ListItems.begin(); it != m_ListItems.end(); ){
		...
		if(delItem->owner == toremove || delItem->value == (void*)toremove){
			...
			if (delItem->owner == NULL ||  delItem->owner->srcarevisible) { // Tux: Improvement: Add and remove sources only from downloading list if they are visible [dolphinX]
				// Remove it from the CListCtrl
				LVFINDINFO find;
				find.flags = LVFI_PARAM;
				find.lParam = (LPARAM)delItem;
...
void CDownloadListCtrl::UpdateItem(void* toupdate)
{
	...
	// Tux: Improvement: Don't refresh item when not needed [start]
	if (theApp.emuledlg->activewnd != theApp.emuledlg->transferwnd  || theApp.emuledlg->transferwnd->GetDownloadList()->IsWindowVisible() == false)
		return;
	// Tux: Improvement: Don't refresh item when not needed [end]
	...
	for(ListItems::const_iterator it = rangeIt.first; it != rangeIt.second; it++){
		...
		// Tux: Improvement: Add and remove sources only from downloading list if they are visible [dolphinX] [start]
		if (updateItem->owner && !updateItem->owner->srcarevisible)// this line
			continue;
		// Tux: Improvement: Add and remove sources only from downloading list if they are visible [dolphinX] [end]
...
// <-- ZZ:DownloadManager
		...
		case 9:		// remaining time & size
			...
			{
				time_t restTime;
				// Tux: LiteMule: Remove advanced calculation method checkbox [start]
				// -> disabled checks
				...
				// Tux: LiteMule: Remove advanced calculation method checkbox [end]
...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		case 0: {	// file name
			...
			if (pPartFile->DissimilarName() && !thePrefs.GetPaintFDCRed() && thePrefs.GetNamecheckenabled()) {
				m_ImageList.Draw(dc, 21, rcDraw.TopLeft(), ILD_NORMAL);
				rcDraw.left += 16;
			}
...
void CDownloadListCtrl::DrawSourceItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		case 0: {	// icon, name, status
			...
			cur_rec.left += 20;
			// Tux: Feature: Client Analyzer [start]: Bad guy icon
			int plusminus = 0;
			...
			cur_rec.left -= plusminus;	// Tux: Feature: Client Analyzer: Bad guy icon
			break;
		}
...
void CDownloadListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CMuleMemDC dc(CDC::FromHandle(lpDrawItemStruct->hDC), &lpDrawItemStruct->rcItem);
	BOOL bCtrlFocused;
	...
	// Tux: Feature: Filename Disparity Check [start]
	// -> We don't add an ugly icon anymore... we paint it red ;)
	if (content && content->type == FILE_TYPE && thePrefs.GetPaintFDCRed()) {
		CPartFile *lpPartFile = (CPartFile*)content->value;
		if (lpPartFile->DissimilarName() && thePrefs.GetNamecheckenabled()) {
	...
	{
		if ((lpDrawItemStruct->itemState & ODS_SELECTED) == 0) { // Tux: LiteMule: no more low-color support: modified
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			int iFilesInCats = 0;
			int iFilesAHL = 0;	// Tux: Feature: Auto Hard Limit
			UINT uPrioMenuItem = 0;
			CPartFile* pSingleSelFile = NULL;	// Tux: Improvement: Menu Improvement: changed line
			...
			while (pos)
			{
				...
				if (bFirstItem)
					pSingleSelFile = const_cast <CPartFile*> (pFile);	// Tux: Improvement: Menu Improvement: changed line
				iSelectedItems++;
			...
			m_PrioMenu.EnableMenuItem(MP_PRIORELATIVE, iSelectedItems > 1 ? MF_ENABLED : MF_GRAYED);	// Tux: Feature: Relative Priority
			...
			int iPreviewMenuEntries = thePreviewApps.GetAllMenuEntries(PreviewWithMenu, (iSelectedItems == 1) ? pSingleSelFile : NULL);	// Tux: Improvement: Menu Improvement: changed line
			if(thePrefs.IsExtControlsEnabled())
			{
				m_FileMenu.EnableMenuItem((UINT_PTR)m_PreviewMenu.m_hMenu, MF_ENABLED);	// Tux: Improvement: Disable preview submenu if not applicable
			...
			m_FileMenu.EnableMenuItem(MP_FILENAME, (iSelectedItems >= 1 ) ? MF_ENABLED : MF_GRAYED);	// Tux: Feature: Filename disparity check

			// Tux: Feature: Drop Menu [start]
			if (thePrefs.IsExtControlsEnabled()) {	// Tux: Improvement: hide more extended controls
				m_FileMenu.EnableMenuItem((UINT_PTR)m_DropMenu.m_hMenu, (iSelectedItems == 1 && iFilesNotDone == 1 && (iFilesToStop == 1 || iFilesToResume == 1)) ? MF_ENABLED : MF_GRAYED);	// Tux: Improvement: only enabled if actually possible
				m_DropMenu.EnableMenuItem(MP_DROP_NNS, (iSelectedItems == 1 && iFilesNotDone == 1 && (iFilesToStop == 1 || iFilesToResume == 1)) ? MF_ENABLED : MF_GRAYED);
				m_DropMenu.EnableMenuItem(MP_DROP_FULLQR, (iSelectedItems == 1 && iFilesNotDone == 1 && (iFilesToStop == 1 || iFilesToResume == 1)) ? MF_ENABLED : MF_GRAYED);
				m_DropMenu.EnableMenuItem(MP_DROP_MANY, (iSelectedItems == 1 && iFilesNotDone == 1 && (iFilesToStop == 1 || iFilesToResume == 1)) ? MF_ENABLED : MF_GRAYED);
				m_DropMenu.EnableMenuItem(MP_DROP_UNKNOWN, (iSelectedItems == 1 && iFilesNotDone == 1 && (iFilesToStop == 1 || iFilesToResume == 1)) ? MF_ENABLED : MF_GRAYED);
			}
			// Tux: Feature: Drop Menu [end]
			...
			if (m_SourcesMenu/* && thePrefs.IsExtControlsEnabled()*/) {	// Tux NOTE: just to be sure
			...
			//m_FileMenu.EnableMenuItem(MP_SEARCHRELATED, theApp.emuledlg->searchwnd->CanSearchRelatedFiles() ? MF_ENABLED : MF_GRAYED); // Tux: LiteMule: remove Search Related Files
			...
			FriendMenu.AddMenuTitle(NULL, true);	// thx 2 Xanatos
			if (client && client->IsEd2kClient()) {
				if (!client->IsFriend())
					FriendMenu.AppendMenu(MF_STRING, MP_ADDFRIEND, GetResString(IDS_ADDFRIEND), _T("ADDFRIEND"));
				else
					FriendMenu.AppendMenu(MF_STRING, MP_REMOVEFRIEND, GetResString(IDS_REMOVEFRIEND), _T("DELETEFRIEND"));
			}
			FriendMenu.AppendMenu(MF_STRING | (client && client->IsFriend() ? MF_ENABLED : MF_GRAYED), MP_FRIENDSLOT, GetResString(IDS_FRIENDSLOT), _T("FRIENDSLOT"));
			FriendMenu.CheckMenuItem(MP_FRIENDSLOT, (client && client->GetFriendSlot()) ? MF_CHECKED : MF_UNCHECKED);
			FriendMenu.AppendMenu(MF_STRING | (client && client->IsFriend() ? MF_ENABLED : MF_GRAYED), MP_DROP_ALL_FRIENDSLOTS, GetResString(IDS_DROP_ALL_FRIENDSLOTS), _T("FRIENDSLOTREMOVE"));	// Tux: Feature: Remove all friend slots
			ClientMenu.AppendMenu(MF_STRING | MF_POPUP | (client ? MF_ENABLED : MF_GRAYED), (UINT_PTR)FriendMenu.m_hMenu, GetResString(IDS_FRIENDHANDLING), _T("FRIEND"));
			// Tux: Feature: Friend handling [end]

			ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && client->GetDownloadState()!=DS_LOWTOLOWIP) ? MF_ENABLED : MF_GRAYED), MP_MESSAGE, GetResString(IDS_SEND_MSG), _T("SENDMESSAGE"));	// Tux: Improvement: added LowID check
			ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && client->GetViewSharedFilesSupport() && client->GetDownloadState()!=DS_LOWTOLOWIP) ? MF_ENABLED : MF_GRAYED), MP_SHOWLIST, GetResString(IDS_VIEWFILES), _T("VIEWFILES"));	// Tux: Improvement: added LowID check
			...
			ClientMenu.AppendMenu(MF_STRING | (GetItemCount() > 0 ? MF_ENABLED : MF_GRAYED), MP_FIND, GetResString(IDS_FIND), _T("SEARCHMENU"));	// Tux: others: changed icon

			CMenu A4AFMenu;
			A4AFMenu.CreateMenu();
			if (thePrefs.IsExtControlsEnabled()) {
				ClientMenu.AppendMenu(MF_SEPARATOR);	// Tux: others: menu stuff
				ClientMenu.AppendMenu(MF_STRING, MP_LIST_REQUESTED_FILES, GetResString(IDS_LIST_REQFILES), _T("FILEINFO"));	// Tux: Feature: List all requested files
// ZZ:DownloadManager -->
#ifdef _DEBUG
...
// <-- ZZ:DownloadManager
				if (A4AFMenu.GetMenuItemCount()>0)
					ClientMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)A4AFMenu.m_hMenu, GetResString(IDS_A4AF), _T("SRC_A4AF"));	// Tux: Improvement: added A4AF icon
			}
			
			// Tux: Feature: Stop this client [start]
			...
			// Tux: Feature: Stop this client [end]
		...
		if (thePrefs.IsExtControlsEnabled()) {
			// Tux: Improvement: Disable preview submenu if not applicable [start]
			/*
			if (!thePrefs.GetPreviewPrio())
			{
			...
			*/
			...
			// Tux: Improvement: Disable preview submenu if not applicable [end]
        }
		else {
			m_FileMenu.EnableMenuItem(MP_PREVIEW, MF_GRAYED);
		}
		m_FileMenu.EnableMenuItem(MP_METINFO, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_FILENAME, MF_GRAYED);	// Tux: Feature: Filename disparity check
		m_FileMenu.EnableMenuItem(MP_VIEWFILECOMMENTS, MF_GRAYED);

		// Tux: Feature: Drop Menu [start]
		if (thePrefs.IsExtControlsEnabled()) {	// Tux: Improvement: hide more extended controls
			m_FileMenu.EnableMenuItem((UINT_PTR)m_DropMenu.m_hMenu, MF_GRAYED);
			m_DropMenu.EnableMenuItem(MP_DROP_NNS, MF_GRAYED);
			m_DropMenu.EnableMenuItem(MP_DROP_FULLQR, MF_GRAYED);
			m_DropMenu.EnableMenuItem(MP_DROP_MANY, MF_GRAYED);
			m_DropMenu.EnableMenuItem(MP_DROP_UNKNOWN, MF_GRAYED);
		}
		// Tux: Feature: Drop Menu [end]
		...
		//m_FileMenu.EnableMenuItem(MP_SEARCHRELATED, MF_GRAYED); // Tux: LiteMule: remove Search Related Files
		m_FileMenu.EnableMenuItem(MP_FIND, GetItemCount() > 0 ? MF_ENABLED : MF_GRAYED);
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			switch (wParam)
			{
				...
				case MP_STOP:
					...
					while (!selectedList.IsEmpty()){
						...
						if (partfile->CanStopFile()){
							HideSources(partfile);
							partfile->ResetFDC();	// Tux: Feature: Filename disparity check
							partfile->StopFile();
						}
						selectedList.RemoveHead();
					}
				...
				// Tux: Feature: Drop sources [start]
				case MP_DROP_NNS:
					SetRedraw(false);
					while(!selectedList.IsEmpty()){
						selectedList.GetHead()->RemoveNNS();
						selectedList.RemoveHead();
					}
					SetRedraw(true);
					break;
				...
				case MP_DROP_MANY:
					SetRedraw(false);
					while(!selectedList.IsEmpty()){
						selectedList.GetHead()->RemoveQueueTooMany();
						selectedList.RemoveHead();
					}
					SetRedraw(true);
					break;
				case MP_DROP_UNKNOWN:
					SetRedraw(false);
					while(!selectedList.IsEmpty()){
						selectedList.GetHead()->RemoveQueueUnknown();
						selectedList.RemoveHead();
					}
					SetRedraw(true);
					break;
				// Tux: Feature: Drop sources [end]
				case MPG_F2:
				...
				// Tux: LiteMule: remove Search Related Files [start]
				/*
				case MP_SEARCHRELATED:
					...
					break;
				*/
				// Tux: LiteMule: remove Search Related Files [end]
				case MP_OPEN:
				case IDA_ENTER:
					if (selectedCount > 1)
						break;
				...
				case MP_PRIORELATIVE:
				{
					SetRedraw(false);
					theApp.downloadqueue->SetRelativePriorities(&selectedList);
					SetRedraw(true);
					break;
				}
				...
				case MP_LIST_REQUESTED_FILES: {
					if (theApp.clientlist->IsValidClient(client))
					{
						...
						if (theApp.downloadqueue->IsPartFile(client->GetRequestFile()))
						{
							...
							fileList += "\n" ; 
							for (POSITION pos2, pos1 = client->m_OtherRequests_list.GetHeadPosition(); (pos2=pos1)!=NULL; )
							{
								if (theApp.downloadqueue->IsPartFile(client->m_OtherRequests_list.GetNext(pos1)))
								{
									fileList += client->m_OtherRequests_list.GetAt(pos2)->GetFileName(); 
									fileList += "\n" ;
								}
							}
						}
						...
						if (client->R4AF_List.IsEmpty())
							fileList += GetResString(IDS_LORQ_NOUL);
						else
						{
							for (POSITION pos2, pos1 = client->R4AF_List.GetHeadPosition(); (pos2=pos1)!=NULL; )
							{
								if (client->R4AF_List.GetNext(pos1))
								{
									fileList += client->R4AF_List.GetAt(pos2)->GetFileName(); 
									fileList += "\n" ;
								}
								else
									client->R4AF_List.RemoveAt(pos2);
							}
						}
...
void CDownloadListCtrl::CreateMenues()
{
	...
	m_PrioMenu.AppendMenu(MF_STRING, MP_PRIORELATIVE, GetResString(IDS_PRIORELATIVE));
	// Tux: Feature: Relative Priority [end]
	...
	m_FileMenu.AppendMenu(MF_STRING, MP_FILENAME, GetResString(IDS_FILENAMEFDC), _T("FILERENAME"));	// Tux: Feature: Filename disparity check
	...
	if (thePrefs.IsExtControlsEnabled()) {	// Tux: Improvement: hide more extended controls
		...
		m_DropMenu.AddMenuTitle(NULL, true);	// Tux NOTE: thx 2 Xanatos
		m_DropMenu.AppendMenu(MF_STRING,MP_DROP_NNS, GetResString(IDS_DROP_NNS), _T("DROPNNS"));
		m_DropMenu.AppendMenu(MF_STRING,MP_DROP_FULLQR, GetResString(IDS_DROP_FQ), _T("DROPFQ"));
		m_DropMenu.AppendMenu(MF_STRING,MP_DROP_MANY, GetResString(IDS_DROP_TM), _T("DROPTM"));
		m_DropMenu.AppendMenu(MF_STRING,MP_DROP_UNKNOWN, GetResString(IDS_DROP_UN), _T("DROPUN"));
		m_FileMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_DropMenu.m_hMenu, GetResString(IDS_DROP_MENU), _T("DROPMENU"));
	}
	// Tux: Feature: Drop Menu [end]
	...
	if (thePrefs.IsExtControlsEnabled()) {
		...
		// Tux: Feature: Auto Hard Limit [start]
		CString cmd;
		cmd.Format(_T("%s (%s)"), GetResString(IDS_USE_AUTOHL), GetResString(IDS_ON));
		m_SourcesMenu.AppendMenu(MF_STRING, MP_AUTOHL_ON, cmd);
		cmd.Format(_T("%s (%s)"), GetResString(IDS_USE_AUTOHL), GetResString(IDS_OFF));
		m_SourcesMenu.AppendMenu(MF_STRING, MP_AUTOHL_OFF, cmd);
		// Tux: Feature: Auto Hard Limit [end]
		m_FileMenu.AppendMenu(MF_STRING|MF_POPUP, (UINT_PTR)m_SourcesMenu.m_hMenu, GetResString(IDS_A4AF), _T("SRC_A4AF"));	// Tux: Improvement: added A4AF icon
	}
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	...
	// Search commands
	//
	m_FileMenu.AppendMenu(MF_STRING, MP_FIND, GetResString(IDS_FIND), _T("SEARCHMENU"));	// Tux: changed search icon
	//m_FileMenu.AppendMenu(MF_STRING, MP_SEARCHRELATED, GetResString(IDS_SEARCHRELATED), _T("KadFileSearch")); // Tux: LiteMule: remove Search Related Files
...
void CDownloadListCtrl::ShowFileDialog(UINT uInvokePage)
{
	...
	if (aFiles.GetSize() > 0)
	{
		...
		dialog.DoModal();
		*/
		aFiles[0]->GetDetailDialogInterface()->OpenDetailDialog(&aFiles, uInvokePage, this);
		// Tux: Feature: Modeless dialogs [end]
	}
}
...
CObject* CDownloadListListCtrlItemWalk::GetPrevSelectableItem(CObject* pCurrentObj)	// Tux: Feature: Modeless dialogs: added CObject
{
...
*/
		...
		// Tux: Feature: Modeless dialogs [end]
			...
			while (iItem-1 >= 0)
			{
				...
				if (ctrl_item->type == m_eItemType)	// Tux: Feature: Modeless dialogs	// skip A4AF because they won't be recognized on next call
				{
...
CObject* CDownloadListListCtrlItemWalk::GetNextSelectableItem(CObject* pCurrentObj)	// Tux: Feature: Modeless dialogs
{
	ASSERT( m_pDownloadListCtrl != NULL );
	if (m_pDownloadListCtrl == NULL)
		return NULL;
...
void CDownloadListCtrl::ShowClientDialog(CUpDownClient* pClient)
{
	...
	dialog.DoModal();
	*/
	pClient->GetDetailDialogInterface()->OpenDetailDialog(this);
	// Tux: Feature: Modeless dialogs [end]
}
...
bool CDownloadListCtrl::ReportAvailableCommands(CList<int>& liAvailableCommands)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			if (iSelectedItems > 0)
			{
				liAvailableCommands.AddTail(MP_FILENAME);	// Tux: Feature: Filename Disparity Check
...
void CDownloadListCtrl::DropAllFullQ()
{
	POSITION pos = theApp.downloadqueue->GetFileHeadPosition();
	while (pos)
	{
		CPartFile* cur_file=theApp.downloadqueue->GetNextFile(pos);
		if (!cur_file->IsStopped())
			cur_file->RemoveFullQueueSources();
	}
}

void CDownloadListCtrl::DropAllNNS()
{
	POSITION pos = theApp.downloadqueue->GetFileHeadPosition();
	while (pos)
	{
...
void CDownloadListCtrl::DropAllTooMany()
{
	POSITION pos = theApp.downloadqueue->GetFileHeadPosition();
	while (pos)
	{
		CPartFile* cur_file=theApp.downloadqueue->GetNextFile(pos);
		if (!cur_file->IsStopped())
			cur_file->RemoveQueueTooMany();
	}
}

void CDownloadListCtrl::DropUnknown()
{
	POSITION pos = theApp.downloadqueue->GetFileHeadPosition();
	while (pos)
	{
...
void CDownloadListCtrl::StopSingleClient(CUpDownClient* single)  
{
	if (single != NULL && single->GetDownloadState() == DS_DOWNLOADING) {
		...
		single->SetDownloadState(DS_ONQUEUE);
	}
}
// Tux: Feature: Stop this client [end]