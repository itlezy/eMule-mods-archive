...
CDownloadQueue::CDownloadQueue()
{
	...
	m_uiHLCount = 0;	// Tux: Feature: Auto Hard Limit
	m_dwLastRelativePrioTime = ::GetTickCount();	// Tux: Feature: Relative Priority
}
...
{
	// Tux: Fix: check if the file is known [Avi3k] [start]
	if (IsFileExisting(pLink->GetHashKey(), true))
		return;
	// Tux: Fix: check if the file is known [Avi3k] [end]
...
void CDownloadQueue::Process(){
	...
	if (thePrefs.IsAutoRelativePrioEnabled() && (::GetTickCount() - m_dwLastRelativePrioTime) > thePrefs.GetRelativePrioAutoTime())
	{
		SetRelativePriorities();
		m_dwLastRelativePrioTime = ::GetTickCount();
	}
...
#ifdef HAVE_WIN7_SDK_H	// Tux: Improvement: SDK inclusion improvement :)
	theStats.m_fGlobalDone = 0;
	theStats.m_fGlobalSize = 0;
	theStats.m_dwOverallStatus=0;
#endif // ..
	UINT tmp_counter = 0;	// Tux: Feature: Auto Hard Limit
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		if (cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY){
			tmp_counter += cur_file->GetMaxSources();	// Tux: Feature: Auto Hard Limit
	...
	m_uiHLCount = tmp_counter;	// Tux: Feature: Auto Hard Limit
...
bool CDownloadQueue::SendNextUDPPacket()
{
	...
	if (!cur_udpserver)
	{
		while ((cur_udpserver = theApp.serverlist->GetSuccServer(cur_udpserver)) != NULL) {
			...
			if (thePrefs.IsDeadServersEnabled()
				&& !cur_udpserver->IsStaticMember()
...
void CDownloadQueue::CheckDiskspace(bool bNotEnoughSpaceLeft)
{
	...
	if (thePrefs.GetMinFreeDiskSpace() == 0)
	{
		...
		for( POSITION pos1 = filelist.GetHeadPosition(); pos1 != NULL; )
		{
			...
			// Tux: Fix: Check diskspace fix [SLUGFILLER] [start]
			// -> replaced lines
			if (CompareDirectories(cur_file->GetTempPath(), cur_tempdir))
				continue;
			// Tux: Fix: Check diskspace fix [SLUGFILLER] [end]
...
// Tux: Feature: Relative Priority [start]
void CDownloadQueue::SetCatRelativePriorities(UINT nCat)
{
	if (!nCat)
	{
		SetRelativePriorities();
	...
	{
		CPartFile* pFile = filelist.GetNext(pos);
		if (pFile->GetCategory() == nCat)
			selectedList.AddTail(pFile);
	}
	SetRelativePriorities(&selectedList);
}

void CDownloadQueue::SetRelativePriorities(CTypedPtrList<CPtrList, CPartFile*>* pFileList)
{
	if (!pFileList)
		pFileList = &filelist;
	if (pFileList->GetCount() < 2)
		return;
	CPartFile* pFile = NULL;
	POSITION pos = pFileList->GetHeadPosition();
	CTypedPtrList<CPtrList, CPartFile*> listFiles;
	uint32 uSources = 0, uAvgSrcPerFile = 0, iFileSrcCount;
	while (pos)
	{
		pFile = pFileList->GetNext(pos);
		if ((pFile->GetStatus() == PS_READY || pFile->GetStatus() == PS_EMPTY) && pFile->GetSourceCount())
		{
			listFiles.AddTail(pFile);
			uSources += pFile->GetSourceCount();
		}
	}
	if (listFiles.GetCount() < 2)
		return;
	uAvgSrcPerFile = (uint32)((uSources / listFiles.GetCount()) + 0.5f);
	pos = listFiles.GetHeadPosition();
	while (pos)
	{
		pFile = listFiles.GetNext(pos);
		bool bResort = (pos == NULL);
		iFileSrcCount = pFile->GetSourceCount();
		pFile->SetAutoDownPriority(false);
		if (iFileSrcCount < uAvgSrcPerFile/2)
			pFile->SetDownPriority(PR_HIGH, bResort);
		else if (iFileSrcCount > uAvgSrcPerFile*1.5)
			pFile->SetDownPriority(PR_LOW, bResort);
		else
			pFile->SetDownPriority(PR_NORMAL, bResort);
	}
}
// Tux: Feature: Relative Priority [end]