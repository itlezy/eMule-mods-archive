...
IMPLEMENT_DYNAMIC(CFileDetailDialog, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs

BEGIN_MESSAGE_MAP(CFileDetailDialog, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs
	ON_WM_DESTROY()
...
void CFileDetailDialogInterface::OpenDetailDialog(const CSimpleArray<CPartFile*>* paFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CModelessPropertySheetInterface*> aInterfaces;
	for (int i = 0; i < paFiles->GetSize(); i++)
		aInterfaces.Add(STATIC_DOWNCAST(CModelessPropertySheetInterface, (*paFiles)[i]->GetDetailDialogInterface()));
	OpenPropertySheet(&aInterfaces, paFiles, uPshInvokePage, pListCtrl);
}

CModelessPropertySheet* CFileDetailDialogInterface::CreatePropertySheet(va_list args)
{
	const CSimpleArray<CPartFile*>* paFiles = (const CSimpleArray<CPartFile*>*)va_arg(args, LPVOID);
	UINT uPshInvokePage = va_arg(args, UINT);
	CListCtrlItemWalk* pListCtrl = va_arg(args, CListCtrlItemWalk*);
	return STATIC_DOWNCAST(CModelessPropertySheet, new CFileDetailDialog(paFiles, uPshInvokePage, pListCtrl));
}
