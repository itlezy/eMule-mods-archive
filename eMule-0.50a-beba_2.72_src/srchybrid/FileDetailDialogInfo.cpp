...
void CFileDetailDialogInfo::RefreshData()
{
	...
	str.Format(_T("%s (%.1f%%)"), CastItoXBytes(uCompression, false, false), uCompleted!=0 ? (uCompression * 100.0 / uCompleted) : 0.0); // Tux: Fix: accurate compression percentage indication fix [BlueSonicBoy]: changed line
