...
BEGIN_MESSAGE_MAP(CFileDetailDialogName, CResizablePage)
	...
	ON_WM_DESTROY()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_RESETFDC, OnBnClickedResetfdc)	// Tux: Feature: Filename disparity check
END_MESSAGE_MAP()
	...
	: CResizablePage(CFileDetailDialogName::IDD, 0)
{
	...
	m_bSelf = false;
	m_bResetFDC = false;	// Tux: Feature: Filename disparity check
}
...
BOOL CFileDetailDialogName::OnInitDialog()
{
	...
	AddAnchor(IDC_RESETFDC, BOTTOM_RIGHT);	// Tux: Feature: Filename disparity check
...
void CFileDetailDialogName::Localize()
{
	...
	GetDlgItem(IDC_RESETFDC)->SetWindowText(GetResString(IDS_RESETFDC)); // Tux: Feature: Filename disparity check
}
