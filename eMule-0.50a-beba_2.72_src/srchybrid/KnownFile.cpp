...
#include "SharedFilesCtrl.h"	// Tux: Feature: Modeless dialogs
...
CKnownFile::CKnownFile()
{
	...
	onuploadqueue = 0;	// Tux: Feature: see OnUploadqueue
}
...
void CKnownFile::CreateHash(CFile* pFile, uint64 Length, uchar* pMd4HashOut, CAICHHashTree* pShaHashOut, bool slowdown) // Tux: Feature: Nice Hash: added boolean value
{
	ASSERT( pFile != NULL );
	...
	uint32 timeStart = ::GetTickCount();	// Tux: Feature: Nice Hash
	...
	while (Required >= 64){
		...
		Required -= len*64;
		// Tux: Feature: Nice Hash [start]
		if (slowdown && ::GetTickCount() - timeStart >= 100)
		{
			Sleep(30);
			timeStart = ::GetTickCount();
		}
		// Tux: Feature: Nice Hash [end]
	}
...
bool CKnownFile::CreateHash(FILE* fp, uint64 uSize, uchar* pucHash, CAICHHashTree* pShaHashOut, bool slowdown) // Tux: Feature: Nice Hash: added boolean value
{
	bool bResult = false;
	CStdioFile file(fp);
	try
	{
		CreateHash(&file, uSize, pucHash, pShaHashOut, slowdown);	// Tux: Feature: Nice Hash: added boolean value
		bResult = true;
	}
...
bool CKnownFile::CreateHash(const uchar* pucData, uint32 uSize, uchar* pucHash, CAICHHashTree* pShaHashOut, bool slowdown)	// Tux: Feature: Nice Hash: added boolean value
{
	bool bResult = false;
...
Packet*	CKnownFile::CreateSrcInfoPacket(const CUpDownClient* forClient, uint8 byRequestedVersion, uint16 nRequestedOptions) const
{
	...
	// if (result->size > 354)	// Tux: Improvement: Save some overhead
		result->PackPacket();
...
// Tux: Improvement: Preview music files - by IceCream [start]
