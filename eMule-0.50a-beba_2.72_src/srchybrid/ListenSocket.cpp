...
bool CClientReqSocket::CheckTimeOut()
{
	...
	if(client)
	{
		...
		if (/*thePrefs.UseLowID2HighIDAutoCallback() &&*/ (client->HasLowID() || theApp.serverconnect->IsLowID()))
			...
			switch(opcode)
			{
				...
				case OP_FILEREQANSNOFIL:
				{
					...
					if (size == 16)
					{
						...
						*/
						//WiZaRd: check whether he meant his own reqfile
				...
				case OP_STARTUPLOADREQ:
				{
					...
					if (size == 16)
					{
						...
						if (reqfile)
						{
							...
							client->SetUploadFileID(reqfile);
							client->SendCommentInfo(reqfile);
							// Tux: Improvement: Find all sources [start]
							if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64)PARTSIZE)
							{
								if (((CPartFile*)reqfile)->GetMaxSourcePerFileSoft() > ((CPartFile*)reqfile)->GetSourceCount())
									theApp.downloadqueue->CheckAndAddKnownSource((CPartFile*)reqfile, client, true);
							}
							// Tux: Improvement: Find all sources [end]
							theApp.uploadqueue->AddClientToQueue(client);
						}
						else
							client->CheckFailedFileIdReqs(packet);
						break;	// Tux: Fix: Small code fix [WiZaRd]
					}
					// Tux: Fix: Small code fix [WiZaRd] [start]
					// break;
					throw GetResString(IDS_ERR_WRONGPACKAGESIZE);
					// Tux: Fix: Small code fix [WiZaRd] [end]
				}
				case OP_QUEUERANK:
				{
					if (thePrefs.GetDebugClientTCPLevel() > 0)
...
// Tux: Feature: Client Analyzer [end]
				...
				case OP_ANSWERSOURCES2:	// Tux: Feature: Client Analyzer
				{
					// Tux: Feature: Client Analyzer [start]
				...
				case OP_REASKCALLBACKTCP:
				{
					...
					else
					{
						if (!bSenderMultipleIpUnknown){
							if (((uint32)theApp.uploadqueue->GetWaitingUserCount() + 50) > thePrefs.GetQueueSize() && !thePrefs.IsInfiniteQueueEnabled())	// Tux: Feature: Infinite Queue: added enabled check
							{
								if (thePrefs.GetDebugClientUDPLevel() > 0)
