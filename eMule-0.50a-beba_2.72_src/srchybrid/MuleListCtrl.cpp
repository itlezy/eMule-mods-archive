...
CMuleListCtrl::CMuleListCtrl(PFNLVCOMPARE pfnCompare, DWORD dwParamSort)
{
	...
	badguy = -1;	// Tux: Feature: Client Analyzer: Bad guy icon
}
...
void CMuleListCtrl::InitItemMemDC(CMuleMemDC *dc, LPDRAWITEMSTRUCT lpDrawItemStruct, BOOL &bCtrlFocused)
{
...
{
	// Tux: Improvement: Don't draw hidden rect [SiRoB] [start]
	RECT clientRect;
	GetClientRect(&clientRect);
	CRect rcItem(lpDrawItemStruct->rcItem);
	if (rcItem.top >= clientRect.bottom || rcItem.bottom <= clientRect.top)
		return;
	// Tux: Improvement: Don't draw hidden rect [SiRoB] [end]
		
	//set up our flicker free drawing
	//CRect rcItem(lpDrawItemStruct->rcItem);	// Tux: Improvement: Don't draw hidden rect [SiRoB]
	CDC *oDC = CDC::FromHandle(lpDrawItemStruct->hDC);
	CMuleMemDC pDC(oDC, &rcItem, m_crWindow);
