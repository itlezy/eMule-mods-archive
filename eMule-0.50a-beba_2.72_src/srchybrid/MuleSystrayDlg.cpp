...
BOOL CMuleSystrayDlg::OnInitDialog() 
{
	...
	if(p)
	{
		...
		m_ctrlConnect.m_hIcon = theApp.LoadIcon(_T("TRAYCONNECT"), m_ctrlConnect.m_sIcon.cx, m_ctrlConnect.m_sIcon.cy);	// Tux: others: changed icon
	...
	if(p)
	{
		...
		m_ctrlDisconnect.m_hIcon = theApp.LoadIcon(_T("TRAYDISCONNECT"), m_ctrlDisconnect.m_sIcon.cx, m_ctrlDisconnect.m_sIcon.cy);	// Tux: others: changed icon
	...
	if(p)
	{
		...
		m_ctrlPreferences.m_hIcon = theApp.LoadIcon(_T("TWEAK"), m_ctrlPreferences.m_sIcon.cx, m_ctrlPreferences.m_sIcon.cy);	// Tux: others: changed icon
	...
	p = GetDlgItem(IDC_OPENINC);
	if(p)
	{
		p->GetWindowRect(r);
		ScreenToClient(r);
		m_ctrlOpenInc.Create(NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_OPENINC);
		m_ctrlOpenInc.m_nBtnID = IDC_OPENINC;
		//p->GetWindowText(m_ctrlOpenInc.m_strText);
		m_ctrlOpenInc.m_strText = GetResString(IDS_OPENINC);
		m_ctrlOpenInc.m_strText.Remove(_T('&'));

		m_ctrlOpenInc.m_bUseIcon = true;
		m_ctrlOpenInc.m_sIcon.cx = 16;
		m_ctrlOpenInc.m_sIcon.cy = 16;
		m_ctrlOpenInc.m_hIcon = theApp.LoadIcon(_T("INCOMING"), m_ctrlOpenInc.m_sIcon.cx, m_ctrlOpenInc.m_sIcon.cy);
		m_ctrlOpenInc.m_bParentCapture = true;
		if(bValidFont)
			m_ctrlOpenInc.m_cfFont.CreateFontIndirect(&lfStaticFont);
	}
	...
	if (winver == _WINVER_95_ || winver == _WINVER_NT4_) // Tux: LiteMule: no more low-color support: modified
	{
