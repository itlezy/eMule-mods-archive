...
void CMuleToolbarCtrl::Init(void)
{
	...
	{		
		...
		if (TBButtons[i].idCommand == TBBTN_CONNECT) // 'Connect' button has 3 states
			iBitmap += 3;
		// Tux: LiteMule: skip removed buttons [start]
		// -> IRC
		else if (TBButtons[i].idCommand == TBBTN_MESSAGES)
			iBitmap += 2;
		// -> Help
		//else if (TBButtons[i].idCommand == TBBTN_TOOLS)
		//	iBitmap += 2;
		// Tux: LiteMule: skip removed buttons [end]
		else
			iBitmap += 1;
	}
...
void CMuleToolbarCtrl::ChangeToolbarBitmap(const CString& path, bool bRefresh)
{
	...
	if (!bResult)
	{
		...
		ImageList.Add(CTempIconLoader(_T("CONNECT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));	// Tux: LiteMule: skipped later, so we choose a random button (was: IRC)
		...
		ImageList.Add(CTempIconLoader(_T("CONNECT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));	// Tux: LiteMule: skipped later, so we choose a random button (was: HELP)
