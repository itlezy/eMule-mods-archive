...
{
	guardian = false;
	bCreated = false;	// Tux: Improvement: Speed guide
}
...
BOOL CPPgConnection::OnInitDialog()
{
	...
	CRect rect;
	GetDlgItem(IDC_SPEEDGUIDE)->GetWindowRect(rect);
	::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rect, 2);
	m_wndSpeedGuide.CreateEx(NULL,0,_T("MsgWnd"),WS_BORDER | WS_VISIBLE | WS_CHILD | HTC_WORDWRAP | HTC_UNDERLINE_HOVER,rect.left,rect.top,rect.Width(),rect.Height(),m_hWnd,0);
	m_wndSpeedGuide.SetBkColor(::GetSysColor(COLOR_3DFACE)); // still not the right color, might fix this later (need to merge the .rc file before it changes ;))
	m_wndSpeedGuide.SetFont(GetFont());
	if (!bCreated){
		bCreated = true;
		m_wndSpeedGuide.AppendHyperLink(GetResString(IDS_INFO_SPEED_LINK),0,CString(GetResString(IDS_INFO_SPEED_LINK)),0,0);
	}
	m_wndSpeedGuide.SetDefaultCursor(LoadCursor(NULL,IDC_HAND));
...
BOOL CPPgConnection::OnApply()
{
	...
	// Tux: Improvement: auto-switch default search type [start]
	if (thePrefs.networkkademlia && !thePrefs.networked2k)
		thePrefs.SetSearchMethod(SearchTypeKademlia);
	// Tux: Improvement: auto-switch default search type [end]
...
{	
	if (m_hWnd)
	{
		...
		// Tux: Improvement: Speed guide [start]
		GetDlgItem(IDC_INFO_SPEED1)->SetWindowText(GetResString(IDS_INFO_SPEED1));
		GetDlgItem(IDC_INFO_SPEED2)->SetWindowText(GetResString(IDS_INFO_SPEED2));
		GetDlgItem(IDC_SPEEDGROUP)->SetWindowText(GetResString(IDS_SPEEDGROUP));
		// Tux: Improvement: Speed guide [end]
	}
}

// Tux: LiteMule: Remove Wizard: removed code block
