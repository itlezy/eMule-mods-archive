...
#include "SearchResultsWnd.h"	// Tux: Fix: Double-click behavior fix [WiZaRd]
...
// #define MAX_TOOLTIP_DELAY_SEC	32	// Tux: LiteMule: Remove Tooltip Delay
...
BEGIN_MESSAGE_MAP(CPPgDisplay, CPropertyPage)
	...
	// ON_EN_CHANGE(IDC_TOOLTIPDELAY, OnSettingsChange)	// Tux: LiteMule: Remove Tooltip Delay
	ON_WM_HSCROLL()
	...
	// ON_BN_CLICKED(IDC_WIN7TASKBARGOODIES, OnSettingsChange) // Tux: LiteMule: Win7 addons always enabled
...
// Tux: LiteMule: Win7 addons always enabled [end]
	...
	// SetDlgItemInt(IDC_TOOLTIPDELAY, thePrefs.m_iToolDelayTime, FALSE);	// Tux: LiteMule: Remove Tooltip Delay
}
...
{
	//TCHAR buffer[510];	// Tux: LiteMule: Remove Tooltip Delay
...
#endif
*/
	theApp.emuledlg->EnableTaskbarGoodies(true);
// Tux: LiteMule: Win7 addons always enabled [end]
	...
	if (thePrefs.m_bDisableKnownClientList != (IsDlgButtonChecked(IDC_DISABLEKNOWNLIST) != 0)) {
		...
		// Tux: Client/Queue list fix by Xman [start]
		// -> replaced two lines
		if (thePrefs.m_bDisableKnownClientList)
		{
			bListDisabled = true;
			theApp.emuledlg->transferwnd->GetClientList()->DeleteAllItems();
		}
		else
			theApp.emuledlg->transferwnd->GetClientList()->ShowKnownClients();
		// Tux: Client/Queue list fix by Xman [end]
		bResetToolbar = true;
	}
	...
	if (thePrefs.m_bDisableQueueList != (IsDlgButtonChecked(IDC_DISABLEQUEUELIST) != 0)) {
		...
		{
			bListDisabled = true;
			theApp.emuledlg->transferwnd->GetQueueList()->DeleteAllItems();
		}
		else
	...
	*/
	...
	theApp.emuledlg->searchwnd->m_pwndResults->searchlistctrl.SetStyle();	// Tux: Fix: Double-click behavior fix [WiZaRd]
	if (bListDisabled)
...
void CPPgDisplay::Localize(void)
{
	if(m_hWnd)
	{
		...
		//GetDlgItem(IDC_TOOLTIPDELAY_LBL)->SetWindowText(GetResString(IDS_PW_TOOL));	// Tux: LiteMule: Remove Tooltip Delay
		...
		// GetDlgItem(IDC_WIN7TASKBARGOODIES)->SetWindowText(GetResString(IDS_SHOWWIN7TASKBARGOODIES)); // Tux: LiteMule: Win7 addons always enabled
	}
}
