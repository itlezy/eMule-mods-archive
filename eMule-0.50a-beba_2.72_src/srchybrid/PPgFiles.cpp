...
BEGIN_MESSAGE_MAP(CPPgFiles, CPropertyPage)
	// ON_BN_CLICKED(IDC_PF_TIMECALC, OnSettingsChange)	// Tux: LiteMule: Remove advanced calculation method checkbox
	...
	// ON_BN_CLICKED(IDC_FULLCHUNKTRANS, OnSettingsChange)	// Tux: Newbie security: remove TransferFullChunks
...
void CPPgFiles::LoadSettings(void)
{
	...
	// Tux: LiteMule: Remove advanced calculation method checkbox
	...
	// Tux: Newbie security: remove TransferFullChunks - code removed
...
void CPPgFiles::Localize(void)
{
	if (m_hWnd)
	{
		...
		// GetDlgItem(IDC_FULLCHUNKTRANS)->SetWindowText(GetResString(IDS_FULLCHUNKTRANS));	// Tux: Newbie security: remove TransferFullChunks
