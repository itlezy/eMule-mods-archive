...
BEGIN_MESSAGE_MAP(CPPgServer, CPropertyPage)
	ON_BN_CLICKED(IDC_REMOVEDEADSERVERS, OnSettingsChange)	// Tux: Feature: Disable server deletion
	...
	// ON_BN_CLICKED(IDC_SAFESERVERCONNECT, OnSettingsChange)	// Tux: Newbie security: remove SafeServerConnect
...
{
	CheckDlgButton(IDC_REMOVEDEADSERVERS, thePrefs.m_bDeleteDeadServers);	// Tux: Feature: Disable server deletion
	...
	// CheckDlgButton(IDC_SAFESERVERCONNECT, thePrefs.m_bSafeServerConnect);	// Tux: Newbie security: remove SafeServerConnect
...
{
	thePrefs.m_bDeleteDeadServers = IsDlgButtonChecked(IDC_REMOVEDEADSERVERS)!=0;	// Tux: Feature: Disable server deletion
	...
	// thePrefs.SetSafeServerConnectEnabled(IsDlgButtonChecked(IDC_SAFESERVERCONNECT)!=0);	// Tux: Newbie security: remove SafeServerConnect
...
void CPPgServer::Localize(void)
{
	if (m_hWnd)
	{
		...
		// GetDlgItem(IDC_SAFESERVERCONNECT)->SetWindowText(GetResString(IDS_PW_FASTSRVCON));	// Tux: Newbie security: remove SafeServerConnect
