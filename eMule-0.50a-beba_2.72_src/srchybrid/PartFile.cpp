...
#define FT_AUTOHL "AUTOHL"	// Tux: Feature: Auto Hard Limit
...
void CPartFile::Init(){
	...
	iNamecontinuitybad = 0;	// Tux: Feature: Filename disparity check
	m_uTransferred = 0;
	...
	m_detailDialogInterface = new CFileDetailDialogInterface(this);	// Tux: Feature: Modeless dialogs
}
	...
	try{
			...
			if (pOutCheckFileFormat == NULL || (pOutCheckFileFormat != NULL && (newtag->GetNameID()==FT_FILESIZE || newtag->GetNameID()==FT_FILENAME))){
			    switch (newtag->GetNameID()){
				   ...
				   // statistics
				    ...
				    default:{
						// Tux: Feature: Auto Hard Limit [start]
						if (newtag->GetNameID() == 0 && strcmp(newtag->GetName(), FT_AUTOHL) == 0)
						{
							ASSERT( newtag->IsInt() );
							if (newtag->IsInt())
		...
		if (bHadAICHHashSetTag)
		{
			if (!m_FileIdentifier.VerifyAICHHashSet())
				if (thePrefs.GetLogAICHEvents())	// Tux: Improvement: log AICH events
...
bool CPartFile::SavePartFile(bool bDontOverrideBak)
{
	...
	try{
		...
		// if (m_uMaxSources)	// Tux: Feature: Auto Hard Limit
		{
	...
	for (POSITION pos =  requestedblocks_list.GetHeadPosition();pos != 0; ){
        ...
        if ((start <= cur_block->EndOffset) && (end >= cur_block->StartOffset)) {
            if(start < cur_block->StartOffset) {
				...
				// Tux: Improvement: Improvement by NetFinity: removed one byte request
            }
...
bool CPartFile::GetNextEmptyBlockInPart(UINT partNumber, Requested_Block_Struct *result, uint64 bytesToRequest) const	// Tux: Feature: Dynamic Block Requests: added bytesToRequest
{
	Gap_Struct *firstGap;
	...
	for (;;)
	{
		...
		// Tux: Feature: Dynamic Block Requests [start]
		...
		// Tux: Feature: Dynamic Block Requests [end]
		...
		if(shrinkSucceeded) {
		// Tux: Fix: uint64 fix [start]: replaced line
...
void CPartFile::DrawStatusBar(CDC* dc, LPCRECT rect, bool bFlat) /*const*/
{
	...
	// Tux: LiteMule: no more low-color support [start]
/*	if (g_bLowColorDesktop)
	...
	{
*/
	// Tux: LiteMule: no more low-color support [end
		if (bFlat)
			crProgress = RGB(0, 150, 0);
		else
			crProgress = RGB(0, 224, 0);
...
uint32 CPartFile::Process(uint32 reducedownload, UINT icounter/*in percent*/)
{
	...
	if ((m_nTotalBufferData && m_nTotalBufferData > thePrefs.GetFileBufferSize()) || (thePrefs.GetFileBufferTimeLimit() && dwCurTick > m_nLastBufferFlushTime + thePrefs.GetFileBufferTimeLimit()))
	...
	else
	{
		...
		memset(net_stats,0,sizeof(net_stats));
		UINT nCountForState;
		uint16 tmp_counter = 0;	// Tux: Feature: Auto Hard Limit
		...
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;)
		{
			...
			{
				case DS_DOWNLOADING:{
					tmp_counter++;	// Tux: Feature: Auto Hard Limit
				...
				case DS_ONQUEUE:
				{
					...
					if (/*(thePrefs.UseLowID2HighIDAutoCallback() == 1) &&*/ cur_src->GetL2HACSupport()
						&& cur_src->HasLowID()
		...
		m_uiSrcCount = tmp_counter;	// Tux: Feature: Auto Hard Limit
	}
...
void CPartFile::CheckFilename(CString FileName)
{
	...
	for (int i=0;i<len;i++)
	{
		...
		if (tc>(wchar_t)0xdf && tc<(wchar_t)0xe6)
			tc=(wchar_t)'a';
		else
		if (tc==(wchar_t)0x0e )
			tc=(wchar_t)'c';
		else
		if (tc>(wchar_t)0xe7 && tc<(wchar_t)0xec)
			tc=(wchar_t)'e';
		else
		if(tc>(wchar_t)0xeb && tc<(wchar_t)0xf0)
			tc=(wchar_t)'i';
		else
		if(tc==(wchar_t)0xf1)
			tc=(wchar_t)'n';
		else
		if(tc>(wchar_t)0xf1 && tc<(wchar_t)0xf7)
			tc=(wchar_t)'o';
		else
		if(tc>(wchar_t)0xf8 && tc<(wchar_t)0xfd)
			tc=(wchar_t)'u';
		else
		if(tc>(wchar_t)0xfc && tc<(wchar_t)0x100)
			tc=(wchar_t)'y';
		else
		//End - convert accents
	...
	if( (OurFilename.Right(4) == ".htm" && FileName.Right(5) == ".html") ||
		...
		(OurFilename.Right(4) == ".jpg" && FileName.Right(5) == ".jpeg") )
		isnotlongext = false;
	...
	if(FileName.Right(4)==OurFilename.Right(4) &&
		(OurFilename.Right(4)==".avi" || OurFilename.Right(4)==".wmv"))
		bothnotvideo = false;
	...
	if( OurFilename.Right(4) != FileName.Right(4) && 
		(OurFilename.GetAt(OurFilename.GetLength() - 4)==_T('.') && FileName.GetAt(FileName.GetLength() - 4)==_T('.')) )
		nontrivialword = 1;
	...
	//compare non-matched words to 75% words.
	if ((uint16)(nontrivialword - matched) > (uint16)((float)nontrivialword/(float)(100.0F/((float)thePrefs.FDCSensitivity))))  iNamecontinuitybad++; //set warning flag for this file
}

bool CPartFile::DissimilarName(void)
{
	// Tux NOTE: removed doubleFDC things...
	if (iNamecontinuitybad > 1)	return true; 
	else return false;
} 
...
void CPartFile::AddSource(LPCTSTR pszURL, uint32 nIP)
{
	...
	// Tux: Improvement: Ban check [start]
		...
		if( cur_src->GetPartStatus() )
		{
			for (UINT i = 0; i < partcount; i++)
			{
				if (cur_src->IsPartAvailable(i))
					m_SrcpartFrequency[i] = m_SrcpartFrequency[i] + (uint16)weight;	// Tux: Feature: Better chunk selection: changed!
			}
			if ( flag )
			{
...
BOOL CPartFile::PerformFileComplete() 
{
	...
	m_sourcesaver.DeleteFile(this);	// Tux: Feature: SLS
...
bool CPartFile::IsPreviewableFileType() const {
    return IsArchive(true) || IsMovie() || IsMusic();	// Tux: Improvement: Preview music files: added IsMusic()
}
...
{
	m_iUpdateHL = NULL;	// Tux: Feature: Auto Hard Limit
...
bool CPartFile::IsReadyForPreview() const
{
	...
	// Tux: Improvement: Preview music files [start]
	if (
		IsMusic() &&
		...
		((GetFreeDiskSpaceX(GetTempPath()) + 100000000) > (2*(uint64)GetFileSize()))
	)
		return true;
	// Tux: Improvement: Preview music files [end]
...
void CPartFile::UpdateAutoDownPriority(){
	if( !IsAutoDownPriority() || status != PS_READY || paused || insufficient || stopped ) // Tux: Improvement: Code improvement
		return;
									  ...
									  uint16* count) /*const*/
{
	...
	uint32	fileDatarate = max(GetDatarate(), 3072); // Always assume file is being downloaded at atleast 3 kB/s
	...
	while(newBlockCount != *count){
		...
		if(tempLastPartAsked != (uint16)-1){
			...
			if(GetNextEmptyBlockInPart(tempLastPartAsked, pBlock, bytesPerRequest) == true){	// Tux: Feature: Dynamic Block Requests: added bytesPerRequest
			...
			if(chunksList.IsEmpty() == TRUE){
				...
				for(uint16 i = 0; i < partCount; i++){
					if(sender->IsPartAvailable(i) == true 
						&& GetNextEmptyBlockInPart(i, NULL, bytesPerRequest) == true){// Tux: Feature: Dynamic Block Requests: changed line
...
// Tux: Feature: Auto Hard Limit: moved down...

// Tux: Feature: Drop sources [start]
void CPartFile::RemoveNNS()
{
	POSITION pos1, pos2;
	if (!srclist.IsEmpty()) {
		for (pos1 = srclist.GetHeadPosition(); (pos2 = pos1) != NULL; ) {
			srclist.GetNext(pos1);
			if (srclist.GetAt(pos2)->GetDownloadState() == DS_NONEEDEDPARTS) {
				if (!srclist.GetAt(pos2)->SwapToAnotherFile(false, false, false, NULL)) {
					theApp.clientlist->m_globDeadSourceList.AddDeadSource(srclist.GetAt(pos2));
...
void CPartFile::RemoveFullQueueSources()
{
	...
	if (!srclist.IsEmpty()) {
		for (pos1 = srclist.GetHeadPosition(); (pos2 = pos1) != NULL; ) {
			srclist.GetNext(pos1);
			if (srclist.GetAt(pos2)->IsRemoteQueueFull() && srclist.GetAt(pos2)->GetDownloadState() == DS_ONQUEUE) {
				theApp.clientlist->m_globDeadSourceList.AddDeadSource(srclist.GetAt(pos2));
				theApp.downloadqueue->RemoveSource(srclist.GetAt(pos2));
			}
		}
...
void CPartFile::RemoveQueueTooMany(unsigned int /*number_sources_to_remove*/)
{
	POSITION pos1, pos2;
	int removed=0;
	if (!srclist.IsEmpty()) {
		for (pos1 = srclist.GetHeadPosition(); (pos2 = pos1) != NULL; ) {
			...
			if (cur_src != NULL && cur_src->GetDownloadState() == DS_TOOMANYCONNS) {
				if (!cur_src->SwapToAnotherFile(GetResString(IDS_A4AF_FOUND), true, true, true, NULL)) {
					if (cur_src) {
						theApp.clientlist->m_globDeadSourceList.AddDeadSource(srclist.GetAt(pos2));
						theApp.downloadqueue->RemoveSource(cur_src);
					}
...
void CPartFile::RemoveQueueUnknown(unsigned int /*number_sources_to_remove*/)
{
	...
	if (!srclist.IsEmpty()) {
		for (pos1 = srclist.GetHeadPosition(); (pos2 = pos1) != NULL; ) {
			srclist.GetNext(pos1);
			CUpDownClient* cur_src = srclist.GetAt(pos2);
			if (cur_src != NULL && cur_src->GetDownloadState() == DS_NONE) {
				if (!cur_src->SwapToAnotherFile(GetResString(IDS_A4AF_FOUND), true, true, true, NULL)) {
					if (cur_src) {
						theApp.clientlist->m_globDeadSourceList.AddDeadSource(srclist.GetAt(pos2));
						theApp.downloadqueue->RemoveSource(cur_src);
