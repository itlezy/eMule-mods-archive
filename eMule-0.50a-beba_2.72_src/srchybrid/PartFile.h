...
#include "Preferences.h"	// Tux: Feature: Auto Hard Limit
#include "SourceSaver.h"	// Tux: Feature: SLS
...
#define BUFFER_TIME_LIMIT	thePrefs.GetFileBufferFlushTime()	// Tux: Feature: IntelliFlush: changed line
...
class CxImage;
class CSafeMemFile;
class CFileDetailDialogInterface;	// Tux: Feature: Modeless dialogs
...
public:
	...
	bool DissimilarName(void);
	void ResetFDC() { iNamecontinuitybad = 0; }
...
//	int		GetCommonFilePenalty() const;
	...
	// UINT	SetPrivateMaxSources(uint32 in)					{ return m_uMaxSources = in; }	// Tux: Feature: Auto Hard Limit
	...
	CFileDetailDialogInterface* GetDetailDialogInterface() const { return m_detailDialogInterface; }	// Tux: Feature: Modeless dialogs

	// Tux: Feature: Drop sources [start]
	void	RemoveNNS();
	void	RemoveFullQueueSources();
	void    RemoveQueueTooMany(unsigned int number_sources_to_remove = -1);
	void    RemoveQueueUnknown(unsigned int number_sources_to_remove = -1);
...
protected:
	bool	GetNextEmptyBlockInPart(UINT partnumber, Requested_Block_Struct* result, uint64 bytesToRequest = EMBLOCKSIZE) const;	// Tux: Feature: Dynamic Block Requests: added bytesToRequest
...
private:
	...
	CFileDetailDialogInterface*	m_detailDialogInterface;	// Tux: Feature: Modeless dialogs
	int 	iNamecontinuitybad;	// Tux: Feature: Filename disparity check
...
public:
	...
	bool	UseAutoHL()	const 			{return (thePrefs.IsUseAutoHL()>=0?thePrefs.IsUseAutoHL()>0:m_bUseAutoHL);}
	void	SetUseAutoHL(const bool& b)	{m_bUseAutoHL = b;}
	void    SetAutoHL();
