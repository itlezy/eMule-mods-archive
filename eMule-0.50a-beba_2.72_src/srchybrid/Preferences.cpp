...
#include "SearchParams.h"	// Tux: Improvement: auto-switch default search type

#pragma warning(disable:4516) // access-declarations are deprecated; member using-declarations provide a better alternative
#pragma warning(disable:4244) // conversion from 'type1' to 'type2', possible loss of data
#pragma warning(disable:4100) // unreferenced formal parameter
#pragma warning(disable:4702) // unreachable code
#include <crypto56/osrng.h>
#pragma warning(default:4702) // unreachable code
#pragma warning(default:4100) // unreferenced formal parameter
#pragma warning(default:4244) // conversion from 'type1' to 'type2', possible loss of data
#pragma warning(default:4516) // access-declarations are deprecated; member using-declarations provide a better alternative
#pragma warning(disable:4505) // unreferenced local function has been removed // Tux: others: Crypto++ warning fix
...
// Tux: LiteMule: Remove Splashscreen
...
bool	CPreferences::m_bDeleteDeadServers;	// Tux: Feature: Disable server deletion
uint8	CPreferences::smartidstate;
// bool	CPreferences::m_bSafeServerConnect;	// Tux: Newbie security: remove SafeServerConnect
...
bool	CPreferences::m_bLogSUQWT;	// Tux: Feature: SUQWT
bool	CPreferences::m_bLogKadSecurityEvents;	// Tux: Improvement: log Kad security events
bool	CPreferences::m_bLogAICHEvents;	// Tux: Improvement: log AICH events
bool	CPreferences::m_bLogSLSEvents;	// Tux: Improvement: log SLS events
bool	CPreferences::m_bLogCryptEvents;	// Tux: Improvement: log CryptLayer events
bool	CPreferences::m_bLogAnalyzerEvents;	// Tux: Feature: Client Analyzer
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
bool	CPreferences::m_bUseDebugDevice = true;
// Tux: Improvement: remove debug overhead [start]
/*
#else
bool	CPreferences::m_bUseDebugDevice = false;
#endif
*/
// Tux: Improvement: remove debug overhead [end]
...
// bool	CPreferences::m_btransferfullchunks;	// Tux: Newbie security: remove TransferFullChunks
...
// bool	CPreferences::m_bCreditSystem;	// Tux: Newbie security: remove UseCreditSystem
...
// bool	CPreferences::scheduler;	// Tux: LiteMule: Remove Scheduler
...
// Tux: Newbie security: Secure identification always on
...
//bool	CPreferences::m_bShowWin7TaskbarGoodies; // Tux: LiteMule: Win7 addons always enabled
...
//bool	CPreferences::m_bUseOldTimeRemaining;	// Tux: LiteMule: Remove advanced calculation method checkbox
...
bool	CPreferences::m_bNamecheckenabled;
bool	CPreferences::m_bPaintFDCRed;
int     CPreferences::FDCSensitivity;
// Tux: Feature: Filename disparity check [end]

uint8	CPreferences::m_uiCreditSystem;	// Tux: Feature: Analyzer CS
uint8	CPreferences::m_uiPaybackFirst;	// Tux: Feature: Payback First
...
int	CPreferences::m_bUseAutoHL;
uint16  CPreferences::m_iMaxSourcesHL;
// Tux: Feature: Auto Hard Limit [end]

// Tux: Feature: Slot Control [start]
uint32  CPreferences::m_uUpDataratePerClient;
bool	CPreferences::m_bOpenMoreSlots;
// Tux: Feature: Slot Control [end]

bool	CPreferences::m_bRandQueue;	// Tux: Feature: Probabilistic Queue
bool	CPreferences::infiniteQueue;	// Tux: Feature: Infinite Queue

// Tux: Feature: Release Bonus [start]
bool	CPreferences::m_bReleaseBonusEnabled;
int	CPreferences::m_iReleaseBonusFactor;
// Tux: Feature: Release Bonus [end]

bool	CPreferences::SlotFocus;	// Tux: Feature: Slot Focus
bool	CPreferences::m_bSnarlDisabled;	// Tux: Feature: Snarl notifications

// Tux: Feature: SLS [start]
bool	CPreferences::m_bUseSaveLoadSources;
uint8	CPreferences::m_iActivationLimitSLS;
uint8	CPreferences::m_iSourcesToSaveSLS;
// Tux: Feature: SLS [end]

// Tux: Feature: IntelliFlush [start]
bool CPreferences::m_bUseIntelliFlush;
UINT CPreferences::m_uiFileBufferFlushTime;
// Tux: Feature: IntelliFlush [end]

// Tux: Feature: Reask sources after IP change [start]
bool	CPreferences::m_bReaskSrcAfterIPChange;
uint32	CPreferences::m_uReaskTimeDif;
// Tux: Feature: Reask sources after IP change [end]

// Tux: Feature: Relative priority [start]
bool	CPreferences::m_bRelativePrioAutoSet;
uint16	CPreferences::m_iRelativePrioAutoTime;
// Tux: Feature: Relative priority [end]

bool	CPreferences::m_bEasterEgg;	// Tux: Feature: TITS :D
bool	CPreferences::m_bDirectoryWatcher;	// Tux: Feature: Automatic shared files updater
...
{
	// Tux: Improvement: Random Number improvement [bscabral] [start]
	//srand((uint32)time(0)); // we need random numbers sometimes
	srand((uint32)time(0) + (uint32)GetFreeDiskSpaceX(GetMuleDirectory(EMULE_CONFIGDIR)));
	// Tux: Improvement: Random Number improvement [bscabral] [end]
	...
	// Tux: Feature: SLS [start]
	CString sSourceListsPath = CString(GetTempDir()) + L"\\Saved Sources";
	...
	switch (bUpDown){
		case true:
			...
			switch (uClientID){
				...
				// Tux: Feature: Enhanced Client Recognition v2 [start]
				// fix by Stulle
				case SO_HYDRANODE:
				case SO_EMULEPLUS:
				case SO_TRUSTYFILES:
				// Tux: Feature: Enhanced Client Recognition v2 [end]
				case SO_CDONKEY:
				case SO_LPHANT:
...
bool CPreferences::LoadStats(int loadBackUp)
{
	...
	if (loadBackUp == 1)
	{
		...
		if (findBackUp.FindFile(sINIBackUp)){
			// Tux: Fix: possible exception fix [WiZaRd] [start]
			...
			*/
			...
			// Tux: Fix: possible exception fix [WiZaRd] [end]
		}
...
void CPreferences::SavePreferences()
{
	...
	// Tux: Improvement: Empty prefs on install [start]
	if (IsFirstStart())
	{
		m_bFirstStart = false; //reset
		CString newpath;
		newpath.Format(_T("%spreferences.old"), GetMuleDirectory(EMULE_CONFIGDIR));
		::MoveFile(GetConfigFile(), newpath);
	}
	// Tux: Improvement: Empty prefs on install [end]
	...
	//ini.WriteInt(L"ToolTipDelay",m_iToolDelayTime);	// Tux: LiteMule: Remove Tooltip Delay
	...
	ini.WriteBool(L"DeleteDeadServers", m_bDeleteDeadServers);	// Tux: Feature: Disable server deletion
	...
	//ini.WriteBool(L"SafeServerConnect",m_bSafeServerConnect);	// Tux: Newbie security: remove SafeServerConnect
	...
	//ini.WriteBool(L"ShowWin7TaskbarGoodies", m_bShowWin7TaskbarGoodies ); // Tux: LiteMule: Win7 addons always enabled
	...
	ini.WriteBool(L"InspectAllFileTypes", m_bInspectAllFileTypes);
	...
	ini.WriteBool(L"AICHTrustEveryHash", m_bTrustEveryHash);
	...
	ini.WriteBool(L"DontRecreateStatGraphsOnResize", dontRecreateGraphs, L"eMule");
	...
	ini.WriteInt(L"MaxFileUploadSizeMB", m_iWebFileUploadSizeLimitMB);
	...
	ini.WriteBool(_T("LogSUQWT"), m_bLogSUQWT);	// Tux: Feature: SUQWT
	ini.WriteBool(L"LogKadSecurityEvents", m_bLogKadSecurityEvents);	// Tux: Improvement: log Kad security events
	ini.WriteBool(L"LogAICHEvents", m_bLogAICHEvents);	// Tux: Improvement: log AICH events
	ini.WriteBool(L"LogSLSEvents", m_bLogSLSEvents);	// Tux: Improvement: log SLS events
	ini.WriteBool(L"LogCryptEvents", m_bLogCryptEvents);	// Tux: Improvement: log CryptLayer events
	ini.WriteBool(L"LogAnalyzerEvents", m_bLogAnalyzerEvents);	// Tux: Feature: Client Analyzer
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
	...
	// ini.WriteBool(L"FullChunkTransfers", m_btransferfullchunks);	// Tux: Newbie security: remove TransferFullChunks
	...
	// ini.WriteBool(L"UseCreditSystem",m_bCreditSystem);	// Tux: Newbie security: remove UseCreditSystem
	ini.WriteBool(L"SaveLogToDisk",log2disk);
	ini.WriteBool(L"SaveDebugToDisk",debug2disk);
	// ini.WriteBool(L"EnableScheduler",scheduler);	// Tux: LiteMule: Remove Scheduler
	...
	//ini.WriteBool(L"UseSimpleTimeRemainingcomputation",m_bUseOldTimeRemaining);	// Tux: LiteMule: Remove advanced calculation method checkbox
	...
	ini.WriteBool(L"ShowCopyEd2kLinkCmd",m_bShowCopyEd2kLinkCmd);	// Tux: Hidden prefs: uncomment.
	...
	///////////////////////////////////////////////////////////////////////////
	// Tux: others: beba prefs
	//
	// Tux: Feature: Emulate others [start]
	ini.WriteBool(L"EmuMLDonkey", m_bEmuMLDonkey, L"beba");
	ini.WriteBool(L"EmueDonkey", m_bEmueDonkey, L"beba");
	ini.WriteBool(L"EmueDonkeyHybrid", m_bEmueDonkeyHybrid, L"beba");
	ini.WriteBool(L"EmuShareaza", m_bEmuShareaza, L"beba");
	ini.WriteBool(L"EmulPhant", m_bEmuLphant, L"beba");
	// Tux: Feature: Emulate others [end]

	// Tux: Feature: Filename disparity check [start]
	ini.WriteBool(L"FilenameDisparityCheck", m_bNamecheckenabled, L"beba");
	ini.WriteBool(L"PaintFDCRed", m_bPaintFDCRed, L"beba");
	ini.WriteInt(L"FDCSensitivity", FDCSensitivity, L"beba");
	// Tux: Feature: Filename disparity check [end]

	ini.WriteInt(L"UseCreditSystem", m_uiCreditSystem, L"beba");	// Tux: Feature: Analyzer CS
	ini.WriteInt(L"PaybackFirst", m_uiPaybackFirst, L"beba");	// Tux: Feature: Payback First

	// Tux: Feature: Auto Hard Limit [start]
	ini.WriteInt(L"AutoHLUpdate", m_iAutoHLUpdateTimer, L"beba");
	ini.WriteInt(L"UseAutoHL", m_bUseAutoHL, L"beba");
	ini.WriteInt(L"MinAutoHL", m_iMinAutoHL, L"beba");
	ini.WriteInt(L"MaxAutoHL", m_iMaxAutoHL, L"beba");
	ini.WriteInt(L"MaxSourcesHL", m_iMaxSourcesHL, L"beba");
	// Tux: Feature: Auto Hard Limit [end]

	// Tux: Feature: Slot Control [start]
	ini.WriteInt(L"UpDataratePerClient", m_uUpDataratePerClient, L"beba");
	ini.WriteBool(L"OpenMoreSlotsIfNeeded", m_bOpenMoreSlots, L"beba");
	// Tux: Feature: Slot Control [end]

	ini.WriteBool(L"ProbabilisticQueue", m_bRandQueue, L"beba");	// Tux: Feature: Probabilistic Queue
	ini.WriteBool(L"InfiniteQueue", infiniteQueue, L"beba");	// Tux: Feature: Infinite Queue
	
	// Tux: Feature: Release Bonus [start]
	ini.WriteBool(L"ReleaseBonusEnabled", m_bReleaseBonusEnabled, L"beba");
	ini.WriteInt(L"ReleaseBonusFactor", m_iReleaseBonusFactor, L"beba");
	// Tux: Feature: Release Bonus [start]
	
	ini.WriteBool(L"SlotFocus", SlotFocus, L"beba");		// Tux: Feature: Slot Focus

	// Tux: Feature: Snarl notifications [start]
	ini.WriteBool(L"DisableSnarl", m_bSnarlDisabled, L"beba");

	if (theApp.emuledlg->hasSnarl == true && m_bSnarlDisabled == true) {
		// user decided to turn off Snarl :/ well then...
		// unregister beba and announce it internally.
		if (theApp.emuledlg->snarlIface != NULL) {
			theApp.emuledlg->hasSnarl = false;

			theApp.emuledlg->snarlIface->Unregister(_T("eMuleBeba"));
			delete theApp.emuledlg->snarlIface;
			theApp.emuledlg->snarlIface = NULL;
		}
	}
	// Tux: Feature: Snarl notifications [end]

	// Tux: Feature: SLS [start]
	ini.WriteBool(L"UseSaveLoadSources", m_bUseSaveLoadSources, L"beba");
	ini.WriteInt(L"ActivationLimitSLS", m_iActivationLimitSLS, L"beba");
	ini.WriteInt(L"SourcesToSaveSLS", m_iSourcesToSaveSLS, L"beba");
	// Tux: Feature: SLS [end]
	
	// Tux: Feature: IntelliFlush [start]
	ini.WriteBool(L"IntelliFlush", m_bUseIntelliFlush, L"beba");
	ini.WriteInt(L"FileBufferFlushTime", m_uiFileBufferFlushTime, L"beba");
	// Tux: Feature: IntelliFlush [end]

	// Tux: Feature: Reask sources after IP change [start]
	ini.WriteBool(L"ReaskSourcesAfterIPChange", m_bReaskSrcAfterIPChange, L"beba");
	uint32 m_uTemp = (m_uReaskTimeDif+FILEREASKTIME)/60000;
	ini.WriteInt(L"ReaskSourcesTime", m_uTemp, L"beba");
	// Tux: Feature: Reask sources after IP change [end]
	
	// Tux: Feature: Relative Priority [start]
	ini.WriteBool(L"RelativePrioAutoSet", m_bRelativePrioAutoSet, L"beba");
	ini.WriteInt(L"RelativePrioAutoTime", m_iRelativePrioAutoTime, L"beba");
	// Tux: Feature: Relative Priority [end]

	ini.WriteBool(L"DirWatcher", m_bDirectoryWatcher, L"beba");	// Tux: Feature: Automatic shared files updater
}
...
void CPreferences::LoadPreferences()
{
	...
	updatenotify=ini.GetBool(L"UpdateNotifyTestClient",false);	// Tux: others: disabled by default
	...
	maxGraphDownloadRate=ini.GetInt(L"DownloadCapacity",250);	// Tux: others: changed default value; was 96
	if (maxGraphDownloadRate==0)
		maxGraphDownloadRate=250;	// Tux: others: changed default value; was 96
	
	maxGraphUploadRate = ini.GetInt(L"UploadCapacityNew",-1);
	if (maxGraphUploadRate == 0)
		maxGraphUploadRate = 96;	// Tux: others: changed default value; was UNLIMITED
	...
	//m_iToolDelayTime=ini.GetInt(L"ToolTipDelay",1);	// Tux: LiteMule: Remove Tooltip Delay
	...
	m_bDeleteDeadServers = ini.GetBool(L"DeleteDeadServers", true);	// Tux: Feature: Disable server deletion
	...
	m_bPreventStandby = ini.GetBool(L"PreventStandby", true);	// Tux: others: active by default
	...
	autoconnect=ini.GetBool(L"Autoconnect",true);	// Tux: others: active by default
	...
	m_bRestoreLastMainWndDlg=ini.GetBool(L"RestoreLastMainWndDlg",true);	// Tux: others: active by default
	m_iLastMainWndDlgID=ini.GetInt(L"LastMainWndDlgID",0);
	m_bRestoreLastLogPane=ini.GetBool(L"RestoreLastLogPane",true);	// Tux: others: active by default
	m_iLastLogPaneID=ini.GetInt(L"LastLogPaneID",0);
	// m_bSafeServerConnect =ini.GetBool(L"SafeServerConnect",false);	// Tux: Newbie security: remove SafeServerConnect

	m_bTransflstRemain =ini.GetBool(L"TransflstRemainOrder",false);
	filterserverbyip=ini.GetBool(L"FilterServersByIP",true);	// Tux: others: active by default
	...
	m_bSparsePartFiles=ini.GetBool(L"SparsePartFiles",true);	// Tux: others: active by default
	...
	if (m_bEnableVerboseOptions)
	{
		...
		m_bLogSUQWT=ini.GetBool(_T("LogSUQWT"), false);	// Tux: Feature: SUQWT
		m_bLogKadSecurityEvents=ini.GetBool(L"LogKadSecurityEvents",false);	// Tux: Improvement: log Kad security events
		m_bLogAICHEvents=ini.GetBool(_T("LogAICHEvents"),false);	// Tux: Improvement: log AICH events
		m_bLogSLSEvents=ini.GetBool(_T("LogSLSEvents"),false);	// Tux: Improvement: log SLS events
		m_bLogCryptEvents=ini.GetBool(_T("LogCryptEvents"),false);	// Tux: Improvement: log CryptLayer events
		m_bLogAnalyzerEvents = ini.GetBool(L"LogAnalyzerEvents", true);	// Tux: Feature: Client Analyzer
	}
	else
	{
...
// Tux: Improvement: remove debug overhead [end]
	...
	m_bpreviewprio=ini.GetBool(L"PreviewPrio",true);	// Tux: others: active by default
	...
	// m_btransferfullchunks=ini.GetBool(L"FullChunkTransfers",true);	// Tux: Newbie security: remove TransferFullChunks
	...
	m_bInspectAllFileTypes=ini.GetBool(L"InspectAllFileTypes", true);	// Tux: others: was: int = 0
	...
	//m_bShowWin7TaskbarGoodies  = ini.GetBool(L"ShowWin7TaskbarGoodies", false); // Tux: LiteMule: Win7 addons always enabled
	...
	m_iSearchMethod=ini.GetInt(L"SearchMethod", networkkademlia && !networked2k ? SearchTypeKademlia : SearchTypeEd2kGlobal);	// Tux: Improvement: auto-switch default search type: changed line
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	m_bDisableKnownClientList=ini.GetBool(L"DisableKnownClientList",true); // Tux: LiteMule: active by default
	m_bDisableQueueList=ini.GetBool(L"DisableQueueList",false);
	// m_bCreditSystem=ini.GetBool(L"UseCreditSystem",true);	// Tux: Newbie security: remove UseCreditSystem
	...
	networkkademlia=ini.GetBool(L"NetworkKademlia",true);	// Tux: others: active by default
	networked2k=ini.GetBool(L"NetworkED2K",false);	// Tux: others: inactive by default
	m_bRemove2bin=ini.GetBool(L"RemoveFilesToBin",true);
	m_bShowCopyEd2kLinkCmd=ini.GetBool(L"ShowCopyEd2kLinkCmd",true);	// Tux: others: active by default
	...
	m_bRemoveFinishedDownloads=ini.GetBool(L"AutoClearCompleted",true);	// Tux: others: active by default
	//m_bUseOldTimeRemaining= ini.GetBool(L"UseSimpleTimeRemainingcomputation",false);	// Tux: LiteMule: Remove advanced calculation method checkbox
	...
	m_bUseSystemFontForMainControls=ini.GetBool(L"UseSystemFontForMainControls",1);	// Tux: others: active by default
	...
	// ZZ:UploadSpeedSense -->
    m_bDynUpEnabled = ini.GetBool(L"USSEnabled", true);	// Tux: Newbie security: active by default
    ...
    m_bHighresTimer = ini.GetBool(L"HighresTimer", true);	// Tux: others: active by default
	...
	m_bWinaTransToolbar = ini.GetBool(L"WinaTransToolbar", false);	// Tux: others: disabled (not default in .48a) .. review?
	m_bShowDownloadToolbar = ini.GetBool(L"ShowDownloadToolbar", false);	// Tux: others: inactive by default

	m_bCryptLayerRequested = ini.GetBool(L"CryptLayerRequested", true);	// Tux: others: active by default
	...
	m_bWebUseUPnP = ini.GetBool(L"WebUseUPnP", true);	// Tux: others: active by default
	...
	//
	// Tux: Feature: Emulate others [start]
	m_bEmuMLDonkey = ini.GetBool(L"EmuMLDonkey", true, L"beba");
	m_bEmueDonkey = ini.GetBool(L"EmueDonkey", true, L"beba");
	m_bEmueDonkeyHybrid = ini.GetBool(L"EmueDonkeyHybrid", true, L"beba");
	m_bEmuShareaza = ini.GetBool(L"EmuShareaza", true, L"beba");
	m_bEmuLphant = ini.GetBool(L"EmulPhant", true, L"beba");
	// Tux: Feature: Emulate others [end]

	// Tux: Feature: Filename disparity check [start]
	m_bNamecheckenabled = ini.GetBool(L"FilenameDisparityCheck", true, L"beba");
	m_bPaintFDCRed = ini.GetBool(L"PaintFDCRed", true, L"beba");
	FDCSensitivity = ini.GetInt(L"FDCSensitivity", 81, L"beba");
	// Tux: Feature: Filename disparity check [end]

	m_uiCreditSystem = (uint8)ini.GetInt(L"UseCreditSystem", 1, L"beba");	// Tux: Feature: Analyzer CS
	m_uiPaybackFirst = (uint8)ini.GetInt(L"PaybackFirst", 3, L"beba");	// Tux: Feature: Payback First
	//0: off - 1: one slot at a time - 2: every 5th slot - 3: auto (mix of 1+2)

	// Tux: Feature: Auto Hard Limit [start]
	m_iAutoHLUpdateTimer = (uint16)ini.GetInt(L"AutoHLUpdate", 60, L"beba");
#define MINMAX(val, mini, maxi)		{val = (min(max(mini, val), maxi));}
	MINMAX(m_iAutoHLUpdateTimer, 10, 600)
	m_bUseAutoHL = ini.GetInt(L"UseAutoHL", 1, L"beba");
	m_iMinAutoHL = (uint16)ini.GetInt(L"MinAutoHL", 25, L"beba");
	m_iMaxAutoHL = (uint16)ini.GetInt(L"MaxAutoHL", _UI16_MAX, L"beba");
	m_iMaxAutoHL = max(m_iMinAutoHL, m_iMaxAutoHL);
	m_iMaxSourcesHL = (uint16)ini.GetInt(L"MaxSourcesHL", _UI16_MAX, L"beba");
	// Tux: Feature: Auto Hard Limit [end]

	// Tux: Feature: Slot Control [start]
	if(maxupload == UNLIMITED){
		if(maxGraphUploadRate == UNLIMITED)
			m_uUpDataratePerClient = (uint8)ini.GetInt(L"UpDataratePerClient", 2, L"beba");
		else {
			UINT MaxUpDatarateTemp = (maxGraphUploadRate < 2) ? 1024 : ((maxGraphUploadRate*1024)/((maxGraphUploadRate < 9) ? 2 :( UINT)sqrtf((float)maxGraphUploadRate)));
			UINT UpSlotsPerClientTemp = ((maxGraphUploadRate*1024)/MaxUpDatarateTemp)+1;
			m_uUpDataratePerClient = (uint8)ini.GetInt(L"UpDataratePerClient", UpSlotsPerClientTemp, L"beba");
		}
	} else {
		UINT MaxUpDatarateTemp = (maxupload < 2) ? 1024 : ((maxupload*1024)/((maxupload < 9) ? 2 : (UINT)sqrtf((float)maxupload)));
		UINT UpSlotsPerClientTemp = ((maxupload*1024)/MaxUpDatarateTemp)+1;
		m_uUpDataratePerClient = (uint8)ini.GetInt(L"UpDataratePerClient", UpSlotsPerClientTemp, L"beba");
	}

	m_bOpenMoreSlots = ini.GetBool(L"OpenMoreSlotsIfNeeded", true, L"beba");
	// Tux: Feature: Slot Control [end]
	
	m_bRandQueue = ini.GetBool(L"ProbabilisticQueue", false, L"beba");	// Tux: Feature: Probabilistic Queue
	infiniteQueue = ini.GetBool(L"InfiniteQueue", false, L"beba");		// Tux: Feature: Infinite Queue
	
	// Tux: Feature: Release Bonus [start]
	m_bReleaseBonusEnabled = ini.GetBool(L"ReleaseBonusEnabled", true, L"beba");
	m_iReleaseBonusFactor = ini.GetInt(L"ReleaseBonusFactor", 32, L"beba");
	// Tux: Feature: Release Bonus [end]
	
	SlotFocus = ini.GetBool(L"SlotFocus", true, L"beba");			// Tux: Feature: Slot Focus
	m_bSnarlDisabled = ini.GetBool(L"DisableSnarl", false, L"beba");			// Tux: Feature: Snarl notifications

	// Tux: Feature: SLS [start]
	int m_uTemp;
	m_bUseSaveLoadSources = ini.GetBool(L"UseSaveLoadSources", true, L"beba");
	m_uTemp = ini.GetInt(L"ActivationLimitSLS", 200, L"beba");
	m_iActivationLimitSLS = (uint8)((m_uTemp >= 0 && m_uTemp <= 200) ? m_uTemp : 200);
	m_uTemp = ini.GetInt(L"SourcesToSaveSLS", 100, L"beba");
	m_iSourcesToSaveSLS = (uint8)((m_uTemp >= 0 && m_uTemp <= 100) ? m_uTemp : 100);
	// Tux: Feature: SLS [end]
	
	// Tux: Feature: IntelliFlush [start]
	m_bUseIntelliFlush = ini.GetBool(L"IntelliFlush", true, L"beba");
	m_uiFileBufferFlushTime = (UINT)ini.GetInt(L"FileBufferFlushTime", 60000, L"beba");
	// Tux: Feature: IntelliFlush [end]

	// Tux: Feature: Reask sources after IP change [start]
	m_bReaskSrcAfterIPChange = ini.GetBool(L"ReaskSourcesAfterIPChange", true, L"beba");
	uint32 temp = ini.GetInt(L"ReaskSourcesTime", 29, L"beba");
	temp = (temp >= 29 && temp <= 55) ? temp : 29;
	m_uReaskTimeDif = (temp-29)*60000;
	// Tux: Feature: Reask sources after IP change [end]
	
	// Tux: Feature: Relative Priority [start]
	m_bRelativePrioAutoSet = ini.GetBool(L"RelativePrioAutoSet", true, L"beba");
	m_iRelativePrioAutoTime = (uint16)ini.GetInt(L"RelativePrioAutoTime", 10, L"beba");
	// Tux: Feature: Relative Priority [end]
	
	m_bEasterEgg = ini.GetBool(L"EasterEgg", true, L"beba");	// Tux: Feature: TITS :D
	m_bDirectoryWatcher = ini.GetBool(L"DirWatcher", true, L"beba");	// Tux: Feature: Automatic shared files updater
