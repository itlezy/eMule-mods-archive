...
void CQueueListCtrl::Init()
{
	...
	InsertColumn(10,GetResString(IDS_MOD),			LVCFMT_LEFT, 150);	// Tux: Feature: Mod Version
...
void CQueueListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.AppendMenu(MF_SEPARATOR);	// Tux: others: added separator
	...
	ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient() && client->IsBanned()) ? MF_ENABLED : MF_GRAYED), MP_UNBAN, GetResString(IDS_UNBAN), _T("UPLOAD"));
	if (thePrefs.m_bExtControls)
		ClientMenu.AppendMenu(MF_STRING | (theApp.clientlist->GetBannedCount() > 0 ? MF_ENABLED : MF_GRAYED), MP_CLEARBANS, GetResString(IDS_CLEARBAN), _T("SMILEY_SMILE"));	// Tux: Feature: Clear Ban List
...
BOOL CQueueListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	switch (wParam)
	{
		case MP_FIND:
			OnFindStart();
			return TRUE;
		// Tux: Feature: Clear Ban List [start]
		case MP_CLEARBANS:
			theApp.clientlist->ClearBanList();
			theApp.emuledlg->transferwnd->ShowQueueCount(theApp.uploadqueue->GetWaitingUserCount());
			break;
		// Tux: Feature: Clear Ban List [end]
	}
