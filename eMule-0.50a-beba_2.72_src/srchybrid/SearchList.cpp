...
void CSearchList::NewSearch(CSearchListCtrl* pWnd, CStringA strResultFileType, uint32 nSearchID, ESearchType eSearchType, CString strSearchExpression)	// Tux: LiteMule: Remove MobileMule
{
	if (pWnd)
		outputwnd = pWnd;
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP)
{
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
			...
			&& md4cmp(parent->GetFileHash(), toadd->GetFileHash()) == 0)
		{
			...
			if (outputwnd)	// Tux: LiteMule: Remove MobileMule: removed check
	...
	try {
		...
		for (int i = 0; i < nCount; i++){
			...
			if (strResultType == ED2KFTSTR_PROGRAM)
				strResultType.Empty();
			NewSearch(NULL, strResultType, pParams->dwSearchID, pParams->eType, pParams->strExpression);	// Tux: LiteMule: Remove MobileMule: removed boolean value
