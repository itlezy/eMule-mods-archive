...
#include "CommentDialogLst.h"
#include "MediaInfo.h"
#include "AntiFake.h" // Tux: Feature: additional fake checks
...
class CSearchResultFileDetailSheet : public CModelessPropertySheet	// Tux: Feature: Modeless dialogs
{
...
IMPLEMENT_DYNAMIC(CSearchResultFileDetailSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs

BEGIN_MESSAGE_MAP(CSearchResultFileDetailSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs
	ON_WM_DESTROY()
...
CSearchResultFileDetailSheet::CSearchResultFileDetailSheet(CTypedPtrList<CPtrList, CSearchFile*>& paFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
	: CModelessPropertySheet(pListCtrl)	// Tux: Feature: Modeless dialogs
{
...
{		
	...
	BOOL bResult = CModelessPropertySheet::OnInitDialog();	// Tux: Feature: Modeless dialogs: changed line
...
void CSearchResultFileDetailSheetInterface::OpenDetailSheet(CTypedPtrList<CPtrList, CSearchFile*>& aFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CModelessPropertySheetInterface*> aInterfaces;
	POSITION pos = aFiles.GetHeadPosition();
	while (pos)
		aInterfaces.Add(STATIC_DOWNCAST(CModelessPropertySheetInterface, aFiles.GetNext(pos)->GetSearchDetailSheetInterface()));
...
{
	CTypedPtrList<CPtrList, CSearchFile*>* paFiles = (CTypedPtrList<CPtrList, CSearchFile*>*)va_arg(args, LPVOID);
	UINT uPshInvokePage = va_arg(args, UINT);
	CListCtrlItemWalk* pListCtrl = va_arg(args, CListCtrlItemWalk*);
	return STATIC_DOWNCAST(CModelessPropertySheet, new CSearchResultFileDetailSheet(*paFiles, uPshInvokePage, pListCtrl));
}
...
void CSearchListCtrl::Init(CSearchList* in_searchlist)
{
	...
	InsertColumn(15,GetResString(IDS_CMT_FAKE),		LVCFMT_LEFT,  250); // Tux: Feature: additional fake checks
...
void CSearchListCtrl::Localize()
{
	...
	for (int icol=0;icol<pHeaderCtrl->GetItemCount();icol++) {
		switch (icol) {
			...
			case 15: strRes = GetResString(IDS_CMT_FAKE); break; // Tux: Feature: additional fake checks
		}
		...
		case 15:
			{
				...
				else
				{
					...
					return CompareLocaleStringNoCase(str1, str2);
				}
			}
		// Tux: Feature: additional fake checks [end]
	}
...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	//m_SearchFileMenu.EnableMenuItem(MP_SEARCHRELATED, iSelected > 0 && theApp.emuledlg->searchwnd->CanSearchRelatedFiles() ? MF_ENABLED : MF_GRAYED); // Tux: LiteMule: remove Search Related Files
...
BOOL CSearchListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (selectedList.GetCount() > 0)
	{
		...
		switch (wParam)
		{
			...
			case MPG_ALTENTER:
			{
				selectedList.GetHead()->GetSearchDetailSheetInterface()->OpenDetailSheet(selectedList, 0, this);	// Tux: Feature: Modeless dialogs: replaced lines
				return TRUE;
			}
			case MP_CMT:
			{
				selectedList.GetHead()->GetSearchDetailSheetInterface()->OpenDetailSheet(selectedList, IDD_COMMENTLST, this);	// Tux: Feature: Modeless dialogs: replaced lines
				return TRUE;
			}
			case MP_PREVIEW:
				if (file){
...
void CSearchListCtrl::CreateMenues()
{
	...
	m_SearchFileMenu.AppendMenu(MF_SEPARATOR);
	m_SearchFileMenu.AppendMenu(MF_STRING, MP_FIND, GetResString(IDS_FIND), _T("SEARCHMENU"));	// Tux: changed search icon
	//m_SearchFileMenu.AppendMenu(MF_STRING, MP_SEARCHRELATED, GetResString(IDS_SEARCHRELATED), _T("KadFileSearch")); // Tux: LiteMule: remove Search Related Files
}
		...
		if (GetKeyState(VK_MENU) & 0x8000)
		{
			...
			if (iSel != -1)
			{
				...
				if (file)
				{
					...
					aFiles.AddTail(file);
					file->GetSearchDetailSheetInterface()->OpenDetailSheet(aFiles, 0, this);	// Tux: Feature: Modeless dialogs: replaced lines
				}
			}
		}
		else
...
//		COLORREF crLine = (!g_bLowColorDesktop || (lpDrawItemStruct->itemState & ODS_SELECTED) == 0) ? RGB(128,128,128) : m_crHighlightText;
		COLORREF crLine = ((lpDrawItemStruct->itemState & ODS_SELECTED) == 0) ? RGB(128,128,128) : m_crHighlightText;
		// Tux: LiteMule: no more low-color support [end]
		CPen pn, *oldpn;
		...
		else if (isOpenRoot || (content->GetListParent() == NULL && content->GetListChildCount() > 1))
		{
			...
			penBlack.CreatePen(PS_SOLID, 1, ((lpDrawItemStruct->itemState & ODS_SELECTED) == 0) ? m_crWindowText : m_crHighlightText);
			// Tux: LiteMule: no more low-color support [end]
			CPen* pOldPen2;
		...
		case 15:
			{
				...
				if (!str.IsEmpty())
					_tcsncpy(pszText, str, cchTextMax);
				break;
			}
		// Tux: Feature: additional fake checks [end]
	}
