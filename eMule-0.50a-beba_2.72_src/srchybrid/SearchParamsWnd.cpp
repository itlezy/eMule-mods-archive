...
#pragma warning(disable:4702) //code not reachable	// Tux: Improvement
#include <list>
#pragma warning(default:4702) //code not reachable	// Tux: Improvement
...
LRESULT CSearchParamsWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	InitMethodsCtrl();
	// Tux: Improvement: auto-switch default search type [start]
	if (thePrefs.GetNetworkKademlia() && !thePrefs.GetNetworkED2K())
...
void CSearchParamsWnd::UpdateControls()
{
	...
	if (Kademlia::CKademlia::IsConnected() && !theApp.serverconnect->IsConnected())
		m_ctlMethod.SetCurSel(iMethod = SearchTypeKademlia);
	// Tux: Improvement: auto-switch default search type [end]
	if (iMethod != CB_ERR)
	{
...
void CSearchParamsWnd::InitMethodsCtrl()
{
	...
	// VERIFY( m_ctlMethod.AddItem(_T("FileDonkey (Web)"), 4) == SearchTypeFileDonkey ); // Tux: LiteMule: removed FileDonkey search method
	VERIFY( m_ctlMethod.AddItem(_T("Content-DB (Web)"), 4) == SearchTypeContentDB ); // Tux: Feature: Content-DB search method
...
SSearchParams* CSearchParamsWnd::GetParameters()
{
	...
	if (ullMinSize == (uint64)-1) {
		...
		AfxMessageBox(GetResString(IDS_SEARCH_EXPRERROR) + _T("\n\n") + strError, MB_ICONWARNING | MB_HELP/*, eMule_FAQ_Search - HID_BASE_PROMPT*/);	// Tux: LiteMule: Remove Help
		return NULL;
	}
