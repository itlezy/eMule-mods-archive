...
// Tux: LiteMule: removed FileDonkey search method [start]
/*
		case SearchTypeFileDonkey:
			...
			return;
*/
// Tux: LiteMule: removed FileDonkey search method [end]

		// Tux: Feature: Content-DB search method [start]
		case SearchTypeContentDB:
			ShellOpenFile(CreateWebQuery(pParams));
			delete pParams;
			return;
		// Tux: Feature: Content-DB search method [end]
...
bool CSearchResultsWnd::StartNewSearch(SSearchParams* pParams)
{
	...
	if (pParams->eType == SearchTypeAutomatic){
		...
		if (!theApp.serverconnect->IsConnected() && Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsConnected())
			...
			pParams->eType = SearchTypeEd2kGlobal;	// Tux: Improvement: was "Server" :)
		else if (!theApp.serverconnect->IsConnected() && (!Kademlia::CKademlia::IsRunning() || !Kademlia::CKademlia::IsConnected())){
	...
	if (eSearchType == SearchTypeEd2kServer || eSearchType == SearchTypeEd2kGlobal)
	{
		...
		{
			AfxMessageBox(ex->m_strMsg, ex->m_uType/*, ex->m_uHelpID*/);	// Tux: LiteMule: Remove Help
			ex->Delete();
			delete pParams;
			return false;
		}
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams)
{
	...
	// Tux: Improvement: auto-clean bad Kad keywords [start]
		TCHAR* p = (TCHAR*)g_aszInvKadKeywordChars;
		int invalstrlen = _tcslen(g_aszInvKadKeywordChars);
		CString originalStr = pParams->strKeyword;
		for (int i = 0; i < invalstrlen;)
		{
			pParams->strKeyword.Remove(*p);
			p++;
			i++;
		}
		//As KAD keywords have to be above 2 characters
		if (pParams->strKeyword.GetLength() > 2)
			::MessageBox(m_hWnd, GetResString(IDS_KADCLEANUP_1) + originalStr + GetResString(IDS_KADCLEANUP_2) + pParams->strKeyword + _T("."), GetResString(IDS_KADCLEANUP_TITLE), MB_ICONWARNING | MB_OK);
		else {
	// Tux: Improvement: auto-clean bad Kad keywords [end]
		CString strError;
	...
	{
		delete[] pSearchTermsData;
		throw new CMsgBoxException(strException, MB_ICONWARNING | MB_HELP/*, eMule_FAQ_Search - HID_BASE_PROMPT*/);	// Tux: LiteMule: Remove Help
	}
