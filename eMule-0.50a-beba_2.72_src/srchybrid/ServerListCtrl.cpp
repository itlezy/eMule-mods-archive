...
{
	ShowWindow(SW_HIDE);
	if (thePrefs.IsDeadServersEnabled())	// Tux: Feature: Disable server deletion
...
{ 
	...
	int iCanConnect = iSelectedItems;	// Tux: Fix: Obfuscation fix [WiZaRd]
	while (pos)
	{
		const CServer* pServer = (CServer*)GetItemData(GetNextSelectedItem(pos));

		// Tux: Fix: Obfuscation fix [WiZaRd] [start]
		...
		if (thePrefs.IsClientCryptLayerRequired() && !pServer->SupportsObfuscationTCP())
			iCanConnect--;
		// Tux: Fix: Obfuscation fix [WiZaRd] [end]
	...
	ServerMenu.AppendMenu(MF_STRING | (iSelectedItems > 0 && iCanConnect > 0 ? MF_ENABLED : MF_GRAYED), MP_CONNECTTO, GetResString(IDS_CONNECTTHIS), _T("TRAYCONNECT"));	// Tux: Fix: Obfuscation fix [WiZaRd]: added check for iCanConnect	// Tux: others: changed icon
	...
	ServerMenu.AppendMenu(MF_SEPARATOR);
	ServerMenu.AppendMenu(MF_ENABLED | (GetItemCount() > 0 ? MF_ENABLED : MF_GRAYED), MP_FIND, GetResString(IDS_FIND), _T("SEARCHMENU"));	// Tux: changed search icon
...
void CServerListCtrl::SetSelectedServersPriority(UINT uPriority)
{
	...
	// Tux: Fix: Use a pointer list when setting priority [moloko+] [start]
	// Not all server priorities are set if the list is sorted by Priority (Preference).
	CTypedPtrList<CPtrList, CServer *> selectedList;
	POSITION pos = GetFirstSelectedItemPosition();
	while (pos != NULL)
		selectedList.AddTail((CServer*)GetItemData(GetNextSelectedItem(pos)));

	while (!selectedList.IsEmpty())
	{
		CServer* pServer = selectedList.GetHead();
	...
	*/
	// Tux: Fix: Use a pointer list when setting priority [moloko+] [end]
