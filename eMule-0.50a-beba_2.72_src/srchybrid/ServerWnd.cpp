...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		...
		m_strClickNewBebaVersion = GetResString(IDS_BEBA_VERSION_CHECK);
		servermsgbox->AppendHyperLink(_T(""), _T(""), m_strClickNewBebaVersion, _T(""));
...
void CServerWnd::OnEnLinkServerBox(NMHDR *pNMHDR, LRESULT *pResult)
{
	*pResult = 0;
	...
	if (pEnLink && pEnLink->msg == WM_LBUTTONDOWN)
	{
		...
		// Tux: Feature: beba version check [start]
		else if (strUrl == m_strClickNewBebaVersion){
			strUrl.Format(CString(_T("http://beba.eMuleFuture.de/vcheck.php?vcheck=%s")), theApp.m_strCurVersionLong);
		}
		// Tux: Feature: beba version check [end]
...
{
	if (delta == 0) return;	// Tux: Improvement: Code optimization [WiZaRd]
...
void CServerWnd::ReattachAnchors()
{
	...
	m_wndSplitter.Invalidate();	// Tux: Fix: repaint splitter [moloko+]
