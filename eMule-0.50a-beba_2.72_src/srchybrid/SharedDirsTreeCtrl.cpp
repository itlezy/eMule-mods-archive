...
void CSharedDirsTreeCtrl::SetAllIcons()
{
	...
	if (pCurImageList != NULL && pCurImageList->GetImageCount() >= 7)
	{
		...
		pCurImageList->Replace(8, CTempIconLoader(_T("SearchFileType_Audio")));
		pCurImageList->Replace(9, CTempIconLoader(_T("SearchFileType_Video")));
		pCurImageList->Replace(10, CTempIconLoader(_T("SearchFileType_Picture")));
		pCurImageList->Replace(11, CTempIconLoader(_T("SearchFileType_Program")));
		pCurImageList->Replace(12, CTempIconLoader(_T("SearchFileType_Document")));
		pCurImageList->Replace(13, CTempIconLoader(_T("SearchFileType_Archive")));
		pCurImageList->Replace(14, CTempIconLoader(_T("SearchFileType_CDImage")));
		pCurImageList->Replace(15, CTempIconLoader(_T("AABCOLLECTIONFILETYPE")));
	...
	else
	{
		...
		// Tux: Feature: ed2k file type view filter [start]
		iml.Add(CTempIconLoader(_T("SearchFileType_Audio"), 16, 16));   // 8
		iml.Add(CTempIconLoader(_T("SearchFileType_Video"), 16, 16));   // 9
		iml.Add(CTempIconLoader(_T("SearchFileType_Picture"), 16, 16)); // 10
		iml.Add(CTempIconLoader(_T("SearchFileType_Program"), 16, 16)); // 11
		iml.Add(CTempIconLoader(_T("SearchFileType_Document"), 16, 16));// 12
		iml.Add(CTempIconLoader(_T("SearchFileType_Archive"), 16, 16)); // 13
		iml.Add(CTempIconLoader(_T("SearchFileType_CDImage"), 16, 16)); // 14
		iml.Add(CTempIconLoader(_T("AABCOLLECTIONFILETYPE"), 16, 16));  // 15

		iml.Add(CTempIconLoader(_T("FileTypeUnknown"), 16, 16));        // 16 - icon for "by file type"
		// Tux: Feature: ed2k file type view filter [end]
...
void CSharedDirsTreeCtrl::FilterTreeReloadTree(){
	...
	while (pos != NULL){
		...
		switch( pCurrent->m_eItemType ){
			...
			// Tux: Feature: ed2k file type view filter [start]
			{
				CDirectoryItem* pEd2kTypes = new CDirectoryItem(L"", 0);
				pEd2kTypes->m_htItem = InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, GetResString(IDS_BY_FILE_TYPE), 16, 16, 0, 0, (LPARAM)pEd2kTypes, pCurrent->m_htItem, TVI_LAST);
				pCurrent->liSubDirectories.AddTail(pEd2kTypes);
				...
				for (int i = 0; i < ARRSIZE(_aEd2kTypeView); i++)
				{
					...
					pEd2kType->m_htItem = InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, GetResString(_aEd2kTypeView[i].uStringId), i+8, i+8, 0, 0, (LPARAM)pEd2kType, pEd2kTypes->m_htItem, TVI_LAST);
