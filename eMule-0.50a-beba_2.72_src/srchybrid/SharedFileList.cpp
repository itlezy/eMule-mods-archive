...
// Tux: Improvement: No beta testfile :-) [start]
/*
#ifdef _BETA
...
#endif
*/
// Tux: Improvement: No beta testfile :-) [end]
}
...
void CSharedFileList::SendListToServer(){
	...
	for(pos=m_Files_map.GetStartPosition(); pos!=0;)
	{
		...
		added=false;
		// Tux: Improvement: Push small incomplete files [WiZaRd] [start]
		//there is no use to publish files below 1 chunk filesize which we do not have complete
		...
		// Tux: Improvement: Push small incomplete files [WiZaRd] [end]
		{
...
void CSharedFileList::ClearED2KPublishInfo()
{
	...
	{
		m_Files_map.GetNextAssoc(pos,bufKey,cur_file);
		if (cur_file->GetPublishedED2K())	// Tux: Improvement: only reset if published
...
void CSharedFileList::Publish()
{
	...
	time_t tNow = time(NULL); // Tux: others: was UINT, whyever
	...
	{ 
		...
		if( Kademlia::CKademlia::GetTotalStoreKey() < KADEMLIATOTALSTOREKEY)
		{
			...
			if (tNow >= (time_t)m_keywords->GetNextPublishTime()) // Tux: others: explicit cast instead of UINT
			{
				...
				if(pPubKw)
				{
					...
					if (tNow >= (time_t)pPubKw->GetNextPublishTime()) // Tux: others: explicit cast instead of UINT
					{
...
//								ASSERT( IsFilePtrInList(aFiles[f]) );
				...
				m_keywords->SetNextPublishTime(KADEMLIAPUBLISHTIME+(UINT)tNow); // Tux: others: explicit cast
			}
		}
		
		if( Kademlia::CKademlia::GetTotalStoreSrc() < KADEMLIATOTALSTORESRC)
		{
			if(tNow >= (time_t)m_lastPublishKadSrc) // Tux: others: explicit cast instead of UINT
			{
		...
		{
			if(tNow >= (time_t)m_lastPublishKadNotes) // Tux: others: explicit cast instead of UINT
			{
...
bool CSharedFileList::ExcludeFile(CString strFilePath)
{
	...
	bool bSingleShared = bShared;	// Tux: Feature: Automatic shared files updater
	...
	// add to exclude list
	if (!bSingleShared)	// Tux: Feature: Automatic shared files updater
