...
class CSharedFileDetailsSheet : public CModelessPropertySheet	// Tux: Feature: Modeless dialogs
{
...
IMPLEMENT_DYNAMIC(CSharedFileDetailsSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs

BEGIN_MESSAGE_MAP(CSharedFileDetailsSheet, CModelessPropertySheet)	// Tux: Feature: Modeless dialogs
	ON_WM_DESTROY()
...
CSharedFileDetailsSheet::CSharedFileDetailsSheet(CTypedPtrList<CPtrList, CShareableFile*>& aFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
	: CModelessPropertySheet(pListCtrl)	// Tux: Feature: Modeless dialogs: changed line
{
...
CSharedFileDetailsSheetInterface::CSharedFileDetailsSheetInterface(CShareableFile* owner)
	: CModelessPropertySheetInterface(STATIC_DOWNCAST(CObject, owner))
{
}

void CSharedFileDetailsSheetInterface::OpenDetailSheet(CTypedPtrList<CPtrList, CShareableFile*>& aFiles, UINT uPshInvokePage, CListCtrlItemWalk* pListCtrl)
{
	CSimpleArray<CModelessPropertySheetInterface*> aInterfaces;
	POSITION pos = aFiles.GetHeadPosition();
	while (pos)
		aInterfaces.Add(STATIC_DOWNCAST(CModelessPropertySheetInterface, aFiles.GetNext(pos)->GetSharedDetailSheetInterface()));
...
{
	CTypedPtrList<CPtrList, CShareableFile*>* paFiles = (CTypedPtrList<CPtrList, CShareableFile*>*)va_arg(args, LPVOID);
	UINT uPshInvokePage = va_arg(args, UINT);
	CListCtrlItemWalk* pListCtrl = va_arg(args, CListCtrlItemWalk*);
	return STATIC_DOWNCAST(CModelessPropertySheet, new CSharedFileDetailsSheet(*paFiles, uPshInvokePage, pListCtrl));
}
...
void CSharedFilesCtrl::Init()
{
	...
	InsertColumn(18,GetResString(IDS_ONQUEUE),			LVCFMT_LEFT,  100,12);	// Tux: Feature: see OnUploadqueue
	SetAllIcons();
	CreateMenues();
	LoadSettings();
...
void CSharedFilesCtrl::AddFile(const CShareableFile* file)
{
	...
	if (m_pDirectoryFilter != NULL){
		...
		switch(m_pDirectoryFilter->m_eItemType){
			...
			{
				if (m_pDirectoryFilter->m_nCatFilter == -1 || m_pDirectoryFilter->m_nCatFilter != GetED2KFileTypeID(file->GetFilePath()))
					return;
				break;
			}
				...
				if (pKnownFile && !pKnownFile->IsPartFile()){
					...
					if (!inputbox.WasCancelled() && newname.GetLength()>0)
					{
						...
						if (pKnownFile->IsKindOf(RUNTIME_CLASS(CPartFile)))
						{
							...
							((CPartFile*) pKnownFile)->SetFullName(newpath);	// Tux: Fix: Unable to open completed file after renaming [SiRoB]
						}
						else
						{
	...
	if (bExtColumn)
		...
		else if (item1->IsKindOf(RUNTIME_CLASS(CKnownFile)) && item2->IsKindOf(RUNTIME_CLASS(CKnownFile)))
		{
			...
			switch (iColumn)
			{
				...
				case 111:{ //kad shared
					uint32 tNow = (uint32)time(NULL); // Tux: others: fixed warning C4244 [modified]
...
void CSharedFilesCtrl::CreateMenues()
{
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_FIND, GetResString(IDS_FIND), _T("SEARCHMENU"));	// Tux: changed search icon
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
...
void CSharedFilesCtrl::ShowFileDialog(CTypedPtrList<CPtrList, CShareableFile*>& aFiles, UINT uPshInvokePage)
{
	if (aFiles.GetSize() > 0)
	{
		aFiles.GetHead()->GetSharedDetailSheetInterface()->OpenDetailSheet(aFiles, uPshInvokePage, this);	// Tux: Feature: Modeless dialogs
	}
}
