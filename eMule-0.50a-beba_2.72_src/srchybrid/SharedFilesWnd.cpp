...
BOOL CSharedFilesWnd::PreTranslateMessage(MSG* pMsg)
{
	...
	else if (!thePrefs.GetStraightWindowStyles() && pMsg->message == WM_MBUTTONUP)
	{
		...
		// Tux: Fix: possible crash fix [WiZaRd] [start]
		...
		// Tux: Fix: possible crash fix [WiZaRd] [end]
		return TRUE;
	}
...
void CSharedFilesWnd::Localize()
{
	...
	m_dlgDetails.Localize();	// Tux: Fix: MetaDataDlg translation fix [WiZaRd]

	GetDlgItem(IDC_RELOADSHAREDFILES)->SetWindowText(GetResString(IDS_SF_RELOAD));
}

// Tux: Improvement: Show meta data dialog dynamically [WiZaRd] [start]
void CSharedFilesWnd::UpdateMetaDataPage()
{
	m_dlgDetails.UpdateMetaDataPage();
}
// Tux: Improvement: Show meta data dialog dynamically [WiZaRd] [end]
...
// Tux: Fix: MetaDataDlg translation fix [WiZaRd] [start]
...
// Tux: Fix: MetaDataDlg translation fix [WiZaRd] [end]
