...
void CStatisticsDlg::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("ServerOpts")));				// Server		// Tux: others: changed icon
	iml.Add(CTempIconLoader(_T("SharedOpts")));				// Shared Files	// Tux: others: changed icon
	...
	iml.Add(CTempIconLoader(L"BADGUY"));					// Tux: Feature: Client Analyzer
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate) 
{
	...
	if (forceUpdate || stattree.IsExpanded(h_Analyzer)) 
	{
		const CString strTypes[] = {
			GetResString(IDS_STATS_CA_NICKTHIEF),
			GetResString(IDS_STATS_CA_MODTHIEF),
			GetResString(IDS_STATS_CA_FILEFAKES),
			GetResString(IDS_STATS_CA_UDPFNFFAKE),
			GetResString(IDS_STATS_CA_FASTASKS),
			GetResString(IDS_STATS_CA_SPAMS),
			GetResString(IDS_STATS_CA_FASTXS),
			GetResString(IDS_STATS_CA_MODFAKES),
		};
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				CRBMap<UINT, CRBMap<CString, UINT>* > clientMods;
				...
				theApp.clientlist->ReleaseModStatistics(&clientMods);	// Tux: Feature: Mod Statistics
...
void CStatisticsDlg::CreateMyTree() 
{
	...
	// Tux: Improvement: small code improvement [WiZaRd] [start]
	// for(int i = 0; i<6; i++)
	for (int i = 0; i < _countof(up_S); i++)
	// Tux: Improvement: small code improvement [WiZaRd] [end]
	...
	stattree.SetItemState(h_Analyzer, TVIS_BOLD, TVIS_BOLD);	// Tux: Feature: Client Analyzer
