...
#define DTOOLBAR_NUM_BUTTONS 17	// Tux: Feature: Filename Disparity Check  // Tux: LiteMule: remove Search Related Files // was: 17, then 18, now 17 again
void CToolbarWnd::FillToolbar()
{
	...
	atb1[8].iBitmap = 7;
	atb1[8].idCommand = MP_FILENAME;
	atb1[8].fsState = TBSTATE_WRAP;
	atb1[8].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[8].iString = m_btnBar->AddString(GetResString(IDS_FILENAMEFDC));
...
LRESULT CToolbarWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	iml.Add(CTempIconLoader(_T("FILERENAME")));	// Tux: Feature: Filename Disparity Check
	...
	//iml.Add(CTempIconLoader(_T("KadFileSearch"))); // Tux: LiteMule: remove Search Related Files
	iml.Add(CTempIconLoader(_T("SEARCHMENU")));	// Tux: others: changed icon
	...
	if (bNeedMonoIcons)
	{
		...
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("FILERENAME"))));	// Tux: Feature: Filename Disparity Check
		...
		//VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("KadFileSearch")))); // Tux: LiteMule: remove Search Related Files
		VERIFY(AddIconGrayscaledToImageList(iml2, CTempIconLoader(_T("SEARCHMENU"))));	// Tux: others: changed icon
