...
BEGIN_MESSAGE_MAP(CTransferWnd, CResizableFormView)
	...
	// Tux: Fix: removed hover tracking [SLUGFILLER] [start]
	/*
	...
	*/
	// Tux: Fix: removed hover tracking [SLUGFILLER] [end]
...
void CTransferWnd::OnInitialUpdate()
{
	...
	m_DropAllFullQBtn.SetIcon(_T("DROPFQ"),NULL);
	m_DropAllFullQBtn.SetAlign(CButtonST::ST_ALIGN_HORIZ);
	m_DropAllFullQBtn.SetLeftAlign(true);
	m_DropAllFullQBtn.SetTooltipText(IDS_DROP_FQ);

	m_DropAllNNSBtn.SetIcon(_T("DROPNNS"),NULL);
	m_DropAllNNSBtn.SetAlign(CButtonST::ST_ALIGN_HORIZ);
	m_DropAllNNSBtn.SetLeftAlign(true);
	m_DropAllNNSBtn.SetTooltipText(IDS_DROP_NNS);

	m_DropTooManyBtn.SetIcon(_T("DROPTM"),NULL);
	m_DropTooManyBtn.SetAlign(CButtonST::ST_ALIGN_HORIZ);
	m_DropTooManyBtn.SetLeftAlign(true);
	m_DropTooManyBtn.SetTooltipText(IDS_DROP_TM);

	m_DropUnknownBtn.SetIcon(_T("DROPUN"),NULL);
	m_DropUnknownBtn.SetAlign(CButtonST::ST_ALIGN_HORIZ);
	m_DropUnknownBtn.SetLeftAlign(true);
	m_DropUnknownBtn.SetTooltipText(IDS_DROP_UN);
	...
	// Tux: Feature: Drop Buttons [start]
	AddAnchor(IDC_TRANSFER_DROP_FULLQ, TOP_LEFT);
	AddAnchor(IDC_TRANSFER_DROP_NNS, TOP_LEFT);
	AddAnchor(IDC_TRANSFER_DROP_TOO_MANY, TOP_LEFT);
	AddAnchor(IDC_TRANSFER_DROP_UNKNOWN, TOP_LEFT);
	// Tux: Feature: Drop Buttons [end]
	...
	//downloadlistactive=true;	// Tux: Fix: removed hover tracking [SLUGFILLER] 
	m_bIsDragging=false;
	...
	m_tooltipCats->SetDelayTime(TTDT_INITIAL, /*thePrefs.GetToolTipDelay()**/1000);	// Tux: LiteMule: Remove Tooltip Delay: changed line :)
...
void CTransferWnd::ShowQueueCount(uint32 number)
{
	...
	{
		TCHAR symbol[2] = _T("\x221E"); // Infinite Sign
		_sntprintf(buffer,_countof(buffer),_T("%u / %s (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(")"), number, symbol, theApp.clientlist->GetBannedCount());
	}
	else
		_sntprintf(buffer, _countof(buffer), _T("%u / %u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(")"), number, (thePrefs.GetQueueSize() + max(thePrefs.GetQueueSize()/4, 200)),theApp.clientlist->GetBannedCount()); 
	/*
...
void CTransferWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	// Tux: Feature: Drop Buttons [start]
	DDX_Control(pDX,IDC_TRANSFER_DROP_FULLQ, m_DropAllFullQBtn);
	DDX_Control(pDX,IDC_TRANSFER_DROP_NNS, m_DropAllNNSBtn);
	DDX_Control(pDX,IDC_TRANSFER_DROP_TOO_MANY, m_DropTooManyBtn);
	DDX_Control(pDX,IDC_TRANSFER_DROP_UNKNOWN, m_DropUnknownBtn);
	// Tux: Feature: Drop Buttons [end]
}
...
void CTransferWnd::UpdateSplitterRange()
{
	...
	// Tux: Feature: Drop Buttons [start]
	RemoveAnchor(IDC_TRANSFER_DROP_FULLQ);
	RemoveAnchor(IDC_TRANSFER_DROP_NNS);
	RemoveAnchor(IDC_TRANSFER_DROP_TOO_MANY);
	RemoveAnchor(IDC_TRANSFER_DROP_UNKNOWN);
	// Tux: Feature: Drop Buttons [end]
...
BOOL CTransferWnd::PreTranslateMessage(MSG* pMsg)
{
		...
		else if (!thePrefs.GetStraightWindowStyles() && pMsg->message == WM_MBUTTONUP)
	{
		...
		POINT pt;
		::GetCursorPos(&pt);
		downloadlistctrl.ScreenToClient(&pt);
		UINT flags;
		downloadlistctrl.HitTest(pt, &flags);
		if (flags & (LVHT_ONITEM | LVHT_NOWHERE))
		/*
...
void CTransferWnd::OnNmRClickDltab(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	PrioMenu.AppendMenu(MF_STRING, MP_PRIORELATIVE, GetResString(IDS_PRIORELATIVE));	// Tux: Feature: Relative Priority
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{ 
		...
		case MP_CAT_REMOVE: {
			...
			VerifyCatTabSize();
			if (toreload)
			{ // Tux: Feature: Automatic shared files updater
				theApp.emuledlg->sharedfileswnd->Reload();
		...
		// Tux: Feature: Drop Buttons [start]
		case IDC_TRANSFER_DROP_FULLQ: {
			downloadlistctrl.DropAllFullQ();
			break;
		}
		case IDC_TRANSFER_DROP_NNS: {
			downloadlistctrl.DropAllNNS();
			break;
		}
		case IDC_TRANSFER_DROP_TOO_MANY: {
			downloadlistctrl.DropAllTooMany();
			break;
		}
		case IDC_TRANSFER_DROP_UNKNOWN: {
			downloadlistctrl.DropUnknown();
			break;
		}
		// Tux: Feature: Drop Buttons [end]
		...
		case MP_PRIORELATIVE: {
			downloadlistctrl.SetRedraw(false);
			theApp.downloadqueue->SetCatRelativePriorities(rightclickindex);
			downloadlistctrl.SetRedraw(true);
			break;
		}
...
void CTransferWnd::VerifyCatTabSize()
{
	...
	// Tux: Feature: Drop Buttons [start]
	// *** thanks to Stulle for pointing out :o
	/*
	CRect rcBtnWnd1;
	...
	*/
	// We take the last button as the reference...
	CRect rcDropUnknownBtn;
	m_DropUnknownBtn.GetWindowRect(rcDropUnknownBtn);
	ScreenToClient(rcDropUnknownBtn);
	if (left < rcDropUnknownBtn.right + 10)
		left = rcDropUnknownBtn.right + 10;
	// Tux: Feature: Drop Buttons [end]
...
void CTransferWnd::ResetTransToolbar(bool bShowToolbar, bool bResetLists)
{
	...
	if (bShowToolbar)
	{
		...
		decal = 75;	// Tux: Feature: Drop Buttons
...
/*---*/
	...
	{
		decal = 0;	// Tux: Feature: Drop Buttons
...
void CTransferWnd::OnPaint()
{
	...
	// Tux: Feature: Drop Buttons [start]
	// -> needed to keep them in their place when toggling the additional toolbar...
	uint16 ui_drop_initial = 208 + (uint16)decal;
	uint8  ui_drop_size    = 22;	// one button's width

	GetDlgItem(IDC_TRANSFER_DROP_FULLQ)->MoveWindow((ui_drop_initial+decal),rcDown.top-28,ui_drop_size,28);
	GetDlgItem(IDC_TRANSFER_DROP_NNS)->MoveWindow((ui_drop_initial+decal)+(ui_drop_size+1),rcDown.top-28,ui_drop_size,28);
	GetDlgItem(IDC_TRANSFER_DROP_TOO_MANY)->MoveWindow((ui_drop_initial+decal)+(ui_drop_size+1)*2,rcDown.top-28,ui_drop_size,28);
	GetDlgItem(IDC_TRANSFER_DROP_UNKNOWN)->MoveWindow((ui_drop_initial+decal)+(ui_drop_size+1)*3,rcDown.top-28,ui_drop_size,28);
			
	VerifyCatTabSize();	// place the cats :)
	// Tux: Feature: Drop Buttons [end]
