...
#include "MenuCmds.h"
#include "UserMsgs.h"
#include "OtherFunctions.h"	// Tux: Improvement: longer tooltip support
...
#define NOTIFYICONDATA_V2_TIP_SIZE	128	// Tux: Improvement: longer tooltip support
...
{
	m_nidIconData.cbSize = (theApp.m_ullComCtrlVer >= MAKEDLLVERULL(5, 0, 0, 0)) ? sizeof(_NOTIFYICONDATAW) : NOTIFYICONDATA_V1_SIZE;	// Tux: Improvement: longer tooltip support: replaced line
...
// Tux: Improvement: removed unused code blocks

void CTrayDialog::TraySetToolTip(LPCTSTR lpszToolTip)
{
	// Tux: Improvement: longer tooltip support [start]
	size_t iMaxLen = (theApp.m_ullComCtrlVer >= MAKEDLLVERULL(5, 0, 0, 0)) ? NOTIFYICONDATA_V2_TIP_SIZE : NOTIFYICONDATA_V1_TIP_SIZE;
	ASSERT( _tcslen(lpszToolTip) > 0 && _tcslen(lpszToolTip) < iMaxLen );
	_tcsncpy(m_nidIconData.szTip, lpszToolTip, iMaxLen);
	m_nidIconData.szTip[iMaxLen - 1] = _T('\0');
	m_nidIconData.uFlags |= NIF_TIP;
	// Tux: Improvement: longer tooltip support [end]
...
{
	BOOL bSuccess = FALSE;
	// if (!m_bTrayIconVisible) // Tux: Fix: avoid tray icon disappearing [dolphinX]
	{
