...
void CUrlClient::SetRequestFile(CPartFile* pReqFile)
{
	...
	if (reqfile)
	{
		...
		// Tux: Fix: Memleak fix [start]
		if (m_abyPartStatus) 
		{ 
			delete[] m_abyPartStatus; 
			m_abyPartStatus = NULL; 
		} 
		// Tux: Fix: Memleak fix [end]
...
{
	if (nErrorCode == 0) {
		ConnectionEstablished();	// Tux: Fix: HTTP downloads actually connect now [WiZaRd]
...
//		Debug("  Start=%I64u  End=%I64u  Size=%u  %s\n", nStartPos, nEndPos, size, DbgGetFileInfo(reqfile->GetFileHash()));
	...
	// -> the question is: may an URLCLIENT have credits at all!?
	if (pAntiLeechData)
