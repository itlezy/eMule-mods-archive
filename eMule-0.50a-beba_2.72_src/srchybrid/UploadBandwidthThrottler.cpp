...
UploadBandwidthThrottler::UploadBandwidthThrottler(void) {
	...
	m_bOpenMoreSlotsIfNeeded = false;	// Tux: Feature: Slot Control
	normalOrder = true;	// Tux: Feature: Slot Focus
...
// Tux: Feature: Slot Control [start]: rewrote function
uint32 UploadBandwidthThrottler::GetSlotLimit(uint32 currentUpSpeed) {
	return currentUpSpeed/thePrefs.GetUpSlots();
}
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	uint32 uLastBlockProcess = timeGetTime() >> 10;
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
	...
	if (thePrefs.IsSlotFocusDisabled()) {	// Tux: Feature: Slot Focus
		...
		// Tux: Feature: Slot Control [start]: replaced block
		uint32 maxSlot = thePrefs.GetUpSlots();

		if (m_bOpenMoreSlotsIfNeeded && thePrefs.maxupload != UNLIMITED)
			m_highestNumberOfFullyActivatedSlots = maxSlot+2; // we need space for minimum 2 new clients if one of them is a very slow one
		else
		...
		// Tux: Feature: Slot Control [end]
			...
			if (theApp.uploadqueue) // Tux: Fix: Crash fix
	...
	if (thePrefs.IsSlotFocusDisabled() && normalOrder)
		normalOrder = false;
	else
		normalOrder = true;

	uint32 listSize = (uint32)m_StandardOrder_list.GetSize();
	// Tux: Feature: Slot Focus [end]
			...
			for(uint32 slotCounter = 0; slotCounter < (uint32)m_StandardOrder_list.GetSize() && bytesToSpend > 0 && spentBytes < (uint64)bytesToSpend; slotCounter++) {
				ThrottledFileSocket* socket = (normalOrder ? m_StandardOrder_list.GetAt(slotCounter) : m_StandardOrder_list.GetAt((listSize-1) - slotCounter));	// Tux: Feature: Slot Focus: changed socket value...
			...
			// Tux: Feature: Drop blocking clients [start]
			if (thisLoopTick >> 10 > uLastBlockProcess)
			{
				uLastBlockProcess = thisLoopTick >> 10;
