...
float CUpDownClient::GetCombinedFilePrioAndCredit() {
	...
	if (pAntiLeechData)
		return (10.0f*credits->GetScoreRatio(GetIP()) * float(GetFilePrioAsNumber())) * pAntiLeechData->GetScore();
	else
 ...
 */
int GetFilePrio(const CKnownFile* currequpfile);	// Tux: Feature: Client Analyzer
 ...
 */
uint32 CUpDownClient::GetScore(bool sysvalue, bool isdownloading, bool onlybasevalue, bool forceTimeScore) const	// Tux: Feature: Probabilistic Queue: added 4th argument
{
	if (!m_pszUsername)
		return 0;
	...
	*/
	...
	if (m_bGPLEvilDoerNick || m_bGPLEvilDoerMod || IsBanned())
		return 0;
	...
	if (!currequpfile)
		return 0;
	// Tux: Feature: Client Analyzer [end]
	bool bRandomQueue = (!forceTimeScore && thePrefs.GetRandQueue());	// Tux: Feature: Probabilistic Queue
	...
	// Tux: Feature: Probabilistic Queue [end]
		...
		// Tux: Feature: SUQWT [start]: changed calculation
		fBaseValue += (float)((int)(::GetTickCount() - m_dwUploadTime) > 900000)? 900000:1800000;
		fBaseValue /= 1000;
	...
	if (!onlybasevalue && // we actually need the complete score with waiting time
		...
		thePrefs.IsReleaseBonus()) // Release Bonus enabled
		fBaseValue += (21600.0f * thePrefs.GetReleaseBonus()); // 6h*factor
	// Tux: Feature: Release Bonus [end]

	// Tux: Feature: Analyzer CS [start]
	// -> replaced original code block
	if (thePrefs.UseCreditSystem() == 1 && pAntiLeechData)
		fBaseValue *= pAntiLeechData->GetScore(); // Analyzer credit system
	else
		fBaseValue *= credits->GetScoreRatio(GetIP()); // official credit system
	// Tux: Feature: Analyzer CS [end]

	if (!onlybasevalue)
	{
		int filepriority = GetFilePrio(currequpfile);	// Tux: Feature: Client Analyzer
	...
	// Tux: Feature: Probabilistic Queue [start]
	// note that this will pretty much drop the anti-leecher protection except for evildoers.
...
void CUpDownClient::CreateNextBlockPackage(bool bBigBuffer){
	...
	try{
        ...
        while (!m_BlockRequests_queue.IsEmpty() &&
			...
			// Tux: Improvement: Optimization by WiZaRd [start]
			bool compFlag = false;
			if (!IsUploadingToPeerCache() && m_byDataCompVer == 1)
			{
			// Tux: Improvement: Optimization by WiZaRd [end]
				...
				if (pos>-1)
					ext = ext.Mid(pos);
				compFlag = (ext!=_T(".zip") && ext!=_T(".cbz") && ext!=_T(".rar") && ext!=_T(".cbr") && ext!=_T(".ace") && ext!=_T(".ogm") && ext!=_T(".7z") && ext!=_T(".ape") && ext!=_T(".flac") && ext!=_T(".wv"));	// Tux: Improvement: added more uncompressible file types
...
bool CUpDownClient::ProcessExtendedInfo(CSafeMemFile* data, CKnownFile* tempreqfile,  bool isUDP)	// Tux: Improvement: better source finding: added bool
{
	...
	// Tux: Improvement: better source finding [start]
	...
	// Tux: Improvement: better source finding [end]
	...
	else
	{
		...
		m_abyUpPartStatus = new uint8[m_nUpPartCount];
		uint16 done = 0;
		uint16 complcount = 0;	// Tux: Feature: Client Analyzer
		while (done != m_nUpPartCount)
		{
			...
			for (UINT i = 0; i != 8; i++)
			{
				...
				// Tux: Improvement: better passive source finding [start]
				if (shouldbechecked && !bPartsNeeded && m_abyUpPartStatus[done] && !((CPartFile*)tempreqfile)->IsComplete((uint64)done*PARTSIZE, ((uint64)(done+1)*PARTSIZE)-1, false))
					bPartsNeeded = true;
				// Tux: Improvement: better passive source finding [end]
...
//					bPartsNeeded = true;
	...
	if (bPartsNeeded){
		if (GetDownloadState()!=DS_ONQUEUE){
			...
			if ((((CPartFile*)tempreqfile)->GetSourceCount() < ((CPartFile*)tempreqfile)->GetMaxSources()) || ((CPartFile*)tempreqfile)->GetSourceCount() < ((CPartFile*)tempreqfile)->GetMaxSources()*0.8f + 1) {
...
void CUpDownClient::CreatePackedPackets(byte* data, uint32 togo, Requested_Block_Struct* currentblock, bool bFromPF){
    ...
    while (togo){
		...
		if (currentblock->StartOffset > 0xFFFFFFFF || currentblock->EndOffset > 0xFFFFFFFF){
			...
			theStats.AddUpDataOverheadFileRequest(28);	// Tux: Fix: Statistics fix [WiZaRd]
		}
		else{
			...
			memfile.Read(&packet->pBuffer[24],nPacketSize);
			theStats.AddUpDataOverheadFileRequest(24);	// Tux: Fix: Statistics fix [WiZaRd]
		}
...
void CUpDownClient::SetUploadFileID(CKnownFile* newreqfile)
{
	...
	for (POSITION pos2,pos1=R4AF_List.GetHeadPosition();(pos2=pos1)!=NULL;) // update R4AF List
		if (R4AF_List.GetNext(pos1) == NULL)
			R4AF_List.RemoveAt(pos2);
	...
	if (newreqfile)
	{
		...
		// Tux: Feature: List all requested files [start]
		if (POSITION pos3 = R4AF_List.Find(newreqfile)) {
			R4AF_List.RemoveAt(pos3);
			R4AF_List.AddHead(newreqfile);
		}
		else
			R4AF_List.AddHead(newreqfile);
		// Tux: Feature: List all requested files [end]
...
void CUpDownClient::SendHashsetPacket(const uchar* pData, uint32 nSize, bool bFileIdentifiers)
{
	...
	else
	{
		...
		// Tux: Fix: prevent hashset probe [Enig123] [start]
		...
		// Tux: Fix: prevent hashset probe [Enig123] [end]
...
void CUpDownClient::Ban(LPCTSTR pszReason)
{
	...
	theApp.clientlist->AddTrackClient(this);
	ClearWaitStartTime();	// Tux: Feature: SUQWT
...
uint32 CUpDownClient::GetWaitStartTime() const
{
	...
	if (((int)(m_dwUploadTime - dwResult) < 0) && IsDownloading()){	// Tux: Feature: SUQWT: changed condition
