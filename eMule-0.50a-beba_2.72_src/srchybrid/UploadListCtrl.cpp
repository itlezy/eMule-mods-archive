...
#include "Clientlist.h"	// Tux: Feature: Clear Ban List
...
// Tux: Feature: Drop blocking clients [start]
#include "ListenSocket.h"
// Tux: Feature: Drop blocking clients [end]
...
void CUploadListCtrl::Init()
{
	...
	InsertColumn(8, GetResString(IDS_MOD),			LVCFMT_LEFT,  100,8);	// Tux: Feature: Mod Version
	...
	switch (iSubItem)
	{
		...
		case 2:
			...
			if (client->GetDatarate() > 0 && client->socket != NULL)
				_sntprintf(pszText, cchTextMax, _T("%s, %0.0f%%"), CastItoXBytes(client->GetDatarate(), false, true), client->socket->GetBlockRatio());
			else
			// Tux: Feature: Drop blocking clients [end]
...
int CUploadListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theApp.emuledlg->transferwnd->GetUploadList()->GetNextSortOrder(lParamSort)) != -1)
...
void CUploadListCtrl::OnNmDblClk(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	if (iSel != -1) {
		...
		if (client){
			...
			dialog.DoModal();
			*/
			client->GetDetailDialogInterface()->OpenDetailDialog(this);
...
void CUploadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.AppendMenu(MF_STRING | MF_ENABLED, MP_SLOTFOCUS, GetResString(IDS_SLOT_FOCUS));
	ClientMenu.CheckMenuItem(MP_SLOTFOCUS,( thePrefs.IsSlotFocusDisabled() ? MF_UNCHECKED : MF_CHECKED ));
	...
	FriendMenu.AddMenuTitle(NULL, true);	// Tux NOTE: thx 2 Xanatos
	...
	FriendMenu.AppendMenu(MF_STRING | (client && client->IsFriend() && !client->GetFriendSlot() ? MF_ENABLED : MF_GRAYED), MP_FRIENDSLOT, GetResString(IDS_FRIENDSLOT), _T("FRIENDSLOT"));
	...
	// Tux: Feature: List all requested files [start]
	ClientMenu.AppendMenu(MF_SEPARATOR);
	ClientMenu.AppendMenu(MF_STRING | ((client && client->IsEd2kClient()) ? MF_ENABLED : MF_GRAYED),MP_LIST_REQUESTED_FILES, GetResString(IDS_LIST_REQFILES), _T("FILEINFO"));
	// Tux: Feature: List all requested files [end]
...
BOOL CUploadListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	switch (wParam)
	{
		...
		// Tux: Feature: Slot Focus [start]
		case MP_SLOTFOCUS:
			thePrefs.SetSlotFocus(!thePrefs.IsSlotFocusDisabled());
			break;
		// Tux: Feature: Slot Focus [end]
	}
	...
	if (iSel != -1){
		...
		switch (wParam){
			...
			case IDA_ENTER:
			{
				client->GetDetailDialogInterface()->OpenDetailDialog(this);	// Tux: Feature: Modeless dialogs: replaced lines
				break;
			}
			case MP_BOOT:
