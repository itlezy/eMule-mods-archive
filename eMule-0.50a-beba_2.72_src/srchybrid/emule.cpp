...
// bool g_bLowColorDesktop = false; // Tux: LiteMule: no more low-color support: don't need this anymore
...
// -> changed output
#ifdef _DEBUG
	m_strCurVersionLong.Format(_T("%u.%u%c.%u | %s"), CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin, _T('a') + CemuleApp::m_nVersionUpd, CemuleApp::m_nVersionBld, MOD_VERSION);
#else
	m_strCurVersionLong.Format(_T("%u.%u%c | %s"), CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin, _T('a') + CemuleApp::m_nVersionUpd, MOD_VERSION);
#endif
...
// Workaround for bugged 'AfxSocketTerm' (needed at least for MFC 7.0, 7.1, 8.0, 9.0, 10.0)
#if _MFC_VER==0x0700 || _MFC_VER==0x0710 || _MFC_VER==0x0800 || _MFC_VER==0x0900 || _MFC_VER==0x0A00 // Tux: Improvement: also take VS2010 here
...
BOOL CemuleApp::InitInstance()
{
	...
	// Tux: Improvement: leak check on exit [start] [WiZaRd]
	...
	// Tux: Improvement: leak check on exit [end] [WiZaRd]
	...
	//
	// Tux: Fix by WiZaRd [start]
	// --> do NOT use the m_strCurVersionLongDbg because it might not be creatable on some FS 
	// --> because of special chars in the modname or such...
	/*
#ifndef _BETA
	...
	*/
	theCrashDumper.Enable(_T("eMule"), true, thePrefs.GetMuleDirectory(EMULE_CONFIGDIR));
	// Tux: Fix [end]
	...
	// Tux: Improvement: prevent switch to ... busy popup during windows startup [start]
	...
	// Tux: Improvement: prevent switch to ... busy popup during windows startup [end]
...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	antileechlist = new CAntiLeechDataList();	// Tux: Feature: Client Analyzer
...
{ 
	// Tux: Improvement: reduced OnlineSig overhead
	// Why suffer a function call overhead just to return?
	// -> removed lines
	...
	if (dwIP != 0){
		...
		if ( GetPublicIP() == 0)
			...
			if(ntohl(Kademlia::CKademlia::GetIPAddress()) != dwIP)
				// Tux: Feature: Reconnect Kad on IP-change [start]
				// --> changed code
			{
				AddDebugLogLine(DLP_DEFAULT, false,  _T("Public IP Address reported from Kademlia (%s) differs from new found (%s), resetting IP"),ipstr(ntohl(Kademlia::CKademlia::GetIPAddress())),ipstr(dwIP));
				//Kad loaded the old IP, we must reset
				if (Kademlia::CKademlia::IsRunning())
					Kademlia::CKademlia::GetPrefs()->SetIPAddress(htonl(dwIP));
				// Tux: Feature: Reconnect Kad on IP-change [end]
			}
...
{
	// Tux: LiteMule: no more low-color support: removed the VGACOLOR part
...
{
	// Tux: LiteMule: no more low-color support: removed major code parts here.
...
{
	if (thePrefs.GetReaskSrcAfterIPChange()==false)
		return;
	...
	for (int i = 0; i < 2; i++) {
		if (m_uLastValidID[i] != 0 && 
			uTempID[i] != 0 &&
			m_uLastValidID[i] != uTempID[i] &&
				::GetTickCount() >= m_dwLastIpCheckDetected+MIN2MS(15)) // no triggering for 15 minutes!
		{
			...
			m_dwLastIpCheckDetected = ::GetTickCount();

			AddLogLine(true, _T("Change from %u (%s ID) to %u (%s ID) detected%s"), 
				m_uLastValidID[i],
		...
		{ 
			...
			if(dwWaitStatus > WAIT_OBJECT_0 && dwWaitStatus < WAIT_OBJECT_0  + nChangeHandles)
			{
				...
				if (reloadShared)
				{
					...
					{
						// Tux NOTE: as minSecondsBetweenReloads is set to 0, this is entirely optional
						//           (if I'll add a slider later, this needs to be commented out)
/*
