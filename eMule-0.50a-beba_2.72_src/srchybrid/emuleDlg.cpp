...
#include "ClientAnalyzer.h"	// Tux: Feature: Client Analyzer
	...
	: CTrayDialog(CemuleDlg::IDD, pParent)
{
	...
	snarlIface = new Snarl::V42::SnarlInterface;
	if (snarlIface->IsSnarlRunning())
	{
		RegisterSnarl();
	}
	else 
...
CemuleDlg::~CemuleDlg()
{
	...
	// Tux: Feature: Snarl notifications [start]
	if (snarlIface != NULL) {
		snarlIface->ClearClasses();
		snarlIface->Unregister(L"eMuleBeba");
		delete snarlIface;
		snarlIface = NULL;
	}
	// Tux: Feature: Snarl notifications [end]
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	if (thePrefs.IsFirstStart())
	{
		...
		// Tux: Improvement: Show beba first start information [start]
		CString strBebaFirstStart = GetResString(IDS_BEBA_FIRST_START);
		AfxMessageBox(strBebaFirstStart, MB_ICONASTERISK);
		// Tux: Improvement: Show beba first start information [end]
	}
	...
	// Tux: Improvement: Startup crash dump improvement [start]
	CFileFind ff;
	CString searchPath = thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + _T("*.dmp");
	//check all part.met files
	if (ff.FindFile(searchPath, 0))
		AfxMessageBox(_T("Obviously beba crashed because a diagnostic .dmp file was found!\nPlease send it to the author to help him resolve the occured problem.\nDelete the .dmp file(s) from your eMule directory to make this message stop popping up. :)"));
	ff.Close();
	// Tux: Improvement: Startup crash dump improvement [end]
	...
	try
	{
		switch(theApp.emuledlg->status){
			...
			case 4:{
				...
				// Tux: Feature: Winsock2 support [start]
				// AddLogLine(false, _T("***************Winsock***************"));
				AddLogLine(false, _T("Winsock: Version %d.%d [%.40s] %.40s"), HIBYTE( theApp.m_wsaData.wVersion ),LOBYTE(theApp.m_wsaData.wVersion ), CString(theApp.m_wsaData.szDescription), CString(theApp.m_wsaData.szSystemStatus));
				...
				// AddLogLine(false, _T("***************Winsock***************"));
				// Tux: Feature: Winsock2 support [end]
...
void CemuleDlg::AddServerMessageLine(UINT uFlags, LPCTSTR pszLine)
{
	...
	if ((uFlags & LOGMSGTYPEMASK) == LOG_INFO)
		// Tux: Improvement: No server ads [start]
		//serverwnd->servermsgbox->AppendText(strMsgLine);
		;
		// Tux: Improvement: No server ads [end]
	else
...
CString CemuleDlg::GetConnectionStateString()
{
	...
	if (theApp.serverconnect->IsConnected())
		// Tux: Feature: show current server in status bar [start]
		//status = _T("eD2K:") + GetResString(IDS_CONNECTED);
		status = _T("eD2K:") + ( theApp.serverconnect->GetCurrentServer() ? theApp.serverlist->GetServerByAddress(theApp.serverconnect->GetCurrentServer()->GetAddress(), theApp.serverconnect->GetCurrentServer()->GetPort())->GetListName() : GetResString(IDS_CONNECTED) );
		// Tux: Feature: show current server in status bar [end]
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	{
		TCHAR buffer2[130];	// Tux: Improvement: longer tooltip support: changed from 64
		// set trayicon-icon
		...
		if (theApp.IsConnected()) {
			...
			//buffer2[_countof(buffer2) - 1] = _T('\0');	// Tux: Improvement: longer tooltip support: don't truncate here
		}
		else {
...
void CemuleDlg::ShowPing()
{
    if (IsWindowVisible())
	{
		...
		// Tux: Fix: USS item on status-bar fix [start]
		//statusbar->SetText(buffer, SBarChatMsg, 0);
		statusbar->SetText(buffer, SBarUSS, 0);
		// Tux: Fix: USS item on status-bar fix [end]
    }
}
...
void CemuleDlg::SetStatusBarPartsSize()
{
	...
	if(thePrefs.IsDynUpEnabled())
	{
        if (thePrefs.IsDynUpUseMillisecondPingTolerance())
			ussShift = 68; // Tux: Fix: USS item on status-bar fix: was 45
        else
            ussShift = 113; // Tux: Fix: USS item on status-bar fix: was 90
	}
	
	int aiWidths[6] = // Tux: Fix: USS item on status-bar fix: was 5
	{ 
		rect.right - 675 - ussShift,
		rect.right - 470 - ussShift,	// Tux: Feature: show current server in status bar: was 440
		rect.right - 275 - ussShift,	// Tux: Feature: show current server in status bar: was 250
		rect.right -  25 - ussShift,
		rect.right - ussShift,          // Tux: Fix: USS item on status-bar fix [start]
		-1
	};
...
LRESULT CemuleDlg::OnWMData(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (data->dwData == OP_ED2KLINK)
	{
		...
		else	// Tux: Feature: Modeless dialogs
			delete pCollection;
	}
...
{
	// Tux: Fix: Crash on shutdown fix [start]
	if (theApp.m_app_state == APP_STATE_SHUTTINGDOWN) {
		UnknownFile_Struct* hashed = (UnknownFile_Struct*)lParam;
		delete hashed;
		return FALSE;
	}
	// Tux: Fix: Crash on shutdown fix [end]
...
void CemuleDlg::OnClose()
{
	...
	theApp.EndDirectoryWatcher();	// Tux: Feature: Automatic shared files updater
	theApp.m_app_state = APP_STATE_SHUTTINGDOWN;
	theApp.serverconnect->Disconnect();
	if (thePrefs.IsOnlineSignatureEnabled())	// Tux: Improvement: reduced OnlineSig overhead
	...
	theApp.emuledlg->preferenceswnd->CloseDialog();	// Tux: Feature: Modeless dialogs
	...
	// theApp.sharedfiles->DeletePartFileInstances();	// Tux: Improvement: removeKindOf
	...
	// Tux: Fix: Crash fix [WiZaRd] [start]
	try
	{
	// Tux: Fix: Crash fix [WiZaRd] [end]
	// Tux: LiteMule: Remove MobileMule
	...
	delete theApp.antileechlist;		theApp.antileechlist = NULL;	// Tux: Feature: Client Analyzer
	// Tux: Fix: Crash fix [WiZaRd] [start]
	}	
	catch(...)
	{
...
void CemuleDlg::OnTrayLButtonUp(CPoint /*pt*/)
{
	...
	// Tux: Feature: Modeless dialogs
	// -> removed prefs dialog state check
...
{
	// Tux: Feature: Modeless dialogs: removed prefs dialog state check
	// the state of the preferences dialog doesn't matter since it's modeless

	if (TrayIsVisible())
		TrayHide();

	if (m_pMiniMule && m_pMiniMule->IsWindowVisible())	// Tux: Fix: Crash fix
		DestroyMiniMule();
...
void CemuleDlg::ShowNotifier(LPCTSTR pszText, int iMsgType, LPCTSTR pszLink, bool bForceSoundOFF)
{
	...
	// Tux: Feature: Snarl notifications [start]
	// note: will fall back to eMule's default notifier system if the interface is not
	//       active (Snarl not found). If hasSnarl differs from IsSnarlRunning(), something
	//       has changed - let us see what and register again if needed.
	if (snarlIface == NULL) {
		snarlIface = new Snarl::V42::SnarlInterface;
	}

	if (snarlIface->IsSnarlRunning() && !hasSnarl) {
		// Snarl responds, but eMule thinks it is gone. Register.
		// Sets hasSnarl to true on success.
		RegisterSnarl();
	}
	else if (!snarlIface->IsSnarlRunning() && hasSnarl) {
		// Snarl is gone, but eMule thinks it is still there. What?
		// Off with the shit. :-)
		hasSnarl = false;
		delete snarlIface;
		snarlIface = NULL;
...
//			m_wndTaskbarNotifier->Show(pszText, iMsgType, pszLink);
	...
	// Tux: Feature: Snarl notifications [start]
	if (bShowIt && hasSnarl && thePrefs.IsSnarlDisabled() == false)
		snarlIface->Notify(snarlClass, snarlTitle, pszText);
	else if (bShowIt && !hasSnarl)
		m_wndTaskbarNotifier->Show(pszText, iMsgType, pszLink);
	

//	if (bShowIt && !bForceSoundOFF && thePrefs.GetNotifierSoundType() != ntfstNoSound)
	if (bShowIt && !hasSnarl && !bForceSoundOFF && thePrefs.GetNotifierSoundType() != ntfstNoSound)
	// Tux: Feature: Snarl notifications [end]
	{
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		// Tux: others: added beba links [start]
		case MP_BEBA_BOARD:
			ShellExecute(NULL, NULL, _T("http://eMule-beba.de.vu"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		case MP_BEBA_BOARD_IT:
			ShellExecute(NULL, NULL, _T("http://eMule-beba-it.de.vu"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		case MP_BEBA_WEBSITE:
			ShellExecute(NULL, NULL, _T("http://beba.eMuleFuture.de"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		// Tux: Feature: beba version check [start]
		case MP_BEBA_VCHECK:{
			CString theUrl;
			theUrl.Format(CString(_T("http://beba.eMuleFuture.de/vcheck.php?vcheck=%s")), theApp.m_strCurVersionLong);
			ShellExecute(NULL, NULL, theUrl, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		// Tux: Feature: beba version check [end]
		// Tux: others: added beba links [end]
		case MP_WEBSVC_EDIT:
			theWebServices.Edit();
			break;
...
void CemuleDlg::ShowToolPopup(bool toolsonly)
{
	...
	// Tux: others: added beba links [start]
	Links.AppendMenu(MF_STRING,MP_BEBA_BOARD, GetResString(IDS_BEBA_BOARD), L"BEBA");
	Links.AppendMenu(MF_STRING,MP_BEBA_BOARD_IT, GetResString(IDS_BEBA_BOARD_IT), L"BEBA");	
	Links.AppendMenu(MF_STRING,MP_BEBA_WEBSITE, GetResString(IDS_BEBA_WEBSITE), L"BEBA");
	Links.AppendMenu(MF_STRING,MP_BEBA_VCHECK, GetResString(IDS_HM_LINKVC) + L" (beba)", L"BEBA");
	// Tux: others: added beba links [end]
	...
	Links.InsertMenu(4, MF_BYPOSITION | MF_SEPARATOR);	// Tux: others: menu improvement (was 3)
	Links.InsertMenu(8, MF_BYPOSITION | MF_SEPARATOR);	// Tux: others: menu improvement
	Links.AppendMenu(MF_STRING, MP_WEBSVC_EDIT, GetResString(IDS_WEBSVEDIT));

	// Tux: LiteMule: Remove Scheduler: removed code block
	if (!toolsonly) {
		if (theApp.serverconnect->IsConnected())
...
// Tux: LiteMule: Remove Splashscreen: removed code block
...
bool CemuleDlg::IsPreferencesDlgOpen() const
{
	...
	return preferenceswnd->IsDialogOpen();
	// Tux: Feature: Modeless dialogs [end]
}
...
LRESULT CemuleDlg::OnWebServerFileRename(WPARAM wParam, LPARAM lParam)
{
	...
	// Tux: Improvement: Code improvement [WiZaRd] [start]
	//sharedfileswnd->sharedfilesctrl.UpdateFile( (CKnownFile*)((CPartFile*)wParam));
	theApp.sharedfiles->UpdateFile((CKnownFile*)((CPartFile*)wParam));
	// Tux: Improvement: Code improvement [WiZaRd] [end]
...
// Tux: Improvement: UPnP removal [WiZaRd] [start]
...
// Tux: Improvement: UPnP removal [WiZaRd] [end]
...
{
	if (m_pTaskbarList /*&& thePrefs.IsWin7TaskbarGoodiesEnabled()*/) // Tux: LiteMule: Win7 addons always enabled: changed
	{
	...
	else
	{
		if (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed())
		{
			...
			if (hTheme != NULL)
			{
				...
				if (g_xpStyle.GetThemeColor(hTheme, TNP_BACKGROUND, 0, TMT_FILLCOLORHINT, &cr) != S_OK) // Tux: others: fixed warning C6217
					ASSERT( false );
...
// Tux: Feature: Snarl notifications [start]
void CemuleDlg::RegisterSnarl() {
	// register with Snarl
	if (thePrefs.IsSnarlDisabled() == true) {
		// the user does not want Snarl.
		if (snarlIface != NULL) {
			delete snarlIface;
			snarlIface = NULL;
		}
	...
	bool isSnarlRegistered = false;
	if (snarlIface == NULL) {
		snarlIface = new Snarl::V42::SnarlInterface;
	}

	if (snarlIface->Register(L"eMuleBeba", L"eMule beba") < 0) {
		// registering failed. is a previous session still registered?
		// note: this is (technically) very unlikely to happen at all.
		if (snarlIface->Unregister(L"eMuleBeba") > -1) {
			// old instance is unregistered now. register (properly) again.
			if (snarlIface->Register(L"eMuleBeba", L"eMule beba") >= 0) {
				// finally.
				isSnarlRegistered = true;
			}
		}
	}
	else {
	...
	if (isSnarlRegistered == true) {
		// add all used notification classes with a more-or-less appropriate description
		// (will be displayed on Snarl's application settings page)
		snarlIface->AddClass(L"Error",GetResString(IDS_ERROR));
		snarlIface->AddClass(L"Log",GetResString(IDS_SV_LOG));
		snarlIface->AddClass(L"DownloadDone",GetResString(IDS_PW_TBN_ONDOWNLOAD));
		snarlIface->AddClass(L"NewDownload",GetResString(IDS_TBN_ONNEWDOWNLOAD));
		snarlIface->AddClass(L"NewChatMsg",GetResString(IDS_PW_TBN_POP_ALWAYS));
		snarlIface->AddClass(L"NewVersion",GetResString(IDS_CB_TBN_ONNEWVERSION));
		snarlIface->AddClass(L"Misc",GetResString(IDS_PW_MISC));

		// tell our eMule instance that we are ready to go
		hasSnarl = true;
	}
	else 
	{
		// we could not register, so we will work without Snarl then
		hasSnarl = false;
	}
}
// Tux: Feature: Snarl notifications [end]