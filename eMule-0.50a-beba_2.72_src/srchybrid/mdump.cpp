...
#include <dbghelp.h>
#include "mdump.h"
// Tux: Feature: Dumpfile enumeration [start]
#include "eMule.h" //for "theApp"
#include "Preferences.h" //for "thePrefs"
// Tux: Feature: Dumpfile enumeration [end]
...
LONG CMiniDumper::TopLevelFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	if (hDll)
	{
		if (pfnMiniDumpWriteDump)
		{
			...
			// Do *NOT* localize that string (in fact, do not use MFC to load it)!
			// if (MessageBox(NULL, _T("eMule crashed :-(\r\n\r\nA diagnostic file can be created which will help the author to resolve this problem. This file will be saved on your Disk (and not sent).\r\n\r\nDo you want to create this file now?"), m_szAppName, MB_ICONSTOP | MB_YESNO) == IDYES)	// Tux: Improvement: Always do it!
			{
				// Create full path for DUMP file
				// Tux: Feature: Dumpfile enumeration [start]
				// -> replaced original code block
				//why do I use this?
				//reason is simple... this thing here might pop up multiple times 
				//until the user finally clicks on OK - thus rendering the initial
				//dumps useless because they will be overwritten...
				CString szDumpPath; 
				int i = 0;
				do{
					i++; 
					szDumpPath.Format(_T("%s%s%03i.dmp"), thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), CString(theApp.m_pszExeName), i);
				}
				while (PathFileExists(szDumpPath));
				// Tux: Feature: Dumpfile enumeration [end]
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					{
						// Do *NOT* localize that string (in fact, do not use MFC to load it)!
						// Tux NOTE: dumps@emule-project.net is stupid in this case ^^
						_sntprintf(szResult, _countof(szResult), _T("Saved dump file to \"%s\".\r\n\r\nPlease send this file together with a detailed bug report to der_tuxman@arcor.de!\r\n\r\nThank you for helping me to improve eMule beba."), szDumpPath);
