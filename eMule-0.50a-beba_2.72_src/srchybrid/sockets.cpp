...
#include "Log.h"
#include "kademlia/kademlia/kademlia.h"	// Tux: Improvement: Reconnect on Kad
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		CTag tagName(CT_NAME, thePrefs.GetAddServersFromServer() ? thePrefs.GetUserNick() : thePrefs.GetUserNick() + L" nolistsrvs");
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:
			...
			Kademlia::CKademlia::Stop();	// Tux: Improvement: Reconnect on Kad
			break;
...
{ 
	...
	try
	{
		// Tux: Improvement: Reconnect on Kad [start]
		if (thePrefs.GetNetworkKademlia())
			Kademlia::CKademlia::Start();
		// Tux: Improvement: Reconnect on Kad [end]
...
void CServerConnect::DestroySocket(CServerSocket* pSck){
	...
	// Tux: Fix: Socket deletion fix [NetFinity] [start]
	// netfinity: Remove from connection attempt list if not already done
	DWORD tmpkey;
	CServerSocket* tmpsock;
	...
	// netfinity: Make sure socket isn't used after deletion if it's the connected one
	if (connectedsocket == pSck) {
		connectedsocket = NULL;
		connected = false;
	}
	// Tux: Fix: Socket deletion fix [NetFinity] [end]
