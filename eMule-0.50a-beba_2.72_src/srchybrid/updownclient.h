...
class CAICHHash;
enum EUtf8Str;
class CAntiLeechData;	// Tux: Feature: Client Analyzer
class CClientDetailDialogInterface;	// Tux: Feature: Modeless dialogs
...
public:
	...
	*/
	...
	CClientDetailDialogInterface*	GetDetailDialogInterface() const { return m_detailDialogInterface; }	// Tux: Feature: Modeless dialogs
	// secure ident
	...
	// void			CheckForGPLEvilDoer();	// Tux: Feature: Client Analyzer
	...
	UINT			GetScore(bool sysvalue, bool isdownloading = false, bool onlybasevalue = false, bool forceTimeScore = true) const;	// Tux: Feature: Probabilistic Queue: added 4th argument
	...
	bool			ProcessExtendedInfo(CSafeMemFile* packet, CKnownFile* tempreqfile, bool isUDP=false);	// Tux: Improvement: better passive source finding
...
// <-- ZZ:DownloadManager
	...
	void			SetLastL2HACExecution(const uint32 m_set_to = 0)	{ m_last_l2hac_exec = m_set_to ? m_set_to : ::GetTickCount(); }
	uint32			GetLastL2HACExecution() const						{ return m_last_l2hac_exec; }
	...
	CTypedPtrList<CPtrList, CKnownFile*> R4AF_List;	// Tux: Feature: List all requested files
	...
	void	SetLastAskedTime(CPartFile* partFile, const uint32& in)	{ m_fileReaskTimes.SetAt(partFile, in); }	// Tux: Feature: Reask sources after IP change
...
protected:
	...
	bool	m_bIsML;
	//--group to aligned int32
	// bool	m_bGPLEvildoer;	// Tux: Feature: Client Analyzer
	...
	uchar		oldfileid[16];	// Tux: Feature: see OnUploadqueue
	UINT		m_slotNumber;
	...
	// Download
	//
public:    // Tux: Feature: Reask sources after IP change
	CPartFile*	reqfile;
protected: // Tux: Feature: Reask sources after IP change
	...
	DWORD	m_dwAskDelay;	// Tux: Feature: spreadReask
	...
	CClientDetailDialogInterface*	m_detailDialogInterface;	// Tux: Feature: Modeless dialogs

// Tux: Feature: Client Analyzer [start]
private: 
	UINT	m_uiRealVersion;
	CAntiLeechData* pAntiLeechData;
	bool	m_bGPLEvilDoerNick;
	bool	m_bGPLEvilDoerMod;
	bool	m_bIsBadClient; // BadClientFlag
public:
	UINT	GetRealVersion() const			{return m_uiRealVersion;}
	CAntiLeechData* GetAntiLeechData() const						{ return pAntiLeechData; }
	void	SetAntiLeechData(CAntiLeechData* data)	{pAntiLeechData = data;}
	...
	bool	IsBadClient() const { return m_bIsBadClient; }; // BadClientFlag
private:
	UINT m_fAICHHashRequested : 1,
	m_fSourceExchangeRequested : 1;
// Tux: Feature: Client Analyzer [end]
};
//#pragma pack()
