...
{
		DWORD dwNumIf = 0;
	if(GetNumberOfInterfaces(&dwNumIf) == NO_ERROR && dwNumIf > 0 ){
			PMIB_IFTABLE mibIfTable=NULL;
			DWORD dwSize = 0;
			DWORD dwRetVal = 0;
			mibIfTable = (MIB_IFTABLE*) malloc(sizeof(MIB_IFTABLE));
		if (GetIfTable(mibIfTable, &dwSize, true) == ERROR_INSUFFICIENT_BUFFER) {
				free(mibIfTable);
				mibIfTable = (MIB_IFTABLE *) malloc (dwSize);
			}

		if ((dwRetVal = GetIfTable(mibIfTable, &dwSize, true)) == NO_ERROR) {
				...
				if(gethostname(hostName, sizeof(hostName)) == 0){
					...
					if (GetIpAddrTable(mibIPAddrTable, &dwSize, FALSE) == ERROR_INSUFFICIENT_BUFFER) {
							free( mibIPAddrTable );
					...
					if(GetIpAddrTable(mibIPAddrTable, &dwSize, FALSE) == NO_ERROR){
