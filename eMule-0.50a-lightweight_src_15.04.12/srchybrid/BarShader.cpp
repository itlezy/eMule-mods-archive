...
void CBarShader::Draw(CDC* dc, int iLeft, int iTop, int iWidth, bool bFlat){
	Draw(dc, iLeft, iTop, iWidth, 0, m_uFileSize, bFlat);
}

void CBarShader::Draw(CDC* dc, int iLeft, int iTop, int iWidth, uint64 start, uint64 partSize, bool bFlat) {
	if(iWidth <= 0 || partSize == 0 || start > m_uFileSize) return;
	...
	if(start + partSize > m_uFileSize)
		partSize = m_uFileSize - start;
	// SLUGFILLER: speedBarShader
	POSITION pos;
	if(start == 0)
	...
	double m_dPixelsPerByte = (double)iWidth / partSize;
	double m_dBytesPerPixel = (double)partSize / iWidth;
