...
class CCollectionViewDialog : public CModResizableDialog // NEO: MLD - [ModelesDialogs] <-- Xanatos --
{
...
public:
	...
	void SetCollection(CCollection* pCollection, bool bSelfDelete = false); // NEO: MLD - [ModelesDialogs] <-- Xanatos --

protected:
	CButton m_AddNewCatagory;
	CStatic m_CollectionViewListLabel;
	CStatic m_CollectionViewListIcon;
	CComboBox m_temp;// X: [TD] - [TempDir]
	...
	bool	m_bSelfDelete; // NEO: MLD - [ModelesDialogs] <-- Xanatos --
	HICON m_icoWnd;
	HICON m_icoColl;
	...
	afx_msg void OnBnClickedCat();// X: [TD] - [TempDir]
};
