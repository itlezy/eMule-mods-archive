...
BOOL CemuleDlg::OnInitDialog()
{
	...
	*/
	...
	// set title
	SetWindowText(_T("eMule v") + theApp.m_strCurVersionLong + _T(" lightweight")); 
...
//<<< WiZaRd::ToolTip-FiX
	...
	if (IsWindowVisible() || bForceAll)
	{
		...
		{
			TCHAR szBuff[128];
			_sntprintf(szBuff, _countof(szBuff) - 1, _T("(D:%.1f U:%.1f) eMule v%s  (%s)"), (float)m_uDownDatarate/1024, (float)m_uUpDatarate/1024, theApp.m_strCurVersionLong + _T(" lightweight"),  CastSecondsToLngHM(sessionRunTime));
			szBuff[_countof(szBuff) - 1] = _T('\0');
