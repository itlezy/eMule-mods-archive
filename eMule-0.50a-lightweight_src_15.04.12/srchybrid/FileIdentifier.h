...
public:
	...
	*/
	...
	bool			SetMD4HashSet(const CAtlArray<uchar*>& aHashset);
	uchar*			GetMD4PartHash(size_t part) const;
	...
	CAtlArray<uchar*>&	GetRawMD4HashSet()									{ return m_aMD4HashSet; }
	...
	const CAtlArray<CAICHHash>&	GetRawAICHHashSet()	const					{ return m_aAICHPartHashSet; }
private:
	EMFileSize&				m_rFileSize;
	CAtlArray<uchar*>	m_aMD4HashSet;
	CAtlArray<CAICHHash>		m_aAICHPartHashSet;
};
...
__inline int CompareAICHHash(const CFileIdentifier& ident1,const CFileIdentifier& ident2, bool bSortAscending)
{
	if (ident1.HasAICHHash())
	{
		if (ident2.HasAICHHash())
			return memcmp(ident1.GetAICHHash().GetRawHashC(),ident2.GetAICHHash().GetRawHashC(), HASHSIZE);
		else
