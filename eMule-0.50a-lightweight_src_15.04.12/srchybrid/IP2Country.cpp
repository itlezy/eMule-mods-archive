...
bool CIP2Country::LoadFromFile(){// X: [SUL] - [SpeedUpLoading]
	...
	if(fread(&wBOM, sizeof(wBOM), 1, readFile) == 1 && (((WORD)wBOM == 0xFEFF) || ((WORD)wBOM == 0xFFFE))){
		fclose(readFile);
		//LogError(_T("Failed to load %s, the file is unicode"), ip2countryCSVfile);
		RemoveAllFlags();
		return false;
	}
	fseek(readFile, (wBOM & 0xFFFFFF) == 0xBFBBEF ? 3 : 0, SEEK_SET);
...
bool CIP2Country::LoadCountryFlagLib(){// X: [SUL] - [SpeedUpLoading]
	...
	CountryFlagImageList.Create(17,15,theApp.m_iDfltImageListColorFlags|ILC_MASK,0,1); //18,16
	...
	for(uint_ptr cur_pos = 0; cur_pos < _countof(countryID); cur_pos++){
		...
		iconHandle = (HICON)::LoadImage(_hCountryFlagDll, MAKEINTRESOURCE(resID[cur_pos]), IMAGE_ICON, 17, 15, LR_DEFAULTCOLOR); //18,16
		if(iconHandle)
		{
...
Country_Struct* CIP2Country::AddCountry(const LPSTR shortCountryName, const LPSTR longCountryName){// X: [IP2L] - [IP2Location]
	...
	newCountry->LongCountryName = OptUtf8ToStr(longCountryName, strlen(longCountryName));
...
void CIP2Country::AddIPRange(uint32 IPfrom,uint32 IPto, const LPSTR shortCountryName/*, const CHAR* midCountryName*/, const LPSTR longCountryName){// X: [SUL] - [SpeedUpLoading]
	...
	if (it1 == locationList.end()) {
		...
		if((locationEnd=strrchr(longCountryName, ']')) && (coutryEnd=strchr(longCountryName, '['))){
			*locationEnd = 0;
			*coutryEnd = 0;
			newLocation->locationName = OptUtf8ToStr(coutryEnd + 1, locationEnd - coutryEnd - 1);
