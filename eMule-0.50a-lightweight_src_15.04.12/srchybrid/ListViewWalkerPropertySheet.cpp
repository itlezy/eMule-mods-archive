...
BOOL CListViewWalkerPropertySheet::OnInitDialog()
{
	...
	if (m_pListCtrl != NULL)
	{
		...
		int iMax = 32767;
		static const int _aiPropSheetButtons[] = { IDOK, IDCANCEL, ID_APPLY_NOW };
		static const int _aiPropSheetButtonsStr[] = { IDS_TREEOPTIONS_OK, IDS_CANCEL, IDS_PW_APPLY };// X: [AL] - [Additional Localize]
		for (size_t i = 0; i < ARRSIZE(_aiPropSheetButtons); i++)
		{
			...
			if (pBtn /*&& pBtn->IsWindowVisible()*/)
			{
				...
				pBtn->SetWindowText(GetResString(_aiPropSheetButtonsStr[i]));// X: [AL] - [Additional Localize]
			}
		}
		...
		for (size_t i = 0; i < ARRSIZE(aCtrls); i++)
		{
...
void CListViewWalkerPropertySheet::ChangeData(void* pObj)
{
	m_aItems.RemoveAll();
	m_aItems.Add(pObj);
...
void CListViewWalkerPropertySheet::OnPrev()
{
	...
	void* pObj = m_pListCtrl->GetPrevSelectableItem();
	if (pObj)
		ChangeData(pObj);
	else
		MessageBeep(MB_OK);
...
void CListViewWalkerPropertySheet::OnNext()
{
	...
	void* pObj = m_pListCtrl->GetNextSelectableItem();
	if (pObj)
		ChangeData(pObj);
	else
		MessageBeep(MB_OK);
...
void* CListCtrlItemWalk::GetPrevSelectableItem()
{
	ASSERT( m_pListCtrl != NULL );
	if (m_pListCtrl == NULL)
		return NULL;
	...
	if (iItemCount >= 2)
	{
		...
		if (pos)
		{
			...
			if (iItem > 0)
			{
				...
				return (void*)m_pListCtrl->GetItemData(iItem);
			}
		}
	}
	return NULL;
...
void* CListCtrlItemWalk::GetNextSelectableItem()
{
	ASSERT( m_pListCtrl != NULL );
	if (m_pListCtrl == NULL)
		return NULL;
