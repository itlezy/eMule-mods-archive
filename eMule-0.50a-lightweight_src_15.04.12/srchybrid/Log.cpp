...
void AddLogTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	...
	_vsntprintf(szLogLine, _countof(szLogLine) - 1, pszLine, argptr);
	...
	else
	{
		...
		int iLen = _sntprintf(szFullLogLine, _countof(szFullLogLine) - 1, _T("%s: %s\r\n"), CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), szLogLine);
		if (iLen < 0){
			_tcsncpy(szFullLogLine + _countof(szFullLogLine) - 4, _T("..."), 4);
			iLen = _countof(szFullLogLine) - 1;
		}
		if (iLen > 0)
		{
			if (!(uFlags & LOG_DEBUG))
...
bool CLogFile::Log(LPCTSTR pszMsg, int iLen)
{
	...
	if (m_eFileFormat == Unicode)
	{
		...
		uint32 uToWrite = ((iLen == -1) ? (uint32)_tcslen(pszMsg) : iLen)*sizeof(TCHAR);
		uWritten = (uint32)fwrite(pszMsg, 1, uToWrite, m_fp);
	}
	else
	{
		TUnicodeToUTF8<2048> utf8(pszMsg, iLen);
		uWritten = (uint32)fwrite((LPCSTR)utf8, 1, utf8.GetLength(), m_fp);
	}
...
bool CLogFile::Logf(LPCTSTR pszFmt, ...)
{
	...
	TCHAR szMsg[1024];
	_vsntprintf(szMsg, _countof(szMsg) - 1, pszFmt, argp);
	szMsg[_countof(szMsg) - 1] = _T('\0');

	TCHAR szFullMsg[1060];
	int iLen = _sntprintf(szFullMsg, _countof(szFullMsg) - 1, _T("%s: %s\r\n"), CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), szMsg);
	if (iLen == 0)
		return false;
	va_end(argp);
	if (iLen < 0){
		_tcsncpy(szFullMsg + _countof(szFullMsg) - 4, _T("..."), 4);
		iLen = _countof(szFullMsg) - 1;
	}
...
void CLogFile::StartNewLogFile()
{
	...
	_tsplitpath_s(m_strFilePath, szDrv, szDir, szNam, szExt);
