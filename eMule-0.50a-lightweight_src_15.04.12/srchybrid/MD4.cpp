...
{
	if (MD4_asm_m_nCount0 != offsetof(CMD4, m_nCount) ){
		ASSERT(0);
		return false;
	}
...
void CMD4::GetHash(MD4* pHash)
{
	md4cpy(pHash->b, m_nState);
}

const BYTE* CMD4::GetHash() const
{
	return (const BYTE*)m_nState;
}
...
void CMD4::Finish()
{
	...
	uint32* m_nCount32=reinterpret_cast<uint32*>(&m_nCount);
	// Save number of bits
	bits[1] = ( m_nCount32[1] << 3 ) + ( m_nCount32[0] >> 29);
	bits[0] = m_nCount32[0] << 3;
	// Pad out to 56 mod 64.
	index = (unsigned int)(m_nCount32[0] & 0x3f);
