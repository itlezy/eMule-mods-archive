...
#ifndef NOT_USE__ASM// X: [64P] - [64BitPortable]
///////////////////////////////////////////////////////////////////////////////
...
{
	if (SHA_asm_m_nCount0 != offsetof(CSHA, m_nCount) ){
		ASSERT(0);
		return false;
	}
...
{
	memcpy(pHash->b, m_hash.b, HASHSIZE);
}
...
void CSHA::Finish()
{
	...
	index = static_cast< uint32 >( m_nCount ) % SHA1_BLOCK_SIZE;
...
CString CSHA::HashToString(const SHA1* pHashIn, BOOL bURN)
{
	...
	for ( uint_ptr nChar = 32 ; nChar ; nChar-- )
	{
		BYTE nBits = 0;

		for ( uint_ptr nBit = 0 ; nBit < 5 ; nBit++ )
		{
			if ( nBit ) nBits <<= 1;
...
CString CSHA::HashToHexString(const SHA1* pHashIn, BOOL bURN)
{
	...
	for ( uint_ptr nByte = 0 ; nByte < 20 ; nByte++, pHash++ )
	{
...
BOOL CSHA::HashFromString(LPCTSTR pszHash, SHA1* pHashIn)
{
	...
	for ( uint_ptr nChars = 32 ; nChars-- ; pszHash++ )
	{
