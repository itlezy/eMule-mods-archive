...
void CSearchParamsWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	else if (cx < MIN_HORZ_WIDTH)
	{
		...
		int iNameWidth = rcClient.Width() - 4;
...
void CSearchParamsWnd::UpdateControls()
{
	...
	if (iMethod != CB_ERR)
	{
		if (iMethod != thePrefs.GetSearchMethod())
		{
			if (iMethod == SearchTypeKademlia)
				...
				||(iMethod == SearchTypeEd2kGlobal && m_searchdlg->IsGlobalEd2kSearchRunning()))// X: [CI] - [Code Improvement]
	...
	m_ctlOpts.SetItemData(orTitle, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal) ? 1 : 0);
	m_ctlOpts.SetItemData(orAlbum, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal) ? 1 : 0);
	m_ctlOpts.SetItemData(orArtist, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal) ? 1 : 0);
}
...
void CSearchParamsWnd::InitMethodsCtrl()
{
	...
	VERIFY( m_ctlMethod.AddItem(GetResString(IDS_SERVER)) == SearchTypeEd2kServer-1 );
	VERIFY( m_ctlMethod.AddItem(GetResString(IDS_GLOBALSEARCH)) == SearchTypeEd2kGlobal-1 );
	VERIFY( m_ctlMethod.AddItem(GetResString(IDS_KADEMLIA) + _T(' ') + GetResString(IDS_NETWORK)) == SearchTypeKademlia-1 );
...
#endif
	SFileTypeCbEntry(const CString& strLabel, LPCSTR pszItemData) {
		m_strLabel = strLabel;
...
void CSearchParamsWnd::InitFileTypesCtrl()
{
	...
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_ANY), ""));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_ARC), ED2KFTSTR_ARCHIVE));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_AUDIO), ED2KFTSTR_AUDIO));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_CDIMG), ED2KFTSTR_CDIMAGE));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_PICS), ED2KFTSTR_IMAGE));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_PRG), ED2KFTSTR_PROGRAM));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_VIDEO), ED2KFTSTR_VIDEO));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_DOC), ED2KFTSTR_DOCUMENT));
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_EMULECOLLECTION), ED2KFTSTR_EMULECOLLECTION));
	...
	{
		int iItem;
		if ((iItem = m_ctlFileType.AddItem((*it).m_strLabel)) != CB_ERR)
...
void CSearchParamsWnd::Localize()// X: [RUL] - [Remove Useless Localize]
{
...
void CSearchParamsWnd::LocalizeAll()// X: [RUL] - [Remove Useless Localize]
{
	Localize();
...
uint_ptr CSearchParamsWnd::GetSearchAttrNumber(const CString& rstrExpr)
{
	CString strExpr(rstrExpr);
	strExpr.Trim();
	LPTSTR endptr = NULL;
	...
	if (endptr && *endptr != _T('\0'))
	{
		...
		if (chModifier == _T('\0'))
			...
			return (uint_ptr)(dbl*1000 + 0.5);
		else if (chModifier == _T('m'))
			return (uint_ptr)(dbl*1000*1000 + 0.5);
		else if (chModifier == _T('g'))
			return (uint_ptr)(dbl*1000*1000*1000 + 0.5);
		else
			return (uint_ptr)-1;
	}
	return (uint_ptr)(dbl + 0.5);
}

uint_ptr CSearchParamsWnd::GetSearchAttrLength(const CString& rstrExpr)
{
	CString strExpr(rstrExpr);
	strExpr.Trim();

	uint_ptr hour = 0, min = 0, sec = 0;
	...
	if (endptr && *endptr != _T('\0'))
	{
		...
		if (chModifier == _T('\0') || chModifier == _T('s'))
			...
			return (uint_ptr)(dbl*60 + 0.5);
		else if (chModifier == _T('h'))
			return (uint_ptr)(dbl*60*60 + 0.5);
		else
			return (uint_ptr)-1;
	}
	return (uint_ptr)(dbl + 0.5);
...
SSearchParams* CSearchParamsWnd::GetParameters()
{
	...
	pParams->eType = (ESearchType)(m_ctlMethod.GetCurSel()+1);
