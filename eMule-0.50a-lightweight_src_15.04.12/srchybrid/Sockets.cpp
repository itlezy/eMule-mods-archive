...
#include "ServerWnd.h"
#include "Log.h"
#include "kademlia/kademlia/Kademlia.h" // X-Ray :: ReconnectToKadAfterConnectionError
...
void CServerConnect::ConnectToAnyServer(UINT startAt, bool prioSort, bool isAuto, bool bNoCrypt)
{
   ...
   singleconnecting = false;
	...
	if (thePrefs.GetAutoConnectToStaticServersOnly() && isAuto)
	{
		...
		{
			if (next_server->IsStaticMember() && (!thePrefs.GetFilterServerByIP() || !next_server->IsFiltered())) {// X: [CI] - [Code Improvement]
				anystatic = true;
				break;
			}
		}
...
void CServerConnect::ConnectToServer(CServer* server, bool multiconnect, bool bNoCrypt)
{
   if (!multiconnect) {
	...
	if(thePrefs.GetFilterServerByIP() && server->IsFiltered()/*theApp.ipfilter->IsFiltered(server->GetIP())*/)// X: [CI] - [Code Improvement]
	{
	...
	newsocket->Create(0, SOCK_STREAM, FD_READ | FD_WRITE | FD_CLOSE | FD_CONNECT, thePrefs.GetBindAddrW());
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
   if (!connecting) {
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		//WiZaRd: according to Lugdunummaster, "nolistsrvs" in the nick tells the server that we do not want the server list
		//thus, the server can save that overhead if we don't intend to use that data anyways
		CTag tagName(CT_NAME, DEFAULT_NICK/*thePrefs.GetUserNick()*/ + L" nolistsrvs");
...
//Morph Start - aux Ports, by lugdunummaster
		CTag tagFlags(CT_SERVER_FLAGS, SRVCAP_ZLIB | SRVCAP_NEWTAGS | SRVCAP_LARGEFILES | SRVCAP_UNICODE | dwCryptFlags | SRVCAP_AUXPORT); // aux port compatible client
//Morph End - aux Ports, by lugdunummaster
		...
		Packet* packet = new Packet(&data, OP_EDONKEYPROT, OP_LOGINREQUEST);
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:
			...
			Kademlia::CKademlia::Stop(); // X-Ray :: ReconnectToKadAfterConnectionError
			break;
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:{
			...
			if (thePrefs.Reconnect() && autoretry && !m_idRetryTimer) {
				...
				if (pServer) {
					...
					size_t iPosInList = theApp.serverlist->GetPositionOfServer(pServer);
					if (iPosInList != (size_t)-1)
		...
		case CS_DISCONNECTED:{
			...
			if(thePrefs.GetNotifier())
				theApp.emuledlg->TraySetBalloonToolTip(GetResString(IDS_WARNING), GetResString(IDS_CONNECTIONLOST), NIIF_WARNING);
			break;
		}
		case CS_ERROR:
		case CS_NOTCONNECTED:{
...
#ifdef REPLACE_ATLMAP
			for (unordered_map<ULONG, CServerSocket*>::const_iterator it = connectionattemps.begin(); it != connectionattemps.end();)
			{
				if (it->second == sender) {
...
{ 
	...
	try
	{
		// X-Ray :: ReconnectToKadAfterConnectionError :: Start
		//if(thePrefs.GetNetworkKademlia()){
			Kademlia::CKademlia::Start();
			theApp.emuledlg->ShowConnectionState();
		//}
		// X-Ray :: ReconnectToKadAfterConnectionError :: End
...
{ 
	...
	for (unordered_map<ULONG, CServerSocket*>::const_iterator it = connectionattemps.begin(); it != connectionattemps.end();)
	{
		DWORD tmpkey = it->first;
		CServerSocket* tmpsock = it->second;
#else
	DWORD tmpkey;
	CServerSocket* tmpsock;
	POSITION pos = connectionattemps.GetStartPosition();
...
void CServerConnect::DestroySocket(CServerSocket* pSck){
...
#ifdef REPLACE_ATLMAP
	for (unordered_map<ULONG, CServerSocket*>::const_iterator it = connectionattemps.begin(); it != connectionattemps.end(); ++it)
	{
		if (it->second == pSck) {
			connectionattemps.erase(it);
			break;
	...
	if(pos != NULL){
		...
		// Make sure socket isn't used after deletion if it's the connected one
...
uint16 CServerConnect::GetUDPPort()
{
	if(udpsocket)
