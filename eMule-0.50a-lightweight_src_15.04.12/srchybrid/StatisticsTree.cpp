...
//TK4 MOD +
	mnuContext.AppendMenu(MF_SEPARATOR);
	if(thePrefs.GetSplitterbarPositionStat() < 97) myFlags = MF_GRAYED;
	 else                                          myFlags = MF_STRING;
    mnuContext.AppendMenu(myFlags, MP_HIDE_SCOPES, _T("Restore Scopes"));
//TK4 MOD -
...
CString CStatisticsTree::GetText(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	CString	strBuffer;
	if (bPrintHeader)
		strBuffer.Format(_T("eMule v%s %s\r\n\r\n"), theApp.m_strCurVersionLong, GetResString(IDS_SF_STATISTICS));
