...
sint_ptr utf8towc(LPCSTR pcUtf8, uint_ptr uUtf8Size, LPWSTR pwc, uint_ptr uWideCharSize)
{
	LPWSTR pwc0 = pwc;
    ...
    while (uUtf8Size && uWideCharSize)
    {
        ...
        else
        {
            ...
            uint_ptr uProcessedBytes = 1 + uExpectedBytes;
            uint_ptr uWideChar = (uint_ptr)(ucChar & ~ucMask);
            if (uExpectedBytes == 1)
			{
                if ((uWideChar & 0x1E) == 0)
                    return -1; // Invalid UTF8 string..
            ...
            if (uWideCharSize < (uint_ptr)(uExpectedBytes > 2) + 1)
                break; // buffer full
...
sint_ptr ByteStreamToWideChar(LPCSTR pcUtf8, uint_ptr uUtf8Size, LPWSTR pwc, uint_ptr uWideCharSize)
{
	sint_ptr iWideChars = utf8towc(pcUtf8, uUtf8Size, pwc, uWideCharSize);
	if (iWideChars < 0)
	{
		LPWSTR pwc0 = pwc;
		while (uUtf8Size && uWideCharSize)
...
CString OptUtf8ToStr(const CStringA& rastr)
{
	...
	sint_ptr iWideChars = utf8towc(rastr, rastr.GetLength(), pwsz, iMaxWideStrLen);
	if (iWideChars <= 0)
	{
		// invalid UTF8 string...
...
CString OptUtf8ToStr(LPCSTR psz, uint_ptr iLen)
{
	CStringW wstr;
	uint_ptr iMaxWideStrLen = iLen;
	LPWSTR pwsz = wstr.GetBuffer(iMaxWideStrLen);
	sint_ptr iWideChars = utf8towc(psz, iLen, pwsz, iMaxWideStrLen);
	if (iWideChars <= 0)
	{
		// invalid UTF8 string...
		wstr.ReleaseBuffer(0);
...
CStringW DecodeDoubleEncodedUtf8(LPCWSTR pszFileName)
{
	...
	sint_ptr iNewChars = utf8towc(strA, nChars, pszW, nChars);
	if (iNewChars < 0) {
