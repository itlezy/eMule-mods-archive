...
void CToolTipCtrlX::CustomPaint(LPNMTTCUSTOMDRAW pNMCD)
{
	...
	while (iPos != -1)
	{
		...
		if (iColon != -1) {
			...
			if(iMaxCol1Width<siz.cx)
				iMaxCol1Width = siz.cx;
		...
		else if (!strLine.IsEmpty() && strLine.Compare(_T("<br>")) != 0) {
			CSize siz;
			if (hTheme && bUseEmbeddedThemeFonts) {
				CRect rcExtent;
				RECT rcBounding = {0, 0, 32767, 32767};
	...
	else
	{
		...
		while (iPos != -1)
		{
			...
			if (iColon != -1) {
				...
				if (bAutoFormatText && strLine.Compare(_T("<br>")) == 0){
					CPen pen;
					pen.CreatePen(0, 1, m_crTooltipTextColor);
					CPen *pOP = pdc->SelectObject(&pen);
