...
struct SServerDNSRequest
{
	...
	CServer* m_pServer;
	CAtlList<SRawServerPacket*> m_aPackets;
};
...
CUDPSocket::CUDPSocket() : m_bWouldBlock(false), m_hWndResolveMessage(NULL), nPort(0)
{
}
...
bool CUDPSocket::Create()
{
	if (thePrefs.GetServerUDPPort())
	{
		...
		if (!CAsyncSocketEx::Create(thePrefs.GetServerUDPPort()==0xFFFF ? 0 : thePrefs.GetServerUDPPort(), SOCK_DGRAM, FD_READ | FD_WRITE, thePrefs.GetBindAddrW())){// X: [SUDPS] - [CAsyncSocketEx UDP Support]
			LogError(LOG_STATUSBAR, _T("Error: Server UDP socket: Failed to create server UDP socket - %s"), GetErrorMessage(GetLastError()));
			return false;
		}
		if (thePrefs.IsUPnPEnabled() && theApp.m_pUPnPFinder != NULL) // add the port to existing mapping without having eMule restarting (if all conditions are met)
		{
			CString client;
			UINT port;
			GetSockName(client, port);
			nPort = port;
			theApp.m_pUPnPFinder->GetImplementation()->ChangePort(PI_ServerUDP, nPort);
		}
			...
			case OP_GLOBFOUNDSOURCES:{
 			...
 			case OP_GLOBSERVSTATRES:{
				...
				if (pServer != NULL){
					...
					uint64 tNow = (uint64)time(NULL);
					pServer->SetLastPingedTime(tNow - (t_rng->getUInt32() % HR2S(1))); // if we used Obfuscated ping, we still need to reset the time properly
				}
				...
				if (size >= 28){
					...
					if (thePrefs.GetDebugServerUDPLevel() > 0){
						...
						if (uUDPFlags & SRV_UDPFLG_EXT_GETSOURCES)
							strInfo.Append(_T("  ExtGetSources=1"));// X: [CI] - [Code Improvement]
						if (uUDPFlags & SRV_UDPFLG_EXT_GETSOURCES2)
							strInfo.Append(_T("  ExtGetSources2=1"));
						if (uUDPFlags & SRV_UDPFLG_EXT_GETFILES)
							strInfo.Append(_T("  ExtGetFiles=1"));
						if (uUDPFlags & SRV_UDPFLG_NEWTAGS)
							strInfo.Append(_T("  NewTags=1"));
						if (uUDPFlags & SRV_UDPFLG_UNICODE)
							strInfo.Append(_T("  Unicode=1"));
						if (uUDPFlags & SRV_UDPFLG_LARGEFILES)
							strInfo.Append(_T("  LargeFiles=1"));
						if (uUDPFlags & SRV_UDPFLG_UDPOBFUSCATION)
							strInfo.Append(_T("  UDP_Obfuscation=1"));
						if (uUDPFlags & SRV_UDPFLG_TCPOBFUSCATION)
							strInfo.Append(_T("  TCP_Obfuscation=1"));
						Debug(_T("%s\n"), strInfo);
					}
#endif
				}
				...
				if (pServer){
					...
					if (pServer->GetLastDescPingedCount() < 2)
					{
						...
						uint32 uDescReqChallenge = ((uint32)t_rng->getUInt16() << 16) + INV_SERV_DESC_LEN; // 0xF0FF = an 'invalid' string length.
...
void CUDPSocket::DnsLookupDone(WPARAM wp, LPARAM lp)
{
	...
	if (nIP != INADDR_NONE)
	{
		...
		if (!bRemoveServer && thePrefs.GetFilterServerByIP() && theApp.ipfilter->IsFiltered(nIP)) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(UDP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter"), pDNSReq->m_pServer->GetAddress(), ipstr(nIP));
			bRemoveServer = true;
		}
		...
		if (pServer)
			pServer->SetFiltered(thePrefs.GetFilterServerByIP() && theApp.ipfilter->IsFiltered(nIP));// X: [CI] - [Code Improvement]
...
// ZZ:UploadBandWithThrottler (UDP) -->
    sendLocker.Lock();
	m_bWouldBlock = false; // X-Ray :: FiXeS :: Bugfix :: SiRoB
...
int CUDPSocket::SendTo(BYTE* lpBuf, int nBufLen, uint32 dwIP, uint16 nPort)
{
	// NOTE: *** This function is invoked from a *different* thread!
	/*
	int iResult = CAsyncSocketEx::SendTo(lpBuf, nBufLen, nPort, ipstr(dwIP));// X: [SUDPS] - [CAsyncSocketEx UDP Support]
	*/	//Enig123
	...
	int iResult = CAsyncSocketEx::SendTo(lpBuf, nBufLen, (SOCKADDR*)&sockAddr, sizeof(sockAddr), 0);// X: [SUDPS] - [CAsyncSocketEx UDP Support]
