...
//>>> WiZaRd::KadAddons
#include "../utils/KadHelper.h" //>>> WiZaRd::Kad Helper
//<<< WiZaRd::KadAddons
...
void CKademliaUDPListener::Bootstrap(uint32 uIP, uint16 uUDPPort, uint8 byKadVersion, const CUInt128* uCryptTargetID)
	{
		...
		SendPacket(&fileIO, KADEMLIA2_BOOTSTRAP_REQ, uIP, uUDPPort, 0, (byKadVersion >= KADEMLIA_VERSION6_49aBETA)?uCryptTargetID:NULL);
	}
...
void CKademliaUDPListener::SendMyDetails(byte byOpcode, uint32 uIP, uint16 uUDPPort, uint8 byKadVersion, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID, bool bRequestAckPackage)
{
	if (byKadVersion > 1)
	{
		...
		uint_ptr uLen = sizeof(byPacket) - byteIOResponse.GetAvailable();
...
void CKademliaUDPListener::SendPublishSourcePacket(const CContact* pContact, const CUInt128 &uTargetID, const CUInt128 &uContactID, const TagList& tags)
{
	...
	if (pContact->GetVersion() >= KADEMLIA_VERSION4_47c)// X: [CI] - [Code Improvement]
	{
	...
	uint_ptr uLen = sizeof(byPacket) - byteIO.GetAvailable();
	if (pContact->GetVersion() >= KADEMLIA_VERSION6_48b) // obfuscated? // X: [CI] - [Code Improvement]
	{
...
//>>> WiZaRd::KadAddons
//>>> WiZaRd::Kad Helper
	//if an error occurred, kill that bad contact now so we won't spread it anymore
	}
	catch(CString err)
	{
		KillBadContact(uIP, uUDPPort, 0);
		throw err;
	}
	catch(...)
	{		
...
//>>> WiZaRd::KadAddons
// Fix possible mem corruption (WiZaRd / taz)
//	CSafeMemFile fileIO(521);
//	CSafeMemFile fileIO(2+16+uNumContacts*25);
	CSafeMemFile fileIO(16+2+1+2+uNumContacts*(16+4+2+2+1));
//<<< WiZaRd::KadAddons
...
//		CSafeMemFile fileIO2( 817 );
		CSafeMemFile fileIO2( 16+1+uCount*(16+4+2+2+1) );
//<<< WiZaRd::KadAddons
...
// BEGIN netfinity: Safe KAD - Ensure we are actually searching for these nodes
	_CheckForValidTarget(__FUNCTION__, uTarget);
...
//<<< WiZaRd::KadAddons
	...
	{
//>>> WiZaRd::KadAddons
		CAtlList<UINT> uProcessedIPs; // netfinity: Safe KAD - Using a list for simplicity
//<<< WiZaRd::KadAddons
...
// BEGIN netfinity: Safe KAD
			...
			if (uIP == uIPResult && uUDPPort == uUDPPortResult)
			{
				if (::thePrefs.GetLogFilteredIPs())
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - IP is identical to the senders IP") , ipstr(uhostIPResult), uUDPPortResult);
				continue;
			}
//>>> WiZaRd
			const UINT ourIP = CKademlia::GetPrefs()->GetIPAddress();
			if (ourIP != 0 && ourIP == uIPResult && thePrefs.GetUDPPort() == uUDPPortResult)
			{
				if (::thePrefs.GetLogFilteredIPs())
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - sender %s:%u sent us our OWN IP"), ipstr(uhostIPResult), uUDPPortResult, ipstr(ntohl(uIP)), uUDPPort);
				continue;
			}
//<<< WiZaRd
			const UINT uFilter = uIPResult & 0xFFFFF000;
			// Only allow one node per IP range
			if (uProcessedIPs.Find(uFilter))
			{
				if (::thePrefs.GetLogFilteredIPs())
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s) - IP too similar to an already processed contact") , ipstr(uhostIPResult));
				continue;
			}
			uProcessedIPs.AddTail(uFilter);
...
//>>> WiZaRd::KadAddons
//>>> WiZaRd::Kad Helper
					if(IsBadContact(uIPResult, uUDPPortResult, uTCPPortResult))
					{
						//if (::thePrefs.GetLogFilteredIPs())
						AddDebugLogLine(false, _T("Ignored blacklisted kad contact (IP=%s)"), ipstr(uhostIPResult));
						continue;
					}
//<<< WiZaRd::Kad Helper
//<<< WiZaRd::KadAddons
					...
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - IP filter") , ipstr(uhostIPResult), uUDPPortResult);
...
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_KEY_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	while( uCount > 0 )
	{
		...
		try
		{
			...
			pEntry->m_tLifetime = (uint64)time(NULL)+KADEMLIAREPUBLISHTIMEK;// X: [64T] - [64BitTime]
			...
			while(uTags > 0)
			{
				...
				if(pTag)
				{
					...
					else if (!pTag->m_name.Compare(TAG_FILESIZE))
					{
						...
						{
						if(pTag->IsBsob() && pTag->GetBsobSize() == HASHSIZE)
						{
...
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_SOURCE_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	try
	{
		...
		pEntry->m_uSourceID.SetValue(uTarget);
		pEntry->m_bSource = false;
		pEntry->m_tLifetime = (uint64)time(NULL)+KADEMLIAREPUBLISHTIMES;// X: [64T] - [64BitTime]
...
void CKademliaUDPListener::Process_KADEMLIA_CALLBACK_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, CKadUDPKey /*senderUDPKey*/)
{
	...
	if (pBuddy != NULL)
	{
		...
		CSafeMemFile fileIO2(16+16+4+2);
...
void CKademliaUDPListener::Process_KADEMLIA2_FIREWALLUDP(const byte *pbyPacketData, uint32 uLenPacket,uint32 uIP, CKadUDPKey /*senderUDPKey*/)
{
	...
	if (nIncomingPort == 0
		|| (nIncomingPort != CKademlia::GetPrefs()->GetExternalKadPort() && nIncomingPort != CKademlia::GetPrefs()->GetInternKadPort())){
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint_ptr uLenData, uint32 uDestinationHost, uint16 uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
	if (uLenData < 2) {
		ASSERT(0);
		return;
	...
	pPacket->pBuffer = new char[uLenData-2];
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint_ptr uLenData, byte byOpcode, uint32 uDestinationHost, uint16 uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
...
{
	AddTrackedOutPacket(uDestinationHost, byOpcode);
	Packet* pPacket = new Packet(pbyData, OP_KADEMLIAHEADER, byOpcode);
	if( pPacket->size > 200 )
		pPacket->PackPacket();
	theStats.AddUpDataOverheadKad(pPacket->size);
	theApp.clientudp->SendPacket(pPacket, ntohl(uDestinationHost), uDestinationPort, true
...
void CKademliaUDPListener::ExpireClientSearch(CKadClientSearcher* pExpireImmediately){
	...
	for (POSITION pos1 = listFetchNodeIDRequests.GetHeadPosition(), pos2;( pos2 = pos1 ) != NULL;){// X: [CI] - [Code Improvement]
		FetchNodeID_Struct sRequest = listFetchNodeIDRequests.GetNext(pos1);
		if (sRequest.pRequester == pExpireImmediately){
			listFetchNodeIDRequests.RemoveAt(pos2);
		}
		else if (sRequest.dwExpire < cur_tick){
