...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
CUpDownClient::CUpDownClient(CPartFile* in_reqfile, uint16 in_port, const CAddress& IP, uint32 in_serverip, uint16 in_serverport)
{
	socket = NULL;
	reqfile = in_reqfile;
	Init();
...
bool CUpDownClient::ProcessHelloTypePacket(CSafeMemFile* data)
{
	...
	for (uint32 i = 0; i < tagcount; i++)
	{
		...
		switch (temptag.GetNameID())
		{
			...
			case CT_NEOMULE_MISCOPTIONS:
				if (temptag.IsInt()) {
					m_NeoMisc.Bits = temptag.GetInt();
					if (bDbgInfo)
						m_strHelloInfo.AppendFormat(_T("\n  NeoOptions=%d"), m_NeoMisc.Bits);
				}
				else if (bDbgInfo)
					m_strHelloInfo.AppendFormat(_T("\n  ***UnkType=%s"), temptag.GetFullInfo());
				break;
	...
	if(!m_UserIP.IsNull())
	{
		if(!HasLowID() || m_nUserIDHybrid == 0 || m_nUserIDHybrid == _ntohl(m_UserIP.ToIPv4())) 
			m_nUserIDHybrid = m_UserIP.ToIPv4();
	}
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		if (m_UserIP.Type() == CAddress::IPv4 // IPv6-TODO: Add IPv6 ban list
		&& !theApp.clientlist->ComparePriorUserhash(_ntohl(m_UserIP.ToIPv4()), m_nUserPort, pFoundCredits)){
#else // NEO: IP6 END
	...
	{
		if ((m_Friend = theApp.friendlist->SearchFriend(m_achUserHash, GetIP(), m_nUserPort)) != NULL){
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	// IPv6-TODO: Add IPv6 server support
	bool bForceSend = theApp.serverconnect->AwaitingTestFromIP(_ntohl(GetConnectIP().ToIPv4()));
#else // NEO: IP6 END
...
//				(RESERVED			     ) 
	...
	NeoMisc.Fields.ExtendedSourceEx = 1;
#endif // NEO: NSX END
#ifdef USE_NAT_T // NEO: NAT - [NatTraversal]
	NeoMisc.Fields.SupportsNatTraversal = thePrefs.IsClientNatTraversalSupported() ? 1 : 0;
#endif // NEO: NAT END
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	NeoMisc.Fields.SupportsIPv6 = 1;
#endif // NEO: IP6 END	
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		Kademlia::CUDPFirewallTester::SetUDPFWCheckResult(false, true, GetConnectIP().ToIPv4(), 0); // inform the tester that this test was cancelled
#else // NEO: IP6 END
...
#endif
{
	...
	bool bUseIPv6 = m_OpenIPv6 && !theApp.GetPublicIPv6().IsNull();
	if(bUseIPv6)
		UpdateIP(m_UserIPv6);
	else
		UpdateIP(m_UserIPv4);
	...
	uint32 uClientIP = bUseIPv6 ? 0 : (!GetIP().IsNull() ? _ntohl(GetIP().ToIPv4()) : _ntohl(GetConnectIP().ToIPv4()));
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	if(!bUseIPv6) // Note: is an IPv6 is specifyed in the hello it means it is not firewalled, and fi we are IPv6 enabled we use it
#endif // NEO: IP6 END
#ifdef USE_NAT_T // NEO: NAT - [NatTraversal]
	if ( HasLowID() && !bUseUTP )
#else // NEO: NAT END
...
#ifdef USE_NAT_T // NEO: NAT - [NatTraversal]
			|| (theApp.serverconnect->IsLocalServer(GetServerIP(), GetServerPort()) && !theApp.IsFirewalled()) )) // Server Callback // X-TODO: add eserver NAT-T support
#else // NEO: NAT END
...
#endif
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	if (!HasLowID() || bUseUTP || bUseIPv6)
#else // NEO: IP6 END
	if (!HasLowID() || bUseUTP)
#endif
#else // NEO: NAT END
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	if (!HasLowID() || bUseIPv6)
#else // NEO: IP6 END
	if (!HasLowID())
#endif
#endif
	...
	else if (HasValidBuddyID() && Kademlia::CKademlia::IsConnected())
	{
		...
		if( GetBuddyIP() && GetBuddyPort())
		{
			...
			if(theApp.IsFirewalled())
			{
				...
				ASSERT(thePrefs.IsClientNatTraversalSupported() && SupportsNatTraversal());
			...
			else
			{
				...
				// FIXME: We dont know which kadversion the buddy has, so we need to send unencrypted
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
				theApp.clientudp->SendPacket(packet, CAddress(_ntohl(GetBuddyIP())), GetBuddyPort(), false, NULL, true, 0);
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
			ChallengeIP = CAddress(_ntohl(theApp.serverconnect->GetClientID()));
#else // NEO: IP6 END
...
bool CUpDownClient::IsBanned() const
{
	...
	if(GetIP().ToIPv4() == 0)
		return false;
	return theApp.clientlist->IsBannedClient(_ntohl(GetIP().ToIPv4()));
#else // NEO: IP6 END
