		...
		if (!IsLowID(userid))
		{
			...
			if (theApp.clientlist->IsBannedClient(userid)){
				...
				if (thePrefs.GetLogBannedClients()){
					...
					CUpDownClient* pClient = theApp.clientlist->FindClientByIP(CAddress(_ntohl(userid)));
#else // NEO: IP6 END
...
#ifdef USE_NEO_SX // NEO: NSX - [NeoSoruceExchange]
		if (uClientSXVersion > (pClient && pClient->SupportsSourceExchangeExt() ? SOURCEEXCHANGEEXT_VERSION : SOURCEEXCHANGE2_VERSION) || uClientSXVersion == 0) {
#else // NEO: NSX END
