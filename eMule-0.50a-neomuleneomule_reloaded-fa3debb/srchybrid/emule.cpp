...
bool CemuleApp::CanDoCallback( CUpDownClient *client )
{
	...
	if(thePrefs.IsClientNatTraversalSupported() && client->SupportsNatTraversal() && client->GetUserPort() == 0)
		return true; // the cleint does not have a TCP port and is nat traversal enabled we can try it
#endif // NEO: NAT END
	...
	if(Kademlia::CKademlia::IsConnected())
	{
		if(theApp.serverconnect->IsConnected())
		{
			if(theApp.serverconnect->IsLowID())
			{
				...
				{
#ifdef USE_NAT_T // NEO: NAT - [NatTraversal]
					if(thePrefs.IsClientNatTraversalSupported() && client->SupportsNatTraversal())
					{
