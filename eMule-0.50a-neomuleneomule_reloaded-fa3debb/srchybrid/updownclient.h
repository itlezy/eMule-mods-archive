...
public:
	...
	CUpDownClient(CPartFile* in_reqfile, uint16 in_port, const CAddress& IP, uint32 in_serverup, uint16 in_serverport);
#endif // NEO: IP6 END
	...
	bool			IsIPv6Open() const								{ return m_OpenIPv6; }
	void			UpdateIP( const CAddress& val )
						{
							m_ConnectIP = val;
							m_UserIP = val;
						}
	void			SetIP( CAddress val )
						{
							if(val.Convert(CAddress::IPv4)) // Check if the IP is a Maed IPv4
								m_UserIPv4 = val;
							else
								m_UserIPv6 = val;
							UpdateIP(val);
	...
	bool			SupportsSourceExchangeExt() const				{ return m_NeoMisc.Fields.ExtendedSourceEx; }
#endif // NEO: NSX END	
...
#ifdef USE_NAT_T // NEO: NAT - [NatTraversal]
	bool			SupportsNatTraversal() const					{ return m_NeoMisc.Fields.SupportsNatTraversal; }
	void			SetNatTraversalSupport(bool bVal)				{ m_NeoMisc.Fields.SupportsNatTraversal = bVal ? 1 : 0; }
#endif // NEO: NAT END
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
	bool			SupportsIPv6() const							{ return m_NeoMisc.Fields.SupportsIPv6; }
#endif // NEO: IP6 END
