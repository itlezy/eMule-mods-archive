...
int CAICHSyncThread::Run()
{
	...
	CSingleLock sharelock(&theAppPtr->sharedfiles->m_mutWriteList);
	sharelock.Lock();

	for (int i = 0; i < theAppPtr->sharedfiles->GetCount(); i++){
		CKnownFile* pCurFile = theAppPtr->sharedfiles->GetFileByIndex(i);
			...
			if (pCurFile->GetAICHHashset()->GetStatus() == AICH_HASHSETCOMPLETE){
				...
				for (POSITION pos = liKnown2Hashs.GetHeadPosition();pos != 0;)
				{
					...
					if (current_hash == pCurFile->GetAICHHashset()->GetMasterHash()){
						...
						//theAppPtr->QueueDebugLogLine(false, _T("%s - %s"), current_hash.GetString(), pCurFile->GetFileName());
#ifdef _DEBUG
...
//			 			pCurFile->GetAICHHashset()->DbgTest();
	...
	theAppPtr->m_AICH_Is_synchronizing=false;
	//Xman end
		...
		(!thePrefs.IsRememberingDownloadedFiles() || thePrefs.DoPartiallyPurgeOldKnownFiles()))
	{
		...
		try {
			...
			while (file.GetPosition() < nExistingSize){
				...
				else if (thePrefs.IsRememberingDownloadedFiles() && theAppPtr->knownfiles->ShouldPurgeAICHHashset(aichHash))
				{
			...
			theAppPtr->QueueDebugLogLine(false, _T("Cleaned up known2.met, removed %u hashsets and purged %u hashsets of old known files (%s)")
	...
	POSITION posk = theAppPtr->knownfiles->GetKnownFiles().GetStartPosition();					
	while(posk){
		CCKey key;
		theAppPtr->knownfiles->GetKnownFiles().GetNextAssoc( posk, key, cur_file );
	...
	if (!m_liToHash.IsEmpty()){
		theAppPtr->QueueLogLine(true, GetResString(IDS_AICH_SYNCTOTAL), m_liToHash.GetCount() );
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.SetAICHHashing(m_liToHash.GetCount());
		...
		while (theAppPtr->sharedfiles->GetHashingCount() != 0){
			Sleep(100);
		...
		for (POSITION pos = m_liToHash.GetHeadPosition();pos != 0; cDone++)
		{
			...
			theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.SetAICHHashing(m_liToHash.GetCount()-cDone);
			if (theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.m_hWnd != NULL)
			...
			if (!(theAppPtr->knownfiles->IsKnownFile(pCurFile) && theAppPtr->sharedfiles->GetFileByID(pCurFile->GetFileHash())) )
				continue;
			theAppPtr->QueueLogLine(false, GetResString(IDS_AICH_CALCFILE), pCurFile->GetFileName());
			if(!pCurFile->CreateAICHHashSetOnly())
				theAppPtr->QueueDebugLogLine(false, _T("Failed to create AICH Hashset while sync. for file %s"), pCurFile->GetFileName());
		}

		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.SetAICHHashing(0);
		if (theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.m_hWnd != NULL)
			theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.ShowFilesCount();
		sLock1.Unlock();
	}

	theAppPtr->QueueDebugLogLine(false, _T("AICHSyncThread finished"));
	return 0;
}
...
bool CAICHSyncThread::ConvertToKnown2ToKnown264(CSafeFile* pTargetFile){
	...
	theAppPtr->QueueLogLine(false, GetResString(IDS_CONVERTINGKNOWN2MET), OLD_KNOWN2_MET_FILENAME, KNOWN2_MET_FILENAME);
	...
	try {
		...
		error->Delete();
		theAppPtr->QueueLogLine(false, GetResString(IDS_CONVERTINGKNOWN2FAILED));
		pTargetFile->Close();
		return false;
	}
	theAppPtr->QueueLogLine(false, GetResString(IDS_CONVERTINGKNOWN2DONE));
