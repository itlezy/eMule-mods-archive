...
BOOL CAddFriend::OnInitDialog()
{
	...
	if (m_pShowFriend)
	{
		SetIcon(m_icnWnd = theAppPtr->LoadIcon(_T("ClientDetails")), FALSE);
	...
	{
		SetIcon(m_icnWnd = theAppPtr->LoadIcon(_T("AddFriend")), FALSE);
...
void CAddFriend::OnAddBtn()
{
	if (!m_pShowFriend)
	{
		...
		if (!theAppPtr->friendlist->AddFriend(NULL, 0, ip, (uint16)uPort, 0, strUserName, 0)){
