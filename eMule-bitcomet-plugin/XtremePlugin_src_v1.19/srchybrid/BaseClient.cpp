...
void CUpDownClient::Init()
{
	...
	*/
	...
	m_structUserCountry = theAppPtr->ip2country->GetCountryFromIP(m_dwUserIP); 
...
CUpDownClient::~CUpDownClient(){
	...
	ASSERT( m_nConnectingState == CCS_NONE || !theAppPtr->emuledlg->IsRunning() );
	theAppPtr->clientlist->RemoveClient(this, _T("Destructing client object"));
	...
	DEBUG_ONLY (theAppPtr->listensocket->Debug_ClientDeleted(this));
...
//Xman end	
	...
	if (thePrefs.GetAddServersFromClients() && m_dwServerIP && m_nServerPort){
		...
		if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(addsrv, true))
			delete addsrv;
	}
	...
	/*
	CClientCredits* pFoundCredits = theAppPtr->clientcredits->GetCredit(m_achUserHash);
	*/
	//zz_fly :: End
	if (credits == NULL){
		//zz_fly :: Optimized on table-arragement :: Enig123 :: Start
		...
		*/
		...
		//Xman Extened credit- table-arragement
		/*
		if (!theAppPtr->clientlist->ComparePriorUserhash(m_dwUserIP, m_nUserPort, pFoundCredits)){
		...
		*/
		if (!theAppPtr->clientlist->ComparePriorUserhash(m_dwUserIP, m_nUserPort, m_achUserHash)){
		...
		//zz_fly :: Optimized on table-arragement :: Enig123 :: Start
			...
			credits = theAppPtr->clientcredits->GetCredit(m_achUserHash);
		}
		//zz_fly :: End
	}
	//zz_fly :: Optimized on table-arragement :: Enig123 :: Start
	...
	*/
		...
		|| GetFriend()->m_nLastUsedPort != GetUserPort())
	{
		...
		/*
		if ((m_Friend = theAppPtr->friendlist->SearchFriend(m_achUserHash, m_dwUserIP, m_nUserPort)) != NULL){
		*/
		if ((m_Friend = theAppPtr->friendlist->SearchFriend(m_achUserHash, GetConnectIP(), m_nUserPort)) != NULL){
		//zz_fly :: end
	...
	{
		if (theAppPtr->GetID()!=m_nUserIDHybrid && memcmp(m_achUserHash, thePrefs.GetUserHash(), 16)==0)
		{
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	CSafeMemFile data(128);
	data.WriteUInt8((uint8)theAppPtr->m_uCurVersionShort);
	...
	*/
	...
	uint32 dwTagValue = (theAppPtr->clientcredits->CryptoAvailable() ? 3 : 0);
...
void CUpDownClient::SendHelloAnswer(){
	...
	bool bForceSend = theAppPtr->serverconnect->AwaitingTestFromIP(GetConnectIP());
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile* data)
{
	...
	if( theAppPtr->clientlist->GetBuddy() && theAppPtr->IsFirewalled() )
		tagcount += 2;
	...
	*/
	...
	if( theAppPtr->clientlist->GetBuddy() && theAppPtr->IsFirewalled() )
	{
		CTag tagBuddyIP(CT_EMULE_BUDDYIP, theAppPtr->clientlist->GetBuddy()->GetIP() ); 
		tagBuddyIP.WriteTagToFile(data);
	
		CTag tagBuddyPort(CT_EMULE_BUDDYUDP, 
//					( RESERVED												)
					((uint32)theAppPtr->clientlist->GetBuddy()->GetUDPPort()  ) 
					);
	...
	const UINT uSupportSecIdent		= theAppPtr->clientcredits->CryptoAvailable() ? 3 : 0;
	// ***
...
//				(RESERVED			     ) 
	...
	if (theAppPtr->serverconnect->IsConnected()){
		dwIP = theAppPtr->serverconnect->GetCurrentServer()->GetIP();
		nPort = theAppPtr->serverconnect->GetCurrentServer()->GetPort();
#ifdef _DEBUG
		if (dwIP == theAppPtr->serverconnect->GetLocalIP()){
			dwIP = 0;
			nPort = 0;
		}
#endif
...
//Xman end
{
	ASSERT( theAppPtr->clientlist->IsValidClient(this) );
	...
	*/
	...
	//Xman end
	{
		...
		// Maella -Upload Stop Reason-
		/*
		theAppPtr->uploadqueue->RemoveFromUploadQueue(this, CString(_T("CUpDownClient::Disconnected: ")) + pszReason);
		*/
		theAppPtr->uploadqueue->RemoveFromUploadQueue(this, CString(_T("CUpDownClient::Disconnected: ")) + pszReason , reason);
		//Xman end
	}
	...
	if (GetDownloadState() == DS_DOWNLOADING){
		...
		if (m_ePeerCacheDownState == PCDS_WAIT_CACHE_REPLY || m_ePeerCacheDownState == PCDS_DOWNLOADING)
			theAppPtr->m_pPeerCache->DownloadAttemptFailed();
		...
		*/
		...
		if (GetDownloadState() == DS_CONNECTED){ // successfully connected, but probably didn't responsed to our filerequest
			theAppPtr->clientlist->m_globDeadSourceList.AddDeadSource(this);
			theAppPtr->downloadqueue->RemoveSource(this);
	    }
		*/
		//Xman end
	}
	...
	if (m_Friend)
		theAppPtr->friendlist->RefreshFriend(m_Friend);
	...
	*/
	switch(m_nUploadState){
		case US_CONNECTING:
			...
			//Xman uploading problem client
			theAppPtr->uploadqueue->RemoveFromUploadQueue(this,pszReason ,CUpDownClient::USR_SOCKET );
			isupprob=true;
			//back to queue
			theAppPtr->uploadqueue->AddClientDirectToQueue(this);
	...
	*/
	...
	switch(m_nDownloadState){
		case DS_CONNECTING:
			{
				...
				{
					//theAppPtr->AddLogLine(false,_T("Client with with IP=%s, Version=%s, Name=%s failed to connect %u times"), GetFullIP(), GetClientVerString(), GetUserName(),m_cFailed);
		...
		case DS_WAITCALLBACK:
			if (bAddDeadSource)
				theAppPtr->clientlist->m_globDeadSourceList.AddDeadSource(this);
			theAppPtr->downloadqueue->AddFailedTCPFileReask(); //Xman Xtreme Mod: count the failed TCP-connections
			if(m_bUDPPending) theAppPtr->downloadqueue->AddFailedUDPFileReasks(); //Xman Xtreme Mod: for correct statistics, if it wasn't counted on connection established //Xman x4 test
	...
	if (GetChatState() != MS_NONE){
		...
		if (GetFriend() != NULL && GetFriend()->IsTryingToConnect())
			...
			theAppPtr->emuledlg->chatwnd->chatselector.ConnectingResult(this,false); // other clients update directly
	}
	
	// Delete Socket
	if (!bFromSocket && socket){
		ASSERT( theAppPtr->listensocket->IsValidSocket(socket) );
		socket->Safe_Delete();
	}
	socket = NULL;
	...
	m_nConnectingState = CCS_NONE;
	theAppPtr->clientlist->RemoveConnectingClient(this);
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, bool bNoCallbacks, CRuntimeClass* pClassSocket)
{
	...
	if (theAppPtr->listensocket->TooManySockets() && !bIgnoreMaxCon)
	{
	...
	if (uClientIP)
	{
		...
		if (theAppPtr->ipfilter->IsFiltered(uClientIP))
		{
			theStats.filteredclients++;
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(true, GetResString(IDS_IPFILTERED), ipstr(uClientIP), theAppPtr->ipfilter->GetLastHit());
		...
		if (theAppPtr->clientlist->IsBannedClient(uClientIP))
		{
			if (thePrefs.GetLogBannedClients())
	...
	if ( HasLowID() )
	{
		...
		if(!theAppPtr->CanDoCallback(this)) // lowid2lowid check used for the whole function, don't remove
		{
		...
		if (!( (SupportsDirectUDPCallback() && thePrefs.GetUDPPort() != 0 && GetConnectIP() != 0) // Direct Callback
			...
			|| theAppPtr->serverconnect->IsLocalServer(GetServerIP(), GetServerPort()) )) // Server Callback
		{
			// Nope
	...
	theAppPtr->clientlist->AddConnectingClient(this); // Starts and checks for the timeout, ensures following Disconnect() or ConnectionEstablished() call
	...
	else if (SupportsDirectUDPCallback() && thePrefs.GetUDPPort() != 0 && GetConnectIP() != 0)
	{
		...
		theStats.AddUpDataOverheadOther(packet->size);
		theAppPtr->clientudp->SendPacket(packet, GetConnectIP(), GetKadPort(), ShouldReceiveCryptUDPPackets(), GetUserHash(), false, 0);
		return true;
	}
	////////////////////////////////////////////////////////////
	...
	if (theAppPtr->serverconnect->IsLocalServer(m_dwServerIP, m_nServerPort))
	{
		...
		theAppPtr->serverconnect->SendPacket(packet);
		return true;
	}
	else if (HasValidBuddyID() && Kademlia::CKademlia::IsConnected())
	{
		...
		if( GetBuddyIP() && GetBuddyPort())
		{
			...
			theAppPtr->clientudp->SendPacket(packet, GetBuddyIP(), GetBuddyPort(), false, NULL, true, 0);
...
void CUpDownClient::ConnectionEstablished()
{
	...
	{
		m_nFailedUDPPackets++;
		theAppPtr->downloadqueue->AddFailedUDPFileReasks();
	}
	m_bUDPPending = false;
	...
	if(theAppPtr->internetmaybedown)
	{
		if(Kademlia::CKademlia::IsConnected() 
			&& (theAppPtr->uploadqueue->waitinglist.GetSize()>10 //check if we have clients queued, otherwise inetmaybedown gives a wrong value
			|| theAppPtr->last_ip_change==0) //we just started the client //Xman new adapter selection 
			&& theAppPtr->last_ip_change  < ::GetTickCount() - MIN2MS(2) //only once in 2 minutes
			) 
			theAppPtr->m_bneedpublicIP=true;
		if(theAppPtr->IsConnected()) //only free the upload if we are connected. important! otherwise we would have problems with nafc-adapter on a hotstart
		{
			theAppPtr->internetmaybedown=false;
			theAppPtr->last_traffic_reception=::GetTickCount();
			theAppPtr->pBandWidthControl->AddeMuleOut(1); //this reopens the upload (internetmaybedown checks for upload==0
		}
	}
	if(theAppPtr->m_bneedpublicIP==true
		&& m_fPeerCache 
	...
	//Xman
	/*
	if (theAppPtr->GetPublicIP() == 0 && theAppPtr->IsConnected() && m_fPeerCache)
	*/
	if (theAppPtr->GetPublicIP() == 0 && theAppPtr->serverconnect->IsConnected() && m_fPeerCache)
	//Xman end
		SendPublicIPRequest();
	...
	if (GetChatState() == MS_CONNECTING || GetChatState() == MS_CHATTING)
	{
		if (GetFriend() != NULL && GetFriend()->IsTryingToConnect()){
			...
			theAppPtr->emuledlg->chatwnd->chatselector.ConnectingResult(this, true); // other clients update directly
	}
	...
	{
		case US_CONNECTING:
			if (theAppPtr->uploadqueue->IsDownloading(this))
			{
				...
				CKnownFile* partfile = theAppPtr->downloadqueue->GetFileByID(this->GetUploadFileID());
				if (partfile && partfile->IsPartFile())
					theAppPtr->downloadqueue->CheckAndAddKnownSource((CPartFile*)partfile,this,true);
				//Xman end
			}
	}
...
void CUpDownClient::ProcessSharedFileList(const uchar* pachPacket, uint32 nSize, LPCTSTR pszDirectory)
{
	if (m_iFileListRequested > 0)
	{
		...
		theAppPtr->searchlist->ProcessSearchAnswer(pachPacket,nSize,this,NULL,pszDirectory);
	}
}
...
void CUpDownClient::SendPublicKeyPacket()
{
	...
	if (socket == NULL || credits == NULL || m_SecureIdentState != IS_KEYANDSIGNEEDED){
		//ASSERT ( false ); //zz_fly :: see the changes in clientcreditlist
		return;
	}
	if (!theAppPtr->clientcredits->CryptoAvailable())
		return;

    Packet* packet = new Packet(OP_PUBLICKEY,theAppPtr->clientcredits->GetPubKeyLen() + 1,OP_EMULEPROT);
	theStats.AddUpDataOverheadOther(packet->size);
	memcpy(packet->pBuffer+1,theAppPtr->clientcredits->GetPublicKey(), theAppPtr->clientcredits->GetPubKeyLen());
	packet->pBuffer[0] = theAppPtr->clientcredits->GetPubKeyLen();
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
	if (thePrefs.GetDebugClientTCPLevel() > 0)
...
void CUpDownClient::SendSignaturePacket()
{
	...
	if (bUseV2){
		if (theAppPtr->serverconnect->GetClientID() == 0 || theAppPtr->serverconnect->IsLowID()){
			// we cannot do not know for sure our public ip, so use the remote clients one
			ChallengeIP = GetIP();
			byChaIPKind = CRYPT_CIP_REMOTECLIENT;
		}
		else{
			ChallengeIP = theAppPtr->serverconnect->GetClientID();
			byChaIPKind  = CRYPT_CIP_LOCALCLIENT;
		}
	}
	//end v2
	uchar achBuffer[250];
	uint8 siglen = theAppPtr->clientcredits->CreateSignature(credits, achBuffer,  250, ChallengeIP, byChaIPKind );
	if (siglen == 0){
		ASSERT ( false );
		return;
	}
...
void CUpDownClient::ProcessSignaturePacket(const uchar* pachPacket, uint32 nSize)
{
	...
	if (socket == NULL || credits == NULL || nSize < 10 || nSize > 250){
		//ASSERT ( false ); //zz_fly :: see the changes in clientcreditlist
		return;
	}
	...
	if (theAppPtr->clientcredits->VerifyIdent(credits, pachPacket+1, pachPacket[0], GetIP(), byChaIPKind ) ){
...
bool CUpDownClient::IsBanned() const
{
	...
	/*
	return theAppPtr->clientlist->IsBannedClient(GetIP());
	*/
	return theAppPtr->clientlist->IsBannedClient(GetConnectIP());
	//Xman end
}
...
void CUpDownClient::ProcessPreviewReq(const uchar* pachPacket, uint32 nSize)
{
	...
	CKnownFile* previewFile = theAppPtr->sharedfiles->GetFileByID(pachPacket);
	if (previewFile == NULL){
...
void CUpDownClient::ProcessPreviewAnswer(const uchar* pachPacket, uint32 nSize)
{
	...
	CSearchFile* sfile = theAppPtr->searchlist->GetSearchFileByHash(Hash);
	if (sfile == NULL){
		//already deleted
		return;
	}
...
CString CUpDownClient::GetUploadStateDisplayString() const
{
	...
	switch (GetUploadState()){
		...
		case US_UPLOADING:
            ...
            } else if(GetSlotNumber() <= theAppPtr->uploadqueue->GetActiveUploadsCount()) {
				strState = GetResString(IDS_TRANSFERRING);
            } else {
...
void CUpDownClient::ProcessPublicIPAnswer(const BYTE* pbyData, UINT uSize){
	...
	if (m_fNeedOurPublicIP == 1){ // did we?
		...
		/*
		if (theAppPtr->GetPublicIP() == 0 && !::IsLowID(dwIP) )
			theAppPtr->SetPublicIP(dwIP);
		*/
		if(dwIP==0) return; //thats wrong in every case
		if(IsLowID(dwIP) && dwIP!=theAppPtr->recheck_ip)
		{
			//check if still needed:
			if(theAppPtr->m_bneedpublicIP
				|| theAppPtr->serverconnect->IsConnected() && theAppPtr->GetPublicIP() == 0)
				theAppPtr->recheck_ip=dwIP;
			return; //will force a new request
		}

		//Xman -Reask sources after IP change- v4 
		if(theAppPtr->m_bneedpublicIP /*&& !::IsLowID(dwIP)*/) //this is the case we have kad-only but no upload->inet down ?
		{
			...
			//Xman new adapter selection 
			{
				...
				theAppPtr->pBandWidthControl->checkAdapterIndex(dwIP);
			}
			//Xman end

			if(theAppPtr->last_valid_ip!=0 && theAppPtr->last_valid_ip != dwIP)
			{

				//if we had a lowID we asume we get it again. Then let the server trigger
				if((theAppPtr->serverconnect->IsConnecting() && IsLowID(theAppPtr->last_valid_serverid))
					|| (theAppPtr->serverconnect->IsConnected() && theAppPtr->serverconnect->IsLowID() && theAppPtr->GetPublicIP(true)!=dwIP))
				{
					//don't trigger
				}
				else
				{
					if(GetTickCount() - theAppPtr->last_ip_change > FILEREASKTIME + 60000)
					{
						theAppPtr->clientlist->TrigReaskForDownload(true);
						AddLogLine(false, _T("Kad Connection detected IP-change, changed IP from %s to %s, all sources will be reasked immediately"), ipstr(theAppPtr->last_valid_ip), ipstr(dwIP));
					}
					else
					{
						theAppPtr->clientlist->TrigReaskForDownload(false);
						AddLogLine(false, _T("Kad Connection detected IP-change, changed IP from %s to %s, all sources will be reasked within the next 10 minutes"), ipstr(theAppPtr->last_valid_ip), ipstr(dwIP));
					}
				...
				{
					theAppPtr->emuledlg->RefreshUPnP(true); //refresh, and RequestAnswer
					AddLogLine(false, _T("Detected IP-change, changed IP from %s to %s, refreshing UPnP..."), ipstr(theAppPtr->last_valid_ip), ipstr(dwIP));
				}
				//ACAT UPnP
				/*
				if(thePrefs.GetUPnPNat() && thePrefs.GetUPnPNatRebind())
				{
					AddLogLine(false, _T("Detected IP-change, changed IP from %s to %s, rebinding UPnP..."), ipstr(theAppPtr->last_valid_ip), ipstr(dwIP));
					theAppPtr->clientudp->RebindUPnP();
					theAppPtr->listensocket->RebindUPnP();
				}
				*/
				...
				if(theAppPtr->serverconnect->IsConnected() && theAppPtr->GetPublicIP(true)!=dwIP)
				{
					theAppPtr->serverconnect->Disconnect();
					theAppPtr->serverconnect->ConnectToAnyServer();
				}
			}
			theAppPtr->last_valid_ip=dwIP;

			theAppPtr->m_bneedpublicIP=false;
			theAppPtr->last_ip_change=::GetTickCount(); //remark: this is set when ever inet was down, even we receive the old ip

		}
		if(theAppPtr->serverconnect->IsConnected())//remark: this is the case we have a lowid-server-connect
			if (theAppPtr->GetPublicIP() == 0 /*&& !::IsLowID(dwIP)*/ )
			{
				theAppPtr->SetPublicIP(dwIP); 
				theAppPtr->last_valid_ip=dwIP;
		...
		//Xman fix
		theAppPtr->recheck_ip=0;
	}	
}

void CUpDownClient::CheckFailedFileIdReqs(const uchar* aucFileHash)
{
	if ( aucFileHash != NULL && (theAppPtr->sharedfiles->IsUnsharedFile(aucFileHash) || theAppPtr->downloadqueue->GetFileByID(aucFileHash)) )
		return;
	//if (GetDownloadState() != DS_DOWNLOADING) // filereq floods are never allowed!
	{
		...
		if (m_fFailedFileIdReqs >= 6)
		{
			...
			/*
			if (theAppPtr->clientlist->GetBadRequests(this) < 2)
				theAppPtr->clientlist->TrackBadRequest(this, 1);
			if (theAppPtr->clientlist->GetBadRequests(this) == 2){
				theAppPtr->clientlist->TrackBadRequest(this, -2); // reset so the client will not be rebanned right after the ban is lifted
...
bool CUpDownClient::ShouldReceiveCryptUDPPackets() const {
	return (thePrefs.IsClientCryptLayerSupported() && SupportsCryptLayer() && theAppPtr->GetPublicIP() != 0
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32 nLength)
{
	...
	if(GetMessagesReceived()==0 &&	//it's the first message we receive
		...
		theAppPtr->dlp->IsDLPavailable()
		)
	{
		LPCTSTR reason=theAppPtr->dlp->DLPCheckMessageSpam(strMessageCheck);
		if(reason)
		{
			theAppPtr->emuledlg->chatwnd->chatselector.EndSession(this);
	...
	theAppPtr->emuledlg->chatwnd->chatselector.ProcessMessage(this, strMessage);
}
...
void CUpDownClient::ProcessCaptchaRequest(CSafeMemFile* data){
	...
	if (GetChatCaptchaState() == CA_ACCEPTING && GetChatState() != MS_NONE
		&& theAppPtr->emuledlg->chatwnd->chatselector.GetItemByClient(this) != NULL)
	{
		...
		if ( nSize > 128 && nSize < 4096)
		{
				...
				&& imgCaptcha.GetWidth() > 10 && imgCaptcha.GetWidth() < 150 )
			{
				...
				if (hbmp != NULL){
					...
					theAppPtr->emuledlg->chatwnd->chatselector.ShowCaptchaRequest(this, hbmp);
					DeleteObject(hbmp);
				}
				else
...
void CUpDownClient::ProcessCaptchaReqRes(uint8 nStatus)
{
		...
		&& theAppPtr->emuledlg->chatwnd->chatselector.GetItemByClient(this) != NULL)
	{
		ASSERT( nStatus < 3 );
		m_nChatCaptchaState = CA_NONE;
		theAppPtr->emuledlg->chatwnd->chatselector.ShowCaptchaResult(this, GetResString((nStatus == 0) ? IDS_CAPTCHASOLVED : IDS_CAPTCHAFAILED));
	}
	else {
		m_nChatCaptchaState = CA_NONE;
...
{
	if (m_Friend != NULL && theAppPtr->friendlist->IsValid(m_Friend))
		return m_Friend;
...
void CUpDownClient::ProcessFirewallCheckUDPRequest(CSafeMemFile* data){
	...
	Kademlia::CKademlia::GetUDPListener()->SendPacket(&fileTestPacket1, KADEMLIA2_FIREWALLUDP, ntohl(GetConnectIP())
		, nRemoteInternPort, Kademlia::CKadUDPKey(dwSenderKey, theAppPtr->GetPublicIP(false)), NULL);
	...
	if (nRemoteExternPort != 0 && nRemoteExternPort != nRemoteInternPort){
		...
		Kademlia::CKademlia::GetUDPListener()->SendPacket(&fileTestPacket2, KADEMLIA2_FIREWALLUDP, ntohl(GetConnectIP())
			, nRemoteExternPort, Kademlia::CKadUDPKey(dwSenderKey, theAppPtr->GetPublicIP(false)), NULL);
	}
...
void CUpDownClient::SendSharedDirectories()
{
	...
	while (pos)
	{
		strDir = theAppPtr->sharedfiles->GetPseudoDirName(thePrefs.shareddir_list.GetNext(pos));
		if (!strDir.IsEmpty())
	...
	{
		strDir = theAppPtr->sharedfiles->GetPseudoDirName(thePrefs.GetCategory(iCat)->strIncomingPath);
		if (!strDir.IsEmpty())
			arFolders.Add(strDir);
	}

	// add temporary folder if there are any temp files
	if (theAppPtr->downloadqueue->GetFileCount() > 0)
	...
	if (theAppPtr->sharedfiles->ProbablyHaveSingleSharedFiles())
...
CString	CUpDownClient::GetCountryName(bool longName) const {
	return theAppPtr->ip2country->GetCountryNameFromRef(m_structUserCountry,longName);
}
...
void CUpDownClient::ResetIP2Country(uint32 m_dwIP){
	m_structUserCountry = theAppPtr->ip2country->GetCountryFromIP((m_dwIP)?m_dwIP:m_dwUserIP);
}
...
bool CUpDownClient::ProcessUnknownHelloTag(CTag *tag, CString &pszReason)
{
	...
	//Xman DLP
	if(theAppPtr->dlp->IsDLPavailable()==false)
		return false;

	bool foundmd4=false;

	LPCTSTR strSnafuTag=theAppPtr->dlp->DLPCheckHelloTag(tag->GetNameID());
	if (strSnafuTag!=NULL)
	{
...
void CUpDownClient::ProcessUnknownInfoTag(CTag *tag, CString &pszReason)
{
	...
	if(theAppPtr->dlp->IsDLPavailable()==false)
		return;
	LPCTSTR strSnafuTag=theAppPtr->dlp->DLPCheckInfoTag(tag->GetNameID());
	if (strSnafuTag!=NULL)
	{
...
void CUpDownClient::ShowRequestedFiles()
{
	...
	fileList += _T("__________________________\n\n") ; 
	CKnownFile* uploadfile = theAppPtr->sharedfiles->GetFileByID(GetUploadFileID());
	if(uploadfile)
...
void CUpDownClient::TestLeecher(){
	...
	if (thePrefs.GetAntiLeecherMod())
	{
		if(old_m_strClientSoftwareFULL.IsEmpty() || old_m_strClientSoftwareFULL!= DbgGetFullClientSoftVer() )
		{
			...
			LPCTSTR reason=theAppPtr->dlp->DLPCheckModstring_Hard(m_strModVersion,m_strClientSoftware);
			if(reason)
			{		
				BanLeecher(reason,5); //hard ban
				return;
			}
			reason=theAppPtr->dlp->DLPCheckModstring_Soft(m_strModVersion,m_strClientSoftware);
			if(reason)
			{
	...
	if(thePrefs.GeTAntiLeecheruserhash() && HasValidHash())
	{
		...
		LPCTSTR reason=theAppPtr->dlp->DLPCheckUserhash(uhash);
		if(reason)
		{
			BanLeecher(_T("*AJ*"),18);
			return;
	...
	if (thePrefs.GetAntiLeecherName())
	{
		...
		if(m_bLeecher!=4 && m_pszUsername!=NULL && (old_m_pszUsername.IsEmpty() || old_m_pszUsername!=m_pszUsername)) //remark: because old_m_pszUsername is CString and there operator != is defined, it isn't a pointer comparison 
		{
			...
			if (HasValidHash())
			{
				...
				LPCTSTR reason=theAppPtr->dlp->DLPCheckNameAndHashAndMod(m_pszUsername,struserhash,m_strModVersion);
				if(reason)
				{
			...
			LPCTSTR reason=theAppPtr->dlp->DLPCheckUsername_Soft(m_pszUsername);
			if(reason)
			{
				BanLeecher(reason,10); //soft ban
				return;
			...
			reason=theAppPtr->dlp->DLPCheckUsername_Hard(m_pszUsername);
			if(reason)
			{
				BanLeecher(reason,5); //hard ban
				return;
