...
BOOL CChatWnd::OnInitDialog()
{
	...
	if (theAppPtr->m_fontChatEdit.m_hObject)
	{
		m_wndMessage.SendMessage(WM_SETFONT, (WPARAM)theAppPtr->m_fontChatEdit.m_hObject, FALSE);
		CRect rcEdit;
	...
	{
		GetDlgItem(IDC_BTN_MENU)->SetFont(&theAppPtr->m_fontSymbol);
	...
	m_wndFormat.ModifyStyle((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0, TBSTYLE_TOOLTIPS);
...
void CChatWnd::StartSession(CUpDownClient* client)
{
	if (!client->GetUserName())
		return;
	theAppPtr->emuledlg->SetActiveDialog(this);
...
void CChatWnd::SetAllIcons()
{
	...
	if (icon_msg)
		VERIFY( DestroyIcon(icon_msg) );
	icon_friend = theAppPtr->LoadIcon(_T("Friend"), 16, 16);
	icon_msg = theAppPtr->LoadIcon(_T("Message"), 16, 16);
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Messages);
	return TRUE;
}

void CChatWnd::OnStnDblClickFriendIcon()
{
	theAppPtr->emuledlg->ShowPreferences(IDD_PPG_MESSAGES);
}
		...
		case MP_GETFRIENDED2KLINK:
			{
				...
				theAppPtr->CopyTextToClipboard(sLink);
			}
			break;
