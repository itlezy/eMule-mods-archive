...
void CClientCredits::AddDownloaded(uint32 bytes, uint32 dwForIP) {
	if ((GetCurrentIdentState(dwForIP) == IS_IDFAILED || GetCurrentIdentState(dwForIP) == IS_IDBADGUY || GetCurrentIdentState(dwForIP) == IS_IDNEEDED) && theAppPtr->clientcredits->CryptoAvailable()) {
		return;
	}
...
const float CClientCredits::GetScoreRatio(const CUpDownClient* client) const
{
	...
	if((GetCurrentIdentState(dwForIP) == IS_IDFAILED || GetCurrentIdentState(dwForIP) == IS_IDBADGUY || GetCurrentIdentState(dwForIP) == IS_IDNEEDED) && 
		theAppPtr->clientcredits->CryptoAvailable() == true){
			// bad guy - no credits for you
			//return 1.0f;
			...
			*/			
			...
			//zz_fly :: End
			{
				...
				if(cur_credit->GetUploadedTotal()==0 && cur_credit->GetDownloadedTotal()==0
					&& theAppPtr->clientlist->FindClientByUserHash(cur_credit->GetKey())==NULL)
				*/
				if(theAppPtr->clientlist->FindClientByUserHash(cur_credit->GetKey())==NULL)
				//zz_fly :: End
				{
					//zz_fly :: Optimized on table-arragement :: Enig123 :: Start
...
void CClientCreditsList::InitalizeCrypting()
{
	...
	try{
		...
		using namespace std;
		CFile fin;
		CString file_path = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + _T("cryptkey.dat");
		fin.Open(file_path, CFile::modeRead | CFile::typeBinary);
		string pub_key;
		pub_key.resize((uint32)fin.GetLength());
		fin.Read((void*)pub_key.data(), (uint32)fin.GetLength());
		fin.Close();
		StringSource strSource(pub_key, true, new Base64Decoder);
		m_pSignkey = new RSASSA_PKCS1v15_SHA_Signer(strSource);
		// modification end
...
bool CClientCreditsList::CreateKeyPair()
{
	try{
		...
		CFile fout;
		CString file_path = thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + _T("cryptkey.dat");
		fout.Open(file_path, CFile::modeCreate | CFile::modeWrite | CFile::typeBinary);
		fout.Write(private_key.data(), private_key.size());
		fout.Close();
		// modification end
			...
			switch (byChaIPKind){
				...
				case CRYPT_CIP_REMOTECLIENT:
					if (theAppPtr->serverconnect->GetClientID() == 0 || theAppPtr->serverconnect->IsLowID()){
						...
						ChallengeIP = theAppPtr->serverconnect->GetLocalIP();
					}
					else
