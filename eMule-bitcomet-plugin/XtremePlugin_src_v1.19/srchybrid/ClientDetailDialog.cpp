	...
	if (m_bDataChanged)
	{
		...
		*/
		...
		if (client->GetServerIP()){
			...
			CServer* cserver = theAppPtr->serverlist->GetServerByIPTCP(client->GetServerIP(), client->GetServerPort());
			if (cserver)
		...
		if(client->GetUploadState()==US_ONUPLOADQUEUE)
			buffer.Format(_T("%u"),theAppPtr->uploadqueue->GetWaitingPosition(client));
		else
			buffer.Format(_T("-"));
