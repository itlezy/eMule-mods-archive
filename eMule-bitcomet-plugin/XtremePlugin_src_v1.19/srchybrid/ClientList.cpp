...
void CClientList::AddClient(CUpDownClient* toadd, bool bSkipDupTest)
{
	...
	theAppPtr->emuledlg->transferwnd->clientlistctrl.AddClient(toadd);
	list.AddTail(toadd);
}
/* Xman
// ZZ:UploadSpeedSense -->
...
void CClientList::RemoveClient(CUpDownClient* toremove, LPCTSTR pszReason){
	...
	if (pos){
        theAppPtr->uploadqueue->RemoveFromUploadQueue(toremove, CString(_T("CClientList::RemoveClient: ")) + pszReason);
		theAppPtr->uploadqueue->RemoveFromWaitingQueue(toremove);
		theAppPtr->downloadqueue->RemoveSource(toremove);
		theAppPtr->emuledlg->transferwnd->clientlistctrl.RemoveClient(toremove);
		list.RemoveAt(pos);
	}
...
void CClientList::DeleteAll(){
	theAppPtr->uploadqueue->DeleteAll();
	theAppPtr->downloadqueue->DeleteAll();
	POSITION pos1, pos2;
...
void CClientList::Process()
{
	...
	for (pos1 = m_KadList.GetHeadPosition(); (pos2 = pos1) != NULL; )
	{
		...
		switch(cur_client->GetKadState())
		{
			...
			case KS_CONNECTED_BUDDY:
				...
				if( m_pBuddy == cur_client && theAppPtr->IsFirewalled() && cur_client->SendBuddyPingPong() )
				{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
					if (thePrefs.GetDebugClientTCPLevel() > 0)
	...
	if( buddy == Disconnected )
	{
		...
		{
			if( Kademlia::CKademlia::IsRunning() && theAppPtr->IsFirewalled() && Kademlia::CUDPFirewallTester::IsFirewalledUDP(true))
			{
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8 byConnectOptions)
{
	...
	if (theAppPtr->serverconnect->GetLocalIP() == nContactIP && thePrefs.GetPort() == contact->GetTCPPort())
		return false;
...
bool CClientList::IncomingBuddy(Kademlia::CContact* contact, Kademlia::CUInt128* buddyID )
{
	...
	else if (theAppPtr->serverconnect->GetLocalIP() == nContactIP && thePrefs.GetPort() == contact->GetTCPPort())
...
void CClientList::ResetIP2Country(){
	...
	for(POSITION pos = list.GetHeadPosition(); pos != NULL; list.GetNext(pos)) { 
		cur_client = theAppPtr->clientlist->list.GetAt(pos); 
