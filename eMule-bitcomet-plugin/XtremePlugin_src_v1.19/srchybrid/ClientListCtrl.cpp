...
int CClientListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theAppPtr->emuledlg->transferwnd->clientlistctrl.GetNextSortOrder(lParamSort)) != -1)
...
void CClientListCtrl::AddClient(const CUpDownClient *client)
{
	...
	theAppPtr->emuledlg->transferwnd->UpdateListCount(CTransferWnd::wnd2Clients, iItemCount + 1);
}
...
void CClientListCtrl::RemoveClient(const CUpDownClient *client)
{
	...
	if (result != -1) {
		DeleteItem(result);
		theAppPtr->emuledlg->transferwnd->UpdateListCount(CTransferWnd::wnd2Clients);
	}
}
...
void CClientListCtrl::RefreshClient(const CUpDownClient *client)
{
	...
	if (theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->transferwnd || !theAppPtr->emuledlg->transferwnd->clientlistctrl.IsWindowVisible())
		return;
...
void CClientListCtrl::ShowKnownClients()
{
	DeleteAllItems();
	int iItemCount = 0;
	for (POSITION pos = theAppPtr->clientlist->list.GetHeadPosition(); pos != NULL; ) {
		const CUpDownClient *cur_client = theAppPtr->clientlist->list.GetNext(pos);
		int iItem = InsertItem(LVIF_TEXT | LVIF_PARAM, iItemCount, LPSTR_TEXTCALLBACK, 0, 0, 0, (LPARAM)cur_client);
		Update(iItem);
		iItemCount++;
	}
	theAppPtr->emuledlg->transferwnd->UpdateListCount(CTransferWnd::wnd2Clients, iItemCount);
}
