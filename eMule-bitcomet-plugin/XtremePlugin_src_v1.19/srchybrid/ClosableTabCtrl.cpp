...
void CClosableTabCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_bCloseable)
	{
		...
		if (iTab != -1)
		{
			...
			bool bVistaThemeActive = theAppPtr->IsVistaThemeActive();
			CRect rcCloseButton;
...
void CClosableTabCtrl::SetAllIcons()
{
	if (m_bCloseable)
	{
		...
		m_ImgLstCloseButton.Create(iIconWidth, iIconHeight, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
