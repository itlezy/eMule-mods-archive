...
BOOL CCollectionViewDialog::OnInitDialog(void)
{
	...
	SetIcon(m_icoWnd = theAppPtr->LoadIcon(_T("Collection_View")), FALSE);

	m_AddNewCatagory.SetCheck(false);

	SetWindowText(GetResString(IDS_VIEWCOLLECTION) + _T(": ") + m_pCollection->m_sCollectionName);

	m_CollectionViewListIcon.SetIcon(m_icoColl = theAppPtr->LoadIcon(_T("AABCollectionFileType")));
	...
	while (pos != NULL)
	{
		...
		int iImage = theAppPtr->GetFileTypeSystemImageIdx(pCollectionFile->GetFileName());
...
void CCollectionViewDialog::DownloadSelected(void)
{
	...
	{
		iNewIndex = theAppPtr->emuledlg->transferwnd->AddCategory(m_pCollection->m_sCollectionName, thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR), _T(""), _T(""), true);
	...
	while (collectionFileList.GetCount() > 0)
	{
		...
		if (pCollectionFile)
			...
			if ( theAppPtr->knownfiles->CheckAlreadyDownloadedFileQuestion(pCollectionFile->GetFileHash(), pCollectionFile->GetFileName()) )
			//Xman end
				theAppPtr->downloadqueue->AddSearchToDownload(::CreateED2kLink(pCollectionFile, true), thePrefs.AddNewFilesPaused(), iNewIndex);			
	}
}
