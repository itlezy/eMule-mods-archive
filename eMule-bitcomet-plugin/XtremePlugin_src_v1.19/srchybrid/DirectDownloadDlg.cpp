		...
		try
		{
			...
			if (pLink)
			{
				if (pLink->GetKind() == CED2KLink::kFile)
				{
					...
					if(theAppPtr->knownfiles->CheckAlreadyDownloadedFileQuestion(pLink->GetFileLink()->GetHashKey(),pLink->GetFileLink()->GetName()))
					//Xman end
						theAppPtr->downloadqueue->AddFileLinkToDownload(pLink->GetFileLink(), (thePrefs.GetCatCount() == 0) ? 0 : m_cattabs.GetCurSel());		
				}
				else
				{
					delete pLink;
...
BOOL CDirectDownloadDlg::OnInitDialog()
{
	...
	SetIcon(m_icnWnd = theAppPtr->LoadIcon(_T("PasteLink")), FALSE);
	...
	if (thePrefs.GetCatCount()==0) {
		...
		if (theAppPtr->m_fontSymbol.m_hObject){
			GetDlgItem(IDC_CATLABEL)->SetFont(&theAppPtr->m_fontSymbol);
