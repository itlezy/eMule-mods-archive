	...
	*/
	//Xman End
	{
		if (thePrefs.GetUseSystemFontForMainControls())
		{
			...
			m_pFontBold = &theAppPtr->m_fontDefaultBold;
	}
...
void CDownloadListCtrl::UpdateItem(void* toupdate)
{
	...
	if( theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->transferwnd  || theAppPtr->emuledlg->transferwnd->downloadlistctrl.IsWindowVisible() == false )
		return;
...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	switch (nColumn)
	{
		case 0: {	// file name
			CRect rcDraw(lpRect);
			int iIconPosY = (rcDraw.Height() > theAppPtr->GetSmallSytemIconSize().cy) ? ((rcDraw.Height() - theAppPtr->GetSmallSytemIconSize().cy) / 2) : 0;
			int iImage = theAppPtr->GetFileTypeSystemImageIdx(pPartFile->GetFileName());
			if (theAppPtr->GetSystemImageList() != NULL)
				::ImageList_Draw(theAppPtr->GetSystemImageList(), iImage, dc->GetSafeHdc(), rcDraw.left, rcDraw.top + iIconPosY, ILD_TRANSPARENT);
			rcDraw.left += theAppPtr->GetSmallSytemIconSize().cx;
	...
	switch (iSubItem)
	{
		...
		case 8: {	// status
			...
			if (pCtrlItem->type == AVAILABLE_SOURCE) {
				...
				//Xman
				//Xman only intern
				//if(pClient->GetDownloadState()==DS_TOOMANYCONNS)
				//	buffer.Format(_T("P:%u,M:%u,Q:%u, %s"), lpUpDownClient->m_downloadpriority, theAppPtr->downloadqueue->GetMaxDownPrio(),lpUpDownClient->GetRemoteQueueRank() ,GetResString(IDS_TOOMANYCONNS));
				//Xman Xtreme Downloadmanager
		...
		case 0: {	// icon, name, status
			...
			*/
			...
			if(theAppPtr->ip2country->ShowCountryFlag() ){
				...
				theAppPtr->ip2country->GetFlagImageList()->Draw(dc, pClient->GetCountryFlagIndex(), point3, ILD_NORMAL);
			...
			*/
			...
			if(pCtrlItem->status == (HBITMAP)NULL || pCtrlItem->dwUpdated + DLC_BARUPDATE < dwTicks || cx !=  iWidth  || !pCtrlItem->dwUpdated) { 
				...
				if (thePrefs.GetUseDwlPercentage() && pCtrlItem->type == AVAILABLE_SOURCE)
				{
					...
					{
						COLORREF crOldBackColor = dc->GetBkColor(); //zz_fly
...
#define	DrawClientPercentText		cdcStatus.DrawText(buffer, buffer.GetLength(),&rec_status, ((DLC_DT_TEXT | DT_RIGHT) & ~DT_LEFT) | DT_CENTER)
						...
						cdcStatus.SetBkColor(crOldBackColor); //zz_fly
					}
				}
				//Xman end
				pCtrlItem->dwUpdated = dwTicks + (rand() % 128); 
...
void CDownloadListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	*/
	...
	if(thePrefs.GetShowActiveDownloadsBold() && 
		( (content->type == FILE_TYPE && ((const CPartFile*)content->value)->GetTransferringSrcCount()) ||
			((content->type == UNAVAILABLE_SOURCE || content->type == AVAILABLE_SOURCE) && ((const CUpDownClient*)content->value)->GetDownloadState() == DS_DOWNLOADING)
		))
		dc.SelectObject(thePrefs.UseNarrowFont() ? &m_fontNarrowBold : m_pFontBold); //Xman narrow font at transferwindow
	else
		dc.SelectObject(pFontToUse);
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content->type == FILE_TYPE)
		{
			...
			m_FileMenu.EnableMenuItem(MP_PASTE, theAppPtr->IsEd2kFileLinkInClipboard() ? MF_ENABLED : MF_GRAYED);
			m_FileMenu.EnableMenuItem(MP_FIND, GetItemCount() > 0 ? MF_ENABLED : MF_GRAYED);
			m_FileMenu.EnableMenuItem(MP_SEARCHRELATED, theAppPtr->emuledlg->searchwnd->CanSearchRelatedFiles() ? MF_ENABLED : MF_GRAYED);
			...
			// [BCPlugin]
			// add reconnect only if Ctrl is currently pressed, for debugging/testing
			if( thePrefs.IsExtControlsEnabled() && (GetKeyState(VK_CONTROL) & 0x8000) )
				ClientMenu.AppendMenu(MF_STRING, MP_CONNECT, _T("Reconnect"), _T("CONNECTION"));
			// [BCPlugin] end
			//Xman Xtreme Downloadmanager
...
BOOL CDownloadListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{
		case MP_PASTE:
			if (theAppPtr->IsEd2kFileLinkInClipboard())
				theAppPtr->PasteClipboard(curTab);
			return TRUE;
		case MP_FIND:
			OnFindStart();
			return TRUE;
		...
		if (content->type == FILE_TYPE)
		{
								...
								switch (selectedList.GetHead()->GetStatus())
								{
									...
									case PS_COMPLETE:
										if (wParam == MP_CANCEL){
											...
											if (delsucc){
												theAppPtr->sharedfiles->RemoveFile(selectedList.GetHead());
											}
											else{
												CString strError;
									...
									default:
										if (selectedList.GetHead()->GetCategory())
											theAppPtr->downloadqueue->StartNextFileIfPrefs(selectedList.GetHead()->GetCategory());
										selectedList.GetHead()->DeleteFile();
										selectedList.RemoveHead();
										bRemovedItems = true;
								}
				...
				case MP_ALL_A4AF_TO_THIS:
					{
						...
						{
							//theAppPtr->downloadqueue->DisableAllA4AFAuto();
							POSITION pos1, pos2;
			...
			switch (wParam){
				...
				case MP_CONNECT:
					...
					{
						if(!client->AskForDownload()) // NOTE: This may *delete* the client!!
							break;
					}
					break;
					// [BCPlugin] end
...
int CDownloadListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	*/
	...
	int dwNextSort;
	if (comp == 0 && (dwNextSort = theAppPtr->emuledlg->transferwnd->downloadlistctrl.GetNextSortOrder(dwOrgSort)) != -1)
					...
					if (HitTest(&hit) >= 0 && (hit.flags & LVHT_ONITEM))
					{
						...
						if (SubItemHitTest(&subhit) >= 0 && subhit.iSubItem == 0)
						{
							...
							if (thePrefs.ShowRatingIndicator() 
								...
								&& pt.x >= sm_iIconOffset + theAppPtr->GetSmallSytemIconSize().cx 
								&& pt.x <= sm_iIconOffset + theAppPtr->GetSmallSytemIconSize().cx + RATING_ICON_WIDTH)
								ShowFileDialog(IDD_COMMENTLST);
...
void CDownloadListCtrl::ShowFilesCount()
{
	//Xman see all sources
	/*
	theAppPtr->emuledlg->transferwnd->UpdateFilesCount(GetFilesCountInCurCat());
	*/
	UINT iCount = 0;
	UINT	countsources=0; 
	UINT  countreadyfiles=0;
	for (POSITION pos = theAppPtr->downloadqueue->filelist.GetHeadPosition();pos != 0; theAppPtr->downloadqueue->filelist.GetNext(pos)){
		CPartFile* cur_file = theAppPtr->downloadqueue->filelist.GetAt(pos);
	...
	theAppPtr->emuledlg->transferwnd->UpdateFilesCount(iCount, countsources, countreadyfiles);
	//Xman End
}
...
CImageList *CDownloadListCtrl::CreateDragImage(int /*iItem*/, LPPOINT lpPoint)
{
	...
	while (pos && iSelectedItems < iMaxSelectedItems)
	{
		...
		if (pCtrlItem && pCtrlItem->type == FILE_TYPE)
		{
			...
			if (theAppPtr->GetSystemImageList())
			{
				int iImage = theAppPtr->GetFileTypeSystemImageIdx(pPartFile->GetFileName());
				ImageList_Draw(theAppPtr->GetSystemImageList(), iImage, dcMem, rcItem.left, rcItem.top, ILD_TRANSPARENT);
			}
