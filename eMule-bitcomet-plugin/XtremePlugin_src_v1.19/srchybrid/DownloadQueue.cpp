...
void CDownloadQueue::AddPartFilesToShare()
{
	for (POSITION pos = filelist.GetHeadPosition(); pos != 0; )
	{
		...
		if (cur_file->GetStatus(true) == PS_READY)
			theAppPtr->sharedfiles->SafeAddKFile(cur_file, true);
	}
}
			...
			*/
			...
			if (eResult == PLR_LOADSUCCESS)
			{
				...
				if (toadd->GetStatus(true) == PS_READY)
					theAppPtr->sharedfiles->SafeAddKFile(toadd); // part files are always shared files
				*/
				// SLUGFILLER End
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.AddFile(toadd);// show in downloadwindow
			}
			else
			{
				delete toadd;
...
// ZZ:DownloadManager -->
    if((!m_dwLastA4AFtime) || (::GetTickCount() - m_dwLastA4AFtime) > MIN2MS(8)) {
        theAppPtr->clientlist->ProcessA4AFClients();
...
void CDownloadQueue::Process(){
	...
	*/
	...
	if(deltaTime > 0){
		/*
		const float maxDownload = (theAppPtr->glob_prefs->GetNAFCEnable() == false && theAppPtr->glob_prefs->GetNAFCFullControl() == false) ? 
		app_prefs->GetMaxDownload() : theAppPtr->pBandWidthControl->GetMaxDownload();
		*/
		//Xman 1:3 Raitio
		//Xman GlobalMaxHarlimit for fairness
		const bool limitbysources=GetGlobalSources()> thePrefs.m_uMaxGlobalSources && thePrefs.m_bAcceptsourcelimit==false;
		const float maxDownload = theAppPtr->pBandWidthControl->GetMaxDownloadEx(limitbysources); //in [kb/s]
		if(limitbysources)
		...
		else 
		{
			...
			//Xman GlobalMaxHarlimit for fairness
			if(limitbysources && maxDownload < theAppPtr->pBandWidthControl->GetMaxDownloadEx(false))
			...
			m_nDownloadSlopeControl -= (sint64)(theAppPtr->pBandWidthControl->GeteMuleIn() - m_lastReceivedBytes);
		...
		//m_lastOverallReceivedBytes = theAppPtr->pBandWidthControl->GeteMuleInOverall();
		m_lastReceivedBytes = theAppPtr->pBandWidthControl->GeteMuleIn();
	...
	//Xman askfordownload priority
	if(udcounter == 0 && !(theAppPtr->listensocket->TooManySockets()) && m_maxdownprionew>0)
		m_maxdownprionew--;
	//Xman end
	udcounter++;
...
bool CDownloadQueue::CheckAndAddSource(CPartFile* sender,CUpDownClient* source){
	...
	if (theAppPtr->clientlist->m_globDeadSourceList.IsDeadSource(source) || sender->m_DeadSourceList.IsDeadSource(source)){
		...
		for (POSITION pos2 = cur_file->srclist.GetHeadPosition();pos2 != 0; ){
			...
			if (cur_client->Compare(source, true) || cur_client->Compare(source, false)){
				...
				if (cur_client->AddRequestForAnotherFile(sender)){
					theAppPtr->emuledlg->transferwnd->downloadlistctrl.AddSource(sender,cur_client,true);
					delete source;
	...
	if (theAppPtr->clientlist->AttachToAlreadyKnown(&source,0)){
#ifdef _DEBUG
		...
		theAppPtr->clientlist->AddClient(source,true);
	}
	...
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.AddSource(sender,source,false);
	//Xman GlobalMaxHarlimit for fairness
	IncGlobSources();
	//Xman end
	return true;
...
bool CDownloadQueue::CheckAndAddKnownSource(CPartFile* sender,CUpDownClient* source, bool bIgnoreGlobDeadList){
	...
	//Xman
	/*
	if ( (theAppPtr->clientlist->m_globDeadSourceList.IsDeadSource(source) && !bIgnoreGlobDeadList) || sender->m_DeadSourceList.IsDeadSource(source)){
	*/
	if ( (!bIgnoreGlobDeadList && theAppPtr->clientlist->m_globDeadSourceList.IsDeadSource(source)) || sender->m_DeadSourceList.IsDeadSource(source)){
	//Xman end
		//if (thePrefs.GetLogFilteredIPs())
		//	AddDebugLogLine(DLP_DEFAULT, false, _T("Rejected source because it was found on the DeadSourcesList (%s) for file %s : %s")
		//	,sender->m_DeadSourceList.IsDeadSource(source)? _T("Local") : _T("Global"), sender->GetFileName(), source->DbgGetClientInfo() );
	...
	for (POSITION pos = filelist.GetHeadPosition();pos != 0;){
		...
		if (cur_file->srclist.Find(source)){
			...
			if (source->AddRequestForAnotherFile(sender))
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.AddSource(sender,source,true);
				//Xman
                /*
	...
	*/
	...
	for(pos3 = toremove->m_OtherRequests_list.GetHeadPosition();(pos4=pos3)!=NULL;)
	{
		...
		{ 
			...
			theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveSource(toremove,toremove->m_OtherRequests_list.GetAt(pos4));
	...
	for(pos3 = toremove->m_OtherNoNeeded_list.GetHeadPosition();(pos4=pos3)!=NULL;)
	{
		...
		{ 
			...
			theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveSource(toremove,toremove->m_OtherNoNeeded_list.GetAt(pos4));
	...
	//Xman end
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveSource(toremove,0);
...
bool CDownloadQueue::SendGlobGetSourcesUDPPacket(CSafeMemFile* data, bool bExt2Packet, uint32 nFiles, uint32 nIncludedLargeFiles)
{
	...
	if (cur_udpserver)
	{
		...
		if (thePrefs.GetDebugServerUDPLevel() > 0)
			Debug(_T(">>> Sending %s to server %-21s (%3u of %3u); FileIDs=%u(%u large)\n"), (packet.opcode == OP_GLOBGETSOURCES2) ? _T("OP__GlobGetSources2") : _T("OP__GlobGetSources1"), ipstr(cur_udpserver->GetAddress(), cur_udpserver->GetPort()), m_iSearchedServers + 1, theAppPtr->serverlist->GetServerCount(), nFiles, nIncludedLargeFiles);
#endif //zz_fly :: DummyCut
		theStats.AddUpDataOverheadServer(packet.size);
		theAppPtr->serverconnect->SendUDPPacket(&packet, cur_udpserver, false);
...
{
	if (   filelist.IsEmpty()
        || !theAppPtr->serverconnect->IsUDPSocketAvailable()
        || !theAppPtr->serverconnect->IsConnected()
		|| thePrefs.IsClientCryptLayerRequired()) // we cannot use sources received without userhash, so dont ask
		return false;

	CServer* pConnectedServer = theAppPtr->serverconnect->GetCurrentServer();
	if (pConnectedServer)
		pConnectedServer = theAppPtr->serverlist->GetServerByAddress(pConnectedServer->GetAddress(), pConnectedServer->GetPort());

	if (!cur_udpserver)
	{
		while ((cur_udpserver = theAppPtr->serverlist->GetSuccServer(cur_udpserver)) != NULL) {
...
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
			if (thePrefs.GetDebugServerUDPLevel() > 0 && thePrefs.GetDebugServerSourcesLevel() > 0)
				Debug(_T(">>> Queued  %s to server %-21s (%3u of %3u); Buff  %u(%u)=%s\n"), bGetSources2Packet ? _T("OP__GlobGetSources2") : _T("OP__GlobGetSources1"), ipstr(cur_udpserver->GetAddress(), cur_udpserver->GetPort()), m_iSearchedServers + 1, theAppPtr->serverlist->GetServerCount(), iFiles, iLargeFiles, DbgGetFileInfo(nextfile->GetFileHash()));
#endif //zz_fly :: DummyCut
		}
	}
	...
	if ( (!m_localServerReqQueue.IsEmpty()) && (m_dwNextTCPSrcReq < ::GetTickCount()) )
	{
		...
		while (!m_localServerReqQueue.IsEmpty() && iFiles < iMaxFilesPerTcpFrame)
		{
			...
			if (posNextRequest != NULL)
			{
				...
				if (cur_file->IsLargeFile() && (theAppPtr->serverconnect->GetCurrentServer() == NULL || !theAppPtr->serverconnect->GetCurrentServer()->SupportsLargeFilesTCP())){
					ASSERT( false );
				...
				uint8 byOpcode = 0;
				if (thePrefs.IsClientCryptLayerSupported() && theAppPtr->serverconnect->GetCurrentServer() != NULL && theAppPtr->serverconnect->GetCurrentServer()->SupportsGetSourcesObfuscation())
		...
		if (iSize > 0)
		{
			...
			theStats.AddUpDataOverheadServer(packet->size);
			theAppPtr->serverconnect->SendPacket(packet, true);
		}
...
void CSourceHostnameResolveWnd::AddToResolve(const uchar* fileid, LPCSTR pszHostname, uint16 port, LPCTSTR pszURL)
{
	...
	// double checking
	if (!theAppPtr->downloadqueue->GetFileByID(fileid))
		return;
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				CPartFile* file = theAppPtr->downloadqueue->GetFileByID(resolved->fileid);
				if (file)
				{
...
void CDownloadQueue::KademliaSearchFile(uint32 searchID, const Kademlia::CUInt128* pcontactID, const Kademlia::CUInt128* pbuddyID, uint8 type, uint32 ip, uint16 tcp, uint16 udp, uint32 dwBuddyIP, uint16 dwBuddyPort, uint8 byCryptOptions)
{
	...
	if (theAppPtr->ipfilter->IsFiltered(ED2Kip))
	{
		if (thePrefs.GetLogFilteredIPs())
			//Xman show filename
			/*
			AddDebugLogLine(false, _T("IPfiltered source IP=%s (%s) received from Kademlia"), ipstr(ED2Kip), theAppPtr->ipfilter->GetLastHit());
			*/
			AddDebugLogLine(false, _T("IPfiltered source IP=%s (%s) received from Kademlia for File: %s"), ipstr(ED2Kip), theAppPtr->ipfilter->GetLastHit(), temp->GetFileName());
			//Xman end
		return;
	}
	...
	if (theAppPtr->clientlist->IsBannedClient(ED2Kip)){
		if (thePrefs.GetLogFilteredIPs() && thePrefs.GetLogBannedClients())
			AddDebugLogLine(false, _T("Banned source IP=%s (%s) received from Kademlia"), ipstr(ED2Kip), theAppPtr->ipfilter->GetLastHit());
		return;
	}
	// X-Ray :: Optimizations :: End

	if( (ip == Kademlia::CKademlia::GetIPAddress() || ED2Kip == theAppPtr->serverconnect->GetClientID()) && tcp == thePrefs.GetPort())
		return;
