...
BOOL CEMSocket::Connect(LPCSTR lpszHostAddress, UINT nHostPort)
{
	InitProxySupport();
	// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	theAppPtr->pBandWidthControl->AddeMuleOutTCPOverall(0); // SYN
	// Maella end	
...
void CEMSocket::OnConnect(int nErrorCode){
	if(nErrorCode == 0){
		// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		theAppPtr->pBandWidthControl->AddeMuleSYNACK();
		// Maella end
...
void CEMSocket::OnReceive(int nErrorCode){
	...
	*/
	...
	//Xman include ACK
	theAppPtr->pBandWidthControl->AddeMuleOutTCPOverall(0); //ACK
...
//Xman include ACK end
{
	...
	*/
	...
	{
		theAppPtr->pBandWidthControl->AddeMuleInTCPOverall(realreceivedbytes);
		theAppPtr->pBandWidthControl->AddeMuleInObfuscation(realreceivedbytes-ret);
	}
	//Xman end
			...
			case OP_COMPRESSEDPART_I64:
				{
					...
					if(client != NULL){						
						if(pendingPacketSize == 0){
							...
							theAppPtr->pBandWidthControl->AddeMuleIn(receivedBytes);
						}
						else {
							client->AddDownloadRate(toCopy); 
							theAppPtr->pBandWidthControl->AddeMuleIn(toCopy);
						}
					}
				}
				break;
...
void CEMSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32 actualPayloadSize, bool bForceImmediateSend){
        ...
        if (controlpacket) {
			...
			if(isreadyforsending)
			{
				if(!IsSocketUploading()) //Xman improved socket queuing
					theAppPtr->uploadBandwidthThrottler->QueueForSendingControlPacket(this, HasSent());
			}
	    } else {
            //Xman unused
			/*
	...
	if (bForceImmediateSend){
		...
		*/
		...
		{		
			theAppPtr->pBandWidthControl->AddeMuleOutOverallNoHeader(socketSentBytes.sentBytesControlPackets);
		}
		//Xman end
	}
}
...
//	{
//		sendLocker.Unlock();
//		theAppPtr->QueueDebugLogLine(false, _T("-=-> uploading socket tries to send control packets"));
//		SocketSentBytes returnVal = { true, 0, 0 };
//		return returnVal;
//	}
//	*/
...
//                ASSERT(0);
//                theAppPtr->QueueDebugLogLine(true,_T("EMSocket: Couldn't get a new packet! There's an error in the first while condition in EMSocket::Send()"));
//
