		...
		case ECS_UNKNOWN:{
			...
			if (!bNormalHeader){
				...
				if (thePrefs.IsClientCryptLayerRequired()){
					...
					if (thePrefs.IsClientCryptLayerRequiredStrict() || (!theAppPtr->serverconnect->AwaitingTestFromIP(sockAddr.sin_addr.S_un.S_addr)
						&& !theAppPtr->clientlist->IsKadFirewallCheckIP(sockAddr.sin_addr.S_un.S_addr)) )
					{
		...
		while (m_NegotiatingState != ONS_COMPLETE && m_nReceiveBytesWanted > 0){		
			...
			switch (m_NegotiatingState){
				...
				case ONS_BASIC_CLIENTA_PADDING:{
					...
					int nSockAddrLen = sizeof(sockAddr);
					GetPeerName((SOCKADDR*)&sockAddr, &nSockAddrLen);	
					const uint8 byPaddingLen = theAppPtr->serverconnect->AwaitingTestFromIP(sockAddr.sin_addr.S_un.S_addr) ? 16 : (thePrefs.GetCryptTCPPaddingLength() + 1);
	...
	if (result == (uint32)SOCKET_ERROR || bDelaySend){
		...
		if(oldState==ONS_BASIC_SERVER_DELAYEDSENDING && m_NegotiatingState==ONS_COMPLETE)
		{
			...
			if(result > initiatedBufLen)
				theAppPtr->pBandWidthControl->AddeMuleOutObfuscation(result-initiatedBufLen);
			...
			theAppPtr->pBandWidthControl->AddeMuleOutObfuscationTCP(result);
		//Xman end
		if (result < nBufLen){
			m_pfiSendBuffer = new CSafeMemFile(128);
