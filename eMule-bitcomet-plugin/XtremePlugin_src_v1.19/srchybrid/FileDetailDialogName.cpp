...
void CFileDetailDialogName::FillSourcenameList()
{
	...
	{ 
		...
		{ 
			...
			int iSystemIconIdx = theAppPtr->GetFileTypeSystemImageIdx(cur_src->GetClientFilename());
			if (theAppPtr->GetSystemImageList() && !m_bAppliedSystemImageList)
			{
				m_listFileNames.ApplyImageList(theAppPtr->GetSystemImageList());
				ASSERT( (m_listFileNames.GetStyle() & LVS_SHAREIMAGELISTS) != 0 );
...
void CFileDetailDialogName::Copy()
{
	int iSel = m_listFileNames.GetNextItem(-1, LVIS_SELECTED | LVIS_FOCUSED);
	if (iSel != -1)
		theAppPtr->CopyTextToClipboard(m_listFileNames.GetItemText(iSel, 0));
}
...
void CFileDetailDialogName::OnLvnColumnClick(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	m_listFileNames.UpdateSortHistory(pNMListView->iSubItem + (sortAscending ? 0 : 10), 10); // SLUGFILLER: multiSort - forgot something? from DolphinX
