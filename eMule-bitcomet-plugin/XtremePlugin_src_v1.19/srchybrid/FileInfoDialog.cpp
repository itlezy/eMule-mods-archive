...
public:
	...
	BOOL Initialize()
	{
		if (!m_bInitialized)
		{
			...
			CString strPath = theAppPtr->GetProfileString(_T("eMule"), _T("MediaInfo_MediaInfoDllPath"), _T("MEDIAINFO.DLL"));
...
bool CGetMediaInfoThread::GetMediaInfo(HWND hWndOwner, const CShareableFile* pFile, SMediaInfo* mi, bool bSingleFile)
{
	...
	if(!pFile->IsPartFile() && !theAppPtr->sharedfiles->IsFilePtrInList((CKnownFile*)pFile)){
		return false;
	}
	//Xman end
	...
	//
	bool bIsAVI = false;
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("MediaInfo_RIFF"), 1))
	{
		try
		{
	...
	//
	bool bIsRM = false;
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("MediaInfo_RM"), 1))
	{
		try
		{
...
#ifdef HAVE_WMSDK_H
	bool bIsWM = false;
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("MediaInfo_WM"), 1))
	{
		try
		{
	...
	//
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("MediaInfo_ID3LIB"), 1) &&
...
#ifdef HAVE_QEDIT_H
		if (theAppPtr->GetProfileInt(_T("eMule"), _T("MediaInfo_MediaDet"), 1)
...
#pragma message("WARNING: Missing 'qedit.h' header file - some features will get disabled. See the file 'emule_site_config.h' for more information.")
	...
	if (!bFoundHeader && bGiveMediaInfoLibHint)
	{
		...
		DWORD dwModPathLen = ::GetModuleFileName(theAppPtr->m_hInstance, szBuff, _countof(szBuff));
