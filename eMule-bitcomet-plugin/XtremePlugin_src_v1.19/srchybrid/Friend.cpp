...
void CFriend::SetLinkedClient(CUpDownClient* linkedClient) {
	...
	theAppPtr->friendlist->RefreshFriend(this);
}

CUpDownClient* CFriend::GetLinkedClient(bool bValidCheck) const
{
	if (bValidCheck && m_LinkedClient != NULL && !theAppPtr->clientlist->IsValidClient(m_LinkedClient)){
		ASSERT( false );
		return NULL;
	}
...
CUpDownClient* CFriend::GetClientForChatSession()
{
	...
	if (GetLinkedClient(true) != NULL)
		...
		theAppPtr->clientlist->AddClient(pResult);
...
bool CFriend::TryToConnect(CFriendConnectionListener* pConnectionReport)
{
	...
	if (GetLinkedClient(true) == NULL)
	{
		//ASSERT( pConnectionReport != &theAppPtr->emuledlg->chatwnd->chatselector ); // shouldn't happen, if the chat connector calls, we he always should have a client for the session already
		ASSERT( false );
...
void CFriend::KadSearchNodeIDByIPResult(Kademlia::EKadClientSearchRes eStatus, const uchar* pachNodeID){
	if (!theAppPtr->friendlist->IsValid(this))
	{
		ASSERT( false );
		return;
	}
