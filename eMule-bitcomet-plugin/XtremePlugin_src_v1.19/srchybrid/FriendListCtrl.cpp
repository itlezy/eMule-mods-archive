...
void CFriendListCtrl::Init()
{
	...
	SetAllIcons();
	theAppPtr->friendlist->SetWindow(this);
	LoadSettings();
	SetSortArrow();
}
...
void CFriendListCtrl::AddFriend(const CFriend* pFriend)
{
	...
	theAppPtr->emuledlg->chatwnd->UpdateFriendlistCount(theAppPtr->friendlist->GetCount());
}
...
void CFriendListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
    ...
    ClientMenu.AppendMenu(MF_STRING | (theAppPtr->IsEd2kFriendLinkInClipboard() ? MF_ENABLED : MF_GRAYED), MP_PASTE, GetResString(IDS_PASTE), _T("PASTELINK"));
...
BOOL CFriendListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	switch (wParam)
	{
		case MP_MESSAGE:
			if (cur_friend)
			{
				theAppPtr->emuledlg->chatwnd->StartSession(cur_friend->GetClientForChatSession());
			}
			break;
		case MP_REMOVEFRIEND:
			if (cur_friend)
			{
				theAppPtr->friendlist->RemoveFriend(cur_friend);
				...
				theAppPtr->emuledlg->chatwnd->UpdateSelectedFriendMsgDetails();
			}
			break;
		case MP_ADDFRIEND:{
			CAddFriend dialog2; 
		...
		case MP_PASTE:
		{
			CString link = theAppPtr->CopyTextFromClipboard();
			link.Trim();
			if ( link.IsEmpty() )
				break;
			try{
        ...
        case MP_GETFRIENDED2KLINK:
		{
			...
			if ( !sCompleteLink.IsEmpty() )
				theAppPtr->CopyTextToClipboard(sCompleteLink);
		}
			break;
...
{
	ShowWindow(SW_HIDE);
	bool ret = theAppPtr->friendlist->AddEmfriendsMetToList(strFile);
	theAppPtr->friendlist->ShowFriends();
	UpdateList();
	ShowWindow(SW_SHOW);
	return ret;
}
