...
void CIP2Country::Reset(){
	theAppPtr->serverlist->ResetIP2Country();
	theAppPtr->clientlist->ResetIP2Country();
}

void CIP2Country::Refresh(){
	theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshAllServer();
}
*/
...
bool CIP2Country::LoadCountryFlagLib(){
	...
	try{
		...
		CountryFlagImageList.Create(18,16,theAppPtr->m_iDfltImageListColorFlags|ILC_MASK,0,1);
