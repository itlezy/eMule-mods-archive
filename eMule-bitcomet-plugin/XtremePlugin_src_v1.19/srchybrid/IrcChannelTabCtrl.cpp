...
{
	CImageList imlist;
	imlist.Create(16, 16, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
...
//#define DEBUG_IRC_TEXT
			...
			)
		{
			...
			pChannel->m_wndTitle.SetFont(&theAppPtr->m_fontHyperText);
			...
			if (eType == Channel::ctStatus) {
				pChannel->m_wndTitle.AddLine(_T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ^?234567890!\"?%&/()=?´`²³{[]}ß\\öäüÖÄÜ+*~#',.-;:_"));
			}
#endif
		...
		pChannel->m_wndLog.SetFont(&theAppPtr->m_fontHyperText);
...
void CIrcChannelTabCtrl::OnTcnSelChange(NMHDR*, LRESULT* pResult)
{
	...
	if(m_pParent->m_wndInput.IsWindowVisible()) // [BCPlugin] avoid set focus when current window is not visible
