			...
			case CED2KLink::kFile:
				{
					...
					{
						theAppPtr->downloadqueue->AddFileLinkToDownload(pFileLink);
					}
					//Xman end
					break;
				}
			case CED2KLink::kServerList:
				{
					...
					if(sAddress.GetLength() != 0)
						theAppPtr->emuledlg->serverwnd->UpdateServerMetFromURL(sAddress);
					break;
				}
			case CED2KLink::kServer:
				{
					...
					if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(pSrv,true))
						delete pSrv;
					else
		...
		if (sCommand == _T("PRIVMSG"))
		{
			...
			if (sRawMessage.Mid(iIndex, 2) == _T(":\001"))
			{
				...
				if( sRawMessage.Mid(iIndex, 9) == _T(":\001VERSION") )
				{
					...
					//Get client version.
					iIndex += 9;
					m_sVersion = _T("eMule") + theAppPtr->m_strCurVersionLong + Irc_Version;
					CString sBuild;
				...
				if( sRawMessage.Mid(iIndex, 11) == _T(":\001RQSFRIEND") )
				{
					...
					if(theAppPtr->serverconnect->IsConnected())
					{
						...
						sIP.Format( _T("%i"), theAppPtr->serverconnect->GetCurrentServer()->GetIP());
						sPort.Format( _T("%i"), theAppPtr->serverconnect->GetCurrentServer()->GetPort());
					}
					else
					{
						//Not on a server.
					...
					//Create our response.
					CString sBuild;
					sBuild.Format(_T("PRIVMSG %s :\001REPFRIEND eMule%s%s|%s|%u:%u|%s:%s|%s|\001"), sTarget, theAppPtr->m_strCurVersionLong, Irc_Version, sVerify, theAppPtr->IsFirewalled() ? 0 : theAppPtr->GetID(), thePrefs.GetPort(), sIP, sPort, md4str(thePrefs.GetUserHash()));
					m_pIRCSocket->SendString(sBuild);
				...
				if( sRawMessage.Mid(iIndex, 11) == _T(":\001REPFRIEND") )
				{
					...
					if (theAppPtr->friendlist->IsAlreadyFriend(sHash))
						return;
					uchar ucharUserID[16];
					if (!strmd4(sHash,ucharUserID))
						throw CString( _T("SMIRC Error: Received Invalid friend reply") );
					theAppPtr->friendlist->AddFriend( ucharUserID, 0, uNewClientID, uNewClientPort, 0, sTarget, 1);
					return;
				}
				if( sRawMessage.Mid(iIndex, 10) == _T(":\001SENDLINK") )
				{
					...
					CString sHash = sRawMessage.Tokenize(_T("|"), iIndex);
					if (thePrefs.GetIRCAcceptLinksFriendsOnly() && !theAppPtr->friendlist->IsAlreadyFriend(sHash)){
						if( !thePrefs.GetIRCIgnoreEmuleSendLinkMsgs() )
						{
