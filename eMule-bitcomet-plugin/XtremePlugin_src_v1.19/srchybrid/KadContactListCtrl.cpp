...
void CKadContactListCtrl::UpdateKadContactCount()
{
	...
	theAppPtr->emuledlg->kademliawnd->GetDlgItem(IDC_KADCONTACTLAB)->SetWindowText(id);
}
			...
			if (cur_rec.left < cur_rec.right && HaveIntersection(rcClient, cur_rec))
			{
				...
				switch (iColumn)
				{
					case colID:
					{
						...
						if(theAppPtr->ip2country->ShowCountryFlag() && thePrefs.m_bShowCountryFlagInKad){
							POINT point2 = {cur_rec.left,cur_rec.top + iIconPosY + 1};
							Country_Struct* tmpCountryStruct = theAppPtr->ip2country->GetCountryFromIP(contact->GetIPAddress());
							theAppPtr->ip2country->GetFlagImageList()->Draw(dc, tmpCountryStruct->FlagIndex, point2, ILD_NORMAL);
