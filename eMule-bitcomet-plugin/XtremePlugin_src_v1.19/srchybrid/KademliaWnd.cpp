...
BOOL CKademliaWnd::OnInitDialog()
{
	...
	CheckDlgButton(IDC_RADNODESURL,1); //zz_fly :: open a default website when nodesurl is invalid
	GetDlgItem(IDC_BOOTSTRAPURL)->SetWindowText(_T("www.nodes-dat.com")); //zz_fly :: open a default website when nodesurl is invalid, set default url
	return true;
}
...
void CKademliaWnd::SetAllIcons()
{
	...
	if (icon_kadcont)
		VERIFY( DestroyIcon(icon_kadcont) );
	icon_kadcont = theAppPtr->LoadIcon(_T("KadContactList"), 16, 16);
	((CStatic*)GetDlgItem(IDC_KADICO1))->SetIcon(icon_kadcont);

	if (icon_kadsea)
		VERIFY( DestroyIcon(icon_kadsea) );
	icon_kadsea = theAppPtr->LoadIcon(_T("KadCurrentSearches"), 16, 16);
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Kad);
	return TRUE;
}
