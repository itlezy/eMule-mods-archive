...
void CKnownFile::UpdateFileRatingCommentAvail(bool bForceUpdate)
{
	...
	if (bOldHasComment != m_bHasComment || uOldUserRatings != m_uUserRating || bForceUpdate)
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.UpdateFile(this);
}
...
void CKnownFile::UpdatePartsInfo()
{
	...
	if (theAppPtr->emuledlg->sharedfileswnd->m_hWnd)
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.UpdateFile(this);
}
...
{ 
	...
	if(theAppPtr->sharedfiles)
		pFile = theAppPtr->sharedfiles->GetFileByID(GetFileHash());

	if (pFile && pFile == this)
		theAppPtr->sharedfiles->RemoveKeywords(this);
	...
	if (pFile && pFile == this)
		theAppPtr->sharedfiles->AddKeywords(this);
} 
...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	for (hashcount = 0; togo >= PARTSIZE; )
	{
		...
		*/
		...
		if (theAppPtr->emuledlg==NULL || !theAppPtr->emuledlg->IsRunning()){ // in case of shutdown while still hashing
			fclose(file);
			delete[] newhash;
			return false;
		}
		...
		if(GetPartCount()>0) //just to be sure
		{
			...
			if (theAppPtr->emuledlg->statusbar->m_hWnd) theAppPtr->emuledlg->statusbar->SetText( pourcent ,0,0);
		}
		//Xman end
		...
		if (pvProgressParam && theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning()){
			...
			VERIFY( PostMessage(theAppPtr->emuledlg->GetSafeHwnd(), TM_FILEOPPROGRESS, uProgress, (LPARAM)pvProgressParam) );
		}
	}
	...
	*/
	...
	if (!hashcount){
		...
		*/
		...
		// SLUGFILLER: SafeHash remove - removed up
		/*
...
bool CKnownFile::CreateAICHHashSetOnly()
{
	...
	theAppPtr->knownfiles->AICHHashChanged(bOldMasterHashValid ? &oldMasterHash : NULL, m_pAICHHashSet->GetMasterHash(), this);
	return true;	
}
...
//Xman end
{
	...
	//Xman
	CSingleLock sLock1(&(theAppPtr->hashing_mut), TRUE);	// SLUGFILLER: SafeHash - only one chunk-hash at a time
...
Packet*	CKnownFile::CreateSrcInfoPacket(const CUpDownClient* forClient, uint8 byRequestedVersion, uint16 nRequestedOptions) const
{
	...
	for (POSITION pos = m_ClientUploadList.GetHeadPosition(); pos != 0; )
	{
		...
		if (!theAppPtr->clientlist->IsValidClient(cur_src))
		{
#ifdef _BETA
...
void CKnownFile::SetUpPriority(uint8 iNewUpPriority, bool bSave)
{
	...
	*/
	...
	{
		m_iUpPriority = iNewUpPriority;
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.UpdateFile(this,true);
	}
	else
		m_iUpPriority = iNewUpPriority;
...
bool CKnownFile::PublishSrc()
{
		...
		(Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) || !Kademlia::CUDPFirewallTester::IsVerified()))
	{
		...
		if( buddy )
		{
			lastBuddyIP = theAppPtr->clientlist->GetBuddy()->GetIP();
...
{
	// continue processing
	if (pSender == theAppPtr->mmserver){
		theAppPtr->mmserver->PreviewFinished(imgResults, nFramesGrabbed);
	}
	else if (theAppPtr->clientlist->IsValidClient((CUpDownClient*)pSender)){
...
void CKnownFile::CalculateAndSetUploadPriority()
{
	...
	float avgpercent = theAppPtr->sharedfiles->m_lastavgPercent;
	...
	if((uint64)GetFileSize() > 500*1024)
	{
		...
		if(GetOnUploadqueue() < theAppPtr->sharedfiles->m_avg_client_on_uploadqueue/8)
			wantedprio=PR_HIGH;
		else if(wantedprio==PR_LOW && GetOnUploadqueue() < theAppPtr->sharedfiles->m_avg_client_on_uploadqueue/4)
			wantedprio=PR_NORMAL;
...
void CKnownFile::CalculateAndSetUploadPriority2()
{
	...
	if((uint64)GetFileSize() > 500*1024)
	{
		...
		if(wantedprio!=PR_HIGH && GetOnUploadqueue() < theAppPtr->sharedfiles->m_avg_client_on_uploadqueue/8)
		{
			wantedprio=PR_HIGH;
...
uint64 CKnownFile::GetWantedUpload()
{
	...
	if(!thePrefs.GetEnableMultiQueue()  )
	{
		...
		pushfaktor=1;
		if(theAppPtr->sharedfiles->m_avg_virtual_sources > 0)
		{
			if(virtualsources < theAppPtr->sharedfiles->m_avg_virtual_sources)
			{
				virtualsources = theAppPtr->sharedfiles->m_avg_virtual_sources - virtualsources;
				pushfaktor = 1 + (float)virtualsources / theAppPtr->sharedfiles->m_avg_virtual_sources;
			}
			else
			{
				if(virtualsources > theAppPtr->sharedfiles->m_avg_virtual_sources*3)
					pushfaktor = 0.67f; //-33%
				else if(virtualsources > theAppPtr->sharedfiles->m_avg_virtual_sources*2)
					pushfaktor = 0.85f; //-15%
			}
		}
...
bool CKnownFile::IsPushSmallFile() const
{
