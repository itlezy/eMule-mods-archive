	...
	if (!file.Open(fullpath, CFile::modeWrite|CFile::modeCreate|CFile::typeBinary|CFile::shareDenyWrite, &fexp)){
		...
		try{
			...
			while( pos != NULL )
			{
				...
				if (!thePrefs.IsRememberingDownloadedFiles() && !theAppPtr->sharedfiles->IsFilePtrInList(pFile)){
					continue;
				}
				else{
...
bool CKnownFileList::SafeAddKFile(CKnownFile* toadd)
{
	...
	if (m_Files_map.Lookup(key, pFileInMap))
	{
		...
		if (theAppPtr->sharedfiles)
		{
			...
			theAppPtr->sharedfiles->RemoveKeywords(pFileInMap);
#else
			...
			if (theAppPtr->sharedfiles->IsFilePtrInList(pFileInMap))
				bRemovedDuplicateSharedFile = theAppPtr->sharedfiles->RemoveFile(pFileInMap);
#endif
			ASSERT( !theAppPtr->sharedfiles->IsFilePtrInList(pFileInMap) );
		}
		...
		ASSERT( theAppPtr->sharedfiles==NULL || !theAppPtr->sharedfiles->IsFilePtrInList(pFileInMap) );
		ASSERT( theAppPtr->downloadqueue==NULL || !theAppPtr->downloadqueue->IsPartFile(pFileInMap) );
		...
		if (theAppPtr->emuledlg && theAppPtr->emuledlg->transferwnd && theAppPtr->emuledlg->transferwnd->downloadlistctrl.m_hWnd)
			theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveFile((CPartFile*)pFileInMap);

		//Xman [MoNKi: -Downloaded History-]
		theAppPtr->emuledlg->sharedfileswnd->historylistctrl.RemoveFileFromView(pFileInMap);
		//Xman end
		delete pFileInMap;
	}
	m_Files_map.SetAt(key, toadd);
	if (bRemovedDuplicateSharedFile) {
		theAppPtr->sharedfiles->SafeAddKFile(toadd);
	}
...
bool CKnownFileList::ShouldPurgeAICHHashset(const CAICHHash& rAICHHash) const
{
	...
	//else
		//ASSERT( false ); //zz_fly :: different clean-up between Xtreme and official
	return true;
}
...
bool CKnownFileList::CheckAlreadyDownloadedFileQuestion(const uchar* hash, CString filename){
	...
	if(theAppPtr->downloadqueue->IsFileExisting(hash)){
		return false;
	}
...
CKnownFilesMap* CKnownFileList::GetDownloadedFiles(){
	...
	while(pos){
		...
		CCKey key;
		m_Files_map.GetNextAssoc( pos, key, cur_file );
		if (!theAppPtr->sharedfiles->IsFilePtrInList(cur_file)){
	...
	while(pos){
		...
		if (!theAppPtr->sharedfiles->IsFilePtrInList(cur_file)){
			...
			if (cur_file->IsKindOf(RUNTIME_CLASS(CPartFile)))
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.ClearCompleted(static_cast<CPartFile*>(cur_file));
			delete cur_file;
		}
	}	
}
