...
{
	SetClient(in_client);
	theAppPtr->listensocket->AddSocket(this);
	ResetTimeOutTimer();
	deletethis = false;
	deltimer = 0;
...
void CClientReqSocket::SetConState( SocketState val )
{
	...
	{
		case SS_Half:
			theAppPtr->listensocket->m_nHalfOpen--;
			break;
		case SS_Complete:
			theAppPtr->listensocket->m_nComp--;
	}
...
CClientReqSocket::~CClientReqSocket()
{
	...
	*/
	...
	client = NULL;
	// Maella End
	theAppPtr->listensocket->RemoveSocket(this);

	DEBUG_ONLY (theAppPtr->clientlist->Debug_SocketDeleted(this));
}
...
bool CClientReqSocket::CheckTimeOut()
{
	...
	{
		//timeout_timer = ::GetTickCount();
		theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("0-Uploadsocket"),CUpDownClient::USR_SOCKET); // Maella -Upload Stop Reason-
	}
	//Xman end
	return false;
}
...
void CClientReqSocket::OnClose(int nErrorCode){
	ASSERT (theAppPtr->listensocket->IsValidSocket(this));
...
bool CClientReqSocket::ProcessPacket(const BYTE* packet, uint32 size, UINT opcode)
{
			...
			switch(opcode)
			{
				case OP_HELLOANSWER:
				{
					...
					if (client)
					{
						...
						theAppPtr->emuledlg->transferwnd->clientlistctrl.RefreshClient(client);
					}
					break;
				}
				case OP_HELLO:
				{
					...
					*/
					...
					if (theAppPtr->clientlist->AttachToAlreadyKnown(&client,this))
					{
					...
					else 
					{
						...
						/*
						theAppPtr->clientlist->AddClient(client);
						*/
						theAppPtr->clientlist->AddClient(client, bNewClient );
						//Xman end
				...
				case OP_REQUESTFILENAME:
				{
					...
					if (size >= 16)
					{
						...
						CKnownFile* reqfile;
						if ( (reqfile = theAppPtr->sharedfiles->GetFileByID(reqfilehash)) == NULL ){
							if ( !((reqfile = theAppPtr->downloadqueue->GetFileByID(reqfilehash)) != NULL
						...
						if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64)PARTSIZE)
						{
							...
							*/
							...
							//Xman end
								theAppPtr->downloadqueue->CheckAndAddKnownSource((CPartFile*)reqfile, client, true);
						}
				...
				case OP_SETREQFILEID:
				{
					...
					if (size == 16)
					{
						if (!client->GetWaitStartTime())
							client->SetWaitStartTime();
						CKnownFile* reqfile;
						if ( (reqfile = theAppPtr->sharedfiles->GetFileByID(packet)) == NULL ){
							if ( !((reqfile = theAppPtr->downloadqueue->GetFileByID(packet)) != NULL
								&& reqfile->GetFileSize() > (uint64)PARTSIZE) )
							{
				...
				case OP_FILEREQANSNOFIL:
				{
					...
					if (size == 16)
					{
						CPartFile* reqfile = theAppPtr->downloadqueue->GetFileByID(packet);
						if (!reqfile){
							client->CheckFailedFileIdReqs(packet);
							break;
						}
						...
						{
							CKnownFile* upfile = theAppPtr->sharedfiles->GetFileByID(client->GetUploadFileID());
							if(upfile && upfile == reqfile) //we speak about the same file
							{
								...
								theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("Src says he does not have the file he's dl'ing"));
								theAppPtr->uploadqueue->RemoveFromWaitingQueue(client);
							}
						}
						//Xman end
						...
						switch (client->GetDownloadState())
						{
							...
							case DS_NONEEDEDPARTS:
                                ...
                                if (!client->SwapToAnotherFile(_T("Source says it doesn't have the file. CClientReqSocket::ProcessPacket()"), true, true, true, NULL, false, false)) { // ZZ:DownloadManager
    								theAppPtr->downloadqueue->RemoveSource(client);
                                }
								*/
				...
				case OP_REQFILENAMEANSWER:
				{
					...
					CPartFile* file = theAppPtr->downloadqueue->GetFileByID(cfilehash);
					if (file == NULL)
				...
				case OP_STARTUPLOADREQ:
				{
					...
					//zz_fly :: moved from CUploadQueue::AddClientToQueue() :: start
					/* //test code
					if (size == 16){
						CKnownFile* reqfile = theAppPtr->sharedfiles->GetFileByID(packet);
						if (reqfile){
							client->AddRequestCount(packet);
						}
					}
					*/
					//zz_fly :: end
					...
					if (size == 16)
					{
						...
						if (reqfile)
						{
							...
							theAppPtr->uploadqueue->AddClientToQueue(client);
						}
						else
							client->CheckFailedFileIdReqs(packet);
					}
				...
				case OP_REQUESTPARTS:
				{
					...
					{
						theAppPtr->uploadqueue->RemoveFromUploadQueue(client,_T("banned client detected during upload")); 
					...
					CKnownFile* reqfileNr1 = theAppPtr->sharedfiles->GetFileByID(reqfilehash);
					CKnownFile* reqfileNr2 = theAppPtr->sharedfiles->GetFileByID(client->GetUploadFileID());
					if(reqfileNr1==NULL) 
					{
						...
						theAppPtr->uploadqueue->RemoveFromUploadQueue(client,_T("Client requested unknown file"),CUpDownClient::USR_DIFFERENT_FILE); // Maella -Upload Stop Reason-
						client->SetUploadFileID(NULL); 
						break;
					}							
					...
					*/
					...
					// <== push small files [sivka] - Stulle
					{
						...
						theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("wrong file"), CUpDownClient::USR_DIFFERENT_FILE,true); // Maella -Upload Stop Reason-
				...
				case OP_CANCELTRANSFER:
				{
					...
					/*
					theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("Remote client cancelled transfer."));
					*/
					if (theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("Remote client cancelled transfer."), CUpDownClient::USR_CANCELLED)){ // Maella -Upload Stop Reason-
						client->SetUploadFileID(NULL);
					}
					//Xman end
					break;
				...
				case OP_END_OF_DOWNLOAD:
				{
					...
					if (size>=16 && !md4cmp(client->GetUploadFileID(),packet))
					{
						//Xman Code Improvement
						/*
						theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("Remote client ended transfer."));
						*/
						if (theAppPtr->uploadqueue->RemoveFromUploadQueue(client, _T("Remote client ended transfer."), CUpDownClient::USR_CANCELLED)){ // Maella -Upload Stop Reason-
							client->SetUploadFileID(NULL);
						}
						//Xman end
					}
				...
				case OP_CHANGE_CLIENT_ID:
				{
					...
					{	// client changed server and has a LowID
						CServer* pNewServer = theAppPtr->serverlist->GetServerByIP(nNewServerIP);
						if (pNewServer != NULL)
						{
				...
				{	
					...
					while (list.GetCount())
					{
						theAppPtr->sharedfiles->CreateOfferedFilePacket((CKnownFile*)list.GetHead(), &tempfile, NULL, client);
						list.RemoveHead();
					}
                ...
                case OP_ASKSHAREDFILESDIR:
				{
                    ...
                    if (thePrefs.CanSeeShares()==vsfaEverybody || (thePrefs.CanSeeShares()==vsfaFriends && client->IsFriend()))
					{
						...
						if (strReqDir == OP_INCOMPLETE_SHARED_FILES)
						{
							...
							int iQueuedFiles = theAppPtr->downloadqueue->GetFileCount();
							for (int i = 0; i < iQueuedFiles; i++)
							{
								CPartFile* pFile = theAppPtr->downloadqueue->GetFileByIndex(i);
						...
						else
						{
							...
							if (!bSingleSharedFiles)
								strReqDir = theAppPtr->sharedfiles->GetDirNameByPseudo(strReqDir);
							if (!strReqDir.IsEmpty())
							{
								...
								for (POSITION pos = theAppPtr->sharedfiles->m_Files_map.GetStartPosition();pos != 0;)
								{
									...
									if (((!bSingleSharedFiles && CompareDirectories(strReqDir, cur_file->GetSharedDirectory()) == 0) || (bSingleSharedFiles && !theAppPtr->sharedfiles->ShouldBeShared(cur_file->GetSharedDirectory(), _T(""), false)))
						...
						while (list.GetCount())
						{
							theAppPtr->sharedfiles->CreateOfferedFilePacket(list.GetHead(), &tempfile, NULL, client);
							list.RemoveHead();
						}
	...
	*/
	catch(CString error)
	{
		...
		theAppPtr->ipfilter->AddIPTemporary(ntohl(sockAddr.sin_addr.S_un.S_addr));
		if (client != NULL){
			if (thePrefs.GetVerbose() && !error.IsEmpty()){
					DebugLogWarning(_T("Error: %s - while processing eDonkey packet: opcode=%s  size=%u; %s"), error, DbgGetDonkeyClientTCPOpcode(opcode), size, DbgGetClientInfo());
		...
		//Xman end
			...
			switch(opcode)
			{
				...
				case OP_MULTIPACKETANSWER:
				{
					...
					uchar reqfilehash[16];
					data_in.ReadHash16(reqfilehash);
					CPartFile* reqfile = theAppPtr->downloadqueue->GetFileByID(reqfilehash);
					...
					if (reqfile==NULL){
						...
						CKnownFile* reqfiletocheck = theAppPtr->sharedfiles->GetFileByID(reqfilehash);
				...
				case OP_REQUESTSOURCES2:
				{
					...
					if (byRequestedVersion > 0 || client->GetSourceExchange1Version() > 1)
					{
						...
						CKnownFile* reqfile;
						if ((reqfile = theAppPtr->sharedfiles->GetFileByID(ucHash)) != NULL ||
							(reqfile = theAppPtr->downloadqueue->GetFileByID(ucHash)) != NULL)
						{
 				...
 				case OP_ANSWERSOURCES:
				{
					...
					CKnownFile* file = theAppPtr->downloadqueue->GetFileByID(hash);
					if (file){
						if (file->IsPartFile()){
				...
				case OP_CALLBACK:
				{
					...
					if(check == Kademlia::CKademlia::GetPrefs()->GetKadID())
					{
						...
						CKnownFile* reqfile;
						if ( (reqfile = theAppPtr->sharedfiles->GetFileByID(fileid2)) == NULL )
						{
							if ( (reqfile = theAppPtr->downloadqueue->GetFileByID(fileid2)) == NULL)
							{
						...
						callback = theAppPtr->clientlist->FindClientByIP(ntohl(ip), tcp);
						if( callback == NULL )
						{
							callback = new CUpDownClient(NULL,tcp,ip,0,0);
							theAppPtr->clientlist->AddClient(callback);
						}
				...
				case OP_REASKCALLBACKTCP:
				{
					...
					CUpDownClient* sender = theAppPtr->uploadqueue->GetWaitingClientByIP_UDP(destip, destport, true, &bSenderMultipleIpUnknown);					
					if (!reqfile)
					{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
						...
						if (sender != NULL)
							theAppPtr->clientudp->SendPacket(response, destip, destport, sender->ShouldReceiveCryptUDPPackets(), sender->GetUserHash(), false, 0);
						else
							theAppPtr->clientudp->SendPacket(response, destip, destport, false, NULL, false, 0);
						break;
					}
				...
				case OP_AICHFILEHASHREQ:
				{
					...
					CKnownFile* pPartFile = theAppPtr->sharedfiles->GetFileByID(abyHash);
					if (pPartFile == NULL){
				...
				case OP_REQUESTPARTS_I64:
				{
					...
					if(reqfileNr1==NULL) 
					{
						...
						theStats.AddUpDataOverheadFileRequest(packet->size);
						client->socket->SendPacket(packet, true, true);
						theAppPtr->uploadqueue->RemoveFromUploadQueue(client,_T("Client requested download unknown file"),CUpDownClient::USR_DIFFERENT_FILE); // Maella -Upload Stop Reason-
						client->SetUploadFileID(NULL); 
						break;
					}								
					// ==> push small files [sivka] - Stulle - added by zz_fly
...
#endif //zz_fly :: DummyCut
					if (theAppPtr->clientlist->IsKadFirewallCheckIP(client->GetIP())){
						if (Kademlia::CKademlia::IsRunning())
...
void CClientReqSocket::OnSend(int nErrorCode)
{
	...
	if(theAppPtr->listensocket->boundcheck)
	{
		theAppPtr->listensocket->boundcheck=false;
		sockaddr_in socketcheck;
		int length= sizeof(socketcheck);
		if(getsockname(m_SocketData.hSocket, (SOCKADDR*)&socketcheck, &length)==0)
		{
			theAppPtr->pBandWidthControl->SetBoundIP(socketcheck.sin_addr.S_un.S_addr);
		}
	}
}
		...
		default:{
			...
			*/
			if (client){
				...
				theAppPtr->clientlist->m_globDeadSourceList.AddDeadSource(client); //Xman Xtreme Mod  I don't AddDeadsource in disconnected
			}
...
{
	theAppPtr->listensocket->AddConnection();
...
bool CListenSocket::Rebind()
{
	...
	{
		if(theAppPtr->m_UPnPNat.RemoveSpecifiedPort(thePrefs.m_iUPnPTCPExternal, MyUPnP::UNAT_TCP))
								  ...
								  LPWSABUF /*lpCalleeId*/, LPWSABUF /*lpCalleeData*/, GROUP FAR* /*g*/, DWORD /*dwCallbackData*/)
{
	if (lpCallerId && lpCallerId->buf && lpCallerId->len >= sizeof SOCKADDR_IN)
	{
		...
		if (theAppPtr->ipfilter->IsFiltered(pSockAddr->sin_addr.S_un.S_addr)){
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Rejecting connection attempt (IP=%s) - IP filter (%s)"), ipstr(pSockAddr->sin_addr.S_un.S_addr), theAppPtr->ipfilter->GetLastHit());
			s_iAcceptConnectionCondRejected = 1;
			return CF_REJECT;
		}

		if (theAppPtr->clientlist->IsBannedClient(pSockAddr->sin_addr.S_un.S_addr)){
			if (thePrefs.GetLogBannedClients()){
				CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(pSockAddr->sin_addr.S_un.S_addr);
				//Xman code fix
				/*
...
void CListenSocket::OnAccept(int nErrorCode)
{
	if (!nErrorCode)
	{
		...
		theAppPtr->pBandWidthControl->AddeMuleInTCPOverall(0); // SYN
		// Maella end
		m_nPendingConnections++;
		...
		if (TooManySockets(true) && !theAppPtr->serverconnect->IsConnecting()){
			StopListening();
			return;
		}
		else if (!bListening)
		...
		while (m_nPendingConnections > 0)
		{
			...
			if (thePrefs.GetConditionalTCPAccept() && !thePrefs.GetProxySettings().UseProxy)
			{
				...
				//Xman
				// - Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
				theAppPtr->pBandWidthControl->AddeMuleOutTCPOverall(0);
				// Maella end
			...
			else
			{
				...
				if (theAppPtr->ipfilter->IsFiltered(SockAddr.sin_addr.S_un.S_addr)){
					if (thePrefs.GetLogFilteredIPs())
						AddDebugLogLine(false, _T("Rejecting connection attempt (IP=%s) - IP filter (%s)"), ipstr(SockAddr.sin_addr.S_un.S_addr), theAppPtr->ipfilter->GetLastHit());
					newclient->Safe_Delete();
					theStats.filteredclients++;
					continue;
				}

				if (theAppPtr->clientlist->IsBannedClient(SockAddr.sin_addr.S_un.S_addr)){
					if (thePrefs.GetLogBannedClients()){
						CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(SockAddr.sin_addr.S_un.S_addr);
						//Xman Code Fix
						/*
...
void CListenSocket::Process()
{
	...
	if ((GetOpenSockets() + 5 < thePrefs.GetMaxConnections() || theAppPtr->serverconnect->IsConnecting()) && !bListening)
		ReStartListening();
}
*/
void CListenSocket::Process()
{
	...
	if((bListening == false) && 
		(GetOpenSockets()+5 < thePrefs.GetMaxConnections() || theAppPtr->serverconnect->IsConnecting() == true)){
			ReStartListening();
		}
}
// Maella end
