...
void CMMServer::ProcessStatusRequest(CMMSocket* sender, CMMPacket* packet){
	...
	theAppPtr->pBandWidthControl->GetFullHistoryDatarates(notUsed, notUsed,
	...
	packet->WriteByte((uint8)theAppPtr->downloadqueue->GetDownloadingFileCount());
	packet->WriteByte((uint8)theAppPtr->downloadqueue->GetPausedFileCount());
	packet->WriteInt((uint32)(theAppPtr->pBandWidthControl->GeteMuleIn()/1048576));
	...
	if (theAppPtr->serverconnect->IsConnected()){
		if(theAppPtr->serverconnect->IsLowID())
			...
			packet->WriteByte(2);
		if (theAppPtr->serverconnect->GetCurrentServer() != NULL){
			packet->WriteInt(theAppPtr->serverconnect->GetCurrentServer()->GetUsers());
			packet->WriteString(theAppPtr->serverconnect->GetCurrentServer()->GetListName());
		}
		else{
			packet->WriteInt(0);
			packet->WriteString("");
...
void CMMServer::ProcessFileListRequest(CMMSocket* sender, CMMPacket* packet){
	...
	nCount = (theAppPtr->downloadqueue->GetFileCount() > m_nMaxDownloads)? m_nMaxDownloads : theAppPtr->downloadqueue->GetFileCount();
	m_SentFileList.SetSize(nCount);
	packet->WriteByte((uint8)nCount);
	for (int i = 0; i != nCount; i++){
		// while this is not the fastest method the trace this list, it's not timecritical here
		CPartFile* cur_file = theAppPtr->downloadqueue->GetFileByIndex(i);
		if (cur_file == NULL){
			delete packet;
...
void CMMServer::ProcessFileCommand(CMMData* data, CMMSocket* sender){
	...
	if (byFileIndex >= m_SentFileList.GetSize()
		|| !theAppPtr->downloadqueue->IsPartFile(m_SentFileList[byFileIndex]))
	{
		CMMPacket* packet = new CMMPacket(MMP_GENERALERROR);
		sender->SendPacket(packet);
		ASSERT ( false );
		...
		case MMT_CANCEL:{
			switch(selFile->GetStatus()) { 
				...
				case PS_PAUSED:
					...
					break;
				default:
                    theAppPtr->downloadqueue->StartNextFileIfPrefs(selFile->GetCategory());
					selFile->DeleteFile(); 
			}
			break;
		}
...
void  CMMServer::ProcessSearchRequest(CMMData* data, CMMSocket* sender){
	...
	theAppPtr->searchlist->NewSearch(NULL, Params.strFileType, MMS_SEARCHID, Params.eType, Params.strExpression, true);
	...
	theAppPtr->serverconnect->SendPacket(searchpacket,true);
	char buffer[500];
...
#define MAXRESULTS	20
	...
	if (bTimeOut){
	...
	else if (theAppPtr->searchlist->GetFoundFiles(MMS_SEARCHID) == 0){
		CMMPacket* packet = new CMMPacket(MMP_SEARCHANS);
		...
		UINT results = theAppPtr->searchlist->GetFoundFiles(MMS_SEARCHID);
		...
		for (UINT i = 0; i < results; i++){
			CSearchFile* cur_file = theAppPtr->searchlist->DetachNextFile(MMS_SEARCHID);
		...
		m_pPendingCommandSocket->SendPacket(packet);
		theAppPtr->searchlist->RemoveResults(MMS_SEARCHID);
	}
	m_pPendingCommandSocket = NULL;
}
...
void  CMMServer::ProcessDownloadRequest(CMMData* data, CMMSocket* sender){
	...
	theAppPtr->downloadqueue->AddSearchToDownload(todownload, 2, 0);
	CMMPacket* packet = new CMMPacket(MMP_DOWNLOADANS);
	if (theAppPtr->downloadqueue->GetFileByID(todownload->GetFileHash()) != NULL){
		packet->WriteByte(MMT_OK);
	}
	else{
...
void  CMMServer::ProcessPreviewRequest(CMMData* data, CMMSocket* sender){
	...
	if (byFileType == MMT_PARTFILFE){
		...
		|| !theAppPtr->knownfiles->IsKnownFile(m_SentFinishedList[byFileIndex]))
		{
			bError = true;	
		}
		else
...
void CALLBACK CMMServer::CommandTimer(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
	...
	__if_exists(Core_Common::InterfaceExceptionDebugger)
	{
		__try
		{
			return CMMServer::CommandTimerImpl(0, 0, 0, 0);
		}
...
VOID CALLBACK CMMServer::CommandTimerImpl(HWND /*hwnd*/, UINT /*uMsg*/, UINT_PTR /*idEvent*/, DWORD /*dwTime*/)
{
#ifdef VERSION_DLL
	...
	try
	{
		KillTimer(0,theAppPtr->mmserver->h_timer);
		theAppPtr->mmserver->h_timer = 0;
		switch(theAppPtr->mmserver->m_byPendingCommand){
			case MMT_SDPC:{
				HANDLE hToken; 
			...
			case MMT_SDEMULE:
				...
				break;
			case MMT_SEARCH:
				theAppPtr->mmserver->SearchFinished(true);
				break;
			case MMT_PREVIEW:
				theAppPtr->mmserver->PreviewFinished(NULL,0);
				break;
		}
	}
...
void  CMMServer::ProcessStatisticsRequest(CMMData* data, CMMSocket* sender){
	...
	CArray<UpDown>* rawData = theAppPtr->webserver->GetPointsForWeb();
