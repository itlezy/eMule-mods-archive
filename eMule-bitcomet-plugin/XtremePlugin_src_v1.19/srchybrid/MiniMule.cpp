	...
	: CDHtmlDialog(CMiniMule::IDD, CMiniMule::IDH, pParent)
{
	...
	m_uAutoCloseTimer = 0;
	m_bAutoClose = theAppPtr->GetProfileInt(_T("eMule"), _T("MiniMuleAutoClose"), 0)!=0;
	m_uWndTransparency = theAppPtr->GetProfileInt(_T("eMule"), _T("MiniMuleTransparency"), 0);
...
BOOL CMiniMule::OnInitDialog()
{
	...
	CString strHtmlFile = theAppPtr->GetSkinFileItem(_T("MiniMule"), _T("HTML"));
	...
	*/
	SetWindowText(_T("eMule v") + theAppPtr->m_strCurVersionLong + _T(" [") + MOD_VERSION + _T("]"));
	//Xman end
...
void CMiniMule::OnClose()
{
	...
	//theAppPtr->emuledlg->PostMessage(UM_CLOSE_MINIMULE, (WPARAM)m_bRestoreMainWnd);
	...
	theAppPtr->emuledlg->m_pMiniMule = NULL;
	bool bRestoreMainWnd = m_bRestoreMainWnd;
	DestroyWindow();
	//NOTE: 'this' IS NO LONGER VALID!
	if (bRestoreMainWnd)
		theAppPtr->emuledlg->RestoreWindow();
}
...
void CMiniMule::UpdateContent(UINT uUpDatarate, UINT uDownDatarate)
{
	...
	if (m_bResolveImages)
	{
		...
		UINT uIconIdx = theAppPtr->emuledlg->GetConnectionStateIconIndex();
	...
	SetElementHtml(_T("connected"), CComBSTR(theAppPtr->IsConnected() ? GetResString(IDS_YES) : GetResString(IDS_NO)));
	SetElementHtml(_T("upRate"), CComBSTR(theAppPtr->emuledlg->GetUpDatarateString(uUpDatarate)));
	SetElementHtml(_T("downRate"), CComBSTR(theAppPtr->emuledlg->GetDownDatarateString(uDownDatarate)));
	UINT uCompleted = 0;
	if (thePrefs.GetRemoveFinishedDownloads())
		uCompleted = thePrefs.GetDownSessionCompletedFiles();
	else if (theAppPtr->emuledlg && theAppPtr->emuledlg->transferwnd && theAppPtr->emuledlg->transferwnd->downloadlistctrl.m_hWnd) {
		int iTotal;
		uCompleted = theAppPtr->emuledlg->transferwnd->downloadlistctrl.GetCompleteDownloads(-1, iTotal);	 // [Ded]: -1 to get the count of all completed files in all categories
	}
...
{
	ASSERT( GetCurrentThreadId() == g_uMainThreadId );
	if (theAppPtr->emuledlg->m_pMiniMule == NULL){
		// FIX ME
...
{
	if (theAppPtr->emuledlg->IsRunning() && !theAppPtr->emuledlg->IsWindowVisible())
	{
		if (!theAppPtr->emuledlg->IsPreferencesDlgOpen())
		{
			KillAutoCloseTimer();
...
HRESULT CMiniMule::OnOpenIncomingFolder(IHTMLElement* /*pElement*/)
{
	...
	{
		theAppPtr->emuledlg->SendMessage(WM_COMMAND, MP_HM_OPENINC);
		if (GetAutoClose())
			PostMessage(WM_CLOSE);
	}
	return S_OK;
...
HRESULT CMiniMule::OnOptions(IHTMLElement* /*pElement*/)
{
	...
	if (theAppPtr->emuledlg->IsRunning())
	{
		...
		KillAutoCloseTimer();
		if (theAppPtr->emuledlg->ShowPreferences() == -1)
			MessageBeep(MB_OK);
		if (GetAutoClose())
			CreateAutoCloseTimer();
	}
