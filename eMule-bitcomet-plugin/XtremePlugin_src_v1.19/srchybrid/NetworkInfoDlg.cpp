...
void CreateNetworkInfo(CRichEditCtrlX& rCtrl, CHARFORMAT& rcfDef, CHARFORMAT& rcfBold, bool bFullInfo)
{
	...
	//Xman links
	//MOD NOTE: if your Mod is based on Xtreme, don't remove this Link:
	rCtrl << _T("Info") << _T(":\t") << _T("http://www.emule-mods.de/?mods=xtreme") << _T("\r\n"); //zz_fly :: new homepage
	//rCtrl << _T("Info") << _T(":\t") << _T("http://xtreme-mod.net") << _T("\r\n");
	//MOD NOTE END
	rCtrl << _T("Support") << _T(":\t") << _T("http://support.emule-web.de") << _T("\r\n");
	rCtrl << _T("server.met") << _T(":\t") <<  _T("http://www.server-met.de") << _T("\r\n");
	rCtrl << _T("\r\n");
	...
	if (bFullInfo && theAppPtr->serverconnect->IsConnected())
	{
		uint32 uTotalUser = 0;
		uint32 uTotalFile = 0;

		theAppPtr->serverlist->GetUserFileStatus(uTotalUser, uTotalFile);
	...
	if (theAppPtr->serverconnect->IsConnected()){
		...
		if (theAppPtr->serverconnect->IsLowID() && theAppPtr->GetPublicIP(true) == 0)
			buffer = GetResString(IDS_UNKNOWN);
		else
			buffer.Format(_T("%s:%u"), ipstr(theAppPtr->GetPublicIP(true)), thePrefs.GetPort());
		rCtrl << _T("\t") << buffer << _T("\r\n");

		rCtrl << GetResString(IDS_ID) << _T(":\t");
		if (theAppPtr->serverconnect->IsConnected()){
			buffer.Format(_T("%u"),theAppPtr->serverconnect->GetClientID());
			rCtrl << buffer;
		}
		rCtrl << _T("\r\n");
		...
		CServer* cur_server = theAppPtr->serverconnect->GetCurrentServer();
		CServer* srv = cur_server ? theAppPtr->serverlist->GetServerByAddress(cur_server->GetAddress(), cur_server->GetPort()) : NULL;
		if (srv){
			rCtrl << _T("\r\n");
			rCtrl.SetSelectionCharFormat(rcfBold);
			...
			if (theAppPtr->serverconnect->IsConnectedObfuscated())
	...
	if(Kademlia::CKademlia::IsConnected()){
		...
		*/
		...
		if (Kademlia::CUDPFirewallTester::IsFirewalledUDP(true)) {
			...
			switch ( theAppPtr->clientlist->GetBuddyStatus() )
			{
				case Disconnected:
	...
	if (thePrefs.GetWSIsEnabled()){
		CString count;
		count.Format(_T("%i %s"), theAppPtr->webserver->GetSessionCount(), GetResString(IDS_ACTSESSIONS));
		rCtrl << _T("\t") << count << _T("\r\n");
		CString strHostname;
		if (!thePrefs.GetYourHostname().IsEmpty() && thePrefs.GetYourHostname().Find(_T('.')) != -1)
			strHostname = thePrefs.GetYourHostname();
		else
			strHostname = ipstr(theAppPtr->serverconnect->GetLocalIP());
		rCtrl << _T("URL:\t") << _T("http://") << strHostname << _T(":") << thePrefs.GetWSPort() << _T("/\r\n");
	}

	//zz_fly :: show UPnP status :: start
	//if(thePrefs.GetUPnPNat())
	if(thePrefs.IsUPnPEnabled() && theAppPtr->m_pUPnPFinder)
	{
		rCtrl << _T("\r\n");
		rCtrl.SetSelectionCharFormat(rcfBold);
		rCtrl << GetResString(IDS_UPNPSTATUS) << _T("\r\n");
		rCtrl.SetSelectionCharFormat(rcfDef);
		//CString upnpinfo = theAppPtr->m_UPnPNat.GetLastError(); //ACAT UPnP
		CString upnpinfo = theAppPtr->m_pUPnPFinder->GetImplementation() ? theAppPtr->m_pUPnPFinder->GetImplementation()->GetStatusString() : _T(""); //Official UPNP
