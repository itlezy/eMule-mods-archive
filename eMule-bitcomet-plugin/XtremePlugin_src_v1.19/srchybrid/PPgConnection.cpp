...
BEGIN_MESSAGE_MAP(CPPgConnection, CPropertyPage)
	...
	*/
	...
	ON_BN_CLICKED(IDC_PREF_UPNPONSTART, OnSettingsChange) //Official UPNP
END_MESSAGE_MAP()
...
void CPPgConnection::OnEnChangePorts(uint8 istcpport)
{
	...
	GetDlgItem(IDC_STARTTEST)->EnableWindow( 
		tcp == theAppPtr->listensocket->GetConnectedPort() && 
		udp == theAppPtr->clientudp->GetConnectedPort() 
	);
...
void CPPgConnection::LoadSettings(void)
{
	if (m_hWnd)
	{
		...
		*/
		...
		if (thePrefs.GetWindowsVersion() == _WINVER_XP_ && IsRunningXPSP2() == 0 && theAppPtr->m_pFirewallOpener->DoesFWConnectionExist())
		{//zz_fly
...
BOOL CPPgConnection::OnApply()
{
...

    {
        ...
        if(ulSpeed > thePrefs.GetMaxUpload()) {
            ...
            theAppPtr->lastCommonRouteFinder->InitiateFastReactionPeriod();
        }
	...
	*/
	...
	if(caphaschanged==true)
		theAppPtr->emuledlg->Localize(); //dirty hack which updated the sysmenu
	//Xman end
	...
	if (GetDlgItem(IDC_PORT)->GetWindowTextLength())
	{
		...
		if (nNewPort != thePrefs.port){
			...
			if (theAppPtr->IsPortchangeAllowed())
	...
	if (lastmaxgd!=thePrefs.maxGraphDownloadRate)
		theAppPtr->emuledlg->statisticswnd->SetARange(true, thePrefs.maxGraphDownloadRate);
	*/
	if(lastMaxGraphUploadRate != thePrefs.GetMaxGraphUploadRate()) 
		theAppPtr->emuledlg->statisticswnd->SetARange(false, (int)thePrefs.GetMaxGraphUploadRate());
	if(lastMaxGraphDownloadRate != thePrefs.GetMaxGraphDownloadRate())
		theAppPtr->emuledlg->statisticswnd->SetARange(true, (int)thePrefs.GetMaxGraphDownloadRate());
	// Maella end
...
{	
	if (m_hWnd)
	{
		...
		*/
		...
		GetDlgItem(IDC_PREF_UPNPONSTART)->SetWindowText(GetResString(IDS_UPNPSTART)); //Official UPNP
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Connection);
}
...
{
	OnApply();
	theAppPtr->m_pFirewallOpener->RemoveRule(EMULE_DEFAULTRULENAME_UDP);
	theAppPtr->m_pFirewallOpener->RemoveRule(EMULE_DEFAULTRULENAME_TCP);
	bool bAlreadyExisted = false;
	if (theAppPtr->m_pFirewallOpener->DoesRuleExist(thePrefs.GetPort(), NAT_PROTOCOL_TCP) || theAppPtr->m_pFirewallOpener->DoesRuleExist(thePrefs.GetUDPPort(), NAT_PROTOCOL_UDP)){
		bAlreadyExisted = true;
	}
	bool bResult = theAppPtr->m_pFirewallOpener->OpenPort(thePrefs.GetPort(), NAT_PROTOCOL_TCP, EMULE_DEFAULTRULENAME_TCP, false);
	if (thePrefs.GetUDPPort() != 0)
		bResult = bResult && theAppPtr->m_pFirewallOpener->OpenPort(thePrefs.GetUDPPort(), NAT_PROTOCOL_UDP, EMULE_DEFAULTRULENAME_UDP, false);
	if (bResult){
		if (!bAlreadyExisted)
