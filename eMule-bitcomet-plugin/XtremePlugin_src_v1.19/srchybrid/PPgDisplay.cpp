...
BOOL CPPgDisplay::OnApply()
{
	...
	if (thePrefs.m_bDisableKnownClientList != (IsDlgButtonChecked(IDC_DISABLEKNOWNLIST) != 0)) {
		...
		if (thePrefs.m_bDisableKnownClientList)
			...
			theAppPtr->emuledlg->transferwnd->clientlistctrl.ShowKnownClients();
		bResetToolbar = true;
	}
	...
	if (thePrefs.m_bDisableQueueList != (IsDlgButtonChecked(IDC_DISABLEQUEUELIST) != 0)) {
		...
		if (thePrefs.m_bDisableQueueList)
			...
			theAppPtr->emuledlg->transferwnd->queuelistctrl.ShowQueueClients();
		bResetToolbar = true;
	}
	...
	theAppPtr->emuledlg->SetToolTipsDelay(thePrefs.GetToolTipDelay()*1000);

	theAppPtr->emuledlg->transferwnd->downloadlistctrl.SetStyle();

	if (bListDisabled)
		theAppPtr->emuledlg->transferwnd->OnDisableList();
	if ((IsDlgButtonChecked(IDC_SHOWTRANSTOOLBAR) != 0) != thePrefs.IsTransToolbarEnabled()) {
		...
		theAppPtr->emuledlg->transferwnd->ResetTransToolbar(thePrefs.m_bWinaTransToolbar);
	}
	...
	if (mintotray_old != thePrefs.mintotray)
		theAppPtr->emuledlg->TrayMinimizeToTrayChange();
	if (!thePrefs.ShowRatesOnTitle())
		// Xman // Maella -Support for tag ET_MOD_VERSION 0x55
		/*
		theAppPtr->emuledlg->SetWindowText(_T("eMule v") + theAppPtr->m_strCurVersionLong);
		*/
		theAppPtr->emuledlg->SetWindowText(_T("eMule v") + theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION); 
		//Xman End
		...
		if (LOWORD(wParam) == psh3/*Apply*/ && HIWORD(wParam) == BN_CLICKED)
		{
			...
			if (pDlg != NULL)
			{
				...
				if (s_pThis->m_eSelectFont == sfLog)
					theAppPtr->emuledlg->ApplyLogFont(&lf);
				else
					theAppPtr->emuledlg->ApplyHyperTextFont(&lf);
			}
		}
		break;
	}
...
void CPPgDisplay::OnBnClickedSelectHypertextFont()
{
	...
	if (m_eSelectFont == sfLog)
		pFont = &theAppPtr->m_fontLog;
	else
		pFont = &theAppPtr->m_fontHyperText;
	LOGFONT lf;
	if (pFont != NULL)
...
{
	theAppPtr->emuledlg->searchwnd->ResetHistory();
	theAppPtr->emuledlg->serverwnd->ResetHistory();
}

void CPPgDisplay::OnHelp()
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Display);
}
