...
void CPPgFiles::LoadSettings(void)
{
	...
	if(thePrefs.IsRememberingDownloadedFiles()==false)
	{
		...
		*/
		CheckDlgButton(IDC_REMEMBERAICH, !thePrefs.DoPartiallyPurgeOldKnownFiles());
		//zz_fly :: end
	//Xman end
...
BOOL CPPgFiles::OnApply()
{
	...
	*/
	thePrefs.m_bPartiallyPurgeOldKnownFiles = (IsDlgButtonChecked(IDC_REMEMBERAICH)==0);
	//zz_fly :: end
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Files);
}
