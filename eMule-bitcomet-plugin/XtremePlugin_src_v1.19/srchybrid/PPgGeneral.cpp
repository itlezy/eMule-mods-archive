...
BOOL CPPgGeneral::OnApply()
{
	...
	*/
	if (strNick.IsEmpty())
	{
		strNick = MOD_HPLINK; //DEFAULT_NICK;
		GetDlgItem(IDC_NICK)->SetWindowText(strNick);
	}
	//Xman DLP check the nick
	CString strNicktocheck=strNick + _T(" [xxx]");
	if(theAppPtr->dlp->IsDLPavailable() && (theAppPtr->dlp->DLPCheckUsername_Hard(strNicktocheck) || theAppPtr->dlp->DLPCheckUsername_Soft(strNicktocheck)))
	{
	...
	if (m_language.GetCurSel() != CB_ERR)
	{
		...
		if (thePrefs.GetLanguageID() != wNewLang)
		{
			...
			{
				ModifyAllWindowStyles(theAppPtr->emuledlg, WS_EX_LAYOUTRTL | WS_EX_RTLREADING | WS_EX_RIGHT | WS_EX_LEFTSCROLLBAR, 0);
				ModifyAllWindowStyles(theAppPtr->emuledlg->preferenceswnd, WS_EX_LAYOUTRTL | WS_EX_RTLREADING | WS_EX_RIGHT | WS_EX_LEFTSCROLLBAR, 0);
				theAppPtr->DisableRTLWindowsLayout();
				thePrefs.m_bRTLWindowsLayout = false;
			}
#endif
			theAppPtr->emuledlg->preferenceswnd->Localize();
			theAppPtr->emuledlg->statisticswnd->CreateMyTree();
			theAppPtr->emuledlg->statisticswnd->Localize();
			theAppPtr->emuledlg->statisticswnd->ShowStatistics(true);
			theAppPtr->emuledlg->serverwnd->Localize();
			theAppPtr->emuledlg->transferwnd->Localize();
			theAppPtr->emuledlg->transferwnd->UpdateCatTabTitles();
			theAppPtr->emuledlg->searchwnd->Localize();
			theAppPtr->emuledlg->sharedfileswnd->Localize();
			theAppPtr->emuledlg->chatwnd->Localize();
			theAppPtr->emuledlg->Localize();
			theAppPtr->emuledlg->ircwnd->Localize();
			theAppPtr->emuledlg->kademliawnd->Localize();
		}
	}
	...
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.SetStyle();
	LoadSettings();
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_General);
}
