...
BOOL CPPgSecurity::OnApply()
{
	...
	if (bIPFilterSettingsChanged)
		theAppPtr->emuledlg->serverwnd->serverlistctrl.RemoveAllFilteredServers();
...
void CPPgSecurity::OnReloadIPFilter()
{
	...
	theAppPtr->ipfilter->LoadFromDefaultFile();
...
void CPPgSecurity::OnLoadIPFFromURL()
{
	...
	if (!url.IsEmpty())
	{
		...
		if (zip.Open(strTempFilePath))
		{
			...
			if (zfile)
			{
				...
				if (zfile->Extract(strTempUnzipFilePath))
				{
					...
					if (_tremove(theAppPtr->ipfilter->GetDefaultFilePath()) != 0)
						TRACE(_T("*** Error: Failed to remove default IP filter file \"%s\" - %s\n"), theAppPtr->ipfilter->GetDefaultFilePath(), _tcserror(errno));
					if (_trename(strTempUnzipFilePath, theAppPtr->ipfilter->GetDefaultFilePath()) != 0)
						TRACE(_T("*** Error: Failed to rename uncompressed IP filter file \"%s\" to default IP filter file \"%s\" - %s\n"), strTempUnzipFilePath, theAppPtr->ipfilter->GetDefaultFilePath(), _tcserror(errno));
		...
		if (!bIsArchiveFile && !bUncompressed)
		{
			...
			{
				(void)_tremove(theAppPtr->ipfilter->GetDefaultFilePath());
				VERIFY( _trename(strTempFilePath, theAppPtr->ipfilter->GetDefaultFilePath()) == 0 );
				bHaveNewFilterFile = true;
			}
			else
			{
	...
	if (bHaveNewFilterFile && theAppPtr->ipfilter->GetIPFilter().GetCount() == 0)
	{
		CString strLoaded;
		strLoaded.Format(GetResString(IDS_IPFILTERLOADED), theAppPtr->ipfilter->GetIPFilter().GetCount());
		CString strError;
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Security);
}
