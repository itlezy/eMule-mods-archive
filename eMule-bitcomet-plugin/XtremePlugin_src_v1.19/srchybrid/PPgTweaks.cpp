...
CPPgTweaks::CPPgTweaks()
	...
	, m_ctrlTreeOptions(theAppPtr->m_iDfltImageListColorFlags)
{
	m_iFileBufferSize = 0;
	m_iQueueSize = 0;
	m_iMaxConnPerFive = 0;
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		*/
		...
		// ==> push small files [sivka] - Stulle
		m_htiEnablePushSmallFile = m_ctrlTreeOptions.InsertCheckBox(GetResString(IDS_PUSH_SMALL), TVI_ROOT, m_bEnablePushSmallFile); 
		m_htiPushSmallFileSize = m_ctrlTreeOptions.InsertItem(GetResString(IDS_PUSH_SMALL_FILE_SIZE), TREEOPTSCTRLIMG_EDIT, TREEOPTSCTRLIMG_EDIT, m_htiEnablePushSmallFile);
		m_ctrlTreeOptions.AddEditBox(m_htiPushSmallFileSize, RUNTIME_CLASS(CNumTreeOptionsEdit));
		m_ctrlTreeOptions.Expand(m_htiEnablePushSmallFile, TVE_EXPAND);
		// <== push small files [sivka] - Stulle
		//zz_fly end
		...
		*/
		...
		m_ctrlTreeOptions.Expand(m_htiUPnP, TVE_EXPAND); //Official UPNP
	...
	*/
	...
	// ==> push small files [sivka] - Stulle
	DDX_TreeCheck(pDX, IDC_EXT_OPTS, m_htiEnablePushSmallFile, m_bEnablePushSmallFile);
	DDX_TreeEdit(pDX, IDC_EXT_OPTS, m_htiPushSmallFileSize, m_iPushSmallFileSize);
	DDV_MinMaxInt(pDX, m_iPushSmallFileSize, 1, 2048);
	// <== push small files [sivka] - Stulle
	//zz_fly end
}
...
BOOL CPPgTweaks::OnInitDialog()
{
	...
	*/
	...
	m_iPushSmallFileSize = thePrefs.GetPushSmallFileSize() / 1024;//bytes to kb
	// <== push small files [sivka] - Stulle
	//zz_fly end

	m_ctrlTreeOptions.SetImageListColorFlags(theAppPtr->m_iDfltImageListColorFlags);
...
BOOL CPPgTweaks::OnApply()
{
	...
	theAppPtr->scheduler->original_cons5s = thePrefs.GetMaxConperFive();
	...
	if (thePrefs.m_bExtControls != m_bExtControls) {
		...
		theAppPtr->emuledlg->transferwnd->downloadlistctrl.CreateMenues();
		theAppPtr->emuledlg->searchwnd->CreateMenus();
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.CreateMenues();
	}
	...
	*/
	...
	thePrefs.SetPushSmallFileSize(m_iPushSmallFileSize * 1024); //kb to bytes
	// <== push small files [sivka] - Stulle
	//zz_fly end

	if (thePrefs.GetEnableVerboseOptions())
	{
	    theAppPtr->emuledlg->serverwnd->ToggleDebugWindow();
		theAppPtr->emuledlg->serverwnd->UpdateLogTabSelection();
	}
...
{	
	if(m_hWnd)
	{
		...
		*/
		...
		if (m_htiPushSmallFileSize) m_ctrlTreeOptions.SetEditLabel(m_htiPushSmallFileSize, GetResString(IDS_PUSH_SMALL_FILE_SIZE));
		// <== push small files [sivka] - Stulle
		//zz_fly end
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Extended_Settings);
}
