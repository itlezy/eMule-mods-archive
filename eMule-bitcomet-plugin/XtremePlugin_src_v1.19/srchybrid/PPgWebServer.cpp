...
{	
	if(m_bModified)
	{
		...
		theAppPtr->webserver->ReloadTemplates();
		...
		if (_tstoi(sBuf)!=oldPort) {
			...
			theAppPtr->webserver->RestartServer();
		}
		...
		if (IsDlgButtonChecked(IDC_WSUPNP))
		{
			...
			if (!thePrefs.m_bWebUseUPnP && thePrefs.GetWSIsEnabled() && theAppPtr->m_pUPnPFinder != NULL) // add the port to existing mapping without having eMule restarting (if all conditions are met)
				theAppPtr->m_pUPnPFinder->GetImplementation()->LateEnableWebServerPort(thePrefs.GetWSPort());
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_WebInterface);
}
