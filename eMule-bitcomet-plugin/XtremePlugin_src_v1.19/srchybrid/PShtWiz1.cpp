...
public:
	...
	afx_msg void OnStartUPnP(); //Official UPNP
...
protected:
	CString lastudp;
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	void			ResetUPnPProgress(); //Official UPNP

	DECLARE_MESSAGE_MAP()

	int m_nUPnPTicks; //Official UPNP
};
...
BEGIN_MESSAGE_MAP(CPPgWiz1Ports, CDlgPageWizard)
	...
	ON_BN_CLICKED(IDC_UPNPSTART, OnStartUPnP) //Official UPNP
	ON_EN_CHANGE(IDC_TCP, OnEnChangeTCP)
	ON_EN_CHANGE(IDC_UDP, OnEnChangeUDP)
	ON_WM_TIMER() //Official UPNP
END_MESSAGE_MAP()
...
void CPPgWiz1Ports::OnPortChange() {
	...
	bool flag= (theAppPtr->IsPortchangeAllowed() && 
		( 
		(theAppPtr->listensocket->GetConnectedPort()!=GetTCPPort()  || theAppPtr->listensocket->GetConnectedPort()==0)
		||
		(theAppPtr->clientudp->GetConnectedPort()!=GetUDPPort() || theAppPtr->clientudp->GetConnectedPort()==0 )    
		)	
	);
...
void CPPgWiz1Ports::OnStartUPnP() {
	...
	theAppPtr->emuledlg->StartUPnP(true, GetTCPPort(), GetUDPPort());
...
void CPPgWiz1Ports::OnTimer(UINT /*nIDEvent*/){
	m_nUPnPTicks++;
	if (theAppPtr->m_pUPnPFinder && theAppPtr->m_pUPnPFinder->GetImplementation()->ArePortsForwarded() == TRIS_UNKNOWN)
	{
		if (m_nUPnPTicks < 40){
	...
	if (theAppPtr->m_pUPnPFinder && theAppPtr->m_pUPnPFinder->GetImplementation()->ArePortsForwarded() == TRIS_TRUE){
...
void CPPgWiz1Ports::OnStartConTest() {
	...
	if ( (tcp!=theAppPtr->listensocket->GetConnectedPort() || udp!=theAppPtr->clientudp->GetConnectedPort() ) ) {
...
BOOL CPPgWiz1Ports::OnInitDialog()
{
	...
	((CProgressCtrl*)GetDlgItem(IDC_UPNPPROGRESS))->SetRange(0, 40); //Official UPNP
	InitWindowStyles(this);
...
BOOL FirstTimeWizard()
{
	...
	if ( (thePrefs.port!=theAppPtr->listensocket->GetConnectedPort()) || (thePrefs.udpport!=theAppPtr->clientudp->GetConnectedPort()) )
		if (!theAppPtr->IsPortchangeAllowed())
			AfxMessageBox(GetResString(IDS_NOPORTCHANGEPOSSIBLE));
		else {
			theAppPtr->listensocket->Rebind() ;
