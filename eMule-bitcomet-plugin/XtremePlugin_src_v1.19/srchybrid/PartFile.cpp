...
void CPartFile::InitializeFromLink(CED2KFileLink* fileLink, UINT cat)
{
	...
	try{
		...
		if (!theAppPtr->downloadqueue->IsFileExisting(m_abyFileHash))
		{
...
CPartFile::~CPartFile()
{
	...
	theAppPtr->clientlist->CleanUp(this);
	// Maella end
...
void CPartFile::CreatePartFile(UINT cat)
{
	...
	CString tempdirtouse=theAppPtr->downloadqueue->GetOptimalTempDir(cat,GetFileSize());
	...
	*/
	...
	SavePartFile();
	SetActive(theAppPtr->IsConnected());
}
...
bool CPartFile::SavePartFile(bool bDontOverrideBak)
{
	...
	try{
		...
		*/
		...
		if (thePrefs.GetCommitFiles() >= 2 || (thePrefs.GetCommitFiles() >= 1 && !theAppPtr->emuledlg->IsRunning())){
	...
	*/
	...
	//Xman end
		...
		if (IsComplete(0, m_nFileSize - (uint64)1, false)){
			if (md4cmp(result->GetFileHash(), GetFileHash())){
				...
				*/
				if (GetED2KPartCount() != GetHashCount()){
	...
	if (!errorfound){
		if (status == PS_COMPLETING){
			...
			if (theAppPtr->sharedfiles->GetFileByID(GetFileHash()) == NULL)
				theAppPtr->sharedfiles->SafeAddKFile(this);
			CompleteFile(true);
			return;
		}
		else
...
void CPartFile::DrawStatusBar(CDC* dc, LPCRECT rect, bool bFlat) 
{
	...
	*/
	...
	{
		chunkBar.Fill(crHave); //Xman Maella -Code Improvement-
		int iOldBkColor = dc->SetBkColor(RGB(255, 255, 0));
		if (theAppPtr->m_brushBackwardDiagonal.m_hObject)
			dc->FillRect(rect, &theAppPtr->m_brushBackwardDiagonal);
		else
...
void CPartFile::AddDownloadingSource(CUpDownClient* client){
	...
	if(pos == NULL){
		...
		theAppPtr->emuledlg->transferwnd->downloadclientsctrl.AddClient(client);
		//Xman
	...
	//zz_fly :: delayed deletion of downloading source :: Enig123 :: Start
		...
		theAppPtr->emuledlg->transferwnd->downloadclientsctrl.AddClient(client); //only need to add it to downloadclientsctrl
	//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
}
...
void CPartFile::RemoveDownloadingSource(CUpDownClient* client){
	...
	if(pos != NULL){
		...
		*/
		...
		//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
		theAppPtr->emuledlg->transferwnd->downloadclientsctrl.RemoveClient(client);
		//Xman
		// Maella -New bandwidth control-
		// We need to detect a change in the source list
		// It's necessary for the download 'balancing' (see ::Process)
...
//Xman end
{
			...
			if(isLimited == false){
				...
				if(maxammount > 6){ // let room for header size
					...
					*/
					...
					receivedBlock = theAppPtr->pBandWidthControl->GeteMuleIn();
					...
					*/
					...
					receivedBlock = theAppPtr->pBandWidthControl->GeteMuleIn() - receivedBlock;
	...
	*/
	...
	if(fullProcess)
	{
		...
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;)
		{
			...
			switch (cur_src->GetDownloadState())
			{
				...
				case DS_ONQUEUE:
				{
					...
					if (theAppPtr->IsConnected() && cur_src->GetTimeUntilReask() < MIN2MS(2) && cur_src->GetTimeUntilReask() > SEC2MS(1) && ::GetTickCount()-cur_src->getLastTriedToConnectTime() > 20*60*1000) // ZZ:DownloadManager (one resk timestamp for each file)
					...
					*/
					...
					if(cur_src->m_faileddownloads>=3 && thePrefs.GetAntiLeecher())
					{
						...
						theAppPtr->ipfilter->AddIPTemporary(ntohl(cur_src->GetConnectIP()));
				...
				case DS_WAITCALLBACKKAD:
				{
					...
					// Maella -Unnecessary Protocol Overload-
					/*
					if (theAppPtr->IsConnected() && cur_src->GetTimeUntilReask() == 0 && ::GetTickCount()-cur_src->getLastTriedToConnectTime() > 20*60*1000) // ZZ:DownloadManager (one resk timestamp for each file)
					{
		...
		*/
		if( GetMaxSourcePerFileUDP() > GetSourceCount() && IsSourceSearchAllowed())
		{
		//Xman end
			if (theAppPtr->downloadqueue->DoKademliaFileRequest() && (Kademlia::CKademlia::GetTotalFile() < KADEMLIATOTALFILE) && (dwCurTick > m_LastSearchTimeKad) &&  Kademlia::CKademlia::IsConnected() && theAppPtr->IsConnected() && !stopped){ //Once we can handle lowID users in Kad, we remove the second IsConnected
				//Kademlia
				theAppPtr->downloadqueue->SetLastKademliaFileRequest();
		...
		if ( !m_bLocalSrcReqQueued && ((!m_LastSearchTime) || (dwCurTick - m_LastSearchTime) > SERVERREASKTIME) && theAppPtr->serverconnect->IsConnected()
			//Xman GlobalMaxHarlimit for fairness
			/*
			&& GetMaxSourcePerFileSoft() > GetSourceCount() && !stopped
			*/
			&& GetMaxSourcePerFileSoft() > GetSourceCount() && !stopped && IsSourceSearchAllowed()
			//Xman end
			&& (!IsLargeFile() || (theAppPtr->serverconnect->GetCurrentServer() != NULL && theAppPtr->serverconnect->GetCurrentServer()->SupportsLargeFilesTCP())))
		{
			m_bLocalSrcReqQueued = true;
			theAppPtr->downloadqueue->SendLocalSrcRequest(this);
		}
		...
		*/
		...
		if ( GetSrcStatisticsValue(DS_DOWNLOADING) != nOldTransSourceCount ){
			if (theAppPtr->emuledlg->transferwnd->downloadlistctrl.curTab == 0)
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.ChangeCategory(0); 
			//Xman
			/*
			else
...
bool CPartFile::CanAddSource(uint32 userid, uint16 port, uint32 serverip, uint16 serverport, UINT* pdebug_lowiddropped, bool Ed2kID)
{
	...
	if (theAppPtr->serverconnect->IsConnected())
	{
		...
		{
			if(theAppPtr->serverconnect->GetClientID() == userid && theAppPtr->serverconnect->GetCurrentServer()->GetIP() == serverip && theAppPtr->serverconnect->GetCurrentServer()->GetPort() == serverport )
				return false;
			if(theAppPtr->serverconnect->GetLocalIP() == userid)
				return false;
		}
		else
		{
			if(theAppPtr->serverconnect->GetClientID() == userid && thePrefs.GetPort() == port)
				return false;
		}
	}
	...
	if ( IsLowID(hybridID) && theAppPtr->IsFirewalled())
	{
	...
	for (UINT i = 0; i < count; i++)
	{
		...
		*/
		...
		if (!IsLowID(userid))
		{
			...
			if (theAppPtr->ipfilter->IsFiltered(userid))
			{
				if (thePrefs.GetLogFilteredIPs())
					AddDebugLogLine(false, _T("Ignored source (IP=%s) received from server - IP filter (%s)"), ipstr(userid), theAppPtr->ipfilter->GetLastHit());
				continue;
			}
			if (theAppPtr->clientlist->IsBannedClient(userid)){
#ifdef _DEBUG
				if (thePrefs.GetLogBannedClients()){
					CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(userid);
					//Xman Code Fix
					/*
					AddDebugLogLine(false, _T("Ignored source (IP=%s) received from server - banned client %s"), ipstr(userid), pClient->DbgGetClientInfo());
					*/
					AddDebugLogLine(false, _T("Ignored source (IP=%s) received from server - banned client %s"), ipstr(userid), pClient ? pClient->DbgGetClientInfo() : _T("unknown"));
					//Xman end
				}
#endif
				continue;
		...
		if(IsLowID(userid) )
		{
			...
			else if(!theAppPtr->serverconnect->IsLocalServer(serverip,serverport))
			{
		...
		*/
		if( GetMaxSources() > this->GetSourceCount() && IsGlobalSourceAddAllowed())
		//Xman end
		{
			...
			theAppPtr->downloadqueue->CheckAndAddSource(this,newsource);
		}
		else
		{
			// since we may received multiple search source UDP results we have to "consume" all data of that packet
...
void CPartFile::AddSource(LPCTSTR pszURL, uint32 nIP)
{
	...
	if (theAppPtr->ipfilter->IsFiltered(nIP))
	{
		if (thePrefs.GetLogFilteredIPs())
			AddDebugLogLine(false, _T("Ignored URL source (IP=%s) \"%s\" - IP filter (%s)"), ipstr(nIP), pszURL, theAppPtr->ipfilter->GetLastHit());
		return;
	}
	...
	if (theAppPtr->downloadqueue->CheckAndAddSource(this, client))
		UpdatePartsInfo();
}
		...
		{		
			for (UINT i = 0; i < partcount; i++)
			{
				if (cur_src->IsPartAvailable(i))
					...
					*/
					m_SrcpartFrequency[i] = m_SrcpartFrequency[i] + (uint16)weight;
					//Xman end
			}
			if ( flag )
			{
...
//				m_nCompleteSourcesCount;
			...
			//  Adjust network accounts for 80%, we account for 20% with what we see and make sure we are still above the normal.
			{
				...
				if( m_nCompleteSourcesCount < m_nCompleteSourcesCountLo )
					m_nCompleteSourcesCount = m_nCompleteSourcesCountLo;
				//Xman Code Fix: must use m_nCompleteSourcesCountLo instead of m_nCompleteSourcesCount
				/*
				m_nCompleteSourcesCountHi= (uint16)((float)(count.GetAt(k)*.8)+(float)(m_nCompleteSourcesCount*.2));
				*/
				m_nCompleteSourcesCountHi= (uint16)((float)(count.GetAt(k)*.8)+(float)(m_nCompleteSourcesCountLo*.2));
				//Xman end
				if( m_nCompleteSourcesCountHi < m_nCompleteSourcesCount )
					m_nCompleteSourcesCountHi = m_nCompleteSourcesCount;
			}
...
{
	theAppPtr->downloadqueue->RemoveLocalServerRequest(this);
	if(GetKadFileSearchID())
		Kademlia::CSearchManager::StopSearch(GetKadFileSearchID(), false);

	if (srcarevisible)
		theAppPtr->emuledlg->transferwnd->downloadlistctrl.HideSources(this);
	...
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.ShowFilesCount();
	if (thePrefs.ShowCatTabInfos())
		theAppPtr->emuledlg->transferwnd->UpdateCatTabTitles();
	UpdateDisplayedInfo(true);
}
...
void UncompressFile(LPCTSTR pszFilePath, CPartFile* pPartFile)
{
	...
	if (hFile == INVALID_HANDLE_VALUE){
		if (thePrefs.GetVerbose())
			theAppPtr->QueueDebugLogLine(true, _T("Failed to open file \"%s\" for decompressing - %s"), pszFilePath, GetErrorMessage(GetLastError(), 1));
		return;
	}
	...
	if (!DeviceIoControl(hFile, FSCTL_SET_COMPRESSION, &usInData, sizeof usInData, NULL, 0, &dwReturned, NULL)){
		if (thePrefs.GetVerbose())
			theAppPtr->QueueDebugLogLine(true, _T("Failed to decompress file \"%s\" - %s"), pszFilePath, GetErrorMessage(GetLastError(), 1));
	}
	CloseHandle(hFile);
}
								   ...
								   LPVOID lpData)
{
	...
	if (TotalFileSize.QuadPart && pPartFile && pPartFile->IsKindOf(RUNTIME_CLASS(CPartFile)))
	{
		...
		if (uProgress != pPartFile->GetFileOpProgress())
		{
			...
			VERIFY( PostMessage(theAppPtr->emuledlg->GetSafeHwnd(), TM_FILEOPPROGRESS, uProgress, (LPARAM)pPartFile) );
		}
	}
	else
		ASSERT(0);
...
BOOL CPartFile::PerformFileComplete() 
{
	...
	try{
		...
		TCHAR buffer[MAX_CFEXP_ERRORMSG];
		error->GetErrorMessage(buffer, ARRSIZE(buffer));
		theAppPtr->QueueLogLine(true, GetResString(IDS_ERR_FILEERROR), m_partmetfilename, GetFileName(), buffer);
		error->Delete();
		//return false;
	}
	...
	{
		theAppPtr->QueueLogLine(true,GetResString(IDS_ERR_COMPLETIONFAILED) + _T(" - \"%s\": ") + GetErrorMessage(dwMoveResult), GetFileName(), strNewname);
		...
		if (strNewname.GetLength() >= MAX_PATH)
			theAppPtr->QueueLogLine(true,GetResString(IDS_ERR_COMPLETIONFAILED) + _T(" - \"%s\": Path too long"),GetFileName(), strNewname);
		...
		if (theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
			VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd, TM_FILECOMPLETED, FILE_COMPLETION_THREAD_FAILED, (LPARAM)this) );
		return FALSE;
	}
	...
	if (_tremove(m_fullname))
		theAppPtr->QueueLogLine(true, GetResString(IDS_ERR_DELETEFAILED) + _T(" - ") + CString(_tcserror(errno)), m_fullname);

	// remove backup files
	CString BAKName(m_fullname);
	BAKName.Append(PARTMET_BAK_EXT);
	if (_taccess(BAKName, 0) == 0 && !::DeleteFile(BAKName))
		theAppPtr->QueueLogLine(true,GetResString(IDS_ERR_DELETE) + _T(" - ") + GetErrorMessage(GetLastError()), BAKName);
	...
	if (theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd, TM_FILECOMPLETED, FILE_COMPLETION_THREAD_SUCCESS | (renamed ? FILE_COMPLETION_THREAD_RENAMED : 0), (LPARAM)this) );
	return TRUE;
}
...
void CPartFile::PerformFileCompleteEnd(DWORD dwResult)
{
	if (dwResult & FILE_COMPLETION_THREAD_SUCCESS)
	{
		...
		if(theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
			theAppPtr->emuledlg->sharedfileswnd->historylistctrl.AddFile(this);
		//Xman end

		//Xman sourcecache
		ClearSourceCache();
		//Xman end
		theAppPtr->knownfiles->SafeAddKFile(this);
		theAppPtr->downloadqueue->RemoveFile(this);
		theAppPtr->mmserver->AddFinishedFile(this);
		if (thePrefs.GetRemoveFinishedDownloads())
			theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveFile(this);
		else
			UpdateDisplayedInfo(true);
		...
		theAppPtr->sharedfiles->RepublishFile(this);
		...
		theAppPtr->emuledlg->ShowNotifier(GetResString(IDS_TBN_DOWNLOADDONE) + _T('\n') + GetFileName(), TBN_DOWNLOADFINISHED, GetFilePath());
		...
		*/
		if(!m_pCollection && HasCollectionExtenesion_Xtreme())
		//Xman end
		{
	...
	theAppPtr->downloadqueue->StartNextFileIfPrefs(GetCategory());
}
	...
	for( pos1 = srclist.GetHeadPosition(); ( pos2 = pos1 ) != NULL; ){
		...
		if (bTryToSwap){
			if (!srclist.GetAt(pos2)->SwapToAnotherFile(_T("Removing source. CPartFile::RemoveAllSources()"), true, true, true, NULL, false, false) ) // ZZ:DownloadManager
				theAppPtr->downloadqueue->RemoveSource(srclist.GetAt(pos2), false);
		}
		else
			theAppPtr->downloadqueue->RemoveSource(srclist.GetAt(pos2), false);
	}
	...
	for(POSITION pos = srclist.GetHeadPosition(); pos != NULL; ){
		...
		if (bTryToSwap){
			if (!cur_src->SwapToAnotherFile(true, true, true, NULL, false, true) ) //Xman x4.1 allow go over hardlimit at this case
				theAppPtr->downloadqueue->RemoveSource(cur_src, false);
		}
		else
			theAppPtr->downloadqueue->RemoveSource(cur_src, false);
	}
	...
	if(!A4AFsrclist.IsEmpty())
	{
		...
		for(pos1 = A4AFsrclist.GetHeadPosition();(pos2=pos1)!=NULL;)
		{
			...
			{ 
				...
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveSource(this->A4AFsrclist.GetAt(pos2),this);
			}
			else{
				...
				{ 
					...
					theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveSource(A4AFsrclist.GetAt(pos2),this);
				}
			}
		}
...
void CPartFile::DeleteFile(){
	...
	theAppPtr->sharedfiles->RemoveFile(this);
	theAppPtr->downloadqueue->RemoveFile(this);
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.RemoveFile(this);
	theAppPtr->knownfiles->AddCancelledFileID(GetFileHash());
...
void CPartFile::SetDownPriority(uint8 np, bool resort)
{
	...
	if ( m_iDownPriority != np )
	{
	    ...
	    if(resort) {
			...
			theAppPtr->downloadqueue->SortByPriority();
			theAppPtr->downloadqueue->CheckDiskspaceTimed();
	    }
...
void CPartFile::StopFile(bool bCancel, bool resort)
{
	...
	*/
    ...
    if(resort) {
	    theAppPtr->downloadqueue->SortByPriority();
	    theAppPtr->downloadqueue->CheckDiskspace();
    }
	UpdateDisplayedInfo(true);
}
...
void CPartFile::PauseFile(bool bInsufficient, bool resort)
{
	...
	for( POSITION pos = srclist.GetHeadPosition(); pos != NULL; )
	{
		...
		if (cur_src->GetDownloadState() == DS_DOWNLOADING)
		{
			...
			*/
			cur_src->SetDownloadState(DS_ONQUEUE, _T("You cancelled the download. Sending OP_CANCELTRANSFER"), CUpDownClient::DSR_PAUSED);
			//Xman end
		}
	}
	delete packet;
...
void CPartFile::ResumeFile(bool resort)
{
	...
	if (status==PS_ERROR && m_bCompletionError){
		...
		//Xman - MORPH - Changed by SiRoB, Flush Thread
		/*
		if (gaplist.IsEmpty()){
		*/
		if (gaplist.IsEmpty() && !m_nTotalBufferData && !m_FlushThread) {
		//Xman end
	...
	for (UINT i = 0; i < nCount; i++)
	{
		...
		if (uPacketSXVersion >= 3)
		{
			...
			if (!IsLowID(dwID))
			{
				...
				if (theAppPtr->ipfilter->IsFiltered(dwIDED2K))
				{
					if (thePrefs.GetLogFilteredIPs())
						//Xman show filename
						/*
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - IP filter (%s)"), ipstr(dwIDED2K), theAppPtr->ipfilter->GetLastHit());
						*/
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - IP filter (%s)  for file: %s"), ipstr(dwIDED2K), theAppPtr->ipfilter->GetLastHit(), GetFileName());
						//Xman end
					continue;
				}
				if (theAppPtr->clientlist->IsBannedClient(dwIDED2K)){
#ifdef _DEBUG
					if (thePrefs.GetLogBannedClients()){
						CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(dwIDED2K);
						//Xman code fix
						/*
						...
						*/
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - banned client %s"), ipstr(dwIDED2K), pClient ? pClient->DbgGetClientInfo() : _T("unknown"));
						//Xman end
					}
#endif
					continue;
		...
		else
		{
			...
			if (!IsLowID(dwID))
			{
				...
				if (theAppPtr->ipfilter->IsFiltered(dwID))
				{
					if (thePrefs.GetLogFilteredIPs())
						//Xman show filename
						/*
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - IP filter (%s)"), ipstr(dwID), theAppPtr->ipfilter->GetLastHit());
						*/
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - IP filter (%s) for file: %s"), ipstr(dwID), theAppPtr->ipfilter->GetLastHit(), GetFileName());
						//Xman end
					continue;
				}
				if (theAppPtr->clientlist->IsBannedClient(dwID)){
#ifdef _DEBUG
					if (thePrefs.GetLogBannedClients()){
						CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(dwID);
						//Xman code fix
						/*
						...
						*/
						AddDebugLogLine(false, _T("Ignored source (IP=%s) received via source exchange - banned client %s"), ipstr(dwID), pClient ? pClient->DbgGetClientInfo() : _T("unknown"));
						//Xman end
					}
#endif
					continue;
		...
		*/
		if (GetMaxSources() > GetSourceCount() && IsGlobalSourceAddAllowed())
		//Xman end
		{
								...
								const CUpDownClient* client)
{
	...
	// [BCPlugin] handle differently if this is a simple client
	if (theDllApp.wrapper_is_simple_client(GetFileHash()))
	{
		return theDllApp.wrapper_simple_client_write_buffer(GetFileHash(), transize, data, start, end, block, client, *this, requestedblocks_list);
	}
	...
	*/
	...
	for (POSITION pos1 = gaplist.GetHeadPosition();pos1 != NULL;){
		...
		// log transferinformation in our "blackbox"
		if(client) // SiRoB: Import Part - added by zz_fly
		...
		*/
		...
		if (forcewait == false || forcedbecauseincreasing==true) //Xman Flush Thread improvement
		{
			...
			if (m_FlushThread) {
				...
				theAppPtr->AddNewDiscAccessThread(m_FlushThread);
				//Xman end
				return;
			} else {
				m_hpartfile.Flush();
...
void CPartFile::FlushDone()
{
		...
		for (int iPartNumber = partCount-1; iPartNumber >= 0; iPartNumber--)
		{
			...
			*/
			if (IsComplete(PARTSIZE * (uint64)uPartNumber, (PARTSIZE * (uint64)(uPartNumber + 1)) - 1, true))
			{
				...
				m_PartsHashing++;
				if (theAppPtr->emuledlg->IsRunning()) //Xman Flush Thread/Safehash (Morph)
				{
					CPartHashThread* parthashthread = (CPartHashThread*) AfxBeginThread(RUNTIME_CLASS(CPartHashThread), THREAD_PRIORITY_BELOW_NORMAL,0, CREATE_SUSPENDED);
	...
	// Update met file
	//Xman MORPH - Flush Thread Moved Down
	/*
	SavePartFile();
	*/
	//Xman end
...
int CPartFileFlushThread::Run()
{
	...
	//theAppPtr->QueueDebugLogLine(false,_T("FLUSH:Start (%s)"),m_partfile->GetFileName()/*, CastItoXBytes(myfile->m_iAllocinfo, false, false)*/ );
	...
	try{
		...
		HANDLE mutexhandle=theAppPtr->hashing_mut.m_hObject;
		...
		CSingleLock sLock1(&(theAppPtr->hashing_mut), TRUE); //SafeHash - wait a current hashing process end before read the chunk
	...
	catch (CFileException* error)
	{
		...
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_FILEALLOCEXC,(WPARAM)m_partfile,(LPARAM)error) );
	...
	catch(...)
	{
		...
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_FILEALLOCEXC,(WPARAM)m_partfile,0) );
		delete[] m_partfile->m_FlushSetting->changedPart;
		delete m_partfile->m_FlushSetting;
		m_partfile->m_FlushSetting = NULL;
		m_partfile->m_FlushThread = NULL;
	...
	m_partfile->m_FlushThread = NULL;
	VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_FLUSHDONE,0,(LPARAM)m_partfile) );
	//theAppPtr->QueueDebugLogLine(false,_T("FLUSH:End (%s)"),m_partfile->GetFileName());
...
void CPartFile::FlushBuffersExceptionHandler(CFileException* error)
{
	if (thePrefs.IsCheckDiskspaceEnabled() && error->m_cause == CFileException::diskFull)
	{
		...
		if (theAppPtr->emuledlg->IsRunning() && thePrefs.GetNotifierOnImportantError()){
			CString msg;
			msg.Format(GetResString(IDS_ERR_OUTOFSPACE), GetFileName());
			theAppPtr->emuledlg->ShowNotifier(msg, TBN_IMPORTANTEVENT);
		}
		...
		if (theAppPtr->emuledlg->IsRunning())
		{
			if (thePrefs.IsCheckDiskspaceEnabled() && thePrefs.GetMinFreeDiskSpace()==0)
				theAppPtr->downloadqueue->CheckDiskspace(true);
			else
				PauseFile(true/*bInsufficient*/);
		}
	}
...
UINT AFX_CDECL CPartFile::AllocateSpaceThread(LPVOID lpParam)
{
	...
	theAppPtr->QueueDebugLogLine(false,_T("ALLOC:Start (%s) (%s)"),myfile->GetFileName(), CastItoXBytes(myfile->m_iAllocinfo, false, false) );
	...
	{
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_FILEALLOCEXC,(WPARAM)myfile,(LPARAM)error) );
	...
	{
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_FILEALLOCEXC,(WPARAM)myfile,0) );
		myfile->m_AllocateThread=NULL;
		return 2;
	}
#endif
	...
	myfile->m_AllocateThread=NULL;
	theAppPtr->QueueDebugLogLine(false,_T("ALLOC:End (%s)"),myfile->GetFileName());
	return 0;
}
...
void CPartFile::UpdateDisplayedInfo(bool force)
{
	if (theAppPtr->emuledlg->IsRunning()){
        ...
        if(force || curTick-m_lastRefreshedDLDisplay > MINWAIT_BEFORE_DLDISPLAY_WINDOWUPDATE+m_random_update_wait) {
			theAppPtr->emuledlg->transferwnd->downloadlistctrl.UpdateItem(this);
...
{
	if (theAppPtr->emuledlg->IsRunning())
		theAppPtr->emuledlg->transferwnd->downloadlistctrl.UpdateCurrentCategoryView(this);
}
									  ...
									  uint16* count) /*const*/
{
	...
	if (theDllApp.wrapper_is_simple_client(GetFileHash()))
	{
		return theDllApp.wrapper_simple_client_get_next_requested_block(sender, newblocks, count, GetFileHash(), GetPartCount(), requestedblocks_list);
	}
	...
	uint32	timeToFileCompletion = max((uint32) (bytesLeftToDownload / (uint64) fileDatarate) + 1, 10); // Always assume it will take at least 10 seconds to complete
	...
	*count = (uint16)min(blockstorequest - remainingblocks , *count);
	...
	while(newBlockCount != *count){
		...
		if(tempLastPartAsked == (uint16)-1){
			...
			*/
			if(chunksList.IsEmpty() == TRUE && chunklist_initialized==false){
			//Xman end
				...
				for(POSITION pos = chunksList.GetHeadPosition(); pos != NULL; ){
					...
					if(isPreviewEnable == true){
						if(cur_chunk.part == 0){
							...
							if( (GetFileSize() - uEnd) < (uint64)PARTSIZE/3
								|| (GetFileSize() - uEnd) < (uint64)GetFileSize()/120	// [BCPlugin] - tail needs 0.83% of file size
								){
					...
					*/
					uint16 transferringClientsScore = (uint16)(m_downloadingSourceList.GetSize() - m_downloadingDeleteList.GetSize());
					//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
					...
					*/
					if((m_downloadingSourceList.GetSize() - m_downloadingDeleteList.GetSize()) > 1) {
					//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
						...
						for(POSITION downloadingClientPos = m_downloadingSourceList.GetHeadPosition(); downloadingClientPos != NULL; ) {
							...
							//zz_fly :: delayed deletion of downloading source :: Enig123 :: Start
							if (m_downloadingDeleteList.Find(downloadingClient))
								continue;
							//zz_fly :: delayed deletion of downloading source :: Enig123 :: End
							if(downloadingClient->IsPartAvailable(cur_chunk.part)) {
								...
								*/
								totalDownloadDatarateForThisPart += downloadingClient->GetDownloadDatarate10() + 500; // + 500 to make sure that a unstarted chunk available at two clients will end up just barely below 2000 (max limit)
								//Xman end
							}
						}
...
void CPartFile::AICHRecoveryDataAvailable(UINT nPart)
{
	...
	//Xman //Stulle: uint16 should enough
	/*
	...
	*/
	...
	if (IsComplete((uint64)nPart*PARTSIZE, (((uint64)nPart*PARTSIZE)+length)-1, true)){
		...
		if (!HashSinglePart(nPart)){
			...
			if (posCorrupted)
				corrupted_list.RemoveAt(posCorrupted);
			if (status == PS_EMPTY && theAppPtr->emuledlg->IsRunning()){
				if (GetHashCount() == GetED2KPartHashCount() && !hashsetneeded){
					// Successfully recovered part, make it available for sharing
					SetStatus(PS_READY);
					theAppPtr->sharedfiles->SafeAddKFile(this);
...
{
	//AddDebugLogLine(false, "GetMaxConnectionReached(%i) GetOpenSockets(%i)",theAppPtr->listensocket->GetMaxConnectionReached(),theAppPtr->listensocket->GetOpenSockets());
	uint32 removed = 0;
...
void CPartFile::ProcessSourceCache()
{
	if(m_lastSoureCacheProcesstime + SOURCECACHEPROCESSLOOP < ::GetTickCount())
	{
			...
			&& IsGlobalSourceAddAllowed()) //Xman GlobalMaxHarlimit for fairness
		{
			...
			if(theAppPtr->downloadqueue->CheckAndAddSource(this,newsource))
				sourcesadded++;
		}
...
bool CPartFile::IsGlobalSourceAddAllowed()
{
	...
	if(theAppPtr->downloadqueue->GetGlobalSources()<thePrefs.m_uMaxGlobalSources)
		return true;
	else
		return false;
}
...
bool CPartFile::IsSourceSearchAllowed()
{
	...
	if(thePrefs.m_uMaxGlobalSources > theAppPtr->downloadqueue->GetGlobalSources() + 50)
		return true;
	else
		return false;
}
...
uint16 CPartHashThread::SetFirstHash(CPartFile* pOwner)
{
	...
	if (!theAppPtr->emuledlg->IsRunning())	// Don't start any last-minute hashing
...
void CPartHashThread::SetSinglePartHash(CPartFile* pOwner, uint16 part, bool ICHused, bool AICHRecover)
{
	...
	if (part >= pOwner->GetPartCount()) {	// Out of bounds, no point in even trying
		if (AICHRecover)
			PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDCORRUPTAICHRECOVER,part,(LPARAM)m_pOwner);
		else if (!ICHused)		// ICH only sends successes
			PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDCORRUPT,part,(LPARAM)m_pOwner);
		return;
	}
	...
	if (file.Open(directory+_T("\\")+filename,CFile::modeRead|CFile::osSequentialScan|CFile::shareDenyNone)){
		for (UINT i = 0; i < (UINT)m_PartsToHash.GetSize(); i++){
			...
			try
			{
				...
				if (!theAppPtr->emuledlg->IsRunning())	// in case of shutdown while still hashing
					break;
			}
			...
			if (md4cmp(hashresult,m_DesiredHashes[i])){
				if (m_AICHRecover)
					PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDCORRUPTAICHRECOVER,partnumber,(LPARAM)m_pOwner);
				else if (!m_ICHused)		// ICH only sends successes
					PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDCORRUPT,partnumber,(LPARAM)m_pOwner);
			} else {
				if (m_ICHused)
					m_pOwner->m_ICHPartsComplete.AddTail(partnumber);	// Time critical, don't use message callback
				else if (m_AICHRecover)
					PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDOKAICHRECOVER,partnumber,(LPARAM)m_pOwner);
				else
					PostMessage(theAppPtr->emuledlg->m_hWnd,TM_PARTHASHEDOK,partnumber,(LPARAM)m_pOwner);
			}
		}
		file.Close();
	}
