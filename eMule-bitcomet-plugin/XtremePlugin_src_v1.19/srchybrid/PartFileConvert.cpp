...
UINT AFX_CDECL CPartFileConvert::run(LPVOID /*lpParam*/)
{
	...
	if (imported)
		theAppPtr->sharedfiles->PublishNextTurn();
...
int CPartFileConvert::performConvertToeMule(CString folder)
{
	...
	if (theAppPtr->downloadqueue->GetFileByID(file->GetFileHash())!=0) {
		delete file;
	...
	theAppPtr->downloadqueue->AddDownload(file,thePrefs.AddNewFilesPaused());
	file->SavePartFile();
	
	if (file->GetStatus(true) == PS_READY)
		theAppPtr->sharedfiles->SafeAddKFile(file); // part files are always shared files
...
void CPartFileConvert::ShowGUI(){
	...
	if (m_convertgui)
		...
		m_convertgui->SetIcon(m_convertgui->m_icnWnd = theAppPtr->LoadIcon(_T("Convert")), FALSE);
