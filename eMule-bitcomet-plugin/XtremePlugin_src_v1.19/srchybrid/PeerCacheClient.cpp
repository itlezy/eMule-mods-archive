...
CPeerCacheUpSocket::~CPeerCacheUpSocket()
{
	...
	/*
	theAppPtr->uploadBandwidthThrottler->RemoveFromAllQueues(this);
	DetachFromClient();
	*/
	DetachFromClient();
	theAppPtr->uploadBandwidthThrottler->RemoveFromAllQueues(this);
	...
	if (GetClient())
	{
		if (GetClient()->m_pPCUpSocket == this) {
			...
			if(GetClient()->IsDownloading() && GetClient()->socket!=NULL)
			{
				...
				theAppPtr->uploadBandwidthThrottler->ReplaceSocket(GetClient()->socket, (CClientReqSocket*)this ,GetClient()->socket);
			}
			else
			{
				DEBUG_ONLY( Debug(_T("DetachFromClient without uploading/normal socket")));
				theAppPtr->uploadBandwidthThrottler->RemoveFromStandardList(this);
			}
...
UINT CUpDownClient::ProcessPeerCacheUpHttpRequest(const CStringAArray& astrHeaders)
{
	...
	CKnownFile* pUploadFile = theAppPtr->sharedfiles->GetFileByID(aucUploadFileID);
	if (pUploadFile == NULL){
		DebugHttpHeaders(astrHeaders);
...
bool CUpDownClient::SendHttpBlockRequests()
{
	...
	sockAddr.sin_port = htons( theAppPtr->m_pPeerCache->GetCachePort() );
	sockAddr.sin_addr.S_un.S_addr = theAppPtr->m_pPeerCache->GetCacheIP();
...
bool CUpDownClient::SendPeerCacheFileRequest()
{
	...
	data.WriteUInt8(5);
	CTag tagCacheIP(PCTAG_CACHEIP, theAppPtr->m_pPeerCache->GetCacheIP());
	...
	CTag tagPublicIP(PCTAG_PUBLICIP, theAppPtr->GetPublicIP());
	tagPublicIP.WriteNewEd2kTag(&data);
	CTag tagCachePort(PCTAG_CACHEPORT, theAppPtr->m_pPeerCache->GetCachePort());
...
bool CUpDownClient::ProcessPeerCacheQuery(const uchar* packet, UINT size)
{
	...
	CKnownFile* pUploadFile = theAppPtr->sharedfiles->GetFileByID(aucFileHash);
	if (pUploadFile == NULL){
		if (thePrefs.GetVerbose())
	...
	//theAppPtr->uploadBandwidthThrottler->AddToStandardList(0, m_pPCUpSocket);
...
bool CUpDownClient::ProcessPeerCacheAnswer(const uchar* packet, UINT size)
{
	...
	theAppPtr->m_pPeerCache->DownloadAttemptStarted();
	ASSERT( m_ePeerCacheDownState == PCDS_WAIT_CACHE_REPLY );
	return true;
}
