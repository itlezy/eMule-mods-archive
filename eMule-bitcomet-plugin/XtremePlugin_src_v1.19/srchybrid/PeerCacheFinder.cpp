	...
	if (!bEnabled){
		...
		if (bLastSearchSuccess){ // sanitycheck
			...
			if (!bOK){
				...
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("Sanitycheck for PeerCache-Port '%u' failed"), nPort));
			}
			else
				m_nPCPort = nPort;
		}
		...
		if (dwLastSearch != 0){
			...
			if ( (difftime(tNow,tLast) / 86400) < RETRYDAYS ){
				if (!bLastSearchSuccess){
					...
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Not starting a search - last failed search is too near")));
					return;
				}
				else{
					...
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: CacheIdent still valid, not trying to revaldite this time")));
				}
			}
			else
				m_bNotReValdited = false;
			...
			switch(m_PCLUState){
				...
				case LUS_FINISHED:
					...
					if (!m_bValdited){
						...
						DEBUG_ONLY(theAppPtr->QueueLogLine(false, GetResString(IDS_PEERCACHE_ENABLED)));
...
void CPeerCacheFinder::DoLookUp(CStringA strHostname){
	if (WSAAsyncGetHostByName(theAppPtr->emuledlg->m_hWnd, UM_PEERCHACHE_RESPONSE, strHostname, s_acDNSBuffer, sizeof(s_acDNSBuffer)) == 0){
...
bool CPeerCacheFinder::IsCacheAvailable() const { 
	...
	return m_PCStatus == PCS_READY && theAppPtr->GetPublicIP() != 0 && m_nPCPort != 0; 
}
		...
		if (pfnDnsQueryConfig && pfnDnsQuery && pfnDnsRecordListFree)
		{
			...
			if (nDnsState == 0)
			{
				...
				if (thePrefs.GetVerbose())
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("ReverseDNS: Failed to get list of DNS servers - %s"), GetErrorMessage(nDnsState, 1)));
			}
			...
			if (nDnsState == 0)
			{
				...
				if (thePrefs.GetVerbose())
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("ReverseDNS: Failed to resolve address \"%s\" - %s"), strDnsQuery, GetErrorMessage(nDnsState, 1)));
			}
...
void CPeerCacheFinder::DoReverseLookUp(uint32 dwIP){
	...
	pThread->m_dwIP = dwIP;
	pThread->m_hwndAsyncResult = theAppPtr->emuledlg->m_hWnd;
...
BOOL CPCValditeThread::Run(){
	...
	if (theAppPtr->emuledlg != NULL && theAppPtr->emuledlg->IsRunning()){
		if (Valdite()){
			m_pOwner->m_bValdited = true;
			m_pOwner->m_nPCPort = m_nPCPort;
			if (m_pOwner->m_PCStatus == PCS_VALDITING){
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Valditing .p2pinfo succeeded")));
				DEBUG_ONLY(theAppPtr->QueueLogLine(false, GetResString(IDS_PEERCACHE_ENABLED)));
...
bool CPCValditeThread::Valdite(){
	...
	for (int i = 0; i < _countof(anPeerCachPorts); i++){
		try
		{
			...
			strError.Trim(_T(" \r\n"));
			pException->Delete();
			DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Failed to retrieve .p2pinfo file on Port %u - %s"),anPeerCachPorts[i], strError));
			if (i == (_countof(anPeerCachPorts)-1)){ // was last try
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Failed to retrieve .p2pinfo file, cache disabled"),anPeerCachPorts[i]));
				return false;
			}
		}
	}
	...
	if (file && nIFileSize > SIGNATURELENGTH){
		...
		if (file->Read(pcContent, uContentSize) != uContentSize)
			DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Failed to read p2pinfo file content")));
		...
		while (pszLine && !bContentCheckFailed)
		{
			...
			if ( posSeperator != -1 && strLine.GetLength() - posSeperator > 1)
			{
				...
				//DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Current line to be processed: %hs"),strLine);
				...
				///////***** ExpireDate
					...
					if (bDateCheckFailed){
						...
						DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: ExpireDate check failed. Expiring date: %hs"),strContent));
					}
				}
				///////***** EnableED2K
					...
					if (atol(strContent) != 1){
						bContentCheckFailed = true;
						DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: EnableED2K check failed. Value: %hs"),strContent));
					}
				}
				else{
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Unused content tag for validity check: %hs"),strLine));
				}
		...
		if (bIPRangeCheckFailed){
			bContentCheckFailed = true;
			DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: CacheIP check failed.")));
		}
		...
		if (bIPCheckFailed){
			bContentCheckFailed = true;
			DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: ClientIPRange check failed. my IP: %s"), ipstr(m_dwMyIP)));
		}
		...
		if( !bContentCheckFailed && HttpQueryInfo(*file, HTTP_QUERY_RAW_HEADERS_CRLF, achBuffer, &nBufferSize, NULL) ){
			...
			while (!strCurHeader.IsEmpty())
			{
				...
				if ( posSeperator != -1 && strCurHeader.GetLength() - posSeperator > 1)
				{
					...
					DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Current Header to be processed: %hs"),strCurHeader));
			...
			if (!m_pOwner->IsClientPCCompatible(CClientVersionInfo(CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin, CemuleApp::m_nVersionUpd, CemuleApp::m_nVersionBld, SO_EMULE))){
				bContentCheckFailed = true;
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Current Version not allowed to use this PC-Server, please update")));
			}
		}
		else if (!bContentCheckFailed)
			DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache Error: Failed to retrieve headers, Errornumber %s"),GetErrorMessage(GetLastError()) ));
		...
		if (!bContentCheckFailed){
			...
			if (!bSignatureCheckResult)
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("PeerCache: Failed to verify PeerCache Server, not using it")));
		}
...
BOOL CPCReverseDnsThread::InitInstance()
{
	...
	else
	{
		...
		if (WSAAsyncGetHostByAddr(theAppPtr->emuledlg->m_hWnd, UM_PEERCHACHE_RESPONSE, (const char*) &IPHost, sizeof(struct in_addr), AF_INET, s_acDNSBuffer, sizeof(s_acDNSBuffer)) == 0){
			if (thePrefs.GetVerbose())
				DEBUG_ONLY(theAppPtr->QueueDebugLogLine(false, _T("DNS Reverse Lookup for own IP failed")));
		}	
	}
	return FALSE;
}
