...
#define EMULE_PLUGIN_REG_PATH _T("Software\\BitComet\\eMulePlugin")
...
//bool CPreferences::m_rememberAICH; //zz_fly :: removed
...
{
	// [BCPlugin]
	return theAppPtr->m_pDllApp->m_pszProfileName;
}
...
uint64 CPreferences::GetMaxDownloadInBytesPerSec(bool dynamic){
	//dont be a Lam3r :)
	UINT maxup;
	if (dynamic && thePrefs.IsDynUpEnabled() && theAppPtr->uploadqueue->GetWaitingUserCount() != 0 && theAppPtr->uploadqueue->GetDatarate() != 0) {
		maxup = theAppPtr->uploadqueue->GetDatarate();
	} else {
...
void CPreferences::SaveStats(int bBackUp){
	...
	*/
	ini.WriteUInt64(L"TotalDownloadedBytes", theAppPtr->pBandWidthControl->GeteMuleIn()+GetTotalDownloaded());
	//Xman end
	...
	*/
	ini.WriteUInt64(L"TotalUploadedBytes", theAppPtr->pBandWidthControl->GeteMuleOut()+GetTotalUploaded()); // Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	//Xman end
	ini.WriteInt(L"UpSuccessfulSessions", theAppPtr->uploadqueue->GetSuccessfullUpCount() + GetUpSuccessfulSessions());
	ini.WriteInt(L"UpFailedSessions", theAppPtr->uploadqueue->GetFailedUpCount() + GetUpFailedSessions());
	ini.WriteInt(L"UpAvgTime", (theAppPtr->uploadqueue->GetAverageUpTime() + GetUpAvgTime())/2);
	...
	uint32 eMuleOut;
	uint32 notUsed;
	theAppPtr->pBandWidthControl->GetDatarates(GetDatarateSamples(),
		eMuleIn, notUsed,
		eMuleOut, notUsed,
		notUsed, notUsed);
	//Xman end
	...
	*/
	...
	// Xman
	/*
	tempRate = (float)theAppPtr->downloadqueue->GetDatarate() / 1024;
	*/
	...
	*/
	...
	// Xman
	/*
	tempRate = (float)theAppPtr->uploadqueue->GetDatarate() / 1024;
	*/
	...
	if (theAppPtr->serverconnect->IsConnected())
		ini.WriteInt(L"ConnAvgConnections", (UINT)((theAppPtr->listensocket->GetAverageConnections() + cumConnAvgConnections)/2));
	
	// Peak Connections
	if (theAppPtr->listensocket->GetPeakConnections() > cumConnPeakConnections)
		cumConnPeakConnections = theAppPtr->listensocket->GetPeakConnections();
	ini.WriteInt(L"ConnPeakConnections", cumConnPeakConnections);
	
	// Max Connection Limit Reached
	if (theAppPtr->listensocket->GetMaxConnectionReached() + cumConnMaxConnLimitReached > cumConnMaxConnLimitReached)
		ini.WriteInt(L"ConnMaxConnLimitReached", theAppPtr->listensocket->GetMaxConnectionReached() + cumConnMaxConnLimitReached);
	...
	if ((UINT)theAppPtr->sharedfiles->GetCount() > cumSharedMostFilesShared)
		cumSharedMostFilesShared = theAppPtr->sharedfiles->GetCount();
	ini.WriteInt(L"SharedMostFilesShared", cumSharedMostFilesShared);

	uint64 bytesLargestFile = 0;
	uint64 allsize = theAppPtr->sharedfiles->GetDatasize(bytesLargestFile);
	...
	if (theAppPtr->sharedfiles->GetCount() != 0) {
		uint64 tempint = allsize/theAppPtr->sharedfiles->GetCount();
										   ...
										   BOOL bUpDown, uint32 bytes, bool sentToFriend)
{
	...
	switch (bUpDown){
		case true:
			...
			theAppPtr->UpdateSentBytes(bytes, sentToFriend);
		...
		case false:
			...
			theAppPtr->UpdateReceivedBytes(bytes);
	}
}
...
void CPreferences::ResetCumulativeStatistics(){
	...
	SaveStats();
	theAppPtr->emuledlg->statisticswnd->ShowStatistics(true);
}
...
void CPreferences::SavePreferences()
{
	...
	//---
	ini.WriteString(L"AppVersion", theAppPtr->m_strCurVersionLong);
	//---
	...
	*/
	...
	ini.WriteBool(L"PartiallyPurgeOldKnownFiles", m_bPartiallyPurgeOldKnownFiles); //zz_fly
	...
	*/
	...
	//Xman remove unused AICH-hashes
	//ini.WriteBool(L"rememberAICH", m_rememberAICH); //zz_fly :: removed
...
void CPreferences::LoadPreferences()
{
	...
	strCurrVersion = theAppPtr->m_strCurVersionLong;
	...
	if (strNick.IsEmpty() || IsDefaultNick(strNick))
		...
		//SetUserNick(_T("http://xtreme-mod.net")); //Xman
		SetUserNick(_T("http://www.emule-mods.de/?mods=xtreme")); //zz_fly :: new default username
	...
	*/
	...
	//Xman increased to 80 sec
	/*
	m_uFileBufferTimeLimit = SEC2MS(ini.GetInt(L"FileBufferTimeLimit", 60));
	*/
	m_uFileBufferTimeLimit = SEC2MS(ini.GetInt(L"FileBufferTimeLimit", 80)); // Max milliseconds before forcing a flush
	//Xman end
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	*/
	bool tmpRememberAICH = ini.GetBool(L"rememberAICH", false); 
	m_bPartiallyPurgeOldKnownFiles = ini.GetBool(L"PartiallyPurgeOldKnownFiles", !tmpRememberAICH);
	//zz_fly :: end
...
//Xman end
	...
	*/
	...
	m_bEnableUPnP = ini.GetBool(L"EnableUPnP", tmpEnableUPnP, L"UPnP");
	//zz_fly :: end
	...
	*/
	...
	SetPushSmallFileSize(ini.GetInt(_T("PushSmallFiles"), 262144));
	SetPushSmallFileBoost(ini.GetInt(_T("PushSmallBoost"), 50));
	// <== push small files [sivka] - Stulle
	...
	//Xman process prio
	m_MainProcessPriority = ini.GetInt(L"MainProcessPriority", ABOVE_NORMAL_PRIORITY_CLASS);
	...
	//Xman remove unused AICH-hashes //zz_fly :: removed
	/*
...
{
	return GetReBarToolbar() && theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(5,8,0,0);
}
...
void CPreferences::EstimateMaxUploadCap(uint32 nCurrentUpload){
	if (maxGraphUploadRateEstimated+1 < nCurrentUpload){
		...
		if (maxGraphUploadRate == UNLIMITED && theAppPtr->emuledlg && theAppPtr->emuledlg->statisticswnd)
			theAppPtr->emuledlg->statisticswnd->SetARange(false, thePrefs.GetMaxGraphUploadRate(true));
	}
}
...
CString CPreferences::GetDefaultDirectory(EDefaultDirectory eDirectory, bool bCreate){
	...
	if (m_astrDefaultDirs[0].IsEmpty()){ // already have all directories fetched and stored?	
		...
		*pszFileName = L'\0';
		...
		if (rkEMuleRegKey.Open(HKEY_CURRENT_USER, EMULE_PLUGIN_REG_PATH, KEY_READ) == ERROR_SUCCESS){
							...
							if (nRegistrySetting == -1){
								...
								if (bRes)
									...
									if (bRes)
										...
										//zz_fly :: this setting is better for the newbies :: start
										/*
...
void CPreferences::ChangeUserDirMode(int nNewMode){
	...
	if (rkEMuleRegKey.Create(HKEY_CURRENT_USER, EMULE_PLUGIN_REG_PATH) == ERROR_SUCCESS){
...
{
	if(NAFCFullControl!=flag && theAppPtr->emuledlg && theAppPtr->emuledlg->m_hWnd && theAppPtr->m_app_state == APP_STATE_RUNNING)
	{
		NAFCFullControl = flag;
		theAppPtr->emuledlg->statisticswnd->RepaintMeters();
	}
	else
		NAFCFullControl = flag;
}
