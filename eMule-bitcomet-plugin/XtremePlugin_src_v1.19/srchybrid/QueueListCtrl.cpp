...
void CQueueListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	dc.SelectObject(thePrefs.UseNarrowFont() ? &m_fontNarrow : GetFont());
	//Xman end
	...
	switch (iSubItem)
	{
		...
		case 4:
		{
			...
			if (client->GetSmallFilePush())
				Sbuffer.AppendFormat(_T(", %s"), GetResString(IDS_SMALL));
...
int CQueueListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theAppPtr->emuledlg->transferwnd->queuelistctrl.GetNextSortOrder(lParamSort)) != -1)
...
void CQueueListCtrl::AddClient(/*const*/ CUpDownClient *client, bool resetclient)
{
	...
	Update(iItem);
	theAppPtr->emuledlg->transferwnd->UpdateListCount(CTransferWnd::wnd2OnQueue, iItemCount + 1);
}
...
void CQueueListCtrl::RemoveClient(const CUpDownClient *client)
{
	...
	if (result != -1) {
		DeleteItem(result);
		theAppPtr->emuledlg->transferwnd->UpdateListCount(CTransferWnd::wnd2OnQueue);
	}
}
...
void CQueueListCtrl::RefreshClient(const CUpDownClient *client)
{
	...
	if (theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->transferwnd || !theAppPtr->emuledlg->transferwnd->queuelistctrl.IsWindowVisible())
		return;
...
void CALLBACK CQueueListCtrl::QueueUpdateTimer(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
	...
	__if_exists(Core_Common::InterfaceExceptionDebugger)
	{
		__try
		{
			return CQueueListCtrl::QueueUpdateTimerImpl(0, 0, 0, 0);
		}
...
// Barry - Refresh the queue every 10 secs
void CALLBACK CQueueListCtrl::QueueUpdateTimerImpl(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
#ifdef VERSION_DLL
	...
	*/
	//Xman end
	{
		if (   !theAppPtr->emuledlg->IsRunning() // Don't do anything if the app is shutting down - can cause unhandled exceptions
			|| !thePrefs.GetUpdateQueueList()
			|| theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->transferwnd
			|| !theAppPtr->emuledlg->transferwnd->queuelistctrl.IsWindowVisible() )
			return;

		//Xman faster Updating of Queuelist
		/*
		const CUpDownClient* update = theAppPtr->uploadqueue->GetNextClient(NULL);
		while( update )
		{
		...
		*/
		if (theAppPtr->emuledlg->transferwnd->queuelistctrl.GetItemCount()>1)
		{
