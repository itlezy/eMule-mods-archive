...
void CSearchListCtrl::UpdateSearch(CSearchFile* toupdate)
{
	...
	if( theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->searchwnd || IsWindowVisible() == FALSE )
		return;
...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	m_SearchFileMenu.EnableMenuItem(MP_SEARCHRELATED, iSelected > 0 && theAppPtr->emuledlg->searchwnd->CanSearchRelatedFiles() ? MF_ENABLED : MF_GRAYED);
			...
			case MP_ADDSEARCHCANCELLED:
				{
					...
					while (pos!=NULL) 
					{
						...
						theAppPtr->knownfiles->AddCancelledFileID(cur_hash);
						Update(cur_sel);
					}
					return TRUE;
				}
				...
				if (file){
					if (file->GetPreviews().GetSize() > 0){
						...
						if (!theAppPtr->clientlist->AttachToAlreadyKnown(&newclient,NULL)){
							...
							*/
							theAppPtr->clientlist->AddClient(newclient, true);
							//Xman end						
						}
...
void CSearchListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	ImageList_Draw(theAppPtr->GetSystemImageList(), iImage, dc, cur_rec.left + iIndent + TREE_WIDTH, cur_rec.top + iIconPosY, ILD_TRANSPARENT);
...
void CSearchListCtrl::DrawSourceChild(CDC *dc, int nColumn, LPRECT lpRect, UINT uDrawTextAlignment, const CSearchFile *src)
{
	...
	{
		case 0: {		// file name
			UINT uOffset = 8 + 8 + theAppPtr->GetSmallSytemIconSize().cy + sm_iLabelOffset;
...
void CSearchListCtrl::DrawSourceParent(CDC *dc, int nColumn, LPRECT lpRect, UINT uDrawTextAlignment, const CSearchFile *src)
{
	...
	{
		case 0: {		// file name
			UINT uOffset = 8 + theAppPtr->GetSmallSytemIconSize().cx + sm_iLabelOffset;
			if (   (thePrefs.ShowRatingIndicator() && (src->HasComment() || src->HasRating() || src->IsKadCommentSearchRunning()))
				|| (thePrefs.IsSearchSpamFilterEnabled() && src->IsConsideredSpam()) )
				uOffset += 16;
			lpRect->left += uOffset;
...
void CSearchListCtrl::SetHighlightColors()
{
	...
	*/
	...
	theAppPtr->LoadSkinColor(GetSkinKey() + _T("Fg_Downloading"), m_crSearchResultDownloading);
	//zz_fly :: bug fix :: DolphinX :: start
	/*
	if (!theAppPtr->LoadSkinColor(_T("Fg_DownloadStopped"), m_crSearchResultDownloadStopped))
	*/
	if (!theAppPtr->LoadSkinColor(GetSkinKey() + _T("Fg_DownloadStopped"), m_crSearchResultDownloadStopped))
	//zz_fly :: end
		m_crSearchResultDownloadStopped = m_crSearchResultDownloading;
	theAppPtr->LoadSkinColor(GetSkinKey() + _T("Fg_Sharing"), m_crSearchResultShareing);
	theAppPtr->LoadSkinColor(GetSkinKey() + _T("Fg_Known"), m_crSearchResultKnown);
	theAppPtr->LoadSkinColor(GetSkinKey() + _T("Fg_AvblyBase"), crSearchResultAvblyBase);
