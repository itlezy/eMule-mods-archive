...
void CSearchResultsWnd::OnInitialUpdate()
{
	...
	theAppPtr->searchlist->SetOutputWnd(&searchlistctrl);
	//zz_fly, fix minor official draw bug around SharedFiles Filter on Language change
	// X-Ray :: FiXeS :: Bugfix :: Start :: WiZaRd
	/*
	...
	*/
	// X-Ray :: FiXeS :: Bugfix :: End :: WiZaRd
	searchlistctrl.Init(theAppPtr->searchlist);
	...
	m_btnSearchListMenu->ModifyStyle(TBSTYLE_TOOLTIPS | ((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0), 0);
	...
	*/
	...
	if (theAppPtr->m_fontSymbol.m_hObject){
		GetDlgItem(IDC_STATIC_DLTOof)->SetFont(&theAppPtr->m_fontSymbol);
...
void CSearchResultsWnd::OnTimer(UINT nIDEvent)
{
	...
	else if (nIDEvent == global_search_timer)
	{
	    if (theAppPtr->serverconnect->IsConnected())
		{
			...
			CServer* toask = NULL;
			while (servercount < theAppPtr->serverlist->GetServerCount()-1)
			{
				servercount++;
				searchprogress.StepIt();

				toask = theAppPtr->serverlist->GetNextSearchServer();
				if (toask == NULL)
					break;
			...
			if (toask)
			{
				...
				if (toask->SupportsLargeFilesUDP() && (toask->GetUDPFlags() & SRV_UDPFLG_EXT_GETFILES))
				{
					...
					theAppPtr->serverconnect->SendUDPPacket(pExtSearchPacket, toask, true);
					bRequestSent = true;
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
					if (thePrefs.GetDebugServerUDPLevel() > 0)
						Debug(_T(">>> Sending %s  to server %-21s (%3u of %3u)\n"),  _T("OP__GlobSearchReq3"), ipstr(toask->GetAddress(), toask->GetPort()), servercount, theAppPtr->serverlist->GetServerCount());
#endif //zz_fly :: DummyCut
				}
...
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
						if (thePrefs.GetDebugServerUDPLevel() > 0)
							Debug(_T(">>> Sending %s  to server %-21s (%3u of %3u)\n"), _T("OP__GlobSearchReq2"), ipstr(toask->GetAddress(), toask->GetPort()), servercount, theAppPtr->serverlist->GetServerCount());
#endif //zz_fly :: DummyCut
						theStats.AddUpDataOverheadServer(searchpacket->size);
						theAppPtr->serverconnect->SendUDPPacket(searchpacket, toask, false);
						bRequestSent = true;
					}
					else{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
						if (thePrefs.GetDebugServerUDPLevel() > 0)
							Debug(_T(">>> Skipped UDP search on server %-21s (%3u of %3u): No large file support\n"), ipstr(toask->GetAddress(), toask->GetPort()), servercount, theAppPtr->serverlist->GetServerCount());
#endif //zz_fly :: DummyCut
					}
				}
				else
...
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
						if (thePrefs.GetDebugServerUDPLevel() > 0)
							Debug(_T(">>> Sending %s  to server %-21s (%3u of %3u)\n"), _T("OP__GlobSearchReq1"), ipstr(toask->GetAddress(), toask->GetPort()), servercount, theAppPtr->serverlist->GetServerCount());
#endif //zz_fly :: DummyCut
						theStats.AddUpDataOverheadServer(searchpacket->size);
						theAppPtr->serverconnect->SendUDPPacket(searchpacket, toask, false);
						bRequestSent = true;
				...
				if (bRequestSent)
					theAppPtr->searchlist->SentUDPRequestNotification(m_nEd2kSearchID, toask->GetIP());
			}
			else
				CancelEd2kSearch();
	    }
...
void CSearchResultsWnd::DownloadSelected(bool bPaused)
{
	...
	while (pos != NULL)
	{
		...
		if (iIndex >= 0)
		{
			...
			if ( theAppPtr->knownfiles->CheckAlreadyDownloadedFileQuestion(tempFile.GetFileHash(), tempFile.GetFileName()) )
			{
				theAppPtr->downloadqueue->AddSearchToDownload(&tempFile, bPaused, GetSelectedCat());
			}
			//Xman end
...
bool CSearchResultsWnd::StartNewSearch(SSearchParams* pParams)
{
	...
	if (pParams->eType == SearchTypeAutomatic){
		...
		if (!theAppPtr->serverconnect->IsConnected() && Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsConnected())
		...
		else if (theAppPtr->serverconnect->IsConnected() && (!Kademlia::CKademlia::IsRunning() || !Kademlia::CKademlia::IsConnected()))
			...
			if (theAppPtr->serverconnect->IsConnected() && theAppPtr->serverconnect->GetCurrentServer() != NULL 
				&& (theAppPtr->serverconnect->GetCurrentServer()->IsStaticMember()
				|| (theAppPtr->serverconnect->GetCurrentServer()->GetUsers() > 40000 && theAppPtr->serverlist->GetServerCount() < 40
					&& theAppPtr->serverconnect->GetCurrentServer()->GetUsers() < 5000000 
					&& theAppPtr->serverconnect->GetCurrentServer()->GetFiles() > 5000000)))
			{
				pParams->eType = SearchTypeEd2kServer;
			}
			else
...
bool CSearchResultsWnd::DoNewEd2kSearch(SSearchParams* pParams)
{
	...
	bool bServerSupports64Bit = theAppPtr->serverconnect->GetCurrentServer() != NULL
								&& (theAppPtr->serverconnect->GetCurrentServer()->GetTCPFlags() & SRV_TCPFLG_LARGEFILES);
	...
	theAppPtr->searchlist->NewSearch(&searchlistctrl, strResultType, m_nEd2kSearchID, pParams->eType, pParams->strExpression);
	canceld = false;
	...
	theAppPtr->serverconnect->SendPacket(packet,false);

	if (pParams->eType == SearchTypeEd2kGlobal && theAppPtr->serverconnect->IsUDPSocketAvailable())
	{
		...
		if (thePrefs.GetUseServerPriorities())
			theAppPtr->serverlist->ResetSearchServerPos();
		...
		servercount = 0;
		searchprogress.SetRange32(0, theAppPtr->serverlist->GetServerCount() - 1);
		globsearch = true;
	}
	else{
		globsearch = false;
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams)
{
	...
	if (strResultType == ED2KFTSTR_PROGRAM)
		strResultType.Empty();
	theAppPtr->searchlist->NewSearch(&searchlistctrl, strResultType, pParams->dwSearchID, pParams->eType, pParams->strExpression);
	CreateNewTab(pParams);
	return true;
}
...
void CSearchResultsWnd::DeleteSearch(uint32 nSearchID)
{
	...
	if (nSearchID == m_nEd2kSearchID)
		m_pwndParams->m_ctlMore.EnableWindow(FALSE);
	theAppPtr->searchlist->RemoveResults(nSearchID);
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Search);
	return TRUE;
}

LRESULT CSearchResultsWnd::OnIdleUpdateCmdUI(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	BOOL bSearchParamsWndVisible = theAppPtr->emuledlg->searchwnd->IsSearchParamsWndVisible();
...
{
	theAppPtr->emuledlg->searchwnd->OpenParametersWnd();
}
...
void CSearchResultsWnd::OnSysCommand(UINT nID, LPARAM lParam)
{
	if (nID == SC_KEYMENU)
	{
		if (lParam == EMULE_HOTMENU_ACCEL)
			theAppPtr->emuledlg->SendMessage(WM_COMMAND, IDC_HOTMENU);
		else
			theAppPtr->emuledlg->SendMessage(WM_SYSCOMMAND, nID, lParam);
		return;
	}
...
{
	return theAppPtr->serverconnect->IsConnected() 
		&& theAppPtr->serverconnect->GetCurrentServer() != NULL 
		&& theAppPtr->serverconnect->GetCurrentServer()->GetRelatedSearchSupport();
}
...
// [BCPlugin] Êä³öËÑË÷½ø¶È
void CSearchResultsWnd::get_search_progress(int& total, int& current)
{
	total = 0;
	current = 0;
	int min = 0;
	searchprogress.GetRange(min, total);
	current = searchprogress.GetPos();

	UINT search_result_cnt = 0;
	if(theAppPtr && theAppPtr->searchlist)
		search_result_cnt = theAppPtr->searchlist->GetED2KResultCount();
	if(current == 0)
	{
		if( canceld )
		{
