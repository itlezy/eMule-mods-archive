...
eResult CSecRunAsUser::PrepareUser(){
	...
	try{
		...
		try{
			...
			// clean up and abort
			theAppPtr->QueueDebugLogLine(false, _T("Run as unpriveleged user: Exception while preparing user account: %s!"), error);
			CoUninitialize();
		...
		// clean up and abort
		theAppPtr->QueueDebugLogLine(false, _T("Run as unpriveleged user: Unexpected fatal error while preparing user account!"));
		CoUninitialize();
		return RES_FAILED;
	}
...
#define FULLACCESS ADS_RIGHT_GENERIC_ALL
	...
	if (!bSucceeded)
		theAppPtr->QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Failed to set directoy permissions!"));
...
bool CSecRunAsUser::SetObjectPermission(CString strDirFile, DWORD lGrantedAccess){
	...
	try {
		...
		fAPISuccess = FALSE;
		theAppPtr->QueueDebugLogLine(false, error);
	}
	// clean up
	if (pUserSID != NULL)
		MHeapFree(pUserSID);
...
eResult CSecRunAsUser::RestartAsUser(){
	...
	try{
		...
		::CloseHandle(theAppPtr->m_hMutexOneInstance);
		...
		theAppPtr->QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Unexpected exception while loading advapi32.dll"));
		FreeAPI();
		return RES_FAILED;
	}
	FreeAPI();
	if (!bResult)
		theAppPtr->QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Failed to restart eMule as different user! Error Code: %i"),GetLastError());
...
eResult CSecRunAsUser::RestartAsRestricted(){
	...
	try{
		...
		theAppPtr->QueueDebugLogLine(false, _T("SecureShellExecute exception: %s!"), strError);
...
eResult CSecRunAsUser::RestartSecure(){
	...
	if (!thePrefs.IsPreferingRestrictedOverUser()){
		res = PrepareUser();;
		if (res == RES_OK){
			theAppPtr->QueueLogLine(false, GetResString(IDS_RAU_RUNNING), EMULEACCOUNTW);
			return RES_OK;
		}
	...
	if (res == RES_OK){
		theAppPtr->QueueLogLine(false, GetResString(IDS_RUNNINGRESTRICTED));
	}
