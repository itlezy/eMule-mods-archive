...
bool CServerList::Init()
{
	...
	if(thePrefs.AutoUpdateIPFilter())
	{
		...
		if(update)
		{
			// [BCPlugin] bc plugin not show splash window
			//if(theAppPtr->IsSplash()) theAppPtr->DestroySplash(); //Xman new slpash-screen arrangement
			theAppPtr->ipfilter->UpdateIPFilterURL();
		}
	}
	//Xman end
...
bool CServerList::AddServerMetToList(const CString& strFile, bool bMerge) 
{
	if (!bMerge)
	{
		theAppPtr->emuledlg->serverwnd->serverlistctrl.DeleteAllItems();
		RemoveAllServers();
	}
	...
	try{
		...
		theAppPtr->emuledlg->serverwnd->serverlistctrl.Hide();
		theAppPtr->emuledlg->serverwnd->serverlistctrl.SetRedraw(FALSE);
		...
		for (UINT j = 0; j < fservercount; j++)
		{
			...
			if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(newserver, true, true))
			{
				CServer* update = theAppPtr->serverlist->GetServerByAddress(newserver->GetAddress(), newserver->GetPort());
				if (update)
				{
					update->SetListName(newserver->GetListName());
					update->SetDescription(newserver->GetDescription());
					theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(update);
				}
				delete newserver;
			}
			else
	...
	theAppPtr->emuledlg->serverwnd->serverlistctrl.SetRedraw(TRUE);
	theAppPtr->emuledlg->serverwnd->serverlistctrl.Visable();
	return true;
}
...
bool CServerList::AddServer(const CServer* pServer, bool bAddTail)
{
	...
	if (thePrefs.GetFilterServerByIP()){
		...
		//if (pServer->HasDynIP())
		//	return false;
		if (theAppPtr->ipfilter->IsFiltered(pServer->GetIP())){
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(AddServer): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pServer->GetListName(), ipstr(pServer->GetIP()), theAppPtr->ipfilter->GetLastHit());
			return false;
		}
	}
	...
	if (pFoundServer){
		...
		theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(pFoundServer);
		return false;
	}
	if (bAddTail)
...
{
	//Xman
    //return theAppPtr->lastCommonRouteFinder->AddHostsToCheck(list);
	return false;
}
// ZZ:UploadSpeedSense <--
...
void CServerList::ServerStats()
{
	...
	if (theAppPtr->IsConnected() && theAppPtr->serverconnect->IsUDPSocketAvailable() && list.GetCount() > 0)
	{
		...
		if (ping_server->GetFailedCount() >= thePrefs.GetDeadServerRetries()) {
			...
			if ((!ping_server->IsStaticMember()) || (!thePrefs.GetDontRemoveStaticServers())) {
				theAppPtr->emuledlg->serverwnd->serverlistctrl.RemoveServer(ping_server);
				return;
			}
			//Xman end
		}
		...
		if (!ping_server->GetCryptPingReplyPending() && (tNow - ping_server->GetLastPingedTime()) >= UDPSERVSTATREASKTIME && theAppPtr->GetPublicIP() != 0 && thePrefs.IsServerCryptLayerUDPEnabled()){
			...
			theAppPtr->serverconnect->SendUDPPacket(NULL, ping_server, true, ping_server->GetPort() + 12, pRawPacket, nPacketLen);
		}
		else if (ping_server->GetCryptPingReplyPending() || theAppPtr->GetPublicIP() == 0 || !thePrefs.IsServerCryptLayerUDPEnabled()){
			...
			theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(ping_server);
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
			if (thePrefs.GetDebugServerUDPLevel() > 0)
				Debug(_T(">>> Sending OP__GlobServStatReq (not obfuscated) to server %s:%u\n"), ping_server->GetAddress(), ping_server->GetPort());
#endif //zz_fly :: DummyCut
			theStats.AddUpDataOverheadServer(packet->size);
			theAppPtr->serverconnect->SendUDPPacket(packet, ping_server, true);
		}
		else
			ASSERT( false );
	}
...
void CServerList::RemoveServer(const CServer* pServer)
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL; )
	{
		...
		if (test_server == pServer){
			if (theAppPtr->downloadqueue->cur_udpserver == pServer)
				theAppPtr->downloadqueue->cur_udpserver = NULL;
			list.RemoveAt(pos2);
			delservercount++;
			delete test_server;
			return;
...
{
	CServerListCtrl& serverListCtrl = theAppPtr->emuledlg->serverwnd->serverlistctrl;
		...
		while (f.ReadString(strLine))
		{
			...
			if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(nsrv, true))
			{
				...
				if (srvexisting) {
					...
					if (theAppPtr->emuledlg->serverwnd)
						theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(srvexisting);
				}
			}
		}
		f.Close();
...
void CServerList::CheckForExpiredUDPKeys() {
	...
	const uint32 dwIP = theAppPtr->GetPublicIP();
