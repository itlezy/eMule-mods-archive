...
void CServerListCtrl::SetAllIcons()
{
	...
	imagelist.Create(16,16,theAppPtr->m_iDfltImageListColorFlags|ILC_MASK,0,1);
...
void CServerListCtrl::RemoveServer(const CServer* pServer)
{
	...
	if (iItem != -1) {
		theAppPtr->serverlist->RemoveServer(pServer);
		DeleteItem(iItem); 
		ShowServerCount();
	}
}
...
{
	ShowWindow(SW_HIDE);
	for (POSITION pos = theAppPtr->serverlist->list.GetHeadPosition(); pos != NULL; )
	{
		const CServer* cur_server = theAppPtr->serverlist->list.GetNext(pos);
		if (cur_server->GetFailedCount() >= thePrefs.GetDeadServerRetries())
		{
			...
			if ((!cur_server->IsStaticMember()) || (!thePrefs.GetDontRemoveStaticServers())) {
				...
				pos = theAppPtr->serverlist->list.GetHeadPosition();
			}
			// [end] Mighty Knife
		}
	}
...
void CServerListCtrl::RemoveAllFilteredServers()
{
	...
	{
		const CServer* cur_server = theAppPtr->serverlist->list.GetNext(pos);
		if (theAppPtr->ipfilter->IsFiltered(cur_server->GetIP()))
		{
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(Updated): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), cur_server->GetListName(), ipstr(cur_server->GetIP()), theAppPtr->ipfilter->GetLastHit());
			RemoveServer(cur_server);
			pos = theAppPtr->serverlist->list.GetHeadPosition();
		}
	}
...
{
	bool bAddTail = !bRandom || ((GetRandomUInt16() % (1 + theAppPtr->serverlist->GetServerCount())) != 0);
	if (!theAppPtr->serverlist->AddServer(pServer, bAddTail))
		return false;
	if (bAddToList)
	{
...
{
	if (!server || !theAppPtr->emuledlg->IsRunning())
		return;
	//MORPH START - SiRoB, Don't Refresh item if not needed
	if( theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->serverwnd || IsWindowVisible() == FALSE )
		return;
...
{
	for (POSITION pos = theAppPtr->serverlist->list.GetHeadPosition(); pos != NULL; theAppPtr->serverlist->list.GetNext(pos))
	{
		RefreshServer(theAppPtr->serverlist->list.GetAt(pos));
	}
}
...
{ 
	...
	ServerMenu.AppendMenu(MF_STRING | (theAppPtr->IsEd2kServerLinkInClipboard() ? MF_ENABLED : MF_GRAYED), MP_PASTE, GetResString(IDS_SW_DIRECTDOWNLOAD), _T("PASTELINK"));
...
BOOL CServerListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
		...
		if (GetSelectedCount() > 1)
		{
			...
			while (pos != NULL)
			{
				...
				if (iItem > -1) {
					...
					theAppPtr->serverlist->MoveServerDown(pServer);
				}
			}
			theAppPtr->serverconnect->ConnectToAnyServer(theAppPtr->serverlist->GetServerCount() - GetSelectedCount(), false, false);
		}
		else
		{
			int iItem = GetNextItem(-1, LVIS_SELECTED | LVIS_FOCUSED);
			if (iItem > -1)
				theAppPtr->serverconnect->ConnectToServer((CServer*)GetItemData(iItem));
		}
	...
	case MP_CUT: {
		...
		if (!strURLs.IsEmpty())
			theAppPtr->CopyTextToClipboard(strURLs);
	...
	case Irc_SetSendLink: {
		...
		if (!strURLs.IsEmpty()) {
			if (wParam == Irc_SetSendLink)
				theAppPtr->emuledlg->ircwnd->SetSendFileString(strURLs);
			else
				theAppPtr->CopyTextToClipboard(strURLs);
		}
		return TRUE;
	...
	case MP_PASTE:
		if (theAppPtr->IsEd2kServerLinkInClipboard())
			theAppPtr->emuledlg->serverwnd->PasteServerFromClipboard();
		return TRUE;
	...
	case MPG_DELETE: {
		...
		while (GetFirstSelectedItemPosition() != NULL)
		{
			POSITION pos = GetFirstSelectedItemPosition();
			int iItem = GetNextSelectedItem(pos);
			theAppPtr->serverlist->RemoveServer((CServer*)GetItemData(iItem));
			DeleteItem(iItem);
		}
		ShowServerCount();
		SetRedraw(TRUE);
	...
	case MP_REMOVEALL:
		...
		if (theAppPtr->serverconnect->IsConnecting()) {
			theAppPtr->downloadqueue->StopUDPRequests();
			theAppPtr->serverconnect->StopConnectionTry();
			theAppPtr->serverconnect->Disconnect();
			theAppPtr->emuledlg->ShowConnectionState();
		}
		ShowWindow(SW_HIDE);
		theAppPtr->serverlist->RemoveAllServers();
		DeleteAllItems();
		ShowWindow(SW_SHOW);
		ShowServerCount();
		return TRUE;
...
void CServerListCtrl::SetSelectedServersPriority(UINT uPriority)
{
	...
	if (bUpdateStaticServersFile)
		theAppPtr->serverlist->SaveStaticServers();
}
...
void CServerListCtrl::OnNmDblClk(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	if (iSel != -1){
		theAppPtr->serverconnect->ConnectToServer((CServer*)GetItemData(iSel));
	   	theAppPtr->emuledlg->ShowConnectionState();
	}
}
...
{
	SetRedraw(FALSE);
	bool bResult = theAppPtr->serverlist->AddServerMetToList(strFile, true);
...
#define UNDEFINED_INT_AT_BOTTOM(i1, i2) \
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theAppPtr->emuledlg->serverwnd->serverlistctrl.GetNextSortOrder(lParamSort)) != -1)
...
bool CServerListCtrl::StaticServerFileAppend(CServer *server)
{
	...
	bResult = theAppPtr->serverlist->SaveStaticServers();
...
bool CServerListCtrl::StaticServerFileRemove(CServer *server)
{
	...
	return theAppPtr->serverlist->SaveStaticServers();
}
...
void CServerListCtrl::ShowServerCount()
{
	...
	theAppPtr->emuledlg->serverwnd->GetDlgItem(IDC_SERVLIST_TEXT)->SetWindowText(GetResString(IDS_SV_SERVERLIST) + counter);
}
...
//Xman end
	...
	if (pnmlvcd->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
	{
		...
		const CServer* pConnectedServer = theAppPtr->serverconnect->GetCurrentServer();
...
void CServerListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if(theAppPtr->serverconnect->IsConnected()
		&& (cur_srv = theAppPtr->serverconnect->GetCurrentServer()) != NULL
		...
		&& _tcsicmp(cur_srv->GetAddress(), server->GetAddress()) == 0)
	{
		...
		if(server->GetFailedCount() >= thePrefs.GetDeadServerRetries() || theAppPtr->ipfilter->IsFiltered(server->GetIP()))
		{
			GetFont()->GetLogFont(&lfFont);
			fontCustom.CreateFontIndirect(&lfFont);
			dc.SelectObject(&fontCustom);
				...
				switch (iColumn)
				{
					case 0:
					{
						...
						if(theAppPtr->ip2country->ShowCountryFlag() ){
							theAppPtr->ip2country->GetFlagImageList()->Draw(dc, server->GetCountryFlagIndex(), point2, ILD_NORMAL);
						}
						else
