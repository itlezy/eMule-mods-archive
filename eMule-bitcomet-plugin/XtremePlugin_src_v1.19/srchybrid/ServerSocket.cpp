...
BOOL CServerSocket::OnHostNameResolved(const SOCKADDR_IN *pSockAddr)
{
	...
	if (cur_server->HasDynIP())
	{
		...
		CServer* pServer = theAppPtr->serverlist->GetServerByAddress(cur_server->GetAddress(), cur_server->GetPort());
		if (pServer) {
			...
			theAppPtr->serverlist->RemoveDuplicatesByIP(pServer);
		}
		...
		//
		if (thePrefs.GetFilterServerByIP() && theAppPtr->ipfilter->IsFiltered(cur_server->GetIP())) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(TCP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pServer ? pServer->GetAddress() : cur_server->GetAddress(), ipstr(cur_server->GetIP()), theAppPtr->ipfilter->GetLastHit());
			if (pServer)
				theAppPtr->emuledlg->serverwnd->serverlistctrl.RemoveServer(pServer);
			m_bIsDeleting = true;
	...
	try
	{
		switch (opcode)
		{
			case OP_SERVERMESSAGE:{
				...
				CServer* pServer = cur_server ? theAppPtr->serverlist->GetServerByAddress(cur_server->GetAddress(), cur_server->GetPort()) : NULL;
				...
				while (!message.IsEmpty())
				{
					...
					if (_tcsnicmp(message, _T("server version"), 14) == 0){
						...
						if (pServer){
							...
							theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(pServer);
							theAppPtr->emuledlg->serverwnd->UpdateMyInfo();
						}
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
						if (thePrefs.GetDebugServerTCPLevel() > 0)
							Debug(_T("%s\n"), message);
						...
						if (dynip.GetLength() && dynip.GetLength() < 51){
							...
							if (pServer && inet_addr(CStringA(dynip)) == INADDR_NONE){
								...
								if (strOldDynIP.CompareNoCase(pServer->GetDynIP()) != 0)
									theAppPtr->serverlist->RemoveDuplicatesByAddress(pServer);
								if (cur_server)
					...
					if (bOutputMessage) {
						if (m_bStartNewMessageLog) {
							...
							theAppPtr->emuledlg->AddServerMessageLine(LOG_INFO, _T(""));
							if (cur_server) {
								CString strMsg;
								if (IsObfusicating())
								...
								theAppPtr->emuledlg->AddServerMessageLine(LOG_SUCCESS, strMsg);
							}
						}
						theAppPtr->emuledlg->AddServerMessageLine(LOG_INFO, message);
					}
			...
			case OP_IDCHANGE:{
				...
				if (cur_server){
					...
					pServer = theAppPtr->serverlist->GetServerByAddress(cur_server->GetAddress(), cur_server->GetPort());
					if (pServer)
				...
				if(la->clientid == 0){
					...
					if(attempts < 3){
						...
						/*
						if (attempts == 1)
							theAppPtr->emuledlg->RefreshUPnP(false); // refresh the UPnP mappings once
						*/
						//zz_fly :: end
						SetConnectionState(CS_ERROR);
				...
				if (::IsLowID(la->clientid) && dwServerReportedIP != 0)
					theAppPtr->SetPublicIP(dwServerReportedIP);
				AddLogLine(false, GetResString(IDS_NEWCLIENTID), la->clientid);

				//Xman -Reask sources after IP change- v4
				if(serverconnect->GetClientID() != 0 && theAppPtr->last_valid_ip != 0
				   && serverconnect->GetClientID() != theAppPtr->last_valid_ip 
				   && serverconnect->GetClientID() != theAppPtr->last_valid_serverid)
				{
...

					{
						...
						if(GetTickCount() - theAppPtr->last_ip_change > FILEREASKTIME + 60000){
							...
							theAppPtr->last_ip_change=::GetTickCount();
							theAppPtr->m_bneedpublicIP=false;
							AddLogLine(false, _T("Change from %u (%s ID) to %u (%s ID) detected%s"), 
								theAppPtr->last_valid_serverid,
								(theAppPtr->last_valid_serverid < 16777216) ? _T("low") : _T("high"),
				...
				if(serverconnect->GetClientID() != 0 && theAppPtr->last_ip_change==0)
					theAppPtr->last_ip_change=::GetTickCount();
				if(serverconnect->GetClientID() != 0 && serverconnect->GetClientID() != theAppPtr->last_valid_serverid){
					// Keep track of a change of the global IP
					theAppPtr->last_valid_serverid = serverconnect->GetClientID();
				}
				theAppPtr->last_valid_ip=theAppPtr->GetPublicIP(true); //can also be 0

				theAppPtr->last_traffic_reception=::GetTickCount();
				theAppPtr->internetmaybedown=false; //we have to reopen here if we are using server only
				// Xman end

				theAppPtr->downloadqueue->ResetLocalServerRequests();
				break;
			}
			case OP_SEARCHRESULT:{
				...
				CServer* pServer = cur_srv ? theAppPtr->serverlist->GetServerByAddress(cur_srv->GetAddress(), cur_srv->GetPort()) : NULL;
				(void)pServer;
				bool bMoreResultsAvailable;
				UINT uSearchResults = theAppPtr->searchlist->ProcessSearchAnswer(packet, size, true/*pServer ? pServer->GetUnicodeSupport() : false*/, cur_srv ? cur_srv->GetIP() : 0, cur_srv ? cur_srv->GetPort() : (uint16)0, &bMoreResultsAvailable);
				theAppPtr->emuledlg->searchwnd->LocalEd2kSearchEnd(uSearchResults, bMoreResultsAvailable);
				break;
			}
			...
			case OP_FOUNDSOURCES:{
				...
				if (cur_server)
				{
					...
					if (CPartFile* file = theAppPtr->downloadqueue->GetFileByID(fileid))
			...
			case OP_SERVERSTATUS:{
				...
				if (pServer){
					...
					theAppPtr->emuledlg->ShowUserCount();
					theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(pServer);
					theAppPtr->emuledlg->serverwnd->UpdateMyInfo();
				}
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
			...
			case OP_SERVERLIST:{
				...
				try{
					...
					while(count)
					{
						...
						if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(srv, true))
							delete srv;
						else
							addcount++;
						count--;
			...
			case OP_CALLBACKREQUESTED:{
				...
				if (size >= 6)
				{
					...
					if (theAppPtr->ipfilter->IsFiltered(dwIP)){
						theStats.filteredclients++;
						if (thePrefs.GetLogFilteredIPs())
							AddDebugLogLine(false, _T("Ignored callback request (IP=%s) - IP filter (%s)"), ipstr(dwIP), theAppPtr->ipfilter->GetLastHit());
						break;
					}

					if (theAppPtr->clientlist->IsBannedClient(dwIP)){
						if (thePrefs.GetLogBannedClients()){
							CUpDownClient* pClient = theAppPtr->clientlist->FindClientByIP(dwIP);
					...
					CUpDownClient* client = theAppPtr->clientlist->FindClientByIP(dwIP,nPort);
					if (client == NULL)
					{
						client = new CUpDownClient(0,nPort,dwIP,0,0,true);
						theAppPtr->clientlist->AddClient(client, true); //Xman Code Improvement don't search new generated clients in lists
					}
