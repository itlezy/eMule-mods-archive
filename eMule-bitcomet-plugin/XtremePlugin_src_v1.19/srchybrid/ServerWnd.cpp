...
{
	if (theAppPtr->m_fontLog.m_hObject == NULL)
	{
		CFont* pFont = GetDlgItem(IDC_SSTATIC)->GetFont();
		LOGFONT lf;
		pFont->GetObject(sizeof lf, &lf);
		theAppPtr->m_fontLog.CreateFontIndirect(&lf);
	}
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		...
		servermsgbox->SetFont(&theAppPtr->m_fontHyperText);
		servermsgbox->ApplySkin();
		servermsgbox->SetTitle(GetResString(IDS_SV_SERVERINFO));

		//Xman ModID
		/*
		servermsgbox->AppendText(_T("eMule v") + theAppPtr->m_strCurVersionLong + _T("\n"));
		*/
		servermsgbox->AppendText(_T("eMule v") + theAppPtr->m_strCurVersionLong + _T(" [") + MOD_VERSION + _T("]") + _T("\n"));
		//Xman end
	...
	if (logbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_LOGBOX)){
		...
		if (theAppPtr->m_fontLog.m_hObject)
			logbox->SetFont(&theAppPtr->m_fontLog);
		logbox->ApplySkin();
	...
	if (debuglog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_DEBUG_LOG)){
		...
		if (theAppPtr->m_fontLog.m_hObject)
			debuglog->SetFont(&theAppPtr->m_fontLog);
		debuglog->ApplySkin();
	...
	if (leecherlog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_LEECHERLOG)){
		...
		if (theAppPtr->m_fontLog.m_hObject)
			leecherlog->SetFont(&theAppPtr->m_fontLog);
	...
	if (thePrefs.GetUseAutocompletion()){
		...
		if (theAppPtr->m_fontSymbol.m_hObject){
			GetDlgItem(IDC_DD)->SetFont(&theAppPtr->m_fontSymbol);
...
void CServerWnd::SetAllIcons()
{
	...
	if (icon_srvlist)
		VERIFY( DestroyIcon(icon_srvlist) );
	icon_srvlist = theAppPtr->LoadIcon(_T("ServerList"), 16, 16);
...
{
	CString strServer = theAppPtr->CopyTextFromClipboard();
	strServer.Trim();
...
bool CServerWnd::AddServer(uint16 nPort, CString strAddress, CString strName, bool bShowErrorMB)
{
	...
	{
		CServer* pFoundServer = theAppPtr->serverlist->GetServerByAddress(toadd->GetAddress(), toadd->GetPort());
		if (pFoundServer == NULL && toadd->GetIP() != 0)
			pFoundServer = theAppPtr->serverlist->GetServerByIPTCP(toadd->GetIP(), toadd->GetPort());
		if (pFoundServer)
		{
...
void CServerWnd::OnBnClickedResetLog()
{
	...
	{
		theAppPtr->emuledlg->ResetLeecherLog();
		theAppPtr->emuledlg->statusbar->SetText(_T(""), SBarLog, 0);
	}
	//Xman end
	...
	*pResult = 0;
	...
	if (pEnLink && pEnLink->msg == WM_LBUTTONDOWN)
	{
		...
		if (strUrl == m_strClickNewVersion){
			// MOD Note: Do not remove this part - Merkur
			strUrl.Format(_T("/en/version_check.php?version=%i&language=%i"),theAppPtr->m_uCurVersionCheck,thePrefs.GetLanguageID());
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Server);
	return TRUE;
}
...
{
	theAppPtr->emuledlg->ShowPreferences(IDD_PPG_SERVER);
}
