...
void CSharedDirsTreeCtrl::SetAllIcons()
{
	...
	if (pCurImageList != NULL && pCurImageList->GetImageCount() >= 7)
	{
		...
		if (strTempDir.Right(1) != _T("\\"))
			strTempDir += _T("\\");
		int nImage = theAppPtr->GetFileTypeSystemImageIdx(strTempDir);			// 5: System Folder Icon
		if (nImage > 0 && theAppPtr->GetSystemImageList() != NULL) {
			HICON hIcon = ::ImageList_GetIcon(theAppPtr->GetSystemImageList(), nImage, 0);
	...
	theAppPtr->LoadSkinColorAlt(_T("SharedDirsTvBk"), _T("SharedFilesLvBk"), crBk);
	theAppPtr->LoadSkinColorAlt(_T("SharedDirsTvFg"), _T("SharedFilesLvFg"), crFg);
	SetBkColor(crBk);
	SetTextColor(crFg);
}
...
bool CSharedDirsTreeCtrl::FileSystemTreeHasSharedSubdirectory(CString strDir, bool bOrFiles)
{
	...
	return bOrFiles && theAppPtr->sharedfiles->ContainsSingleSharedFiles(strDir);
}
