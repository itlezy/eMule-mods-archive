...
#include "ShareableFile.h" //Xman remove unused AICH-hashes
#include "SR13-ImportParts.h" //MORPH - Added by SiRoB, Import Parts [SR13] - added by zz_fly
#include "UploadQueue.h" //Xman advanced upload-priority
...
int CAddFileThread::Run()
{
	...
	if ( !(m_pOwner || m_partfile) || m_strFilename.IsEmpty() || !theAppPtr->emuledlg->IsRunning() )
		return 0;
	//Xman
	...
	*/
	...
	/*
	if (newrecord->CreateFromFile(m_strDirectory, m_strFilename, m_partfile) && theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning()) // SLUGFILLER: SafeHash - in case of shutdown while still hashing
	*/
	...
	//zz_fly :: end
	{
		...
		if (theAppPtr->emuledlg == NULL || !theAppPtr->emuledlg->IsRunning() || // SLUGFILLER: SafeHash - in case of shutdown while still hashing 
			!PostMessage(theAppPtr->emuledlg->m_hWnd, TM_FINISHEDHASHING, (m_pOwner ? 0: (WPARAM)m_partfile), (LPARAM)newrecord))
			delete newrecord;
	}
	else
	{
		...
		*/
		...
		if (m_pOwner && theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
		{
			UnknownFile_Struct* hashed = new UnknownFile_Struct;
			hashed->strDirectory = m_strDirectory;
			hashed->strName = m_strFilename;
			if (!PostMessage(theAppPtr->emuledlg->m_hWnd, TM_HASHFAILED, 0, (LPARAM)hashed))
				delete hashed;
		}
		// SLUGFILLER: SafeHash
		delete newrecord;
	...
	*/
	{
		...
		while (pos)
		{
			...
			if (cur_file->IsKindOf(RUNTIME_CLASS(CPartFile)) 
				&& !theAppPtr->downloadqueue->IsPartFile(cur_file) 
				&& !theAppPtr->knownfiles->IsFilePtrInList(cur_file)
		...
		ASSERT( theAppPtr->downloadqueue );
		if (theAppPtr->downloadqueue)
			theAppPtr->downloadqueue->AddPartFilesToShare(); // read partfiles
	}
...
bool CSharedFileList::SafeAddKFile(CKnownFile* toadd, bool bOnlyAdd)
{
	...
	if (bAdded && output)
	{
		...
		if(!toadd->IsPartFile())
			theAppPtr->emuledlg->sharedfileswnd->historylistctrl.AddFile(toadd); 
		//Xman end
	}
	m_lastPublishED2KFlag = true;
	return bAdded;
...
bool CSharedFileList::AddFile(CKnownFile* pFile)
{
	...
	*/
	...
	if (m_Files_map.Lookup(key, pFileInMap))
	{
		...
		if (!pFileInMap->IsKindOf(RUNTIME_CLASS(CPartFile)) || theAppPtr->downloadqueue->IsPartFile(pFileInMap))
...
void CSharedFileList::FileHashingFinished(CKnownFile* file)
{
	...
	ASSERT( !theAppPtr->knownfiles->IsFilePtrInList(file) );
	...
	if (found_file == NULL)
	{
		...
		if (!ShouldBeShared(file->GetSharedDirectory(), file->GetFilePath(), false)){
			...
			if (!IsFilePtrInList(file) && !theAppPtr->knownfiles->IsFilePtrInList(file))
				delete file;
			else
				ASSERT(0);
		}
		...
		{
			SafeAddKFile(file);
			theAppPtr->knownfiles->SafeAddKFile(file);
		}
	}
	else
	{
											  ...
											  CServer* pServer, CUpDownClient* pClient)
{
	...
	{
		if (theAppPtr->IsConnected() && !theAppPtr->IsFirewalled())
		{
			nClientID = theAppPtr->GetID();
			nClientPort = thePrefs.GetPort();
		}
...
void CSharedFileList::HashNextFile(){
	...
	//Xman
	/*
	if (!theAppPtr->emuledlg || !::IsWindow(theAppPtr->emuledlg->m_hWnd))	// wait for the dialog to open
	*/
	if (!theAppPtr->emuledlg || !theAppPtr->emuledlg->IsRunning() || !::IsWindow(theAppPtr->emuledlg->m_hWnd))	// wait for the dialog to open
	//Xman end
		return;
	if (theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
		theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.ShowFilesCount();
...
void CSharedFileList::Publish()
{
	...
	UINT tNow = time(NULL);
	bool isFirewalled = theAppPtr->IsFirewalled();
	...
	if( Kademlia::CKademlia::IsConnected() && ( !isFirewalled || ( isFirewalled && theAppPtr->clientlist->GetBuddyStatus() == Connected) || bDirectCallback) && GetCount() && Kademlia::CKademlia::GetPublish())
	{ 
...
void CSharedFileList::DeletePartFileInstances() const
{
	...
	ASSERT( theAppPtr->m_app_state == APP_STATE_SHUTTINGDOWN );
	ASSERT( theAppPtr->knownfiles );
	...
	while (pos)
	{
		...
		if (cur_file->IsKindOf(RUNTIME_CLASS(CPartFile)))
		{
			if (!theAppPtr->downloadqueue->IsPartFile(cur_file) && !theAppPtr->knownfiles->IsFilePtrInList(cur_file))
...
void CSharedFileList::CheckAndAddSingleFile(const CFileFind& ff){
	...
	if (toadd)
	{
		...
		else
		{
			...
			if(toadd->GetAICHHashset()->GetStatus()==AICH_EMPTY
				&& theAppPtr->m_AICH_Is_synchronizing == false  //AICH-Sync-Thread has finished the observation of shared files
				)
			{
				if (!IsHashing(strFoundDirectory, ff.GetFileName()) && !theAppPtr->downloadqueue->IsTempFile(strFoundDirectory, ff.GetFileName()) && !thePrefs.IsConfigFile(strFoundDirectory, ff.GetFileName())){
	...
	else
	{
		...
		*/
		if (!IsHashing(strFoundDirectory, strFoundFileName) && !theAppPtr->downloadqueue->IsTempFile(strFoundDirectory, strFoundFileName) && !thePrefs.IsConfigFile(strFoundDirectory, strFoundFileName)){
		//Xman end
			UnknownFile_Struct* tohash = new UnknownFile_Struct;
			tohash->strDirectory = strFoundDirectory;
...
void CSharedFileList::CalculateUploadPriority(bool force)
{
	...
	if(force || ::GetTickCount() - lastprocess > MIN2MS(2))
	{
		...
		if(m_Files_map.GetCount()>0)
		{
			...
			m_avg_client_on_uploadqueue = theAppPtr->uploadqueue->GetWaitingUserCount() / m_Files_map.GetCount();
		}
		else
		{
			m_avg_virtual_sources = 0;
