...
void CSharedFilesCtrl::Init()
{
	...
	InsertColumn(18,GetResString(IDS_ONQUEUE),LVCFMT_LEFT,50,12); //Xman see OnUploadqueue
	InsertColumn(19,GetResString(IDS_SHAREFACTOR),LVCFMT_LEFT,100,13); //Xman advanced upload-priority
...
//Xman end
{
	...
	if( theAppPtr->emuledlg->activewnd != theAppPtr->emuledlg->sharedfileswnd && !force) //Xman advanced upload-priority 
		return;
...
{
	//Xman [MoNKi: -Downloaded History-]
	if(!theAppPtr->emuledlg->sharedfileswnd->historylistctrl.IsWindowVisible()) //Xman Code Improvement for ShowFilesCount
	{
		CString str;
				...
				switch (iColumn)
				{
					case 0: {
						...
						if (CheckBoxesEnabled())
						{
							...
							if ((file->IsShellLinked() && theAppPtr->sharedfiles->ShouldBeShared(file->GetSharedDirectory(), file->GetFilePath(), false)) 
								|| (theAppPtr->sharedfiles->ShouldBeShared(file->GetSharedDirectory(), file->GetFilePath(), true)))
							{
								iState = CBS_CHECKEDDISABLED;
								iNoStyleState = DFCS_CHECKED | DFCS_INACTIVE;
							}
							else if (theAppPtr->sharedfiles->ShouldBeShared(file->GetSharedDirectory(), file->GetFilePath(), false))
							{
...
void CSharedFilesCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	while (pos)
	{
		...
		bContainsUnshareableFile = !pFile->IsShellLinked() && !pFile->IsPartFile() && (bContainsUnshareableFile || (theAppPtr->sharedfiles->ShouldBeShared(pFile->GetSharedDirectory(), pFile->GetFilePath(), false)
			&& !theAppPtr->sharedfiles->ShouldBeShared(pFile->GetSharedDirectory(), pFile->GetFilePath(), true)));
...
#endif
	m_SharedFilesMenu.EnableMenuItem(Irc_SetSendLink, (!bContainsOnlyShareableFile && iSelectedItems == 1 && theAppPtr->emuledlg->ircwnd->IsConnected()) ? MF_ENABLED : MF_GRAYED);
		...
		|| selectedList.GetCount() > 0)
	{
		...
		switch (wParam){
			case Irc_SetSendLink:
				if (pKnownFile != NULL)
					theAppPtr->emuledlg->ircwnd->SetSendFileString(CreateED2kLink(pKnownFile));
				break;
			case MP_GETED2KLINK:{
				CString str;
			...
			case MP_GETKADSOURCELINK:{
				...
				while (pos != NULL)
				{
					...
					if (file->IsKindOf(RUNTIME_CLASS(CKnownFile))){
						if (!str.IsEmpty())
							str += _T("\r\n");
						str += theAppPtr->CreateKadSourceLink((CKnownFile*)file);
					}
				}
				theAppPtr->CopyTextToClipboard(str);
				break;
			...
			case MPG_F2:
				if (pKnownFile && !pKnownFile->IsPartFile()){
					...
					if (!inputbox.WasCancelled() && newname.GetLength()>0)
					{
						...
						{
							//theAppPtr->sharedfiles->RemoveKeywords(pKnownFile); //zz_fly :: http://forum.emule-project.net/index.php?showtopic=138576
							pKnownFile->SetFileName(newname);
							//theAppPtr->sharedfiles->AddKeywords(pKnownFile); //zz_fly
						}
			...
			case MPG_DELETE:
			{
				...
				while (!selectedList.IsEmpty())
				{
					...
					if (delsucc){
						if (myfile->IsKindOf(RUNTIME_CLASS(CKnownFile))) 
							theAppPtr->sharedfiles->RemoveFile((CKnownFile*)myfile, true);
						else
				...
				if (bRemovedItems) {
					...
					// the item.
					theAppPtr->emuledlg->sharedfileswnd->ShowSelectedFilesSummary();
					theAppPtr->emuledlg->sharedfileswnd->OnSingleFileShareStatusChanged(); // might have been a single shared file
				}
				break; 
			}
			case MP_UNSHAREFILE:
			{
				...
				{
					CShareableFile* myfile = selectedList.RemoveHead();
					if (!myfile || myfile->IsPartFile() || !theAppPtr->sharedfiles->ShouldBeShared(myfile->GetPath(), myfile->GetFilePath(), false)
						|| theAppPtr->sharedfiles->ShouldBeShared(myfile->GetPath(), myfile->GetFilePath(), true))
					{
						continue;
					}

					bUnsharedItems |= theAppPtr->sharedfiles->ExcludeFile(myfile->GetFilePath());
					ASSERT( bUnsharedItems );
				}
				SetRedraw(TRUE);
				if (bUnsharedItems) {
					theAppPtr->emuledlg->sharedfileswnd->ShowSelectedFilesSummary();
					theAppPtr->emuledlg->sharedfileswnd->OnSingleFileShareStatusChanged();
			...
			case MP_ULFEEDBACK: 
				{ 
					...
					if(!selectedList.IsEmpty())
					{
						...
						feed.AppendFormat(_T("Mod: %s%s[%s] \r\n"),_T("eMule"), theAppPtr->m_strCurVersionLong, MOD_VERSION);  
					}

					while (!selectedList.IsEmpty())
					{
						CKnownFile* file = (CKnownFile*)selectedList.RemoveHead();
					...
					theAppPtr->CopyTextToClipboard(feed); 
					break; 
				} 
			//Xman end
...
{
	const CStringArray& rastrFilter = theAppPtr->emuledlg->sharedfileswnd->m_astrFilter;
	...
	GetItemDisplayText(pFile, theAppPtr->emuledlg->sharedfileswnd->GetFilterColumn(),
...
void CSharedFilesCtrl::AddShareableFiles(CString strFromDir)
{
	...
	while (!end)
	{
		...
		CKnownFile* toadd = theAppPtr->knownfiles->FindKnownFile(strFoundFileName, fdate, ullFoundFileSize);
		if (toadd != NULL && theAppPtr->sharedfiles->GetFileByID(toadd->GetFileHash()) != NULL) // check for shared
		{
...
void CSharedFilesCtrl::CheckBoxClicked(int iItem)
{
	...
	if (theAppPtr->sharedfiles->ShouldBeShared(pFile->GetPath(), pFile->GetFilePath(), false)){
		// this is currently shared so unshare it
		if (theAppPtr->sharedfiles->ShouldBeShared(pFile->GetPath(), pFile->GetFilePath(), true))
			return; // not allowed to unshare this file
		VERIFY( theAppPtr->sharedfiles->ExcludeFile(pFile->GetFilePath()) );
		// update GUI stuff
		ShowFilesCount();
		theAppPtr->emuledlg->sharedfileswnd->ShowSelectedFilesSummary();
		theAppPtr->emuledlg->sharedfileswnd->OnSingleFileShareStatusChanged();
	...
	else
	{
		...
		*/
		// SLUGFILLER: SafeHash remove - removed installation dir unsharing
		VERIFY( theAppPtr->sharedfiles->AddSingleSharedFile(pFile->GetFilePath()) );
		ShowFilesCount();
		theAppPtr->emuledlg->sharedfileswnd->ShowSelectedFilesSummary();
		theAppPtr->emuledlg->sharedfileswnd->OnSingleFileShareStatusChanged();
		UpdateFile(pFile);
		...
		if (hDrop != NULL)
		{
				...
				if (DragQueryFile(hDrop, nFile, strFilePath.GetBuffer(MAX_PATH), MAX_PATH) > 0 )
				{
					...
					if (ff.FindFile(strFilePath, 0))
					{
						...
						if (ff.IsDots() || ff.IsSystem() || ff.IsTemporary()
							...
							*/
							// SLUGFILLER: SafeHash remove - removed installation dir unsharing
							|| (ff.IsDirectory() && theAppPtr->sharedfiles->ShouldBeShared(ff.GetFilePath()+ _T('\\'), _T(""), false))
							|| (!ff.IsDirectory() && theAppPtr->sharedfiles->ShouldBeShared(ff.GetFilePath(), ff.GetFilePath().Left(ff.GetFilePath().ReverseFind('\\') + 1), false)) )
						{
			...
			if (!liToAddFiles.IsEmpty() || !liToAddDirs.IsEmpty())
			{
				...
				for (POSITION pos = liToAddDirs.GetHeadPosition(); pos != NULL; )
					VERIFY( theAppPtr->sharedfiles->AddSingleSharedDirectory(liToAddDirs.GetNext(pos)) ); // should always succeed

				bool bHaveFiles = false;
				while (!liToAddFiles.IsEmpty())
					bHaveFiles |= theAppPtr->sharedfiles->AddSingleSharedFile(liToAddFiles.RemoveHead()); // could fail, due to the dirs added above

				// GUI updates
				if (!liToAddDirs.IsEmpty())
					theAppPtr->emuledlg->sharedfileswnd->m_ctlSharedDirTree.Reload(true);
				if (bHaveFiles)
					theAppPtr->emuledlg->sharedfileswnd->OnSingleFileShareStatusChanged();
				...
				if (bHaveFiles && liToAddDirs.IsEmpty() && bFromSingleDirectory)
				{
					...
					VERIFY( theAppPtr->emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowFileSystemDirectory(strSingleDirectory) );
				}
				else if (!liToAddDirs.IsEmpty() && !bHaveFiles)
				{
					...
					if (liToAddDirs.GetCount() == 1)
						theAppPtr->emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowSharedDirectory(liToAddDirs.GetHead());
					else
						theAppPtr->emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowSharedDirectory(_T(""));
				}
				else
				{
					// otherwise select the All Shared Files category
					theAppPtr->emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowAllSharedFiles();
				}
			}
