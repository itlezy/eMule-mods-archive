...
void CStatisticsDlg::SetAllIcons()
{
	...
	CImageList iml;
	iml.Create(16, 16, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
	...
	theAppPtr->LoadSkinColorAlt(_T("StatisticsTvBk"), _T("DefLvBk"), crBk);
	theAppPtr->LoadSkinColorAlt(_T("StatisticsTvFg"), _T("DefLvFg"), crFg);
...
BOOL CStatisticsDlg::OnInitDialog()
{
	...
	if (theAppPtr->m_fontSymbol.m_hObject)
	{
		GetDlgItem(IDC_BNMENU)->SetFont(&theAppPtr->m_fontSymbol);
...
void CStatisticsDlg::SetCurrentRate(float uploadrate, float downloadrate)
{
	...
	m_dPlotDataUp[4] = uploadrate - (float)theAppPtr->uploadqueue->GetToNetworkDatarate() / 1024;
	m_UploadOMeter.AppendPoints(m_dPlotDataUp);

	// Connections
	CDownloadQueue::SDownloadStats myStats;
	theAppPtr->downloadqueue->GetDownloadSourcesStats(myStats);
	m_dPlotDataMore[0] = theAppPtr->listensocket->GetActiveConnections();
	m_dPlotDataMore[1] = theAppPtr->uploadqueue->GetActiveUploadsCount();
	m_dPlotDataMore[2] = theAppPtr->uploadqueue->GetUploadQueueLength();
	...
	updown.connections = theAppPtr->listensocket->GetActiveConnections();
	theAppPtr->webserver->AddStatsLine(updown);
}
*/
void CStatisticsDlg::RepaintMeters() 
{
...
void CStatisticsDlg::ShowGraphs() {	
	...
	plotStatistic[0] = (double)(theAppPtr->listensocket->GetOpenSockets());      // Active connections
	plotStatistic[1] = (double)(theAppPtr->uploadqueue->GetUploadQueueLength()); // Active upload
	...
	theAppPtr->pBandWidthControl->GetDatarates(wantedinterval,
										   plotinData[CURRENT], plotinData[OVERALL],
										   plotOutData[CURRENT], plotOutData[OVERALL],
										   plotinData[ADAPTER], plotOutData[ADAPTER]);
	//Xman end
	theAppPtr->pBandWidthControl->GetFullHistoryDatarates(plotinData[MINUTE], plotOutData[MINUTE],
...
void CStatisticsDlg::Process() {
	...
	// => security
	if(::GetTickCount() > theAppPtr->pBandWidthControl->GetStartTick() + 5000){
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate) 
{
	...
	// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	//cache needed values
	uint64 bEmuleIn= theAppPtr->pBandWidthControl->GeteMuleIn();
	uint64 bEmuleOut= theAppPtr->pBandWidthControl->GeteMuleOut();
	//Xman end
		...
		*/
		...
		if (forceUpdate || stattree.IsExpanded(h_download)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				*/
				cbuffer.Format( GetResString( IDS_STATS_DDATA) + _T(" / %s")   , CastItoXBytes( bEmuleIn, false, false ) , CastItoXBytes( theAppPtr->pBandWidthControl->GetSessionNetworkIn(), false, false ) );
				//Xman end
				...
				{ 
					...
					cbuffer.Format(_T("%s: %s, %s: %s (%.1f%%)"), GetResString(IDS_UDPREASKS), CastItoIShort(theAppPtr->downloadqueue->GetUDPFileReasks()), GetResString(IDS_UFAILED), CastItoIShort(theAppPtr->downloadqueue->GetFailedUDPFileReasks()), theAppPtr->downloadqueue->GetUDPFileReasks() ? (theAppPtr->downloadqueue->GetFailedUDPFileReasks() * 100.0 / theAppPtr->downloadqueue->GetUDPFileReasks()) : 0.0 );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;

					//Xman x4 Xtreme Mod: count failed tcp-connections:
					cbuffer.Format(_T("%s: %s, %s: %s (%.1f%%)"), _T("TCP-connections"), CastItoIShort(theAppPtr->downloadqueue->GetTCPFileReasks()), _T("failed"), CastItoIShort(theAppPtr->downloadqueue->GetFailedTCPFileReasks()), theAppPtr->downloadqueue->GetTCPFileReasks() ? (theAppPtr->downloadqueue->GetFailedTCPFileReasks() * 100.0 / theAppPtr->downloadqueue->GetTCPFileReasks()) : 0.0 );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
					//Xman end

					cbuffer.Format(_T("%s: %s (%s + %s)"), GetResString(IDS_DEADSOURCES), CastItoIShort(theAppPtr->clientlist->m_globDeadSourceList.GetDeadSourcesCount() + myStats.a[22]), CastItoIShort(theAppPtr->clientlist->m_globDeadSourceList.GetDeadSourcesCount()), CastItoIShort((UINT)myStats.a[22]));
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
				}
				...
				*/
				...
				if (forceUpdate || stattree.IsExpanded(hdown_soh)) 
				{
					...
					cbuffer.Format(_T("Obfuscation: %s"), CastItoXBytes(theAppPtr->pBandWidthControl->GeteMuleInObfuscation(),false,false));
					stattree.SetItemText(down_soh[i], cbuffer);
					i++;
					//Xman end
				}
		...
		if (forceUpdate || stattree.IsExpanded(h_upload)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_up_session)) 
			{
				...
				*/
				cbuffer.Format(GetResString(IDS_STATS_UDATA) + _T(" / %s")  ,CastItoXBytes(bEmuleOut, false, false),CastItoXBytes(theAppPtr->pBandWidthControl->GetSessionNetworkOut(), false, false));
				//Xman end
				...
				//Xman
				/*
				cbuffer.Format(GetResString(IDS_STATS_ACTUL_ZZ),theAppPtr->uploadqueue->GetActiveUploadsCount()); //theAppPtr->uploadqueue->GetUploadQueueLength()
				*/
				cbuffer.Format(GetResString(IDS_STATS_ACTUL_ZZ),theAppPtr->uploadBandwidthThrottler->GetNumberOfFullyActivatedSlots()); //theAppPtr->uploadqueue->GetUploadQueueLength()
				//Xman end
				stattree.SetItemText(up_S[2], cbuffer);

                // Set Set Total Uploads                
				cbuffer.Format(GetResString(IDS_STATS_TOTALUL),theAppPtr->uploadqueue->GetUploadQueueLength());
				stattree.SetItemText(up_S[3], cbuffer);

				// Set Queue Length
				cbuffer.Format(GetResString(IDS_STATS_WAITINGUSERS),theAppPtr->uploadqueue->GetWaitingUserCount());
				stattree.SetItemText(up_S[4], cbuffer);

				// Set Upload Sessions
				statGoodSessions = theAppPtr->uploadqueue->GetSuccessfullUpCount() + theAppPtr->uploadqueue->GetUploadQueueLength();
				statBadSessions = theAppPtr->uploadqueue->GetFailedUpCount();
				...
				if (forceUpdate || stattree.IsExpanded(up_S[5])) 
				{
					...
					if(thePrefs.GetVerbose() == true){
						...
						details.Format(_T(" (active: %u, socket: %u, completed: %u, cancelled/ended: %u, different file: %u, exception: %u,blocking: %u, others: %u)"),
									   theAppPtr->uploadqueue->GetUploadQueueLength(),
									   ...
									   theAppPtr->uploadqueue->GetSuccessfullUpCount() - 
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_SOCKET) -
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_COMPLETEDRANSFER) -
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_CANCELLED) -
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_DIFFERENT_FILE) -
					...
					// Set Avg Upload time
					//DWORD running=theAppPtr->uploadqueue->GetAverageUpTime();
					cbuffer.Format(GetResString(IDS_STATS_AVEUPTIME),CastSecondsToLngHM(theAppPtr->uploadqueue->GetAverageUpTime()));
				...
				if (forceUpdate || stattree.IsExpanded(hup_soh)) 
				{
					...
					i++;
					//Xman bandwidthcontrol:  count obfuscation data
					cbuffer.Format(_T("Obfuscation: %s"), CastItoXBytes(theAppPtr->pBandWidthControl->GeteMuleOutObfuscation(),false,false));
					stattree.SetItemText(up_soh[i], cbuffer);
					i++;
					//Xman end
				}
			...
			if (forceUpdate || stattree.IsExpanded(h_up_total)) 
			{
				...
				*/
				...
				// Upload Sessions
				statGoodSessions = theAppPtr->uploadqueue->GetSuccessfullUpCount() + thePrefs.GetUpSuccessfulSessions() + theAppPtr->uploadqueue->GetUploadQueueLength();
				statBadSessions = theAppPtr->uploadqueue->GetFailedUpCount() + thePrefs.GetUpFailedSessions();
				cbuffer.Format(_T("%s: %u"), GetResString(IDS_STATS_ULSES), statGoodSessions + statBadSessions);
				...
				if (forceUpdate || stattree.IsExpanded(up_T[1])) 
				{
					...
					// Set Avg Upload time
					uint32 avguptime = theAppPtr->uploadqueue->GetAverageUpTime();
	...
	{		
		...
		{			
			...
			if (forceUpdate || stattree.IsExpanded(hconn_sg)) 
			{
				...
				i++;
				// Active Connections
				cbuffer.Format(_T("%s: %i (%s:%u | %s:%u | %s:%u)"),GetResString(IDS_SF_ACTIVECON),theAppPtr->listensocket->GetActiveConnections(), GetResString(IDS_HALF), theAppPtr->listensocket->GetTotalHalfCon(), GetResString(IDS_CONCOMPL) ,theAppPtr->listensocket->GetTotalComp(), GetResString(IDS_STATS_PRTOTHER) ,theAppPtr->listensocket->GetActiveConnections() - theAppPtr->listensocket->GetTotalHalfCon() - theAppPtr->listensocket->GetTotalComp());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Average Connections
				cbuffer.Format(_T("%s: %i"),GetResString(IDS_SF_AVGCON),(int)theAppPtr->listensocket->GetAverageConnections());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Peak Connections
				cbuffer.Format(_T("%s: %i"),GetResString(IDS_SF_PEAKCON),theAppPtr->listensocket->GetPeakConnections());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Connect Limit Reached
				uint32 m_itemp = theAppPtr->listensocket->GetMaxConnectionReached();
		...
		{			
			...
			if (forceUpdate || stattree.IsExpanded(hconn_tg)) 
			{
				...
				i++;
				// Average Connections
				cbuffer.Format(_T("%s: %i"), GetResString(IDS_SF_AVGCON), (int) (theAppPtr->listensocket->GetActiveConnections() + thePrefs.GetConnAvgConnections()) / 2 );
				stattree.SetItemText(conn_tg[i], cbuffer);
				i++;
				// Peak Connections
				...
				cbuffer.Format(_T("%s: %i"), GetResString(IDS_SF_MAXCONLIMITREACHED), theAppPtr->listensocket->GetMaxConnectionReached() + thePrefs.GetConnMaxConnLimitReached());
				stattree.SetItemText(conn_tg[i], cbuffer);
				i++;
				...
				if (forceUpdate || stattree.IsExpanded(time_aaph[mx])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hup[mx])) 
					{
						...
						*/
						...
						// Upload Sessions
						uint32 statGoodSessions = (uint32)((theAppPtr->uploadqueue->GetSuccessfullUpCount() + thePrefs.GetUpSuccessfulSessions() + theAppPtr->uploadqueue->GetUploadQueueLength()) * avgModifier[mx]);
						uint32 statBadSessions = (uint32)((theAppPtr->uploadqueue->GetFailedUpCount() + thePrefs.GetUpFailedSessions()) * avgModifier[mx]);
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		theAppPtr->clientlist->GetStatistics(totalclient, myStats, 
...
//Xman Code Improvement:
		...
		*/
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				theAppPtr->clientlist->GetModStatistics(&clientMods);
				// Slugfiller: modid
				...
				theAppPtr->clientlist->ReleaseModStatistics(&clientMods);	//Xman  // Slugfiller: modid
		...
		cbuffer.Format(_T("%s: %u"), GetResString(IDS_BANNED), theAppPtr->clientlist->GetBannedCount());
		...
		if (forceUpdate || stattree.IsExpanded(h_clients))
		{
				...
				if(totalMODs)
				{
					...
					while (!clientMODs.IsEmpty())
					{
						...
						{
							cbuffer.Format(_T("%s: %u (%.1f%%)"), theAppPtr->clientlist->GetMODType(top_pos), dwLastTop, static_cast<double>(100*dwLastTop)/totalMODs);
		...
		if (forceUpdate || stattree.IsExpanded(h_clients))
		{
			...
			if (theAppPtr->ip2country->IsIP2Country())
			{
				if (totalclient)
				{
					...
					while (!clientCountries.IsEmpty())
					{
						...
						if ((dwLastTop = dwCurrTop) != 0)
						{
							cbuffer.Format(_T("%s: %u (%.1f%%)"), theAppPtr->ip2country->GetCountryNameFromRef(iTopIdx,true), dwLastTop, static_cast<double>(100*dwLastTop)/totalclient);
	...
	{		
		...
		float	servocc;
		theAppPtr->serverlist->GetStatus( servtotal, servfail, servuser, servfile, servlowiduser, servtuser, servtfile, servocc);
		...
		cbuffer.Format(_T("%s: %s"),GetResString(IDS_SF_DELCOUNT),CastItoIShort(theAppPtr->serverlist->GetDeletedServerCount()));stattree.SetItemText(srv[2], cbuffer);
	...
	{		
		...
		cbuffer.Format(GetResString(IDS_SHAREDFILESCOUNT),theAppPtr->sharedfiles->GetCount());
		// SLUGFILLER: SafeHash - extra statistics
		if (theAppPtr->sharedfiles->GetHashingCount())
		{
			CString tempbuffer;
			tempbuffer.Format(GetResString(IDS_HASHINGFILESCOUNT),theAppPtr->sharedfiles->GetHashingCount());
			cbuffer += tempbuffer;
		}
		// SLUGFILLER: SafeHash
		...
		uint64 allsize=theAppPtr->sharedfiles->GetDatasize(bytesLargestFile); // Func returns total share size and sets pointeredd uint64 to largest single filesize
		CString cbuffer2;
		if(theAppPtr->sharedfiles->GetCount() != 0)
			cbuffer2.Format( _T("%s"), CastItoXBytes(allsize/(uint64)theAppPtr->sharedfiles->GetCount(), false, false));
		else 
	...
	{			
		...
		int iActiveFiles = theAppPtr->downloadqueue->GetDownloadFilesStats(ui64TotalFileSize, ui64TotalLeftToTransfer, ui64TotalAdditionalNeededSpace);
...
{
	theAppPtr->emuledlg->ShowPreferences(IDD_PPG_STATS);
}
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Statistics);
	return TRUE;
}

HBRUSH CStatisticsDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	HBRUSH hbr = theAppPtr->emuledlg->GetCtlColor(pDC, pWnd, nCtlColor);
	if (hbr)
		return hbr;
