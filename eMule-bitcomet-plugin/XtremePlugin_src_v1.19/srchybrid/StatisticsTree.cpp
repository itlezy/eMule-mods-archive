...
CString CStatisticsTree::GetHTML(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	if (firstItem)
		...
		/*
		strBuffer.Format(_T("<font face=\"Tahoma,Verdana,Courier New,Helvetica\" size=\"2\">\r\n<b>eMule v%s %s [%s]</b>\r\n<br /><br />\r\n"), theAppPtr->m_strCurVersionLong, GetResString(IDS_SF_STATISTICS), thePrefs.GetUserNick());
		*/
		strBuffer.Format(_T("<font face=\"Tahoma,Verdana,Courier New,Helvetica\" size=\"2\">\r\n<b>eMule v%s %s [%s]</b>\r\n<br /><br />\r\n"), theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION, GetResString(IDS_SF_STATISTICS), thePrefs.GetUserNick());
		//Xman end
		...
		case MP_STATTREE_HTMLCOPYSEL:
			{
				...
				if (selectedItem != NULL) {
					...
					if (theHTML.IsEmpty())
						return false;
					theAppPtr->CopyTextToClipboard(theHTML);
					return true;
				}
				return false;
			}
...
CString CStatisticsTree::GetText(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	if (bPrintHeader)
		...
		*/
		strBuffer.Format(_T("eMule v%s %s [%s]\r\n\r\n"), theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION, GetResString(IDS_SF_STATISTICS) ,thePrefs.GetUserNick()); //Xman // Maella -Support for tag ET_MOD_VERSION 0x55
		//Xman end
		...
		case MP_STATTREE_COPYSEL:
			{
				...
				if (selectedItem != NULL) {
					...
					if (theText.IsEmpty())
						return false;
					theAppPtr->CopyTextToClipboard(theText);
					return true;
				}
				return false;
			}
