...
BOOL CTransferWnd::OnInitDialog()
{
	...
	switch (thePrefs.GetTransferWnd1()) {
		default:
		case 0: {
			//zz_fly :: remove redunant code :: DolphinX :: start
			/* //these work is done by ShowSplitWindow()
	...
	if (theAppPtr->IsVistaThemeActive())
...
{
	TCHAR buffer[100];
	_sntprintf(buffer, _countof(buffer), _T("%u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(")"), number, theAppPtr->clientlist->GetBannedCount());
		...
		case IDC_DOWNLOADLIST + IDC_UPLOADLIST: {
			...
			switch (m_uWnd2)
			{
				case wnd2Uploading: {
					...
					//Xman Xtreme Upload
					/*
					uint32 activeCount = theAppPtr->uploadqueue->GetActiveUploadsCount();
					if (activeCount >= itemCount)
						strBuffer.Format(_T(" (%i)"), itemCount);
					else
						strBuffer.Format(_T(" (%i/%i)"), activeCount, itemCount);
					*/
					uint32 activeCount = theAppPtr->uploadBandwidthThrottler->GetNumberOfFullyActivatedSlots();
					//Xman upload health
					//Xman count block/success send
					float health = theAppPtr->uploadBandwidthThrottler->GetAvgHealth();
					float avgblocks = theAppPtr->uploadBandwidthThrottler->GetAvgBlockRatio();
...
{
	CImageList iml;
	iml.Create(16, 16, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 1, 1);
...
void CTransferWnd::OnBnClickedQueueRefreshButton()
{
	...
	/*
	CUpDownClient* update = theAppPtr->uploadqueue->GetNextClient(NULL);

	while( update ){
		theAppPtr->emuledlg->transferwnd->queuelistctrl.RefreshClient( update);
		update = theAppPtr->uploadqueue->GetNextClient(update);
	}
	*/
	if (queuelistctrl.GetItemCount()>1)
	{
		theAppPtr->emuledlg->transferwnd->queuelistctrl.UpdateAll();
	}
	//Xman end
}
...
void CTransferWnd::OnLvnBeginDragDownloadList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (m_pDragImage == NULL)
	{
		...
		m_pDragImage->Create(16, 16, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 0);
	...
	{ 
		case MP_CAT_ADD: {
			...
			if (dialog.DoModal() == IDOK)
			{
				...
				if (CompareDirectories(thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR), thePrefs.GetCatPath(newindex)))
					theAppPtr->emuledlg->sharedfileswnd->Reload();
			}
			else
		...
		case MP_CAT_EDIT: {
			...
			if (dialog.DoModal() == IDOK)
			{
				...
				theAppPtr->emuledlg->searchwnd->UpdateCatTabs();
				theAppPtr->emuledlg->transferwnd->downloadlistctrl.UpdateCurrentCategoryView();
				thePrefs.SaveCats();
		...
		case MP_CAT_REMOVE: {
			...
			theAppPtr->downloadqueue->ResetCatParts(rightclickindex);
		...
		case MP_PAUSE:
			theAppPtr->downloadqueue->SetCatStatus(rightclickindex,MP_PAUSE);
			break;
		case MP_STOP:
			theAppPtr->downloadqueue->SetCatStatus(rightclickindex,MP_STOP);
			break;
		case MP_CANCEL:
			if (AfxMessageBox(GetResString(IDS_Q_CANCELDL),MB_ICONQUESTION|MB_YESNO) == IDYES)
				theAppPtr->downloadqueue->SetCatStatus(rightclickindex,MP_CANCEL);
			break;
		case MP_RESUME:
			theAppPtr->downloadqueue->SetCatStatus(rightclickindex,MP_RESUME);
			break;
		case MP_RESUMENEXT:
			theAppPtr->downloadqueue->StartNextFile(rightclickindex,false);
			break;
		...
		case MP_DOWNLOAD_ALPHABETICAL: {
            ...
            if(newSetting) {
                ...
                theAppPtr->downloadqueue->RemoveAutoPrioInCat(rightclickindex, PR_NORMAL);
            }
            break;
		}
...
void CTransferWnd::EditCatTabLabel(int index,CString newlabel)
{
	...
	if (thePrefs.ShowCatTabInfos()) {
		CPartFile* cur_file;
		count=dwl=0;
		for (int i=0;i<theAppPtr->downloadqueue->GetFileCount();i++) {
			cur_file=theAppPtr->downloadqueue->GetFileByIndex(i);
		...
		theAppPtr->emuledlg->transferwnd->downloadlistctrl.GetCompleteDownloads(index, count);
...
CString CTransferWnd::GetTabStatistic(int tab)
{
	...
	for (int i = 0; i < theAppPtr->downloadqueue->GetFileCount(); i++)
	{
		/*const*/ CPartFile* cur_file = theAppPtr->downloadqueue->GetFileByIndex(i);
		if (cur_file == 0)
			continue;
	...
	int total;
	int compl = theAppPtr->emuledlg->transferwnd->downloadlistctrl.GetCompleteDownloads(tab, total);
...
void CTransferWnd::OnTabMovement(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	theAppPtr->downloadqueue->MoveCat((uint8)from,(uint8)to);
...
void CTransferWnd::ResetTransToolbar(bool bShowToolbar, bool bResetLists)
{
	...
	if (bShowToolbar)
	{
		...
		m_btnWnd1->ModifyStyle((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0, TBSTYLE_TOOLTIPS);
		...
		if (theAppPtr->m_ullComCtrlVer > MAKEDLLVERULL(5,82,0,0)) //zz_fly :: in the case of disable theme service in xp and 2k3
		{
			CSize size;
...
/*---*/
		// Vista: Remove the TBSTYLE_TRANSPARENT to avoid flickering (can be done only after the toolbar was initially created with TBSTYLE_TRANSPARENT !?)
		m_btnWnd2->ModifyStyle((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0, TBSTYLE_TOOLTIPS);
	...
	{
		// Vista: Remove the TBSTYLE_TRANSPARENT to avoid flickering (can be done only after the toolbar was initially created with TBSTYLE_TRANSPARENT !?)
		m_btnWnd1->ModifyStyle(TBSTYLE_TOOLTIPS | ((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0), 0);
		m_btnWnd1->SetExtendedStyle(m_btnWnd1->GetExtendedStyle() & ~TBSTYLE_EX_MIXEDBUTTONS);
		m_btnWnd1->RecalcLayout(true);

		// Vista: Remove the TBSTYLE_TRANSPARENT to avoid flickering (can be done only after the toolbar was initially created with TBSTYLE_TRANSPARENT !?)
		m_btnWnd2->ModifyStyle(TBSTYLE_TOOLTIPS | ((theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6, 16, 0, 0)) ? TBSTYLE_TRANSPARENT : 0), 0);
		m_btnWnd2->SetExtendedStyle(m_btnWnd2->GetExtendedStyle() & ~TBSTYLE_EX_MIXEDBUTTONS);
...
{
	theAppPtr->ShowHelp(eMule_FAQ_GUI_Transfers);
	return TRUE;
}
