...
BOOL CTreePropSheet::OnInitDialog() 
{
	if (m_bTreeViewMode)
	{
		...
		if (m_DefaultImages.GetSafeHandle())
		{
			...
			m_Images.Create(ii.rcImage.right - ii.rcImage.left, ii.rcImage.bottom - ii.rcImage.top, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
		}
		else
			m_Images.Create(16, 16, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
	}
