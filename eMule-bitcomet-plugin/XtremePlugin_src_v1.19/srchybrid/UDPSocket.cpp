...
bool CUDPSocket::Create()
{
	if (thePrefs.GetServerUDPPort())
	{
		...
		if(thePrefs.GetUPnPNat()){
			...
			theAppPtr->AddUPnPNatPort(&mapping, thePrefs.GetUPnPNatTryRandom());
		}
		*/
		//ACAT UPnP end
		return true;
...
void CUDPSocket::OnReceive(int nErrorCode)
{
	...
	if (length != SOCKET_ERROR)
	{
		...
		if(length > 0){ 		
			theAppPtr->pBandWidthControl->AddeMuleInUDPOverall(length);
		}
		// Maella end

		int nPayLoadLen = length;
		CServer* pServer = theAppPtr->serverlist->GetServerByIPUDP(sockAddr.sin_addr.S_un.S_addr, ntohs(sockAddr.sin_port), true);
...
bool CUDPSocket::ProcessPacket(const BYTE* packet, UINT size, UINT opcode, uint32 nIP, uint16 nUDPPort)
{
	try
	{
		...
		CServer* pServer = theAppPtr->serverlist->GetServerByIPUDP(nIP, nUDPPort, true);
		if (pServer) {
...
#endif //zz_fly :: DummyCut
					UINT uResultCount = theAppPtr->searchlist->ProcessUDPSearchAnswer(data, true/*pServer->GetUnicodeSupport()*/, nIP, nUDPPort-4);
					theAppPtr->emuledlg->searchwnd->AddGlobalEd2kSearchResults(uResultCount);
...
#endif //zz_fly :: DummyCut
						theAppPtr->serverconnect->SendUDPPacket(packet, pServer, true);
					}
					else
					{
...
void CUDPSocket::ProcessPacketError(UINT size, UINT opcode, uint32 nIP, uint16 nUDPPort, LPCTSTR pszError)
{
	...
	{
		CString strName;
		CServer* pServer = theAppPtr->serverlist->GetServerByIPUDP(nIP, nUDPPort);
		if (pServer)
...
void CUDPSocket::DnsLookupDone(WPARAM wp, LPARAM lp)
{
	...
	if (nIP != INADDR_NONE)
	{
		...
		if (!bRemoveServer && thePrefs.GetFilterServerByIP() && theAppPtr->ipfilter->IsFiltered(nIP)) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(UDP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pDNSReq->m_pServer->GetAddress(), ipstr(nIP), theAppPtr->ipfilter->GetLastHit());
			bRemoveServer = true;
		}

		CServer* pServer = theAppPtr->serverlist->GetServerByAddress(pDNSReq->m_pServer->GetAddress(), pDNSReq->m_pServer->GetPort());
		if (pServer) {
			pServer->SetIP(nIP);
...
int CUDPSocket::SendTo(BYTE* lpBuf, int nBufLen, uint32 dwIP, uint16 nPort)
{
	...
	if (iResult == SOCKET_ERROR) {
		...
		if (dwError == WSAEWOULDBLOCK) {
			...
			if (thePrefs.GetVerbose())
				theAppPtr->QueueDebugLogLine(false, _T("Error: Server UDP socket: Failed to send packet to %s:%u - %s"), ipstr(dwIP), nPort, GetErrorMessage(dwError, 1));
			return 0; // error
		}
	}
	return 1; // success
