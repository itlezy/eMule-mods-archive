...
//	//bool isready=false;
//
//	if(oldsocket==NULL)theAppPtr->QueueLogLine(false,_T("oldsocket NULL"));
//	if(newsocket==NULL) theAppPtr->QueueLogLine(false,_T("newsocket NULL"));
//
...
//		theAppPtr->QueueLogLine(false,_T("->replaced socket on pos: %u isfull: %u "),slotnumber, isfull);
//	}
//	else
//	{
//		/*
...
//		recalculate=true;
//		*/
//		theAppPtr->QueueLogLine(false,_T("-->tried to replace a not existing socket"));
//	}
//
//	sendLocker.Unlock();
//}
...
bool UploadBandwidthThrottler::ReplaceSocket(ThrottledFileSocket* normalsocket, ThrottledFileSocket* pcsocket, ThrottledFileSocket* newsocket)
{
	...
	if(newsocket==NULL)
	{
		theAppPtr->QueueDebugLogLine(false, _T("ReplaceSocket-> NULL socket!"));
		sendLocker.Unlock();
		return false;
	}
...
bool UploadBandwidthThrottler::RemoveFromStandardListNoLock(ThrottledFileSocket* socket) {
	...
	if(pos!=NULL)
	{
		...
		if(m_StandardOrder_list.GetSize())
		{
			...
			theAppPtr->pBandWidthControl->GetDatarates(20,
				notused, notused,
			...
			uint32 realallowedDatarate = (uint32)(theAppPtr->pBandWidthControl->GetMaxUpload()*1024 - m_AvgOverhead);
			//Xman 4.6
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	uint64  m_lastOverallSentBytes = theAppPtr->pBandWidthControl->GeteMuleOutOverall();
    uint64  m_lastNetworkOut = theAppPtr->pBandWidthControl->GetNetworkOut();
...
#define TIME_BETWEEN_UPLOAD_LOOPS_MAX 50 //25
	   ...
	   if(timeSinceLastLoop < sleeptime) 
	   {
		...
		{	continue; //shouldn't happen
			//theAppPtr->QueueDebugLogLine(false,_T("UploadBandwidthThrottler: Application to fast"));
		}
		if(timeSinceLastLoop>1000)
		{
			timeSinceLastLoop=1000;	//compensate to 1 sec
			//theAppPtr->QueueDebugLogLine(false,_T("UploadBandwidthThrottler: Application hang"));
		}

		// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		theAppPtr->pBandWidthControl->Process(); 

		// Get current speed from prefs
		uint32 old_value;
		old_value=allowedDataRate;
		allowedDataRate = (uint32)(theAppPtr->pBandWidthControl->GetMaxUpload()*1024); 
		...
		m_currentOverallSentBytes=theAppPtr->pBandWidthControl->GeteMuleOutOverall();
		m_currentNetworkOut=theAppPtr->pBandWidthControl->GetNetworkOut();
		...
		{
			theAppPtr->QueueDebugLogLine(false, _T("Warning m_highestNumberOfFullyActivatedSlots not equal list"));
			recalculate=true;
		}
		...
		else
		{
			...
			for(uint16 i=m_highestNumberOfFullyActivatedSlots;i<m_StandardOrder_list.GetSize();i++)
			{
				...
				if(socket!=NULL && socket->isready==true) 
				{
					...
					{
						theAppPtr->QueueDebugLogLine(false, _T("Warning trickle on wrong possition"));
						recalculate=true;
					}
		...
		*/
		...
		if(uSlope>0)
		{
			...
			while(uSlope>0 && k < m_StandardOrder_list_full.GetCount())
			{
				...
				if(socket!=NULL  && socket->isready==true) 
				{
					...
					{
						theAppPtr->QueueDebugLogLine(false, _T("Warning full on wrong possition"));
						recalculate=true;
					}
		...
		// - Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		theAppPtr->pBandWidthControl->AddeMuleOutOverallNoHeader(spentBytes);
		theAppPtr->pBandWidthControl->AddeMuleOut(spentBytes-spentOverhead);
