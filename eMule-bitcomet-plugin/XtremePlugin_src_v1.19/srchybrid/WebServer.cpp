...
{
	CServer* server=theAppPtr->serverlist->GetServerByAddress(sIP, (uint16)nPort);
	if (server!=NULL) 
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_SERVER_REMOVE, (LPARAM)server);
}
...
void CWebServer::_AddToStatic(CString sIP, int nPort)
{
	CServer* server=theAppPtr->serverlist->GetServerByAddress(sIP, (uint16)nPort);
	if (server!=NULL) 
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_ADD_TO_STATIC, (LPARAM)server);
}
...
void CWebServer::_RemoveFromStatic(CString sIP, int nPort)
{
	CServer* server=theAppPtr->serverlist->GetServerByAddress(sIP, (uint16)nPort);
	if (server!=NULL) 
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_REMOVE_FROM_STATIC, (LPARAM)server);
}
...
void CWebServer::_ConnectToServer(CString sIP, int nPort)
{
	CServer* server=theAppPtr->serverlist->GetServerByAddress(sIP, (uint16)nPort);
	if (server!=NULL) 
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_CONNECTTOSERVER,(LPARAM)server);
}
...
void CWebServer::ProcessURL(ThreadData Data)
{
	...
	if (_ParseURL(Data.sURL, _T("w")) == _T("password"))
	{
		...
		if(MD5Sum(_ParseURL(Data.sURL, _T("p"))).GetHash() == thePrefs.GetWSPass())
		{
			...
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_UPDATEMYINFO,0);
	...
	if(_IsLoggedIn(Data, lSession))
	{
		if (_ParseURL(Data.sURL, _T("w")) == _T("close") && IsSessionAdmin(Data,sSession) && thePrefs.GetWebAdminAllowedHiLevFunc() )
		{
			...
			SendMessage(theAppPtr->emuledlg->m_hWnd,WM_CLOSE,0,0);
		...
		else if (_ParseURL(Data.sURL, _T("w")) == _T("shutdown") && IsSessionAdmin(Data,sSession))
		{
			...
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_WINFUNC,1);
		...
		else if (_ParseURL(Data.sURL, _T("w")) == _T("reboot") && IsSessionAdmin(Data,sSession))
		{
			...
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_WINFUNC,2);
		...
		else if (_ParseURL(Data.sURL, _T("w")) == _T("commentlist"))
		{
			...
			uchar FileHash[16];
			CKnownFile* kf=theAppPtr->sharedfiles->GetFileByID(_GetFileHash(_ParseURL(Data.sURL, _T("filehash")),FileHash) );
...
//	Auto-refresh code
	...
	strVersionCheck.Format(_T("/en/version_check.php?version=%i&language=%i"),theAppPtr->m_uCurVersionCheck,thePrefs.GetLanguageID());
	...
	/*
	Out.Replace(_T("[version]"), theAppPtr->m_strCurVersionLong);
	*/
	Out.Replace(_T("[version]"), theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION);
	//Xman end
	...
	if ((theAppPtr->serverconnect->IsConnecting() && !disconnectissued) || connectissued)
	{
		HTTPConState = _T("connecting");
		HTTPConText = _GetPlainResString(IDS_CONNECTING);
	}
	else if (theAppPtr->serverconnect->IsConnected() && !disconnectissued)
	{
		if(!theAppPtr->serverconnect->IsLowID())
			HTTPConState = _T("high");
		else
			HTTPConState = _T("low");
		CServer* cur_server = theAppPtr->serverlist->GetServerByAddress(
			theAppPtr->serverconnect->GetCurrentServer()->GetAddress(),
			theAppPtr->serverconnect->GetCurrentServer()->GetPort() );
	...
	uint32 allUsers = 0;
	uint32 allFiles = 0;
	for (uint32 sc=0;sc<theAppPtr->serverlist->GetServerCount();sc++)
	{
		CServer* cur_server = theAppPtr->serverlist->GetServerAt(sc);
	...
	if(thePrefs.GetMaxUpload() == UNLIMITED)
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * theAppPtr->uploadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxGraphUploadRate(true));
	else
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * theAppPtr->uploadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxUpload());
	Out.Replace(_T("[UploadValue]"), HTTPHeader);

	if(thePrefs.GetMaxDownload() == UNLIMITED)
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * theAppPtr->downloadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxGraphDownloadRate());
	else
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * theAppPtr->downloadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxDownload());
	Out.Replace(_T("[DownloadValue]"), HTTPHeader);

	_stprintf(HTTPHeader, _T("%.1f"), (static_cast<double>(theAppPtr->listensocket->GetOpenSockets()))/(thePrefs.GetMaxConnections())*100.0);
	Out.Replace(_T("[ConnectionValue]"), HTTPHeader);
	_stprintf(HTTPHeader, _T("%.1f"), (static_cast<double>(theAppPtr->uploadqueue->GetDatarate())/1024.0));
	Out.Replace(_T("[CurUpload]"), HTTPHeader);
	_stprintf(HTTPHeader, _T("%.1f"), (static_cast<double>(theAppPtr->downloadqueue->GetDatarate())/1024.0));
	Out.Replace(_T("[CurDownload]"), HTTPHeader);
	_stprintf(HTTPHeader, _T("%.0f"), (static_cast<double>(theAppPtr->listensocket->GetOpenSockets())));
	...
	*/
	...
	if(thePrefs.GetMaxUpload() == UNLIMITED)
		...
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * eMuleOut) / 1024 / thePrefs.GetMaxUpload());
	Out.Replace(_T("[UploadValue]"), HTTPHeader);

	if(thePrefs.GetMaxDownload() == UNLIMITED)
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * eMuleIn) / 1024 / thePrefs.GetMaxGraphDownloadRate());
	else
		_stprintf(HTTPHeader, _T("%.1f"), static_cast<double>(100 * eMuleIn) / 1024 / thePrefs.GetMaxDownload());
	Out.Replace(_T("[DownloadValue]"), HTTPHeader);
...
CString CWebServer::_GetServerList(ThreadData Data)
{
	...
	{
		if(sIP.IsEmpty())
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_CONNECTTOSERVER,0);
		else
	...
	else if (bAdmin && sCmd.CompareNoCase(_T("disconnect")) == 0)
	{
		...
		if (theAppPtr->serverconnect->IsConnecting())
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_STOPCONNECTING,NULL);
		else
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_DISCONNECT,1);
	}
	...
	else if (bAdmin && sCmd.CompareNoCase(_T("priolow")) == 0)
	{
		if(!sIP.IsEmpty())
		{
			...
			if (server) {
				...
				SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_UPDATESERVER,(LPARAM)server);
			}
		}
	}
	...
	for (uint32 sc=0;sc<theAppPtr->serverlist->GetServerCount();sc++)
	{
		CServer* cur_file = theAppPtr->serverlist->GetServerAt(sc);
		ServerEntry Entry;
		...
		{
			tempServer = theAppPtr->serverconnect->GetCurrentServer();
...
CString CWebServer::_GetTransferList(ThreadData Data)
{
	...
	if (bAdmin && !sClear.IsEmpty())
	{
		...
		{
			theAppPtr->emuledlg->SendMessage(WEB_CLEAR_COMPLETED, (WPARAM)0, (LPARAM)cat );
		}
		else
		{
			uchar	FileHash[16];
			...
			if (!sClear.IsEmpty())
			{
				...
				theAppPtr->emuledlg->SendMessage(WEB_CLEAR_COMPLETED, (WPARAM)1, reinterpret_cast<LPARAM>(pFileHash));
			}
		}
	}
	...
	if (bAdmin && !HTTPTemp.IsEmpty())
		theAppPtr->emuledlg->SendMessage(WEB_ADDDOWNLOADS, (WPARAM)(LPCTSTR)HTTPTemp, cat);
	...
	else if (HTTPTemp.CompareNoCase(_T("menuprio")) == 0)
	{
		if(bAdmin)
		{
			...
			SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_CATPRIO, (WPARAM)cat,(LPARAM)prio);
		}
	}
	...
	if (bAdmin)
	{
		...
		if (!sOp.IsEmpty())
		{
			...
			if (!sFile.IsEmpty())
			{
				CPartFile *found_file = theAppPtr->downloadqueue->GetFileByID(_GetFileHash(sFile, FileHash));
				if(found_file)
				{	// SyruS all actions require a found file (removed double-check inside)
					...
					else if (sOp.CompareNoCase(_T("cancel")) == 0)
					{
						...
						SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_UPD_CATTABS,0);
					}
					...
					else if (sOp.CompareNoCase(_T("rename")) == 0)
					{
						...
						theAppPtr->emuledlg->SendMessage(WEB_FILE_RENAME, (WPARAM)found_file, (LPARAM)(LPCTSTR)sNewName);
					}
			...
			else
			{
				...
				{
					CUpDownClient* cur_client = theAppPtr->clientlist->FindClientByUserHash(UserHash);
					if(cur_client)
					{
						if (sOp.CompareNoCase(_T("addfriend")) == 0)
							SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_ADDREMOVEFRIEND,(WPARAM)cur_client,(LPARAM)1);
						else if (sOp.CompareNoCase(_T("removefriend")) == 0) {
							CFriend* f=theAppPtr->friendlist->SearchFriend(UserHash,0,0);
							if (f)
								SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_ADDREMOVEFRIEND,(WPARAM)f,(LPARAM)0);
						}
					}
				}
			}
	...
	theAppPtr->emuledlg->transferwnd->downloadlistctrl.GetDisplayedFiles(&partlist);
	...
	for (int i=0;i<partlist.GetCount();i++) {
		...
		if (pPartFile)
		{
			...
			*/
			...
			dFile.sED2kLink = theAppPtr->CreateED2kSourceLink(pPartFile);
			else
...
//	uint32	dwClientSoft;
	...
	for (POSITION pos = theAppPtr->uploadqueue->GetFirstFromUploadList();
		pos != 0;theAppPtr->uploadqueue->GetNextFromUploadList(pos))
	{
		cur_client = theAppPtr->uploadqueue->GetQueueClientAt(pos);
		...
		*/
		...
		CKnownFile* file = theAppPtr->sharedfiles->GetFileByID(cur_client->GetUploadFileID() );
		if (file)
...
CString CWebServer::_CreateTransferList(CString Out, CWebServer *pThis, ThreadData* Data, void* _FilesArray, void* _UploadArray, bool bAdmin) {
	...
	for (POSITION pos = theAppPtr->uploadqueue->waitinglist.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = theAppPtr->uploadqueue->waitinglist.GetNext(pos);
		QueueUsers dUser;
		CString sTemp;
	...
	if (theAppPtr->uploadqueue->waitinglist.GetHeadPosition() != 0)
	{
	...
	for(int i = 0; i < FilesArray->GetCount(); i++)
	{
		...
		//CPartFile *found_file = theAppPtr->downloadqueue->GetFileByID(_GetFileHash(dwnlf.sFileHash, FileHashA4AF));
...
CString CWebServer::_GetSharedFilesList(ThreadData Data)
{
	...
	if(!_ParseURL(Data.sURL, _T("hash")).IsEmpty() && !_ParseURL(Data.sURL, _T("prio")).IsEmpty() && IsSessionAdmin(Data,sSession))
	{
		...
		{
			cur_file = theAppPtr->sharedfiles->GetFileByID(fileid);
			...
			if (cur_file != 0)
			{
				...
				SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_UPD_SFUPDATE, (LPARAM)cur_file);
			}
		}
	}
	...
	if(_ParseURL(Data.sURL, _T("reload")) == _T("true"))
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_SHARED_FILES_RELOAD,0);
	...
	if(_ParseURL(Data.sURL, _T("reload")) == _T("true"))
	{
		CString strResultLog = _SpecialChars(theAppPtr->emuledlg->GetLastLogEntry() );	//Pick-up last line of the log
	...
	// Populating array
	for (int ix=0;ix<theAppPtr->sharedfiles->GetCount();ix++)
	{
		CCKey bufKey;
		CKnownFile* cur_file = theAppPtr->sharedfiles->GetFileByIndex(ix);
...
//		uint16 nCountLo, nCountHi;
		...
		if (theAppPtr->serverconnect->IsConnected())
			dFile.sED2kLink = theAppPtr->CreateED2kSourceLink(cur_file);
		else
	...
	for(int i = 0; i < SharedArray.GetCount(); i++)
	{
		...
		if (hash.GetLength()==32 && DecodeBase16(hash, hash.GetLength(), fileid, ARRSIZE(fileid)))
		{
			...
			CKnownFile* cur_file = theAppPtr->sharedfiles->GetFileByID(fileid);
			if (cur_file != NULL)
			{
...
CString CWebServer::_GetAddServerBox(ThreadData Data)
{
	...
	CString resultlog = _SpecialChars(theAppPtr->emuledlg->GetLastLogEntry() ); //Pick-up last line of the log
	...
	if(_ParseURL(Data.sURL, _T("addserver")) == _T("true"))
	{
		...
		if (!strServerAddress.IsEmpty() && !strServerPort.IsEmpty())
		{
			...
			if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(nsrv,true)) {
				delete nsrv;
				...
				SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_UPDATESERVER,(LPARAM)nsrv);
				...
				if(_ParseURL(Data.sURL, _T("addtostatic")) == _T("true"))
				{
					...
					resultlog += _T("<br />");
					resultlog += _SpecialChars(theAppPtr->emuledlg->GetLastLogEntry() ); //Pick-up last line of the log
				}
	...
	else if(_ParseURL(Data.sURL, _T("updateservermetfromurl")) == _T("true"))
	{
		...
		SendMessage(theAppPtr->emuledlg->m_hWnd, WEB_GUI_INTERACTION, WEBGUIIA_UPDATESERVERMETFROMURL, (LPARAM)urlbuf);

		CString resultlog = _SpecialChars(theAppPtr->emuledlg->GetLastLogEntry() );
		resultlog = resultlog.TrimRight(_T('\n'));
		resultlog = resultlog.Mid(resultlog.ReverseFind(_T('\n')));
		Out.Replace(_T("[Message]"),resultlog);
	}
...
CString CWebServer::_GetLog(ThreadData Data)
{
	...
	if (_ParseURL(Data.sURL, _T("clear")) == _T("yes") && IsSessionAdmin(Data,sSession))
		theAppPtr->emuledlg->ResetLog();

	Out.Replace(_T("[Clear]"), _GetPlainResString(IDS_PW_RESET));
	Out.Replace(_T("[Log]"), _SpecialChars(theAppPtr->emuledlg->GetAllLogEntries(),false)+ _T("<br /><a name=\"end\"></a>") );
	Out.Replace(_T("[Session]"), sSession);
...
CString CWebServer::_GetServerInfo(ThreadData Data)
{
	...
	if (_ParseURL(Data.sURL, _T("clear")) == _T("yes") && IsSessionAdmin(Data,sSession))
		theAppPtr->emuledlg->ResetServerInfo();

	Out.Replace(_T("[Clear]"), _GetPlainResString(IDS_PW_RESET));
	Out.Replace(_T("[ServerInfo]"), _SpecialChars(theAppPtr->emuledlg->GetServerInfoText(),false )+ _T("<br /><a name=\"end\"></a>") );
	Out.Replace(_T("[Session]"), sSession);
...
CString CWebServer::_GetDebugLog(ThreadData Data)
{
	...
	if (_ParseURL(Data.sURL, _T("clear")) == _T("yes") && IsSessionAdmin(Data,sSession))
		theAppPtr->emuledlg->ResetDebugLog();

	Out.Replace(_T("[Clear]"), _GetPlainResString(IDS_PW_RESET));
	Out.Replace(_T("[DebugLog]"), _SpecialChars(theAppPtr->emuledlg->GetAllDebugLogEntries() ,false)+ _T("<br /><a name=\"end\"></a>") );
	Out.Replace(_T("[Session]"), sSession);
...
CString CWebServer::_GetMyInfo(ThreadData Data)
{
	...
	Out.Replace(_T("[MYINFOLOG]"), theAppPtr->emuledlg->serverwnd->GetMyInfoString() );
...
CString CWebServer::_GetKadDlg(ThreadData Data)
{
	...
	if (_ParseURL(Data.sURL, _T("bootstrap")) != _T("") && IsSessionAdmin(Data,sSession) ) {
		...
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_KAD_BOOTSTRAP, (LPARAM)ipbuf );
	}

	if (_ParseURL(Data.sURL, _T("c")) == _T("connect") && IsSessionAdmin(Data,sSession) ) {
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_KAD_START, 0 );
	}

	if (_ParseURL(Data.sURL, _T("c")) == _T("disconnect") && IsSessionAdmin(Data,sSession) ) {
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_KAD_STOP, 0 );
	}
	if (_ParseURL(Data.sURL, _T("c")) == _T("rcfirewall") && IsSessionAdmin(Data,sSession) ) {
		SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION,WEBGUIIA_KAD_RCFW, 0 );
	}
	...
	// numbers
	buffer.Format(_T("%i<br />%i"),	theAppPtr->emuledlg->kademliawnd->GetContactCount(), 
									theAppPtr->emuledlg->kademliawnd->searchList->GetItemCount());
...
CString CWebServer::_GetStats(ThreadData Data)
{
	...
	// refresh statistics
	SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_GUI_INTERACTION, WEBGUIIA_SHOWSTATISTICS, 1);

	CString Out = pThis->m_Templates.sStats;
	// eklmn: new stats
	Out.Replace(_T("[Stats]"), theAppPtr->emuledlg->statisticswnd->stattree.GetHTMLForExport());
...
CString CWebServer::_GetDownloadGraph(ThreadData Data, CString filehash)
{
	...
	pPartFile = theAppPtr->downloadqueue->GetFileByID(fileid);
...
CString	CWebServer::_GetSearch(ThreadData Data)
{
	...
	if (!_ParseURL(Data.sURL, _T("downloads")).IsEmpty() && IsSessionAdmin(Data,sSession))
	{
		...
		while (!resToken.IsEmpty())
		{
			if (resToken.GetLength()==32)
				SendMessage(theAppPtr->emuledlg->m_hWnd,WEB_ADDDOWNLOADS, (LPARAM)(LPCTSTR)resToken, cat);
			resToken= downloads.Tokenize(_T("|"),curPos);
		}
	}
	...
	if(_ParseURL(Data.sURL, _T("tosearch")) != _T("") && IsSessionAdmin(Data,sSession) )
	{
		...
		try
		{
			if (pParams->eType != SearchTypeKademlia){
				if (!theAppPtr->emuledlg->searchwnd->DoNewEd2kSearch(pParams)){
					delete pParams;
				...
				if (!theAppPtr->emuledlg->searchwnd->DoNewKadSearch(pParams)){
					delete pParams;
	...
	theAppPtr->searchlist->GetWebList(&SearchFileArray, pThis->m_iSearchSortby);
	...
	for (uint16 i = 0; i < SearchFileArray.GetCount(); ++i)
	{
		...
		if (theAppPtr->downloadqueue->GetFileByID(aFileHash)!=NULL) {
				nBlue  = 128;
				nGreen = 128;
		} else {
			sameFile = theAppPtr->sharedfiles->GetFileByID(aFileHash);
			if (sameFile == NULL)
				sameFile = theAppPtr->knownfiles->FindKnownFileByID(aFileHash);
...
//	For each user category index...
	for (int i = 0; i <thePrefs.GetCatCount(); i++)
	{
		...
		if (!sFileHash.IsEmpty())
			found_file=theAppPtr->downloadqueue->GetFileByID(_GetFileHash(sFileHash, FileHash));
...
CString CWebServer::_GetRemoteLinkAddedOk(ThreadData Data)
{
	...
	theAppPtr->emuledlg->SendMessage(WEB_ADDDOWNLOADS, (WPARAM)buf, cat);
...
CString CWebServer::GetClientSummary(CUpDownClient* client) {
	...
	CKnownFile* file = theAppPtr->sharedfiles->GetFileByID(client->GetUploadFileID() );
	ASSERT(file);
	if (file) {
...
CString CWebServer::_GetCommentlist(ThreadData Data)
{
	...
	uchar FileHash[16];
	CPartFile* pPartFile=theAppPtr->downloadqueue->GetFileByID(_GetFileHash(_ParseURL(Data.sURL, _T("filehash")),FileHash) );
