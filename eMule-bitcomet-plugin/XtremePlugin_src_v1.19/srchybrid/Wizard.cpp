...
void CConnectionWizardDlg::OnBnClickedApply()
{
	...
	*/
	...
	if (upload > 0 && download > 0)
	{
		...
		theAppPtr->emuledlg->statisticswnd->SetARange(false, thePrefs.maxGraphUploadRate);
		theAppPtr->emuledlg->statisticswnd->SetARange(true, thePrefs.maxGraphDownloadRate);
		*/
		thePrefs.maxGraphDownloadRate = download;
		thePrefs.maxGraphUploadRate = upload;
		//Xman 6.0.1 better newbie settings:
		thePrefs.SetMaxUpload(theAppPtr->emuledlg->GetRecMaxUpload());
		...
		theAppPtr->emuledlg->statisticswnd->SetARange(false, (int)thePrefs.maxGraphUploadRate);
		theAppPtr->emuledlg->statisticswnd->SetARange(true, (int)thePrefs.maxGraphDownloadRate);
		//Xman end
	...
	theAppPtr->emuledlg->preferenceswnd->m_wndConnection.LoadSettings();
	theAppPtr->emuledlg->Localize(); //Xman dirty hack to update systemmenu
	CDialog::OnOK();
}
...
BOOL CConnectionWizardDlg::OnInitDialog()
{
	...
	SetIcon(m_icnWnd = theAppPtr->LoadIcon(_T("Wizard")), FALSE);
