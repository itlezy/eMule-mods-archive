...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	//	if (!bStartMinimized)
	//		bStartMinimized = theAppPtr->DidWeAutoStart();
			...
			if (pLink)
			{
				if (pLink->GetKind() == CED2KLink::kFile)
				{
					...
					if ( askIfAlreadyDownloaded )
					{
						...
						theAppPtr->downloadqueue->AddFileLinkToDownload(pLink->GetFileLink(),cat);
					//Xman end
				}
				else
				{
...
void CemuleApp::CreateAllFonts()
{
	...
	//
	LOGFONT lfDefault;
	m_pMainWnd->GetFont()->GetLogFont(&lfDefault);
	...
	if (plfHyperText==NULL || plfHyperText->lfFaceName[0]==_T('\0') || !m_fontHyperText.CreateFontIndirect(plfHyperText))
		//CreatePointFont(m_fontHyperText, 10 * 10, _T("MS Shell Dlg")); //Xtreme 6.1
...
//	m_fontLog.Attach(hFontMono);
	...
	// systems.
	//
	//CreatePointFont(m_fontChatEdit, 11 * 10, _T("MS Shell Dlg")); //Xtreme 6.1
...
void CemuleApp::ResetStandByIdleTimer()
{
	...
	*/
	...
	uint32 notUsed;
	theAppPtr->pBandWidthControl->GetDatarates(thePrefs.GetDatarateSamples(),
		eMuleIn, notUsed, notUsed, notUsed, notUsed, notUsed);

	if (IsConnected() || (uploadqueue != NULL && uploadqueue->GetUploadQueueLength() > 0)
		  || eMuleIn>0 ) //thanks Bruno Cabral and Stulle
	//Xman end
	{
...
bool CemuleApp::IsXPThemeActive() const
{
	...
	return theAppPtr->m_ullComCtrlVer < MAKEDLLVERULL(6,16,0,0) && g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed();
}
