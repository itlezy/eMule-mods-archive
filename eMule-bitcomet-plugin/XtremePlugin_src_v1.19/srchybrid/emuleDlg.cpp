...
#include "DllWrapper/ExceptionDebugger/InterfaceExceptionDebugger.h"

#define MAIN_DLG_TITLE_PREFIX _T("eMule[bcplugin]")
...
BEGIN_MESSAGE_MAP(CemuleDlg, CTrayDialog)
	...
	// UPnP
	ON_MESSAGE(UM_UPNP_RESULT, OnUPnPResult) //Official UPNP
	...
	: CTrayDialog(CemuleDlg::IDD, pParent)
{
	...
	m_hIcon = NULL;
	theAppPtr->m_app_state = APP_STATE_RUNNING;
	ready = false; 
...
// [BCPlugin]
void DialogCreateIndirect(CDialog *pWnd, UINT uID, CWnd* pParentWnd)
{
#if 0
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	if (!m_bStartMinimized)
		m_bStartMinimized = theAppPtr->DidWeAutoStart();
	...
	*/
	...
	theAppPtr->CreateAllFonts();
	theAppPtr->CreateBackwardDiagonalBrush();
	...
	/*
	SetWindowText(_T("eMule v") + theAppPtr->m_strCurVersionLong);
	*/
	//SetWindowText(_T("eMule v") + theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION);
	//Xman end

	// [BCPlugin]
	SetWindowText( MAIN_DLG_TITLE_PREFIX _T(" v") + theAppPtr->m_strCurVersionLong);
	...
	DialogCreateIndirect(transferwnd, IDD_TRANSFER, this);
	...
	// [BCPlugin] Do not need show this dialog if invisible
	if(IsWindowVisible())
		SetWindowPlacement(&wp);

	if (thePrefs.GetWSIsEnabled())
		theAppPtr->webserver->StartServer();
	theAppPtr->mmserver->Init();
	...
	// initalize PeerCache
	theAppPtr->m_pPeerCache->Init(thePrefs.GetPeerCacheLastSearch(), thePrefs.WasPeerCacheFound(), thePrefs.IsPeerCacheDownloadEnabled(), thePrefs.GetPeerCachePort());
	...
	*/
	...
	// [BCPlugin] avoid set focus when current window is not visible
...
{
	// [BCPlugin] timer callback wrapper
	__if_exists(Core_Common::InterfaceExceptionDebugger)
	{
		__try
		{
			return CemuleDlg::StartupTimerImpl(0, 0, 0, 0);
		}
	...
	__if_not_exists(Core_Common::InterfaceExceptionDebugger)
	{
		return CemuleDlg::StartupTimerImpl(0, 0, 0, 0);
	}
}

void CALLBACK CemuleDlg::StartupTimerImpl(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
	// [BCPlugin] timer callback impl
#ifdef VERSION_DLL
	...
	if (!::IsWindow(theAppPtr->emuledlg->m_hWnd))
		return;
	if (!::IsWindow(theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl.m_hWnd))
		return;
	if (!::IsWindow(theAppPtr->emuledlg->serverwnd->serverlistctrl.m_hWnd))
		return;
	if (!::IsWindow(theAppPtr->emuledlg->transferwnd->downloadlistctrl.m_hWnd))
		return;
	//Xman end
	// NOTE: Always handle all type of MFC exceptions in TimerProcs - otherwise we'll get mem leaks
	try
	{
		switch(theAppPtr->emuledlg->status){
			case 0:
				theAppPtr->emuledlg->status++;
				theAppPtr->emuledlg->ready = true;
				//Xman
				// SLUGFILLER: SafeHash remove - moved down
				/*
				theAppPtr->sharedfiles->SetOutputCtrl(&theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl);
				*/
				//Xman end
				theAppPtr->emuledlg->status++;
				break;
			...
			case 4:{
				bool bError = false;
				theAppPtr->emuledlg->status++;
				// [BCPlugin] bc plugin not show splash window
				//theAppPtr->UpdateSplash(_T("initializing files to download ...")); //Xman new slpash-screen arrangement
				...
				if (!theAppPtr->listensocket->StartListening()) {
					CString strError;
					strError.Format(GetResString(IDS_MAIN_SOCKETERROR), thePrefs.GetPort());
					LogError(LOG_STATUSBAR, _T("%s"), strError);
					if (thePrefs.GetNotifierOnImportantError())
						theAppPtr->emuledlg->ShowNotifier(strError, TBN_IMPORTANTEVENT);
					bError = true;
				}
				if (!theAppPtr->clientudp->Create()) {
					CString strError;
				...
				{//Xman
					...
					AddLogLine(false,_T("Winsock: Version %d.%d [%.40s] %.40s"), HIBYTE( theAppPtr->m_wsaData.wVersion ),LOBYTE(theAppPtr->m_wsaData.wVersion ),
						CString(theAppPtr->m_wsaData.szDescription), CString(theAppPtr->m_wsaData.szSystemStatus));
					if (theAppPtr->m_wsaData.iMaxSockets!=0)
						AddLogLine(false,_T("Winsock: max. sockets %d"), theAppPtr->m_wsaData.iMaxSockets);
					else
					...
					/*
					AddLogLine(true, GetResString(IDS_MAIN_READY), theAppPtr->m_strCurVersionLong);
					*/
					AddLogLine(true, GetResString(IDS_MAIN_READY),theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION); 
					//Xman end
				//Xman
				}
				// SLUGFILLER: SafeHash remove - moved down
				...
				if (thePrefs.DoAutoConnect())
					theAppPtr->emuledlg->OnBnClickedButton2();
				*/
				//Xman end
				break;
			}
			case 5:
				if (thePrefs.IsStoringSearchesEnabled())
					theAppPtr->searchlist->LoadSearches();
				theAppPtr->emuledlg->status++;
				break;
			//Xman
			...
			case 6:
				...
				theAppPtr->m_AICH_Is_synchronizing=true;
				theAppPtr->sharedfiles->SetOutputCtrl(&theAppPtr->emuledlg->sharedfileswnd->sharedfilesctrl);
				theAppPtr->emuledlg->sharedfileswnd->historylistctrl.Init(); //Xman [MoNKi: -Downloaded History-]

				// BEGIN SiRoB: SafeHash fix originaly in OnInitDialog (delay load shared files)
				// start aichsyncthread
				// [BCPlugin] bc plugin not show splash window
				//theAppPtr->UpdateSplash(_T("Synchronize AICH-Hashes...")); //Xman new slpash-screen arrangement
				AfxBeginThread(RUNTIME_CLASS(CAICHSyncThread), THREAD_PRIORITY_BELOW_NORMAL,0);
				// END SiRoB: SafeHash
				theAppPtr->emuledlg->status++;
				break;
			...
			default:
				//Xman
				// BEGIN SLUGFILLER: SafeHash
				theAppPtr->emuledlg->status = 255;
				// [BCPlugin] bc plugin not show splash window
				//theAppPtr->UpdateSplash(_T("Ready")); //Xman new slpash-screen arrangement
				...
				if (thePrefs.IsFirstStart())
				{
					...
					// [BCPlugin] do not display wizard any more...
					//if (FirstTimeWizard()){
					//	// start connection wizard
					//	CConnectionWizardDlg conWizard;
					//	conWizard.DoModal();
...
void CemuleDlg::StopTimer()
{
	...
	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->spashscreenfinished=true; //Xman new slpash-screen arrangement
	...
	if (theAppPtr->pstrPendingLink != NULL){
		OnWMData(NULL, (LPARAM)&theAppPtr->sendstruct);
		delete theAppPtr->pstrPendingLink;
	}

	// [BCPlugin] 任务列表加载完成后的初始化
	theDllApp.wrapper_on_tasklist_loaded();
}
...
void CemuleDlg::OnBnClickedButton2(){
	if (!theAppPtr->IsConnected())
		//connect if not currently connected
		if (!theAppPtr->serverconnect->IsConnecting() && !Kademlia::CKademlia::IsRunning() ){
			StartConnection();
		}
		else {
			CloseConnection();
...
void CemuleDlg::AddLogText(UINT uFlags, LPCTSTR pszText)
{
	...
	{
		theAppPtr->QueueLogLineEx(uFlags, _T("%s"), pszText);
		return;
	}
	...
	if (uFlags & LOG_STATUSBAR)
	{
		...
		{
			if (theAppPtr->m_app_state != APP_STATE_SHUTTINGDOWN)
				statusbar->SetText(pszText, SBarLog, 0);
		}
		else
		{//Xman
			// [BCPlugin] bc plugin not show splash window
			//theAppPtr->DestroySplash(); //Xman new slpash-screen arrangement
...
{
	if (theAppPtr->serverconnect->IsConnected() && !Kademlia::CKademlia::IsConnected())
	{
	...
	else if (!theAppPtr->serverconnect->IsConnected() && Kademlia::CKademlia::IsConnected())
	{
	...
	{
		if (theAppPtr->serverconnect->IsLowID() && Kademlia::CKademlia::IsFirewalled())
			return 4; // LowLow
		else if (theAppPtr->serverconnect->IsLowID())
			return 5; // LowHigh
...
CString CemuleDlg::GetConnectionStateString()
{
	...
	else if (theAppPtr->serverconnect->IsConnecting())
...
{
	theAppPtr->downloadqueue->OnConnectionState(theAppPtr->IsConnected());
...
void CemuleDlg::ShowUserCount()
{
	...
	theAppPtr->serverlist->GetUserFileStatus( totaluser, totalfile );
	CString buffer;
	if (theAppPtr->serverconnect->IsConnected() && Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsConnected())
...
{
	m_uUpDatarate = uUpDatarate != (UINT)-1 ? uUpDatarate : theAppPtr->uploadqueue->GetDatarate();
*/
CString CemuleDlg::GetUpDatarateString(UINT /*uUpDatarate*/)
{
//Xman end
...
{
	m_uDownDatarate = uDownDatarate != (UINT)-1 ? uDownDatarate : theAppPtr->downloadqueue->GetDatarate();
*/
CString CemuleDlg::GetDownDatarateString(UINT /*uDownDatarate*/)
{
//Xman end
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	/*
	m_uDownDatarate = theAppPtr->downloadqueue->GetDatarate();
	m_uUpDatarate = theAppPtr->uploadqueue->GetDatarate();
	*/
	// Retrieve the current datarates
	uint32 eMuleIn;	uint32 eMuleInOverall;
	uint32 eMuleOut; uint32 eMuleOutOverall;
	uint32 notUsed;
	theAppPtr->pBandWidthControl->GetDatarates(thePrefs.GetDatarateSamples(),
	...
	if (TrayIsVisible() || bForceAll)
	{
		...
		if (theAppPtr->IsConnected()) {
			// [BCPlugin]
			_sntprintf(buffer2, _countof(buffer2), MAIN_DLG_TITLE_PREFIX _T(" v%s (%s)\r\n%s"), theAppPtr->m_strCurVersionLong, GetResString(IDS_CONNECTED), strTransferRate);
			buffer2[_countof(buffer2) - 1] = _T('\0');
		}
		else {
			// [BCPlugin]
			_sntprintf(buffer2, _countof(buffer2), MAIN_DLG_TITLE_PREFIX _T(" v%s (%s)\r\n%s"), theAppPtr->m_strCurVersionLong, GetResString(IDS_DISCONNECTED), strTransferRate);
			buffer2[_countof(buffer2) - 1] = _T('\0');
		}
	...
	if (IsWindowVisible() || bForceAll)
	{
		...
		if(theAppPtr->downloadqueue->GetLimitState()==1)
			strTransferRate.Append(_T(" r"));
		else if(theAppPtr->downloadqueue->GetLimitState()>=2)
			strTransferRate.Append(_T(" R"));
		//Xman end
	...
	if (IsWindowVisible() && thePrefs.ShowRatesOnTitle())
	{
		...
		// Maella -Support for tag ET_MOD_VERSION 0x55
		/*
		_sntprintf(szBuff, _countof(szBuff), _T("(U:%.1f D:%.1f) eMule v%s"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theAppPtr->m_strCurVersionLong);
		*/
		//_sntprintf(szBuff, _countof(szBuff), _T("(U:%.1f D:%.1f) eMule v%s"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theAppPtr->m_strCurVersionLong + _T(" ") + MOD_VERSION);
		//Xman end
		// [BCPlugin]
		_sntprintf(szBuff, _countof(szBuff), _T("(U:%.1f D:%.1f) ") MAIN_DLG_TITLE_PREFIX _T(" v%s"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theAppPtr->m_strCurVersionLong);
		szBuff[_countof(szBuff) - 1] = _T('\0');
...
void CemuleDlg::ShowPing()
{
    ...
    if (IsWindowVisible())
	{
		...
		{
			CurrentPingStruct lastPing = theAppPtr->lastCommonRouteFinder->GetCurrentPing();
...
void CemuleDlg::SetActiveDialog(CWnd* dlg)
{
	...
	if(dlg->IsWindowVisible()) // [BCPlugin] avoid set focus when current window is not visible
		dlg->SetFocus();
	activewnd = dlg;
		...
		case CED2KLink::kFile:
			{
				...
				if(theAppPtr->knownfiles->CheckAlreadyDownloadedFileQuestion(pFileLink->GetHashKey(),pFileLink->GetName()))
				{
					theAppPtr->downloadqueue->AddFileLinkToDownload(pFileLink,searchwnd->GetSelectedCat());
				}
				//Xman end
			}
			break;
		...
		case CED2KLink::kFriend:
			{
				...
				if ( ! theAppPtr->friendlist->IsAlreadyFriend(userHash) )
					theAppPtr->friendlist->AddFriend(userHash, 0U, 0U, 0U, 0U, pFriendLink->GetUserName(), 1U);
				else
				{
					CString msg;
...
LRESULT CemuleDlg::OnWMData(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (data->dwData == OP_ED2KLINK)
	{
		...
		if (clcommand==_T("disconnect")) {theAppPtr->serverconnect->Disconnect(); return true;}
		if (clcommand==_T("resume")) {theAppPtr->downloadqueue->StartNextFile(); return true;}
		//Xman do not ask exit from command prompt (leuk_he)
		/*
		if (clcommand==_T("exit")) {OnClose(); return true;}
		*/
		if (clcommand==_T("exit")) {theAppPtr->m_app_state = APP_STATE_SHUTTINGDOWN;OnClose(); return true;}
		//Xman end
		if (clcommand==_T("restore")) {RestoreWindow();return true;}
		if (clcommand==_T("reloadipf")) {theAppPtr->ipfilter->LoadFromDefaultFile(); return true;}
		...
		if (clcommand==_T("status")) {
			...
			if (file){
				...
				// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
				/*
				strBuff.Format(GetResString(IDS_UPDOWNSMALL), (float)theAppPtr->uploadqueue->GetDatarate()/1024, (float)theAppPtr->downloadqueue->GetDatarate()/1024);
				*/
				uint32 eMuleIn;
				uint32 eMuleOut;
				uint32 notUsed;
...
LRESULT CemuleDlg::OnFileHashed(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam)
	{
		...
		if (theAppPtr->downloadqueue->IsPartFile(requester))
	...
	else
	{
		...
		theAppPtr->sharedfiles->FileHashingFinished(result);
	}
	return TRUE;
}
...
LRESULT CemuleDlg::OnHashFailed(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	theAppPtr->sharedfiles->HashFailed((UnknownFile_Struct*)lParam);
	return 0;
}
// SLUGFILLER: SafeHash
...
LRESULT CemuleDlg::OnFileAllocExc(WPARAM wParam,LPARAM lParam)
{
	...
	ASSERT(!(theAppPtr->m_app_state == APP_STATE_RUNNING && theAppPtr->downloadqueue==NULL)); 

	CFileException* error = (CFileException*)lParam;
	if (theAppPtr->m_app_state != APP_STATE_RUNNING || theAppPtr->downloadqueue==NULL || !theAppPtr->downloadqueue->IsPartFile((CPartFile*)wParam)) { //MORPH - Changed by SiRoB, Flush Thread
		if (error != NULL)
			error->Delete();
		return FALSE;
	}
...
void CemuleDlg::OnEndSession(BOOL bEnding)
{
	...
	if (bEnding && theAppPtr->m_app_state == APP_STATE_RUNNING)
	{
		...
		theAppPtr->m_app_state = APP_STATE_SHUTTINGDOWN;
		OnClose();
	}
...
LRESULT CemuleDlg::OnConsoleThreadEvent(WPARAM wParam, LPARAM lParam)
{
	...
	// 
	if (theAppPtr->m_app_state == APP_STATE_RUNNING)
	{
#if 1
...
{
	if (theAppPtr->m_app_state == APP_STATE_RUNNING && thePrefs.IsConfirmExitEnabled())
	{
...
{
	// [BCPlugin] bc plugin not show splash window
	//if(theAppPtr->IsSplash()) 
	//	theAppPtr->DestroySplash(); //Xman new slpash-screen arrangement

	// [BCPlugin] 调用清理函数
	theDllApp.wrapper_on_emule_dlg_close();

	if(theAppPtr == NULL)
		return;

	// [BCPlugin] 不再显示关闭确认对话框
	//if (!CanClose())
	//	return;
	...
	//			break;
	//		default:
	//			theAppPtr->ShowSplash(false);
	//		}
	//	}
	//theAppPtr->UpdateSplash(_T("Shutting down ..."));
	//Xman end
	// [BCPlugin] End
	...
	theAppPtr->ForeAllDiscAccessThreadsToFinish();

	theAppPtr->serverconnect->Disconnect();
	theAppPtr->OnlineSig(); // Added By Bouc7 
	...
	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->UpdateSplash(_T("waiting for hash end")); //Xman new slpash-screen arrangement

	// try to wait untill the hashing thread notices that we are shutting down
	CSingleLock sLock1(&theAppPtr->hashing_mut); // only one filehash at a time
	sLock1.Lock(2000);
	//Xman queued disc-access for read/flushing-threads
	...
	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->UpdateSplash(_T("saving settings ...")); //Xman new slpash-screen arrangement

	// saving data & stuff
	theAppPtr->emuledlg->preferenceswnd->m_wndSecurity.DeleteDDB();

	theAppPtr->knownfiles->Save();										// CKnownFileList::Save
	theAppPtr->sharedfiles->Save();
	...
	if(theAppPtr->pBandWidthControl->GetwasNAFCLastActive()==true)
		thePrefs.SetNAFCFullControl(true);
	//Xman end

	theAppPtr->m_pPeerCache->Save();
	theAppPtr->scheduler->RestoreOriginals();
	theAppPtr->searchlist->SaveSpamFilter();
	if (thePrefs.IsStoringSearchesEnabled())
		theAppPtr->searchlist->StoreSearches();

	//Official UPNP
	// close uPnP Ports
	theAppPtr->m_pUPnPFinder->GetImplementation()->StopAsyncFind();
	if (thePrefs.CloseUPnPOnExit())
		theAppPtr->m_pUPnPFinder->GetImplementation()->DeletePorts();
	//Official UPNP end
	...
	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->UpdateSplash(_T("clearing displayed items ...")); //Xman new slpash-screen arrangement
	...
	theAppPtr->clientlist->DeleteAll();
    ...
    theAppPtr->uploadBandwidthThrottler->EndThread();
    //Xman
	/*
    theAppPtr->lastCommonRouteFinder->EndThread();
	*/
	//Xman end

	theAppPtr->sharedfiles->DeletePartFileInstances();

	searchwnd->SendMessage(WM_CLOSE);

	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->UpdateSplash(_T("clearing lists ..."));  //Xman new slpash-screen arrangement

	//Xman
	theAppPtr->m_threadlock.WriteLock();	// SLUGFILLER: SafeHash - Last chance, let all running threads close before we start deleting
	...
	delete theAppPtr->mmserver;			theAppPtr->mmserver = NULL;
	delete theAppPtr->listensocket;		theAppPtr->listensocket = NULL;
	delete theAppPtr->clientudp;		theAppPtr->clientudp = NULL;
	delete theAppPtr->sharedfiles;		theAppPtr->sharedfiles = NULL;
	delete theAppPtr->serverconnect;	theAppPtr->serverconnect = NULL;
	delete theAppPtr->serverlist;		theAppPtr->serverlist = NULL;		// CServerList::SaveServermetToFile
	delete theAppPtr->knownfiles;		theAppPtr->knownfiles = NULL;
	delete theAppPtr->searchlist;		theAppPtr->searchlist = NULL;
	// [BCPlugin] bc plugin not show splash window
	//theAppPtr->UpdateSplash(_T("saving credits ..."));  //Xman new slpash-screen arrangement
	delete theAppPtr->clientcredits;	theAppPtr->clientcredits = NULL;	// CClientCreditsList::SaveList
	//theAppPtr->UpdateSplash(_T("clearing queues ..."));  //Xman new slpash-screen arrangement
	delete theAppPtr->downloadqueue;	theAppPtr->downloadqueue = NULL;	// N * (CPartFile::FlushBuffer + CPartFile::SavePartFile)
	delete theAppPtr->uploadqueue;		theAppPtr->uploadqueue = NULL;
	delete theAppPtr->clientlist;		theAppPtr->clientlist = NULL;
	delete theAppPtr->friendlist;		theAppPtr->friendlist = NULL;		// CFriendList::SaveList
	delete theAppPtr->scheduler;		theAppPtr->scheduler = NULL;
	//theAppPtr->UpdateSplash(_T("Unloading IPfilter ..."));  //Xman new slpash-screen arrangement
	delete theAppPtr->ipfilter;			theAppPtr->ipfilter = NULL;			// CIPFilter::SaveToDefaultFile
	delete theAppPtr->webserver;		theAppPtr->webserver = NULL;
	delete theAppPtr->m_pPeerCache;		theAppPtr->m_pPeerCache = NULL;
	delete theAppPtr->m_pFirewallOpener;theAppPtr->m_pFirewallOpener = NULL;
	//theAppPtr->UpdateSplash(_T("shutdown bandwidthcontrol ..."));  //Xman new slpash-screen arrangement
	delete theAppPtr->uploadBandwidthThrottler; theAppPtr->uploadBandwidthThrottler = NULL;
	//Xman
	/*
	delete theAppPtr->lastCommonRouteFinder; theAppPtr->lastCommonRouteFinder = NULL;
	*/
	//Xman End
	delete theAppPtr->m_pUPnPFinder;	theAppPtr->m_pUPnPFinder = NULL; //Official UPNP
	//theAppPtr->m_UPnPNat.clearNATPortMapping(); //ACAT UPnP

	//Xman
	// Maella [patch] -Bandwidth: overall bandwidth measure-	
	delete theAppPtr->pBandWidthControl;theAppPtr->pBandWidthControl = NULL;
	// Maella end

	//theAppPtr->UpdateSplash(_T("unload IP to Country ..."));  //Xman new slpash-screen arrangement

	//EastShare Start - added by AndCycle, IP to Country
	delete theAppPtr->ip2country;		theAppPtr->ip2country = NULL;
	//EastShare End   - added by AndCycle, IP to Country

	delete theAppPtr->dlp; theAppPtr->dlp=NULL; //Xman DLP

	//theAppPtr->UpdateSplash(_T("Shutdown done")); //Xman new slpash-screen arrangement

	thePrefs.Uninit();
	theAppPtr->m_app_state = APP_STATE_DONE;
...
{
	if (   (!theAppPtr->serverconnect->IsConnecting() && !theAppPtr->serverconnect->IsConnected())
		|| !Kademlia::CKademlia::IsRunning())
	{
		...
		//Official UPNP end
			...
			// ed2k
			if ((thePrefs.GetNetworkED2K() || m_bEd2kSuspendDisconnect) && !theAppPtr->serverconnect->IsConnecting() && !theAppPtr->serverconnect->IsConnected()) {
				theAppPtr->serverconnect->ConnectToAnyServer();
			}
...
void CemuleDlg::CloseConnection()
{
	...
	if (theAppPtr->serverconnect->IsConnecting()){
		theAppPtr->serverconnect->StopConnectionTry();
	}
...
LRESULT CemuleDlg::OnTaskbarNotifierClicked(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	switch (m_wndTaskbarNotifier->GetMessageType())
	{
		...
		{
			CString theUrl;
			theUrl.Format(_T("/en/version_check.php?version=%i&language=%i"),theAppPtr->m_uCurVersionCheck,thePrefs.GetLanguageID());
...
void CemuleDlg::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
	...
	//theAppPtr->UpdateLargeIconSize();
	theAppPtr->UpdateDesktopColorDepth();
...
HBRUSH CemuleDlg::GetCtlColor(CDC* /*pDC*/, CWnd* /*pWnd*/, UINT nCtlColor)
{
	...
	// use WM_CTLCOLOR...
	//
	//if (theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(6,16,0,0) && g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed()) {
...
void CemuleDlg::SetAllIcons()
{
	...
	connicons[0] = theAppPtr->LoadIcon(_T("ConnectedNotNot"), 16, 16);
	connicons[1] = theAppPtr->LoadIcon(_T("ConnectedNotLow"), 16, 16);
	connicons[2] = theAppPtr->LoadIcon(_T("ConnectedNotHigh"), 16, 16);
	connicons[3] = theAppPtr->LoadIcon(_T("ConnectedLowNot"), 16, 16);
	connicons[4] = theAppPtr->LoadIcon(_T("ConnectedLowLow"), 16, 16);
	connicons[5] = theAppPtr->LoadIcon(_T("ConnectedLowHigh"), 16, 16);
	connicons[6] = theAppPtr->LoadIcon(_T("ConnectedHighNot"), 16, 16);
	connicons[7] = theAppPtr->LoadIcon(_T("ConnectedHighLow"), 16, 16);
	connicons[8] = theAppPtr->LoadIcon(_T("ConnectedHighHigh"), 16, 16);
	ShowConnectionStateIcon();
	...
	if (transicons[2]) VERIFY( ::DestroyIcon(transicons[2]) );
	if (transicons[3]) VERIFY( ::DestroyIcon(transicons[3]) );
	transicons[0] = theAppPtr->LoadIcon(_T("UP0DOWN0"), 16, 16);
	transicons[1] = theAppPtr->LoadIcon(_T("UP0DOWN1"), 16, 16);
	transicons[2] = theAppPtr->LoadIcon(_T("UP1DOWN0"), 16, 16);
	transicons[3] = theAppPtr->LoadIcon(_T("UP1DOWN1"), 16, 16);
	ShowTransferStateIcon();

	// users state
	if (usericon) VERIFY( ::DestroyIcon(usericon) );
	usericon = theAppPtr->LoadIcon(_T("StatsClients"), 16, 16);
	ShowUserStateIcon();
	...
	if (m_icoSysTrayDisconnected) VERIFY( ::DestroyIcon(m_icoSysTrayDisconnected) );
	if (m_icoSysTrayLowID) VERIFY( ::DestroyIcon(m_icoSysTrayLowID) );
	m_icoSysTrayConnected = theAppPtr->LoadIcon(_T("TrayConnected"), 16, 16);
	m_icoSysTrayDisconnected = theAppPtr->LoadIcon(_T("TrayNotConnected"), 16, 16);
	m_icoSysTrayLowID = theAppPtr->LoadIcon(_T("TrayLowID"), 16, 16);
	ShowTransferRate(true);
	...
	if (imicons[2]) VERIFY( ::DestroyIcon(imicons[2]) );
	imicons[0] = NULL;
	imicons[1] = theAppPtr->LoadIcon(_T("Message"), 16, 16);
	imicons[2] = theAppPtr->LoadIcon(_T("MessagePending"), 16, 16);
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		case MP_HM_LINK3: {
			CString theUrl;
			theUrl.Format( thePrefs.GetVersionCheckBaseURL() + CString(_T("/en/version_check.php?version=%i&language=%i")),theAppPtr->m_uCurVersionCheck,thePrefs.GetLanguageID());
			ShellExecute(NULL, NULL, theUrl, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		case MP_WEBSVC_EDIT:
		...
		case MP_HM_SCHEDONOFF:
			...
			theAppPtr->scheduler->Check(true);
			break;
	...
	if (wParam>=MP_WEBURL && wParam<=MP_WEBURL+99) {
		...
		theAppPtr->scheduler->ActivateSchedule(wParam-MP_SCHACTIONS);
		theAppPtr->scheduler->SaveOriginals(); // use the new settings as original
	}
...
{
	return (theAppPtr->m_app_state == APP_STATE_RUNNING);
}
...
void CemuleDlg::ShowToolPopup(bool toolsonly)
{
	...
	if (theAppPtr->scheduler->GetCount()>0) {
		scheduler.AppendMenu(MF_SEPARATOR);
		for (UINT i=0; i<theAppPtr->scheduler->GetCount();i++)
			scheduler.AppendMenu(MF_STRING,MP_SCHACTIONS+i, theAppPtr->scheduler->GetSchedule(i)->title);
	}
...
{
	theAppPtr->m_fontHyperText.DeleteObject();
	if (theAppPtr->m_fontHyperText.CreateFontIndirect(plf))
	{
		thePrefs.SetHyperTextFont(plf);
		serverwnd->servermsgbox->SetFont(&theAppPtr->m_fontHyperText);
		chatwnd->chatselector.UpdateFonts(&theAppPtr->m_fontHyperText);
		ircwnd->UpdateFonts(&theAppPtr->m_fontHyperText);
	}
}

void CemuleDlg::ApplyLogFont(LPLOGFONT plf)
{
	theAppPtr->m_fontLog.DeleteObject();
	if (theAppPtr->m_fontLog.CreateFontIndirect(plf))
	{
		thePrefs.SetLogFont(plf);
		serverwnd->logbox->SetFont(&theAppPtr->m_fontLog);
		serverwnd->debuglog->SetFont(&theAppPtr->m_fontLog);
		serverwnd->leecherlog->SetFont(&theAppPtr->m_fontLog); //Xman Anti-Leecher-Log
	}
}
...
LRESULT CemuleDlg::OnFrameGrabFinished(WPARAM wParam,LPARAM lParam){
	...
	if (theAppPtr->m_app_state != APP_STATE_SHUTTINGDOWN) {
	//MORPH END   - Added by SiRoB, Fix crash at shutdown
		if (theAppPtr->knownfiles->IsKnownFile(pOwner) || theAppPtr->downloadqueue->IsPartFile(pOwner) ){
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				if (dwResult > *(uint32*)abyCurVer){
					// [BCPlugin] bc plugin not show splash window
					//if(theAppPtr->IsSplash()) theAppPtr->DestroySplash(); //Xman new slpash-screen arrangement
					SetActiveWindow();
#ifndef _BETA
					...
					if (AfxMessageBox(GetResString(IDS_NEWVERSIONAVLBETA)+GetResString(IDS_VISITVERSIONCHECK),MB_OK)==IDOK) {
						CString theUrl;
						theUrl.Format(_T("/en/download.php?version=%i&language=%i"),theAppPtr->m_uCurVersionCheck,thePrefs.GetLanguageID());
						theUrl = thePrefs.GetVersionCheckBaseURL()+theUrl;
						ShellExecute(NULL, NULL, theUrl, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
					}
#endif
...
LRESULT CemuleDlg::OnKickIdle(UINT /*nWhy*/, long lIdleCount)
{
	...
	*/
	// [BCPlugin] bc plugin not show splash window
	//if (theAppPtr->IsSplash() && theAppPtr->spashscreenfinished)
	//{
	//	if (::GetCurrentTime() - theAppPtr->m_dwSplashTime > 3500) 
	//	{
	//		// timeout expired, destroy the splash window
	//		theAppPtr->DestroySplash();
	////Xman end
...
//#endif
			...
			/*
			lResult = theAppPtr->OnIdle(lIdleCount);
			*/
			// [BCPlugin]
			//static uint32 lastprocess;
			//if(lIdleCount>0)
			//{
			//	theAppPtr->OnIdle(lIdleCount); //free maps
			//	lastprocess=::GetTickCount();
			//	return 0;
			//}

			//if(theAppPtr->OnIdle(0 /*lIdleCount*/) && ::GetTickCount() - lastprocess > MIN2MS(1)) 
			//	lResult=1;
			//else
			//	lResult=0;
			//Xman end
...
{
	return theAppPtr->m_pPeerCache->OnPeerCacheCheckResponse(wParam,lParam);
}
...
LRESULT CemuleDlg::OnWebAddDownloads(WPARAM wParam, LPARAM lParam)
{
	...
	if (link.GetLength()==32 && link.Left(4).CompareNoCase(_T("ed2k"))!=0) {
		...
		theAppPtr->searchlist->AddFileToDownloadByHash(fileid,(uint8)lParam);

	} else
		theAppPtr->AddEd2kLinksToDownload((TCHAR*)wParam, (int)lParam);
...
{
	if (lParam==0) { // remove
		theAppPtr->friendlist->RemoveFriend((CFriend*)wParam);
	} else {		// add
		theAppPtr->friendlist->AddFriend((CUpDownClient*)wParam);
	}
...
{
	theAppPtr->downloadqueue->SetCatPrio(wParam,(uint8)lParam);
	return 0;
}
LRESULT CemuleDlg::OnWebServerClearCompleted(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		CKnownFile* file=theAppPtr->knownfiles->FindKnownFileByID(pFileHash);
		if (file)
...
LRESULT CemuleDlg::OnWebGUIInteraction(WPARAM wParam, LPARAM lParam) {
	...
	switch (wParam) {
		...
		case WEBGUIIA_UPD_CATTABS:
			theAppPtr->emuledlg->transferwnd->UpdateCatTabTitles();
			break;
		case WEBGUIIA_UPD_SFUPDATE: {
			CKnownFile* kf=(CKnownFile*)lParam;
			if (kf)
				theAppPtr->sharedfiles->UpdateFile(kf);
			}
			break;
		...
		case WEBGUIIA_CONNECTTOSERVER: {
			...
			if (server==NULL) 
				...
				theAppPtr->serverconnect->ConnectToServer(server);
			break;
			}
		...
		case WEBGUIIA_UPDATESERVERMETFROMURL:
			theAppPtr->emuledlg->serverwnd->UpdateServerMetFromURL((TCHAR*)lParam);
			break;
		case WEBGUIIA_SHOWSTATISTICS:
			theAppPtr->emuledlg->statisticswnd->ShowStatistics(lParam!=0);
			break;
...
void CALLBACK CemuleDlg::UPnPTimeOutTimer(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
	...
	__if_exists(Core_Common::InterfaceExceptionDebugger)
	{
		__try
		{
			return CemuleDlg::UPnPTimeOutTimerImpl(0, 0, 0, 0);
		}
		__except ( Core_Common::InterfaceExceptionDebugger::RecordExceptionInfo(GetExceptionInformation(), _T("") ) )
		{
			// Do nothing here - RecordExceptionInfo() has already done
...
void CALLBACK CemuleDlg::UPnPTimeOutTimerImpl(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
#ifdef VERSION_DLL
	// [BCPlugin] timer callback impl
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	::PostMessage(theAppPtr->emuledlg->GetSafeHwnd(), UM_UPNP_RESULT, (WPARAM)CUPnPImpl::UPNP_TIMEOUT, 0);
}
...
LRESULT CemuleDlg::OnUPnPResult(WPARAM wParam, LPARAM lParam){
	...
	if (!bWasRefresh && wParam == CUPnPImpl::UPNP_FAILED){		
		...
		if (theAppPtr->m_pUPnPFinder->SwitchImplentation()){
			StartUPnP(false);
			return 0;
		}
		else
	...
	if (!bWasRefresh && wParam == CUPnPImpl::UPNP_OK){
		...
		thePrefs.SetLastWorkingUPnPImpl(theAppPtr->m_pUPnPFinder->GetImplementation()->GetImplementationID());
		Log(GetResString(IDS_UPNPSUCCESS), theAppPtr->m_pUPnPFinder->GetImplementation()->GetUsedTCPPort()
			, theAppPtr->m_pUPnPFinder->GetImplementation()->GetUsedUDPPort());
	}
	else if (!bWasRefresh)
	...
	switch (wParam) {
		case PBT_APMRESUMEAUTOMATIC:
		{
			if (m_bEd2kSuspendDisconnect || m_bKadSuspendDisconnect)
			{
				...
				RefreshUPnP(true); //Official UPNP
		...
		{		
			...
			*/
			...
			// <== Proper English [ginger] - Stulle
			m_bEd2kSuspendDisconnect = theAppPtr->serverconnect->IsConnected();
...
void CemuleDlg::StartUPnP(bool bReset, uint16 nForceTCPPort, uint16 nForceUDPPort) {
	if (theAppPtr->m_pUPnPFinder != NULL && (m_hUPnPTimeOutTimer == 0 || !bReset)){
		if (bReset){
			theAppPtr->m_pUPnPFinder->Reset();
			Log(GetResString(IDS_UPNPSETUP));
		}
		try
		{
			if (theAppPtr->m_pUPnPFinder->GetImplementation()->IsReady()){
				theAppPtr->m_pUPnPFinder->GetImplementation()->SetMessageOnResult(GetSafeHwnd(), UM_UPNP_RESULT);
				if (bReset)
					VERIFY( (m_hUPnPTimeOutTimer = ::SetTimer(NULL, NULL, SEC2MS(40), UPnPTimeOutTimer)) != NULL );
				theAppPtr->m_pUPnPFinder->GetImplementation()->StartDiscovery(((nForceTCPPort != 0) ? nForceTCPPort : thePrefs.GetPort())
					, ((nForceUDPPort != 0) ? nForceUDPPort :thePrefs.GetUDPPort())
					, thePrefs.GetWSUseUPnP() ? thePrefs.GetWSPort() : 0);
			}
			else
				::PostMessage(theAppPtr->emuledlg->GetSafeHwnd(), UM_UPNP_RESULT, (WPARAM)CUPnPImpl::UPNP_FAILED, 0);
		}
...
void CemuleDlg::RefreshUPnP(bool bRequestAnswer)
{
	if (!thePrefs.IsUPnPEnabled())
		return;
	if (theAppPtr->m_pUPnPFinder != NULL && m_hUPnPTimeOutTimer == 0){
		try
		{
			if (theAppPtr->m_pUPnPFinder->GetImplementation()->IsReady())
			{
				if (bRequestAnswer)
					theAppPtr->m_pUPnPFinder->GetImplementation()->SetMessageOnResult(GetSafeHwnd(), UM_UPNP_RESULT);
				if (theAppPtr->m_pUPnPFinder->GetImplementation()->CheckAndRefresh() && bRequestAnswer)
				{
					VERIFY( (m_hUPnPTimeOutTimer = ::SetTimer(NULL, NULL, SEC2MS(10), UPnPTimeOutTimer)) != NULL );
				}
				else
					theAppPtr->m_pUPnPFinder->GetImplementation()->SetMessageOnResult(0, 0);
			}
			else
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				dwResult &= 0x00FFFFFF;
				thePrefs.UpdateLastMVC();
				if (theAppPtr->dlp->IsDLPavailable() && dwResult > theAppPtr->dlp->GetDLPVersion()){
					// [BCPlugin] bc plugin not show splash window
					//if(theAppPtr->IsSplash()) theAppPtr->DestroySplash(); //Xman new slpash-screen arrangement
					SetActiveWindow();
...
LRESULT CemuleDlg::OnPartHashedOK(WPARAM wParam,LPARAM lParam)
{
	//Xman
	// BEGIN SiRoB: Fix crash at shutdown
	if (theAppPtr->m_app_state != APP_STATE_RUNNING || theAppPtr->downloadqueue==NULL)
		return FALSE;
	// END SiRoB: Fix crash at shutdown
	CPartFile* pOwner = (CPartFile*)lParam;
	if (theAppPtr->downloadqueue->IsPartFile(pOwner))	// could have been canceled
...
LRESULT CemuleDlg::OnReadBlockFromFileDone(WPARAM wParam,LPARAM lParam)
{
	...
	if (theAppPtr->m_app_state == APP_STATE_RUNNING && theAppPtr->uploadqueue->IsDownloading(client))	// could have been canceled
	{
...
LRESULT CemuleDlg::OnFlushDone(WPARAM /*wParam*/,LPARAM lParam)
{
	...
	CPartFile* partfile = (CPartFile*) lParam;
	if (theAppPtr->m_app_state == APP_STATE_RUNNING && theAppPtr->downloadqueue!=NULL && theAppPtr->downloadqueue->IsPartFile(partfile))	// could have been canceled
		partfile->FlushDone();
	return 0;
}
...
{
	CPartFile* partfile = (CPartFile*) lParam;
	if (theAppPtr->m_app_state != APP_STATE_SHUTTINGDOWN && AfxIsValidAddress(partfile,sizeof(CPartFile)) &&  theAppPtr->downloadqueue->IsPartFile(partfile)) {	// could have been canceled 
...
afx_msg LRESULT CemuleDlg::DoTimer(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if(wParam == NULL)
		theAppPtr->uploadqueue->UploadTimer();
	/* only used for uploadtimer
	else if(wParam == -1)
		theAppPtr->emuledlg->StartupTimer();
	*/
	else
		ASSERT(0);
	return 0;
...
// [BCPlugin] 在主程序关闭插件时调用
void CemuleDlg::CloseWnd()
{
	OnClose();
}