...
CSearch::CSearch()
{
	...
	theAppPtr->emuledlg->kademliawnd->searchList->SearchAdd(this);
...
CSearch::~CSearch()
{
	...
	theAppPtr->emuledlg->kademliawnd->searchList->SearchRem(this);

	// Check if a source search is currently being done.
	CPartFile* pPartFile = theAppPtr->downloadqueue->GetFileByKadFileSearchID(GetSearchID());
	...
	if (m_uType == NOTES){
		CAbstractFile* pAbstractFile = theAppPtr->knownfiles->FindKnownFileByID(CUInt128(GetTarget().GetData()).GetData());
		if (pAbstractFile != NULL)
			pAbstractFile->SetKadCommentSearchRunning(false);

		pAbstractFile = theAppPtr->downloadqueue->GetFileByID(CUInt128(GetTarget().GetData()).GetData());
		if (pAbstractFile != NULL)
			pAbstractFile->SetKadCommentSearchRunning(false);

		theAppPtr->searchlist->SetNotesSearchStatus(CUInt128(GetTarget().GetData()).GetData(), false);
	}
...
void CSearch::Go()
{
	...
	theAppPtr->emuledlg->kademliawnd->searchList->SearchRef(this);
}
		...
		case FILE:
			{
				...
				if (pFromContact->GetVersion() >= 3/*47b*/)
				{
					...
					CKnownFile* pFile = theAppPtr->downloadqueue->GetFileByID(ucharFileid);
					if(pFile)
					{
		...
		case NOTES:
			{
				...
				if (pFromContact->GetVersion() >= 3/*47b*/)
				{
					...
					uchar ucharFileid[16];
					m_uTarget.ToByteArray(ucharFileid);
					CKnownFile* pFile = theAppPtr->sharedfiles->GetFileByID(ucharFileid);
					if(pFile)
					{
						m_pfileSearchTerms.WriteUInt64(pFile->GetFileSize());
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE) //zz_fly :: DummyCut :: 090213
		...
		case STOREFILE:
			{
				...
				if (pFile)
				{
					...
					if( theAppPtr->IsFirewalled() )
					{
						...
						else if( theAppPtr->clientlist->GetBuddy() ) // We are firewalled, make sure we have a buddy.
						{
							...
							listTag.push_back(new CKadTagUInt(TAG_SERVERIP, theAppPtr->clientlist->GetBuddy()->GetIP()));
							listTag.push_back(new CKadTagUInt(TAG_SERVERPORT, theAppPtr->clientlist->GetBuddy()->GetUDPPort()));
...
void CSearch::ProcessResultFile(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	switch( uType )
	{
		...
		case 6:
			m_uAnswers++;
			theAppPtr->emuledlg->kademliawnd->searchList->SearchRef(this);
			theAppPtr->downloadqueue->KademliaSearchFile(m_uSearchID, &uAnswer, &uBuddy, uType, uIP, uTCPPort, uUDPPort, uBuddyIP, uBuddyPort, byCryptOptions);
			break;
	}
}
...
void CSearch::ProcessResultNotes(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	bool bFlag = theAppPtr->searchlist->AddNotes(pEntry, ucharFileid);

	// Check if this hash is in our shared files..
	CAbstractFile* pFile = (CAbstractFile*)theAppPtr->sharedfiles->GetFileByID(ucharFileid);

	// If we didn't find a file in the shares check if it's in our download queue.
	if(!pFile)
		pFile = (CAbstractFile*)theAppPtr->downloadqueue->GetFileByID(ucharFileid);
...
/*#ifdef _DEBUG
	...
	theAppPtr->searchlist->KademliaSearchKeyword(m_uSearchID, &uAnswer, sName, uSize, sType, uPublishInfo, m_pSearchTerm, 8,
