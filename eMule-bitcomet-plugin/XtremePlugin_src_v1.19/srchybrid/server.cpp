...
CServer::CServer(const ServerMet_Struct* in_data)
{
	...
	m_structServerCountry = theAppPtr->ip2country->GetCountryFromIP(ip); //EastShare - added by AndCycle, IP to Country
}
...
uint32 CServer::GetServerKeyUDP(bool bForce) const{
	if (m_dwIPServerKeyUDP != 0 && m_dwIPServerKeyUDP == theAppPtr->GetPublicIP() || bForce)
...
void CServer::SetServerKeyUDP(uint32 dwServerKeyUDP){
	ASSERT( theAppPtr->GetPublicIP() != 0 || dwServerKeyUDP == 0 );
	m_dwServerKeyUDP = dwServerKeyUDP;
	m_dwIPServerKeyUDP = theAppPtr->GetPublicIP();
}

//EastShare Start - added by AndCycle, IP to Country
CString CServer::GetCountryName() const{
	return theAppPtr->ip2country->GetCountryNameFromRef(m_structServerCountry);
}
...
void CServer::ResetIP2Country(){
	m_structServerCountry = theAppPtr->ip2country->GetCountryFromIP(ip);
}
