...
void CServerConnect::TryAnotherConnectionRequest()
{
	...
	{
		CServer* next_server = theAppPtr->serverlist->GetNextServer(m_bTryObfuscated);
		if (next_server == NULL)
		{
...
void CServerConnect::ConnectToAnyServer(UINT startAt, bool prioSort, bool isAuto, bool bNoCrypt)
{
	...
	if (thePrefs.GetAutoConnectToStaticServersOnly() && isAuto)
	{
		...
		CServer *next_server;
		theAppPtr->serverlist->SetServerPosition(startAt);
		while ((next_server = theAppPtr->serverlist->GetNextServer(false)) != NULL)
		{
			if (next_server->IsStaticMember()) {
				anystatic = true;
				break;
	...
	if (thePrefs.GetUseUserSortedServerList() && startAt == 0 && prioSort)
		theAppPtr->serverlist->GetUserSortedServers();
	if (thePrefs.GetUseServerPriorities() && prioSort)
		theAppPtr->serverlist->Sort();

	if (theAppPtr->serverlist->GetServerCount() == 0) {
		connecting = false;
		LogError(LOG_STATUSBAR, GetResString(IDS_ERR_NOVALIDSERVERSFOUND));
		return;
	}
...
void CServerConnect::ConnectToServer(CServer* server, bool multiconnect, bool bNoCrypt)
{
	...
	if(theAppPtr->ipfilter->IsFiltered(server->GetIP()))
	{
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		CServer* pServer = theAppPtr->serverlist->GetServerByAddress(sender->cur_server->GetAddress(), sender->cur_server->GetPort());
		if (pServer) {
	...
	else if (sender->GetConnectionState() == CS_CONNECTED)
	{
		...
		StopConnectionTry();
		theAppPtr->sharedfiles->ClearED2KPublishInfo();
		theAppPtr->sharedfiles->SendListToServer();
		theAppPtr->emuledlg->serverwnd->serverlistctrl.RemoveAllDeadServers();
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:{
			...
			if (thePrefs.Reconnect() && autoretry && !m_idRetryTimer) {
				...
				if (pServer) {
					...
					int iPosInList = theAppPtr->serverlist->GetPositionOfServer(pServer);
					if (iPosInList >= 0)
						m_uStartAutoConnectPos = (iPosInList + 1) % theAppPtr->serverlist->GetServerCount();
				}
				VERIFY( (m_idRetryTimer = SetTimer(NULL, 0, 1000*CS_RETRYCONNECTTIME, RetryConnectTimer)) != NULL );
				if (thePrefs.GetVerbose() && !m_idRetryTimer)
					DebugLogError(_T("Failed to create 'server connect retry' timer - %s"), GetErrorMessage(GetLastError()));
		...
		case CS_DISCONNECTED:{
			...
			theAppPtr->emuledlg->searchwnd->CancelEd2kSearch();
			...
			if (thePrefs.GetNotifierOnImportantError())
				theAppPtr->emuledlg->ShowNotifier(GetResString(IDS_CONNECTIONLOST), TBN_IMPORTANTEVENT);
			break;
		}
		case CS_ERROR:
		case CS_NOTCONNECTED:{
...
void CALLBACK CServerConnect::RetryConnectTimer(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/)
{
	...
	__if_exists(Core_Common::InterfaceExceptionDebugger)
	{
		__try
		{
			return CServerConnect::RetryConnectTimerImpl(0, 0, 0, 0);
		}
...
VOID CALLBACK CServerConnect::RetryConnectTimerImpl(HWND /*hWnd*/, UINT /*nMsg*/, UINT /*nId*/, DWORD /*dwTime*/) 
{ 
#ifdef VERSION_DLL
	...
	try
	{
		CServerConnect *_this = theAppPtr->serverconnect;
		ASSERT( _this );
		if (_this)
		{
			...
			if (_this->IsConnected())
				return;
			if (_this->m_uStartAutoConnectPos >= theAppPtr->serverlist->GetServerCount())
...
bool CServerConnect::Disconnect()
{
	if (connected && connectedsocket)
	{
		theAppPtr->sharedfiles->ClearED2KPublishInfo();
		connected = false;
		CServer* pServer = theAppPtr->serverlist->GetServerByAddress(connectedsocket->cur_server->GetAddress(), connectedsocket->cur_server->GetPort());
		if (pServer)
			theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(pServer);
		theAppPtr->SetPublicIP(0);
