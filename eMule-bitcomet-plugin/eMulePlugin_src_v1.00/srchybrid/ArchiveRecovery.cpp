...
UINT AFX_CDECL CArchiveRecovery::run(LPVOID lpParam)
{
	...
	if (!performRecovery(tp->partFile, tp->filled, tp->preview, tp->bCreatePartFileCopy))
		theAppPtr->QueueLogLine(true, _T("%s \"%s\""), GetResString(IDS_RECOVERY_FAILED), tp->partFile->GetFileName());
									   ...
									   bool preview, bool bCreatePartFileCopy)
{
	...
	try
	{
		...
		if (success)
		{
			theAppPtr->QueueLogLine(true, _T("%s \"%s\""), GetResString(IDS_RECOVERY_SUCCESSFUL), partFile->GetFileName());
			theAppPtr->QueueDebugLogLine(false, _T("Archive recovery: Part file size: %s, temp. archive file size: %s (%.1f%%)"), CastItoXBytes(partFile->GetFileSize()), CastItoXBytes(ulTempFileSize), partFile->GetFileSize() > (uint64)0 ? (ulTempFileSize * 100.0 / (uint64)partFile->GetFileSize()) : 0.0);
								  ...
								  CTypedPtrList<CPtrList, Gap_Struct*> *filled, bool fullSize)
{
	...
	theAppPtr->QueueLogLine(true, _T("%s"), msg);
