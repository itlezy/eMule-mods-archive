...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	{
		//if (thePrefs.GetLogUlDlEvents() && GetUploadState()==US_UPLOADING && m_fSentOutOfPartReqs==0 && !theAppPtr->uploadqueue->IsOnUploadQueue(this))
	...
	if (GetChatState() != MS_NONE){
		bDelete = false;
		theAppPtr->emuledlg->chatwnd->chatselector.ConnectingResult(this,false);
	}
...
{
	if (theAppPtr->listensocket->TooManySockets() && !bIgnoreMaxCon && !(socket && socket->IsConnected()))
	{
	...
	if ( HasLowID() )
	{
		...
		//If ANYTHING changes with the "if(!theAppPtr->DoCallback(this))" above that will let you fall through 
		//with the condition that the source is firewalled and we are firewalled, we must
		//recheck it before the this check..
		if( HasValidBuddyID() && !GetBuddyIP() && !GetBuddyPort() && !theAppPtr->serverconnect->IsLocalServer(GetServerIP(), GetServerPort()))
		{
...
void CUpDownClient::ConnectionEstablished()
{
	...
	if (GetChatState() == MS_CONNECTING || GetChatState() == MS_CHATTING)
		theAppPtr->emuledlg->chatwnd->chatselector.ConnectingResult(this,true);
