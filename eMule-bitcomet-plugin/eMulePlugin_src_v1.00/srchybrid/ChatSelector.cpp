...
CChatItem* CChatSelector::StartSession(CUpDownClient* client, bool show)
{
	...
	chatitem->log->SetFont(&theAppPtr->m_fontHyperText);
...
void CChatSelector::ProcessMessage(CUpDownClient* sender, const CString& message)
{
	...
	else
	{
        ...
        if (isNewChatWindow || thePrefs.GetNotifierOnEveryChatMsg())
			theAppPtr->emuledlg->ShowNotifier(GetResString(IDS_TBN_NEWCHATMSG) + _T(" ") + CString(sender->GetUserName()) + _T(":'") + message + _T("'\n"), TBN_CHAT);
...
void CChatSelector::OnTimer(UINT_PTR /*nIDEvent*/)
{
	...
	if (globalnotify) {
		theAppPtr->emuledlg->ShowMessageState(m_blinkstate ? 1 : 2);
		m_lastemptyicon = false;
	}
	else if (!m_lastemptyicon) {
		theAppPtr->emuledlg->ShowMessageState(0);
...
BOOL CChatSelector::OnCommand(WPARAM wParam, LPARAM lParam)
{
	switch (wParam) {
		...
		case MP_ADDFRIEND:{
			const CChatItem* ci = GetItemByIndex(m_iContextIndex);
			if (ci) {
				CFriend* fr = theAppPtr->friendlist->SearchFriend(ci->client->GetUserHash(), 0, 0);
				if (!fr)
					theAppPtr->friendlist->AddFriend(ci->client);
			}
			return TRUE;
		}
		case MP_REMOVEFRIEND:{
			...
			if (ci) {
				CFriend* fr = theAppPtr->friendlist->SearchFriend(ci->client->GetUserHash(), 0, 0);
				if (fr)
					theAppPtr->friendlist->RemoveFriend(fr);
			}
			return TRUE;
		}
		case MP_REMOVE:{
...
void CChatSelector::OnContextMenu(CWnd*, CPoint point)
{
	...
	CFriend* pFriend = theAppPtr->friendlist->SearchFriend(ci->client->GetUserHash(), 0, 0);
