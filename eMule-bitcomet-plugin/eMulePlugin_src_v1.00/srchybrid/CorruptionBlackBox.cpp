...
void CCorruptionBlackBox::CorruptedData(uint64 nStartPos, uint64 nEndPos){
	...
	for(int k = 0; k < aGuiltyClients.GetCount();){
		...
		if (theAppPtr->clientlist->IsBannedClient(aGuiltyClients[k])){
	...
	if (aGuiltyClients.GetCount() > 0){
		...
		for(int k = 0; k < aGuiltyClients.GetCount(); k++){
			...
			if ( nCorruptPercentage > CBB_BANTHRESHOLD){
				...
				CUpDownClient* pEvilClient = theAppPtr->clientlist->FindClientByIP(aGuiltyClients[k]);
				if (pEvilClient != NULL){
					...
					theAppPtr->clientlist->AddTrackClient(pEvilClient);
					...
					theAppPtr->clientlist->AddBannedClient(aGuiltyClients[k]);
				}
			}
			else{
				CUpDownClient* pSuspectClient = theAppPtr->clientlist->FindClientByIP(aGuiltyClients[k]);
				if (pSuspectClient != NULL){
					...
					theAppPtr->clientlist->AddTrackClient(pSuspectClient);
				}
				else
