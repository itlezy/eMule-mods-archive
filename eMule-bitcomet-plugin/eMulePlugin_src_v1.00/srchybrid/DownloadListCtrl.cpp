...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, CtrlItem_Struct *lpCtrlItem)
{
	if(lpRect->left < lpRect->right)
	{
		...
		case 0:{	// file name
			CRect rcDraw(lpRect);
			int iImage = theAppPtr->GetFileTypeSystemImageIdx(lpPartFile->GetFileName());
			if (theAppPtr->GetSystemImageList() != NULL)
				::ImageList_Draw(theAppPtr->GetSystemImageList(), iImage, dc->GetSafeHdc(), rcDraw.left, rcDraw.top, ILD_NORMAL|ILD_TRANSPARENT);
					...
					if (HitTest(&hit) >= 0 && (hit.flags & LVHT_ONITEM))
					{
						...
						if (SubItemHitTest(&subhit) >= 0 && subhit.iSubItem == 0)
						{
							...
							if (thePrefs.ShowRatingIndicator() 
								...
								&& pt.x >= FILE_ITEM_MARGIN_X+theAppPtr->GetSmallSytemIconSize().cx 
								&& pt.x <= FILE_ITEM_MARGIN_X+theAppPtr->GetSmallSytemIconSize().cx+RATING_ICON_WIDTH)
								ShowFileDialog(IDD_COMMENTLST);
