			...
			for (long i = 0; i < lLinks; i++)
			{
				...
				if (links->item(vaIndex, vaNull, &item) == S_OK)
				{
					...
					if (SUCCEEDED(item->QueryInterface(&anchor)))
					{
						...
						{
							theAppPtr->emuledlg->ProcessED2KLink(CString(bstrHref));
							hrPasteResult = S_OK;
						}
				...
				while (*pwsz != L'\0')
				{
					...
					if (IsUrlSchemeSupportedW(pwsz))
					{
						...
						if (iLen > 0)
						{
							...
							theAppPtr->emuledlg->ProcessED2KLink(strURL);
							hrPasteResult = S_OK;
							pwsz += iLen;
						}
					}
	...
	if ((hMem = data.GetGlobalData(cfData)) != NULL)
	{
		...
		if ((pszUrlA = (LPCSTR)GlobalLock(hMem)) != NULL)
		{
			...
			if (strncmp(pszUrlA, "ed2k://|", 8) == 0)
			{
				...
				while (!str.IsEmpty())
				{
					theAppPtr->emuledlg->ProcessED2KLink(str);
					hrPasteResult = S_OK;
			...
			if (SUCCEEDED(hrResult = pIUrl.QueryInterface(&pIFile)))
			{
				...
				if (SUCCEEDED(hrResult = pIFile->Load(CComBSTR(T2CW(pszFileName)), STGM_READ | STGM_SHARE_DENY_WRITE)))
				{
					...
					if ((hrResult = pIUrl->GetURL(&pwszUrl)) == S_OK)
					{
						...
						{
							theAppPtr->emuledlg->ProcessED2KLink(W2T(pwszUrl));
							hrResult = S_OK;
						}
