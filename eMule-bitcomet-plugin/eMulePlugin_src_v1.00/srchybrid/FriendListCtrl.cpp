		...
		case MP_MESSAGE:
			if (cur_friend)
			{
				if (cur_friend->GetLinkedClient(true))
					theAppPtr->emuledlg->chatwnd->StartSession(cur_friend->GetLinkedClient());
				else
				{
					...
					theAppPtr->clientlist->AddClient(chatclient);
					theAppPtr->emuledlg->chatwnd->StartSession(chatclient);
				}
			}
			break;
