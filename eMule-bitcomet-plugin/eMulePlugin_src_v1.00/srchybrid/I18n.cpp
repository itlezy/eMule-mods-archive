...
CString GetResString(UINT uStringID)
{
	...
	if (resString.IsEmpty())
	{
		...
		ASSERT(hmod);
		resString.LoadString(hmod, uStringID);
	}
	return resString;
}
	...
	if (dwVerInfSize != 0)
	{
		...
		if (pucVerInf)
		{
			if (GetFileVersionInfo(const_cast<LPTSTR>((LPCTSTR)rstrLangDLL), 0, dwVerInfSize, pucVerInf))
			{
				...
				{
					bResult = (pFileInf->dwProductVersionMS == theAppPtr->m_dwProductVersionMS &&
                               pFileInf->dwProductVersionLS == theAppPtr->m_dwProductVersionLS);
				}
			}
			free(pucVerInf);
		}
...
void CPreferences::InitThreadLocale()
{
	...
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("SetLanguageACP"), 0) != 0)
	{
	...
	else if (theAppPtr->GetProfileInt(_T("eMule"), _T("SetSystemACP"), 0) != 0)
	{
...
bool CheckThreadLocale()
{
	if (theAppPtr->GetProfileInt(_T("eMule"), _T("SetLanguageACP"), 0) != 0)
		return true;
	int iSetSysACP = theAppPtr->GetProfileInt(_T("eMule"), _T("SetSystemACP"), -1);
	if (iSetSysACP != -1)
		return true;
	iSetSysACP = 0;
	...
	VERIFY( theAppPtr->WriteProfileInt(_T("eMule"), _T("SetSystemACP"), iSetSysACP) );
	return true;
}
