...
void CMuleStatusBarCtrl::OnLButtonDblClk(UINT /*nFlags*/, CPoint point)
{
	...
	switch (iPane)
	{
		...
		case SBarUsers:
			theAppPtr->emuledlg->serverwnd->ShowNetworkInfo();
			break;
		
		case SBarUpDown:
			theAppPtr->emuledlg->SetActiveDialog(theAppPtr->emuledlg->statisticswnd);
			break;
		...
		case SBarChatMsg:
			theAppPtr->emuledlg->SetActiveDialog(theAppPtr->emuledlg->chatwnd);
			break;
	}
}
...
CString CMuleStatusBarCtrl::GetPaneToolTipText(EStatusBarPane iPane) const
{
	...
	{
	case SBarConnected:
		if (theAppPtr->serverconnect && theAppPtr->serverconnect->IsConnected())
		{
