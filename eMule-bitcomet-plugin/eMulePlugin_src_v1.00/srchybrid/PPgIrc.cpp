...
BOOL CPPgIRC::OnApply()
{
	...
	if (bOldSmileys != thePrefs.GetIRCEnableSmileys())
		theAppPtr->emuledlg->ircwnd->EnableSmileys(thePrefs.GetIRCEnableSmileys());
	...
	if (GetDlgItem(IDC_IRC_NICK_BOX)->GetWindowTextLength())
	{
		...
		if (_tcscmp(buffer, thePrefs.m_strIRCNick) != 0)
		{
			...
			if (input != _T(""))
			{
				theAppPtr->emuledlg->ircwnd->SendString(_T("NICK ") + input);
...
void CPPgIRC::OnHelp()
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_IRC);
}
