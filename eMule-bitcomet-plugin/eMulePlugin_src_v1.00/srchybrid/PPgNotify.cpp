...
void CPPgNotify::OnBnClickedTestNotification()
{
	...
	CString strTest;
	strTest.Format(GetResString(IDS_MAIN_READY), theAppPtr->m_strCurVersionLong);
	theAppPtr->emuledlg->ShowNotifier(strTest, TBN_IMPORTANTEVENT);
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Notifications);
}
