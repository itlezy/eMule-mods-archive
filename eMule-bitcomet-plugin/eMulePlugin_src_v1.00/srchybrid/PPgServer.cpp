...
BOOL CPPgServer::OnApply()
{
	...
	if (uCurDeadServerRetries != thePrefs.m_uDeadServerRetries) {
		theAppPtr->emuledlg->serverwnd->serverlistctrl.Invalidate();
		theAppPtr->emuledlg->serverwnd->serverlistctrl.UpdateWindow();
	}
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Server);
}
