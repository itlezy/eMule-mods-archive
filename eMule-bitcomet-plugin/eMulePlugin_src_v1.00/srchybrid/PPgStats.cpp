...
BOOL CPPgStats::OnApply()
{
	...
	if (m_bModified)
	{
		...
		if (bInvalidateGraphs){
			theAppPtr->emuledlg->statisticswnd->UpdateConnectionsGraph(); // Set new Y upper bound and Y ratio for active connections.
			theAppPtr->emuledlg->statisticswnd->Localize();
			theAppPtr->emuledlg->statisticswnd->ShowInterval();
		}
		theAppPtr->emuledlg->statisticswnd->RepaintMeters();
		theAppPtr->emuledlg->statisticswnd->GetDlgItem(IDC_STATTREE)->EnableWindow(thePrefs.GetStatsInterval() > 0);
		SetModified(FALSE);
	}
...
{
	theAppPtr->ShowHelp(eMule_FAQ_Preferences_Statistics);
}
