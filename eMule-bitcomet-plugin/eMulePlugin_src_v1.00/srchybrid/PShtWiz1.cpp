...
void CPPgWiz1Ports::OnStartUPnP() {
	...
	try
	{
		if ( theAppPtr->m_pUPnPFinder->AreServicesHealthy() )
			theAppPtr->m_pUPnPFinder->StartDiscovery(GetTCPPort(), GetUDPPort());
	}
...
void CPPgWiz1Ports::OnTimer(UINT /*nIDEvent*/){
	m_nUPnPTicks++;
	if (theAppPtr->m_pUPnPFinder && theAppPtr->m_pUPnPFinder->m_bUPnPPortsForwarded == TRIS_UNKNOWN)
	{
		if (m_nUPnPTicks < 40){
	...
	if (theAppPtr->m_pUPnPFinder && theAppPtr->m_pUPnPFinder->m_bUPnPPortsForwarded == TRIS_TRUE){
...
BOOL FirstTimeWizard()
{
	...
	VERIFY( bmWatermark.LoadImage(IDR_WIZ1_WATERMARK, _T("GIF"), AfxGetInstanceHandle(), GetSysColor(COLOR_WINDOW)) );
	CEnBitmap bmHeader;
	VERIFY( bmHeader.LoadImage(IDR_WIZ1_HEADER, _T("GIF"), AfxGetInstanceHandle(), GetSysColor(COLOR_WINDOW)) );
