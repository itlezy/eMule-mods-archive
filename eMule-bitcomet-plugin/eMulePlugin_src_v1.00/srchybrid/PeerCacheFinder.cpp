...
void CPeerCacheFinder::DoLookUp(CStringA strHostname){
	if (WSAAsyncGetHostByName(theAppPtr->emuledlg->m_hWnd, UM_PEERCHACHE_RESPONSE, strHostname, _acDNSBuffer, sizeof(_acDNSBuffer)) == 0){
...
BOOL CPCReverseDnsThread::InitInstance()
{
	...
	else
	{
		...
		if (WSAAsyncGetHostByAddr(theAppPtr->emuledlg->m_hWnd, UM_PEERCHACHE_RESPONSE, (const char*) &IPHost, sizeof(struct in_addr), AF_INET, _acDNSBuffer, sizeof(_acDNSBuffer)) == 0){
			if (thePrefs.GetVerbose())
