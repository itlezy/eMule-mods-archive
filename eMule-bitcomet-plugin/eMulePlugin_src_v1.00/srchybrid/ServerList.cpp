...
bool CServerList::AddServerMetToList(const CString& strFile, bool bMerge) 
{
	...
	try{
		...
		for (UINT j = 0; j < fservercount; j++)
		{
			...
			if (!theAppPtr->emuledlg->serverwnd->serverlistctrl.AddServer(newserver, true))
			{
...
bool CServerList::AddServer(const CServer* pServer)
{
	...
	if (thePrefs.GetFilterServerByIP()){
		...
		if (theAppPtr->ipfilter->IsFiltered(pServer->GetIP())){
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pServer->GetListName(), ipstr(pServer->GetIP()), theAppPtr->ipfilter->GetLastHit());
			return false;
		}
	}
