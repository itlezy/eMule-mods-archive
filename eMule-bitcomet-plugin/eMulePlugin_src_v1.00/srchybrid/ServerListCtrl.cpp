...
void CServerListCtrl::RemoveAllDeadServers()
{
	...
	{
		const CServer* cur_server = theAppPtr->serverlist->list.GetAt(pos);
	...
	for (POSITION pos = theAppPtr->serverlist->list.GetHeadPosition(); pos != NULL; theAppPtr->serverlist->list.GetNext(pos))
	{
		...
		if (theAppPtr->ipfilter->IsFiltered(cur_server->GetIP()))
		{
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), cur_server->GetListName(), ipstr(cur_server->GetIP()), theAppPtr->ipfilter->GetLastHit());
			RemoveServer(cur_server);
			pos = theAppPtr->serverlist->list.GetHeadPosition();
		}
	}
...
{
	if (!theAppPtr->serverlist->AddServer(pServer))
		return false;
	if (bAddToList)
	{
...
bool CServerListCtrl::StaticServerFileAppend(CServer *server)
{
	try
	{
					...
					server->GetListName()) != EOF) 
		{
			...
			theAppPtr->emuledlg->serverwnd->serverlistctrl.RefreshServer(server);
		}
