	...
	if (cur_server->HasDynIP())
	{
		...
		if (thePrefs.GetFilterServerByIP() && theAppPtr->ipfilter->IsFiltered(cur_server->GetIP())) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pServer ? pServer->GetAddress() : cur_server->GetAddress(), ipstr(cur_server->GetIP()), theAppPtr->ipfilter->GetLastHit());
			if (pServer)
