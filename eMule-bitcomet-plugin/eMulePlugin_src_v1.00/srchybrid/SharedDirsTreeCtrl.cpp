...
void CSharedDirsTreeCtrl::SetAllIcons()
{
	...
	int nImage = theAppPtr->GetFileTypeSystemImageIdx(strTempDir); // System Folder Icon
	if (theAppPtr->GetSystemImageList() != NULL){
		iml.Add(::ImageList_GetIcon(theAppPtr->GetSystemImageList(), nImage, 0));
	}
	else{
	...
	theAppPtr->LoadSkinColorAlt(_T("SharedDirsTvBk"), _T("DefLvBk"), crBk);
	theAppPtr->LoadSkinColorAlt(_T("SharedDirsTvFg"), _T("DefLvFg"), crFg);
	SetBkColor(crBk);
	SetTextColor(crFg);
}
