...
void CSharedFileList::AddFilesFromDirectory(const CString& rstrDirectory)
{
	...
	while (!end)
	{
		...
		CKnownFile* toadd = theAppPtr->knownfiles->FindKnownFile(ff.GetFileName(), fdate, ff.GetLength());
		if (toadd)
		{
...
int CAddFileThread::Run()
{
	...
	if (newrecord->CreateFromFile(m_strDirectory, m_strFilename, m_partfile) && theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning()) // SLUGFILLER: SafeHash - in case of shutdown while still hashing
	{
		...
		VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd, TM_FINISHEDHASHING, (m_pOwner ? 0: (WPARAM)m_partfile), (LPARAM)newrecord) );
	}
	else
	{
		if (theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
		...
		if (m_pOwner && theAppPtr->emuledlg && theAppPtr->emuledlg->IsRunning())
		{
			...
			VERIFY( PostMessage(theAppPtr->emuledlg->m_hWnd,TM_HASHFAILED,0,(LPARAM)hashed) );
		}
		// SLUGFILLER: SafeHash
		delete newrecord;
	}
...
void CSharedFileList::Publish()
{
	...
	if( Kademlia::CKademlia::IsConnected() && ( !isFirewalled || ( isFirewalled && theAppPtr->clientlist->GetBuddyStatus() == Connected)) && GetCount() && Kademlia::CKademlia::GetPublish())
	{ 
