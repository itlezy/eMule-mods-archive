...
BOOL CSmileySelector::Create(CWnd *pWndParent, const RECT *pRect, CEdit *pwndEdit)
{
	...
	m_iBitmaps = 0;
	CImageList iml;
	iml.Create(sizSmiley.cx, sizSmiley.cy, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 0, 1);
	for (int i = 0; i < _countof(g_aSmileys); i++)
	{
		HICON hIcon = theAppPtr->LoadIcon(g_aSmileys[i].pszResource, sizSmiley.cx, sizSmiley.cy);
		if (hIcon) {
			iml.Add(hIcon);
			m_iBitmaps++;
		}
