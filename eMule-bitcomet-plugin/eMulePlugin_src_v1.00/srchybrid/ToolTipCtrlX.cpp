...
void CToolTipCtrlX::OnNMCustomDraw(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pNMCD->nmcd.dwDrawStage == CDDS_PREPAINT)
	{
		...
		int iIconSize = theAppPtr->GetBigSytemIconSize().cx;
		...
		while (iPos != -1)
		{
			...
			if (iColon != -1) {
				...
				if (theAppPtr->GetBigSystemImageList() != NULL)
					::ImageList_Draw(theAppPtr->GetBigSystemImageList(), iImage, pdc->GetSafeHdc(), ptText.x, rcDT.top + ((iCaptionHeigth - iIconSize) / 2), ILD_NORMAL|ILD_TRANSPARENT);
			}
			else {
