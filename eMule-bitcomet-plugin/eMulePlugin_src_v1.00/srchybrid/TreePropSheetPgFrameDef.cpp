...
void CPropPageFrameDefault::SetCaption(LPCTSTR lpszCaption, HICON hIcon /*= NULL*/)
{
	...
	if (hIcon)
	{
		...
		if (!m_Images.Create(bm.bmWidth, bm.bmHeight, theAppPtr->m_iDfltImageListColorFlags|ILC_MASK, 0, 1))
			return;
...
void CPropPageFrameDefault::DrawCaption(CDC *pDc, CRect rect, LPCTSTR lpszCaption, HICON hIcon)
{
	...
	CFont		*pFont = (CFont*)pDc->SelectObject(&theAppPtr->m_fontDefaultBold);
