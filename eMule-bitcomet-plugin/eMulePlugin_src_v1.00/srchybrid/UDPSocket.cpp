...
void CUDPSocket::DnsLookupDone(WPARAM wp, LPARAM lp)
{
	...
	if (nIP != INADDR_NONE)
	{
		...
		if (!bRemoveServer && theAppPtr->ipfilter->IsFiltered(nIP)) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pDNSReq->m_pServer->GetAddress(), ipstr(nIP), theAppPtr->ipfilter->GetLastHit());
			bRemoveServer = true;
		}
