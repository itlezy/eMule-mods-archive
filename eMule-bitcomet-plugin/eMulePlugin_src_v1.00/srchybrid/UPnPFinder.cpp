	...
	for ( DWORD nIf = 0 ; nIf < nCount ; nIf++ )
	{
		if ( ipAddr->table[ nIf ].dwIndex == nInterfaceIndex )
		{
			...
			strLocalIP.Format( L"%d.%d.%d.%d", ( ip & 0x0000ff ),
                ...
                ( ip >> 24 ) );
			// theAppPtr->m_sUPnPExternalIP = strExternalIP;
			break;
		}
	}
			...
			LPCWSTR pszStateVarName, VARIANT varValue)
{
	...
	if ( !m_instance.IsAsyncFindRunning() )
	{
		...
		{
			theAppPtr->m_pUPnPFinder->m_bUPnPDeviceConnected = strValue.CompareNoCase( L"Disconnected" ) == 0
					? TRIS_FALSE
		...
		//			theAppPtr->m_sUPnPExternalIP = strValue.Trim();
	}
