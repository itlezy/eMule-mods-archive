...
BOOL CemuleDlg::OnInitDialog()
{
	...
	serverwnd->Create(IDD_SERVER, this);
	sharedfileswnd->Create(IDD_FILES, this);
	searchwnd->Create(this);
	chatwnd->Create(IDD_CHAT, this);
	transferwnd->Create(IDD_TRANSFER, this);
	statisticswnd->Create(IDD_STATISTICS, this);
	kademliawnd->Create(IDD_KADEMLIAWND, this);
	ircwnd->Create(IDD_IRC, this);
	...
	if (theAppPtr->m_pUPnPFinder != NULL && thePrefs.IsUPnPEnabled()){
		try
		{
			if (theAppPtr->m_pUPnPFinder->AreServicesHealthy()){
				theAppPtr->m_pUPnPFinder->SetMessageOnResult(GetSafeHwnd(), UM_UPNP_RESULT);
				VERIFY( (m_hUPnPTimeOutTimer = ::SetTimer(NULL, NULL, SEC2MS(30), UPnPTimeOutTimer)) != NULL );
				theAppPtr->m_pUPnPFinder->StartDiscovery(thePrefs.GetPort(), thePrefs.GetUDPPort());
			}
		}
...
void CemuleDlg::OnClose()
{
	...
	// close uPnP Ports
	theAppPtr->m_pUPnPFinder->StopAsyncFind();
	if (thePrefs.CloseUPnPOnExit())
		theAppPtr->m_pUPnPFinder->DeletePorts();
...
void CemuleDlg::StartConnection()
{
		...
		|| !Kademlia::CKademlia::IsRunning())
	{
		...
		if (m_hUPnPTimeOutTimer != 0 && !m_bConnectRequestDelayedForUPnP){
			...
			// ed2k
			if (thePrefs.GetNetworkED2K() && !theAppPtr->serverconnect->IsConnecting() && !theAppPtr->serverconnect->IsConnected()) {
...
void CemuleDlg::UPnPTimeOutTimer(HWND /*hwnd*/, UINT /*uiMsg*/, UINT /*idEvent*/, DWORD /*dwTime*/){
	::PostMessage(theAppPtr->emuledlg->GetSafeHwnd(), UM_UPNP_RESULT, (WPARAM)CUPnPFinder::UPNP_TIMEOUT, 0);
}
