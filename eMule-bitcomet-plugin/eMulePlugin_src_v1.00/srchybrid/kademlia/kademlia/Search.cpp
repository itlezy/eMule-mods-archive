		...
		case STOREFILE:
			{
				...
				if (pFile)
				{
					...
					if( theAppPtr->IsFirewalled() )
					{
						...
						if( theAppPtr->clientlist->GetBuddy() )
						{
...
void CSearch::ProcessResultFile(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	switch( uType )
	{
		...
		case 5:
			m_uAnswers++;
			theAppPtr->emuledlg->kademliawnd->searchList->SearchRef(this);
			theAppPtr->downloadqueue->KademliaSearchFile(m_uSearchID, &uAnswer, &uBuddy, uType, uIP, uTCPPort, uUDPPort, uServerIP, uServerPort, byCryptOptions);
			break;
	}
}
...
void CSearch::ProcessResultKeyword(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	theAppPtr->searchlist->KademliaSearchKeyword(m_uSearchID, &uAnswer, sName, uSize, sType, 8,
