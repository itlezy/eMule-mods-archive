						...
						if (::IsGoodIPPort(uhostIP, uUDPPort))
						{
							if (::theAppPtr->ipfilter->IsFiltered(uhostIP))
							{
								if (::thePrefs.GetLogFilteredIPs())
									AddDebugLogLine(false, _T("Ignored kad contact (IP=%s)--read known.dat -- - IP filter (%s)") , ipstr(uhostIP), ::theAppPtr->ipfilter->GetLastHit());
							}
							else
							{
...
bool CRoutingZone::Add(const CUInt128 &uID, uint32 uIP, uint16 uUDPPort, uint16 uTCPPort, uint8 uVersion, bool bUpdate)
{
	...
	if (::IsGoodIPPort(uhostIP, uUDPPort))
	{
		if (!::theAppPtr->ipfilter->IsFiltered(uhostIP)) {
			...
			AddDebugLogLine(false, _T("Ignored kad contact (IP=%s) - IP filter (%s)"), ipstr(uhostIP), ::theAppPtr->ipfilter->GetLastHit());
	}
	else if (::thePrefs.GetLogFilteredIPs())
