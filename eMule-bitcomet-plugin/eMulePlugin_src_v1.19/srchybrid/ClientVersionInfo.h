...
public:
	...
	CClientVersionInfo(CString strPCEncodedVersion)
	{
		...
		if (posSeperator == (-1) || strPCEncodedVersion.GetLength() - posSeperator < 2){
			theAppPtr->QueueDebugLogLine( false, _T("PeerCache Error: Bad Version info in PeerCache Descriptor found: %s"), strPCEncodedVersion);
			return;
		}
		...
		// can add more types here
			...
			theAppPtr->QueueDebugLogLine(false, _T("PeerCache Warning: Unknown Clienttype in descriptor file found"));
			m_ClientTypeMajor = SO_UNKNOWN;
		}
