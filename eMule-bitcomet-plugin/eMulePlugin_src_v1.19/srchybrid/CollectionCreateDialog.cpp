...
BOOL CCollectionCreateDialog::OnInitDialog(void)
{
	...
	SetIcon(m_icoWnd = theAppPtr->LoadIcon(_T("AABCollectionFileType")), FALSE);
	if (m_bCreatemode)
	...
	m_AddCollectionButton.SetIcon(m_icoForward = theAppPtr->LoadIcon(_T("FORWARD")));
	m_RemoveCollectionButton.SetIcon(m_icoBack = theAppPtr->LoadIcon(_T("BACK")));
	m_CollectionListIcon.SetIcon(m_icoColl = theAppPtr->LoadIcon(_T("AABCollectionFileType")));
	m_CollectionSourceListIcon.SetIcon(m_icoFiles = theAppPtr->LoadIcon(_T("SharedFilesList")));
...
void CCollectionCreateDialog::OnBnClickedOk()
{
	...
	if (!sFileName.IsEmpty())
	{
		...
		else
		{
			...
			{
				CKnownFile* pKnownFile = theAppPtr->knownfiles->FindKnownFileByPath(sFilePath);
				if (pKnownFile)
				{
					theAppPtr->sharedfiles->RemoveFile(pKnownFile);
					if (pKnownFile->IsKindOf(RUNTIME_CLASS(CPartFile)))
						theAppPtr->emuledlg->transferwnd->downloadlistctrl.ClearCompleted(static_cast<CPartFile*>(pKnownFile));
				}
				m_pCollection->WriteToFileAddShared(pSignkey);
			}
			else
...
void CCollectionCreateDialog::UpdateAvailFiles(void)
{
	...
	if (m_bSharedFiles)
		theAppPtr->sharedfiles->CopySharedFileMap(Files_Map);
	else
		theAppPtr->knownfiles->CopyKnownFileMap(Files_Map);
