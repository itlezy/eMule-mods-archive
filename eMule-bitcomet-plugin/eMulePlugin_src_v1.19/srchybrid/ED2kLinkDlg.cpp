...
{ 
	...
	if (theAppPtr->IsConnected() && !theAppPtr->IsFirewalled() && !thePrefs.GetYourHostname().IsEmpty() && thePrefs.GetYourHostname().Find(_T('.')) != -1)
...
void CED2kLinkDlg::UpdateLink()
{
	...
	const bool bSource = ((CButton*)GetDlgItem(IDC_LD_SOURCECHE))->GetCheck() == BST_CHECKED && theAppPtr->IsConnected() && !theAppPtr->IsFirewalled();
	const bool bHostname = ((CButton*)GetDlgItem(IDC_LD_HOSTNAMECHE))->GetCheck() == BST_CHECKED && theAppPtr->IsConnected() && !theAppPtr->IsFirewalled()
...
void CED2kLinkDlg::OnBnClickedClipboard()
{
	...
	theAppPtr->CopyTextToClipboard(strBuffer);
}
