...
void CMuleListCtrl::SetColors()
{
	...
	if (pszSkinProfile != NULL && pszSkinProfile[0] != _T('\0'))
	{
		...
		if (theAppPtr->LoadSkinColorAlt(strKey + _T("Bk" ), _T("DefLvBk"), m_crWindow))
			m_crWindowTextBk = m_crWindow;
		theAppPtr->LoadSkinColorAlt(strKey + _T("Fg"), _T("DefLvFg"), m_crWindowText);
		theAppPtr->LoadSkinColorAlt(strKey + _T("Hl"), _T("DefLvHl"), crHighlight);
	...
	if(iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) {
		...
		if (hInstRes != NULL){
			...
			if (hbmSortStates != NULL){
				...
				if (imlSortStates.Create(14, 14, theAppPtr->m_iDfltImageListColorFlags | ILC_MASK, 1, 0)){
					...
					if (theAppPtr->m_ullComCtrlVer >= MAKEDLLVERULL(5,8,0,0)){
