...
Pinger::Pinger() {
    ...
    if (hICMP_DLL == 0) {
        theAppPtr->QueueDebugLogLine(false,_T("Pinger: LoadLibrary() failed: Unable to locate ICMP.DLL!"));
        return;
    }
    ...
    if ((!lpfnIcmpCreateFile) || 
        ...
        theAppPtr->QueueDebugLogLine(false,_T("Pinger: GetProcAddr() failed for at least one function."));
        return;
    }
    ...
    if (hICMP == INVALID_HANDLE_VALUE) {
        ...
        theAppPtr->QueueDebugLogLine(false, _T("Pinger: IcmpCreateFile() failed, err: %u"), nErr);
        PIcmpErr(nErr);
        return;
    }
...
Pinger::~Pinger() {
    ...
    if (fRet == FALSE) {
        int nErr = GetLastError();
        theAppPtr->QueueDebugLogLine(false,_T("Error closing ICMP handle, err: %u"), nErr);
        PIcmpErr(nErr);
    }
...
PingStatus Pinger::PingUDP(uint32 lAddr, uint32 ttl, bool doLog) {
	...
	while((usResTime += m_time.Tick()) < TIMEOUT){
		...
		if (((icmphdr->type == ICMP_TTL_EXPIRE) || (icmphdr->type == ICMP_DEST_UNREACH)) &&
			...
			if(doLog) {
				theAppPtr->QueueDebugLogLine(false,_T("Reply (UDP-pinger) from %s: bytes=%d time=%3.2fms TTL=%i"),
					ipstr(stDestAddr),
					nRet,
					usResTime,
					returnValue.ttl);
			...
			//}
			if(doLog) {
				theAppPtr->QueueDebugLogLine(false,_T("Filtered reply (UDP-pinger) from %s: bytes=%d time=%3.2fms TTL=%i type=%i"),
					ipstr(stDestAddr),
					nRet,
					usResTime,
					64 - (reply->ttl & 63),
...
PingStatus Pinger::PingICMP(uint32 lAddr, uint32 ttl, bool doLog) {
    ...
    if (dwReplyCount != 0) {
        ...
        if(doLog) {
            theAppPtr->QueueDebugLogLine(false,_T("Reply (ICMP-pinger) from %s: bytes=%d time=%3.2fms (%3.2fms %ldms) TTL=%i"),
                                                        ipstr(stDestAddr),
        ...
        returnValue.error = lastError;
        if(doLog) {
			theAppPtr->QueueDebugLogLine(false,_T("Error from %s: Error=%i"),
				ipstr(stDestAddr),
				returnValue.error);
        }
    }
...
void Pinger::PIcmpErr(int nICMPErr) {
    ...
    if ((nICMPErr > MAX_ICMP_ERR_STRING) || 
		...
		theAppPtr->QueueDebugLogLine(false,_T("Pinger: %s"),GetErrorMessage(nICMPErr,1));
    } else {

        // Display ICMP Error String
        theAppPtr->QueueDebugLogLine(false,_T("%s"), aszSendEchoErr[nErrIndex]);
    }
}
