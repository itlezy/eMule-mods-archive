...
{
	CKnownFile* reqfile = theAppPtr->sharedfiles->GetFileByID(hash);
	if (reqfile != NULL)
		return reqfile->GetFileName();

	CPartFile* partfile = theAppPtr->downloadqueue->GetFileByID(hash);
	if (partfile != NULL)
		return partfile->GetFileName();

	CKnownFile* knownfile = theAppPtr->knownfiles->FindKnownFileByID(hash);
	if (knownfile != NULL)
...
uint8 GetMyConnectOptions(bool bEncryption, bool bCallback){
	...
	const uint8 uDirectUDPCallback	= (bCallback && theAppPtr->IsFirewalled() && Kademlia::CKademlia::IsRunning() && !Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) && Kademlia::CUDPFirewallTester::IsVerified()) ? 1 : 0;
