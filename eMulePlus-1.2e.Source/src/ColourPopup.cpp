...
ColourTableEntry CColourPopup::m_crColoursLimited[] = 
{
    { RGB(255, 255, 255),		_T("White")				},
    { RGB(0, 0, 0),				_T("Black")				},
    { RGB(0, 0, 127),			_T("Dark Blue")			},
    { RGB(0, 127, 0),			_T("Dark Green")		},
    { RGB(255, 0, 0),			_T("Red")				},
    { RGB(127, 0, 0),			_T("Dark Red")			},
    { RGB(156, 0, 156),			_T("Dark Violet")		},
    { RGB(252, 127, 0),			_T("Orange")			},
    { RGB(255, 255, 0),			_T("Yellow")			},
    { RGB(0, 252, 0),			_T("Green")				},
    { RGB(0, 147, 147),			_T("Teal")				},
    { RGB(0, 255, 255),			_T("Turquoise")			},
    { RGB(0, 0, 252),			_T("Blue")				},
    { RGB(255, 0, 255),			_T("Violet")			},
    { RGB(127, 127, 127),		_T("Grey - 50")			},
    { RGB(210, 210, 210),		_T("Light Grey")		}
};
						   ...
						   bool bLimited)
{
    ...
    CColourPopup::Create(p, crColour, pParentWnd, szDefaultText, szCustomText, bLimited);
}

void CColourPopup::Initialize()
{
	m_nNumColours       = ((m_bLimited) ? sizeof(m_crColoursLimited) : sizeof(m_crColours)) / sizeof(ColourTableEntry);
    ...
    {
		pLogPalette->palPalEntry[i].peRed   = (m_bLimited) ? GetRValue(m_crColoursLimited[i].crColour) : GetRValue(m_crColours[i].crColour);
		pLogPalette->palPalEntry[i].peGreen = (m_bLimited) ? GetGValue(m_crColoursLimited[i].crColour) : GetGValue(m_crColours[i].crColour);
		pLogPalette->palPalEntry[i].peBlue  = (m_bLimited) ? GetBValue(m_crColoursLimited[i].crColour) : GetBValue(m_crColours[i].crColour);
...
{    
    ...
    point = CPoint((int)(short)LOWORD(pos), (int)(short)HIWORD(pos));
...
void CColourPopup::EndSelection(int nMessage)
{
    ...
    m_pParent->SendMessage(nMessage, (WPARAM)m_crColour, (LPARAM)m_nCurrentSel);
...
void CColourPopup::DrawCell(CDC* pDC, int nIndex)
{
    ...
    CBrush brush(GetColour(nIndex));		//Cax2 shorter now! :)
    CPen   pen;
