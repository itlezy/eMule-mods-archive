...
void CFileVersionInfo::Clear()
{
	m_strFilename.Empty();
	m_dwTransInfo = 0L;
	memzero(&m_fixedFileInfo, sizeof(VS_FIXEDFILEINFO));
...
BOOL CFileVersionInfo::ReadVersionInfo(const CString& strFile)
{
	...
	// **********************************
	if (!::GetFileVersionInfo((LPTSTR) (LPCTSTR)strFile, 0L, nSize, m_pVersionInfo))
	{
		delete[] m_pVersionInfo;
		m_pVersionInfo = NULL;
		return FALSE;
	...
	memcpy2(&m_fixedFileInfo, lpVoid, nSize);
...
CString CFileVersionInfo::GetVersionInfo(int nIndex) const
{
	...
	// ****************************************************
	if ((nIndex < SFI_FIRST) || (nIndex > SFI_LAST) || (m_pVersionInfo == NULL))
		return CString(_T(""));

	CString	strRes, strFmt;
	LPVOID	lpInfo;
	UINT	nLen;
	...
	// ********************************************
	if (::VerQueryValue(m_pVersionInfo, strFmt.GetBuffer(_MAX_PATH), &lpInfo, &nLen))
...
BOOL CFileVersionInfo::IsVersionInfoAvailable(int nIndex) const
{
	...
	// ********************************************************
	return ::VerQueryValue(m_pVersionInfo, strFmt.GetBuffer(_MAX_PATH), &lpInfo, &nLen);
}
