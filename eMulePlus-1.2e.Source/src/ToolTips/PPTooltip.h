...
#define UDM_TOOLTIP_POP			   (UDM_TOOLTIP_FIRST+1) //User was changed the data
...
public:
	...
	void AddTool(CWnd *pWnd, const TCHAR *pcTooltipText, HICON hIcon = NULL, LPCRECT lpRectTool = NULL, UINT nIDTool = 0); //Adds tool
	void AddTool(CWnd *pWnd, const TCHAR *pcTooltipText, UINT nIdIcon, CSize szIcon = CSize(0, 0), LPCRECT lpRectTool = NULL, UINT nIDTool = 0); //Adds tool
	void AddTool(const PPTOOLTIP_INFO &ti); //Adds tool
	...
	int  FindIdOfResource(const CString &strName);
	...
	void SetDirection(UINT nDirection = PPTOOLTIP_RIGHT_TOP, int nIndex = PPTOOLTIP_TOOL_NOEXIST);
...
protected:
	...
	CSize PrintTitleString(CDC * pDC, CRect rect, const CString &str, BOOL bCalculate = TRUE); //Prints the title string
	...
	CString SearchBeginOfTag(const CString &str, int &nIndex);
	void SearchEndOfTag(const CString &str, int &nIndex);
	CString GetNameOfTag(const CString &str, int &nIndex);
	CString GetPropertiesOfTag(const CString &str, int &nIndex);
	CString GetStringValue(const CString &str, int &nIndex);
	UINT GetUIntValue(const CString &str, int &nIndex, UINT nDefValue);

	int   GetNextHorizDirection(int nDirection) const;
	int   GetNextVertDirection(int nDirection) const;
	BOOL  TestHorizDirection(int x, int cx, LPRECT w_rect, int nDirection, LPRECT rect) const;
	BOOL  TestVertDirection(int y, int cy, LPRECT w_rect, int nDirection, LPRECT rect) const;
...
private:
	void GetSystemToolTipFont(LOGFONT *pLogFont) const;
};
